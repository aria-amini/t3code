...
async function shouldReleaseNightly({
	...
	if (now - Date.parse(lastNightly.published_at) < MINIMUM_RELEASE_GAP_MS) {
		core.info(
			`Nightly ${lastNightly.tag_name} was published less than six hours ago. Skipping.`,
		);
		return false;
	}
	...
	core.info(
		`New commits since ${lastNightly.tag_name}, and the six-hour gap has passed.`,
	);
	return true;
}
...
async function resolveLatestNightlyCommit({ github, context, core }) {
	...
	if (!lastNightly) {
		throw new Error(
			"No published nightly found. Stable releases build the latest nightly commit.",
		);
	}
	...
	core.info(
		`Latest nightly ${tag} shipped ${commit.sha} as a preview of ${version}.`,
	);
