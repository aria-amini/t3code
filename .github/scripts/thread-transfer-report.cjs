...
function assertMetric(value, label) {
	if (!Number.isSafeInteger(value) || value < 0 || value > 1_000_000_000) {
		throw new Error(
			`${label} must be a non-negative safe integer below 1,000,000,000`,
		);
	}
}
...
function validateResult(value) {
	...
	for (const provider of PROVIDERS) {
		...
		assertExactKeys(
			entry.observed,
			OBSERVED_KEYS,
			`result.providers.${provider}.observed`,
		);
		assertExactKeys(
			entry.ceiling,
			CEILING_KEYS,
			`result.providers.${provider}.ceiling`,
		);
		for (const key of OBSERVED_KEYS) {
			assertMetric(
				entry.observed[key],
				`result.providers.${provider}.observed.${key}`,
			);
		}
		for (const key of CEILING_KEYS) {
			assertMetric(
				entry.ceiling[key],
				`result.providers.${provider}.ceiling.${key}`,
			);
		}
	}
...
function readResult(directory) {
	...
	if (!stat.isFile() || stat.size > 64 * 1_024) {
		throw new Error(
			"thread transfer result must be a regular file smaller than 64 KiB",
		);
	}
...
function formatImpact(current, baseline, kind) {
	...
	const percent =
		baseline === 0
			? ""
			: ` (${prefix}${Math.abs((delta / baseline) * 100).toFixed(1)}%)`;
...
const METRICS = [
	...
	{
		key: "threadSnapshotWireBytes",
		label: "Thread snapshot wire",
		kind: "bytes",
	},
	{
	...
	{
		key: "measuredTurnWebSocketMessages",
		label: "Live turn messages",
		kind: "messages",
	},
];
...
function renderComment(input) {
	...
	const comparable =
		baseline !== undefined && sameScenario(current.scenario, baseline.scenario);
	const rows = [];
	...
	for (const provider of PROVIDERS) {
		for (const metric of METRICS) {
			...
			if (
				baseline &&
				baseline.providers[provider].ceiling[metric.key] !== ceiling
			) {
				ceilingChanges.push(
...
async function resolve({ github, context, core }) {
	...
	if (pull.head.sha !== source.head_sha) {
		core.info(
			`Skipping stale CI result ${source.head_sha}; PR head is ${pull.head.sha}.`,
		);
		core.setOutput("publish", "false");
		return;
	}
...
async function upsertCommentForCurrentHead(
	...
	if (pull.head.sha !== expectedSha) {
		core.info(
			`Skipping stale CI result ${expectedSha}; PR head is ${pull.head.sha}.`,
		);
		return false;
	}
