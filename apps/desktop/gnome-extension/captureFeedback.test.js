...
vi.mock("gi://St", () => ({
	default: {
		Settings: { get: () => ({ enable_animations: shell.animations }) },
	},
}));
...
beforeEach(() => {
	...
	Object.assign(globalThis, {
		get_window_actors: () =>
			shell.windows.map((meta_window) => ({ meta_window })),
...
it("activates the caller, flies its frozen capture, and retains the landed image until disconnect", async () => {
	...
	const flight = feedback.animate(":1.23", {
		x: 0.1,
		y: 0.8,
		width: 0.2,
		height: 0.1,
	...
	expect(actor.transition).toMatchObject({
		x: -1100,
		y: 660,
		scale_x: 0.2,
		scale_y: 0.1,
...
it("rejects another sender and locked-session activation", async () => {
	shell.windows = [target()];
	const { feedback } = begin();
	await expect(feedback.activate(":1.99", "T3")).rejects.toThrow(
		"No active capture",
	);
	shell.locked = true;
	await expect(feedback.activate(":1.23", "T3")).rejects.toThrow(
		"No active capture",
	);
	expect(shell.activate).not.toHaveBeenCalled();
	feedback.dispose();
