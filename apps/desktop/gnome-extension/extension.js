		...
		Gio.DBus.session.call(
			...
			(connection, result) => {
				try {
					...
					if (error.matches(Gio.DBusError, Gio.DBusError.NAME_HAS_NO_OWNER))
						resolve(null);
					else reject(error);
				}
			},
...
const getProcessId = (sender) =>
	busIdentity("GetConnectionUnixProcessID", sender, "(u)");
	...
	return new Promise((resolve, reject) => {
		screenshot.screenshot_window(true, false, stream, (source, result) => {
			try {
				...
				if (!success)
					throw new Error("GNOME could not capture the active window.");
				stream.close(null);
...
export default class T3SnapShotExtension extends Extension {
	...
	CaptureAsync(_params, invocation) {
		void this._service
			...
			.catch((error) =>
				invocation.return_dbus_error(`${NAME}.Failed`, error.message),
			);
	}
		...
		void this._service
			...
			.then(({ png, metadata, animationStarted }) =>
				invocation.return_value(
					new GLib.Variant("(aysb)", [png, metadata, animationStarted]),
				),
			)
			.catch((error) =>
				invocation.return_dbus_error(`${NAME}.Failed`, error.message),
