...
it("activates only a window of the process authenticated by D-Bus", () => {
	...
	expect(
		findCaptureDestination([window(99, "T3 Code"), target], 42, "T3 Code"),
	).toBe(target);
	expect(
		findCaptureDestination([window(99, "T3 Code")], 42, "T3 Code"),
	).toBeUndefined();
	expect(findCaptureDestination([target], 42, "Title before navigation")).toBe(
		target,
	);
});
...
it("does not guess between ambiguous windows from the same process", () => {
	expect(
		findCaptureDestination([window(42, "T3"), window(42, "T3")], 42, "T3"),
	).toBeUndefined();
	expect(
		findCaptureDestination([window(42, "A"), window(42, "B")], 42, "C"),
	).toBeUndefined();
});
...
it.each([NaN, Infinity, -1, 2])(
	"rejects invalid/off-window animation geometry %s",
	(x) => {
		expect(() =>
			captureDestinationFrame(
