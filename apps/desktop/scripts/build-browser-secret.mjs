...
if (hostPlatform === "linux") {
	...
	if (machine === undefined)
		throw new Error(`Unsupported Linux architecture: ${values.arch}`);
	const root = NodeURL.fileURLToPath(
		new URL("../../../native/browser-secret/", import.meta.url),
	);
	const source = NodePath.resolve(root, "main.c");
	const output =
		values.output ??
		NodePath.resolve(root, "build", values.arch, "t3-browser-secret");
	...
	if (!current) {
		let flags;
		try {
			flags = NodeChildProcess.execFileSync(
				"pkg-config",
		...
		try {
			...
			if (!matchesArchitecture(temporary))
				throw new Error(
					`C compiler did not produce a Linux ${values.arch} executable.`,
				);
