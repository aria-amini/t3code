...
export function makeDevelopmentEnvironmentScript(environment) {
	const envEntries = [
		...
		[
			...
			environment.T3CODE_OTLP_EXPORT_INTERVAL_MS,
		],
...
function writeDevelopmentEnvironmentScript() {
	NodeFS.mkdirSync(NodePath.dirname(developmentEnvironmentFilePath), {
		recursive: true,
	});
...
export function resolveMacLauncherIconPaths(
	runtimeDir,
	development = isDevelopment,
) {
	return {
		sourceIconPath: development
...
function ensureMacIconIcns(runtimeDir) {
	const { sourceIconPath, generatedIconPath } =
		resolveMacLauncherIconPaths(runtimeDir);
	NodeFS.mkdirSync(runtimeDir, { recursive: true });

	if (!NodeFS.existsSync(sourceIconPath)) {
		throw new Error(
			`Desktop macOS icon source is missing at ${sourceIconPath}`,
		);
	}
	...
	const iconsetRoot = NodeFS.mkdtempSync(
		NodePath.join(runtimeDir, "dev-iconset-"),
	);
...
function patchMainBundleInfoPlist(appBundlePath, iconPath, executableName) {
	...
	for (const [key, value] of Object.entries(
		resolveMacBundleInfoPlistStrings(executableName),
	)) {
...
function patchHelperBundleInfoPlists(appBundlePath) {
	const helperBundleNames = [
		...
		[
			...
			"helper.gpu",
			`${APP_DISPLAY_NAME} Helper (GPU)`,
		],
		[
			"Electron Helper (Plugin).app",
			"helper.plugin",
			`${APP_DISPLAY_NAME} Helper (Plugin)`,
		],
		[
			"Electron Helper (Renderer).app",
			"helper.renderer",
			`${APP_DISPLAY_NAME} Helper (Renderer)`,
		],
	];
...
export function resolveMacLauncherPaths(
	appBundlePath,
	...
	return {
		...
		launcherBinaryPath: NodePath.posix.join(
			executableDir,
			launcherExecutableName,
		),
...
function buildMacLauncher(electronBinaryPath) {
	const sourceAppBundlePath = NodePath.resolve(
		NodePath.dirname(electronBinaryPath),
		"../..",
	);
	const runtimeDir = NodePath.join(desktopDir, ".electron-runtime");
	const targetAppBundlePath = NodePath.join(
		runtimeDir,
...
function isLinuxSetuidSandboxConfigured(electronBinaryPath) {
	...
	const sandboxPath = NodePath.join(
		NodePath.dirname(electronBinaryPath),
		"chrome-sandbox",
	);
	try {
