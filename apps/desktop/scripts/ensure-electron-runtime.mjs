...
function getPlatformPath() {
	switch (hostPlatform) {
		...
		default:
			throw new Error(
				`Electron builds are not available on platform: ${hostPlatform}`,
			);
	}
}
...
function invalidRuntimePaths(electronDir, platformPath) {
	...
	return [
		...
		(runtimePath) => NodeFS.existsSync(runtimePath) && !isMachO(runtimePath),
	);
}
...
function installElectronRuntime(electronDir, version) {
	const tempDir = NodeFS.mkdtempSync(
		NodePath.join(NodeOS.tmpdir(), "t3-electron-"),
	);
	const zipPath = NodePath.join(
		tempDir,
		`electron-v${version}-${hostPlatform}-${hostArch}.zip`,
	);
...
export function ensureElectronRuntime() {
	...
	const electronPackageJson = JSON.parse(
		NodeFS.readFileSync(electronPackageJsonPath, "utf8"),
	);
	...
	if (missingBeforeInstall.length > 0 || invalidBeforeInstall.length > 0) {
		if (NodeFS.existsSync(NodePath.join(electronDir, "dist"))) {
			NodeFS.rmSync(NodePath.join(electronDir, "dist"), {
				recursive: true,
				force: true,
			});
		}
...
if (
	process.argv[1] &&
	NodeURL.pathToFileURL(process.argv[1]).href === import.meta.url
) {
