...
const inspectBundle = (source) => {
	...
	const visit = (node) => {
		if (node.type === "ImportExpression") {
			throw new Error(
				"Desktop preload bundle contains a dynamic import() call",
			);
		}
		...
		if (node.type === "CallExpression" && node.callee.type === "Identifier") {
			if (node.callee.name === "require") {
				...
				if (node.arguments.length !== 1 || argument?.type !== "Literal") {
					throw new Error(
						"Desktop preload bundle contains a dynamic require() call",
					);
				}
...
export const verifyPreloadBundle = (source) => {
	...
	if (!exposedGlobals.has("desktopBridge"))
		missingApis.unshift("desktopBridge exposure");
	if (!exposedGlobals.has(clerkPasskeysGlobal))
		missingApis.push(`${clerkPasskeysGlobal} exposure`);

	if (missingApis.length > 0) {
		throw new Error(
			`Desktop preload bundle is missing executable APIs: ${missingApis.join(", ")}`,
		);
	}
};
