...
const DESKTOP_BACKEND_PORT_PROBE_HOSTS = [
	"127.0.0.1",
	"0.0.0.0",
	"::",
] as const;
...
const resolveDesktopBackendPort = Effect.fn("resolveDesktopBackendPort")(
	function* (configuredPort: Option.Option<number>) {
		...
		for (
			let port = DEFAULT_DESKTOP_BACKEND_PORT;
			port <= MAX_TCP_PORT;
			port += 1
		) {
...
const fatalStartupCause = <E>(stage: string, cause: Cause.Cause<E>) =>
	handleFatalStartupError(stage, Cause.pretty(cause)).pipe(
...
const bootstrap = Effect.gen(function* () {
	...
	if (
		...
		environment.defaultDesktopSettings.serverExposureMode
	) {
		yield* logBootstrapInfo(
			"bootstrap restoring persisted server exposure mode",
			{
