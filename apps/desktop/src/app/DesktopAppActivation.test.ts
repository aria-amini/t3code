...
describe("desktop app control server", () => {
		...
		Effect.gen(function* () {
			...
			yield* Effect.promise(async () => {
				const root = await NodeFSP.mkdtemp(
					NodePath.join(NodeOS.tmpdir(), "t3-app-control-test-"),
				);
				const target = makeTarget(
					NodePath.join(root, "userdata"),
					platform,
					userId,
				);
				...
				if (target.directory !== null) {
					await expect(NodeFSP.stat(target.address)).rejects.toMatchObject({
						code: "ENOENT",
					});
				}
	...
	it.effect("cancels a queued request when the client disconnects", () =>
		Effect.gen(function* () {
			...
			yield* Effect.promise(async () => {
				const root = await NodeFSP.mkdtemp(
					NodePath.join(NodeOS.tmpdir(), "t3-app-cancel-test-"),
				);
				const target = makeTarget(
					NodePath.join(root, "userdata"),
					platform,
				...
				await new Promise<void>((resolve, reject) => {
					...
					socket.once("connect", () => {
						socket.write(
							`${JSON.stringify(request("request-canceled", platform))}\n`,
							() => {
								socket.destroy();
								resolve();
							},
