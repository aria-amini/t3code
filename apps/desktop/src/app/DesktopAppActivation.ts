...
export async function startDesktopAppControlServer(input: {
	...
	const server = NodeNet.createServer((socket) => {
		...
		socket.on("data", (chunk) => {
			...
			if (Buffer.byteLength(buffer, "utf8") > MAX_REQUEST_BYTES) {
				...
				finish(
					invalidResponse(
						"invalid-request",
						"The desktop app request is too large.",
					),
				);
				return;
			}
			...
			try {
				...
				finish(
					invalidResponse(
						"invalid-request",
						"The desktop app request is not valid JSON.",
					),
				);
				return;
			}
			...
			if (!isDesktopAppActivationRequest(parsed)) {
				finish(
					invalidResponse(
						...
						"The desktop app request is invalid.",
					),
				);
				return;
			}
			...
			void input.handle(parsed).then(finish, () => {
				finish(
					invalidResponse(
						parsed.requestId,
						"T3 Code could not process the desktop app request.",
					),
				);
			});
		});
		...
		socket.on("close", () => {
			...
			if (!responseSent && activeRequestId !== null)
				input.cancel(activeRequestId);
		});
	});
	...
	return {
		close: async () => {
			...
			if (input.directory !== null) {
				await NodeFSP.unlink(input.address).catch(
		...
		start: Effect.acquireRelease(
			Effect.tryPromise({
				try: () =>
					...
					new DesktopAppActivationStartError({
			...
			(server) =>
				Effect.promise(() => server.close()).pipe(
					Effect.catchCause((cause) =>
						logWarning("failed to close the desktop app control socket", {
							cause,
						}),
					),
