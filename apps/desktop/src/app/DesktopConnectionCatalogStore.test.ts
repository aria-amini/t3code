...
const withStore = <A, E, R>(
	effect: Effect.Effect<
		A,
		E,
		R | DesktopConnectionCatalogStore.DesktopConnectionCatalogStore
	>,
	encryptionAvailable = true,
) =>
	Effect.gen(function* () {
		...
		return yield* effect.pipe(
			Effect.provide(makeLayer(baseDir, encryptionAvailable)),
		);
...
describe("DesktopConnectionCatalogStore", () => {
	...
	it.effect(
		"migrates legacy relay, SSH, bearer profile, and credential data",
		() =>
			withStore(
				Effect.gen(function* () {
					const store =
					...
					assert.equal(
						catalog.credentials[0]?.connectionId,
						"bearer:bearer-environment",
					);
					assert.equal(
						catalog.credentials[0]?.credential._tag,
					...
					if (
						...
						assert.equal(
							catalog.credentials[0].credential.token,
							"legacy-token",
						);
					}
	...
	it.effect(
		"surfaces catalog filesystem failures instead of treating them as missing",
		() =>
			Effect.gen(function* () {
				const path = yield* Path.Path;
				...
				const store =
					yield* DesktopConnectionCatalogStore.DesktopConnectionCatalogStore.pipe(
				...
				assert.equal(
					error.catalogPath,
					path.join(baseDir, "userdata", "connection-catalog.json"),
				);
	...
	it.effect(
		"surfaces a catalog that can no longer be decrypted without deleting it",
		() =>
			Effect.gen(function* () {
				const path = yield* Path.Path;
				const fileSystem = yield* FileSystem.FileSystem;
				...
				const decryptError =
					error.cause as ElectronSafeStorage.ElectronSafeStorageDecryptError;
				...
				assert.deepStrictEqual(
					yield* store.get,
					Option.some('{"schemaVersion":1,"targets":[]}'),
				);
			}).pipe(Effect.provide(NodeServices.layer), Effect.scoped),
	);
});
