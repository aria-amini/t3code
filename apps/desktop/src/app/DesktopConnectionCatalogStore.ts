...
type EncryptedConnectionCatalogDocument =
	typeof EncryptedConnectionCatalogDocument.Type;

const EncryptedConnectionCatalogDocumentJson = fromLenientJson(
	...
	override get message(): string {
		const environment =
			this.environmentId === undefined
				? ""
				: ` for environment ${this.environmentId}`;
...
const writeDocument = Effect.fn("desktop.connectionCatalogStore.writeDocument")(
	function* (input: {
		...
		const encoded = yield* encodeEncryptedConnectionCatalogDocumentJson(
			input.document,
		).pipe(
			Effect.mapError(
				(cause) =>
		...
		yield* Effect.gen(function* () {
			...
			Effect.ensuring(
				input.fileSystem.remove(tempPath, { force: true }).pipe(
					Effect.catch((error) =>
						Effect.logWarning(
							"Could not remove a temporary connection catalog file.",
							{
								tempPath,
								error,
							},
...
> {
	const targets: Array<
		RuntimeConnectionCatalogDocumentType["targets"][number]
	> = [];
	const profiles: Array<
		RuntimeConnectionCatalogDocumentType["profiles"][number]
	> = [];
	const credentials: Array<
		RuntimeConnectionCatalogDocumentType["credentials"][number]
	> = [];
...
export const make = Effect.gen(function* () {
	...
	const writeCatalog = Effect.fn("desktop.connectionCatalogStore.writeCatalog")(
	...
	const migrateLegacyCatalog = Effect.gen(function* () {
		...
		const catalog = yield* migrateSavedEnvironmentRecords(
			records,
			savedEnvironments,
			catalogPath,
		);
		const encoded = yield* encodeRuntimeConnectionCatalogDocumentJson(
			catalog,
		).pipe(
			Effect.mapError(
				(cause) =>
