...
const decodeEarlyDesktopSettingsJson = Schema.decodeSync(
	EarlyDesktopSettingsJson,
);
...
export function resolveEarlyLinuxPasswordStorePreference(
	...
	try {
		const parsed = decodeEarlyDesktopSettingsJson(
			input.readFileString(settingsPath),
		);
