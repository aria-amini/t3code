...
function makeElectronAppLayer(
	...
	return Layer.succeed(ElectronApp.ElectronApp, {
		...
		onBeforeQuitForUpdate: (listener) =>
			registerListener("before-quit-for-update", listener),
...
describe("DesktopLifecycle", () => {
	for (const platform of [
		...
		"win32",
		"linux",
	] satisfies ReadonlyArray<NodeJS.Platform>) {
	...
	it.effect("destroys windows before waiting for backend shutdown", () =>
		Effect.gen(function* () {
			...
			const desktopShutdownLayer = Layer.succeed(
				DesktopShutdown.DesktopShutdown,
				{
					...
					markComplete: Deferred.succeed(allowShutdown, undefined).pipe(
						Effect.asVoid,
					),
