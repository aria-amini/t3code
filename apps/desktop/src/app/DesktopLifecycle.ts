...
const requestDesktopShutdownAndWait = Effect.fn(
	"desktop.lifecycle.requestShutdownAndWait",
)(function* (
			...
			yield* requestDesktopShutdownAndWait(
				electronWindow.destroyAll.pipe(
					Effect.catchCause((cause) =>
						logLifecycleError("failed to destroy windows before shutdown", {
							cause,
						}),
					),
				),
...
export const make = DesktopLifecycle.of({
	...
	register: Effect.gen(function* () {
		...
		const environment = yield* DesktopEnvironment.DesktopEnvironment;
		const context =
			yield* Effect.context<DesktopLifecycleRegistrationServices>();
		...
		yield* electronApp.onBeforeQuitForUpdate(() => {
			...
			Effect.runSyncWith(context)(
				electronWindow.destroyAll.pipe(
					...
					Effect.catchCause((cause) =>
						logLifecycleError("failed to destroy windows before updater quit", {
							cause,
						}),
					),
