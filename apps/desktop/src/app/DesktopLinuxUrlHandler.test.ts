		...
		Layer.provide(
			Layer.mergeAll(
				Layer.succeed(
					...
					makeEnvironment(input.environment),
				),
...
describe("DesktopLinuxUrlHandler", () => {
	...
	it("carries structured context on registration errors", () => {
		const writeError =
			new DesktopLinuxUrlHandler.DesktopLinuxUrlHandlerRegistrationError({
	...
	it.effect(
		"writes the handler entry and claims the scheme default via xdg-mime",
		() => {
	...
	it.effect(
		"does not rewrite the pre-ready entry while the portal can be reading it",
		() => {
			const recorded = emptyRecording();
	...
	it.effect(
		"writes the portal identity without claiming the URL scheme in development",
		() => {
