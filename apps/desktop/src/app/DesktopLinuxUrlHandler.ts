...
const { logInfo, logWarning } = makeComponentLogger(
	"desktop-linux-url-handler",
);
	...
	override get message(): string {
		const exitCode =
			this.exitCode === undefined
				? ""
				: `, xdg-mime exit code ${this.exitCode}`;
...
export const make = Effect.gen(function* () {
	...
	const writeDesktopEntry = Effect.gen(function* () {
		...
		const execTarget = Option.getOrElse(
		...
		yield* fileSystem.makeDirectory(environment.linuxApplicationsDir, {
			recursive: true,
		});
