		...
		Layer.provide(
			Layer.mergeAll(
				...
				DesktopConfig.layerTest({
					T3CODE_HOME: baseDir,
					VITE_DEV_SERVER_URL: isDevelopment
						? "http://127.0.0.1:5733"
						: undefined,
				}),
			),
...
describe("DesktopObservability", () => {
	it("advances a retained output offset instead of repeatedly copying a full head chunk", () => {
		...
		const next = DesktopObservability.appendBoundedOutputChunk(
			initial,
			"stderr",
			Uint8Array.of(1),
		);
	...
	it.effect(
		"persists desktop Effect logs as span events in desktop.trace.ndjson",
		() =>
			Effect.gen(function* () {
				...
				const record = records.find(
					(entry) => entry.name === "desktop-observability-test",
				);
	...
	it.effect(
		"buffers backend child output and persists it only when a failure is reported",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const baseDir = yield* fileSystem.makeTempDirectoryScoped({
				...
				yield* Effect.gen(function* () {
					...
					yield* outputLog.writeOutputChunk(
						"stdout",
						new TextEncoder().encode("hello server\n"),
					);
					assert.isFalse(yield* fileSystem.exists(logPath));
				...
				const output = yield* decodeDesktopBackendChildLogRecord(
					lines[1] ?? "",
				);
				...
				assert.isFalse(
					traceRecords.some(
						(record) =>
							record.name ===
							"desktop.observability.backendOutput.writeOutputChunk",
					),
				);
			}).pipe(
				Effect.scoped,
	...
	it.effect(
		"keeps buffering output after a non-terminal failure snapshot",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const baseDir = yield* fileSystem.makeTempDirectoryScoped({
				...
				yield* Effect.gen(function* () {
					...
					yield* outputLog.writeOutputChunk(
						"stdout",
						new TextEncoder().encode("before timeout\n"),
					);
					yield* outputLog.persistFailureSnapshot({
				...
				assert.equal(
					records.some(
						(record) => record.annotations.text === "after timeout\n",
					),
					true,
				);
	...
	it.effect("retains only the last mebibyte of backend child output", () =>
		Effect.gen(function* () {
			...
			const lines = (yield* fileSystem.readFileString(logPath))
				.trimEnd()
				.split("\n");
