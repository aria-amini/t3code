...
export interface DesktopBackendOutputLogShape {
	...
	readonly persistFailureSnapshot: (input: {
		readonly details: string;
	}) => Effect.Effect<void>;
	readonly persistFailure: (input: {
		readonly details: string;
	}) => Effect.Effect<void>;
...
export function appendBoundedOutputChunk(
	...
	const chunks = [
		...session.chunks,
		{ streamName, chunk: retainedChunk, offset: 0 },
	];
	let byteLength = session.byteLength + retainedChunk.byteLength;
	let overflow = Math.max(
		0,
		byteLength - DESKTOP_BACKEND_OUTPUT_BUFFER_MAX_BYTES,
	);
	...
	const excessChunks = Math.max(
		...
		chunks.length -
			firstRetainedIndex -
			DESKTOP_BACKEND_OUTPUT_BUFFER_MAX_CHUNKS,
	);
	for (
		let index = firstRetainedIndex;
		index < firstRetainedIndex + excessChunks;
		index += 1
	) {
...
const makeRotatingLogFileWriter = Effect.fn("makeRotatingLogFileWriter")(
	function* (input: {
	...
	> {
		...
		const currentSize = yield* Ref.make(
			yield* refreshFileSize(fileSystem, input.filePath),
		);
		...
		const rotate = Effect.gen(function* () {
			yield* fileSystem
				.remove(withSuffix(maxFiles), { force: true })
				.pipe(Effect.ignore);
...
const writeBackendChildLogRecord = Effect.fn(
...
const sanitizeInstanceIdForFileName = (id: string): string =>
	...
	Option.match(sink, {
		...
		onSome: (logFile) =>
			Effect.gen(function* () {
				...
				const writeFailure = Effect.fn(
					"desktop.observability.backendOutput.writeFailure",
				...
				return {
					beginSession: Effect.fn(
						"desktop.observability.backendOutput.beginSession",
					)(function* ({ details }) {
					...
					writeOutputChunk: Effect.fnUntraced(function* (streamName, chunk) {
						...
						yield* Ref.update(
							sessionRef,
							Option.map((session) =>
								appendBoundedOutputChunk(session, streamName, chunk),
							),
						);
					}),
					...
					persistFailure: Effect.fn(
						"desktop.observability.backendOutput.persistFailure",
					)(function* ({ details }) {
...
const tracerLayer = Layer.unwrap(
	Effect.gen(function* () {
		...
		const tracePath = environment.path.join(
			environment.logDir,
			...
			: yield* OtlpTracer.make({
					...
					resource: {
						...
						attributes: {
							...
							"service.mode": environment.isDevelopment
								? "development"
								: "packaged",
						},
					},
