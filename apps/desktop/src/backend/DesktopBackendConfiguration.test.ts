...
const serverExposureLayer = Layer.succeed(
...
const withPackagedWslHarness = <A, E, R>(
	...
	Effect.gen(function* () {
		...
		const mountedEntryPath = path.join(
			baseDir,
			"app.asar.unpacked/apps/server/dist/bin.mjs",
		);
		yield* fileSystem.makeDirectory(path.dirname(mountedEntryPath), {
			recursive: true,
		});
...
describe("DesktopBackendConfiguration", () => {
	it.effect("resolvePrimary produces a stable scoped bootstrap token", () =>
		withHarness(
			Effect.gen(function* () {
				...
				assert.equal(
					second.bootstrap.desktopBootstrapToken,
	...
	it.effect(
		"resolvePrimary starts from server.asar without materializing the WSL tree",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
							...
							Layer.provideMerge(
								Layer.succeed(
									...
									DesktopWslServerTree.DesktopWslServerTree.of({
											...
											"Windows primary must not extract the WSL server tree",
										),
										cleanupLegacy: Effect.die(
											"Windows primary must not clean the WSL server tree",
										),
									}),
								),
							),
	...
	it.effect("resolveWsl reuses the primary's bootstrap token", () =>
		withHarness(
			Effect.gen(function* () {
				...
				assert.equal(
					wsl.bootstrap.desktopBootstrapToken,
					primary.bootstrap.desktopBootstrapToken,
				);
			}),
		),
	);
	...
	it.effect(
		"resolveWsl pins a default-tracking run to the concrete default distro",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
	...
	it.effect(
		"resolveWsl launches a packaged backend from the WSL-local runtime cache",
		() => {
			...
			return withPackagedWslHarness(
				{
					...
					forbidFallback:
						"A valid WSL archive must not extract the Windows fallback",
	...
	it.effect(
		"resolveWsl changes the cache id when the packaged archive changes",
		() => {
			...
			return withPackagedWslHarness(
				{
					...
					wsl: () => ({
						prepareRuntime: (_distro, archive) => {
							...
							return {
								ok: true,
								linuxAppRoot: `/runtime/${archive.runtimeId}`,
							};
						},
						ensureNodePty: () => ({
							ok: true,
				...
				({ hashPath, mountedAppRoot }) =>
					Effect.gen(function* () {
						...
						const configuration =
							yield* DesktopBackendConfiguration.DesktopBackendConfiguration;
						const first = yield* configuration.resolveWsl({
							port: 5000,
							distro: "Ubuntu",
						});
						yield* fileSystem.writeFileString(hashPath, secondHash);
						const second = yield* configuration.resolveWsl({
							port: 5000,
							distro: "Ubuntu",
						});
						yield* fileSystem.writeFileString(hashPath, "not-a-sha256");
						const invalidIdentity = yield* configuration.resolveWsl({
							port: 5000,
							distro: "Ubuntu",
						});
	...
	it.effect(
		"resolveWsl falls back to the mounted runtime when archive staging fails",
		() => {
			const observedNodePtyRoots: string[] = [];
			return withPackagedWslHarness(
				{
	...
	it.effect(
		"resolveWsl retires a staged runtime that cannot load node-pty",
		() => {
			...
			return withPackagedWslHarness(
					...
					wsl: () => ({
						...
						ensureNodePty: (_distro, root) => {
							...
							return root === stagedAppRoot
								? {
										ok: false,
										reason: "pty.node could not be loaded",
										fatal: true,
									}
								: {
										ok: true,
				...
				({ mountedAppRoot, mountedEntryPath }) =>
					Effect.gen(function* () {
						...
						assert.deepEqual(observedNodePtyRoots, [
							stagedAppRoot,
							mountedAppRoot,
						]);
						assert.include(
	...
	it.effect(
		"resolveWsl keeps the staged runtime when the mounted tree fails too",
		() => {
	...
	it.effect(
		"resolveWsl keeps WSL retryable when the mounted fallback fails transiently",
		() => {
			const stagedAppRoot = "/home/test/.t3/wsl-runtime/cache";
			const invalidatedRuntimeIds: string[] = [];
			return withPackagedWslHarness(
						...
						ensureNodePty: (_distro, root) =>
							root === stagedAppRoot
								...
								: {
										ok: false,
										reason:
											"WSL backend preflight timed out while probing for Node.js.",
										fatal: false,
									},
					}),
				},
	...
	it.effect(
		"resolveWsl retries the staged runtime after a transient probe failure",
		() => {
			const invalidatedRuntimeIds: string[] = [];
			return withPackagedWslHarness(
				{
					...
					forbidFallback:
						"A transient probe failure must not extract the fallback",
					forbidCleanup:
						"A transient probe failure must not clean the fallback tree",
					wsl: () => ({
						prepareRuntime: () => ({
							ok: true,
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				const nodePath =
					"/home/test user's/.nvm/versions/node/v22.0.0/bin/node";
				const linuxAppRoot = "/tmp/t3 code's launch";
				const linuxEntryPath = `${linuxAppRoot}/apps/server/dist/bin.mjs`;
				const resolvedPath =
					"/home/test user/bin:/opt/test's tools/bin:/usr/bin:/bin";
				const devServerUrl =
					"http://127.0.0.1:5733/dev%20assets/?label=hello%20world";
				const config = yield* Effect.gen(function* () {
					const configuration =
						yield* DesktopBackendConfiguration.DesktopBackendConfiguration;
	...
	it.effect(
		"resolvePrimary and resolveWsl share one token under concurrent resolution",
		() =>
			withHarness(
				Effect.gen(function* () {
					const configuration =
					...
					const [primary, wsl] = yield* Effect.all(
						[
							...
							configuration.resolveWsl({ port: 5000, distro: null }),
						],
	...
	it.effect(
		"resolvePrimary surfaces persisted backend observability endpoints",
		() =>
			withHarness(
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
					...
					yield* fileSystem.makeDirectory(
						environment.path.dirname(environment.serverSettingsPath),
						{
							recursive: true,
						},
					);
					...
					assert.equal(
						config.bootstrap.otlpTracesUrl,
						"http://127.0.0.1:4318/v1/traces",
					);
					assert.equal(
						config.bootstrap.otlpMetricsUrl,
	...
	it.effect(
		"resolvePrimary omits backend observability endpoints when settings are missing",
		() =>
			withHarness(
				Effect.gen(function* () {
					const configuration =
	...
	it.effect(
		"logs structured context when persisted observability settings cannot be read",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
	...
	it.effect(
		"resolvePrimary captures backend output in dev so child logs can be persisted",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const baseDir = yield* fileSystem.makeTempDirectoryScoped({
	...
	it.effect(
		"resolveWsl preserves existing WSLENV entries when forwarding backend secrets",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const baseDir = yield* fileSystem.makeTempDirectoryScoped({
				...
				try {
					process.env.WSLENV =
						"GOPATH/p:OPENAI_API_KEY/u:EMPTY::AZURE_DEVOPS_EXT_PAT/u";
					...
					yield* Effect.gen(function* () {
						...
						Effect.provide(
							DesktopBackendConfiguration.layer.pipe(
								...
								Layer.provideMerge(
									makeEnvironmentLayer(baseDir, { platform: "win32" }),
								),
							),
						),
					);
				...
				yield* Effect.gen(function* () {
					...
					Effect.provide(
						DesktopBackendConfiguration.layer.pipe(
							...
							Layer.provideMerge(DesktopWslServerTree.layerTest()),
							Layer.provideMerge(
								DesktopWslEnvironment.layerTest({ isAvailable: false }),
							),
							Layer.provideMerge(
								makeEnvironmentLayer(baseDir, { platform: "win32" }),
							),
					...
					DesktopBackendConfiguration.layer.pipe(
						...
						Layer.provideMerge(
							DesktopWslEnvironment.layerTest({
								isAvailable: true,
								distroListError:
									new DesktopWslEnvironment.DesktopWslDistroListError({
	...
	it.effect(
		"resolveWsl surfaces sidecar extraction failures through typed preflight",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const baseDir = yield* fileSystem.makeTempDirectoryScoped({
	...
	it.effect(
		"resolveWsl marks a missing selected distro as a fatal preflight failure",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const baseDir = yield* fileSystem.makeTempDirectoryScoped({
	...
	it.effect(
		"resolvePrimaryLabel reports the WSL distro when wsl-only and WSL is available",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const baseDir = yield* fileSystem.makeTempDirectoryScoped({
	...
	it.effect(
		"prefers the external packaged resource monitor over the copy inside the asar",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
	...
	it.effect(
		"prefers the release resource monitor when both development builds exist",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
				...
				yield* fileSystem.makeDirectory(path.dirname(releaseMonitorPath), {
					recursive: true,
				});
				yield* fileSystem.makeDirectory(path.dirname(debugMonitorPath), {
					recursive: true,
				});
	...
	it.effect(
		"resolvePrimaryLabel reports the local environment on non-Windows platforms",
		() =>
			withHarness(
				Effect.gen(function* () {
					const configuration =
	...
	it.effect(
		"resolvePrimaryLabel reports Windows when wsl-only but WSL is unavailable",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const baseDir = yield* fileSystem.makeTempDirectoryScoped({
				...
				Layer.provideMerge(
					Layer.succeed(
						...
						ChildProcessSpawner.make(() =>
							Effect.die(
								"spawner should not be used while probing WSL availability",
							),
						),
					),
				),
