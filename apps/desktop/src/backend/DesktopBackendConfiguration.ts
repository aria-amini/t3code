...
const runWslPreflight = Effect.fn("desktop.backendConfiguration.wslPreflight")(
	function* (input: {
		readonly distro: string | null;
	...
	> {
		...
		const runningDistro = input.distro
			? (installedDistros.find(
					(installed) =>
						installed.name.toLowerCase() === input.distro?.toLowerCase(),
				)?.name ?? null)
			: (installedDistros.find((installed) => installed.isDefault)?.name ??
				null);
		...
		const resolveMountedAppRoot = Effect.gen(function* () {
			...
			const mountedAppRoot = yield* wslEnv.windowsToWslPath(
				runningDistro,
				serverTree.root,
			);
		...
		if (!nodePtyResult.ok) {
			...
			return failedNodePty(
				stagedFailure && nodePtyResult.fatal
					? stagedFailure.nodePty
					: nodePtyResult,
			);
		}
...
const isLocalHostIpv4 = (ip: string): boolean => {
	...
	for (const list of Object.values(interfaces)) {
		...
		for (const entry of list) {
			...
			if ((family === "IPv4" || family === "4") && entry.address === ip)
				return true;
		}
	}
	return false;
...
const resolvePrimaryStartConfig = Effect.fn(
	"desktop.backendConfiguration.resolvePrimary",
)(function* (
...
const resolveWslStartConfig = Effect.fn(
	"desktop.backendConfiguration.resolveWsl",
)(function* (
	input: SharedBootstrapInput & {
		readonly port: number;
		readonly distro: string | null;
...
> {
	...
	const archivePath = environment.path.join(
		environment.resourcesPath,
	...
	const runningDistro =
		preflight._tag === "Ready" ? preflight.runningDistro : null;
	...
	if (preflight._tag === "Failed") {
		const retryLimit =
			preflight.retryLimit ??
			(preflight.fatal ? undefined : WSL_TRANSIENT_PREFLIGHT_RETRY_LIMIT);
		return {
			...baseConfig,
	...
	const nodeBinDir =
		lastSlash > 0 ? preflight.nodePath.slice(0, lastSlash) : "/usr/bin";
...
export const make = Effect.gen(function* () {
	...
	const sharedInputs = Effect.gen(function* () {
		...
		const observabilitySettings =
			yield* readPersistedBackendObservabilitySettings.pipe(
	...
	const describePrimary = Effect.gen(function* () {
		const persistedSettings = yield* settings.get;
		const wslRequested =
			persistedSettings.wslOnly && persistedSettings.wslBackendEnabled;
	...
	return DesktopBackendConfiguration.of({
		...
		resolvePrimaryLabel: Effect.gen(function* () {
			...
			Effect.withSpan("desktop.backendConfiguration.resolvePrimaryLabel"),
		),
		resolveWsl: (input) =>
			Effect.gen(function* () {
				const shared = yield* sharedInputs;
