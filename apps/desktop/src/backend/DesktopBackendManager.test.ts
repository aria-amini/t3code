...
const isBackendProcessError = Schema.is(
	DesktopBackendManager.BackendProcessError,
);
...
function makeProcess(options?: {
	...
	return ChildProcessSpawner.makeHandle({
		...
		all: Stream.merge(
			options?.stdout ?? Stream.empty,
			options?.stderr ?? Stream.empty,
		),
		exitCode:
			options?.exitCode ?? Effect.succeed(ChildProcessSpawner.ExitCode(0)),
...
function makeTestInstance(input: MakeInstanceInput) {
	...
	const servicesLayer = Layer.mergeAll(
		...
		Layer.succeed(DesktopTelemetryPublisher.DesktopTelemetryPublisher, {
			...
			handleControlForSource: (_sourceId, message) =>
				(input.desktopTelemetryPublisher?.handleControl ?? (() => Effect.void))(
					message,
				),
		...
		DesktopWslEnvironment.layerTest(
			input.pruneRuntimes === undefined
				? {}
				: { pruneRuntimes: input.pruneRuntimes },
		),
	);
	...
	const instance = DesktopBackendManager.makeBackendInstance({
		...
		configResolve:
			input.configResolve ?? Effect.succeed(input.config ?? baseConfig),
		...(input.onReady ? { onReady: () => input.onReady! } : {}),
		...(input.onShutdown ? { onShutdown: () => input.onShutdown! } : {}),
		...(input.onPreflightFailed
			? { onPreflightFailed: input.onPreflightFailed }
			: {}),
	});
...
describe("DesktopBackendManager", () => {
		...
		Effect.scoped(
			Effect.gen(function* () {
						...
						Effect.gen(function* () {
							...
							if (command._tag === "StandardCommand") {
								...
								if (fd3?.type === "input" && fd3.stream) {
									bootstrapJson = yield* fd3.stream.pipe(
										Stream.decodeText(),
										Stream.mkString,
									);
								}
								...
								if (fd4?.type === "input" && fd4.stream) {
									telemetryJson = yield* fd4.stream.pipe(
										Stream.decodeText(),
										Stream.mkString,
									);
								}
							...
							return makeProcess({
								exitCode: Deferred.await(ready).pipe(
				...
				const instance = yield* makeTestInstance({
					...
					backendOutputLog: {
						persistFailure: () =>
							Queue.offer(exited, void 0).pipe(Effect.asVoid),
					},
				});
				...
				assert.equal(
					Duration.toMillis(
						Duration.fromInputUnsafe(spawnedCommand.options.forceKillAfter),
					),
					2_000,
				);
	...
	it.effect("preserves the readiness timeout cause and process context", () =>
		Effect.gen(function* () {
			const requested =
				yield* Deferred.make<HttpClientRequest.HttpClientRequest>();
			const layer = Layer.merge(
				TestClock.layer(),
				httpClientLayer((request) =>
					Deferred.succeed(requested, request).pipe(
			...
			yield* Effect.gen(function* () {
				...
				assert.instanceOf(
					error,
					DesktopBackendManager.BackendReadinessTimeoutError,
				);
	...
	it.effect(
		"reports bootstrap encoding failures with stable process context",
		() =>
			Effect.gen(function* () {
				const spawnerLayer = Layer.succeed(
					ChildProcessSpawner.ChildProcessSpawner,
					ChildProcessSpawner.make(() =>
						Effect.die("unexpected backend spawn"),
					),
				);
				...
				if (error._tag !== "BackendProcessBootstrapEncodeError") {
					return assert.fail(
						`Expected bootstrap encode error, received ${error._tag}`,
					);
				}
				assert.equal(error.executablePath, "/electron");
				assert.equal(error.entryPath, "/server/bin.mjs");
	...
	it.effect(
		"preserves spawn failures without deriving their message from the cause",
		() =>
			Effect.gen(function* () {
				const spawnCause = PlatformError.systemError({
					...
					description:
						"low-level detail that must not become the public message",
				});
				const spawnerLayer = Layer.succeed(
					ChildProcessSpawner.ChildProcessSpawner,
				...
				if (error._tag !== "BackendProcessSpawnError") {
					return assert.fail(
						`Expected backend spawn error, received ${error._tag}`,
					);
				}
				assert.equal(error.executablePath, "/electron");
				assert.equal(error.entryPath, "/server/bin.mjs");
	...
	it.effect(
		"preserves exit-status failures without copying their detail into the message",
		() =>
			Effect.gen(function* () {
				...
				if (error._tag !== "BackendProcessExitStatusError") {
					return assert.fail(
						`Expected backend exit-status error, received ${error._tag}`,
					);
				}
				...
				assert.equal(
					error.message,
					"Failed to read the exit status of desktop backend process 123.",
				);
	...
	it.effect(
		"reports output stream failures with process and stream context",
		() =>
			Effect.gen(function* () {
				...
				const reported =
					yield* Deferred.make<DesktopBackendManager.BackendProcessOutputError>();
				const spawnerLayer = Layer.succeed(
					ChildProcessSpawner.ChildProcessSpawner,
					ChildProcessSpawner.make(() =>
						Effect.succeed(
							makeProcess({
								...
								exitCode: Deferred.await(reported).pipe(
									Effect.as(ChildProcessSpawner.ExitCode(0)),
								),
							}),
						),
				...
				const exit = yield* DesktopBackendManager.runBackendProcess({
					...
					onOutputFailure: (error) =>
						Deferred.succeed(reported, error).pipe(Effect.asVoid),
				}).pipe(
					Effect.scoped,
					Effect.provide(Layer.merge(spawnerLayer, healthyHttpClientLayer)),
				);
				...
				if (error._tag !== "BackendProcessOutputReadError") {
					return assert.fail(
						`Expected output read error, received ${error._tag}`,
					);
				}
				assert.equal(error.executablePath, "/electron");
				assert.equal(error.entryPath, "/server/bin.mjs");
				...
				assert.equal(
					error.message,
					"Failed to read stdout from desktop backend process 123.",
				);
	...
	it.effect(
		"reports output handler failures separately from stream read failures",
		() =>
			Effect.gen(function* () {
					...
					ChildProcessSpawner.make(() =>
						Effect.succeed(
							makeProcess({
								...
								exitCode: Deferred.await(drained).pipe(
									Effect.as(ChildProcessSpawner.ExitCode(0)),
								),
							}),
						),
				...
				if (error._tag !== "BackendProcessOutputHandlingError") {
					return assert.fail(
						`Expected output handling error, received ${error._tag}`,
					);
				}
				assert.equal(error.executablePath, "/electron");
				assert.equal(error.entryPath, "/server/bin.mjs");
	...
	it.effect(
		"reports child exit before waiting for trailing output to drain",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					...
					const runFiber = yield* DesktopBackendManager.runBackendProcess({
						...
						desktopTelemetryStream: Stream.empty,
						onExitObserved: () =>
							Deferred.succeed(exitObserved, void 0).pipe(Effect.asVoid),
					}).pipe(
						Effect.provide(Layer.merge(spawnerLayer, healthyHttpClientLayer)),
						Effect.forkChild,
					);
					...
					assert.equal(
						(yield* Fiber.join(runFiber)).code.pipe(Option.getOrUndefined),
						1,
					);
				}),
			),
	...
	it.effect(
		"continues routing desktop telemetry control messages after an invalid line",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
								...
								makeProcess({
									getOutputFd: (fd) =>
										fd === 5
											...
											: Stream.empty,
									exitCode: Deferred.await(handled).pipe(
										Effect.as(ChildProcessSpawner.ExitCode(0)),
									),
								}),
							),
						...
						desktopTelemetryPublisher: {
							handleControl: (message) =>
								message.type === "setDiagnosticsDemand"
									? Deferred.succeed(handled, message.enabled).pipe(
											Effect.asVoid,
										)
									: Effect.void,
						},
	...
	it.effect(
		"drains trailing child output before persisting an unexpected exit",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					...
					const instance = yield* makeTestInstance({
						...
						backendOutputLog: {
							writeOutputChunk: (_streamName, chunk) =>
								Ref.update(outputChunks, (current) => [
									...current,
							...
							persistFailure: () =>
								Ref.get(outputChunks).pipe(
									Effect.flatMap((chunks) =>
										Deferred.succeed(persistedOutput, chunks),
									),
									Effect.asVoid,
								),
						},
					...
					assert.deepEqual(yield* Deferred.await(persistedOutput), [
						"trailing output\n",
					]);
				...
				const instance = yield* makeTestInstance({
					...
					pruneRuntimes: (distro, runtimeId) =>
						Effect.sync(() => {
							...
							Effect.andThen(Deferred.succeed(pruneComplete, void 0)),
							Effect.asVoid,
						),
					...
					onReady: Effect.sync(() => {
						...
						Effect.andThen(Deferred.succeed(backendReady, void 0)),
						Effect.asVoid,
					),
					backendOutputLog: {
						persistFailure: () =>
	...
	it.effect(
		"starts the configured backend and closes the scoped process on stop",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					let startCount = 0;
					...
					assert.deepEqual(
						yield* instance.currentConfig,
						Option.some(baseConfig),
					);
	...
	it.effect("restarts when start is requested during stop teardown", () =>
		Effect.scoped(
			Effect.gen(function* () {
				...
				assert.equal(
					yield* Queue.take(starts).pipe(Effect.timeout("1 second")),
					2,
				);
	...
	it.effect(
		"retries config resolution after a start request during stop teardown",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const starts = yield* Queue.unbounded<number>();
					...
					const configResolve = Ref.updateAndGet(
						configAttempts,
						(attempt) => attempt + 1,
					).pipe(
						Effect.flatMap((attempt) =>
							attempt === 2
								? Effect.fail(configFailure)
	...
	it.effect(
		"does not notify shutdown before the first start has prior state",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					let shutdownCount = 0;
	...
	it.effect(
		"restarts an unexpectedly exited backend with the Effect clock",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const starts = yield* Queue.unbounded<number>();
	...
	it.effect(
		"does not notify shutdown when a scheduled restart starts from non-ready state",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					let shutdownCount = 0;
	...
	it.effect(
		"surfaces a fatal preflight failure once and stops looping after the cap",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
	...
	it.effect(
		"can be started again after a fatal preflight cap once config recovers",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
	...
	it.effect(
		"keeps retrying a transient (non-fatal) preflight failure without surfacing",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const failures: string[] = [];
	...
	it.effect(
		"surfaces a bounded transient preflight failure after its retry limit",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const failures: string[] = [];
	...
	it.effect(
		"cancels a scheduled restart when start is requested manually",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const starts = yield* Queue.unbounded<number>();
					...
					const spawnerLayer = Layer.succeed(
						...
						ChildProcessSpawner.make(() =>
							Effect.gen(function* () {
								...
								const scope = yield* Scope.Scope;
								const close = Deferred.succeed(secondClosed, void 0).pipe(
									Effect.asVoid,
								);
								yield* Scope.addFinalizer(scope, close);
								return makeProcess({
