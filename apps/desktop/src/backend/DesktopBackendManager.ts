...
type BackendProcessLayerServices =
...
interface RunBackendProcessOptions extends DesktopBackendStartConfig {
	...
	readonly onOutputFailure?: (
		error: BackendProcessOutputError,
	) => Effect.Effect<void>;
}
...
const activePid = (
	active: Option.Option<ActiveBackendRun>,
): Option.Option<number> => Option.flatMap(active, (run) => run.pid);
...
export const waitForHttpReady = (
	...
	return waitForHttpReadyShared({
		...
		probeTimeoutMs: Duration.toMillis(
			DEFAULT_BACKEND_READINESS_REQUEST_TIMEOUT,
		),
		makeError: ({ cause }) =>
...
const encodeBootstrapJson = Schema.encodeEffect(
...
> {
			...
			Stream.runForEach((line) =>
				decodeDesktopTelemetryControlLine(line).pipe(
					...
					Effect.catchCause((cause) =>
						logBackendProcessWarning(
							"ignored invalid desktop telemetry control message",
							{
								fd: controlFd,
	...
	yield* Effect.forEach(outputFibers, Fiber.await, {
		...
		Effect.timeout(
			options.outputDrainTimeout ?? DEFAULT_BACKEND_OUTPUT_DRAIN_TIMEOUT,
		),
		Effect.ignore,
	);
...
> {
	...
	const backendOutputLogFactory =
		yield* DesktopObservability.DesktopBackendOutputLogFactory;
	const backendOutputLog = yield* backendOutputLogFactory.forInstance(spec.id);
	const desktopTelemetryPublisher =
	...
	const { logWarning: logInstanceWarning, logError: logInstanceError } =
		DesktopObservability.makeComponentLogger(
			`desktop-backend-instance:${spec.id}`,
		);

	const updateActiveRun = (
		runId: number,
		f: (run: ActiveBackendRun) => ActiveBackendRun,
	...
	const start: Effect.Effect<void> = Effect.suspend(() =>
			...
			Effect.gen(function* () {
				...
				const config = yield* spec.configResolve.pipe(
					Effect.tapError((error) =>
						logInstanceError(
							"failed to generate desktop backend configuration",
							{
								cause: error.message,
							},
						),
				...
				if (Option.isSome(preflightFailure)) {
					...
					const attempt = yield* Ref.modify(state, (latest) => {
						...
						return [
							next,
							{ ...latest, preflightFailureAttempt: next },
						] as const;
					});
					if (attempt > attemptLimit) {
						...
						yield* logInstanceError(
							"backend preflight still failing after fallback; stopping",
							{
								reason,
								attempt,
							},
					...
					if (attempt === attemptLimit) {
						...
						const shouldRestart = yield* (
							spec.onPreflightFailed?.(preflightFailure.value) ??
				...
				const finalizeRun = Effect.fn("desktop.backendInstance.finalizeRun")(
				...
				const program = runBackendProcess({
					...
					onStarted: Effect.fn("desktop.backendInstance.onStarted")(
						function* (pid) {
					...
					onReadinessFailure: Effect.fn(
						"desktop.backendInstance.onReadinessFailure",
					)(function* (error) {
		...
		Effect.withSpan("desktop.backendInstance.start", {
			attributes: { id: spec.id },
		}),
	);

	const scheduleRestart = Effect.fn("desktop.backendInstance.scheduleRestart")(
		function* (reason: string) {
			...
			yield* Option.match(scheduled, {
				onNone: () => Effect.void,
				onSome: Effect.fn("desktop.backendInstance.scheduleRestartFiber")(
					function* (delay) {
						yield* logInstanceError(
							"backend exited unexpectedly; restart scheduled",
							{
								reason,
	...
	const stop = Effect.fn("desktop.backendInstance.stop")(function* (options?: {
		...
		const { active, restartFiber, notifyShutdown } = yield* mutex.withPermits(
			1,
		)(
			Effect.gen(function* () {
