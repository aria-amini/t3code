			...
			Layer.mergeAll(
				...
				Layer.succeed(
					...
					ChildProcessSpawner.make(() =>
						Effect.die("unexpected child process spawn"),
					),
				),
				Layer.succeed(
...
describe("DesktopBackendPool", () => {
	it.effect("layerTest exposes registered instances by id", () =>
		Effect.gen(function* () {
			...
			const fetchedPrimary = yield* pool.get(
				DesktopBackendPool.PRIMARY_INSTANCE_ID,
			);
			const fetchedWsl = yield* pool.get(
				DesktopBackendPool.BackendInstanceId("wsl:ubuntu"),
			);
			const fetchedMissing = yield* pool.get(
				DesktopBackendPool.BackendInstanceId("missing"),
			);
	...
	it.effect(
		"resolves the primary label lazily after pool layer construction",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
