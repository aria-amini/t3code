...
export type DesktopBackendInstance =
...
export class DesktopBackendPool extends Context.Service<
	DesktopBackendPool,
	{
		...
		) => Effect.Effect<Option.Option<DesktopBackendInstance>>;
...
export const layer = Layer.effect(
	...
	Effect.gen(function* () {
		...
		const factoryContext =
			yield* Effect.context<BackendInstanceFactoryRequirements>();
		...
		const handlePrimaryPreflightFailure = Effect.fn(
			...
			yield* logBackendPoolWarning(
				"primary WSL preflight failed; falling back to Windows",
				{
					reason,
				},
			);
		...
		const primary = yield* DesktopBackendManager.makeBackendInstance({
			...
			onReady: (httpBaseUrl) =>
				desktopWindow.handleBackendReady(httpBaseUrl).pipe(
					Effect.catch((error) =>
						logBackendPoolWarning(
							"failed to open main window after backend readiness",
							{
								error: error.message,
							},
						),
				...
				SynchronizedRef.modifyEffect(
					...
					> => {
						...
						if (existing?._tag === "Active") {
							return Effect.fail(
								new DesktopBackendPoolInstanceAlreadyRegisteredError({
									id: spec.id,
								}),
							);
						}
					...
					Effect.flatMap((result) =>
						result._tag === "Registered"
							...
							: Deferred.await(result.done).pipe(
									Effect.andThen(register(spec)),
								),
					),
				),
			);
		...
		Effect.gen(function* () {
			if (instances.length === 0) {
				return yield* Effect.die(
					"DesktopBackendPool.layerTest requires at least one instance",
				);
			}
			...
			return DesktopBackendPool.of({
				...
				primary: Effect.succeed(primary),
				register: () =>
					Effect.die("DesktopBackendPool.layerTest does not support register"),
				unregister: () =>
					Effect.die(
						"DesktopBackendPool.layerTest does not support unregister",
					),
			});
		}),
	);
