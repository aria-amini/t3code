...
function makeEnvironmentLayer(
	...
	return DesktopEnvironment.layer({
		...
		Layer.provide(
			Layer.mergeAll(
				NodeServices.layer,
				DesktopConfig.layerTest({ T3CODE_HOME: baseDir, ...env }),
			),
		),
	);
}
...
function makeLayer(input: {
	...
	const networkLayer = Layer.succeed(
		DesktopNetworkInterfaces.DesktopNetworkInterfaces,
		{
...
describe("DesktopServerExposure", () => {
	it.effect(
		"falls back to local-only without losing the requested network preference",
		() =>
			withHarness(
				emptyNetworkInterfaces,
				Effect.gen(function* () {
					const serverExposure =
						yield* DesktopServerExposure.DesktopServerExposure;
					...
					assert.equal(
						(yield* settings.get).serverExposureMode,
						"network-accessible",
					);
					...
					assert.equal(
						backendConfig.httpBaseUrl.href,
						"http://127.0.0.1:4173/",
					);
				}),
			),
	...
	it.effect(
		"returns a typed error when network access is explicitly unavailable",
		() =>
			withHarness(
				emptyNetworkInterfaces,
				Effect.gen(function* () {
					...
					const error = yield* serverExposure
						...
						.pipe(Effect.flip);
					assert.ok(
						error._tag === "DesktopServerExposureNoNetworkAddressError",
					);
	...
	it.effect(
		"persists network-accessible mode and updates backend binding state",
		() =>
			withHarness(
	...
	it.effect(
		"persists tailscale serve preferences atomically and reports no-op updates",
		() =>
			withHarness(
				emptyNetworkInterfaces,
				Effect.gen(function* () {
	...
	it.effect(
		"preserves persistence request context and the settings failure chain",
		() => {
			...
			return withHarness(
				...
				Effect.gen(function* () {
					...
					const modeError = yield* serverExposure
						.setMode("network-accessible")
						.pipe(Effect.flip);
					assert.instanceOf(
						modeError,
	...
	it.effect(
		"keeps LAN and Tailscale endpoints distinct when Tailscale is enumerated first",
		() =>
			withHarness(
	...
	it.effect(
		"does not spawn the tailscale CLI while server exposure is local-only",
		() =>
			withHarness(
				lanNetworkInterfaces,
				Effect.gen(function* () {
	...
	it.effect(
		"advertises loopback, LAN, and configured manual endpoints from runtime state",
		() =>
			withHarness(
				lanNetworkInterfaces,
				Effect.gen(function* () {
