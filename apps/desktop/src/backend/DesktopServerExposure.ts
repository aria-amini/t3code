...
const resolveDesktopServerExposure = (input: {
	...
	return {
		...
		endpointUrl: advertisedHost
			? `http://${advertisedHost}:${input.port}`
			: null,
		advertisedHost,
	};
};
...
export type DesktopServerExposureSetModeError =
	typeof DesktopServerExposureSetModeError.Type;
...
const initialRuntimeState = (): RuntimeState =>
	runtimeStateFromResolvedExposure({
		requestedMode:
			DesktopAppSettings.DEFAULT_DESKTOP_SETTINGS.serverExposureMode,
...
function resolveRuntimeState(input: {
	...
	const advertisedHostOverride = Option.getOrUndefined(
		input.advertisedHostOverride,
	);
...
export const make = Effect.gen(function* () {
	...
	const configureFromSettings = Effect.fn(
		"desktop.serverExposure.configureFromSettings",
	...
	const setMode = Effect.fn("desktop.serverExposure.setMode")(function* (
		...
		if (resolved.unavailable) {
			return yield* new DesktopServerExposureNoNetworkAddressError({
				port: previous.port,
			});
		}
