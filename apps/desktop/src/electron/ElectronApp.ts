...
export class ElectronApp extends Context.Service<
	ElectronApp,
	{
		...
		readonly getAppMetrics: Effect.Effect<
			ReadonlyArray<Electron.ProcessMetric>
		>;
		...
		readonly removeCommandLineSwitch: (
			switchName: string,
		) => Effect.Effect<void>;
...
export const make = ElectronApp.of({
	metadata: Effect.gen(function* () {
		...
		return {
			...
			runningUnderArm64Translation:
				Electron.app.runningUnderARM64Translation === true,
		};
	}),
	...
	systemLocale: Effect.sync(() =>
		Electron.app.getSystemLocale().replace(/_/g, "-"),
	),
	...
	setAsDefaultProtocolClient: (protocol, path, args) =>
		Effect.sync(() => {
			...
			return Electron.app.setAsDefaultProtocolClient(protocol, path, [
				...(args ?? []),
			]);
		}),
