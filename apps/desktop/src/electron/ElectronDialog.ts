...
export class ElectronDialogPickFolderError extends Schema.TaggedError<ElectronDialogPickFolderError>()(
	...
	override get message(): string {
		...
		const defaultPath =
			this.defaultPath === null ? "no default path" : this.defaultPath;
...
export class ElectronDialogShowMessageBoxError extends Schema.TaggedError<ElectronDialogShowMessageBoxError>()(
	...
	{
		type: Schema.NullOr(
			Schema.Literals(["none", "info", "error", "question", "warning"]),
		),
...
export const make = ElectronDialog.of({
	pickFolder: Effect.fn("desktop.electron.dialog.pickFolder")(
		function* (input) {
			...
			const openDialogOptions: Electron.OpenDialogOptions = Option.match(
				input.defaultPath,
				{
					onNone: () => ({
