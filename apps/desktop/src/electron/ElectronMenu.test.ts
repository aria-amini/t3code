...
describe("ElectronMenu", () => {
	...
	it.effect(
		"returns none without building a menu when there are no valid items",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"keeps a preceding non-destructive action in the destructive section",
		() =>
			Effect.gen(function* () {
				buildFromTemplateMock.mockImplementation(() => ({
	...
	it.effect(
		"defers popupTemplate side effects until the returned Effect runs",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"preserves context-menu failures with normalized item context",
		() =>
			Effect.gen(function* () {
