...
export type ElectronThermalState = ReturnType<
	Electron.PowerMonitor["getCurrentThermalState"]
>;
export type ElectronIdleState = ReturnType<
	Electron.PowerMonitor["getSystemIdleState"]
>;
...
export class ElectronPowerMonitor extends Context.Service<
	ElectronPowerMonitor,
	{
		...
		) => Effect.Effect<ElectronIdleState>;
...
const onSimpleEvent: ElectronPowerMonitor["Service"]["onSimpleEvent"] = (
	eventName,
	listener,
) =>
...
const onSpeedLimitChange: ElectronPowerMonitor["Service"]["onSpeedLimitChange"] =
	(listener) => {
		const wrapped = (
