...
describe("ElectronProtocol", () => {
	...
	it.effect(
		"proxies the stable renderer origin to the current app server",
		() =>
			Effect.gen(function* () {
				...
				assert.equal(
					...
					"http://127.0.0.1:3773/api/health?verbose=1",
				);
				const forwardedHeaders = new Headers(
					netFetchMock.mock.calls[0]?.[1]?.headers,
				);
			...
			const response = yield* Effect.scoped(
				Effect.gen(function* () {
					...
					return yield* Effect.promise(() =>
						handler!(new Request("t3code://other/")),
					);
				}),
			);
		...
		Effect.gen(function* () {
			...
			const response = yield* Effect.scoped(
				Effect.gen(function* () {
					...
					return yield* Effect.promise(() =>
						handler!(new Request("t3code-dev://app/")),
					);
				}),
			);
	...
	it.effect("preserves protocol registration failures", () =>
		Effect.gen(function* () {
			...
			assert.equal(
				error.message,
				'Failed to register Electron protocol scheme "t3code-dev".',
			);
		}).pipe(Effect.provide(ElectronProtocol.layer)),
	);
	...
	it.effect("preserves protocol unregistration failures", () =>
		Effect.gen(function* () {
			...
			if (exit._tag === "Failure") {
				...
				assert.equal(
					error.message,
					'Failed to unregister Electron protocol scheme "t3code".',
				);
			}
		}).pipe(Effect.provide(ElectronProtocol.layer)),
	);
