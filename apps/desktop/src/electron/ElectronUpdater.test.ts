...
describe("ElectronUpdater", () => {
	...
	it.effect("scopes updater event listeners", () =>
		Effect.gen(function* () {
			...
			assert.deepEqual(autoUpdaterMock.on.mock.calls, [
				["update-available", listener],
			]);
			assert.deepEqual(autoUpdaterMock.removeListener.mock.calls, [
				["update-available", listener],
			]);
	...
	it.effect(
		"wraps rejected update checks in the method-specific typed error",
		() =>
			Effect.gen(function* () {
				...
				assert.equal(
					error.message,
					"Electron updater failed to check for updates on channel beta.",
				);
	...
	it.effect("preserves the execution-time channel on download failures", () =>
		Effect.gen(function* () {
			...
			assert.instanceOf(
				error,
				ElectronUpdater.ElectronUpdaterDownloadUpdateError,
			);
	...
	it.effect(
		"preserves quit-and-install flags and the execution-time channel",
		() =>
			Effect.gen(function* () {
				...
				assert.instanceOf(
					error,
					ElectronUpdater.ElectronUpdaterQuitAndInstallError,
				);
				...
				assert.notInclude(error.message, cause.message);
				assert.deepEqual(autoUpdaterMock.quitAndInstall.mock.calls, [
					[true, false],
				]);
			}).pipe(Effect.provide(ElectronUpdater.layer)),
	);
