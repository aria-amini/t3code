...
const {
	...
	loadWindowsForegroundApiMock:
		vi.fn<() => Promise<{ getForegroundWindow: () => bigint }>>(),
...
describe("ElectronWindow", () => {
	beforeEach(() => {
		...
		loadWindowsForegroundApiMock
			.mockReset()
			.mockImplementation(() =>
				Promise.resolve({ getForegroundWindow: foregroundWindowMock }),
			);
	...
	it.effect(
		"preserves schema-safe creation context and the Electron cause",
		() =>
			Effect.gen(function* () {
				const cause = new Error("native BrowserWindow construction failed");
				browserWindowMock.mockImplementationOnce(
					function BrowserWindowFailure() {
						throw cause;
					},
				);
				const options = {
				...
				assert.equal(
					error.message,
					'Failed to create Electron BrowserWindow "T3 Code" (1100x780).',
				);
	...
	it.effect(
		"uses native Windows activation without querying the foreground window",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"focuses the exact T3 window before activating from a shell-hosted app",
		() =>
			Effect.gen(function* () {
				const operations: Array<string> = [];
	...
	it.effect.each([4, 8])(
		...
		(handleBytes) =>
			Effect.gen(function* () {
				...
				const electronWindow = yield* ElectronWindow.ElectronWindow;
				yield* electronWindow.prepareReveal(
					window as unknown as Electron.BrowserWindow,
				);
				window.getTitle.mockClear();

				yield* electronWindow.reveal(
					window as unknown as Electron.BrowserWindow,
				);
	...
	it.effect("continues native focus when the foreground query fails", () =>
		Effect.gen(function* () {
			...
			loadWindowsForegroundApiMock.mockRejectedValue(
				new Error("Foreground query unavailable"),
			);
			const electronWindow = yield* ElectronWindow.ElectronWindow;
			yield* electronWindow.prepareReveal(
				window as unknown as Electron.BrowserWindow,
	...
	it.effect(
		"cancels native focus when destroyed during the foreground query",
		() =>
			Effect.gen(function* () {
				const window = makeWindowsRevealWindow();
				activateWindowsForegroundMock.mockRejectedValueOnce(
	...
	it.effect(
		"preserves message delivery failures with window and channel context",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"an ordinary reveal on Windows does not touch the Win32 foreground helpers",
		() =>
			Effect.gen(function* () {
				const window = makeWindowsRevealWindow();
				const electronWindow = yield* ElectronWindow.ElectronWindow;
	...
	it.effect(
		"starts the Windows focus worker lazily and closes it with the layer",
		() =>
			Effect.gen(function* () {
				yield* ElectronWindow.ElectronWindow.pipe(
					Effect.provide(testLayer("win32")),
				);
				assert.lengthOf(startWindowsForegroundFocusThreadMock.mock.calls, 0);
				...
				yield* Effect.gen(function* () {
					const electronWindow = yield* ElectronWindow.ElectronWindow;
					const window =
						makeWindowsRevealWindow() as unknown as Electron.BrowserWindow;
					yield* electronWindow.prepareReveal(window);
					yield* electronWindow.prepareReveal(window);
