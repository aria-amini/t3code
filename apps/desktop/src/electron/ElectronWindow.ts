...
async function isWindowsBrowserWindowForeground(
	window: Electron.BrowserWindow,
): Promise<boolean> {
	const foreground = await loadWindowsForegroundApi()
		.then((api) => api.getForegroundWindow())
		.catch(() => undefined);
	if (window.isDestroyed() || foreground === undefined || foreground === 0n)
		return false;
...
export class ElectronWindowOperationError extends Schema.TaggedError<ElectronWindowOperationError>()(
	...
	override get message(): string {
		...
		const channel =
			this.channel === null
				? ""
				: ` on channel ${JSON.stringify(this.channel)}`;
...
export const make = Effect.gen(function* () {
	...
	let windowsForegroundFocus:
		| ReturnType<typeof startWindowsForegroundFocusThread>
		| undefined;
	...
	yield* Effect.addFinalizer(() =>
		Effect.sync(() => windowsForegroundFocus?.close()),
	);
	...
	return ElectronWindow.of({
		...
		reveal: (window) =>
			Effect.tryPromise({
				try: async () => {
					...
					const captureReveal =
						platform === "win32" && captureRevealWindows.delete(window.id);
					const shellHostedForeground =
						captureReveal &&
						(await isWindowsShellHostedForeground().catch(() => false));
					...
					if (captureReveal) {
						...
						try {
							...
							const needsFocus =
								!(await isWindowsBrowserWindowForeground(window));
		...
		syncAllAppearance: Effect.fn("desktop.electron.window.syncAllAppearance")(
			function* <E, R>(
				sync: (window: Electron.BrowserWindow) => Effect.Effect<void, E, R>,
			) {
