export interface WindowsForegroundApi {
	...
	readonly getWindowRect: (windowHandle: bigint) =>
		| {
				readonly x: number;
				readonly y: number;
...
function nativeWindowHandle(buffer: Buffer): bigint {
	...
	throw new Error(
		`Unsupported Windows window handle size: ${String(buffer.length)} bytes.`,
	);
}
...
export function activateWindowsForegroundWithApi(
	...
	const foregroundThreadId =
		foregroundWindow === 0n ? 0 : api.getWindowThreadId(foregroundWindow);
	const shouldAttach =
		currentThreadId !== 0 &&
	...
	const attached =
		shouldAttach &&
		api.attachThreadInput(currentThreadId, foregroundThreadId, true);
...
export function isWindowsShellHostedForegroundWithApi(
	api: WindowsForegroundApi,
): boolean {
	const foregroundWindow = api.getForegroundWindow();
	return (
		foregroundWindow !== 0n &&
		WINDOWS_SHELL_HOSTED_WINDOW_CLASSES.has(
			api.getWindowClassName(foregroundWindow),
		)
	);
}
...
export function loadWindowsForegroundApi(): Promise<WindowsForegroundApi> {
	windowsForegroundApiPromise ??= import("ffi-rs").then(
		({ DataType, load, open }) => {
			...
			return {
				...
				getWindowClassName: (windowHandle) => {
					...
					return length > 0
						? buffer.subarray(0, length * 2).toString("utf16le")
						: "";
				},
				...
				getProcessImagePath: (processId) => {
					...
					try {
						...
						return ok
							? buffer.subarray(0, size.readUInt32LE(0) * 2).toString("utf16le")
							: "";
					} finally {
						load({
							library: kernel32,
...
export async function activateWindowsForeground(
	handleBuffer: Buffer,
): Promise<void> {
...
export async function isWindowsShellHostedForeground(): Promise<boolean> {
	return isWindowsShellHostedForegroundWithApi(
		await loadWindowsForegroundApi(),
	);
}
