...
describe("preview IPC methods", () => {
	...
	it("preserves lone surrogates without collapsing them to replacement characters", () => {
		...
		const replacement = PreviewIpc.resolvePartitionScope(
			"environment",
			"profile-�",
		);
	...
	it("keeps the legacy default partition scope and incognito persistence", () => {
		expect(
			PreviewIpc.resolvePartitionScope("environment::legacy", undefined),
		).toEqual({
	...
	effectIt.effect(
		"targets imports at the same partition tuple as the renderer",
		() => {
			const received: Array<
				Parameters<BrowserImport.BrowserImport["Service"]["importCookies"]>[0]
			> = [];
			...
			return Effect.gen(function* () {
				...
				expect(received[0]).toMatchObject(
					PreviewIpc.resolvePartitionScope("a", "b"),
				);
	...
	effectIt.effect(
		"rejects invalid webContents ids before resolving the preview service",
		() =>
			Effect.map(
				...
				(exit) => {
					...
					expect(
						Option.isSome(error) && Schema.isSchemaError(error.value),
					).toBe(true);
					expect(fromPartition).not.toHaveBeenCalled();
				},
			),
