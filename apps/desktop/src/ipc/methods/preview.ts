...
export const navigate = DesktopIpc.makeIpcMethod({
	...
	result: Schema.Void,
	handler: Effect.fn("desktop.ipc.preview.navigate")(function* ({
		tabId,
		url,
	}) {
		const manager = yield* PreviewManager.PreviewManager;
...
export const setAudioMuted = DesktopIpc.makeIpcMethod({
	...
	result: Schema.Void,
	handler: Effect.fn("desktop.ipc.preview.setAudioMuted")(function* ({
		tabId,
		audioMuted,
	}) {
		const manager = yield* PreviewManager.PreviewManager;
...
export const clearCookies = DesktopIpc.makeIpcMethod({
	...
	handler: Effect.fn("desktop.ipc.preview.clearCookies")(function* ({
		...
		yield* manager.clearCookies(
			yield* resolveClearPartitions(manager, environmentId, profileId),
		);
	}),
});
 ...
 */
const resolveClearPartitions = Effect.fn(
	"desktop.ipc.preview.resolveClearPartitions",
)(function* (
	manager: PreviewManager.PreviewManager["Service"],
	environmentId: string,
	profileId: string | undefined,
...
export const getPreviewConfig = DesktopIpc.makeIpcMethod({
	...
	handler: Effect.fn("desktop.ipc.preview.getConfig")(function* ({
		...
		return {
			...
			preloadUrl: NodeURL.pathToFileURL(`${__dirname}/preview-pick-preload.cjs`)
				.href,
		};
	}),
});
...
export const listBrowserImportSources = DesktopIpc.makeIpcMethod({
	...
	handler: Effect.fn("desktop.ipc.preview.listBrowserImportSources")(
		function* () {
...
export const setAnnotationTheme = DesktopIpc.makeIpcMethod({
	...
	result: Schema.Void,
	handler: Effect.fn("desktop.ipc.preview.setAnnotationTheme")(function* ({
		theme,
	}) {
		const manager = yield* PreviewManager.PreviewManager;
...
export const captureScreenshot = DesktopIpc.makeIpcMethod({
	...
	handler: Effect.fn("desktop.ipc.preview.captureScreenshot")(function* ({
		tabId,
	}) {
		const manager = yield* PreviewManager.PreviewManager;
...
export const revealArtifact = DesktopIpc.makeIpcMethod({
	...
	result: Schema.Void,
	handler: Effect.fn("desktop.ipc.preview.revealArtifact")(function* ({
		path,
	}) {
		const manager = yield* PreviewManager.PreviewManager;
...
export const copyArtifactToClipboard = DesktopIpc.makeIpcMethod({
	...
	payload: DesktopPreviewArtifactInputSchema,
	result: Schema.Void,
	handler: Effect.fn("desktop.ipc.preview.copyArtifactToClipboard")(function* ({
		path,
	}) {
		const manager = yield* PreviewManager.PreviewManager;
...
export const automationStatus = DesktopIpc.makeIpcMethod({
	...
	handler: Effect.fn("desktop.ipc.preview.automationStatus")(function* ({
		tabId,
	}) {
		const manager = yield* PreviewManager.PreviewManager;
