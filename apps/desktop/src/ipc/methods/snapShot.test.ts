...
describe("window capture IPC", () => {
	...
	it.effect(
		"requires a trusted renderer for both config read and write approval",
		() => {
			const calls: string[] = [];
			return Effect.gen(function* () {
				...
				yield* applySnapShotConfig.handler(configPreview.id, {
					sender: { id: 7 },
				});
	...
	it.effect(
		"uses only the file returned by the native custom config picker",
		() => {
	...
	it.effect(
		"forwards a trusted renderer animation destination in screen coordinates",
		() => {
	...
	it.effect(
		"forwards the accessibility permission preference from a trusted renderer",
		() => {
			...
			return Effect.gen(function* () {
				yield* requestSnapShotPermissions.handler(false, {
					sender: webContents,
				});
	...
	it.effect("checks shortcut availability for a trusted renderer", () => {
		const layer = Layer.mergeAll(
			...
			Layer.succeed(
				...
				DesktopSnapShot.DesktopSnapShot.of({
					checkShortcut: () =>
						Effect.succeed({ available: true, message: null }),
				} as unknown as DesktopSnapShot.DesktopSnapShot["Service"]),
			),
		);
