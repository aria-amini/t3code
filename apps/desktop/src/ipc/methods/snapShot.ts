...
const ensureTrustedSnapShotSender = Effect.fn(
	"desktop.ipc.snapShot.ensureTrustedSender",
...
export const requestSnapShotPermissions = DesktopIpc.makeIpcMethod({
	...
	handler: Effect.fn("desktop.ipc.snapShot.requestPermissions")(
		function* (includeAccessibility, event) {
			...
			yield* (yield* DesktopSnapShot.DesktopSnapShot).requestPermissions(
...
export const previewSnapShotConfig = DesktopIpc.makeIpcMethod({
	...
	handler: Effect.fn("desktop.ipc.snapShot.previewConfig")(
					...
					filters: [
						{
							name: "Desktop config",
							extensions:
								state.linuxBackend === "niri" ? ["kdl"] : ["conf", "lua"],
						},
					],
					multiple: false,
				});
...
export const checkSnapShotShortcut = DesktopIpc.makeIpcMethod({
	...
	handler: Effect.fn("desktop.ipc.snapShot.checkShortcut")(
		function* (shortcut, event) {
			yield* ensureTrustedSnapShotSender(event);
			return yield* (yield* DesktopSnapShot.DesktopSnapShot).checkShortcut(
				shortcut,
			);
		},
	),
...
export const setSnapShotShortcutSuppressed = DesktopIpc.makeIpcMethod({
	...
	payload: Schema.Boolean,
	result: Schema.Void,
	handler: Effect.fn("desktop.ipc.snapShot.setShortcutSuppressed")(
		function* (suppressed, event) {
			yield* ensureTrustedSnapShotSender(event);
			yield* (yield* DesktopSnapShot.DesktopSnapShot).setShortcutSuppressed(
				suppressed,
			);
		},
	),
...
export const setSnapShotAnimationDestination = DesktopIpc.makeIpcMethod({
	...
	handler: Effect.fn("desktop.ipc.snapShot.setAnimationDestination")(
		function* (destination, event) {
			...
			yield* (yield* DesktopSnapShot.DesktopSnapShot).setAnimationDestination(
				destination.id,
				{
					...
					borderWidth:
						destination.borderWidth * window.webContents.getZoomFactor(),
					cornerRadius:
						destination.cornerRadius * window.webContents.getZoomFactor(),
...
export const dismissSnapShotAnimation = DesktopIpc.makeIpcMethod({
	...
	payload: DesktopSnapShotId,
	result: Schema.Void,
	handler: Effect.fn("desktop.ipc.snapShot.dismissAnimation")(
		function* (id, event) {
			yield* ensureTrustedSnapShotSender(event);
