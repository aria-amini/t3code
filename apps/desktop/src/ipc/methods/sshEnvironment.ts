...
type DesktopSshEnvironmentRequestCause =
...
const isEnvironmentOperationForbiddenError = Schema.is(
	EnvironmentOperationForbiddenError,
);
const isEnvironmentRequestInvalidError = Schema.is(
	EnvironmentRequestInvalidError,
);
const isEnvironmentScopeRequiredError = Schema.is(
	EnvironmentScopeRequiredError,
);

function readSshHttpStatus(
...
export class DesktopSshEnvironmentRequestError extends Data.TaggedError(
	...
	override get message() {
		const prefix =
			this.sshHttpStatus === null ? "" : `[ssh_http:${this.sshHttpStatus}] `;
...
const withLoopbackSshApi =
	...
	(
		httpBaseUrl: string,
	): Effect.Effect<A, DesktopSshEnvironmentRequestError, R> =>
...
export const resolveSshHost = DesktopIpc.makeIpcMethod({
	...
	handler: Effect.fn("desktop.ipc.sshEnvironment.resolveHost")(
		function* (alias) {
			const sshEnvironment = yield* DesktopSshEnvironment.DesktopSshEnvironment;
...
export const ensureSshEnvironment = DesktopIpc.makeIpcMethod({
	...
	handler: Effect.fn("desktop.ipc.sshEnvironment.ensureEnvironment")(
...
export const disconnectSshEnvironment = DesktopIpc.makeIpcMethod({
	...
	result: Schema.Void,
	handler: Effect.fn("desktop.ipc.sshEnvironment.disconnectEnvironment")(
		function* (target) {
			const sshEnvironment = yield* DesktopSshEnvironment.DesktopSshEnvironment;
...
export const fetchSshEnvironmentDescriptor = DesktopIpc.makeIpcMethod({
	...
	handler: Effect.fn("desktop.ipc.sshEnvironment.fetchDescriptor")(function* ({
		httpBaseUrl,
	}) {
		return yield* withLoopbackSshApi(
			"fetch-environment-descriptor",
			(resolvedHttpBaseUrl) =>
...
export const bootstrapSshBearerSession = DesktopIpc.makeIpcMethod({
	...
	handler: Effect.fn("desktop.ipc.sshEnvironment.bootstrapBearerSession")(
		function* ({ httpBaseUrl, credential }) {
			return yield* withLoopbackSshApi(
				"bootstrap-bearer-session",
				(resolvedHttpBaseUrl) =>
					bootstrapRemoteBearerSession({
...
export const fetchSshSessionState = DesktopIpc.makeIpcMethod({
	...
	handler: Effect.fn("desktop.ipc.sshEnvironment.fetchSessionState")(
		function* ({ httpBaseUrl, bearerToken }) {
			return yield* withLoopbackSshApi(
				"fetch-session-state",
				(resolvedHttpBaseUrl) =>
					fetchRemoteSessionState({
...
export const issueSshWebSocketTicket = DesktopIpc.makeIpcMethod({
	...
	handler: Effect.fn("desktop.ipc.sshEnvironment.issueWebSocketTicket")(
		function* ({ httpBaseUrl, bearerToken }) {
			return yield* withLoopbackSshApi(
				"issue-websocket-ticket",
				(resolvedHttpBaseUrl) =>
...
export const resolveSshPasswordPrompt = DesktopIpc.makeIpcMethod({
	...
	result: Schema.Void,
	handler: Effect.fn("desktop.ipc.sshEnvironment.resolvePasswordPrompt")(
		function* ({ requestId, password }) {
			const prompts =
				yield* DesktopSshPasswordPrompts.DesktopSshPasswordPrompts;
