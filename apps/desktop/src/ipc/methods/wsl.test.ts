...
function makeWslBackendLayer(
	input: { readonly onReconcile?: Effect.Effect<void> } = {},
) {
...
const unusedLifecycleRuntimeLayer = Layer.mergeAll(
	...
	Layer.succeed(
		...
		ElectronTheme.ElectronTheme.of(
			{} as ElectronTheme.ElectronTheme["Service"],
		),
	),
	Layer.succeed(
...
describe("WSL IPC", () => {
	it.effect(
		"stages dual-backend preferences before enabling without relaunching",
		() => {
	...
	it.effect(
		"stages WSL-only preferences and relaunches only after enabling",
		() => {
			const relaunchReasons: Array<string> = [];
			const layer = Layer.mergeAll(
				DesktopAppSettings.layerTest({
	...
	it.effect(
		"relaunches when enabling the WSL backend while wsl-only is already persisted",
		() => {
			const relaunchReasons: Array<string> = [];
	...
	it.effect(
		"clears wsl-only before relaunching when disabling a WSL-only backend",
		() => {
			const relaunchReasons: Array<string> = [];
			let reconcileCount = 0;
			const layer = Layer.mergeAll(
