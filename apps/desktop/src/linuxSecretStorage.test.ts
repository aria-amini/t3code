...
describe("linuxSecretStorage", () => {
	it("preserves explicit supported password-store preferences", () => {
		expect(normalizeLinuxPasswordStorePreference("gnome-libsecret")).toBe(
			"gnome-libsecret",
		);
	...
	it("leaves canonical KDE sessions to Electron's own wallet selection", () => {
		...
		expect(
			autoSwitch({ XDG_CURRENT_DESKTOP: "KDE", KDE_SESSION_VERSION: "6" }),
		).toBeNull();
		expect(
			autoSwitch({ XDG_CURRENT_DESKTOP: "KDE", KDE_SESSION_VERSION: "5" }),
		).toBeNull();
	...
	it("forces gnome-libsecret for unrecognized Linux desktop sessions", () => {
		...
		expect(autoSwitch({ XDG_CURRENT_DESKTOP: "Hyprland" })).toBe(
			"gnome-libsecret",
		);
	...
	it("forces gnome-libsecret for desktops Electron recognizes but leaves on basic text", () => {
		...
		expect(autoSwitch({ XDG_CURRENT_DESKTOP: "LXQt:GNOME" })).toBe(
			"gnome-libsecret",
		);
		expect(autoSwitch({ XDG_CURRENT_DESKTOP: "LXQt:KDE" })).toBe(
			"gnome-libsecret",
		);
		expect(autoSwitch({ XDG_CURRENT_DESKTOP: "LXQt:plasma" })).toBe(
			"gnome-libsecret",
		);
	});
	...
	it("does not treat lowercase desktop names as ones Electron recognizes", () => {
		...
		expect(autoSwitch({ XDG_CURRENT_DESKTOP: "gnome" })).toBe(
			"gnome-libsecret",
		);
	...
	it("overrides KDE sessions identified only by legacy variables", () => {
		...
		expect(autoSwitch({ XDG_CURRENT_DESKTOP: "plasma" })).toBe(
			"gnome-libsecret",
		);
		for (const session of [
			...
			expect(autoSwitch({ XDG_SESSION_DESKTOP: session })).toBe(
				"gnome-libsecret",
			);
			expect(autoSwitch({ GDMSESSION: session })).toBe("gnome-libsecret");
		}
		expect(
			autoSwitch({ DESKTOP_SESSION: "kde", KDE_SESSION_VERSION: "6" }),
		).toBe("gnome-libsecret");
		expect(autoSwitch({ KDE_SESSION_VERSION: "6" })).toBe("gnome-libsecret");
		expect(
			autoSwitch({ KDE_FULL_SESSION: "true", KDE_SESSION_VERSION: "6" }),
		).toBe("gnome-libsecret");
	});
	...
	it("ignores stale session hints when XDG_CURRENT_DESKTOP is authoritative", () => {
		...
		expect(
			autoSwitch({ XDG_CURRENT_DESKTOP: "niri", KDE_SESSION_VERSION: "6" }),
		).toBe("gnome-libsecret");
		expect(
			autoSwitch({ XDG_CURRENT_DESKTOP: "niri", XDG_SESSION_DESKTOP: "GNOME" }),
		).toBe("gnome-libsecret");
		expect(autoSwitch({ XDG_CURRENT_DESKTOP: "niri:plasma" })).toBe(
			"gnome-libsecret",
		);
		expect(
			autoSwitch({
