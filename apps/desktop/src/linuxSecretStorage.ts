export type LinuxPasswordStorePreference =
	...
	| "kwallet5"
	| "kwallet6";
export type LinuxPasswordStoreSwitch = Exclude<
	LinuxPasswordStorePreference,
	"auto"
>;

export const DEFAULT_LINUX_PASSWORD_STORE: LinuxPasswordStorePreference =
	"auto";
...
function electronSelectsProtectedBackend(env: NodeJS.ProcessEnv): boolean {
	for (const name of splitDesktopNameList(env.XDG_CURRENT_DESKTOP)) {
		...
		if (
			trimmed === ELECTRON_KDE_DESKTOP ||
			ELECTRON_LIBSECRET_DESKTOPS.has(trimmed)
		) {
			return true;
		}
