...
describe("resolveAnnotationSubmission", () => {
	it("attaches on Enter and sends on Cmd/Ctrl+Enter", () => {
		...
		expect(resolveAnnotationSubmission(keyboardEvent({ metaKey: true }))).toBe(
			"send",
		);
		expect(resolveAnnotationSubmission(keyboardEvent({ ctrlKey: true }))).toBe(
			"send",
		);
	});
	...
	it("leaves Shift+Enter and composition events available for editing", () => {
		expect(
			resolveAnnotationSubmission(keyboardEvent({ shiftKey: true })),
		).toBeNull();
		expect(
			resolveAnnotationSubmission(keyboardEvent({ isComposing: true })),
		).toBeNull();
