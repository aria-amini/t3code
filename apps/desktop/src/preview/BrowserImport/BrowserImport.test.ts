	...
	const importer = yield* BrowserImport.BrowserImport.pipe(
		Effect.provide(
			BrowserImport.layer.pipe(
				...
				Layer.provide(
					Layer.succeed(HostProcessExecutablePath, "/Applications/T3 Code.app"),
				),
...
describe("BrowserImport.writeCookies", () => {
	...
	it.effect(
		"flushes the store after writing, and reports success if the flush fails",
		() =>
			Effect.gen(function* () {
