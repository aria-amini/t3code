...
const unavailableReason = Effect.fn("BrowserImport.unavailableReason")(
	...
	> {
		...
		if (definition.engine === "safari") {
			...
			if (jar !== undefined && (yield* safariAccessDenied(jar)))
...
export const writeCookies = Effect.fn("BrowserImport.writeCookies")(function* (
	...
	for (const cookie of read.cookies) {
		const written = yield* Effect.tryPromise({
			...
			Effect.tapError((error) =>
				Effect.logDebug(error.message, { cause: error.cause }),
			),
			Effect.catchTags({
				BrowserCookieWriteError: () => Effect.succeed(false),
			}),
		);
		if (written) {
			imported += 1;
	...
	if (imported > 0) {
		yield* Effect.tryPromise(() => session.cookies.flushStore()).pipe(
			Effect.tapError((error) =>
				Effect.logWarning("Imported cookies could not be flushed to disk", {
					cause: error.cause,
				}),
			),
			Effect.ignore,
	...
	return {
		imported,
		skipped,
		skippedDomains: [...skippedDomains].slice(0, 20),
	};
});
...
export const make = Effect.gen(function* BrowserImportMake() {
	...
	const listSources: Effect.Effect<ReadonlyArray<BrowserImportSource>> =
		Effect.forEach(
			BROWSER_IMPORT_SOURCES,
				...
				return {
					...
					profiles:
						unavailable === undefined
							? yield* listSourceProfiles(definition, pathContext)
							: [],
	...
	const importCookies = Effect.fn("BrowserImport.importCookies")(
		function* (input: {
			readonly input: BrowserImportInput;
			readonly scope: string;
			readonly persistent: boolean;
			...
			const sourceProfiles = yield* listSourceProfiles(
				definition,
				pathContext,
			).pipe(Effect.provide(platformServices));
