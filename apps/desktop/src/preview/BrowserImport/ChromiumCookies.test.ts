...
describe("cookieScope", () => {
	...
	it("matches the scheme to the secure flag", () => {
		expect(cookieScope("example.test", "/", false).url).toBe(
			"http://example.test/",
		);
	});
...
describe("readChromiumCookieDatabase", () => {
	...
	it.effect(
		...
		() =>
          ...
          top_frame_site_key text not null default ''
				...
				const partial = yield* readChromiumCookieDatabase(
					filename,
					keys,
					"linux",
				);
				...
				const partitioned = yield* readChromiumCookieDatabase(
					filename,
					keys,
					"linux",
				);
	...
	it.effect(
		"reads plaintext, encrypted, and genuinely empty cookie values",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const directory = yield* fileSystem.makeTempDirectoryScoped({
        ...
        `;
				...
				const result = yield* readChromiumCookieDatabase(
					filename,
					{ cbcV10: key },
					"darwin",
				);
				...
				expect(
					result.cookies.map(({ name, value }) => ({ name, value })),
				).toEqual([
	...
	it.effect("enforces domain binding only for schema 24 and newer", () =>
        ...
        `;
          ...
          ('short.example', 'short', '', ${encryptV10("short value", key)}, '/', 0, 1, 0, 0)`;
			...
			expect(
				result.cookies.map(({ name, value }) => ({ name, value })),
			).toEqual([{ name: "valid", value: "kept" }]);
			expect(result.undecryptable).toBe(2);
			expect(result.undecryptableHosts).toEqual([
				"wrong.example",
				"short.example",
			]);
		}).pipe(Effect.provide(NodeServices.layer), Effect.scoped),
	...
	it.effect(
		"decrypts mixed v10 and v11 cookies with their respective keys",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const directory = yield* fileSystem.makeTempDirectoryScoped({
        ...
        `;
          ...
          ('v11.example', 'v11-cookie', '', ${encryptChromium("v11", boundValue("v11.example", "v11 value"), cbcV11)}, '/', 0, 1, 0, 0)`;
				...
				const complete = yield* readChromiumCookieDatabase(
					filename,
					{ cbcV10, cbcV11 },
					"linux",
				);
				expect(
					complete.cookies.map(({ name, value }) => ({ name, value })),
				).toEqual([
				...
				const v10Only = yield* readChromiumCookieDatabase(
					filename,
					{ cbcV10 },
					"linux",
				);
				expect(
					v10Only.cookies.map(({ name, value }) => ({ name, value })),
				).toEqual([{ name: "v10-cookie", value: "v10 value" }]);
	...
	it.effect("recovers records written with the empty-passphrase key", () =>
        ...
        `;
          ...
          ('ev11.example', 'empty-v11', '', ${encryptChromium("v11", "empty v11 value", cbcEmpty)}, '/', 0, 1, 0, 0)`;
			...
			expect(
				recovered.cookies.map(({ name, value }) => ({ name, value })),
			).toEqual([
			...
			const noV11 = yield* readChromiumCookieDatabase(
				filename,
				{ cbcV10, cbcEmpty },
				"linux",
			);
	...
	it.effect(
		"preserves arbitrary long encrypted values from pre-24 schemas",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const directory = yield* fileSystem.makeTempDirectoryScoped({
	...
	it.effect(
		"treats unversioned encrypted values as legacy plaintext on macOS and Linux",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const directory = yield* fileSystem.makeTempDirectoryScoped({
        ...
        `;
          ...
          ('legacy.example', 'legacy', '', ${Buffer.from("legacy cleartext")}, '/', 0, 0, 0, 0)`;
				...
				const mac = yield* readChromiumCookieDatabase(
					filename,
					{ cbcV10: key },
					"darwin",
				);
				const linux = yield* readChromiumCookieDatabase(
					filename,
					{ cbcV10: key },
					"linux",
				);
	...
	it.effect(
		"skips partitioned cookies without breaking pre-CHIPS schemas",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const directory = yield* fileSystem.makeTempDirectoryScoped({
        ...
        `;
          ...
          ('legacy.example', 'legacy', 'kept', ${new Uint8Array()}, '/', 0, 0, 0, 0)`;
					...
					Effect.provide(NodeSqliteClient.layer({ filename: legacyFilename })),
				);
        ...
        `;
          ...
          ('partitioned.example', 'partitioned', 'must skip', ${new Uint8Array()}, '/', 0, 1, 0, 0, 'https://top.example')`;
					...
					Effect.provide(NodeSqliteClient.layer({ filename: chipsFilename })),
				);

				const legacy = yield* readChromiumCookieDatabase(
					legacyFilename,
					{ cbcV10: key },
					"darwin",
				);
				const chips = yield* readChromiumCookieDatabase(
					chipsFilename,
					{ cbcV10: key },
					"darwin",
				);
