...
export function decryptChromiumValue(
	...
	if (platform === "darwin" || platform === "linux") {
		return (
			stripDomainBinding(buffer, domain, schemaVersion)?.toString("utf8") ??
			null
		);
	}
	return null;
...
export const readChromiumCookieDatabase = Effect.fn(
	"ChromiumCookies.readChromiumCookieDatabase",
)(function* (
	snapshotPath: string,
	keys: ChromiumKeyMaterial,
	...
	const result = yield* Effect.gen(function* () {
		...
		const schemaVersion =
			yield* sql`select value from meta where key = 'version' limit 1`.pipe(
	...
	if (
		...
		result.rows.some(
			(row) =>
				...
				Buffer.from(row.encrypted_value.subarray(0, 3)).toString("latin1") ===
					"v11",
		)
	) {
...
export const readChromiumCookies = Effect.fn(
	"ChromiumCookies.readChromiumCookies",
)(function* (
...
> {
	...
	const snapshotPath = yield* snapshotCookieDatabase(
	...
	return yield* readChromiumCookieDatabase(
		snapshotPath,
		keys,
		source.platform,
	).pipe(
