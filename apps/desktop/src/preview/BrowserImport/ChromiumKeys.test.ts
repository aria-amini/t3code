...
const helperLayer = (input: {
	...
	Layer.merge(
		Layer.succeed(
			...
			"/bundled/browser-secret/t3-browser-secret",
		),
		Layer.succeed(
					...
					: Effect.succeed(
							ChildProcessSpawner.makeHandle({
								...
								exitCode: Effect.succeed(
									ChildProcessSpawner.ExitCode(input.exitCode ?? 0),
								),
							...
							Effect.tap(() =>
								Effect.sync(() => input.capture?.(command as CapturedCommand)),
							),
						),
			),
		),
...
describe("Linux Chromium secrets", () => {
	it.effect(
		"retains a missing helper failure alongside the keyring-free fallback",
		() =>
			Effect.gen(function* () {
	...
	it.effect("reports an unconfigured helper without searching PATH", () =>
		readLinuxSecret("chromium").pipe(
			Effect.flip,
			Effect.tap((error) =>
				Effect.sync(() => expect(error.reason).toBe("keychainUnavailable")),
			),
	...
	it.effect(
		"reports an unavailable Secret Service backend as a read failure",
		() =>
			Effect.gen(function* () {
	...
	it.effect("drains stdout and stderr concurrently", () =>
		Effect.gen(function* () {
			...
			const secret = yield* readLinuxSecret("chrome").pipe(
				Effect.provide(
					helperLayer({ stdoutStream: stdout, stderrStream: stderr }),
				),
			);
	...
	it.effect(
		"does not discard a denied unlock prompt while resolving keys",
		() =>
			Effect.gen(function* () {
				...
				Effect.provide(
					helperLayer({ stderr: "Keyring is locked", exitCode: 3 }),
				),
			),
	);

	it.effect(
		"keeps the v10 fallback when the Secret Service backend is unavailable",
		() =>
			Effect.gen(function* () {
				const keys = yield* resolveChromiumKeys({
					platform: "linux",
...
describe("Windows Chromium secrets", () => {
	it.effect("accepts only DPAPI-wrapped non-app-bound keys", () =>
		Effect.gen(function* () {
			...
			const encoded = Buffer.concat([Buffer.from("DPAPI"), wrapped]).toString(
				"base64",
			);
	...
	it.effect(
		"unwraps the binary key through PowerShell without placing it in argv",
		() => {
			let captured: CapturedCommand | undefined;
			const wrapped = Buffer.from("wrapped-key");
