...
export class ChromiumKeyError extends Schema.TaggedError<ChromiumKeyError>()(
	"ChromiumKeyError",
	{
 ...
 */
const readKeychainSecret = Effect.fn("ChromiumKeys.readKeychainSecret")(
	function* (service: string, account: string) {
 ...
 */
export const readLinuxSecret = Effect.fn("ChromiumKeys.readLinuxSecret")(
		...
		return yield* Effect.scoped(
			Effect.gen(function* () {
				...
				const handle = yield* spawner
					.spawn(
						ChildProcess.make(helper, [application], {
							stdin: "ignore",
							env: environment,
						}),
					)
					.pipe(
						Effect.mapError(
							(cause) =>
								new ChromiumKeyError({ reason: "keychainUnavailable", cause }),
						),
					);
...
export const decodeWindowsWrappedKey = Effect.fn(
	"ChromiumKeys.decodeWindowsWrappedKey",
...
export const unwrapWindowsDpapiKey = Effect.fn(
	"ChromiumKeys.unwrapWindowsDpapiKey",
)(function* (wrapped: Buffer) {
	const environment = yield* HostProcessEnvironment;
	return yield* Effect.scoped(
		Effect.gen(function* () {
			const spawner = yield* ChildProcessSpawner.ChildProcessSpawner;
			...
			const plain = yield* Effect.fromResult(
				Encoding.decodeBase64(plainEncoded),
			).pipe(
				Effect.mapError(
					(cause) => new ChromiumKeyError({ reason: "readFailed", cause }),
				),
...
export const readWindowsKey = Effect.fn("ChromiumKeys.readWindowsKey")(
	function* (localStatePath: string) {
		...
		return yield* unwrapWindowsDpapiKey(
			yield* decodeWindowsWrappedKey(contents),
		);
	},
);
...
export const resolveChromiumKeys = Effect.fn(
	"ChromiumKeys.resolveChromiumKeys",
)(function* (
...
> {
	if (request.platform === "darwin") {
		...
		const secret = yield* readKeychainSecret(
