...
describe("snapshotCookieDatabase", () => {
		...
		runNode(
			Effect.gen(function* () {
				...
				const rows = yield* Effect.gen(function* () {
					...
					Effect.provide(
						NodeSqliteClient.layer({ filename: snapshot, readonly: true }),
					),
				);
	...
	it.effect(
		"propagates snapshot failures and removes its temporary directory",
		() =>
			runNode(
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
					...
					const temporaryEntries = yield* fileSystem.readDirectory(
						path.dirname(sourceDirectory),
					);
					expect(
						temporaryEntries.some((entry) => entry.startsWith(prefix)),
					).toBe(false);
				}),
			),
	);
	...
	it.effect("removes a successful snapshot when its scope closes", () =>
		runNode(
			Effect.gen(function* () {
				...
				const snapshot = yield* snapshotCookieDatabase(source).pipe(
					Effect.scoped,
				);
