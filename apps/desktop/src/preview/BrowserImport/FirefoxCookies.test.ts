...
const parsePosixFirefoxProfiles = (
	ini: string,
	root = "/home/user/.mozilla/firefox",
) =>
	Effect.gen(function* () {
...
const parseWindowsFirefoxProfiles = (
	ini: string,
	root = "C:\\Users\\user\\Firefox",
) =>
	Effect.gen(function* () {
		const path = yield* Path.Path;
		return parseFirefoxProfiles(ini, path, root);
...
describe("readFirefoxCookies", () => {
	...
	it.effect(
		"keeps an unset SameSite unspecified instead of widening it to none",
		() =>
			run(
				Effect.gen(function* () {
					...
					expect(
						cookies.map(({ name, sameSite }) => ({ name, sameSite })),
					).toEqual([
	...
	it.effect("imports only the default container", () =>
		run(
			Effect.gen(function* () {
				...
				expect(cookies.map((cookie) => cookie.value)).toEqual([
					"default-container",
				]);
			}),
		),
...
describe("parseFirefoxProfiles", () => {
	it.effect("reads named profiles and ignores Install sections", () =>
		Effect.gen(function* () {
			...
			expect(parsed).toEqual([
				{
					directory: "Profiles/abcd1234.default-release",
					name: "default-release",
				},
	...
	it.effect("falls back to the path when a profile has no name", () =>
		Effect.gen(function* () {
			expect(
				yield* parsePosixFirefoxProfiles(
					["[Profile0]", "Path=Profiles/x.default"].join("\n"),
				),
			).toEqual([
