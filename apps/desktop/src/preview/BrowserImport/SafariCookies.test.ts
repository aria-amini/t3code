...
describe("parseBinaryCookies", () => {
	...
	it("reads cookies spread across multiple pages", () => {
		...
		const parsed = parseBinaryCookies(
			Buffer.concat([header, firstPage, secondPage]),
		);
	...
	it("rejects a page that runs past the end of the file", () => {
		...
		expect(() =>
			parseBinaryCookies(Buffer.concat([header, firstPage, secondPage])),
	...
	it("rejects record offsets that point into the page header or an earlier record", () => {
		...
		expect(() => parseBinaryCookies(overlapping)).toThrow(
			SafariCookieReadError,
		);
	...
	it("accepts the checksum and property-list trailer Safari writes", () => {
		...
		expect(
			parseBinaryCookies(Buffer.concat([file, checksum, plistLength, plist])),
		).toHaveLength(1);
	});
	...
	it("rejects a jar whose page table stops short of its contents", () => {
		...
		expect(() =>
			parseBinaryCookies(
				Buffer.concat([first, Buffer.alloc(8), badLength, Buffer.from("x")]),
			),
		).toThrow(SafariCookieReadError);
	});
...
describe("readSafariCookies", () => {
	it.effect(
		"adds the cookie path and parser cause to malformed jar failures",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const directory = yield* fileSystem.makeTempDirectoryScoped({
					prefix: "t3code-safari-",
				});
	...
	it.effect("reports a TCC denial as a permission the user can grant", () =>
		Effect.gen(function* () {
			...
			const error = yield* readSafariCookies(
				"/protected/Cookies.binarycookies",
			).pipe(
				Effect.flip,
				Effect.provide(
					FileSystem.layerNoop({ readFile: () => Effect.fail(denied) }),
				),
			);
	...
	it.effect(
		"reports an ordinary permission failure as a plain read failure",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const directory = yield* fileSystem.makeTempDirectoryScoped({
				...
				yield* fileSystem.writeFile(
					jar,
					new Uint8Array([0x63, 0x6f, 0x6f, 0x6b]),
				);
	...
	it.effect("reports a missing jar as a plain read failure", () =>
		Effect.gen(function* () {
			...
			const error = yield* readSafariCookies(
				`${directory}/absent.binarycookies`,
			).pipe(Effect.flip);
...
describe("safariAccessDenied", () => {
	...
	it.effect(
		"does not read a readable jar, or any other failure, as denied",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const directory = yield* fileSystem.makeTempDirectoryScoped({
				...
				assert.isFalse(
					yield* safariAccessDenied(`${directory}/absent.binarycookies`),
				);
			}).pipe(Effect.provide(NodeServices.layer), Effect.scoped),
	);
});
...
describe("isPermissionDenied", () => {
	...
	const platformError = (
		...
		({
			_tag: "PlatformError",
			reason: { _tag: reasonTag, cause: { code } },
		}) as never;
	...
	it("does not send an ordinary EACCES failure to the Full Disk Access grant", () => {
		...
		expect(
			isPermissionDenied(platformError("PermissionDenied", "EACCES")),
		).toBe(false);
	});
