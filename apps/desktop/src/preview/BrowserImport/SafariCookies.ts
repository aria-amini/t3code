...
export const SafariCookieReadFailure = Schema.Literals([
	"needsFullDiskAccess",
	"readFailed",
]);
...
export function parseBinaryCookies(
	buffer: Buffer,
 ...
 */
export const safariAccessDenied = Effect.fnUntraced(function* (
	cookiePath: string,
) {
...
export const readSafariCookies = Effect.fn("SafariCookies.readSafariCookies")(
		...
		const contents = yield* fileSystem.readFile(cookiePath).pipe(
			Effect.mapError((cause) => {
				...
				return new SafariCookieReadError({
					reason: isPermissionDenied(cause)
						? "needsFullDiskAccess"
						: "readFailed",
