...
describe("Linux Chromium secret applications", () => {
	it("pins the libsecret application attribute for each supported fork", () => {
		assert.deepEqual(
			Object.fromEntries(
				BROWSER_IMPORT_SOURCES.filter((source) =>
					source.platforms.includes("linux"),
				).map((source) => [source.id, source.linuxSecretApplication]),
			),
			{
				chrome: "chrome",
				edge: "msedge",
...
const withSourceHome = Effect.fnUntraced(function* () {
	...
	const home = yield* fileSystem.makeTempDirectoryScoped({
		prefix: "t3code-sources-",
	});
	const context = yield* sourcePathContext.pipe(
		Effect.provideService(HostProcessEnvironment, { HOME: home }),
		Effect.provideService(HostProcessPlatform, "darwin"),
	);
	yield* fileSystem.makeDirectory(userDataDirectory(context), {
		recursive: true,
	});
	return context;
});
...
const userDataDirectory = (context: BrowserImportPathContext) => {
	...
	if (root === undefined)
		throw new Error("Helium has no macOS user-data directory");
	return root;
};
...
const writeCookieDatabase = (file: string, count: number) =>
	Effect.sync(() => {
		...
		for (let index = 0; index < count; index += 1)
			insert.run("example.test", `c${index}`);
		database.close();
	});
...
const writeFirefoxCookieDatabase = (
	...
	Effect.sync(() => {
		...
		for (let index = 0; index < defaultContainerCount; index += 1)
			insert.run("");
		for (let index = 0; index < containerCount; index += 1)
			insert.run("^userContextId=2");
		database.close();
	});

describe("Helium on Linux", () => {
	it.effect.skipIf(!symlinksSupported)(
		"discovers its profiles and checks the user-data lock",
		() =>
			run(
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
					const home = yield* fileSystem.makeTempDirectoryScoped({
						prefix: "t3code-helium-linux-",
					});
					const context = yield* sourcePathContext.pipe(
						Effect.provideService(HostProcessEnvironment, { HOME: home }),
					...
					yield* fileSystem.makeDirectory(`${root}/Default`, {
						recursive: true,
					});
...
describe("Helium on Windows", () => {
	it.effect(
		"uses Helium's local app-data profile while other Chromium forks stay disabled",
		() =>
			run(
				Effect.gen(function* () {
					const context = yield* sourcePathContext.pipe(
...
describe("isSourceRunning", () => {
	it.effect(
		"uses the held cookie database as Chromium's Windows running signal",
		() =>
			run(
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
					...
					yield* fileSystem.makeDirectory(
						context.path.join(profile, "Network"),
						{ recursive: true },
					);
...
describe("chromiumSingletonLockIsHeld", () => {
	it.effect("ignores a positively dead PID on the current host", () =>
		Effect.gen(function* () {
			...
			const held = yield* chromiumSingletonLockIsHeld(
				"current-host-4321",
				"current-host",
				(pid) =>
					Effect.sync(() => {
	...
	it.effect(
		"keeps foreign-host and malformed targets without probing a PID",
		() =>
			Effect.gen(function* () {
				let probes = 0;
				...
				assert.isTrue(
					yield* chromiumSingletonLockIsHeld(
						"another-host-4321",
						"current-host",
						probe,
					),
	...
	it.effect("ignores a user-data directory that holds no cookie database", () =>
		run(
			Effect.gen(function* () {
				...
				yield* fileSystem.makeDirectory(`${root}/NativeMessagingHosts`, {
					recursive: true,
				});
				...
				yield* fileSystem.makeDirectory(`${root}/Profile 1`, {
					recursive: true,
				});
	...
	it.effect("detects a Chromium 127+ install with cookies under Network/", () =>
		run(
			Effect.gen(function* () {
				...
				yield* fileSystem.makeDirectory(`${root}/Default/Network`, {
					recursive: true,
				});
				yield* fileSystem.writeFileString(
...
describe("listSourceProfiles", () => {
	it.effect("ignores a profile whose Cookies entry is not a file", () =>
		run(
			Effect.gen(function* () {
				...
				yield* fileSystem.makeDirectory(`${root}/Broken/Cookies`, {
					recursive: true,
				});
	...
	it.effect(
		"discovers profiles by their cookie database when Local State is absent",
		() =>
			run(
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
...
`,
				...
				yield* fileSystem.makeDirectory(
					`${root}/Profiles/abcd.default-release`,
					{
						recursive: true,
					},
				);
				...
				yield* fileSystem.makeDirectory(`${root}/Profiles/wxyz.empty`, {
					recursive: true,
				});

				assert.deepEqual(yield* listSourceProfiles(firefox, context), [
					{
						directory: context.path.join("Profiles", "abcd.default-release"),
						name: "original",
					},
				]);
			}),
	...
	it.effect(
		"drops empty profiles when falling back to the Profiles/ scan",
		() =>
			run(
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
					const context = yield* withSourceHome();
					const root = firefox.userDataDirectory(context)!;
					yield* fileSystem.makeDirectory(`${root}/Profiles/filled.default`, {
						recursive: true,
					});
					yield* fileSystem.writeFileString(
						`${root}/Profiles/filled.default/cookies.sqlite`,
						"db",
					);
					yield* fileSystem.makeDirectory(`${root}/Profiles/empty.default`, {
						recursive: true,
					});
	...
	it.effect(
		"discovers profiles with cookies under Network/ (Chromium 127+)",
		() =>
			run(
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
	...
	it.effect(
		"falls through to the legacy database when Network/Cookies is a directory",
		() =>
			run(
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
					...
					yield* fileSystem.makeDirectory(`${root}/Default/Network/Cookies`, {
						recursive: true,
					});
...
describe("cookieDatabaseCandidatePaths", () => {
	it.effect(
		"prefers Network/Cookies and falls back to the legacy Cookies",
		() =>
			run(
				Effect.gen(function* () {
					const context = yield* withSourceHome();
					...
					assert.deepEqual(
						cookieDatabaseCandidatePaths(helium, context, "Profile 1"),
						[
...
const firefox = BROWSER_IMPORT_SOURCES.find(
...
describe("Firefox Snap profiles", () => {
	...
	it.effect(
		"keeps matching profile names in native and Snap installs distinct",
		() =>
			run(
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
					...
					for (const root of [native, snap]) {
						yield* fileSystem.makeDirectory(`${root}/abcd.default`, {
							recursive: true,
						});
						yield* writeFirefoxCookieDatabase(
							`${root}/abcd.default/cookies.sqlite`,
							1,
							0,
						);
						yield* fileSystem.writeFileString(
...
describe("listSourceProfiles Firefox fallback", () => {
	const cases = [
		{ platform: "linux" as const, profileDirectory: "linux.default" },
		{
			...
			profileDirectory: NodePath.join("Profiles", "macos.default"),
		},
		{
			platform: "win32" as const,
			profileDirectory: NodePath.join("Profiles", "windows.default"),
		},
	];
			...
			() =>
				run(
					Effect.gen(function* () {
						...
						const root = firefox.userDataDirectory(context)!;
						const scanRoot =
							platform === "linux" ? root : path.join(root, "Profiles");
						yield* fileSystem.makeDirectory(path.join(root, profileDirectory), {
							recursive: true,
						});
						yield* fileSystem.writeFileString(
						...
						yield* fileSystem.makeDirectory(
							path.join(scanRoot, "stale.default"),
							{
								recursive: true,
							},
						);
						yield* fileSystem.writeFileString(
							path.join(scanRoot, "stale-file.default"),
							"not-dir",
						);
	...
	it.effect(
		"scans for profiles when profiles.ini declares only ones without cookies",
		() =>
			run(
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
					...
					yield* fileSystem.makeDirectory(
						path.join(root, "Profiles", "stale.default"),
						{
							recursive: true,
						},
					);
					...
					yield* writeFirefoxCookieDatabase(
						path.join(realDirectory, "cookies.sqlite"),
						3,
						0,
					);
					yield* fileSystem.writeFileString(
	...
	it.effect(
		"counts only importable cookies for declared and fallback profiles",
		() =>
			run(
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
					...
					const root = firefox.userDataDirectory(context)!;
					const declaredDirectory = path.join(
						root,
						"Profiles",
						"declared.default",
					);
					yield* fileSystem.makeDirectory(declaredDirectory, {
						recursive: true,
					});
					yield* writeFirefoxCookieDatabase(
						path.join(declaredDirectory, "cookies.sqlite"),
						2,
						3,
					);
					yield* fileSystem.writeFileString(
						...
						[
							...
							"Name=Declared",
							"IsRelative=1",
							"Path=Profiles/declared.default",
						].join("\n"),
					);
					...
					yield* fileSystem.makeDirectory(fallbackDirectory, {
						recursive: true,
					});
					yield* writeFirefoxCookieDatabase(
						path.join(fallbackDirectory, "cookies.sqlite"),
						1,
						4,
					);
...
describe("isSourceRunning for Firefox", () => {
	...
	it.effect(
		"reports not-held when no interpreter can run the fcntl probe",
		() =>
			run(
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
					...
					// good.
					assert.isFalse(
						yield* posixLockIsHeld(lock, ["/nonexistent/python3"]),
					);
	...
	it.effect(
		"reads a Firefox lock symlink's pid to tell live from crashed",
		() =>
			Effect.gen(function* () {
				...
				assert.isTrue(
					yield* firefoxSymlinkLockIsHeld("127.0.0.1:4242", local, alive),
				);
				assert.isTrue(
					yield* firefoxSymlinkLockIsHeld("127.0.1.1:+4242", local, alive),
				);
				assert.isTrue(
					yield* firefoxSymlinkLockIsHeld("192.168.1.20:+4242", local, alive),
				);
				// A crash leaves the symlink behind with a dead pid, on any local address.
				assert.isFalse(
					yield* firefoxSymlinkLockIsHeld("127.0.0.1:+9999", local, alive),
				);
				assert.isFalse(
					yield* firefoxSymlinkLockIsHeld("192.168.1.20:+9999", local, alive),
				);
				...
				assert.isTrue(
					yield* firefoxSymlinkLockIsHeld("10.0.0.7:+9999", local, alive),
				);
			}),
	);

	it.effect(
		"does not treat a stale parent.lock file as a running browser",
		() =>
			run(
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
...
describe("listSourceProfiles hardening", () => {
	it.effect(
		"drops profile directories that are not a single plain segment",
		() =>
			run(
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
...
describe("Safari profiles", () => {
	const safari = BROWSER_IMPORT_SOURCES.find(
	...
	const fixture = Effect.fnUntraced(function* () {
		...
		yield* fileSystem.makeDirectory(context.path.join(library, "Safari"), {
			recursive: true,
		});
	...
	it.effect(
		"discovers named profiles and resolves only the selected profile's cookies",
		() =>
			run(
				Effect.gen(function* () {
            ...
            deleted INTEGER DEFAULT 0, order_index INTEGER DEFAULT 0
					...
					const selected = yield* resolveCookieDatabase(
						safari,
						context,
					...
					assert.strictEqual(
						selected,
						context.path.join(store(workUuid), "Cookies.binarycookies"),
					);
					const fileSystem = yield* FileSystem.FileSystem;
					assert.strictEqual(
						yield* fileSystem.readFileString(selected!),
						workUuid,
					);
					yield* fileSystem.remove(selected!);
					assert.isUndefined(
						yield* resolveCookieDatabase(
							safari,
							context,
							profiles[1]!.directory,
						),
					...
					assert.deepEqual(
						yield* listSourceProfiles(safari, context),
						profiles,
					);
				}),
			),
	...
	for (const metadataState of ["missing", "corrupt"] as const) {
		it.effect(
			`recovers separate cookie stores when metadata is ${metadataState}`,
			() =>
				run(
					Effect.gen(function* () {
						...
						if (metadataState === "corrupt")
							yield* fileSystem.writeFileString(metadata, "invalid");
						yield* fileSystem.remove(
							context.path.join(store(otherUuid), "Cookies.binarycookies"),
						);
