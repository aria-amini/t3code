...
export const BROWSER_IMPORT_SOURCES: ReadonlyArray<BrowserImportSourceDefinition> =
	[
		...
		},
		{
			...
			userDataDirectory: (context) => {
				if (context.platform === "darwin")
					return macApplicationSupport(context, "Firefox");
				if (context.platform === "win32") {
...
const decodeLocalState = Schema.decodeUnknownEffect(
	Schema.fromJsonString(LocalState),
);
...
const countProfileCookies = Effect.fnUntraced(function* (
	...
	return yield* Effect.gen(function* () {
		...
		Effect.provide(
			NodeSqliteClient.layer({ filename: database, readonly: true }),
		),
...
const listSafariProfiles = Effect.fnUntraced(function* (
    ...
    `,
		...
		Effect.provide(
			NodeSqliteClient.layer({ filename: metadata, readonly: true }),
		),
		Effect.orElseSucceed(() => []),
	);
	const defaultProfile = declared.find(
		(profile) => profile.external_uuid === "DefaultProfile",
	);
	const profiles: Array<BrowserImportSourceProfile> = [
		{
			directory: ".",
			name: defaultProfile
				? defaultProfile.title?.trim() || "Personal"
				: "Safari",
		},
	];
	...
	if (declared.length === 0) {
		...
		for (const entry of entries.filter(isSafariProfileUuid).sort()) {
			...
			if (
				yield* databaseFileExists(
					context.path.join(directory, "Cookies.binarycookies"),
				)
			) {
...
> {
	...
	if (definition.engine === "firefox") {
		const declared = yield* fileSystem
			.readFileString(context.path.join(root, "profiles.ini"))
			.pipe(
				Effect.map((ini) => parseFirefoxProfiles(ini, context.path, root)),
				Effect.orElseSucceed(
					() => [] as ReadonlyArray<BrowserImportSourceProfile>,
				),
			);
		...
		if (declared.length > 0) {
			const found = yield* Effect.forEach(declared, (profile) =>
				Effect.forEach(
					...
					Effect.map((results) =>
						results.some(Boolean) ? profile : undefined,
					),
				),
			);
		...
		const found = yield* Effect.forEach(scanned, (entry) => {
			const directory =
				context.platform === "linux"
					? entry
					: context.path.join("Profiles", entry);
			return resolveCookieDatabase(definition, context, directory).pipe(
				Effect.map((database) =>
					database === undefined ? undefined : { directory, name: entry },
				),
			);
		});
	...
	const declared = yield* fileSystem
		.readFileString(context.path.join(root, "Local State"))
		.pipe(
			...
			// directory.
			Effect.map((entries) =>
				entries.filter(([directory]) => isSafeProfileDirectory(directory)),
			),
			Effect.map((entries) =>
				entries.map(([directory, info]) => ({
					directory,
	...
	if (declared.length > 0)
		return yield* withCookieCounts(definition, context, declared);
	...
	const found = yield* Effect.forEach(
		entries.filter(isSafeProfileDirectory),
		(directory) =>
 ...
 */
export const listSourceProfiles = Effect.fn(
	"BrowserImportSources.listSourceProfiles",
)(function* (
	definition: BrowserImportSourceDefinition,
	context: BrowserImportPathContext,
): Effect.fn.Return<
...
export const chromiumProcessIsAlive = (
	pid: number,
	signalProcess: (pid: number, signal: 0) => unknown = process.kill.bind(
		process,
	),
) =>
	Effect.sync(() => {
...
const processIsAlive: ProcessLivenessProbe = (pid) =>
	chromiumProcessIsAlive(pid);
...
export const isWindowsLockHeldError = (
	error: PlatformError.PlatformError,
): boolean => error.reason._tag === "Busy";
...
const firefoxProfileIsHeld = Effect.fnUntraced(function* (
	...
	const fileSystem = yield* FileSystem.FileSystem;
	if (context.platform === "win32") {
		return yield* windowsLockIsHeld(
			context.path.join(directory, "parent.lock"),
		);
	}
	// Linux additionally writes the `lock` symlink; a live pid there settles it
	// without spawning anything.
	const symlinkHeld = yield* fileSystem
		.readLink(context.path.join(directory, "lock"))
		.pipe(
			Effect.flatMap((target) =>
				firefoxSymlinkLockIsHeld(target, localAddresses, processIsAlive),
			),
			Effect.orElseSucceed(() => false),
		);
...
export const isSourceRunning = Effect.fn(
	"BrowserImportSources.isSourceRunning",
)(function* (
	definition: BrowserImportSourceDefinition,
	context: BrowserImportPathContext,
): Effect.fn.Return<
...
> {
	...
	if (definition.engine !== "firefox") {
		...
		return yield* fileSystem.readLink(lock).pipe(
			Effect.flatMap((target) =>
				chromiumSingletonLockIsHeld(target, currentHost, processIsAlive),
			),
	...
	const localAddresses: ReadonlySet<string> =
		context.platform === "win32"
			? new Set()
			: yield* yield* HostProcessAddresses;
 ...
 */
export const isSourceInstalled = Effect.fn(
	"BrowserImportSources.isSourceInstalled",
)(function* (
	definition: BrowserImportSourceDefinition,
	context: BrowserImportPathContext,
