...
describe("BrowserSession", () => {
	...
	it.effect(
		"keeps scopes that differ only by a lone surrogate in separate partitions",
		() =>
			Effect.gen(function* () {
				const browserSessions = yield* BrowserSession.BrowserSession;
	...
	it.effect(
		"keeps legacy defaults disjoint from nondefault profile partitions",
		() =>
			Effect.gen(function* () {
				const browserSessions = yield* BrowserSession.BrowserSession;
				...
				const nondefaultProfile = yield* browserSessions.getPartition(
					"a::b",
					true,
					"profile",
				);
				...
				assert.strictEqual(
					legacyDefault,
					"persist:t3code-preview-78f0be89237d77f7a70e",
				);
				assert.strictEqual(
					nondefaultProfile,
					"persist:t3code-preview-profile-78f0be89237d77f7a70e",
				);
	...
	it.effect(
		"grants clipboard-sanitized-write through both the request and check handlers",
		() =>
			Effect.gen(function* () {
				const browserSessions = yield* BrowserSession.BrowserSession;
				const partition = yield* browserSessions.getPartition("scope-a");
	...
	it.effect("preserves partition scope and the platform failure chain", () => {
		...
		return Effect.gen(function* () {
			const browserSessions = yield* BrowserSession.BrowserSession;
			const error = yield* browserSessions
				.getPartition("environment-a")
				.pipe(Effect.flip);

			assert.instanceOf(
				error,
				BrowserSession.BrowserSessionPartitionDerivationError,
			);
			...
			Effect.provide(
				BrowserSession.layer.pipe(Layer.provide(failingCryptoLayer)),
			),
		);
	});

	it.effect(
		"preserves session scope, partition, and the Electron failure",
		() =>
			Effect.gen(function* () {
	...
	it.effect("clears storage and cache for every created session", () =>
		Effect.gen(function* () {
			...
			for (const browserSession of sessions.values()) {
				assert.strictEqual(
					browserSession.clearStorageData.mock.calls.length,
					1,
				);
	...
	it.effect(
		"correlates clear failures while still attempting every session",
		() =>
			Effect.gen(function* () {
				const browserSessions = yield* BrowserSession.BrowserSession;
				yield* browserSessions.getSession("scope-a");
				...
				secondSession.clearStorageData.mockImplementationOnce(() =>
					Promise.reject(storageCause),
				);
				const storageError = yield* browserSessions
					.clearCookies()
					.pipe(Effect.flip);

				assert.instanceOf(
					storageError,
					BrowserSession.BrowserSessionStorageClearError,
				);
				...
				firstSession.clearCache.mockImplementationOnce(() =>
					Promise.reject(cacheCause),
				);
				const cacheError = yield* browserSessions
					.clearCache()
					.pipe(Effect.flip);

				assert.instanceOf(
					cacheError,
					BrowserSession.BrowserSessionCacheClearError,
				);
