...
export type BrowserSessionGetSessionError =
	typeof BrowserSessionGetSessionError.Type;
...
export const make = Effect.gen(function* BrowserSessionMake() {
	...
	const sessionsRef = yield* SynchronizedRef.make<ReadonlyMap<string, Session>>(
		new Map(),
	);
	...
	const getPartition = Effect.fn("BrowserSession.getPartition")(function* (
		...
		const digest = yield* crypto
			.digest("SHA-256", encodeScopeForDigest(scope))
			.pipe(
				Effect.mapError(
					(cause) =>
		...
		const prefix = persistent
			...
			: PREVIEW_EPHEMERAL_PARTITION_PREFIX;
	...
	return BrowserSession.of({
		...
		getSession,
		clearCookies: Effect.fn("BrowserSession.clearCookies")(
			function* (partitions?) {
				const sessions = yield* SynchronizedRef.get(sessionsRef);
				yield* Effect.all(
					selectSessions(sessions, partitions).map(
