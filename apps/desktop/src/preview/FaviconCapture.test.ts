...
function sourceJpeg(
	...
	const app1Segments = (
		typeof orientations === "number" ? [orientations] : orientations
...
function sourceJpegExifSegment(
	...
	const writeUInt16 = (value: number, offset: number) =>
		littleEndian
			? exif.writeUInt16LE(value, offset)
			: exif.writeUInt16BE(value, offset);
	const writeUInt32 = (value: number, offset: number) =>
		littleEndian
			? exif.writeUInt32LE(value, offset)
			: exif.writeUInt32BE(value, offset);
...
function sourceJpegWithOrientationEntries(
	...
	return sourceJpegWithApp1Segments(width, height, [
		sourceJpegExifSegment(orientations),
	]);
}

function sourceJpegWithEndianAlias(
	alias: number,
	byteOrder: "II" | "MM",
): Buffer {
...
function sourceJpegExifWithOverlappingSubIfds(
	pointerCount: number,
	...
	const rootEntries = pointerCount + 1;
	const subIfdOffset = rootIfdOffset + 2 + rootEntries * 12 + 4;
	const exif = Buffer.alloc(
		subIfdOffset + pointerCount * 2 + 2 + subIfdEntries * 12,
	);
	exif.write("Exif\0\0", 0, "binary");
	exif.write("II", 6, "ascii");
	exif.writeUInt32LE(8, 10);
...
describe("captureFavicon", () => {
	it.each([
		...
		"uses the explicit credential policy for $label requests",
		async (testCase) => {
	...
	it("decodes base64 and percent-encoded inline images without fetching", async () => {
		const { webContents, fetch, executeJavaScriptInIsolatedWorld } =
			makeWebContents();
		...
		for (const candidate of [
			SOURCE_PNG_URL,
			`data:image/png,${percentEncodedPng}`,
		]) {
			expect(
				await captureFavicon({
					webContents,
	...
	it("ends candidate fallback when the overall capture deadline expires", async () => {
		...
		try {
			...
			timeoutController.abort(
				new DOMException("capture timed out", "TimeoutError"),
			);
	...
	it("does not publish a rasterization that completes after the capture deadline", async () => {
		...
		const timeout = vi
			...
			.mockImplementation((milliseconds) =>
				milliseconds === 5_000
					? captureTimeoutController.signal
					: rasterTimeoutController.signal,
			);
		...
		try {
			...
			captureTimeoutController.abort(
				new DOMException("capture timed out", "TimeoutError"),
			);
	...
	it.each([
		...
		"preserves $label aspect ratio within the 32x32 output",
		async (testCase) => {
			const { webContents } = makeWebContents({
				rasterize: async (code) => {
			...
			expect(
				await captureFavicon({
					...
					pageUrl: "https://example.com/page",
					candidates: [
						`data:image/png;base64,${testCase.source.toString("base64")}`,
					],
					signal: new AbortController().signal,
				}),
			).toEqual({ kind: "captured", dataUrl: PNG });
	...
	it.each([
		...
		{
			label:
				"does not consult a later EXIF segment after an invalid orientation",
			layout: JPEG_LANDSCAPE_LAYOUT,
		...
		},
		{
			...
			layout: JPEG_PORTRAIT_LAYOUT,
			source: sourceJpegWithApp1Segments(64, 32, [
				sourceJpegExifSegment([6], { padding: 0xff }),
			]),
		},
		{
			...
			layout: JPEG_PORTRAIT_LAYOUT,
			source: sourceJpegWithApp1Segments(64, 32, [
				sourceJpegExifSegment([6], { byteOrder: "MM" }),
			]),
		},
		{
			...
			layout: JPEG_PORTRAIT_LAYOUT,
			source: sourceJpegWithApp1Segments(64, 32, [
				sourceJpegExifSegment([6], { magic: 0 }),
			]),
		},
		{
	...
	it("rejects JPEGs with multiple frame headers before rasterization", async () => {
		const buffer = Buffer.concat([
			sourceJpeg(4096, 4096),
			sourceJpeg(1, 1).subarray(2),
		]);
		const { webContents, executeJavaScriptInIsolatedWorld } = makeWebContents();
	...
	it.each([
		...
		[
			"GIF later frame",
			"image/gif",
			sourceGif(1, 1, 1, 1, [{ width: 4096, height: 4096 }]),
		],
		[
		...
		"rejects unsafe or unsupported %s before rasterization",
		async (_label, mime, buffer) => {
			const { webContents, executeJavaScriptInIsolatedWorld } =
				makeWebContents();
