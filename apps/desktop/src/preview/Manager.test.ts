...
describe("fitPictureInPictureContentSize", () => {
	it("preserves the PiP content area across aspect-ratio changes", () => {
		expect(
			PreviewManager.fitPictureInPictureContentSize([480, 320], 16 / 9),
		).toEqual([523, 294]);
		expect(
			PreviewManager.fitPictureInPictureContentSize([480, 320], 9 / 16),
		).toEqual([294, 523]);
	});

	it("does not collapse toward the minimum size when orientation changes repeatedly", () => {
		const portrait = PreviewManager.fitPictureInPictureContentSize(
			[523, 294],
			9 / 16,
		);
		const landscape = PreviewManager.fitPictureInPictureContentSize(
			portrait,
			16 / 9,
		);
...
describe("recordingFileExtension", () => {
	it("derives the artifact extension from the recorder's actual mime type", () => {
		expect(
			PreviewManager.recordingFileExtension("video/mp4;codecs=avc1.640028"),
		).toBe("mp4");
		expect(PreviewManager.recordingFileExtension("video/webm;codecs=vp9")).toBe(
			"webm",
		);
		expect(PreviewManager.recordingFileExtension("video/x-matroska")).toBe(
			"matroska",
		);
	});
});
...
describe("isPreviewRefreshShortcut", () => {
	...
	it("recognizes the platform refresh chord without matching modified variants", () => {
		...
		expect(
			PreviewManager.isPreviewRefreshShortcut(
				input({ meta: false, control: true }),
			),
		).toBe(true);
		expect(
			PreviewManager.isPreviewRefreshShortcut(input({ shift: true })),
		).toBe(false);
		expect(
			PreviewManager.isPreviewRefreshShortcut(input({ type: "keyUp" })),
		).toBe(false);
	});
});
...
describe("isPreviewEditingShortcut", () => {
	...
	it.each(["darwin", "linux", "win32"] as const)(
		...
		(platform) => {
			...
			const redo =
				platform === "win32"
					? input(platform, "y")
					: input(platform, "z", { shift: true });
			expect(PreviewManager.isPreviewEditingShortcut(redo, platform)).toBe(
				true,
			);
			expect(
				PreviewManager.isPreviewEditingShortcut(
	...
	it("recognizes macOS Paste and Match Style when Option changes the key to a symbol", () => {
		const pasteAndMatchStyle = input("darwin", "◊", {
			code: "KeyV",
			alt: true,
			shift: true,
		});
		expect(
			PreviewManager.isPreviewEditingShortcut(pasteAndMatchStyle, "darwin"),
		).toBe(true);
		for (const modifiers of [
			{ code: "KeyC" },
			{ alt: false },
...
describe("previewWindowOpenAction", () => {
	...
	it("opens a real window for scripted popups so the opener survives", () => {
		...
		expect(
			PreviewManager.previewWindowOpenAction(
				details({ url: "http://localhost:5173/auth" }),
			),
		).toBe("popup");
	});
	...
	it("does not hand a window to schemes that cannot be hardened", () => {
		...
		for (const url of [
			...
			expect(PreviewManager.previewWindowOpenAction(details({ url }))).toBe(
				"navigate",
			);
		}
	});
...
const {
	...
	createFromPath: vi.fn(
		(): { readonly isEmpty: () => boolean; readonly toPNG: () => Buffer } => ({
			isEmpty: () => false,
			toPNG: () => Buffer.from("png"),
		}),
	),
	fromId: vi.fn<(_id?: number) => Electron.WebContents | null>(
		(_id?: number) => null,
	),
...
const encodePreviewManagerError = Schema.encodeSync(
...
type TestPreviewWebContents = Electron.WebContents & {
	readonly setBackgroundThrottling: ReturnType<
...
const makeFaviconWebContents = (options?: {
	...
	const executeJavaScriptInIsolatedWorld = vi.fn(
		async (
			...
			options?.rasterize
				? options.rasterize(scripts[0]?.code ?? "")
				: TEST_FAVICON,
	);
	const reload = vi.fn();
...
const makeTestPictureInPictureWindow = (
	loadURL: () => Promise<void> = async () => undefined,
) => {
...
describe("PreviewManager", () => {
	...
	effectIt.effect(
		"preserves focused browser editing in tabs and sign-in popups",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const preview = makeFaviconWebContents();
					...
					for (const browser of [preview, popup]) {
						...
						expect(contents.setIgnoreMenuShortcuts).toHaveBeenLastCalledWith(
							false,
						);
	...
	effectIt.effect(
		"reports an unregistered webview as temporarily unavailable",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
		...
		withManager((manager) =>
			Effect.gen(function* () {
				...
				const exit = yield* Effect.exit(
					manager.registerWebview("tab_destroyed_registration", 42),
				);
	...
	effectIt.effect(
		"isolates failed state listeners and continues delivery",
		() => {
			...
			return withManager((manager) =>
				Effect.gen(function* () {
					...
					expect(loggedErrors[0]).toBeInstanceOf(
						ElectronWindow.ElectronWindowOperationError,
					);
				...
				const exit = yield* Effect.scoped(
					Effect.gen(function* () {
						...
						return yield* Effect.exit(
							manager.createTab("tab_interrupted_listener"),
						);
					}),
				);
	...
	effectIt.effect(
		"detaches through the pinned debugger after the webview is destroyed",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					...
					expect(debuggerOff).toHaveBeenCalledWith(
						"message",
						expect.any(Function),
					);
	...
	effectIt.effect(
		"does not let destroyed-webview cleanup detach a same-id replacement",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					...
					yield* manager.setColorScheme(
						"tab_destroyed_replacement_race",
						"dark",
					);
					const replacementRegistration = yield* Effect.promise(
						() => replacementReady,
					).pipe(
						Effect.flatMap(() =>
							manager.registerWebview("tab_destroyed_replacement_race", 42),
						),
	...
	effectIt.effect(
		"publishes a canonical favicon origin while the page is loading",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const preview = makeFaviconWebContents({
					...
					expect(states.at(-1)?.favicon?.capturedAt).toEqual(
						expect.any(Number),
					);
				}),
			),
	...
	effectIt.effect(
		"shares an identical in-flight event and lets a changed event win",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					...
					faviconUpdated(
						{} as never,
						["http://localhost:3200/first.png"] as never,
					);
					faviconUpdated(
						{} as never,
						["http://localhost:3200/first.png"] as never,
					...
					faviconUpdated(
						{} as never,
						["http://localhost:3200/second.png"] as never,
					);
					yield* settle(() => states.at(-1)?.favicon !== undefined);
					resolveFirst(
						new Response(new Uint8Array(makeSourcePng()), {
					...
					expect(
						states.filter((state) => state.favicon !== undefined),
					).toHaveLength(1);
				}),
			),
	);
	...
	effectIt.effect(
		"allows an identical retry after an undecodable capture",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					...
					const preview = makeFaviconWebContents({
						rasterize: async () =>
							++rasterizations === 1 ? null : TEST_FAVICON,
					});
					fromId.mockReturnValue(preview.webContents);
					const states: PreviewManager.PreviewTabState[] = [];
					yield* manager.subscribeStateChanges((_tabId, state) =>
	...
	effectIt.effect(
		"retains a favicon when reloading the current URL without a new event",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const preview = makeFaviconWebContents();
	...
	effectIt.effect(
		"clears a published favicon after a confirmed cross-origin navigation",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const preview = makeFaviconWebContents();
	...
	effectIt.effect(
		"does not resurrect an icon after a confirmed about:blank document",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const preview = makeFaviconWebContents();
	...
	effectIt.effect(
		"clears a published favicon when a replacement webview attaches",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
	...
	effectIt.effect(
		"ignores an old capture that completes after webview replacement",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					let resolveFetch!: (response: Response) => void;
					...
					expect(
						states.some(
							(state) =>
								state.webContentsId === 43 && state.favicon !== undefined,
						),
					).toBe(false);
				}),
			),
	...
	effectIt.effect(
		"preserves a favicon when the active attachment registers again",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const preview = makeFaviconWebContents();
	...
	effectIt.effect(
		"keeps the tab's own zoom instead of the guest's reported zoom",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
	...
	effectIt.effect(
		"re-applies each tab's own zoom when the app window zooms",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const setZoomFactor = vi.fn();
	...
	effectIt.effect(
		"re-asserts the tab's zoom when the active guest registers again",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const setZoomFactor = vi.fn();
	...
	effectIt.effect(
		"emulates prefers-color-scheme and re-applies it across webview swaps",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const makeWebContents = (id: number) => {
	...
	effectIt.effect(
		"mutes the guest and re-applies the mute across webview swaps",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
		...
		withManager((manager) =>
			Effect.gen(function* () {
				...
				const exit = yield* manager
					.setAudioMuted("tab_audio_fail", true)
					.pipe(Effect.exit);
	...
	effectIt.effect(
		"still registers a guest that refuses the mute reassert",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const first = makeAudioWebContents(42);
					...
					const exit = yield* manager
						.registerWebview("tab_audio_attach_fail", 43)
						.pipe(Effect.exit);
	...
	effectIt.effect(
		"reconciles audibility that changed while the guest attached",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const guest = makeAudioWebContents(42);
	...
	effectIt.effect(
		"blocks late webview and capture starts during tab close",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const capturePage = vi.fn(async () => ({
					...
					const replacementListenerSpies =
						replacementWebContents as unknown as {
					...
					const recordingExit = yield* Effect.exit(
						manager.startRecording("tab_close_register_race"),
					);
	...
	effectIt.effect(
		"keeps a main-frame load failure visible until a retry starts",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
	...
	effectIt.effect("captures a PNG screenshot into browser artifacts", () =>
		withManager((manager) =>
			Effect.gen(function* () {
				...
				capturePage.mockRejectedValueOnce(new Error("UnknownVizError"));
				const retriedFiber = yield* Effect.exit(
					manager.captureScreenshot("tab_1"),
				).pipe(Effect.forkChild({ startImmediately: true }));
				...
				const failingFiber = yield* Effect.exit(
					manager.captureScreenshot("tab_1"),
				).pipe(Effect.forkChild({ startImmediately: true }));
				yield* TestClock.adjust(1_000);
	...
	effectIt.effect(
		"keeps every recorded guest unthrottled until its frame capture stops",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					...
					const host = makeTestHostWebContents();
					const firstWebContents = makeTestPreviewWebContents(
						capturePage,
						41,
						host,
					);
					const secondWebContents = makeTestPreviewWebContents(
						capturePage,
						42,
						host,
					);
					...
					expect(firstWebContents.setBackgroundThrottling.mock.calls).toEqual([
						[false],
					]);
					expect(secondWebContents.setBackgroundThrottling.mock.calls).toEqual([
						[false],
					]);
	...
	effectIt.effect(
		"does not commit failed starts and retries throttle restoration",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const setBackgroundThrottling = vi.fn<(enabled: boolean) => void>();
	...
	effectIt.effect(
		"rolls back window throttling when a recorded guest cannot be unthrottled",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					...
					yield* manager.setMainWindow({
						...
						once: vi.fn(),
						webContents: {
							setBackgroundThrottling: setWindowBackgroundThrottling,
						},
					} as never);
					...
					expect(wc.setBackgroundThrottling.mock.calls).toEqual([
						[false],
						[false],
						[true],
					]);
	...
	effectIt.effect(
		"does not publish a replacement window when capture reconciliation fails",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const setBackgroundThrottling = vi.fn(() => {
	...
	effectIt.effect("ignores close events from replaced main windows", () =>
		withManager((manager) =>
			Effect.gen(function* () {
				...
				expect(replacementWindowThrottling.mock.calls).toEqual([
					[false],
					[true],
				]);
			}),
	...
	effectIt.effect("releases frame capture when the main window closes", () =>
		withManager((manager) =>
			Effect.gen(function* () {
				...
				closeMainWindow?.();
				const racedStart = yield* Effect.exit(
					manager.startRecording("tab_window_close_race"),
				);
				expect(Exit.isFailure(racedStart)).toBe(true);
				if (Exit.isFailure(racedStart)) {
					expect(
						Option.getOrThrow(Cause.findErrorOption(racedStart.cause)),
					).toMatchObject({
	...
	effectIt.effect(
		"does not arm recording after the main window closes during warmup",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					let closeMainWindow: (() => void) | undefined;
					...
					const host = makeTestHostWebContents();
					fromId.mockReturnValue(
						makeTestPreviewWebContents(capturePage, 42, host),
					);
					...
					expect(
						host.session.setDisplayMediaRequestHandler,
	...
	effectIt.effect(
		"stops capture retries when the tab swaps during the retry delay",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const capturePage = vi.fn(async () => ({
	...
	effectIt.effect(
		"discards a screenshot that resolves after its guest is replaced",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const image = {
	...
	effectIt.effect(
		"releases snapshot control when every capture attempt stalls",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const capturePage = vi.fn(
						() => new Promise<TestCapturedPreviewImage>(() => {}),
					);
					const wc = makeTestPreviewWebContents(capturePage);
					...
					const snapshot = yield* Effect.exit(
						manager.automationSnapshot("tab_1"),
					).pipe(Effect.forkChild({ startImmediately: true }));
	...
	effectIt.effect(
		"grants each concurrent preview recording its own tab frame",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					...
					yield* Effect.all(
						[manager.stopRecording("tab_1"), manager.stopRecording("tab_2")],
						{
							concurrency: 2,
							discard: true,
						},
	...
	effectIt.live(
		"settles both starts when two tabs race for the capture stream",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const { host, grants, takeGrant } =
						yield* setupRecordingRaceTabs(manager);
					...
					expect(
						Option.getOrThrow(Cause.findErrorOption(loserExit.cause)),
					).toMatchObject({
					...
					takeGrant();
					expect(grants).toEqual([
						{ video: { routingId: Exit.isSuccess(exitA) ? 41 : 42 } },
					]);
					expect(
						host.session.setDisplayMediaRequestHandler,
	...
	effectIt.effect(
		"releases an armed slot that the renderer never redeemed",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					...
					const blocked = yield* Effect.exit(
						manager.startRecording("tab_race_b"),
					);
					if (Exit.isSuccess(blocked))
						throw new Error("expected the second tab to be refused");
					expect(
						Option.getOrThrow(Cause.findErrorOption(blocked.cause)),
					).toMatchObject({
						_tag: "PreviewRecordingArmConflictError",
						tabId: "tab_race_b",
	...
	effectIt.effect(
		"denies a display-media request that arrives after the arm went stale",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const { grants, takeGrant } = yield* setupRecordingRaceTabs(manager);
	...
	effectIt.effect(
		"only lets the host frame that armed a recording claim its stream",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const { grants, takeGrant } = yield* setupRecordingRaceTabs(manager);
	...
	effectIt.effect("reclaims the arm slot from a destroyed webContents", () =>
		withManager((manager) =>
			Effect.gen(function* () {
				const { grants, takeGrant, destroy } =
					yield* setupRecordingRaceTabs(manager);
	...
	effectIt.effect(
		"continues native recording when the source warmup fails",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const capturePage = vi.fn(async () => {
					...
					const host = makeTestHostWebContents();
					const webContents = Object.assign(
						makeTestPreviewWebContents(capturePage, 42, host),
						{
							executeJavaScript: vi.fn(async () => ({
								width: 1280,
	...
	effectIt.effect(
		"serializes recording source acquisition with webview replacement",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const capturedImage = {
					...
					const replacementWebContents = Object.assign(
						makeTestPreviewWebContents(capturePage, 43),
						{
							on: replacementOn,
						},
					);
					...
					expect(initialWebContents.setBackgroundThrottling.mock.calls).toEqual(
						[[false]],
					);
					expect(
						replacementWebContents.setBackgroundThrottling.mock.calls,
					).toEqual([[false]]);
	...
	effectIt.effect(
		"shares background frame capture between recording and picture-in-picture",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const setBackgroundThrottling = vi.fn();
					const mainWindowWebContents = Object.assign(
					...
					expect(
						pictureInPictureWindow.setVisibleOnAllWorkspaces,
					).toHaveBeenCalledWith(true, {
						visibleOnFullScreen: true,
						skipTransformProcessType: true,
					});
					expect(pictureInPictureWindow.setAspectRatio.mock.calls).toEqual([
						[0],
						[1280 / 720],
					]);
					expect(pictureInPictureWindow.setContentSize).toHaveBeenCalledWith(
						523,
						294,
						false,
					);
					expect(
						pictureInPictureWindow.setAspectRatio.mock.invocationCallOrder[0],
					).toBeLessThan(
						pictureInPictureWindow.setContentSize.mock.invocationCallOrder[0] ??
							0,
					);
					expect(
						pictureInPictureWindow.setContentSize.mock.invocationCallOrder[0],
					).toBeLessThan(
						pictureInPictureWindow.setAspectRatio.mock.invocationCallOrder[1] ??
							0,
					);
					...
					const pictureInPictureFramesBeforeRecording =
						pictureInPictureSend.mock.calls.length;
					...
					yield* TestClock.adjust(100);
					expect(capturePage).toHaveBeenCalledTimes(3);
					expect(pictureInPictureSend).toHaveBeenCalledTimes(
						pictureInPictureFramesBeforeRecording,
					);
					expect(recordingFrames).toHaveLength(0);

					yield* manager.stopRecording("tab_pip");
					expect(setBackgroundThrottling.mock.calls).toEqual([[false]]);
					const framesBeforePictureInPictureOnlyTick =
						pictureInPictureSend.mock.calls.length;
					yield* TestClock.adjust(100);
					expect(capturePage).toHaveBeenCalledTimes(4);
					expect(pictureInPictureSend.mock.calls.length).toBe(
	...
	effectIt.effect(
		"keeps picture-in-picture capture separate from recording warmup",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const jpeg = Buffer.from("shared-preview-frame");
					...
					fromId.mockReturnValue(makeTestPreviewWebContents(capturePage));
					const { pictureInPictureWindow, send } =
	...
	effectIt.effect(
		"stops frame capture when the native picture-in-picture window closes",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const capturePage = vi.fn(async () => ({
	...
	effectIt.effect(
		"retries an unchanged picture-in-picture frame after delivery fails",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
	...
	effectIt.effect(
		"replays an unchanged picture-in-picture frame after its renderer reloads",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
	...
	effectIt.effect(
		"does not publish picture-in-picture readiness after window teardown",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const capturePage = vi.fn(async () => ({
					...
					const openExit = yield* Effect.exit(
						manager.openPictureInPicture("tab_pip_teardown"),
					);
	...
	effectIt.effect(
		"closes an initializing picture-in-picture without blocking later opens",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const capturePage = vi.fn(async () => ({
					...
					fromId.mockReturnValue(makeTestPreviewWebContents(capturePage));
					const { pictureInPictureWindow: initializingWindow } =
						makeTestPictureInPictureWindow(
							() =>
								new Promise<void>(() => {
					...
					const { pictureInPictureWindow: reopenedWindow } =
						makeTestPictureInPictureWindow();
					browserWindowConstructor
	...
	effectIt.effect(
		"rejects picture-in-picture when its webview changes during initialization",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					...
					const openExits = yield* Effect.all([
						Fiber.await(open),
						Fiber.await(concurrentOpen),
					]);
					for (const openExit of openExits) {
						...
						const error = Option.getOrThrow(
							Cause.findErrorOption(openExit.cause),
						);
						expect(error).toMatchObject({
							_tag: "PreviewOperationError",
	...
	effectIt.effect(
		"keeps element picking active during subframe navigation",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					const listeners = new Map<string, (...args: unknown[]) => void>();
	...
	effectIt.effect(
		"settles the pick when the annotation screenshot never arrives",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					let onPicked:
					...
					onPicked?.(
						{},
						{
							...
							elements: [],
							regions: [
								{ id: "region_1", rect: { x: 5, y: 6, width: 20, height: 30 } },
							],
							strokes: [],
							styleChanges: [],
							screenshot: null,
	...
	effectIt.effect(
		"a stale capture from a replaced pick never touches the next pick",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					let onPicked:
					...
					const secondPick = yield* manager
						.pickElement("tab_1")
						.pipe(Effect.forkChild);
					yield* Effect.yieldNow;
	...
	effectIt.effect(
		"navigates the guest history when the thumb-button ipc fires",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					let mouseNavigate:
					...
					fromId.mockReturnValue({
						...
						ipc: {
							on: vi.fn((channel: string, listener: typeof mouseNavigate) => {
								if (channel === "preview:mouse-navigate")
									mouseNavigate = listener;
							}),
							off: vi.fn(),
						},
						send: webviewSend,
	...
	effectIt.effect(
		"reveals only files inside the configured browser artifact directory",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					...
					const exit = yield* Effect.exit(
						manager.revealArtifact("/tmp/t3/dev/settings.json"),
					);
					expect(Exit.isFailure(exit)).toBe(true);
					if (Exit.isSuccess(exit)) return;
					const error = Option.getOrThrow(Cause.findErrorOption(exit.cause));
	...
	effectIt.effect("copies screenshot artifacts to the system clipboard", () =>
		withManager((manager) =>
			Effect.gen(function* () {
				...
				const invalidImageExit = yield* Effect.exit(
				...
				expect(
					Option.getOrThrow(Cause.findErrorOption(invalidImageExit.cause)),
				).toMatchObject({
				...
				expect(
					Option.getOrThrow(Cause.findErrorOption(writeExit.cause)),
				).toMatchObject({
					_tag: "PreviewOperationError",
	...
	effectIt.effect(
		"emits the resolved pointer target before dispatching an automation click",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					let humanInput:
						| ((_event: unknown, signal: unknown) => void)
						| undefined;
					...
					const sendCommand = vi.fn(
						async (method: string, params?: Record<string, unknown>) => {
							...
							if (
								...
								humanInput?.(
									{},
									{ kind: "pointer", x: params.x, y: params.y, button: 0 },
								);
							}
							return undefined;
						},
	...
	effectIt.effect(
		"types in background webviews and enables native key input",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
	...
	effectIt.effect(
		"still interrupts agent control for a different human pointer event",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
					let humanInput:
	...
	effectIt.effect(
		"derives evaluation detail kind and length from the same non-empty source",
		() =>
			withManager((manager) =>
				Effect.gen(function* () {
...
describe("Preview automation diagnostics", () => {
	it("keeps browser exception detail out of structural diagnostics", () => {
		...
		const { cause: encodedCause, ...encodedDiagnostics } =
		...
		expect(error.message).toBe(
			"Preview JavaScript evaluation failed in tab tab_1",
		);
		expect(error.message).not.toContain(secret);
		expect(JSON.stringify(encodedDiagnostics)).not.toContain(secret);
		expect("detail" in error).toBe(false);
		expect(
			PreviewManager.PreviewAutomationEvaluationError.toTimelineMessage(error),
		).toBe(detail);
		expect(
			PreviewManager.PreviewAutomationEvaluationError.toTimelineMessage(error),
	...
	it("retains bounded selector diagnostics without exposing selector or reason text", () => {
		...
		expect(
			PreviewManager.PreviewAutomationInvalidSelectorError.toTimelineMessage(
				error,
			),
		).toBe(reason);
	});
