...
export const fitPictureInPictureContentSize = (
	...
	const currentWidth = Math.max(
		1,
		current[0] ?? PICTURE_IN_PICTURE_INITIAL_WIDTH,
	);
	const currentHeight = Math.max(
		1,
		current[1] ?? PICTURE_IN_PICTURE_INITIAL_HEIGHT,
	);
...
export const recordingFileExtension = (mimeType: string): string => {
	const subtype =
		mimeType.split(";", 1)[0]?.trim().toLowerCase().split("/")[1] ?? "";
...
export type PreviewAutomationSelectorKind =
	typeof PreviewAutomationSelectorKind.Type;
...
const previewAutomationEvaluationDetail = (exceptionDetails: unknown) => {
	...
	if (typeof description === "string" && description.length > 0) {
		return {
			detailKind: "exception-description" as const,
			detail: description,
		};
	}
...
const findZoomStep = (current: number): number => {
	...
	return Math.abs(ZOOM_LEVELS[index]! - current) < ZOOM_EPSILON
		? index
		: index - 1;
};
...
export const previewWindowOpenAction = (details: {
	...
	details.disposition === "new-window" && isPopupUrl(details.url)
		? "popup"
		: "navigate";
...
export const isPreviewEditingShortcut = (
	...
	if (isMac ? !input.meta || input.control : !input.control || input.meta)
		return false;
...
const inputSignalsMatch = (
	left: PreviewInputSignal,
	right: PreviewInputSignal,
): boolean => {
...
const makeNativeOperations = Effect.fn("PreviewManager.makeOperations")(
	function* (artifactDirectory: string, pictureInPicturePreloadPath: string) {
		...
		const mainWindowRef = yield* Ref.make<Option.Option<BrowserWindow>>(
			Option.none(),
		);
		const tabsRef = yield* SynchronizedRef.make<
			ReadonlyMap<string, PreviewTabState>
		>(new Map());
		const attachedRef = yield* Ref.make<ReadonlyMap<number, ManagedListeners>>(
			new Map(),
		);
		const listenersRef = yield* Ref.make<ReadonlySet<Listener>>(new Set());
		const pointerEventListenersRef = yield* Ref.make<
			ReadonlySet<PointerEventListener>
		>(new Set());
		const recordingFrameListenersRef = yield* Ref.make<
			ReadonlySet<RecordingFrameListener>
		>(new Set());
		const pickSessionsRef = yield* Ref.make<ReadonlyMap<string, PickSession>>(
			new Map(),
		);
		...
		const diagnosticsRef = yield* Ref.make<
			ReadonlyMap<number, BrowserDiagnostics>
		>(new Map());
		const expectedAgentInputsRef = yield* Ref.make<
			ReadonlyMap<string, ReadonlyArray<ExpectedAgentInput>>
		>(new Map());
		const controlEpochRef = yield* Ref.make<ReadonlyMap<string, number>>(
			new Map(),
		);
		...
		>(new Map());
		const pictureInPictureAspectRatiosRef = yield* Ref.make<
		...
		const capturePageWithRetry = Effect.fn(
			"PreviewManager.capturePageWithRetry",
		)(function* (
			errorContext: PreviewOperationContext,
			tabId: string,
			wc: Electron.WebContents,
		...
		const encodeJson = (
			...
			encodeUnknownJson(value).pipe(
				Effect.mapError(
					(cause) => new PreviewOperationError({ ...errorContext, cause }),
				),
			);
				...
				return lifecycle.semaphore.withPermit(effect).pipe(
					Effect.ensuring(
						Effect.sync(() => {
							...
							if (
								lifecycle.users === 0 &&
								tabLifecycleLocks.get(tabId) === lifecycle
							) {
		...
		const setFrameCaptureBackgroundThrottling = Effect.fnUntraced(function* (
			enabled: boolean,
		) {
		...
		const setFrameCaptureWebContentsBackgroundThrottling = Effect.fnUntraced(
			function* (wc: Electron.WebContents, enabled: boolean) {
		...
		const restoreFrameCaptureWebContentsBackgroundThrottling =
			Effect.fnUntraced(function* (webContentsIds: ReadonlySet<number>) {
				yield* Effect.forEach(
					webContentsIds,
					(webContentsId) => {
						const wc = webContents.fromId(webContentsId);
						if (!wc || wc.isDestroyed()) return Effect.void;
						return setFrameCaptureWebContentsBackgroundThrottling(
							wc,
							true,
						).pipe(
							...
							Effect.catch((error) =>
								Effect.logWarning(
									"Failed to restore preview webview frame capture throttling.",
									{
										webContentsId,
										error,
									},
							...
							() =>
								[
									...
									replaceMap(sessions, (copy) => {
										copy.set(tabId, {
											...current,
											unthrottledWebContentsIds: new Set([
		...
		const stopFrameCapture = Effect.fn("PreviewManager.stopFrameCapture")(
			function* (tabId: string, consumer: FrameCaptureConsumer) {
				yield* SynchronizedRef.modifyEffect(
					frameCaptureSessionsRef,
					(sessions) =>
						Effect.gen(function* () {
									...
									replaceMap(sessions, (copy) => {
										copy.set(tabId, {
											...
											lastPictureInPictureFrame:
												consumer === "picture-in-picture"
													? null
													: current.lastPictureInPictureFrame,
										});
									}),
								] as const;
							}
							...
							if (remainingSessions.size === 0) {
								yield* setFrameCaptureBackgroundThrottling(true).pipe(
									...
									Effect.catch((error) =>
										Effect.logWarning(
											"Failed to restore preview frame capture throttling.",
											{ error },
										),
									),
								);
					...
					Effect.flatMap((captureScope) =>
						captureScope
							? Scope.close(captureScope, Exit.void).pipe(Effect.ignore)
							: Effect.void,
					),
		...
		const stopAllRecordings = Effect.fn("PreviewManager.stopAllRecordings")(
			function* () {
				pendingRecording = null;
				const sessions = yield* SynchronizedRef.get(frameCaptureSessionsRef);
				yield* Effect.forEach(
					sessions.keys(),
					(tabId) => stopFrameCapture(tabId, "recording"),
					{
		...
		const emit = Effect.fn("PreviewManager.emit")(function* (
			tabId: string,
			...
			yield* Effect.forEach(
				listeners,
				(listener) =>
					deliverEvent("state-change", tabId, () => listener(tabId, state)),
				{ discard: true },
			);
		});
		 ...
		 */
		const assertTabZoom = Effect.fn("PreviewManager.assertTabZoom")(function* (
			tabId: string,
		) {
			...
			yield* attempt(
				{ operation: "assertTabZoom", tabId, webContentsId: wc.id },
		 ...
		 */
		const assertTabAudioMuted = Effect.fn("PreviewManager.assertTabAudioMuted")(
			function* (tabId: string) {
				...
				if (!wc || wc.isDestroyed()) return;
				yield* attempt(
					{ operation: "assertTabAudioMuted", tabId, webContentsId: wc.id },
		 ...
		 */
		const syncTabAudible = Effect.fn("PreviewManager.syncTabAudible")(
			function* (tabId: string, wc: Electron.WebContents, audible: boolean) {
				if (wc.isDestroyed()) return;
				const updatedAt = yield* currentIso;
				const next = yield* SynchronizedRef.modify(tabsRef, (tabs) => {
					const current = tabs.get(tabId);
		...
		const requireWebContents = Effect.fn("PreviewManager.requireWebContents")(
			function* (tabId: string) {
				const tabs = yield* SynchronizedRef.get(tabsRef);
		...
		const tabIdForWebContents = Effect.fnUntraced(function* (
			webContentsId: number,
		) {
			const tabs = yield* SynchronizedRef.get(tabsRef);
			return (
				Array.from(tabs.entries()).find(
					([, tab]) => tab.webContentsId === webContentsId,
				)?.[0] ?? null
			);
		});
		...
		const pushBounded = <A>(
			...
			entry: A,
		): ReadonlyArray<A> => [...buffer, entry].slice(-DIAGNOSTIC_BUFFER_LIMIT);
			...
			yield* Ref.update(diagnosticsRef, (allDiagnostics) => {
				...
				const next = (() => {
					...
					if (method === "Runtime.exceptionThrown") {
						const details =
							typeof params["exceptionDetails"] === "object" &&
		...
		const detachControlSession = Effect.fn(
			"PreviewManager.detachControlSession",
		...
		const ensureControlSession = Effect.fn(
			"PreviewManager.ensureControlSession",
		)(function* (wc: Electron.WebContents) {
			return yield* SynchronizedRef.modifyEffect(
				...
				> => {
					...
					const createControlSession = Effect.fn(
						"PreviewManager.createControlSession",
					)(function* () {
							...
							if (method === "Page.screencastFrame") {
								...
								if (typeof sessionId === "number") {
									yield* attemptPromise(
										...
										() =>
											wcDebugger.sendCommand("Page.screencastFrameAck", {
												sessionId,
											}),
									).pipe(Effect.ignore);
								}
								...
								if (tabId && typeof params["data"] === "string") {
									...
									if (captureSession?.consumers.has("recording")) {
										...
										yield* Effect.forEach(
											listeners,
											(listener) =>
												deliverEvent("recording-frame", frame.tabId, () =>
													listener(frame),
												),
											{ discard: true },
										);
						...
						const initialize = Effect.fn(
							"PreviewManager.initializeControlSession",
						)(function* () {
							yield* Ref.update(diagnosticsRef, (diagnostics) =>
								replaceMap(diagnostics, (copy) => {
									copy.set(wc.id, {
							...
							yield* Effect.all(
								[
									...
									attemptPromise(
										{
											operation: `initializeDebugger.${method}`,
											webContentsId: wc.id,
										},
		...
		const prepareAutomationInput = Effect.fn(
			"PreviewManager.prepareAutomationInput",
		)(function* (send: SendCommand, enableRuntime: boolean) {
			yield* Effect.all(
				[
		...
		const withControlSession = Effect.fn("PreviewManager.withControlSession")(
			function* <A>(
				tabId: string,
				wc: Electron.WebContents,
				action: string,
				...
				const execute = Effect.fn("PreviewManager.executeControlAction")(
					function* () {
						...
						const send: SendCommand = Effect.fn("PreviewManager.sendCommand")(
							function* (method, commandParams) {
								const before =
									(yield* Ref.get(controlEpochRef)).get(tabId) ?? 0;
								if (before !== epoch) {
						...
						const sendCleanup: SendCommand = Effect.fn(
							"PreviewManager.sendCleanupCommand",
						)(function* (method, commandParams) {
				...
				const finalize = Effect.fn("PreviewManager.finalizeControlAction")(
					function* (exit: Exit.Exit<A, PreviewManagerError>) {
						...
						if (exit._tag === "Success") {
							...
							const interrupted =
								isPreviewAutomationControlInterruptedError(error);
							const errorMessage = isPreviewOperationError(error)
								...
								: isPreviewAutomationEvaluationError(error)
									...
									: isPreviewAutomationInvalidSelectorError(error)
										? PreviewAutomationInvalidSelectorError.toTimelineMessage(
												error,
											)
				...
				return yield* control.semaphore.withPermit(
					execute().pipe(Effect.onExit(finalize)),
				);
			},
		);
		...
		const evaluateWithDebugger = <A = unknown>(
			...
			send("Runtime.evaluate", {
				...
				Effect.flatMap((rawResponse) => {
					...
					const detail = previewAutomationEvaluationDetail(
		...
		const automationSelectorDiagnostics = (input: {
			...
			if (input.locator !== undefined) {
				return {
					...
					selectorLength: input.locator.length,
				};
			}
		...
		const ensurePlaywrightInjected = Effect.fn(
			"PreviewManager.ensurePlaywrightInjected",
		)(function* (tabId: string, send: SendCommand) {
		...
		const cancelPickElement = Effect.fn("PreviewManager.cancelPickElement")(
			function* (tabId: string) {
		...
		const detachListeners = Effect.fn("PreviewManager.detachListeners")(
			function* (webContentsId: number) {
		...
		const consumeExpectedAgentInput = Effect.fn(
			"PreviewManager.consumeExpectedAgentInput",
		...
		const expectAgentInput = Effect.fn("PreviewManager.expectAgentInput")(
			function* (tabId: string, signal: PreviewInputSignal) {
				const now = yield* currentMillis;
		...
		const attachListeners = Effect.fn("PreviewManager.attachListeners")(
			function* (tabId: string, wc: Electron.WebContents) {
				...
				const syncState = Effect.fn("PreviewManager.syncWebContentsState")(
					function* (
						...
						const next = yield* SynchronizedRef.modify(tabsRef, (tabs) => {
							...
							const clearFavicon =
								...
								safeHttpOrigin(current.favicon.pageUrl) !==
									safeHttpOrigin(
										navStatus.kind === "Idle" ? wc.getURL() : navStatus.url,
									);
				...
				const publishFavicon = Effect.fn("PreviewManager.publishFavicon")(
					function* (input: {
				...
				const faviconUpdated = (
					_event: Event,
					rawCandidates: ReadonlyArray<string>,
				): void => {
				...
				const handleHumanInput = Effect.fn("PreviewManager.handleHumanInput")(
					function* (rawSignal?: unknown) {
						if (
				...
				const mouseNavigate = (_event: unknown, payload?: unknown): void => {
					...
					runFork(
						attempt(
							{ operation: "mouseNavigate", tabId, webContentsId: wc.id },
							() => {
								if (direction === "back") {
				...
				const beforeInput = (
					...
					if (isPreviewRefreshShortcut(input)) {
						...
						runFork(
							attempt(
								{ operation: "shortcut.refresh", tabId, webContentsId: wc.id },
								() => wc.reload(),
							).pipe(Effect.ignore),
						);
						return;
				...
				yield* Scope.addFinalizer(
					scope,
					attempt(
						{ operation: "detachListeners", tabId, webContentsId: wc.id },
						() => {
							cancelFaviconCapture();
				...
				const install = Effect.fn("PreviewManager.installWebContentsListeners")(
					function* () {
						yield* attempt(
							{ operation: "attachListeners", tabId, webContentsId: wc.id },
							() => {
		...
		const createTabUnlocked = Effect.fn("PreviewManager.createTabUnlocked")(
			function* (tabId: string, defaults?: DesktopPreviewTabDefaults) {
				const updatedAt = yield* currentIso;
				...
				if (result.created) {
					tabLifecycleGenerations.set(
						tabId,
						(tabLifecycleGenerations.get(tabId) ?? 0) + 1,
					);
				}
		...
		const createTab = Effect.fn("PreviewManager.createTab")(function* (
			...
			return yield* withTabLifecycleLock(
				tabId,
				createTabUnlocked(tabId, defaults),
			);
		});

		const closeTabUnlocked = Effect.fn("PreviewManager.closeTabUnlocked")(
			function* (tabId: string) {
		...
		const registerWebviewUnlocked = Effect.fn(
			"PreviewManager.registerWebviewUnlocked",
		)(function* (
			tabId: string,
			webContentsId: number,
			...
			if (
				...
				yield* attempt(
					{ operation: "registerWebview.sendTheme", tabId, webContentsId },
			...
			yield* attempt(
				{
					operation: "registerWebview.restoreZoomFactor",
					tabId,
					webContentsId,
				},
			...
			yield* attempt(
				{
					operation: "registerWebview.restoreAudioMuted",
					tabId,
					webContentsId,
				},
				...
				(tabs) =>
					Effect.gen(function* () {
						...
						const next: PreviewTabState = {
							...
							webContentsId,
							navStatus:
								pendingUrl === null ? computeNavStatus(wc) : current.navStatus,
			...
			if (Option.isNone(registration)) {
				yield* Effect.all(
					[detachControlSession(webContentsId), detachListeners(webContentsId)],
					{
						concurrency: 2,
						discard: true,
					},
			...
			const latestNavStatus = (yield* SynchronizedRef.get(tabsRef)).get(
				tabId,
			)?.navStatus;
			if (
				pendingUrl &&
				...
				runFork(
					attemptPromise(
						{
							operation: "registerWebview.loadPendingUrl",
							tabId,
							webContentsId,
						},
		...
		const registerWebview = Effect.fn("PreviewManager.registerWebview")(
		...
		const navigate = Effect.fn("PreviewManager.navigate")(function* (
			...
			const url = yield* attempt(
				{ operation: "navigate.normalizeUrl", tabId },
			...
			const pending = yield* SynchronizedRef.modify(tabsRef, (tabs) => {
				...
				const next: PreviewTabState = {
					...
					navStatus: {
						...
						url,
						title:
							current?.navStatus.kind === "Idle" || !current
								? ""
								: current.navStatus.title,
					},
			...
			if (!wc || wc.isDestroyed()) {
				const expectedAttachment = (yield* Ref.get(attachedRef)).get(
					webContentsId,
				);
				yield* withTabLifecycleLock(
					...
					Effect.gen(function* () {
						const currentTab = (yield* SynchronizedRef.get(tabsRef)).get(tabId);
						const currentAttachment = (yield* Ref.get(attachedRef)).get(
							webContentsId,
						);
			...
			if (wc.getURL() === url) {
				yield* attempt(
					{ operation: "navigate.reload", tabId, webContentsId: wc.id },
					() => wc.reload(),
				);
				return;
			}
		...
		const withWebContents = Effect.fn("PreviewManager.withWebContents")(
			function* (
				operation: string,
				tabId: string,
				...
				yield* attempt({ operation, tabId, webContentsId: wc.id }, () =>
					use(wc),
				);
			},
		);
		...
		const refresh = (tabId: string) =>
			withWebContents("refresh", tabId, (wc) => wc.reload());
		const hardReload = (tabId: string) =>
			withWebContents("hardReload", tabId, (wc) => wc.reloadIgnoringCache());

		const openDevTools = Effect.fn("PreviewManager.openDevTools")(function* (
			tabId: string,
		) {
			const wc = yield* requireWebContents(tabId);
			if (wc.isDevToolsOpened()) {
				yield* attempt(
					{ operation: "openDevTools.focus", tabId, webContentsId: wc.id },
			...
			yield* attempt(
				{ operation: "openDevTools", tabId, webContentsId: wc.id },
				() => {
		...
		const setAnnotationTheme = Effect.fn("PreviewManager.setAnnotationTheme")(
			function* (theme: DesktopPreviewAnnotationTheme) {
		...
		const pickElement = Effect.fn("PreviewManager.pickElement")(function* (
			...
			return yield* Effect.callback<
				PreviewAnnotationSubmissionResult | null,
				PreviewManagerError
			>((resume) => {
				...
				const cleanup = Effect.fn("PreviewManager.cleanupPickElement")(
					function* () {
						yield* attempt(
							{ operation: "pickElement.cleanup", tabId, webContentsId: wc.id },
							() => {
				...
				const finishPick = Effect.fn("PreviewManager.finishPickElement")(
					function* (payload: PreviewAnnotationSubmissionResult | null) {
						yield* cleanup();
						resume(Effect.succeed(payload));
					},
				);
				const settlePick = Effect.fn("PreviewManager.settlePickElement")(
					function* (payload: PreviewAnnotationSubmissionResult | null) {
				...
				const cancelPickSession = Effect.fn("PreviewManager.cancelPickSession")(
					function* () {
						if (!claimSettle()) return;
						yield* cleanup();
						const tabs = yield* SynchronizedRef.get(tabsRef);
				...
				const registerPickElement = Effect.fn(
					"PreviewManager.registerPickElement",
				)(function* () {
					...
					if (settled) return;
					yield* attempt(
						{ operation: "pickElement.register", tabId, webContentsId: wc.id },
						() => {
		...
		const applyZoom = Effect.fn("PreviewManager.applyZoom")(function* (
			...
			if (tab.webContentsId != null) {
				...
				if (wc && !wc.isDestroyed()) {
					yield* attempt(
						{ operation: "applyZoom", tabId, webContentsId: wc.id },
		...
		const applyColorScheme = Effect.fn("PreviewManager.applyColorScheme")(
			function* (
				tabId: string,
				wc: Electron.WebContents,
				colorScheme: DesktopPreviewColorScheme,
		...
		const setColorScheme = Effect.fn("PreviewManager.setColorScheme")(
			function* (tabId: string, colorScheme: DesktopPreviewColorScheme) {
				const tab = (yield* SynchronizedRef.get(tabsRef)).get(tabId);
				if (!tab) {
					return yield* new PreviewTabNotFoundError({ tabId });
				}
				...
				const webContentsId = (yield* SynchronizedRef.get(tabsRef)).get(
					tabId,
				)?.webContentsId;
		...
		const setAudioMuted = Effect.fn("PreviewManager.setAudioMuted")(function* (
			...
			yield* withTabLifecycleLock(
				...
				Effect.gen(function* () {
					...
					const previous = (yield* SynchronizedRef.get(tabsRef)).get(
						tabId,
					)?.audioMuted;
		...
		const captureScreenshot = Effect.fn("PreviewManager.captureScreenshot")(
			function* (tabId: string) {
				const wc = yield* requireWebContents(tabId);
		...
		const capturePreviewFrame = Effect.fn("PreviewManager.capturePreviewFrame")(
			function* (tabId: string) {
				const captureSession = (yield* SynchronizedRef.get(
					frameCaptureSessionsRef,
				)).get(tabId);
				...
				const currentCaptureSession = yield* Effect.all(
					[
						SynchronizedRef.get(frameCaptureSessionsRef),
				...
				const frameSession = (yield* SynchronizedRef.get(
					frameCaptureSessionsRef,
				)).get(tabId);
				...
				if (pictureInPicture) {
					const pictureInPictureWindow = (yield* SynchronizedRef.get(
						pictureInPictureSessionsRef,
					)).get(tabId)?.window;
					if (pictureInPictureWindow && !pictureInPictureWindow.isDestroyed()) {
						deliveries.push(
							Effect.gen(function* () {
								const previousAspectRatio = (yield* Ref.get(
								...
								if (
									...
									yield* attempt(
										...
										() => {
											...
											pictureInPictureWindow.setContentSize(
												contentSize[0],
												contentSize[1],
												false,
											);
								...
								Effect.catch((error) =>
									Effect.logWarning(
										"Picture-in-picture frame delivery failed.",
										{
											tabId,
											error,
										},
		...
		const startFrameCapture = Effect.fn("PreviewManager.startFrameCapture")(
			function* (tabId: string, consumer: FrameCaptureConsumer) {
				...
				const captureNextFrame = Effect.sleep(
						...
						return Effect.gen(function* () {
							...
							if (current) {
								...
								if (!current.unthrottledWebContentsIds.has(wc.id)) {
									yield* setFrameCaptureWebContentsBackgroundThrottling(
										wc,
										false,
									);
								}
							...
							yield* setFrameCaptureWebContentsBackgroundThrottling(
								...
								Effect.onError(() =>
									sessions.size === 0
										? setFrameCaptureBackgroundThrottling(true).pipe(
												Effect.ignore,
											)
										: Effect.void,
								),
							...
							const scope =
								consumer === "picture-in-picture"
									? yield* Scope.fork(parentScope, "sequential")
									: null;
							if (scope !== null) {
								yield* Effect.forkIn(Effect.forever(captureNextFrame), scope);
							}
		...
		const releasePictureInPicture = Effect.fn(
			"PreviewManager.releasePictureInPicture",
		)(function* (
			tabId: string,
			...
			yield* Scope.close(expectedSession.initializationScope, Exit.void).pipe(
				Effect.ignore,
			);
		...
		const closePictureInPictureUnlocked = Effect.fn(
			"PreviewManager.closePictureInPictureUnlocked",
		)(function* (tabId: string) {
			const pictureInPictureSession = (yield* SynchronizedRef.get(
				pictureInPictureSessionsRef,
			)).get(tabId);
		...
		const closePictureInPicture = Effect.fn(
			"PreviewManager.closePictureInPicture",
		)(function* (tabId: string) {
			yield* pictureInPictureMutationSemaphore.withPermit(
				closePictureInPictureUnlocked(tabId),
			);
		});

		const closeAllPictureInPicture = Effect.fn(
			"PreviewManager.closeAllPictureInPicture",
		)(function* () {
		...
		const openPictureInPicture = Effect.fn(
			"PreviewManager.openPictureInPicture",
		)(function* (tabId: string) {
			const claim = yield* pictureInPictureMutationSemaphore.withPermit(
				Effect.gen(function* () {
					const existing = (yield* SynchronizedRef.get(
						pictureInPictureSessionsRef,
					)).get(tabId);
					...
					const pictureInPictureWindow = yield* attempt(
						...
						() =>
							new BrowserWindow({
								...
								title:
									title.length > 0 ? `Preview · ${title}` : "Browser preview",
								show: false,
								alwaysOnTop: true,
					...
					const initializationScope = yield* Scope.fork(
						parentScope,
						"sequential",
					);
					...
					const onDidFinishLoad = () => {
						runFork(
							SynchronizedRef.update(frameCaptureSessionsRef, (sessions) => {
								const current = sessions.get(tabId);
								if (!current?.consumers.has("picture-in-picture"))
									return sessions;
								return replaceMap(sessions, (copy) => {
									copy.set(tabId, {
										...current,
					...
					yield* attempt(
						...
						Effect.onError(() =>
							Effect.all(
								[
									Scope.close(initializationScope, Exit.void).pipe(
										Effect.ignore,
									),
			...
			if (claim.kind === "existing") {
				...
				return yield* pictureInPictureMutationSemaphore.withPermit(
					Effect.gen(function* () {
						...
						if (
							current !== pictureInPictureSession ||
							pictureInPictureSession.window.isDestroyed()
						) {
							return yield* new PreviewOperationError({
								...
								cause: new Error(
									"Picture-in-picture session closed before it became visible.",
								),
							});
						}
						yield* attempt(
			...
			const initialize = Effect.gen(function* () {
				...
				if (
					...
					return yield* new PreviewOperationError({
						...
						webContentsId: pictureInPictureSession.webContentsId,
						cause: new Error(
							"Preview webview changed while picture-in-picture was opening.",
						),
					});
				}
		 ...
		 */
		const armPendingRecording = Effect.fn("PreviewManager.armPendingRecording")(
			function* (
				tabId: string,
				wc: Electron.WebContents,
		...
		const installDisplayMediaRequestHandler = (session: Session) => {
			...
			session.setDisplayMediaRequestHandler((request, callback) => {
				...
				if (
					request.frame?.frameTreeNodeId !== armed.requestingFrameTreeNodeId
				) {
					callback({});
					return;
				}
						...
						const captureRequested = yield* attemptPromise(
							...
							() =>
								requestWebContents.executeJavaScript(
									requestRecordingCaptureExpression(tabId),
									true,
								),
						);
		...
		const saveRecording = Effect.fn("PreviewManager.saveRecording")(function* (
			...
			const [createdAt, millis] = yield* Effect.all([
				currentIso,
				currentMillis,
			]);
		...
		const automationStatus = Effect.fn("PreviewManager.automationStatus")(
			function* (tabId: string) {
				const tab = (yield* SynchronizedRef.get(tabsRef)).get(tabId);
				if (!tab || tab.webContentsId == null) {
					...
					return {
						...
						title:
							!navStatus || navStatus.kind === "Idle" ? null : navStatus.title,
		...
		const captureAutomationSnapshot = Effect.fn(
		...
		const automationSnapshot = Effect.fn("PreviewManager.automationSnapshot")(
			function* (tabId: string) {
				const wc = yield* requireWebContents(tabId);
		...
		const resolveClickPoint = Effect.fn("PreviewManager.resolveClickPoint")(
			function* (
				tabId: string,
				send: SendCommand,
		...
		const emitPointerEvent = Effect.fn("PreviewManager.emitPointerEvent")(
			function* (event: DesktopPreviewPointerEvent) {
				const listeners = yield* Ref.get(pointerEventListenersRef);
				yield* Effect.forEach(
					listeners,
					(listener) =>
						deliverEvent("pointer-event", event.tabId, () => listener(event)),
					{ discard: true },
				);
			},
		);

		const performAutomationClick = Effect.fn(
			"PreviewManager.performAutomationClick",
		)(function* (
			tabId: string,
			input: PreviewAutomationClickInput,
			send: SendCommand,
		...
		const automationClick = Effect.fn("PreviewManager.automationClick")(
			function* (tabId: string, input: PreviewAutomationClickInput) {
				const wc = yield* requireWebContents(tabId);
		...
		const typeIntoAutomationTarget = Effect.fn(
			"PreviewManager.typeIntoAutomationTarget",
		)(function* (
			tabId: string,
			send: SendCommand,
			...
			const locatorJson = locator
				? yield* encodeJson(
						{ operation: "automationType.encodeLocator", tabId },
						locator,
					)
				: null;
		...
		const performAutomationType = Effect.fn(
			"PreviewManager.performAutomationType",
		)(function* (
			tabId: string,
			input: PreviewAutomationTypeInput,
			send: SendCommand,
		...
		const automationType = Effect.fn("PreviewManager.automationType")(
			function* (tabId: string, input: PreviewAutomationTypeInput) {
				const wc = yield* requireWebContents(tabId);
		...
		const performAutomationPress = Effect.fn(
			"PreviewManager.performAutomationPress",
		)(function* (
			tabId: string,
			wc: Electron.WebContents,
			...
			const previouslyFocused = yield* attempt(
				{
					operation: "automationPress.getFocusedWebContents",
					tabId,
					webContentsId: wc.id,
				},
			...
			yield* Effect.gen(function* () {
				yield* attempt(
					{
						operation: "automationPress.focusWebContents",
						tabId,
						webContentsId: wc.id,
					},
					() => wc.focus(),
		...
		const automationPress = Effect.fn("PreviewManager.automationPress")(
		...
		const performAutomationScroll = Effect.fn(
			"PreviewManager.performAutomationScroll",
		)(function* (
			tabId: string,
			...
			const locatorJson = locator
				? yield* encodeJson(
						{ operation: "automationScroll.encodeLocator", tabId },
						locator,
					)
				: null;
			const result = yield* evaluateWithDebugger<
		...
		const automationScroll = Effect.fn("PreviewManager.automationScroll")(
			function* (tabId: string, input: PreviewAutomationScrollInput) {
				const wc = yield* requireWebContents(tabId);
		...
		const performAutomationEvaluate = Effect.fn(
			"PreviewManager.performAutomationEvaluate",
		)(function* (
			tabId: string,
		...
		const automationEvaluate = Effect.fn("PreviewManager.automationEvaluate")(
			function* (tabId: string, input: PreviewAutomationEvaluateInput) {
				const wc = yield* requireWebContents(tabId);
		...
		const performAutomationWaitFor = Effect.fn(
			"PreviewManager.performAutomationWaitFor",
		)(function* (
			tabId: string,
			...
			const [locatorJson, textJson, urlIncludesJson] = yield* Effect.all([
				locator
					? encodeJson(
							{ operation: "automationWaitFor.encodeLocator", tabId },
							locator,
						)
					: Effect.succeed(null),
				input.text
					? encodeJson(
							{ operation: "automationWaitFor.encodeText", tabId },
							input.text,
						)
					: Effect.succeed(null),
				input.urlIncludes
					? encodeJson(
							{ operation: "automationWaitFor.encodeUrl", tabId },
							input.urlIncludes,
						)
					: Effect.succeed(null),
			]);
			...
			while ((yield* currentMillis) <= deadline) {
              ...
              try {
                ...
                const urlMatched = ${
									urlIncludesJson
										? `location.href.includes(${urlIncludesJson})`
										: "true"
								};
		...
		const automationWaitFor = Effect.fn("PreviewManager.automationWaitFor")(
			function* (tabId: string, input: PreviewAutomationWaitForInput) {
				const wc = yield* requireWebContents(tabId);
		...
		const revealArtifact = Effect.fn("PreviewManager.revealArtifact")(
			function* (artifactPath: string) {
				const resolvedPath = yield* resolveArtifactPath(artifactPath);
				yield* attempt(
					{ operation: "revealArtifact", artifactPath: resolvedPath },
					() => shell.showItemInFolder(resolvedPath),
				);
			},
		);

		const copyArtifactToClipboard = Effect.fn(
			"PreviewManager.copyArtifactToClipboard",
		)(function* (artifactPath: string) {
			const resolvedPath = yield* resolveArtifactPath(artifactPath);
			const image = yield* attempt(
				{
					operation: "copyArtifactToClipboard.load",
					artifactPath: resolvedPath,
				},
				() => nativeImage.createFromPath(resolvedPath),
			);
			if (image.isEmpty()) {
				return yield* new PreviewArtifactImageLoadError({
					artifactPath: resolvedPath,
				});
			}
			yield* attemptPromise(
		...
		return {
			...
			subscribeStateChanges: (listener: Listener) =>
				subscribe(listenersRef, listener),
			zoomIn: (tabId: string) =>
				applyZoom(tabId, (current) => nextZoomLevel(current, "in")),
			zoomOut: (tabId: string) =>
				applyZoom(tabId, (current) => nextZoomLevel(current, "out")),
		};
	},
);
	...
	override get message(): string {
		const context = [
			...
			this.webContentsId === undefined
				? undefined
				: `WebContents ${this.webContentsId}`,
...
export class PreviewAutomationEvaluationError extends Schema.TaggedError<PreviewAutomationEvaluationError>()(
	...
	static toTimelineMessage(error: PreviewAutomationEvaluationError): string {
		return (
			previewAutomationEvaluationDetail(error.cause).detail ?? error.message
		);
	}
...
export class PreviewAutomationTargetNotFoundError extends Schema.TaggedError<PreviewAutomationTargetNotFoundError>()(
	...
	override get message(): string {
		const target = previewAutomationTargetLabel(
			this.selectorKind,
			this.selectorLength,
		);
...
export class PreviewAutomationInvalidSelectorError extends Schema.TaggedError<PreviewAutomationInvalidSelectorError>()(
	...
	static toTimelineMessage(
		error: PreviewAutomationInvalidSelectorError,
	): string {
...
const isPreviewAutomationEvaluationError = Schema.is(
	PreviewAutomationEvaluationError,
);
const isPreviewAutomationInvalidSelectorError = Schema.is(
	PreviewAutomationInvalidSelectorError,
);
...
export class PreviewManager extends Context.Service<
	PreviewManager,
	{
		...
		readonly pickElement: (
			...
			PreviewManagerError
		>;
		readonly cancelPickElement: (
			tabId: string,
		) => Effect.Effect<void, PreviewManagerError>;
		readonly captureScreenshot: (
			tabId: string,
		) => Effect.Effect<DesktopPreviewScreenshotArtifact, PreviewManagerError>;
		readonly revealArtifact: (
			path: string,
		) => Effect.Effect<void, PreviewManagerError>;
		readonly copyArtifactToClipboard: (
			path: string,
		) => Effect.Effect<void, PreviewManagerError>;
		readonly openPictureInPicture: (
			tabId: string,
		) => Effect.Effect<void, PreviewManagerError>;
		readonly closePictureInPicture: (
			tabId: string,
		) => Effect.Effect<void, PreviewManagerError>;
...
export const make = Effect.gen(function* PreviewManagerMake() {
	...
	return PreviewManager.of({
		...
		clearCookies: Effect.fn("PreviewManager.clearCookies")(
			function* (partitions) {
				yield* browserSession
					...
					.pipe(
						Effect.mapError(
							(cause) =>
								new PreviewOperationError({ operation: "clearCookies", cause }),
						),
					);
			},
		),
		clearCache: Effect.fn("PreviewManager.clearCache")(function* (partitions) {
			yield* browserSession
				...
				.pipe(
					Effect.mapError(
						(cause) =>
							new PreviewOperationError({ operation: "clearCache", cause }),
					),
				);
		}),
