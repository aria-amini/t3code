...
const navigationDirectionForButton = (
	button: number,
): "back" | "forward" | null => {
...
const suppressNavigationButton = (event: MouseEvent): void => {
	if (!event.isTrusted || navigationDirectionForButton(event.button) === null)
		return;
...
const isUsableRect = (rect: PreviewAnnotationRect): boolean =>
...
function styleControl(input: HTMLInputElement | HTMLSelectElement): void {
	input.setAttribute(
		"aria-label",
		input.getAttribute("aria-label") ?? "Style value",
	);
	input.className =
...
function createField(
	...
	label.append(
		text,
		input instanceof HTMLInputElement && input.dataset.unit
			? createUnitControl(input)
			: input,
	);
	return label;
}
...
function pathFromPoints(points: ReadonlyArray<PreviewAnnotationPoint>): string {
	...
	if (points.length === 1)
		return `M ${points[0]!.x} ${points[0]!.y} l 0.01 0.01`;
...
function startAnnotation(): void {
	...
	const resizeComment = (): void => {
		...
		comment.style.overflowY =
			comment.scrollHeight > maxHeight ? "auto" : "hidden";
		queueEditorLayout();
	};
	...
	const updateStatus = (): void => {
		...
		stylePanel.style.display =
			editorExpanded && selected.size > 0 ? "grid" : "none";
		queueEditorLayout();
	...
	const removeSelected = (target: SelectedElement): void => {
		if (
			...
			target.element instanceof SVGElement
		) {
		...
		for (const target of selected.values()) {
			...
			if (!target.baselineStyles.has(property)) {
				target.baselineStyles.set(
					property,
					target.element.style.getPropertyValue(property),
				);
			}
	...
	fontFamily.addEventListener("change", () =>
		setStyleForSelected("font-family", fontFamily.value),
	);
	...
	fontWeight.addEventListener("change", () =>
		setStyleForSelected("font-weight", fontWeight.value),
	);
	...
	lineHeight.addEventListener("change", () => {
		if (lineHeight.value.trim())
			setStyleForSelected("line-height", lineHeight.value.trim());
	});
	...
	opacity.addEventListener("input", () =>
		setStyleForSelected("opacity", opacity.value),
	);
	...
	dimensionLabel.className =
		"grid gap-2 font-sans text-xs font-medium text-muted-foreground";
	dimensionLabel.innerHTML = "<span>Width</span><span>Height</span>";
	const dimensionControls = document.createElement("div");
	dimensionControls.style.cssText =
		"position:relative;display:grid;gap:3px;padding-left:22px";
	...
	const syncStyleControls = (): void => {
		...
		fontSize.value = String(
			Math.round(Number.parseFloat(computed.fontSize) || 16),
		);
		fontWeight.value = computed.fontWeight.match(/^[0-9]+$/)
			? computed.fontWeight
			: "400";
		...
		radius.value = String(
			Math.round(Number.parseFloat(computed.borderRadius) || 0),
		);
		borderWidth.value = String(
			Math.round(Number.parseFloat(computed.borderWidth) || 0),
		);
	...
	const onPointerMove = (event: PointerEvent): void => {
		...
		if (tool === "select" && dragStart === null) {
			...
			if (target)
				positionBox(
					hoverOutline,
					rectFromDomRect(target.getBoundingClientRect()),
				);
	...
	const onKeyDown = (event: KeyboardEvent): void => {
		if (isAnnotationNode(event.target as Element) && event.key !== "Escape")
			return;
		if (event.key === "Escape") {
			event.preventDefault();
			event.stopPropagation();
	...
	const submitAnnotation = (submission: PreviewAnnotationSubmission): void => {
		...
		const submittedStyleChanges = Array.from(
			styleChanges.values(),
			(change) => ({ ...change }),
		);
		void Promise.all(
	...
	root.addEventListener("keydown", (event) => {
		const submission =
			event.target === comment ? resolveAnnotationSubmission(event) : null;
