...
describe("isPickedElementPayload", () => {
	...
	it("rejects malformed stack arrays", () => {
		expect(
			isPickedElementPayload(validPayload({ stack: "not-an-array" })),
		).toBe(false);
		expect(
			isPickedElementPayload(validPayload({ stack: [{ bogus: true }] })),
		).toBe(false);
	});
});
...
describe("isPreviewAnnotationPayload", () => {
	...
	it("rejects screenshots supplied by the guest preload", () => {
		expect(
			isPreviewAnnotationPayload(
				validAnnotation({ screenshot: { dataUrl: "bad" } }),
			),
		).toBe(false);
	});
	...
	it("rejects malformed geometry and nested element payloads", () => {
		expect(
			isPreviewAnnotationPayload(
				validAnnotation({
					regions: [{ id: "region_1", rect: { x: 0, y: 0, width: "wide" } }],
				}),
			),
		).toBe(false);
		expect(
			isPreviewAnnotationPayload(
				validAnnotation({
					elements: [{ id: "element_1", element: {}, rect: {} }],
				}),
			),
		).toBe(false);
	});
