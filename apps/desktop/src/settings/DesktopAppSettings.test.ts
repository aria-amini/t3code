...
const DesktopSettingsPatch = Schema.Struct({
	...
	serverExposureMode: Schema.optionalKey(
		Schema.Literals(["local-only", "network-accessible"]),
	),
...
const decodeDesktopSettingsPatch = Schema.decodeEffect(
	Schema.fromJsonString(DesktopSettingsPatch),
);
const encodeDesktopSettingsPatch = Schema.encodeEffect(
	Schema.fromJsonString(DesktopSettingsPatch),
);
		...
		return yield* effect.pipe(
			Effect.provide(
				DesktopAppSettings.layer.pipe(
					Layer.provideMerge(
						makeEnvironmentLayer(baseDir, options?.appVersion),
					),
...
describe("DesktopSettings", () => {
	...
	it("defaults packaged nightly builds to the nightly update channel", () => {
		assert.deepEqual(
			DesktopAppSettings.resolveDefaultDesktopSettings(
	...
	it.effect("loads persisted settings and applies semantic updates", () =>
		withSettings(
			Effect.gen(function* () {
				...
				assert.equal(
					updateChannel.settings.updateChannelConfiguredByUser,
					true,
				);
			}),
		),
	...
	it.effect(
		"reports the failed desktop settings write operation and path",
		() =>
			withSettings(
				Effect.gen(function* () {
					const environment = yield* DesktopEnvironment.DesktopEnvironment;
					...
					const error = yield* settings
						.setServerExposureMode("network-accessible")
						.pipe(Effect.flip);
					assert.instanceOf(
						error,
	...
	it.effect(
		"migrates legacy implicit update channels to the runtime default",
		() =>
			withSettings(
				Effect.gen(function* () {
					const settings = yield* DesktopAppSettings.DesktopAppSettings;
	...
	it.effect(
		"persists wsl backend toggle and normalizes invalid distro names",
		() =>
			withSettings(
				Effect.gen(function* () {
					const settings = yield* DesktopAppSettings.DesktopAppSettings;
	...
	it.effect(
		"applies WSL Windows fallback with persisted and volatile updates",
		() =>
			withSettings(
				Effect.gen(function* () {
					const settings = yield* DesktopAppSettings.DesktopAppSettings;
					...
					const volatileFallback =
						yield* settings.applyWslWindowsFallbackInMemory;
