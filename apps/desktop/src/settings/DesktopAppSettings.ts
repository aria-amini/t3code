...
export const DesktopWindowBoundsSchema = Schema.Struct({
	...
	width: Schema.Int.check(
		Schema.isGreaterThanOrEqualTo(MIN_MAIN_WINDOW_SIZE.width),
	),
	height: Schema.Int.check(
		Schema.isGreaterThanOrEqualTo(MIN_MAIN_WINDOW_SIZE.height),
	),
});
...
const DesktopSettingsDocument = Schema.Struct({
	...
	mainWindowBounds: Schema.optionalKey(
		Schema.NullOr(DesktopWindowBoundsDocument),
	),
...
const decodeDesktopWindowBounds = Schema.decodeUnknownOption(
...
function normalizeDesktopSettingsDocument(
	...
	return {
		linuxPasswordStore: normalizeLinuxPasswordStorePreference(
			parsed.linuxPasswordStore,
		),
		mainWindowBounds,
		mainWindowMaximized:
			mainWindowBounds !== null && parsed.mainWindowMaximized === true,
		serverExposureMode:
...
function toDesktopSettingsDocument(
	...
	if (
		settings.updateChannelConfiguredByUser !==
		defaults.updateChannelConfiguredByUser
	) {
		document.updateChannelConfiguredByUser =
			settings.updateChannelConfiguredByUser;
	}
			...
			Option.match({
				...
				onSome: (raw) =>
					decodeDesktopSettingsJson(raw).pipe(
						Effect.map((parsed) =>
							normalizeDesktopSettingsDocument(parsed, appVersion),
						),
...
const writeSettings = Effect.fn("desktop.settings.writeSettings")(
	function* (input: {
...
export const make = Effect.gen(function* () {
	...
	return DesktopAppSettings.of({
		...
		setMainWindowBounds: (bounds, isMaximized) =>
			persist((settings) =>
				setMainWindowBounds(settings, bounds, isMaximized),
			).pipe(
		...
		setServerExposureMode: (mode) =>
			persist((settings) => setServerExposureMode(settings, mode)).pipe(
				Effect.withSpan("desktop.settings.setServerExposureMode", {
					attributes: { mode },
				}),
			),
		setTailscaleServe: (input) =>
			persist((settings) => setTailscaleServe(settings, input)).pipe(
				Effect.withSpan("desktop.settings.setTailscaleServe", {
					attributes: input,
				}),
			),
		...
		setWslBackendEnabled: (enabled) =>
			persist((settings) => setWslBackendEnabled(settings, enabled)).pipe(
				Effect.withSpan("desktop.settings.setWslBackendEnabled", {
					attributes: { enabled },
				}),
			),
		setWslDistro: (distro) =>
		...
		setWslOnly: (enabled) =>
			persist((settings) => setWslOnly(settings, enabled)).pipe(
				Effect.withSpan("desktop.settings.setWslOnly", {
					attributes: { enabled },
				}),
			),
		...
		applyWslWindowsFallbackInMemory: updateInMemory(
			applyWslWindowsFallback,
		).pipe(Effect.withSpan("desktop.settings.applyWslWindowsFallbackInMemory")),
	});
});
	...
	Layer.effect(
		...
		Effect.gen(function* () {
			...
			return DesktopAppSettings.of({
				...
				setTailscaleServe: (input) =>
					update((settings) => setTailscaleServe(settings, input)),
				setUpdateChannel: (channel) =>
					update((settings) => setUpdateChannel(settings, channel)),
				setWslBackendEnabled: (enabled) =>
					update((settings) => setWslBackendEnabled(settings, enabled)),
				setWslDistro: (distro) =>
					update((settings) => setWslDistro(settings, distro)),
				setWslOnly: (enabled) =>
					update((settings) => setWslOnly(settings, enabled)),
