...
function makeLayer(fileSystemLayer: Layer.Layer<FileSystem.FileSystem>) {
	...
	return DesktopClientSettings.layer.pipe(
		Layer.provideMerge(
			Layer.mergeAll(environmentLayer, NodeServices.layer, fileSystemLayer),
		),
	);
}
...
describe("DesktopClientSettings diagnostics", () => {
	it.effect("treats a missing settings file as expected without warning", () =>
		Effect.gen(function* () {
			...
			if (result.result._tag !== "Success")
				return assert.fail("expected a successful read");
	...
	it.effect(
		"reports non-missing filesystem failures and logs the settings path",
		() => {
			...
			return Effect.gen(function* () {
				...
				if (result.result._tag !== "Failure")
					return assert.fail("expected a read failure");
				assert.instanceOf(
				...
				assert.equal(
					result.records[0]?.annotations.settingsPath,
					result.settingsPath,
				);
			});
		},
	...
	it.effect(
		"reports malformed settings documents and logs the settings path",
		() =>
			Effect.gen(function* () {
				const result = yield* readWithLogs(
					FileSystem.layerNoop({
				...
				if (result.result._tag !== "Failure")
					return assert.fail("expected a decode failure");
				assert.instanceOf(
					result.result.failure,
					DesktopClientSettings.DesktopClientSettingsReadError,
				);
				...
				assert.equal(
					"_tag" in schemaError ? schemaError._tag : undefined,
					"SchemaError",
				);
				assert.equal(
					result.records[0]?.annotations.settingsPath,
