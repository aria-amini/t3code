...
const decodeClientSettingsJson = Schema.decodeEffect(
	Schema.fromJsonString(ClientSettingsSchema),
);
...
describe("DesktopClientSettings", () => {
	...
	for (const failure of [
		...
		it.effect(
			`preserves saved preferences across ${failure.label} read failures and retries`,
			() =>
				withClientSettings(
					Effect.gen(function* () {
						const environment = yield* DesktopEnvironment.DesktopEnvironment;
						...
						const savedContents = yield* fileSystem.readFileString(
						...
						failRead = false;
						assert.deepEqual(
							yield* retryableSettings.get,
							Option.some(savedSettings),
						);
					}),
				),
		);
	...
	it.effect("reports the failed client settings write operation and path", () =>
		withClientSettings(
			Effect.gen(function* () {
				...
				assert.instanceOf(
					error,
					DesktopClientSettings.DesktopClientSettingsWriteError,
				);
	...
	it.effect("loads defaults from empty client settings documents", () =>
		withClientSettings(
			Effect.gen(function* () {
				...
				assert.deepEqual(
					yield* settings.get,
					Option.some(yield* decodeClientSettingsJson("{}")),
				);
			}),
		),
	);
	...
	for (const document of [
		...
		{
			label: "invalid legacy settings",
			contents: '{"settings":{"fontSizeCode":"large"}}',
		},
	]) {
		it.effect(
			`reports ${document.label} without treating the settings file as absent`,
			() =>
				withClientSettings(
					Effect.gen(function* () {
						const environment = yield* DesktopEnvironment.DesktopEnvironment;
