...
const withSavedEnvironments = <A, E, R>(
	effect: Effect.Effect<
		A,
		E,
		R | DesktopSavedEnvironments.DesktopSavedEnvironments
	>,
	options?: {
		readonly availableSecretStorage?: boolean;
		readonly availabilityError?: unknown;
...
describe("DesktopSavedEnvironments", () => {
		...
		withSavedEnvironments(
			Effect.gen(function* () {
				...
				const savedEnvironments =
					yield* DesktopSavedEnvironments.DesktopSavedEnvironments;
	...
	it.effect(
		"persists encrypted saved environment secrets when encryption is available",
		() =>
			withSavedEnvironments(
				Effect.gen(function* () {
					const savedEnvironments =
					...
					assert.deepEqual(
						yield* savedEnvironments.getSecret(
							savedRegistryRecord.environmentId,
						),
	...
	it.effect(
		"reports invalid saved secret encoding without exposing the secret",
		() =>
			withSavedEnvironments(
				Effect.gen(function* () {
					const environment = yield* DesktopEnvironment.DesktopEnvironment;
	...
	it.effect(
		"returns false when writing secrets while encryption is unavailable",
		() =>
			withSavedEnvironments(
				Effect.gen(function* () {
					const savedEnvironments =
	...
	it.effect(
		"adds saved-environment context to safe storage availability failures",
		() => {
	...
	it.effect("removes saved environment secrets", () =>
		withSavedEnvironments(
			Effect.gen(function* () {
				...
				yield* savedEnvironments.removeSecret(
					savedRegistryRecord.environmentId,
				);
	...
	it.effect(
		"removes saved environment metadata and its embedded secret atomically",
		() =>
			withSavedEnvironments(
				Effect.gen(function* () {
					const savedEnvironments =
					...
					yield* savedEnvironments.removeEnvironment(
						savedRegistryRecord.environmentId,
					);
	...
	it.effect("surfaces malformed saved environment documents", () =>
		withSavedEnvironments(
			Effect.gen(function* () {
				...
				const registryError = yield* savedEnvironments.getRegistry.pipe(
					Effect.flip,
				);
				assert.instanceOf(
					registryError,
	...
	it.effect(
		"reports saved environment filesystem reads separately from document decoding",
		() =>
			Effect.gen(function* () {
				...
				const savedEnvironments =
					yield* DesktopSavedEnvironments.DesktopSavedEnvironments.pipe(
				...
				assert.equal(
					error.message,
					`Failed to read desktop saved environments at ${registryPath}.`,
				);
	...
	it.effect(
		"reports the failed saved environment write operation and path",
		() =>
			Effect.gen(function* () {
				const baseFileSystem = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
