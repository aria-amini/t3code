...
function runShellEnvironment(input: {
	...
	const program = Effect.gen(function* () {
		const shellEnvironment =
			yield* DesktopShellEnvironment.DesktopShellEnvironment;
		yield* shellEnvironment.installIntoProcess;
	}).pipe(
		Effect.provide(
			DesktopShellEnvironment.layer.pipe(
				Layer.provide(
					Layer.mergeAll(environmentLayer, NodeServices.layer, spawnerLayer),
				),
			),
		),
	);
...
describe("DesktopShellEnvironment", () => {
	it.effect(
		"hydrates PATH and missing SSH_AUTH_SOCK from the login shell on macOS",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"does not mix login-shell locale categories into an inherited locale",
		() =>
			Effect.gen(function* () {
				const env: NodeJS.ProcessEnv = {
					SHELL: "/bin/zsh",
	...
	it.effect(
		"falls back to a UTF-8 LC_CTYPE when no locale is available on macOS",
		() =>
			Effect.gen(function* () {
				const env: NodeJS.ProcessEnv = {
					SHELL: "/bin/zsh",
	...
	it.effect("does not apply the locale fallback on linux", () =>
		Effect.gen(function* () {
			...
			yield* runShellEnvironment({
				...
				platform: "linux",
				handler: () =>
					envOutput({ PATH: "/home/linuxbrew/.linuxbrew/bin:/usr/bin" }),
			});
	...
	it.effect(
		"hydrates PATH and missing SSH_AUTH_SOCK from the login shell on linux",
		() =>
			Effect.gen(function* () {
				const env: NodeJS.ProcessEnv = {
					SHELL: "/bin/zsh",
	...
	it.effect(
		"falls back to launchctl PATH on macOS when shell probing does not return one",
		() =>
			Effect.gen(function* () {
				const env: NodeJS.ProcessEnv = {
				...
				yield* runShellEnvironment({
					...
					handler: (command) => {
						...
						return command.command === "/bin/launchctl"
	...
	it.effect(
		"prefers login-shell desktop session hints over inherited values on linux",
		() =>
			Effect.gen(function* () {
				const env: NodeJS.ProcessEnv = {
					SHELL: "/bin/zsh",
	...
	it("resolves dbus runtime dir candidates with existence checks", () => {
		const busPath =
			DesktopShellEnvironment.resolveDefaultLinuxDbusSessionBusAddress({
	...
	it.effect(
		"logs command failures with safe probe context and the exact cause",
		() => {
			const env: NodeJS.ProcessEnv = {
				SHELL: "/bin/bash",
				PATH: "/usr/bin",
