...
export function resolveDefaultLinuxDbusSessionBusAddress(input: {
	...
	return busPath !== null && input.exists(busPath)
		? `unix:path=${busPath}`
		: null;
}
...
const mergePaths = (
	...
	return entries.length > 0
		? Option.some(entries.join(delimiter))
		: Option.none();
};

const listLoginShellCandidates = (
...
const runCommandOutput = Effect.fn("desktop.shellEnvironment.runCommandOutput")(
	function* (input: {
...
const readWindowsEnvironment = Effect.fn(
	"desktop.shellEnvironment.readWindowsEnvironment",
)(function* (
	names: ReadonlyArray<string>,
...
const installWindowsEnvironment = Effect.fn(
...
const installPosixEnvironment = Effect.fn(
...
> {
	...
	if (
		config.platform === "darwin" &&
		LOCALE_ENV_NAMES.every((name) =>
			Option.isNone(trimNonEmpty(config.env[name])),
		)
	) {
	...
	if (
		...
		for (const runtimeDir of linuxRuntimeDirCandidates(
			config.env,
			process.getuid?.(),
		)) {
