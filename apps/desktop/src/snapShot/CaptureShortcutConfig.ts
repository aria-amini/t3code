...
export function niriCaptureConfigPath(
	env = process.env,
	home = NodeOS.homedir(),
) {
	return (
...
const defaultTools = {
	...
	reloadHyprland: async () => {
		...
		if (errors.stdout.trim())
			throw new Error("Hyprland reported config errors.");
	},
};
...
export class CaptureShortcutConfig {
			...
			bindings.some(
				(binding) =>
					...
					!(
						...
						binding.arg === `${appId}:capture-window`
					),
			)
		)
			throw new Error(
				`${keys.label} is already used by Hyprland. Choose another shortcut.`,
			);
	}
	...
	async preview(
		...
		if (this.applying)
			throw new Error("Wait for the current config change to finish.");
		...
		if (
			root.stat.uid !== process.getuid?.() ||
			/\/omarchy\/default\//.test(root.resolvedPath)
		)
			throw new Error(
				"Choose your own config, not system or Omarchy defaults.",
			);
		...
		if (
			...
			throw new Error(
				"Choose a .kdl Niri config or a .conf/.lua Hyprland config.",
			);
		...
		if (target.desktop === "niri") {
			const visit = async (file: Snapshot, depth: number) => {
				...
				for (const include of niriConfigIncludes(file.text)) {
					...
					if (files.some((item) => item.resolvedPath === child.resolvedPath))
						continue;
					files.push(child);
					if (
	...
	async apply(
		...
		if (
			...
			throw new Error(
				"This preview expired. Review changes again before saving.",
			);
		this.pending = undefined;
		this.applying = true;
		...
		const temporary = NodePath.join(
			directory,
			`.t3-capture-${NodeCrypto.randomUUID()}.tmp`,
		);
		let staged = false;
		try {
			const unchanged = async () => {
				...
				for (const original of files) {
					const current = await readSnapshot(original.path).catch(
						() => undefined,
					);
					if (
						!current ||
			...
			await NodeFSP.writeFile(backupPath, root.bytes, {
				flag: "wx",
				mode: 0o600,
			});
