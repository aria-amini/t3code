...
const {
	...
	accessibilityProcessReadMock: vi.fn<
		(
			...
			result: Promise<
				| import("./SnapShotAccessibility.ts").CapturedWindowAccessibilityContext
				| undefined
			>;
		}
	>(),
	...
	regionCaptureMock:
		...
		>(),
	screenToDipRectMock: vi.fn(
		(_window: unknown, bounds: Electron.Rectangle) => bounds,
	),
...
vi.mock("./WindowsCaptureFeedback.ts", () => ({
	showWindowsCaptureOverlay: (window: Electron.BaseWindow) =>
		window.showInactive(),
}));
vi.mock("./MacSnapShot.ts", () => ({
	captureMacWindowSnapshot: macCaptureMock,
}));
...
vi.mock("electron", () => {
	...
	return {
		...
		globalShortcut: {
			register: registerShortcutMock,
			unregister: unregisterShortcutMock,
		},
		screen: {
		...
		systemPreferences: {
			...
			isTrustedAccessibilityClient: (prompt: boolean) =>
				accessibilityTrustedMock(prompt),
		},
	};
});
...
const accessibilityApp = {
	...
	typeof SnapShotAccessibility.readAccessibleWindowContextWithApp
>[0];
...
const readAccessibleWindowText = async (
	...
	(await readAccessibleWindowContext(active, platform, sourceTitle))
		?.accessibleText;
...
function concurrentCaptureFixture(
	platform: NodeJS.Platform,
	animations: boolean,
) {
	...
	const captures = ["Discord", "File Explorer", "Terminal"].map(
		(title, index) => ({
			title,
			id: index + 42,
			processId: index + 100,
			...
			context:
				Promise.withResolvers<SnapShotAccessibility.CapturedWindowAccessibilityContext>(),
	...
	const takeSnapshot = async () => {
		...
		capture.oldOverlaysCleared = flashWindows.every(
	...
	macCaptureMock
		.mockReset()
		.mockImplementation(async (_active: unknown, imagePath: string) => {
			const capture = await takeSnapshot();
	...
	const readAccessibility =
		accessibilityProcessReadMock.getMockImplementation()!;
	accessibilityProcessReadMock.mockImplementation(({ active }) => ({
		started: Promise.resolve(),
		result: captures.find(
			(capture) => capture.processId === active.owner.processId,
		)!.context.promise,
	}));
	const handoff = Effect.sync(() =>
		captures[state.handoffs++]!.handoff.resolve(),
	);
	let randomByte = 0;
	const layer = Layer.mergeAll(
		testLayer(platform, {
			...
			writeFileString: (path, text) => {
				...
				return state.failFirstPersistence &&
		...
		Layer.succeed(
			...
			DesktopWindow.DesktopWindow.of({
				...
				prepareCaptureReveal: Effect.sync(() => {
					state.preparations++;
					state.preparedWithoutOverlay &&= flashWindows.every(
						(window) => window.destroyed,
					);
				}),
				dispatchMenuAction: () => Effect.void,
	...
	return {
		...
		reset: () => {
			...
			accessibilityProcessReadMock
				.mockReset()
				.mockImplementation(readAccessibility);
	...
	Effect.scoped(
		Effect.gen(function* () {
			...
			const previewError = yield* service
				...
				.pipe(Effect.flip);
			const applyError = yield* service
				.applyConfig("unapproved-preview")
				.pipe(Effect.flip);
			...
			for (const error of [previewError, applyError]) {
				...
				assert.equal(
					error.message,
					"Config setup requires a Niri or Hyprland session.",
				);
			}
		}),
...
it.effect(
	"reads and acknowledges queued captures through Effect services",
	() => {
		...
		const layer = testLayer("linux", {
			readDirectory: () =>
				Effect.succeed([captureId + ".json", "invalid.json"]),
...
it.effect(
	"captures the active Windows window without enumerating desktop sources",
	() => {
		...
		accessibilityByPidMock
			.mockReset()
			.mockResolvedValue({ children: async () => [] });
		regionCaptureMock
			.mockReset()
			.mockResolvedValue({ width: 800, height: 600, png });
...
it.effect.each([
	...
	({ length, suffix, expectedLength }) => {
		...
		macCaptureMock
			.mockReset()
			.mockResolvedValue({ source: { name: title }, png });
	...
	(platform) => {
		...
		accessibilityProcessReadMock.mockImplementation(({ active }) => ({
			...
			result: Promise.resolve({
				accessibleText: `Window from process ${active.owner.processId}`,
			}),
		}));
		let metadata = "";
		...
		return Effect.scoped(
			Effect.gen(function* () {
				...
				assert.equal(
					...
					`Window from process ${t3.owner.processId}`,
				);
				assert.deepEqual(images, [t3.png]);
				assert.equal(
					prepareCaptureRevealMock.mock.calls.length,
					platform === "win32" ? 1 : 0,
				);
				if (platform === "linux") {
...
it.effect.each(["win32", "darwin", "linux"] as const)(
	...
	(platform) => {
		...
		return Effect.scoped(
			...
			Effect.ensuring(Effect.sync(fixture.reset)),
		);
	},
);
	...
	({ platform, animations }) => {
		...
		return Effect.scoped(
			Effect.gen(function* () {
				...
				if (platform === "linux")
					assert.isTrue(fixture.second.oldNativeFeedbackClosed);
				const secondOverlays = flashWindows.filter(
					(window) => !window.destroyed,
				);
				if (platform !== "linux") assert.isNotEmpty(secondOverlays);

				fixture.second.context.resolve({
					accessibleText: "Explorer accessibility",
				});
				yield* Fiber.join(second);
				assert.lengthOf(fixture.readyIds, 1);
				fixture.first.context.resolve({
					accessibleText: "Discord accessibility",
				});
...
it.effect(
	"keeps newer native feedback when older accessibility persistence fails",
	() => {
...
it.effect("skips accessibility capture when the setting is disabled", () => {
	...
	return Effect.scoped(
		Effect.gen(function* () {
			const service = yield* DesktopSnapShot.make;
			yield* service.configure(
				enabledSettings({ snapShotIncludeAccessibility: false }),
			);
			yield* service.capture;
...
it.effect(
	"keeps an accessibility helper warm only while capture data is enabled",
	() => {
		accessibilityProcessWarmMock.mockClear();
...
it.effect(
	"rejects X11 capture without registering shortcuts or loading capture backends",
	() => {
...
it.effect.each(["gnome", "niri", "kde", "hyprland"] as const)(
	...
	(backend) => {
		...
		const window = {
			...
			...(backend === "kde"
				? { clientBounds: { x: 479, y: 371, width: 700, height: 520 } }
				: {}),
		};
		linuxCaptureMock.mockResolvedValueOnce({
			png: Buffer.from([1, 2, 3]),
		...
		return Effect.scoped(
			Effect.gen(function* () {
				...
				assert.lengthOf(activeWindowMock.mock.calls, 0);
				assert.lengthOf(getSourcesMock.mock.calls, 0);
				assert.deepEqual(accessibilityByPidMock.mock.calls, [
					[123, { timeout: 0 }],
				]);
				const saved = yield* decodePendingMetadata(metadata);
...
it.effect(
	"persists a window capture from a portal activation without a setup test",
	() => {
		vi.stubEnv("XDG_SESSION_TYPE", "wayland");
...
it.effect(
	"does not fall back to the picker when an automatic Wayland capture fails",
	() => {
		vi.stubEnv("XDG_SESSION_TYPE", "wayland");
	...
	() => {
		...
		return Effect.scoped(
			Effect.gen(function* () {
				...
				assert.strictEqual(
					Array.isArray(warning) ? warning[1] : undefined,
					activationFailure,
				);
...
it.effect(
	"does not read unverified accessibility context for a Wayland portal capture",
	() => {
		vi.stubEnv("XDG_SESSION_TYPE", "wayland");
		const png = Buffer.from([1, 2, 3]);
...
it.effect(
	"uses display-local macOS capture surfaces across the source and main displays",
	() => {
		const png = Buffer.from([1, 2, 3]);
		const active = {
			platform: "macos",
		...
		return Effect.scoped(
			Effect.gen(function* () {
				...
				const transitionWindows = flashWindows.filter(
					(window) => window.kind === "browser",
				);
				assert.deepEqual(
				...
				for (const transitionWindow of transitionWindows) {
					assert.deepEqual(transitionWindow.alwaysOnTopCalls, [
						[true, "pop-up-menu"],
					]);
				}
			}),
	...
	(outcome) => {
		...
		return Effect.scoped(
			Effect.gen(function* () {
				...
				const capture = yield* service.capture.pipe(
					Effect.forkChild({ startImmediately: true }),
				);
...
it.effect(
	"uses the unfocused main window for a macOS cross-display transition",
	() => {
		const png = Buffer.from([1, 2, 3]);
		const active = {
			platform: "macos",
		...
		macCaptureMock
			.mockReset()
			.mockResolvedValue({ source: { name: "Terminal" }, png });
		animationSettingsMock.mockReturnValueOnce({
			prefersReducedMotion: false,
			shouldRenderRichAnimation: true,
		});
		...
		return Effect.scoped(
			Effect.gen(function* () {
				...
				assert.deepEqual(
					flashWindows
						.filter((window) => window.kind === "browser")
...
it.each([
	...
	(_source, fileIcon, capturedIcon, expectedLabel) => {
		const dataUrl = DesktopSnapShot.snapShotIconDataUrl(fileIcon, capturedIcon);

		assert.strictEqual(
			dataUrl,
			"data:image/png;base64," + expectedLabel + ":64x64:best@2",
		);
	},
);
...
it("keeps the Windows transition above the revealed main window", async () => {
	...
	try {
		...
		assert.deepEqual(flashWindows[0]?.alwaysOnTopCalls, [
			[true, "pop-up-menu"],
		]);
	} finally {
		transition.dispose();
...
it.each(["darwin", "win32"] as const)(
	...
	async (platform) => {
		...
		accessibilityByPidMock
			.mockReset()
			.mockResolvedValue({ children: async () => [window] });
		accessibilityForegroundMock
			.mockReset()
		...
		assert.lengthOf(
			accessibilityByPidMock.mock.calls,
			platform === "win32" ? 0 : 1,
		);
		assert.lengthOf(
			accessibilityForegroundMock.mock.calls,
			platform === "win32" ? 1 : 0,
		);
	},
);
...
it.each([
	...
	"reads a changing Wayland title only when unambiguous: $names",
...
it.each([20, 1_350, 2_999])(
	...
	async (duration) => {
		vi.useFakeTimers();
		vi.stubEnv("XDG_SESSION_TYPE", "wayland");
		const tree = Promise.withResolvers<{
			value: string;
...
it("times out after three seconds without overlapping the outstanding accessibility read", async () => {
	...
	try {
		...
		assert.strictEqual(vi.getTimerCount(), 0);
		assert.isUndefined(
			await readAccessibleWindowText(active, "darwin", "main.ts"),
		);
...
it.effect(
	"does not request permissions or create the flash during desktop startup",
	() => {
		flashWindows.length = 0;
...
it.effect(
	"keeps snapshots disabled when client settings cannot be read at startup",
	() => {
		...
		return Effect.scoped(
			...
			Effect.provide(
				testLayer("win32", {}, Option.none(), Effect.fail(readError)),
			),
		);
	},
);
...
it.effect(
	"does not request macOS permissions while synchronizing enabled settings",
	() => {
		accessibilityTrustedMock.mockReset().mockReturnValue(false);
		mediaAccessStatusMock.mockReset().mockReturnValue("not-determined");
		getSourcesMock.mockReset().mockResolvedValue([]);
...
it.effect(
	"requests macOS permissions only for an explicit enable action",
	() => {
		accessibilityTrustedMock.mockReset().mockReturnValue(false);
		mediaAccessStatusMock.mockReset().mockReturnValue("not-determined");
		getSourcesMock.mockReset().mockResolvedValue([]);
...
it.effect(
	"does not request macOS accessibility permission when capture data is disabled",
	() => {
		accessibilityTrustedMock.mockReset().mockReturnValue(false);
		mediaAccessStatusMock.mockReset().mockReturnValue("not-determined");
		getSourcesMock.mockReset().mockResolvedValue([]);
...
it.effect(
	"reports macOS permission status and requests each permission on its own",
	() => {
		accessibilityTrustedMock.mockReset().mockReturnValue(false);
		...
		return Effect.scoped(
			Effect.gen(function* () {
				const service = yield* DesktopSnapShot.make;
				const state = yield* service.state;
				assert.deepEqual(state.macPermissions, {
...
it.effect(
	"flags revoked macOS permissions on read and re-registers once they return",
	() => {
		...
		return Effect.scoped(
			Effect.gen(function* () {
				...
				assert.deepEqual(revoked.macPermissions, {
...
it.effect(
	"rejects macOS permission actions off macOS and omits macPermissions there",
	() => {
		return Effect.scoped(
			Effect.gen(function* () {
				const service = yield* DesktopSnapShot.make;
...
it.effect(
	"registers macOS capture without accessibility permission when data is disabled",
	() => {
		accessibilityTrustedMock.mockReset().mockReturnValue(false);
		mediaAccessStatusMock.mockReset().mockReturnValue("granted");
		registerShortcutMock.mockReset().mockReturnValue(true);
...
it.effect("starts the Shift listener outside the Electron main process", () => {
	...
	return Effect.scoped(
		Effect.gen(function* () {
			...
			assert.strictEqual(
				shortcutForkOptions[0]?.env?.ELECTRON_RUN_AS_NODE,
				"1",
			);
...
it.effect(
	"registers a configured key chord instead of the Shift listener",
	() => {
		registerShortcutMock.mockReset().mockReturnValue(true);
		shortcutProcesses.length = 0;
		const settings = {
		...
		return Effect.scoped(
			Effect.gen(function* () {
				...
				assert.isTrue((yield* service.state).shortcutRegistered);
				assert.strictEqual(
					registerShortcutMock.mock.calls[0]?.[0],
					"Control+Alt+K",
				);
				assert.lengthOf(shortcutProcesses, 0);
			}),
...
it.effect(
	"an unrelated client-setting change keeps the registered shortcut",
	() => {
		registerShortcutMock.mockReset().mockReturnValue(true);
		...
		return Effect.scoped(
			Effect.gen(function* () {
				...
				assert.deepEqual(unregisterShortcutMock.mock.calls, [
					["Control+Alt+K"],
				]);
				assert.deepEqual(
					registerShortcutMock.mock.calls.map(([accelerator]) => accelerator),
...
it.effect(
	"uses an external Niri shortcut without registering an Electron accelerator",
	() => {
		vi.stubEnv("XDG_SESSION_TYPE", "wayland");
		...
		return Effect.scoped(
			Effect.gen(function* () {
				...
				assert.isFalse(
					(yield* service.checkShortcut(settings.snapShotShortcut)).available,
				);
...
it.effect(
	"defers ordinary Wayland shortcut registration until settings are applied",
	() => {
		vi.stubEnv("XDG_SESSION_TYPE", "wayland");
		registerShortcutMock.mockReset().mockReturnValue(false);
		...
		return Effect.scoped(
			Effect.gen(function* () {
				...
				assert.match(
					pair.message ?? "",
					/Modifier-pair shortcuts aren't available/,
				);
				assert.lengthOf(registerShortcutMock.mock.calls, 0);
			}),
		).pipe(
...
it.effect.each(["GNOME", "KDE", "niri", "Hyprland"] as const)(
	...
	(desktop) => {
		...
		linuxBackendMock.mockRejectedValueOnce(
			...
			for (const [desktop, backend] of [
				...
				yield* Effect.scoped(
					Effect.gen(function* () {
						...
						assert.isFalse(state.shortcutVerified);
						assert.equal(
							state.gnomeExtension?.status,
							desktop === "GNOME" ? "enabled" : undefined,
						);
						assert.equal(
							state.kdeHelper?.status,
							desktop === "KDE" ? "not-installed" : undefined,
						);
						assert.equal(
...
it.effect(
	"does not register a Wayland modifier-pair shortcut when enabled",
	() => {
		vi.stubEnv("XDG_SESSION_TYPE", "wayland");
		registerShortcutMock.mockReset().mockReturnValue(true);
		const settings = { ...DEFAULT_CLIENT_SETTINGS, snapShotEnabled: true };
...
it.effect(
	"exposes pending portal permission and then the real assigned shortcut",
	() => {
		vi.stubEnv("XDG_SESSION_TYPE", "wayland");
		registerShortcutMock.mockReset().mockReturnValue(true);
...
it.effect(
	"verifies only native shortcut delivery and captures normally",
	() => {
		vi.stubEnv("XDG_SESSION_TYPE", "wayland");
		vi.stubEnv("XDG_CURRENT_DESKTOP", "test-desktop");
		registerShortcutMock.mockReset().mockReturnValue(true);
		...
		return Effect.scoped(
			Effect.gen(function* () {
				...
				assert.equal(
					portalShortcutInstances[0]!.configure.mock.calls.length,
					1,
				);
				yield* service.configure({ ...settings, snapShotEnabled: false });
				assert.isFalse((yield* service.state).shortcutVerified);
...
it.effect("advises about the system menu for a meta pair on Windows", () =>
	Effect.scoped(
		Effect.gen(function* () {
			const service = yield* DesktopSnapShot.make;
			const result = yield* service.checkShortcut({
				kind: "modifier-pair",
				modifier: "meta",
			});
...
for (const fails of [false, true]) {
	it.effect(
		`tests macOS capture without publishing it and cleans up, failure=${fails}`,
		() => {
			const active = {
				platform: "macos",
				id: 42,
			...
			if (fails) macCaptureMock.mockRejectedValue(new Error("Capture denied"));
				...
				macCaptureMock.mockResolvedValue({
					source: { name: "Setup" },
					png: Buffer.from("png"),
				});
			accessibilityProcessReadMock.mockClear();
			...
			return Effect.scoped(
				Effect.gen(function* () {
					...
					assert.equal(
						macCaptureMock.mock.calls[0]?.[1],
						"/tmp/setup-test/test.png",
					);
