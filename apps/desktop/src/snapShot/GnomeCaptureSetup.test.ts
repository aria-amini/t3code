...
const installedPath = () =>
	NodePath.join(dataHome, "gnome-shell/extensions", GNOME_CAPTURE_UUID);
beforeEach(async () => {
	dataHome = await NodeFSP.mkdtemp(
		NodePath.join(NodeOS.tmpdir(), "t3-gnome-setup-test-"),
	);
});
afterEach(async () => {
...
function fixture(
	...
	const bus = Object.assign(new NodeEvents.EventEmitter(), {
		call,
		disconnect,
	});
	return {
		setup: new GnomeCaptureSetup(
			{ bundle, dataHome },
...
it("installs exactly the bundled payload offline and leaves other extensions alone", async () => {
	...
	expect((await NodeFSP.readdir(installedPath())).sort()).toEqual(
		[...GNOME_CAPTURE_FILES].sort(),
	);
	for (const file of GNOME_CAPTURE_FILES) {
		expect(
			await NodeFSP.readFile(NodePath.join(installedPath(), file)),
		).toEqual(await NodeFSP.readFile(NodePath.join(bundle, file)));
	}
	expect(await NodeFSP.readFile(NodePath.join(other, "keep.txt"), "utf8")).toBe(
		"keep",
	);
});
...
it("preserves the replaced extension as a recoverable backup", async () => {
	...
	await NodeFSP.writeFile(
		NodePath.join(installedPath(), "custom.txt"),
		"local change",
	);
	await installGnomeCaptureBundle({ bundle, dataHome });
...
it("does not damage an installed extension if a bundled file is missing", async () => {
	...
	await expect(
		installGnomeCaptureBundle({ bundle: incomplete, dataHome }),
	).rejects.toThrow();
	expect(
		await NodeFSP.readFile(NodePath.join(installedPath(), "extension.js")),
	).toEqual(await NodeFSP.readFile(NodePath.join(bundle, "extension.js")));
	expect(await NodeFSP.readdir(NodePath.dirname(installedPath()))).toEqual([
		GNOME_CAPTURE_UUID,
	]);
});
...
it("installs only on explicit action without invoking remote installation", async () => {
	...
	expect(await NodeFSP.readdir(installedPath())).toHaveLength(
		GNOME_CAPTURE_FILES.length,
	);
	expect(
		call.mock.calls.every(([message]) =>
			["GetAll", "GetExtensionInfo"].includes(message.member),
		),
	).toBe(true);
	setup.close();
});
...
it("enables and disables only the capture UUID", async () => {
	...
	for (const member of ["EnableExtension", "DisableExtension"]) {
		expect(
			call.mock.calls.find(([message]) => message.member === member)?.[0],
		).toMatchObject({
...
it("never changes the global user-extensions preference", async () => {
	...
	expect(
		call.mock.calls.every(([message]) => message.member.startsWith("Get")),
	).toBe(true);
	setup.close();
});
...
it("surfaces desktop rejection and disconnect as actionable failures", async () => {
	...
	expect(await setup.state()).toEqual({
		status: "error",
		message: "Session bus disconnected",
	});
	setup.close();
});

it("does not offer host extension installation in another desktop or sandbox", () => {
	expect(isGnomeCaptureSession({ XDG_CURRENT_DESKTOP: "ubuntu:GNOME" })).toBe(
		true,
	);
	expect(
		isGnomeCaptureSession({ XDG_CURRENT_DESKTOP: "niri", GDMSESSION: "gnome" }),
	).toBe(false);
	expect(
		isGnomeCaptureSession({
