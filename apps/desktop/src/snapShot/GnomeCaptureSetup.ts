...
const decodeMetadata = Schema.decodeUnknownSync(
	Schema.fromJsonString(Metadata),
);
...
export async function installGnomeCaptureBundle({
	bundle,
	dataHome,
}: SetupPaths) {
	...
	const existing = await NodeFSP.lstat(target).catch(
	...
	const staged = await NodeFSP.mkdtemp(
		NodePath.join(parent, ".t3-capture-install-"),
	);
	...
	private async call(message: Omit<MessageLike, "destination" | "path">) {
		...
		try {
			const reply = await Promise.race([
				this.bus.call(
					new Message({ destination: SHELL, path: SHELL_PATH, ...message }),
				),
				this.disconnected,
	...
	async state(): Promise<DesktopCaptureExtensionState> {
		try {
			const [properties, info, bundled, installed] = await Promise.all([
				...
				NodeFSP.readFile(
					NodePath.join(this.paths.bundle, "metadata.json"),
					"utf8",
				).then(decodeMetadata),
				NodeFSP.readFile(
					NodePath.join(
			...
			if (
				installed &&
				(!info.state ||
					(info.version && installed.version > info.version.value))
			)
				return {
			...
			return {
				...
				message:
					error instanceof Error
						? error.message
						: "Could not check GNOME extension setup.",
			};
		}
	}

	async perform(
		action: "install-extension" | "enable-extension" | "disable-extension",
	) {
		...
		const result = await this.call({
			interface: EXTENSIONS,
			member:
				action === "enable-extension" ? "EnableExtension" : "DisableExtension",
			signature: "s",
			body: [GNOME_CAPTURE_UUID],
		});
		if (result !== true)
