...
beforeEach(async () => {
	directory = await NodeFSP.mkdtemp(
		NodePath.join(NodeOS.tmpdir(), "t3-hypr-test-"),
	);
	paths = {
...
it("does not select Hyprland from a stale socket environment or sandbox", () => {
	expect(isHyprlandCaptureSession({ XDG_CURRENT_DESKTOP: "Hyprland" })).toBe(
		true,
	);
	expect(
		isHyprlandCaptureSession({
	...
	expect(
		isHyprlandCaptureSession({ XDG_CURRENT_DESKTOP: "Hyprland", SNAP: "app" }),
	).toBe(false);
});
...
it("installs offline at a stable executable path and probes only capabilities", async () => {
	...
	expect(
		(await NodeFSP.stat(hyprlandCaptureExecutable(paths))).mode & 0o777,
	).toBe(0o755);
...
it.each([true, false])(
	"uses the active config syntax and user bindings (Lua: %s)",
	async (lua) => {
		...
		const result = await hyprlandCaptureShortcut(
			"com.t3tools.T3Code",
			directory,
		);
		...
		expect(await NodeFSP.readFile(result.shortcutConfigPath, "utf8")).toBe(
			"custom bindings",
		);
	},
);
it("captures exact-window metadata for accessibility and focuses only through the installed helper", async () => {
	...
	expect(
		execute.mock.calls.find(([, args]) => args[0] === "activate")?.slice(0, 2),
	).toEqual([
		hyprlandCaptureExecutable(paths),
		["activate", String(process.pid), "T3 destination"],
	]);
	const captureDirectory = execute.mock.calls.find(
		([, args]) => args[0] === "capture",
	)![1][1];
	await expect(NodeFSP.stat(captureDirectory)).rejects.toMatchObject({
		code: "ENOENT",
	});
...
it("surfaces a capture denial without a picker fallback and removes temporary output", async () => {
	await setup.perform("install-hyprland-helper");
	denial = true;
	await expect(captureHyprlandWindow(paths)).rejects.toThrow(
		"screen-sharing permission",
	);
	const path = execute.mock.calls.find(
		([, args]) => args[0] === "capture",
	)![1][1];
