...
function hyprlandCaptureBinding(appId: string, lua: boolean): string {
	if (!/^[A-Za-z0-9_.-]+$/.test(appId))
		throw new Error("Invalid capture application ID.");
...
export async function hyprlandCaptureShortcut(
	...
	const config = (await exists(bindings))
		? bindings
		: lua
			? "hyprland.lua"
			: "hyprland.conf";
...
async function regularFile(path: string): Promise<Buffer | undefined> {
	const stat = await NodeFSP.lstat(path).catch(
...
export class HyprlandCaptureSetup {
	...
	async state(): Promise<DesktopCaptureHelperState> {
		try {
			const installed = await regularFile(
				hyprlandCaptureExecutable(this.paths),
			);
			if (!installed)
				return {
			...
			return {
				...
				...decodeCapabilities(
					await run(hyprlandCaptureExecutable(this.paths), ["check"]),
				),
			};
		} catch (error) {
			return {
				...
				message:
					error instanceof Error
						? error.message
						: "Couldn't check Hyprland capture access.",
			};
		}
	}
	async perform(action: "install-hyprland-helper" | "remove-hyprland-helper") {
		...
		const stat = await NodeFSP.lstat(directory).catch(
			(error: NodeJS.ErrnoException) => {
				if (error.code !== "ENOENT") throw error;
				return undefined;
			},
		...
		if (!bundle)
			throw new Error(
				"The Hyprland capture helper is missing from this build.",
			);
		await NodeFSP.mkdir(directory, { recursive: true });
		const staging = await NodeFSP.mkdtemp(
			NodePath.join(directory, ".install-"),
		);
		try {
...
export async function captureHyprlandWindow(
	...
	if (state.status !== "ready")
		throw new Error(
			`${state.message} Open Settings → SnapShots to continue setup.`,
		);
	const executable = hyprlandCaptureExecutable(paths);
	const directory = await NodeFSP.mkdtemp(
		NodePath.join(NodeOS.tmpdir(), "t3-hyprland-capture-"),
	);
		...
		return {
			...
			feedback: {
				...
				activate: async (title) => {
					...
					await run(
						executable,
						["activate", String(process.pid), title],
						activation.signal,
					);
				},
