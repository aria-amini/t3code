...
vi.mock("./NativeCaptureFeedback.ts", () => ({
	startNativeCaptureFeedback: startEffects,
}));
vi.mock("electron", () => ({
	nativeImage: {
		createFromBuffer: () => ({
...
beforeEach(async () => {
	directory = await NodeFSP.mkdtemp(
		NodePath.join(NodeOS.tmpdir(), "t3-kde-test-"),
	);
	paths = {
	...
	execute.mockImplementation(
		(
			...
			if (args[0] === "capture") {
				void NodeFSP.writeFile(
					NodePath.join(args[1]!, "capture.png"),
					png,
				).then(() => callback(null, JSON.stringify({ window }), ""));
				...
				callback(
					null,
					executable === "kbuildsycoca6"
						? ""
						: '{"ready":true,"feedbackAvailable":true}',
					"",
				);
		},
	);
...
it("detects KDE only outside a sandbox", () => {
	...
	expect(
		isKdeCaptureSession({ XDG_CURRENT_DESKTOP: "KDE", FLATPAK_ID: "app" }),
	).toBe(false);
	expect(
		isKdeCaptureSession({ XDG_CURRENT_DESKTOP: "KDE", SNAP: "/snap/app" }),
	).toBe(false);
});
...
it("does not install or request screenshots during initial discovery", async () => {
	...
	await expect(NodeFSP.stat(paths.dataHome)).rejects.toMatchObject({
		code: "ENOENT",
	});
});
...
it("installs offline at a stable path and verifies permissions using the installed identity", async () => {
	...
	expect(await NodeFSP.readFile(desktop, "utf8")).toBe(
		kdeCaptureDesktopEntry(executable),
	);
	expect(kdeCaptureDesktopEntry(executable)).toContain(
		`Exec="${executable}" check`,
	);
...
it("does not mistake installed files for KDE authorization, or fall back to a picker on denial", async () => {
	...
	expect(execute.mock.calls.some(([, args]) => args[0] === "capture")).toBe(
		false,
	);
});
...
it("repairs the old semicolon permission entry without replacing unrelated registrations", async () => {
	...
	expect(await NodeFSP.readdir(NodePath.dirname(desktop))).toEqual([
		NodePath.basename(desktop),
	]);
});
...
it("refuses symlink destinations and unrelated desktop entries", async () => {
	...
	expect(await NodeFSP.readFile(paths.bundle, "utf8")).toBe(
		"bundled executable",
	);
});
...
it("retains the frozen screenshot for feedback and removes it after the renderer acknowledges landing", async () => {
	...
	expect(await NodeFSP.readFile(NodePath.join(args[1], "capture.png"))).toEqual(
		png,
	);
