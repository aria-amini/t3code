...
const decodeCapabilities = Schema.decodeUnknownSync(
	Schema.fromJsonString(
		Schema.Struct({ feedbackAvailable: Schema.optional(Schema.Boolean) }),
	),
);
...
export class KdeCaptureSetup {
	...
	async state(): Promise<DesktopCaptureHelperState> {
		try {
			...
			if (
				...
				entry.toString() !== kdeCaptureDesktopEntry(executable)
			)
				return {
			...
			return {
				...
				message:
					error instanceof Error
						? error.message
						: "Couldn't check KDE capture access.",
			};
		}
	}
	...
	async perform(action: "install-kde-helper" | "remove-kde-helper") {
		...
		const existing = await NodeFSP.lstat(directory).catch(
			(error: NodeJS.ErrnoException) => {
				if (error.code !== "ENOENT") throw error;
				return undefined;
			},
...
export async function captureKdeWindow(
	...
	const directory = await NodeFSP.mkdtemp(
		NodePath.join(NodeOS.tmpdir(), "t3-kde-capture-"),
	);
	let retained = false;
