...
it.runIf(hasDbus)(
	"captures through real D-Bus marshalling on a private bus",
	async () => {
		const directory = await NodeFSP.mkdtemp(
			NodePath.join(NodeOS.tmpdir(), "t3-private-dbus-"),
		);
		...
		try {
			...
			await server.requestName(
				"org.gnome.Shell.Extensions.T3SnapShot",
				NameFlag.DO_NOT_QUEUE,
			);
				...
				if (message.member === "Register") {
					...
					server!.send(
						Message.newMethodReturn(message, "a{sv}", [
							{
								state: new Variant("d", shellExtensionState),
								version: new Variant("d", 2),
							},
						]),
					);
				} else if (
					message.member === "EnableExtension" ||
					message.member === "DisableExtension"
				) {
					...
					const signal = Message.newSignal(
						...
						[
							0,
							{ uri: new Variant("s", NodeURL.pathToFileURL(imagePath).href) },
						],
					);
					...
					message.member === "Capture" ||
					message.member === "CaptureWithFeedback"
				) {
			...
			const connect = () => {
				const client = new LinuxCaptureConnection(
			...
			expect(await portal.backend("com.t3tools.T3Code")).toBe(
				"screenshot-portal",
			);
			...
			expect(await extension.backend("com.t3tools.T3Code")).toBe(
				"gnome-extension",
			);
			expect(
				await extension.captureExtension("com.t3tools.T3Code"),
			).toMatchObject({
				png,
				window: { processId: 42 },
			});
			...
			expect(await updated.backend("com.t3tools.T3Code")).toBe(
				"gnome-extension",
			);
			...
			await snapshot.feedback!.animateTo({
				x: 0.1,
				y: 0.8,
				width: 0.2,
				height: 0.1,
