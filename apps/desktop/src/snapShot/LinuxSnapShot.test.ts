...
beforeEach(async () => {
	...
	directory = await NodeFSP.mkdtemp(
		NodePath.join(NodeOS.tmpdir(), "t3-portal-test-"),
	);
...
it("uses the standard path when KWin's native capability is missing, but not when denied", async () => {
	vi.stubEnv("XDG_CURRENT_DESKTOP", "KDE");
	bus.kdeVersion = undefined;
	expect((await getLinuxCaptureSupport(appId)).linuxBackend).toBe(
		"screenshot-portal",
	);
	bus.kdeVersion = 1;
	bus.portalVersion = 2;
	expect((await getLinuxCaptureSupport(appId)).linuxBackend).toBe("picker");
	bus.kdeError = new DBusError(
...
it("retains the authenticated connection until the compositor flight has landed", async () => {
	bus.portalVersion = 2;
	bus.extensionVersion = 2;
	const snapshot = await captureLinuxWindow(appId, {
		flash: false,
		animate: true,
	});
	...
	expect(snapshot).toMatchObject({
		png,
		window: metadata,
		feedback: { animationStarted: true },
	});
	...
	const flight = feedback.animateTo({
		x: 0.1,
		y: 0.8,
		width: 0.2,
		height: 0.1,
	...
	expect(
		bus.calls
			.filter((call) =>
				["CaptureWithFeedback", "Activate", "Animate"].includes(call.member),
			)
			.map((call) => call.body),
...
it("keeps capture working with v1 and does not request unavailable effects", async () => {
	...
	expect(
		await captureLinuxWindow(appId, { flash: true, animate: true }),
	).toEqual({
		png,
		window: metadata,
	});
	expect(bus.calls.some((call) => call.member === "CaptureWithFeedback")).toBe(
		false,
	);
	expect(bus.disconnect).toHaveBeenCalledOnce();
});
...
it("can activate without animation, and expires a renderer-abandoned capture", async () => {
	...
	await expect(snapshot!.feedback!.activate("T3 Code")).rejects.toThrow(
		"No window",
	);
...
it.each([false, true])(
	...
	async (differentHandle) => {
		...
		expect(
			bus.calls.findIndex((call) => call.member === "AddMatch"),
		).toBeLessThan(bus.calls.indexOf(screenshot!));
...
it.each([false, true])(
	...
	async (wrongSender) => {
		...
		const capture = expect(captureLinuxWindow(appId)).rejects.toThrow(
			"timed out",
		);
...
it("tolerates an older portal without Registry, but not registration denial", async () => {
	...
	await expect(getLinuxCaptureSupport(appId)).rejects.toThrow(
		"Denied identity",
	);
});
...
it("reads only local PNG files without removing the portal file", async () => {
	...
	expect(await readPortalPng(bus.uri)).toEqual(png);
	await expect(
		readPortalPng("https://example.com/image.png"),
	).rejects.toThrow();
