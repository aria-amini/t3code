...
const UInt = Schema.Int.check(
	Schema.isBetween({ minimum: 0, maximum: 0xffff_ffff }),
);
const UIntVariant = Schema.Struct({
	signature: Schema.Literal("u"),
	value: UInt,
});
const StringVariant = Schema.Struct({
	signature: Schema.Literal("s"),
	value: Schema.String,
});
...
const decodeExtension = Schema.decodeUnknownSync(
	Schema.Struct({ Version: UIntVariant }),
);
const decodeKde = Schema.decodeUnknownSync(
	Schema.Struct({ Version: UIntVariant }),
);
...
const decodeWindow = Schema.decodeUnknownSync(
	Schema.fromJsonString(WindowMetadata),
);
...
function unavailable(error: unknown): boolean {
	return (
		...
		[
			...
			"UnknownInterface",
			"UnknownMethod",
		].some((name) => error.type === `${DBUS}.Error.${name}`)
	);
}
...
export class LinuxCaptureConnection {
	...
	async backend(appId: string): Promise<LinuxCaptureBackend> {
		...
		if (properties !== undefined) {
			...
			if (
				parsed.version.value >= 3 &&
				((parsed.AvailableTargets?.value ?? 0) & 8) !== 0
			) {
				return "screenshot-portal";
			}
		}
		if (!isGnomeCaptureSession(process.env)) return "picker";
		const extension = await this.properties(
			EXTENSION,
			EXTENSION_PATH,
			EXTENSION,
		).catch((error: unknown) => {
		...
		if (extension !== undefined) {
			...
			if (this.extensionVersion === 1 || this.extensionVersion === 2)
	...
	async capturePortal(): Promise<LinuxWindowSnapshot> {
		...
		try {
			...
			if (!handle.startsWith(namespace))
				throw new Error("Invalid screenshot request handle.");
			...
			if (status !== 0)
				throw new Error("Your desktop did not allow the snapshot.");
	...
	async captureExtension(
		...
		if (result !== RequestNameReply.PRIMARY_OWNER) {
			throw new Error(
				"Another T3 Code instance is capturing a window. Try again.",
			);
		}
		...
		const reply = await this.call({
			...
			...(withFeedback
				? { signature: "bb", body: [options.flash, options.animate] }
				: {}),
		});
		const bytes: unknown = reply.body[0];
		if (!(bytes instanceof Uint8Array))
			throw new Error("Invalid extension screenshot.");
...
export async function getLinuxCaptureSupport(appId: string) {
	...
	const { niriSocketPath, checkNiriCaptureSupport } =
		await import("./NiriSnapShot.ts");
	...
	try {
		...
		return {
			linuxBackend,
			linuxFeedbackAvailable: connection.feedbackAvailable,
		};
	} finally {
		connection.close();
	}
...
export async function captureLinuxWindow(
	...
	try {
		switch (await connection.backend(appId)) {
			case "kde": {
				if (!kdePaths)
					throw new Error("KDE capture setup is unavailable in this build.");
