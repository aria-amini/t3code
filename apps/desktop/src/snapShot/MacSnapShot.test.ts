...
const { createFromBufferMock, execFileMock, readFileMock, writeFileMock } =
	vi.hoisted(() => ({
...
it("bounds Retina captures without changing their aspect ratio", async () => {
	...
	assert.deepEqual(resizeMock.mock.calls, [
		[{ width: 1_000, height: 600, quality: "best" }],
	]);
...
it("rejects a successful command that did not produce a PNG", async () => {
	...
	await expect(
		captureMacWindowSnapshot(active, "/tmp/window.png", {
			width: 100,
			height: 80,
		}),
