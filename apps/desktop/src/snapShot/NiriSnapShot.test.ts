...
import {
	captureNiriWindow,
	checkNiriCaptureSupport,
	niriSocketPath,
} from "./NiriSnapShot.ts";
...
type Request =
	...
	| {
			Action: {
				ScreenshotWindow?: { id: number; path: string };
...
beforeEach(async () => {
	directory = await NodeFSP.mkdtemp(
		NodePath.join(NodeOS.tmpdir(), "t3-niri-test-"),
	);
	...
	handler = async (request, socket) => {
		if (request === "EventStream") {
			...
			send(socket, { Ok: { Version: version } });
		else if (request === "FocusedWindow")
			send(socket, { Ok: { FocusedWindow: focused } });
...
it("selects the native adapter without needing a portal or GNOME extension", async () => {
	...
	expect(
		await NodeFSP.stat(NodePath.dirname(capturePath!)).catch(() => undefined),
	).toBeUndefined();
});
...
it("does not activate T3 until requested, then matches PID and title", async () => {
	...
	expect(
		calls.some((call) => typeof call !== "string" && call.Action.FocusWindow),
	).toBe(false);
	windows = [
		...
		if (request === "EventStream")
			send(socket, {
				WindowOpenedOrChanged: {
					window: { ...window, id: 4, pid: process.pid, title: "T3 Code" },
				},
			});
	};
	await snapshot.feedback!.activate("T3 Code");
...
it("cancels pending activation when capture feedback is closed", async () => {
	...
	const activation = expect(
		snapshot.feedback!.activate("T3 Code"),
...
it("rejects compositor errors and cleans up its temporary image", async () => {
	...
	await expect(captureLinuxWindow("com.t3tools.T3Code")).rejects.toThrow(
		"window disappeared",
	);
	expect(
		await NodeFSP.stat(NodePath.dirname(capturePath!)).catch(() => undefined),
...
it.each(["25.11", "26.04", "niri 26.04 (abc)"])(
	"accepts Niri version %s",
	async (value) => {
		version = value;
...
it("bounds requests even when the compositor never answers", async () => {
	...
	const result = expect(checkNiriCaptureSupport(socketPath)).rejects.toThrow(
		"timed out",
	);
...
it("does not use stale, relative, or sandboxed Niri sockets", () => {
	...
	expect(
		niriSocketPath({ XDG_CURRENT_DESKTOP: "GNOME", NIRI_SOCKET: socketPath }),
	).toBeUndefined();
	expect(
		niriSocketPath({ XDG_CURRENT_DESKTOP: "niri", NIRI_SOCKET: "relative" }),
	).toBeUndefined();
	expect(
		niriSocketPath({
