...
const decodeWindows = Schema.decodeUnknownSync(
	Schema.Struct({ Windows: Schema.Array(Window) }),
);
...
export function niriSocketPath(env = process.env): string | undefined {
	...
	if (
		!env.XDG_CURRENT_DESKTOP?.split(":").some(
			(name) => name.toLowerCase() === "niri",
		)
	)
		return undefined;
	return env.NIRI_SOCKET && NodePath.isAbsolute(env.NIRI_SOCKET)
		? env.NIRI_SOCKET
		: undefined;
}
				...
				try {
					...
					fail(
						error instanceof Error
							? error
							: new Error("Invalid Niri response."),
					);
				}
			};
			const timer = setTimeout(
				() => fail(new Error("Niri snapshot timed out.")),
				TIMEOUT_MS,
			);
...
async function request(path: string, message: unknown): Promise<unknown> {
	...
	try {
		const reply = connection.waitFor((value) => ({
			value: decodeReply(value),
		}));
...
export async function checkNiriCaptureSupport(path: string): Promise<void> {
	...
	if (
		!match ||
		Number(match[1]) < 25 ||
		(Number(match[1]) === 25 && Number(match[2]) < 11)
	)
		throw new Error("SnapShots require Niri 25.11 or newer.");
}

async function activateNiriWindow(
	path: string,
	title: string,
	signal: AbortSignal,
): Promise<void> {
	...
	try {
		const target = connection.waitFor((value) => {
			if (isWindowsChanged(value)) {
				windows.clear();
				for (const window of value.WindowsChanged.windows)
			...
			if (matches.length > 1)
				throw new Error(
					"More than one T3 Code window matches the capture destination.",
				);
			return matches[0];
		});
...
export async function captureNiriWindow(
	path: string,
): Promise<LinuxWindowSnapshot> {
	const directory = await NodeFSP.mkdtemp(
		NodePath.join(NodeOS.tmpdir(), "t3-niri-capture-"),
	);
	...
	try {
		...
		const ready = events.waitFor((value) =>
			isWindowsChanged(value) ? true : undefined,
		);
		events.send("EventStream");
		await ready;
		const window = decodeFocused(
			await request(path, "FocusedWindow"),
		).FocusedWindow;
		if (!window) throw new Error("Niri has no focused window to capture.");
		const captured = events.waitFor((value) =>
			isScreenshotCaptured(value) && value.ScreenshotCaptured.path === imagePath
				? true
				: undefined,
		);
		...
		const identityUnchanged =
			...
			after.layout.window_size.every(
				(size, i) => size === window.layout.window_size[i],
			);
