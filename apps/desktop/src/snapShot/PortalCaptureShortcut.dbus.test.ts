...
it.runIf(NodeChildProcess.spawnSync("dbus-daemon", ["--version"]).status === 0)(
	...
	async () => {
		const dir = await NodeFSP.mkdtemp(
			NodePath.join(NodeOS.tmpdir(), "t3-shortcut-dbus-"),
		);
		...
		try {
			...
			server.addMethodHandler((message: Message) => {
				if (message.member === "Register") {
					...
					server!.send(
						Message.newMethodReturn(message, "v", [new Variant("u", 2)]),
					);
				} else if (message.member === "Close") {
					closed.add(message.path);
					closeWaiters.get(message.path)?.();
				} else if (
					["CreateSession", "BindShortcuts"].includes(message.member)
				) {
					expect(identities.has(message.sender)).toBe(true);
					const options = message.body.at(-1) as Record<
						string,
						Variant<string>
					>;
							...
							[
								id,
								{
									...
									trigger_description: new Variant(
										"s",
										properties.preferred_trigger.value,
									),
								},
							],
						];
