...
import {
	PortalCaptureShortcut,
	portalShortcutTrigger,
} from "./PortalCaptureShortcut.ts";
...
type Shortcut = [
	string,
	{ description?: Variant<string>; trigger_description?: Variant<string> },
];
...
class FakeBus extends NodeEvents.EventEmitter {
	...
	response(
		path: string,
		status: number,
		results: Record<string, Variant<unknown>>,
	) {
		this.signal("org.freedesktop.portal.Request", "Response", path, [
			status,
			results,
		]);
	}
	...
	async call(message: Message) {
		...
		if (
			message.member === "AddMatch" ||
			message.member === "ConfigureShortcuts"
		)
			return reply();
...
it("accepts Hyprland's action-only binding without claiming the keys are reserved", async () => {
	...
	expect(
		bus.calls.find((call) => call.member === "BindShortcuts")?.body[1],
	).toEqual([
...
it("normalizes Ctrl/Mod and rejects unsupported keys before requesting access", () => {
	expect(
		portalShortcutTrigger({ ...chord, ctrlKey: true, metaKey: true }),
	).toBe("CTRL+SHIFT+LOGO+2");
...
it("replaces a binding without restarting the application and stops the old callback", async () => {
	...
	expect(
		next.bus.calls.find((call) => call.member === "BindShortcuts")?.body[1],
	).toEqual([
		[
			next.bus.boundId,
			{
	...
	expect(
		first.bus.sends.some(
			(message) =>
				message.interface === "org.freedesktop.portal.Session" &&
...
it("guides users to manual desktop settings when the portal cannot open them", async () => {
	...
	await expect(client.configure()).rejects.toThrow(
		"Open your desktop's shortcut settings",
	);
	expect(
		bus.calls.some((message) => message.member === "ConfigureShortcuts"),
	).toBe(false);
	client.close();
...
it("closes an outstanding consent request and ignores its late response", async () => {
	...
	expect(
		bus.sends.filter((message) => message.member === "Close"),
	).toHaveLength(2);
...
it("accepts old portals without Registry but does not ignore permission denial", async () => {
	...
	expect(denied.calls.some((call) => call.member === "CreateSession")).toBe(
		false,
	);
});
