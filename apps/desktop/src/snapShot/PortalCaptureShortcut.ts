...
const decodeShortcutsResult = Schema.decodeUnknownSync(
	Schema.Struct({
		shortcuts: Schema.Struct({
			signature: Schema.Literal("a(sa{sv})"),
			value: Shortcuts,
		}),
	}),
);
const decodeSession = Schema.decodeUnknownSync(
	Schema.Struct({ session_handle: StringVariant }),
);
...
export class PortalCaptureShortcut {
	state: PortalShortcutState = {
		...
		shortcutMessage:
			"Waiting for shortcut permission. Approve the desktop prompt if one appears.",
	};
	...
	private version = 0;
	private pending:
		| { path: string; resolve: (body: unknown) => void }
		| undefined;
	...
	private update(state: PortalShortcutState) {
		...
		this.state = {
			shortcutCanRetry:
				!this.managedByHyprland && (!this.hasSession || this.version >= 2),
			...state,
		};
		this.onStateChanged();
	}
			...
			return await Promise.race([
				...
				new Promise<never>((_, reject) => {
					timer = setTimeout(
						() =>
							reject(
								new Error("Shortcut permission request timed out. Try again."),
							),
						timeoutMs,
					);
				}),
	...
	private message = (message: Message) => {
		...
		if (
			...
			this.failed(
				new Error(
					"The desktop shortcut service restarted. Retry the shortcut request.",
				),
			);
			return;
		}
		...
		if (
			...
			if (message.path === this.pending?.path)
				this.pending.resolve(message.body);
			else if (this.responses.size < 8)
				this.responses.set(message.path, message.body);
			return;
		}
		if (
			this.session &&
			...
			this.failed(
				new Error(
					"Your desktop closed the capture shortcut. Retry the shortcut request.",
				),
			);
			return;
		}
	...
	private async request(
		...
		try {
			...
			if (!handle.startsWith(this.namespace))
				throw new Error("Invalid shortcut request handle.");
			...
			if (status !== 0) {
				...
				throw new Error(
					"Your desktop could not create a capture shortcut session.",
				);
			}
			return results;
		} finally {
			if (!completed && !this.closed && this.pending)
				this.closeObject(this.pending.path, REQUEST);
	...
	private async initialize(appId: string, shortcut: SnapShotKeyChord) {
		...
		if (!session.startsWith(sessionNamespace))
			throw new Error("Invalid shortcut session handle.");
