...
export function startRegionSnapShotProcess(
	...
	try {
		...
		return {
			alive: false,
			capture: () =>
				Promise.reject(new Error("Windows window capture is unavailable.")),
			close: () => undefined,
		};
	}
	...
	const finish = (
		error?: Error,
		value?: { width: number; height: number; png: Buffer },
	) => {
		if (settled) return;
		settled = true;
		ready.resolve();
	...
	child.onExit(() =>
		finish(new Error("Windows window capture helper exited.")),
	);
	...
	return {
		...
		capture: async (region) => {
			if (settled)
				throw new Error("Windows window capture helper already used.");
...
export function makeRegionSnapShotPool(
	...
	start: (
		workerPath: string,
	) => RegionSnapShotProcess = startRegionSnapShotProcess,
): RegionSnapShotPool {
	...
	return {
		...
		capture: async (region) => {
			...
			if (inFlight?.alive) {
				throw new Error(
					"Windows window capture is still in progress. Try again.",
				);
			}
