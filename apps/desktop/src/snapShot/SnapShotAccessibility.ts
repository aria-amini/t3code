		...
		progress.richComplete && richTree
			...
			: progress.flatComplete && accessibleText
				? {
						...
						text: accessibleText,
						truncated:
							accessibleText.length >= SNAP_SHOT_ACCESSIBLE_TEXT_MAX_CHARS,
					}
				: richTree
					? {
							format: "element-tree",
...
async function readCapturedWindowAccessibility(
	...
	const foreground =
		platform === "win32" ? await App.foreground({ timeout: 0 }) : undefined;
	const windows =
		foreground !== undefined
			? foreground.pid === active.owner.processId
				...
				: []
			: await (
					await App.byPid(active.owner.processId, { timeout: 0 })
				).children();
	const matchMode = isWaylandSession(platform, process.env)
		? "wayland"
		: "screen-bounds";
	...
	const flatRead = window
		...
		.then((tree) => {
			progress.accessibleText =
				accessibleWindowText(tree, SNAP_SHOT_ACCESSIBLE_TEXT_MAX_CHARS) ||
				undefined;
	...
	const richRead = accessibleWindowElementTree(
		window,
		sourceBounds,
		imageSize,
		{
