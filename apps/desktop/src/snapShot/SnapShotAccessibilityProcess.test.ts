...
it("returns accessibility extracted by the helper", async () => {
	...
	assert.strictEqual(
		forkMock.mock.calls[0]?.[2]?.env?.ELECTRON_RUN_AS_NODE,
		"1",
	);
	assert.strictEqual(
		forkMock.mock.calls[0]?.[2]?.execPath,
...
it("hands a warm helper to the capture and prewarms its replacement", async () => {
	...
	forkMock
		.mockReset()
		.mockReturnValueOnce(warmWorker)
	...
	warmWorker.emit("message", {
		type: "result",
		context: { accessibleText: "Ready" },
	});
...
it("replaces a warm helper that exits before capture", async () => {
	...
	captureWorker.emit("message", {
		type: "result",
		context: { accessibleText: "Recovered" },
	});
