...
export function snapShotAnimationDurationMs(
	...
	return (
		...
		(MAX_DURATION_MS - MIN_DURATION_MS) *
			Math.exp(-distance / DURATION_DISTANCE_SCALE)
	);
}
...
export function snapShotAnimationDisplayBounds(
	...
	const flight = snapShotAnimationOverlayBounds([
		{ bounds: source },
		{ bounds: destination },
	]);
	return displays
...
export class SnapShotTransition {
	...
	constructor(options: SnapShotTransitionOptions = {}) {
		this.boundOverlayToCaptureDisplays =
			options.boundOverlayToCaptureDisplays ?? false;
		...
		const requestedBounds = this.boundOverlayToCaptureDisplays
			? snapShotAnimationDisplayBounds(
					[
						...
						...Electron.screen
							.getAllDisplays()
							.filter((d) => d.id !== sourceDisplay.id),
					],
					source,
				...
				overlays.map(async (overlay) => {
					await overlay.window.loadURL(
						"data:text/html;charset=utf-8," +
							encodeURIComponent(
								transitionHtml(active.source, overlay.bounds, flash),
							),
					);
			...
			if (this.waitForCompositorFrame) {
				...
				await Promise.all(
					overlays.map(async (overlay) => {
						...
						await overlay.window.webContents.capturePage({
							x: 0,
							y: 0,
							width: 1,
							height: 1,
			...
			if (destination.details) {
				for (const overlay of active.overlays) {
					...
					void overlay.window.webContents
						.executeJavaScript(
							"window.updateCaptureDetails(" +
								JSON.stringify(destination.details) +
								")",
						)
						.catch(() => undefined);
				}
	...
	private async runFlight(
		...
		if (this.boundOverlayToCaptureDisplays) {
			...
			if (missingBounds.length > 0) {
				await this.showOverlays(active, missingBounds, false).catch(
					() => undefined,
				);
			}
		}
		if (this.active !== active) return;
		const durationMs = snapShotAnimationDurationMs(
			active.source,
			destination.frame,
		);
		...
		await Promise.allSettled(
			prepared.map((result) => {
				if (
					...
					!result.value ||
					result.value.window.isDestroyed()
				) {
					return;
				}
				return result.value.window.webContents.executeJavaScript(
					"window.playCaptureTransition()",
				);
			}),
		);
	}
