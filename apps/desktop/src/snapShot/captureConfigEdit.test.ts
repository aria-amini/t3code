...
describe("Niri capture config edits", () => {
	it("inserts into the existing top-level binds, preserving unrelated text", () => {
		...
		expect(
			readKdlNodes(result.after).filter((node) => node.name === "binds"),
		).toHaveLength(1);
	});
	it("ignores commented, nested, and string-valued binds and braces", () => {
		...
		expect(
			result.after.startsWith(valid.slice(0, valid.indexOf('"binds" {'))),
		).toBe(true);
	});
	it.each(["binds {}", "binds { Mod+Q { quit; }; }", "input { }\n", ""])(
		...
		(before) => {
			const result = editCaptureConfig(before, "niri", app, "install");
			const block = readKdlNodes(result.after).find(
				(node) => node.name === "binds",
			)!;
			expect(block.children.some((node) => node.name === "Ctrl+Shift+2")).toBe(
				true,
			);
		},
	);
	...
	it("replaces and removes only this app's capture binding", () => {
		const other = captureConfigBinding(
			"niri",
			"com.t3tools.Other",
			"Ctrl+Alt+4",
		);
		const before = `binds {\n    ${binding}\n    ${other}\n}\n`;
		const result = editCaptureConfig(
			before,
			"niri",
			app,
			"install",
	...
	it("refuses occupied keys regardless of modifier order, without removing them", () => {
		...
		expect(() => editCaptureConfig(before, "niri", app, "install")).toThrow(
			"already used",
		);
	});
	...
	it.each([
		...
		"/* unfinished",
		'spawn "unfinished',
	])("fails safely for malformed structure: %s", (source) => {
...
describe("Hyprland capture config edits", () => {
	it.each(["hyprland", "hyprland-lua"] as const)(
		...
		(format) => {
			...
			expect(editCaptureConfig(added.after, format, app, "install").after).toBe(
				added.after,
			);
			const changed = editCaptureConfig(
				added.after,
				format,
				app,
				"install",
			...
			expect(changed.after).not.toContain(
				captureConfigBinding(format, app, "Ctrl+Shift+2"),
			);
			expect(changed.after).toContain(
				captureConfigBinding(format, app, "Ctrl+Alt+Y"),
			);
			expect(
				editCaptureConfig(changed.after, format, app, "remove").after.trim(),
			).toBe(before.trim());
		},
	);
	...
	it("leaves commented bindings alone", () => {
		...
		expect(editCaptureConfig(before, "hyprland", app, "remove").after).toBe(
			before,
		);
	});
	it.each([
		...
		expect(() =>
			editCaptureConfig(source, "hyprland-lua", app, "install"),
		).toThrow("manual edit");
	});
	it.each(['Ctrl+"\nexec', "Ctrl+;", "2", "Mod+2"])(
		...
		(keys) => {
			expect(() =>
				editCaptureConfig("", "hyprland", app, "install", keys),
			).toThrow();
		},
	);
});
