...
export function captureConfigBinding(
	...
	if (format === "niri")
		return niriCaptureBinding(appId).replace("Ctrl+Shift+2", chord.label);
...
function niriCaptureNode(node: KdlNode, appId: string) {
	const expected = readKdlNodes(
		captureConfigBinding("niri", appId, "Ctrl+Shift+2"),
	)[0]!.children[0]!;
	...
	return readKdlNodes(source)
		...
		.map((node) => {
			const path = node.header.find(
				(token, index) =>
					index > 0 && token.quoted && node.header[index - 1]?.value !== "=",
			)?.value;
			if (!path)
				throw new Error(
...
export function niriConfigConflict(
	source: string,
	appId: string,
	keys: string,
) {
...
function removeNodes(source: string, ranges: { start: number; end: number }[]) {
	...
	for (const range of [...ranges].sort((a, b) => b.start - a.start)) {
		...
		const wholeLine =
			...
			!source.slice(range.end, lineEnd).trim();
		result =
			result.slice(0, wholeLine ? lineStart : range.start) +
			result.slice(
				wholeLine ? Math.min(lineEnd + 1, source.length) : range.end,
			);
	}
	return result;
}
...
export function editCaptureConfig(
	...
	if (
		format === "hyprland-lua" &&
		(/^\s*return\b/m.test(source) || /\[=*\[/.test(source))
	)
		throw new Error(
	...
	if (conflict)
		throw new Error(
			`${keys} is already used in this config. Choose another shortcut.`,
		);
