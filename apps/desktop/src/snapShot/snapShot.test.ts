...
describe("accessibleWindowElementTree", () => {
	...
	it("uses null child bounds when a Wayland provider reports only the root origin", async () => {
		...
		expect(tree?.root.bounds).toEqual({
			x: 0,
			y: 0,
			width: 1_600,
			height: 1_200,
	...
	it("keeps Wayland child bounds after observing real position variation", async () => {
		...
		expect(tree?.root.children[0]?.bounds).toEqual({
			x: 40,
			y: 60,
			width: 200,
			height: 100,
...
describe("findAccessibleWindow", () => {
	...
	it("matches a Wayland window whose accessibility provider omits its screen position", () => {
		...
		const windows = [
			{ name: captured.title, bounds: { x: 0, y: 0, width: 700, height: 520 } },
		];
	...
	it("matches a decorated Wayland window by its verified client size", () => {
		...
		const windows = [
			{ name: captured.title, bounds: { ...clientBounds, x: 0, y: 0 } },
		];
		expect(
			findAccessibleWindow(windows, { ...captured, clientBounds }, "wayland"),
		).toBe(windows[0]);
		expect(
			findAccessibleWindow(windows, { ...captured, clientBounds }),
		).toBeUndefined();
	...
	it("does not match an untitled Wayland window by size alone", () => {
		...
		expect(
			findAccessibleWindow(windows, { ...captured, title: "" }, "wayland"),
		).toBeUndefined();
	});
	...
	it.each([
		...
		"matches a Wayland spinner starting or stopping: %s → %s",
		(title, name) => {
			const windows = [{ name, bounds: captured.bounds }];
...
describe("isWaylandSession", () => {
	it.each([
		...
		"detects %s session %o as portal=%s",
		...
		() =>
			Effect.gen(function* () {
				...
				yield* Effect.promise(async () => {
					...
					try {
						expect(
							isWaylandSession("linux", { XDG_RUNTIME_DIR: runtimeDirectory }),
						).toBe(false);
						...
						expect(
							isWaylandSession("linux", {
								XDG_RUNTIME_DIR: "/nonexistent-t3-test",
							}),
						).toBe(false);
					} finally {
						if (server.listening) {
