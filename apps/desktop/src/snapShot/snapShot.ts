...
export function snapShotShortcutSystemConflict(
	...
	if (shortcut.altKey && key === "tab")
		return "The system uses Alt+Tab to switch apps.";
...
export function accessibleWindowText(
	...
	while (
		...
		visited < MAX_ACCESSIBILITY_TREE_NODES
	) {
...
export function boundedSnapShotString(
	...
	const end = /[\uD800-\uDBFF]/.test(candidate[maxChars - 1] ?? "")
		? maxChars - 1
		: maxChars;
...
function accessibilityNode(
	...
	const state = {
		...
		...(checked === "on" || checked === "off" || checked === "mixed"
			? { checked }
			: {}),
		...(safeProperty(() => element.editable) === true
			? { editable: true }
			: {}),
		...(safeProperty(() => element.enabled) === false
			? { enabled: false }
			: {}),
		...(typeof expanded === "boolean" ? { expanded } : {}),
		...(safeProperty(() => element.focused) === true ? { focused: true } : {}),
		...(safeProperty(() => element.selected) === true
			? { selected: true }
			: {}),
		...(safeProperty(() => element.visible) === false
			? { visible: false }
			: {}),
	};
...
export async function accessibleWindowElementTree(
	...
	| { readonly root: SnapShotAccessibilityNode; readonly truncated: boolean }
	| undefined
> {
	...
	const visit = async (
		...
		if (
			...
			(Math.abs(elementBounds.x - rootBounds.x) > 2 ||
				Math.abs(elementBounds.y - rootBounds.y) > 2)
		) {
...
>(
	...
	const normalizeTitle = (value: string) => {
		...
		return matchMode === "wayland"
			? title.replace(/^[⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏](?:\s+|$)/u, "")
			: title;
	};
	const titles = new Set(
		[captured.title, captured.sourceTitle ?? ""]
			.map(normalizeTitle)
			.filter(Boolean),
	);
	...
	const activeMatches = matches.filter(
		(window) => safeProperty(() => window.active) === true,
	);
