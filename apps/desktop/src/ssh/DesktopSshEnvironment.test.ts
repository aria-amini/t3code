...
describe("sshEnvironment", () => {
	it("keeps prompt presentation diagnostics distinct from the legacy wrapper message", () => {
		const cause =
			new DesktopSshPasswordPrompts.DesktopSshPromptPresentationError({
				requestId: "prompt-1",
				destination: "devbox",
		...
		assert.equal(
			cause.message,
			"Failed to present SSH password prompt for devbox.",
		);
		assert.equal(
	...
	it.effect(
		"wires desktop host discovery through the ssh package runtime",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				...
				yield* fs.makeDirectory(path.join(sshDir, "config.d"), {
					recursive: true,
				});
				...
				const sshEnvironment =
					yield* DesktopSshEnvironment.DesktopSshEnvironment;
					...
					DesktopSshEnvironment.layer().pipe(
						Layer.provideMerge(
							Layer.succeed(
								...
								{
									request: () =>
										Effect.die("unexpected password prompt request"),
									resolve: () =>
										Effect.die("unexpected password prompt resolution"),
								},
							),
						),
