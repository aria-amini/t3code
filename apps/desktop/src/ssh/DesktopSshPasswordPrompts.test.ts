	...
	return Layer.succeed(
		...
		ElectronWindow.ElectronWindow.of({
			...
			currentMainOrFirst: Effect.succeed(
				Option.some(window as Electron.BrowserWindow),
			),
...
describe("DesktopSshPasswordPrompts", () => {
	...
	it.effect("times out pending renderer prompts with a typed error", () => {
		...
		return Effect.gen(function* () {
			...
			assert.instanceOf(
				error,
				DesktopSshPasswordPrompts.DesktopSshPromptTimedOutError,
			);
	...
	it.effect("cleans up a prompt that fails during renderer delivery", () => {
		...
		return Effect.gen(function* () {
			...
			assert.instanceOf(
				error,
				DesktopSshPasswordPrompts.DesktopSshPromptPresentationError,
			);
			...
			assert.instanceOf(
				resolveError,
				DesktopSshPasswordPrompts.DesktopSshPromptExpiredError,
			);
		}).pipe(Effect.provide(makeLayer(testWindow.window)), Effect.scoped);
	});

	it.effect(
		"keeps a submitted password when a later presentation step fails",
		() => {
			const testWindow = makeTestWindow({
	...
	it.effect("classifies a failed initial window availability check", () => {
		const testWindow = makeTestWindow({
			isDestroyedError: new Error("window unavailable"),
		});
