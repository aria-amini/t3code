...
describe("DesktopTelemetryPublisher", () => {
	it.effect(
		"stops when its scope closes during an Electron telemetry sample",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"publishes Electron metrics and event-driven power state over NDJSON",
		() =>
			Effect.gen(function* () {
				const onBattery = yield* Ref.make(false);
				const systemIdleState =
					yield* Ref.make<ElectronPowerMonitor.ElectronIdleState>("active");
				let beforeSystemIdleState: Effect.Effect<void> = Effect.void;
				let metricsReadCount = 0;
				const simpleListeners = new Map<string, () => void>();
				let thermalListener:
					| ((state: ElectronPowerMonitor.ElectronThermalState) => void)
					| null = null;
				...
				const powerLayer = Layer.succeed(
					...
					ElectronPowerMonitor.ElectronPowerMonitor.of({
						...
						getSystemIdleState: () =>
							beforeSystemIdleState.pipe(
								Effect.andThen(Ref.get(systemIdleState)),
							),
						getCurrentThermalState: Effect.succeed("nominal"),
				...
				yield* Effect.gen(function* () {
					const publisher =
						yield* DesktopTelemetryPublisher.DesktopTelemetryPublisher;
					...
					if (hello?.type !== "desktopTelemetryHello") {
						return assert.fail(
							"Expected the first telemetry message to be the hello.",
						);
					}
					...
					if (initialSnapshot?.type !== "desktopTelemetry") {
						return assert.fail(
							"Expected the second telemetry message to be a snapshot.",
						);
					}
					...
					const nextSnapshotFiber = yield* Stream.runHead(
						publisher.changes,
					...
					const demandedSnapshot = Option.getOrThrow(
						yield* Fiber.join(nextSnapshotFiber),
					);
					assert.equal(demandedSnapshot.electronProcesses[0]?.pid, 4_242);
					assert.equal(
						demandedSnapshot.electronProcesses[0]?.creationTimeMs,
						1_001,
					);
					assert.equal(demandedSnapshot.electronProcesses[0]?.cpuPercent, 12.5);
					assert.equal(
						demandedSnapshot.electronProcesses[0]?.workingSetBytes,
						2_048 * 1_024,
					);
					...
					const batterySnapshotFiber = yield* Stream.runHead(
						publisher.changes,
					).pipe(Effect.forkChild);
					yield* Effect.yieldNow;
					simpleListeners.get("on-battery")?.();
					const batterySnapshot = Option.getOrThrow(
						yield* Fiber.join(batterySnapshotFiber),
					);
					...
					assert.equal(
						(yield* publisher.latest).pipe(Option.getOrThrow).power.onBattery,
						"true",
					);
					...
					const suspendedSnapshotFiber = yield* Stream.runHead(
						publisher.changes,
					).pipe(Effect.forkChild);
					yield* Effect.yieldNow;
					simpleListeners.get("suspend")?.();
					const suspendedSnapshot = Option.getOrThrow(
						yield* Fiber.join(suspendedSnapshotFiber),
					);
					assert.isTrue(suspendedSnapshot.power.suspended);

					const constrainedSnapshotFiber = yield* Stream.runHead(
						publisher.changes,
					).pipe(Effect.forkChild);
					yield* Effect.yieldNow;
					thermalListener?.("serious");
					const constrainedSnapshot = Option.getOrThrow(
						yield* Fiber.join(constrainedSnapshotFiber),
					);
					...
					const recoveredSnapshotFiber = yield* Stream.runHead(
						publisher.changes,
					).pipe(Effect.forkChild);
					...
					const recoveredSnapshot = Option.getOrThrow(
						yield* Fiber.join(recoveredSnapshotFiber),
					);
					assert.isFalse(recoveredSnapshot.power.suspended);

					const speedLimitSnapshotFiber = yield* Stream.runHead(
						publisher.changes,
					).pipe(Effect.forkChild);
					yield* Effect.yieldNow;
					speedLimitListener?.(65);
					const speedLimitSnapshot = Option.getOrThrow(
						yield* Fiber.join(speedLimitSnapshotFiber),
					);
					assert.equal(
						Option.getOrNull(speedLimitSnapshot.speedLimitPercent),
						65,
					);

					const encodedSpeedLimit = yield* publisher.encoded.pipe(
						Stream.take(2),
						Stream.runCollect,
					);
					const decodedSpeedLimit = yield* decodeMessage(
						decoder.decode(encodedSpeedLimit[1]).trim(),
					);
					if (decodedSpeedLimit.type !== "desktopTelemetry") {
						return assert.fail(
							"Expected the encoded telemetry message to be a snapshot.",
						);
					}
					assert.equal(
						Option.getOrNull(decodedSpeedLimit.speedLimitPercent),
						65,
					);
					assert.equal(decodedSpeedLimit.electronProcesses[0]?.pid, 4_242);
					assert.equal(
						decodedSpeedLimit.electronProcesses[0]?.creationTimeMs,
						1_001,
					);
					...
					assert.equal(
						(yield* publisher.latest).pipe(Option.getOrThrow)
							.electronProcesses[0]?.pid,
						4_242,
					);

					const stoppedSnapshotFiber = yield* Stream.runHead(
						publisher.changes,
					).pipe(Effect.forkChild);
					yield* Effect.yieldNow;
					...
					const stoppedSnapshot = Option.getOrThrow(
						yield* Fiber.join(stoppedSnapshotFiber),
					);
					assert.deepEqual(stoppedSnapshot.electronProcesses, []);
					const metricsAfterStopping = metricsReadCount;
					const configuredSnapshotFiber = yield* Stream.runHead(
						publisher.changes,
					).pipe(Effect.forkChild);
					yield* Effect.yieldNow;
					yield* publisher.handleControl({
					...
					const lockedSequence = (yield* publisher.latest).pipe(
						Option.getOrThrow,
					).sequence;
					assert.equal(
						(yield* publisher.latest).pipe(Option.getOrThrow).power.locked,
						"true",
					);
					...
					const unlockedSnapshot = (yield* publisher.latest).pipe(
						Option.getOrThrow,
					);
					...
					const concurrentEventSnapshotFiber = yield* Stream.runHead(
						publisher.changes,
					).pipe(Effect.forkChild);
					yield* Effect.yieldNow;
					yield* publisher.handleControl({
	...
	it.effect(
		"routes requestDesktopUpdate control messages and replays update reports",
		() =>
			Effect.gen(function* () {
				const powerLayer = Layer.succeed(
					ElectronPowerMonitor.ElectronPowerMonitor,
				...
				yield* Effect.gen(function* () {
					...
					const requestFiber = yield* Stream.runHead(
						publisher.updateRequests,
					).pipe(Effect.forkChild);
					yield* Effect.yieldNow;
