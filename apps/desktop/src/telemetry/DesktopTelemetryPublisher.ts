...
type PowerEvent =
	...
	| {
			...
			readonly value: HostPowerSnapshot["thermalState"];
	  }
...
export class DesktopTelemetryPublisher extends Context.Service<
	DesktopTelemetryPublisher,
	{
        ...
        relaunch). */
		readonly publishUpdateReport: (
			report: DesktopUpdateStatusReport,
		) => Effect.Effect<void>;
...
function idleState(
	value: ElectronPowerMonitor.ElectronIdleState,
): HostPowerSnapshot["idle"] {
	switch (value) {
		case "active":
			return "false";
		case "idle":
...
export const make = Effect.fn("desktop.telemetryPublisher.make")(function* () {
	...
	const diagnosticsDemandSources = yield* Ref.make<ReadonlySet<string>>(
		new Set(),
	);
	...
	const latestUpdateReport = yield* Ref.make(
		Option.none<DesktopUpdateStatusReport>(),
	);
	const updateReportChanges =
		yield* PubSub.sliding<DesktopUpdateStatusReport>(16);
	const updateRequestQueue =
		yield* Queue.unbounded<DesktopTelemetryRequestDesktopUpdate>();
	const updateCommitQueue =
		yield* Queue.unbounded<DesktopTelemetryCommitDesktopUpdate>();
	const updateCancellationQueue =
		yield* Queue.unbounded<DesktopTelemetryCancelDesktopUpdate>();
	...
	yield* Effect.all(
		[
			powerMonitor.onSimpleEvent("lock-screen", () =>
				offer({ type: "locked", value: true }),
			),
			powerMonitor.onSimpleEvent("unlock-screen", () =>
				offer({ type: "locked", value: false }),
			),
			powerMonitor.onSimpleEvent("suspend", () =>
				offer({ type: "suspended", value: true }),
			),
			powerMonitor.onSimpleEvent("resume", () =>
				offer({ type: "suspended", value: false }),
			),
			powerMonitor.onSimpleEvent("on-battery", () =>
				offer({ type: "onBattery", value: true }),
			),
			powerMonitor.onSimpleEvent("on-ac", () =>
				offer({ type: "onBattery", value: false }),
			),
			powerMonitor.onThermalStateChange((value) =>
				offer({ type: "thermal", value }),
			),
			powerMonitor.onSpeedLimitChange((value) =>
				offer({ type: "speedLimit", value }),
			),
		],
	...
	yield* Effect.forever(
		Queue.take(powerEvents).pipe(
			Effect.flatMap((event) =>
				Ref.update(powerState, (state) => updatePowerState(state, event)),
			),
	...
	const sampleOnce = (allowSuspendRecovery: boolean) =>
		Effect.gen(function* () {
			...
			const [currentPower, idleSeconds, systemIdleState, onBattery, metrics] =
				yield* Effect.all(
					[
						Ref.get(powerState),
			...
			const observedPower = yield* Ref.modify(powerState, (latestPower) => {
				...
				const next: PowerState = {
					...
					lockedEventPending:
						latestPower.lockedEventPending &&
						(systemIdleState === "unknown" ||
							polledLocked !== latestPower.locked),
					suspended:
						allowSuspendRecovery && !preserveEventSuspended
							? false
							: latestPower.suspended,
					suspendedEventPending: false,
					onBattery: preserveEventOnBattery
						? latestPower.onBattery
						: polledOnBattery,
					onBatteryEventPending:
						latestPower.onBatteryEventPending &&
						polledOnBattery !== latestPower.onBattery,
				};
				return [next, next] as const;
			});
			const nextSequence = yield* Ref.modify(sequence, (current) => [
				current + 1,
				current + 1,
			]);
			const snapshot: DesktopHostTelemetrySnapshot = {
				...
				electronProcesses: metrics.map((metric) => ({
					...
					peakWorkingSetBytes: Math.max(
						0,
						Math.round(metric.memory.peakWorkingSetSize * 1024),
					),
				})),
			};
	...
	yield* Effect.gen(function* () {
		...
		while (true) {
			const [currentPower, demand, intervals] = yield* Effect.all([
				Ref.get(powerState),
				Ref.get(diagnosticsDemandSources).pipe(
					Effect.map((sources) => sources.size > 0),
				),
			...
			const allowSuspendRecovery = yield* Effect.raceFirst(
				...
				Effect.sleep(sampleInterval(currentPower, demand, intervals)).pipe(
					Effect.as(true),
				),
			);
		...
		(sourceId, message) => {
			switch (message.type) {
				...
				case "cancelDesktopUpdate":
					return Queue.offer(updateCancellationQueue, message).pipe(
						Effect.asVoid,
					);
			}
		};
	...
	const handleControl: DesktopTelemetryPublisher["Service"]["handleControl"] = (
		message,
	...
	const encoded = Stream.concat(
		...
		Stream.map((message) => textEncoder.encode(`${encodeMessage(message)}\n`)),
	);
