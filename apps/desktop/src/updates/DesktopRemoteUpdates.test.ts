...
function runRemoteUpdatesTest(
	...
	body: (context: {
		...
		void,
		never,
		DesktopUpdates.DesktopUpdates | DesktopState.DesktopState
	>,
) {
...
function terminalReports(
	reports: DesktopUpdateStatusReport[],
): DesktopUpdateStatusReport[] {
...
describe("DesktopRemoteUpdates", () => {
	it.effect(
		"drives check, download, and install with no local confirmation",
		() => {
	...
	it.effect(
		"joins an in-progress install on commit instead of failing the token",
		() => {
	...
	it.effect(
		"does not misread a lingering install error as a failed retry",
		() => {
	...
	it.effect(
		"ignores an unrelated updater event while an install is starting",
		() => {
	...
	it.effect(
		"retries a download refused while the check still holds the reservation",
		() => {
			...
			const harness = makeHarness({
				checkForUpdates: Deferred.await(releaseCheck),
			});
	...
	it.effect(
		"waits for the download reservation before reporting prepared",
		() => {
			...
			const harness = makeHarness({
				downloadUpdate: Deferred.await(releaseDownload),
			});
	...
	it.effect(
		"waits for a background check before installing a prepared update",
		() => {
			...
			return runRemoteUpdatesTest(harness, ({ requests, commits }) =>
				Effect.gen(function* () {
					...
					const backgroundCheck = yield* updates
						.check("poll")
	...
	it.effect(
		"fails a prepared install when its background check stays blocked",
		() => {
			const backgroundCheckStarted = Deferred.makeUnsafe<void>();
			const releaseBackgroundCheck = Deferred.makeUnsafe<void>();
			let checks = 0;
			...
			return runRemoteUpdatesTest(harness, ({ reports, requests, commits }) =>
				Effect.gen(function* () {
					...
					assert.deepEqual(
						terminalReports(reports)
							.filter(
								(report) => report.requestId === "req-blocked-background-check",
							)
							.map((report) => report.outcome),
						["ready-to-install", "failed"],
					);
	...
	it.effect(
		"rejects a new preparation while an install commit is active",
		() => {
			const installStarted = Deferred.makeUnsafe<void>();
			const releaseInstall = Deferred.makeUnsafe<void>();
			const harness = makeHarness({
			...
			return runRemoteUpdatesTest(harness, ({ reports, requests, commits }) =>
				Effect.gen(function* () {
					...
					assert.equal(
						overlap?.reason,
						"A prepared desktop update is already in progress.",
					);
	...
	it.effect(
		"remembers a cancellation that arrives before its request starts",
		() => {
			const harness = makeHarness();
			...
			({ reports, requests, commits, cancellations }) =>
				Effect.gen(function* () {
					...
					assert.deepEqual(
						terminalReports(reports)
							.filter(
								(report) => report.requestId === "req-cancel-before-commit",
							)
							.map((report) => report.outcome),
						["ready-to-install", "failed"],
					);
	...
	it.effect(
		"retains an install failure after cancellation loses the commit claim",
		() => {
			const installStarted = Deferred.makeUnsafe<void>();
			const releaseInstall = Deferred.makeUnsafe<void>();
			const harness = makeHarness({
				...
				({ reports, requests, commits, cancellations }) =>
					Effect.gen(function* () {
						...
						assert.deepEqual(
							terminalReports(reports)
								.filter(
									(report) => report.requestId === "req-cancel-after-commit",
								)
								.map((report) => report.outcome),
							["ready-to-install", "failed", "failed"],
						);
	...
	it.effect(
		"reports up-to-date without installing when there is no update",
		() => {
			const harness = makeHarness();
