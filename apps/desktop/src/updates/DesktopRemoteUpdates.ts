...
const { logInfo, logError } = DesktopObservability.makeComponentLogger(
	"desktop-remote-updates",
);
...
> = Effect.gen(function* () {
	...
	const clearActiveRequest = (requestId: string) =>
		Ref.update(activeRequestIdRef, (active) =>
			Option.isSome(active) && active.value === requestId
				? Option.none()
				: active,
		);
			...
			yield* Stream.runForEach(changes, (state) =>
				Effect.gen(function* () {
					...
					if (
						...
						const reason =
							state.message ?? "The desktop app failed to install the update.";
						if (
							yield* recordPreparedFailure(prepared.value.requestId, reason)
						) {
							yield* publishReport(
								state,
	...
	const handleRequest = (
		request: DesktopTelemetryRequestDesktopUpdate,
	): Effect.Effect<void> =>
		Effect.scoped(
			Effect.gen(function* () {
				...
				yield* logInfo("remote update requested", {
					requestId: request.requestId,
				});
				const { latest, changes } = yield* updates.subscribe;
				...
				const step = (
					...
					Effect.gen(function* () {
						const next = nextRemoteDesktopUpdateStep(
							state,
							attempts,
							disabledReason,
						);
						switch (next.action) {
							...
							case "download":
								...
								yield* updates.download.pipe(
									Effect.flatMap((result) => {
										...
										attempts = {
											...attempts,
											downloads: attempts.downloads - 1,
										};
										return retryLater(state);
									}),
									Effect.forkScoped,
							...
							case "install": {
								...
								yield* Ref.set(
									...
									Option.some({
										...
										expiresAt:
											...
											Duration.toMillis(PREPARED_UPDATE_TTL),
									}),
								);
								yield* publishReport(
									state,
								...
								yield* logInfo("remote update prepared", {
									requestId: request.requestId,
								});
								return true;
							}
	...
	yield* Stream.runForEach(publisher.updateRequests, handleRequest).pipe(
		Effect.forkScoped,
	);
	...
	yield* Stream.runForEach(publisher.updateCancellations, (cancellation) =>
		Effect.gen(function* () {
			...
			const matchedPrepared = yield* Ref.modify(
				preparedUpdateRef,
				(prepared) => {
					if (
						Option.isNone(prepared) ||
						prepared.value.requestId !== cancellation.requestId
					) {
						return [false, prepared] as const;
					}
					return [
						true,
						prepared.value.status === "prepared" ? Option.none() : prepared,
					] as const;
				},
			);
			if (matchedPrepared) {
		...
		Effect.gen(function* () {
			...
			if (!result.accepted || result.failed) {
				const reason =
					...
					"The desktop app could not start the install.";
				if (yield* recordPreparedFailure(commit.requestId, reason)) {
					yield* publishReport(
						result.state,
						{ outcome: "failed", reason },
