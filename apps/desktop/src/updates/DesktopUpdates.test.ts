...
describe("DesktopUpdates", () => {
	it("preserves complete causes for update poller and event failures", () => {
		...
		const unexpectedActionError =
			new DesktopUpdates.DesktopUpdateUnexpectedActionError({
				action: "install",
				cause,
			});
		...
		assert.equal(
			eventError.message,
			"Failed to handle desktop update download-progress event.",
		);
	...
	it.effect(
		"configures the updater and runs startup checks on the test clock",
		() => {
	...
	it.effect(
		"subscribe delivers the latest state plus subsequent changes",
		() => {
			const harness = makeHarness();
			...
			return Effect.scoped(
				Effect.gen(function* () {
					...
					const nextState = yield* Stream.runHead(changes).pipe(
						Effect.forkChild,
					);
	...
	it.effect(
		"enables nightly full changelog release notes and broadcasts summaries",
		() => {
			const harness = makeHarness();
			...
			return Effect.scoped(
				Effect.gen(function* () {
					...
					assert.deepEqual(state.releaseNotes, [
						{
							version: "1.2.4-nightly.20260709.766",
							items: [
								"feat(client): persist offline environment data by @juliusmarminge in #3795",
							],
							totalItems: 1,
						},
						{
						...
						{
							version: "1.2.4-nightly.20260709.764",
							items: ["Change 764"],
							totalItems: 1,
						},
						{
							version: "1.2.4-nightly.20260709.763",
							items: ["Change 763"],
							totalItems: 1,
						},
						{
							version: "1.2.4-nightly.20260709.762",
							items: ["Change 762"],
							totalItems: 1,
						},
						{
							version: "1.2.4-nightly.20260709.761",
							items: ["Change 761"],
							totalItems: 1,
						},
					]);
					...
					assert.deepEqual(
						harness.sentStates.at(-1)?.releaseNotes,
						state.releaseNotes,
					);
	...
	it.effect(
		"checks for newer releases after an update has been downloaded",
		() => {
			const harness = makeHarness();
	...
	it.effect(
		"preserves a queued installer when the feed offers another channel",
		() => {
			const harness = makeHarness();
			...
			return Effect.scoped(
				Effect.gen(function* () {
					...
					yield* updates.check("poll");
					harness.emit("update-available", {
						version: "1.2.5-nightly.20260710.1",
					});
					yield* flushCallbacks;
			...
			Effect.gen(function* () {
				...
				yield* Effect.scoped(
					Effect.gen(function* () {
						...
						const checkFiber = yield* updates
							.check("manual")
	...
	it.effect("keeps raw updater event failures out of update state", () => {
		...
		return Effect.scoped(
			Effect.gen(function* () {
				...
				assert.equal(
					state.message,
					"Desktop updater background operation reported an error.",
				);
	...
	it.effect(
		"preserves a queued installer after a background updater error",
		() => {
			const harness = makeHarness();
	...
	it.effect(
		"logs bounded updater failure context without exposing the cause",
		() => {
			const cause = new Error(
				"request failed for https://user:secret@example.com/update?token=secret",
			);
			...
			const harness = makeHarness({
				checkForUpdates: Effect.fail(updaterError),
			});
	...
	it.effect(
		"restores download state and permits retry after interruption",
		() =>
			Effect.gen(function* () {
				...
				const harness = makeHarness({
					setDisableDifferentialDownload: Effect.suspend(() => {
						...
						if (disableDifferentialCalls === 2) {
							return Deferred.succeed(actionStarted, undefined).pipe(
				...
				yield* Effect.scoped(
					Effect.gen(function* () {
						...
						const downloadFiber = yield* updates.download.pipe(
							Effect.forkScoped,
						);
	...
	it.effect(
		"clears quitting state after an unexpected install setup failure",
		() => {
			const harness = makeHarness({
	...
	it.effect(
		"keeps windows and restarts backends when quitAndInstall fails",
		() => {
			const harness = makeHarness({
	...
	it.effect(
		"holds the install reservation until failed-install recovery finishes",
		() => {
	...
	it.effect(
		"recovers when quitAndInstall reports failure through an updater event",
		() => {
			const harness = makeHarness();
	...
	it.effect(
		"rejects a prepared install when the downloaded version changed",
		() => {
			const harness = makeHarness();
	...
	it.effect(
		"does not persist an unchanged update channel as a user preference",
		() => {
			const harness = makeHarness();
	...
	it.effect(
		"fails channel changes with a typed error while a check is in progress",
		() =>
			Effect.gen(function* () {
				const checkStarted = yield* Deferred.make<void>();
				const releaseCheck = yield* Deferred.make<void>();
				...
				yield* Effect.scoped(
					Effect.gen(function* () {
						...
						if (exit._tag === "Failure") {
							...
							assert.instanceOf(
								error,
								DesktopUpdates.DesktopUpdateActionInProgressError,
							);
	...
	it.effect(
		"rejects checks while an update channel change is being persisted",
		() =>
			Effect.gen(function* () {
				const channelChangeStarted = yield* Deferred.make<void>();
				const releaseChannelChange = yield* Deferred.make<void>();
				const harness = makeHarness({
					beforeSetUpdateChannel: Deferred.succeed(
						channelChangeStarted,
						undefined,
					).pipe(Effect.andThen(Deferred.await(releaseChannelChange))),
				});
				...
				yield* Effect.scoped(
					Effect.gen(function* () {
						...
						const channelFiber = yield* updates
							.setChannel("nightly")
							.pipe(Effect.forkScoped);
	...
	it.effect(
		"preserves settings failure context when an update channel cannot be persisted",
		() => {
			...
			return Effect.scoped(
				Effect.gen(function* () {
					...
					assert.instanceOf(
						error,
						DesktopUpdates.DesktopUpdateChannelPersistenceError,
					);
					...
					assert.equal(
						error.message,
						"Failed to persist the nightly desktop update channel.",
					);
