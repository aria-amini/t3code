...
const decodeDownloadProgressInfo =
	Schema.decodeUnknownEffect(DownloadProgressInfo);
...
export type DesktopUpdateSetChannelError =
	typeof DesktopUpdateSetChannelError.Type;
...
function parseAppUpdateYml(
	raw: string,
): Effect.Effect<Option.Option<AppUpdateYmlConfig>> {
	...
	return decodeAppUpdateYmlConfig(entries).pipe(
		Effect.map((config) =>
			config.provider ? Option.some(config) : Option.none(),
		),
...
function isArm64HostRunningIntelBuild(
...
export const make = Effect.gen(function* () {
	...
	const appUpdateYmlConfigRef = yield* Ref.make<
		Option.Option<AppUpdateYmlConfig>
	>(Option.none());
	const activeUpdateActionRef = yield* Ref.make<Option.Option<UpdateAction>>(
		Option.none(),
	);
	...
	const readAppUpdateYml = fileSystem
	...
	const tryStartChannelChange = Ref.modify(
		activeUpdateActionRef,
		(activeAction) =>
			Option.isSome(activeAction)
	...
	const finishUpdateAction = (action: UpdateAction): Effect.Effect<void> =>
		Ref.modify(activeUpdateActionRef, (activeAction) => {
			const finished =
				Option.isSome(activeAction) && activeAction.value === action;
			return [finished, finished ? Option.none() : activeAction] as const;
		}).pipe(
			Effect.flatMap((finished) =>
				finished
					? PubSub.publish(finishedUpdateActions, action).pipe(Effect.asVoid)
					: Effect.void,
			),
		);

	const applyAutoUpdaterChannel = Effect.fn(
		"desktop.updates.applyAutoUpdaterChannel",
	)(function* (channel: DesktopUpdateChannel) {
	...
	const checkForUpdates = Effect.fn("desktop.updates.checkForUpdates")(
		function* (
			...
			if (
				actionReservation === "acquire" &&
				!(yield* tryStartUpdateAction("check"))
			)
				return false;
		...
		return yield* Effect.gen(function* () {
			...
			Effect.catchTags({
				ElectronUpdaterDownloadUpdateError: Effect.fn(
					"desktop.updates.handleDownloadFailure",
				)(function* (error) {
					yield* updateState((current) =>
			...
			Effect.onInterrupt(() =>
				updateState((current) =>
					current.status === "downloading" ? state : current,
				).pipe(Effect.asVoid),
			),
			Effect.catchCause((cause) => {
				...
				const error = new DesktopUpdateUnexpectedActionError({
					action: "download",
					cause,
				});
	...
	const recoverFailedInstall = Effect.fn(
		"desktop.updates.recoverFailedInstall",
	)(function* (message: string) {
		const ownsRecovery = yield* Ref.modify(
			activeUpdateActionRef,
			(activeAction) =>
		...
		yield* Effect.gen(function* () {
			...
			const restartExit = yield* Effect.forEach(
				instances,
			...
			if (Exit.isFailure(restartExit)) {
				yield* logUpdaterError(
					"Desktop update install recovery could not restart every backend.",
				);
			}
		}).pipe(
			Effect.catchCause(() =>
	...
	const installDownloadedUpdate = (expectedVersion?: string) =>
		Effect.scoped(
			Effect.gen(function* () {
				const actionCompletions = yield* PubSub.subscribe(
					finishedUpdateActions,
				);
				let admission: "admitted" | "refused" | "wait-for-check" =
					"wait-for-check";
						...
						Effect.gen(function* () {
							...
							const hasExpectedDownload =
								...
								(expectedVersion === undefined ||
									state.downloadedVersion === expectedVersion);
							if (
							...
							if (Option.isSome(activeAction)) {
								return activeAction.value === "check" &&
									expectedVersion !== undefined
				...
				return yield* Effect.gen(function* () {
					...
					Effect.catchTags({
						ElectronUpdaterQuitAndInstallError: Effect.fn(
							"desktop.updates.handleInstallFailure",
						)(function* (error) {
	...
	const startUpdatePollers: Effect.Effect<void, never, Scope.Scope> =
		Effect.gen(function* () {
			yield* Effect.sleep(AUTO_UPDATE_STARTUP_DELAY).pipe(
				...
				Effect.catchCause((cause) => {
					...
					const error = new DesktopUpdatePollerError({
						poller: "startup",
						cause,
					});
	...
	const handleUpdateAvailable = Effect.fn(
		"desktop.updates.handleUpdateAvailable",
		...
		yield* decodeUpdateInfo(raw).pipe(
			...
			Effect.catchCause((cause) => {
				...
				const error = new DesktopUpdateEventHandlingError({
	...
	const handleUpdaterError = Effect.fn("desktop.updates.handleUpdaterError")(
		function* (cause: unknown) {
			...
			const error = new DesktopUpdaterReportedError({
				operation: Option.match(activeAction, {
					...
					onSome: (action) =>
						action === "install-recovery" ? "install" : action,
				}),
				cause,
			});
	...
	const handleDownloadProgress = Effect.fn(
		"desktop.updates.handleDownloadProgress",
	)(function* (raw: unknown) {
		yield* decodeDownloadProgressInfo(raw).pipe(
			Effect.flatMap(
				Effect.fn("desktop.updates.applyDownloadProgress")(
					function* (progress) {
						const state = yield* Ref.get(updateStateRef);
						const percent = Math.floor(progress.percent);
						if (
							shouldBroadcastDownloadProgress(state, progress.percent) ||
							state.message !== null
						) {
							yield* setState(
								reduceDesktopUpdateStateOnDownloadProgress(
						...
						const lastLoggedMilestone = yield* Ref.get(
							lastLoggedDownloadMilestoneRef,
						);
	...
	const handleUpdateDownloaded = Effect.fn(
		"desktop.updates.handleUpdateDownloaded",
	)(function* (raw: unknown) {
		yield* decodeUpdateInfo(raw).pipe(
			Effect.flatMap(
	...
	return DesktopUpdates.of({
		...
		isInstallActive: activeUpdateAction.pipe(
			Effect.map(
				(action) => Option.isSome(action) && action.value === "install",
			),
		),
		...
		setChannel: Effect.fn("desktop.updates.setChannel")(function* (
			...
			if (Option.isSome(activeAction)) {
				return yield* new DesktopUpdateActionInProgressError({
					action:
						activeAction.value === "install-recovery"
							? "install"
							: activeAction.value,
			...
			return yield* Effect.gen(function* () {
				...
				yield* desktopSettings.setUpdateChannel(nextChannel).pipe(
					Effect.mapError(
						(cause) =>
							new DesktopUpdateChannelPersistenceError({
								channel: nextChannel,
								cause,
							}),
					),
		...
		install: installWithExpectedVersion().pipe(
			Effect.map(({ accepted, completed, state }) => ({
				accepted,
				completed,
				state,
			})),
		),
		installPrepared: (expectedVersion) =>
			installWithExpectedVersion(expectedVersion),
	});
});
