...
function isElectronReleaseNoteInfo(
	value: unknown,
): value is ElectronReleaseNoteInfo {
...
function decodeHtmlEntities(input: string): string {
	return input.replace(
		...
		(_, entity: string) => decodeHtmlEntity(entity),
	);
}
...
function extractReleaseNoteItems(
	note: string | null | undefined,
): ExtractedReleaseNoteItems {
...
export function normalizeDesktopUpdateReleaseNotes(
	...
	return {
		...
		omittedReleaseCount: Math.max(
			0,
			normalizedNotes.length - MAX_RELEASE_NOTE_GROUPS,
		),
	};
}
