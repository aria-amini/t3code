...
describe("nextRemoteDesktopUpdateStep", () => {
	it("fails immediately when updates are disabled, preferring the known reason", () => {
		...
		assert.deepEqual(
			nextRemoteDesktopUpdateStep(state, NO_ATTEMPTS, "dev build"),
			{
				action: "done",
				outcome: "failed",
				reason: "dev build",
	...
	it("installs only from the downloaded status the updater will accept", () => {
		...
		assert.deepEqual(
			nextRemoteDesktopUpdateStep(staleError, NO_ATTEMPTS, null),
			{
				action: "check",
			},
		);
	...
	it("downloads an available update until the attempt cap, then fails", () => {
		const available = makeState({
			status: "available",
			availableVersion: "1.2.4",
		});
		assert.deepEqual(
			nextRemoteDesktopUpdateStep(available, NO_ATTEMPTS, null),
			{
				action: "download",
			},
		);
	...
	it("checks from idle until the attempt cap, then fails", () => {
		assert.deepEqual(
			nextRemoteDesktopUpdateStep(makeState(), NO_ATTEMPTS, null),
			{
				action: "check",
			},
		);
	...
	it("drops blank reasons so the wire report still encodes", () => {
		...
		assert.equal(normalizeRemoteUpdateReason("   "), undefined);
		assert.equal(
			normalizeRemoteUpdateReason("  feed unreachable  "),
			"feed unreachable",
		);
	});
});
