...
export function nextRemoteDesktopUpdateStep(
	...
	if (!state.enabled || state.status === "disabled") {
		return {
			...
			outcome: "failed",
			reason:
				disabledReason ?? "Automatic updates are disabled on this machine.",
		};
	}
	...
	if (state.status === "available") {
		if (attempts.downloads >= MAX_REMOTE_UPDATE_DOWNLOADS) {
			return {
				...
				outcome: "failed",
				reason:
					state.message ?? "The desktop app failed to download the update.",
			};
		}
 ...
 */
export function normalizeRemoteUpdateReason(
