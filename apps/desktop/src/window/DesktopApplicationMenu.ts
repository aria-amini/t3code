...
const dispatchMenuAction = Effect.fn("desktop.menu.dispatchMenuAction")(
	function* (
		action: string,
	): Effect.fn.Return<
		void,
...
const handleCheckForUpdatesMenuClick = Effect.gen(function* () {
	...
	if (Option.isSome(disabledReason)) {
		yield* logUpdaterInfo(
			"manual update check requested, but updates are disabled",
			{
...
export const make = Effect.gen(function* () {
	...
	const context =
		yield* Effect.context<DesktopApplicationMenuRuntimeServices>();
