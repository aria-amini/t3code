...
const desktopClientSettingsLayer = Layer.mock(
...
const desktopServerExposureLayer = Layer.succeed(
	DesktopServerExposure.DesktopServerExposure,
	{
		...
		setTailscaleServeEnabled: () =>
			Effect.die("unexpected setTailscaleServeEnabled"),
...
function makeTestLayer(input: {
	...
	let desktopSettings =
		input.desktopSettings ?? DesktopAppSettings.DEFAULT_DESKTOP_SETTINGS;
	const desktopAppSettingsLayer = Layer.succeed(
		DesktopAppSettings.DesktopAppSettings,
		{
			...
			setMainWindowBounds: (bounds, isMaximized) =>
				Effect.gen(function* () {
					...
					const changed =
						...
						!desktopWindowBoundsEquivalence(
							desktopSettings.mainWindowBounds,
							bounds,
						) ||
			...
			setServerExposureMode: () =>
				Effect.die("unexpected server exposure update"),
			...
			applyWslWindowsFallbackInMemory: Effect.die(
				"unexpected WSL Windows fallback",
			),
			...
			Layer.mergeAll(
				...
				Layer.mock(PreviewManager.PreviewManager)({
					...
					isBrowserPartition: (partition) =>
						partition.startsWith("persist:t3code-preview-"),
					getBrowserPartition: () =>
						Effect.succeed("persist:t3code-preview-test"),
					reapplyZoom: () =>
						Effect.sync(() => {
							input.previewZoomReapplies?.push(
								input.window.webContents.getZoomLevel(),
							);
						}),
				}),
			),
...
const makeSplashScenario = (
	...
	Effect.gen(function* () {
		...
		const electronWindowShape = {
			...
			prepareReveal: () => Effect.succeed(false),
			reveal: (window) =>
				Ref.update(revealedWindows, (windows) => [...windows, window]),
			sendAll: () => Effect.void,
			destroyAll: Effect.void,
			syncAllAppearance: (sync) =>
				fallbackWindow ? sync(fallbackWindow) : Effect.void,
		} satisfies ElectronWindow.ElectronWindow["Service"];
...
const captureOne = DesktopSnapShotId.make(
	"11111111-1111-4111-8111-111111111111",
);
const captureTwo = DesktopSnapShotId.make(
	"22222222-2222-4222-8222-222222222222",
);

describe("DesktopWindow", () => {
	it.effect(
		"shows native context menus for browser guests and sign-in popups",
		() =>
			Effect.gen(function* () {
				...
				const layer = makeTestLayer({
					...
					mainWindow: yield* Ref.make<Option.Option<Electron.BrowserWindow>>(
						Option.none(),
					),
					copiedTexts,
				...
				yield* Effect.gen(function* () {
					...
					for (const [contents, owner] of [
						...
						const correction = menu.input.template.find(
						...
						assert.deepEqual(contents.replaceMisspelling.mock.calls, [
							["hello"],
						]);
						...
						const copyImage = imageMenu.template.find(
							(item) => item.label === "Copy Image",
						);
						const copyLink = imageMenu.template.find(
							(item) => item.label === "Copy Link",
						);
	...
	it("restores bounds only when the window fits within a connected display", () => {
		...
		assert.deepEqual(
			DesktopWindow.resolveInitialMainWindowBounds(persistedBounds, [
				displays[0]!,
			]),
	...
	it.effect(
		"does not open a development window until the backend is ready",
		() =>
			Effect.gen(function* () {
				const fakeWindow = makeFakeBrowserWindow();
				const createCount = yield* Ref.make(0);
				const mainWindow = yield* Ref.make<
				...
				yield* Effect.gen(function* () {
					...
					assert.isFalse(
						createdWindowOptions[0]?.webPreferences?.backgroundThrottling,
					);
	...
	it.effect(
		"blocks only repeated Cmd+W input before it reaches the native window menu",
		() =>
			Effect.gen(function* () {
				const fakeWindow = makeFakeBrowserWindow();
				const createCount = yield* Ref.make(0);
				...
				yield* Effect.gen(function* () {
					...
					const beforeInput =
						fakeWindow.webContentsListeners.get("before-input-event");
					if (!beforeInput) {
						return yield* Effect.die(
							"before-input-event listener was not registered",
						);
					}
	...
	it.effect(
		"restores the preview browser's own zoom after zooming the app",
		() =>
			Effect.gen(function* () {
				const fakeWindow = makeFakeBrowserWindow();
				const createCount = yield* Ref.make(0);
	...
	it.effect(
		"uses the persisted main window bounds when opening the window",
		() =>
			Effect.gen(function* () {
				const fakeWindow = makeFakeBrowserWindow();
				const createCount = yield* Ref.make(0);
	...
	it.effect("restores the persisted maximized state", () =>
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				...
				if (!readyToShow) {
					return yield* Effect.die(
						"window ready-to-show listener was not registered",
					);
				}
				readyToShow();
	...
	it.effect("re-enables background throttling on first reveal", () =>
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				...
				readyToShow();
				assert.deepEqual(fakeWindow.setBackgroundThrottling.mock.calls, [
					[true],
				]);
			}).pipe(Effect.provide(layer));
		}),
	...
	it.effect("debounces move and resize bounds updates", () =>
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				...
				if (!move || !resize) {
					return yield* Effect.die(
						"window bounds listeners were not registered",
					);
				}
				...
				assert.deepEqual(mainWindowBoundsUpdates, [
					{ x: 160, y: 100, width: 1360, height: 900 },
				]);
			}).pipe(Effect.provide(layer));
		}),
	);
	...
	it.effect("persists normal bounds and state for a maximized window", () =>
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				...
				assert.deepEqual(mainWindowBoundsUpdates, [
					{ x: 220, y: 140, width: 1380, height: 920 },
				]);
	...
	it.effect(
		"persists normal bounds and state from the native maximize event",
		() =>
			Effect.gen(function* () {
				const fakeWindow = makeFakeBrowserWindow();
				const createCount = yield* Ref.make(0);
				...
				yield* Effect.gen(function* () {
					...
					if (!maximize) {
						return yield* Effect.die(
							"window maximize listener was not registered",
						);
					}
	...
	it.effect(
		"preserves unrestorable bounds until the user changes the window",
		() =>
			Effect.gen(function* () {
				const fakeWindow = makeFakeBrowserWindow();
				const createCount = yield* Ref.make(0);
				...
				yield* Effect.gen(function* () {
					...
					if (!close || !move) {
						return yield* Effect.die(
							"window lifecycle listeners were not registered",
						);
					}
					...
					yield* Effect.promise(() => Promise.resolve());
					assert.deepEqual(mainWindowBoundsUpdates, [
						{ x: 80, y: 60, width: 1280, height: 840 },
					]);
				}).pipe(Effect.provide(layer));
			}),
	);
	...
	it.effect(
		"flushes normal bounds when fullscreen before the debounce completes",
		() =>
			Effect.gen(function* () {
				const fakeWindow = makeFakeBrowserWindow();
				fakeWindow.getBounds.mockReturnValue({
				...
				yield* Effect.gen(function* () {
					...
					if (!resize) {
						return yield* Effect.die(
							"window resize listener was not registered",
						);
					}
					resize();
					yield* TestClock.adjust(250);
					...
					assert.deepEqual(mainWindowBoundsUpdates, [
						{ x: 200, y: 130, width: 1400, height: 940 },
					]);
					assert.equal(fakeWindow.getBounds.mock.calls.length, 0);
					assert.equal(fakeWindow.getNormalBounds.mock.calls.length, 1);
				}).pipe(Effect.provide(layer));
	...
	it.effect(
		"flushes normal bounds when minimized before the debounce completes",
		() =>
			Effect.gen(function* () {
				const fakeWindow = makeFakeBrowserWindow();
				fakeWindow.getBounds.mockReturnValue({
				...
				yield* Effect.gen(function* () {
					...
					assert.deepEqual(mainWindowBoundsUpdates, [
						{ x: 180, y: 120, width: 1440, height: 960 },
					]);
					assert.equal(fakeWindow.getBounds.mock.calls.length, 0);
					assert.equal(fakeWindow.getNormalBounds.mock.calls.length, 1);
				}).pipe(Effect.provide(layer));
	...
	it.effect(
		"logs display lookup failures before falling back to the default size",
		() =>
			Effect.gen(function* () {
				...
				const warning = logRecords.find(
					(record) =>
						...
						record.message[0] ===
							"failed to read connected displays; using defaults",
				);
	...
	it.effect(
		"persists the current main window bounds before the window closes",
		() =>
			Effect.gen(function* () {
				const fakeWindow = makeFakeBrowserWindow();
				fakeWindow.getBounds.mockReturnValue({
				...
				yield* Effect.gen(function* () {
					...
					if (!close) {
						return yield* Effect.die(
							"window close listener was not registered",
						);
					}
					close();
	...
	it.effect(
		"recovers when the development renderer is temporarily unreachable",
		() =>
			Effect.gen(function* () {
				const fakeWindow = makeFakeBrowserWindow();
				const createCount = yield* Ref.make(0);
				...
				yield* Effect.gen(function* () {
					...
					const didFailLoad =
						fakeWindow.webContentsListeners.get("did-fail-load");
					const didFinishLoad =
						fakeWindow.webContentsListeners.get("did-finish-load");
					if (!didFailLoad || !didFinishLoad) {
						return yield* Effect.die(
							"renderer load listeners were not registered",
						);
					}
	...
	it.effect(
		"opens safe off-origin renderer navigations in the system browser",
		() =>
			Effect.gen(function* () {
				const fakeWindow = makeFakeBrowserWindow();
				const createCount = yield* Ref.make(0);
				...
				yield* Effect.gen(function* () {
					...
					const willNavigate =
						fakeWindow.webContentsListeners.get("will-navigate");
					if (!willNavigate) {
						return yield* Effect.die(
							"will-navigate listener was not registered",
						);
					}
					let prevented = false;
					willNavigate(
	...
	it.effect(
		"does not dispatch menu actions to the splash before the backend is ready",
		() =>
			Effect.gen(function* () {
				const splash = makeFakeBrowserWindow();
				const main = makeFakeBrowserWindow();
	...
	it.effect(
		"dispatches menu actions after backend readiness when no main window exists",
		() =>
			Effect.gen(function* () {
				const splash = makeFakeBrowserWindow();
				const main = makeFakeBrowserWindow();
	...
	it.effect(
		"delivers capture completion and late errors without taking focus back",
		() =>
			Effect.gen(function* () {
				...
				yield* Effect.gen(function* () {
					...
					yield* desktopWindow.dispatchSnapShotEvent({
						type: "started",
						id: captureOne,
					});
	...
	it.effect("delivers the renderer event even when the reveal fails", () =>
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				...
				yield* Effect.exit(
					desktopWindow.dispatchSnapShotEvent({
						type: "started",
						id: captureOne,
					}),
				);
	...
	it.effect(
		"leaves a completed capture pending while only the connecting splash exists",
		() =>
			Effect.gen(function* () {
				const splash = makeFakeBrowserWindow();
				const scenario = yield* makeSplashScenario([splash.window]);
	...
	it.effect(
		"does not reopen a closed main window for a completed capture",
		() =>
			Effect.gen(function* () {
				const fakeWindow = makeFakeBrowserWindow();
				const createCount = yield* Ref.make(0);
	...
	it.effect(
		"waits for a loading renderer without foregrounding it when capture is ready",
		() =>
			Effect.gen(function* () {
				const fakeWindow = makeFakeBrowserWindow();
				const createCount = yield* Ref.make(0);
				...
				vi.mocked(
					fakeWindow.window.webContents.isLoadingMainFrame,
