...
export class DesktopWindow extends Context.Service<
	DesktopWindow,
	{
		...
		readonly ensureMain: Effect.Effect<
			...
			DesktopWindowError
		>;
		readonly revealOrCreateMain: Effect.Effect<
			Electron.BrowserWindow,
			DesktopWindowError
		>;
...
export function resolveInitialMainWindowBounds(
	...
	if (
		...
		displays.some((display) =>
			windowFitsWithinDisplay(persistedBounds, display),
		)
	) {
...
function buildConnectingSplashDataUrl(shouldUseDarkColors: boolean): string {
	...
	const track = shouldUseDarkColors
...
export function isSameOriginRendererNavigation(input: {
	...
	try {
		return (
			new URL(input.applicationUrl).origin ===
			new URL(input.navigationUrl).origin
		);
	} catch {
		return false;
	}
...
export const make = Effect.gen(function* () {
	...
	const splashWindowRef = yield* Ref.make<
	...
	const withoutSplash = (window: Option.Option<Electron.BrowserWindow>) =>
		Ref.get(splashWindowRef).pipe(
			Effect.map((splash) =>
				Option.isSome(splash) &&
				Option.isSome(window) &&
				window.value === splash.value
	...
	const createWindow = Effect.fn("desktop.window.createWindow")(
		function* (): Effect.fn.Return<Electron.BrowserWindow, DesktopWindowError> {
			...
			const restoredPersistedBounds =
				persistedBounds !== null && initialBounds === persistedBounds;
			if (
				persistedBounds !== null &&
				initialBounds === DesktopAppSettings.DEFAULT_MAIN_WINDOW_SIZE
			) {
				yield* logWindowWarning(
					"saved main window bounds could not be restored; using defaults",
				);
			}
			...
			let boundsPersistenceEnabled =
				persistedBounds === null || restoredPersistedBounds;
			...
			const fallbackWindowBounds = boundsPersistenceEnabled
				? null
				: readPersistableBounds();
			...
			const persistCurrentBounds = (): Fiber.Fiber<void, never> | undefined => {
				...
				pendingBoundsPersistFiber = runFork(
					desktopSettings
						.setMainWindowBounds(bounds, window.isMaximized())
						.pipe(
							Effect.asVoid,
							Effect.catch((error) =>
			...
			const installContextMenu = (
				...
				contents.on("context-menu", (event, params) => {
					...
					if (
						Option.isSome(ElectronShell.parseSafeExternalUrl(params.linkURL))
					) {
						menuTemplate.push(
							{
								label: "Copy Link",
			...
			window.webContents.on("before-input-event", (event, input) => {
				...
				const modifier =
					environment.platform === "darwin" ? input.meta : input.control;
				if (
					modifier &&
					!input.alt &&
					!input.shift &&
				...
				runFork(
					Effect.gen(function* () {
						...
						const shouldRecover =
							recoverable &&
							!window.isDestroyed() &&
							rendererRecoveryTimestamps.length <
	...
	const dispatchRendererEvent = Effect.fn(
		"desktop.window.dispatchRendererEvent",
	)(function* (
		channel: string,
		payload: unknown,
		{ reveal = true }: { readonly reveal?: boolean } = {},
	) {
		const existingWindow = yield* reveal
			? focusedMainWindow
			: electronWindow.main;
		if (
			Option.isNone(existingWindow) &&
			(!reveal || !(yield* Ref.get(backendReadyRef)))
		)
			return;
		...
		const dispatch = reveal
			? send.pipe(
					Effect.andThen(
						electronWindow.reveal(targetWindow).pipe(Effect.ignoreCause),
					),
				)
			: send;
	...
	return DesktopWindow.of({
		...
		dispatchSnapShotEvent: Effect.fn("desktop.window.dispatchSnapShotEvent")(
			function* (event) {
