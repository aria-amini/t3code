...
function makeHarness(options?: {
	...
	const handler = makeQuitShortcutHandler({
		...
		getMode:
			options?.getMode ?? (() => Promise.resolve(options?.mode ?? "hold")),
...
describe("makeQuitShortcutHandler", () => {
	...
	it("quits when a completed hold goes quiet without release events", async () => {
		const harness = makeHarness();
		await harness.send(makeInput({}));
		await harness.holdFor(
			QUIT_HOLD_DURATION_MS + QUIT_HOLD_RELEASE_GRACE_MS * 2,
		);
	...
	it("uses control on non-mac platforms", async () => {
		...
		await harness.send(
			makeInput({ type: "keyUp", meta: false, control: true }),
		);
		expect(harness.quit).toHaveBeenCalledTimes(1);
	});
});
