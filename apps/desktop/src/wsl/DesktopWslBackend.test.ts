...
describe("DesktopWslBackend", () => {
	it.effect(
		"clears the stored preflight error when a registered WSL backend becomes ready",
		() => {
			...
			return Effect.gen(function* () {
				...
				assert.isFalse(
					yield* recordFailure({ reason: "Node.js not found", fatal: true }),
				);
				assert.deepEqual(
					yield* backend.lastPreflightError,
					Option.some("Node.js not found"),
				);
	...
	it.effect(
		"retries an unchanged WSL instance when it is idle after failed preflight",
		() => {
			let startCount = 0;
			const primary = makeStubInstance({
				id: DesktopBackendPool.PRIMARY_INSTANCE_ID,
