...
const resolveTargetInstanceId = (
	distro: string | null,
): DesktopBackendPool.BackendInstanceId =>
	DesktopBackendPool.BackendInstanceId(
		distro === null
			? WSL_DEFAULT_DISTRO_ID
			: `${WSL_INSTANCE_ID_PREFIX}${distro}`,
	);
...
const scanForWslPort = Effect.fn("desktop.wslBackend.scanForWslPort")(
	function* (
		startPort: number,
	...
	Effect.gen(function* () {
		...
		const findExistingWslInstance = pool.list.pipe(
			Effect.map((instances) =>
				instances.find((instance) => isWslInstanceId(instance.id)),
			),
			...
			pool.unregister(id).pipe(
				Effect.catchTags({
					DesktopBackendPoolCannotUnregisterPrimaryError: (cause) =>
						...
						logWslBackendWarning(
							"refusing to unregister primary as wsl instance",
							{
								id,
								error: cause.message,
							},
		...
		const startNew = Effect.fn("desktop.wslBackend.startNew")(
			function* (input: { readonly distro: string | null }) {
				...
				const instance = yield* pool
					...
					.pipe(
						...
						Effect.catch((error) =>
							logWslBackendWarning(
								"WSL backend already registered, skipping start",
								{
									id: targetId,
									error: error.message,
								},
		...
		const reconcileBody = Effect.gen(function* () {
			...
			const shouldRun =
				settings.wslBackendEnabled && available && !settings.wslOnly;
			...
			if (
				...
				const isIdle =
					!snapshot.ready &&
					Option.isNone(snapshot.activePid) &&
					!snapshot.restartScheduled;
				if (isIdle) {
