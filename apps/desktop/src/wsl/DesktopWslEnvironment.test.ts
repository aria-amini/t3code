...
const posixShellRunner = (() => {
	...
	const probe = [
		"[ -d /proc/1 ] || exit 1",
		...REQUIRED_SHELL_TOOLS.map(
			(tool) => `command -v ${tool} >/dev/null || exit 1`,
		),
	].join("\n");
	return (
...
const runShell = (script: string) => {
	if (posixShellRunner === null)
		throw new Error("no POSIX shell runner available");
...
const readField = (stdout: string, field: string) => {
	...
	if (line === undefined)
		throw new Error(`missing ${field} in fixture output: ${stdout}`);
	return line.slice(field.length + 1).trim();
};

const SERVER_ENTRY_SOURCE = 'console.log("t3code wsl runtime test server");';

const makeDistroListSpawner = (result: {
...
describe("WSL runtime cache", () => {
	it.each([
		[
			"install",
			(id: string) =>
				buildWslRuntimeInstallScript("/runtime.tar.gz", id, "b".repeat(64)),
		],
	...
	it("installs through a temporary directory and only reuses valid completed caches", () => {
		...
		expect(script).toContain(
			'mktemp -d "$runtime_parent/.1.2.3-x64.tmp.XXXXXX"',
		);
		expect(script).toContain(
		...
		const readinessAfterLock = script.indexOf(
			"if runtime_is_ready; then",
			lockAcquired + 1,
		);
		const existingRuntimeMoved = script.indexOf(
			'mv -T "$runtime_root" "$runtime_stale"',
		);
	...
	it("moves an in-use runtime aside instead of deleting it under a live backend", () => {
		...
		const inUseChecked = script.indexOf(
	...
	it("re-hashes the server entry against the digest the install recorded", () => {
		...
		const markerWritten = script.indexOf(
			'> "$runtime_tmp/.t3code-wsl-runtime-ready"',
		);
	...
	it("refuses to mark an archive without a native payload as ready", () => {
		...
		const payloadValidated = script.indexOf(
			'node_pty_payload_present "$runtime_tmp"',
		);
		const markerWritten = script.indexOf(
			'> "$runtime_tmp/.t3code-wsl-runtime-ready"',
		);
	...
	it("parses only absolute Linux runtime paths", () => {
		expect(
			parseWslRuntimeRoot("runtimeRoot:/home/josh/.t3/wsl-runtime/1.2.3-x64\n"),
		).toBe("/home/josh/.t3/wsl-runtime/1.2.3-x64");
	...
	it("prunes completed runtimes except the current and newest previous cache", () => {
		...
		expect(script).toContain(
			'[ -f "$candidate/.t3code-wsl-runtime-ready" ] || continue',
		);
	...
	it("invalidates a cache by dropping its ready marker, not the tree", () => {
		...
		expect(script).toContain(
			'rm -f "$HOME/.t3/wsl-runtime/1.2.3_x64/.t3code-wsl-runtime-ready"',
		);
...
describe.skipIf(posixShellRunner === null)(
	"WSL runtime install script (executed)",
	() => {
		...
		const createFixture = () => {
			...
			return {
				...
				installScript,
				install: (archive?: string, sha?: string) =>
					runShell(installScript(archive, sha)),
			};
		};
		...
		it("reinstalls a cache whose server entry was truncated", () => {
			const fixture = createFixture();
			expect(fixture.install().status).toBe(0);
			expect(runShell(`set -eu\n: > ${sh(fixture.serverEntry)}`).status).toBe(
				0,
			);
...
describe("parseToolchainReport", () => {
	it("returns no missing tools and no node version on empty output", () => {
		expect(parseToolchainReport("")).toEqual({
			missingTools: [],
			nodeVersion: null,
		});
	});
...
describe("parseNodePath", () => {
	it("extracts the absolute node path from a nodePath: line", () => {
		...
		expect(parseNodePath(stdout)).toBe(
			"/home/josh/.nvm/versions/node/v22.16.0/bin/node",
		);
	});
...
describe("parseNodeVersion", () => {
	...
	it("returns null when there is no nodeVersion line at all", () => {
		expect(
			parseNodeVersion("nodePath:/usr/bin/node\nresolvedPath:/usr/bin"),
		).toBeNull();
	});
...
describe("formatMissingToolsReason", () => {
	it("returns null when everything is present and node is in range", () => {
		expect(
			formatMissingToolsReason(
				{ missingTools: [], nodeVersion: "24.10.0" },
				"^24.10",
			),
		).toBeNull();
	});
	...
	it("returns null when range is not specified and tools are present", () => {
		expect(
			formatMissingToolsReason(
				{ missingTools: [], nodeVersion: "18.0.0" },
				null,
			),
		).toBeNull();
	});
