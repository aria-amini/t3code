...
export class DesktopWslEnvironment extends Context.Service<
	DesktopWslEnvironment,
	{
		...
		readonly probeDistros: Effect.Effect<
			readonly WslDistro[],
			DesktopWslDistroListError
		>;
...
const sanitizeWslRuntimeId = (value: string): string =>
...
export const formatMissingToolsReason = (
	...
	const buildToolsMissing = report.missingTools.filter(
		(tool) => tool !== "node",
	);
...
> =>
	Effect.gen(function* () {
		...
		if (options.allowBuild !== true) {
			const packagedProbeFailure = formatNodePtyProbeFailureReason(
				probe.exitCode,
			);
		...
		const missingReason = formatMissingToolsReason(
			report,
			options.nodeEngineRange?.trim() || null,
		);
		...
		const buildTransportFailure = formatWslShellTransportFailureReason(
			build.transportFailure,
		);
...
const prepareWslRuntimeImpl = Effect.fn("desktop.wsl.prepareRuntimeImpl")(
	function* (
		distro: string | null,
		archive: WslRuntimeArchive,
		windowsToWslPath: (
		...
		ChildProcessSpawner.ChildProcessSpawner
	> {
		const linuxArchivePath = yield* windowsToWslPath(
			distro,
			archive.windowsPath,
		);
		...
		if (install.exitCode !== 0) {
			const trimmedTail = `${install.stdout}${install.stderr}`
				.trim()
				.slice(-500);
			return {
				ok: false,
		...
		return linuxAppRoot === null
			? {
					ok: false,
					reason:
						"WSL runtime installation completed without reporting its cache path.",
				}
			: { ok: true, linuxAppRoot };
	},
);

const pruneWslRuntimesImpl = Effect.fn("desktop.wsl.pruneRuntimesImpl")(
	function* (
		distro: string | null,
		runtimeId: string,
...
const invalidateWslRuntimeImpl = Effect.fn("desktop.wsl.invalidateRuntimeImpl")(
	function* (
		distro: string | null,
		runtimeId: string,
	): Effect.fn.Return<void, never, ChildProcessSpawner.ChildProcessSpawner> {
		...
		yield* Effect.logWarning(
			"Could not invalidate the staged WSL runtime cache.",
			{
				distro,
				runtimeId,
				detail: detail || `exit ${result.exitCode}`,
...
> =>
	Effect.scoped(
		Effect.gen(function* () {
			...
			const candidate = raw
				.split(/\s+/)
				.find((part) => IPV4_PATTERN.test(part));
	...
	return Layer.succeed(
		...
		DesktopWslEnvironment.of({
			...
			windowsToWslPath: (distro, windowsPath) =>
				Effect.succeed(
					stub.windowsToWslPath?.(distro, windowsPath) ?? Option.none(),
				),
			getUserHome: (distro) =>
				Effect.succeed(stub.getUserHome?.(distro) ?? Option.none<string>()),
			getDistroIp: (distro) =>
				Effect.succeed(stub.getDistroIp?.(distro) ?? Option.none<string>()),
			...
			pruneRuntimes: (distro, runtimeId) =>
				stub.pruneRuntimes?.(distro, runtimeId) ?? Effect.void,
...
export const layer = Layer.effect(
	...
	Effect.gen(function* () {
		...
		const wslAvailable = yield* makeIsAvailable(
		...
		const getDistroIp = (distro: string | null) =>
			provideSpawner(getDistroIpImpl(distro)).pipe(
		...
		return DesktopWslEnvironment.of({
			...
			probeDistros,
			preWarm: (distro) =>
				provideSpawner(preWarmImpl(distro)).pipe(
					Effect.withSpan("desktop.wsl.preWarm"),
				),
			windowsToWslPath,
			getUserHome,
			getDistroIp,
			prepareRuntime: (distro, archive) =>
				provideSpawner(
					prepareWslRuntimeImpl(distro, archive, windowsToWslPath),
