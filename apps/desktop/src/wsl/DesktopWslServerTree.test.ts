...
describe("DesktopWslServerTree", () => {
	it.effect("bounds entry work across an eight-way nested tree", () =>
		Effect.gen(function* () {
			...
			yield* DesktopWslServerTree.forEachBoundedTree(
				[{ depth: 0, id: "root" }],
				(node) =>
					Effect.acquireUseRelease(
						Effect.gen(function* () {
							...
							yield* Ref.update(maxActive, (maximum) =>
	...
	it.effect(
		"returns the server root unchanged when it is a plain directory (dev)",
		() =>
			withTempDir((tempDir) =>
				Effect.gen(function* () {
	...
	it.effect(
		"extracts an archive root into a version-keyed state directory",
		() =>
			withTempDir((tempDir) =>
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
					...
					yield* fileSystem.makeDirectory(
						path.join(serverRoot, "apps/server/dist"),
						{
							recursive: true,
						},
					);
					...
					yield* fileSystem.makeDirectory(
						path.join(serverRoot, "node_modules/effect"),
						{
							recursive: true,
						},
					);
					...
					const dep = yield* fileSystem.exists(
	...
	it.effect(
		"serializes concurrent extraction callers and publishes one complete tree",
		() =>
			withTempDir((tempDir) =>
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
					...
					const results = yield* Effect.gen(function* () {
						const tree = yield* DesktopWslServerTree.DesktopWslServerTree;
						return yield* Effect.all([tree.ensure, tree.ensure], {
						...
						Effect.provide(
							DesktopWslServerTree.layer.pipe(
								Layer.provideMerge(
									environmentLayer({ baseDir: tempDir, resourcesPath }),
								),
							),
						),
					);
					...
					assert.equal(
						yield* fileSystem.readFileString(
							path.join(roots[0] ?? "", "apps/server/dist/bin.mjs"),
						),
						"server-entry",
					);
				}),
	...
	it.effect(
		"sweeps stale version directories and leftover partials after extraction",
		() =>
			withTempDir((tempDir) =>
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
					...
					yield* fileSystem.makeDirectory(path.join(treeRoot, "1.0.0"), {
						recursive: true,
					});
					yield* fileSystem.makeDirectory(
						path.join(treeRoot, "1.2.3.partial"),
						{ recursive: true },
					);
					...
					assert.isTrue(result.ok);
					assert.isFalse(
						yield* fileSystem.exists(path.join(treeRoot, "1.0.0")),
					);
					assert.isFalse(
						yield* fileSystem.exists(path.join(treeRoot, "1.2.3.partial")),
					);
	...
	it.effect(
		"removes the legacy Windows extraction tree without preparing a fallback",
		() =>
			withTempDir((tempDir) =>
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
					...
					const resourcesPath = path.join(tempDir, "resources");
					const treeRoot = path.join(tempDir, "userdata", "wsl-server-tree");
					yield* fileSystem.makeDirectory(path.join(treeRoot, "1.2.3"), {
						recursive: true,
					});
					yield* fileSystem.writeFileString(
						path.join(treeRoot, "1.2.3", "legacy"),
						"old",
					);
	...
	it.effect(
		"re-extracts after legacy cleanup partially deletes the completed tree",
		() =>
			withTempDir((tempDir) =>
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
					...
					yield* fileSystem.makeDirectory(path.dirname(sourceEntryPath), {
						recursive: true,
					});
					yield* fileSystem.writeFileString(
						sourceEntryPath,
					...
					const initial = yield* ensureWith({
						baseDir: tempDir,
						resourcesPath,
					});
					...
					const extractedEntryPath = path.join(
						versionDir,
						"apps/server/dist/bin.mjs",
					);
					...
					assert.isTrue(result.ok);
					assert.equal(
						yield* fileSystem.readFileString(extractedEntryPath),
						"fresh-server-entry",
					);
				}),
			).pipe(Effect.provide(NodeServices.layer)),
