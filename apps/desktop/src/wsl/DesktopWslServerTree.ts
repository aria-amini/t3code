...
export const make = Effect.gen(function* () {
	...
	const sweepStale = Effect.gen(function* () {
		...
		yield* Effect.forEach(
			...
			(entry) =>
				fs
					.remove(join(treeRoot, entry), { recursive: true })
					.pipe(Effect.ignore),
			{ discard: true },
		);
	});
	...
	const extract = Effect.gen(function* () {
		yield* Effect.log(
			`[wsl-server-tree] Extracting ${serverRoot} to ${versionDir}...`,
		);
		...
		yield* Effect.gen(function* () {
			...
			yield* fs.writeFileString(
				join(partialDir, MARKER_FILE_NAME),
				`${markerJson}\n`,
			);
			...
			Effect.ensuring(
				fs
					.remove(partialDir, { recursive: true, force: true })
					.pipe(Effect.ignore),
			),
		);
		yield* Effect.log(
			`[wsl-server-tree] Extraction complete at ${versionDir}.`,
		);
	}).pipe(
		Effect.mapError(
			(cause) =>
				new DesktopWslServerTreeExtractError({ targetDir: versionDir, cause }),
		),
	);
		...
		.withPermits(1)(
			needsExtraction
				? Effect.gen(function* () {
						...
						yield* fs.remove(join(versionDir, MARKER_FILE_NAME), {
							force: true,
						});
						...
						Effect.catch((cause) =>
							Effect.logWarning(
								"[wsl-server-tree] Could not remove the legacy extraction cache.",
								{
									treeRoot,
									cause,
								},
	...
	Layer.effect(
		...
		Effect.gen(function* () {
			...
			return DesktopWslServerTree.of({
				ensure: Effect.succeed(
					stub.result ?? { ok: true, root: environment.appRoot },
				),
