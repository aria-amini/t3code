...
export function parseWslDistroList(stdout: Buffer): readonly WslDistro[] {
	const hasUtf16Bom =
		stdout.length >= 2 && stdout[0] === 0xff && stdout[1] === 0xfe;
	const likelyUtf16Le =
		hasUtf16Bom ||
...
export function wslUncPathToLinuxPath(windowsPath: string): string | null {
	const match = /^\\\\(?:wsl\.localhost|wsl\$)\\([^\\]+)(?:\\(.*))?$/i.exec(
		windowsPath.trim(),
	);
	if (!match) return null;
...
function resolveWslHomeUncPath(
	...
	const distroName =
		config.distro ?? distros.find((distro) => distro.isDefault)?.name ?? null;
...
export function resolveWslPickFolderDefaultPath(
	...
	const normalizedUserHome =
		userHome && userHome.startsWith("/") ? userHome : null;
