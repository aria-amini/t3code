...
const scriptDirectory = NodePath.dirname(
...
const generatedModulePath = NodePath.join(
	moduleDirectory,
...
const customSprite = webIconSource.match(
	/const T3_FILE_ICON_SPRITE = `([\s\S]*?)`;/,
)?.[1];

if (!customSprite) {
	throw new Error(
		"Could not read the T3 Pierre icon sprite from apps/web/src/pierre-icons.ts",
	);
}
...
const builtInTokens = [
	...builtInSprite.matchAll(/<symbol id="file-tree-builtin-([^"]+)"/g),
]
	.map((match) => match[1])
	.sort();
...
for (const [token, symbolId] of Object.entries(customIcons)) {
	renderIcon(
		token,
		symbolFromSprite(customSprite, symbolId),
		colors[token] ?? colors.default,
	);
}

const tokens = [
	...new Set([...builtInTokens, ...Object.keys(customIcons)]),
].sort();
const generatedSource = `import type { ImageSourcePropType } from "react-native";\n\nexport const MARKDOWN_FILE_ICON_SOURCES = {\n${tokens
	.map(
		(token) =>
			`  ${token}: require("../assets/file-icons/pierre_${token}.png"),`,
	)
	.join(
		"\n",
