...
export const MarkdownFileContextMenuContext =
	createContext<MarkdownFileContextMenuHandlers | null>(null);
	...
	return {
		...
		...(hasParagraphStyle
			? {
					...
					shadowRadius:
						PARAGRAPH_STYLE_ENCODING_OFFSET + (run.paragraphSpacing ?? 0),
				}
			: {}),
	};
}
...
export function NativeMarkdownSelectableText(props: {
	...
	const containsInlineIcon = props.runs.some(
		(run) =>
			...
			(run.externalHost != null &&
				resolveMarkdownLinkIcon(run.externalHost) !== null),
	);
	...
	const keyedRuns = props.runs.map((run) => {
		...
		if (run.fileIcon && Platform.OS === "ios") {
			...
			run.externalHost &&
			run.href &&
			!prefixedExternalLinks.has(run.href)
		) {
	...
	return (
		...
		>
			{keyedRuns.map(({ key, run, text, linkIcon }) => {
				const href = run.href;
				const contextMenu =
					run.fileIcon && href ? menu?.fileContextMenu(href) : undefined;
				return (
					<MarkdownTextPrimitive
						key={key}
						nativeID={
						...
						contextMenuConfig={
							contextMenu ? JSON.stringify(contextMenu) : undefined
						}
						...
						onContextMenuAction={
							contextMenu && href && menu
								? (event) =>
										menu.onFileContextMenuAction(
											href,
											event.nativeEvent.actionIdentifier,
										)
								: undefined
						}
					>
						{Platform.OS === "android" && run.fileIcon ? (
							<Image
								source={markdownFileIconSource(run.fileIcon)}
