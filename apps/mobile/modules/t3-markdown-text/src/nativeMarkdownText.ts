...
function sameRunStyle(
	left: NativeMarkdownTextRun,
	right: NativeMarkdownTextRun,
): boolean {
	return (
...
function appendRun(
	...
	const run: NativeMarkdownTextRun = {
		...
		...(context.firstLineHeadIndent !== undefined
			...
			: {}),
		...(context.headIndent !== undefined
			? { headIndent: context.headIndent }
			: {}),
...
function appendNode(
	...
	switch (node.type) {
		...
		case "code_inline": {
			...
			const presentation = context.href
				? null
				: resolveMarkdownInlineCodePresentation(content);
			return presentation
...
export function nativeMarkdownTextRuns(
	node: MarkdownNode,
): ReadonlyArray<NativeMarkdownTextRun> {
	return appendChildren([], node, EMPTY_CONTEXT);
}

export function nativeMarkdownWithPreservedSoftBreaks(
	node: MarkdownNode,
): MarkdownNode {
...
export function nativeMarkdownListItemBlocks(
	node: MarkdownNode,
