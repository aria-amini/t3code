...
function withPopupMenuStyles(config) {
	return withAndroidStyles(config, (config) => {
		...
		if (appTheme) {
			...
			assignStyleItem(
				appTheme,
				"android:listMenuViewStyle",
				"@style/AppPopupMenuListMenuView",
			);
			...
			assignStyleItem(
				...
				"@style/AppPopupMenuCheckBox",
			);
			assignStyleItem(
				appTheme,
				"checkboxStyle",
				"@style/AppPopupMenuCheckBoxCompat",
			);
		}
		...
		resources.style.push(
			{
				...
				item: [
					{
						_: "@drawable/popup_menu_background",
			...
			{
				$: {
					name: "AppPopupMenu.TextAppearance",
					parent: "TextAppearance.AppCompat.Menu",
				},
				item: [
					{ _: "15sp", $: { name: "android:textSize" } },
					{
						_: "@color/popup_menu_item_text",
			...
			// subMenuArrow, so replacing the style wholesale is safe.
			{
				...
				item: [
					{
						_: "@drawable/popup_menu_submenu_arrow",
						$: { name: "android:subMenuArrow" },
					},
				],
			},
			{
				...
				item: [
					{
						_: "@drawable/popup_menu_check_button",
						$: { name: "android:button" },
					},
				],
			},
...
function withPopupMenuBackgroundDrawable(config) {
	return withDangerousMod(config, [
		...
		async (config) => {
			...
			fs.writeFileSync(
				path.join(drawableDir, "ic_menu_check.xml"),
				CHECK_DRAWABLE,
			);
			fs.writeFileSync(
...
module.exports = function withAndroidModernPopupMenu(config) {
	return withPopupMenuBackgroundDrawable(
		withPopupMenuColors(withPopupMenuStyles(config)),
	);
};
