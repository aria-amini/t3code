...
import {
	...
	ThreadFileScreen,
} from "./features/files/ThreadFilesRouteScreen";
...
function workspacePathFromState(state: NavigationState): string {
	const routes = state.routes.filter(
		(route) => !WORKSPACE_OVERLAY_ROUTES.has(route.name),
	);
...
function RootStackLayout(props: {
	...
	useEffect(() => {
		...
		const transition = transitionIncomingSharePresentation(
		...
		ThreadReviewComment: createNativeStackScreen({
			...
			options: {
				...
				...(Platform.OS === "android"
					...
					: FORM_SHEET_PRESENTATION_OPTIONS),
				sheetAllowedDetents:
					Platform.OS === "android" ? undefined : [0.55, 0.92],
