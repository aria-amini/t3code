...
export function ComposerAttachmentThumbnail(
	...
	const upload = useComposerAttachmentUploadState(
		props.environmentId,
		props.attachment.id,
	);
	return (
		<View style={{ width: props.size, height: props.size }}>
			...
			{upload && upload.status !== "ready" ? (
				<Pressable
					...
					accessibilityHint={
						upload.status === "failed" ? upload.reason : undefined
					}
 ...
 * container move.
 */
function useComposerImagePreviewUri(
	...
	useEffect(() => {
		...
		void (async () => {
			...
			const owned = resolveOwnedComposerAttachmentFileUri(
				fileUri,
				Paths.document.uri,
			);
			// Re-render only when the container actually moved.
			if (!cancelled && owned !== null && owned !== previewUri)
				setRebased({ fileUri, uri: owned });
		})();
		return () => {
			cancelled = true;
		};
	}, [fileUri, previewUri]);
	return fileUri !== undefined && rebased?.fileUri === fileUri
		? rebased.uri
		: previewUri;
}
...
function ComposerAttachmentContent(props: ComposerAttachmentThumbnailProps) {
	...
	const canPreview =
		isPdfFile(attachment) && props.onPressPreview !== undefined;
	...
	return (
		<PresentationSource identifier={sourceIdentifier}>
			...
			>
				...
				{!props.compact ? (
					<Text
						className="w-full text-center text-2xs text-foreground"
						numberOfLines={1}
					>
						{attachment.name}
					</Text>
