	...
	return (
		<Pressable
			...
			accessibilityState={
				props.static
					? undefined
					: { disabled: props.disabled, selected: props.selected }
			}
		...
		>
			{props.iconNode ? (
				...
				<SymbolView
					...
					size={16}
					tintColorClassName={
						props.emphasized || props.selected
							? "accent-icon"
							: "accent-icon-muted"
					}
					type="monochrome"
...
export function ComposerToolbarScroller(props: {
	...
	const scrollEdges = useMemo(() => {
		...
		return {
			...
			showRightFade:
				metrics.offsetX < maxOffset - COMPOSER_TOOLBAR_SCROLL_EPSILON,
		};
	}, [metrics]);
	...
	return (
		<Pressable
			...
			className={cn(
				...
				variant === "primary" &&
					"shadow-lg shadow-adaptive-black-a10-a25 disabled:shadow-none",
