...
function LoadedMediaVideo(props: {
	...
	const [loadState, setLoadState] = useState<"pending" | "complete" | "error">(
		"pending",
	);
	...
	const loadSource = useEffectEvent(async (signal: AbortSignal) => {
		const uri = props.resolvePlaybackUri
			? await props.resolvePlaybackUri()
			: props.uri;
	...
	useEffect(() => {
		active.current =
			focused && !props.paused && AppState.currentState === "active";
	...
	return (
		<View collapsable={false} style={{ flex: 1 }}>
			...
			/>
			{loadState === "error" ||
			(loadState === "complete" && status === "error") ? (
...
function MediaVideoPlayerContent(props: MediaVideoPlayerProps) {
	...
	const [playbackUri, setPlaybackUri] = useState<string | null>(
		props.expanded ? props.uri : null,
	);
	// Keep an opened player mounted while signing or reconnecting temporarily has no usable URL.
	if (playbackUri === null && props.expanded && props.uri !== null)
		setPlaybackUri(props.uri);
