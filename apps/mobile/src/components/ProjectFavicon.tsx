...
export function ProjectFavicon(props: {
	...
	const renderableFaviconUrl = isProjectFaviconFallbackUrl(faviconUrl)
		? null
		: faviconUrl;
...
function ProjectFaviconImage(props: {
	...
	const [activeFaviconRequest, setActiveFaviconRequest] =
		useState<typeof faviconRequest>(null);
	useLayoutEffect(() => {
	...
	const [status, setStatus] = useState<"loading" | "loaded" | "error">(() =>
		props.faviconUrl?.startsWith("data:") ||
		hasLoadedProjectFavicon(props.cacheKey)
			? "loaded"
			: "loading",
	);

	const requestIsActive =
		faviconRequest !== null && activeFaviconRequest === faviconRequest;
		...
		>
			...
			{!showImage ? (
				<SymbolView
					name={{
						ios: "folder.fill",
						android: props.open ? "folder_open" : "folder",
					}}
