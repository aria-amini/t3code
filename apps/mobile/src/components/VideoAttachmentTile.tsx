...
export function VideoAttachmentTile(props: {
	...
	const mediaActions = useMediaActions(
		props.disabled ? undefined : props.actionsSource,
	);
		...
		<PresentationSource
			...
			onAccessibilityAction={({ nativeEvent }) => {
				...
				mediaActions.actions
					.find(({ id }) => id === nativeEvent.actionName)
					?.run();
			}}
		>
			<MediaActionsMenu media={mediaActions}>
				<Pressable
					...
					accessibilityLabel={`Play ${props.name}`}
					accessibilityHint={
						hasActions ? "Touch and hold for media actions" : undefined
					}
					accessibilityState={{ disabled: props.disabled ?? false }}
					disabled={props.disabled}
					onPress={() => props.onPress(props.sourceIdentifier)}
					className={cn(
						"items-center justify-center overflow-hidden bg-black/80",
						props.className,
					)}
					style={props.style}
				>
					<VideoThumbnailImage
						cacheKey={props.sourceIdentifier}
