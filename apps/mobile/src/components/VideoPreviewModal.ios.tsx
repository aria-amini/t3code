...
function NativeVideoPreview(props: {
	...
	const environmentId =
		source.type === "media" && "environmentId" in source
			...
			: null;
	const resource =
		source.type === "media" && "resource" in source ? source.resource : null;
	const preparedConnection = usePreparedConnection(environmentId);
	const assetUrl = useAssetUrlState(environmentId, resource);
	const refreshAssetUrl = useEffectEvent(
		useRefreshAssetUrl(environmentId, resource),
	);
	...
	useEffect(() => {
		if (playbackUrl === null && resolvedUrl !== null)
