...
function useMediaPlayback(source: MediaVideoPreviewSource): PlaybackState {
	...
	return {
		...
		...(resource !== null
			? {
					resolvePlaybackUri: async () =>
						mediaVideoPreviewUri(source, await refreshAssetUrl()),
				}
			: {}),
		unavailable:
			uri === null &&
...
function useLocalPlayback(source: LocalVideoPreviewSource): PlaybackState {
	...
	useEffect(() => {
		...
		const loading = loadLocalAttachmentPreview(
	...
	return {
		...
		error,
		actionsSource:
			uri === null ? undefined : { name: attachment.name, mimeType, uri },
	};
}
		...
		>
			...
			>
				<View className="min-h-14 flex-row items-center gap-3 pl-4 pr-2">
					<AppText
						className="flex-1 font-t3-medium text-base text-white"
						numberOfLines={2}
					>
						{props.name}
					</AppText>
