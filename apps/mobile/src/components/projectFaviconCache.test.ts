...
describe("project favicon cache", () => {
	it("ignores callbacks from a superseded URL", () => {
		...
		const expiredUrl =
			"https://environment.example/api/assets/expired/v1-favicon.svg";
		const refreshedUrl =
			"https://environment.example/api/assets/refreshed/v1-favicon.svg";
		...
		const refreshedRequest = createProjectFaviconRequest(
			cacheKey,
			refreshedUrl,
		);
	...
	it("evicts the URL that actually failed", () => {
		...
		const faviconUrl =
			"https://environment.example/api/assets/current/v2-favicon.svg";
	...
	it("does not supersede a request until the next request begins", () => {
		...
		const committedUrl =
			"https://environment.example/api/assets/current/v3-favicon.svg";
		const abandonedUrl =
			"https://environment.example/api/assets/abandoned/v3-favicon.svg";
		const committedRequest = createProjectFaviconRequest(
			cacheKey,
			committedUrl,
		);
	...
	it("requires a cache key before creating a URL-bearing request", () => {
		...
		const secondUrl =
			"https://environment.example/api/assets/second/favicon.svg";
