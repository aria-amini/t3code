...
export function beginProjectFaviconRequest(request: ProjectFaviconRequest) {
	...
	return () => {
		...
		const remainingCount =
			(activeRequests.urls.get(request.faviconUrl) ?? 1) - 1;
		if (remainingCount > 0) {
		...
		if (activeRequests.currentUrl === request.faviconUrl) {
			activeRequests.currentUrl = Array.from(activeRequests.urls.keys()).at(
				-1,
			)!;
		}
	};
...
function isCurrentProjectFaviconRequest(request: ProjectFaviconRequest) {
	return (
		activeFaviconRequests.get(request.cacheKey)?.currentUrl ===
		request.faviconUrl
	);
}
