...
const decodeConnectionCatalogDocument = Schema.decodeEffect(
...
const decodeCatalog = Effect.fn("mobile.connectionStorage.decodeCatalog")(
	function* (raw: string) {
...
const encodeCatalog = Effect.fn("mobile.connectionStorage.encodeCatalog")(
...
export const make = Effect.fn("mobile.connectionStorage.makeCatalogStore")(
	function* () {
		const storage = yield* MobileSecureStorage.MobileSecureStorage;
		const getItem = (key: string) =>
			storage
				.getItem(key)
				.pipe(Effect.mapError((cause) => catalogError("load", cause)));
		...
		const deleteItem = (key: string) =>
			storage
				.removeItem(key)
				.pipe(Effect.mapError((cause) => catalogError("delete", cause)));
		const state = yield* Ref.make<Option.Option<ConnectionCatalogDocumentType>>(
			Option.none(),
		);
		const lock = yield* Semaphore.make(1);

		const loadLegacyCatalog = Effect.fn(
			"mobile.connectionStorage.loadLegacyCatalog",
		)(function* () {
			...
			const catalog =
				legacyRaw === null || legacyRaw.trim() === ""
					...
					: yield* migrateLegacyConnectionCatalog(legacyRaw).pipe(
							...
							Effect.catch((error) =>
								Effect.logWarning(
									"Discarding corrupt legacy mobile connections",
									error,
		...
		const loadUnlocked = Effect.fn("mobile.connectionStorage.loadCatalog")(
			function* () {
				...
				if (raw !== null && raw.trim() !== "") {
					catalog = yield* decodeCatalog(raw).pipe(
						Effect.catch((error) =>
							Effect.logWarning(
								"Discarding corrupt mobile connection catalog",
								error,
							).pipe(
		...
		const update: CatalogStore["update"] = Effect.fn(
			"mobile.connectionStorage.updateCatalog",
		)(function* (transform) {
