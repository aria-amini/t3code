		...
		loadCache: (environmentId, kind, cacheKey) =>
			Effect.succeed(
				Option.fromUndefinedOr(
					values.get(cacheId(environmentId, kind, cacheKey)),
				),
			),
		listCache: (kind) =>
			Effect.sync(() =>
...
describe("mobile SQLite environment cache store", () => {
	it.effect("round-trips schema-validated VCS refs", () =>
		Effect.gen(function* () {
			...
			const store = yield* make().pipe(
				Effect.provideService(MobileDatabase, memory.database),
			);

			yield* store.saveVcsRefs(ENVIRONMENT_ID, "/repo", REFS);

			expect(yield* store.loadVcsRefs(ENVIRONMENT_ID, "/repo")).toEqual(
				Option.some(REFS),
			);
		}),
	);
	...
	it.effect("removes one persisted VCS ref snapshot", () =>
		Effect.gen(function* () {
			...
			expect(memory.removed).toContain(
				cacheId(ENVIRONMENT_ID, "vcs-refs", "/repo"),
			);
		}),
	);
	...
	it.effect("clears every persisted VCS ref snapshot in one environment", () =>
		Effect.gen(function* () {
			...
			expect(
				yield* store.loadVcsRefs(ENVIRONMENT_ID, "/repo-worktree"),
			).toEqual(Option.none());
			expect(yield* store.loadVcsRefs(otherEnvironmentId, "/repo")).toEqual(
				Option.some(REFS),
			);
		}),
	);
