...
const decodeStoredVcsRefs = Schema.decodeUnknownEffect(
	Schema.fromJsonString(StoredVcsRefs),
);
const encodeStoredVcsRefs = Schema.encodeEffect(
	Schema.fromJsonString(StoredVcsRefs),
);
...
function mapDatabaseError(operation: CacheOperation) {
	return (error: MobileDatabase.MobileDatabaseError) =>
...
function loadDecodedCache<A, B>(input: {
	...
	return input.database
		.loadCache(input.environmentId, input.kind, input.cacheKey)
		.pipe(
				...
				Option.match({
					...
					onSome: (raw) =>
						input.decode(raw).pipe(
							...
							Effect.catch((cause) =>
								Effect.logWarning(
									"Discarding corrupt mobile client cache record.",
									{
...
export const make = Effect.fn("MobileEnvironmentCacheStore.make")(function* () {
	...
	return EnvironmentCacheStore.of({
		...
		saveShell: Effect.fn("MobileEnvironmentCache.saveShell")(
			function* (environmentId, snapshot) {
		...
		loadThread: Effect.fn("MobileEnvironmentCache.loadThread")(
		...
		saveThread: Effect.fn("MobileEnvironmentCache.saveThread")(
			function* (environmentId, snapshot) {
		...
		removeThread: Effect.fn("MobileEnvironmentCache.removeThread")(
			(environmentId, threadId) =>
				database
					.removeCache(environmentId, "thread", threadId)
					.pipe(Effect.mapError(mapDatabaseError("remove-thread"))),
		),
		loadServerConfig: Effect.fn("MobileEnvironmentCache.loadServerConfig")(
			(environmentId) =>
				loadDecodedCache({
					database,
					environmentId,
		...
		loadVcsRefs: Effect.fn("MobileEnvironmentCache.loadVcsRefs")(
			(environmentId, cwd) =>
				loadDecodedCache({
					database,
					environmentId,
		...
		saveVcsRefs: Effect.fn("MobileEnvironmentCache.saveVcsRefs")(
			function* (environmentId, cwd, refs) {
				const payload = yield* encodeStoredVcsRefs({
					...
					Effect.mapError((cause) => persistenceError("save-vcs-refs", cause)),
				);
				yield* database
					.saveCache(
						environmentId,
						"vcs-refs",
						cwd,
						VCS_REFS_CACHE_SCHEMA_VERSION,
						payload,
					)
					.pipe(Effect.mapError(mapDatabaseError("save-vcs-refs")));
			},
		),
		removeVcsRefs: Effect.fn("MobileEnvironmentCache.removeVcsRefs")(
			(environmentId, cwd) =>
				database
					.removeCache(environmentId, "vcs-refs", cwd)
					.pipe(Effect.mapError(mapDatabaseError("remove-vcs-refs"))),
		),
		clearVcsRefs: Effect.fn("MobileEnvironmentCache.clearVcsRefs")(
			(environmentId) =>
				database
