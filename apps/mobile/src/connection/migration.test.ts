...
describe("migrateLegacyConnectionCatalog", () => {
	it.effect(
		"migrates bearer and relay-managed connections into the new catalog",
		() =>
			Effect.gen(function* () {
				...
				expect(
					catalog.targets.find(
						(target) => target.environmentId === bearerEnvironmentId,
					)?._tag,
				).toBe("BearerConnectionTarget");
				expect(
					catalog.targets.find(
						(target) => target.environmentId === relayEnvironmentId,
					)?._tag,
