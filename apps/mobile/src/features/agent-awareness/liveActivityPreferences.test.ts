...
describe("liveActivityPreferences", () => {
	...
	it.effect(
		"pushes disabled Live Activity preferences to relay registrations",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"pushes enabled Live Activity preferences to relay registrations",
		() =>
			Effect.gen(function* () {
				yield* setLiveActivityUpdatesEnabled({
					enabled: true,
	...
	it.effect(
		"does not try to re-link managed relay connections without bearer credentials",
		() => {
	...
	it.effect(
		"restores relay preferences when an environment update fails",
		() => {
			...
			return Effect.gen(function* () {
				...
				expect(linkEnvironmentToCloudWithPreference).toHaveBeenNthCalledWith(
					1,
					{
						clerkToken: "clerk-token",
						connection,
