...
describe("extractAgentNotificationDeepLink", () => {
	...
	it("ignores malformed or external links", () => {
		expect(
			extractAgentNotificationDeepLink(
				responseWithData({ deepLink: "https://example.com" }),
			),
		).toBeNull();
		expect(
			extractAgentNotificationDeepLink(
				responseWithData({ deepLink: "/settings" }),
			),
		).toBeNull();
		expect(
			extractAgentNotificationDeepLink(
				responseWithData({ deepLink: "//example.com" }),
			),
		).toBeNull();
		expect(
			extractAgentNotificationDeepLink(
				responseWithData({ deepLink: "/threads/env/thread?x=1" }),
			),
		).toBeNull();
