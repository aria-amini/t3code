	...
	digest: (algorithm: string, data: unknown) => {
		if (!(data instanceof Uint8Array)) {
			return Promise.reject(
				new TypeError("expo-crypto digest data must be a typed array."),
			);
		}
		return Promise.resolve(
			new Uint8Array(NodeCrypto.createHash(algorithm).update(data).digest())
				.buffer,
		);
	},
}));
...
vi.mock("../../persistence/imperative", () => ({
	...
	loadPreferences: vi.fn(() =>
		Promise.resolve({ liveActivitiesEnabled: false }),
	),
	loadAgentAwarenessRegistrationRecord: vi.fn(() =>
		Promise.resolve(registrationRecordStore.current),
	),
	saveAgentAwarenessRegistrationRecord: vi.fn(
		(record: { identity: string; signature: string }) => {
...
const relayTestLayer = managedRelayClientLayer(
	"https://relay.example.test",
).pipe(Layer.provide(Layer.mergeAll(FetchHttpClient.layer, cryptoLayer)));

const runBackgroundOperations = Effect.fn(
	"TestRemoteRegistration.runBackgroundOperations",
)(function* () {
	let idlePasses = 0;
	for (;;) {
...
describe("makeRelayDeviceRegistrationRequest", () => {
	...
	it.effect(
		"registers at most one listener while a Live Activity push token is pending",
		() => {
			...
			return Effect.gen(function* () {
				expect(
					yield* registerLiveActivityPushToken({ activity: activity as never }),
				).toBe(false);
				expect(
					yield* registerLiveActivityPushToken({ activity: activity as never }),
				).toBe(false);
	...
	it.effect("preserves Live Activity push-token lookup failures", () => {
		...
		return Effect.gen(function* () {
			...
			expect(error).toMatchObject({
				...
				cause,
				message:
					"Agent awareness operation read-live-activity-push-token failed.",
			});
		}).pipe(Effect.provide(relayTestLayer));
	});
	...
	it.effect(
		...
		() => {
			...
			setAgentAwarenessRelayTokenProvider(() =>
				Promise.resolve("clerk-token-user-a"),
			);
	...
	it.effect(
		"refreshes APNs registration for connected environments after settings changes",
		() => {
			registerAgentAwarenessConnection(savedConnection());
			return Effect.gen(function* () {
				yield* runBackgroundOperations();
	...
	it.effect(
		"registers the APNs device when cloud auth becomes available",
		() => {
	...
	it.effect(
		"marks registration failed when device registration cannot complete",
		() => {
			Constants.expoConfig!.extra = {
				relay: {
					url: "https://relay.example.test/",
	...
	it.effect(
		"resets a pending status to unknown when relay config is missing",
		() => {
	...
	it.effect(
		"does not re-register the same account when nothing has changed",
		() => {
			Constants.expoConfig!.extra = {
				relay: {
					url: "https://relay.example.test/",
	...
	it.effect(
		"dedupes rapid activity-token re-registrations within the replay window",
		() => {
	...
	it.effect(
		"coalesces simultaneous sign-in and environment connection registrations",
		() => {
			const fetchMock = vi.fn((request: RequestInfo | URL) => {
				const url = request instanceof Request ? request.url : String(request);
				return Promise.resolve(
	...
	it.effect(
		"continues queued device registration after a failed auth lookup",
		() => {
			Constants.expoConfig!.extra = {
				relay: {
					url: "https://relay.example.test/",
	...
	it("only registers again when the authenticated identity changes", () => {
		expect(shouldRegisterAgentAwarenessDeviceForProvider(null, "user-a")).toBe(
			true,
		);
		expect(
			shouldRegisterAgentAwarenessDeviceForProvider("user-a", "user-a"),
		).toBe(false);
		expect(
			shouldRegisterAgentAwarenessDeviceForProvider("user-a", "user-b"),
		).toBe(true);
		expect(
			shouldRegisterAgentAwarenessDeviceForProvider("user-a", undefined),
		).toBe(true);
	});

	it.effect(
		"registers rotated APNs tokens without rereading the native token",
		() => {
			const fetchMock = vi.fn((request: RequestInfo | URL) => {
				const url = request instanceof Request ? request.url : String(request);
				return Promise.resolve(
