...
// not read as enabled.
export type AgentAwarenessRegistrationStatus =
	| "unknown"
	| "pending"
	| "registered"
	| "failed";
...
export function subscribeAgentAwarenessRegistrationStatus(
	listener: () => void,
): () => void {
...
let activeLiveActivityRegistrationRetry: ReturnType<typeof setTimeout> | null =
	null;
	...
	if (!provider) {
		...
		void clearAgentAwarenessRegistrationRecord().catch((error: unknown) => {
			logRegistrationError(
				"clear registration record on sign-out failed",
				error,
			);
		});
		return;
	...
	return Effect.gen(function* () {
		if (expectedGeneration !== deviceRegistrationGeneration) {
			logRegistrationDebug(
				"device registration cancelled before relay request",
				{
					expectedGeneration,
		...
		if (!token) {
			logRegistrationDebug(
				"relay device registration skipped; user is not signed in",
			);
			setRegistrationStatus("unknown");
			return;
		}
		...
		if (expectedGeneration !== deviceRegistrationGeneration) {
			...
			logRegistrationDebug(
				"device registration cancelled after record lookup",
				{
					expectedGeneration,
					currentGeneration: deviceRegistrationGeneration,
				},
		...
		if (expectedGeneration !== deviceRegistrationGeneration) {
			...
			logRegistrationDebug(
				"device registration completed after sign-out; result discarded",
				{
					expectedGeneration,
					currentGeneration: deviceRegistrationGeneration,
				},
...
function unregisterDeviceWithRelay(input: {
	...
	return Effect.gen(function* () {
		...
		if (!token) {
			logRegistrationDebug(
				"relay device unregistration skipped; user is not signed in",
			);
			return;
		}
...
function environmentPublishesAgentActivity(
...
export function armAgentAwarenessLiveActivityForLocalWork(input: {
	...
	if (!environmentPublishesAgentActivity(input.environmentId)) {
		logRegistrationDebug(
			"live activity arming skipped; environment does not publish",
			{
...
function registerLiveActivityWithRelay(
	...
	return Effect.gen(function* () {
		...
		if (!token) {
			logRegistrationDebug(
				"relay live activity registration skipped; user is not signed in",
			);
			return false;
		}
...
function runRegistrationInBackground(
	...
	void (async () => {
		const result = await settleAsyncResult(() =>
			runtime.runPromiseExit(operation),
		);
...
function enqueueDeviceRegistration(
	...
	pendingDeviceRegistration = pendingDeviceRegistration
		? {
				input: mergeDeviceRegistrationInput(
					pendingDeviceRegistration.input,
					input,
				),
				context,
			}
...
function registerDevice(
	...
	return Effect.gen(function* () {
		if (!canRegisterRemoteLiveActivities()) {
			logRegistrationDebug(
				"device registration skipped; platform does not support it",
			);
			return;
		}

		logRegistrationDebug("device registration loading local state", {
			expectedGeneration,
		});
...
export function registerAgentAwarenessConnection(
...
export function unregisterAgentAwarenessConnection(
	environmentId: EnvironmentId,
): void {
	...
	return registerDevice({ preferencesOverride }).pipe(
		Effect.tapError((error) =>
			Effect.sync(() => {
				...
				logRegistrationError(
					"device preference registration refresh failed",
					error,
				);
			}),
		),
...
function registerLiveActivityPushTokenValue(input: {
	...
	return Effect.gen(function* () {
		const acceptedAt = registeredActivityPushTokens.get(
			...
			if (preferences?.liveActivitiesEnabled !== false) {
				...
				const armedMeanwhile = yield* Effect.try({
					...
					Effect.orElseSucceed(
						() => [] as ReadonlyArray<LiveActivity<AgentActivityProps>>,
					),
				);
		...
		const registrationResults = yield* Effect.forEach(activities, (activity) =>
			registerLiveActivityPushToken({ activity }).pipe(
				...
				Effect.catch((error) =>
					Effect.sync(() => {
						logRegistrationError(
							"active live activity token registration failed",
							error,
						);
						return true;
					}),
