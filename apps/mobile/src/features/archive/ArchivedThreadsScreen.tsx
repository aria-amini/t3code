...
function ArchivedThreadsHeader(props: {
	...
	const hasCustomFilter =
		props.selectedEnvironmentId !== null || props.sortOrder !== "newest";
		...
		(event: { nativeEvent: { event: string } }) => {
			...
			if (action === "environment:all") {
				...
				props.onEnvironmentChange(
					action.slice("environment:".length) as EnvironmentId,
				);
				...
				items: [
					{
						...
						title: "Newest first",
						state:
							props.sortOrder === "newest" ? ("on" as const) : ("off" as const),
					...
					},
					{
						...
						title: "Oldest first",
						state:
							props.sortOrder === "oldest" ? ("on" as const) : ("off" as const),
...
function ArchivedThreadRow(props: {
	...
	const subtitle = [props.environmentLabel, props.thread.branch].filter(
...
export function ArchivedThreadsScreen(props: {
	...
	const listItems = useMemo<ReadonlyArray<ArchivedThreadListItem>>(() => {
		...
		for (const group of props.groups) {
			const environmentLabel =
				environmentLabelsById.get(group.project.environmentId) ?? null;
			items.push({
				kind: "project",
	...
	const isInitialLoad =
		props.isLoading && props.groups.length === 0 && props.error === null;
	const isFiltered =
		props.searchQuery.trim().length > 0 || props.selectedEnvironmentId !== null;
