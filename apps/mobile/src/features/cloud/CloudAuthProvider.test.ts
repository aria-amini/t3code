...
describe("CloudAuthProvider relay account isolation", () => {
	it("clears relay and agent-awareness credentials before cleanup can fail", async () => {
		...
		expect(appAtomRegistry.get(managedRelaySessionAtom)?.accountId).toBe(
			"account-1",
		);

		deactivateCloudRelayAccount();
		const cleanup = Promise.reject(
			new Error("Persistence removal failed."),
		).catch(() => undefined);

		expect(appAtomRegistry.get(managedRelaySessionAtom)).toBeNull();
		expect(
			vi.mocked(setAgentAwarenessRelayTokenProvider),
