...
function CloudAuthBridge(props: { readonly children: ReactNode }) {
	...
	useEffect(() => {
		...
		const queueAccountCleanup = (
			previous: typeof previousTokenProviderRef.current,
		) => {
			const previousTransition =
				accountTransitionRef.current ?? Promise.resolve();
		...
		if (!isSignedIn || !userId) {
			...
			if (previousObservedAccount !== null) {
				void settlePromise(() => queueAccountCleanup(previous)).then(
					(result) => {
						reportAtomCommandResult(result, { label: "cloud account cleanup" });
					},
				);
		...
		if (
			...
			activateAfterTransition(
				(accountTransitionRef.current ?? Promise.resolve()).catch(() => {}),
			);
		}
