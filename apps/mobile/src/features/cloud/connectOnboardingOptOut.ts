...
export async function isConnectOnboardingOptedOut(
	accountId: string,
): Promise<boolean> {
	const preferences = await loadPreferences();
	return (
		preferences.connectOnboardingOptOutAccounts?.includes(accountId) ?? false
	);
}

/** Persists "Don't show this again" for the account. */
export async function optOutOfConnectOnboarding(
	accountId: string,
): Promise<void> {
