...
describe("mobile DPoP", () => {
	it.effect(
		"passes typed-array digest input through the Expo Crypto adapter",
		() =>
			Effect.gen(function* () {
				const crypto = yield* Crypto.Crypto;
				const digest = yield* crypto.digest(
					"SHA-256",
					new TextEncoder().encode("typed-array"),
				);
	...
	it.effect(
		"signs connect and bootstrap proofs with the same ephemeral proof key",
		() =>
			Effect.gen(function* () {
