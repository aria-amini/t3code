		...
		digest: (algorithm, data) =>
			Effect.promise(async () => {
				...
				return new Uint8Array(
					await ExpoCrypto.digest(toExpoDigestAlgorithm(algorithm), input),
				);
			}),
	}),
);
...
function publicJwkFromUncompressedPublicKey(
	publicKey: Uint8Array,
): DpopPublicJwk {
	if (publicKey.length !== 65 || publicKey[0] !== 0x04) {
		throw new Error(
			"Generated DPoP public key is not an uncompressed P-256 point.",
		);
	}
	return {
		kty: "EC",
...
> {
	return Effect.gen(function* () {
		...
		if (stored) {
			...
			const restored = yield* Effect.try({
				try: () => {
					...
					if (
						publicJwk.x !== storedPrivateJwk.x ||
						publicJwk.y !== storedPrivateJwk.y
					) {
			...
			const thumbprint = yield* computeDpopJwkThumbprintEffect(
				restored.publicJwk,
			);
			return {
				...restored,
		...
		const encodedPrivateJwk = yield* encodeDpopPrivateJwkJson(
...
> {
	return Effect.gen(function* () {
		...
		const jti = yield* Crypto.Crypto.pipe(
			...
			Effect.mapError(
				cloudDpopError("Could not generate DPoP proof identifier."),
			),
		);
		...
		const signature = yield* Effect.try({
			try: () =>
				p256
					.sign(signatureInputHash, privateKey, { prehash: false })
					.toCompactRawBytes(),
