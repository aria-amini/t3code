...
const stableClerkToken =
	"eyJhbGciOiJub25lIiwidHlwIjoiSldUIn0.eyJzdWIiOiJ1c2VyXzEyMyJ9.test";
...
function cloudClientLayer() {
	const httpClientLayer = remoteHttpClientLayer((input, init) =>
	...
	return Layer.mergeAll(
		...
		ManagedRelay.layer({
			...
			Layer.provideMerge(testDpopSignerLayer),
...
function validDpopAccessTokenResponse(
	scope = "environment:status environment:connect",
) {
	return {
...
describe("mobile cloud link environment client", () => {
	...
	it.effect(
		"decodes relay environment list responses before returning records",
		() =>
			Effect.gen(function* () {
				vi.stubGlobal(
					"fetch",
	...
	it.effect(
		"reuses one valid DPoP access token while probing multiple environment statuses",
		() =>
			Effect.gen(function* () {
				const fetchMock = vi.fn((url: string | URL, _init?: RequestInit) => {
					if (String(url).endsWith("/v1/environments")) {
						return Promise.resolve(
							Response.json({
								environments: [
									listedEnvironment("env-1"),
									listedEnvironment("env-2"),
								],
							}),
						);
					}
					if (String(url).endsWith("/v1/client/dpop-token")) {
						return Promise.resolve(
							Response.json(validDpopAccessTokenResponse()),
						);
					}
					const environmentId = String(url).includes("/env-1/")
						? "env-1"
						: "env-2";
					return Promise.resolve(
						Response.json({
				...
				yield* withCloudServices(
					listCloudEnvironmentsWithStatus({ clerkToken: stableClerkToken }),
				);
	...
	it.effect(
		"reuses the status-and-connect token when connecting from the cloud list",
		() =>
			Effect.gen(function* () {
				const fetchMock = vi.fn((url: string | URL, _init?: RequestInit) => {
					if (String(url).endsWith("/v1/environments")) {
				...
				expect(
					new URLSearchParams(requestBodyText(exchangeRequest?.body)).get(
						"scope",
					),
				).toBe("environment:status environment:connect");
	...
	it.effect(
		"keeps advertised environments visible when status probing fails",
		() =>
			Effect.gen(function* () {
				vi.stubGlobal(
					"fetch",
					vi.fn((url: string | URL) => {
						...
						if (
							String(url) === "https://relay.example.test/v1/client/dpop-token"
						) {
							return Promise.resolve(
								Response.json(validDpopAccessTokenResponse()),
							);
				...
				expect(records).toMatchObject([
					{
						...
						status: null,
						statusError:
							"https://relay.example.test/v1/environments/env-1/status failed",
					},
				]);
			}),
	);
	...
	it.effect(
		"rejects status responses for a different advertised environment",
		() =>
			Effect.gen(function* () {
				vi.stubGlobal(
					"fetch",
	...
	it.effect(
		"preserves typed local environment failures while obtaining a link proof",
		() =>
			Effect.gen(function* () {
				const fetchMock = vi.fn((url: string | URL) => {
					if (String(url).endsWith("/v1/client/environment-link-challenges")) {
	...
	it.effect(
		"preserves typed relay error bodies while linking environments",
		() =>
			Effect.gen(function* () {
				const fetchMock = vi.fn((url: string | URL) => {
					if (String(url).endsWith("/v1/client/environment-link-challenges")) {
	...
	it.effect(
		"rejects relay link credentials for a different managed endpoint provider",
		() =>
			Effect.gen(function* () {
				const fetchMock = vi.fn((url: string | URL) => {
					if (String(url).endsWith("/v1/client/environment-link-challenges")) {
	...
	it.effect(
		"preserves disabled Live Activity preferences when linking an environment",
		() =>
			Effect.gen(function* () {
				loadPreferences.mockReturnValueOnce(
					Effect.succeed({ liveActivitiesEnabled: false }),
				);
				...
				const fetchMock = vi.fn((url: string | URL, init?: RequestInit) => {
					...
					return Promise.resolve(
						Response.json({
							ok: true,
							endpointRuntimeStatus: { status: "configured" },
						}),
					);
				});
				vi.stubGlobal("fetch", fetchMock);
	...
	it.effect(
		"uses an explicit Live Activity preference when persisted state is unavailable",
		() =>
			Effect.gen(function* () {
				loadPreferences.mockReturnValueOnce(
					Effect.die("persisted preferences must not be read"),
				);
				...
				expect(
					bodies.filter((body) => "liveActivitiesEnabled" in body),
				).toEqual([
			...
			Effect.gen(function* () {
				...
				const fetchMock = vi.fn((url: string | URL, init?: RequestInit) => {
					if (String(url).endsWith("/v1/client/dpop-token")) {
						return Promise.resolve(
							Response.json(
								validDpopAccessTokenResponse("environment:connect"),
							),
						);
					}
					if (String(url).endsWith("/.well-known/t3/environment")) {
				...
				expect(connection.pairingUrl).not.toContain(
					"one-time-cloud-credential",
				);
	...
	it.effect(
		"refreshes a saved environment against a rotated managed endpoint",
		() =>
			Effect.gen(function* () {
				vi.stubGlobal(
					"fetch",
	...
	it.effect(
		"preserves relay DPoP auth failures while connecting environments",
		() =>
			Effect.gen(function* () {
				vi.stubGlobal(
					"fetch",
