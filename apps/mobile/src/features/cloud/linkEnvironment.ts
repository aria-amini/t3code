...
export class CloudEnvironmentLinkError extends Data.TaggedError(
...
function cloudEnvironmentLinkError(
	message: string,
	...
	return (cause: unknown) => {
		...
		const dpopAuthError = options?.dpop
			? findEnvironmentAuthInvalidError(cause)
			: null;
...
function findEnvironmentAuthInvalidError(
	cause: unknown,
): EnvironmentAuthInvalidError | null {
...
export function linkEnvironmentToCloudWithPreference(
	...
	void,
	CloudEnvironmentLinkError,
	LinkEnvironmentToCloudRequirements
> {
	return Effect.gen(function* () {
		...
		const deviceId = yield* storage.loadOrCreateAgentAwarenessDeviceId.pipe(
			Effect.mapError(
				cloudEnvironmentLinkError("Could not load the mobile device id."),
			),
		);
			...
			.pipe(
				Effect.mapError(
					decodedRelayClientError(
						`${relayUrl}/v1/client/environment-link-challenges failed`,
					),
				),
			);
		...
		const proof = yield* environmentClient.connect
			...
			.pipe(
				Effect.mapError(
					cloudEnvironmentLinkError("Could not obtain environment link proof."),
				),
			);
		const link = yield* relayClient
			...
			.pipe(
				Effect.mapError(
					decodedRelayClientError(
						`${relayUrl}/v1/client/environment-links failed`,
					),
				),
			);
...
> {
	return MobilePreferences.MobilePreferencesStore.pipe(
		...
		Effect.mapError(
			cloudEnvironmentLinkError(
				"Could not load mobile notification preferences.",
			),
		),
...
> {
	return Effect.gen(function* () {
		...
		return yield* relayClient
			...
			.pipe(
				Effect.mapError(
					decodedRelayClientError(`${relayUrl}/v1/environments failed`),
				),
			);
	});
}
...
> {
	return Effect.gen(function* () {
		...
		yield* ensureStatusMatchesEnvironment({
...
const loadAgentAwarenessDeviceId = Effect.fn(
	"mobile.cloud.loadAgentAwarenessDeviceId",
)(function* () {
	const storage = yield* MobileStorage.MobileStorage;
...
const connectRelayManagedEnvironment = Effect.fn(
	"mobile.cloud.connectRelayManagedEnvironment",
)(function* (input: {
	readonly clerkToken: string;
	...
	const descriptor = yield* fetchRemoteEnvironmentDescriptor({
		...
		Effect.mapError(
			cloudEnvironmentLinkError(
				"Could not fetch the connected environment descriptor.",
			),
		),
	);
	...
	const bootstrapDpop = yield* signer
		...
		.pipe(
			Effect.mapError(
				cloudEnvironmentLinkError("Could not create bootstrap DPoP proof."),
			),
		);
	const bootstrap = yield* exchangeRemoteDpopAccessToken({
		...
		Effect.mapError(
			cloudEnvironmentLinkError(
				"Could not exchange a managed endpoint DPoP access token.",
				{
					dpop: true,
				},
			),
