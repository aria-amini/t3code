...
export const managedRelayQueryManager = createManagedRelayQueryManager(
	managedRelayAtomRuntime,
	{
		onQueryEvent: (event) =>
			cloudDebugLog(`query:${event.operation}:${event.stage}:${event.phase}`, {
				...event,
			}),
	},
);
...
const EMPTY_ENVIRONMENTS_ATOM = Atom.make(
	...
	Atom.keepAlive,
	Atom.withLabel("managed-relay:mobile:environments:null"),
);
...
export function refreshManagedRelayEnvironments(): void {
	...
	if (session) {
		managedRelayQueryManager.refreshEnvironments(
			appAtomRegistry,
			session.accountId,
		);
	}
