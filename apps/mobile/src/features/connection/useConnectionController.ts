...
export function useConnectionController() {
	...
	const updateBearer = useAtomCommand(updateBearerConnection, {
		reportFailure: false,
	});
	const registerEnvironment = useAtomCommand(
		environmentCatalog.register,
		"environment register",
	);
	const removeEnvironmentMutation = useAtomCommand(
		environmentCatalog.remove,
		"environment remove",
	);
	const retryEnvironmentMutation = useAtomCommand(
	...
	const availableRelayEnvironments = useMemo(
		() =>
			relayEnvironments.filter(
				(entry) => !registeredIds.has(entry.environment.environmentId),
			),
