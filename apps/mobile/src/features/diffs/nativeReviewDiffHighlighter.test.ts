...
vi.mock("@shikijs/core", async (importOriginal) => {
	...
	return {
		...
		createHighlighterCore: async (
			...args: Parameters<typeof original.createHighlighterCore>
		) => {
			const highlighter = await original.createHighlighterCore(...args);
			return {
				...highlighter,
				codeToTokensBase: (
					...input: Parameters<typeof highlighter.codeToTokensBase>
				) => {
...
function makeLine(
	input: Pick<
		NativeReviewDiffRow,
		"id" | "content" | "change" | "oldLineNumber" | "newLineNumber"
	>,
): NativeReviewDiffRow {
	return {
		kind: "line",
...
describe("highlightNativeReviewDiffVisibleRows", () => {
	it("does not carry grammar state across hunk boundaries", async () => {
		...
		expect(highlighted.tokensByRowId[exportRow.id]).toEqual(
			standalone.tokensByRowId[exportRow.id],
		);
	});
...
describe.each(["native", "javascript"] as const)(
	"%s highlighting budgets",
	(engine) => {
		const highlightRows = (
			rows: ReadonlyArray<NativeReviewDiffRow>,
			signal?: AbortSignal,
		...
		it("still highlights a line at the length limit", async () => {
			...
			expect(
				result.tokensByRowId["line-1"]?.some((token) => token.color !== null),
			).toBe(true);
		});
		...
		it("keeps only the long line plain and resumes highlighting after it", async () => {
			...
			expect(Object.keys(result.tokensByRowId)).toEqual([
				"line-1",
				"line-2",
				"line-3",
				"line-100",
			...
			for (const id of ["line-1", "line-3", "line-100"]) {
				expect(
					result.tokensByRowId[id]?.some((token) => token.color !== null),
				).toBe(true);
			}
			expect(tokenization.calls.some((code) => code.includes(longLine))).toBe(
				false,
			);
		});
		...
		it("highlights the rows after a long line the same regardless of the first window", async () => {
			...
			for (const id of ["line-3", "line-4"]) {
				expect(
					spanning.tokensByRowId[id]?.some((token) => token.color !== null),
				).toBe(true);
				expect(afterLongLine.tokensByRowId[id]).toEqual(
					spanning.tokensByRowId[id],
				);
			}
		});
		...
		it("preserves multiline grammar and row mapping across character-limited batches", async () => {
			...
			for (const row of [opening, ...body, closing, trailing]) {
				expect(
					result.tokensByRowId[row.id]?.map((token) => token.content).join(""),
				).toBe(row.content);
			}
			expect(result.tokensByRowId[closing.id]).toEqual(
				expected.tokensByRowId[closing.id],
			);
			expect(result.tokensByRowId[trailing.id]).toEqual(
				expected.tokensByRowId[trailing.id],
			);
		});
