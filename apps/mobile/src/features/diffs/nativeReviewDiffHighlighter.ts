...
const PIERRE_LIGHT_SHIKI_THEME = {
	...
	settings: [
		...
		{
			scope:
				"variable.parameter, variable.other.readwrite, meta.object-literal.key",
...
let nativeHighlighterPromise: Promise<NativeReviewDiffHighlighterHandle> | null =
	null;
let javascriptHighlighterPromise: Promise<NativeReviewDiffHighlighterHandle> | null =
	null;
	...
	return {
		...
		async tokenize(code, { lang, theme, signal }) {
			...
			while (start < lines.length) {
				...
				if (
					lines[start]!.length > NATIVE_REVIEW_DIFF_TOKENIZE_MAX_LINE_LENGTH
				) {
					highlighted.push([
						{ content: lines[start] || " ", color: null, fontStyle: null },
					]);
		...
		.catch(async (cause: unknown) => {
			...
			try {
				javascriptHighlighterPromise ??=
					createJavascriptReviewDiffHighlighter();
				return await javascriptHighlighterPromise;
			} catch (fallbackCause) {
				javascriptHighlighterPromise = null;
				throw new NativeReviewDiffHighlighterInitializationError({
...
function isHighlightableLineRow(
	row: NativeReviewDiffRow,
): row is NativeReviewDiffLineRow {
	return (
		row.kind === "line" &&
...
function hasOnlyCommentRowsBetween(
	...
	for (
		let rowIndex = previousRowIndex + 1;
	...
	if (previous.row.change === "delete" || next.row.change === "delete") {
		return (
			...
			hasConsecutiveLineNumbers(
				previous.row.oldLineNumber,
				next.row.oldLineNumber,
			)
		);
	}

	if (previous.row.change === "add" || next.row.change === "add") {
		return hasConsecutiveLineNumbers(
			previous.row.newLineNumber,
			next.row.newLineNumber,
		);
	}
...
export async function highlightNativeReviewDiffVisibleRows(
	...
	const highlighter = await getNativeReviewDiffHighlighter(
		input.engine ?? "native",
	);
	...
	const overscanRows =
		input.overscanRows ?? NATIVE_REVIEW_DIFF_VISIBLE_OVERSCAN_ROWS;
	...
	const flushSegment = async () => {
		...
		if (
			...
			charactersSinceYield + code.length >
				NATIVE_REVIEW_DIFF_TOKENIZE_MAX_CHARACTERS
		) {
		...
		segmentRows.forEach(({ row }, rowIndex) => {
			tokensByRowId[row.id] =
				tokenLines[rowIndex] ?? makePlainTokenFallback(row);
		});
		segmentRows = [];
		segmentFile = undefined;
	};
