...
function setExpoViewConfigAvailable() {
	globalThis.expo = {
		getViewConfig: vi
			.fn()
			.mockReturnValue({ validAttributes: {}, directEventTypes: {} }),
...
describe("resolveNativeReviewDiffView", () => {
	...
	it("returns null when the native review diff view config is unavailable", async () => {
		const { resolveNativeReviewDiffView } =
			await import("./nativeReviewDiffSurface");
	...
	it("returns the payload bridge when the native review diff view is installed", async () => {
		...
		expect(expoMocks.requireNativeView).toHaveBeenCalledWith(
			"T3ReviewDiffSurface",
		);
	});
	...
	it("recognizes registration races for the installed native view name", async () => {
		...
		expect(
			isPendingNativeViewRegistration(
				new Error(
					"Unable to find the 'T3ReviewDiffSurface' view for this native tag",
				),
			),
		).toBe(true);
		expect(
			isPendingNativeViewRegistration(
				new Error(
					"Unable to find the 'T3ReviewDiffView' view for this native tag",
				),
			),
		).toBe(false);
		expect(
...
describe("isNativeReviewDiffDrawEvent", () => {
	it("accepts only native events emitted after diff rows draw", async () => {
		...
		expect(isNativeReviewDiffDrawEvent({ message: "visible-range" })).toBe(
			true,
		);
		expect(isNativeReviewDiffDrawEvent({ message: "rows-decoded" })).toBe(
			false,
		);
	});
});
