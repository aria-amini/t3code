...
export function isNativeReviewDiffDrawEvent(
	...
	return (
		payload.message === "draw-metrics" || payload.message === "visible-range"
	);
}
...
let cachedNativeReviewDiffRawView:
	| ComponentType<NativeReviewDiffRawViewProps>
	| undefined;
...
export function isPendingNativeViewRegistration(error: unknown): boolean {
	...
	return (
		error.message.includes(
			`Unable to find the '${NATIVE_REVIEW_DIFF_MODULE_NAME}' view`,
		) ||
...
function getExpoViewConfig(moduleName: string) {
	return (
		globalThis as typeof globalThis & ExpoGlobalWithViewConfig
	).expo?.getViewConfig?.(moduleName);
}
...
export function resolveNativeReviewDiffView(): ComponentType<NativeReviewDiffViewProps> | null {
	...
	try {
		cachedNativeReviewDiffRawView =
			requireNativeView<NativeReviewDiffRawViewProps>(
