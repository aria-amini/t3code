								...
								return (
									...
									>
										{token.content.length > 0
											? renderVisibleWhitespace(token.content)
											: " "}
									</NativeText>
								);
							});
...
function useSourceFileModel(props: SourceFileSurfaceProps) {
	...
	const document = useMemo(
		() => prepareSourceFileDocument(props.contents),
		[props.contents],
	);
	const { contents: normalizedContents, lines, rowsJson } = document;
	const targetIndex =
		props.initialLine !== null &&
		props.initialLine !== undefined &&
		props.initialLine > 0
			? Math.min(
					Math.floor(props.initialLine) - 1,
	...
	const tokens = AsyncResult.isSuccess(highlightResult)
...
function SourceHighlightStatusView(props: {
	readonly status: SourceHighlightStatus;
}) {
...
function NativeSourceFileSurface(
	...
	const { codeSurface, codeWordBreak, nativeSourceStyle } =
	...
	const tokensJson = useMemo(
		() => JSON.stringify(buildNativeSourceTokens(tokens)),
		[tokens],
	);
	const selectedRowIdsJson = useMemo(
		() =>
			JSON.stringify(
				targetIndex === null ? [] : [nativeSourceRowId(targetIndex)],
			),
		[targetIndex],
	);
	const themeJson = useMemo(
		() =>
			JSON.stringify(
				createNativeReviewDiffTheme(themeAppearance, themeId, appTheme),
			),
		[appTheme, themeAppearance, themeId],
	);
	const styleJson = useMemo(
		() => JSON.stringify(nativeSourceStyle),
		[nativeSourceStyle],
	);
