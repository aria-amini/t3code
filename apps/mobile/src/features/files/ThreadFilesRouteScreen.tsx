		...
		>
			<Svg width="100%" height="100%">
				...
				<Rect
					width="100%"
					height="100%"
					fill="url(#files-toolbar-bottom-fade)"
				/>
			</Svg>
		</View>
	);
...
export function ThreadFilesTreeScreen(props: ThreadFilesRouteScreenProps) {
	...
	const { cwd, environmentId, projectName, selectedThread, threadId } =
			...
			<NativeStackScreenOptions
				options={{
					...
					unstable_headerSubtitle:
						Platform.OS === "ios" && projectName.length > 0
							? projectName
							: undefined,
...
export function ThreadFileScreen(props: ThreadFileRouteScreenProps) {
	useAdaptiveWorkspacePaneRole("inspector");
	const navigation = useNavigation();
	const { fileInspector, panes, toggleAuxiliaryPane } =
		useAdaptiveWorkspaceLayout();
	...
	const assetPreviewPath =
		isBrowserFile || isImageFile || isVideoFile ? relativePath : null;
	...
	const assetPreviewUri =
		assetPreview._tag === "Success" ? assetPreview.url : null;
		...
		() =>
			...
			assetPreview.resource?._tag === "media-file"
				? {
						...
						name: basename(relativePath),
						mimeType:
							videoMimeType({ name: relativePath, mimeType: "" }) ??
							"video/mp4",
	...
	const fileMenuActions = useMemo(() => {
		...
		return [
			...
			(isBrowserFile || isImageFile || isVideoFile)
				? ({
						id: "refresh",
						title: "Refresh",
						icon: "arrow.clockwise",
	...
	const handleAndroidFileMenuAction = useCallback(
		(event: { nativeEvent: { event: string } }) => {
			const action = fileMenuActions.find(
				({ id }) => id === event.nativeEvent.event,
			);
			void action?.onPress();
		},
		[fileMenuActions],
	...
	return (
		<View className="flex-1 bg-sheet">
			...
			<FileContent
				...
				previewFailure={
					assetPreview._tag === "Failure" ? assetPreview.reason : null
				}
