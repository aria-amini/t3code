...
export function defaultExpandedTreePaths(
...
function valueMatchesSearchToken(
	value: string,
	token: string,
	fuzzy: boolean,
): boolean {
...
function nodeMatchesSearch(
	...
	return tokens.every(
		(token) =>
			node.searchSegments.some((segment) =>
				valueMatchesSearchToken(segment, token, false),
			) ||
			node.searchWords.some((word) =>
				valueMatchesSearchToken(word, token, true),
			),
	);
}
