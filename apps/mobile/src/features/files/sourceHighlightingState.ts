...
export function createSourceHighlightAtomFamily(options?: {
	...
	const family = Atom.family((request: SourceHighlightCacheKey) =>
		Atom.make(
			...
			Atom.withLabel(
				`mobile:source-highlight:${request.theme}:${request.path}`,
			),
		),
	);

	return (input: SourceHighlightInput) =>
		family(new SourceHighlightCacheKey(input));
}
