...
interface HomeScreenProps {
	...
	readonly environments: ReadonlyArray<
		HomeListFilterMenuEnvironment &
			Pick<WorkspaceEnvironment, "connectionState">
	>;
	...
	readonly onRegenerateThreadTitle: (
		thread: EnvironmentThreadShell,
	) => Promise<boolean>;
...
export function HomeScreen(props: HomeScreenProps) {
	...
	const updateGroupDisplay = useCallback(
		(key: string, action: HomeGroupDisplayAction) => {
			...
			next.set(
				key,
				nextGroupDisplayState(
					next.get(key) ?? DEFAULT_GROUP_DISPLAY_STATE,
					action,
				),
			);
	...
	const showMoreSettled = useCallback(
		() =>
			setSettledVisibleCount(
				(count) => count + THREAD_LIST_V2_SETTLED_PAGE_COUNT,
			),
		[],
	);
	const {
	...
	const threadListV2Layout = useMemo(() => {
		...
		return buildThreadListV2Items({
			...
			environmentId: props.selectedEnvironmentId,
			projectRefs:
				v2ScopedProjectGroup === null ? null : v2ScopedProjectGroup.projectRefs,
			searchQuery: props.searchQuery,
			matchedThreadKeys,
			settlementEnvironmentIds,
			snoozeEnvironmentIds,
	...
	useEffect(() => {
		...
		const delayMs = Math.min(
			Math.max(0, wakeAtMs - Date.now()) + 50,
			2_147_483_647,
		);
		const id = setTimeout(
	...
	const renderV2Item = useCallback(
		({
			...
			readonly item: ThreadListV2ListItem;
					...
					<ThreadListV2PendingRow
						...
						environmentLabel={
							Object.keys(props.savedConnectionsById).length > 1
								? (props.savedConnectionsById[item.pendingTask.environmentId]
			...
			return (
				<ThreadListV2Row
					...
					providerInstance={resolveThreadProviderInstance(
						serverConfigs,
						thread,
					)}
					environmentLabel={
						Object.keys(props.savedConnectionsById).length > 1
							? (props.savedConnectionsById[thread.environmentId]
									?.environmentLabel ?? null)
							: null
					}
					...
					settlementSupported={settlementEnvironmentIds.has(
						thread.environmentId,
					)}
					...
					canMoveUp={
						pendingOrder === null && movePlanner(movedId, "up") !== null
					}
					canMoveDown={
						pendingOrder === null && movePlanner(movedId, "down") !== null
					}
	...
	const v2KeyExtractor = useCallback(
		(item: ThreadListV2ListItem) => item.key,
		[],
	);
				...
				case "header":
					return (
						<ThreadListGroupHeader
							...
							threadCount={
								item.group.threads.length + item.group.pendingTasks.length
							}
				...
				case "thread": {
					...
					return (
						<ThreadListRow
							...
							variant="compact"
							thread={thread}
							hasQueuedMessages={queuedThreadKeys.has(
	...
	const selectedEnvironmentLabel =
		props.selectedEnvironmentId === null
			? null
			: (props.savedConnectionsById[props.selectedEnvironmentId]
	...
	if (!hasAnyThreads) {
		return (
			<View
				...
				style={{
					paddingBottom:
						Math.max(insets.bottom, 24) + iosBottomToolbarClearance,
							...
							threadListV2Layout.hiddenSettledCount > 0 ? (
								...
								>
									<Text className="text-xs font-t3-medium text-foreground-muted">
										Show more ({threadListV2Layout.hiddenSettledCount} settled
										hidden)
									</Text>
								</Pressable>
							) : null
