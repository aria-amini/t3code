...
const HomeListOptionsContext =
	createContext<HomeListOptionsContextValue | null>(null);
...
export function HomeListOptionsProvider({
	...
	const [options, setOptions] = useState<HomeListOptions>(
		defaultHomeListOptions,
	);
	const value = useMemo(
...
export function hasCustomHomeListOptions(
	...
	return (
		...
		(options.selectedProjectKey !== null &&
			options.selectedProjectKey !== undefined) ||
...
export function useHomeListOptions(
	...
	const [localOptions, setLocalOptions] = useState<HomeListOptions>(
		defaultHomeListOptions,
	);
