...
function loadRouters() {
	...
	const corePackage = requireFromNative.resolve(
		"@react-navigation/core/package.json",
	);
...
type ThreadSelection = Parameters<
	typeof createHomeThreadNavigationAction
>[0]["thread"];
...
describe("createHomeThreadNavigationAction", () => {
	...
	it("uses a fresh key for the same thread selected during dismissal", () => {
		...
		const overlapState = selectThread(
			outgoingState,
			thread("thread-a"),
	...
	it("coalesces a second overlap selection onto the fresh incoming route", () => {
		...
		const incomingRouteKey =
			firstOverlapState.routes[firstOverlapState.index]?.key;
