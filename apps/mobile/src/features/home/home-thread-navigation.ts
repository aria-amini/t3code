...
export function createHomeThreadNavigationAction(input: {
	...
	if (
		input.dismissingRouteKey !== null &&
		currentRoute?.key === input.dismissingRouteKey
	) {
...
export function useHomeThreadSelection() {
	const navigation =
		useNavigation<
			NativeStackNavigationProp<ReactNavigation.RootParamList, "Home">
		>();
		...
		const removeTransitionStart = navigation.addListener(
			...
			({ data }) => {
				...
				dismissingRouteKey.current =
					!data.closing && currentRoute?.name === "Thread"
						? currentRoute.key
						: null;
			},
		);
