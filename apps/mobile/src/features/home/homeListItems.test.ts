...
describe("buildHomeListLayout", () => {
	...
	it("limits large groups to the initial visible count with a show-more row", () => {
		...
		expect(
			threadItems.every((item) => item.type === "thread" && !item.isLast),
		).toBe(true);
	});
	...
	it("reveals more threads per show-more step and offers show-less when exhausted", () => {
		...
		expect(
			expandedOnce.items.filter((item) => item.type === "thread"),
		).toHaveLength(HOME_INITIAL_VISIBLE_THREADS + HOME_SHOW_MORE_STEP);
		...
		expect(
			fullyExpanded.items.filter((item) => item.type === "thread"),
		).toHaveLength(20);
	...
	it("offers show-less after expanding a stale group whose baseline is below the page size", () => {
		...
		expect(
			collapsedToRecent.items.filter((item) => item.type === "thread"),
		).toHaveLength(3);
		...
		expect(
			expanded.items.filter((item) => item.type === "thread"),
		).toHaveLength(10);
	...
	it("suspends collapse and pagination while searching", () => {
		...
		expect(layout.items.filter((item) => item.type === "thread")).toHaveLength(
			12,
		);
