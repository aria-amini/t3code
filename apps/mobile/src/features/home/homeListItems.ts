...
export const EMPTY_HOME_LIST_LAYOUT: HomeListLayout = {
	items: [],
	stickyHeaderIndices: [],
};
...
export function nextGroupDisplayState(
	...
	switch (action) {
		...
		case "show-more":
			return {
				...current,
				visibleCount: current.visibleCount + HOME_SHOW_MORE_STEP,
			};
		case "show-less":
 ...
 * toggles.
 */
export function homeListItemsAreEqual(
	previous: HomeListItem,
	item: HomeListItem,
): boolean {
	switch (item.type) {
		case "header":
...
export function buildHomeListLayout(input: {
	...
	for (const [groupIndex, group] of input.groups.entries()) {
		const display =
			input.displayStates.get(group.key) ?? DEFAULT_GROUP_DISPLAY_STATE;
