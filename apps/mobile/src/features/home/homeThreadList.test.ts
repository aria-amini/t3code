...
function makeProject(
	input: Partial<EnvironmentProject> &
		Pick<EnvironmentProject, "environmentId" | "id" | "title">,
): EnvironmentProject {
	return {
...
function makeThread(
	input: Partial<EnvironmentThreadShell> &
		Pick<
			EnvironmentThreadShell,
			"environmentId" | "id" | "projectId" | "title"
		>,
): EnvironmentThreadShell {
	return {
		modelSelection: {
...
describe("buildHomeThreadGroups", () => {
	...
	it("routes stale duplicate project refs through the canonical repository group", () => {
		...
		expect(
			scopes[0]?.projectRefs.map((projectRef) => projectRef.projectId),
		).toEqual([local.id, stale.id, canonicalRemote.id]);
	...
	it("sorts the newest thread first regardless of snapshot order", () => {
		...
		expect(
			buildGroups([project], threads)[0]?.threads.map((thread) => thread.id),
	...
	it("filters both projects and threads to one environment", () => {
		...
		expect(groups[0]?.threads.map((thread) => thread.environmentId)).toEqual([
			remoteEnvironmentId,
		]);
	});
	...
	it("matches web repository, repository-path, and separate grouping modes", () => {
		...
		expect(
			buildGroups(projects, threads, { projectGroupingMode: "repository" }),
		).toHaveLength(1);
		expect(
	...
	it("includes a thread matched by message content", () => {
		...
		expect(groups[0]?.threads.map((candidate) => candidate.id)).toEqual([
			"thread-content",
		]);
	});
