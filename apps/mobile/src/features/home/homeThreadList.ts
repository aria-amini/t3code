...
export function sortHomeProjectScopes(input: {
	...
	const recordActivity = (scopeKey: string | undefined, timestamp: number) => {
		...
		latestActivityByScope.set(
			scopeKey,
			Math.max(
				latestActivityByScope.get(scopeKey) ?? Number.NEGATIVE_INFINITY,
				timestamp,
			),
		);
	};
...
function selectRecentThreads(
	...
	const recent = sortedThreads.filter(
		(thread) =>
			...
			queuedThreadKeys?.has(
				scopedThreadKey(thread.environmentId, thread.id),
			) === true,
	);
	return recent.length > 0
		? recent
		: sortedThreads.slice(0, RECENT_THREAD_FALLBACK_COUNT);
}
...
export function buildHomeThreadGroups(input: {
	...
	for (const pendingTask of input.pendingTasks ?? []) {
		if (
			input.environmentId !== null &&
			pendingTask.environmentId !== input.environmentId
		) {
			continue;
		}
				...
				projects: [
					{
						...
						workspaceRoot:
							pendingTask.projectCwd ?? String(pendingTask.projectId),
	...
	for (const group of groups.values()) {
		...
		if (
			!representative ||
			(group.threads.length === 0 && group.pendingTasks.length === 0)
		) {
			continue;
		}
