...
interface ThreadSwipeAction {
	...
	readonly menu?: {
		...
		readonly onPressAction: NonNullable<
			ComponentProps<typeof ControlPillMenu>["onPressAction"]
		>;
...
function ThreadSwipeableRow(props: ThreadSwipeableProps) {
	...
	const fullSwipeThreshold = Math.max(
		actionsWidth + 44,
		props.fullSwipeWidth * 0.58,
	);
	const fullSwipeAction =
		props.fullSwipeAction ??
		(props.secondaryAction === undefined ? "delete" : "primary");
	...
	useLayoutEffect(() => {
		...
		return () => {
			...
			if (activeTranslationRef.current)
				cancelAnimation(activeTranslationRef.current);
	...
	const dismiss = useCallback(
		(translation: SharedValue<number>) => {
			...
			translation.set(
				withTiming(
					Math.min(translation.value, -rowWidth.value),
					timing,
					(finished) => {
						if (!finished) return;
						collapse.set(
	...
	const dismissStyle = useAnimatedStyle(() => ({
		height: dismissing.value
			? rowHeight.value * (1 - collapse.value)
			: undefined,
...
function SwipeActionButton(props: {
	...
	const circleSize = props.compact
		? COMPACT_ACTION_CIRCLE_SIZE
		: ACTION_CIRCLE_SIZE;
	...
	const actionStyle = useAnimatedStyle(() => {
		...
		return {
			opacity: stretchesOnFullSwipe
				? entryProgress
				: entryProgress * (1 - fullSwipeProgress),
			transform: [
				{
					translateX:
						interpolate(entryProgress, [0, 1], [22, 0]) -
						(stretchesOnFullSwipe ? 0 : stretch),
				},
	...
	const circleStyle = useAnimatedStyle(() => {
		const reveal = Math.max(-translation.value, 0);
		const stretch = stretchesOnFullSwipe
			? Math.max(reveal - actionsWidth, 0)
			: 0;
...
export function ThreadSwipeActions(props: {
	...
	const fullSwipeIsPrimary =
		props.fullSwipeAction === "primary" || secondaryAction === null;
