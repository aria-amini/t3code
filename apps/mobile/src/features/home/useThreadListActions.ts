...
function environmentSupportsSnooze(
	environmentId: EnvironmentThreadShell["environmentId"],
) {
	return (
		appAtomRegistry.get(environmentServerConfigsAtom).get(environmentId)
...
function environmentSupportsPinning(
	environmentId: EnvironmentThreadShell["environmentId"],
) {
	return (
		appAtomRegistry.get(environmentServerConfigsAtom).get(environmentId)
			?.environment.capabilities.threadPinning === true
	);
}

function environmentSupportsPinReorder(
	environmentId: EnvironmentThreadShell["environmentId"],
) {
	return (
		appAtomRegistry.get(environmentServerConfigsAtom).get(environmentId)
...
function useThreadActionExecutor(
	...
	const archiveMutation = useAtomCommand(threadEnvironment.archive, {
		reportFailure: false,
	});
	const unarchiveMutation = useAtomCommand(threadEnvironment.unarchive, {
		reportFailure: false,
	});
	const deleteMutation = useAtomCommand(threadEnvironment.delete, {
		reportFailure: false,
	});
	const settleMutation = useAtomCommand(threadEnvironment.settle, {
		reportFailure: false,
	});
	const unsettleMutation = useAtomCommand(threadEnvironment.unsettle, {
		reportFailure: false,
	});
		...
		async (action: ThreadListAction, thread: EnvironmentThreadShell) => {
			...
			try {
				...
				if (result._tag === "Failure") {
					Alert.alert(
						actionFailureTitle(action),
						actionFailureMessage(action, result.cause),
					);
					return false;
				}
...
export function useThreadListActions(): {
	...
	readonly regenerateThreadTitle: (
		thread: EnvironmentThreadShell,
	) => Promise<boolean>;
} {
	const executeAction = useThreadActionExecutor();
	const snoozeMutation = useAtomCommand(threadEnvironment.snooze, {
		reportFailure: false,
	});
	const unsnoozeMutation = useAtomCommand(threadEnvironment.unsnooze, {
		reportFailure: false,
	});
	const pinMutation = useAtomCommand(threadEnvironment.pin, {
		reportFailure: false,
	});
	const unpinMutation = useAtomCommand(threadEnvironment.unpin, {
		reportFailure: false,
	});
	...
	const settleThread = useCallback(
		async (thread: EnvironmentThreadShell) =>
			(await executeAction("settle", thread)) === true,
		[executeAction],
	);
	...
	const reorderActiveMutation = useAtomCommand(
		...
		async (thread: EnvironmentThreadShell, direction: "up" | "down") => {
			...
			const assignments = createThreadMovePlanner({
				...
				reorderableEnvironmentIds: new Set(
					[...configs.keys()].filter(supportsReorder),
				),
			...
			let succeeded = false;
			const reorder =
				section === "pinned" ? reorderPinnedMutation : reorderActiveMutation;
			try {
