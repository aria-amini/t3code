...
export function HardwareKeyboardCommandProvider({
	...
	const activeThreadRef = useMemo(
		() => parseActiveThreadPath(pathname),
		[pathname],
	);
				...
				: resolveThreadReferenceCopyTarget({
						...
						linkedPullRequestUrl:
							(
								...
								activeThread?.branchPullRequest
							)?.url ?? null,
					}),
		[activeThread, activeThreadRef],
	);
	const [copyFeedback, setCopyFeedback] =
		useState<GitActionProgress>(EMPTY_COPY_FEEDBACK);
	...
	const enabledCommands = useMemo(() => {
		const commands = new Set<HardwareKeyboardCommand>(
			getRegisteredHardwareKeyboardCommands(),
		);
	...
	return (
		<>
			...
			<GitActionProgressOverlay
				progress={copyFeedback}
				onDismiss={dismissCopyFeedback}
			/>
		</>
	);
}
