...
interface AdaptiveWorkspaceContextValue {
	...
	readonly activateAuxiliaryPaneRole: (
		role: WorkspaceAuxiliaryPaneRole,
	) => () => void;
	/**
...
export function useAdaptiveWorkspacePaneRole(role: WorkspaceAuxiliaryPaneRole) {
	...
	useFocusEffect(
		useCallback(
			() => activateAuxiliaryPaneRole(role),
			[activateAuxiliaryPaneRole, role],
		),
	);
}
...
export function useRegisterWorkspaceInspector(
	...
	const syncRegistration = useCallback(() => {
		...
		deactivateRef.current = registerWorkspaceInspector(
			wrappedRenderRef.current,
		);
...
export function AdaptiveWorkspaceLayout(props: {
	...
	const groupingSettings = resolveMobileProjectGroupingSettings(
...
function AdaptiveWorkspaceLayoutContent(
	...
	const [primarySidebarPreferredVisible, setPrimarySidebarPreferredVisible] =
		useState(true);
	const [
		supplementaryPanePreferredVisible,
		setSupplementaryPanePreferredVisible,
	] = useState(true);
	const [supplementaryPanePreferredWidth, setSupplementaryPanePreferredWidth] =
		useState<number | null>(null);
	const [fileInspectorPreferredVisible, setFileInspectorPreferredVisible] =
		useState(true);
	const [fileInspectorPreferredWidth, setFileInspectorPreferredWidth] =
		useState<number | null>(null);
	const [primarySidebarSearchQuery, setPrimarySidebarSearchQuery] =
		useState("");
	const [focusedAuxiliaryPaneRole, setFocusedAuxiliaryPaneRole] =
		useState<WorkspaceAuxiliaryPaneRole | null>(null);
	const baseLayout = useMemo(
		() => deriveLayout({ width, height }),
		[height, width],
	);
	...
	const auxiliaryPaneRole: WorkspaceAuxiliaryPaneRole =
		focusedAuxiliaryPaneRole ??
		(/\/files(?:\/|$)/.test(pathname) ? "inspector" : "supplementary");
	...
	const handleWorkspaceInspectorClosed = useCallback(() => {
		setWorkspaceInspector((current) =>
			current !== null && !current.active ? null : current,
		);
	}, []);
	const activateAuxiliaryPaneRole = useCallback(
		(role: WorkspaceAuxiliaryPaneRole) => {
