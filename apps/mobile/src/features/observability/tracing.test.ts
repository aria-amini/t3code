...
it.effect("exports spans through the scoped mobile OTLP layer", () => {
	...
	const tracedApplication = Layer.effectDiscard(
		Effect.void.pipe(
			Effect.withSpan("mobile.test.span"),
		...
		Effect.andThen(
			Effect.sync(() => {
				...
				expect(new Headers(init?.headers).get("authorization")).toBe(
					"Bearer public-ingest-token",
				);
				expect(new Headers(init?.headers).get("x-axiom-dataset")).toBe(
					"mobile-traces",
				);
...
it.effect(
	"does not let OTLP serialization failures alter application effects",
	() => {
		const fetchFn = vi.fn<typeof fetch>(
			async () => new Response(null, { status: 202 }),
		);
		...
		const tracedApplication = Layer.effectDiscard(
			Effect.fail(failure).pipe(
				...
				Effect.flatMap((exit) => {
					const reason =
						exit._tag === "Failure" ? exit.cause.reasons[0] : undefined;
		...
		return Layer.build(tracedApplication).pipe(
			...
			Effect.andThen(
				Effect.sync(() => {
					...
					).toContain("mobile.test.failed-span");
				}),
			),
		);
	},
