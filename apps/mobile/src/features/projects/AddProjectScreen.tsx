...
function useBrowsePathInput(
	...
	const clonePathCaseSensitive = !isWindowsPlatform(
	...
	useEffect(() => {
		if (
			environmentId !== null &&
			environmentId !== previousEnvironmentIdRef.current
		) {
...
function SourceControlRow(props: {
	...
	const title =
		props.source === "url"
			? "Git URL"
			: `${addProjectRemoteSourceLabel(props.source)} repository`;
	const subtitle =
		props.source === "url"
				...
				<>
					<ListSection>
						...
						{(
							[
								"url",
								...sortAddProjectProviderSources(readiness),
							] as AddProjectRemoteSource[]
						).map((candidate) => (
							<SourceControlRow
								key={candidate}
								source={candidate}
...
export function AddProjectRepositoryScreen(props: {
	...
	const lookupRepositoryQuery = useAtomQueryRunner(
	...
	const lookupRepository = useCallback(async () => {
		if (!environment || repositoryInput.trim().length === 0 || isSubmitting)
			return;
		setError(null);
...
function FolderBrowser(props: {
	...
	const browseFilterQuery = pinnedDirectoryMatches
		? ""
...
export function AddProjectDestinationScreen(props: {
	...
	const { isBrowseNavigating, navigateToBrowsePath, pathInput, setPathInput } =
		useBrowsePathInput(environment, repositoryName);
	const [isSubmitting, setIsSubmitting] = useState(false);
	const [error, setError] = useState<string | null>(null);

	const submitPath = useCallback(async () => {
		if (!environment || !remoteUrl || isBrowseNavigating || isSubmitting)
			return;
		setError(null);
		const resolved = resolveAddProjectPath({
			rawPath: pathInput,
