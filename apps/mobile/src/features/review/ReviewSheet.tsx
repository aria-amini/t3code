...
const ReviewNotice = memo(function ReviewNotice(props: {
	...
	return (
		...
		>
			...
			<View className="mt-1 flex-row gap-2">
				<Text className="text-2xs font-t3-bold text-emerald-600">
					+{file.additions}
				</Text>
				<Text className="text-2xs font-t3-bold text-rose-600">
					-{file.deletions}
				</Text>
			</View>
		</Pressable>
...
function ReviewFileNavigator({
	...
	const headerScrollEdgeEffects = nativeHeaderScrollEdgeEffects(
		Platform.OS,
		Platform.Version,
	);
	...
	const fileList = (
		<FlatList
			...
			contentContainerStyle={{
				...
				paddingTop:
					Platform.OS === "ios" ? insets.top + IOS_NAV_BAR_HEIGHT + 8 : 8,
			}}
			scrollIndicatorInsets={
				Platform.OS === "ios"
					? { top: insets.top + IOS_NAV_BAR_HEIGHT }
					: undefined
			}
...
export function ReviewSheet(props: ReviewSheetProps) {
	...
	const { panes, showAuxiliaryPane, toggleAuxiliaryPane } =
		useAdaptiveWorkspaceLayout();
	...
	const handleVisibleFileChange = useCallback(
		(event: NativeSyntheticEvent<{ readonly fileId?: string | null }>) => {
			reviewFileNavigatorRef.current?.setVisibleFile(
				event.nativeEvent.fileId ?? null,
			);
		},
		[],
	);
	...
	const parsedDiffNotice =
		parsedDiff.kind === "files" || parsedDiff.kind === "raw"
			? parsedDiff.notice
			: null;
	const hasCachedSelectedDiff = selectedSection?.diff != null;
	const hasAnyCachedDiff = reviewSections.some(
		(section) => section.diff != null,
	);
	const sectionMenu = useMemo(
		() => buildReviewSectionMenu(reviewSections),
		[reviewSections],
	);
	const { showConnectionNotice, showSectionToolbar } =
	...
	const androidSectionMenuActions = useMemo<MenuAction[]>(() => {
		const sectionAction = (
			...
			title:
				section?.id === selectedSection?.id ? `${title} (selected)` : title,
	...
	const listHeader = useMemo(() => {
		...
		if (parsedDiffNotice) {
			children.push(
				<ReviewNotice key="review-notice" notice={parsedDiffNotice} />,
			);
		}
		...
		<>
					...
					isAndroid
						...
						: {
								...
								unstable_headerSubtitle:
									Platform.OS === "ios" && headerSubtitle.length > 0
										? headerSubtitle
										: undefined,
							}
				}
			...
			<View className="flex-1 bg-sheet">
					...
					>
						...
						{!selectedSection ? (
							<View className="border-b border-border bg-card px-4 py-5">
								...
								<Text className="text-xs leading-normal text-foreground-muted">
									This thread has no ready turn diffs and the worktree diff is
									empty.
								</Text>
							</View>
				...
				<ReviewSelectionActionBar
					...
					onOpenComment={
						commentSelection.selectionAction?.onOpenComment ?? null
					}
