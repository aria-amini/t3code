...
describe("getCachedNativeReviewDiffData", () => {
	...
	it("reuses equivalent comment contents and invalidates changed comments", () => {
		const first = getCachedNativeReviewDiffData(
			buildInput([makeComment("First")]),
		);
		const equivalent = getCachedNativeReviewDiffData(
			buildInput([makeComment("First")]),
		);
		const changed = getCachedNativeReviewDiffData(
			buildInput([makeComment("Changed")]),
		);

		expect(equivalent).toBe(first);
		expect(changed).not.toBe(first);
		expect(
			changed.rows.find((row) => row.kind === "comment")?.commentText,
		).toBe("Changed");
	});

	it("reuses source rows and word matching when a file comment changes", () => {
		const diff = buildReviewParsedDiff(
			filesPatch(["example.ts", "second.ts"]),
			"comment-reuse",
		);
		...
		try {
			...
			expect(
				base.rows.filter((row) => row.wordDiffRanges?.length),
			).toHaveLength(8);
			...
			const changed = getCachedNativeReviewDiffData({
				parsedDiff: diff,
				comments: [
					{ ...firstComment, text: "Changed first comment" },
					secondComment,
				],
			});
			...
			const removed = getCachedNativeReviewDiffData({
				parsedDiff: diff,
				comments: [],
			});
			...
			expect(matchWords).toHaveBeenCalledTimes(4);
			expect(
				first.rows.find((row) => row.id === firstComment.id)?.commentText,
			).toBe("First file comment");
		} finally {
	...
	it("updates comment titles and locations without changing source coordinates", () => {
		...
		expect(
			renamed.rows.find((row) => row.id === comment.id)?.commentSectionTitle,
		).toBe("Branch comparison");
		const moved = getCachedNativeReviewDiffData({
			parsedDiff: diff,
			comments: [
				{ ...comment, filePath: "second.ts", endIndex: 99, rangeLabel: "+2" },
			],
		});
		...
		expect(
			first.rows.find((row) => row.id === comment.id)?.commentSectionTitle,
		).toBe("Dirty worktree");
	});
	...
	it("keeps cached IDs and targets local to each parsed section and file layout", () => {
		...
		const variants = [
			...
			buildReviewParsedDiff(
				filesPatch(["second.ts", "example.ts"]),
				"first-section",
			),
		...
		for (const diff of [...variants, variants[0]!]) {
			const input = {
				parsedDiff: diff,
				comments: [makeComment("Coordinates")],
			};
			expect(getCachedNativeReviewDiffData(input)).toEqual(
				buildNativeReviewDiffData(input),
			);
		}
	});
});
...
describe("createNativeReviewDiffTheme", () => {
	...
	it("uses the selected app palette for native code surfaces", () => {
		...
		const iris = createNativeReviewDiffTheme(
			"dark",
			"iris",
			appTheme("iris", "dark"),
		);
