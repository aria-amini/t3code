...
interface PreparedNativeReviewDiffData extends Omit<
...
const nativeReviewDiffDataCache = new WeakMap<
	ReviewParsedDiff,
	CachedNativeReviewDiffData
>();

function buildReviewCommentsCacheKey(
...
export function createNativeReviewDiffTheme(
	...
	const background = opaqueNativeHexColor(
		appTheme["--color-sheet"],
		appTheme["--color-screen"],
	);
	const nativeColor = (color: string) =>
		opaqueNativeHexColor(color, background);
...
function getLanguageForPath(
	...
	if (normalizedPath.endsWith(".js") || normalizedPath.endsWith(".cjs"))
		return "javascript";
...
function shouldUseWordDiffRanges(
	...
	if (
		ranges.length === 0 ||
		ranges.length > NATIVE_REVIEW_MAX_WORD_DIFF_RANGE_COUNT
	) {
		return false;
	}
	...
	const highlightedLength = ranges.reduce(
		(total, range) =>
			total + nonWhitespaceLength(content.slice(range.start, range.end)),
		0,
	);
	return (
		highlightedLength / meaningfulLength <= NATIVE_REVIEW_MAX_WORD_DIFF_COVERAGE
	);
}
...
function addNativeWordDiffRanges(
	...
	while (index < nextRows.length) {
		...
		const pairedCount = Math.min(
			deletedRowIndexes.length,
			addedRowIndexes.length,
		);
		for (let pairIndex = 0; pairIndex < pairedCount; pairIndex += 1) {
			...
			const deletionRanges = trimWordDiffRanges(
				deletedRow.content,
				ranges.deletion,
			);
			const additionRanges = trimWordDiffRanges(
				addedRow.content,
				ranges.addition,
			);

			if (shouldUseWordDiffRanges(deletedRow.content, deletionRanges)) {
				nextRows[deletedRowIndex] = {
					...deletedRow,
					wordDiffRanges: deletionRanges,
				};
			}
			if (shouldUseWordDiffRanges(addedRow.content, additionRanges)) {
				nextRows[addedRowIndex] = {
					...addedRow,
					wordDiffRanges: additionRanges,
				};
			}
		}
...
function prepareFileRows(
	...
	const lineRows = file.rows.filter(
...
function prepareNativeReviewDiffData(
	parsedDiff: ReviewParsedDiff,
): PreparedNativeReviewDiffData {
...
export function buildNativeReviewDiffData(
	...
	return buildCommentedNativeReviewDiffData(
		prepareNativeReviewDiffData(parsedDiff),
		comments,
	);
}
...
export function getCachedNativeReviewDiffData(
	...
	const prepared =
		cached?.prepared ?? prepareNativeReviewDiffData(input.parsedDiff);
