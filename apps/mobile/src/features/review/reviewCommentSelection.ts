...
export function getReviewUnifiedLineNumber(
	line: ReviewRenderableLineRow,
): number | null {
	return line.newLineNumber ?? line.oldLineNumber;
}

export function getReviewChangeMarker(
...
export function buildReviewCommentTarget(
	target: Pick<
		ReviewCommentTarget,
		"sectionId" | "sectionTitle" | "filePath" | "lines"
	>,
	anchorIndex: number,
	lineIndex: number,
): ReviewCommentTarget {
...
export function formatReviewSelectedRangeLabel(
	target: ReviewCommentTarget,
): string {
...
export function formatReviewCommentContext(
	target: ReviewCommentTarget,
	comment: string,
): string {
