...
export type ReviewRenderableRow =
...
function countReviewRenderableLineRows(file: ReviewRenderableFile): number {
	return file.rows.reduce(
		(total, row) => total + (row.kind === "line" ? 1 : 0),
		0,
	);
}
...
function countReviewRenderableCharacters(file: ReviewRenderableFile): number {
	return file.rows.reduce(
		(total, row) =>
			total + (row.kind === "line" ? row.content.length : row.header.length),
		0,
	);
}

export function getReviewFilePreviewState(
	file: ReviewRenderableFile,
): ReviewFilePreviewState {
	...
	if (
		lineCount > LARGE_DIFF_LINE_THRESHOLD ||
		characterCount > LARGE_DIFF_CHARACTER_THRESHOLD
	) {
		return {
			kind: "suppressed",
			reason: "large",
...
): ReadonlyArray<ReviewRenderableRow> {
...
function mapRenderableFile(file: FileDiffMetadata): ReviewRenderableFile {
	const path =
		stripGitPrefix(file.name) ?? stripGitPrefix(file.prevName) ?? file.name;
	...
	const cacheKey =
		file.cacheKey ?? `${previousPath ?? "none"}:${path}:${file.type}`;
...
export function buildReviewSectionItems(input: {
	...
	const turnItems = getReadyReviewCheckpoints(
		input.checkpoints,
	).map<ReviewSectionItem>((checkpoint) => {
	...
	const hasDirtyWorktreeItem = gitItems.some(
		(item) => item.id === DIRTY_WORKTREE_SECTION_ID,
	);
