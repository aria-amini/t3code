...
it("reuses unchanged parsed diffs and replaces changed sections", () => {
	...
	const changed = {
		...reviewInput,
		diff: reviewInput.diff.replace("value = 1", "value = 2"),
	};
...
it("evicts the least recently used section across threads without changing live results or IDs", () => {
	...
	for (let index = 0; index < 6; index += 1) {
		getCachedReviewParsedDiff({
			...reviewInput,
			threadKey: `env-local:thread-${index}`,
		});
	}
	...
	getCachedReviewParsedDiff({
		...reviewInput,
		threadKey: "env-local:thread-new",
	});
	...
	assert.strictEqual(
		getCachedNativeReviewDiffData({ parsedDiff: oldest }),
		native,
	);
	assert.deepStrictEqual(
		getCachedNativeReviewDiffData({ parsedDiff: rebuilt }),
		native,
	);
});
