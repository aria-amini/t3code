...
>({});
const EMPTY_REVIEW_GIT_SECTIONS_ATOM = Atom.make(
	EMPTY_GIT_REVIEW_SECTIONS,
).pipe(Atom.keepAlive, Atom.withLabel("mobile:review:git-sections:null"));
const EMPTY_REVIEW_TURN_DIFFS_ATOM = Atom.make(EMPTY_REVIEW_TURN_DIFFS).pipe(
	Atom.keepAlive,
	Atom.withLabel("mobile:review:turn-diffs:null"),
);
const EMPTY_REVIEW_SELECTED_SECTION_ID_ATOM = Atom.make<string | null>(
	null,
).pipe(
	Atom.keepAlive,
...
const EMPTY_REVIEW_SECTION_FILE_IDS_ATOM = Atom.make(
	EMPTY_REVIEW_SECTION_FILE_IDS,
).pipe(Atom.keepAlive, Atom.withLabel("mobile:review:section-file-ids:null"));
...
const reviewSelectedSectionIdByThreadKeyAtom = Atom.family(
	(threadKey: string) =>
...
const reviewRevealedLargeFileIdsByThreadKeyAtom = Atom.family(
	(threadKey: string) =>
		Atom.make(EMPTY_REVIEW_SECTION_FILE_IDS).pipe(
			Atom.keepAlive,
...
export function useReviewCacheForThread(input: {
	...
	const gitSections = useAtomValue(
		threadKey
			? reviewGitSectionsByThreadKeyAtom(threadKey)
			: EMPTY_REVIEW_GIT_SECTIONS_ATOM,
	);
	const turnDiffById = useAtomValue(
		threadKey
			? reviewTurnDiffByThreadKeyAtom(threadKey)
			: EMPTY_REVIEW_TURN_DIFFS_ATOM,
	);
	...
	const asyncState = useAtomValue(
		threadKey
			? reviewAsyncStateByThreadKeyAtom(threadKey)
			: EMPTY_REVIEW_ASYNC_STATE_ATOM,
	);
	...
	const viewedFileIdsBySection = useAtomValue(
		threadKey
			? reviewViewedFileIdsByThreadKeyAtom(threadKey)
			: EMPTY_REVIEW_SECTION_FILE_IDS_ATOM,
	);
...
export function setReviewSelectedSectionId(
	threadKey: string,
...
export function setReviewAsyncError(
	threadKey: string,
	error: string | null,
): void {
	updateReviewAsyncState(threadKey, (current) => ({
...
export function getReviewAsyncStateSnapshot(
	threadKey: string,
): ReviewAsyncState {
...
export function getReviewParsedDiffSourceCharacterCount(input: {
	...
	const cached = cache.entries.get(
		buildSectionCacheKey(input.threadKey, input.sectionId),
	);
