...
export function applyDiffRangesToTokens(
	...
	tokens.forEach((token) => {
		...
		while (
			...
			ranges[rangeIndex]!.end <= tokenStart
		) {
			rangeIndex += 1;
		}
		...
		if ((ranges[rangeIndex]?.start ?? Number.POSITIVE_INFINITY) >= tokenEnd) {
			nextTokens.push(
				token.diffHighlight ? { ...token, diffHighlight: false } : token,
			);
			return;
		}
		...
		while (
			(ranges[localRangeIndex]?.start ?? Number.POSITIVE_INFINITY) < tokenEnd
		) {
			...
			if (highlightedEnd > highlightedStart) {
				appendTokenSegment(
					...
					token,
					token.content.slice(
						highlightedStart - tokenStart,
						highlightedEnd - tokenStart,
					),
					true,
				);
			}
