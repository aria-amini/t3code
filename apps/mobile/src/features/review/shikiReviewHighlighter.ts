...
import {
	applyDiffRangesToTokens,
	computeWordAltDiffRanges,
} from "./reviewWordDiffs";
...
const REVIEW_HIGHLIGHTER_ENGINE_PREFERENCE =
	resolveReviewHighlighterEnginePreference(REVIEW_HIGHLIGHTER_ENGINE_ENV_VALUE);
...
let activeHighlighterEnginePromise: Promise<ReviewHighlighterEngine> | null =
	null;
...
function logReviewHighlighterDiagnosticError(
	message: string,
	error: unknown,
): void {
	if (!isReviewHighlighterDebugLoggingEnabled()) {
...
async function highlightLines(
	...
	for (let lineIndex = 0; lineIndex < sourceLines.length; lineIndex += 1) {
		...
		if (
			...
			(shortLineBatch.length === 0 ||
				line.length > REVIEW_TOKENIZE_MAX_LINE_LENGTH)
		) {
...
export async function highlightCodeSnippet(input: {
	...
	const language = await resolveLanguageFromPath(
...
export async function highlightReviewSelectedLines(input: {
	...
	const loadedLanguage = resolveLoadedLanguageFromPath(
		input.filePath,
		input.languageHint ?? null,
	);
	const language =
		loadedLanguage ??
		(await resolveLanguageFromPath(input.filePath, input.languageHint ?? null));
