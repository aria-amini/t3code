...
export function useNativeReviewDiffBridge(input: {
	...
	const collapsedFileIdsJson = useMemo(
		() => JSON.stringify(collapsedFileIds),
		[collapsedFileIds],
	);
	const viewedFileIdsJson = useMemo(
		() => JSON.stringify(viewedFileIds),
		[viewedFileIds],
	);
	...
	const { tokensPatchJson, updateVisibleRange } =
