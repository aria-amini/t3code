...
function prepareSelection(
	...
	for (const pending of getReviewDiffPrewarmSections({
		threadKey,
		sections,
		selectedSectionId,
	})) {
	...
	assert.strictEqual(
		getCachedNativeReviewDiffData({ parsedDiff: parsed }),
		native,
	);
...
it("reserves a larger retained source when a neighboring input loses whitespace", () => {
	const parsed = getCachedReviewParsedDiff({
		threadKey,
		sectionId: "turn:1",
		diff: reviewDiff.padEnd(
			MAX_CACHED_REVIEW_SOURCE_CHARACTERS - reviewDiff.length,
			" ",
		),
	});
	const native = getCachedNativeReviewDiffData({ parsedDiff: parsed });
...
it("does not parse an oversized direct prewarm that the cache cannot retain", () => {
	...
	prewarmReviewDiffSection({
		...
		section: makeSection(
			0,
			"x".repeat(MAX_CACHED_REVIEW_SOURCE_CHARACTERS + 1),
		),
	});
