...
export function useReviewSections(input: {
	...
	const selectedSection = useMemo(
		() =>
			reviewSections.find(
				(section) => section.id === reviewCache.selectedSectionId,
			) ??
			reviewSections[0] ??
			null,
	...
	if (selectedSection?.kind === "turn") {
		activeCheckpoint =
			checkpointBySectionId[selectedSection.id] ?? activeCheckpoint;
	}
	...
	return {
		error:
			diffPreview.error ?? activeTurnDiff.error ?? reviewCache.asyncState.error,
