...
export function SettingsClientStorageRouteScreen() {
	...
	const summary = AsyncResult.isSuccess(summaryResult)
		? summaryResult.value
		: null;
	const environmentSummaries = useMemo(
		() =>
			[...(summary?.environments ?? [])].sort((left, right) => {
				const leftLabel =
					savedConnectionsById[left.environmentId]?.environmentLabel ?? "";
				const rightLabel =
					savedConnectionsById[right.environmentId]?.environmentLabel ?? "";
				return leftLabel.localeCompare(rightLabel);
			}),
		[savedConnectionsById, summary?.environments],
	);

	const confirmClearEnvironment = (
		environment: EnvironmentClientCacheSummary,
	) => {
		const label =
			...
			>
				<SettingsSection title="Environment caches">
					{AsyncResult.isFailure(summaryResult) ? (
							...
							<CacheEnvironmentRow
								...
								environmentLabel={
									savedConnectionsById[environment.environmentId]
										?.environmentLabel ?? environment.environmentId
								}
						...
						<View className="items-center gap-2 px-6 py-8">
							...
							<Text className="text-center text-sm text-foreground-muted">
								Offline cache records will appear here after environments are
								used.
							</Text>
						</View>
					)}
				...
				<View className="gap-3">
					...
					<Text className="px-2 text-sm leading-normal text-foreground-muted">
						Clearing caches never removes environment connections, credentials,
						account data, or appearance preferences.
					</Text>
					{AsyncResult.isFailure(summaryResult) ||
					AsyncResult.isFailure(clearResult) ? (
						<Text selectable className="px-2 text-sm text-danger-foreground">
							Client storage is temporarily unavailable. Try again after
							restarting the app.
						</Text>
					) : null}
				</View>
