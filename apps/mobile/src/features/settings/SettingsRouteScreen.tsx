...
function ConfiguredSettingsRouteScreen() {
	...
	const agentAwarenessPlatform = resolveAgentAwarenessPlatformPresentation(
		Platform.OS,
	);
	const insets = useSafeAreaInsets();
	const navigation = useNavigation();
	...
	const [liveActivityStatus, setLiveActivityStatus] =
		useState<LiveActivityStatus>("checking");
	const deviceRegistered = useDeviceRegistered();
	const liveActivitiesPreferenceEnabled = AsyncResult.isSuccess(
		preferencesResult,
	)
			...
			>
				<View className="gap-3">
					...
					<Text className="px-2 text-sm text-foreground-muted">
						T3 Code works locally without signing in. Cloud features are
						optional.
					</Text>
				</View>
...
const AUTO_SETTLE_DEFAULT_DAYS =
	DEFAULT_SERVER_SETTINGS.sidebarAutoSettleAfterDays ?? 3;
	...
	return (
		<>
			<SettingsSwitchRow
				...
				onValueChange={(value) =>
					writeToAll({ sidebarAutoSettleOnMerge: value })
				}
			/>
			<SettingsSwitchRow
				icon="clock"
				...
				subtitle={
					afterDays === null
						? undefined
						: `After ${afterDays} days without activity`
				}
						...
						onPress={() => {
							...
							for (const mismatch of mismatches) {
								const target = environments.find(
									(candidate) =>
										candidate.environmentId === mismatch.environmentId,
								);
								void updateSettings({
...
function LegacySettingsSection() {
	...
	const planModeEnabled =
		...
		preferences.value.planModeEnabled === true;
		...
		<View className="gap-3">
			<SettingsSection title="Legacy">
				<SettingsSwitchRow
					...
					onValueChange={(value) =>
						savePreferences({ legacyThreadListEnabled: value })
					}
				/>
				<SettingsSwitchRow
					icon="hammer"
			...
			<Text className="px-2 text-sm text-foreground-muted">
				Opt into retired interfaces kept for compatibility. Plan Mode restores
				the Build/Plan control; otherwise every task runs in Build mode.
			</Text>
		</View>
	);
}
