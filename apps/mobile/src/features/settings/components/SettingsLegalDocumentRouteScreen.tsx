...
export function SettingsLegalDocumentExternalHeaderButton({
	...
	const safeExternalUrl = isLegalDocumentUrl(externalUrl)
		? externalUrl
		: LEGAL_URL;
	...
	return (
		<Pressable
			...
			hitSlop={12}
			onPress={() =>
				void Linking.openURL(safeExternalUrl).catch(() => undefined)
			}
			className="p-2 active:opacity-60"
		>
			<SymbolView
	...
	return (
		<View className="relative flex-1 bg-sheet">
			{loadProgress > 0 && loadProgress < 1 ? (
				<LoadingStrip progress={loadProgress} />
			) : null}
			<WebView
				key={reloadKey}
				...
				onError={(event) => {
					setLoadProgress(0);
					setLoadError(
						event.nativeEvent.description || "The page could not be loaded.",
					);
				}}
				onHttpError={(event) => {
					...
					setLoadProgress(0);
					setLoadError(
						`The server returned status ${event.nativeEvent.statusCode}.`,
					);
				}}
				renderLoading={() => (
