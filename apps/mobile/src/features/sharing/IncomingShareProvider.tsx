...
const IncomingShareContext =
	React.createContext<IncomingShareContextValue | null>(null);
...
async function resolvedPayloadsForFiles(): Promise<
	ReadonlyArray<ResolvedSharePayload>
> {
	try {
		...
		// resolution fails.
		console.warn(
			"[incoming-share] could not resolve shared file metadata",
			error,
		);
		return [];
	}
...
async function incomingShareIdForPayloads(
...
async function removeOwnedFile(uri: string): Promise<void> {
	...
	try {
		...
		const ownedRootUris = [
			Paths.document.uri,
			Paths.cache.uri,
			...Object.values(Paths.appleSharedContainers ?? {}).map(
		...
		console.warn(
			"[incoming-share] could not remove temporary shared file",
			error,
		);
	}
}

async function removeReplayedPayloadFiles(
	payloads: ReadonlyArray<SharePayload>,
): Promise<void> {
...
const incomingShareInbox = new IncomingShareInbox({
	...
	buildDraft: async ({ payloads, id, createdAt }) => {
		...
		const resolvedPayloads =
			!hasGenericFilePayload &&
			payloads.some((payload) => payload.shareType === "image")
				? await resolvedPayloadsForFiles()
				: [];
	...
	onClearError: (error) => {
		console.warn(
			"[incoming-share] could not acknowledge native payload",
			error,
		);
	},
...
export function IncomingShareProvider(props: React.PropsWithChildren) {
	...
	return (
		<IncomingShareContext.Provider value={value}>
			{props.children}
...
export function useIncomingShare(): IncomingShareContextValue {
	...
	if (value === null) {
		throw new Error(
			"useIncomingShare must be used within IncomingShareProvider.",
		);
	}
	return value;
}
