...
import {
	IncomingShareInbox,
	type IncomingShareInboxDependencies,
} from "./incoming-share-inbox";
...
function createHarness(
	overrides: Partial<IncomingShareInboxDependencies> = {},
) {
...
describe("IncomingShareInbox", () => {
	...
	it("rejects moving a reserved share to another project", async () => {
		...
		expect(persisted.get("share-stable")?.destination?.projectId).toBe(
			"project-1",
		);
	});
	...
	it("conditionally releases a reservation when its project is unavailable", async () => {
		...
		await expect(
			inbox.releaseReservation("share-stable", destination),
		).resolves.toEqual([draft("share-stable")]);
