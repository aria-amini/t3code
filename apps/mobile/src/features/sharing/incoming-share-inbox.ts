...
export interface IncomingShareInboxDependencies {
	...
	readonly cleanupReplayedPayloads?: (
		payloads: ReadonlyArray<SharePayload>,
	) => Promise<void>;
	readonly idForPayloads: (
		payloads: ReadonlyArray<SharePayload>,
	) => Promise<string>;
	readonly now: () => string;
		...
		return this.runExclusive(async () => {
			...
			if (loaded.some((draft) => draft.id === shareId)) {
				if (this.dependencies.cleanupReplayedPayloads) {
					await this.cleanup(() =>
						this.dependencies.cleanupReplayedPayloads!(payloads),
					);
				}
			...
			if (!hasIncomingShareContent(draft)) {
				...
				throw new Error(
					draft.warnings[0] ??
						"The shared content is not supported by the composer.",
				);
			}
		...
		return this.runExclusive(async () => {
			...
			if (target.destination) {
				if (
					...
					throw new Error(
						"The shared content is already reserved for another project draft.",
					);
				}
		...
		return this.runExclusive(async () => {
			...
			if (
				target.destination.environmentId !==
					expectedDestination.environmentId ||
				target.destination.projectId !== expectedDestination.projectId
			) {
				throw new Error(
					"The shared content reservation changed before it could be released.",
				);
			}
