...
const decodeIncomingShareDraftSync = Schema.decodeUnknownSync(
...
export function selectIncomingShareAttachments(input: {
	...
	for (const attachment of input.attachments) {
		...
		if (input.maxFileAttachmentBytes === null) {
			warnings.push(
				`'${attachment.name}' was skipped because this server does not support files.`,
			);
			continue;
		}
		...
		if (attachment.sizeBytes > maxFileAttachmentBytes) {
			warnings.push(
				fileAttachmentTooLargeMessage(attachment.name, maxFileAttachmentBytes),
			);
			continue;
		}
		attachments.push(attachment);
...
export function selectIncomingShareAttachmentsForServer(input: {
	...
	const hasFiles = input.attachments.some(
...
async function releaseOwnedFiles(
	...
	for (const uri of new Set(
		uris.filter((candidate): candidate is string => Boolean(candidate)),
	)) {
		try {
	...
	for (const [index, payload] of input.payloads.entries()) {
		...
		if (payload.shareType !== "image") {
			...
			const sharedFileName =
				...
				payload.originalName.trim().length > 0
					? payload.originalName
					: undefined;
			const name =
				resolved?.originalName ??
				sharedFileName ??
				fallbackName(uri, index, mimeType);
			if (!uri) {
			...
			try {
				let sizeBytes =
					...
					(await input.fileReader.readSize?.(uri)) ??
					null;
				if (
					(sizeBytes === null ||
						(sizeBytes === 0 && uri.startsWith("content:"))) &&
					input.fileReader.persistFile
				) {
					persistedFileUri = await input.fileReader.persistFile(uri, name);
					sizeBytes =
						(await input.fileReader.readSize?.(persistedFileUri)) ?? null;
				}
				if (sizeBytes === null) {
				...
				await releaseOwnedFiles(
					input.fileReader,
					[uri, payload.value].filter(
						(candidate) => candidate !== retainedFileUri,
					),
				);
			}
			continue;
