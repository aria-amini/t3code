...
export async function loadIncomingShareDrafts(options?: {
	...
	try {
		...
		for (const entry of (await getDirectory()).list()) {
			...
			try {
				drafts.push(
					decodeIncomingShareDraft(JSON.parse(await entry.text()) as unknown),
				);
			} catch (cause) {
				const error = new IncomingShareStorageError({
					operation: "load",
					shareId: null,
					cause,
				});
		...
		throw new IncomingShareStorageError({
			operation: "load",
			shareId: null,
			cause,
		});
...
export async function writeIncomingShareDraft(
