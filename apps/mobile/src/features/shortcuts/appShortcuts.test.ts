...
describe("withRecentThreadShortcut", () => {
	it("prepends a newly opened thread", () => {
		...
		expect(next.map((entry) => entry.threadId)).toEqual([
			"thread-b",
			"thread-a",
		]);
	});
	...
	it("moves a reopened thread to the front without duplicating it", () => {
		const next = withRecentThreadShortcut(
			[thread("a"), thread("b")],
			thread("b"),
		);
		expect(next.map((entry) => entry.threadId)).toEqual([
...
describe("shortcutHref", () => {
	it("accepts exactly the destinations shortcuts can produce", () => {
		expect(
			shortcutHref({ id: "x", title: "x", params: { href: "/new" } }),
		).toBe("/new");
		expect(
			shortcutHref({
				id: "x",
				title: "x",
				params: { href: "/threads/env-1/thread-2" },
			}),
		).toBe("/threads/env-1/thread-2");
		expect(
			shortcutHref({
				id: "x",
				title: "x",
				params: { href: "/threads/env%201/thread%2F2" },
			}),
	...
	it("rejects everything else", () => {
		for (const href of [
			...
			expect(shortcutHref({ id: "x", title: "x", params: { href } })).toBe(
				null,
			);
		}
		expect(shortcutHref({ id: "x", title: "x", params: { href: 3 } })).toBe(
			null,
		);
...
describe("launcher shortcut ids", () => {
	it("cannot collide across different env/thread pairs", () => {
		const a = buildShortcutActions([
			{ environmentId: "a-b", threadId: "c", title: "x" },
		]);
		const b = buildShortcutActions([
			{ environmentId: "a", threadId: "b-c", title: "x" },
		]);
