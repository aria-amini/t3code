...
function useShortcutNavigation(): void {
	...
	useEffect(() => {
		...
		if (!handledInitialAction.current) {
			...
			const initialHref = QuickActions.initial
				? shortcutHref(QuickActions.initial)
				: null;
			if (initialHref !== null) {
				linkTo(initialHref);
			}
...
function useRecentThreadShortcutSync(state: NavigationState): void {
	...
	const [recents, setRecents] =
		useState<ReadonlyArray<RecentThreadShortcut> | null>(null);
	...
	useEffect(() => {
		...
		setRecents((current) => {
			...
			const next = withRecentThreadShortcut(current, {
				environmentId,
				threadId,
				title,
			});
	...
	useEffect(() => {
			...
			saveQueueRef.current = saveQueueRef.current.then(
				() =>
					saveRecentThreadShortcuts(recents).catch((error) => {
						console.warn(
							"[app-shortcuts] failed to persist recent threads",
							error,
						);
					}),
				() => undefined,
		...
		void QuickActions.setItems(buildShortcutActions(recents)).catch((error) => {
			console.warn(
				"[app-shortcuts] failed to update launcher shortcuts",
				error,
			);
		});
	}, [recents]);
