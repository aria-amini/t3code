...
function sceneFromPathname(pathname: string): ShowcaseScene | null {
	...
	if (
		routePath === "/settings" ||
		routePath.endsWith("/settings/environments")
	) {
		return "environments";
	}
...
export function ShowcaseCaptureCoordinator(props: {
	...
	const [requestedScene, setRequestedScene] = useState<ShowcaseScene | null>(
		null,
	);
	const [requestedTheme, setRequestedTheme] = useState<MobileThemeId | null>(
		null,
	);
	...
	useEffect(() => {
		...
		void retryShowcaseOperation(
			async () => applyNativeShowcaseOrientation(orientation),
			{
			...
			await Promise.all(
				pairingUrls.map(async (pairingUrl) => {
					...
					const paired = await retryShowcaseOperation(
						async () =>
							AsyncResult.isSuccess(await connectPairingUrl(pairingUrl)),
						{ isCancelled: () => cancelled },
					);
	...
	const showcaseThread = threads.find(
		(thread) => String(thread.id) === SHOWCASE_THREAD_ID,
	);
	...
	useEffect(() => {
		...
		if (pendingTasks.length !== SHOWCASE_PENDING_TASK_DEFINITIONS.length)
			return;
