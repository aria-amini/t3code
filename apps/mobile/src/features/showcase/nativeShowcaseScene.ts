...
interface NativeShowcaseControls {
	...
	readonly applyShowcaseOrientation?: (
...
function nativeShowcaseControls(): NativeShowcaseControls | null {
	return requireOptionalNativeModule<NativeShowcaseControls>(
		"T3NativeControls",
	);
}
...
export function getNativeShowcaseOrientation(): ShowcaseOrientation | null {
	try {
		const orientation = nativeShowcaseControls()
			?.getShowcaseOrientation?.()
			?.trim();
...
export async function applyNativeShowcaseOrientation(
	...
	if (
		!controls?.applyShowcaseOrientation ||
		!controls.getInterfaceOrientation
	) {
