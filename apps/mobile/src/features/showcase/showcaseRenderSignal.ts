...
export function subscribeToShowcaseRenderSignal(
	listener: () => void,
): () => void {
...
export function reportShowcaseSceneRendered(
	signal: ShowcaseRenderSignal,
): void {
	renderSignal = signal;
...
export function isShowcaseNativeContentReady(input: {
	...
	return (
		input.renderSignal?.scene === "review" &&
		input.renderSignal.themeId === input.themeId
	);
}
