...
function pickRunningTerminalSessionForBootstrap(
	...
	return (
		running.find(
			(session) => session.target.terminalId === DEFAULT_TERMINAL_ID,
		) ??
		running[0] ??
		null
	);
...
export function ThreadTerminalRouteScreen(
	props: ThreadTerminalRouteScreenProps,
) {
	...
	const resizeTerminal = useAtomCommand(
		terminalEnvironment.resize,
		"terminal resize",
	);
	const clearTerminal = useAtomCommand(
		terminalEnvironment.clear,
		"terminal clear",
	);
	const closeTerminal = useAtomCommand(
	...
	const routeEnvironmentId = routeEnvironmentIdRaw
		...
		: null;
	const routeThreadId = routeThreadIdRaw
		? ThreadId.make(routeThreadIdRaw)
		: null;
	const environment = useEnvironmentPresentation(routeEnvironmentId);
	const isEnvironmentReady =
		environment.presentation?.connection.phase === "connected";
	...
	const [pendingLaunchEntry, setPendingLaunchEntry] = useState<{
		...
		key: launchTargetKey,
		launch:
			launchTarget === null ? null : takePendingTerminalLaunch(launchTarget),
	}));
	const pendingLaunch =
		pendingLaunchEntry.key === launchTargetKey
			? pendingLaunchEntry.launch
			: null;
	...
	const initialAttachGridSize =
		initialAttachGridEntry.key === launchTargetKey
			? initialAttachGridEntry.size
			: null;
	...
	const bufferReplayTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
		null,
	);
	...
	const [readyBufferReplayKey, setReadyBufferReplayKey] = useState<
		string | null
	>(null);
	...
	const hostOs =
		routeEnvironmentId === null
			? null
			: (serverConfigs.get(routeEnvironmentId)?.environment.platform.os ??
				null);
	...
	const pendingModifier =
		pendingModifierState.terminalId === terminalId
			? pendingModifierState.value
			: null;
	const headerSubtitle = selectedThreadProject?.title ?? "";
	const terminalToolbarActions = useMemo<
		ReadonlyArray<TerminalToolbarAction>
	>(() => {
	...
	useEffect(() => {
		const keyboardWillShow = KeyboardEvents.addListener(
			"keyboardWillShow",
			() => {
				setIsAccessoryDismissed(false);
			},
		);
		const keyboardWillHide = KeyboardEvents.addListener(
			"keyboardWillHide",
			() => {
	...
	const pasteSessionRef = useRef<ReturnType<
		typeof createTerminalPasteSession
	> | null>(null);
		...
		navigation.dispatch(
			StackActions.replace("ThreadTerminal", {
				...
				terminalId: nextOpenTerminalId({
					listedTerminalIds: terminalMenuSessions.map(
		...
		(action: TerminalToolbarAction) => {
			if (action.kind === "modifier") {
				setPendingModifierState((current) => ({
					terminalId,
					value:
						(current.terminalId === terminalId ? current.value : null) ===
						action.modifier
							? null
							: action.modifier,
				}));
			...
			<NativeStackScreenOptions
				options={{
					...
					unstable_headerSubtitle:
						usesNativeHeaderGlass && headerSubtitle.length > 0
							? headerSubtitle
							: undefined,
				}}
			/>
					...
					>
						...
						{terminalMenuSessions.map((session) => (
							<NativeHeaderToolbar.MenuAction
								...
								icon={
									session.terminalId === terminalId ? "checkmark" : "terminal"
								}
			...
			>
				{!isEnvironmentReady ? (
					...
					<>
						<View
							className="flex-1"
							style={{ paddingBottom: terminalBottomInset }}
						>
							<TerminalSurface
