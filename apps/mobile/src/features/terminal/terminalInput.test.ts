...
describe("encodeTerminalPaste", () => {
	it("passes single-line text through unchanged", () => {
		expect(encodeTerminalPaste("git switch -c fix/paste")).toBe(
			"git switch -c fix/paste",
		);
	...
	it("replaces unsafe control bytes with spaces but keeps tabs", () => {
		expect(encodeTerminalPaste(`a${byte(0)}b${ESC}c${byte(0x7f)}d\te`)).toBe(
			"a b c d\te",
		);
	});
...
describe("chunkTerminalWrite", () => {
	it("leaves writes within the wire limit whole", () => {
		...
		expect(
			chunkTerminalWrite("x".repeat(TERMINAL_WRITE_MAX_LENGTH)),
		).toHaveLength(1);
	});

	it("splits oversized writes so every chunk fits the contract", () => {
		const chunks = chunkTerminalWrite(
			"y".repeat(TERMINAL_WRITE_MAX_LENGTH * 2 + 5),
		);
