...
const terminalMenuSessionOrder = Order.make<TerminalMenuSession>(
	(left, right) => {
		const comparison = left.terminalId.localeCompare(
			right.terminalId,
			undefined,
			{ numeric: true },
		);
...
export function previousLiveTerminalId(input: {
	...
	const below = live.filter(
		(session) =>
			session.terminalId.localeCompare(input.exitedTerminalId, undefined, {
				numeric: true,
			}) < 0,
	);
