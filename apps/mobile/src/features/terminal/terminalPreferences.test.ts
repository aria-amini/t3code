...
describe("normalizeTerminalFontSize", () => {
	it("returns the default size for missing or invalid values", () => {
		expect(normalizeTerminalFontSize(undefined)).toBe(
			DEFAULT_TERMINAL_FONT_SIZE,
		);
	...
	it("clamps below the minimum", () => {
		expect(normalizeTerminalFontSize(MIN_TERMINAL_FONT_SIZE - 4)).toBe(
			MIN_TERMINAL_FONT_SIZE,
		);
	});

	it("clamps above the maximum", () => {
		expect(normalizeTerminalFontSize(MAX_TERMINAL_FONT_SIZE + 4)).toBe(
			MAX_TERMINAL_FONT_SIZE,
		);
	});
