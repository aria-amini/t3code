...
const SKILL_SOURCE_SYMBOL_BY_KIND: Record<
...
function itemIcon(item: ComposerCommandItem): AppSymbolName | null {
	switch (item.type) {
		...
		case "provider-slash-command":
			return "terminal";
		case "skill":
			return SKILL_SOURCE_SYMBOL_BY_KIND[
				resolveProviderSkillSourceKind(item.skill)
			];
		case "path":
			return null;
	}
...
const CommandRow = memo(function CommandRow(props: {
	...
	return (
		...
		>
			...
			<Text
				className="shrink-0 text-base font-t3-medium text-foreground"
				numberOfLines={1}
			>
				...
				>
					{props.items.map((item, index) => (
						<CommandRow
							...
							isSlashSkill={
								props.triggerKind === "slash-command" && item.type === "skill"
							}
						/>
					))}
				</ScrollView>
