...
export function NewTaskDraftRouteScreen({
	route,
}: StaticScreenProps<NewTaskDraftRouteParams>) {
	...
	const needsPreparation = Boolean(
		initialProjectRef.branch && !pendingTaskId && !draftId,
	);
	...
	useEffect(() => {
		if (!needsPreparation || !initialProjectRef.branch || waitingForProject)
			return;
		...
		checkoutTail.current = checkoutTail.current.then(async () => {
			...
			setPendingCheckouts((count) => count - 1);
			if (active)
				setPreparation({ request: initialProjectRef, result, workspaceRoot });
		});
		return () => {
			active = false;
		};
	...
	const result =
		preparation?.request === initialProjectRef &&
		preparation.workspaceRoot === workspaceRoot
			? preparation.result
			: null;
	...
	const checkoutPending =
		pendingCheckouts > 0 || (needsPreparation && result === null);
	...
	const preparingBranch =
		checkoutPending || (needsPreparation && result?._tag !== "Success");
