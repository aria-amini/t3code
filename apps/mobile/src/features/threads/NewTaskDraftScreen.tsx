...
export function NewTaskDraftScreen(props: {
	...
	const { projectScopes, selectedProject, selectedProjectKey, setProject } =
		flow;
	...
	const modelUnavailable =
		environmentConnected && flow.selectedModelOption?.isUnavailable === true;
	...
	const [previewVideo, setPreviewVideo] = useState<VideoPreviewSource | null>(
		null,
	);
	const [previewFile, setPreviewFile] = useState<FilePreviewSource | null>(
		null,
	);
	...
	const openVideoPreview = useCallback(
		(attachment: DraftComposerFileAttachment, sourceIdentifier: string) => {
			...
			setPreviewFile(null);
			setPreviewVideo(
				(current) => current ?? { type: "local", attachment, sourceIdentifier },
			);
		},
		[isComposerFocused],
	);
	...
	useEffect(() => {
		if (
			!settingsSheetPresentation.isVisible ||
			settingsRoutePresentedRef.current
		) {
			return;
		}
	...
	const [importingShareKey, setImportingShareKey] = useState<string | null>(
		null,
	);
	const [isCancellingShareImport, setIsCancellingShareImport] = useState(false);
	const [cancelledIncomingShareId, setCancelledIncomingShareId] = useState<
		string | null
	>(null);
	const [isReturningToProjectPicker, setIsReturningToProjectPicker] =
		useState(false);
	const [submitNavigationAction, setSubmitNavigationAction] =
	...
	const incomingShare = props.incomingShareId
		? getShare(props.incomingShareId)
		: null;
	...
	const isIncomingShareAwaitingServerConfig = Boolean(
		incomingShare?.attachments.some(
	...
	const isComposerInteractionLocked =
		isIncomingShareTransferPending || flow.submitting;
	...
	const voiceInput = useVoiceInputController({
		...
		disabled:
			isIncomingShareTransferPending || isImportingShare || flow.submitting,
		onChangeDraftMessage: flow.setPrompt,
	...
	const hasImportedIncomingShare = Boolean(
		...
		flow.draftKey &&
		getComposerDraftSnapshot(flow.draftKey).importedShareIds?.includes(
			props.incomingShareId,
		),
	);
	...
	useEffect(() => {
		...
		void waitForComposerDraftsLoaded().then(() => {
			...
			if (
				getComposerDraftSnapshot(draftId).project !== undefined &&
				projects.length === 0
			) {
	...
	useEffect(() => {
		if (
			!props.pendingTaskId ||
			editingPendingTask?.messageId === props.pendingTaskId
		) {
			return;
		}
	...
	useEffect(() => {
		...
		startedShareImportKeyRef.current = importKey;
		const draftBackup =
			shareImportDraftBackupRef.current.get(importKey) ??
		...
		void (async () => {
			...
			const retainedAttachmentIds = new Set(
				getComposerDraftSnapshot(draftKey).attachments.map(
			...
			const warnings = [
				...incomingShare.warnings,
				...selectedAttachments.warnings,
			];
									...
									try {
										if (needsDraftRestore) {
											...
											const mergedAttachments =
												getComposerDraftSnapshot(draftKey).attachments;
	...
	async function handlePickFiles(): Promise<void> {
		...
		const rejectedCount =
			result.files.length > 0 ? flow.appendAttachments(result.files) : 0;
	...
	async function handleStart(): Promise<void> {
		...
		const message = flow.buildPendingTaskMessage(metadata, {
			...
			currentCheckoutBranch: queuesInsteadOfStarting
				? null
		...
		try {
			...
			Alert.alert(
				...
				error instanceof Error
					? error.message
					: "The task could not be saved to the outbox.",
			);
			return;
		} finally {
	...
	const chooseProject = () => {
		...
		void KeyboardController.dismiss({ animated: true });
		navigation.dispatch(
			StackActions.push("NewTask", { incomingShareId: props.incomingShareId }),
		);
	};
	const openContextPicker = (
		routeName: "NewTaskBranch" | "NewTaskEnvironment",
	) => {
		if (isComposerInteractionLocked) {
			return;
		}
			...
			<ComposerInlineControl
				...
				onPress={
					flow.environments.length > 1
						? () => openContextPicker("NewTaskEnvironment")
						: undefined
				}
			...
			<ComposerInlineControl
				...
				onPress={() =>
					flow.setWorkspaceMode(
						flow.workspaceMode === "local" ? "worktree" : "local",
					)
				}
				showChevron={false}
			/>
	...
	const composerDock = (
		<View
			className="bg-sheet px-[12px] pt-1"
						...
						>
									...
									>
										...
										{flow.planModeEnabled ? (
											<ComposerInlineControl
												...
												label={
													flow.interactionMode === "plan" ? "Plan" : "Build"
												}
												onPress={() =>
													flow.setInteractionMode(
														flow.interactionMode === "plan"
							...
							{voicePresentation.showsSend ? (
								<ComposerActionButton
									...
									disabled={!canStart}
									icon={
										queuesInsteadOfStarting ? "tray.and.arrow.up" : "arrow.up"
									}
