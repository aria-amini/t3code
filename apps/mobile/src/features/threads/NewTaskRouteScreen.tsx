...
export function NewTaskRouteScreen({
	route,
}: StaticScreenProps<NewTaskRouteParams | undefined>) {
		...
		<View collapsable={false} className="flex-1 bg-sheet">
			{Platform.OS === "android" ? (
				<>
					...
					<AndroidScreenHeader
						...
						onBack={
							layout.usesSplitView ? () => navigation.goBack() : undefined
						}
						actions={
			...
			>
				{projectScopes.length === 0 ? (
					<View
						collapsable={false}
						className="items-center gap-3 rounded-[24px] bg-card px-6 py-8"
					>
						...
						{projectScopes.map((scope, scopeIndex) => {
							...
							return (
								<View
									key={scope.key}
									className={cn(
										scopeIndex > 0 && "border-t border-border-subtle",
									)}
								>
									<Pressable
