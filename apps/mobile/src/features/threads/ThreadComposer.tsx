...
export const COMPOSER_LAYOUT_TRANSITION =
	Platform.OS === "android"
		? undefined
		: LinearTransition.duration(COMPOSER_TRANSITION_DURATION_MS).reduceMotion(
				ReduceMotion.System,
			);
...
const COMPOSER_ATTACHMENT_ENTERING =
	Platform.OS === "android"
		? FadeIn.duration(160)
		: FadeIn.delay(COMPOSER_TRANSITION_DURATION_MS)
				.duration(160)
...
export function ComposerSurface(props: {
	...
	const shouldAnimate =
		props.animateLayout !== false && Platform.OS !== "android";
	useLayoutEffect(() => {
		...
		<Animated.View
			...
			style={[
				animatedShapeStyle,
				{
					...
					elevation:
						Platform.OS === "android" && Platform.Version < 28 ? 10 : undefined,
				},
			]}
		>
...
export const ThreadComposer = memo(function ThreadComposer(
	...
	const settingsRoutePresentation =
		useExistingThreadSettingsRoutePresentation();
	...
	const composerOwnerKey = scopedThreadKey(
		props.environmentId,
	...
	const composerMenu = useComposerCommandMenu({
		...
		onUsageLimits:
			usageLimitsOffered && props.draftAttachments.length === 0
				? openUsageLimits
				: undefined,
	});
	...
	const threadProviderGroups = useMemo(
		() =>
			providerGroups.filter(
				(group) => group.providerKey === currentModelSelection.instanceId,
			),
				...
				>
							...
							>
								<ComposerAttachmentStrip
									...
									onRemove={
										voiceInput.isBusy
											? () => undefined
											: props.onRemoveDraftImage
									}
									onPressPreview={
										voiceInput.isBusy ? undefined : onPressPreview
									}
						...
						>
							<ComposerEditor
								...
								contentInsetVertical={
									isExpanded || Platform.OS === "android" ? 0 : 6
								}
								style={
									isExpanded
										? {
					...
					<Animated.View
						...
						importantForAccessibility={
							isToolbarVisible ? "auto" : "no-hide-descendants"
						}
						layout={COMPOSER_LAYOUT_TRANSITION}
							...
							>
								...
								{isVoiceInputPresented ? (
									...
									<View className="min-w-0 flex-1 flex-row items-center justify-between">
										<ComposerAttachmentButton
											supportsFiles={Boolean(
												props.serverConfig?.environment.capabilities
													.fileAttachments,
											)}
											onPickMedia={props.onPickDraftMedia}
											onPickFiles={props.onPickDraftFiles}
