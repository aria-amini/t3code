...
export interface ThreadDetailScreenProps {
	...
	readonly activePendingUserInputDrafts: Record<
		...
		PendingUserInputDraftAnswer
	>;
	readonly activePendingUserInputAnswers: Record<
		string,
	...
	readonly onUpdateThreadModelSelection: (
...
export const ThreadDetailScreen = memo(function ThreadDetailScreen(
	...
	const navigationHeaderHeight =
		useContext(HeaderHeightContext) || insets.top + IOS_NAV_BAR_HEIGHT;
	...
	const feedTouchStartRef = useRef<{ pageX: number; pageY: number } | null>(
		null,
	);
	...
	const [anchorMessageId, setAnchorMessageId] = useState<MessageId | null>(
		null,
	);
	const [submittedMessageId, setSubmittedMessageId] =
	...
	const activeUserInputRequestId =
		props.activePendingUserInput?.requestId ?? null;
	...
	const userInputCollapsed =
		...
		collapsedUserInputRequestId === activeUserInputRequestId;
	...
	useEffect(() => {
		if (
			...
			liveKeyboardHeight !== lastKnownKeyboardHeight
		) {
	...
	const userInputCoverageApplies =
		Platform.OS === "ios" && activeUserInputRequestId !== null;
	...
	const overlayRepinTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
		null,
	);
	...
	useEffect(() => {
		...
		if (
			(!threadChanged && !visibilityChanged) ||
			(threadChanged && !showFloatingStatus)
		) {
			return;
		}
	...
	useEffect(() => {
		if (
			...
			(!selectedThreadFeed.some(
				...
				!props.queuedMessages.some(
					(message) => message.messageId === submittedMessageId,
				))
		) {
			return;
		}
	...
	const handleUseArtifactTemplate = useCallback(
		(template: CodexArtifactTemplate) => {
			...
			const nextDraft = appendCodexArtifactTemplateUsePrompt(
				currentDraft,
				template,
			);
	...
	const showScrollToEndButton =
		contentPresentationKind === "ready" && !endFollowEnabled;
