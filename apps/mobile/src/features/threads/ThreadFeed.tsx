...
} from "../../components/VideoPreviewModal";
...
const THREAD_FEED_LAYOUT_TRANSITION = LinearTransition.duration(
	THREAD_DISCLOSURE_TRANSITION_MS,
);
...
function isFreshTimestamp(input: string): boolean {
	const timestamp = Date.parse(input);
	return (
		Number.isFinite(timestamp) && Date.now() - timestamp < FRESH_ENTRY_WINDOW_MS
	);
}
	...
	if (videoType !== null) {
		...
		return (
			<VideoAttachmentTile
				...
				onPress={(sourceIdentifier) =>
					props.onPressVideo(attachment, sourceIdentifier)
				}
...
function ThreadMediaVisibility(props: { readonly children: ReactNode }) {
	...
	return (
		<ThreadMediaVisibleContext value={visible}>
			{props.children}
		</ThreadMediaVisibleContext>
	);
}
...
function ThreadMarkdownVideo(props: {
	...
	const uri = mediaVideoPreviewUri(
		source,
		asset._tag === "Success" ? asset.url : null,
	);
	return (
		<MediaVideoPlayer
			key={thumbnailKey}
...
const MarkdownExternalLink = memo(function MarkdownExternalLink(props: {
	...
	const faviconUrl = linkIcon
		? null
		: faviconUrlForOrigin(`https://${props.host}`);
...
function MarkdownInlineCode(props: {
	...
	const presentation = insideLink
		? null
		: resolveMarkdownInlineCodePresentation(props.content);
	return (
		<NativeText
			className={presentation ? "font-t3-bold" : "font-mono"}
			onPress={
				presentation ? () => props.onLinkPress(presentation.href) : undefined
			}
			style={{
	...
	return (
		<View className="my-2 min-w-0 flex-row items-center gap-3 rounded-2xl border border-border bg-card px-3 py-3">
			...
			<View className="min-w-0 flex-1">
				<Text
					className="font-t3-bold text-sm text-foreground"
					numberOfLines={1}
				>
...
const AssistantMarkdownContent = memo(function AssistantMarkdownContent(props: {
	...
	readonly onUseArtifactTemplate?:
		| ((template: CodexArtifactTemplate) => void)
		| undefined;
...
function useMarkdownStyles(
	...
	const markdownUserInlineCodeText =
		theme["--color-user-bubble-foreground-muted"];
	...
	const userBubbleSkillForeground =
		theme["--color-user-bubble-skill-foreground"];
	const userBubbleForegroundMuted =
		theme["--color-user-bubble-foreground-muted"];
	...
	if (entry.type === "agent-spawn") {
		return (
			<ThreadAgentSpawnCard
				...
				onCopy={() =>
					props.onCopyWorkRow(entry.activity.id, entry.activity.getCopyText())
				}
			/>
		);
	}
				...
				<View className="mb-5 items-end">
					...
					>
						{message.text.trim().length > 0 ? (
							<MarkdownImageAvailableWidthContext
								value={
									props.userBubbleMaxWidth - USER_BUBBLE_HORIZONTAL_PADDING * 2
								}
							>
								<UserMessageContent
									text={renderedText}
		...
		return (
			...
			>
				{renderedText.trim().length > 0 ? (
					<MarkdownImageAvailableWidthContext
						value={props.markdownContentWidth}
					>
...
function UserMessageContent(props: {
	...
	const hasReviewComment = segments.some(
...
const ReviewCommentCard = memo(function ReviewCommentCard(props: {
	...
	const { themeAppearance: appearanceScheme, themeId } =
		useAppearancePreferences();
	const appTheme = useUniwindTheme();
	const NativeReviewDiffView = resolveNativeReviewDiffView();
	const patch = useMemo(
		() => buildReviewCommentPatch(props.comment),
		[props.comment],
	);
	...
	const nativeReviewDiffData = useMemo(
		() => buildNativeReviewDiffData(parsedDiff),
		[parsedDiff],
	);
	...
	const nativeRowsJson = useMemo(
		() => JSON.stringify(compactNativeRows),
		[compactNativeRows],
	);
	...
	const shouldRenderNativeDiff =
		NativeReviewDiffView != null && compactNativeRows.length > 0;
...
export const ThreadFeed = memo(function ThreadFeed(props: ThreadFeedProps) {
	...
	const [disclosureToggleSettling, setDisclosureToggleSettling] =
		useState(false);
	...
	const transitionEndFollow = useCallback(
		(event: ThreadFeedLiveFollowEvent) => {
			setEndFollow(
				resolveThreadFeedLiveFollow(endFollowEnabledRef.current, event),
			);
		},
		[setEndFollow],
	);
	...
	const { copiedRowId, expandedWorkGroups, expandedWorkRows, expandedTurnIds } =
		interactionState;
	const [expandedFile, setExpandedFile] = useState<FilePreviewSource | null>(
		null,
	);
	const [expandedVideo, setExpandedVideo] = useState<VideoPreviewSource | null>(
		null,
	);
	...
	const contentWidth = Math.max(
		0,
		viewportWidth - contentHorizontalPadding * 2,
	);
	const userBubbleMaxWidth = contentWidth * 0.85;
	const markdownContentWidth = Math.max(
		0,
		contentWidth - ASSISTANT_ROW_HORIZONTAL_PADDING * 2,
	);
	const reviewCommentBubbleWidth = Math.min(
		Math.max(280, contentWidth * 0.85),
		contentWidth,
	);
	const insets = useSafeAreaInsets();
	const topContentInset =
		props.contentTopInset ?? insets.top + IOS_NAV_BAR_HEIGHT;
	...
	const onMarkdownLinkPress = useCallback(
		(href: string) => {
			...
			if (presentation.kind !== "file" && presentation.href) {
				if (
					/^https?:\/\//i.test(presentation.href) &&
					isPdfFile({ name: presentation.href })
				) {
					setExpandedFile(
						(current) =>
							current ?? {
	...
	const unsettledTurnId =
		props.latestTurn &&
		(props.latestTurn.completedAt === null ||
			props.latestTurn.state === "running")
	...
	const handleScroll = useCallback(
		(event: NativeSyntheticEvent<NativeScrollEvent>) => {
			...
			reportHeaderMaterialVisibility(
				event.nativeEvent.contentOffset.y + anchorTopInset > 6,
			);
	...
	const finishUserScroll = useCallback(
		(releaseIsAtEnd?: boolean) => {
			...
			transitionEndFollow({
				...
				isAtEnd:
					releaseIsAtEnd ?? props.listRef.current?.getState().isAtEnd ?? false,
	...
	const handleScrollEndDrag = useCallback(() => {
		...
		userScrollSettleTimerRef.current = setTimeout(
			() => finishUserScroll(releaseIsAtEnd),
			160,
		);
	...
	useEffect(() => {
		...
		if (props.latestTurn.turnId === previous.turnId) {
			if (
				...
				props.latestTurn.state === "interrupted"
			) {
				const interruptedTurnId = props.latestTurn.turnId;
				setInteractionState((current) => ({
					...current,
					expandedTurnIds: new Set(current.expandedTurnIds).add(
						interruptedTurnId,
					),
				}));
			}
	...
	const onCopyWorkRow = useCallback((rowId: string, value: string) => {
		...
		copyFeedbackTimeoutRef.current = setTimeout(() => {
			setInteractionState((current) =>
				current.copiedRowId === rowId
					? { ...current, copiedRowId: null }
					: current,
			);
			...
			switch (entry.type) {
				...
				case "activity-group":
					...
					return entry.activities.some(
						(activity) => expandedWorkRows[activity.id],
					)
						? undefined
	...
	return (
		<PresentationSource
			identifier={fileShareSourceIdentifier}
			style={{ flex: 1 }}
		>
			<View className="flex-1" onLayout={handleViewportLayout}>
				<View className="flex-1">
					<KeyboardAwareLegendList
						...
						contentInsetAdjustmentBehavior={
							usesNativeAutomaticInsets ? "automatic" : "never"
						}
				...
				<FilePreviewModal
					source={expandedFile}
					onRequestClose={() => setExpandedFile(null)}
				/>
			</View>
		</PresentationSource>
	);
