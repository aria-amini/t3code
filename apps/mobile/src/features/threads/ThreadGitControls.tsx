...
function useThreadGitControlModel(props: ThreadGitMenuProps) {
	...
	const currentBranchLabel =
		gitStatus?.refName ?? props.currentBranch ?? "Detached HEAD";
...
function useThreadGitHeaderActionItems(
	props: ThreadGitControlsProps,
): ThreadGitHeaderActionItems {
				...
				menu: {
					items: [
						...props.projectScripts.map((script) => ({
							...
							icon: {
								name: projectScriptMenuIcon(script.icon),
...
export function useThreadGitCenterHeaderItems(
	props: ThreadGitControlsProps,
): HeaderItems {
	const actionItems = useThreadGitHeaderActionItems(props);
	return useMemo(
