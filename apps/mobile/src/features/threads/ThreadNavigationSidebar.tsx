...
function ThreadNavigationSidebarPane(
	...
	const { openPendingTask, confirmDeletePendingTask } =
	...
	const selectedProjectScope = useMemo(
		() =>
			selectedProjectKey === null
				? null
				: (projectScopes.find((scope) => scope.key === selectedProjectKey) ??
					null),
		...
		(key: string, action: HomeGroupDisplayAction) => {
			setGroupDisplayStates((previous) => {
				...
				next.set(
					key,
					nextGroupDisplayState(
						previous.get(key) ?? DEFAULT_GROUP_DISPLAY_STATE,
						action,
					),
				);
				return next;
	...
	const threadListV2Layout = useMemo(() => {
		...
		return buildThreadListV2Items({
			...
			environmentId: options.selectedEnvironmentId,
			projectRefs:
				selectedProjectScope === null ? null : selectedProjectScope.projectRefs,
			searchQuery: props.searchQuery,
			matchedThreadKeys,
			settlementEnvironmentIds,
			snoozeEnvironmentIds,
		...
		({ nativeEvent }: { readonly nativeEvent: { readonly event: string } }) => {
			...
			if (event.startsWith("environment:")) {
				const environment = environments.find(
					(candidate) =>
						String(candidate.environmentId) ===
						event.slice("environment:".length),
				);
			...
			if (event.startsWith("project:")) {
				...
				if (
					projectFilterOptions.some((project) => project.key === projectKey)
				) {
	...
	const stickyHeaderHeight =
		measuredHeaderHeight ?? insets.top + SIDEBAR_STICKY_HEADER_HEIGHT;
	...
	const sidebarItemsAreEqual = useCallback(
		(previous: SidebarListItem, item: SidebarListItem): boolean => {
			...
			if (
				...
				return (
					previous.count === item.count && previous.expanded === item.expanded
				);
			}
			if (
		...
		({ item }: { readonly item: SidebarListItem }) => {
			switch (item.type) {
				...
				case "v2-thread": {
					...
					const scopeKey = scopedProjectKey(
					...
					return (
						<ThreadListV2Row
							...
							environmentLabel={
								Object.keys(savedConnectionsById).length > 1
									? (savedConnectionsById[thread.environmentId]
											?.environmentLabel ?? null)
									: null
							}
							environmentMachine={machineByEnvironmentId.get(
