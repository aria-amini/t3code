...
export function ThreadRouteScreen(props: ThreadRouteScreenProps) {
	...
	const environmentId = environmentIdRaw
		? EnvironmentId.make(environmentIdRaw)
		: null;
	const routeEnvironmentRuntime = useRemoteEnvironmentRuntime(environmentId);
	const routeConnectionState =
		routeEnvironmentRuntime?.connectionState ??
		(environmentId ? "available" : connectionState);
...
function ThreadRouteContent(
	props: ThreadRouteScreenProps & {
		readonly selectedThreadDetailState: ReturnType<
			typeof useSelectedThreadDetailState
		>;
	},
) {
	const {
	...
	const loadEarlierTurns = useMemo(() => {
		if (
			selectedThread === null ||
			!threadHasOlderTurns(selectedThreadDetailState)
		) {
			return null;
		}
		return {
	...
	const interruptThreadTurn = useAtomCommand(
	...
	const routeThreadIdentity =
		environmentIdRaw !== null && threadId !== null
			? `${environmentIdRaw}:${threadId}`
			: null;
	const [inspectorSelection, setInspectorSelection] =
		useState<ThreadInspectorSelection | null>(() =>
		...
		() =>
			selectedThread
				? {
						...selectedThread,
						modelSelection:
							composer.modelSelection ?? selectedThread.modelSelection,
						runtimeMode: composer.runtimeMode ?? selectedThread.runtimeMode,
						interactionMode:
							composer.interactionMode ?? selectedThread.interactionMode,
					}
				: null,
		[
	...
	const selectedThreadDetailWorktreePath =
		selectedThreadDetail?.worktreePath ?? null;
	...
	const activeInspectorRenderer =
		inspectorMode === null ? undefined : renderInspectorStack;
	...
	const handleRunProjectScript = useCallback(
		async (script: ProjectScript) => {
			...
			const targetTerminalId = resolveProjectScriptTerminalId({
				existingTerminalIds: terminalMenuSessions.map(
					(session) => session.terminalId,
				),
	...
	const threadGitControlProps = {
		...
		onOpenFilesInspector:
			fileInspector.supported && selectedThreadCwd !== null
				...
				: undefined,
		onOpenGitInspector: fileInspector.supported
		...
		projectScripts: selectedThreadProject
			? resolveProjectScripts(
					routeEnvironmentRuntime?.serverConfig?.settings ??
						DEFAULT_SERVER_SETTINGS,
					selectedThreadProject,
				)
			: [],
	...
	const threadCenterHeaderItems = useThreadGitCenterHeaderItems(
		threadGitControlProps,
	);
	const compactRightHeaderItems = useThreadGitRightHeaderItems(
		threadGitControlProps,
	);
		...
		() => [
			...
			withNativeGlassHeaderItem({
				accessibilityLabel: panes.primarySidebarVisible
					...
					: "Show thread sidebar",
				icon: {
					name: panes.primarySidebarVisible
	...
	const androidHeaderActions = useMemo<
		ReadonlyArray<AndroidHeaderAction>
	>(() => {
				...
				options={{
					...
					unstable_headerRightItems:
						Platform.OS === "ios"
							...
							: undefined,
					unstable_headerSubtitle: usesNativeHeaderGlass
						? headerSubtitle
						: undefined,
				}}
			/>
