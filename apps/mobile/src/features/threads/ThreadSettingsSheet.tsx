...
const THREAD_SETTINGS_CATALOG_LAYOUT_TRANSITION =
...
const THREAD_SETTINGS_OPTIONS_LAYOUT_TRANSITION =
	LinearTransition.duration(180);
...
const THREAD_SETTINGS_HEADER_SCROLL_EDGE_EFFECTS =
...
function ModelRow(props: {
	...
	return (
		<Pressable
			accessibilityLabel={[props.option.label, props.option.subtitle]
				.filter(Boolean)
				.join(", ")}
...
function ProviderHeader(props: {
	...
	return (
		<View
			...
			className="mx-4 min-h-9 flex-row items-center gap-2 px-1 pt-1"
		>
			{content}
		</View>
	);
...
export function ExistingThreadSettingsRouteProvider(props: {
	readonly children: ReactNode;
}) {
	const [session, setSession] =
		useState<ExistingThreadSettingsRouteSession | null>(null);
...
const ThreadSettingsSessionContext =
	createContext<ThreadSettingsSessionValue | null>(null);
...
function ThreadSettingsSessionProvider(
	...
	const hasLegacyModels = useMemo(
		() =>
			props.providerGroups.some((group) =>
	...
	const applyOptionChange = useCallback(
		(id: string, value: string | boolean) => {
			const next = applyProviderOptionSelection(displayedDescriptors, {
				id,
				value,
			});
			if (!next) {
...
function useThreadSettingsSession() {
	...
	if (!value) {
		throw new Error(
			"useThreadSettingsSession must be used inside ThreadSettingsSessionProvider.",
		);
	}
	return value;
}
		...
		() =>
			session.providerGroups.flatMap((group) => {
				...
				const catalogModels = session.showLegacy
					? group.models
					: group.models.filter(
							(model) => !model.isLegacy || session.isDisplayed(model),
						);
				...
				const isPrimary =
					driver !== undefined && PRIMARY_PROVIDER_DRIVERS.has(driver);
				...
				const collapsed = providerSectionIsCollapsed({
					...
					hasExpansionOverride: session.providerExpansionOverrides.has(
						group.providerKey,
					),
					isNarrowed,
				});
							...
							<Animated.View
								...
								entering={
									props.animationsReady
										? THREAD_SETTINGS_OPTION_ENTER_TRANSITION
										: undefined
								}
								exiting={
									props.animationsReady
										? THREAD_SETTINGS_OPTION_EXIT_TRANSITION
										: undefined
								}
								layout={THREAD_SETTINGS_OPTIONS_LAYOUT_TRANSITION}
							>
					...
					<DisclosureRow
						...
						value={
							RUNTIME_MODE_CHOICES.find(
								(choice) => choice.mode === session.runtimeMode,
							)?.label
						}
...
function ThreadSettingsMainContent(props: {
	...
	const usesTransparentNativeHeader =
		...
		(itemProps: LegendListRenderItemProps<ThreadSettingsCatalogItem>) => {
			...
			return (
				<Animated.View
					...
					entering={
						animationsReady
							? THREAD_SETTINGS_CATALOG_ENTER_TRANSITION
							: undefined
					}
					exiting={
						animationsReady
							? THREAD_SETTINGS_CATALOG_EXIT_TRANSITION
							: undefined
					}
				>
					{content}
	...
	return (
		<AnimatedLegendList
			...
			contentInsetAdjustmentBehavior={
				usesTransparentNativeHeader ? "never" : "automatic"
			}
			data={listItems}
			...
			maintainVisibleContentPosition={
				THREAD_SETTINGS_MAINTAIN_VISIBLE_CONTENT_POSITION
			}
			ListHeaderComponent={
				<>
					{usesTransparentNativeHeader ? (
						<View style={{ height: nativeHeaderHeight }} />
					) : null}
					{Platform.OS === "android" ? (
...
function ThreadSettingsChoiceContent(props: {
	...
	const session = useThreadSettingsSession();
	const descriptorId =
		props.submenu.kind === "descriptor" ? props.submenu.id : null;

	const activeDescriptor =
		descriptorId !== null
			? session.displayedDescriptors.find(
					(descriptor) =>
						descriptor.type === "select" && descriptor.id === descriptorId,
				)
			: undefined;
...
const ThreadSettingsPickerStack =
...
function ThreadSettingsModelsScreen() {
	...
	const navigation =
		useNavigation<NativeStackNavigationProp<ThreadSettingsPickerStackParams>>();
	const usesNativeMailSearchToolbar =
		Platform.OS === "ios" && NATIVE_MAIL_SEARCH_TOOLBAR_SUPPORTED;
	const hasCustomCatalogFilter =
		session.providerFilter !== null || session.showLegacy;
	const refreshProvidersCommand = useAtomCommand(
			...
			items: [
				{
					...
					items: [
						...
						...session.providerGroups.map((group) => ({
							...
							state:
								session.providerFilter === group.providerKey
									? ("on" as const)
									: ("off" as const),
	...
	return (
		<>
			{Platform.OS === "android" ? (
				<AndroidScreenHeader
					actions={[
						...
						{
							accessibilityLabel: session.pendingModel
					...
					headerSearchBarOptions:
						Platform.OS === "ios" && !usesNativeMailSearchToolbar
							? {
									...
									onChangeText: (event) =>
										session.setSearchQuery(event.nativeEvent.text),
									placeholder: "Find a model",
								}
							: undefined,
				}}
			...
			<NativeHeaderToolbar placement="right">
				...
				<NativeHeaderToolbar.Button
					accessibilityLabel={
						session.pendingModel ? "Save thread settings" : "Done"
					}
...
function ThreadSettingsChoiceScreen() {
	...
	const route =
		useRoute<
			RouteProp<ThreadSettingsPickerStackParams, "ThreadSettingsChoice">
		>();
...
function ThreadSettingsPickerNavigator(
	props: ThreadSettingsPickerPresentation,
) {
