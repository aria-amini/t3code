...
beforeEach(async () => {
	directory = await NodeFSP.mkdtemp(
		NodePath.join(NodeOS.tmpdir(), "t3-branch-selection-"),
	);
...
function selectBranch(
	switchRef: Parameters<typeof checkoutNewTaskBranch>[0]["switchRef"],
) {
...
const switchRef: Parameters<typeof checkoutNewTaskBranch>[0]["switchRef"] = (
	request,
) =>
	settlePromise(async () => {
...
describe("new-task branch checkout", () => {
	...
	it("returns checkout failure without selecting the branch or losing dirty files", async () => {
		await NodeFSP.writeFile(
			NodePath.join(cwd, "file.txt"),
			"unsaved local changes\n",
		);
		const result = await selectBranch(switchRef);
		expect(result._tag).toBe("Failure");
		if (result._tag !== "Failure") throw new Error("Expected checkout to fail");
		expect(String(squashAtomCommandFailure(result))).toContain(
	...
	it("fails when the source project is unavailable instead of releasing a composer selection", async () => {
		...
		if (result._tag !== "Failure")
			throw new Error("Expected an unavailable-project failure");
		expect(String(squashAtomCommandFailure(result))).toContain(
	...
	it("reuses an existing worktree without switching the project checkout", async () => {
		...
		const result = await checkoutNewTaskBranch({
			...
			switchRef: () => {
				throw new Error(
					"Existing worktrees must not switch the project checkout",
				);
			},
		});
		expect(result._tag).toBe("Success");
		...
		expect((await git("branch", "--show-current")).stdout.trim()).toBe("main");
		expect(
			await NodeFSP.readFile(NodePath.join(worktreePath, "file.txt"), "utf8"),
		).toBe("feature\n");
	});
});
