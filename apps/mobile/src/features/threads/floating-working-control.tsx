...
const CONTROL_OVERLAY_OFFSET =
	CONTROL_HEIGHT + CONTROL_GAP - COMPOSER_CAPSULE_INSET;
export const FLOATING_WORKING_CONTROL_COVERAGE =
	CONTROL_OVERLAY_OFFSET + CONTROL_GAP;
...
export function FloatingWorkingControl(props: {
	...
	const labelWidth = Math.max(
		0,
		Math.min(overlayWidth, windowWidth) - CONTROL_HEIGHT - 16,
	);
	const separationProgress = useSharedValue(props.showScrollToEnd ? 1 : 0);

	useEffect(() => {
		separationProgress.value = withTiming(
			props.showScrollToEnd ? 1 : 0,
			CONTROL_TIMING,
		);
				...
				>
					...
					<AnimatedGlassView
						...
						importantForAccessibility={
							props.showScrollToEnd ? "auto" : "no-hide-descendants"
						}
	...
	if (props.status.kind === "syncing") {
		return (
			...
			>
				...
				<Text
					className="shrink font-t3-medium text-xs text-foreground"
					numberOfLines={1}
				>
					{props.status.label}
				</Text>
