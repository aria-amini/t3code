...
const status = (
	connectionState: Parameters<
		typeof connectionFloatingStatus
	>[0]["connectionState"],
	overrides: {
	...
	connectionFloatingStatus({
		...
		connectionState,
		environmentLabel:
			overrides.environmentLabel === undefined
				? "Mac mini"
...
describe("connectionFloatingStatus", () => {
	...
	it("names the environment it is retrying, and says so only after a failure", () => {
		...
		expect(
			status("reconnecting", { connectionError: "ECONNREFUSED" }),
		).toMatchObject({
			tone: "reconnecting",
	...
	it("carries the reconnect handler so the pill can trigger it", () => {
		...
		if (pill?.kind !== "connection")
			throw new Error("expected a connection pill");
		pill.onPress();
