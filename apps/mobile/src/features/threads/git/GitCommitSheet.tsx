...
export function GitCommitSheet(_props: GitCommitSheetProps) {
	...
	const selectedInsertions = selectedFiles.reduce(
		(sum, file) => sum + file.insertions,
		0,
	);
	const selectedDeletions = selectedFiles.reduce(
					...
					<View className="flex-row items-center justify-between gap-3">
						<View className="gap-1">
							...
							<Text className="text-foreground-muted text-xs leading-normal">
								{selectedFiles.length} selected · +{selectedInsertions} / -
								{selectedDeletions}
							</Text>
						</View>
						...
						<View className="gap-2">
							...
							{selectedFiles.length > selectedFilePreview.length ? (
								<Text className="text-foreground-muted text-xs leading-snug">
									+{selectedFiles.length - selectedFilePreview.length} more
									files
								</Text>
							) : null}
						</View>
