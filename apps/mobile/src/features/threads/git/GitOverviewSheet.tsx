...
const HEADER_SCROLL_EDGE_EFFECTS = nativeHeaderScrollEdgeEffects(
	Platform.OS,
	Platform.Version,
);
...
export function GitOverviewSheet(props: GitOverviewSheetProps) {
	...
	const { selectedThreadCwd, selectedThreadWorktreePath } =
	...
	const currentBranchLabel =
		gitStatus.data?.refName ?? selectedThread?.branch ?? "Detached HEAD";
	...
	const openExistingPr = useCallback(async () => {
		const prUrl =
			gitStatus.data?.pr?.state === "open" ? gitStatus.data.pr.url : null;
		if (!prUrl) {
			Alert.alert(
				"No open PR",
			...
			>
				...
				<SheetListRow
					...
					onPress={() => {
						...
						navigation.dispatch(
							resolveGitOverviewReviewNavigationAction(presentation) ===
								"replace"
	...
	if (isInspector && Platform.OS === "ios") {
		return (
			<View
				collapsable={false}
				className="flex-1 border-l border-border bg-sheet"
			>
		...
		>
			...
			{isInspector ? (
				<View className="gap-1 border-b border-border px-4 pb-4 pt-3">
					...
					<Text className="pr-10 text-xl font-t3-bold">
						{currentBranchLabel}
					</Text>
