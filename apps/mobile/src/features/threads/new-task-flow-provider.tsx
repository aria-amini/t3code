...
export function branchBadgeLabel(input: {
	...
	if (
		...
		input.branch.worktreePath !== input.project?.workspaceRoot
	) {
		return "worktree";
	}
...
const NewTaskFlowContext = React.createContext<NewTaskFlowContextValue | null>(
	null,
);
...
export function NewTaskFlowProvider(props: React.PropsWithChildren) {
	...
	const [selectedEnvironmentIdOverride, setSelectedEnvironmentId] =
		useState<EnvironmentId | null>(null);
	const selectedEnvironmentId =
		selectedEnvironmentIdOverride !== null &&
		projects.some(
			(project) => project.environmentId === selectedEnvironmentIdOverride,
		)
	...
	const [editingPendingTask, setEditingPendingTask] =
		useState<QueuedThreadMessage | null>(null);
	...
	const selectedProject =
		...
		selectedProjectKey ===
			scopedProjectKey(
				editingPendingProject.environmentId,
				editingPendingProject.id,
			)
	...
	const selectedRepositoryKey =
		selectedProject?.repositoryIdentity?.canonicalKey ?? null;
	...
	const selectedWorkspaceBasename =
		selectedProject?.workspaceRoot.split("/").at(-1) || null;
	...
	const t3ProjectFileData =
		t3ProjectFileQuery.data as ProjectReadFileResult | null;
	const t3ProjectFileDefaultMode = useMemo(() => {
		if (t3ProjectFileData === null || t3ProjectFileData.truncated) return null;
		return (
			parseT3ProjectFile(t3ProjectFileData.contents)?.defaultThreadEnvMode ??
			null
		);
	}, [t3ProjectFileData]);
	...
	const interactionMode = planModeEnabled
		? (selectedProjectDraft.interactionMode ??
			DEFAULT_PROVIDER_INTERACTION_MODE)
		: DEFAULT_PROVIDER_INTERACTION_MODE;
	...
	const appendAttachments = useCallback(
		(nextAttachments: ReadonlyArray<DraftComposerAttachment>): number => {
			...
			return appendComposerDraftAttachments(
				selectedProjectDraftKey,
				nextAttachments,
			);
		},
	...
	const debouncedBranchQuery = useDebouncedValue(
		branchQuery,
	...
	const branchSearchIsDebouncing =
		branchQuery.trim() !== debouncedBranchQuery.trim();
	...
	const hasMoreBranches =
		...
		branchState.data?.nextCursor !== undefined;
	...
	const openDraft = useCallback(
		(draftKey: string): boolean => {
			...
			const projectLoaded = projects.some(
				(project) =>
					project.environmentId === stamp.environmentId &&
					project.id === stamp.projectId,
			);
			if (!projectLoaded) {
				return false;
			}
			...
			setSelectedProjectKey(
				scopedProjectKey(stamp.environmentId, stamp.projectId),
			);
			return true;
		},
		[projects],
	...
	const selectEnvironment = useCallback(
		(environmentId: EnvironmentId) => {
			...
			setSelectedProjectKey(
				match ? scopedProjectKey(match.environmentId, match.id) : null,
			);
		},
	...
	const beginEditingPendingTask = useCallback((messageId: string): boolean => {
		...
		setSelectedProjectKey(
			scopedProjectKey(message.environmentId, message.creation.projectId),
		);
		activeEditingMessageId = message.messageId;
		editingPendingTaskRef.current = message;
		editingRevisionRef.current = capturePendingTaskEditorWriteBaseline(
			message.messageId,
		);
			...
			return {
				...
				interactionMode: resolvePendingTaskInteractionMode({
					...
					provider: selectedEnvironmentServerConfig?.providers.find(
						(candidate) =>
							candidate.instanceId === draftModelSelection.instanceId,
					),
				}),
				creation: {
					projectId: selectedProject.id,
	...
	useEffect(() => {
		...
		const stillQueued = flattenQueuedThreadMessages(
			queuedMessagesByThreadKey,
		).some((candidate) => candidate.messageId === editing.messageId);
		if (!stillQueued) {
	...
	return (
		<NewTaskFlowContext.Provider value={value}>
			{props.children}
		</NewTaskFlowContext.Provider>
	);
}
