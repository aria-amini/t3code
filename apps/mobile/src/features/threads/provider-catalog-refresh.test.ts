...
describe("mobile provider catalog refresh", () => {
	...
	it("reports a discovery error and allows retry after the failed command settles", async () => {
		const failure = AsyncResult.failure<never, Error>(
			Cause.fail(new Error("discovery failed")),
		);
		const success = AsyncResult.success("refreshed");
		let callCount = 0;
		const refreshProviders = vi.fn(async () =>
			callCount++ === 0 ? failure : success,
		);
		const refresh = createProviderCatalogRefreshRunner(refreshProviders);
		const environmentId = EnvironmentId.make("environment-mobile");

		expect(providerCatalogRefreshError(await refresh(environmentId))).toBe(
			"discovery failed",
		);
		expect(
			providerCatalogRefreshError(await refresh(environmentId)),
		).toBeNull();
