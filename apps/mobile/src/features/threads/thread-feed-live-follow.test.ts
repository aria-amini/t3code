...
describe("tool-group scroll restoration", () => {
	...
	it("starts normally when the saved row no longer belongs to the group", () => {
		expect(
			resolveThreadWorkGroupInitialScroll([{ id: "other" }], position),
		).toBeUndefined();
		expect(
			resolveThreadWorkGroupInitialScroll([{ id: "read-output" }], undefined),
		).toBeUndefined();
	});
});

describe("tool-group append following", () => {
	const previousRows = Array.from({ length: 10 }, (_, index) => ({
		id: `call-${index}`,
	}));
	...
	it.each([
		...
		{
			name: "dragging before leaving the edge",
			changes: { userScrolling: true },
		},
		{
			name: "opening detail during an append",
			changes: { detailsChanged: true },
		},
		{
			name: "streaming a result",
			changes: { rows: previousRows, contentHeight: 600 },
		},
		{
			name: "updating a lifecycle label",
			changes: { rows: previousRows, contentHeight: 289 },
		},
		{
			name: "prepending old calls",
			changes: { rows: [{ id: "older" }, ...previousRows] },
		},
		{
		...
		expect(shouldFollowThreadWorkGroupAppend({ ...atEnd, ...changes })).toBe(
			false,
		);
	});
});
...
describe("resolveThreadFeedLiveFollow", () => {
	it("pauses immediately when the user starts scrolling", () => {
		expect(
			resolveThreadFeedLiveFollow(true, { type: "user-scroll-begin" }),
		).toBe(false);
	});
	...
	it.each([
		...
		"reconciles follow after a disclosure settles: %j",
		({ expected, ...state }) => {
			expect(
				resolveThreadFeedLiveFollow(!expected, {
