...
export const ThreadListGroupHeader = memo(
	function ThreadListGroupHeader(props: {
		...
		return (
			<View
				className={
					compact ? "flex-row items-center bg-screen" : "flex-row items-center"
				}
				style={{
			...
			>
				<Pressable
					...
					accessibilityHint={
						props.collapsed ? "Expands the project" : "Collapses the project"
					}
					className={
						compact
							? "flex-1 flex-row items-center gap-2.5"
...
export const ThreadListShowMoreRow = memo(
	function ThreadListShowMoreRow(props: {
		readonly variant: ThreadListVariant;
		...
		const button = (
			...
			<Pressable
				accessibilityRole="button"
				accessibilityLabel={
					label === "Show more" ? "Show more threads" : "Show fewer threads"
				}
		...
		return (
			<View
				className={
					compact
						? "flex-row items-center gap-2.5 bg-screen"
			...
			>
				...
				{props.canShowLess
					? button("Show less", "chevron.up", handleShowLess)
					: null}
			</View>
		);
	},
...
export const PendingTaskListRow = memo(function PendingTaskListRow(props: {
	...
	const statusPill = isDraft ? (
		<View className="rounded-full bg-adaptive-amber-500-a12-a16 px-1.5 py-0.5">
			<Text className="text-3xs font-t3-bold text-adaptive-amber-700-300">
				Draft
			</Text>
		</View>
	) : (
		...
		subtitleParts.length > 0 ? (
			<View className="mt-px flex-row items-center gap-1.5">
				<SymbolView
					...
					size={10}
					tintColorClassName={
						compact ? "accent-icon-subtle" : "accent-foreground-muted"
					}
					type="monochrome"
				/>
	...
	const rowContent = compact ? (
		...
		>
			<View
				...
				style={{ paddingLeft: THREAD_LIST_COMPACT_INSET }}
			>
				<View
					className={cn(
						"gap-[3px] pb-[10px]",
						!props.isLast && "border-b border-separator",
					)}
				>
...
export const ThreadListRow = memo(function ThreadListRow(props: {
	...
	const threadAccessibilityLabel = [
		...
		.filter(Boolean)
		.join(", ");
	const subtitleParts = [props.environmentLabel, thread.branch].filter(
	...
	const handleMenuAction = useCallback(
		({ nativeEvent }: { readonly nativeEvent: { readonly event: string } }) => {
			if (nativeEvent.event === "new-thread-on-branch")
	...
	const statusPill = effectiveStatus ? (
		<View
			className={`${effectiveStatus.pillClassName} rounded-full px-1.5 py-0.5`}
		>
			<Text
				className={`text-3xs font-t3-bold ${effectiveStatus.textClassName}`}
			>
						...
						<View className="flex-row items-center justify-between gap-2">
							...
							<View className="flex-row items-center gap-2">
								{props.hasQueuedMessages ? (
									<QueuedMessageIcon selected={selected} />
								) : null}
								{statusPill}
								<Text className="text-base tabular-nums text-foreground-tertiary">
									{timestamp}
