...
export const ThreadListV2SectionDivider = memo(
	function ThreadListV2SectionDivider(props: {
...
export const ThreadListV2SnoozedShelfHeader = memo(
	function ThreadListV2SnoozedShelfHeader(props: {
...
export const ThreadListV2SettledShelfHeader = memo(
	function ThreadListV2SettledShelfHeader(props: {
		readonly count: number;
		readonly disabled?: boolean;
		readonly expanded: boolean;
		readonly onToggle: () => void;
 ...
 */
export const ThreadListV2PendingRow = memo(
	function ThreadListV2PendingRow(props: {
          ...
          second line is usually a stray word or emoji rather than meaning. */}
				...
				{branch || props.environmentLabel ? (
					<View className="mt-1 flex-row items-center gap-1">
						<Text
							className="shrink text-xs text-foreground-muted"
							numberOfLines={1}
						>
							{branch ? (
								<Text
		...
		return (
			<>
				...
				<ControlPillMenu
					actions={
						isDraft ? DRAFT_TASK_MENU_ACTIONS : PENDING_TASK_MENU_ACTIONS
					}
...
export const ThreadListV2Row = memo(function ThreadListV2Row(props: {
	...
	useEffect(() => {
		...
		const delayMs = Math.min(
			Math.max(0, snoozeGateExpiryMs - Date.now()) + 50,
			2_147_483_647,
		);
		const id = setTimeout(
			() => bumpSnoozeGateTick((tick) => tick + 1),
			delayMs,
		);
