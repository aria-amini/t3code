...
describe("resolveThreadProviderInstance", () => {
	it("resolves two environments with the same default instance id independently", () => {
		...
		expect(
			resolveThreadProviderInstance(serverConfigs, threadA)?.accentColor,
		).toBe("#ff8800");
		expect(
			resolveThreadProviderInstance(serverConfigs, threadB)?.accentColor,
		).toBeUndefined();
	});
	...
	it("hides the badge for a single instance with no accent color", () => {
		...
		expect(
			resolveThreadProviderInstance(serverConfigs, thread)?.showBadge,
		).toBe(false);
	});
});
