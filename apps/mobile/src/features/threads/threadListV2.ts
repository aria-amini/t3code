...
export function getThreadListV2OrderedSection(input: {
	...
	const threads = input.threads.filter((thread) => {
		...
		if (
			...
			input.queuedThreadKeys?.has(`${thread.environmentId}:${thread.id}`) !==
				true
		) {
			return false;
		}
...
export function buildThreadListV2ListItems(input: {
	...
	const activeEnd =
		snoozedShelfHeaderIndex ?? settledShelfHeaderIndex ?? threadItems.length;
	const snoozedEnd = settledShelfHeaderIndex ?? threadItems.length;
	const result: ThreadListV2ListItem[] = [
		...threadItems.slice(0, activeEnd),
		...pendingItems,
	];
...
export function buildThreadListV2Items(input: {
	...
	const projectKeys = input.projectRefs
		? new Set(
				input.projectRefs.map((ref) => `${ref.environmentId}:${ref.projectId}`),
			)
		: null;
		...
		if (supportsSnooze && effectiveSnoozed(thread, { now })) {
			...
			if (
				...
				(nextSnoozeWakeAt === null ||
					parseTimestampMs(thread.snoozedUntil) <
						parseTimestampMs(nextSnoozeWakeAt))
			) {
	...
	const visibleSnoozed =
		input.snoozedShelfExpanded === true
			...
			: orderedSnoozed.filter(
					(thread) =>
						`${thread.environmentId}:${thread.id}` === selectedThreadKey,
				);
	...
	const pagedSettled =
		orderedSettled.length > settledLimit
			? orderedSettled.slice(0, settledLimit)
			: orderedSettled;
	...
	const snoozedShelfHeaderIndex =
		orderedSnoozed.length > 0 ? items.length : null;
	...
	const settledShelfHeaderIndex =
		orderedSettled.length > 0 ? items.length : null;
