...
export interface PendingThreadOrder {
	...
	readonly before: ReadonlyMap<
		string,
		{ readonly key: string | null; readonly anchor: string }
	>;
...
export function createPendingThreadOrder(input: {
	...
	return {
		...
		orderedIds,
		before: new Map(
			input.ordered.map((row) => [rowId(row), rowOrder(row, input.section)]),
		),
		assignments: new Map(
			input.assignments.map(({ id, orderKey }) => [id, orderKey]),
		),
		confirmed: new Set(),
...
export function reconcilePendingThreadOrder(
	...
	if (pending.commandsComplete && confirmed.size === pending.assignments.size)
		return null;
	return confirmed.size === pending.confirmed.size
		? pending
		: { ...pending, confirmed };
}
...
export function applyPendingThreadOrder<T extends OrderRow>(
	...
	return [...rows].sort(
		(left, right) =>
			(rank.get(rowId(left)) ?? Infinity) -
			(rank.get(rowId(right)) ?? Infinity),
	);
}
