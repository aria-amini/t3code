...
export function buildComposerSlashCommandItems(input: {
	...
	const allowInteractionMode =
		...
		input.selectedProviderStatus?.showInteractionModeToggle !== false;
	const builtIn = [
		{
			id: "cmd:model",
			type: "slash-command",
	...
	for (const command of input.selectedProviderStatus?.slashCommands ?? []) {
		...
		if (command.name === "compact" && !input.hasCompactableConversation)
			continue;
		...
		if (
			command.name === USAGE_LIMITS_COMMAND.name &&
			input.offersUsageLimits &&
			!input.hasThread
		) {
			continue;
		}
