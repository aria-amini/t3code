...
function makeAvailableUpdateClient(
	overrides: Partial<AppUpdateClient> = {},
): AppUpdateClient {
...
describe("runAppUpdateCheck", () => {
	...
	it("flushes pending writes before restarting", async () => {
		...
		const flushOrder = vi.mocked(environment.flushPendingWrites).mock
			.invocationCallOrder[0]!;
		const reloadOrder = vi.mocked(client.reloadAsync).mock
			.invocationCallOrder[0]!;
	...
	it("keeps the background install armed when the foreground prompt is declined", async () => {
		const client = makeAvailableUpdateClient();
		const { backgroundCallbacks, environment, foregroundStayCallbacks } =
	...
	it("re-arms instead of restarting when the app is no longer safely backgrounded", async () => {
		...
		expect(vi.mocked(environment.onNextBackground).mock.calls[0]![1]).toBe(
			true,
		);
		...
		expect(vi.mocked(environment.onNextBackground).mock.calls[1]![1]).toBe(
			false,
		);
	...
	it("honors an immediate request that joined an in-flight background check", async () => {
		...
		const backgroundCheck = runAppUpdateCheck({
			client,
			deferral,
			environment,
		});
	...
	it.each(["ERR_NOT_AVAILABLE_IN_DEV_CLIENT", "ERR_UPDATES_DISABLED"])(
		...
		async (code) => {
			const reportError = vi
				.spyOn(console, "error")
				.mockImplementation(() => {});
			const error = Object.assign(new Error("Updates are unavailable"), {
				code,
			});
			const client = makeUpdateClient({
				checkForUpdateAsync: vi.fn(async () => {
		...
		const initialCheck = runAppUpdateCheck({
			...
			onStateChange: (state) => {
				...
				reentrantCheck = runAppUpdateCheck({
					client,
					onStateChange: (reentrantState) =>
						reentrantStates.push(reentrantState),
				});
			},
		});
...
describe("shouldRecheckAppUpdateOnForeground", () => {
	it("requires a meaningful background gap", () => {
		expect(shouldRecheckAppUpdateOnForeground(null, 100_000, false)).toBe(
			false,
		);
		expect(
			shouldRecheckAppUpdateOnForeground(
