...
interface AppUpdateCheckInFlight {
	readonly failureListeners: Set<
		NonNullable<AppUpdateCheckOptions["onFailure"]>
	>;
	readonly progress: AppUpdateCheckProgress;
	readonly promise: Promise<void>;
	readonly stateListeners: Set<
		NonNullable<AppUpdateCheckOptions["onStateChange"]>
	>;
}
...
function notifyListeners<A>(
	listeners: ReadonlySet<(value: A) => void>,
	value: A,
): void {
...
async function installAppUpdate(
	...
	if (reloaded._tag === "Failure") {
		reportUpdateFailure(
			reloaded,
			"Downloaded, but could not restart the app.",
			options.onFailure,
		);
		setState("idle");
		deferral.installInProgress = false;
...
async function applyDeferredAppUpdateInstall(
	...
	deferral.installInProgress = true;
	const flushed = await settlePromise(() => environment.flushPendingWrites());
	const safe = await settlePromise(() =>
		environment.isSafeToRestartInBackground(),
	);
...
async function defaultFlushPendingWrites(): Promise<void> {
	...
	const results = await Promise.allSettled([
		import("../../state/use-composer-drafts").then((drafts) =>
			drafts.flushComposerDrafts(),
		),
		import("../../state/thread-outbox").then((outbox) =>
			outbox.flushThreadOutbox(),
		),
	]);
...
async function defaultIsSafeToRestartInBackground(): Promise<boolean> {
	const { isForegroundHandoffActive } =
		await import("../../lib/foreground-handoff");
	if (isForegroundHandoffActive()) return false;
	const { AppState } = await import("react-native");
	return AppState.currentState === "background";
}

function defaultOnNextBackground(
	apply: () => void,
	includeCurrent: boolean,
): void {
...
function isAppUpdateUnavailableError(error: unknown): boolean {
	...
	return (
		typeof code === "string" && UPDATE_CHECK_UNAVAILABLE_ERROR_CODES.has(code)
	);
}
...
export const startAppUpdateForegroundRecheck =
