...
export function UsageLimitsSection({
	...
	const presentations = useAtomValue(
	...
	const selected =
		selectedEnvironmentIds === null
			? presentations
			: new Map(
					[...presentations].filter(([id]) => selectedEnvironmentIds.has(id)),
				);
		...
		<View className="gap-6">
			{failedLabels.length ? (
				<Text className="text-sm text-foreground-muted">
					{failedLabels.join(", ")} could not refresh limits. Showing the last
					known values.
				</Text>
			) : null}
			{pools.length === 0 ? (
				...
				<View key={pool.driver} className="gap-3">
					...
					{pool.windows.map((window) => (
						<PoolWindowCard
							...
							pool={window}
							color={
								pool.driver === "claudeAgent" ? colors.claude : colors.codex
							}
							now={now}
							environmentIds={
								selectedEnvironmentIds === null
...
export function UsageLimitAccountScreen({ route }: AccountScreenProps) {
	...
	const { accountKey, windowId, windowKind, environmentIds, now } =
		route.params;
	const selectedIds =
		environmentIds === null
			? null
			: new Set(environmentIds.map((id) => EnvironmentId.make(id)));
	const selected =
		selectedIds === null
			? presentations
	...
	const pool = collectLimitPools(accounts, now)
		...
		?.windows.find(
			(candidate) => candidate.id === windowId && candidate.kind === windowKind,
		);
	const window = pool?.members.find(
		(member) => member.account.key === accountKey,
	)?.window;
	const reset = pool?.resets.find(
		(candidate) => candidate.member.account.key === accountKey,
	);
			...
			>
				{!account || !window ? (
					<Text className="text-base text-foreground-muted">
						This account is no longer reporting limits on the selected
						environments.
					</Text>
				) : (
					<>
						<View className="gap-2">
							...
							{account.email ? (
								<Pressable
									accessibilityRole="button"
									accessibilityLabel={
										revealed ? "Hide account email" : "Reveal account email"
									}
