...
function AccountInstanceLabel({ value }: { readonly value: string }) {
	...
	if (!value.includes("@")) {
		return (
			<Text
				className="shrink text-xs text-foreground-tertiary"
				numberOfLines={1}
			>
				· {value}
			</Text>
	...
	return (
		<Pressable
			...
			accessibilityLabel={
				revealed ? "Hide account label" : "Reveal account label"
			}
	...
	return (
		<View
			className={
				props.first
					? `gap-3 ${padding}`
					: `gap-3 border-t border-border-subtle ${padding}`
			}
		>
			<View className="flex-row items-center gap-2">
				...
				<View className="min-w-0 flex-1 flex-row items-baseline gap-2">
					...
					{props.detail ? (
						<Text
							className="shrink text-sm text-foreground-muted"
							numberOfLines={1}
						>
							· {props.detail}
						</Text>
