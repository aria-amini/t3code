...
export function UsageRouteScreen() {
	...
	const { merged, environments, selectedEnvironments, isPending, refresh } =
	...
	const showEnvironmentFilter =
		environments.length > 0 || selectedEnvironmentIds !== null;
		...
		() => [
			{
				...
				subtitle: undefined,
				state:
					selectedEnvironmentIds === null ? ("on" as const) : ("off" as const),
			},
	...
	const selectEnvironment = useCallback(
		(value: string) => {
			...
			setSelectedEnvironmentIds((selected) =>
				toggleUsageEnvironment(selected, environments, id),
			);
		},
		[environments],
	);
			...
			showEnvironmentFilter ? (
				<ControlPillMenu
					...
					onPressAction={({ nativeEvent }) =>
						selectEnvironment(nativeEvent.event)
					}
				>
					<Pressable
						accessibilityRole="button"
			...
			<ScrollView
				...
				refreshControl={
					<RefreshControl
						...
						onRefresh={
							showingLimits ? () => void limits.refresh() : refreshWindow
						}
					/>
				}
			>
                ...
                both change every number below, so they share one bar. */}
							...
							{merged.duplicateSources.length > 0 ? (
								<Text className="text-sm text-foreground-muted">
									Counted once across environments sharing a transcript
									directory: {merged.duplicateSources.join(", ")}
								</Text>
							) : null}
							{isPending ? (
			...
			{props.options.map((option) => {
				...
				return (
					<Pressable
						...
						accessibilityRole={
							Platform.OS === "ios" ? "button" : (props.role ?? "button")
						}
				...
				return (
					<View
						...
						className={
							index === 0
								? "gap-2 p-4"
								: "gap-2 border-t border-border-subtle p-4"
						}
					>
...
function TotalsSection(props: {
	...
	const { merged } = props;
	const activePeriods = (
		props.isPast24Hours ? merged.hourly : merged.daily
	).filter((period) => period.totalTokens > 0).length;
	const periodAverage =
		activePeriods === 0 ? 0 : merged.totalTokens / activePeriods;
	const observedInput = merged.uncachedInputTokens + merged.cachedInputTokens;
	const cachedShare =
		observedInput === 0 ? 0 : merged.cachedInputTokens / observedInput;
...
function MetricCell(props: {
	...
	return (
		<View className="w-1/2 gap-0.5 p-4">
			...
			<Text className="text-xl font-t3-medium tabular-nums text-foreground">
				{props.value}
			</Text>
...
function isUsageLoading(environment: EnvironmentUsageStatus) {
	return (
		...
		(environment.summary === null && environment.error === null)
	);
}
...
function usageEnvironmentStatus(environment: EnvironmentUsageStatus): string {
	...
	if (!environment.isConnected)
		return environment.summary
			? "Disconnected · showing saved usage"
