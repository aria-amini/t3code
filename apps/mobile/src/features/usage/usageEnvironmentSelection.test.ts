...
describe("usage environment selection", () => {
	...
	it("can deselect the last environment", () => {
		expect(toggleUsageEnvironment(new Set([a]), environments, a)).toEqual(
			new Set(),
		);
	});
	...
	it("does not count removed IDs toward selecting all current environments", () => {
		expect(
			toggleUsageEnvironment(new Set([a, removed]), environments, b),
		).toEqual(new Set([a, b]));
	});

	it("returns to all mode despite stale IDs when every current environment is selected", () => {
		expect(
			toggleUsageEnvironment(new Set([a, c, removed]), environments, b),
		).toBeNull();
	});

	it("ignores a menu action for an environment that was removed", () => {
		expect(toggleUsageEnvironment(new Set([a]), environments, removed)).toEqual(
			new Set([a]),
		);
	});
	...
	it("includes newly connected environments only in all mode", () => {
		...
		expect(toggleUsageEnvironment(null, expanded, a)).toEqual(
			new Set([b, c, removed]),
		);
		expect(toggleUsageEnvironment(new Set([a, b, c]), expanded, a)).toEqual(
			new Set([b, c]),
		);
	});
});
