...
const DICTATION_LAYOUT =
	Platform.OS === "android"
		? undefined
		: LinearTransition.duration(DICTATION_TIMING.duration).reduceMotion(
				ReduceMotion.System,
			);
					...
					{ perspective: TOOLBAR_PERSPECTIVE },
					{
						translateY: withTiming(-TOOLBAR_HALF_HEIGHT * toSine, {
							...
							easing: (time) => {
								const angle =
									fromRadians +
									(toRadians - fromRadians) * TOOLBAR_FLIP_TIMING.easing(time);
	...
	const animatedStyle = useAnimatedStyle(() => ({
		...
		transform: compact
			? [
					...
					{
						translateY:
							-TOOLBAR_HALF_HEIGHT * Math.sin((rotation.value * Math.PI) / 2),
					},
		...
		<View className="relative h-[44px] overflow-hidden">
			{props.visible !== false ? (
				<Animated.View
					...
					entering={
						props.showsDictation
							? DICTATION_TOOLBAR_ENTERING
							: DRAFT_TOOLBAR_ENTERING
					}
					exiting={
						props.showsDictation
							? DICTATION_TOOLBAR_EXITING
							: DRAFT_TOOLBAR_EXITING
					}
			...
			transform: [
				{
					scaleY: withTiming(
						(WAVEFORM_MIN_BAR_HEIGHT +
							level * (WAVEFORM_BAR_HEIGHT - WAVEFORM_MIN_BAR_HEIGHT)) /
							WAVEFORM_BAR_HEIGHT,
						WAVEFORM_TIMING,
					),
				},
...
function VoiceActionButton(props: {
	...
	useLayoutEffect(() => {
		loadingVisibility.value = withTiming(
			props.loading ? 1 : 0,
			DICTATION_TIMING,
		);
	}, [loadingVisibility, props.loading]);
	const primaryStyle = useAnimatedStyle(() => ({
		opacity: 1 - loadingVisibility.value,
	}));
...
export function ComposerDictationStatus(props: {
	...
	const recordingVisibility = useSharedValue(
		props.phase === "recording" ? 1 : 0,
	);
	useLayoutEffect(() => {
		recordingVisibility.value = withTiming(
			props.phase === "recording" ? 1 : 0,
			DICTATION_TIMING,
		);
		...
		<View className="relative h-11 min-w-0 flex-1 justify-center">
			{isError ? (
				<View className="min-w-0 flex-row items-center gap-1.5 px-2">
					<Text
						className="min-w-0 flex-1 text-sm text-red-400"
						numberOfLines={2}
					>
				...
				<View
					...
					accessibilityLiveRegion={
						props.phase === "recording" ? "none" : "polite"
					}
					className="h-11"
				>
					<Animated.View
					...
					<Animated.View
						className="absolute inset-0 justify-center px-2"
						style={labelStyle}
					>
						<Text
...
export function ComposerDictationStartAction(props: {
	...
	const openSettings =
		props.state.phase === "error" && props.state.errorAction === "settings";
	return (
		<VoiceActionButton
			accessibilityLabel={
