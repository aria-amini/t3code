...
describe("normalizeVoiceInputDecibels", () => {
	...
	])("treats a missing or invalid reading %s as silence", (decibels) => {
		expect(normalizeVoiceInputDecibels(decibels)).toBe(0);
	});

	it.each([-160, -90, -60])(
		"keeps a reading at or below the noise floor %s silent",
		(decibels) => {
			expect(normalizeVoiceInputDecibels(decibels)).toBe(0);
		},
	);
	...
	it("makes near-speech changes visible without an early ceiling", () => {
		expect(
			normalizeVoiceInputDecibels(-6) - normalizeVoiceInputDecibels(-12),
		).toBeGreaterThan(0.18);
		expect(
			normalizeVoiceInputDecibels(-3) - normalizeVoiceInputDecibels(-12),
		).toBeGreaterThan(0.3);
	});
	...
	it.each([0, 6, 160])(
		"caps only full-scale or higher readings %s at one",
		(decibels) => {
