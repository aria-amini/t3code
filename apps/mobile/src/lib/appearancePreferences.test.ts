...
describe("appearancePreferences", () => {
	...
	it("migrates the legacy markdownFontSize key to baseFontSize", () => {
		expect(
			resolveAppearancePreferences({ markdownFontSize: 18 }).baseFontSize,
		).toBe(18);
		expect(
			resolveAppearancePreferences({ baseFontSize: 16, markdownFontSize: 18 })
				.baseFontSize,
		).toBe(16);
	});

	it("keeps explicit overrides and treats missing values as automatic", () => {
		const preferences = resolveAppearancePreferences({
			terminalFontSize: 12,
			codeFontSize: 14,
		});
		...
		expect(
			resolveAppearancePreferences({ terminalFontSize: null }).terminalFontSize,
		).toBe(null);
	});

	it("derives terminal and code sizes from the base size when not overridden", () => {
		const appearance = resolveAppearance(
			resolveAppearancePreferences({ baseFontSize: 15 }),
		);
		...
		const scaled = resolveAppearance(
			resolveAppearancePreferences({ baseFontSize: 22 }),
		);
	...
	it("clamps base and code font sizes", () => {
		...
		expect(resolveAppearancePreferences({ codeFontSize: 4 }).codeFontSize).toBe(
			8,
		);
		expect(
			resolveAppearancePreferences({ codeFontSize: 30 }).codeFontSize,
		).toBe(18);
	});
	...
	it("keeps explicit code word break enabled", () => {
		expect(
			resolveAppearancePreferences({ codeWordBreak: true }).codeWordBreak,
		).toBe(true);
	});
