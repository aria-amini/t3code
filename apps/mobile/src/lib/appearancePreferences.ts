...
export interface NativeMarkdownTypography {
	...
	readonly headingFontSizes: readonly [
		number,
		number,
		number,
		number,
...
export function normalizeBaseFontSize(
	...
	return Math.min(
		MAX_BASE_FONT_SIZE,
		Math.max(MIN_BASE_FONT_SIZE, Math.round(value)),
	);
}
...
function normalizeCodeFontSize(value: number | null | undefined): number {
	...
	return Math.min(
		MAX_CODE_FONT_SIZE,
		Math.max(MIN_CODE_FONT_SIZE, Math.round(value)),
	);
}
...
function deriveTerminalFontSize(baseFontSize: number): number {
	...
	return normalizeTerminalFontSize(
		Math.round(DEFAULT_TERMINAL_FONT_SIZE * scale * 2) / 2,
	);
}
...
export function resolveAppearancePreferences(
	...
	return {
		baseFontSize: normalizeBaseFontSize(
			stored?.baseFontSize ?? stored?.markdownFontSize,
		),
		terminalFontSize:
			...
			Number.isFinite(stored.terminalFontSize)
				...
				: null,
		codeFontSize:
			typeof stored?.codeFontSize === "number" &&
			Number.isFinite(stored.codeFontSize)
...
export function resolveAppearance(
	...
	return {
		...
		terminalFontSize:
			...
			deriveTerminalFontSize(preferences.baseFontSize),
		codeFontSize:
			preferences.codeFontSize ?? deriveCodeFontSize(preferences.baseFontSize),
...
export function resolveMobileCodeSurface(
	codeFontSize: number,
	...
	return {
		fontSize,
		lineNumberFontSize: Math.max(
			8,
			Math.round(MOBILE_CODE_SURFACE.lineNumberFontSize * scale),
		),
...
export function resolveMarkdownFontSizes(
	baseFontSize: number,
): ResolvedMarkdownFontSizes {
	...
	return {
		...
		bodyLineHeight: Math.max(
			18,
			Math.round(MOBILE_TYPOGRAPHY.body.lineHeight * scale),
		),
		codeBlockFontSize,
 ...
 */
export function resolveTextScaleVariables(
	baseFontSize: number,
	...
	for (const [name, role] of Object.entries(TEXT_SCALE_VARIABLE_ROLES)) {
		...
		variables[`${name}--line-height`] = scaledTypographyLineHeight(
			role,
			baseFontSize,
		);
	}
...
export function resolveNativeMarkdownTypography(
	baseFontSize: number,
...
export function stepTerminalFontSize(
	...
	const next =
		direction === -1
			? current - TERMINAL_FONT_SIZE_STEP
			: current + TERMINAL_FONT_SIZE_STEP;
