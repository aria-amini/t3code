...
const DOWNLOAD_DIRECTORY_NAME =
	/^(\d+)-[\da-f]{8}-[\da-f]{4}-[\da-f]{4}-[\da-f]{4}-[\da-f]{12}$/i;
...
function downloadFileName(name: string): string {
	...
	const extension =
		extensionMatch && extensionMatch.index > 0 ? extensionMatch[0] : "";
...
function removeDownloadDirectory(directory: Directory): void {
	try {
		...
		console.warn(
			"[attachment-downloads] could not remove a cached file",
			error,
		);
	}
}
...
async function createCachedAttachmentFile(attachment: AttachmentFileMetadata) {
	...
	try {
		...
		console.warn(
			"[attachment-downloads] could not inspect cached files",
			error,
		);
	}
		...
		share: async (signal, sourceIdentifier) => {
			...
			try {
				...
				try {
					...
					if (sourceIdentifier) {
						const { shareFileFromSource } =
							await import("./shareFileFromSource");
...
export async function shareLocalAttachment(input: {
	...
	try {
		...
		try {
			...
			if (input.signal.aborted) return;
			throw new Error("Could not prepare the attachment for sharing.", {
				cause,
			});
		}
		if (!input.signal.aborted) {
