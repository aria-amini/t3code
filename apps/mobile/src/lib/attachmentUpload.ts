...
export function validateDraftFileAttachments(input: {
	...
	if (input.serverConfig === null)
		return "Server attachment support is still loading.";
	...
	return oversized
		? fileAttachmentTooLargeMessage(oversized.name, maxBytes)
		: null;
}
...
async function releaseCreatedUploadsQuietly(
	...
	try {
		...
		// on the server.
		console.warn(
			"[attachments] could not delete abandoned pending uploads",
			error,
		);
	}
}
...
async function composerImageAttachmentDataUrl(
	...
	if (!isFileBackedComposerAttachment(attachment)) {
		throw new Error(
			`'${attachment.name}' is no longer available. Attach the image again.`,
		);
	}
...
async function uploadFileBytes(
	...
	const file =
		fileUri === undefined
			...
			: new File(
					resolveOwnedComposerAttachmentFileUri(fileUri, Paths.document.uri) ??
						fileUri,
				);
	try {
		if (fileUri === undefined && inlineDataUrl !== undefined) {
		...
		if (result.status < 200 || result.status >= 300) {
			throw new Error(
				`Upload failed for '${attachment.name}' (${result.status}).`,
			);
		}
	} finally {
...
export async function prepareTurnAttachments(input: {
	...
	if (
		...
		(files.length === 0 && !input.supportsImageUploads)
	) {
		try {
	...
	try {
		for (const attachment of input.attachments) {
			...
			if (attachment.type === "image" && !input.supportsImageUploads) {
				uploadedAttachments.push(
					...(await toUploadChatImageAttachments([attachment])),
				);
				continue;
			}
			...
			if (
				...
				if (verification.status === "verified") {
					...
					uploadedAttachments.push(
						uploadedReference(attachment, attachment.uploadedAttachmentId),
					);
					continue;
				}
			...
			uploadedAttachments.push(
				uploadedReference(attachment, result.attachmentId),
			);
		}
		...
		if (referencesChanged && input.persistUploadedReferences) {
			if (
				(await input.persistUploadedReferences(draftAttachments)) === "abandon"
			) {
