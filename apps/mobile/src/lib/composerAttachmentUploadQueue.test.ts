...
describe("composer attachment upload queue", () => {
	...
	it("cancels on disconnect and resumes from the same local draft on reconnect", async () => {
		...
		const upload = vi.fn(
			async (
				...
				return new Promise<boolean>((resolve) =>
					currentSignal.addEventListener("abort", () => resolve(false), {
						once: true,
					}),
				);
			},
		...
		expect(
			states[composerAttachmentUploadKey(environmentId, local.attachment.id)],
		).toEqual({
			status: "ready",
		});
		expect(local.attachment).toMatchObject({
			fileUri: "file:///documents/offline-draft.pdf",
		});
		queue.dispose();
	});
	...
	it("ignores a late completion after removal or environment switch", async () => {
		...
		await queue.settled();
		expect(states).toEqual({
			[composerAttachmentUploadKey(other, "photo")]: { status: "ready" },
		});
		queue.sync([]);
		expect(states).toEqual({});
		queue.dispose();
...
describe("draft upload scope and offline submission", () => {
	it("resolves thread, new-task, and queued-task drafts without crossing environments", () => {
		expect(composerDraftEnvironmentId("environment-1:thread", [])).toBe(
			environmentId,
		);
		expect(
			composerDraftEnvironmentId("new-task:environment-1:project", []),
		).toBe(environmentId);
		expect(
			composerDraftEnvironmentId("pending-task:message", [
				{ messageId: "message", environmentId },
			]),
		).toBe(environmentId);
		expect(composerDraftEnvironmentId("pending-task:missing", [])).toBeNull();
		const colonEnvironment = EnvironmentId.make("a:vcs-status:b");
		expect(composerDraftEnvironmentId(`${colonEnvironment}:thread`, [])).toBe(
			colonEnvironment,
		);
		expect(
			composerDraftEnvironmentId(`new-task:${colonEnvironment}:project`, []),
	...
	it("reads an id-keyed new-task draft's environment from its project stamp", () => {
		...
		expect(
			composerDraftEnvironmentId("new-task:abc123-def456", [], stamped),
		).toBe(environmentId);
		...
		expect(
			composerDraftEnvironmentId("new-task:environment-2:project", [], stamped),
		).toBe(environmentId);
	});
	...
	it("only a failed upload blocks sending; an in-flight one queues instead", () => {
		...
		expect(
			composerAttachmentUploadBlockReason({ ...input, connected: false }),
		).toBeNull();
		expect(
			composerAttachmentUploadBlockReason({
				...input,
		...
		expect(
			composerAttachmentsStillUploading({ ...input, serverConfig: null }),
		).toBe(false);
	});
});
