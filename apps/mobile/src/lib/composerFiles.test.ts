...
describe("composer file attachments", () => {
	beforeEach(() => {
		...
		mocks.size.mockImplementation((uri: string) =>
			uri.startsWith("content:") ? null : 42,
		);
	});
	...
	describe("photo library image conversion", () => {
		...
		it.each([
			...
			"preserves original $extension bytes instead of the picker's JPEG",
			async (original) => {
		...
		it("retains a converted photo when another original cannot be read", async () => {
			mocks.pickMedia.mockResolvedValue({
				canceled: false,
				assets: [
					{ ...photo, fileName: "missing.gif", mimeType: "image/gif" },
					photo,
				],
			});
	...
	describe("photo library videos", () => {
		...
		it("applies the remaining attachment slots to photos and videos together", async () => {
			...
			expect(result.error).toBe(
				"You can attach up to 8 attachments per message.",
			);
			expect(mocks.pickMedia).toHaveBeenCalledWith(
		...
		it("reports a native video retrieval error and ends the foreground handoff", async () => {
			mocks.pickMedia.mockRejectedValue(
				new Error("Could not download video from iCloud."),
			);

			await expect(
				pickComposerMedia({ existingCount: 0, maxVideoBytes: 1024 }),
			).resolves.toEqual({
				attachments: [],
	...
	it("preserves Android picker metadata instead of using the content URI document id", async () => {
		const uri =
			"content://com.android.providers.media.documents/document/video%3A18";
		mocks.pickFile.mockResolvedValue({
			canceled: false,
			assets: [
				{
		...
		expect(mocks.pickFile).toHaveBeenCalledWith({
			multiple: true,
	...
	it("persists provider selections that require a readable cache copy", async () => {
		...
		mocks.copy.mockImplementation((uri: string) => {
			if (uri === providerUri)
				throw new Error("The provider URI is not directly readable.");
		});
		...
		expect(result.files).toEqual([
			expect.objectContaining({
				name: "Cloud recording.mp4",
				fileUri:
					"file:///documents/t3-composer-attachments/attachment-id-Cloud recording.mp4",
			}),
		]);
		expect(mocks.copy).toHaveBeenCalledWith(
			cachedUri,
			result.files[0]!.fileUri,
		);
	});
	...
	it("reports picker failures and releases the foreground handoff", async () => {
		mocks.pickFile.mockRejectedValue(
			new Error("The document provider is unavailable."),
		);
	...
	it("rejects files that exceed the environment's advertised upload limit", async () => {
		...
		await expect(
			pickComposerFiles({ existingCount: 0, maxBytes: 1024 * 1024 }),
		).resolves.toEqual({
			files: [],
	...
	it.each([0, undefined])(
		"copies an Android SAF file when the picker size is %s",
		async (size) => {
			const reader = {
				readBytes: vi
					.fn()
			...
			mocks.size.mockImplementation((uri: string) =>
				uri.startsWith("content:") ? 0 : 42,
			);
			mocks.open.mockImplementation((uri: string) =>
				uri.startsWith("content:") ? reader : writer,
			);
			mocks.pickFile.mockResolvedValue({
				canceled: false,
				assets: [
