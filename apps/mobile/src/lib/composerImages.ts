...
export interface DraftComposerImageAttachment extends Omit<
...
export type FileBackedComposerAttachment = DraftComposerAttachment & {
	readonly fileUri: string;
};
...
export async function persistComposerAttachmentFile(
	...
	try {
		...
		try {
			...
			console.warn(
				"[composer-attachments] could not remove a partial copy",
				cleanupError,
			);
		}
		throw error;
	...
	if (maxBytes !== undefined && copiedSize !== null && copiedSize > maxBytes) {
		try {
			...
			console.warn(
				"[composer-attachments] could not remove an oversized copy",
				cleanupError,
			);
		}
		throw new Error(fileAttachmentTooLargeMessage(name, maxBytes));
...
export async function removePersistedComposerAttachmentFile(
	uri: string,
): Promise<void> {
	try {
...
async function createComposerFileAttachment(input: {
	...
	try {
		...
		if (sizeBytes > input.maxBytes) {
			throw new Error(
				fileAttachmentTooLargeMessage(input.name, input.maxBytes),
			);
		}
		return {
			id: uuidv4(),
...
export async function pickComposerFiles(input: {
	...
	for (const file of result.assets) {
		...
		try {
			...
			error =
				cause instanceof Error ? cause.message : `Could not read '${name}'.`;
		}
	}
...
async function loadImagePicker() {
	try {
		...
		throw new Error("The photo library is unavailable right now.", {
			cause: error,
		});
	}
}
...
async function loadClipboard() {
	try {
		...
		throw new Error("Clipboard paste is unavailable right now.", {
			cause: error,
		});
	}
}
...
export async function pickComposerImages(input: {
	...
	return {
		images: result.attachments.filter(
...
export async function pickComposerMedia(input: {
	...
	try {
		result = await imagePicker.launchImageLibraryAsync({
			mediaTypes:
				input.maxVideoBytes === undefined ? ["images"] : ["images", "videos"],
	...
	for (const asset of result.assets) {
		...
		if (asset.type === "video" || mimeType?.startsWith("video/")) {
			...
			try {
				...
				error =
					cause instanceof Error
						? cause.message
						: `Could not read '${asset.fileName ?? "video"}'.`;
			}
			continue;
		}
		...
		attachments.push({
			...
			dataUrl,
			previewUri:
				mimeType === asset.mimeType?.toLowerCase() ? asset.uri : dataUrl,
		});
	}
	...
	try {
		...
		return {
			...
			error:
				error instanceof Error
					? error.message
					: "Clipboard paste is unavailable right now.",
		};
	}
...
export function isOwnedPastedImageUri(uri: string): boolean {
	try {
		...
		return (
			segments.at(-2) === OWNED_PASTED_IMAGE_DIRECTORY &&
