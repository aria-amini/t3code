...
describe("mobile remote connection records", () => {
	...
	it("keeps existing DPoP tunnel records read-only after upgrading", () => {
		expect(isRelayManagedConnection({ authenticationMethod: "dpop" })).toBe(
			true,
		);
		expect(isRelayManagedConnection({ authenticationMethod: "bearer" })).toBe(
			false,
		);
	});
	...
	it("drops short-lived managed environment credentials from stable records", () => {
		...
		expect(toStableSavedRemoteConnection(connection)).not.toHaveProperty(
			"dpopAccessToken",
		);
	});
});
