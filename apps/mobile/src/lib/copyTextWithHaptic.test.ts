...
describe("copyTextWithHaptic", () => {
	...
	it("returns false when the clipboard write fails", async () => {
		...
		mocks.setStringAsync.mockRejectedValueOnce(
			new Error("native clipboard failure"),
		);
	...
	it("reports structured failures without including clipboard contents", async () => {
		const content =
			"https://accounts.google.com/auth?state=private-state&code=private-code";
		...
		expect(consoleError).toHaveBeenCalledWith(
			...
			expect.objectContaining({
				_tag: "CopyTextHapticFeedbackError",
