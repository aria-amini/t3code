...
describe("thread work-log text sizing", () => {
	it.each([11, 16, 22])(
		...
		(baseFontSize) => {
			expect(
				deriveThreadWorkLogSizing({ baseFontSize, fontScale: 1 }),
			).toMatchObject({
	...
	it("invalidates native text measurements even when the minimum row height is unchanged", () => {
		const original = deriveThreadWorkLogSizing({
			baseFontSize: 16,
			fontScale: 1,
		});
...
describe("resizable pane constraints", () => {
	it("preserves a useful main pane while constraining a trailing pane", () => {
		...
		expect(
			constrainAuxiliaryPaneWidth({ preferredWidth: 440, availableWidth: 900 }),
		).toBe(340);
		expect(
			constrainAuxiliaryPaneWidth({
				preferredWidth: 100,
...
describe("deriveLayout", () => {
	...
	it("switches only after both space requirements are met", () => {
		expect(
			deriveLayout({
				...
				height: SPLIT_LAYOUT_MIN_HEIGHT,
			}).variant,
		).toBe("split");
		expect(
			deriveLayout({
				width: SPLIT_LAYOUT_MIN_WIDTH - 1,
				height: SPLIT_LAYOUT_MIN_HEIGHT,
			}).variant,
		).toBe("compact");
		expect(
			deriveLayout({
				width: SPLIT_LAYOUT_MIN_WIDTH,
				height: SPLIT_LAYOUT_MIN_HEIGHT - 1,
			}).variant,
		).toBe("compact");
	});
	...
	it("keeps the sidebar within usable native-column bounds", () => {
		...
		expect(deriveLayout({ width: 1_600, height: 1_000 }).listPaneWidth).toBe(
			380,
		);
	});
});
...
describe("deriveWorkspacePaneLayout", () => {
	...
	it("keeps file navigation on the native stack below the inspector breakpoint", () => {
		...
		expect(
			deriveFileInspectorPaneLayout({ layout, viewportWidth: 744 }),
		).toEqual({
			supported: false,
			width: null,
		});
