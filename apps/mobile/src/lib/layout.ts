...
export function deriveThreadWorkLogSizing(input: {
	...
	const lineHeight = scaledTypographyLineHeight(
...
export function deriveWorkspacePaneLayout(input: {
	...
	if (auxiliaryPaneRole === "inspector") {
		...
		const auxiliaryPaneVisible =
			fileInspector.supported && input.auxiliaryPanePreferredVisible;
		...
		const primarySidebarWidth = primarySidebarVisible
			? (input.layout.listPaneWidth ?? 0)
			: 0;
	...
	const contentPaneWidth = Math.max(
		0,
		viewportWidth - preferredPrimarySidebarWidth,
	);
	const supportsAuxiliaryPane =
		input.layout.usesSplitView &&
		contentPaneWidth >= AUXILIARY_PANE_MIN_CONTENT_WIDTH;
	const auxiliaryPaneVisible =
		supportsAuxiliaryPane && input.auxiliaryPanePreferredVisible;
	...
	return {
		...
		auxiliaryPaneWidth: supportsAuxiliaryPane
			? constrainAuxiliaryPaneWidth({
					preferredWidth:
						input.auxiliaryPanePreferredWidth ?? defaultAuxiliaryPaneWidth,
...
export function deriveFileInspectorPaneLayout(input: {
	...
	const availableContentWidth = Math.max(
		0,
		viewportWidth - reservedLeadingWidth,
	);
	const supported =
		input.layout.usesSplitView &&
		viewportWidth >= FILE_INSPECTOR_MIN_VIEWPORT_WIDTH;
...
export function constrainAuxiliaryPaneWidth(input: {
	...
	const maxWidth = Math.max(
		...
		Math.min(
			...
			availableWidth - FILE_INSPECTOR_MIN_MAIN_WIDTH,
		),
	);
	return clamp(
		Math.round(safePreferredWidth),
		AUXILIARY_PANE_MIN_WIDTH,
		maxWidth,
	);
}
...
export function deriveCenteredContentHorizontalPadding(input: {
	...
	return (
		minimumPadding + Math.max(0, (viewportWidth - input.maxContentWidth) / 2)
	);
}
