...
describe("resolveMarkdownLinkPresentation", () => {
	it("treats protocol-relative media as an external URL, not a filesystem path", () => {
		expect(
			resolveMarkdownLinkPresentation(
				"//cdn.example.com/clip.mp4?sig=a%2fb#t=2",
			),
		).toEqual({
			kind: "external",
	...
	it("separates encoded filename characters from a video playback fragment", () => {
		expect(
			resolveMarkdownLinkPresentation("/tmp/clip%23one.mp4#t=2"),
		).toMatchObject({
	...
	it("extracts external link hosts", () => {
		expect(
			resolveMarkdownLinkPresentation("https://example.com/docs?q=1"),
		).toEqual({
			kind: "external",
	...
	it("recognizes relative source paths and bare filenames", () => {
		expect(
			resolveMarkdownLinkPresentation("apps/mobile/src/index.ts:10"),
		).toEqual({
			kind: "file",
	...
	it.each(["md", "html", "xml"])(
		"recognizes a bare spaced .%s filename",
		(extension) => {
			expect(
				resolveMarkdownLinkPresentation(
					`Updated%20cutover%20checklist.${extension}`,
				),
			).toMatchObject({
				kind: "file",
	...
	it("recognizes spaced relative paths", () => {
		expect(
			resolveMarkdownLinkPresentation("docs/My%20Folder/checklist.xml"),
		).toMatchObject({
			kind: "file",
