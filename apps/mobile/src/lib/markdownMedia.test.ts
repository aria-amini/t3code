...
describe("resolveMarkdownMediaPreview", () => {
	it("decodes remote filenames once without changing the authored URL", () => {
		const href =
			"https://cdn.example.com/clip%20one%2520%2Emp4?signature=a%2fb#t=2";
	...
	it("provides extensionless image actions only for image embeds", () => {
		...
		expect(
			resolveMarkdownMediaPreview(href, { ...input, imageEmbed: true }),
		).toMatchObject({
			kind: "image",
			source: {
				actionsSource: {
	...
	it("separates a video playback fragment from literal filename characters", () => {
		expect(
			resolveMarkdownMediaPreview("/tmp/clip%23one.mp4#t=2", input),
		).toMatchObject({
			kind: "video",
			source: {
				srcFragment: "#t=2",
				...
				actionsSource: {
					reference: { kind: "file", path: "/tmp/clip#one.mp4" },
				},
			},
		});
	});
	...
	it("serves a linked T3 attachment file in place like any other host path", () => {
		const path =
			"/home/demo/.t3/userdata/attachments/11111111-1111-4111-8111-111111111111-mp4.mp4";
	...
	it("resolves protocol-relative media for native APIs without rewriting its signed query", () => {
		expect(
			resolveMarkdownMediaPreview(
				"//cdn.example.com/clip.mp4?signature=a%2fb#t=2",
				input,
			),
		).toMatchObject({
			kind: "video",
			source: {
				...
				actionsSource: {
					reference: {
						kind: "url",
						url: "//cdn.example.com/clip.mp4?signature=a%2fb#t=2",
					},
				},
			},
		});
