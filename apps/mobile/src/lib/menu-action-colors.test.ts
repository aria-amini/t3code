...
describe("withMenuActionIconColors", () => {
	it.each(["#111111", "#eeeeee"])(
		...
		(icon) => {
			...
			const result = withMenuActionIconColors(actions, {
				icon,
				destructiveIcon: "#ff0000",
			});
			...
			expect(result[1]?.subactions?.[0]?.subactions?.[0]?.imageColor).toBe(
				icon,
			);
	...
	it.each(["#123456", "transparent", 0])(
		"preserves explicit icon color %s",
		(imageColor) => {
			const action: MenuAction = {
				title: "Delete",
				image: "trash",
