...
function compositeOver(overlay: string, background: string): string {
	...
	const channels = [1, 2, 3].map((index) =>
		Math.round(
			Number(overlayMatch[index]) * alpha +
				backgroundChannels[index - 1]! * (1 - alpha),
		),
	);
...
describe("mobile themes", () => {
	it("declares every runtime theme variable in the static stylesheet", () => {
		const generatedVariables = createMobileThemeVariables(
			BUILT_IN_THEMES[0].colors,
			"light",
		);
		expect(
			Object.keys(readDefaultMobileThemeVariables("light")).sort(),
		).toEqual(Object.keys(generatedVariables).sort());
	...
	it("shares all built-in desktop palettes", () => {
		...
		for (const themeId of BUILT_IN_THEME_IDS) {
			expect(
				getMobileThemeVariables(themeId, "light")["--color-screen"],
			).toMatch(/^#/);
			expect(
				getMobileThemeVariables(themeId, "dark")["--color-screen"],
			).toMatch(/^#/);
		}
	});

	it("preserves the existing mobile palette as the default", () => {
		expect(readDefaultMobileThemeVariables("light")["--color-screen"]).toBe(
			"#f2f2f7",
		);
		expect(readDefaultMobileThemeVariables("dark")["--color-screen"]).toBe(
			"#0a0a0a",
		);
		expect(
	...
	it("uses the same preview roles and standard artwork as desktop", () => {
		expect(
			getMobileThemePreviewColors(DEFAULT_MOBILE_THEME_ID, "light"),
		).toEqual({
			canvas: "#fcfcfc",
			accent: "#f4f4f5",
	...
	it("normalizes persisted theme preferences", () => {
		...
		expect(normalizeMobileThemeId("missing-theme")).toBe(
			DEFAULT_MOBILE_THEME_ID,
		);
	...
	it("migrates one theme choice to both appearances and preserves independent choices", () => {
		...
		expect(
			resolveMobileThemeIds({ themeId: "grove", lightThemeId: "missing" }),
		).toEqual({
	...
	it("changes either theme without switching the active appearance", () => {
		...
		expect(
			createMobileThemeSelectionPatch(themeIds, "light", "dark", "ocean"),
		).toEqual({
		...
		expect(
			createMobileThemeSelectionPatch(themeIds, "light", "light", "iris"),
		).toEqual({
			lightThemeId: "iris",
			darkThemeId: "grove",
			themeId: "iris",
	...
	it("changes native palette color opacity for fades", () => {
		...
		expect(themeColorWithAlpha("rgba(18, 52, 86, 0.98)", 0)).toBe(
			"rgba(18, 52, 86, 0)",
		);
	});
