...
export type MobileThemeIds = Readonly<
	Record<MobileThemeAppearance, MobileThemeId>
>;
...
export type MobileThemeVariables = Readonly<
	Record<MobileThemeVariable, string>
>;

export function normalizeMobileThemeId(value: unknown): MobileThemeId {
	return typeof value === "string" &&
		(MOBILE_THEME_IDS as readonly string[]).includes(value)
...
function readableMessageAccent(accent: string, surface: string): string {
	...
	const target =
		contrastRatio(black, surfaceChannels) >=
		contrastRatio(white, surfaceChannels)
			? black
			: white;
...
export function createMobileThemeVariables(
	...
	return {
		...
		"--color-md-user-fence-bg": withAlpha(
			"#000000",
			appearance === "dark" ? 0.28 : 0.16,
		),
		...
		"--color-user-bubble-foreground-muted": withAlpha(
			c.messageForeground,
			0.78,
		),
		...
		"--color-backdrop": withAlpha(
			"#000000",
			appearance === "dark" ? 0.48 : 0.22,
		),
		"--color-drawer": withAlpha(c.sidebar, 0.99),
		"--color-drawer-shadow": withAlpha(
			"#000000",
			appearance === "dark" ? 0.32 : 0.12,
		),
...
export function getMobileThemeVariables(
	...
	const theme =
		BUILT_IN_THEMES.find((candidate) => candidate.id === themeId) ??
		BUILT_IN_THEMES[0];
	...
	return overrides
		? ({ ...baseVariables, ...overrides } as MobileThemeVariables)
		: baseVariables;
}
