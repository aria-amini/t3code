...
describe("mobile model options", () => {
	...
	it("does not materialize catalog defaults for missing stored options", () => {
		...
		expect(option?.capabilities?.optionDescriptors?.[0]?.id).toBe(
			"serviceTier",
		);
		...
		expect(explicitOption?.selection.options).toEqual([
	...
	describe("Antigravity selections", () => {
				...
				optionDescriptors: [
					{
						...
						type: "select",
						options: [
							{ id: "current/default", label: "Default", isDefault: true },
						],
		...
		it.each([
			...
			"keeps a %s provider's selection and known model details",
			(_state, update) => {
				...
				expect(
					resolveSelectableModelSelection(unavailableConfig, selection),
				).toBe(selection);
				expect(
					resolveDefaultableModelSelection(unavailableConfig, selection),
				).toBe(selection);
				expect(isModelSelectionUnavailable(unavailableConfig, selection)).toBe(
					true,
				);
		...
		it("keeps an exact selection when its model leaves and returns to the catalog", () => {
			...
			expect(resolveDefaultableModelSelection(changedConfig, selection)).toBe(
				selection,
			);
			...
			expect(
				resolveNewTaskModelSelection({
					draftSelection: null,
					projectDefaultSelection: resolveDefaultableModelSelection(
						changedConfig,
						selection,
					),
			...
			expect(resolveDefaultableModelSelection(config, selection)).toBe(
				selection,
			);
			expect(
				buildModelOptions(config, null)[0]?.selection.options,
			).toBeUndefined();
		});
		...
		it("uses configured instance metadata when provider status is missing", () => {
			...
			expect(
				resolveDefaultableModelSelection(missingStatusConfig, selection),
			).toBe(selection);
			expect(isModelSelectionUnavailable(missingStatusConfig, selection)).toBe(
				true,
			);
		...
		it("keeps offline selections without assuming that an unknown instance is Antigravity", () => {
			...
			expect(buildModelOptions(null, selection)[0]?.isUnavailable).not.toBe(
				true,
			);
			expect(isModelSelectionUnavailable(unknownConfig, selection)).toBe(false);
			expect(
				resolveSelectableModelSelection(unknownConfig, selection),
			).toBeNull();
		});
	});
