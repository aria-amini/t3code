...
export function resolveDefaultableModelSelection(
	...
	const provider = config.providers.find(
		(candidate) => candidate.instanceId === usable.instanceId,
	);
	const model = provider?.models.find(
		(candidate) => candidate.slug === usable.model,
	);
	return provider?.driver !== "antigravity" && model?.isLegacy === true
		? null
		: usable;
}
...
export function resolveNewTaskModelSelection(input: {
	...
	return (
		...
		input.modelOptions.find(
			(option) => option.isDefault && !option.isUnavailable,
		)?.selection ??
...
export function buildModelOptions(
	...
	if (fallbackModelSelection) {
		...
		if (existing) {
			...
			const provider = config?.providers.find(
				(candidate) =>
					candidate.instanceId === fallbackModelSelection.instanceId,
			);
			const instanceConfig =
				config?.settings?.providerInstances[fallbackModelSelection.instanceId];
			const model = provider?.models.find(
...
export function groupByProvider(
	...
	const groups = new Map<
		string,
		{ providerLabel: string; models: ModelOption[] }
	>();
