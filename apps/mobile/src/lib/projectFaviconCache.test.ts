...
const native = vi.hoisted(() => ({
	load: vi.fn(
		async (_url: string, options: { maxWidth: number; maxHeight: number }) => ({
...
describe("mobile project icon thumbnails", () => {
	it("reduces an oversized encoding and deletes temporary thumbnail files", async () => {
		...
		const thumbnail = await downscaleProjectFavicon(
			image,
			new AbortController().signal,
		);
		expect(thumbnail).toBe(`data:image/png;base64,${png}`);
		expect(
			native.load.mock.calls.map(([, options]) => options.maxWidth),
		).toEqual([96, 48]);
	...
	it("releases a decoded image when its request was canceled", async () => {
		...
		await expect(
			downscaleProjectFavicon(image, controller.signal),
		).rejects.toThrow();
	...
	it("rejects an image the native decoder did not downsize", async () => {
		...
		await expect(
			downscaleProjectFavicon(image, new AbortController().signal),
