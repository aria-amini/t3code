 ...
 */
export function attachProjectFaviconDatabase(
	service: MobileDatabase.MobileDatabase["Service"],
) {
	database = service;
}
...
const runDatabase = <A, E>(
	use: (
		database: MobileDatabase.MobileDatabase["Service"],
	) => Effect.Effect<A, E>,
) =>
	database
...
export async function downscaleProjectFavicon(
	...
	for (const size of [
		PROJECT_FAVICON_THUMBNAIL_SIZE,
		PROJECT_FAVICON_THUMBNAIL_SIZE / 2,
	]) {
		signal.throwIfAborted();
		const decoded = await Image.loadAsync(image.url, {
			maxWidth: size,
			maxHeight: size,
		});
			...
			try {
				...
				const mimeType = base64.startsWith("/9j/")
					...
					: base64.startsWith("iVBORw0KGgo")
						...
						: null;
				if (!mimeType)
					throw new Error("Unsupported project icon thumbnail encoding.");
				const dataUrl = `data:${mimeType};base64,${base64}`;
				if (dataUrl.length <= PROJECT_FAVICON_MAX_DATA_URL_LENGTH)
					return dataUrl;
			} finally {
				file.delete();
			}
...
export const projectFaviconCache = createProjectFaviconCache({
	storage: {
		...
		remove: (key, entry) =>
			runDatabase((database) =>
				database.removeCache(entry.environmentId, CACHE_KIND, key),
			),
	},
