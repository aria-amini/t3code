...
describe("pending user input answers", () => {
	it("accepts free-text answers to async questions without options", () => {
		...
		const questions = derivePendingRequests([requested]).userInputs[0]
			?.questions;
		expect(questions).toEqual([question]);
		expect(
			buildPendingUserInputAnswers(questions!, {
				"0": { customAnswer: "Example" },
			}),
		).toEqual({
			"0": "Example",
		});
	...
	it("keeps duplicate labels and whitespace-sensitive native values separate", () => {
		...
		expect(
			isPendingUserInputOptionSelected(nativeQuestion, first, " choice "),
		).toBe(true);
		expect(
			isPendingUserInputOptionSelected(nativeQuestion, first, "choice"),
		).toBe(false);
		expect(
			buildPendingUserInputAnswers([nativeQuestion], { choice: first }),
		).toEqual({
			choice: " choice ",
		});
		...
		expect(
			isPendingUserInputOptionSelected(nativeQuestion, second, " choice "),
		).toBe(false);
		expect(
			isPendingUserInputOptionSelected(nativeQuestion, second, "choice"),
		).toBe(true);
		expect(
			buildPendingUserInputAnswers([nativeQuestion], { choice: second }),
		).toEqual({
			choice: "choice",
		});
	});
	...
	it("keeps exact native values in multi-select answers", () => {
		...
		expect(
			buildPendingUserInputAnswers([question], { choice: second }),
		).toEqual({
			choice: ["choice"],
		});
	});
	...
	it("ignores custom answers when a question only accepts choices", () => {
		...
		expect(
			setPendingUserInputCustomAnswer(nativeQuestion, draft, "Custom text"),
		).toBe(draft);
		expect(
			isPendingUserInputOptionSelected(nativeQuestion, draft, " choice "),
		).toBe(true);
		expect(
			buildPendingUserInputAnswers([nativeQuestion], { choice: draft }),
		).toEqual({
			choice: " choice ",
		});
	});
...
function makeThread(
	input: Partial<OrchestrationThread> &
		Pick<OrchestrationThread, "id" | "projectId" | "title">,
): OrchestrationThread {
	return {
		modelSelection: {
...
describe("buildThreadFeed", () => {
	it("reuses unchanged feed and presentation rows during an assistant text update", () => {
		...
		const expandedGroups = new Set([
			"work-group:completed-tool",
			"work-group:active-tool",
		]);
	...
	it("regroups cached activities for message changes and pagination", () => {
		...
		expect(initial[0]).toMatchObject({
			activities: [{ id: "work-1" }, { id: "work-3" }],
		});
		...
		const reordered = buildThreadFeed({
			...
			messages: [
				messages[0]!,
				{ ...messages[1]!, createdAt: "2026-04-01T00:00:06.000Z" },
			],
		});
		...
		const prepended = buildThreadFeed(thread, {
			loadedMessages: [olderMessage, ...messages],
		});
	...
	it.each([
		...
		"preserves existing runtime details and ignores invalid messages: $message",
		(input) => {
			const thread = makeThread({
	...
	it.each([
		...
		"expands the full $summary label once, retaining its formatting and copy text",
		(input) => {
			const thread = makeThread({
	...
	it("keeps setup failures visible without routine setup notices before or after a turn", () => {
		...
		for (const currentTurn of [null, latestTurn]) {
			...
			if (group?.type !== "activity-group")
				throw new Error("Expected the setup failure group");
			expect(group.activities[0]?.getCopyText()).toContain(
				"Setup command was not found",
			);
		}
	});
	...
	it.each(["setup-script.requested", "setup-script.started"])(
		...
		(kind) => {
			...
			expect(feed).toMatchObject([
				{
					type: "activity-group",
					activities: [{ id: "setup-error", status: "failure" }],
				},
			]);
		},
	);
	...
	it("keeps MCP inputs available to expanded mobile work rows", () => {
		...
		expect(group.activities[0]?.getFullDetail()).toContain(
			'"query": "work log"',
		);
		expect(workEntryRowLabel(group.activities[0]!.workEntry, true)).toBe(
			"repository.search",
		);
		expect(group.activities[0]?.getFullDetail()).not.toContain(
			"repository.search",
		);
	});
	...
	it.each([
		...
		({
			...
			if (item)
				expect(activity.getFullDetail()).toContain(
			...
			if (settledDisplayName) {
				...
				expect(
					settledRows.find((entry) => entry.type === "work-toggle"),
				).toMatchObject({
	...
	it("retains Claude MCP metadata behind friendly row and running labels", () => {
		...
		expect(activity.getCopyText()).toBe(
			`MCP tool call\n${detail}\n${fullDetail}`,
		);
		expect(
			deriveThreadFeedPresentation(
	...
	it.each([
		...
		({ status, displayName, liveDisplayName, detail, hasFailure }) => {
			...
			const present = (currentThread: OrchestrationThread) =>
				deriveThreadFeedPresentation(
					...
					currentThread.latestTurn?.state === "running"
						? currentThread.latestTurn.startedAt
						: null,
				);
			...
			expect(
				settledRows.find((entry) => entry.type === "activity-group"),
			).toMatchObject({
				id: `work-details:${groupId}`,
				activities: [
					{ id: "preview-click-started", summary: displayName, live: false },
				],
			});
		},
	);
	...
	it.each([
		...
		(commandCount, summary, summaryKind) => {
			...
			expect(
				...
				{ type: "work-toggle", summary, summaryKind, live: false },
			]);
		},
	);
	...
	it("defers large tool output expansion until a work row is opened or copied", () => {
		...
		if (expanded[1]?.type === "activity-group") {
			...
			expect(expanded[1].activities[0]?.getFullDetail).toBe(
				group.activities[0]?.getFullDetail,
			);
		}
		expect(serializedToolOutputs).toBe(0);
	...
	it("keeps the first and terminal assistant messages visible around settled work", () => {
		...
		expect(retimed[1]).toMatchObject({
			type: "turn-fold",
			label: "Worked for 23s",
		});
		expect(collapsed[1]).toMatchObject({
			type: "turn-fold",
			label: "Worked for 17s",
	...
	it("keeps an active turn expanded and classifies error-shaped tool output", () => {
		...
		const feed = buildThreadFeed(thread);
		expect(
			deriveThreadFeedPresentation(feed, thread.latestTurn, new Set()),
		).toMatchObject([
			{
				type: "work-toggle",
			...
			(
				[
					...
					{ lifecycleStatus: "failed", summary: "Failed pnpm", shimmer: false },
					{
						...
						summary: "Declined pnpm",
						shimmer: false,
					},
					{
						lifecycleStatus: "stopped",
						summary: "Stopped pnpm",
						shimmer: false,
					},
				] as const
		...
		({ command, lifecycleStatus, summary, shimmer }) => {
			...
			const activity = (
				...
				workEntry: {
					...
					...(command
						? { command, itemType: "command_execution" as const }
						: {}),
				},
			});
			const feed: ThreadFeedEntry[] = [
			...
			expect(rows.slice(0, 3).map((entry) => [entry.id, entry.type])).toEqual([
				...
				[
					shimmer ? "live-activity-row" : "work-live:work-group:activity-3",
					"work-toggle",
				],
			]);
			expect(
				rows
					.slice(0, 3)
					.map((entry) => entry.type === "work-toggle" && entry.live),
			...
			expect(rows.filter((entry) => entry.type === "thinking")).toHaveLength(
				shimmer ? 0 : 1,
			);
			...
			expect(stoppedRows.some((entry) => entry.type === "thinking")).toBe(
				false,
			);
			expect(
				stoppedRows.filter((entry) => entry.type === "work-toggle"),
			).toMatchObject([
			...
			expect(
				completedRows.filter((entry) => entry.type === "work-toggle"),
			).toMatchObject([
				{ live: false, shimmer: false },
				{ live: false, shimmer: false },
			]);
	...
	it("keeps one live slot while calls fail and restart", () => {
		...
		const liveIds = (
			activities: ReadonlyArray<ReturnType<typeof makeActivity>>,
		) =>
			deriveThreadFeedPresentation(
				buildThreadFeed(
					makeThread({
		...
		expect(liveIds([call(1, "inProgress")])).toEqual([
		...
		).toEqual(["work-toggle:live-activity-row"]);
		...
		expect(
			liveIds([call(1, "inProgress"), errorRow, call(2, "inProgress")]),
		).toEqual([
			"work-toggle:work-live:work-group:tool:turn-failing-calls:call-1",
	...
	it.each([
		...
		(status, shimmer) => {
			...
			expect(
				rows.some((entry) => entry.type === "work-toggle" && entry.shimmer),
			).toBe(shimmer);
		},
	);
	...
	it("collapses interleaved tool lifecycles by call identity", () => {
		...
		expect(
			runningRows.find((entry) => entry.type === "activity-group"),
		).toMatchObject({
			id: `work-details:${groupId}`,
			activities: [
				{
		...
		expect(
			completedRows.find((entry) => entry.type === "activity-group"),
		).toMatchObject({
			id: `work-details:${groupId}`,
			activities: [
				{
		...
		const correctedGroup = correctedFeed.find(
		...
		expect(correctedGroup?.activities[0]?.getCopyText()).toContain(
			"Corrected failure output",
		);
		expect(activityGroup?.activities[0]?.getCopyText()).toContain(
			"first output",
		);
		...
		expect(
			correctedRows.find((entry) => entry.type === "activity-group"),
		).toMatchObject({
			id: "call-a-1",
...
describe("quiet timeline: nested agents", () => {
	...
	it("folds a turn's direct spawns into one batch row that tracks their states", () => {
		...
		const activities = [
			...
			agent("a-progress", "task.progress", "a", "running", 3, {
				detail: "Reading files",
			}),
			shell,
			agent("b-progress", "task.progress", "b", "running", 6, {
				detail: "Grepping",
			}),
		];
		const rowsFor = (
			extraActivities: ReadonlyArray<ReturnType<typeof makeActivity>>,
		) =>
			buildThreadFeed(
				makeThread({
	...
	it("folds the tool call that launched an agent into its spawn card", () => {
		...
		const rows = feed.flatMap((entry) =>
			entry.type === "activity-group"
				? entry.activities.map((row) => row.id)
				: [],
		);
		expect(rows).toEqual(["agent-started"]);
		expect(
			deriveThreadFeedPresentation(feed, null, new Set([turnId])).map(
				(row) => row.type,
			),
	...
	it("presents a spawn batch as one card whose status line follows the newest member activity", () => {
		...
		const settled = presentFor([
			...
			agent("b-start", "task.started", "b", 2),
			agent("a-done", "task.completed", "a", 4, { status: "completed" }),
			agent("b-done", "task.completed", "b", 5, {
				status: "failed",
				error: "boom",
			}),
		]);
	...
	it("folds bypassed Claude workflow members into the coordinator's batch and settles them with it", () => {
		...
		expect(rows[0]?.getFullDetail()).toBe(
			"Reviewer 0 · completed\nReviewer 1 · completed",
		);
	});
	...
	it("summarizes a spawn card from the newest member report and the batch outcome", () => {
		...
		expect(
			agentSpawnSummary(
				direct([member("Agent 0", "declined", "", 1)]),
				"declined",
			),
		...
		expect(
			agentSpawnSummary(
				workflow([member("Reviewer", "completed", "", 3)]),
				"failed",
			),
			...
			agentSpawnSummary(
				{
					workflowId: "wf",
					agentTaskIds: ["wf"],
					agents: [member("review", undefined, "", 1)],
				},
				"inProgress",
			),
		).toMatchObject({
