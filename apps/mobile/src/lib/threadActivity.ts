...
function resolvePendingUserInputOptionValue(
	...
	return label.length > 0 &&
		question.options.some(
			(option) => option.value === undefined && option.label.trim() === label,
		)
		? label
		: null;
}
...
function deriveWorkLogEntries(
	...
	for (const activity of ordered) {
		if (activity.tone !== "error" && isWorktreeSetupActivity(activity.kind))
			continue;
...
function isNoContentRuntimeWarning(
	activity: OrchestrationThreadActivity,
): boolean {
	return (
...
function toDerivedWorkLogEntry(
	...
	const commandOutput = commandPreview.command
		? extractCommandOutputText(payload?.data)
		: null;
	const output = commandOutput
		? stripTrailingExitCode(commandOutput).output
		: null;
	...
	if (itemType === "mcp_tool_call") {
		const data = asRecord(payload?.data);
		const toolData =
			typeof data?.toolName === "string" ? (data.item ?? data) : data?.item;
...
function agentSpawnRow(
	...
	const coordinator =
		workflowId === null ? undefined : members[agentTaskIds.indexOf(workflowId)];
	const agents =
...
function agentSpawnLifecycleStatus(
	...
	if (
		statuses.some((status) => status === undefined || status === "inProgress")
	) {
		return "inProgress";
	}
...
function collapseDerivedWorkLogEntries(
	...
	const spawnToolCallIds = new Set(
		entries.flatMap((entry) =>
			entry.agentSpawnToolCallId !== undefined
	...
	for (const entry of entries) {
		...
		if (isTaskRow && entry.taskId !== undefined) {
			if (entry.isBackgroundTask) {
				...
				if (existingIndex !== undefined) {
					collapsed[existingIndex] = mergeDerivedWorkLogEntries(
			...
			const groupKey =
				spawnGroupByTaskId.get(entry.taskId) ?? agentSpawnGroupKey(entry);
			spawnGroupByTaskId.set(entry.taskId, groupKey);
			const existingIndex = spawnRowIndex.get(groupKey);
			if (existingIndex !== undefined) {
				collapsed[existingIndex] = mergeAgentSpawnEntries(
					collapsed[existingIndex]!,
					entry,
				);
				continue;
		...
		if (lifecycleKey !== undefined) {
			...
			const matchingEntry =
				matchingIndex === undefined ? undefined : collapsed[matchingIndex];
			if (
				matchingIndex !== undefined &&
				matchingEntry &&
				shouldCollapseToolLifecycleEntries(matchingEntry, entry)
			) {
				collapsed[matchingIndex] = mergeDerivedWorkLogEntries(
					matchingEntry,
					entry,
				);
				continue;
...
function toolLifecycleCollapseMapKey(
	entry: DerivedWorkLogEntry,
): string | undefined {
	if (
...
function buildWorkEntryExpandedBody(entry: WorkLogEntry): string | null {
	...
	const appendBlock = (value: string | null | undefined) => {
		...
		if (
			trimmed &&
			(entry.command || (trimmed !== visibleLabel && !blocks.includes(trimmed)))
		) {
			blocks.push(trimmed);
		}
	};
...
function workEntryCanExpand(entry: WorkLogEntry): boolean {
	...
	return Boolean(
		(entry.rawCommand ?? entry.command)?.trim() || entry.detail?.trim(),
	);
}
...
export function workEntryRowLabel(
	...
	const compactPreview =
		preview === null ? null : collapseWhitespace(stripShellWrapper(preview));
 ...
 * terminal state yet.
 */
export function agentSpawnLabel(
...
function agentSpawnMembers(spawn: NonNullable<WorkLogEntry["agentSpawn"]>) {
	return spawn.agents.filter(
		(_, index) => spawn.agentTaskIds[index] !== spawn.workflowId,
	);
}
	...
	if (tone === "working") {
		...
		const latest = working
			...
			.reduce<(typeof working)[number] | undefined>(
				(newest, member) =>
					newest === undefined || member.updatedAt > newest.updatedAt
						? member
						: newest,
				undefined,
			);
		const status =
			latest?.detail ??
			(members.length > 1
				? `${working.length} of ${members.length} working`
				: "Working");
...
function agentSpawnExpandedBody(
	spawn: NonNullable<WorkLogEntry["agentSpawn"]>,
): string | null {
...
const activityOrder = Order.combineAll<OrchestrationThreadActivity>([
	...
	Order.mapInput(Order.Number, (activity) =>
		compareActivityLifecycleRank(activity.kind),
	),
...
function groupAdjacentActivities(
	entries: ReadonlyArray<RawThreadFeedEntry>,
): ThreadFeedEntry[] {
	const grouped: ThreadFeedEntry[] = [];
	let firstActivityEntry: Extract<
		RawThreadFeedEntry,
		{ readonly type: "activity" }
	> | null = null;
	...
	const flushGroup = () => {
		...
		if (
			...
			cached.activities.every(
				(activity, index) => activity === openGroupActivities[index],
			)
		) {
			grouped.push(cached);
		} else {
...
function deriveUnsettledTurnId(
	...
	const settled =
		latestTurn.completedAt !== null && latestTurn.state !== "running";
...
function deriveThreadFeedTurnFolds(
	...
	for (const [turnId, group] of groupsByTurnId) {
		...
		if (
			entries.some(
				(entry) => entry.type === "message" && entry.message.streaming,
			)
		) {
			continue;
		}
		...
		const terminalAssistantMessageId =
			terminalAssistantMessageIdByTurn.get(turnId);
		...
		const firstHiddenEntry = entries.find((entry) =>
			hiddenEntryIds.has(entry.id),
		);
...
function appendPresentedFeedEntry(
	...
	entry: Exclude<
		ThreadFeedEntry,
		{ readonly type: "turn-fold" | "work-toggle" | "thinking" }
	>,
	...
	if (
		...
		cached.rows.some(
			(row) =>
				...
				(row.type === "agent-spawn" &&
					expandedWorkGroupIds.has(row.id) !== row.expanded),
		)
	) {
...
function appendToolGroupRows(
	...
	const primarySourceIcon = primarySourceKey
		? (activities.find(
			...
			primarySourceActivity?.workEntry.toolSource?.icon)
		: undefined;
	const groupToolSurface =
		primarySourceActivity?.workEntry.toolSurface ??
		latestActivity.workEntry.toolSurface ??
		activities.findLast(
			(activity) => activity.workEntry.toolSurface !== undefined,
		)?.workEntry.toolSurface;
	const groupToolIcon =
		primarySourceIcon ??
		latestActivity.workEntry.toolIcon ??
		activities.findLast((activity) => activity.workEntry.toolIcon !== undefined)
			?.workEntry.toolIcon;
	const summaryToolIcon = live
		...
		: singleActivity !== null &&
			  singleActivity.toolLike &&
			  toolGroupAction(singleActivity.workEntry) !== "edit"
			? resolveWorkEntryToolPresentation(singleActivity.workEntry, "completed")
					?.icon
			: undefined;
	result.push({
		type: "work-toggle",
		...
		id: shimmer
			...
			: `${live ? "work-live" : "work-toggle"}:${groupId}`,
		...
		summaryKind: toolGroupSummaryKind(
			(live ? [latestActivity] : activities).map(
				(activity) => activity.workEntry,
			),
		),
		...
		hasFailure:
			activities.findLast((activity) => activity.toolLike)?.status ===
			"failure",
		live,
		shimmer,
	});
...
export function setPendingUserInputCustomAnswer(
	...
	return {
		customAnswer,
		...(selectedOptionValues && selectedOptionValues.length > 0
...
export function isPendingUserInputOptionSelected(
	...
	const resolvedOptionValue = resolvePendingUserInputOptionValue(
		question,
		optionValue,
	);
	return (
...
export function togglePendingUserInputOptionSelection(
	...
	if (question.multiSelect) {
		...
		const nextSelectedOptionValues = selectedOptionValues.includes(
			resolvedOptionValue,
		)
