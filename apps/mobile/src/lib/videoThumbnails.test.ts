...
describe("video thumbnails", () => {
	it("reuses a frame for duplicate requests and refreshed signed URLs", async () => {
		...
		const refreshed = vi.fn(async () => ({
			...source(),
			uri: "https://host/new-token/clip.mp4",
		}));
		expect(
			await thumbnails.loadVideoThumbnail("env:clip", refreshed, signal),
		).toBe(frame);
	...
	it("serializes decoding and skips queued requests that scroll out of view", async () => {
		...
		const queued = thumbnails.loadVideoThumbnail(
			"removed",
			skipped,
			removed.signal,
		);
	...
	it("releases failed extractions and permits a later retry", async () => {
		const broken = player();
		broken.generateThumbnailsAsync.mockRejectedValue(
			new Error("Invalid video"),
		);
