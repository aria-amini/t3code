...
describe("hasWideMarkdownBlock", () => {
	it("ignores prose, inline code, and emphasis", () => {
		...
		expect(
			hasWideMarkdownBlock("I found it in `secteurs_intervention` earlier"),
		).toBe(false);
	...
	it("can limit ordered-list width pinning to Android", () => {
		...
		expect(
			hasWideMarkdownBlock(orderedList, { includeOrderedLists: true }),
		).toBe(true);
		expect(
			hasWideMarkdownBlock(orderedList, { includeOrderedLists: false }),
		).toBe(false);
		expect(
			hasWideMarkdownBlock("```\ncode\n```", { includeOrderedLists: false }),
		).toBe(true);
		expect(
			hasWideMarkdownBlock("| a | b |\n| --- | --- |", {
				includeOrderedLists: false,
			}),
		).toBe(true);
	});
	...
	it("detects blockquotes only when the native renderer needs width pinning", () => {
		expect(hasWideMarkdownBlock("> quoted", { includeBlockquotes: true })).toBe(
			true,
		);
		expect(
			hasWideMarkdownBlock("  > quoted", { includeBlockquotes: true }),
		).toBe(true);
		expect(hasWideMarkdownBlock("> quoted")).toBe(false);
		expect(
			hasWideMarkdownBlock("prose > quoted", { includeBlockquotes: true }),
		).toBe(false);
		expect(
			hasWideMarkdownBlock("    > indented code", { includeBlockquotes: true }),
		).toBe(true);
	});
