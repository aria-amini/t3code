...
export function NativeStackScreenOptions(props: {
	...
	const latestOptionFunctionsRef = useRef(
		new Map<string, (...args: unknown[]) => unknown>(),
	);
	const optionFunctionWrappersRef = useRef(
		new Map<string, (...args: unknown[]) => unknown>(),
	);
	const normalizedOptions = useMemo(
		() => normalizeScreenOptions(props.options),
		[props.options],
	);
	...
	useEffect(() => {
		...
		const subscriptions = Object.entries(props.listeners).map(
			([eventName, listener]) =>
...
type ToolbarElementProps = Record<string, unknown> & {
...
function NativeHeaderToolbarRoot(props: {
	...
	const navigation = useNativeStackNavigation();
	const items = useMemo(
		() => collectToolbarItems(props.children),
		[props.children],
	);
...
NativeHeaderToolbarSearchBarSlot.displayName =
