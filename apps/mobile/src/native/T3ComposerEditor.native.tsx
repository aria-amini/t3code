...
const NativeView =
	requireNativeView<NativeComposerEditorProps>(NATIVE_MODULE_NAME);
...
function fileIconUri(path: string): string {
	return Image.resolveAssetSource(
		markdownFileIconSource(resolveMarkdownFileIcon(path)),
	).uri;
}
...
export function ComposerEditor({
	...
	useImperativeHandle(
		...
		() => ({
			...
			setSelection: (nextSelection) =>
				void nativeRef.current?.setSelection(
					nextSelection.start,
					nextSelection.end,
				),
		}),
	...
	useEffect(() => {
		...
		if (
			isNativeEcho ||
			controlledEventCount !== mostRecentEventCountRef.current
		)
			return;
