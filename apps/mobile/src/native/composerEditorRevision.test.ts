...
describe("isComposerNativeEcho", () => {
	const snapshots = [
		{ eventCount: 3, value: "native", selection: { start: 6, end: 6 } },
	];

	it("matches the exact native text revision and selection", () => {
		expect(
			isComposerNativeEcho("native", { start: 6, end: 6 }, 3, snapshots),
		).toBe(true);
	});

	it("does not claim parent-driven selection or repeated-text updates", () => {
		expect(
			isComposerNativeEcho("native", { start: 2, end: 2 }, 3, snapshots),
		).toBe(false);
		expect(
			isComposerNativeEcho("native", { start: 6, end: 6 }, 4, snapshots),
		).toBe(false);
		expect(
			isComposerNativeEcho("parent edit", { start: 6, end: 6 }, 3, snapshots),
		).toBe(false);
	});
	...
	it("does not claim a controlled selection against an assumed state without one", () => {
		...
		expect(
			isComposerNativeEcho("native", { start: 0, end: 0 }, 3, assumed),
		).toBe(false);
		expect(
			isComposerNativeEcho("other", { start: 0, end: 0 }, 3, assumed),
		).toBe(false);
	});
...
describe("resolveComposerControlledEventCount", () => {
	...
	it("does not control selection when no selection prop is provided", () => {
		expect(resolveComposerControlledEventCount("ab", null, 4, snapshots)).toBe(
			4,
		);
	});
});
...
describe("pruneAcknowledgedComposerNativeEvents", () => {
	...
	it("retains native events that arrive after the acknowledged render", () => {
		...
		expect(pruneAcknowledgedComposerNativeEvents(snapshots, 40)).toEqual(
			snapshots,
		);
	});
	...
	it("retains the newest acknowledged snapshot so settled re-renders stay echoes", () => {
		...
		expect(isComposerNativeEcho("abc", { start: 3, end: 3 }, 42, pruned)).toBe(
			true,
		);
	});
	...
	it("keeps the newest of several snapshots sharing the acknowledged revision", () => {
		...
		expect(pruneAcknowledgedComposerNativeEvents(snapshots, 41)).toEqual([
			snapshots[1],
		]);
	});
});
...
describe("assumeComposerControlledState", () => {
	...
	it("applies a parent caret move on the assumed value at the assumed revision", () => {
		...
		expect(
			isComposerNativeEcho("typed", { start: 2, end: 2 }, 3, snapshots),
		).toBe(false);
		expect(
			resolveComposerControlledEventCount(
				"typed",
	...
	it("re-applies a parent value that round-trips back to an acknowledged state", () => {
		...
		expect(
			isComposerNativeEcho("typed", { start: 5, end: 5 }, 3, snapshots),
		).toBe(false);
		expect(
			resolveComposerControlledEventCount(
				"typed",
