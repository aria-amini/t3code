...
const mocks = vi.hoisted(() => ({
	...
	transcribe:
		vi.fn<
			(audio: ArrayBufferLike, locale: string) => Promise<TranscriptionResult>
		>(),
...
describe("getLocalVoiceTranscriber", () => {
	it("keeps the selected language and Apple's resolved locale when the device language changes", async () => {
		...
		await expect(
			prepared.transcribe("file:///voice.m4a", options),
		).resolves.toBe("Hej världen.");
		...
		async (phase) => {
			...
			const operation =
				phase === "prepare"
					...
					: (await transcriber.prepare(options)).transcribe(
							"file:///voice.m4a",
							options,
						);
