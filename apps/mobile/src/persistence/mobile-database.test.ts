...
describe("mobile database legacy cache migration", () => {
	it.effect("keeps acquisition failures typed on database operations", () =>
		Effect.scoped(
			Effect.gen(function* () {
				openDatabaseAsync.mockRejectedValueOnce(
	...
	it("maps legacy thread records to their SQLite identity", () => {
		...
		expect(
			decodeLegacyCacheRecord("connection-thread-snapshots", payload),
		).toEqual({
			environmentId: "environment-1",
			kind: "thread",
			cacheKey: "thread-1",
	...
	it("skips malformed legacy records", () => {
		expect(
			decodeLegacyCacheRecord("connection-vcs-refs", "{not-json"),
		).toBeNull();
		expect(
