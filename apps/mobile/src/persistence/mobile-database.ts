...
async function migrateLegacyFileCaches(
	...
	try {
		...
		const listFiles = (
			...
			directory
				.list()
				.flatMap((entry) =>
					entry instanceof File ? [entry] : listFiles(entry),
				);
		...
		for (const directoryName of LEGACY_CACHE_DIRECTORIES) {
			try {
             ...
             ON CONFLICT (environment_id, kind, cache_key) DO NOTHING`,
				...
				complete = false;
				console.warn(
					`[mobile-database] could not migrate legacy cache ${directoryName}`,
					cause,
				);
			}
		}
		...
		console.warn(
			"[mobile-database] could not load legacy cache migration",
			cause,
		);
		return false;
	}
...
const makeAvailable = Effect.gen(function* () {
	const database = yield* Effect.acquireRelease(
		...
		(openDatabase) =>
			Effect.promise(() => openDatabase.closeAsync()).pipe(Effect.ignore),
	);
	...
	yield* Effect.tryPromise({
		try: async () => {
			...
			const schema = await database.getFirstAsync<{
	...
	return MobileDatabase.of({
		loadCache: Effect.fn("MobileDatabase.loadCache")(
		...
		removeCache: Effect.fn("MobileDatabase.removeCache")(
			(environmentId, kind, cacheKey) =>
				Effect.tryPromise({
					try: () =>
						database.runAsync(
		...
		clearCacheKind: Effect.fn("MobileDatabase.clearCacheKind")(
			(environmentId, kind) =>
				Effect.tryPromise({
					try: () =>
						database.runAsync(
		...
		clearEnvironmentCache: Effect.fn("MobileDatabase.clearEnvironmentCache")(
			(environmentId) =>
				Effect.tryPromise({
					try: () =>
						database.runAsync(
							"DELETE FROM client_cache WHERE environment_id = ?",
							environmentId,
						),
		...
		savePreferencesJson: Effect.fn("MobileDatabase.savePreferencesJson")(
			(payload, updatedAt) =>
				Effect.tryPromise({
					try: () =>
						database.runAsync(
...
function makeUnavailable(
	error: MobileDatabaseError,
): MobileDatabase["Service"] {
...
export const make = Effect.result(makeAvailable).pipe(
	Effect.map((result) =>
		result._tag === "Success"
			? result.success
			: makeUnavailable(result.failure),
	),
);
