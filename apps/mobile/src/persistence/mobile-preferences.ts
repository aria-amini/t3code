...
function sanitizePreferences(parsed: Preferences): Preferences {
	...
	if (Array.isArray(parsed.connectOnboardingOptOutAccounts)) {
		preferences.connectOnboardingOptOutAccounts =
			parsed.connectOnboardingOptOutAccounts.filter(
...
export const make = Effect.fn("MobilePreferencesStore.make")(function* () {
	...
	const saveJson = Effect.fn("MobilePreferencesStore.saveJson")(function* (
		...
		const databaseResult = yield* Effect.result(
			database.savePreferencesJson(payload, timestamp),
		);
		if (databaseResult._tag === "Failure") {
			yield* Effect.logWarning(
				"Database unavailable; saving preferences to secure storage.",
			).pipe(Effect.annotateLogs({ cause: databaseResult.failure }));
		...
		yield* secureStorage
			...
			.pipe(
				Effect.catch((error) =>
					Effect.logWarning(
						"Could not remove the mobile preferences fallback.",
	...
	const loadUnlocked = Effect.gen(function* () {
		...
		if (databaseResult._tag === "Failure") {
			yield* Effect.logWarning(
				"Database unavailable; loading fallback preferences.",
			).pipe(Effect.annotateLogs({ cause: databaseResult.failure }));
		}

		const fallbackResult = yield* Effect.result(
			secureStorage.getItem(PREFERENCES_FALLBACK_KEY),
		);
		...
		if (fallbackResult._tag === "Success") {
			...
			yield* Effect.logWarning(
				"Could not inspect the mobile preferences fallback.",
			).pipe(Effect.annotateLogs({ error: fallbackResult.failure }));
		}
		...
		const fallbackIsNewer =
			...
			(storedPreferences === null ||
				(Option.isSome(storedJson) &&
					fallback.updatedAt > storedJson.value.updatedAt));

		let parsed: Preferences | null = null;
		if (fallbackIsNewer) {
			parsed = fallback.preferences;
			yield* Ref.update(lastUpdatedAt, (last) =>
				Math.max(last, fallback.updatedAt),
			);
			...
			yield* Ref.update(lastUpdatedAt, (last) =>
				Math.max(last, storedJson.value.updatedAt),
			);
			if (fallbackJson !== null) {
				yield* secureStorage
					...
					.pipe(
						Effect.catch((error) =>
							Effect.logWarning(
								"Could not remove a stale mobile preferences fallback.",
							).pipe(Effect.annotateLogs({ error })),
						),
					);
			}
	...
	const load = lock
		...
		.pipe(
			Effect.mapError((cause) => new MobilePreferencesLoadError({ cause })),
		);
