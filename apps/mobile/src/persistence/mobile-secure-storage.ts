...
const MobileSecureStorageOperation = Schema.Literals([
	"read",
	"write",
	"delete",
]);
...
export class MobileSecureStorage extends Context.Service<
	MobileSecureStorage,
	{
		readonly getItem: (
			key: string,
		) => Effect.Effect<string | null, MobileSecureStorageError>;
		readonly setItem: (
			key: string,
			value: string,
		) => Effect.Effect<void, MobileSecureStorageError>;
		readonly removeItem: (
			key: string,
		) => Effect.Effect<void, MobileSecureStorageError>;
	}
	...
	getItem: Effect.fn("MobileSecureStorage.getItem")((key) =>
		Effect.tryPromise({
			try: () => SecureStore.getItemAsync(key),
				...
				new MobileSecureStorageError({ operation: "read", key, cause }),
		}),
	),
	setItem: Effect.fn("MobileSecureStorage.setItem")((key, value) =>
		Effect.tryPromise({
			try: () => SecureStore.setItemAsync(key, value),
				...
				new MobileSecureStorageError({ operation: "write", key, cause }),
		}),
	),
	removeItem: Effect.fn("MobileSecureStorage.removeItem")((key) =>
		Effect.tryPromise({
			try: () => SecureStore.deleteItemAsync(key),
				...
				new MobileSecureStorageError({ operation: "delete", key, cause }),
		}),
	),
});
