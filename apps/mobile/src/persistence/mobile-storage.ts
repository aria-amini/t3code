...
export const make = Effect.fn("MobileStorage.make")(function* () {
	...
	const saveConnection = Effect.fn("MobileStorage.saveConnection")(function* (
		...
		const next = current.some(
			(entry) => entry.environmentId === connection.environmentId,
		)
			? pipe(
					current,
					Arr.map((entry) =>
						entry.environmentId === connection.environmentId
							? stableConnection
							: entry,
					),
				)
	...
	const clearSavedConnection = Effect.fn("MobileStorage.clearSavedConnection")(
	...
	const loadOrCreateAgentAwarenessDeviceId = Effect.gen(function* () {
		const existing = yield* secureStorage.getItem(
	...
	const loadAgentAwarenessRegistrationRecord =
		readJson<AgentAwarenessRegistrationRecord>(
			...
			Effect.map((parsed) => {
				...
				return {
					...
					...(typeof parsed.pushToStartToken === "string" &&
					parsed.pushToStartToken
	...
	return MobileStorage.of({
		...
		saveRecentThreadShortcuts: (threads) =>
			writeJson(RECENT_THREAD_SHORTCUTS_KEY, { threads }),
	});
});
