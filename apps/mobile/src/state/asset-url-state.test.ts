...
describe("deriveAssetUrlState", () => {
	it("passes a resolved URL through on a live environment", () => {
		expect(
			deriveAssetUrlState({ connectionPhase: "connected", shared: SUCCESS }),
		).toEqual(SUCCESS);
	});
	...
	it("waits while the query is pending and the environment is still reachable", () => {
		for (const connectionPhase of [
			...
			expect(
				deriveAssetUrlState({ connectionPhase, shared: { _tag: "Loading" } }),
			).toEqual({
				_tag: "Loading",
			});
		}
	...
	it("reports disconnected when the query failed while the environment is down", () => {
		for (const connectionPhase of [
			...
			expect(
				deriveAssetUrlState({ connectionPhase, shared: { _tag: "Failure" } }),
			).toEqual({
				_tag: "Failure",
				reason: "disconnected",
			});
