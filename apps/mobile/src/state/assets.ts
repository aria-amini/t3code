...
const EMPTY_CONNECTION_STATE_ATOM = Atom.make(
	AsyncResult.initial<never, never>(false),
).pipe(Atom.withLabel("mobile-asset-connection-state:empty"));
...
export function useAssetUrlState(
	...
	return deriveAssetUrlState({
		...
		shared:
			shared._tag === "Failure" && result.waiting
				? { _tag: "Loading" }
				: shared,
	});
}
...
export function useRefreshAssetUrl(
	...
	return useCallback(async () => {
		if (environmentId === null || resource === null || httpBaseUrl === null)
			return null;
