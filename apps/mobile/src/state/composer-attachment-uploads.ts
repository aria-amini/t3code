...
let uploadQueue: ReturnType<typeof createComposerAttachmentUploadQueue> | null =
	null;
...
export function useComposerAttachmentUploadState(
	...
	return useAtomValue(
		uploadStateAtom(
			environmentId
				? composerAttachmentUploadKey(environmentId, attachmentId)
				: "",
		),
	);
}
...
export function useComposerAttachmentUploadWorker() {
	...
	const queueRef = useRef<ReturnType<
		typeof createComposerAttachmentUploadQueue
	> | null>(null);
	...
	useEffect(() => {
		...
		const queue = createComposerAttachmentUploadQueue({
			onChange: (states) =>
				appAtomRegistry.set(composerAttachmentUploadsAtom, states),
					...
					const result = await prepareTurnAttachments({
						...
						persistUploadedReferences: async ([uploaded]) => {
							...
							for (const [key, draft] of Object.entries(
								...
								if (
									composerDraftEnvironmentId(key, queued, draft) ===
										environmentId &&
									...
									retained =
										setComposerDraftAttachmentUpload(key, uploaded) || retained;
								}
							}
