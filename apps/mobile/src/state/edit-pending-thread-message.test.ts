...
vi.mock("./atom-registry", () => ({
	appAtomRegistry: {
		get: (atom: string) =>
			atom === "dispatching" ? state.dispatching : state.held,
	},
}));
...
vi.mock("./use-composer-drafts", () => ({
	...
	undoComposerDraftMerge: async (
		_key: string,
		snapshot: typeof state.draft,
	) => {
...
describe("editing a pending message", () => {
	...
	it("rolls back the draft if removing the queued message fails", async () => {
		...
		await expect(editPendingThreadMessage(message)).rejects.toThrow(
			"disk error",
		);
