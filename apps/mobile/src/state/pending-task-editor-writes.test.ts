...
function queuedMessageText(
	...
	return (
		messages.find((message) => message.messageId === messageId)?.text ?? null
	);
}
...
describe("pending task editor writes", () => {
	it("chains a reopened editor that closes before the previous save finishes", async () => {
		...
		const secondEdit = queuedMessage(
			"message-close-before-save",
			"second edit",
		);
		...
		const firstBaseline = capturePendingTaskEditorWriteBaseline(
			original.messageId,
		);
		...
		const secondBaseline = capturePendingTaskEditorWriteBaseline(
			original.messageId,
		);
		...
		await expect(secondWriteGate.started).resolves.toMatchObject({
	...
	it("keeps a captured predecessor after that predecessor finishes", async () => {
		...
		const firstEdit = queuedMessage(
			"message-finished-predecessor",
			"first edit",
		);
		const secondEdit = queuedMessage(
			"message-finished-predecessor",
	...
	it("chains three rapid editor saves in order", async () => {
		...
		const thirdBaseline = capturePendingTaskEditorWriteBaseline(
			original.messageId,
		);
	...
	it("keeps the handed-off revision when the middle editor write fails", async () => {
		...
		const failedBaseline = capturePendingTaskEditorWriteBaseline(
			original.messageId,
		);
		...
		const finalBaseline = capturePendingTaskEditorWriteBaseline(
			original.messageId,
		);
		...
		await expect(failedSave).rejects.toMatchObject({
			_tag: "ThreadOutboxManagerError",
		});
	...
	it("does not overwrite an unrelated update accepted after capture", async () => {
		...
		const unrelatedEdit = queuedMessage(
		...
		const editorBaseline = capturePendingTaskEditorWriteBaseline(
			original.messageId,
		);
		const revision = harness.manager.revisionOf(original.messageId);
		await expect(harness.manager.update(unrelatedEdit, revision)).resolves.toBe(
			true,
		);
	...
	it("lets a later editor retry after its predecessor write fails", async () => {
		...
		const retryBaseline = capturePendingTaskEditorWriteBaseline(
			original.messageId,
		);
