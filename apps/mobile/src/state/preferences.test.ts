...
describe("mobile preferences state", () => {
	it.effect("shares one preference load across consumers", () =>
		Effect.gen(function* () {
			const load = vi.fn(() =>
				Promise.resolve<Preferences>({ baseFontSize: 17 }),
			);
	...
	it.effect(
		"preserves an optimistic patch when the initial load finishes later",
		() =>
			Effect.gen(function* () {
				const pendingLoad = deferred<Preferences>();
				const savePatch = vi.fn((patch: Partial<Preferences>) =>
				...
				const preferences = yield* AtomRegistry.getResult(
					registry,
					state.preferencesAtom,
					{
						suspendOnWaiting: true,
					},
				);
				...
				expect(
					AsyncResult.isFailure(registry.get(state.updatePreferencesAtom)),
				).toBe(false);
	...
	it.effect(
		"falls back to empty preferences when secure storage cannot be read",
		() =>
			Effect.gen(function* () {
				const state = makePreferencesState({
					load: Effect.fail(
	...
	it.effect(
		"does not roll back a newer optimistic write with the same value",
		() =>
			Effect.gen(function* () {
				let saveCount = 0;
				const state = makePreferencesState({
				...
				yield* Effect.promise(() =>
					vi.waitFor(() => {
						...
						expect(
							Option.getOrThrow(
								AsyncResult.value(registry.get(state.preferencesAtom)),
							),
						).toEqual({
							baseFontSize: 18,
							codeFontSize: 15,
	...
	it.effect(
		"rolls back to the last confirmed value after a later save fails",
		() =>
			Effect.gen(function* () {
				let saveCount = 0;
				const state = makePreferencesState({
				...
				yield* Effect.promise(() =>
					vi.waitFor(() => {
						...
						expect(registry.get(state.updatePreferencesAtom).waiting).toBe(
							false,
						);
						expect(
							Option.getOrThrow(
