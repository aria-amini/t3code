...
export function createMobilePreferencesState(
	runtime: Atom.AtomRuntime<MobilePreferencesStore>,
) {
	...
	const optimisticPatchAtom = Atom.make<OptimisticPreferences>({
		values: {},
		versions: {},
	}).pipe(
		Atom.keepAlive,
				...
				return MobilePreferencesStore.pipe(
					...
					Effect.tap((saved) =>
						Effect.sync(() => {
							...
							const optimistic = get(optimisticPatchAtom);
							const values = { ...optimistic.values } as Record<
								string,
								unknown
							>;
							const currentVersions = { ...optimistic.versions } as Record<
								string,
								unknown
							>;
							...
							get.set(optimisticPatchAtom, {
								...
								versions: currentVersions as Partial<
									Record<keyof Preferences, number>
								>,
							});
						}),
					),
...
export const mobilePreferencesState = createMobilePreferencesState(
	mobilePreferencesRuntime,
);

export const mobilePreferencesAtom = mobilePreferencesState.preferencesAtom;
export const updateMobilePreferencesAtom =
	mobilePreferencesState.updatePreferencesAtom;
