...
describe("remote environment projections", () => {
	...
	it("preserves saved identity across config-only updates and refreshes runtime state", () => {
		...
		const initialPresentation = harness.registry.get(
			harness.presentationAtom(ENVIRONMENT_ID),
		);
		...
		harness.registry.set(
			...
			initialPresentation === null
				? null
				: { ...initialPresentation, serverConfig: config },
		);
	...
	it("keeps missing environments null", () => {
		...
		expect(
			harness.registry.get(
				harness.projections.savedConnectionAtom(ENVIRONMENT_ID),
			),
		).toBeNull();
		expect(
			harness.registry.get(
				harness.projections.runtimeStateAtom(ENVIRONMENT_ID),
			),
		).toBeNull();
	});
});
