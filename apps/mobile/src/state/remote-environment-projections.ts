...
export function createRemoteEnvironmentProjectionAtoms(input: {
	...
	const savedConnectionAtom = Atom.family((environmentId: EnvironmentId) => {
		...
		return Atom.make((get) => {
			...
			const prepared = Option.getOrNull(
				get(input.preparedConnectionAtom(environmentId)),
			);
			if (
				previous !== null &&
			...
			previous = {
				...
				wsBaseUrl,
				bearerToken:
					authorization?._tag === "Bearer" ? authorization.token : null,
				...(relayManaged
					? {
	...
	const runtimeStateAtom = Atom.family((environmentId: EnvironmentId) => {
		...
		return Atom.make((get) => {
			...
			Atom.withLabel(`mobile:environment-runtime-state:${environmentId}`),
		);
	});
