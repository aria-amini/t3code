...
vi.mock("./threads", async () => {
	...
	return {
		environmentThreadShells: {
			threadShellsAtom: Atom.make([]).pipe(Atom.keepAlive),
		},
	};
});
...
vi.mock("./use-thread-outbox", async () => {
	const { Atom } = await import("effect/unstable/reactivity");
	return {
		queuedThreadKeysAtom: Atom.make(new Set<string>()).pipe(Atom.keepAlive),
	};
});
...
function fixture() {
	...
	const upsert = (id: string, key: string) => {
		...
		appAtomRegistry.set(
			shellsAtom,
			current.map((row) =>
				row.id === id ? { ...row, activeOrderKey: key } : row,
			),
		);
	};
...
describe("shared mobile pending move", () => {
	...
	it.each(["failure", "interruption"])(
		"releases a %s without restoring old canonical keys",
		() => {
			const { start, upsert } = fixture();
			const move = start();
			upsert("b", "aa");
