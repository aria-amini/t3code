...
export const pendingThreadOrderAtom = Atom.make<PendingThreadOrder | null>(
	null,
).pipe(Atom.keepAlive);

export function usePendingThreadOrder(
	nowMinute: string,
...
export function beginPendingThreadOrder(pending: PendingThreadOrder) {
	...
	const refresh = () => {
		...
		if (next === null) cancel();
			...
			appAtomRegistry.set(pendingThreadOrderAtom, next);
	};
	...
	return {
		...
		complete: () => {
			...
			const current = appAtomRegistry.get(pendingThreadOrderAtom);
			if (current !== null) {
				appAtomRegistry.set(pendingThreadOrderAtom, {
					...current,
