		...
		loadPromise = serialize(async () => {
			...
			const recovered = result.messages.filter(
				(message) =>
					...
					!revisions.has(message.messageId),
			);
			...
			if (result.errors.length > 0) {
				throw new AggregateError(
					result.errors,
					"Some queued messages could not be read.",
				);
			}
			return true;
		}).catch((cause) => {
	...
	const enqueue = (message: QueuedThreadMessage): Promise<void> => {
		...
		return serialize(async () => {
			try {
				...
				setMessages(
					currentMessages().filter((candidate) => candidate !== message),
				);
	...
	const clearEnvironment = (
		...
		return serialize(async () => {
			...
			const candidateRevisions = new Map(
				candidates.map(
					(message) =>
						[message.messageId, revisions.get(message.messageId) ?? 0] as const,
				),
			);
			...
			await Promise.all(
				candidates.map(async (message) => {
					if (
						...
						(revisions.get(message.messageId) ?? 0) ===
							candidateRevisions.get(message.messageId)
					) {
						return;
					}
			...
			const removedMessageIds = new Set(
				removed.map((message) => message.messageId),
			);
