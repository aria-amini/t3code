...
const decodeStoredQueuedThreadMessage = Schema.decodeUnknownSync(
	QueuedThreadMessageSchema,
);
const encodeStoredQueuedThreadMessage = Schema.encodeUnknownSync(
	QueuedThreadMessageSchema,
);
...
export function modelSelectionsEqual(
	left: ModelSelectionType,
	right: ModelSelectionType,
): boolean {
	return (
		left.instanceId === right.instanceId &&
		left.model === right.model &&
		JSON.stringify(left.options ?? null) ===
			JSON.stringify(right.options ?? null)
	);
}

export function encodeQueuedThreadMessage(
	message: QueuedThreadMessage,
): unknown {
...
export function decodeQueuedThreadMessage(value: unknown): QueuedThreadMessage {
	const { schemaVersion: _, ...message } =
		decodeStoredQueuedThreadMessage(value);
	return message;
}
...
export function threadOutboxRetryDelayMs(attempt: number): number {
	return Math.min(
		...
		THREAD_OUTBOX_MAX_RETRY_DELAY_MS,
	);
}
...
export function resolveThreadOutboxDeliveryAction(input: {
	...
	if (input.isCreation) {
		...
		return input.environmentConnected && input.shellStatus === "live"
			? "send"
			: "wait";
	}
	if (!input.threadExists) {
...
export function resolveThreadOutboxDispatchStep(input: {
	...
	readonly serverConfig: {
		readonly maxFileUploadBytes: number | undefined;
	} | null;
