...
const harness = vi.hoisted(() => ({
	cleanup: vi.fn(),
	clearDraft:
		vi.fn<typeof import("./use-composer-drafts").clearComposerDraft>(),
	flushDrafts: vi.fn<
		typeof import("./use-composer-drafts").flushComposerDrafts
	>(async () => {}),
	waitForDrafts: vi.fn<
		typeof import("./use-composer-drafts").waitForComposerDraftsLoaded
	>(async () => {}),
...
describe("thread outbox removal", () => {
	...
	it("keeps an edited message and its files when a revision-checked removal loses", async () => {
		...
		await expect(removeThreadOutboxMessage(message, revision)).resolves.toBe(
			false,
		);
	...
	it("clears only removed pending drafts and keeps drafts for live messages", async () => {
		...
		const replacement = {
			...replaced,
			text: "replacement queued while drafts hydrate",
		};
		const editorOnlyFile = {
		...
		appAtomRegistry.set(composerDraftsAtom, {
			...
			[`pending-task:${replaced.messageId}`]: {
				text: "replacement",
				attachments: [],
			},
			[`pending-task:${kept.messageId}`]: {
				text: "other environment",
				attachments: [],
			},
		});
		await Promise.all([
		...
		expect(harness.clearDraft).toHaveBeenCalledExactlyOnceWith(
			clearedDraftKey,
			{
				deferAttachmentCleanup: true,
			},
	...
	it("keeps removal successful when pending draft persistence fails", async () => {
		...
		appAtomRegistry.set(composerDraftsAtom, {
			[`pending-task:${message.messageId}`]: {
				text: "edited",
				attachments: [],
			},
		});
