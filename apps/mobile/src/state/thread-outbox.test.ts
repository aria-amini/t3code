...
describe("thread outbox", () => {
	it.each(["read", "json", "schema"] as const)(
		...
		async (failure) => {
			...
			outboxFiles.set(
				"message-1.json",
				JSON.stringify(encodeQueuedThreadMessage(first)),
			);
			...
			await expect(manager.load()).resolves.toBe(false);
			expect(registry.get(manager.queuedMessagesByThreadKeyAtom)).toBe(
				beforeRetry,
			);
			await expect(
				manager.clearEnvironment(first.environmentId),
			...
			outboxFiles.set(
				"message-2.json",
				JSON.stringify(encodeQueuedThreadMessage(second)),
			);
	...
	it("preserves queued messages when environment cleanup cannot read the outbox", async () => {
		...
		await expect(
			manager.clearEnvironment(message.environmentId),
		).rejects.toMatchObject({
			operation: "clear-environment-load",
		});
	...
	it("keeps in-session edits and removals when an incomplete load is retried", async () => {
		...
		const edited = {
			...message,
			text: "Accepted while storage was being read",
		};
	...
	it("reads file-backed images from v4 queued messages", () => {
		...
		expect(decodeQueuedThreadMessage({ ...message, schemaVersion: 4 })).toEqual(
			message,
		);
	});
	...
	it("persists the exact selector snapshot while remaining compatible with v1 messages", () => {
		...
		expect(
			decodeQueuedThreadMessage(encodeQueuedThreadMessage(selectedMessage)),
		).toEqual(selectedMessage);
		expect(
	...
	it("drops the disk entry when a failed enqueue leaves no queued message behind", async () => {
		...
		await expect(manager.enqueue(message)).rejects.toBeInstanceOf(
			ThreadOutboxManagerError,
		);
	...
	it("updates a queued message in place but never resurrects a removed one", async () => {
		...
		await manager.remove(edited);
		await expect(
			manager.update({ ...message, text: "stale flush" }),
		).resolves.toBe(false);
		expect(registry.get(manager.queuedMessagesByThreadKeyAtom)).toEqual({});
	...
	it("rejects a stale revision before its payload reaches durable storage", async () => {
		...
		await expect(
			manager.update({ ...original, text: "stale upload" }, revision),
		).resolves.toBe(false);
	...
	it("refuses to remove a message that was rewritten after the removal decision", async () => {
		...
		await expect(
			manager.remove(edited, manager.revisionOf(edited.messageId)),
	...
	it("restores a message when its live removal predicate changes during storage removal", async () => {
		...
		const removal = manager.remove(
			message,
			manager.revisionOf(message.messageId),
			() => canRemove,
		);
		await removeStarted.promise;
		canRemove = false;
	...
	it("preserves concurrent enqueues while clearing an environment", async () => {
		...
		const manager = createThreadOutboxManager({
			...
			storage: {
				...
				remove: async (message) => {
					if (
						message.environmentId === EnvironmentId.make("environment-clear")
					) {
						removeStarted.resolve();
						await removeBarrier.promise;
					}
		...
		expect([...stored.values()]).toEqual(
			expect.arrayContaining([replacement, added, kept]),
		);
		registry.dispose();
	});
	...
	it("keeps an enqueue published while an environment clear waits to start", async () => {
		...
		const clearing = manager.clearEnvironment(
			EnvironmentId.make("environment-clear"),
		);
		const added = queuedMessage({
			environmentId: "environment-clear",
	...
	it("gates a sending file message on the server's file support and limit", () => {
		expect(
			...
			step: "restore",
			reason: "This server does not support file attachments.",
		});
		expect(
			resolveThreadOutboxDispatchStep({
				deliveryAction: "send",
			...
			step: "restore",
			reason: "'big.zip' exceeds the 1 MB attachment limit.",
		});
		expect(
			resolveThreadOutboxDispatchStep({
				deliveryAction: "send",
	...
	it("round-trips queued creations and gates incomplete ones from sending", () => {
		...
		expect(
			decodeQueuedThreadMessage(encodeQueuedThreadMessage(creationMessage)),
		).toEqual(creationMessage);
	...
	it("retries transport failures but drops deterministic command failures", () => {
		expect(
			shouldRetryThreadOutboxDelivery(new Error("Socket is not connected")),
		).toBe(true);
		expect(
			shouldRetryThreadOutboxDelivery({
		...
		expect(
			shouldRetryThreadOutboxDelivery(new Error("Thread no longer exists")),
		).toBe(false);
		expect(
			shouldRetryThreadOutboxDelivery(
	...
	it("retries every in-flight transport failure by tag, not by message text", () => {
		const socketReasons = [
			new Socket.SocketReadError({
				cause: new Error("The network connection was lost."),
			}),
		...
		expect(
			shouldRetryThreadOutboxDelivery(
				new EnvironmentNotRegisteredError({
