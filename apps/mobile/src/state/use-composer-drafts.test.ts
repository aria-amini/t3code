...
const incomingShareStorageMocks = vi.hoisted(() => ({
	load: vi.fn<
		typeof import("../features/sharing/incoming-share-storage").loadIncomingShareDrafts
	>(async () => []),
}));
...
vi.mock("@t3tools/client-runtime/state/runtime", () => ({
	...
	squashAtomCommandFailure: (result: { readonly error: unknown }) =>
		result.error,
}));

vi.mock("../lib/attachmentUpload", () => ({
	releasePendingAttachmentUploads:
		composerAttachmentCleanupMocks.releaseUploads,
}));
...
describe("mobile composer drafts", () => {
	...
	it("releases videos rejected by the live draft limit and keeps accepted files", async () => {
		...
		expect(
			composerAttachmentCleanupMocks.remove,
		).toHaveBeenCalledExactlyOnceWith(makeAttachment("incoming-2").fileUri);
		...
		expect(
			appAtomRegistry.get(composerDraftsAtom)[draftKey]?.attachments,
		).toHaveLength(9);
	});
	...
	it("retains a referenced file-backed image and releases it once unreferenced", async () => {
		...
		expect(
			composerAttachmentCleanupMocks.remove,
		).toHaveBeenCalledExactlyOnceWith(image.fileUri);
	});

	it("keeps an image through an inline read after its draft is removed", async () => {
		const { prepareTurnAttachments } = await vi.importActual<
			typeof import("../lib/attachmentUpload")
		>("../lib/attachmentUpload");
		const image = {
			id: "image-reading",
			type: "image" as const,
			name: "photo.png",
		...
		composerDraftFileMocks.readImage.mockImplementation(async () => {
			...
			await read.promise;
			if (!imageExists)
				throw new Error("The image was deleted during its read");
			return "YWJj";
		});
		composerAttachmentCleanupMocks.remove.mockImplementationOnce(async () => {
			imageExists = false;
		...
		clearComposerDraftContent("environment-1:thread-1", {
	...
	it.each(["file", "image"] as const)(
		...
		async (type) => {
			...
			appAtomRegistry.set(composerDraftsAtom, {
				...
				"pending-task:queued-1": {
					text: "Edited queued task",
					attachments: [file],
				},
			});
			appAtomRegistry.set(threadOutboxManager.queuedMessagesByThreadKeyAtom, {
	...
	it("fails sign-out preservation before cleanup if a durable backup cannot be written", async () => {
		...
		expect(
			appAtomRegistry.get(composerCloudDraftsAtom).signedOut["account-a"]
				?.drafts["environment-1:thread-1"],
		).toEqual(DRAFT);
		expect(composerAttachmentCleanupMocks.remove).not.toHaveBeenCalled();
		composerDraftFileMocks.setWriteError(null);
		await archiveCloudComposerDrafts(
			null,
			new Set([EnvironmentId.make("environment-1")]),
		);
		expect(
			decodePersistedComposerState(
				JSON.parse(composerDraftFileMocks.getDocument()),
			).cloudDrafts.signedOut["account-a"]?.drafts["environment-1:thread-1"],
		).toEqual(DRAFT);
	});
	...
	it.each(["file", "image"] as const)(
		...
		async (type) => {
			...
			const releaseShareCopy =
				retainComposerAttachmentFileForPreview(currentFile);
			...
			expect(composerAttachmentCleanupMocks.remove.mock.calls).toEqual([
				[currentFile.fileUri],
			]);
		},
	);
	...
	it("completes local cleanup when pending upload deletion fails", async () => {
		...
		await expect(
			releaseUnusedComposerAttachmentFiles([file]),
	...
	it("does not delete files when incoming share ownership cannot be loaded", async () => {
		...
		const warning = vi
			.spyOn(console, "warn")
			.mockImplementation(() => undefined);
		incomingShareStorageMocks.load.mockRejectedValueOnce(
	...
	it("migrates archived signed-out new-task drafts the same way as live ones", () => {
		...
		const archived = Object.entries(
			decoded.cloudDrafts.signedOut["account-1"]?.drafts ?? {},
		);
	...
	it("migrates project-keyed new-task drafts to id keys with the project stamped in", () => {
		...
		expect(migrateLegacyNewTaskDraft("new-task:some-id", stamped, now)).toEqual(
			["new-task:some-id", stamped],
		);
		expect(
			migrateLegacyNewTaskDraft("environment-1:thread-1", DRAFT, now),
		).toEqual(["environment-1:thread-1", DRAFT]);
		expect(
			migrateLegacyNewTaskDraft("pending-task:message-1", DRAFT, now),
		).toEqual(["pending-task:message-1", DRAFT]);
	});
	...
	it("retargets a new-task draft to another project without losing its text", () => {
		...
		expect(
			findNewTaskDraftKeys(appAtomRegistry.get(composerDraftsAtom), from),
		).toEqual([]);
		expect(
			findNewTaskDraftKeys(appAtomRegistry.get(composerDraftsAtom), to),
		).toEqual([key]);
	});
	...
	it.each(["read", "decode"] as const)(
		...
		async (failure) => {
			...
			composerDraftFileMocks.setDocument({
				...
				drafts: {
					"environment-1:saved": { text: "Saved draft", attachments: [file] },
				},
			});
			...
			await expect(flushComposerDrafts()).rejects.toMatchObject({
				operation: failure,
			});
	...
	it("serializes environment cleanup after an older queued write", async () => {
		...
		const clear = clearComposerDraftsEnvironment(
	...
	it("merges shared content into a project draft without duplicating retries", () => {
		...
		expect(mergeComposerDraftContentState(merged, draftKey, content)).toBe(
			merged,
		);
		...
		const edited = {
			...
			[draftKey]: {
				...merged[draftKey]!,
				text: "User edited the imported context",
			},
		};
		expect(mergeComposerDraftContentState(edited, draftKey, content)).toBe(
			edited,
		);
	});
	...
	it("propagates a flush write failure instead of resolving as saved", async () => {
		...
		try {
			await expect(flushComposerDrafts()).rejects.toBeInstanceOf(
				ComposerDraftPersistenceError,
			);
		} finally {
			composerDraftFileMocks.setWriteError(null);
	...
	it("keeps text appended after a merge when rolling it back", () => {
		...
		const retried = mergeComposerDraftContentState(
			rolledBack,
			draftKey,
			content,
		);
	...
	it("keeps attachment files and uploads after a recovered message leaves an incomplete outbox", async () => {
		...
		const { removeThreadOutboxMessage } =
			await import("./thread-outbox-removal");
