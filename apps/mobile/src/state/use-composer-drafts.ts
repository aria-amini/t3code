...
export const composerDraftsAtom = Atom.make<Record<string, ComposerDraft>>(
	{},
).pipe(Atom.keepAlive, Atom.withLabel("mobile:composer-drafts"));

export const stickyComposerModelSelectionAtom =
	Atom.make<ModelSelection | null>(null).pipe(
		Atom.keepAlive,
...
export function migrateLegacyNewTaskDraft(
	...
	const legacy =
		draft.project === undefined ? parseLegacyNewTaskDraftKey(key) : null;
	if (legacy === null) {
		return [key, draft];
	}
	return [
...
async function writePersistedComposerState(
	...
	try {
		...
		const document = {
			...
			...(cloudDrafts.accountId
				? { cloudAccountId: cloudDrafts.accountId }
				: {}),
				...
				? {
						signedOutDrafts: Object.fromEntries(
							Object.entries(cloudDrafts.signedOut).map(([id, saved]) => [
								...
								{
									drafts: saved.drafts,
									queuedMessages: saved.queuedMessages.map(
										encodeQueuedThreadMessage,
									),
								},
							]),
...
function signedOutAttachmentOwners() {
	return Object.values(
		appAtomRegistry.get(composerCloudDraftsAtom).signedOut,
	).flatMap((saved) => [
...
function isComposerAttachmentFileReferenced(fileUri: string): boolean {
	...
	return [...drafts, ...queuedMessages, ...signedOutAttachmentOwners()].some(
		(owner) =>
...
export async function releaseUnusedComposerAttachmentFiles(
	...
	for (const attachment of attachments) {
		...
		const ids =
			uploadCandidates.get(attachment.uploadEnvironmentId) ?? new Set<string>();
	...
	const allFilesReferenced = [...candidates].every(
		isComposerAttachmentFileReferenced,
	);
	const allUploadsReferenced = [...uploadCandidates].every(
		([environmentId, attachmentIds]) =>
	...
	try {
		const { loadIncomingShareDrafts } =
			await import("../features/sharing/incoming-share-storage");
		...
		console.warn(
			"[composer-attachments] could not verify incoming share ownership",
			error,
		);
		return;
	}

	const { removePersistedComposerAttachmentFile } =
		await import("../lib/composerImages");
	...
	if (uploadCandidates.size > 0) {
		const { releasePendingAttachmentUploads } =
			await import("../lib/attachmentUpload");
	...
	const loading = loadPersistedComposerState().then((persisted) => {
		...
		if (
			...
			appAtomRegistry.set(
				...
				persisted.stickyModelSelection,
			);
		}
	});
	loadPromise = loading;
...
export async function archiveCloudComposerDrafts(
	...
	const savedMessages = new Map(
		(cloud.signedOut[owner]?.queuedMessages ?? []).map((message) => [
			message.messageId,
			message,
		]),
	);
	...
	appAtomRegistry.set(composerCloudDraftsAtom, {
		...
		signedOut: {
			...
			[owner]: {
				drafts: savedDrafts,
				queuedMessages: [...savedMessages.values()],
			},
		},
	});
...
export async function removeDeliveredCloudQueuedMessage(
	...
	for (const [accountId, saved] of Object.entries(signedOut)) {
		...
		if (
			...
			(editor.text !== message.text ||
				...
				(editor.modelSelection !== undefined &&
					JSON.stringify(editor.modelSelection) !==
						JSON.stringify(message.modelSelection)) ||
				(editor.runtimeMode !== undefined &&
					editor.runtimeMode !== message.runtimeMode) ||
				...
				(editor.workspaceSelection !== undefined &&
					(editor.workspaceSelection.mode !== message.creation?.workspaceMode ||
						...
						editor.workspaceSelection.worktreePath !==
							message.creation?.worktreePath ||
		...
		signedOut[accountId] = {
			drafts,
			queuedMessages: saved.queuedMessages.filter(
...
export async function restoreCloudComposerDrafts(
	accountId: string,
): Promise<void> {
	await waitForComposerDraftsLoaded();
	const cloud = appAtomRegistry.get(composerCloudDraftsAtom);
...
export function setStickyComposerModelSelection(
...
export function appendComposerDraftAttachments(
	...
	updateComposerDrafts((current) => {
		...
		const remaining = options?.allowOverflow
			...
			: Math.max(
					0,
					PROVIDER_SEND_TURN_MAX_ATTACHMENTS - existing.attachments.length,
				);
...
export function mergeComposerDraftContentState(
	...
	const existing = normalizeDraft(current[draftKey]);
	if (
		content.sourceShareId &&
		existing.importedShareIds?.includes(content.sourceShareId)
	) {
		return current;
	}
	const attachmentIds = new Set(
...
export async function mergeComposerDraftContent(
	...
	const currentAttachmentIds = new Set(
		normalizeDraft(current[draftKey]).attachments.map(
			(attachment) => attachment.id,
		),
	);
	const nextAttachmentIds = new Set(
		normalizeDraft(next[draftKey]).attachments.map(
			(attachment) => attachment.id,
		),
	);
...
export function sameComposerDraftState(
	a: ComposerDraft,
	b: ComposerDraft,
): boolean {
	return (
...
export function undoComposerDraftMergeState(
	...
	>(
		...
		existing[key] === merged[key] ? snapshot[key] : existing[key];
	const text =
	...
	return Object.fromEntries(
		Object.entries(drafts).filter(
			([draftKey, draft]) =>
				...
				draft.project?.environmentId !== environmentId,
		),
	);
}
...
export function retargetNewTaskDraft(
	...
	updateComposerDrafts((current) => {
		...
		const { workspaceSelection: _workspaceSelection, ...retained } =
...
export function findNewTaskDraftKeys(
	...
	return Object.entries(drafts)
		...
		.sort(([, left], [, right]) =>
			(right.project?.createdAt ?? "").localeCompare(
...
export async function clearComposerDraftsEnvironment(
	environmentId: EnvironmentId,
): Promise<void> {
	ensureComposerDraftsLoaded();
	if (loadPromise !== null) {
