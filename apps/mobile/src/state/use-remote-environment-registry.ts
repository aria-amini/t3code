...
const EMPTY_SAVED_CONNECTION_ATOM = Atom.make<SavedRemoteConnection | null>(
	null,
).pipe(Atom.withLabel("mobile:saved-connection:empty"));

const EMPTY_RUNTIME_STATE_ATOM = Atom.make<EnvironmentRuntimeState | null>(
	null,
).pipe(Atom.withLabel("mobile:environment-runtime-state:empty"));
...
export function useRemoteConnections() {
	...
	const pendingConnectionError = useAtomValue(pendingConnectionErrorAtom);
	const { connectedEnvironments, connectionError, connectionState } =
	...
	const onReconnectEnvironment = useCallback(
		(environmentId: EnvironmentId) =>
			controller.retryEnvironment(environmentId),
		[controller],
	);
