...
export function useSelectedThreadGitActions() {
	...
	> => {
		branchState.refresh();
		return dedupeRemoteBranchesWithLocalMatches(
	...
	const syncSelectedThreadBranchState = useCallback(
		async (input: {
			...
			if (input.nextThreadState) {
				const updateResult = await updateThreadGitContext(
					input.thread,
					input.nextThreadState,
				);
		...
		async (
			...
			return await runSelectedThreadGitMutation(
				...
				async ({ thread, cwd }) => {
					...
					if (
						result.value.branch.status === "created" &&
						result.value.branch.name
					) {
						const syncResult = await syncSelectedThreadBranchState({
							thread,
							cwd,
