...
export function appendReviewCommentToDraft(input: {
	...
	const existing =
		appAtomRegistry.get(composerDraftsAtom)[threadKey]?.text ?? "";
	const separator =
	...
	if (input.attachments && input.attachments.length > 0) {
		...
		const rejectedCount = appendComposerDraftAttachments(
			threadKey,
			input.attachments,
		);
		if (rejectedCount > 0) {
...
export function useThreadComposerState() {
	...
	const dismissFeedback = useCallback(
		(id: MessageId) => {
			...
			setFeedbackSubmissionsByThreadKey((current) => ({
				...current,
				[selectedThreadKey]: (current[selectedThreadKey] ?? []).filter(
					(entry) => entry.id !== id,
				),
			}));
		},
		...
		const pendingAcknowledgments = acknowledgedMessages.filter(
			(message) =>
				...
				!selectedThreadQueuedMessages.some(
					(queued) => queued.messageId === message.messageId,
				),
		);
		if (pendingAcknowledgments.length === 0) return feed;
		return appendPendingThreadMessages(feed, feed, pendingAcknowledgments).map(
			(entry) =>
	...
	useEffect(() => {
		...
		if (
			acknowledgedMessages.some((message) => echoedIds.has(message.messageId))
		) {
			appAtomRegistry.set(
	...
	const selectedDraft = selectedThreadKey
	...
	const runtimeMode =
		selectedDraft?.runtimeMode ?? selectedThread?.runtimeMode ?? null;
	...
	const isCompacting = useMemo(() => {
		...
		const compactRequestIsActive =
			...
			(latestCompactMessage.createdAt >
				(selectedThread?.latestTurn?.requestedAt ??
					latestCompactMessage.createdAt) ||
				(selectedThread?.latestTurn?.state === "running" &&
					latestCompactMessage.createdAt ===
						selectedThread.latestTurn.requestedAt));
		const compactionSettled = selectedThreadDetail?.activities.some(
			(activity) => {
				if (
					!["context-compaction", "provider.turn.start.failed"].includes(
						activity.kind,
					)
				)
					return false;
	...
	const onPasteIntoDraft = useCallback(async () => {
		...
		const rejectedPasteCount = appendComposerDraftAttachments(
			threadKey,
			result.images,
		);
		if (result.text) {
