...
const harness = vi.hoisted(() => ({
	...
	removeOutboxMessage: vi.fn(
		async (_message: QueuedThreadMessage) => undefined,
	),
	prepareTurnAttachments:
		vi.fn<typeof import("../lib/attachmentUpload").prepareTurnAttachments>(),
...
vi.mock("./use-thread-outbox", async () => {
	const { Atom } = await import("effect/unstable/reactivity");
	return {
		editingQueuedMessageIdsAtom: Atom.make<Record<string, boolean>>({}).pipe(
			Atom.keepAlive,
		),
...
vi.mock("./thread-outbox", async () => {
	...
	return {
		...
		confirmThreadOutboxMessageQueued: (message: never) =>
			manager.confirmQueued(message),
...
describe("thread outbox attachment preparation", () => {
	...
	it("keeps an unchanged queued payload ready after attachment reuse", async () => {
		...
		await expect(
			prepareQueuedMessageAttachments(message),
	...
	it("uses the known next revision after persisting uploaded references", async () => {
		...
		harness.prepareTurnAttachments.mockImplementationOnce(async (input) => {
			expect(await input.persistUploadedReferences?.(uploadedAttachments)).toBe(
				"persisted",
			);
			return {
				status: "ready",
	...
	it("does not prepare a payload that was already replaced", async () => {
		const message = queuedMessage({
			messageId: "message-stale-before-upload",
			text: "old",
		});
		await harness.manager.enqueue(message);
...
describe("thread outbox drain delivery cleanup", () => {
	it("removes an acknowledged outbox item even when the sign-out archive write fails", async () => {
		const message = queuedMessage({
			messageId: "archive-write-failure",
			text: "Delivered",
		});
		await harness.manager.enqueue(message);
	...
	it.each([false, true])(
		...
		async (cleared) => {
			...
			expect(
				appAtomRegistry.get(composerDrafts.composerCloudDraftsAtom).signedOut[
					"account-a"
				]?.queuedMessages,
			).toEqual([message]);
			...
			await expect(
				completeQueuedMessageDelivery(message, deliveryRevision),
			).resolves.toBe(cleared ? "edited" : "removed");
	...
	it("preserves an archived edit when an older payload finishes delivery", async () => {
		const message = queuedMessage({
			messageId: "edited-during-sign-out",
			text: "Original",
		});
		await harness.manager.enqueue(message);
		const deliveryRevision = harness.manager.revisionOf(message.messageId);
	...
	it("retries only cleanup after an acknowledged send removal fails", async () => {
		const message = queuedMessage({
			messageId: "message-acknowledged",
			text: "delivered",
		});
		const acknowledged = new Set([message.messageId]);
		harness.removeOutboxMessage.mockRejectedValueOnce(
			new Error("storage unavailable"),
		);
		await harness.manager.enqueue(message);

		await expect(
			removeAcknowledgedExistingThreadMessage(message, acknowledged),
		).resolves.toBe(false);
...
describe("thread outbox delivered creation recovery", () => {
	it("keeps an edit accepted while the older payload is persisted to the draft", async () => {
		...
		const originalMergeComposerDraftContent =
			composerDrafts.mergeComposerDraftContent;
		...
		const mergeSpy = vi
			...
			.mockImplementation(async (draftKey, content) => {
				const result = await originalMergeComposerDraftContent(
					draftKey,
					content,
				);
		...
		try {
			...
			const newer = {
				...message,
				text: "edited while recovery persisted the draft",
			};
	...
	it("retries a failed removal without duplicating recovered draft content", async () => {
		...
		try {
			...
			await expect(recoverEditedCreationAfterDelivery(message)).resolves.toBe(
				false,
			);
			expect(remainingMessages()).toEqual([message]);
...
describe("thread outbox recovery rollback", () => {
	...
	it("keeps setup edits recoverable when saving their recovery draft fails", async () => {
		...
		await recoverFailedThreadDraft(message);
		expect(
			composerDrafts.getComposerDraftSnapshot("new-task:restored-failed-save")
				.text,
	...
	it("restores a rejected new task as its own draft for the project", async () => {
		const message: QueuedThreadMessage = {
			...queuedMessage({
				messageId: "message-creation-restore",
				text: "new task text",
			}),
			modelSelection: {
		...
		await expect(
			restoreRejectedQueuedMessage(message, "rejected by server"),
	...
	it("keeps a failed outcome until its thread screen consumes it", async () => {
		const message: QueuedThreadMessage = {
			...queuedMessage({
				messageId: "message-creation-kept",
				text: "new task text",
			}),
			modelSelection: {
				instanceId: ProviderInstanceId.make("codex"),
		...
		expect(
			appAtomRegistry.get(pendingThreadCreationOutcomesAtom)[key]?.kind,
		).toBe("failed");

		clearPendingThreadCreationOutcome(key);
		expect(
			appAtomRegistry.get(pendingThreadCreationOutcomesAtom)[key],
		).toBeUndefined();
	});

	it("does not record a creation outcome for a rejected follow-up message", async () => {
		const message = queuedMessage({
			messageId: "message-followup-restore",
			text: "follow up",
		});
		await harness.manager.enqueue(message);

		await expect(
			restoreRejectedQueuedMessage(message, "rejected"),
		).resolves.toBe("restored");
	...
	it("rolls a failed recovery merge back so the retry cannot duplicate the text", async () => {
		...
		harness.draftFile.setWriteError(new Error("disk full"));
		await expect(
			restoreRejectedQueuedMessage(message, "too large"),
		).resolves.toBe("retry");
