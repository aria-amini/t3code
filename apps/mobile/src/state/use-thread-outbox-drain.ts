...
export async function completeQueuedMessageDelivery(
	...
	try {
		await removeDeliveredCloudQueuedMessage(queuedMessage).catch((error) => {
			console.warn(
				"[thread-outbox] could not update sign-out snapshot after delivery",
				{
...
export async function removeAcknowledgedExistingThreadMessage(
	...
	try {
		...
		console.warn(
			"[thread-outbox] failed to remove acknowledged queued message",
			{
				environmentId: queuedMessage.environmentId,
				threadId: queuedMessage.threadId,
				messageId: queuedMessage.messageId,
...
export async function recoverEditedCreationAfterDelivery(
	...
	try {
		...
		updateComposerDraftSettings(draftKey, {
			...(kept.modelSelection !== undefined
				? { modelSelection: kept.modelSelection }
				: {}),
			...(kept.runtimeMode !== undefined
				? { runtimeMode: kept.runtimeMode }
				: {}),
			...(kept.interactionMode !== undefined
	...
	try {
		...
		console.warn(
			"[thread-outbox] could not remove recovered pending task",
			error,
		);
		return false;
	}
...
export async function restoreRejectedQueuedMessage(
	...
	try {
		...
		if (
			existingAttachmentIds.size + addedAttachmentCount >
			PROVIDER_SEND_TURN_MAX_ATTACHMENTS
		) {
			setPendingConnectionError(
		...
		if (rollback !== null) {
			...
			await undoComposerDraftMerge(
				draftKey,
				rollback.snapshot,
				rollback.merged,
			).catch((undoError) => {
				console.warn(
					"[thread-outbox] failed to persist a recovery rollback",
					undoError,
				);
			});
		}
		console.warn(
			"[thread-outbox] failed to restore an undeliverable message",
			error,
		);
		setPendingConnectionError(
			error instanceof Error
				? error.message
				: "The unsent message could not be restored.",
		);
		return "retry";
	}
}
...
function stampRecoveryDraftProject(
	queuedMessage: QueuedThreadMessage,
	draftKey: string,
): void {
	if (!queuedMessage.creation) {
...
async function preserveUploadedAttachmentsForEditor(
	...
	const uploadedById = new Map(
		uploadedMessage.attachments.map(
...
export function useThreadOutboxDrain(): void {
	...
	const dispatchingQueuedMessageId = useAtomValue(
		dispatchingQueuedMessageIdAtom,
	);
	...
	const retryTimersRef = useRef(
		new Map<MessageId, ReturnType<typeof setTimeout>>(),
	);
	...
	const restoreQueuedMessage = useCallback(
		async (
			...
			if (
				!blockedRecoverySubscriptionsRef.current.has(queuedMessage.messageId)
			) {
				const draftKey = recoveryDraftKey(queuedMessage);
				...
				const editorAttachments =
					editorDraftKey === null
						? undefined
						: currentDrafts[editorDraftKey]?.attachments;
				const unsubscribe = appAtomRegistry.subscribe(
					...
					(drafts) => {
						if (
							...
							(editorDraftKey === null ||
								drafts[editorDraftKey]?.attachments === editorAttachments)
						) {
							return;
						}
						const active = blockedRecoverySubscriptionsRef.current.get(
							queuedMessage.messageId,
						);
						if (!active) {
							return;
						}
						blockedRecoverySubscriptionsRef.current.delete(
							queuedMessage.messageId,
						);
						active.unsubscribe();
						setRetryTick((current) => current + 1);
	...
	const sendQueuedMessage = useCallback(
		async (
			...
			try {
				...
				if (!shouldRetryThreadOutboxDelivery(error)) {
					return restoreQueuedMessage(
						...
						error instanceof Error
							? error.message
							: "An attachment could not upload.",
					);
				}
				return false;
			}
			...
			acknowledgedExistingThreadMessageIdsRef.current.add(
	...
	const sendQueuedCreation = useCallback(
		async (
			...
			const outcome = await completeQueuedMessageDelivery(
				persistedMessage,
				deliveryRevision,
			);
			if (outcome === "edited") {
	...
	useEffect(() => {
		...
		for (const [threadKey, queuedMessages] of Object.entries(
			...
			if (
				...
				acknowledgedExistingThreadMessageIdsRef.current.has(
					nextQueuedMessage.messageId,
				)
			) {
				if (
					(retryNotBeforeRef.current.get(nextQueuedMessage.messageId) ?? 0) >
					Date.now()
				) {
					continue;
				}
				...
				void removeAcknowledgedExistingThreadMessage(
					...
					.then((removed) => {
						...
						const pendingTimer = retryTimersRef.current.get(
							nextQueuedMessage.messageId,
						);
						if (pendingTimer !== undefined) {
							clearTimeout(pendingTimer);
			...
			const shellStatus =
				shellStatuses.get(nextQueuedMessage.environmentId) ?? "empty";
