...
const pullRequestSummaryAtom = createLinkedPullRequestSummaryAtomFamily(
...
const threadPrSnapshotsAtom = Atom.make<ReadonlyMap<string, ThreadPrSnapshot>>(
	new Map(),
).pipe(Atom.keepAlive, Atom.withLabel("mobile:thread-pr-snapshots"));
...
export function useThreadPr(
	...
	const snapshotEntry = useAtomValue(
		...
		useCallback(
			(current: ReadonlyMap<string, ThreadPrSnapshot>) =>
				current.get(threadKey),
			[threadKey],
		),
	);
	const snapshot =
		snapshotEntry?.identity === snapshotIdentity
			? snapshotEntry.presentation
			: null;
	...
	useEffect(() => {
		...
		appAtomRegistry.modify(threadPrSnapshotsAtom, (current) => {
			...
			if (
				existing?.identity === snapshotIdentity &&
				existing.presentation === live
			) {
