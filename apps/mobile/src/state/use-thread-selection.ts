...
): OrchestrationThread["updatedAt"] | null {
...
function useResolvedThreadSelection(
	...
	const [previousCreation, setPreviousCreation] =
		useState<PendingThreadCreation | null>(null);
	...
	const selectedEnvironmentConnection = useSavedRemoteConnection(
...
export function useThreadSelection(): ThreadSelectionState {
	const route =
		useRoute<
			RouteProp<Record<string, ThreadSelectionRouteParams | undefined>>
		>();
