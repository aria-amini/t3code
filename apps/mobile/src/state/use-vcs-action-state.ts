...
const gitActionResultAtom = Atom.make<GitActionResultNotification | null>(
	null,
).pipe(Atom.keepAlive, Atom.withLabel("mobile:git-action-result"));
...
export function useGitActionProgress(
	...
	if (actionState.isRunning) {
		const description =
			...
			formatElapsedSeconds(
				actionState.hookStartedAtMs ?? actionState.phaseStartedAtMs,
			);
		return {
			phase: "running",
