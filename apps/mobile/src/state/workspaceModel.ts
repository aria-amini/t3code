...
function overallConnectionState(
	...
	if (
		environments.some(
			(environment) => environment.connectionState === "reconnecting",
		)
	) {
		return "reconnecting";
	}
	...
	if (
		environments.some((environment) => environment.connectionState === "error")
	) {
		return "error";
	}
	if (
		environments.some(
			(environment) => environment.connectionState === "offline",
		)
	) {
		return "offline";
	}
	...
	return {
		...
		connectionError:
			input.environments.find(
				(environment) => environment.connectionError !== null,
