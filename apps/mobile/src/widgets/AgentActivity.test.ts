...
vi.mock("expo-widgets", () => ({
	createLiveActivity: vi.fn((name: string, layout: unknown) => ({
		layout,
		name,
	})),
}));
...
function makeRow(
	overrides: Partial<AgentActivityRowProps>,
...
describe("AgentActivity widget layout", () => {
	...
	it("orders rows attention-first in the banner", () => {
		...
		expect(banner.indexOf("Blocked thread")).toBeLessThan(
			banner.indexOf("Working thread"),
		);
	});
	...
	it("omits the deep link for unsafe paths and empty aggregates", () => {
		expect(
			JSON.stringify(AgentActivity(props, environment as never)),
