...
export const measureHttpGet = Effect.fn("TransferBudget.measureHttpGet")(
	function* (input: {
		readonly url: string;
								...
								try {
									...
									const decodedBody =
										contentEncoding === "gzip"
											? NodeZlib.gunzipSync(encodedBody)
											: encodedBody;
									resolve({
										...
										wireBytes: Math.max(
											0,
											response.socket.bytesRead - socketBytesBeforeResponse,
										),
									});
								} catch (cause) {
									reject(cause);
	...
	return {
		...
		awaitOpen: Effect.promise(() => opened).pipe(
			Effect.timeoutOrElse({
				duration: "10 seconds",
				orElse: () =>
					Effect.die(new Error("Timed out waiting for the WebSocket to open")),
			}),
		),
	};
}
...
export function countingWsRpcProtocolLayer(input: {
	...
	const webSocketConstructorLayer = Layer.succeed(
		...
		(url, protocols) => input.recorder.connect(url, protocols, input.cookie),
	);
...
export type CountingWsRpcClient = Effect.Success<
	typeof makeCountingWsRpcClient
>;
 ...
 */
export const openMeasuredWsClient = Effect.fn(
	"TransferBudget.openMeasuredWsClient",
