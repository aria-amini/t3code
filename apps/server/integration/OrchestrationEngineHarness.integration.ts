...
class WaitForTimeoutError extends Schema.TaggedError<WaitForTimeoutError>()(
	"WaitForTimeoutError",
	{
...
export const makeOrchestrationIntegrationHarness = (
	...
	Effect.gen(function* () {
		...
		const { stateDir, dbPath } = yield* deriveServerPaths(
			rootDir,
			undefined,
		...
		const checkpointReactor = yield* tryRuntimePromise(
			"load CheckpointReactor service",
			...
			(predicate, timeoutMs) =>
				waitFor(
					Stream.runCollect(engine.readEvents(0)).pipe(
						Effect.map((chunk): ReadonlyArray<OrchestrationEvent> =>
							Array.from(chunk),
						),
					),
