...
export const TRANSFER_BUDGETS: Readonly<
	Record<string, ProviderTransferBudget>
> = {
...
export function formatTransferBudgetResult(
	...
	const providers = Object.fromEntries(
		runs.flatMap((run) => {
			...
			return ceiling
				? [[run.provider, { observed: observedTransfer(run), ceiling }]]
				: [];
		}),
	);
...
export function transferBudgetViolations(
	runs: ReadonlyArray<TransferBudgetRun>,
): string[] {
	...
	for (const run of runs) {
		...
		for (const [metric, observed, maximum] of checks) {
			if (observed > maximum) {
				violations.push(
					`${run.provider}: ${metric} was ${observed}, maximum ${maximum}`,
				);
			}
		}
	}
