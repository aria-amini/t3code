...
export const TRANSFER_MEASURED_TURN_CREATED_AT = turnTimestamp(
	TRANSFER_MEASURED_TURN_INDEX,
);

const waitForTurnQuiesced = Effect.fn("TransferBudget.waitForTurnQuiesced")(
	function* (
...
export const seedTransferBudgetHistory = Effect.fn(
	"TransferBudget.seedHistory",
)(function* (
	harness: OrchestrationIntegrationHarness,
	...
	if (!harness.adapterHarness) {
		return yield* Effect.die(
			new Error("Transfer budget history requires the replay adapter."),
		);
	}
	...
	for (
		let turnIndex = 0;
		turnIndex < TRANSFER_HISTORY_TURN_COUNT;
		turnIndex += 1
	) {
...
export const queueMeasuredTransferTurn = Effect.fn(
	"TransferBudget.queueMeasuredTurn",
)(function* (
	harness: OrchestrationIntegrationHarness,
	provider: ProviderDriverKind,
) {
	if (!harness.adapterHarness) {
		return yield* Effect.die(
			new Error("Transfer budget measurement requires the replay adapter."),
		);
	}
	const response = makeRecordedTransferTurn(
		provider,
		TRANSFER_MEASURED_TURN_INDEX,
	);
...
export function expectedMeasuredAssistantText(
	provider: ProviderDriverKind,
): string {
...
export const collectQueueUntil = Effect.fn("TransferBudget.collectQueueUntil")(
	function* <A>(
		queue: Queue.Queue<A>,
		...
		return yield* Effect.gen(function* () {
			...
			Effect.timeoutOrElse({
				duration: "10 seconds",
				orElse: () =>
					Effect.die(new Error(`Timed out waiting for ${waitDescription}`)),
			}),
		);
	},
);
 ...
 */
export const subscribeThreadItems = Effect.fn(
	"TransferBudget.subscribeThreadItems",
)(function* (measured: MeasuredWsClient, afterSequence: number) {
...
export const subscribeShellItems = Effect.fn(
	"TransferBudget.subscribeShellItems",
)(function* (measured: MeasuredWsClient, afterSequence: number) {
...
export const awaitSubscriptionSynchronized = Effect.fn(
	...
	Item extends OrchestrationThreadStreamItem | OrchestrationShellStreamItem,
>(items: Queue.Queue<Item>, waitDescription: string) {
