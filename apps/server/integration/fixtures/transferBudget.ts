...
function diagnosticOutput(input: {
	...
	const chunks: string[] = [];
	const providerSeed =
		input.provider === "codex" ? 0x43_4f_44_45 : 0x43_4c_41_55;
	let length = 0;
	let lineIndex = 0;

	while (length < input.targetBytes) {
		const modulePath =
			sourceModules[(input.toolIndex + lineIndex) % sourceModules.length];
		const seed =
			providerSeed +
			input.turnIndex * 100_003 +
			input.toolIndex * 10_007 +
			lineIndex * 101;
		const line =
...
function baseEvent(
	...
	FixtureProviderRuntimeEvent,
	"eventId" | "provider" | "createdAt" | "threadId"
> {
	return {
		...
		events.push(
			{
				...
				payload: {
					...
					detail:
						"Inspecting the HTTP snapshot and WebSocket projection boundaries.",
					data: {
						threadId: FIXTURE_THREAD_ID,
						turnId,
	...
	events.push(
		{
			...
			payload: {
				...
				detail:
					"Reading a retained diagnostic result from the provider history.",
				data: {
					startedAtMs: turnIndex * 60_000 + eventIndex,
					threadId: FIXTURE_THREAD_ID,
					turnId,
		...
		},
		{
			...
			payload: {
				...
				title: "fixture-history · inspect_transfer_log",
				detail:
					"Retained a deterministic diagnostic result in the thread history.",
				data: {
					completedAtMs: turnIndex * 60_000 + eventIndex,
					threadId: FIXTURE_THREAD_ID,
					turnId,
