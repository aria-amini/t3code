...
const stoppedBindingThreadId = ThreadId.make(
	"thread-startup-orphan-stopped-binding",
);
const resumeCursor = {
	schemaVersion: 1,
...
const startupDependencies = Layer.mergeAll(
	...
	Layer.succeed(ServerEnvironment.ServerEnvironment, {
		getEnvironmentId: Effect.succeed(
			EnvironmentId.make("environment-startup-orphan"),
		),
	...
	() =>
			...
			yield* Effect.gen(function* () {
				...
				yield* engine.dispatch({
					...
					commandId: CommandId.make(
						"command-start-stopped-binding-pending-turn",
					),
					threadId: stoppedBindingThreadId,
					message: {
				...
				yield* engine.dispatch({
					type: "thread.session.set",
					commandId: CommandId.make(
						"command-mark-stopped-binding-session-starting",
					),
					threadId: stoppedBindingThreadId,
					session: {
						threadId: stoppedBindingThreadId,
        ...
        `;
				...
				return {
					...
					stoppedBindingSessionStatus:
						restartedStoppedBindingThread.session?.status,
	...
	(restart) =>
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
					...
					Effect.provide(
						ServerSettings.layerTest({
							continueThreadsAfterServerUpdate:
								restart === "opt-in desktop restart",
						}),
					),
				);
				...
				Effect.provide(
					Layer.mergeAll(
						makePersistedRuntimeLayer(config.dbPath),
						startupDependencies,
					),
				),
			);
