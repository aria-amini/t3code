...
const emitXAiAskUserQuestion =
	process.env.T3_ACP_EMIT_XAI_ASK_USER_QUESTION === "1";
...
const emitXAiRateLimitThenHang =
	process.env.T3_ACP_EMIT_XAI_RATE_LIMIT_THEN_HANG === "1";
...
const exitOnSetConfigOption =
	process.env.T3_ACP_EXIT_ON_SET_CONFIG_OPTION === "1";
		...
		return [
			{
				...
				options: antigravityModels.map((model) => ({
					value: model.modelId,
					name: model.name,
				})),
			},
...
function modelConfigOptionsFor(
	modelId: string,
...
function modelState(): AcpSchema.SessionModelState {
	...
	const modelId = grokAcpModels.some(
...
const program = Effect.gen(function* () {
		...
		Effect.gen(function* () {
			...
			if (emitStaleXAiPromptCompleteBeforeSecondHang && promptCount === 2) {
				const currentPromptId =
					promptIdFromRequestMeta(request) ?? "mock-current-xai-prompt-2";
			...
			if (emitXAiRateLimitThenHang) {
				writeJsonRpcNotification("_x.ai/session/prompt_complete", {
					sessionId: requestedSessionId,
					promptId:
						promptIdFromRequestMeta(request) ?? "mock-xai-rate-limit-prompt-1",
			...
			if (emitToolCalls) {
				...
				for (let index = 0; index < permissionRequestCount; index++) {
					const command =
						index > 0
							? (process.env.T3_ACP_SECOND_PERMISSION_COMMAND ??
					...
					const permission = yield* agent.client.requestPermission({
						...
						toolCall: {
							toolCallId:
								index === 0 ? toolCallId : `${toolCallId}-${index + 1}`,
							...
							rawInput: {
								...
								command,
								description:
									index === 0 ? "Read package metadata" : "Read it again",
							},
							content: [
								{
									type: "content",
			...
			if (emitXAiAskUserQuestion || emitXAiAskUserQuestionThenHang) {
				const result = yield* agent.client.extRequest(
	...
	yield* agent.handleUnknownExtRequest((method, params) => {
		...
		if (method === "_test/finish-cancel") {
			return Deferred.succeed(nativeCancelRelease, undefined).pipe(
				Effect.as({}),
			);
		}
				...
				for (const [metadataSessionId, commandName, modeId] of [
					...
					yield* agent.client.sessionUpdate({
						...
						update: {
							sessionUpdate: "available_commands_update",
							availableCommands: [
								{ name: commandName, description: "Native command" },
							],
						},
					});
					yield* agent.client.sessionUpdate({
							...
							configOptions: configOptions().map((option) =>
								option.type === "select" && option.category === "model"
									? {
											...option,
											currentValue:
												metadataSessionId === sessionId ? "gpt-5.4" : "default",
										}
									: option,
							),
						},
					...
					yield* agent.client.sessionUpdate({
						...
						update: {
							...
							content: {
								type: "text",
								text: "Startup transcript must not replay.",
							},
						},
					});
				}
