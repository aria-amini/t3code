...
const preparePublishIcons = Effect.fn("preparePublishIcons")(function* (
	...
	const icons = resolveWebIconOverrides(brand, "dist/client").map(
		(override) => ({
	...
	for (const icon of icons) {
		if (!(yield* fs.exists(icon.sourcePath))) {
			return yield* new ServerCliPublishIconSourceMissingError({
				sourcePath: icon.sourcePath,
			});
		}
		if (!(yield* fs.exists(icon.targetPath))) {
			return yield* new ServerCliPublishIconTargetMissingError({
				...
				(resource) =>
					Effect.gen(function* () {
						yield* fs.writeFileString(
							packageJsonPath,
							`${resource.packageJsonString}\n`,
						);
						for (const icon of resource.icons) {
							yield* fs.writeFile(icon.targetPath, icon.publish);
						}
						yield* Effect.log(
							"[cli] Applied package metadata and publish icon overrides",
						);
