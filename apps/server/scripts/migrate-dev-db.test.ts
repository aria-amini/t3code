...
const createFixtureSource = Effect.fn("createMigrateDevDbFixtureSource")(
...
it.layer(NodeServices.layer)("migrate-dev-db", (it) => {
	it.effect(
		"keeps only stopped threads from live projects and clears auth state",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
				const sourceDir = yield* fs.makeTempDirectoryScoped({
					prefix: "migrate-dev-db-src-",
				});
				const destDir = yield* fs.makeTempDirectoryScoped({
					prefix: "migrate-dev-db-dest-",
				});
				...
				assert.equal(
					result.databasePath,
					path.join(destDir, "userdata", "state.sqlite"),
				);
	...
	it.effect("fails loudly on a migration slot collision", () =>
		Effect.gen(function* () {
			const fs = yield* FileSystem.FileSystem;
			const sourceDir = yield* fs.makeTempDirectoryScoped({
				prefix: "migrate-dev-db-slot-",
			});
			const destDir = yield* fs.makeTempDirectoryScoped({
				prefix: "migrate-dev-db-slot-dest-",
			});
			const source = yield* createFixtureSource(sourceDir);
	...
	it.effect("refuses while a dev server holds the destination", () =>
		Effect.gen(function* () {
			...
			const path = yield* Path.Path;
			const sourceDir = yield* fs.makeTempDirectoryScoped({
				prefix: "migrate-dev-db-busy-",
			});
			const destDir = yield* fs.makeTempDirectoryScoped({
				prefix: "migrate-dev-db-busy-dest-",
			});
			const source = yield* createFixtureSource(sourceDir);
	...
	it.effect("refuses a source that resolves to a destination path", () =>
		Effect.gen(function* () {
			...
			const sharedDir = yield* fs.makeTempDirectoryScoped({
				prefix: "migrate-dev-db-overlap-",
			});
			const destDir = yield* fs.makeTempDirectoryScoped({
				prefix: "migrate-dev-db-overlap-dest-",
			});
			// A leftover snapshot from a prior failed run, passed as --source: it
			// must not be deleted before it is read.
			const leftoverSnapshot = path.join(
				destDir,
				"userdata",
				"state.sqlite.migrate-dev-db-tmp",
			);
			yield* fs.makeDirectory(path.dirname(leftoverSnapshot), {
				recursive: true,
			});
