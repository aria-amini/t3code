...
const removeDatabaseFiles = Effect.fn("removeDatabaseFiles")(function* (
	databasePath: string,
) {
...
const ensureNotInUse = Effect.fn("ensureDevDbNotInUse")(function* (
	databasePath: string,
) {
	const fs = yield* FileSystem.FileSystem;
	...
	const checkpoint = yield* Effect.gen(function* () {
		...
		return yield* sql.unsafe<{ busy: number }>(
...
const pruneSnapshot = Effect.fn("pruneDevDbSnapshot")(function* (
	...
	const monitorFilter = threadColumns.some(
		(column) => column.name === "monitor_json",
	)
...
export const runMigrateDevDb = Effect.fn("runMigrateDevDb")(function* (
	...
	if (input.projects < 1 || input.threadsPerProject < 0) {
		return yield* Effect.die(
			"projects must be >= 1 and threadsPerProject >= 0",
		);
	}
	const fs = yield* FileSystem.FileSystem;
	const path = yield* Path.Path;
	...
	const wrapPhase =
		...
		<A, E, R>(effect: Effect.Effect<A, E, R>) =>
			effect.pipe(
				Effect.mapError(
					(cause) =>
						new MigrateDevDbPhaseError({
							phase,
							databasePath: phaseDatabasePath,
							cause,
						}),
				),
			);
	...
	const { executedMigrations, pruned } = yield* Effect.gen(function* () {
		...
		yield* Effect.gen(function* () {
			...
			Effect.provide(
				NodeSqliteClient.layer({ filename: sourcePath, readonly: true }),
			),
	...
	"migrate-dev-db",
	{
		...
		threadsPerProject: Flag.integer("threads-per-project").pipe(
			...
			Flag.withDescription(
				"How many recent stopped threads to keep per project.",
			),
		),
		baseDir: Flag.string("base-dir").pipe(
			Flag.optional,
			Flag.withDescription(
				"Isolated .t3 directory. Defaults to the current worktree's .t3.",
			),
		),
