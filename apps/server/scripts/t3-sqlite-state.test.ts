...
const createFixtureDatabase = Effect.fn("createSqliteStateFixtureDatabase")(
...
it.layer(NodeServices.layer)("t3-sqlite-state", (it) => {
	it.effect("reports each invalid SQL source with a specific error", () =>
		Effect.gen(function* () {
			...
			assert.equal(
				multipleSources.message,
				"Provide only one of --sql or --file.",
			);

			const missingSource = yield* runSqliteState({
				operation: "query",
				baseDir,
			}).pipe(Effect.flip);
