...
const encodeSqliteStateResult = Schema.encodeEffect(
...
const resolveSqlSource = Effect.fn("resolveSqliteStateSqlSource")(function* (
	...
	if (sql !== undefined) {
		...
		source = yield* fs
			...
			.pipe(
				Effect.mapError(
					(cause) => new SqliteStateSqlFileError({ filePath, cause }),
				),
			);
	}
	...
	"t3-sqlite-state",
	{
		operation: Argument.choice("operation", SqliteStateOperation.literals).pipe(
			Argument.withDescription(
				"Run a read-only query or a backed-up fixture mutation.",
			),
		),
		baseDir: Flag.string("base-dir").pipe(
			Flag.withDescription(
				"Explicit T3 base directory containing userdata/state.sqlite.",
			),
		),
