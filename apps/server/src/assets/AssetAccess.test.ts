...
describe("AssetAccess", () => {
	it.effect(
		"issues exact URLs for media and browser documents outside the workspace",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
				const root = yield* fs.makeTempDirectoryScoped({
					prefix: "t3-media-root-",
				});
				const outside = yield* fs.makeTempDirectoryScoped({
					prefix: "t3-media-outside-",
				});
				for (const [name, mimeType] of [
					...
					const suffix = result.relativeUrl.slice(
						`${ASSET_ROUTE_PREFIX}/`.length,
					);
					const separator = suffix.indexOf("/");
					const token = suffix.slice(0, separator);
					expect(
						yield* resolveAsset(token, suffix.slice(separator + 1)),
					).toMatchObject({
						kind: "file",
						path: canonicalFile,
						mimeType,
	...
	it.effect(
		"reports pixel dimensions from an image header and nothing for other files",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
				const root = yield* fs.makeTempDirectoryScoped({
					prefix: "t3-media-dimensions-",
				});
				const png = Uint8Array.from([
					0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 0, 0, 0, 13, 0x49,
					0x48, 0x44, 0x52, 0, 0, 0x06, 0x40, 0, 0, 0x03, 0x84,
				]);
				...
				expect((yield* issue("shot.png")).imageDimensions).toEqual({
					width: 1600,
					height: 900,
				});
	...
	it.effect(
		"resolves relative media paths from the thread workspace, including outside it",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
				const directory = yield* fs.makeTempDirectoryScoped({
					prefix: "t3-media-relative-",
				});
	...
	it.effect.skipIf(!symlinksSupported)(
		...
		() =>
			Effect.gen(function* () {
				...
				const path = yield* Path.Path;
				const root = yield* fs.makeTempDirectoryScoped({
					prefix: "t3-media-validation-",
				});
				for (const name of [
					"report.md",
					"secret.txt",
					"secret.%70ng",
					"secret.png#private.txt",
				]) {
			...
			Effect.gen(function* () {
				...
				for (const [range, expected, status] of [
					...
					const response = HttpServerResponse.toWeb(
						yield* assetFileResponse(asset, range),
					);
	...
	it.effect.skipIf(!symlinksSupported)(
		...
		() =>
			Effect.gen(function* () {
				...
				yield* fs.writeFileString(
					path.join(privateDirectory, "recording.mp4"),
					"private video",
				);
				const canonicalPath = yield* fs.realPath(filePath);
				const result = yield* issueAssetUrl({
				...
				yield* Effect.addFinalizer(() =>
					Effect.sync(() =>
						realpathSpy.mockReset().mockImplementation(native.realpath),
					),
				);
				const swappingFileSystem = FileSystem.FileSystem.of({
					...fs,
	...
	it.effect(
		"keeps in-place edits readable but requires a new URL after atomic replacement",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
				const root = yield* fs.makeTempDirectoryScoped({
					prefix: "t3-media-replacement-",
				});
				const filePath = path.join(root, "recording.mp4");
				...
				const suffix = original.relativeUrl.slice(
					`${ASSET_ROUTE_PREFIX}/`.length,
				);
				const separator = suffix.indexOf("/");
				const token = suffix.slice(0, separator);
				...
				const editedResponse = HttpServerResponse.toWeb(
					yield* assetFileResponse(edited),
				);
				expect(yield* Effect.promise(() => editedResponse.text())).toBe(
					"in-place edit",
				);
				...
				const renewedSuffix = renewed.relativeUrl.slice(
					`${ASSET_ROUTE_PREFIX}/`.length,
				);
				...
				const renewedResponse = HttpServerResponse.toWeb(
					yield* assetFileResponse(renewedAsset),
				);
				expect(yield* Effect.promise(() => renewedResponse.text())).toBe(
					"replacement",
				);
				yield* fs.remove(filePath);
				expect(
	...
	it.effect(
		"issues workspace URLs that resolve the entry file and sibling assets",
		() =>
			Effect.gen(function* () {
				...
				expect(
					yield* resolveAsset(`${token}tampered`, "report.html"),
				).toBeNull();
			}).pipe(Effect.provide(testLayer)),
	);
	...
	it.effect(
		"preserves non-missing canonical path failures when issuing asset URLs",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
	...
	it.effect(
		"serves document attachments inline when a viewer requests it",
		() =>
			Effect.gen(function* () {
				const config = yield* ServerConfig.ServerConfig;
				const fileSystem = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
				const attachmentId =
					"thread-1-00000000-0000-4000-8000-000000000001-pdf";
				const attachmentPath = path.join(
					config.attachmentsDir,
					`${attachmentId}.pdf`,
				);
	...
	it.effect(
		"keeps inline requests for other attachment types as downloads",
		() =>
			Effect.gen(function* () {
				const config = yield* ServerConfig.ServerConfig;
				const fileSystem = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
				const attachmentId =
					"thread-1-00000000-0000-4000-8000-000000000002-zip";
				const attachmentPath = path.join(
					config.attachmentsDir,
					`${attachmentId}.zip`,
				);
	...
	it.effect("issues project favicon capabilities with a signed fallback", () =>
		Effect.gen(function* () {
			...
			expect(faviconResult.relativeUrl).toMatch(
				/\/v[0-9a-f]{64}-favicon\.svg$/,
			);
			expect(
				yield* issueAssetUrl({
					resource: { _tag: "project-favicon", cwd: root },
			...
			const faviconSuffix = faviconResult.relativeUrl.slice(
				`${ASSET_ROUTE_PREFIX}/`.length,
			);
			...
			expect(
				updatedFaviconResult.relativeUrl.slice(
					updatedFaviconResult.relativeUrl.lastIndexOf("/"),
				),
			).not.toBe(
				faviconResult.relativeUrl.slice(
					faviconResult.relativeUrl.lastIndexOf("/"),
				),
			);
			...
			expect(
				fallbackResult.relativeUrl.endsWith(
					`/${PROJECT_FAVICON_FALLBACK_MARKER}`,
				),
			).toBe(true);
			expect(fallbackResult.sourcePath).toBeUndefined();
			const fallbackSuffix = fallbackResult.relativeUrl.slice(
				`${ASSET_ROUTE_PREFIX}/`.length,
			);
	...
	it.effect("issues project favicon capabilities for a saved override", () =>
		Effect.gen(function* () {
			...
			yield* fileSystem.writeFileString(
				path.join(root, "brand", "custom.svg"),
				"<svg />",
			);
			yield* fileSystem.writeFileString(
	...
	it.effect("ignores a client favicon path hint", () =>
		Effect.gen(function* () {
			...
			yield* fileSystem.makeDirectory(path.join(root, "brand"));
			yield* fileSystem.writeFileString(
				path.join(root, "brand", "hint.svg"),
				"<svg>hint</svg>",
			);
			yield* fileSystem.writeFileString(
				path.join(root, "brand", "saved.svg"),
				"<svg>saved</svg>",
			);
	...
	it.effect(
		"keeps automatic favicon resolution separate from a saved override",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
		...
		Effect.gen(function* () {
			...
			const crossingCrypto = Crypto.make({
				...
				digest: (algorithm, data) =>
					TestClock.adjust("2 millis").pipe(
