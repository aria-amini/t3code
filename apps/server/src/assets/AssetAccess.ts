...
function decodeClaims(encodedPayload: string): AssetClaims | null {
	try {
		return Option.getOrNull(
			decodeAssetClaims(base64UrlDecodeUtf8(encodedPayload)),
		);
	} catch {
		return null;
	}
...
const resolveCanonicalFile = Effect.fn("AssetAccess.resolveCanonicalFile")(
...
const resolveCanonicalWorkspaceFile = Effect.fn(
	"AssetAccess.resolveCanonicalWorkspaceFile",
)(function* (input: {
 ...
 */
const HEADER_IMAGE_EXTENSIONS = new Set([
	".png",
	".jpg",
	".jpeg",
	".gif",
...
const readImageDimensionsFromOpenFile = (
	filePath: string,
	file: OpenMediaFile,
) =>
...
const readImageDimensionsFromHeader = (filePath: string) =>
	openMediaFile(filePath).pipe(
		Effect.flatMap((file) =>
			file === null
				? Effect.succeed(null)
				: readImageDimensionsFromOpenFile(filePath, file),
		),
		Effect.scoped,
		Effect.orElseSucceed((): ImageDimensions | null => null),
	);

export const issueAssetUrl = Effect.fn("AssetAccess.issueAssetUrl")(
	function* (input: {
		...
		switch (input.resource._tag) {
			...
			case "attachment": {
				...
				const extension = parseAttachmentFileExtension(
					input.resource.attachmentId,
				);
				const isGenericFile = extension !== null;
				const videoMimeType =
					input.resource.mimeType?.split(";", 1)[0]?.trim() ?? "";
				...
				claims = {
					...
					...(isGenericFile && !isVideo && inlineDocumentMimeType === undefined
						? { download: true }
						: {}),
					...(input.resource.fileName !== undefined
			...
			case "project-favicon": {
				...
				const faviconResolver =
					yield* ProjectFaviconResolver.ProjectFaviconResolver;
				...
				const isExternalOverride =
					...
					path.normalize(faviconPath) ===
						path.normalize(input.projectFaviconPath);
				const relativePath =
					faviconPath && !isExternalOverride
						? path.relative(workspaceRoot, faviconPath)
						: null;
				const sourceFaviconPath = isExternalOverride
					? faviconPath
					: relativePath;
				if (
					sourceFaviconPath &&
					!isWorkspaceImagePreviewPath(sourceFaviconPath)
				) {
					return yield* new AssetPreviewTypeValidationError({
						resource: input.resource,
					});
				...
				const canonicalFaviconPath = sourceFaviconPath
					? yield* (
							isExternalOverride
								...
								: resolveCanonicalWorkspaceFile({
										workspaceRoot,
										relativePath: sourceFaviconPath,
									})
						).pipe(
							Effect.mapError(
								(cause) =>
...
export const resolveAsset = Effect.fn("AssetAccess.resolveAsset")(function* (
	...
	if (
		!timingSafeEqualBase64Url(
			signature,
			signPayload(encodedPayload, signingSecret),
		)
	)
		return null;
	...
	if (claims.kind === "attachment") {
		...
		return Option.isSome(info) && info.value.type === "File"
			? ({
					...
					...(claims.fileName !== undefined
						? { fileName: claims.fileName }
						: {}),
					...(claims.mimeType !== undefined
	...
	if (claims.kind === "native-app-icon") {
		const nativeAppIconResolver =
			yield* NativeAppIconResolver.NativeAppIconResolver;
	...
	if (claims.kind === "media-file-exact") {
		...
		const mimeType = hostPreviewMimeTypeFromExtension(
			path.extname(canonicalFile),
		);
		if (!mimeType) return null;
		const file = yield* openMediaFile(canonicalFile, claims).pipe(
			Effect.tapError((cause) =>
				Effect.logError("Failed to open canonical media file.", {
					filePath: canonicalFile,
					cause,
				}),
			),
