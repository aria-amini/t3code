...
const testLayer = ServerSecretStore.layer.pipe(
	Layer.provideMerge(
		ServerConfig.layerTest(process.cwd(), { prefix: "t3-attachment-upload-" }),
	),
...
describe("AttachmentUpload", () => {
	it.effect(
		"signs the attachment metadata and validates the upload token",
		() =>
			Effect.gen(function* () {
				...
				const token = issued.relativeUrl.slice(
					`${ATTACHMENT_UPLOAD_ROUTE_PREFIX}/`.length,
				);
	...
	it.effect("rejects tampered and malformed upload tokens", () =>
		Effect.gen(function* () {
			...
			expect(
				yield* validateAttachmentUploadToken(`${payload}x.${signature}`),
			).toBeNull();
	...
	it.effect(
		"accepts unexpired image upload tokens issued before file support",
		() =>
			Effect.gen(function* () {
				const issued = yield* issueAttachmentUploadUrl(uploadInput);
	...
	it.effect(
		"removes expired pending uploads while issuing a new upload URL",
		() =>
			Effect.gen(function* () {
	...
	it.effect("stores the expected bytes without leaving temporary files", () =>
		Effect.gen(function* () {
			...
			expect(
				yield* storeAttachmentUpload(claims, new Uint8Array([1, 2, 3])),
			).toMatchObject({
				ok: false,
				status: 400,
			});
			expect(yield* storeAttachmentUpload(claims, new Uint8Array(6))).toEqual({
				ok: true,
			});
			expect(
				NodeFS.existsSync(
					NodePath.join(config.attachmentsDir, `${issued.attachmentId}.png`),
				),
			).toBe(true);
			expect(
	...
	it.effect(
		"streams generic files to a path with their original extension",
		() =>
			Effect.gen(function* () {
				const config = yield* ServerConfig.ServerConfig;
				...
				expect(
					NodeFS.readFileSync(
						NodePath.join(config.attachmentsDir, `${issued.attachmentId}.pdf`),
					),
	...
	it.effect(
		"removes partial streamed uploads that exceed their signed size",
		() =>
			Effect.gen(function* () {
				const config = yield* ServerConfig.ServerConfig;
				const issued = yield* issueAttachmentUploadUrl(uploadInput);
				...
				expect(
					yield* storeAttachmentUpload(claims, Stream.make(new Uint8Array(7))),
				).toMatchObject({
					ok: false,
					status: 400,
				});
	...
	it.effect(
		"removes partial streamed uploads when the upload is interrupted",
		() =>
			Effect.gen(function* () {
				const config = yield* ServerConfig.ServerConfig;
				const issued = yield* issueAttachmentUploadUrl(uploadInput);
				...
				const body = Stream.make(new Uint8Array([1, 2, 3])).pipe(
					Stream.concat(
						Stream.fromEffect(
							Deferred.succeed(nextChunkRequested, undefined).pipe(
								Effect.andThen(Effect.never),
							),
						),
					),
				);
				const upload = yield* storeAttachmentUpload(claims, body).pipe(
					Effect.forkScoped,
				);
	...
	it.effect(
		"deletes pending uploads without deleting thread-owned copies",
		() =>
			Effect.gen(function* () {
				const config = yield* ServerConfig.ServerConfig;
