...
const attachmentUploadClaimsJson = Schema.fromJsonString(
	AttachmentUploadClaims,
);
const decodeAttachmentUploadClaims = Schema.decodeUnknownOption(
	attachmentUploadClaimsJson,
);
const encodeAttachmentUploadClaims = Schema.encodeSync(
	attachmentUploadClaimsJson,
);
...
function decodeClaims(encodedPayload: string): AttachmentUploadClaims | null {
	try {
		return Option.getOrNull(
			decodeAttachmentUploadClaims(base64UrlDecodeUtf8(encodedPayload)),
		);
	} catch {
		return null;
	}
...
export const issueAttachmentUploadUrl = Effect.fn("AttachmentUpload.issueUrl")(
	function* (input: AttachmentCreateUploadUrlInput) {
		const secret = yield* loadSigningSecret.pipe(
			Effect.mapError(
				(cause) => new AttachmentUploadSigningKeyError({ cause }),
			),
		);
		const config = yield* ServerConfig.ServerConfig;
		const nowMs = yield* Clock.currentTimeMillis;
		const previousSweep = lastPendingSweepByDirectory.get(
...
export const validateAttachmentUploadToken = Effect.fn(
	"AttachmentUpload.validateToken",
	...
	const secret = yield* loadSigningSecret.pipe(
		Effect.tapError((cause) =>
			Effect.logError("Failed to load the attachment upload signing key.", {
				cause,
			}),
		),
	...
	if (
		!secret ||
		!timingSafeEqualBase64Url(signature, signPayload(encodedPayload, secret))
	) {
		return null;
	}
...
export const storeAttachmentUpload = Effect.fn("AttachmentUpload.store")(
	function* (
		...
		return yield* Effect.gen(function* () {
			yield* fileSystem.makeDirectory(path.dirname(finalPath), {
				recursive: true,
			});
			yield* Stream.run(
				bodyStream.pipe(
...
export const deletePendingAttachment = Effect.fn(
	"AttachmentUpload.deletePending",
