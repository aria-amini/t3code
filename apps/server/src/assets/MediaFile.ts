...
class MediaFileOpenError extends Schema.TaggedError<MediaFileOpenError>()(
	"MediaFileOpenError",
	{
		path: Schema.String,
		cause: Schema.Defect(),
...
class MediaFileReadError extends Schema.TaggedError<MediaFileReadError>()(
	"MediaFileReadError",
	{
		path: Schema.String,
		cause: Schema.Defect(),
...
class MediaFileStatError extends Schema.TaggedError<MediaFileStatError>()(
	"MediaFileStatError",
	{
		path: Schema.String,
		cause: Schema.Defect(),
...
const realpathLikeFileSystem = (filePath: string) =>
	new Promise<string>((resolve, reject) => {
		NodeFS.realpath(filePath, (error, resolved) =>
	...
	return yield* Effect.acquireRelease(
		Effect.tryPromise({
			try: async () => {
				...
				if (
					identity &&
					(before.dev.toString() !== identity.device ||
						before.ino.toString() !== identity.inode)
				) {
					return null;
				}
				...
				try {
					...
					if ((await realpathLikeFileSystem(filePath)) !== filePath)
						return null;
...
export const readMediaFileHeader = (
	filePath: string,
	file: OpenMediaFile,
	byteCount: number,
) =>
