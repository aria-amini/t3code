...
const existingFile = Effect.fn("NativeAppIconResolver.existingFile")(function* (
	filePath: string,
) {
...
const commandOutput = Effect.fn("NativeAppIconResolver.commandOutput")(
...
const resolveApplicationPath = Effect.fn(
	"NativeAppIconResolver.resolveApplicationPath",
)(function* (app: ToolActivityNativeAppReference) {
...
const resolveNativeAppIconUncached = Effect.fn(
	"NativeAppIconResolver.resolveUncached",
)(function* (app: ToolActivityNativeAppReference) {
	const fileSystem = yield* FileSystem.FileSystem;
	const path = yield* Path.Path;
	...
	const sourceIconCandidate =
		(iconFileName
			? yield* existingFile(path.join(resourcesDirectory, iconFileName))
			: null) ??
