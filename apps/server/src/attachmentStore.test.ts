...
describe("attachmentStore", () => {
	...
	it("reserves the pending attachment segment", () => {
		...
		expect(
			parseThreadSegmentFromAttachmentId(createAttachmentId("pending")!),
		).toBe("_pending");
		expect(
			parseThreadSegmentFromAttachmentId(createAttachmentId("pending_thread")!),
		).toBe("pending_thread");
	});
	...
	it("preserves safe file extensions in attachment ids and paths", () => {
		...
		expect(
			createAttachmentId("x".repeat(80), ".abcdefghij")?.length,
	...
	it("plans pending attachment claims with direct filename lookups", () => {
		...
		try {
			...
			expect(parseThreadSegmentFromAttachmentId(claim.finalId)).toBe(
				"thread-1",
			);
			expect(parseAttachmentUuid(claim.finalId)).not.toBe(uuid);
			expect(claim.finalPath).toBe(
				NodePath.join(attachmentsDir, `${claim.finalId}.png`),
			);
		} finally {
			NodeFS.rmSync(attachmentsDir, { recursive: true, force: true });
		}
	...
	it("rejects thread-owned attachments even when thread segments collide", () => {
		...
		try {
			...
			NodeFS.writeFileSync(
				NodePath.join(attachmentsDir, `${attachmentId}.png`),
				"pixels",
			);

			expect(
				planAttachmentClaim({ attachmentsDir, threadId: "a b", attachmentId }),
			).toEqual({
				ok: false,
	...
	it("removes expired pending and partial files without touching thread attachments", () => {
		...
		try {
			...
			expect(
				sweepStalePendingAttachments({ attachmentsDir, nowMs: now }),
			).toEqual({ deleted: 3 });
