...
function attachmentIdExtensionSuffix(extension: string | undefined): string {
	...
	return new RegExp(`^${ATTACHMENT_ID_FILE_EXTENSION_PATTERN}$`).test(
		normalized,
	)
		? `-${normalized}`
		: "-bin";
...
export function parseAttachmentFileExtension(
	attachmentId: string,
): string | null {
	const normalizedId = normalizeAttachmentRelativePath(attachmentId);
	if (
...
export function parseThreadSegmentFromAttachmentId(
	attachmentId: string,
): string | null {
	const normalizedId = normalizeAttachmentRelativePath(attachmentId);
	if (
...
export function planAttachmentClaim(input: {
	...
	const requestedSegment = parseThreadSegmentFromAttachmentId(
		input.attachmentId,
	);
	...
	const fileExtension =
		parseAttachmentFileExtension(input.attachmentId) ?? undefined;
...
export function parseAttachmentIdFromRelativePath(
	relativePath: string,
): string | null {
