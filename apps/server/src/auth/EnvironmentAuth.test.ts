...
const makeServerConfigLayer = (
	...
	Layer.effect(
		...
		Layer.provide(
			ServerConfig.layerTest(process.cwd(), { prefix: "t3-auth-server-test-" }),
		),
	);
...
it.layer(NodeServices.layer)("EnvironmentAuth.layer", (it) => {
	it.effect(
		"classifies invalid bootstrap credential failures for the HTTP boundary",
		() =>
			Effect.sync(() => {
	...
	it.effect("prefers a bearer token over a stale legacy cookie", () =>
		Effect.gen(function* () {
			...
			Effect.provide(
				makeEnvironmentAuthLayer({ mode: "web", host: "192.168.1.50" }),
			),
		),
	);

	it.effect(
		"does not exchange ordinary pairing grants for administrative access tokens",
		() =>
			Effect.gen(function* () {
				const serverAuth = yield* EnvironmentAuth.EnvironmentAuth;
				const pairingCredential = yield* serverAuth.issuePairingCredential();
	...
	it.effect(
		"inherits a constrained pairing grant when token exchange omits scope",
		() =>
			Effect.gen(function* () {
				const serverAuth = yield* EnvironmentAuth.EnvironmentAuth;
				const pairingCredential = yield* serverAuth.issuePairingCredential({
	...
	it.effect(
		"rotates desktop bearer sessions without accumulating authorized clients",
		() =>
			Effect.gen(function* () {
				const serverAuth = yield* EnvironmentAuth.EnvironmentAuth;
				const sessions = yield* SessionStore.SessionStore;
				...
				expect(active.map((entry) => entry.sessionId)).toContain(
					browserSession.sessionId,
				);
				expect(active.map((entry) => entry.sessionId)).toContain(
					secondSession.sessionId,
				);
				expect(active.map((entry) => entry.sessionId)).not.toContain(
	...
	it.effect("keeps user-issued administrative pairing links manageable", () =>
		Effect.gen(function* () {
			...
			expect(
				listedPairingLinks.find(
					(pairingLink) => pairingLink.id === pairingCredential.id,
				)?.subject,
			).toBe("one-time-token");
		}).pipe(Effect.provide(makeEnvironmentAuthLayer())),
	);
	...
	it.effect(
		"issues startup pairing URLs that bootstrap administrative sessions",
		() =>
			Effect.gen(function* () {
				const serverAuth = yield* EnvironmentAuth.EnvironmentAuth;
				const sessions = yield* SessionStore.SessionStore;
				...
				const token = new URLSearchParams(
					new URL(pairingUrl).hash.slice(1),
				).get("token");
				const listedPairingLinks = yield* serverAuth.listPairingLinks();
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				expect(listedPairingLinks.map((entry) => entry.id)).toContain(
					pairingCredential.id,
				);
				expect(
					listedPairingLinks.find((entry) => entry.id === pairingCredential.id)
						?.label,
				).toBe("Julius iPhone");
				expect(clientsBeforeRevoke).toHaveLength(2);
				expect(
					clientsBeforeRevoke.find(
						(entry) => entry.sessionId === administrativeSession.sessionId,
					)?.current,
				).toBe(true);
				expect(
					clientsBeforeRevoke.find(
						(entry) => entry.sessionId === clientSession.sessionId,
					)?.current,
				).toBe(false);
				expect(
					clientsBeforeRevoke.find(
				...
				expect(clientsAfterRevoke[0]?.sessionId).toBe(
					administrativeSession.sessionId,
				);
			}).pipe(
				Effect.provide(
