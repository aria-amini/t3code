...
export type ServerAuthInvalidRequestError =
	typeof ServerAuthInvalidRequestError.Type;
export const isServerAuthInvalidRequestError = Schema.is(
...
export class EnvironmentAuth extends Context.Service<
	EnvironmentAuth,
	{
		...
		readonly revokeClientSession: (
			...
			boolean,
			ServerAuthForbiddenOperationError | ServerAuthInternalError
		>;
...
const bySessionPriority = (
	left: AuthClientSession,
	right: AuthClientSession,
) => {
...
function parseDpopToken(
	request: HttpServerRequest.HttpServerRequest,
): string | null {
	const header = request.headers["authorization"];
	if (
		typeof header !== "string" ||
		!header.startsWith(DPOP_AUTHORIZATION_PREFIX)
	) {
		return null;
	}
...
export function selectRequestCredential(
	...
	const legacyToken = legacyCookieName
		? request.cookies[legacyCookieName]
		: undefined;
...
export const make = Effect.gen(function* () {
	...
	const getSessionState: EnvironmentAuth["Service"]["getSessionState"] = (
		request,
	) =>
				...
				Effect.flatMap((grant) =>
					sessions
						...
						.pipe(
							Effect.mapError(
								(cause) =>
									new ServerAuthAuthenticatedSessionIssueError({ cause }),
							),
						),
				),
				Effect.map(
				...
				Effect.flatMap((grant) =>
					Effect.gen(function* () {
						...
						return yield* sessions
							.issue({
								method: input?.proofKeyThumbprint
									? "dpop-access-token"
									: "bearer-access-token",
								subject: grant.subject,
								...
								replaceActiveForSubjectAndMethod:
	...
	const createPairingLink: EnvironmentAuth["Service"]["createPairingLink"] =
		Effect.fn("EnvironmentAuth.createPairingLink")(
			function* (input) {
			...
			Effect.mapError(
				(cause) => new ServerAuthPairingLinkCreationError({ cause }),
			),
		);

	const listPairingLinks: EnvironmentAuth["Service"]["listPairingLinks"] = (
		input,
	) =>
		bootstrapCredentials.listActive().pipe(
			Effect.map((pairingLinks) => {
				...
				return pairingLinks
					.filter(
						(pairingLink) => !excludedSubjects.includes(pairingLink.subject),
					)
					.toSorted(
						(left, right) =>
							right.createdAt.epochMilliseconds -
							left.createdAt.epochMilliseconds,
					);
			}),
			Effect.mapError(
				(cause) => new ServerAuthPairingLinksListError({ cause }),
			),
			Effect.withSpan("EnvironmentAuth.listPairingLinks"),
		);

	const revokePairingLink: EnvironmentAuth["Service"]["revokePairingLink"] = (
		id,
	) =>
		bootstrapCredentials.revoke(id).pipe(
			Effect.mapError(
				(cause) => new ServerAuthPairingLinkRevocationError({ cause }),
			),
	...
	const issueSession: EnvironmentAuth["Service"]["issueSession"] = (input) =>
		sessions
			...
			.pipe(
				...
				Effect.mapError(
					(cause) => new ServerAuthSessionTokenIssueError({ cause }),
				),
	...
	const revokeSession: EnvironmentAuth["Service"]["revokeSession"] = (
		sessionId,
	) =>
		sessions.revoke(sessionId).pipe(
			Effect.mapError(
				(cause) => new ServerAuthSessionRevocationError({ cause }),
			),
	...
	const issuePairingCredential: EnvironmentAuth["Service"]["issuePairingCredential"] =
		(input) =>
	...
	const listClientSessions: EnvironmentAuth["Service"]["listClientSessions"] = (
		currentSessionId,
	) =>
		listSessions().pipe(
	...
	const revokeClientSession: EnvironmentAuth["Service"]["revokeClientSession"] =
		Effect.fn("EnvironmentAuth.revokeClientSession")(
	...
	const issueWebSocketTicket: EnvironmentAuth["Service"]["issueWebSocketTicket"] =
		(session) =>
			sessions.issueWebSocketToken(session.sessionId).pipe(
				Effect.mapError(
					(cause) => new ServerAuthWebSocketTokenIssueError({ cause }),
				),
				Effect.map(
					(issued) =>
						({
	...
	const authenticateWebSocketUpgrade: EnvironmentAuth["Service"]["authenticateWebSocketUpgrade"] =
		Effect.fn("EnvironmentAuth.authenticateWebSocketUpgrade")(
			function* (request) {
				const requestUrl = HttpServerRequest.toURL(request);
				if (Option.isSome(requestUrl)) {
					const websocketTicket = requestUrl.value.searchParams.get(
	...
	return EnvironmentAuth.of({
		getDescriptor: () =>
			Effect.succeed(descriptor).pipe(
				Effect.withSpan("EnvironmentAuth.getDescriptor"),
			),
		getSessionState,
...
export const storageLayer = Layer.mergeAll(
