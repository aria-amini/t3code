...
it.layer(NodeServices.layer)(
	"EnvironmentAuth administrative operations",
	(it) => {
		...
		it.effect(
			"issues bearer access token sessions without exposing raw tokens",
			() =>
				Effect.gen(function* () {
					const environmentAuth = yield* EnvironmentAuth.EnvironmentAuth;
					...
					const revoked = yield* environmentAuth.revokeSession(
						issued.sessionId,
					);
