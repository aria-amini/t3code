...
it.layer(NodeServices.layer)("EnvironmentAuthPolicy.layer", (it) => {
	...
	it.effect(
		"keeps desktop cookies port-scoped on the port a second instance lands on",
		() =>
			Effect.gen(function* () {
				const policy = yield* EnvironmentAuthPolicy.EnvironmentAuthPolicy;
				const descriptor = yield* policy.getDescriptor();
	...
	it.effect(
		"uses remote-reachable policy for desktop mode when bound beyond loopback",
		() =>
			Effect.gen(function* () {
				const policy = yield* EnvironmentAuthPolicy.EnvironmentAuthPolicy;
				const descriptor = yield* policy.getDescriptor();
	...
	it.effect("uses loopback-browser policy for loopback web hosts", () =>
		Effect.gen(function* () {
			...
			expect(descriptor.sessionCookieName).toMatch(
				/^t3_session_3773_[a-f0-9]{12}$/,
			);
		}).pipe(
			Effect.provide(
				makeEnvironmentAuthPolicyLayer({
