...
const makeServerConfigLayer = (
	overrides?: Partial<
		Pick<ServerConfig.ServerConfig["Service"], "desktopBootstrapToken">
	>,
) =>
	Layer.effect(
...
it.layer(NodeServices.layer)("PairingGrantStore.layer", (it) => {
	...
	it.effect(
		"issues one-time bootstrap tokens that can only be consumed once",
		() =>
			Effect.gen(function* () {
				const bootstrapCredentials = yield* PairingGrantStore.PairingGrantStore;
				const issued = yield* bootstrapCredentials.issueOneTimeToken({
				...
				const second = yield* Effect.flip(
					bootstrapCredentials.consume(issued.credential),
				);
	...
	it.effect(
		"atomically consumes a one-time token when multiple requests race",
		() =>
			Effect.gen(function* () {
				const bootstrapCredentials = yield* PairingGrantStore.PairingGrantStore;
	...
	it.effect("seeds the desktop bootstrap credential as a reusable grant", () =>
		Effect.gen(function* () {
			const bootstrapCredentials = yield* PairingGrantStore.PairingGrantStore;
			const first = yield* bootstrapCredentials.consume(
				"desktop-bootstrap-token",
			);
			const second = yield* bootstrapCredentials.consume(
				"desktop-bootstrap-token",
			);
			const third = yield* bootstrapCredentials.consume(
				"desktop-bootstrap-token",
			);
	...
	it.effect(
		"reports seeded desktop bootstrap credentials as expired after their ttl",
		() =>
			Effect.gen(function* () {
				const bootstrapCredentials = yield* PairingGrantStore.PairingGrantStore;
				...
				const stillValid = yield* bootstrapCredentials.consume(
					"desktop-bootstrap-token",
				);
				...
				const expired = yield* Effect.flip(
					bootstrapCredentials.consume("desktop-bootstrap-token"),
				);
	...
	it.effect("keeps credentials out of pairing lists and change events", () =>
		Effect.gen(function* () {
			...
			for (const input of [{}, { label: "Synthetic phone" }]) {
				...
				const listed = (yield* grants.listActive()).find(
					(link) => link.id === issued.id,
				);
	...
	it.effect("lists and revokes active pairing links", () =>
		Effect.gen(function* () {
			...
			const revokedConsume = yield* Effect.flip(
				bootstrapCredentials.consume(first.credential),
			);

			expect(revoked).toBe(true);
			expect(activeAfterRevoke.map((entry) => entry.id)).not.toContain(
				first.id,
			);
	...
	it.effect(
		"identifies consume-available failures and preserves their cause",
		() => {
