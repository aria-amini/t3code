...
export type BootstrapCredentialInvalidError =
	typeof BootstrapCredentialInvalidError.Type;
export const isBootstrapCredentialInvalidError = Schema.is(
...
export type BootstrapCredentialInternalError =
	typeof BootstrapCredentialInternalError.Type;
export const isBootstrapCredentialInternalError = Schema.is(
...
const PAIRING_TOKEN_REJECTION_LIMIT =
	Math.floor(256 / PAIRING_TOKEN_ALPHABET.length) *
	PAIRING_TOKEN_ALPHABET.length;

export const make = Effect.gen(function* () {
	const crypto = yield* Crypto.Crypto;
	const config = yield* ServerConfig.ServerConfig;
	const pairingLinks = yield* AuthPairingLinks.AuthPairingLinkRepository;
	const seededGrantsRef = yield* Ref.make(
		new Map<string, StoredBootstrapGrant>(),
	);
	...
	const generatePairingToken = Effect.gen(function* () {
		...
		while (credential.length < PAIRING_TOKEN_LENGTH) {
			const bytes = yield* crypto.randomBytes(PAIRING_TOKEN_LENGTH).pipe(
				Effect.mapError(
					(cause) =>
						new PairingCredentialRandomGenerationError({
			...
			for (const byte of bytes) {
				...
				credential +=
					PAIRING_TOKEN_ALPHABET[byte % PAIRING_TOKEN_ALPHABET.length]!;
	...
	const revoke: PairingGrantStore["Service"]["revoke"] = Effect.fn(
		"PairingGrantStore.revoke",
	)(function* (id) {
		...
		const revoked = yield* pairingLinks
			...
			.pipe(
				Effect.mapError(
					(cause) => new PairingLinkRevokeError({ pairingLinkId: id, cause }),
				),
			);
		if (revoked) {
	...
	const issueOneTimeToken: PairingGrantStore["Service"]["issueOneTimeToken"] =
		Effect.fn("PairingGrantStore.issueOneTimeToken")(function* (input) {
			...
			const isDevStartupToken =
				config.devUrl !== undefined && input?.purpose === "startup";
			const ttl =
				input?.ttl ??
				(isDevStartupToken
			...
			const expiresAt = DateTime.add(now, {
				milliseconds: Duration.toMillis(ttl),
			});
	...
	const consume: PairingGrantStore["Service"]["consume"] = Effect.fn(
		"PairingGrantStore.consume",
		...
		const seededResult: ConsumeResult = yield* Ref.modify(
			...
			(
				current,
			): readonly [ConsumeResult, Map<string, StoredBootstrapGrant>] => {
				...
				if (
					...
					grant.proofKeyThumbprint !== input?.proofKeyThumbprint
				) {
					return [
						{
							_tag: "error",
		...
		const consumed = yield* pairingLinks
			...
			.pipe(
				Effect.mapError(
					(cause) => new BootstrapCredentialConsumeAvailableError({ cause }),
				),
			);
		...
		const matching = yield* pairingLinks
			...
			.pipe(
				Effect.mapError(
					(cause) => new BootstrapCredentialLookupError({ cause }),
				),
			);
