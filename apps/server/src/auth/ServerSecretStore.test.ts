...
it.layer(NodeServices.layer)("ServerSecretStore.layer", (it) => {
	...
	it.effect("reuses an existing secret instead of regenerating it", () =>
		Effect.gen(function* () {
			...
			const first = yield* secretStore.getOrCreateRandom(
				"session-signing-key",
				32,
			);
			const second = yield* secretStore.getOrCreateRandom(
				"session-signing-key",
				32,
			);
	...
	it.effect(
		"uses restrictive permissions for the secret directory and files",
		() =>
			Effect.gen(function* () {
				const chmodCalls: Array<{
				...
				const secretStore = yield* Effect.service(
				...
				assert.isTrue(
					chmodCalls.some(
						(call) => call.mode === 0o700 && /[\\/]secrets$/.test(call.path),
					),
				);
				assert.isAtLeast(
					chmodCalls.filter((call) => call.mode === 0o600).length,
					2,
				);
	...
	it.effect("propagates read failures other than missing-file errors", () =>
		Effect.gen(function* () {
			...
			assert.include(
				error.message,
				"Failed to read secret session-signing-key.",
			);
	...
	it.effect(
		"propagates write failures instead of treating them as success",
		() =>
			Effect.gen(function* () {
				const secretStore = yield* ServerSecretStore.ServerSecretStore;
				...
				assert.include(
					error.message,
					"Failed to persist secret session-signing-key.",
				);
				assert.instanceOf(error.cause, PlatformError.PlatformError);
				assert.equal(
					(error.cause as PlatformError.PlatformError).reason._tag,
	...
	it.effect("propagates remove failures other than missing-file errors", () =>
		Effect.gen(function* () {
			...
			const error = yield* Effect.flip(
				secretStore.remove("session-signing-key"),
			);

			assert.instanceOf(error, ServerSecretStore.SecretStoreRemoveError);
			assert.include(
				error.message,
				"Failed to remove secret session-signing-key.",
			);
			assert.instanceOf(error.cause, PlatformError.PlatformError);
			assert.equal(
				(error.cause as PlatformError.PlatformError).reason._tag,
