...
const isPlatformError = (
	value: unknown,
): value is PlatformError.PlatformError =>
...
export class ServerSecretStore extends Context.Service<
	ServerSecretStore,
	{
		readonly get: (
			name: string,
		) => Effect.Effect<Option.Option<Uint8Array>, SecretStoreError>;
		readonly set: (
			name: string,
			value: Uint8Array,
...
export const make = Effect.gen(function* () {
	...
	const resolveSecretPath = (name: string) =>
		path.join(serverConfig.secretsDir, `${name}.bin`);
	...
	const getOrCreateRandom: ServerSecretStore["Service"]["getOrCreateRandom"] = (
		name,
		bytes,
	) =>
		get(name).pipe(
