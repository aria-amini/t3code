...
const failingSessionLookupRepositoryLayer = Layer.succeed(
...
const failingSessionLookupCredentialLayer = Layer.effect(
	...
	Layer.provide(SqlitePersistenceMemory),
	Layer.provide(
		makeServerEnvironmentLayer(EnvironmentId.make("test-environment")),
	),
...
it.layer(NodeServices.layer)("SessionStore.layer", (it) => {
	it.effect(
		"keys remote cookies by environment identity instead of state directory",
		() =>
			Effect.gen(function* () {
					...
					Effect.gen(function* () {
						...
						Effect.provide(
							makeSessionStoreLayer(
								{ mode: "web", host: "192.168.1.50", stateDir },
								environmentId,
							),
						),
					);
	...
	it.effect(
		"preserves repository failures while verifying session and websocket credentials",
		() =>
			Effect.gen(function* () {
				const sessions = yield* SessionStore.SessionStore;
				const issued = yield* sessions.issue({
				...
				const websocketError = yield* Effect.flip(
					sessions.verifyWebSocketToken(websocket.token),
				);
				const revokeError = yield* Effect.flip(
					sessions.revoke(issued.sessionId),
				);
				const revokeOthersError = yield* Effect.flip(
					sessions.revokeAllExcept(issued.sessionId),
				);
	...
	it.effect(
		"atomically replaces active sessions with the same subject and method",
		() =>
			Effect.gen(function* () {
				const sessions = yield* SessionStore.SessionStore;
				...
				expect(
					active.find((entry) => entry.sessionId === browser.sessionId),
				).toBeDefined();
				expect(
					active.filter(
						(entry) =>
							entry.subject === "desktop-bootstrap" &&
							entry.method === "bearer-access-token",
					),
				).toHaveLength(1);
	...
	it.effect(
		"keeps the previous desktop session valid when replacement fails",
		() =>
			Effect.gen(function* () {
				const sessions = yield* SessionStore.SessionStore;
      ...
      `;
				...
				expect((yield* sessions.verify(previous.token)).sessionId).toBe(
					previous.sessionId,
				);
				expect(
					(yield* sessions.listActive()).map((session) => session.sessionId),
				).toEqual([previous.sessionId]);
			}).pipe(
				Effect.provide(
					Layer.mergeAll(makeSessionStoreLayer(), SqlitePersistenceMemory),
				),
			),
	);

	it.effect(
		"rejects websocket tokens once the parent session has expired",
		() =>
			Effect.gen(function* () {
				const sessions = yield* SessionStore.SessionStore;
				const issued = yield* sessions.issue({
				...
				if (error._tag === "WebSocketSessionExpiredError") {
					...
					expect(error.expiresAt.epochMilliseconds).toBe(
	...
	it.effect(
		"includes expiry context when session and websocket tokens expire",
		() =>
			Effect.gen(function* () {
				const sessions = yield* SessionStore.SessionStore;
				const issued = yield* sessions.issue({
				...
				const websocket = yield* sessions.issueWebSocketToken(
					issued.sessionId,
					{
						ttl: Duration.seconds(1),
					},
				...
				if (sessionError._tag === "SessionTokenExpiredError") {
					expect(sessionError.sessionId).toBe(issued.sessionId);
					expect(sessionError.expiresAt.epochMilliseconds).toBe(
						issued.expiresAt.epochMilliseconds,
					);
	...
	it.effect(
		"lists active sessions, tracks connectivity, and revokes other sessions",
		() =>
			Effect.gen(function* () {
				const sessions = yield* SessionStore.SessionStore;
				...
				const clientWebSocket = yield* sessions.issueWebSocketToken(
					client.sessionId,
				);

				yield* sessions.markConnected(client.sessionId);
				const beforeRevoke = yield* sessions.listActive();
				const revokedCount = yield* sessions.revokeAllExcept(
					administrative.sessionId,
				);
				...
				expect(
					beforeRevoke.find((entry) => entry.sessionId === client.sessionId)
						?.connected,
				).toBe(true);
				expect(
					beforeRevoke.find((entry) => entry.sessionId === client.sessionId)
				...
				expect(
					beforeRevoke.find(
						(entry) => entry.sessionId === administrative.sessionId,
					)?.client.deviceType,
				).toBe("desktop");
				...
				if (revokedClient._tag === "SessionTokenRevokedError") {
					...
					expect(
						revokedClient.revokedAt.epochMilliseconds,
					).toBeGreaterThanOrEqual(0);
				}
				expect(revokedClientWebSocket._tag).toBe(
				...
				if (revokedClientWebSocket._tag === "WebSocketSessionRevokedError") {
					...
					expect(
						revokedClientWebSocket.revokedAt.epochMilliseconds,
					).toBeGreaterThanOrEqual(0);
				}
			}).pipe(Effect.provide(makeSessionStoreLayer())),
	);
	...
	it.effect(
		"persists lastConnectedAt on first connect and updates it after reconnect",
		() =>
			Effect.gen(function* () {
				const sessions = yield* SessionStore.SessionStore;
				const issued = yield* sessions.issue({
				...
				expect(stillConnected[0]?.lastConnectedAt?.toString()).toBe(
				...
				expect(afterDisconnect[0]?.lastConnectedAt?.toString()).toBe(
					firstConnectedAt?.toString(),
				);
				...
				expect(afterReconnect[0]?.lastConnectedAt?.toString()).not.toBe(
					firstConnectedAt?.toString(),
				);
			}).pipe(
				Effect.provide(Layer.merge(makeSessionStoreLayer(), TestClock.layer())),
	...
	it.effect(
		"keeps connected relay sessions visible through expiry and HTTP renewal",
		() =>
			Effect.gen(function* () {
				const sessions = yield* SessionStore.SessionStore;
				...
				expect(
					yield* Effect.flip(sessions.verify(original.token)),
				).toMatchObject({
					_tag: "SessionTokenExpiredError",
				});
				expect(
					yield* Effect.flip(sessions.verifyWebSocketToken(websocket.token)),
				).toMatchObject({
	...
	it.effect.each([1, 2])(
		...
		(socketCount) =>
			Effect.gen(function* () {
				...
				const changes =
					yield* Queue.unbounded<SessionStore.SessionCredentialChange>();
		...
		(operation) =>
			Effect.gen(function* () {
				...
				if (operation === "revoke") {
					...
					expect(
						yield* sessions.revokeAllExcept(administrative.sessionId),
					).toBe(1);
				}
	...
	it.effect(
		"records client connection metadata without clearing prior values",
		() =>
			Effect.gen(function* () {
				const sessions = yield* SessionStore.SessionStore;
				const sql = yield* SqlClient.SqlClient;
