...
const appendCredentialResponseHeaders = HttpEffect.appendPreResponseHandler(
	(_request, response) =>
		Effect.succeed(
			HttpServerResponse.setHeaders(response, CREDENTIAL_RESPONSE_HEADERS),
		),
);

const appendDpopChallengeHeader = HttpEffect.appendPreResponseHandler(
	(_request, response) =>
		Effect.succeed(
...
const appendDpopChallengeOnUnauthorized = (
	error: EnvironmentAuthInvalidError,
) =>
	Effect.gen(function* () {
		const request = yield* HttpServerRequest.HttpServerRequest;
		const usesDpop =
			(request.originalUrl.startsWith("/oauth/token") &&
				request.headers.dpop !== undefined) ||
...
export function failEnvironmentScopeRequired(
...
export function failEnvironmentNotFound(
...
export function failEnvironmentInternal(
...
const appendSessionCookie = (
	...
	Effect.fromResult(
		...
		Effect.catch(() =>
			failEnvironmentInternal("browser_session_cookie_failed"),
		),
...
export const requireEnvironmentScope = Effect.fn(
	"environment.auth.requireScope",
	...
	Effect.gen(function* () {
		...
		return (httpEffect) =>
			Effect.gen(function* () {
				...
				Effect.catchTag(
					"EnvironmentAuthInvalidError",
					appendDpopChallengeOnUnauthorized,
				),
			);
	}),
);
		...
		return handlers
			...
			.handle(
				...
				Effect.fn("environment.auth.token")(
					function* (args) {
						...
						const proofKeyThumbprint = args.headers.dpop
							? yield* verifyRequestDpopProof({ request }).pipe(
									...
									Effect.catchIf(
										EnvironmentAuth.isServerAuthInternalError,
										(error) =>
					...
					Effect.catchIf(
						EnvironmentAuth.isServerAuthInvalidRequestError,
						(error) =>
							failEnvironmentInvalidRequest(
								EnvironmentAuth.serverAuthInvalidRequestReason(error),
							),
					),
					Effect.catchIf(EnvironmentAuth.isServerAuthInternalError, (error) =>
				...
				Effect.fn("environment.auth.pairingCredential")(
					function* (args) {
						...
						const delegatedScopes =
							args.payload.scopes ?? AuthStandardClientScopes;
						if (
							delegatedScopes.length === 0 ||
							new Set<AuthEnvironmentScope>(delegatedScopes).size !==
								delegatedScopes.length
						) {
							return yield* failEnvironmentInvalidRequest("invalid_scope");
						}
			...
			.handle(
				...
				Effect.fn("environment.auth.revokePairingLink")(
					function* (args) {
						yield* annotateEnvironmentRequest(args.endpoint.name);
						yield* requireEnvironmentScope(AuthAccessWriteScope);
						const revoked = yield* serverAuth.revokePairingLink(
							args.payload.id,
						);
						return { revoked };
					},
