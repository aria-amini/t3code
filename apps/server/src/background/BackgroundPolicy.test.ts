...
function makeReport(
	overrides: Partial<ClientActivityReportInput> = {},
...
describe("BackgroundPolicy", () => {
	it.effect("records foreground scoped client demand", () =>
		Effect.gen(function* () {
			...
			assert.equal(
				yield* policy.hasDemand({ type: "vcs-status", cwd: "/repo" }),
				true,
			);
			assert.equal(
				yield* policy.hasDemand({ type: "vcs-status", cwd: "/other" }),
				false,
			);
			assert.equal(
			...
			assert.equal(
				yield* policy.shouldRunScopeWork({ type: "vcs-status", cwd: "/other" }),
				false,
			);
	...
	it.effect(
		"keeps leases from another session when rpc client ids are reused",
		() =>
			Effect.gen(function* () {
				const policy = yield* BackgroundPolicy.BackgroundPolicy;
	...
	it.effect("serializes lease mutation publications", () =>
		Effect.gen(function* () {
			...
			const layer = BackgroundPolicy.layer.pipe(
				Layer.provide(
					Layer.merge(hostLayer, ServerSettingsService.layerTest()),
				),
			);
	...
	it.effect(
		"host low power mode disables opportunistic work without dropping scoped demand",
		() =>
			Effect.gen(function* () {
				const policy = yield* BackgroundPolicy.BackgroundPolicy;
				yield* policy.reportClientActivity(
				...
				assert.equal(
					yield* policy.shouldRunScopeWork({
						type: "vcs-status",
						cwd: "/repo",
					}),
					false,
	...
	it.effect(
		"keeps background demand visible while preventing scoped work",
		() =>
			Effect.gen(function* () {
				const policy = yield* BackgroundPolicy.BackgroundPolicy;
				yield* policy.reportClientActivity(
	...
	it.effect(
		"keeps scoped work active after a recently used visible window loses focus",
		() =>
			Effect.gen(function* () {
				const policy = yield* BackgroundPolicy.BackgroundPolicy;
				yield* policy.reportClientActivity(
	...
	it.effect(
		"pauses scoped work for a visible unfocused client without recent interaction",
		() =>
			Effect.gen(function* () {
				const policy = yield* BackgroundPolicy.BackgroundPolicy;
				yield* policy.reportClientActivity(
