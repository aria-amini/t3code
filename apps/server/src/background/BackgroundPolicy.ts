	...
	if (!next.has(key)) {
		...
		for (const [currentKey, current] of next) {
			if (
				...
				current.rpcClientId !== lease.rpcClientId
			) {
				continue;
			}
			...
			if (
				...
				updatedAtMs < oldestConnectionLease.updatedAtMs
			) {
...
function isHostConstrained(
	...
	if (settings.pauseWhenHostLowPower && hostPower.lowPowerMode === "true")
		return true;
...
function isClientConstrained(
	...
	if (settings.pauseWhenClientLowPower && lease.lowPowerMode === "true")
		return true;
...
function leaseMayRunScopedWork(
	...
	const activeWithScope =
		isLeaseActive(lease, now) && leaseHasScope(lease, scope);
...
function computeSnapshot(input: {
	...
	const foregroundLeases = activeLeases.filter((lease) =>
		isForegroundLease(lease, input.now),
	);
	...
	return {
		...
		shouldRunOpportunisticWork:
			foregroundLeases.some(
				(lease) => !isClientConstrained(lease, input.settings),
...
export const make = Effect.fn("background.policy.make")(function* () {
				...
				Effect.gen(function* () {
					...
					const lease: ClientActivityLease = {
						...
						...(input.appState !== undefined
							...
							: {}),
						...(input.lowPowerMode !== undefined
							? { lowPowerMode: input.lowPowerMode }
							: {}),
						...(input.batteryState !== undefined
					...
					yield* Ref.update(leasesRef, (leases) =>
						upsertClientActivityLease(leases, lease, now),
					);
	...
	const hasDemand: BackgroundPolicy["Service"]["hasDemand"] = (scope) =>
		Effect.map(snapshot, (current) =>
			current.activeScopeKeys.includes(scopeKey(scope)),
		);

	const shouldRunScopeWork: BackgroundPolicy["Service"]["shouldRunScopeWork"] =
		(scope) =>
			Effect.gen(function* () {
				const [current, settings] = yield* Effect.all([
					snapshot,
					backgroundActivitySettings,
				]);
	...
	yield* Stream.runForEach(
		hostPowerMonitor.streamChanges,
		() => publishSnapshot,
