...
describe("HostPowerMonitor", () => {
	it.effect(
		"publishes semantic power changes without idle-time heartbeat churn",
		() =>
			Effect.gen(function* () {
				...
				const nextChange = yield* Stream.runHead(monitor.streamChanges).pipe(
					Effect.forkChild,
				);
				...
				expect(Option.getOrThrow(yield* Fiber.join(nextChange)).locked).toBe(
					"true",
				);
			}),
	);
	...
	it.effect(
		"ignores host power reports older than the latest accepted snapshot",
		() =>
			Effect.gen(function* () {
				const initial = {
					source: "electron-main",
				...
				expect(DateTime.toEpochMillis(snapshot.updatedAt)).toBe(
					DateTime.toEpochMillis(latestAt),
				);
			}),
	);

	it.effect(
		"consumes desktop power directly without retaining diagnostics telemetry",
		() =>
			Effect.gen(function* () {
				const sampledAt = DateTime.makeUnsafe("2026-06-17T12:00:00.000Z");
				const desktopChanges =
					yield* PubSub.sliding<DesktopHostTelemetrySnapshot>(1);
				...
				const receiverLayer = DesktopTelemetryReceiver.layerTest({
					...
					setDiagnosticsDemand: () =>
						Ref.update(diagnosticsDemandWrites, (count) => count + 1),
				});
				...
				yield* Effect.gen(function* () {
					...
					const nextPower = yield* Stream.runHead(monitor.streamChanges).pipe(
						Effect.forkChild,
					);
					yield* Effect.yieldNow;
					...
					expect(
						Option.getOrThrow(yield* Fiber.join(nextPower)).onBattery,
					).toBe("true");
