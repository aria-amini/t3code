...
function samePowerState(
	left: HostPowerSnapshot,
	right: HostPowerSnapshot,
): boolean {
	return (
...
export const make = Effect.fn("background.hostPower.make")(function* (
	...
	const initial =
		initialSnapshot ?? makeUnknownSnapshot("unknown", yield* DateTime.now);
	...
	const report: HostPowerMonitor["Service"]["report"] = (snapshot) =>
		Ref.modify(latestRef, (current) => {
			...
			return [
				samePowerState(current, snapshot)
					? Option.none()
