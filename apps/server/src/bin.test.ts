...
const makeProjectLookupFixture = Effect.fn("makeProjectLookupFixture")(
	function* (withThread: boolean, removeWorkspace: boolean) {
		...
		NodeChildProcess.execFileSync(
			"git",
			["init", "--initial-branch=main", workspaceRoot],
			{
				stdio: "ignore",
			},
		);
...
it.layer(NodeServices.layer)(
	"project lookup with unavailable workspaces",
	(it) => {
		it.effect(
			"removes an empty project by ID without force after its directory is gone",
			() =>
				Effect.gen(function* () {
					...
					assert.isNotNull(
						after.projects.find((candidate) => candidate.id === project.id)!
							.deletedAt,
					);
				}),
		);
		...
		it.effect.each([true, false])(
			...
			(removeWorkspace) =>
				Effect.gen(function* () {
					...
					assert.isNull(
						retained.projects.find((candidate) => candidate.id === project.id)!
							.deletedAt,
					);
					assert.isNull(
						retained.threads.find(
							(thread) => thread.id === "thread-project-lookup",
						)!.deletedAt,
					);
					yield* runCliWithRuntime([
						"project",
		...
		it.effect(
			"cannot remove the old environment's ID from a replacement empty database",
			() =>
				Effect.gen(function* () {
					const { baseDir, project } = yield* makeProjectLookupFixture(
						true,
					...
					assert.isNull(
						original.projects.find((candidate) => candidate.id === project.id)!
							.deletedAt,
					);
		...
		it.effect(
			"renames by ID and stored path, then force removes after the directory is gone",
			() =>
				Effect.gen(function* () {
					const { baseDir, workspaceRoot, project } =
						yield* makeProjectLookupFixture(true, true);
					yield* runCliWithRuntime([
						"project",
						"rename",
						project.id,
		...
		it.effect(
			"preserves normalized paths and distinct symlink project entries",
			() =>
				Effect.gen(function* () {
					const { baseDir, workspaceRoot, project } =
						yield* makeProjectLookupFixture(false, false);
					...
					assert.equal(
						renamed.projects.find((candidate) => candidate.id === project.id)!
							.title,
						"Normalized",
					);
					...
					assert.isNotNull(
						after.projects.find(
							(candidate) => candidate.id === aliasProject.id,
						)!.deletedAt,
					);
					assert.isNull(
						after.projects.find((candidate) => candidate.id === project.id)!
...
it.layer(NodeServices.layer)("bin cli parsing", (it) => {
	...
	it.effect("accepts canonical --no-<flag> boolean negation", () =>
		Effect.gen(function* () {
			const { output } = yield* captureStdout(
				runCli(["--no-log-websocket-events", "--version"]),
			);
	...
	it.effect(
		"rejects invalid log-level casing before launching the server",
		() =>
			Effect.gen(function* () {
				const error = yield* runCliWithRuntime(["--log-level", "Debug"]).pipe(
					Effect.flip,
				);
	...
	it.effect(
		"rejects connect commands when public configuration is missing",
		() =>
			Effect.gen(function* () {
				const error = yield* runCli(["connect", "status"], noConnectCli).pipe(
					Effect.flip,
				);
				...
				assert.include(
					...
					"missing T3 Connect public configuration",
				);
				...
				Effect.provide(Layer.mergeAll(CliRuntimeLayer, TestConsole.layer)),
			),
	);

	it.effect(
		"exposes service lifecycle commands without T3 Connect configuration",
		() =>
			Effect.gen(function* () {
				const { output } = yield* captureStdout(
					runCli(["service", "--help"], noConnectCli),
				);
	...
	it.effect(
		"accepts the --headless login override without enabling access",
		() =>
			Effect.gen(function* () {
				const baseDir = NodeFS.mkdtempSync(
					NodePath.join(NodeOS.tmpdir(), "t3-cli-cloud-login-test-"),
				);
				const { secretsDir } = yield* ServerConfig.deriveServerPaths(
					baseDir,
					undefined,
				);
	...
	it.effect(
		"logs out of headless connect and removes the stored CLI authorization",
		() =>
			Effect.gen(function* () {
				const baseDir = NodeFS.mkdtempSync(
	...
	it.effect(
		"executes auth pairing subcommands and redacts secrets from list output",
		() =>
			Effect.gen(function* () {
				const baseDir = NodeFS.mkdtempSync(
	...
	it.effect(
		"executes auth session subcommands and redacts secrets from list output",
		() =>
			Effect.gen(function* () {
				const baseDir = NodeFS.mkdtempSync(
	...
	it.effect(
		"adds, renames, and removes projects offline through the orchestration engine",
		() =>
			Effect.gen(function* () {
				const baseDir = NodeFS.mkdtempSync(
	...
	it.effect("force removes projects that still contain threads", () =>
		Effect.gen(function* () {
			...
			const workspaceRoot = NodeFS.mkdtempSync(
				NodePath.join(
					NodeOS.tmpdir(),
					"t3-cli-projects-force-remove-workspace-",
				),
			);
			...
			const afterRemove = yield* readPersistedSnapshot(baseDir);
			assert.isTrue(
				(afterRemove.projects.find((candidate) => candidate.id === project!.id)
					?.deletedAt ?? null) !== null,
			);
			assert.isTrue(
				(afterRemove.threads.find(
					(thread) => thread.id === "thread-cli-force-remove",
				)?.deletedAt ?? null) !== null,
			);
		}),
	);
	...
	it.effect(
		"routes project commands through a running server when runtime state is present",
		() =>
			Effect.gen(function* () {
				const baseDir = NodeFS.mkdtempSync(
	...
	it.effect("rejects dev-url on project commands", () =>
		Effect.gen(function* () {
			const workspaceRoot = NodeFS.mkdtempSync(
				NodePath.join(
					NodeOS.tmpdir(),
					"t3-cli-projects-unknown-option-workspace-",
				),
			);
			const error = yield* runCliWithRuntime([
				"project",
			...
			if (!optionError || optionError._tag !== "UnrecognizedOption") {
				assert.fail(
					`Expected UnrecognizedOption, got ${String(optionError?._tag)}`,
				);
			}
