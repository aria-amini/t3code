		...
		openSync: (...args: Parameters<typeof actual.openSync>) => {
			...
			if (
				...
				const error = new Error(
					`open failed with ${openSyncInterceptor.errorCode}`,
				);
...
const encodeTestEnvelopeSchema = Schema.encodeEffect(
	Schema.fromJsonString(TestEnvelopeSchema),
);
...
it.layer(NodeServices.layer)("readBootstrapEnvelope", (it) => {
	...
	it.effect(
		"preserves fd path, platform, and cause when opening the input stream fails",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const filePath = yield* fs.makeTempFileScoped({
	...
	it.effect(
		"preserves fd and cause when stat fails for a non-availability reason",
		() =>
			Effect.gen(function* () {
				const fd = yield* Effect.acquireRelease(
				...
				try {
					...
					assert.equal(
						error.message,
						`Failed to stat bootstrap file descriptor ${fd}.`,
					);
				} finally {
	...
	it.effect(
		"preserves fd and schema cause when decoding the envelope fails",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const filePath = yield* fs.makeTempFileScoped({
				...
				const _writer = yield* Effect.acquireRelease(
					Effect.sync(() =>
						NodeChildProcess.spawn(
							"sh",
							["-c", 'exec 3>"$1"; sleep 60', "sh", fifoPath],
							{
