...
describe("CheckpointDiffQuery.layer", () => {
	it.effect(
		"uses the narrow full-thread context lookup for all-turns diffs",
		() =>
			Effect.gen(function* () {
				...
				const layer = CheckpointDiffQuery.layer.pipe(
					Layer.provideMerge(
						Layer.succeed(CheckpointStore.CheckpointStore, checkpointStore),
					),
					Layer.provideMerge(
						Layer.succeed(ProjectionSnapshotQuery.ProjectionSnapshotQuery, {
							...
							getCommandReadModel: () =>
								Effect.die(
									"CheckpointDiffQuery should not request the command read model",
								),
							getSnapshot: () =>
								Effect.die(
							...
							getArchivedShellSnapshot: () =>
								Effect.die(
									"CheckpointDiffQuery should not request archived shell snapshots",
								),
							getSnapshotSequence: () =>
