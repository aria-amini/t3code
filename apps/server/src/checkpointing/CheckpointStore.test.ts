...
const VcsProcessTestLayer = VcsProcess.layer.pipe(
...
it.layer(TestLayer)("CheckpointStore.layer", (it) => {
	...
	describe("diffCheckpoints", () => {
		...
		it.effect(
			"can hide indentation churn when changes wrap existing lines",
			() =>
				Effect.gen(function* () {
					const tmp = yield* makeTmpDir();
					yield* initRepoWithCommit(tmp);
	...
	describe("checkpoint file summaries", () => {
		...
		it.effect(
			"preserves file paths and turn ranges without changing the user index",
			() =>
				Effect.gen(function* () {
					const tmp = yield* makeTmpDir();
					yield* initRepoWithCommit(tmp);
					...
					const renamedPath =
						platform === "win32"
							...
							: "renamed\tcafé\nname.txt";
					const addedPath =
						platform === "win32" ? "new café.txt" : "new\tfile\n名.txt";
					...
					yield* checkpointStore.captureCheckpoint({
						cwd: tmp,
						checkpointRef: firstTurn,
					});
					const userIndex = yield* fileSystem.readFile(
						NodePath.join(tmp, ".git/index"),
					);
					const input = {
						cwd: tmp,
					...
					yield* writeTextFile(
						NodePath.join(tmp, "copy-source.txt"),
						"replacement\n",
					);
					yield* checkpointStore.captureCheckpoint({
						cwd: tmp,
						checkpointRef: secondTurn,
					});
					...
					expect(inclusiveSummary).toEqual(
						expectedFiles
							...
							.map((file) =>
								file.path === "copy-source.txt"
									? { ...file, additions: 1, deletions: 20 }
									: file,
							),
					);
					expect(
						yield* checkpointStore.diffCheckpoints({
							...input,
							toCheckpointRef: baseline,
						}),
					).toBe("");
					expect(
						yield* fileSystem.readFile(NodePath.join(tmp, ".git/index")),
					).toEqual(userIndex);
				}),
		);
		...
		it.effect("uses HEAD for a missing baseline only when requested", () =>
			Effect.gen(function* () {
				...
				const error = yield* Effect.flip(
					checkpointStore.diffCheckpoints(input),
				);
