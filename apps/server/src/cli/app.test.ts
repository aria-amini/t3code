...
async function startFakeDesktop(input: {
	...
	const target = resolveDesktopAppControlAddress({
		stateDir: NodePath.join(
			input.baseDir,
			input.stateSubdirectory ?? "userdata",
		),
	...
	if (target.directory !== null) {
		...
		await NodeFSP.unlink(target.address).catch(
	...
	const server = NodeNet.createServer((socket) => {
		...
		socket.on("data", (chunk) => {
			...
			) as DesktopAppActivationRequest;
...
describe("t3 app", () => {
	...
	it.effect(
		"does not create state when only a server or no desktop app is running",
		() =>
				...
				Effect.gen(function* () {
					...
					expect(error).toMatchObject({
						...
						message: expect.stringContaining(
							"Could not reach the T3 Code desktop app.",
						),
						cause: { code: "ENOENT" },
					});
					expect(yield* pathExists(baseDir)).toBe(false);
	...
	it.effect(
		"uses T3CODE_HOME or --base-dir and sends the default or explicit path",
		() =>
			withTempDirectory("t3-app-command-test-", (root) =>
				Effect.gen(function* () {
					...
					expect(
						desktop.received.map((request) => request.workspaceRoot),
					).toEqual([workingDirectory, explicitPath]);
					expect(
						desktop.received.every((request) => request.platform === platform),
					).toBe(true);
				}).pipe(Effect.scoped),
			),
	);
	...
	it.effect(
		"prefers the installed desktop app when a dev desktop is also running",
		() =>
	...
	it.effect(
		"finds the dev desktop when the default desktop socket is absent",
		() =>
	...
	it.effect(
		"never searches a dev state directory for an explicit T3 home",
		() =>
			withTempDirectory("t3-app-explicit-test-", (root) =>
				Effect.gen(function* () {
					...
					const flagError = yield* runCli(["app", "--base-dir", baseDir]).pipe(
						Effect.flip,
					);
					const envError = yield* runCli(["app"], {
						T3CODE_HOME: baseDir,
					}).pipe(Effect.flip);

					expect(flagError).toMatchObject({
						_tag: "DesktopAppUnreachableError",
					});
	...
	for (const responseKind of ["failure", "invalid"] as const) {
		it.effect(
			`never falls back after the default desktop sends a ${responseKind} response`,
			() =>
