...
): platform is DesktopAppActivationPlatform {
	...
	return new Promise((resolve, reject) => {
		...
		const finish = (
			result:
				| {
						...
						readonly response: DesktopAppActivationResponse;
				  }
		...
		socket.on("data", (chunk) => {
			...
			if (Buffer.byteLength(buffer, "utf8") > MAX_RESPONSE_BYTES) {
				finish({
					type: "failure",
					error: new Error("The desktop app response is too large."),
				});
				return;
			}
			...
			if (!isDesktopAppActivationResponse(parsed)) {
				finish({
					type: "failure",
					error: new Error("The desktop app response is invalid."),
				});
				return;
			}
			if (parsed.requestId !== input.request.requestId) {
				finish({
					type: "failure",
					error: new Error(
						"The desktop app response did not match this request.",
					),
				});
				return;
			}
		...
		socket.once("end", () => {
			finish({
				type: "failure",
				error: new Error("The desktop app closed the connection."),
			});
		});
	});
}
...
const runAppCommand = Effect.fn("cli.app")(function* (flags: {
	...
	if (
		Option.isSome(environment.sshConnection) ||
		Option.isSome(environment.sshTty)
	) {
		return yield* new DesktopAppSshUnsupportedError({});
	}
	if (!isDesktopPlatform(hostPlatform)) {
		return yield* new DesktopAppPlatformUnsupportedError({
	...
	const configuredBaseDir =
		Option.getOrUndefined(flags.baseDir) ?? environment.t3Home;
	const baseDir = yield* resolveBaseDir(configuredBaseDir);
	const allowDevFallback =
		Option.isNone(flags.baseDir) && !environment.t3Home?.trim();
	const rawWorkspaceRoot =
		Option.getOrUndefined(flags.workspaceRoot) ??
		(yield* HostProcessWorkingDirectory);
