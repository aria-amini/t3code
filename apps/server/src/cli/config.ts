...
export const modeFlag = Flag.choice(
	"mode",
	ServerConfig.RuntimeMode.literals,
).pipe(
...
export const tailscaleServePortFlag = Flag.integer("tailscale-serve-port").pipe(
	Flag.withSchema(PortSchema),
	Flag.withDescription(
		"HTTPS port for Tailscale Serve when --tailscale-serve is enabled.",
	),
	Flag.optional,
);

const EnvServerConfig = Config.all({
	logLevel: Config.logLevel("T3CODE_LOG_LEVEL").pipe(
		Config.withDefault("Info"),
	),
	traceMinLevel: Config.logLevel("T3CODE_TRACE_MIN_LEVEL").pipe(
		Config.withDefault("Info"),
	),
	traceTimingEnabled: Config.boolean("T3CODE_TRACE_TIMING_ENABLED").pipe(
	...
	traceMaxBytes: Config.int("T3CODE_TRACE_MAX_BYTES").pipe(
		Config.withDefault(10 * 1024 * 1024),
	),
	traceMaxFiles: Config.int("T3CODE_TRACE_MAX_FILES").pipe(
		Config.withDefault(10),
	),
	traceBatchWindowMs: Config.int("T3CODE_TRACE_BATCH_WINDOW_MS").pipe(
	...
	t3Home: Config.string("T3CODE_HOME").pipe(
		Config.option,
		Config.map(Option.getOrUndefined),
	),
...
const loadPersistedObservabilitySettings = Effect.fn(function* (
	settingsPath: string,
) {
	...
	Effect.gen(function* () {
		...
		const normalizedFlags = {
			...
			autoBootstrapProjectFromCwd:
				flags.autoBootstrapProjectFromCwd ?? Option.none(),
			logWebSocketEvents: flags.logWebSocketEvents ?? Option.none(),
			tailscaleServeEnabled: flags.tailscaleServeEnabled ?? Option.none(),
			tailscaleServePort: flags.tailscaleServePort ?? Option.none(),
		} satisfies CliServerFlags;
		const bootstrapFd =
			Option.getOrUndefined(normalizedFlags.bootstrapFd) ?? env.bootstrapFd;
			...
			decode: (value) => {
				...
				return Effect.fail(
					new SchemaIssue.InvalidValue({
						message:
							"Invalid duration. Use values like 5m, 1h, 30d, or 15 minutes.",
					}),
				);
			},
