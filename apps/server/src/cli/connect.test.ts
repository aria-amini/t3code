...
it.effect("detects headless operation from individual SSH config values", () =>
	Effect.gen(function* () {
		...
		assert.isTrue(
			yield* readHeadlessSessionConfig({ SSH_CONNECTION: "client server" }),
		);
...
it.effect(
	"treats cancelling optional background setup as a successful skip",
	() =>
		Effect.gen(function* () {
...
it.effect(
	"keeps a successful connection when a remote service update is pending",
	() =>
		Effect.gen(function* () {
			const result = yield* recoverServiceOnboardingOffer(
...
it.effect(
	"does not install the relay client when the user declines the managed download",
	() =>
		Effect.gen(function* () {
			let installCalls = 0;
...
it.effect(
	"installs the relay client after the user accepts the managed download",
	() =>
		Effect.gen(function* () {
			let installCalls = 0;
...
it.effect(
	"keeps disconnect causes in structured logs and out of console warnings",
	() => {
		...
		return reportCloudDisconnectResults({
			...
			Effect.tap(() =>
				Effect.sync(() => {
					...
					assert.include(
						warningText,
						"running server could not stop its tunnel",
					);
					assert.include(
						warningText,
						"Could not revoke the relay-side environment record",
					);
					assert.notInclude(warningText, liveFailure);
					assert.notInclude(warningText, relayFailure);
					assert.deepEqual(
						logs.map(({ operation, clearAuthorization }) => ({
							operation,
							clearAuthorization,
						})),
						[
