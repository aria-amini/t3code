...
const isCloudCliTokenManagerError = Schema.is(
	CliTokenManager.CloudCliTokenManagerError,
);

const headlessFlag = Flag.boolean("headless").pipe(
	Flag.withDescription(
		"Authorize without a local browser using out-of-band OAuth.",
	),
	Flag.withDefault(false),
);
...
export const headlessSessionConfig = Config.all({
	...
	Config.map(
		({ sshConnection, sshTty }) =>
			Option.isSome(sshConnection) || Option.isSome(sshTty),
	),
);

const promptForOutOfBandOAuthCode = Effect.fn(
	"cloud.cli.prompt_for_out_of_band_oauth_code",
)(function* ({
	authorizeUrl,
	validate,
	...
	return yield* Prompt.run(
		Prompt.text({ message: "Authorization code", validate }),
	);
});
...
function formatRelayClientStatus(
	executable: RelayClient.RelayClientStatus,
...
function relayClientInstallProgressMessage(
	stage: RelayClientInstallProgressStage,
): string {
	switch (stage) {
		case "checking":
...
const reportRelayClientInstallProgress = (
	event: RelayClientInstallProgressEvent,
) =>
	event.type === "progress"
		? Console.log(
				`Relay client: ${relayClientInstallProgressMessage(event.stage)}...`,
			)
		: Effect.void;

export const acquireRelayClientForLink = Effect.fn(
	"cloud.cli.acquire_relay_client_for_link",
)(function* <ConfirmError, ConfirmContext>(
	relayClient: RelayClient.RelayClient["Service"],
	confirmInstall: (
		version: string,
	) => Effect.Effect<boolean, ConfirmError, ConfirmContext>,
	reportProgress: (
		event: RelayClientInstallProgressEvent,
	) => Effect.Effect<void>,
) {
...
const runLiveCloudUnlink = Effect.fn("cloud.cli.run_live_unlink")(function* () {
	...
	return Exit.isSuccess(result)
		...
		: ({
				status: "failed",
				cause: result.cause,
			} satisfies LiveCloudActionResult);
});
...
const unlinkRelayEnvironment = Effect.fn("cloud.cli.unlink_relay_environment")(
	function* () {
		const tokens = yield* CliTokenManager.CloudCliTokenManager;
...
export const reportCloudDisconnectResults = Effect.fn(
	"cloud.cli.report_disconnect_results",
)(function* (input: {
...
const runCloudCommand = Effect.fn("cloud.cli.run_cloud_command")(function* <
	A,
	E,
>(
...
const connectedAs = (identity: string | null): string =>
...
const linkEnvironmentForConnect = Effect.fn("cloud.cli.link_environment")(
	function* (options: {
		readonly headless: boolean;
		...
		if (!publishOnly) {
			...
			if (Option.isNone(installed)) {
				yield* Console.log(
					"T3 Connect setup cancelled. The relay client was not installed.",
				);
				return null;
			}
...
const connectLoginCommand = Command.make("login", {
	...
	Command.withDescription(
		"Authorize the T3 Connect CLI without enabling remote access.",
	),
...
const connectLinkCommand = Command.make("link", {
	...
	Command.withDescription(
		"Authorize this environment for T3 Connect on next start.",
	),
	Command.withHandler((flags) =>
		runCloudCommand(
			flags,
			...
			Effect.gen(function* () {
				...
				const status: CloudCliStatus = {
					...
					cloudUserId: Option.isSome(cloudUserId)
						...
						: null,
					relayUrl: Option.isSome(relayUrl)
						? bytesToString(relayUrl.value)
						: null,
					publishAgentActivity: isAgentActivityPublishingEnabledValue(
						Option.isSome(publishAgentActivity)
							? bytesToString(publishAgentActivity.value)
							: null,
					),
...
const connectPublishCommand = Command.make("publish", {
	...
	disable: Flag.boolean("disable").pipe(
		Flag.withDescription(
			"Stop publishing agent activity to your mobile clients.",
		),
		Flag.withDefault(false),
	),
}).pipe(
		...
		runCloudCommand(
			...
			Effect.gen(function* () {
				...
				if (!enabled) {
					...
					const linkedNow = Option.isSome(
						yield* secrets.get(CLOUD_LINKED_USER_ID),
					);
					if (
						!linkedNow &&
						(yield* CliState.readCliDesiredLinkMode) === "publish_only"
					) {
						yield* CliState.setCliDesiredCloudLink(false);
						yield* Console.log(
							"Cancelled the pending publish-only T3 Connect link.",
						);
					}
...
const connectUnlinkCommand = Command.make("unlink", {
	...
	Command.withDescription(
		"Disable T3 Connect while retaining the stored authorization.",
	),
	Command.withHandler((flags) =>
...
const connectLogoutCommand = Command.make("logout", {
	...
	Command.withDescription(
		"Disable T3 Connect and clear the stored CLI authorization.",
	),
	Command.withHandler((flags) =>
