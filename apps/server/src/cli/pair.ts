...
export const resolveDirectPairingBaseUrl = (
...
export const resolveTailscaleLocalTarget = (
	...
	if (state.devUrl !== undefined) {
		...
		const localPort =
			devUrl.port.length > 0 ? Number.parseInt(devUrl.port, 10) : 80;
...
const resolveTailscalePairingBase = Effect.fn(
	"pair.resolveTailscalePairingBase",
)(function* (input: {
	readonly target: DiscoveredPairTarget;
	...
	if (existing._tag === "descriptor") {
		if (
			existing.descriptor.environmentId !==
			input.target.descriptor.environmentId
		) {
	...
	yield* ensureTailscaleServe({
		...
		servePort: input.servePort,
		...(localTarget.localHost !== undefined
	...
	return yield* Effect.gen(function* () {
		...
		Effect.provide(
			EnvironmentAuth.runtimeLayer.pipe(
				...
				Layer.provide(
					Layer.succeed(References.MinimumLogLevel, input.config.logLevel),
				),
			),
		),
	);
...
const labelFlag = Flag.string("label").pipe(
	Flag.withDescription(
		"Optional label shown in the server's connections list.",
	),
	Flag.optional,
);
...
const tailscaleServePortFlag = Flag.integer("tailscale-serve-port").pipe(
	...
	Flag.withDescription(
		"HTTPS port for Tailscale Serve when --tailscale is enabled.",
	),
...
export const pairCommand = Command.make("pair", {
	...
	Command.withHandler((flags) =>
		Effect.gen(function* () {
			...
			const target = yield* discoverPairTarget(
