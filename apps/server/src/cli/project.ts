...
export function projectCommandErrorFromLiveServerRequest(
	cause: unknown,
): ProjectCommandError {
	...
	return new ProjectLiveServerRequestError({
		operation: "callLiveServer",
		cause,
	});
}
...
const findActiveProjectTarget = Effect.fn("findActiveProjectTarget")(
	function* (input: {
		...
		const activeProjects = input.snapshot.projects.filter(
			(project) => project.deletedAt === null,
		);
		const exactIdMatch = activeProjects.find(
			(project) => project.id === trimmedIdentifier,
		);
		if (exactIdMatch) {
			return {
				id: exactIdMatch.id,
		...
		const exactWorkspaceMatch = activeProjects.find(
			(project) =>
				...
				(normalizedWorkspaceRoot ?? trimmedIdentifier),
		);
...
const tryResolveLiveProjectExecutionMode = Effect.fn(
	...
	yield* Effect.logDebug(
		"Failed to connect to the persisted project CLI server.",
		{
			origin: runtimeState.value.origin,
...
const runProjectMutation = Effect.fn("runProjectMutation")(function* (
	...
	return yield* Effect.gen(function* () {
		...
		const liveMode = yield* tryResolveLiveProjectExecutionMode(
			environmentAuth,
			config,
		);
...
const projectAddCommand = Command.make("add", {
	...
	title: Flag.string("title").pipe(
		Flag.withDescription("Optional project title."),
		Flag.optional,
	),
}).pipe(
	...
	Command.withHandler((flags) =>
		runProjectMutation(
			...
			Effect.fn("projectAddMutation")(function* ({
				...
				const title = yield* resolveProjectTitle(
					workspaceRoot,
...
const projectRenameCommand = Command.make("rename", {
	...
	title: Argument.string("title").pipe(
		Argument.withDescription("New project title."),
	),
}).pipe(
	...
	Command.withHandler((flags) =>
		runProjectMutation(
			...
			Effect.fn("projectRenameMutation")(function* ({
				...
				const nextTitle = yield* resolveProjectTitle(
					project.workspaceRoot,
					flags.title,
				);
