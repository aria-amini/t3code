...
it("explains where the service is supported", () => {
	assert.include(
		formatServiceStatus(
			{ ...status, supported: false, installed: false },
			"0.0.29",
		),
...
function makeTestService(serviceStatus: BootService.BootServiceStatus) {
	const installOptions: Array<
		Parameters<BootService.BootService["Service"]["install"]>[0]
	> = [];
...
it.layer(Layer.mergeAll(NodeServices.layer, NetService.layer))(
	...
	(it) => {
			...
			(command) =>
				Effect.gen(function* () {
					...
					const baseDir = yield* fs.makeTempDirectoryScoped({
						prefix: "t3-service-cli-test-",
					});
					const { service, installOptions } =
						makeTestService(newerServiceStatus);
					vi.spyOn(BootService, "layer").mockReturnValue(
						Layer.succeed(BootService.BootService, service),
					);

					const error = yield* Command.runWith(serviceCommand, {
						version: packageJson.version,
					})([command, "--base-dir", baseDir]).pipe(
		...
		it.effect.each(["install", "update"] as const)(
			"%s allows an explicit downgrade",
			(command) =>
				Effect.gen(function* () {
					const fs = yield* FileSystem.FileSystem;
					const baseDir = yield* fs.makeTempDirectoryScoped({
					...
					yield* Command.runWith(serviceCommand, {
						version: packageJson.version,
					})([command, "--base-dir", baseDir, "--allow-downgrade"]).pipe(
						Effect.provideService(HostProcessEnvironment, {}),
						Effect.provide(
							ConfigProvider.layer(ConfigProvider.fromEnv({ env: {} })),
						),
...
it.effect.each([
	{
		name: "a new service",
		state: { ...status, installed: false, current: false },
	},
	{
		name: "an older service",
		state: { ...status, current: false, installedVersion: "0.0.0" },
	},
	{
...
it.effect(
	"leaves a newer service unchanged during onboarding without prompting",
	() =>
		Effect.gen(function* () {
			...
			const terminal = Terminal.make({
				...
				readInput: Effect.die(
					"Onboarding must not prompt to replace a newer service.",
				),
				readLine: Effect.die(
					"Onboarding must not prompt to replace a newer service.",
				),
				display: () =>
					Effect.die("Onboarding must not prompt to replace a newer service."),
			});
...
it.effect(
	"keeps onboarding successful when a newer version appears before install",
	() =>
		Effect.gen(function* () {
...
it.effect(
	"keeps the manual-server fallback when background prerequisites fail",
	() =>
		Effect.gen(function* () {
			const ready = yield* recoverServiceOnboardingOffer(
				Effect.fail(
					new BootService.BootServicePrerequisiteError({
