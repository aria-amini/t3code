...
export const reconcileService = Effect.fn("cli.service.reconcile")(
...
export function formatServiceStatus(
	...
	const problems = (status.problems ?? []).map(
		(problem) =>
			`  [${problem}] ${BootService.formatBootServiceProblem(problem)}`,
	);
	if (
		!status.current &&
		status.installedVersion !== undefined &&
	...
	return [
		...
		...(status.current
			? []
			: [`  Next: Run \`npx t3@${cliVersion} service update\`.`]),
	].join("\n");
}
...
const serviceReconcileFlags = {
	...
	allowDowngrade: Flag.boolean("allow-downgrade").pipe(
		Flag.withDescription(
			"Allow replacing a newer installed service with this older CLI version.",
		),
...
const serviceInstallCommand = Command.make(
	...
	Command.withDescription(
		"Install T3 Code as a background service for this user.",
	),
...
const serviceUninstallCommand = Command.make(
	"uninstall",
	projectLocationFlags,
).pipe(
...
const serviceStatusCommand = Command.make("status", projectLocationFlags).pipe(
	Command.withDescription(
		"Show whether the T3 Code background service is installed.",
	),
	Command.withHandler((flags) =>
		runServiceCommand(
			flags,
...
export const offerServiceDuringOnboarding = Effect.gen(function* () {
	...
	if (installed && current) {
		yield* Console.log(
			"T3 Code is already set up to run in the background on this machine.",
		);
		return true;
	}
