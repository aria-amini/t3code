...
const runCli = (args: ReadonlyArray<string>) =>
	Command.runWith(cli, { version: "0.0.0" })(args).pipe(
		Effect.provide(
			Layer.mergeAll(NodeServices.layer, NetService.layer, TestConsole.layer),
		),
	);

const makeBaseDir = () =>
	NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "t3code-theme-cli-"));

const settingsPathFor = (baseDir: string) =>
...
const writeSettings = (baseDir: string, settings: Record<string, unknown>) => {
	NodeFS.mkdirSync(NodePath.dirname(settingsPathFor(baseDir)), {
		recursive: true,
	});
	NodeFS.writeFileSync(
		settingsPathFor(baseDir),
...
describe("t3 theme", () => {
	...
	it.effect("publishes a theme file under an explicit id", () =>
		Effect.gen(function* () {
			...
			assert.equal(
				NodeFS.existsSync(
					NodePath.join(baseDir, "userdata", "themes", "nightfall.json"),
				),
				true,
			);
			assert.equal(readSettings(baseDir).defaultTheme, "nightfall");
	...
	it.effect("rejects a file that is not a theme and sets nothing", () =>
		Effect.gen(function* () {
			...
			assert.equal(
				NodeFS.existsSync(NodePath.join(baseDir, "userdata", "themes")),
				false,
			);
		...
		() =>
			Effect.gen(function* () {
				...
				try {
					...
					assert.equal(
						NodeFS.existsSync(NodePath.join(themesDir, "nightfall.json")),
						false,
					);
				} finally {
	...
	it.effect.skipIf(!symlinksSupported)(
		"never writes through a symlink at the staging path",
		() =>
			Effect.gen(function* () {
				const baseDir = makeBaseDir();
				...
				NodeFS.symlinkSync(
					victim,
					NodePath.join(themesDir, `nightfall.json.staging-${process.pid}`),
				);
				const themeFile = NodePath.join(baseDir, "nightfall.json");
				NodeFS.writeFileSync(themeFile, NIGHTFALL_THEME_JSON);
	...
	it.effect(
		"rejects a path that does not exist instead of storing it as an id",
		() =>
			Effect.gen(function* () {
				const baseDir = makeBaseDir();
				const failure = yield* runCli([
	...
	it.effect("publishes an extensionless file", () =>
		Effect.gen(function* () {
			...
			assert.equal(
				NodeFS.existsSync(
					NodePath.join(baseDir, "userdata", "themes", "brand.json"),
				),
				true,
			);
	...
	it.effect.skipIf(windowsHost)(
		...
		() =>
			Effect.gen(function* () {
				...
				assert.equal(
					Object.hasOwn(readSettings(baseDir), "defaultTheme"),
					false,
				);
			}),
	);
	...
	it.effect("rejects an id whose published file the watcher would skip", () =>
		Effect.gen(function* () {
			...
			NodeFS.mkdirSync(themesDir, { recursive: true });
			NodeFS.writeFileSync(
				NodePath.join(themesDir, "broken.json"),
				"{ not json\n",
			);
	...
	it.effect("treats a bare id as an id even when a file shares its name", () =>
		Effect.gen(function* () {
			...
			assert.equal(
				NodeFS.existsSync(
					NodePath.join(baseDir, "userdata", "themes", "ocean.json"),
				),
				false,
			);
		}),
