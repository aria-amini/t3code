...
const decodeSettingsJson = Schema.decodeUnknownEffect(
	fromLenientJson(SparseSettings),
);
const encodeSettingsJson = Schema.encodeEffect(
	fromJsonStringPretty(SparseSettings),
);
...
const resolveThemePaths = Effect.fn(function* (
...
const readSettingsObject = Effect.fn(function* (settingsPath: string) {
	...
	const exists = yield* fs
		...
		.pipe(
			Effect.mapError(
				(cause) => new ThemeSettingsUnreadableError({ settingsPath, cause }),
			),
		);
	...
	const settings = yield* decodeSettingsJson(raw).pipe(
		Effect.mapError(
			(cause) => new ThemeSettingsMalformedError({ settingsPath, cause }),
		),
	);
	return { raw, settings };
});
...
const publishThemeFile = Effect.fn(function* (input: {
	...
	const info = yield* fs
		...
		.pipe(
			Effect.mapError(
				(cause) =>
					new ThemeFileUnreadableError({ filePath: input.filePath, cause }),
			),
		);
	if (info.type !== "File") {
		return yield* Effect.fail(
			new ThemeFileUnreadableError({ filePath: input.filePath }),
		);
	}
	if (Number(info.size) > MAX_THEME_FILE_BYTES) {
		return yield* Effect.fail(
			new ThemeFileTooLargeError({
	...
	if (!environmentThemeFileHasColors(decoded.value)) {
		return yield* Effect.fail(
			new ThemeFileColorlessError({ filePath: input.filePath }),
		);
	}
	...
	if (!isEnvironmentThemeId(themeId) || UNPUBLISHABLE_THEME_IDS.has(themeId)) {
		return yield* Effect.fail(
			new ThemeFileIdInvalidError({ themeId, filePath: input.filePath }),
		);
	}
	...
	yield* fs
		...
		.pipe(
			Effect.mapError(
				(cause) => new ThemePublishError({ themesDir: input.themesDir, cause }),
			),
		);
...
const themeSetCommand = Command.make("set", {
	...
	id: Flag.string("id").pipe(
		Flag.withDescription(
			"Theme id to publish a file under, instead of its filename.",
		),
		Flag.optional,
	),
	...
	Command.withDescription(
		"Set the environment's theme; connected clients switch to it.",
	),
...
const themeClearCommand = Command.make("clear", { baseDir: baseDirFlag }).pipe(
	Command.withDescription(
		"Remove the environment's theme; clients keep what they have.",
	),
	Command.withHandler((flags) =>
		Effect.gen(function* () {
			const paths = yield* resolveThemePaths(flags.baseDir);
			...
			yield* Console.log(
				`Environment theme cleared in ${paths.settingsPath}.\n`,
			);
		}),
	),
);
...
const themeShowCommand = Command.make("show", { baseDir: baseDirFlag }).pipe(
	Command.withDescription(
		"Show the environment's theme and its published themes.",
	),
	Command.withHandler((flags) =>
		Effect.gen(function* () {
			const paths = yield* resolveThemePaths(flags.baseDir);
			...
			const defaultTheme =
				typeof settings.defaultTheme === "string" &&
				settings.defaultTheme.length > 0
