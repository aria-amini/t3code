...
const describeServerProcess = Effect.fn("triage.describeServerProcess")(
	function* (serverRuntimeStatePath: string) {
...
const pickAgent = (agents: ReadonlyArray<TriageAgent>) =>
	Effect.promise(async () => {
		...
		try {
			...
			for (;;) {
				const answer = (
					await readline.question(`Run triage with:\n${menu}\n> `)
				).trim();
...
const runInteractiveSession = (input: {
	...
	Effect.callback<number, TriageAgentSpawnError>((resume) => {
		...
		child.once("error", (cause) =>
			resume(
				Effect.fail(
					new TriageAgentSpawnError({ command: input.command, cause }),
				),
			),
		);
		// Signal death has no exit code; report failure rather than success.
		child.once("exit", (code, signal) =>
			resume(Effect.succeed(code ?? (signal === null ? 0 : 1))),
		);
	});

const agentFlag = Flag.choice("agent", ["claude", "codex"]).pipe(
	Flag.withDescription(
		"Agent CLI to use. Default: ask when both are installed.",
	),
	Flag.optional,
);
...
const modelFlag = Flag.string("model").pipe(
	Flag.withDescription(
		"Model passed through to the agent CLI. Default: the agent's default.",
	),
	Flag.optional,
);
	...
	Command.withHandler((flags) =>
		Effect.gen(function* () {
			...
			const baseDir = yield* resolveBaseDir(
				explicitBaseDir ?? Option.getOrUndefined(envHome),
			);
			const paths = yield* ServerConfig.deriveServerPaths(
				baseDir,
				undefined,
				{},
			);
			...
			yield* fs.writeFileString(
				promptFilePath,
				buildTriageSeedPrompt(contextFilePath),
			);
			...
			yield* Console.log(
				`Starting ${selected.label}. It will ask what went wrong.\n`,
			);
			const exitCode = yield* runInteractiveSession({
				...spawnSpec,
				cwd: scratchDir,
			});
			if (exitCode !== 0) {
