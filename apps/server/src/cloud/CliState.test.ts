...
it.layer(NodeServices.layer)("CliState", (it) => {
	it.effect(
		"persists desired exposure and clears provisioned relay state",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"round-trips the desired link mode and defaults legacy links to managed",
		() =>
			Effect.gen(function* () {
				const secrets = yield* ServerSecretStore.ServerSecretStore;
				...
				yield* secrets.set(
					CliState.CLOUD_CLI_DESIRED_LINK_SECRET,
