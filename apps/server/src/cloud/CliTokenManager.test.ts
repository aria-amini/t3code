...
const TestIdTokenHeaderJson = Schema.fromJsonString(
	Schema.Struct({ alg: Schema.Literal("none") }),
);
...
const idTokenWithEmail = (() => {
	const header = Encoding.encodeBase64Url(
		encodeTestIdTokenHeader({ alg: "none" }),
	);
...
const isAuthorizationError = Schema.is(
	CliTokenManager.CloudCliAuthorizationError,
);
...
it.layer(NodeServices.layer)("CliTokenManager.outOfBandOAuthLogin", (it) => {
	it.effect(
		"prints a hosted authorize URL and exchanges the out-of-band code with PKCE",
		() =>
			Effect.gen(function* () {
				...
				const { token, identity } = yield* CliTokenManager.outOfBandOAuthLogin(
					({ authorizeUrl, validate }: OutOfBandOAuthPromptInput) =>
						Effect.gen(function* () {
							...
							const request = readConnectAuthorizeRequest(
								new URL(authorizeUrl),
							);
							assert.isNotNull(request);
							return yield* validate(`clerk-code-123.${request!.state}`).pipe(
								Effect.mapError(
									(message) => new PromptRejectedError({ message }),
								),
							);
						}),
				).pipe(
	...
	it.effect(
		"rejects out-of-band codes whose state does not match the request",
		() =>
			Effect.gen(function* () {
				const requests: Array<RecordedTokenRequest> = [];
					...
					({ validate }: OutOfBandOAuthPromptInput) =>
						validate("clerk-code-123.wrong-state").pipe(
							Effect.tapError((message) =>
								Effect.sync(() => validationErrors.push(message)),
							),
							Effect.mapError(
								(message) => new PromptRejectedError({ message }),
							),
	...
	it.effect("ignores an id_token whose claims are not valid JSON", () =>
		Effect.gen(function* () {
			...
			const { identity } = yield* CliTokenManager.outOfBandOAuthLogin(
				...
				Effect.provide(
					makeTokenEndpointLayer(requests, { idToken: malformedIdToken }),
				),
				provideTestEnv,
			);
	...
	it.effect(
		"fails without touching the token endpoint when the prompt returns garbage",
		() =>
			Effect.gen(function* () {
				const requests: Array<RecordedTokenRequest> = [];
