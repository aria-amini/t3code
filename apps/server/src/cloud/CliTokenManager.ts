...
const boldTerminalText = (value: string): string =>
	`\u001b[1m${value}\u001b[22m`;
...
const decodeOidcIdentityClaimsJson = Schema.decodeUnknownOption(
	OidcIdentityClaimsJson,
);
...
export class CloudCliTokenManager extends Context.Service<
	CloudCliTokenManager,
	{
		...
		) => Effect.Effect<void, CloudCliTokenManagerError>;
 ...
 */
export const outOfBandOAuthLogin = Effect.fn(
	"cloud.cli_token.out_of_band_oauth_login",
)(function* <E, R>(
	promptForCode: (
		input: OutOfBandOAuthPromptInput,
	...
	const authorizationCode = yield* promptForCode({
		authorizeUrl: buildConnectAuthorizeRequestUrl({
			hostedAppUrl,
			state,
			challenge,
		}),
...
export const make = Effect.gen(function* () {
	...
	const persist = Effect.fn("cloud.cli_token.persist")(function* (
		token: PersistedToken,
	) {
	...
	const read = Effect.fn("cloud.cli_token.read")(function* () {
		...
		return Option.some(
			yield* decodePersistedToken(bytesToString(encoded.value)),
		);
	});

	const refresh = Effect.fn("cloud.cli_token.refresh")(function* (
		token: PersistedToken,
	) {
		const metadata = yield* cloudCliOAuthConfig;
			...
			Effect.gen(function* () {
				...
				if (url.searchParams.get("state") !== state || !code) {
					return HttpServerResponse.text(
						"Invalid T3 Connect authorization callback.",
						{
							status: 400,
						},
					);
	...
	const getExistingNoLock = Effect.fn("cloud.cli_token.get_existing_no_lock")(
		function* () {
			const token = yield* read();
			if (Option.isNone(token)) return token;
			const now = yield* Clock.currentTimeMillis;
			if (
				token.value.expiresAtEpochMs - CLOUD_CLI_OAUTH_REFRESH_EARLY_MS >
				now
			) {
				return token;
			}
			return Option.some(
				yield* refresh(token.value).pipe(Effect.flatMap(persist)),
			);
		},
	);
	...
	const get = semaphore.withPermits(1)(
		Effect.gen(function* () {
			...
			return authorization._tag === "Authorized"
				? ({
						_tag: "Authorized",
						token: yield* persist(authorization.token),
					} as const)
				: authorization;
		}).pipe(
			Effect.mapError((cause) =>
				Terminal.isQuitError(cause)
					? cause
					: new CloudCliAuthorizationError({ cause }),
			),
			Effect.provide(services),
		),
	);
	const store = Effect.fn("cloud.cli_token.store")(function* (
		token: PersistedToken,
	) {
