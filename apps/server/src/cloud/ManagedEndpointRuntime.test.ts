...
describe("CloudManagedEndpointRuntime", () => {
	it("classifies Cloudflare connection and warning output", () => {
		...
		expect(
			ManagedEndpointRuntime.classifyRelayClientOutput(
				"2026-06-17T02:00:00Z PNC runtime panic",
			),
		).toBe("warning");
	});

	it.effect(
		"starts, deduplicates, rotates, and stops the Cloudflare connector",
		() =>
			Effect.gen(function* () {
				...
				expect(spawned.map((command) => command.options.stdout)).toEqual([
					"pipe",
					"pipe",
				]);
				expect(spawned.map((command) => command.options.stderr)).toEqual([
					"pipe",
					"pipe",
				]);
				expect(spawned.map((command) => command.options.detached)).toEqual([
					false,
					false,
				]);
				expect(spawned.map((command) => command.options.shell)).toEqual([
					false,
					false,
				]);
	...
	it.effect(
		"stops an active connector when a non-Cloudflare runtime config is applied",
		() =>
			Effect.gen(function* () {
				const killed: Array<number> = [];
	...
	it.effect(
		"supervises the active connector and restarts it after process exit",
		() =>
			Effect.gen(function* () {
				const spawned: Array<number> = [];
				const killed: Array<number> = [];
	...
	it.effect("backs off restarts while the connector crash-loops", () =>
		Effect.gen(function* () {
			const { spawner, spawned, exits, spawnSignals } =
				yield* makeCrashLoopSpawner(600, 4);
			const runtime = yield* buildCloudManagedEndpointRuntime(spawner);
	...
	it.effect("resets the backoff after the connector runs stably", () =>
		Effect.gen(function* () {
			const { spawner, spawned, exits, spawnSignals } =
				yield* makeCrashLoopSpawner(700, 5);
			const runtime = yield* buildCloudManagedEndpointRuntime(spawner);
	...
	it.effect(
		"an explicit config change clears the backoff and preempts a delayed restart",
		() =>
			Effect.gen(function* () {
				const { spawner, spawned, exits, spawnSignals } =
					yield* makeCrashLoopSpawner(800, 3);
				const runtime = yield* buildCloudManagedEndpointRuntime(spawner);
