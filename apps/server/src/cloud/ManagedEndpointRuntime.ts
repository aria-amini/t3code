...
export const make = Effect.gen(function* () {
	...
	const desiredConfigRef =
		yield* Ref.make<RelayManagedEndpointRuntimeConfig | null>(null);
	...
	const superviseConnector = (connector: ActiveConnector) =>
		Effect.gen(function* () {
			...
			const uptimeMillis =
				(yield* Clock.currentTimeMillis) - connector.startedAtMillis;
			...
			const restartDelayMillis = yield* Ref.modify(
				restartDelayRef,
				(current) => {
			...
			if (restartDelayMillis > 0) {
				yield* Effect.logWarning(
					"Relay client is crash-looping; delaying restart",
					{
						pid: Number(connector.child.pid),
						uptimeMillis,
						restartDelayMillis,
			...
			Effect.catchCause((cause) =>
				Effect.logWarning("Relay client supervisor failed", { cause }),
			),
		);
				...
				switch (classifyRelayClientOutput(line)) {
					...
					case "warning":
						return Effect.logWarning(
							"Relay client reported a transport warning",
							attributes,
						);
					case "debug":
			...
			if (active?.configKey === nextConfigKey) {
				...
				if (isRunning) {
					return {
						...
						...(active.config.tunnelId
							? { tunnelId: active.config.tunnelId }
							: {}),
						...(active.config.tunnelName
