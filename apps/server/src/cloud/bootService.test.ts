...
it("keeps systemd pinned to the stable launcher rather than a versioned server", () => {
	...
	expect(unit).toContain(
		"ExecStart=/usr/bin/node /home/theo/.t3/runtime/service-launcher.mjs",
	);
...
it("keeps launchd pinned to the stable launcher rather than a versioned server", () => {
	...
	expect(plist).toContain(
		"<string>/Users/theo/.t3/runtime/service-launcher.mjs</string>",
	);
...
it("escapes XML in host paths", () => {
	const plist = BootService.renderBootServicePlist(
		...
		{
			homeDir: "/Users/theo",
			environmentPath: "/Users/theo/Tools & <Scripts>:/usr/bin",
		},
	);

	expect(plist).toContain("<string>/Users/theo/T3 &amp; &lt;Co&gt;</string>");
	expect(plist).toContain(
		"<string>/Users/theo/Tools &amp; &lt;Scripts&gt;:/usr/bin</string>",
	);
});
	...
	const runner = ProcessRunner.ProcessRunner.of({
		run: Effect.fn("test.run_boot_service_command")(function* (
			...
			if (
				...
				yield* fs
					.writeFileString(statePath, control.stateAfterStop)
					.pipe(Effect.orDie);
			}
			return {
				stdout:
...
it.layer(NodeServices.layer)("boot service install", (it) => {
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				const { service, fs, statePath, commands, control, runtime } =
					yield* makeHarness();
				...
				expect(
					commands.some(
						(command) =>
							command.startsWith("npm ") || command.includes("--version"),
					),
				).toBe(false);
				expect(
					commands.some(
						(command) =>
							command.includes("daemon-reload") || command.includes("restart"),
					),
				).toBe(false);
				expect(yield* fs.readFileString(before.logPath)).toContain(
					"[linger-disabled]",
				);
			}),
	);
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				const { service, fs, statePath, commands, control } =
					yield* makeHarness();
	...
	it.effect(
		"enables lingering before installing and repairs stopped or disabled services",
		() =>
			Effect.gen(function* () {
				...
				expect(
					commands.indexOf("loginctl enable-linger --no-ask-password 501"),
				).toBeLessThan(commands.indexOf("systemctl --user daemon-reload"));
	...
	it.effect.each([
		...
		"reports failed prerequisite probes without installing: $command",
		({ command, problem }) =>
			Effect.gen(function* () {
	...
	it.effect("installs, reports current state, and uninstalls", () =>
		Effect.gen(function* () {
			const { service, fs, statePath, commands, timeouts } =
				yield* makeHarness();
			const plan = yield* service.install();
			...
			expect(commands.some((command) => command.startsWith("npm "))).toBe(
				false,
			);
			// The stop can block up to systemd's 90s TimeoutStopSec; the runner's
			// 60s default would cancel it mid-shutdown.
			expect(
				timeouts.get("systemctl --user disable --now t3code.service"),
			...
			Effect.gen(function* () {
				...
				expect(
					commands.filter(
						(command) =>
							command.startsWith(
								platform === "linux" ? "systemctl " : "launchctl ",
	...
	it.effect("allows an explicit downgrade", () =>
		Effect.gen(function* () {
			...
			expect(
				parseServiceState(yield* fs.readFileString(statePath))?.activeVersion,
			).toBe("1.2.3");
			expect((yield* service.status).current).toBe(true);
		}),
	);
	...
	it.effect(
		"repairs versions with equal SemVer precedence without an override",
		() =>
			Effect.gen(function* () {
				const { service, fs, statePath } = yield* makeHarness();
				yield* service.install();
	...
	it.effect("restarts without overwriting a pending remote update", () =>
		Effect.gen(function* () {
			...
			for (const allowDowngrade of [false, true]) {
				...
				expect(
					(yield* service.install({ allowDowngrade }).pipe(Effect.flip))._tag,
				).toBe("BootServiceUpdatePendingError");
				expect(
					serviceStateHasPendingUpdate(yield* fs.readFileString(statePath)),
				).toBe(true);
				expect(
					commands.filter(
						(command) =>
	...
	it.effect("installs, reports current state, and uninstalls on macOS", () =>
		Effect.gen(function* () {
			...
			expect(commands.some((command) => command.startsWith("systemctl "))).toBe(
				false,
			);
	...
	it.effect("restarts the launch agent when repair fails", () =>
		Effect.gen(function* () {
			...
			expect(error._tag).toBe("BootServiceCommandError");
			expect(
				commands.filter((command) => command.startsWith("launchctl ")),
			).toEqual([
				"launchctl bootout --wait gui/501/com.t3tools.t3code.service",
	...
	it.effect(
		"reconstructs a launch agent search path when the installer has no PATH",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"adds missing provider directories to a minimal installer PATH",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"keeps an installed launch agent current when the process PATH changes",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"drops PATH directories that cannot be represented in a launch agent plist",
		() =>
			Effect.gen(function* () {
				const { service, fs } = yield* makeHarness(
					"darwin",
	...
	it.effect(
		"restarts without overwriting a pending remote update on macOS",
		() =>
			Effect.gen(function* () {
				const { service, fs, statePath, commands } =
					yield* makeHarness("darwin");
				yield* service.install();
				const plistPath = (yield* service.status).unitPath;
				// @effect-diagnostics-next-line preferSchemaOverJson:off - fixed launcher-owned test document.
