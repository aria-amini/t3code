...
export const make = Effect.fn("cloud.boot_service.make")(function* (input: {
	...
	const installerPath = yield* Config.string("PATH").pipe(
		Config.withDefault(""),
	);
		...
		Effect.scoped(
			Effect.gen(function* () {
				...
				yield* (yield* fs.open(directory, { flag: "r" })).sync.pipe(
					Effect.catchIf(
						(error) =>
							(error.reason.cause as NodeJS.ErrnoException | undefined)
								?.code === "EPERM",
						() => Effect.void,
					),
				);
	...
	const logFailure = (error: { readonly message: string }) =>
		DateTime.now.pipe(
			Effect.flatMap((now) =>
				fs.writeFileString(
					logPath,
					`${DateTime.formatIso(now)} ${error.message}\n`,
					{ flag: "a" },
				),
			),
			Effect.ignore,
	...
	const probe = (command: string, args: ReadonlyArray<string>) =>
		runner
			.run({ command, args, timeout: Duration.seconds(5) })
			.pipe(Effect.option);
	...
	const readSystemdProblems = Effect.fn(
		"cloud.boot_service.read_systemd_problems",
	)(function* (includeService: boolean) {
		const [manager, linger] = yield* Effect.all(
			[
				probe("systemctl", ["--user", "show-environment"]),
				probe("loginctl", lingerArgs),
			],
		...
		const lingering =
			succeeded(linger) && Option.isSome(linger)
				? linger.value.stdout.trim()
				: "";
		if (lingering !== "yes") {
			problems.push(
				lingering === "no" ? "linger-disabled" : "linger-unavailable",
			);
		}
	...
	const requireSystemdPrerequisites = Effect.gen(function* () {
		...
		const unavailable = problems.find(
			(problem) => problem !== "linger-disabled",
		);
		if (unavailable)
			return yield* new BootServicePrerequisiteError({ problem: unavailable });
		...
		yield* runStep("enabling lingering for this user", "loginctl", [
			...
			Effect.mapError(
				(cause) =>
					new BootServicePrerequisiteError({
		...
		if (remaining[0])
			return yield* new BootServicePrerequisiteError({ problem: remaining[0] });
	});
	...
	const install = Effect.fn("cloud.boot_service.install")(function* (options?: {
		...
		yield* Effect.gen(function* () {
			if (installed) {
				const previousStateText = yield* fs
				...
				if (Option.isSome(previousStateText)) {
					...
					const installedVersion = serviceStateActiveVersion(
						previousStateText.value,
					);
					if (
	...
	const uninstall: BootService["Service"]["uninstall"] = Effect.gen(
		function* () {
			const manager = yield* requireManager;
			if (
				!(yield* fs
	...
	const status: BootService["Service"]["status"] = Effect.gen(function* () {
		...
		const state = Option.isSome(stateText)
			? parseServiceState(stateText.value)
			: undefined;
