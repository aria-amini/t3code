	...
	ServerSecretStore.layer.pipe(
		Layer.provide(
			ServerConfig.layerTest(process.cwd(), {
				prefix: "t3-environment-keys-test-",
			}),
		),
	);
	...
	(it) => {
		it.effect("persists one atomic keypair secret and reuses it", () =>
			Effect.gen(function* () {
				...
				const first =
					yield* getOrCreateEnvironmentKeyPairFromSecretStore(secretStore);
				const second =
					yield* getOrCreateEnvironmentKeyPairFromSecretStore(secretStore);

				assert.deepEqual(second, first);
				assert.isTrue(
					Option.isSome(yield* secretStore.get("cloud-link-ed25519-key-pair")),
				);
				assert.isTrue(
					Option.isNone(
						yield* secretStore.get("cloud-link-ed25519-private-key"),
					),
				);
				assert.isTrue(
					Option.isNone(
						yield* secretStore.get("cloud-link-ed25519-public-key"),
					),
				);
