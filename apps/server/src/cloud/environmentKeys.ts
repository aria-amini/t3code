...
const decodeEnvironmentKeyPair = Schema.decodeUnknownEffect(
	EnvironmentKeyPairJson,
);
...
const keyPairDecodeError = (
	cause: unknown,
): ServerSecretStore.SecretStoreDecodeError =>
	new ServerSecretStore.SecretStoreDecodeError({
		resource: KEY_PAIR_RESOURCE,
		cause,
	});

const keyPairEncodeError = (
	cause: unknown,
...
const keyPairConcurrentReadError =
	(): ServerSecretStore.SecretStoreConcurrentReadError =>
...
const readEnvironmentKeyPair = Effect.fn("readEnvironmentKeyPair")(function* (
	...
	const decoded = yield* decodeEnvironmentKeyPair(
		bytesToString(encoded.value),
	).pipe(Effect.mapError(keyPairDecodeError));
	return Option.some(decoded);
});

const persistEnvironmentKeyPair = Effect.fn("persistEnvironmentKeyPair")(
	function* (
		secrets: ServerSecretStore.ServerSecretStore["Service"],
		...
		return yield* secrets
			.create(CLOUD_LINK_KEY_PAIR, stringToBytes(encoded))
			.pipe(
				Effect.as(keyPair),
...
export const getOrCreateEnvironmentKeyPairFromSecretStore = Effect.fn(
