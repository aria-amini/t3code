...
const unusedSecretStoreOperation = () =>
	Effect.die("unused secret-store operation");
...
it("preserves messages surfaced by cloud 500 responses", () => {
	...
	expect([
		new EnvironmentAuth.ServerAuthLinkedCloudAccountVerificationError({ cause })
			.message,
		new EnvironmentAuth.ServerAuthLinkedCloudAccountReadError({ cause })
			.message,
...
describe("relay request tracing", () => {
	it.effect("does not accept an unauthenticated request trace parent", () =>
		Effect.gen(function* () {
			...
			const request = HttpServerRequest.fromWeb(
				new Request(
					"https://environment.example.test/api/t3-cloud/mint-credential",
					{
						headers: {
							traceparent:
	...
	it.effect(
		"continues an authenticated relay trace with the product tracer",
		() =>
			Effect.gen(function* () {
				const spans: Array<Tracer.Span> = [];
				const productTracer = Tracer.make({
				...
				expect(Option.getOrUndefined(span.parent)?.spanId).toBe(
					"0123456789abcdef",
				);
			}),
	);
...
describe("reconcileDesiredCloudLink", () => {
	it.effect(
		"requires stored CLI authorization without exposing an HTTP endpoint",
		() =>
			Effect.gen(function* () {
				const error = yield* Effect.flip(
...
describe("releaseManagedTunnelOnShutdown", () => {
	...
	function makeMemorySecretStore(
		...
		const values = new Map<string, Uint8Array>(
			Array.from(
				initial,
				([name, value]) => [name, new TextEncoder().encode(value)] as const,
			),
		);
	...
	const provideReleaseHarness =
		...
		<A, E, R>(effect: Effect.Effect<A, E, R>) =>
			effect.pipe(
				...
				Effect.provide(
					ServerConfigModule.layerTest("/", {
						prefix: "t3-http-release-test-",
	...
	it.effect(
		"does nothing for links without a stored managed tunnel runtime config",
		() => {
	...
	it.effect(
		"leaves the tunnel of a web/mobile-installed link untouched",
		() => {
	...
	it.effect(
		"leaves the tunnel of a publish-only desired link untouched",
		() => {
			const { store, values } = makeMemorySecretStore([
				[CLOUD_ENDPOINT_RUNTIME_CONFIG, "runtime-config"],
				[RELAY_URL_SECRET, "https://relay.example.test"],
	...
	it.effect(
		"keeps the tunnel when shutdown hands off to a pending update",
		() => {
			const { store, values } = makeMemorySecretStore(managedLinkSecrets);
			const applyConfigCalls: Array<unknown> = [];
			const requests: Array<HttpClientRequest.HttpClientRequest> = [];
	...
	it.effect(
		"still releases a pending update when the launcher is stopping",
		() => {
			...
			return Effect.gen(function* () {
				...
				yield* fs.writeFileString(
					path.join(config.baseDir, "runtime", SERVICE_STOP_MARKER_FILE),
					"",
				);
	...
	it.effect(
		"keeps a runtime config that a fast restart replaced mid-release",
		() => {
			const { store, values } = makeMemorySecretStore(managedLinkSecrets);
			const applyConfigCalls: Array<unknown> = [];
			const requests: Array<HttpClientRequest.HttpClientRequest> = [];
	...
	it.effect(
		"keeps the stored connector token when the relay skipped the release",
		() => {
	...
	it.effect(
		"keeps the stored connector token when the relay release request fails",
		() => {
			const { store, values } = makeMemorySecretStore(managedLinkSecrets);
			const applyConfigCalls: Array<unknown> = [];
			const requests: Array<HttpClientRequest.HttpClientRequest> = [];
...
describe("link proof provider kinds", () => {
	...
	it("accepts managed and manual endpoints but not t3_relay", () => {
		expect(isSupportedLinkProviderKind(proofRequest("cloudflare_tunnel"))).toBe(
			true,
		);
	...
	it("only claims the managed-tunnel scope for tunnel links", () => {
		...
		expect(linkProofScopes(proofRequest("manual"))).toEqual([
			"agent_activity_notifications",
		]);
	});
});
