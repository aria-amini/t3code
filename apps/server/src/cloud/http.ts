...
const appendCloudCredentialResponseHeaders =
	HttpEffect.appendPreResponseHandler((_request, response) =>
		Effect.succeed(
			HttpServerResponse.setHeaders(
				response,
				CLOUD_CREDENTIAL_RESPONSE_HEADERS,
			),
		),
	);
...
const failEnvironmentCloudInternalError =
	...
	(cause: unknown): Effect.Effect<never, EnvironmentHttpInternalServerError> =>
		Effect.logError(message, { cause }).pipe(
			Effect.flatMap(() =>
				Effect.fail(new EnvironmentHttpInternalServerError({ message })),
			),
		);

const failCloudCliTokenManagerError = (
	error: CliTokenManager.CloudCliTokenManagerError,
...
function validateRelayConfigPayload(
	...
	if (
		payload.relayIssuer !== undefined &&
		!isSecureRelayUrl(payload.relayIssuer)
	) {
		return Effect.fail(
			new EnvironmentHttpBadRequestError({
...
function validateLinkedCloudUser(input: {
	...
	void,
	EnvironmentAuth.ServerAuthInternalError | EnvironmentHttpConflictError
> {
...
function hasForwardedAuthorityHeaders(
	request: HttpServerRequest.HttpServerRequest,
): boolean {
	return (
		firstForwardedHeaderValue(request.headers["x-forwarded-host"]) !==
			undefined ||
		firstForwardedHeaderValue(request.headers["x-forwarded-proto"]) !==
			undefined
	);
}
...
export function isSupportedLinkProviderKind(
...
const decodeCloudMintProof = Schema.decodeUnknownEffect(
	RelayCloudMintCredentialProofPayload,
);
...
const makeCloudLinkProof = Effect.fn("environment.cloud.makeLinkProof")(
	function* (
		dependencies: CloudHttpDependencies,
		request: RelayLinkProofRequest,
		requestUrl: string,
	) {
		const keyPair = yield* getOrCreateEnvironmentKeyPairFromSecretStore(
...
const applyCloudRelayConfig = Effect.fn("environment.cloud.applyRelayConfig")(
	function* (
		dependencies: CloudHttpDependencies,
		payload: RelayEnvironmentConfigRequest,
	) {
		...
		const endpointRuntimeStatus =
			yield* dependencies.endpointRuntime.applyConfig(payload.endpointRuntime);
		const ok =
		...
		yield* dependencies.secrets.set(
			RELAY_URL_SECRET,
			stringToBytes(payload.relayUrl),
		);
		yield* dependencies.secrets.set(
			RELAY_ISSUER_SECRET,
		...
		yield* dependencies.secrets.set(
			CLOUD_LINKED_USER_ID,
			stringToBytes(payload.cloudUserId),
		);
		yield* dependencies.secrets.set(
			RELAY_ENVIRONMENT_CREDENTIAL_SECRET,
...
const reconcileDesiredCloudLinkWith = Effect.fn(
...
export const reconcileDesiredCloudLink = Effect.fn(
	"environment.cloud.reconcileDesiredLink",
)(function* (localOrigin: string) {
	return yield* reconcileDesiredCloudLinkWith(
		yield* cloudHttpDependencies,
		localOrigin,
	);
});
...
export const pendingServiceUpdateExists = Effect.gen(function* () {
	...
	const stateText = yield* fs
		...
		.pipe(Effect.option);
	return (
		Option.isSome(stateText) && serviceStateHasPendingUpdate(stateText.value)
	);
});
...
export const releaseManagedTunnelOnShutdown = Effect.fn(
	...
	// tunnel to release.
	const runtimeConfig = yield* dependencies.secrets.get(
		CLOUD_ENDPOINT_RUNTIME_CONFIG,
	);
	...
	if (
		!(yield* readCliDesiredCloudLink) ||
		(yield* readCliDesiredLinkMode) !== "managed"
	) {
		return false;
	}
	...
	if (yield* pendingUpdateHandoffExists) {
		yield* Effect.logInfo(
			"Keeping the managed tunnel across the update restart",
		);
		return false;
	}
	...
	const storedConfig = yield* dependencies.secrets.get(
		CLOUD_ENDPOINT_RUNTIME_CONFIG,
	);
	if (
...
const readCloudLinkState = Effect.fn("environment.cloud.readLinkState")(
	function* (dependencies: CloudHttpDependencies) {
		...
		return {
			...
			cloudUserId: Option.isSome(cloudUserId)
				? bytesToString(cloudUserId.value)
				: null,
			relayUrl: Option.isSome(relayUrl) ? bytesToString(relayUrl.value) : null,
			relayIssuer: Option.isSome(relayIssuer)
				? bytesToString(relayIssuer.value)
				: null,
...
const cloudMintCredentialHandler = Effect.fn(
	"environment.cloud.mintCredential",
)(
	function* (
		dependencies: CloudHttpDependencies,
		...
		if (
			...
			proofOption.value.environmentId !== environmentId ||
			proofOption.value.sub !== linkedCloudUserId ||
			proofOption.value.cnf.jkt !==
				proofOption.value.clientProofKeyThumbprint ||
			!hasBoundedCloudProofLifetime({ ...proofOption.value, nowSeconds }) ||
			!hasExactScope({
				scopes: proofOption.value.scope,
				expected: "environment:connect",
			})
		) {
			return yield* new EnvironmentHttpUnauthorizedError({
	...
	Effect.fnUntraced(function* (handlers) {
		...
		return handlers
			.handle("linkProof", ({ payload }) =>
				cloudLinkProofHandler(dependencies, payload),
			)
			.handle("relayConfig", ({ payload }) =>
				cloudRelayConfigHandler(dependencies, payload),
			)
			...
			.handle("preferences", ({ payload }) =>
				cloudPreferencesHandler(dependencies, payload),
			)
			.handle("health", ({ payload }) =>
				cloudEnvironmentHealthHandler(dependencies, payload),
			)
