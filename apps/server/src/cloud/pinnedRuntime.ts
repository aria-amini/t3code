...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(
		...
		return yield* Effect.gen(function* () {
			...
			yield* runner
				...
				.pipe(
					...
					Effect.mapError(
						(cause) =>
							new PinnedRuntimeInstallError({ step: installStep, cause }),
					),
					Effect.filterOrFail(
			...
			yield* fs
				...
				.pipe(
					Effect.mapError(
						(cause) =>
							new PinnedRuntimeInstallError({
								step: "recording the completed install",
								cause,
							}),
					),
				);
...
export const ensurePinnedRuntimeInstalled = (
