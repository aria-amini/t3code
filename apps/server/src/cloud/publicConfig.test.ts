...
it.effect(
	"uses the statically injected relay URL when no runtime override exists",
	() =>
		Effect.gen(function* () {
			const relayUrl = yield* makeRelayUrlConfig(
...
it.effect(
	"prefers a runtime relay URL override over the statically injected value",
	() =>
		Effect.gen(function* () {
			const relayUrl = yield* makeRelayUrlConfig(
				"https://embedded.example.test",
...
it.effect(
	"requires a relay URL when the server bundle has no injected value",
...
it.effect("rejects an injected relay URL with a non-origin path", () =>
	makeRelayUrlConfig("https://embedded.example.test/path").pipe(
		provideEnv({}),
		Effect.flip,
	),
);
...
it.effect(
	"derives direct Clerk OAuth endpoints from statically injected public config",
	() =>
		Effect.gen(function* () {
...
it.effect(
	"prefers runtime Clerk OAuth config overrides over statically injected values",
	() =>
		Effect.gen(function* () {
			const config = yield* makeCloudCliOAuthConfig({
			...
			assert.equal(
				...
				"https://runtime.example.test/oauth/token",
			);
...
it.effect(
	"requires Clerk OAuth config when the server bundle has no injected values",
	() =>
		makeCloudCliOAuthConfig({
...
it.effect(
	"reports malformed Clerk publishable keys as typed configuration failures",
	() =>
		Effect.gen(function* () {
...
it("resolves relay client tracing from runtime config with build-time fallback", () => {
		...
		resolveRelayClientTracingConfig(
			{
				T3CODE_RELAY_CLIENT_OTLP_TRACES_URL:
					"https://runtime.example.test/v1/traces",
	...
	assert.equal(
		resolveRelayClientTracingConfig(
			{
				T3CODE_RELAY_CLIENT_OTLP_TRACES_URL:
					"http://insecure.example.test/v1/traces",
				T3CODE_RELAY_CLIENT_OTLP_TRACES_DATASET: "runtime-dataset",
				T3CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN: "runtime-token",
			},
			fallback,
