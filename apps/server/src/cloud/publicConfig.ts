...
declare const __T3CODE_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__:
	| string
	| undefined;
declare const __T3CODE_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__:
	| string
	| undefined;
...
export function resolveRelayClientTracingConfig(
	...
	const tracesUrl =
		env.T3CODE_RELAY_CLIENT_OTLP_TRACES_URL?.trim() || fallback.tracesUrl;
	const tracesDataset =
		env.T3CODE_RELAY_CLIENT_OTLP_TRACES_DATASET?.trim() ||
		fallback.tracesDataset;
	const tracesToken =
		env.T3CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN?.trim() || fallback.tracesToken;
...
export function makeRelayUrlConfig(fallback = buildTimeRelayUrl) {
	...
	return (
		fallback ? runtimeConfig.pipe(Config.withDefault(fallback)) : runtimeConfig
	).pipe(Config.mapOrFail(validateRelayUrl));
}
				...
				Effect.map(
					(clerkFrontendApiUrl) =>
						({
							...
							redirectUri: connectLoopbackRedirectUri(
								CLOUD_CLI_OAUTH_LOOPBACK_PORT,
							),
...
export const hasCloudPublicConfig = Boolean(
	(normalizeSecureRelayUrl(process.env.T3CODE_RELAY_URL ?? "") ??
		buildTimeRelayUrl) &&
