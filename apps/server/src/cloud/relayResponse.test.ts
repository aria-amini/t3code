		...
		new HttpClientError.HttpClientError({
			reason: new HttpClientError.TransportError({
				request: HttpClientRequest.post(
					"https://relay.example.test/link?token=private-token",
				),
...
it.effect(
	"reports the tunnel limit and relay trace instead of a generic 403",
	() =>
		Effect.gen(function* () {
...
it.effect(
	"reports an unrecognized access denial without printing its response body",
	() =>
		Effect.gen(function* () {
			const error = yield* filterRelayResponse(
...
it.effect.each([
	...
	"stops rejected startup links but retries temporary failures: $status",
	({ status, attempts }) =>
		Effect.gen(function* () {
			let requests = 0;
			const result = yield* Effect.suspend(() => {
				...
				return filterRelayResponse(
					response(requests === 1 ? status : 200, "{}"),
				);
			}).pipe(
...
it.effect(
	"keeps the relay failure reason and trace when tunnel cleanup fails",
	() =>
		Effect.gen(function* () {
			const error = yield* filterRelayResponse(
				response(500, {
