...
export const shouldRetryCloudLink = (error: unknown): boolean =>
	!isPermanentCloudLinkError(error);
...
export const filterRelayResponse = Effect.fn("cloud.filter_relay_response")(
	function* (response: HttpClientResponse.HttpClientResponse) {
		...
		if (response.status === 401)
			return yield* new EnvironmentHttpUnauthorizedError({ message });
		if (response.status === 403)
			return yield* new EnvironmentHttpForbiddenError({ message });
		if (
			response.status >= 400 &&
			response.status < 500 &&
			response.status !== 408 &&
