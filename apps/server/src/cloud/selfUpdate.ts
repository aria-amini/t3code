...
export const withRunningThreadContinuation = Effect.fn(
				...
				.pipe(
					Effect.tap((result) => {
						if (
							...
							return Ref.update(
								desktopContinuationTokens,
								HashSet.add(result.desktopUpdateToken),
							);
						}
						return Effect.void;
					}),
	...
	return ServerSelfUpdate.of({
		...
		commitDesktopUpdate: (requestId) =>
			Effect.gen(function* () {
				const shouldContinue = yield* Ref.modify(
					desktopContinuationTokens,
					(tokens) => [
	...
	const update: ServerSelfUpdate["Service"]["update"] = Effect.fn(
		...
		return yield* Effect.gen(function* () {
			...
			yield* Effect.logInfo(
				"Server update prepared; handing off to the service launcher.",
				{
					updateId,
					targetVersion,
