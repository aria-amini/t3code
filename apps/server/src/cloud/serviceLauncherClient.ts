...
export const ServiceLauncherHostProcess =
	Context.Reference<ServiceLauncherProcess>(
...
export class ServiceLauncherClient extends Context.Service<
	ServiceLauncherClient,
	{
		...
		readonly requestUpdate: (input: {
			...
			string,
			ServiceLauncherClientError | ServiceLauncherRejectedError
		>;
...
const resolveStartup = Effect.fn(
	"cloud.service_launcher_client.resolve_startup",
	...
	const context =
		rawContext === undefined
			? undefined
			: decodeServiceLauncherContext(rawContext);
...
export const resolveServiceLauncherMode = Effect.fn(
	"cloud.service_launcher_client.resolve_mode",
)(function* () {
	const { managed } = yield* resolveStartup();
	return { managed };
});

export const make = Effect.fn("cloud.service_launcher_client.make")(
	function* (options?: { readonly currentVersion?: string }) {
		...
		const exchange = (
			...
			Effect.callback<ServiceLauncherParentMessage, ServiceLauncherClientError>(
				(resume) => {
					if (!managed) {
						resume(
							Effect.fail(
								new ServiceLauncherClientError({ operation: "unmanaged" }),
							),
						);
						return;
					}
					...
					const onMessage = (...args: ReadonlyArray<unknown>) => {
						...
						if (reply !== undefined && accept(reply))
							settle(Effect.succeed(reply));
					};
					const onDisconnect = () =>
						settle(
							Effect.fail(
								new ServiceLauncherClientError({ operation: "disconnect" }),
							),
						);
					...
					try {
						...
						settle(
							Effect.fail(
								new ServiceLauncherClientError({ operation: "send", cause }),
							),
						);
					}
				...
				Effect.timeoutOrElse({
					...
					orElse: () =>
						Effect.fail(
							new ServiceLauncherClientError({ operation: "timeout" }),
						),
				}),
			);
		...
		const requestUpdate = (input: {
			...
			exchange(
				...
				(reply) =>
					reply.type === "update-accepted" || reply.type === "update-rejected",
			).pipe(
				Effect.flatMap((reply) =>
					reply.type === "update-accepted"
						...
						: reply.type === "update-rejected"
							...
							: Effect.die(
									"service launcher returned an impossible update response",
								),
				),
			);

		const pending =
			context?.update?.status === "pending" ? context.update : undefined;
		const outcome =
		...
		const prepareTrial =
			pending !== undefined
				? exchange(
						...
						(reply) =>
							reply.type === "committed" && reply.updateId === pending.id,
					).pipe(
						Effect.flatMap((reply) => {
							if (reply.type !== "committed") {
								return Effect.die(
									"service launcher returned an impossible prepared response",
								);
							}
