...
interface HarnessOptions {
	...
	readonly reports?: (
		requestId: string,
	) => readonly DesktopUpdateStatusReport[];
	readonly keepOpen?: boolean;
}

const makeHarness = Effect.fn("test.make_desktop_app_update_harness")(
	function* (options: HarnessOptions = {}) {
		const fs = yield* FileSystem.FileSystem;
		const baseDir = yield* fs.makeTempDirectoryScoped({
			prefix: "t3-desktop-app-update-test-",
		});
			...
			Deferred.await(requestIdDeferred).pipe(
				Effect.map((requestId) => {
					...
					return options.keepOpen
						? Stream.concat(emitted, Stream.never)
						: emitted;
				}),
			),
		);
				...
				Layer.mergeAll(
					DesktopTelemetryReceiver.layerTest({
						requestDesktopUpdate: (requestId) =>
							Deferred.succeed(requestIdDeferred, requestId).pipe(
								Effect.asVoid,
							),
...
it.layer(NodeServices.layer)("desktop app update", (it) => {
	it.effect("is unavailable without desktop mode or the control fd", () =>
		Effect.gen(function* () {
			...
			expect(
				(yield* noFd.service.run(() => Effect.void).pipe(Effect.flip)).reason,
			).toContain("not started by the T3 Code desktop app");
	...
	it.effect(
		"collapses state reports into progress stages and succeeds on installing",
		() =>
			Effect.gen(function* () {
				const { service } = yield* makeHarness({
					reports: (requestId) => [
						...
						report(
							requestId,
							makeState({ status: "available", availableVersion: "1.2.4" }),
						),
						report(
							requestId,
							makeState({ status: "downloading", downloadPercent: 40 }),
						),
						// Reports from another run must be ignored.
						report(
							"other-run",
							makeState({ status: "error", message: "unrelated" }),
						),
						report(
							requestId,
				...
				const stages: string[] = [];
				const result = yield* service.run((stage) =>
	...
	it.effect("maps up-to-date and failed outcomes to readable errors", () =>
		Effect.gen(function* () {
			const upToDate = yield* makeHarness({
				reports: (requestId) => [
					report(requestId, makeState({ status: "up-to-date" }), {
						outcome: "up-to-date",
					}),
				],
			});
			expect(
				(yield* upToDate.service.run(() => Effect.void).pipe(Effect.flip))
					.reason,
			).toBe(
			...
			expect(
				(yield* failed.service.run(() => Effect.void).pipe(Effect.flip)).reason,
			).toBe("feed unreachable");
		}),
	);
	...
	it.effect(
		"fails when the desktop stops reporting before a terminal outcome",
		() =>
			Effect.gen(function* () {
				const { service } = yield* makeHarness({
					reports: (requestId) => [
				...
				expect(
					(yield* service.run(() => Effect.void).pipe(Effect.flip)).reason,
				).toBe("The desktop app stopped reporting its update.");
			}),
	);
	...
	it.effect("allows only one desktop update at a time", () =>
		Effect.gen(function* () {
			...
			expect(
				(yield* service.run(() => Effect.void).pipe(Effect.flip)).reason,
			).toBe("A desktop app update is already in progress.");
