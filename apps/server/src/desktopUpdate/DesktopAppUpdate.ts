...
export const make = Effect.fn("desktopUpdate.desktopAppUpdate.make")(
	function* () {
		...
		const consumeReports = (
			...
			Effect.gen(function* () {
				const lastStage = yield* Ref.make<ServerSelfUpdateProgressStage | null>(
					null,
				);
				const emitStage = (
				...
				const terminal = yield* changes.pipe(
					...
					Stream.mapEffect(
						(
							...
							Option.Option<DesktopUpdateStatusReport>,
							ServerSelfUpdateError
						> =>
				...
				return yield* failWith(
					report.reason ??
						...
						"The desktop app update failed.",
				);
			});

		const run: DesktopAppUpdate["Service"]["run"] = Effect.fn(
			"desktopUpdate.desktopAppUpdate.run",
				...
				Effect.gen(function* () {
					const requestId = yield* crypto.randomUUIDv4.pipe(
						Effect.mapError((error) =>
							failWith(
								"Could not generate a desktop update request id.",
								error,
							),
						),
					);
					...
					yield* receiver
						...
						.pipe(
							Effect.mapError((error) =>
								failWith(
									"Could not reach the T3 Code desktop app on this machine.",
									error,
								),
							),
						);
					return yield* consumeReports(requestId, changes, reportProgress).pipe(
						Effect.onInterrupt(() =>
							receiver.cancelDesktopUpdate(requestId).pipe(Effect.ignore),
						),
					);
				}),
			).pipe(
		...
		const commit: DesktopAppUpdate["Service"]["commit"] = Effect.fn(
			...
			if (!available) {
				return yield* failWith(
					"This server cannot commit a desktop app update.",
				);
			}
			const terminal = yield* Effect.scoped(
				Effect.gen(function* () {
					...
					return yield* reports.pipe(
						Stream.filter(
							(report) =>
								report.requestId === requestId && report.outcome === "failed",
						),
						Stream.runHead,
