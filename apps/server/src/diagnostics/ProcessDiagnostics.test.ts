...
function makeNativeSnapshot(
	...
	return {
		...
		type: "snapshot",
		sequence: 1,
		sampledAtUnixMs: DateTime.toEpochMillis(
...
function makeTelemetryLayer(
	...
	const desktopLayer = desktopSnapshot
		? DesktopTelemetryReceiver.layerTest({
				...
				health: Effect.succeed({
					status: "healthy",
					lastSampleAt: Option.some(
						DateTime.makeUnsafe(desktopSnapshot.sampledAtUnixMs),
					),
					lastError: Option.none(),
				}),
			})
	...
	return ResourceTelemetry.layer.pipe(
		Layer.provide(
			Layer.mergeAll(nativeLayer, desktopLayer, ResourceAttribution.layer),
		),
	);
}
...
describe("ProcessDiagnostics", () => {
	it.effect("projects live process data from resource telemetry", () =>
		Effect.gen(function* () {
			...
			expect(diagnostics.processes.map((process) => process.pid)).toEqual([
				4242,
			]);
	...
	it.effect("rejects stale process identities before signaling", () =>
		Effect.gen(function* () {
			...
			const result = yield* Effect.service(
			...
			expect(result).toEqual({
				...
				signaled: false,
				message: Option.some(
					"Process 4242 no longer matches the selected process identity.",
				),
			});
		}),
	);
	...
	it.effect(
		"refuses to signal when a fresh identity check cannot be collected",
		() =>
			Effect.gen(function* () {
				const snapshot = makeNativeSnapshot([
					{
				...
				const staleTelemetry = yield* Effect.service(
	...
	it.effect("rejects Electron processes as signal targets", () =>
		Effect.gen(function* () {
			...
			expect(result).toEqual({
				...
				signaled: false,
				message: Option.some(
					"Process 4242 is not a signalable T3 backend descendant.",
				),
			});

			const diagnostics = yield* Effect.service(
				ProcessDiagnostics.ProcessDiagnostics,
			).pipe(
