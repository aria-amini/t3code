...
export const make = Effect.fn("makeProcessDiagnostics")(function* () {
	...
	const read: ProcessDiagnostics["Service"]["read"] = refreshedTelemetry.pipe(
		Effect.map((snapshot) => {
			...
			return {
				...
				totalRssBytes: processes.reduce(
					(total, entry) => total + entry.rssBytes,
					0,
				),
				totalCpuPercent: processes.reduce(
					(total, entry) => total + entry.cpuPercent,
					0,
				),
				processes,
				error: Option.map(snapshot.health.native.lastError, (message) => ({
					message,
				})),
			};
		}),
	...
	const signal: ProcessDiagnostics["Service"]["signal"] = Effect.fn(
		...
		const selected = current.value.processes.find(
			(entry) =>
				entry.identity.pid === input.pid &&
				entry.identity.startTimeMs === input.startTimeMs,
		);
		if (!selected) {
			return {
				pid: input.pid,
		...
		if (!canSignalCategory(selected.category)) {
			return {
				...
				signaled: false,
				message: Option.some(
					`Process ${input.pid} is not a signalable T3 backend descendant.`,
				),
			};
		}
