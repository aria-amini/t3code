...
describe("TraceDiagnostics", () => {
	it.effect(
		"aggregates failures, slow spans, log levels, and parse errors",
		() =>
			Effect.sync(() => {
				...
				assert.equal(
					DateTime.formatIso(diagnostics.readAt),
				...
				assert.equal(
					diagnostics.commonFailures[0]?.name,
				...
				assert.equal(
					diagnostics.topSpansByCount[0]?.name,
					"orchestration.dispatch",
				);
			}),
	);
	...
	it.effect("returns a not-found diagnostic when no files are available", () =>
		Effect.sync(() => {
			...
			assert.equal(
				Option.getOrUndefined(diagnostics.error)?.kind,
	...
	it.effect("preserves full failure causes and log messages", () =>
		Effect.sync(() => {
			const longCause =
				`VcsProcessSpawnError: ${"missing executable ".repeat(80)}`.trim();
			const longMessage =
				`provider warning: ${"retrying command ".repeat(80)}`.trim();
			const diagnostics = TraceDiagnostics.aggregateTraceDiagnostics({
				traceFilePath: "/tmp/server.trace.ndjson",
				readAt: DateTime.makeUnsafe("2026-05-05T10:00:00.000Z"),
				files: [
	...
	it.effect(
		"keeps loaded trace data when one rotated trace file fails to read",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"keeps only the slowest span occurrences while aggregating large inputs",
		() =>
			Effect.sync(() => {
				const diagnostics = TraceDiagnostics.aggregateTraceDiagnostics({
					traceFilePath: "/tmp/server.trace.ndjson",
