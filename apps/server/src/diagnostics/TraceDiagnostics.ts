...
export function aggregateTraceDiagnostics(
	...
	const slowSpanThresholdMs =
		input.slowSpanThresholdMs ?? DEFAULT_SLOW_SPAN_THRESHOLD_MS;
	const scannedFilePaths =
		input.scannedFilePaths ?? input.files.map((file) => file.path);
	...
	for (const file of input.files) {
		...
		for (const line of lines) {
			...
			firstSpanAt =
				startedAt &&
				(firstSpanAt === null || DateTime.isLessThan(startedAt, firstSpanAt))
					? startedAt
					: firstSpanAt;
			lastSpanAt =
				lastSpanAt === null || DateTime.isGreaterThan(endedAt, lastSpanAt)
					? endedAt
					: lastSpanAt;
			...
			spanSummary.maxDurationMs = Math.max(
				spanSummary.maxDurationMs,
				durationMs,
			);
			...
			if (isFailure) {
				...
				const isLatestFailure =
					!existing || DateTime.isGreaterThan(endedAt, existing.lastSeenAt);
		...
		commonFailures: [...failuresByKey.values()]
			.toSorted(
				(left, right) =>
					...
					DateTime.toEpochMillis(right.lastSeenAt) -
						DateTime.toEpochMillis(left.lastSeenAt),
			)
			.slice(0, TOP_LIMIT),
		latestFailures: latestFailures
			.toSorted(
				(left, right) =>
					DateTime.toEpochMillis(right.endedAt) -
					DateTime.toEpochMillis(left.endedAt),
			)
			.slice(0, RECENT_LIMIT),
		latestWarningAndErrorLogs: latestWarningAndErrorLogs
			.toSorted(
				(left, right) =>
					DateTime.toEpochMillis(right.seenAt) -
					DateTime.toEpochMillis(left.seenAt),
			)
			.slice(0, RECENT_LIMIT),
		partialFailure: input.partialFailure ? Option.some(true) : Option.none(),
		error: Option.fromNullishOr(input.error),
...
export const make = Effect.gen(function* () {
	...
	const read: TraceDiagnostics["Service"]["read"] = Effect.fn(
		...
		return aggregateTraceDiagnostics({
			...
			slowSpanThresholdMs,
			...(readFailureError
				? { partialFailure: true, error: readFailureError }
				: {}),
		});
	});
