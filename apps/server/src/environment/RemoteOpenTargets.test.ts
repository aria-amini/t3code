			...
			Effect.succeed(
				ChildProcessSpawner.makeHandle({
					...
					exitCode: Effect.succeed(
						ChildProcessSpawner.ExitCode(input.exitCode),
					),
...
const netLayer = (input: { readonly ipv4: boolean; readonly ipv6: boolean }) =>
	Layer.succeed(NetService.NetService, {
		...
		hasListenerOnHost: (_port, host) =>
			Effect.succeed(host === "::1" ? input.ipv6 : input.ipv4),
...
const resolveTargets = (input: {
	...
	Effect.flatMap(RemoteOpenTargets.RemoteOpenTargets, (service) =>
		...
		Effect.provide(
			RemoteOpenTargets.layer.pipe(
				Layer.provide(
					Layer.mergeAll(netLayer(input.sshd), spawnerLayer(input.tailscale)),
				),
			),
		),
	);
