...
it.layer(NodeServices.layer)("ServerEnvironmentLive", (it) => {
	it.effect.each([
		...
		"concurrent initializers recover a $name environment id file",
		({ content }) =>
			Effect.gen(function* () {
				...
				yield* fileSystem.makeDirectory(serverConfig.stateDir, {
					recursive: true,
				});
	...
	it.effect(
		"reports agent activity publishing from the current secret state",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const baseDir = yield* fileSystem.makeTempDirectoryScoped({
	...
	it.effect(
		"advertises desktopAppUpdate only with desktop mode and the control fd",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const baseDir = yield* fileSystem.makeTempDirectoryScoped({
					...
					Effect.gen(function* () {
						...
						Effect.provide(
							ServerEnvironment.layer.pipe(
								Layer.provide(ServerSecretStore.layer),
								Layer.provide(
									ServerConfig.layer({ ...serverConfig, ...overrides }),
								),
							),
						),
					);
				...
				expect(
					withoutFd.capabilities.serverUpdateThreadContinuation,
				).toBeUndefined();
				...
				const error = yield* Effect.gen(function* () {
					...
					Effect.provide(
						ServerEnvironment.layer.pipe(
							...
							Layer.provide(
								Layer.merge(
									ServerConfig.layer(serverConfig),
									failingFileSystemLayer,
								),
							),
						),
					),
