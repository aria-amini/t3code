...
): ExecutionEnvironmentDescriptor["platform"]["os"] {
	switch (platform) {
		case "darwin":
			return "darwin";
		case "linux":
...
const makeIdentity = Effect.gen(function* () {
	...
	const persistEnvironmentId = Effect.fn(
		"ServerEnvironmentIdentity.persistEnvironmentId",
	)(
...
export const identityLayer = Layer.effect(
	ServerEnvironmentIdentity,
	makeIdentity,
);
