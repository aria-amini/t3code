...
describe("resolveServerEnvironmentLabel", () => {
	it.effect("uses hostname fallback regardless of launch mode", () =>
		Effect.gen(function* () {
			const result =
				yield* ServerEnvironmentLabel.resolveServerEnvironmentLabel({
					cwdBaseName: "t3code",
				}).pipe(
					Effect.provide(withHostPlatform(TestLayer, "win32", "macbook-pro")),
				);
		...
		Effect.gen(function* () {
			...
			const result =
				yield* ServerEnvironmentLabel.resolveServerEnvironmentLabel({
					...
					Effect.provide(withHostPlatform(TestLayer, "darwin", "macbook-pro")),
				);
		...
		Effect.gen(function* () {
			...
			const result =
				yield* ServerEnvironmentLabel.resolveServerEnvironmentLabel({
					...
					Effect.provide(withHostPlatform(TestLayer, "linux", "runner-01")),
				);
	...
	it.effect(
		"falls back to the hostname when friendly labels are unavailable",
		() =>
			Effect.gen(function* () {
				const result =
					yield* ServerEnvironmentLabel.resolveServerEnvironmentLabel({
						...
						Effect.provide(
							withHostPlatform(TestLayer, "win32", "JULIUS-LAPTOP"),
						),
					);
	...
	it.effect(
		"falls back to the hostname when the friendly-label command is missing",
		() => {
	...
	it.effect(
		"continues to hostnamectl after a machine-info inspect failure",
		() => {
			const logs: CapturedLog[] = [];
			const logger = Logger.make(({ fiber, message }) => {
				logs.push({
			...
			return Effect.gen(function* () {
				...
				Effect.provide(
					Layer.mergeAll(
						withHostPlatform(
							Layer.merge(ProcessRunnerTest, fileSystemLayer),
							"linux",
							"buildbox",
						),
						Logger.layer([logger], { mergeWithExisting: false }),
