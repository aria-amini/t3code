...
type ServerEnvironmentLabelCommandProbe =
	typeof ServerEnvironmentLabelCommandProbe.Type;
...
const runFriendlyLabelCommand = Effect.fn("runFriendlyLabelCommand")(
	function* (input: {
...
const resolveFriendlyHostLabel = Effect.fn("resolveFriendlyHostLabel")(
	function* () {
		...
		if (platform === "linux") {
			...
			if (machineInfo) {
				const prettyHostname = parseMachineInfoValue(
					machineInfo,
					"PRETTY_HOSTNAME",
				);
				if (prettyHostname) {
...
export const resolveServerEnvironmentLabel = Effect.fn(
	"resolveServerEnvironmentLabel",
)(function* (input: ResolveServerEnvironmentLabelInput) {
