...
export function machineKindFromAppleProductName(
	name: string,
...
export function machineKindFromDmi(input: {
	...
	const vendorAndProduct =
		`${input.sysVendor ?? ""} ${productName}`.toLowerCase();
	if (
		VIRTUALIZATION_MARKERS.some((marker) => vendorAndProduct.includes(marker))
	) {
		return "cloud";
	}
	...
	return input.chassisType === null
		? null
		: (DMI_CHASSIS_KINDS[input.chassisType] ?? null);
}

const readOptionalFile = Effect.fn("readOptionalFile")(function* (
	path: string,
) {
...
const runProbe = Effect.fn("runMachineProbe")(function* (input: {
	...
	return yield* processRunner
		...
		.pipe(
			Effect.map((result) =>
				result.code === 0 ? normalize(result.stdout) : null,
			),
			Effect.catch(() => Effect.succeed(null)),
		);
});
...
const detectDarwinMachineKind = Effect.fn("detectDarwinMachineKind")(
	function* () {
		...
		const productName =
			ioreg?.match(/"product-name"\s*=\s*<"([^"]+)">/)?.[1] ?? null;
		const fromProductName =
			productName === null
				? null
				: machineKindFromAppleProductName(productName);
...
const detectLinuxMachineKind = Effect.fn("detectLinuxMachineKind")(
	function* () {
		const [kernelRelease, chassisType, sysVendor, productName] =
			yield* Effect.all([
...
export const detectServerEnvironmentMachineKind = Effect.fn(
