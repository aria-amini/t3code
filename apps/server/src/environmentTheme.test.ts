...
const encodeThemeFile = Schema.encodeSync(
...
const withEnvironmentThemes = <A, E>(
	...
	Effect.gen(function* () {
		...
		const baseDir = yield* fs.makeTempDirectoryScoped({
			prefix: "t3code-environment-theme-",
		});
		...
		withEnvironmentThemes(
			...
			Effect.gen(function* () {
				const environmentTheme =
					yield* EnvironmentTheme.EnvironmentThemeService;
				const first = yield* environmentTheme.streamChanges.pipe(
					Stream.runHead,
				);
				assert.deepEqual(Option.getOrNull(first), [
					{ id: "nightfall", ...NIGHTFALL_THEME },
				]);
			}),
		),
	);
	...
	it.effect.skipIf(!symlinksSupported)("ignores a symlinked theme file", () =>
		withEnvironmentThemes(
			...
			Effect.gen(function* () {
				...
				yield* fs.symlink(
					outside,
					path.join(environmentThemesDir, "nightfall.json"),
				);
				assert.deepEqual(yield* currentThemes, []);
			}),
		),
		...
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				const environmentTheme =
					yield* EnvironmentTheme.EnvironmentThemeService;
				const seen =
					yield* Queue.unbounded<ReadonlyArray<{ readonly id: string }>>();
