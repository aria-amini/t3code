 ...
 */
export const readThemeFileGuarded = (
	filePath: string,
	maxBytes: number,
): string | null => {
	let fd: number;
	try {
		if (
			NodeFS.constants.O_NOFOLLOW === undefined &&
			NodeFS.lstatSync(filePath).isSymbolicLink()
		) {
			return null;
		}
		fd = NodeFS.openSync(
	...
	for (const entry of entries.toSorted()) {
		...
		if (examined > MAX_THEME_FILES) {
			yield* Effect.logWarning(
				"ignoring environment theme files past the limit",
				{
					path: themesDir,
		...
		if (!environmentThemeFileHasColors(file)) {
			yield* Effect.logWarning("ignoring environment theme without colors", {
				path: filePath,
			});
			continue;
		}
		...
		if (totalBytes > MAX_THEME_TOTAL_BYTES) {
			yield* Effect.logWarning(
				"ignoring environment themes past the total size limit",
				{
					path: themesDir,
...
const make = Effect.gen(function* () {
	...
	const refresh = refreshSemaphore.withPermits(1)(
		Effect.gen(function* () {
			...
			const [changed, next] = yield* Ref.modify(
				...
				(
					previous,
				): readonly [readonly [boolean, PublishedThemes], PublishedThemes] => {
					if (Equal.equals(previous.themes, themes))
	...
	const watchEvents = fs
		.watch(environmentThemesDir)
	...
	yield* Stream.runForEach(watchEvents, () =>
		refresh.pipe(Effect.ignoreCause({ log: true })),
	).pipe(
