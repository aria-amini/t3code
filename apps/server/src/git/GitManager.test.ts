...
function normalizeFakePullRequestSummary(
	raw: unknown,
): GitHubCli.GitHubPullRequestSummary | null {
	...
	const state =
		typeof record.state === "string"
			...
			: undefined;
	const isDraft =
		typeof record.isDraft === "boolean" ? record.isDraft : undefined;
...
function createGitHubCliWithFakeGh(scenario: FakeGhScenario = {}): {
	...
	const prListQueueByHeadSelector = new Map(
		Object.entries(scenario.prListSequenceByHeadSelector ?? {}).map(
			([headSelector, values]) => [headSelector, [...values]],
		),
	);
	...
	const execute: GitHubCli.GitHubCli["Service"]["execute"] = (input) => {
		...
		if (args[0] === "pr" && args[1] === "list") {
			...
			const mappedStdout =
				typeof headSelector === "string"
					...
					: undefined;
			const stdout =
				(mappedQueue ?? mappedStdout ?? prListQueue.shift() ?? "[]") + "\n";
		...
		if (args[0] === "repo" && args[1] === "view") {
			...
			if (
				...
				args.includes("nameWithOwner,url,sshUrl")
			) {
			...
			return Effect.succeed(
				fakeGhOutput(`${scenario.defaultBranch ?? "main"}\n`),
			);
		}
		...
		service: {
				...
				execute({
					...
					Effect.map((raw) =>
						raw
							...
							.filter(
								(entry): entry is GitHubCli.GitHubPullRequestSummary =>
									entry !== null,
							),
					),
				),
			...
			getPullRequest: (input) =>
				execute({
					...
					Effect.map(
						(result) =>
							JSON.parse(result.stdout) as GitHubCli.GitHubPullRequestSummary,
					),
				),
			...
			createRepository: (input) =>
				Effect.fail(
					new GitHubCli.GitHubCliCommandError({
						...
						cwd: input.cwd,
						cause: new Error(
							`Unexpected repository create: ${input.repository}`,
						),
					}),
				),
...
it.layer(GitManagerTestLayer)("GitManager", (it) => {
	it.effect(
		"status includes draft PR metadata when branch already has a draft PR",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"status trims PR metadata returned by gh before publishing it",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
	...
	it.effect(
		"status ignores invalid gh pr list entries and keeps valid ones",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
	...
	it.effect(
		"status preserves lowercase merged and closed PR states from gh json",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				yield* runGit(repoDir, [
					"checkout",
					"-b",
					"feature/status-lowercase-state",
				]);
				const remoteDir = yield* createBareRemote();
				yield* runGit(repoDir, ["remote", "add", "origin", remoteDir]);
				yield* runGit(repoDir, [
	...
	it.effect(
		"status returns an explicit non-repo result for deleted directories",
		() =>
			Effect.gen(function* () {
	...
	it.effect("status briefly caches repeated lookups for the same cwd", () =>
		Effect.gen(function* () {
			...
			expect(
				ghCalls.filter((call) => call.startsWith("pr list ")),
			).toHaveLength(1);
		}),
	);

	it.effect(
		"a warm PR cache does not reread repository identity for status",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				const identityReads = gitConfigReads.filter(
					(key) =>
						key === "branch.feature/status-identity-cache.remote" ||
	...
	it.effect(
		"status skips the provider lookup for a branch that was never pushed",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
	...
	it.effect(
		"branch PR lookup returns null when the repository has no remotes",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
	...
	it.effect(
		"branch PR lookup uses a saved tracked branch without changing checkout",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				expect(
					(yield* runGit(repoDir, ["branch", "--show-current"])).stdout.trim(),
				).toBe("main");
			}),
	);

	it.effect(
		"branch PR lookup uses the default branch from a non-origin remote",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
	...
	it.effect(
		"branch PR lookup uses the saved name after the local branch is deleted",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				expect(
					ghCalls.some((call) =>
						call.includes("--head feature/deleted-local-branch"),
					),
				).toBe(true);
			}),
	);
	...
	it.effect(
		"branch PR lookup recovers a deleted fork branch from its remote-tracking ref",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				expect(
					ghCalls.some((call) =>
						call.includes("--head contributor:feature/deleted-fork-branch"),
					),
				).toBe(true);
			}),
	);
	...
	it.effect(
		"branch PR lookup rejects ambiguous deleted-branch remote refs",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				expect(error).toMatchObject({
					...
					detail:
						"Multiple remotes track feature/ambiguous-remote. Its pull request is ambiguous.",
				});
				expect(ghCalls).toHaveLength(0);
			}),
	);
	...
	it.effect(
		"branch PR lookup does not reuse a cached PR after the remote is repointed",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
	...
	it.effect(
		"branch PR lookup shares the status cache for the same repository identity",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
	...
	it.effect("branch PR lookup propagates provider failures", () =>
		Effect.gen(function* () {
			...
			const refreshError = yield* manager
				.branchPullRequest(
					{ cwd: repoDir, branch: "feature/lookup-failure" },
					{ refresh: true },
				)
				.pipe(Effect.flip);
	...
	it.effect(
		"status finds a merged PR after its remote branch was deleted",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				expect(configuredMerge.stdout.trim()).toBe(
				...
				expect(
					ghCalls.filter((call) => call.startsWith("pr list ")).length,
				).toBeGreaterThan(0);
			}),
	);

	it.effect(
		"status still looks up PRs for a branch pushed without --set-upstream",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
	...
	it("backs off repeated PR lookup failures past the healthy refresh cadence", () => {
		...
		expect(Duration.toMillis(GitManager.prLookupFailureTtl(4))).toBeGreaterThan(
			120_000,
		);
	...
	it.each([
		...
		"reads the repository from the returned PR URL %s",
		(url, expected) => {
	...
	it.effect("distinguishes Enterprise forks with the same head branch", () =>
		Effect.gen(function* () {
			...
			expect(
				yield* manager.branchPullRequest({ cwd: repoDir, branch: "feature" }),
			).toMatchObject({
				number: 1,
	...
	it.effect.each([
		...
		Effect.gen(function* () {
			...
			const output = encodeCliJson([
				{
					...
					web_url:
						"https://gitlab.com/Group/Upstream/Repository/-/merge_requests/2",
					target_branch: "main",
				...
				},
				{
					...
					web_url:
						"https://gitlab.com/Group/Upstream/Repository/-/merge_requests/1",
					target_branch: "main",
					source_branch: branch,
					state: "opened",
			...
			expect(
				yield* manager.branchPullRequest({ cwd: repoDir, branch }),
			).toMatchObject({
				number: 1,
		...
		() =>
			Effect.gen(function* () {
				...
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "fork-pr.txt"),
					"fork pr\n",
				);
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				expect(
					ghCalls.some((call) =>
						call.includes("pr list --head upstream/effect-atom "),
					),
				).toBe(false);
				expect(
					ghCalls.some((call) =>
						call.includes("pr list --head pingdotgg:upstream/effect-atom "),
					),
				).toBe(false);
				expect(
					ghCalls.some((call) =>
						call.includes(
							"pr list --head my-org/upstream:upstream/effect-atom ",
						),
					),
				).toBe(false);
			}),
	...
	it.effect(
		"status does not inherit a merged PR from a feature branch's default upstream",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
	...
	it.effect(
		"status finds a PR pushed under the branch's own name despite a default upstream",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				expect(ghCalls.some((call) => call.includes("--head main"))).toBe(
					false,
				);
			}),
	);
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				expect(
					ghCalls.some((call) =>
						call.includes("--head contributor:feature/fork-plain"),
					),
				).toBe(true);
				expect(ghCalls.some((call) => call.includes("--head main"))).toBe(
					false,
	...
	it.effect(
		"branch PR lookup verifies identity on the fork that holds the own-name ref",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
	...
	it.effect(
		"status keeps an own-name PR when a later lookup fails on a default upstream",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				const upstreamCause =
					"GraphQL rate limit for user ID 51714798 and token secret-value";
				const { manager } = yield* makeManager({
					ghScenario: {
	...
	it.effect(
		"status keeps the last known PR when the branch gains its first upstream",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
	...
	it.effect(
		"status drops the last known PR when the tracked remote is repointed",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
	...
	it.effect(
		"status keeps the last known PR when the current remote URL can't be resolved",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
	...
	it.effect(
		"falls back when the dedicated source control writer is unavailable",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
	...
	it.effect(
		"includes local agent instructions when recent history is empty",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
	...
	it.effect("commits only selected files when filePaths is provided", () =>
		Effect.gen(function* () {
			...
			const statusStdout = yield* runGit(repoDir, [
				"status",
				"--porcelain",
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				yield* runGit(repoDir, ["remote", "add", "origin", remoteDir]);
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "feature.txt"),
					"feature\n",
				);
	...
	it.effect(
		"pushes existing clean commits without rerunning commit logic",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				yield* runGit(repoDir, ["remote", "add", "origin", remoteDir]);
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "push-only.txt"),
					"push only\n",
				);
	...
	it.effect(
		"pushes existing commits without committing dirty worktree changes",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				yield* runGit(repoDir, ["remote", "add", "origin", remoteDir]);
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "push-dirty.txt"),
					"push dirty\n",
				);
				...
				NodeFS.writeFileSync(
					NodePath.join(repoDir, ".vercel", "project.json"),
					"{}\n",
				);
	...
	it.effect(
		"create_pr pushes a clean branch before creating the PR when needed",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				yield* runGit(repoDir, ["remote", "add", "origin", remoteDir]);
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "create-pr-only.txt"),
					"create pr\n",
				);
				...
				expect(
					ghCalls.some((call) =>
						call.includes(
							"pr create --base main --head feature/create-pr-only",
						),
					),
				).toBe(true);
			}),
	...
	it.effect(
		"create_pr falls back to main when source control provider detection fails",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "provider-fallback.txt"),
					"fallback\n",
				);
				...
				expect(
					ghCalls.some((call) =>
						call.includes(
							"pr create --base main --head feature/provider-fallback",
						),
					),
				).toBe(true);
			}),
	...
	it.effect(
		"create_pr targets the remote default branch when it is not main",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "master-default.txt"),
					"master default\n",
				);
				...
				expect(
					ghCalls.some((call) =>
						call.includes(
							"pr create --base master --head feature/master-default",
						),
					),
				).toBe(true);
			}),
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				expect(ghCalls.some((call) => call.startsWith("pr create "))).toBe(
					false,
				);
			}),
		12_000,
		...
		() =>
			Effect.gen(function* () {
				...
				const openLookupCalls = ghCalls.filter((call) =>
					call.includes("--state open --limit 1"),
				);
	...
	it.effect(
		"rejects same-repo PR metadata when matching a cross-repo head context",
		() =>
			Effect.sync(() => {
				const headContext = {
	...
	it.effect(
		"accepts fork PR metadata when origin is the fork checkout remote",
		() =>
			Effect.sync(() => {
				const headContext = {
	...
	it.effect("creates PR when one does not already exist", () =>
		Effect.gen(function* () {
			...
			let generatedPolicy: TextGeneration.PrContentGenerationInput["policy"] =
				undefined;
	...
	it.effect(
		"generates PR content against the remote base when the local base is stale",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				yield* runGit(repoDir, [
					"config",
					"branch.feature/remote-base.gh-merge-base",
					"main",
				]);
		...
		() =>
			Effect.gen(function* () {
				...
				expect(
					ghCalls.some((call) =>
						call.includes(
							"pr create --base statemachine --head octocat:statemachine",
						),
					),
				).toBe(false);
			}),
	...
	it.effect(
		"prepares pull request threads in local mode by checking out the PR branch",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				const branch = (yield* runGit(repoDir, [
					"branch",
					"--show-current",
				])).stdout.trim();
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "upstream.txt"),
					"upstream\n",
				);
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "no-head-repo.txt"),
					"upstream\n",
				);
	...
	it.effect(
		"prepares pull request threads in worktree mode on the PR head branch",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "worktree.txt"),
					"worktree\n",
				);
	...
	it.effect(
		"preserves both branch materialization failures when the fallback also fails",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
	...
	it.effect(
		"preserves fork upstream tracking when preparing a worktree PR thread",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
	...
	it.effect(
		"preserves fork upstream tracking when preparing a local PR thread",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "local-fork.txt"),
					"local fork\n",
				);
	...
	it.effect(
		"derives fork repository identity from PR URL when GitHub omits nameWithOwner",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "derived-fork.txt"),
					"derived fork\n",
				);
	...
	it.effect(
		"reuses an existing dedicated worktree for the PR head branch",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				yield* runGit(repoDir, [
					"checkout",
					"-b",
					"feature/pr-existing-worktree",
				]);
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "existing.txt"),
					"existing\n",
				);
				...
				expect(
					result.worktreePath &&
						NodeFS.realpathSync.native(result.worktreePath),
				).toBe(NodeFS.realpathSync.native(worktreePath));
	...
	it.effect(
		"refreshes a reused PR worktree onto the updated pull request head",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				yield* runGit(repoDir, [
					...
					"-b",
					"author-push",
					"origin/feature/pr-reused-stale",
				]);
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "authored.txt"),
					"authored\n",
				);
				...
				yield* runGit(repoDir, [
					"push",
					"origin",
					"author-push:feature/pr-reused-stale",
				]);
				...
				expect(
					(yield* runGit(worktreePath, ["rev-parse", "HEAD"])).stdout.trim(),
				).toBe(updatedHead);
			}),
	);

	it.effect(
		"runs the setup script when a reused PR worktree moves onto the new head",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "reused-setup.txt"),
					"reused setup\n",
				);
				...
				yield* runGit(repoDir, [
					"push",
					"origin",
					"setup-author-push:feature/pr-reused-setup",
				]);
				yield* runGit(repoDir, ["checkout", "main"]);
	...
	it.effect(
		"leaves the setup script alone when a reused PR worktree is already on the head",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "reused-current.txt"),
					"reused current\n",
				);
				...
				const currentHead = (yield* runGit(worktreePath, [
					"rev-parse",
					"HEAD",
				])).stdout.trim();
	...
	it.effect(
		"resets a clean reused PR worktree onto a force-pushed pull request head",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "force-pushed.txt"),
					"first\n",
				);
				...
				const staleHead = (yield* runGit(worktreePath, [
					"rev-parse",
					"HEAD",
				])).stdout.trim();
				...
				const rewrittenHead = (yield* runGit(repoDir, [
					"rev-parse",
					"author-rewrite",
				])).stdout.trim();
				...
				expect(
					NodeFS.readFileSync(
						NodePath.join(worktreePath, "force-pushed.txt"),
						"utf8",
					),
				).toBe("rewritten\n");
			}),
	...
	it.effect(
		"keeps a reused PR worktree that carries its own commit off the rewritten head",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "local-commit.txt"),
					"first\n",
				);
				...
				const upstreamHead = (yield* runGit(worktreePath, [
					"rev-parse",
					"HEAD",
				])).stdout.trim();
				...
				NodeFS.writeFileSync(
					NodePath.join(worktreePath, "thread-work.txt"),
					"thread work\n",
				);
				...
				const worktreeHead = (yield* runGit(worktreePath, [
					"rev-parse",
					"HEAD",
				])).stdout.trim();
				...
				expect(
					NodeFS.existsSync(NodePath.join(worktreePath, "thread-work.txt")),
				).toBe(true);
				expect(setupCalls).toHaveLength(0);
			}),
	);
	...
	it.effect(
		"keeps a dirty reused PR worktree off the rewritten pull request head",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "dirty.txt"),
					"rewritten\n",
				);
				yield* runGit(repoDir, ["add", "dirty.txt"]);
				yield* runGit(repoDir, [
				...
				NodeFS.writeFileSync(
					NodePath.join(worktreePath, "dirty.txt"),
					"uncommitted edit\n",
				);
				...
				expect(
					NodeFS.readFileSync(NodePath.join(worktreePath, "dirty.txt"), "utf8"),
	...
	it.effect(
		"refreshes a reused PR worktree that has no upstream from the pull request ref",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "ref-only.txt"),
					"ref only\n",
				);
				...
				yield* runGit(repoDir, [
					"push",
					"origin",
					"ref-only-author:refs/pull/90/head",
				]);
				const updatedHead = (yield* runGit(repoDir, [
					"rev-parse",
					"ref-only-author",
	...
	it.effect(
		"never moves an unrelated local branch that shares the fork head branch name",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "contributor.txt"),
					"contributor\n",
				);
				...
				const localMainBefore = (yield* runGit(repoDir, [
					"rev-parse",
					"main",
				])).stdout.trim();
				...
				expect(
					(yield* runGit(repoDir, ["rev-parse", "main"])).stdout.trim(),
				).toBe(localMainBefore);
				expect(
				...
				expect(
					NodeFS.existsSync(NodePath.join(mainWorktreePath, "contributor.txt")),
				).toBe(false);
				expect(result.isOnPullRequestHead).toBe(false);
				expect(setupCalls).toHaveLength(0);
			}),
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "fork-main.txt"),
					"fork main\n",
				);
				...
				yield* runGit(repoDir, [
					...
					"-u",
					"fork-seed",
					"fork-main-source:main",
				]);
				yield* runGit(repoDir, ["checkout", "main"]);
				const mainBefore = (yield* runGit(repoDir, [
					"rev-parse",
					"main",
				])).stdout.trim();
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				yield* runGit(repoDir, ["checkout", "-b", "fork-main-source"]);
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "fork-main-second.txt"),
					"fork main second\n",
				);
	...
	it.effect(
		"reuses an existing PR worktree and restores fork upstream tracking",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "reused-fork.txt"),
					"reused fork\n",
				);
	...
	it.effect(
		"does not fail PR worktree prep when setup terminal startup fails",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "setup-failure.txt"),
					"setup failure\n",
				);
	...
	it.effect(
		"rejects worktree prep when the PR head branch is checked out in the main repo",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
	...
	it.effect("emits action_failed when a commit hook rejects", () =>
		Effect.gen(function* () {
			...
			yield* initRepo(repoDir);
			NodeFS.writeFileSync(
				NodePath.join(repoDir, "hook-failure.txt"),
				"broken\n",
			);
			NodeFS.writeFileSync(
				NodePath.join(repoDir, ".git", "hooks", "pre-commit"),
			...
			expect(errorMessage).toContain(
				"Git command failed in GitVcsDriver.commit.commit",
			);
	...
	it.effect(
		"create_pr emits only the PR phase when the branch is already pushed",
		() =>
			Effect.gen(function* () {
				const repoDir = yield* makeTempDir("t3code-git-manager-");
				yield* initRepo(repoDir);
				...
				yield* runGit(repoDir, ["remote", "add", "origin", remoteDir]);
				NodeFS.writeFileSync(
					NodePath.join(repoDir, "pr-only.txt"),
					"pr only\n",
				);
