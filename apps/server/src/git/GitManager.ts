...
export interface GitRemoteStatusOptions
	extends GitVcsDriver.GitRemoteStatusOptions {
...
export class GitManager extends Context.Service<
	GitManager,
	{
		...
		readonly invalidateRemoteStatus: (
			cwd: string,
		) => Effect.Effect<void, never>;
 ...
 */
export function prLookupFailureTtl(
	consecutiveFailures: number,
): Duration.Duration {
	const exponent = Math.max(0, consecutiveFailures - 1);
	const backoffMs =
		Duration.toMillis(PR_LOOKUP_FAILURE_BASE_TTL) * Math.pow(2, exponent);
	return Duration.min(Duration.millis(backoffMs), PR_LOOKUP_FAILURE_MAX_TTL);
}
type StripProgressContext<T> = T extends any
	? Omit<T, "actionId" | "cwd" | "action">
	: never;
type GitActionProgressPayload = StripProgressContext<GitActionProgressEvent>;
type GitActionProgressEmitter = (
...
const pullRequestUpdatedAtDescOrder: Order.Order<PullRequestInfo> =
	Order.mapInput(
...
function parseRepositoryNameFromPullRequestUrl(url: string): string | null {
	...
	const match = /^https?:\/\/[^/]+\/[^/]+\/([^/]+)\/pull\/\d+(?:\/.*)?$/i.exec(
		trimmed,
	);
...
function resolveHeadRepositoryNameWithOwner(
	...
	const explicitRepository =
		pullRequest.headRepositoryNameWithOwner?.trim() ?? "";
...
function resolvePullRequestWorktreeLocalBranchName(
	...
	const sanitizedHeadBranch = sanitizeBranchFragment(
...
function parseRepositoryOwnerLogin(
...
function normalizeOptionalOwnerLogin(
	value: string | null | undefined,
): string | null {
	const normalized = normalizeOptionalString(value);
	return normalized ? normalized.toLowerCase() : null;
...
function resolvePullRequestHeadRepositoryNameWithOwner(
	...
	const explicitRepository = normalizeOptionalString(
...
function resolveExpectedHeadIdentity(
	headContext: Pick<
		BranchHeadContext,
		"headRepositoryNameWithOwner" | "headRepositoryOwnerLogin"
	>,
...
export const make = Effect.gen(function* () {
	...
	const projectSetupScriptRunner =
	...
	const sourceControlProvider = (cwd: string) =>
		sourceControlProviders.resolve({ cwd });
	const serverSettingsService = yield* ServerSettings.ServerSettingsService;
	const readRepositoryInstructions = (cwd: string, fileName: string) =>
		Effect.gen(function* () {
			const root = yield* fileSystem.realPath(cwd);
			const instructionPath = yield* fileSystem.realPath(
		...
		Effect.gen(function* () {
			switch (settings.style.mode) {
				...
				case "repo_conventions": {
					...
					const agentInstructions = yield* readRepositoryInstructions(
						cwd,
						"AGENTS.md",
					);
	...
	const configurePullRequestHeadUpstreamBase = Effect.fn(
		...
		const cloneUrls = yield* (yield* sourceControlProvider(
			cwd,
	...
	const configurePullRequestHeadUpstream = (
		...
		configurePullRequestHeadUpstreamBase(cwd, pullRequest, localBranch).pipe(
			Effect.catch((error) =>
				Effect.logWarning(
					"GitManager.configurePullRequestHeadUpstream failed",
					{
						cwd,
						localBranch,
	...
	const materializePullRequestHeadBranchBase = Effect.fn(
	...
	const localStatusResultCache = yield* Cache.makeWith(readLocalStatus, {
		...
		timeToLive: (exit) =>
			Exit.isSuccess(exit) ? STATUS_RESULT_CACHE_TTL : Duration.zero,
	});
	...
	const resolveLastKnownPr = (
		...
		current: Pick<
			LastKnownPr,
			"upstreamRef" | "headBranch" | "remoteName" | "headRemoteUrlKey"
		>,
		...
		if (
			lastKnown.headRemoteUrlKey !== null &&
	...
	const resolveHostingProvider = Effect.fn("resolveHostingProvider")(function* (
		...
		const preferredRemoteName =
			branch === null
				? "origin"
				: ((yield* readConfigValueNullable(cwd, `branch.${branch}.remote`)) ??
					"origin");
		const remoteUrl =
			(yield* readConfigValueNullable(
				cwd,
	...
	const resolveRemoteRepositoryContext = Effect.fn(
		"resolveRemoteRepositoryContext",
	)(function* (cwd: string, remoteName: string | null) {
		if (!remoteName) {
			return {
				remoteUrlKey: null,
		...
		const remoteUrl = yield* readConfigValueNullable(
			cwd,
			`remote.${remoteName}.url`,
		);
		const repositoryNameWithOwner =
			parseRepositoryNameWithOwnerFromRemoteUrl(remoteUrl);
		return {
	...
	const resolvePrLookupRepositoryIdentity = Effect.fn(
		"resolvePrLookupRepositoryIdentity",
		...
		return {
			remoteName,
			headRemoteUrlKey:
				headRemote.remoteUrlKey ??
				(remoteName === null ? targetRemote.remoteUrlKey : null),
			targetRemoteUrlKey: targetRemote.remoteUrlKey,
		};
	});

	const resolveBranchHeadContext = Effect.fn("resolveBranchHeadContext")(
		function* (
			cwd: string,
			details: {
				branch: string;
			...
			const remoteAliasHeadSelector =
				remoteName && headBranch.length > 0
			...
			if (isCrossRepository && shouldProbeRemoteOwnedSelectors) {
				...
				appendUnique(
					headSelectors,
					remoteAliasHeadSelector !== ownerHeadSelector
			...
			appendUnique(
				headSelectors,
				headBranch !== details.branch ? headBranch : null,
			);
			if (!isCrossRepository && shouldProbeRemoteOwnedSelectors) {
				appendUnique(headSelectors, ownerHeadSelector);
				appendUnique(
			...
			return {
				...
				preferredHeadSelector:
					ownerHeadSelector && isCrossRepository
						? ownerHeadSelector
						: headBranch,
				remoteName,
				headRemoteUrlKey:
	...
	const findRemoteTrackingRemote = Effect.fn("findRemoteTrackingRemote")(
		function* (
			cwd: string,
			branch: string,
			...
			return yield* Effect.gen(function* () {
				...
				const matching = remoteNames.filter((name) =>
					refs.has(`refs/remotes/${name}/${branch}`),
				);
				if (
					preferredRemoteName !== null &&
					matching.includes(preferredRemoteName)
				) {
	...
	const resolveLookupHeadContext = Effect.fn("resolveLookupHeadContext")(
		function* (
			cwd: string,
			details: {
				branch: string;
			...
			const upstreamHeadIsDefault =
				...
				(details.defaultBranch === null &&
					(headContext.headBranch === "main" ||
						headContext.headBranch === "master"));
			if (
	...
	const isUnpublishedBranch = Effect.fn("isUnpublishedBranch")(function* (
		...
		return yield* Effect.gen(function* () {
			const [configuredRemote, configuredMerge] = yield* Effect.all(
				[
					gitCore.readConfigValue(
						cwd,
						`branch.${headContext.localBranch}.remote`,
					),
					gitCore.readConfigValue(
						cwd,
						`branch.${headContext.localBranch}.merge`,
					),
				],
				{ concurrency: "unbounded" },
			);
			...
			const [tracksAnyRemote, tracksThisBranch] = yield* Effect.all(
				[
					matchesRef("refs/remotes"),
					matchesRef(`refs/remotes/*/${headContext.headBranch}`),
				],
				{ concurrency: "unbounded" },
			);
	...
	const findOpenPr = Effect.fn("findOpenPr")(function* (
		...
		for (const headSelector of headContext.headSelectors) {
			const pullRequests = yield* (yield* sourceControlProvider(
				cwd,
			)).listChangeRequests({
				cwd,
				headSelector,
	...
	const findLatestPrForHeadContext = Effect.fn("findLatestPrForHeadContext")(
		function* (cwd: string, headContext: BranchHeadContext) {
	...
	const buildCompletionToast = Effect.fn("buildCompletionToast")(function* (
		...
		const terms = yield* sourceControlProvider(cwd).pipe(
			Effect.map((provider) =>
				getChangeRequestTerminologyForKind(provider.kind),
			),
	...
	const resolveBaseBranch = Effect.fn("resolveBaseBranch")(function* (
		...
		const defaultFromRemote = yield* gitCore.resolvePrimaryRemoteName(cwd).pipe(
			Effect.flatMap((remoteName) =>
				gitCore.resolveDefaultBranchName(cwd, remoteName),
			),
			Effect.orElseSucceed(() => null),
		);
	...
	const resolveCommitAndBranchSuggestion = Effect.fn(
	...
	const runCommitStep = Effect.fn("runCommitStep")(function* (
		...
		let suggestion: CommitAndBranchSuggestion | null | undefined =
	...
	const localStatus: GitManager["Service"]["localStatus"] = Effect.fn(
		"localStatus",
	)(function* (input) {
		const cacheKey = yield* normalizeStatusCacheKey(input.cwd);
		return yield* Cache.get(localStatusResultCache, cacheKey);
	});
	const remoteStatus: GitManager["Service"]["remoteStatus"] = Effect.fn(
		"remoteStatus",
	...
	const status: GitManager["Service"]["status"] = Effect.fn("status")(
		function* (input) {
			const [local, remote] = yield* Effect.all(
				[localStatus(input), remoteStatus(input)],
				{
					concurrency: "unbounded",
				},
			);
	...
	const branchPullRequest: GitManager["Service"]["branchPullRequest"] =
		Effect.fn("branchPullRequest")(function* ({ cwd, branch }, options) {
			...
			const defaultRemoteName = remoteNames.includes("origin")
				? "origin"
				: firstRemoteName;
			...
			const canVerifyIdentity = (
				...
				!(
					...
					(headContext.targetRemoteUrlKey !== null &&
						identity.targetRemoteUrlKey === null)
				);
	...
	const invalidateLocalStatus: GitManager["Service"]["invalidateLocalStatus"] =
		Effect.fn("invalidateLocalStatus")(function* (cwd) {
			yield* invalidateLocalStatusResultCache(cwd);
		});
	const invalidateRemoteStatus: GitManager["Service"]["invalidateRemoteStatus"] =
		Effect.fn("invalidateRemoteStatus")(function* (cwd) {
			yield* invalidateRemoteStatusResultCache(cwd);
		});
	const invalidateStatus: GitManager["Service"]["invalidateStatus"] = Effect.fn(
		"invalidateStatus",
	)(function* (cwd) {
		yield* invalidateLocalStatusResultCache(cwd);
		yield* invalidateRemoteStatusResultCache(cwd);
	...
	const resolvePullRequest: GitManager["Service"]["resolvePullRequest"] =
		Effect.fn("resolvePullRequest")(function* (input) {
	...
	const preparePullRequestThread: GitManager["Service"]["preparePullRequestThread"] =
		Effect.fn("preparePullRequestThread")(function* (input) {
			const maybeRunSetupScript = (worktreePath: string) => {
				...
				return projectSetupScriptRunner
					...
					.pipe(
						Effect.catch((error) =>
							Effect.logWarning(
								"GitManager.preparePullRequestThread setup script failed",
								{
									threadId: input.threadId,
									worktreePath,
									cause: error,
			...
			return yield* Effect.gen(function* () {
				const normalizedReference = normalizePullRequestReference(
					input.reference,
				);
				...
				if (input.mode === "local") {
					yield* (yield* sourceControlProvider(
						input.cwd,
				...
				const ensureExistingWorktreeUpstream = Effect.fn(
					"ensureExistingWorktreeUpstream",
				...
				const findLocalHeadBranch = Effect.fn("findLocalHeadBranch")(function* (
					...
					const localBranch = result.refs.find(
						(branch) =>
							!branch.isRemote && branch.name === localPullRequestBranch,
					);
					if (localBranch) {
						return localBranch;
					}
					...
					for (const branch of result.refs) {
						...
						const worktreePath = yield* canonicalizeExistingPath(
							branch.worktreePath,
						);
				...
				const existingBranchBeforeFetchPath =
				...
				const existingBranchAfterFetchPath =
	...
	const runStackedAction: GitManager["Service"]["runStackedAction"] = Effect.fn(
		"runStackedAction",
	)(function* (input, options) {
		const progress = yield* createProgressEmitter(input, options);
		const currentPhase = yield* Ref.make<Option.Option<GitActionProgressPhase>>(
			Option.none(),
		);

		const runAction = Effect.fn("runStackedAction.runAction")(
			...
			> {
				...
				const changeRequestTerms = wantsPr
					? yield* sourceControlProvider(input.cwd).pipe(
							...
							Effect.orElseSucceed(() =>
								getChangeRequestTerminologyForKind("unknown"),
							),
						)
					: null;
