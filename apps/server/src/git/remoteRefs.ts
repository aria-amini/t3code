export function parseRemoteNamesInGitOrder(
	stdout: string,
...
export function parseRemoteNames(stdout: string): ReadonlyArray<string> {
	return parseRemoteNamesInGitOrder(stdout).toSorted(
...
export function extractBranchNameFromRemoteRef(
	...
	if (normalized.startsWith("refs/remotes/")) {
		return extractBranchNameFromRemoteRef(
			normalized.slice("refs/remotes/".length),
			options,
		);
	}

	const remoteNames = options?.remoteName
		? [options.remoteName]
		: (options?.remoteNames ?? []);
