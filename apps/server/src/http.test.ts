...
const fileResponseLayer = Layer.mergeAll(
...
describe("video asset byte ranges", () => {
	it.effect(
		"uses current descriptor metadata after an in-place truncate or extension",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
				const directory = yield* fs.makeTempDirectoryScoped({
					prefix: "t3-guarded-current-stat-",
				});
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				const path = yield* Path.Path;
				const directory = yield* fs.makeTempDirectoryScoped({
					prefix: "t3-guarded-offset-limit-",
				});
				const filePath = path.join(directory, "clip.mp4");
				yield* fs.writeFileString(filePath, "0123456789");
				const canonicalPath = yield* fs.realPath(filePath);
				...
				for (const [range, status] of [
					...
					yield* Effect.addFinalizer(() =>
						Effect.sync(() => statSpy.mockRestore()),
					);
					const response = HttpServerResponse.toWeb(
						yield* assetFileResponse(
							{ path: canonicalPath, file, mimeType: "video/mp4" },
	...
	it.effect(
		"streams guarded file ranges, including suffixes and conditional requests",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
				const directory = yield* fs.makeTempDirectoryScoped({
					prefix: "t3-guarded-range-",
				});
				const filePath = path.join(directory, "clip.mp4");
				yield* fs.writeFileString(filePath, "0123456789");
				const canonicalPath = yield* fs.realPath(filePath);
	...
	it.effect(
		"closes guarded descriptors after full, HEAD, rejected, and cancelled responses",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
				const directory = yield* fs.makeTempDirectoryScoped({
					prefix: "t3-guarded-cleanup-",
				});
				const filePath = path.join(directory, "clip.mp4");
	...
	it.effect(
		"streams exactly the requested bytes and leaves full downloads intact",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
				const directory = yield* fs.makeTempDirectoryScoped({
					prefix: "t3-video-range-",
				});
				...
				for (const [header, expected, contentRange] of [
					...
					const response = HttpServerResponse.toWeb(
						yield* assetFileResponse(asset, header),
					);
...
describe("assetResponseHeaders", () => {
	...
	it("serves inline attachment documents with their declared mime type", () => {
		expect(
			assetResponseHeaders("/attachments/upload.bin", {
	...
	it("downloads uploaded documents without executing their content", () => {
		expect(
			assetResponseHeaders("/attachments/upload.html", { download: true }),
		).toMatchObject({
...
describe("downloadContentDisposition", () => {
	it("quotes plain names and strips quotes and control characters", () => {
		expect(downloadContentDisposition("report.pdf")).toBe(
			'attachment; filename="report.pdf"',
		);
		expect(downloadContentDisposition('we"ird\n.pdf')).toBe(
			'attachment; filename="we_ird_.pdf"',
		);
	});
