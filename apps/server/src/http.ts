		...
		...(options?.download
			? {
					...
					"Content-Type":
						...
						isSafeDownloadMimeType(options.mimeType)
							? options.mimeType
							: "application/octet-stream",
				}
			: inlineMimeType !== undefined &&
				  isSafeInlineVideoMimeType(inlineMimeType)
				? { "Content-Type": inlineMimeType }
				: inlineMimeType !== undefined &&
					  isSafeInlineDocumentMimeType(inlineMimeType)
					? {
							"Content-Type":
...
function assetByteRange(header: string, size: bigint) {
	...
	const end =
		first === null || last === null || last >= size ? size - 1n : last;
	if (
...
export const assetFileResponse = Effect.fn("assetFileResponse")(function* (
	...
	const mediaInfo = mediaFile
		? yield* statMediaFile(asset.path, mediaFile)
		: undefined;
	const isVideo =
		headers["Content-Type"]?.toLowerCase().startsWith("video/") === true;
	...
	if (mediaFile && mediaInfo) {
		...
		if (!body) {
			return HttpServerResponse.text("File is too large to preview.", {
				status: 413,
			});
		}
...
export const otlpTracesProxyRouteLayer = HttpRouter.add(
	...
	Effect.gen(function* () {
		...
		const browserTraceCollector =
			yield* BrowserTraceCollector.BrowserTraceCollector;
	...
	Effect.gen(function* () {
		...
		if (
			...
			return HttpServerResponse.text(
				"Content-Length must match the upload size.",
				{
					status: 400,
				},
			);
...
const loadImmutableBuildAssets = Effect.gen(function* () {
	...
	const staticDir =
		config.staticDir ??
		(config.devUrl ? yield* ServerConfig.resolveStaticDir() : undefined);
	...
	Stream.unfold(
		...
		Effect.fnUntraced(function* (offset: bigint) {
			...
			const bytes = yield* file.readAlloc(
				remaining < 65_536n ? remaining : 65_536n,
			);
...
const handleStaticAndDevRequest = Effect.fn("handleStaticAndDevRequest")(
	function* (immutableBuildAssets: ReadonlySet<string>) {
		...
		const modifiedAt = isHtml
			? undefined
			: Option.getOrUndefined(fileInfo.mtime);
			...
			ifNoneMatch !== undefined
				? ifNoneMatch.split(",").some((value) => {
						...
						return (
							candidate === "*" ||
							(etag !== undefined &&
								candidate.replace(/^W\//i, "") === etag.slice(2))
						);
					})
		...
		return HttpServerResponse.stream(
			streamStaticFile(opened.file, fileInfo.size),
			{
				headers,
				contentType,
...
export const staticAndDevRouteLayer = Layer.unwrap(
	loadImmutableBuildAssets.pipe(
		Effect.map((assets) =>
			HttpRouter.add("GET", "*", handleStaticAndDevRequest(assets)),
		),
	),
);
