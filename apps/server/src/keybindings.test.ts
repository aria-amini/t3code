...
const decodeKeybindingsConfigJson = Schema.decodeUnknownEffect(
	KeybindingsConfigJson,
);
...
const writeKeybindingsConfig = (
	...
	Effect.gen(function* () {
		...
		yield* fileSystem.makeDirectory(path.dirname(configPath), {
			recursive: true,
		});
...
it.layer(NodeServices.layer)("keybindings", (it) => {
	...
	it.effect(
		"formats invalid resolved keybinding rules with the custom message",
		() =>
			Effect.sync(() => {
	...
	it.effect(
		"replacing with a rule that already exists elsewhere does not duplicate it",
		() =>
			Effect.gen(function* () {
				const { keybindingsConfigPath } = yield* ServerConfig.ServerConfig;
				yield* writeKeybindingsConfig(keybindingsConfigPath, [
