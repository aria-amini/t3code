		...
		SchemaTransformation.transformOrFail({
			...
			encode: (resolved) =>
				Effect.gen(function* () {
					...
					const when = resolved.whenAst
						? encodeWhenAst(resolved.whenAst)
						: undefined;
					return {
						key,
...
function hasSameShortcutContext(
	left: KeybindingRule,
	right: KeybindingRule,
): boolean {
...
function keybindingRuleFromUpsertInput(
...
function replaceTargetFromUpsertInput(
	input: ServerUpsertKeybindingInput,
): KeybindingRule | null {
...
const decodeResolvedKeybindingFromConfigExit = Schema.decodeExit(
	ResolvedKeybindingFromConfig,
);
const decodeRawKeybindingsEntriesExit = Schema.decodeUnknownExit(
	RawKeybindingsEntries,
);
const encodeKeybindingsConfigPrettyJson = Schema.encodeEffect(
...
export class Keybindings extends Context.Service<
	Keybindings,
	{
		 ...
		 */
		readonly syncDefaultKeybindingsOnStartup: Effect.Effect<
			void,
			KeybindingsConfigError
		>;
		 ...
		 */
		readonly loadConfigState: Effect.Effect<
...
const make = Effect.gen(function* () {
	...
	const loadWritableCustomKeybindingsConfig = Effect.fn(
		...
		> {
			...
			return yield* Effect.forEach(rawConfig, (entry) =>
				Effect.gen(function* () {
					...
					const resolved = decodeResolvedKeybindingFromConfigExit(
						decodedRule.value,
					);
	...
	const loadRuntimeCustomKeybindingsConfig = Effect.fn(
		function* (): Effect.fn.Return<
			{
		...
		> {
			...
			for (const [index, entry] of decodedEntries.value.entries()) {
				...
				const resolvedRule = decodeResolvedKeybindingFromConfigExit(
					decodedRule.value,
				);
	...
	const syncDefaultKeybindingsOnStartup = upsertSemaphore.withPermits(1)(
		Effect.gen(function* () {
			...
			const existingCommands = new Set(
				customConfig.map((entry) => entry.command),
			);
			...
			const matchingDefaults = Array.filterMap(
				DEFAULT_KEYBINDINGS,
				(defaultRule) =>
			...
			const availableSlots = Math.max(
				0,
				MAX_KEYBINDINGS_COUNT - customConfig.length,
			);
		...
		const debouncedKeybindingsEvents = fs.watch(keybindingsConfigDir).pipe(
			Stream.filter((event) => {
				return (
					...
					path.resolve(keybindingsConfigDir, event.path) ===
						keybindingsConfigPathResolved
				);
			}),
				...
				Effect.gen(function* () {
					...
					const nextConfig = [
						...customConfig.filter((entry) => {
							if (replaceTarget) {
								return (
									!isSameKeybindingRule(entry, replaceTarget) &&
									!isSameKeybindingRule(entry, rule)
								);
							}
