		...
		Effect.scoped(
			Effect.gen(function* () {
				...
				expect(snapshot.content).toEqual([
					{ type: "text", text: "Preview snapshot failed." },
				]);
...
it.effect(
	"rejects non-boolean snapshot image options before selecting a browser host",
	() =>
		Effect.gen(function* () {
			const server = yield* McpServer.McpServer;
...
it.effect(
	"registers annotated tools and preserves authenticated request context",
	() =>
		Effect.scoped(
			Effect.gen(function* () {
				const server = yield* McpServer.McpServer;
				...
				const statusTool = server.tools.find(
					({ tool }) => tool.name === "preview_status",
				);
				...
				const snapshotTool = server.tools.find(
				...
				const clickTool = server.tools.find(
					({ tool }) => tool.name === "preview_click",
				);
				...
				const navigateTool = server.tools.find(
					({ tool }) => tool.name === "preview_navigate",
				);
				...
				expect(snapshot.isError).toBe(false);
				expect(
					snapshot.content.some((content) => content.type === "image"),
				).toBe(true);
				expect(snapshot.structuredContent).toMatchObject({
					screenshot: { mimeType: "image/png", width: 10, height: 5 },
				});
				expect(
					routedRequests.find(({ operation }) => operation === "snapshot")
						?.tabId,
				).toBe(alternateTabId);
