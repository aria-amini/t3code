...
const tokenFromBytes = (bytes: Uint8Array): string =>
...
const makeWithOptions = Effect.fn("McpSessionRegistry.make")(function* (
	...
	const currentTimeMillis = options.now
		? Effect.sync(options.now)
		: Clock.currentTimeMillis;
	const livenessWindowMs =
		options.livenessWindowMs ?? DEFAULT_LIVENESS_WINDOW_MS;
	const endpoint =
	...
	const issue: McpSessionRegistryShape["issue"] = Effect.fn(
	...
	const resolve: McpSessionRegistryShape["resolve"] = Effect.fn(
		"McpSessionRegistry.resolve",
	)(function* (rawToken) {
	...
	const touch: McpSessionRegistryShape["touch"] = Effect.fn(
		"McpSessionRegistry.touch",
	)(function* (threadId) {
		const timestamp = yield* currentTimeMillis;
		yield* SynchronizedRef.update(state, ({ records }) => {
			const current = pruneDead(records, timestamp);
	...
	const revokeWhere = (predicate: (record: CredentialRecord) => boolean) =>
		SynchronizedRef.update(state, ({ records }) => ({
			records: new Map(
				Array.from(records).filter(([, record]) => !predicate(record)),
			),
		}));
	...
	return McpSessionRegistry.of({
		...
		resolve,
		touch,
		revokeProviderSession: Effect.fn(
			"McpSessionRegistry.revokeProviderSession",
		)(function* (providerSessionId) {
			yield* revokeWhere(
				(record) => record.scope.providerSessionId === providerSessionId,
			);
		}),
		revokeThread: Effect.fn("McpSessionRegistry.revokeThread")(
			function* (threadId) {
...
export const touchActiveMcpThread = (
	...
	activeMcpSessionRegistry
		? activeMcpSessionRegistry.touch(threadId)
		: Effect.void;
