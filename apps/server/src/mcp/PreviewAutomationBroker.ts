...
export class PreviewAutomationBroker extends Context.Service<
	PreviewAutomationBroker,
	{
		...
		readonly focusHost: (
			host: PreviewAutomationHostFocus,
		) => Effect.Effect<void>;
		readonly respond: (
...
): string => `${scope.environmentId}\u0000${scope.providerSessionId}`;
	...
	switch (error._tag) {
		...
		case "PreviewAutomationTargetNotEditableError": {
			...
			return new PreviewAutomationTargetNotEditableError({
				...context,
				...remoteDiagnostics,
				...(remoteSelectorKind === undefined &&
				context.selectorKind === undefined
					? {}
					: { selectorKind: remoteSelectorKind ?? context.selectorKind }),
				...(remoteSelectorLength === undefined &&
				context.selectorLength === undefined
					? {}
...
export const make = Effect.gen(function* PreviewAutomationBrokerMake() {
	...
	const closeConnection = Effect.fn("PreviewAutomationBroker.closeConnection")(
		function* (
			queue: ClientConnection["queue"],
			...
			yield* Effect.forEach(
				...
				({ deferred, context }) =>
					Deferred.fail(
						deferred,
						new PreviewAutomationClientDisconnectedError(context),
					),
				{ discard: true },
			);
	...
	const acquireConnection = Effect.fn(
		"PreviewAutomationBroker.acquireConnection",
	)(function* (host: PreviewAutomationHost) {
		...
		if (registration.previousConnection) {
			yield* closeConnection(
				registration.previousConnection.queue,
	...
	const focusHost: PreviewAutomationBroker["Service"]["focusHost"] = Effect.fn(
		...
		yield* SynchronizedRef.update(state, (current) => {
			...
			const clients = new Map(current.clients);
			const focusSequence = host.focused
				? current.focusSequence + 1
	...
	>(
		...
		const route = yield* SynchronizedRef.modify(state, (current) => {
				...
				supportsOperation(assignedConnection, input.operation)
					...
					: hasLiveAssignment
						...
						: Array.from(current.clients.values())
								...
								.sort(
									(left, right) =>
										right.supportedOperations.size -
											left.supportedOperations.size ||
			...
			const tabId =
				input.tabId ?? (canReuseAssignedTab ? assigned.tabId : undefined);
		...
		const awaitResponse = Effect.fn("PreviewAutomationBroker.awaitResponse")(
			function* () {
				...
				return yield* Option.match(result, {
					onNone: () =>
						Effect.fail(new PreviewAutomationTimeoutError(requestContext)),
		...
		const resultTabId =
			responseTabId === undefined ? input.tabId : responseTabId;
