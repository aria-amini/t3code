	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				const claim = claimPreviewRecording(
					ThreadId.make("thread-1"),
					response,
				);
				const [first, second] = yield* Effect.all([claim, claim], {
				...
				const wrongPath = yield* claimPreviewRecording(
					ThreadId.make("thread-1"),
					{
						...response,
	...
	it.effect.each([6, 5])(
		...
		(sizeBytes) =>
			Effect.gen(function* () {
				...
				const result = yield* claimPreviewRecording(
					ThreadId.make("thread-1"),
					response,
				).pipe(Effect.result);
				if (sizeBytes === 6) {
					...
					expect(parseThreadSegmentFromAttachmentId(result.success.id)).toBe(
						"thread-1",
					);
					expect(yield* fileSystem.readFileString(result.success.path)).toBe(
						"video!",
					);
					expect(yield* fileSystem.exists(pendingPath)).toBe(false);
				} else {
