...
const decodeUploadedRecordingArtifact = Schema.decodeUnknownEffect(
	UploadedRecordingArtifact,
);

export const claimPreviewRecording = Effect.fn("PreviewToolkit.claimRecording")(
	function* (threadId: ThreadId, response: unknown) {
		...
		if (!artifact.uploadedAttachmentId) {
			return yield* new PreviewAutomationRecordingDesktopUpdateRequiredError({
				threadId,
			});
		}
		...
		yield* Effect.gen(function* () {
			...
			Effect.mapError(
				(cause) =>
					new PreviewAutomationRecordingTransferError({ threadId, cause }),
			),
		);
		const { uploadedAttachmentId: _uploadedAttachmentId, ...recording } =
			artifact;
...
const handlers = {
	...
	preview_resize: (input) =>
		invokeTargeted<PreviewAutomationResizeResult>(
			"resize",
			input,
			input.timeoutMs,
		),
	preview_set_appearance: (input) =>
		invokeTargeted<PreviewAutomationSetColorSchemeResult>(
			"setColorScheme",
			input,
		),
...
export const PreviewStandardToolkitHandlersLive =
	PreviewStandardToolkit.toLayer(standardHandlers);

export const PreviewSnapshotToolkitHandlersLive =
	PreviewSnapshotToolkit.toLayer({
		preview_snapshot,
	});
