...
const schemaHasMultipleAllOfDescriptions = (schema: unknown): boolean => {
	...
	return (
		descriptionCount > 1 ||
		Object.values(record).some(schemaHasMultipleAllOfDescriptions)
	);
};
...
it("exports provider-compatible object schemas with described parameters", () => {
	for (const tool of Object.values(PreviewToolkit.tools)) {
		...
		expect(
			schema.type,
			`${tool.name} must export a top-level object schema`,
		).toBe("object");
		expect(
			schema.anyOf,
			`${tool.name} must not export a root anyOf`,
		).toBeUndefined();
		expect(
			schema.oneOf,
			`${tool.name} must not export a root oneOf`,
		).toBeUndefined();
...
it("exports exact object result schemas for preview actions", () => {
	...
	for (const name of actionNames) {
		expect(
			Tool.getJsonSchemaFromSchema(PreviewToolkit.tools[name].successSchema),
		).toEqual({
			type: "object",
