...
describe("RpcInstrumentation", () => {
	it.effect("records success metrics for unary RPC handlers", () =>
		Effect.gen(function* () {
			yield* observeRpcEffect(
				"rpc.instrumentation.success",
				Effect.succeed("ok"),
				{
					"rpc.aggregate": "test",
				},
	...
	it.effect(
		"records subscription activation metrics for stream RPC handlers",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"records failure outcomes for direct stream RPC handlers during consumption",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"records direct stream durations from nanosecond clock readings",
		() =>
			Effect.gen(function* () {
				...
				const snapshots = yield* Metric.snapshot;
				const snapshot = findHistogramSnapshot(
					snapshots,
	...
	it.effect(
		"records failure outcomes when a stream RPC effect produces a failing stream",
		() =>
			Effect.gen(function* () {
				const exit = yield* Stream.runCollect(
					observeRpcStreamEffect(
						...
						Stream.withSpan("rpc.instrumentation.stream.effect.failure.span"),
					),
				).pipe(Effect.exit);
	...
	it.effect("records spans for traced stream RPC handlers", () =>
		Effect.gen(function* () {
					...
					observeRpcStream(
						...
						Stream.fromEffect(
							Effect.succeed("ok").pipe(
								Effect.withSpan("rpc.instrumentation.traced.stream.child"),
							),
						),
						{ "rpc.aggregate": "test" },
					),
			...
			assert.equal(
				spanNames.includes("ws.rpc.rpc.instrumentation.traced.stream"),
				true,
			);
			assert.equal(
				spanNames.includes("rpc.instrumentation.traced.stream.child"),
				true,
			);
		}),
	);
		...
		Effect.gen(function* () {
			const spanNames = yield* collectSpanNames(
				observeRpcEffect(
					...
					Effect.succeed("ok").pipe(
						Effect.withSpan("rpc.instrumentation.disabled.unary.child"),
					),
					{ "rpc.aggregate": "test" },
				),
			);
	...
	it.effect(
		"does not create spans for disabled direct stream RPC handlers",
		() =>
			Effect.gen(function* () {
				const spanNames = yield* collectSpanNames(
					Stream.runCollect(
	...
	it.effect(
		"does not create spans for disabled stream effect RPC handlers",
		() =>
			Effect.gen(function* () {
				const spanNames = yield* collectSpanNames(
					Stream.runCollect(
						observeRpcStreamEffect(
							...
							Effect.succeed(
								Stream.fromEffect(
									Effect.succeed("ok").pipe(
										Effect.withSpan(
											"rpc.instrumentation.disabled.stream.effect.consume",
										),
									),
								),
							).pipe(
								Effect.withSpan(
									"rpc.instrumentation.disabled.stream.effect.create",
								),
							),
							{ "rpc.aggregate": "test" },
						),
