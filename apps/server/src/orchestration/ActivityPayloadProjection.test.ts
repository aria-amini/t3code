...
describe("projectActivityPayload", () => {
	...
	it("keeps a bounded Codex command output summary", () => {
		...
		const data = (projected.payload as Record<string, unknown>).data as Record<
			string,
			unknown
		>;
	...
	it("keeps preview normalization and fence-only fallback while scanning lines", () => {
		...
		expect(
			(preview.payload as { data: { rawOutput: unknown } }).data.rawOutput,
		).toEqual({
			content: "actual result",
		});
		expect(
			(fences.payload as { data: { rawOutput: unknown } }).data.rawOutput,
		).toEqual({
			content: "2 lines",
		});
	});
	...
	it("keeps bounded Claude and ACP command output summaries", () => {
		...
		const claudeData = (claude.payload as Record<string, unknown>)
			.data as Record<string, unknown>;
		const acpData = (acp.payload as Record<string, unknown>).data as Record<
			string,
			unknown
		>;
	...
	it("keeps full Claude Read image paths through repeated projection", () => {
		...
		expect(textRead.payload).not.toMatchObject({
			data: { imagePath: expect.anything() },
		});
	});
			...
			activity({
				...
				data: {
					item: {
						...
						result: {
							content: [
								{ type: "text", text: `PR body line one\n${"x".repeat(5000)}` },
							],
