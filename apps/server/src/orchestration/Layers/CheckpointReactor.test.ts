...
function createGitRepository() {
	const cwd = NodeFS.mkdtempSync(
		NodePath.join(NodeOS.tmpdir(), "t3-checkpoint-handler-"),
	);
...
describe("CheckpointReactor", () => {
	...
	async function createHarness(options?: {
		...
		const refreshAfterTurn = Effect.sync(
			() => void pullRequestRefreshes.push(1),
		);
				...
				Effect.sync(() => {
					...
					Effect.as({
						...
						isDefaultRef:
							...
							options.localStatusRefName === "main",
						refName:
							options?.localStatusRefName !== undefined
		...
		const receipts = await Effect.runPromise(
			Effect.gen(function* () {
				const receipts =
					yield* Queue.unbounded<RuntimeReceiptBus.OrchestrationRuntimeReceipt>();
	...
	effectIt.effect(
		"captures baseline and large turn summaries before completion receipts",
		() =>
			Effect.gen(function* () {
	...
	effectIt.effect(
		"captures and reverts checkpoints from a nested Git workspace",
		() =>
			Effect.gen(function* () {
				...
				yield* Effect.promise(harness.drain);
				expect(
					gitRefExists(repositoryRoot, checkpointRefForThreadTurn(threadId, 0)),
				).toBe(true);
				expect(yield* harness.nextReceipt).toMatchObject({
					type: "checkpoint.baseline.captured",
				});
				...
				expect(
					gitRefExists(repositoryRoot, checkpointRefForThreadTurn(threadId, 1)),
				).toBe(false);
	...
	effectIt.effect.each(["turn.completed", "turn.aborted"] as const)(
		...
		(terminalEventType) =>
			Effect.gen(function* () {
				...
				NodeFS.writeFileSync(
					NodePath.join(harness.cwd, "early.ts"),
				...
				NodeFS.writeFileSync(
					NodePath.join(harness.cwd, "late.ts"),
					"export const late = 2;\n",
				);
				yield* harness.engine.dispatch({
					type: "thread.session.set",
					...
					session: {
						threadId,
						status:
							terminalEventType === "turn.aborted" ? "interrupted" : "ready",
						providerName: "codex",
						runtimeMode: "approval-required",
						activeTurnId: null,
						lastError: null,
				...
				yield* Effect.promise(harness.drain);
				expect(
					gitRefExists(harness.cwd, checkpointRefForThreadTurn(threadId, 1)),
				).toBe(true);
				expect(yield* harness.nextReceipt).toMatchObject({
					type: "checkpoint.diff.finalized",
					turnId,
				...
				expect(
					followUp?.checkpoints.find(
						(checkpoint) => checkpoint.turnId === followUpTurnId,
					),
	...
	effectIt.effect(
		"captures files while the pull request lookup is still pending",
		() =>
			Effect.gen(function* () {
	...
	it("adopts a drifted checkout as the thread branch on a dedicated worktree", async () => {
		...
		await waitForEvent(
			...
			(event) =>
				...
				(event as unknown as { payload: { branch?: string } }).payload
	...
	effectIt.effect(
		"captures a checkpoint without a summary when the baseline is missing",
		() =>
			Effect.gen(function* () {
				const harness = yield* Effect.promise(() =>
					createHarness({ seedFilesystemCheckpoints: false }),
	...
	effectIt.effect.each([
		...
		"resumes checkpointing after git init $timing (commit: $commit)",
		({ timing, commit }) =>
			Effect.gen(function* () {
				const harness = yield* Effect.promise(() =>
					createHarness({
				...
				yield* Effect.promise(harness.drain);
				expect(
					(yield* Effect.promise(harness.readModel)).threads[0]?.checkpoints,
				).toEqual([]);
				...
				yield* Effect.promise(harness.drain);
				const firstCheckpoint = (yield* Effect.promise(harness.readModel))
					.threads[0]?.checkpoints[0];
				...
				expect(
					gitRefExists(harness.cwd, checkpointRefForThreadTurn(threadId, 0)),
				).toBe(timing === "between turns");
	...
	it("does not create checkpoints while importing historical user messages", async () => {
		...
		if (runtime === null)
			throw new Error("Checkpoint test runtime was not initialized.");
	...
	effectIt.effect(
		"rejects unsupported rewind before changing files, checkpoints, or history",
		() =>
			Effect.gen(function* () {
				const harness = yield* Effect.promise(() =>
					createHarness({
						providerName: ProviderDriverKind.make("antigravity"),
					}),
				);
				const threadId = ThreadId.make("thread-1");
				const createdAt = "2026-01-01T00:00:00.000Z";
							...
							Effect.andThen(
								Effect.fail(
									new ProviderValidationError({
										...
										issue:
											"Provider 'antigravity' does not support conversation rewind.",
									}),
								),
							),
						),
				...
				for (const turnCount of [1, 2]) {
					yield* harness.engine.dispatch({
						...
						commandId: CommandId.make(
							`cmd-unsupported-rewind-message-${turnCount}`,
						),
						threadId,
						message: {
							messageId: MessageId.make(
								`message-unsupported-rewind-${turnCount}`,
							),
							role: "user",
					...
					yield* harness.engine.dispatch({
						type: "thread.turn.diff.complete",
						commandId: CommandId.make(
							`cmd-unsupported-rewind-diff-${turnCount}`,
						),
						threadId,
