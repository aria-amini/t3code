...
function checkpointStatusFromRuntime(
...
const make = Effect.gen(function* () {
	...
	const resolveSessionRuntimeForThread = Effect.fn(
	...
	// a git repository.
	const resolveCheckpointCwd = Effect.fn("resolveCheckpointCwd")(
		function* (input: {
			readonly threadId: ThreadId;
			readonly thread: {
	...
	const captureAndDispatchCheckpoint = Effect.fn(
		...
		const assistantMessageId =
			...
			input.thread.messages
				...
				.find(
					(entry) =>
						entry.role === "assistant" && entry.turnId === input.turnId,
				)?.id ??
	...
	const captureCheckpointFromTurnCompletion = Effect.fn(
	...
	const ensurePreTurnBaselineFromTurnStart = Effect.fn(
	...
	const refreshLocalGitStatusFromTurnCompletion = Effect.fn(
		...
		const local = yield* vcsStatusBroadcaster
			...
			.pipe(
				Effect.catch((error) =>
					Effect.logWarning(
						"failed to refresh local git status after turn completion",
						{
	...
	const refreshPullRequestAfterTurn = Effect.fn("refreshPullRequestAfterTurn")(
		function* (input: {
			readonly threadId: ThreadId;
			readonly turnId: TurnId | null;
			readonly cwd: string;
			...
			yield* vcsStatusBroadcaster.refreshPullRequestStatus(input.cwd).pipe(
				Effect.catch((error) =>
					Effect.logWarning(
						"failed to refresh pull request status after turn completion",
						{
							threadId: input.threadId,
							cwd: input.cwd,
							detail: error.message,
	...
	const followWorktreeBranchDrift = Effect.fn("followWorktreeBranchDrift")(
		function* (input: {
			readonly threadId: ThreadId;
			readonly cwd: string;
			readonly local: VcsStatusLocalResult;
			...
			refreshLocalGitStatusFromTurnCompletion(event).pipe(
				Effect.catchCause((cause) =>
					Cause.hasInterruptsOnly(cause)
						...
						: Effect.logWarning(
								"failed to refresh git status after turn completion",
								{
									threadId: event.threadId,
									cause: Cause.pretty(cause),
								},
	...
	const handleRevertRequested = Effect.fn("handleRevertRequested")(function* (
		...
		yield* providerService.assertConversationRollbackSupported(
			event.payload.threadId,
		);
	...
	const processRuntimeEvent = Effect.fn("processRuntimeEvent")(function* (
		...
		if (event.type === "turn.started") {
			...
			const activeTurnId = (yield* providerService.listSessions()).find(
				(session) => sameId(session.threadId, event.threadId),
			)?.activeTurnId;
			const mayReplace =
				pending.has(event.threadId) && sameId(activeTurnId, turnId);
			if (
				turnId !== null &&
	...
	const start: CheckpointReactorShape["start"] = Effect.fn("start")(
		function* () {
			yield* forkParked(
