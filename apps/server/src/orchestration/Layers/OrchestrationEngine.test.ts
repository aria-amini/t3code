...
describe("OrchestrationEngine", () => {
	it.each(["running", "stopped"] as const)(
		...
		async (status) => {
			...
			try {
				...
				expect(
					before.threads[0]?.activities.some(
				...
				expect(
					after.threads[0]?.activities.find(
				...
				const events = await system.run(
					Stream.runCollect(system.engine.readEvents(0)),
				);
				expect(
					Array.from(events)
				...
				const afterEviction = Option.getOrThrow(
					await system.readThread(threadId),
				);
				expect(
					afterEviction.activities.some(
						(activity) => activity.kind === "user-input.resolved",
					),
				).toBe(false);
				if (status === "stopped") {
	...
	it("bootstraps command handling from persisted projections without reading the full snapshot", async () => {
		...
		const eventStore: OrchestrationEventStoreShape = {
			...
			getAggregateReplayStats: () =>
				Effect.die("unused aggregate replay stats"),
		};
	...
	effectIt.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				expect(
					Option.getOrNull(yield* receipts.getByCommandId({ commandId })),
				).toMatchObject({
					commandId,
	...
	effectIt.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				expect(
					afterUpdate.threads.find((thread) => thread.id === guardedThreadId)
						?.updatedAt,
				...
				for (const [taskType, expectedLiveness] of [
					...
					expect(
						backgroundLiveness.getThreadBackgroundLiveness(liveThreadId),
					).toBe(expectedLiveness);
				...
				for (const threadId of [guardedThreadId, liveThreadId]) {
					const thread = settled.threads.find(
	...
	it("archives and unarchives threads through orchestration commands", async () => {
		...
		expect(
			(await system.readModel()).threads.find(
		...
		async (change) => {
			...
			try {
					...
					system.engine.dispatch(
						change === "project"
							...
							: change === "delete"
								? {
										type: "thread.delete",
										commandId: CommandId.make("pr-race-delete"),
										threadId,
									}
								: {
										type: "thread.meta.update",
				...
				const error = await system.run(
					system.engine.dispatch(command).pipe(Effect.flip),
				);
	...
	it("allows authoritative worktree bootstrap to assign a temporary branch", async () => {
		...
		expect(snapshot.threads[0]?.worktreePath).toBe(
			"/tmp/project-worktree-bootstrap-worktree",
		);
		await system.dispose();
	});
		...
		await system.run(
			engine.dispatch({
				...
				checkpointRef: asCheckpointRef(
					"refs/t3/checkpoints/thread-turn-diff/turn/1",
				),
				status: "ready",
				files: [],
		...
		const flakyStore: OrchestrationEventStoreShape = {
			append(event) {
				if (
					...
					event.commandId === CommandId.make("cmd-flaky-1")
				) {
	...
	it("replays the accepted receipt for a genuine retry of the same command", async () => {
		...
		const thread = readModel.threads.find(
			(candidate) => candidate.id === "thread-retry",
		);
		expect(
	...
	it("rejects reusing an accepted command id for a different aggregate", async () => {
		...
		expect(
			targetThread?.messages.filter((message) => message.role === "user"),
		).toHaveLength(0);
	...
	it("stamps the dispatching client's origin onto persisted event metadata", async () => {
		...
		const withOrigin = events.find(
			(event) => event.commandId === "cmd-origin-project-create",
		);
