...
const isOrchestrationCommandIdConflictError = Schema.is(
	OrchestrationCommandIdConflictError,
);
		...
		return Effect.exit(
			Effect.gen(function* () {
				...
				if (Option.isSome(existingReceipt)) {
					...
					if (
						existingReceipt.value.aggregateKind !==
				...
				const plannedEvents = Array.isArray(eventBase)
					? eventBase
					: [eventBase];
					...
					.withTransaction(
						Effect.gen(function* () {
							...
							for (const nextEvent of eventBases) {
								...
								nextCommandReadModel = yield* projectEvent(
									nextCommandReadModel,
									savedEvent,
								);
								const cleanup =
									yield* projectionPipeline.projectEventDeferred(savedEvent);
					...
					if (index === 0) {
						yield* Metric.update(
							...
							Duration.millis(
								Math.max(
									0,
									(yield* Clock.currentTimeMillis) - envelope.startedAtMs,
								),
							),
						);
					}
