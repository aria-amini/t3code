...
it.layer(
	Layer.fresh(
		makeProjectionPipelinePrefixedTestLayer("t3-projection-cursor-batch-"),
	),
...
it.layer(
	Layer.fresh(makeProjectionPipelinePrefixedTestLayer("t3-import-shell-")),
)("imported thread shell projection", (it) => {
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				const readLatestUserMessageAt = sql<{
...
it.layer(
	Layer.fresh(
		makeProjectionPipelinePrefixedTestLayer("t3-branch-pr-projection-"),
	),
)("branch pull request projection", (it) => {
	it.effect(
		"persists branch pull request updates without changing manual links",
		() =>
			Effect.gen(function* () {
				const projectionPipeline = yield* OrchestrationProjectionPipeline;
				const eventStore = yield* OrchestrationEventStore;
				...
				const updates = [
					{
						payload: { linkedPullRequest, branchPullRequest },
        ...
        `;
					assert.deepEqual(rows, [
						{
							linkedPullRequest:
								encodeThreadLinkedPullRequest(linkedPullRequest),
							branchPullRequest:
								update.expected === null
									? null
									: encodeThreadLinkedPullRequest(update.expected),
						},
					]);
				}
			}),
...
it.layer(BaseTestLayer)("OrchestrationProjectionPipeline", (it) => {
	it.effect("bootstraps all projection states and writes projection rows", () =>
      ...
      `;
        ...
        `;
				assert.deepEqual(rows, [
					{ activeOrderKey: "gm", updatedAt: orderUpdatedAt },
				]);
			}
...
it.layer(Layer.fresh(makeProjectionPipelinePrefixedTestLayer("t3-base-")))(
	...
	(it) => {
		it.effect(
			"stores message attachment references without mutating payloads",
			() =>
				Effect.gen(function* () {
					const projectionPipeline = yield* OrchestrationProjectionPipeline;
					const eventStore = yield* OrchestrationEventStore;
...
it.layer(
	Layer.fresh(
		makeProjectionPipelinePrefixedTestLayer("t3-projection-attachments-safe-"),
	),
...
it.layer(
	Layer.fresh(
		makeProjectionPipelinePrefixedTestLayer(
			"t3-projection-attachments-overwrite-",
		),
	),
)("OrchestrationProjectionPipeline", (it) => {
	it.effect(
...
it.layer(
	Layer.fresh(
		makeProjectionPipelinePrefixedTestLayer(
			"t3-projection-attachments-rollback-",
		),
	),
)("OrchestrationProjectionPipeline", (it) => {
	it.effect(
					...
					eventStore
						...
						.pipe(
							Effect.flatMap((savedEvent) =>
								projectionPipeline.projectEvent(savedEvent),
							),
						);
      ...
      `;
				...
				const result = yield* Effect.result(
					projectionPipeline.projectEvent(pendingEvent),
				);
				assert.equal(result._tag, "Failure");
				assert.deepEqual(
					yield* projectionState.listAll(),
					cursorsBeforeFailure,
				);
...
it.layer(
	...
	it.effect(
		"prunes reverted attachments only after every projector commits",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
				...
				const keepAttachmentId =
					"thread-revert-files-00000000-0000-4000-8000-000000000001";
				const keepFileAttachmentId =
					"thread-revert-files-00000000-0000-4000-8000-000000000004-pdf";
				const removeAttachmentId =
					"thread-revert-files-00000000-0000-4000-8000-000000000002";
      ...
      `;
				...
				const laterAttachmentId =
					"thread-revert-files-00000000-0000-4000-8000-000000000005";
				...
				const cleanup = yield* sql.withTransaction(
					Effect.gen(function* () {
						const cleanup =
							yield* projectionPipeline.projectEventDeferred(revertedEvent);
						yield* appendAndProject({
							type: "thread.message-sent",
...
it.layer(
	Layer.fresh(
		makeProjectionPipelinePrefixedTestLayer(
			"t3-projection-attachments-revert-",
		),
	),
)("OrchestrationProjectionPipeline", (it) => {
	it.effect("removes thread attachment directory when thread is deleted", () =>
		Effect.gen(function* () {
			...
			const attachmentId =
				"thread-delete-files-00000000-0000-4000-8000-000000000001";
			const fileAttachmentId =
				"thread-delete-files-00000000-0000-4000-8000-000000000003-pdf";
			const otherThreadAttachmentId =
			...
			const threadFileAttachmentPath = path.join(
				attachmentsDir,
				`${fileAttachmentId}.pdf`,
			);
...
it.layer(
	Layer.fresh(
		makeProjectionPipelinePrefixedTestLayer(
			"t3-projection-attachments-delete-",
		),
	),
)("OrchestrationProjectionPipeline", (it) => {
	it.effect("ignores unsafe thread ids for attachment cleanup paths", () =>
		Effect.gen(function* () {
			...
			const eventStore = yield* OrchestrationEventStore;
			const now = "2026-01-01T00:00:00.000Z";
			const { attachmentsDir: attachmentsRootDir, stateDir } =
				yield* ServerConfig;
			const attachmentsSentinelPath = path.join(
				attachmentsRootDir,
				"sentinel.txt",
			);
			...
			yield* fileSystem.writeFileString(
				attachmentsSentinelPath,
				"keep-attachments-root",
			);
...
it.layer(
	Layer.fresh(
		makeProjectionPipelinePrefixedTestLayer(
			"t3-projection-attachments-replay-",
		),
	),
)("OrchestrationProjectionPipeline", (it) => {
	it.effect(
		"replaying a superseded thread.deleted keeps the re-created thread's files",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
...
it.layer(BaseTestLayer)("OrchestrationProjectionPipeline", (it) => {
	it.effect(
		"replays a bootstrap backlog larger than the event store default limit",
		() =>
			Effect.gen(function* () {
				const projectionPipeline = yield* OrchestrationProjectionPipeline;
				const eventStore = yield* OrchestrationEventStore;
      ...
      `;
				...
				const lastSequence =
					appendedEvents[appendedEvents.length - 1]!.sequence;
			...
			Effect.gen(function* () {
				...
				yield* appendAndProject({
					...
					payload: {
						...
						checkpointTurnCount: 1,
						checkpointRef: CheckpointRef.make(
							"refs/t3/checkpoints/thread-conflict/turn/1",
						),
						status: "ready",
						files: [],
	...
	it.effect(
		"clears stale pending approvals from projected shell summaries",
		() =>
			Effect.gen(function* () {
				const projectionPipeline = yield* OrchestrationProjectionPipeline;
				const eventStore = yield* OrchestrationEventStore;
					...
					payload: {
						...
						activity: {
							...
							payload: {
								requestId: "approval-request-stale-1",
								detail:
									"Unknown pending permission request: approval-request-stale-1",
							},
							turnId: null,
	...
	it.effect(
		"reads only user-input activities when refreshing shell summaries",
		() =>
			Effect.gen(function* () {
				const projectionPipeline = yield* OrchestrationProjectionPipeline;
				const eventStore = yield* OrchestrationEventStore;
	...
	it.effect(
		"maintains shell summaries without decoding message or plan bodies",
		() =>
			Effect.gen(function* () {
				const projectionPipeline = yield* OrchestrationProjectionPipeline;
				const eventStore = yield* OrchestrationEventStore;
				...
				const refreshEvents = [
					...
					},
					{
						...
						payload: {
							...
							checkpointTurnCount: 1,
							checkpointRef: CheckpointRef.make(
								"refs/t3/checkpoints/thread-shell-summary/1",
							),
							status: "ready",
							files: [],
							assistantMessageId: MessageId.make(
								"message-shell-summary-assistant",
							),
			...
			Effect.gen(function* () {
				...
				yield* appendAndProject({
					...
					payload: {
						...
						checkpointTurnCount: 1,
						checkpointRef: CheckpointRef.make(
							"refs/t3/checkpoints/thread-revert/turn/1",
						),
						status: "ready",
						files: [],
				...
				yield* appendAndProject({
					...
					payload: {
						...
						checkpointTurnCount: 2,
						checkpointRef: CheckpointRef.make(
							"refs/t3/checkpoints/thread-revert/turn/2",
						),
						status: "ready",
						files: [],
...
it.layer(
	makeProjectionPipelinePrefixedTestLayer("t3-pending-turn-terminal-test-"),
)("OrchestrationProjectionPipeline pending turn cleanup", (it) => {
	it.effect(
		"clears pending turn starts when startup reaches a terminal session state",
		() =>
			Effect.gen(function* () {
				const projectionPipeline = yield* OrchestrationProjectionPipeline;
				const eventStore = yield* OrchestrationEventStore;
				...
				for (const [index, status] of (
					["error", "interrupted", "stopped"] as const
				).entries()) {
	...
	it.effect(
		"only clears the compact request that produced the compaction activity",
		() =>
			Effect.gen(function* () {
				const projectionPipeline = yield* OrchestrationProjectionPipeline;
				const eventStore = yield* OrchestrationEventStore;
...
engineLayer("OrchestrationProjectionPipeline via engine dispatch", (it) => {
	it.effect("projects dispatched engine events immediately", () =>
      ...
      `;
			assert.deepEqual(projectRows, [
				{ title: "Live Project", scriptsJson: "[]" },
			]);
	...
	it.effect(
		"re-creating a deleted thread id starts from an empty projection",
		() =>
			Effect.gen(function* () {
				const engine = yield* OrchestrationEngineService;
        ...
        `.pipe(Effect.map((rows) => rows[0]?.count ?? 0));
				...
				for (const table of perThreadTables) {
					assert.isAbove(
						yield* countRowsForThread(table),
						0,
						`${table} should be populated`,
					);
				}
				const populatedShell = Option.getOrThrow(
				...
				assert.isTrue(
					Option.isNone(yield* snapshotQuery.getThreadShellById(threadId)),
				);
	...
	it.effect("cleans attachments only after the command receipt commits", () =>
      ...
      `;
			...
			assert.equal(
				yield* fileSystem.readFileString(attachmentPath),
				"keep this attachment",
			);
			const rolledBackThreads = yield* sql<{
      ...
      `;
			...
			yield* fileSystem.writeFileString(
				path.join(blockedAttachmentPath, "keep.txt"),
				"keep",
			);
			const cleanupFailureCommandId = CommandId.make(
				"cmd-cleanup-failure-delete",
			);
			yield* engine.dispatch({
				type: "thread.delete",
