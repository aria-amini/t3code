...
projectionSnapshotLayer("ProjectionSnapshotQuery", (it) => {
	it.effect(
      ...
      `;
        ...
        `;
				...
				assert.deepEqual(
					commandSnapshot.threads[0]?.branchPullRequest,
					branchPullRequest,
				);
				...
				if (threadShell._tag === "Some") {
					assert.deepEqual(
						threadShell.value.branchPullRequest,
						branchPullRequest,
					);
				}
      ...
      `;
				...
				const detailWithoutActivities =
					yield* snapshotQuery.getThreadDetailById(ThreadId.make("thread-1"), {
						activityKinds: [],
					});
				...
				if (detailWithoutActivities._tag === "Some") {
					...
					assert.deepEqual(
						detailWithoutActivities.value.messages,
				...
				const detailWithTaskActivities =
					yield* snapshotQuery.getThreadDetailById(ThreadId.make("thread-1"), {
      ...
      `;
				...
				if (changedContext._tag === "Some") {
					...
					assert.equal(
						changedContext.value.session?.providerName,
						"claudeAgent",
					);
					assert.equal(
					...
					assert.equal(
						changedContext.value.session?.lastError,
	...
	it.effect(
		"reads one turn-start message without decoding unrelated history",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"keeps compaction and queued-message eligibility in the turn-start query",
		() =>
			Effect.gen(function* () {
				const query = yield* ProjectionSnapshotQuery;
				const sql = yield* SqlClient.SqlClient;
      ...
      `;
				...
				for (const { text, attachments, hasOtherUserMessages } of [
					...
					{
						text: "/compact keep recent errors",
						attachments: "[]",
        ...
        `;
					...
					if (context._tag === "Some") {
						assert.equal(
							context.value.hasOtherUserMessages,
	...
	it.effect("keeps archived threads out of the main shell snapshot", () =>
      ...
      `;
			...
			assert.deepEqual(
				archivedShellSnapshot.threads[0]?.branchPullRequest,
				branchPullRequest,
			);
			...
			for (const threadId of ["thread-archived", "thread-missing"]) {
				assert.equal(
					(yield* snapshotQuery.getThreadRuntimeContext(
	...
	it.effect(
		"keeps settled threads in the shell snapshot with non-null settlement fields",
		() =>
			Effect.gen(function* () {
				const snapshotQuery = yield* ProjectionSnapshotQuery;
				const sql = yield* SqlClient.SqlClient;
      ...
      `;
				...
				assert.equal(
					shellSnapshot.threads[0]?.settledAt,
					"2026-04-06T00:00:04.000Z",
				);
	...
	it.effect(
      ...
      `;
				...
				const project =
					yield* snapshotQuery.getActiveProjectByWorkspaceRoot(
						"/tmp/workspace",
					);
				...
				const missingProject =
					yield* snapshotQuery.getActiveProjectByWorkspaceRoot("/tmp/missing");
				assert.equal(missingProject._tag, "None");

				const firstThreadId =
					yield* snapshotQuery.getFirstActiveThreadIdByProjectId(
	...
	it.effect(
		"reads single-thread checkpoint context without hydrating unrelated threads",
		() =>
			Effect.gen(function* () {
				const snapshotQuery = yield* ProjectionSnapshotQuery;
				const sql = yield* SqlClient.SqlClient;
	...
	it.effect(
		"keeps thread detail activity ordering consistent with shell snapshot ordering",
		() =>
			Effect.gen(function* () {
				const snapshotQuery = yield* ProjectionSnapshotQuery;
				const sql = yield* SqlClient.SqlClient;
	...
	it.effect(
		"uses projection_threads.latest_turn_id for targeted thread latest turn queries",
		() =>
			Effect.gen(function* () {
				const snapshotQuery = yield* ProjectionSnapshotQuery;
				const sql = yield* SqlClient.SqlClient;
      ...
      `;
				...
				if (threadDetail._tag === "Some") {
					assert.equal(
						threadDetail.value.latestTurn?.turnId,
						asTurnId("turn-running"),
					);
					assert.equal(threadDetail.value.latestTurn?.state, "running");
					assert.equal(
						threadDetail.value.latestTurn?.startedAt,
						"2026-04-02T00:00:30.000Z",
					);
				}
			}),
	...
	it.effect(
		"uses projection_threads.latest_turn_id for bulk command and shell snapshots",
		() =>
			Effect.gen(function* () {
				const snapshotQuery = yield* ProjectionSnapshotQuery;
				const sql = yield* SqlClient.SqlClient;
      ...
      `;
				...
				assert.equal(
					commandReadModel.threads[0]?.latestTurn?.turnId,
					asTurnId("turn-running"),
				);
				...
				const shellSnapshot = yield* snapshotQuery.getShellSnapshot();
				assert.equal(
					shellSnapshot.threads[0]?.latestTurn?.turnId,
					asTurnId("turn-running"),
				);
				...
				assert.equal(
					fullSnapshot.threads[0]?.latestTurn?.turnId,
					asTurnId("turn-running"),
				);
	...
	it.effect(
		"keeps deleted project and thread tombstones in the command read model",
		() =>
			Effect.gen(function* () {
				const snapshotQuery = yield* ProjectionSnapshotQuery;
				const sql = yield* SqlClient.SqlClient;
      ...
      `;
				...
				const commandReadModel = yield* snapshotQuery.getCommandReadModel();
				assert.equal(
					commandReadModel.projects[0]?.id,
					asProjectId("project-deleted"),
				);
				assert.equal(
					commandReadModel.projects[0]?.deletedAt,
					"2026-04-05T00:00:02.000Z",
				);
				assert.equal(
					commandReadModel.threads[0]?.id,
					ThreadId.make("thread-deleted"),
				);
				assert.equal(
					commandReadModel.threads[0]?.deletedAt,
					"2026-04-05T00:00:05.000Z",
				);
				assert.equal(
					commandReadModel.threads[0]?.latestTurn?.turnId,
					asTurnId("turn-deleted"),
				);
				assert.equal(
					commandReadModel.threads[0]?.latestTurn?.state,
					"completed",
				);

				const fullSnapshot = yield* snapshotQuery.getSnapshot();
				assert.equal(
					fullSnapshot.threads[0]?.id,
					ThreadId.make("thread-deleted"),
				);
				assert.equal(
					fullSnapshot.threads[0]?.latestTurn?.turnId,
	...
	it.effect(
      ...
      `;
				...
				assert.deepStrictEqual(
					(yield* snapshotQuery.searchThreads({ query: "interim needle" }))
						.matches,
					[],
				);
				assert.deepStrictEqual(
					(yield* snapshotQuery.searchThreads({ query: "system needle" }))
						.matches,
					[],
				);
				assert.deepStrictEqual(
					(yield* snapshotQuery.searchThreads({ query: "hidden needle" }))
						.matches,
					[],
				);
				yield* sql`
      ...
      `;
				assert.deepStrictEqual(
					(yield* snapshotQuery.searchThreads({ query: "user needle" }))
						.matches,
					[],
				);
			}),
...
it.effect(
	...
	() => {
      ...
      `;
			...
			assert.equal(
				shellSnapshot.projects[0]?.repositoryIdentity?.rootPath,
				"/tmp/shared-root",
			);
			assert.equal(
				shellSnapshot.projects[1]?.repositoryIdentity?.rootPath,
				"/tmp/shared-root",
			);
			...
			const fullSnapshot = yield* snapshotQuery.getSnapshot();
			assert.deepStrictEqual(resolveCalls.toSorted(), [
				"/tmp/deleted-root",
				"/tmp/shared-root",
			]);
			assert.equal(fullSnapshot.projects.length, 3);
			assert.equal(
				fullSnapshot.projects[2]?.repositoryIdentity?.rootPath,
				"/tmp/deleted-root",
			);
...
projectionSnapshotLayer(
	"ProjectionSnapshotQuery windowed thread detail",
	(it) => {
    ...
    `;
      ...
      `;
		...
		const messageIds = (snapshot: {
			thread: { messages: ReadonlyArray<{ id: string }> };
		}) => snapshot.thread.messages.map((message) => message.id).toSorted();
		const activityIds = (snapshot: {
			thread: { activities: ReadonlyArray<{ id: string }> };
		}) => snapshot.thread.activities.map((activity) => activity.id).toSorted();

		it.effect(
			"returns the full thread with no page metadata when no window is requested",
			() =>
				Effect.gen(function* () {
		...
		it.effect(
			"windows to the last N user-anchored turns with subagent turns riding along",
			() =>
				Effect.gen(function* () {
					yield* seedFanOutThread();
					const snapshotQuery = yield* ProjectionSnapshotQuery;
		...
		it.effect(
			"subagent turns between user turns ride along inside the window",
			() =>
				Effect.gen(function* () {
					yield* seedFanOutThread();
					const snapshotQuery = yield* ProjectionSnapshotQuery;
		...
		it.effect(
			"cursors survive a projection rewrite that reassigns turn row ids",
			() =>
				Effect.gen(function* () {
					...
					const firstPage = yield* snapshotQuery.getThreadDetailSnapshot(
						threadW,
						{ turnLimit: 2 },
					);
      ...
      `;
        ...
        `;
					...
					const olderPage = yield* snapshotQuery.getThreadDetailSnapshot(
						threadW,
						{
							turnLimit: 1,
							beforeCursor: cursor,
		...
		it.effect(
			"keeps imported history on the oldest page after resumed turns",
			() =>
				Effect.gen(function* () {
					yield* seedFanOutThread({ importedMessageCount: 12 });
					const snapshotQuery = yield* ProjectionSnapshotQuery;

					const completePage = yield* snapshotQuery.getThreadDetailSnapshot(
						threadW,
						{ turnLimit: 50 },
					);
					...
					assert.equal(
						completePage.value.thread.messages.filter((message) =>
					...
					const recentPage = yield* snapshotQuery.getThreadDetailSnapshot(
						threadW,
						{ turnLimit: 2 },
					);
					...
					assert.equal(
						recentPage.value.thread.messages.some((message) =>
							message.id.startsWith("import:"),
						),
						false,
					);
					...
					const oldestPage = yield* snapshotQuery.getThreadDetailSnapshot(
						threadW,
						{
							turnLimit: 1,
							beforeCursor: cursor,
		...
		it.effect(
			"a cursor for a different thread degrades to the first page",
			() =>
				Effect.gen(function* () {
					yield* seedFanOutThread();
					const snapshotQuery = yield* ProjectionSnapshotQuery;
					...
					if (snapshot._tag === "Some") {
						assert.deepEqual(
							messageIds(snapshot.value),
							messageIds(firstPage.value),
						);
					}
				}),
		);
		...
		it.effect(
			"a malformed cursor degrades to the first page instead of failing",
			() =>
				Effect.gen(function* () {
					yield* seedFanOutThread();
					const snapshotQuery = yield* ProjectionSnapshotQuery;
		...
		it.effect(
			"windows never split below the raw-turn ceiling boundary contiguously",
			() =>
				Effect.gen(function* () {
					yield* seedFanOutThread();
					const snapshotQuery = yield* ProjectionSnapshotQuery;
					...
					for (let page = 0; page < 10; page += 1) {
						...
						seenActivities.push(
							...snapshot.value.thread.activities.map(
		...
		it.effect(
			"bounds activity hydration and preserves unresolved requests",
			() =>
				Effect.gen(function* () {
					yield* seedFanOutThread();
					const snapshotQuery = yield* ProjectionSnapshotQuery;
      ...
      `;
					...
					if (fullDetail._tag === "Some") {
						...
						assert.equal(
							fullDetail.value.activities[0]?.id,
							asEventId("activity-0002"),
						);
						assert.equal(
							fullDetail.value.activities.at(-1)?.id,
							asEventId("activity-0501"),
						);
					}

					const windowedDetail = yield* snapshotQuery.getThreadDetailSnapshot(
						threadW,
						{
							turnLimit: 2,
						},
      ...
      `;
					...
					const detailWithPinnedRequests =
					...
					if (detailWithPinnedRequests._tag === "Some") {
						const ids = new Set(
							detailWithPinnedRequests.value.activities.map(
								(activity) => activity.id,
							),
						);
					...
					const windowWithPinnedRequests =
						yield* snapshotQuery.getThreadDetailSnapshot(threadW, {
							turnLimit: 2,
						});
					...
					if (windowWithPinnedRequests._tag === "Some") {
						const ids = new Set(
							windowWithPinnedRequests.value.thread.activities.map(
								(activity) => activity.id,
							),
						);
						assert.equal(
							windowWithPinnedRequests.value.thread.activities.length,
							503,
						);
						assert.equal(ids.has(asEventId("approval-old")), true);
						assert.equal(ids.has(asEventId("user-input-old")), true);
					...
					if (
						...
						const projectedFullSnapshot = projectThreadDetailSnapshot(
							fullSnapshot.value,
						);
						...
						const rawActivitiesById = new Map(
							detailWithPinnedRequests.value.activities.map((activity) => [
								activity.id,
								activity,
							]),
						);
						...
						const projectedWindowBaseline = projectThreadDetailSnapshot({
							...
							thread: {
								...
								activities:
									windowWithPinnedRequests.value.thread.activities.map(
										(activity) =>
											rawActivitiesById.get(activity.id) ?? activity,
									),
							},
						});
						assert.deepStrictEqual(
						...
						const projectedIds = new Set(
							projectedFullSnapshot.thread.activities.map(
								(activity) => activity.id,
							),
						);
		...
		it.effect(
			"a thread with no turns returns its content unwindowed on the first page",
			() =>
				Effect.gen(function* () {
					const sql = yield* SqlClient.SqlClient;
					const snapshotQuery = yield* ProjectionSnapshotQuery;
      ...
      `;
					for (const projector of Object.values(
...
projectionSnapshotLayer("ProjectionSnapshotQuery imported sources", (it) => {
	...
	it.effect(
		"reads completed source copies without decoding message bodies",
		() =>
			Effect.gen(function* () {
				const sql = yield* SqlClient.SqlClient;
				const query = yield* ProjectionSnapshotQuery;
      ...
      `;
				...
				assert.deepEqual(sources, [
					imported,
					{ threadId: imported.threadId, source: copiedSource },
				]);
				assert.equal(counter.count(), 1);
			}),
	);
	...
	it.effect(
		"requires active project threads, a binding, and an imported message",
		() =>
			Effect.gen(function* () {
				const sql = yield* SqlClient.SqlClient;
				const query = yield* ProjectionSnapshotQuery;
      ...
      `;
				const partials = yield* Effect.forEach(
					...
					(providerSessionId) =>
						seedImportedSession(projectId, { ...source, providerSessionId }),
				);
				const [
					noBinding,
					noHistory,
      ...
      `;
				...
				assert.deepEqual(
					yield* query.getImportedAgentSessionSources(projectId),
					[completed],
				);
				assert.deepEqual(
					yield* query.getImportedAgentSessionSources(otherProjectId),
					[otherProject],
				);
				assert.deepEqual(
					yield* query.getImportedAgentSessionSources(deletedProjectId),
					[],
				);
				assert.deepEqual(
					yield* query.getImportedAgentSessionSources(
						ProjectId.make("project-import-missing"),
					),
					[],
				);
			}),
	...
	it.effect(
		"keeps original sources when the current runtime provider and cursor change",
		() =>
			Effect.gen(function* () {
				const sql = yield* SqlClient.SqlClient;
				const query = yield* ProjectionSnapshotQuery;
	...
	it.effect(
		"skips invalid source payloads and entries without dropping valid sources",
		() =>
			Effect.gen(function* () {
				const sql = yield* SqlClient.SqlClient;
				const query = yield* ProjectionSnapshotQuery;
