...
const decodeAgentSessionImportSource = Schema.decodeUnknownOption(
	AgentSessionImportSource,
);
...
const ProjectionTurnStartMessageDbRowSchema =
	ProjectionThreadMessageDbRowSchema.mapFields(
...
function mapTitleRegeneration(
	...
	return row.titleRegenerationRequestId != null &&
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
	...
	const resolveRepositoryIdentitiesForProjects = Effect.fn(
		...
		const uniqueWorkspaceRoots = [
			...new Set(filteredProjectRows.map((row) => row.workspaceRoot)),
		];
	...
	const getUserInputActivity: ProjectionSnapshotQueryShape["getUserInputActivity"] =
		(input) =>
...
pending_approval_requests AS (
  ...
  `;
			...
			.pipe(
				Effect.flatMap(
					([
						...
						Effect.gen(function* () {
							...
							>();
							const checkpointsByThread = new Map<
								string,
								Array<OrchestrationCheckpointSummary>
							>();
							...
							const threads: ReadonlyArray<OrchestrationThread> =
								threadRows.map((row) => ({
									id: row.threadId,
				...
				Effect.mapError((error) => {
					...
					return toPersistenceSqlError(
						"ProjectionSnapshotQuery.getSnapshot:query",
					)(error);
				}),
			);
			...
			sql
				...
				.pipe(
					Effect.flatMap(
						([
							...
							Effect.gen(function* () {
								...
								const latestTurnByThread = new Map(
									latestTurnRows.map(
										(row) => [row.threadId, mapLatestTurn(row)] as const,
									),
								);
								const sessionByThread = new Map(
									sessionRows.map(
										(row) => [row.threadId, mapSessionRow(row)] as const,
									),
								);
									...
									threads: Arr.filterMap(threadRows, (row) =>
										row.deletedAt === null
											? Result.succeed({
													...
													planProgress:
														threadPlanProgress.getThreadPlanProgress(
															row.threadId,
														),
					...
					Effect.mapError((error) => {
						...
						return toPersistenceSqlError(
							"ProjectionSnapshotQuery.getShellSnapshot:query",
						)(error);
					}),
				);
			...
			sql
				...
				.pipe(
						...
						([
							...
							Effect.gen(function* () {
								...
								const activeProjectIds = new Set(
									threadRows.map((row) => row.projectId),
								);
								const repositoryIdentities =
									yield* resolveRepositoryIdentitiesForProjects(
								...
								const snapshot = {
									...
									threads: threadRows.map((row): OrchestrationThreadShell => ({
										...
										planProgress: threadPlanProgress.getThreadPlanProgress(
											row.threadId,
										),
									})),
									updatedAt: updatedAt ?? "1970-01-01T00:00:00.000Z",
					...
					Effect.mapError((error) => {
						...
						return toPersistenceSqlError(
							"ProjectionSnapshotQuery.getArchivedShellSnapshot:query",
						)(error);
					}),
				);
	...
	const getEventReplayStats: ProjectionSnapshotQueryShape["getEventReplayStats"] =
		(input) =>
	...
	const searchThreads: ProjectionSnapshotQueryShape["searchThreads"] =
		Effect.fn("ProjectionSnapshotQuery.searchThreads")(function* (input) {
	...
	const getProjectShellById: ProjectionSnapshotQueryShape["getProjectShellById"] =
		(projectId) =>
					...
					Option.isNone(option)
						...
						: repositoryIdentityResolver
								...
								.pipe(
									Effect.map((repositoryIdentity) =>
										Option.some(
											mapProjectShellRow(option.value, repositoryIdentity),
										),
									),
								),
				),
	...
	const getImportedAgentSessionSources: ProjectionSnapshotQueryShape["getImportedAgentSessionSources"] =
		Effect.fn("ProjectionSnapshotQuery.getImportedAgentSessionSources")(
			function* (projectId) {
				const rows = yield* listImportedAgentSessionSourceRows({
					projectId,
				}).pipe(
					Effect.mapError(
						toPersistenceSqlOrDecodeError(
			...
			Effect.gen(function* () {
				...
				return Option.some({
					...
					session: Option.isSome(sessionRow)
						? mapSessionRow(sessionRow.value)
						: null,
					latestUserMessageAt: threadRow.value.latestUserMessageAt,
					hasPendingApprovals: threadRow.value.pendingApprovalCount > 0,
					hasPendingUserInput: threadRow.value.pendingUserInputCount > 0,
					hasActionableProposedPlan:
						threadRow.value.hasActionableProposedPlan > 0,
					backgroundLiveness:
						threadBackgroundLiveness.getThreadBackgroundLiveness(
	...
	const getThreadRuntimeContext: ProjectionSnapshotQueryShape["getThreadRuntimeContext"] =
		Effect.fn("ProjectionSnapshotQuery.getThreadRuntimeContext")(
			function* (threadId) {
	...
	const getTurnStartMessage: ProjectionSnapshotQueryShape["getTurnStartMessage"] =
		Effect.fn("ProjectionSnapshotQuery.getTurnStartMessage")(function* (input) {
	...
	const listProjectedThreadActivities = Effect.fn(
		...
		const activityIds = [
			...new Set(
				[...activityIdRows, ...pinnedActivityIdRows].map(
		...
		for (
			...
			const batchRows = yield* listThreadActivityRowsByIds({
				activityIds: batchIds,
			}).pipe(
				Effect.mapError(
					toPersistenceSqlOrDecodeError(
		...
		Effect.gen(function* () {
			...
			return Option.some(
				yield* decodeThread(thread).pipe(
					Effect.mapError(
						toPersistenceDecodeError(
							"ProjectionSnapshotQuery.getThreadDetailById:decodeThread",
						),
					),
				),
			);
			...
			sql
				.withTransaction(
					Effect.gen(function* () {
						...
						const cursor =
							decodedCursor?.threadId === threadId ? decodedCursor : null;
