...
async function waitFor(
	...
	const deadline =
		(await Effect.runPromise(Clock.currentTimeMillis)) + timeoutMs;
...
describe("ProviderCommandReactor", () => {
	...
	async function createHarness(input?: {
		...
		readonly compactThreadEffect?: () => Effect.Effect<
			...
			ProviderAdapterRequestError
		>;
		readonly interruptTurnEffect?: () => Effect.Effect<
			void,
			ProviderAdapterRequestError
		>;
		readonly stopSessionEffect?: () => Effect.Effect<
			void,
			ProviderAdapterRequestError
		>;
		...
		const baseDir =
			input?.baseDir ??
			NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "t3code-reactor-"));
		...
		const tryHandlePromptCommand = vi.fn<
			ProviderAuthService["Service"]["tryHandlePromptCommand"]
		...
		const generateBranchName = vi.fn<
			TextGeneration["Service"]["generateBranchName"]
		>((_) =>
			Effect.fail(
		...
		const generateThreadTitle = vi.fn<
			TextGeneration["Service"]["generateThreadTitle"]
		>((_) =>
			Effect.fail(
				new TextGenerationError({
					...
					dispatch: (command) => {
						if (command.type === "thread.title.regeneration.complete") {
							...
							if (
								...
								return Effect.die(
									new Error("Injected title regeneration completion failure"),
								);
							}
						}
						return (
		...
		const layer = ProviderCommandReactorLive.pipe(
			...
			Layer.provideMerge(
				Layer.succeed(VcsStatusBroadcaster, {
					getStatus: () =>
						Effect.die("getStatus should not be called in this test"),
					refreshLocalStatus: () =>
						Effect.die("refreshLocalStatus should not be called in this test"),
					refreshStatus,
					refreshPullRequestStatus: () =>
						Effect.die(
							"refreshPullRequestStatus should not be called in this test",
						),
					streamStatus: () =>
	...
	effectIt.effect.each(["new", "ready", "stopped"] as const)(
		...
		(sessionStatus) =>
			Effect.gen(function* () {
				...
				yield* harness.engine.dispatch({
					...
					modelSelection: {
						instanceId:
							sessionStatus === "new"
								? instanceId
								: ProviderInstanceId.make("antigravity-other"),
				...
				expect(
					yield* Effect.promise(() => harness.readPendingTurnStarts()),
				).toEqual([]);
	...
	effectIt.effect(
		"clears a failed sign-out request without sending it as a prompt",
		() =>
			Effect.gen(function* () {
				const instanceId = ProviderInstanceId.make("antigravity-personal");
				const handled = yield* Deferred.make<void>();
				...
				expect(
					thread?.activities.some(
						(activity) => activity.kind === "provider.auth.signed-out",
					),
				).toBe(false);
				expect(
					yield* Effect.promise(() => harness.readPendingTurnStarts()),
	...
	effectIt.effect.each([
		{
			label: "a command mention",
			text: "What does /logout do?",
			attachments: [],
		},
		{
		...
		"sends $label when the provider auth handler leaves it unhandled",
		({ text, attachments }) =>
			Effect.gen(function* () {
	...
	effectIt.effect(
		"retains a turn dispatched immediately after start until activation",
		() =>
			Effect.gen(function* () {
	...
	effectIt.effect(
		"starts a turn and generates its title without loading old message bodies",
		() =>
			Effect.gen(function* () {
				const started = yield* Deferred.make<void>();
	...
	effectIt.effect(
		"keeps turns blocked until compaction restores the session",
		() =>
			Effect.gen(function* () {
				...
				yield* Effect.promise(() =>
					waitFor(() => harness.sendTurn.mock.calls.length === 1),
				);
				yield* harness.engine.dispatch({
					type: "thread.session.set",
	...
	effectIt.effect(
		"does not overwrite concurrent session state after compaction failure",
		() =>
			Effect.gen(function* () {
				...
				yield* Effect.promise(() =>
					waitFor(() => harness.compactThread.mock.calls.length === 1),
				);
				const compactingThread = (yield* Effect.promise(() =>
					harness.readModel(),
				)).threads.find((entry) => entry.id === threadId);
				...
				yield* Effect.promise(() =>
					waitFor(() => harness.stopSession.mock.calls.length === 1),
				);
				...
				const stoppingThread = (yield* Effect.promise(() =>
					harness.readModel(),
				)).threads.find((entry) => entry.id === threadId);
				...
				yield* Effect.promise(() =>
					waitFor(() => harness.compactThread.mock.calls.length === 2),
				);
				yield* harness.engine.dispatch({
					type: "thread.session.stop",
					commandId: CommandId.make(
						"cmd-failed-stop-before-compaction-settles",
					),
					threadId,
				...
				yield* Effect.promise(() => harness.drain());
				const runningThread = (yield* Effect.promise(() =>
					harness.readModel(),
				)).threads.find((entry) => entry.id === threadId);
	...
	effectIt.effect(
		"projects starting before a slow provider session finishes",
		() =>
			Effect.gen(function* () {
				...
				yield* Effect.promise(() =>
					waitFor(() => harness.startSession.mock.calls.length === 1),
				);
				const duringStartup = yield* Effect.promise(() => harness.readModel());
				expect(
					duringStartup.threads.find(
						(entry) => entry.id === ThreadId.make("thread-1"),
					)?.session?.status,
				).toBe("starting");
				expect(harness.sendTurn).not.toHaveBeenCalled();
	...
	effectIt.effect(
		"shows the missing workspace message without a provider stack trace",
		() =>
			Effect.gen(function* () {
				...
				const thread = (yield* Effect.promise(() =>
					harness.readModel(),
				)).threads.find((entry) => entry.id === ThreadId.make("thread-1"));
				expect(thread?.session).toMatchObject({
					status: "error",
					activeTurnId: null,
				...
				expect(failure?.payload).toMatchObject({
					detail: missingWorkspace.message,
				});
	...
	effectIt.effect(
		"settles a failed provider startup and allows a clean retry",
		() =>
			Effect.gen(function* () {
				...
				let thread = readModel.threads.find(
					(entry) => entry.id === ThreadId.make("thread-1"),
				);
	...
	it("regenerates a thread title from the current conversation", async () => {
		...
		await harness.runEffect(
			harness.engine.dispatch({
				...
				commandId: CommandId.make(
					"cmd-assistant-complete-before-title-regeneration",
				),
				threadId: ThreadId.make("thread-1"),
				messageId: asMessageId("assistant-message-before-title-regeneration"),
	...
	it("pins the first user message when regeneration context is truncated", async () => {
		...
		expect(
			message.startsWith(
				`USER:\nReview subagent monitoring risks. ${quoteText} `,
			),
		).toBe(true);
		...
		expect(
			readModel.threads
				...
				?.messages.find(
					(entry) =>
						entry.id ===
						asMessageId("user-message-before-long-title-regeneration"),
				)?.text,
	...
	it("keeps the current title when regeneration returns the fallback", async () => {
		...
		const now = "2026-01-01T00:00:00.000Z";
		harness.generateThreadTitle.mockReturnValue(
			Effect.succeed({ title: "New thread" }),
		);
		...
		await harness.runEffect(
			harness.engine.dispatch({
				type: "thread.meta.update",
				commandId: CommandId.make(
					"cmd-thread-title-before-fallback-regeneration",
				),
				threadId: ThreadId.make("thread-1"),
	...
	it("clears title regeneration state when generation fails", async () => {
		...
		await harness.runEffect(
			harness.engine.dispatch({
				type: "thread.meta.update",
				commandId: CommandId.make(
					"cmd-thread-title-before-failed-regeneration",
				),
				threadId: ThreadId.make("thread-1"),
	...
	it("retries a failed completion and continues regenerating", async () => {
		...
		harness.generateThreadTitle
			.mockReturnValueOnce(
				Effect.succeed({ title: "Title lost to completion failure" }),
			)
			.mockReturnValueOnce(
				Effect.succeed({ title: "Recovered regeneration worker" }),
			);
		...
		await harness.runEffect(
			harness.engine.dispatch({
				type: "thread.meta.update",
				commandId: CommandId.make(
					"cmd-thread-title-regeneration-completion-failure",
				),
				threadId: ThreadId.make("thread-1"),
				regenerateTitle: true,
			}),
		...
		await harness.runEffect(
			harness.engine.dispatch({
				type: "thread.meta.update",
				commandId: CommandId.make(
					"cmd-thread-title-regeneration-after-completion-failure",
				),
				threadId: ThreadId.make("thread-1"),
				regenerateTitle: true,
			}),
	...
	it("pins the first user context and attachment before the retained tail", async () => {
		...
		await harness.runEffect(
			harness.engine.dispatch({
				type: "thread.meta.update",
				commandId: CommandId.make(
					"cmd-thread-title-before-truncated-regeneration",
				),
				threadId: ThreadId.make("thread-1"),
				title: "Existing title",
			}),
		...
		await harness.runEffect(
			harness.engine.dispatch({
				type: "thread.message.assistant.delta",
				commandId: CommandId.make(
					"cmd-assistant-truncated-regeneration-context",
				),
				threadId: ThreadId.make("thread-1"),
		...
		await harness.runEffect(
			harness.engine.dispatch({
				type: "thread.message.assistant.complete",
				commandId: CommandId.make(
					"cmd-assistant-truncated-regeneration-context-complete",
				),
				threadId: ThreadId.make("thread-1"),
				messageId: asMessageId("assistant-truncated-regeneration-context"),
				createdAt: "2026-01-01T00:00:02.000Z",
		...
		await harness.runEffect(
			harness.engine.dispatch({
				type: "thread.meta.update",
				commandId: CommandId.make(
					"cmd-thread-title-regenerate-truncated-context",
				),
				threadId: ThreadId.make("thread-1"),
				regenerateTitle: true,
			}),
	...
	it("does not overwrite a manual rename while title regeneration is running", async () => {
		...
		await harness.runEffect(
			harness.engine.dispatch({
				type: "thread.meta.update",
				commandId: CommandId.make(
					"cmd-thread-manual-rename-during-regeneration",
				),
				threadId: ThreadId.make("thread-1"),
		...
		await harness.runEffect(
			Deferred.succeed(generatedTitle, {
				title: "Generated title should not win",
			}),
		);
		await harness.drain();
	...
	it("does not overwrite a manual rename while title regeneration is queued", async () => {
		...
		const harness = await createHarness({
			startSessionEffect: (session) =>
				Effect.promise(() => startGate).pipe(Effect.as(session)),
		});
		const now = "2026-01-01T00:00:00.000Z";
		harness.generateThreadTitle.mockReturnValue(
		...
		await harness.runEffect(
			harness.engine.dispatch({
				type: "thread.meta.update",
				commandId: CommandId.make(
					"cmd-thread-title-before-queued-regeneration",
				),
				threadId: ThreadId.make("thread-1"),
				title: "Existing thread title",
			}),
		...
		await harness.runEffect(
			harness.engine.dispatch({
				type: "thread.meta.update",
				commandId: CommandId.make(
					"cmd-thread-manual-rename-before-regeneration-starts",
				),
				threadId: ThreadId.make("thread-1"),
	...
	it("matches the client-seeded title even when the outgoing prompt is reformatted", async () => {
		...
		expect(
			harness.generateThreadTitle.mock.calls[0]?.[0].message,
		).not.toContain("t3-citation://");
		const readModel = await harness.readModel();
		const thread = readModel.threads.find(
			(entry) => entry.id === ThreadId.make("thread-1"),
		);
		...
		expect(
			thread?.messages.find(
				(entry) => entry.id === asMessageId("user-message-title-formatted"),
			)?.text,
		).toBe(prompt);
		expect(harness.sendTurn.mock.calls[0]?.[0]).toMatchObject({
			input: prompt,
	...
	it("generates a worktree branch name for the first turn", async () => {
		...
		harness.refreshStatus.mockImplementation((cwd) =>
			refreshStatus(cwd).pipe(
				Effect.tap(() => Deferred.succeed(statusRefreshed, undefined)),
			),
		);
		...
		expect(harness.refreshStatus.mock.calls[0]?.[0]).toBe(
		...
		expect(
			readModel.threads
				.find((entry) => entry.id === ThreadId.make("thread-1"))
				?.messages.find(
					(entry) => entry.id === asMessageId("user-message-branch-model"),
				)?.text,
		).toBe(prompt);
	});
	...
	it("recreates a missing worktree from the thread branch before starting a turn", async () => {
		...
		await waitFor(() => harness.startSession.mock.calls.length === 1);
		expect(harness.pruneWorktrees).toHaveBeenCalledWith({
			cwd: "/tmp/provider-project",
		});
	...
	it("starts a first turn on the requested provider instance even when it differs from the thread model", async () => {
		...
		expect(thread?.session?.providerInstanceId).toBe(
			ProviderInstanceId.make("claudeAgent"),
		);
		expect(
			thread?.activities.find(
	...
	effectIt.effect(
		"stops a starting session without a bound turn when interrupt fails",
		() =>
			Effect.gen(function* () {
				const harness = yield* Effect.promise(() =>
					createHarness({
				...
				yield* harness.engine.dispatch({
					type: "thread.turn.interrupt",
					commandId: CommandId.make(
						"cmd-turn-interrupt-starting-provider-failure",
					),
					threadId: ThreadId.make("thread-1"),
					createdAt: now,
				});
	...
	effectIt.effect(
		"does not overwrite a session that became ready while an interrupt failed",
		() =>
			Effect.gen(function* () {
				const harness = yield* Effect.promise(() => createHarness());
				const now = "2026-01-01T00:00:00.000Z";
	...
	effectIt.effect(
		"stops a provider session without reading unrelated message bodies",
		() =>
			Effect.gen(function* () {
				const harness = yield* Effect.promise(() =>
					createHarness({ unreadableHistory: true }),
				);
				const now = "2026-01-01T00:00:00.000Z";
	...
	effectIt.effect(
		"stops a ready provider session after automatic settlement",
		() =>
			Effect.gen(function* () {
				...
				const harness = yield* Effect.promise(() =>
					createHarness({
						stopSessionEffect: () =>
							Deferred.succeed(sessionStopped, undefined).pipe(Effect.asVoid),
					}),
				);
				const now = "2026-01-01T00:00:00.000Z";
				...
				const beforeSettlement = yield* Effect.promise(() =>
					harness.readModel(),
				);
