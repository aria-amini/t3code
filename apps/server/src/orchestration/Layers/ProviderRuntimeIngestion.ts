...
const providerTurnKey = (threadId: ThreadId, turnId: TurnId) =>
	`${threadId}:${turnId}`;
const providerTaskKey = (threadId: ThreadId, taskId: string) =>
	`${threadId}:${taskId}`;
...
function compactedTokenCountsFromActivities(
	activities: ReadonlyArray<
		Pick<
			...
			"kind" | "payload" | "sequence" | "createdAt"
		>
	>,
	...
	const activitiesSinceLastCompaction = activities.slice(
				...
				projectActivityPayload({
					...
					payload: {
						...
						...(event.payload.toolSurface
							...
							: {}),
						...(event.payload.toolIcon
							? { toolIcon: event.payload.toolIcon }
							: {}),
...
const make = Effect.gen(function* () {
	...
	const projectionThreadProposedPlans =
	...
	const providerCommandId = (event: ProviderRuntimeEvent, tag: string) =>
		crypto.randomUUIDv4.pipe(
			Effect.map((uuid) =>
				CommandId.make(`provider:${event.eventId}:${tag}:${uuid}`),
			),
		);
	...
	const assistantSegmentStateByTurnKey = yield* Cache.make<
		string,
	...
	>({
		...
		lookup: () =>
			Effect.die(
				new Error(
					"assistant segment state should be read through getOption before initialization",
				),
			),
	});
	...
	const rememberTaskDescription = (
		...
		Cache.set(
			...
			providerTaskKey(threadId, taskId),
			description,
		);
	...
	const lookupTaskDescription = (threadId: ThreadId, taskId: string) =>
		Cache.getOption(
			...
			Effect.map((description) =>
				Option.filter(description, (value) => value.length > 0).pipe(
					Option.getOrUndefined,
				),
			),
		);

	const resolveThreadRuntimeContext = Effect.fn("resolveThreadRuntimeContext")(
			...
			Effect.flatMap((existingState) =>
				Effect.gen(function* () {
					const nextState = Option.match(existingState, {
						...
						onSome: (state) => {
							const segmentIndex =
								state.baseKey === input.baseKey ? state.nextSegmentIndex : 0;
			...
			yield* Effect.forEach(
				...
				(messageId) =>
					flushBufferedAssistantMessage({
						...
						Effect.tap((flushed) =>
							flushed
								? Effect.sync(() => flushedMessageIds.add(messageId))
								: Effect.void,
						),
					),
				{ concurrency: 1 },
		...
		yield* orchestrationEngine.dispatch({
			...
			proposedPlan: {
				...
				createdAt:
					...
					(bufferedPlan?.createdAt || input.updatedAt),
	...
	const clearTurnStateForSession = (threadId: ThreadId) =>
		Effect.gen(function* () {
			...
			const assistantSegmentKeys = Array.from(
				yield* Cache.keys(assistantSegmentStateByTurnKey),
			);
			const proposedPlanKeys = Array.from(
				yield* Cache.keys(bufferedProposedPlanById),
			);
			const taskDescriptionKeys = Array.from(
				yield* Cache.keys(taskDescriptionByTaskKey),
			);
			yield* Effect.forEach(
				turnKeys,
				(key) =>
			...
			yield* Effect.forEach(
				...
				(key) =>
					key.startsWith(prefix)
						? Cache.invalidate(taskDescriptionByTaskKey, key)
						: Effect.void,
				{ concurrency: 1 },
			).pipe(Effect.asVoid);
		});
	...
	const getExpectedProviderTurnIdForThread = Effect.fn(
	...
	const markSourceProposedPlanImplemented = Effect.fn(
	...
	const processRuntimeEvent = (event: ProviderRuntimeEvent) =>
		Effect.gen(function* () {
			...
			if (assistantCompletion) {
				...
				const [existingAssistantMessage, hasAssistantMessagesForTurn] =
					yield* Effect.all([
				...
				if (turnId) {
					...
					for (const requestId of pendingRequestIds) {
						yield* orchestrationEngine.dispatch({
							...
							commandId: yield* providerCommandId(
								event,
								"terminal-user-input-resolved",
							),
							threadId: thread.id,
							activity: {
								id: EventId.make(
									`${event.eventId}:user-input-resolved:${requestId}`,
								),
								createdAt: now,
								tone: "info",
								kind: "user-input.resolved",
			...
			if (event.type === "task.completed") {
				taskTitle = yield* lookupTaskDescription(
					thread.id,
					event.payload.taskId,
				);
				if (!taskTitle) {
					const taskActivity =
						yield* projectionThreadActivityRepository.getLatestTaskActivity({
							threadId: thread.id,
							taskId: event.payload.taskId,
						});
				...
				if (tokenCounts) {
					activityEvent = {
						...
						payload: {
							...
							beforeTokens:
								activityEvent.payload.beforeTokens ?? tokenCounts.beforeTokens,
							afterTokens:
								activityEvent.payload.afterTokens ?? tokenCounts.afterTokens,
						},
					};
				}
			}
