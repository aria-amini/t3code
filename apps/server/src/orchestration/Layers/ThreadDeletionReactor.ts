...
type ThreadDeletedEvent = Extract<
	OrchestrationEvent,
	{ type: "thread.deleted" }
>;
...
const make = Effect.gen(function* () {
		...
		processThreadDeleted(event).pipe(
			Effect.catchCause((cause) => {
				...
				return Effect.logWarning(
					"thread deletion reactor failed to process event",
					{
	...
	const start: ThreadDeletionReactorShape["start"] = Effect.fn("start")(
		function* () {
			yield* forkParked(
				Stream.runForEach(
					orchestrationEngine.streamDomainEvents.pipe(
						...
						Stream.onStart(
							orchestrationEngine.latestSequence.pipe(Effect.flatMap(noteSeen)),
						),
					),
					(event) =>
...
export const ThreadDeletionReactorLive = Layer.effect(
	ThreadDeletionReactor,
	make,
);
