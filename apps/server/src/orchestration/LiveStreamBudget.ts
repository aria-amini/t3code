...
export const makeLiveStreamBudget = Effect.fn("makeLiveStreamBudget")(
	function* (limits?: {
		...
		const maxSerializedBytes =
			limits?.maxSerializedBytes ?? LIVE_STREAM_MAX_SERIALIZED_BYTES;
		...
		const replace = <A extends object>(
			...
			Effect.suspend(() => {
				...
				let nextSerializedBytes =
					retainedSerializedBytes +
					next.reduce((sum, item) => sum + item.serializedBytes, 0);
		...
		const deliver = <A, E, R>(
			stream: Stream.Stream<RetainedLiveItem<A>, E, R>,
		) =>
			Stream.fromPull(
				Effect.gen(function* () {
					yield* check;
						...
						Effect.catchTags({
							OrchestrationGetSnapshotError: (error) =>
								Effect.sync(() => {
									...
									Effect.andThen(
										Effect.sync(() => {
											const delivered = new Set<RetainedLiveItem<unknown>>(
												inFlight,
											);
					...
					return Effect.gen(function* () {
						...
						inFlight = [];
						yield* check;
						const items = yield* Effect.raceFirst(
							source.pull,
