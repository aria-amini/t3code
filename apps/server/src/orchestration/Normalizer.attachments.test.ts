...
describe("normalizeDispatchCommand attachments", () => {
	it.effect(
		"preserves inline image attachments from existing mobile clients",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"claims uploaded attachments while retaining a retryable pending copy",
		() =>
			Effect.gen(function* () {
				const config = yield* ServerConfig.ServerConfig;
				...
				const normalized = yield* normalizeDispatchCommand(
					turnStartCommand({
						attachments: [
							{ id: `pending-${attachmentUuid}`, sizeBytes: bytes.byteLength },
						],
					}),
				);
				if (normalized.type !== "thread.turn.start") {
				...
				const claimedPngPath = NodePath.join(
					config.attachmentsDir,
					`${attachmentId}.png`,
				);
				...
				expect(NodeFS.statSync(claimedPngPath).ino).not.toBe(
					NodeFS.statSync(pendingPath).ino,
				);
	...
	it.effect("normalizes inline and uploaded attachments in the same turn", () =>
		Effect.gen(function* () {
			...
			expect(
				normalized.message.attachments[1]?.id.startsWith("thread-1-"),
			).toBe(true);
		}).pipe(Effect.provide(testLayer)),
	);

	it.effect(
		"claims uploaded documents without changing their original extension",
		() =>
			Effect.gen(function* () {
				const config = yield* ServerConfig.ServerConfig;
				...
				expect(NodeFS.statSync(claimedPath).ino).not.toBe(
					NodeFS.statSync(pendingPath).ino,
				);
			}).pipe(Effect.provide(testLayer)),
	);
	...
	it.effect("retries a failed bootstrap with a fresh thread id", () =>
		Effect.gen(function* () {
			...
			NodeFS.rmSync(
				NodePath.join(
					config.attachmentsDir,
					`${first.message.attachments[0]!.id}.png`,
				),
			);
			...
			expect(
				retried.message.attachments[0]?.id.startsWith("thread-retry-"),
			).toBe(true);
		}).pipe(Effect.provide(testLayer)),
	);

	it.effect(
		"removes failed attachment claims without deleting their pending uploads",
		() =>
			Effect.gen(function* () {
				const config = yield* ServerConfig.ServerConfig;
				const pendingPath = NodePath.join(
	...
	it.effect(
		"removes a failed claimed copy after its pending original was removed",
		() =>
			Effect.gen(function* () {
				const config = yield* ServerConfig.ServerConfig;
				const pendingPath = NodePath.join(
	...
	it.effect("keeps concurrent claims independent when one dispatch fails", () =>
		Effect.gen(function* () {
			...
			if (
				failed.type !== "thread.turn.start" ||
				succeeded.type !== "thread.turn.start"
			) {
	...
	it.effect(
		"removes earlier claimed copies when a later attachment cannot be normalized",
		() =>
			Effect.gen(function* () {
				const config = yield* ServerConfig.ServerConfig;
	...
	it.effect("rejects uploaded attachments with the wrong size or thread", () =>
		Effect.gen(function* () {
			...
			const wrongThread = yield* normalizeDispatchCommand(
				turnStartCommand({
					attachments: [
						{ id: `another-thread-${attachmentUuid}`, sizeBytes: 6 },
					],
				}),
			).pipe(Effect.flip);
			...
			const mismatchedType = yield* normalizeDispatchCommand({
				...
				message: {
					...
					attachments: mismatchedTypeCommand.message.attachments.map(
						(attachment) => ({
							...attachment,
							mimeType: "image/jpeg",
						}),
