...
const removeClaimedAttachmentPaths = Effect.fn(
	"Normalizer.removeClaimedAttachmentPaths",
...
export const normalizeDispatchCommand = (command: ClientOrchestrationCommand) =>
	Effect.gen(function* () {
		...
		const normalizedAttachments = yield* Effect.forEach(
			...
			Effect.tapError(() =>
				removeClaimedAttachmentPaths(claimedAttachmentPaths),
			),
		);
...
export const cleanupFailedUploadedAttachments = Effect.fn(
	...
	for (const [
		index,
		attachment,
	] of normalizedCommand.message.attachments.entries()) {
