...
const encodeEvent = Schema.encodeSync(
	Schema.fromJsonString(OrchestrationEvent),
);
...
describe("ThreadLiveEventCoalescer", () => {
	it("coalesces only calls with a stable toolCallId", () => {
		...
		expect(
			coalesceLiveToolUpdatedEvents(events).map((event) => event.sequence),
		).toEqual([2, 3]);
	});
	...
	it.effect(
		"flushes pending tool updates as soon as an unrelated event arrives",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					...
					yield* Effect.forEach(
						...
						(sequence) =>
							coalescer.offer({
								kind: "event",
								event: makeToolActivity(sequence),
							}),
						{ discard: true },
					);
	...
	it.effect(
		"flushes pending tool updates as soon as a synchronization marker arrives",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const coalescer = yield* makeThreadLiveEventCoalescer({
	...
	it.effect(
		"fails and clears pending updates when their serialized payload fills the budget",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					...
					const marker = yield* coalescer
						.offer({ kind: "synchronized" })
						.pipe(Effect.result);
					expect(marker._tag).toBe("Failure");
					const delivered = yield* coalescer.stream.pipe(
						Stream.runCollect,
						Effect.result,
					);
	...
	it.effect(
		"keeps the flush timer alive when an offer's shorter scope closes",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const coalescer = yield* makeThreadLiveEventCoalescer({
						coalesceWindow: "50 millis",
					});
					yield* Effect.scoped(
						coalescer.offer({ kind: "event", event: makeToolActivity(1) }),
					);
	...
	it.effect(
		"keeps an unacknowledged batch charged and clears later events on overflow",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const coalescer = yield* makeThreadLiveEventCoalescer({
					...
					yield* Effect.scoped(
						Effect.gen(function* () {
							...
							const overflow = yield* coalescer
								.offer({
									kind: "event",
									event: makeToolActivity(4, { kind: "tool.completed" }),
								})
								.pipe(Effect.result);
							expect(overflow._tag).toBe("Failure");
