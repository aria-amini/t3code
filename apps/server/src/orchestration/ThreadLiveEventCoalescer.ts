...
export const makeThreadLiveEventCoalescer = Effect.fn(
	...
	const cancelWindow = Effect.fn("ThreadLiveEventCoalescer.cancelWindow")(
		function* () {
	...
	const flushPending = Effect.fn("ThreadLiveEventCoalescer.flushPending")(
		function* () {
			...
			const items = yield* budget.replace(
				...
				coalesceLiveToolUpdatedEvents(
					pendingUpdates.map((item) => item.value),
				).map((event) => ({
					kind: "event" as const,
					event,
				})),
	...
	const flushWindow = (generation: number) =>
		Effect.sleep(coalesceWindow).pipe(
			Effect.andThen(
				mutex.withPermits(1)(
					Effect.suspend(() =>
						generation === windowGeneration ? flushPending() : Effect.void,
					),
				),
			),
			Effect.ensuring(
					...
					Effect.gen(function* () {
						...
						if (input.kind === "event") {
							...
							yield* budget.retain(projectActivityEvent(input.event)).pipe(
								Effect.tap((item) =>
									Effect.sync(() => pendingUpdates.push(item)),
								),
								Effect.uninterruptible,
							);
						}
						if (input.kind === "event" && isToolUpdated(input.event)) {
							if (pendingUpdates.length === 1) {
								...
								windowFiber = yield* Effect.forkIn(
									flushWindow(generation),
									coalescerScope,
								);
							}
