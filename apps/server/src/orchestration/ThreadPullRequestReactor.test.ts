...
const makeHarness = Effect.fn("makeThreadPullRequestHarness")(
	function* (options: {
		...
		const dependencies = Layer.mergeAll(
			...
			Layer.mock(GitManager)({
				branchPullRequest: (input, readOptions) =>
					Ref.update(branchCalls, (calls) => [
						...
						Effect.andThen(
							options.branchPullRequest?.(input, readOptions) ??
								Effect.succeed(null),
						),
					),
			}),
			Layer.mock(PullRequestService)({
				summary: (input, readOptions) =>
					Ref.update(summaryCalls, (calls) => [...calls, input]).pipe(
						Effect.andThen(
							options.summary?.(input, readOptions) ??
								Effect.succeed(summary(input, "open")),
						),
					),
			}),
			Layer.mock(RepositoryIdentityResolver)({
				resolve:
					...
					(() =>
						Effect.succeed(
							options.project?.repositoryIdentity ?? project.repositoryIdentity,
						)),
			}),
			...
			FileSystem.layerNoop({
				exists: (path) =>
					Effect.succeed(options.existingWorktrees?.includes(path) ?? false),
			}),
		);
...
describe("ThreadPullRequestReactor", () => {
	it.effect(
		"discovers saved branch PRs without a client and shares branch lookups",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					...
					yield* Effect.gen(function* () {
						...
						expect(
							(yield* Ref.get(fixture.snapshots)).threads[0]?.branchPullRequest,
						).toEqual(reference(42));
	...
	it.effect(
		"replaces terminal manual links but preserves open links and explicit unlink",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const fixture = yield* makeHarness({
					...
					yield* Effect.gen(function* () {
						...
						expect(
							snapshot.threads.map((current) => current.linkedPullRequest),
						).toEqual([reference(42), reference(42), reference(3), null]);
						expect(
							snapshot.threads.every(
								(current) => current.branchPullRequest?.number === 42,
							),
						).toBe(true);
					}).pipe(Effect.provide(fixture.layer));
				}),
			...
			Effect.gen(function* () {
				...
				const fixture = yield* makeHarness({
					...
					branchPullRequest: (_input, options) =>
						options?.refresh
							? Ref.set(detected, branchPullRequest()).pipe(
									Effect.andThen(Ref.get(detected)),
								)
							: Ref.get(detected),
				});
				yield* Effect.gen(function* () {
					...
					yield* reactor.drain;
					expect(
						(yield* Ref.get(fixture.commands))[0]?.branchPullRequest,
					).toEqual(reference(42));
					expect(
	...
	it.effect(
		"uses live worktrees and falls back to the project for removed worktrees",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const fixture = yield* makeHarness({
					...
					yield* Effect.gen(function* () {
						...
						expect(
							new Set(
								(yield* Ref.get(fixture.branchCalls)).map((call) => call.cwd),
							),
						).toEqual(new Set(["/workspace/project", "/workspace/worktree"]));
					}).pipe(Effect.provide(fixture.layer));
				}),
			),
	);
	...
	it.effect(
		"retains only terminal PRs on shared checkouts and clears a removed branch",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const fixture = yield* makeHarness({
						...
						summary: (input) =>
							Effect.succeed(
								summary(input, input.number === 1 ? "merged" : "open"),
							),
					});
					yield* Effect.gen(function* () {
						yield* fixture.start();
						const snapshot = yield* Ref.get(fixture.snapshots);
						expect(
							snapshot.threads.map((current) => current.branchPullRequest),
						).toEqual([reference(1), null, null, null]);
						expect(snapshot.threads[3]?.linkedPullRequest).toEqual(
							reference(3),
						);
					}).pipe(Effect.provide(fixture.layer));
				}),
	...
	it.effect(
		"keeps saved links on lookup failures and rejects a different repository",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const fixture = yield* makeHarness({
					...
					yield* Effect.gen(function* () {
						yield* fixture.start();
						expect(
							(yield* Ref.get(fixture.snapshots)).threads.map(
								(current) => current.branchPullRequest,
							),
	...
	it.effect(
		"retries failed settled backfills and stops querying them after success",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					...
					yield* Effect.gen(function* () {
						...
						yield* Queue.take(fixture.reads);
						yield* reactor.drain;
						expect((yield* Ref.get(fixture.commands))[0]?.threadId).toBe(
							"backfill",
						);
						yield* TestClock.adjust("1 minute");
						yield* Queue.take(fixture.reads);
						yield* reactor.drain;
						expect(
							(yield* Ref.get(fixture.branchCalls)).map((call) => call.branch),
						).toEqual(["feature", "feature", "feature"]);
					}).pipe(Effect.provide(fixture.layer));
				}),
			),
	);
	...
	it.effect(
		"stops retrying a settled backfill after repeated lookup failures",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const fixture = yield* makeHarness({
	...
	it.effect(
		"matches Azure SSH projects to HTTPS PRs with the provider repository selector",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const fixture = yield* makeHarness({
		...
		(changedRemote) =>
			Effect.scoped(
				Effect.gen(function* () {
					...
					const fixture = yield* makeHarness({
						threads: [
							thread("manual", { linkedPullRequest: reference(1) }),
							thread("automatic"),
						],
						...
						resolveRepositoryIdentity: (_cwd, options) =>
							options?.refresh
								? Ref.get(identity)
								: Effect.succeed(project.repositoryIdentity),
						summary: (input) =>
					...
					yield* Effect.gen(function* () {
						...
						expect(
							(yield* Ref.get(fixture.snapshots)).threads[0]?.linkedPullRequest,
						).toEqual(reference(1));
					}).pipe(Effect.provide(fixture.layer));
				}),
			),
