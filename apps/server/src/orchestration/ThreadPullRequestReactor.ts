...
export const make = Effect.gen(function* () {
	...
	const synchronize = Effect.fn("ThreadPullRequestReactor.synchronize")(
		function* (request: RefreshRequest) {
			...
			yield* Effect.forEach(
				...
				(group) =>
					Effect.gen(function* () {
						...
						if (first.branch !== null && repository === null)
						...
						if (
							detected !== null &&
							!pullRequestMatchesProject(detected, project)
						) {
							return finishBackfill(group);
						}
						...
						const plans = yield* Effect.forEach(group, (thread) =>
							Effect.gen(function* () {
								...
								if (
									...
									const previous = yield* pullRequests.summary(
								...
								if (
									...
									const linked = yield* pullRequests.summary(
								...
								Effect.catchCause((cause) =>
									Cause.hasInterruptsOnly(cause)
										...
										: Effect.logWarning(
												"thread pull request discovery failed",
												{
													threadId: thread.id,
												...
												Effect.tap(() =>
													Effect.sync(() => failBackfill([thread])),
												),
												Effect.as(null),
											),
								),
								...
								Effect.gen(function* () {
									...
									yield* engine.dispatch({
										...
										commandId: CommandId.make(
											`server:thread-pull-request:${thread.id}:${uuid}`,
										),
										threadId: thread.id,
										projectId: project.id,
						...
						Effect.catchCause((cause) =>
							Cause.hasInterruptsOnly(cause)
								? Effect.failCause(cause)
								: Effect.logWarning(
										"thread branch pull request lookup failed",
										{
											threadIds: group.map((thread) => thread.id),
											cause: Cause.pretty(cause),
										},
									).pipe(
										Effect.tap(() => Effect.sync(() => failBackfill(group))),
									),
						),
					),
