...
describe("resolveAutoSettlementAt", () => {
	...
	it("keeps a thread active at the exact inactivity boundary", () => {
		expect(
			decide(makeThread({ latestUserMessageAt: "2026-08-25T12:00:00.000Z" })),
		).toBe(false);
	});
	...
	it.each(["closed", "merged"] as const)(
		...
		(state) => {
			...
			expect(
				decide(makeThread(), { state, updatedAt: NOW }, { days: null }),
			).toBe(false);
		},
	);
	...
	it("requires a comparable PR timestamp for immediate settlement", () => {
		...
		expect(decide(recentThread, { state: "closed", closedAt: null })).toBe(
			false,
		);
		expect(decide(recentThread, { state: "merged", mergedAt: "unknown" })).toBe(
			false,
		);
		expect(decide(makeThread(), { state: "closed", closedAt: null })).toBe(
			true,
		);
	});
	...
	it("uses user request time instead of completion time as the PR anchor", () => {
		...
		expect(
			decide(thread, { state: "merged", mergedAt: "2026-08-26T00:00:00.000Z" }),
		).toBe(true);
	});
	...
	it("blocks pins, snooze, pending work, live sessions, and queued starts", () => {
		...
		expect(
			decide(makeThread({ snoozedUntil: "2026-08-29T00:00:00.000Z" })),
		).toBe(false);
		...
		expect(decide(makeThread({ backgroundLiveness: "monitoring" }))).toBe(
			false,
		);
		expect(
			decide(
