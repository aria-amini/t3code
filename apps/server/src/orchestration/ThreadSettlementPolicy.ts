...
export function threadHasQueuedTurnStart(
	...
	if (thread.latestUserMessageAt === null || thread.session?.status === "error")
		return false;
	const messageAt = Date.parse(thread.latestUserMessageAt);
	const age = Date.parse(now) - messageAt;
	if (Number.isNaN(age) || Math.abs(age) > QUEUED_TURN_START_GRACE_MS)
		return false;
...
export function resolveAutoSettlementAt(input: {
	...
	return Date.parse(activityAt) <
		Date.parse(input.now) - input.autoSettleAfterDays * DAY_MS
		? activityAt
		: null;
}
...
export function isAutoSettlementCandidate(
	...
	if (thread.archivedAt !== null || thread.settledOverride !== null)
		return false;
	...
	if (
		...
		Date.parse(thread.snoozedUntil) <= Date.parse(now)
	)
		return true;
	const wokeOnError =
