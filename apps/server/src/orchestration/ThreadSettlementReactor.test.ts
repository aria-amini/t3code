...
type AutoSettleCommand = Extract<
	OrchestrationCommand,
	{ readonly type: "thread.auto-settle" }
>;
...
const makeHarness = Effect.fn("makeThreadSettlementHarness")(function* (
	...
	const branchPullRequest: GitManager["Service"]["branchPullRequest"] = (
		...
		Ref.update(branchCalls, (calls) => [...calls, input]).pipe(
			Effect.andThen(
				options.branchPullRequest?.(input, readOptions) ?? Effect.succeed(null),
			),
		);
	const pullRequestSummary: PullRequestService["Service"]["summary"] = (
		input,
		readOptions,
	) =>
		Effect.gen(function* () {
	...
	const serverSettings = ServerSettingsService.of({
		...
		getSettings: Ref.get(settings).pipe(
			Effect.tap((value) => Queue.offer(settingsReads, value)),
		),
		updateSettings,
	...
	const dependencies = Layer.mergeAll(
		...
		Layer.mock(GitManager)({
			...
			invalidateStatus: (cwd) =>
				Ref.update(invalidatedCwds, (cwds) => [...cwds, cwd]),
		}),
		...
		FileSystem.layerNoop({
			exists: (path) =>
				Effect.succeed(options.existingWorktreePaths?.includes(path) ?? false),
		}),
	);
...
describe("ThreadSettlementReactor", () => {
	it.effect(
		"uses saved PRs without settling resumed threads or branches with newer PRs",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					...
					const fixture = yield* makeHarness({
						snapshot: makeSnapshot(
							[
								makeThread("retained-terminal", {
									branch: "main",
									branchPullRequest: previous,
								}),
								makeThread("reused-manual", {
									branch: "reused",
									linkedPullRequest: previous,
								}),
								makeThread("reused-detected", {
									branch: "reused",
									branchPullRequest: previous,
								}),
								makeThread("foreign-branch-pr", {
									branch: "foreign",
									linkedPullRequest: previous,
								}),
					...
					yield* Effect.gen(function* () {
						const reactor =
							yield* ThreadSettlementReactor.ThreadSettlementReactor;
						yield* startHarness(
							reactor,
							fixture.activation,
						...
						assert.deepStrictEqual(
							new Set(
								(yield* Ref.get(fixture.commands)).map(
									(command) => command.threadId,
								),
							),
							new Set([
								ThreadId.make("retained-terminal"),
								ThreadId.make("foreign-branch-pr"),
							]),
						);
	...
	it.effect(
		"skips PR work on startup, timer and merge sweeps when settlement is disabled",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					yield* TestClock.setTime(Date.parse(NOW));
	...
	it.effect(
		"starts without clients and skips protected threads before pull request lookup",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					yield* TestClock.setTime(Date.parse(NOW));
					...
					const fixture = yield* makeHarness({
						...
						pullRequestSummary: (input) =>
							Effect.succeed(
								makePullRequestSummary({ ...input, state: "closed" }),
							),
					});
					...
					yield* Effect.gen(function* () {
						...
						assert.deepStrictEqual(yield* Ref.get(fixture.summaryRecovery), [
							false,
						]);
					}).pipe(Effect.provide(fixture.layer));
				}),
	...
	it.effect(
		"reevaluates inactivity and pull request state once per minute",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					yield* TestClock.setTime(Date.parse(NOW));
					...
					const fixture = yield* makeHarness({
						...
						branchPullRequest: () =>
							Ref.get(pullRequest).pipe(
								Effect.map((state) => makeBranchPullRequest(state)),
							),
					});
						...
						branchPullRequest: () =>
							Ref.get(state).pipe(
								Effect.map((pullRequestState) =>
									makeBranchPullRequest(pullRequestState),
								),
							),
						onDispatch: () => Deferred.succeed(mergedThreadSettled, undefined),
					});
	...
	it.effect(
		"a merge does not settle threads linked to an unrelated pull request",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					yield* TestClock.setTime(Date.parse(NOW));
					...
					const fixture = yield* makeHarness({
						...
						pullRequestSummary: (input) =>
							Ref.updateAndGet(lookupCount, (count) => count + 1).pipe(
								...
								Effect.tap((count) =>
									count === 3
										? Deferred.succeed(mergeLookupStarted, undefined)
										: Effect.void,
								),
								Effect.tap((count) =>
									count === 3
										? Deferred.await(releaseMergeLookup)
										: Effect.void,
								),
								Effect.map(() =>
									makePullRequestSummary({ ...input, state: "open" }),
								),
							),
						onDispatch: () => Deferred.succeed(mergedThreadSettled, undefined),
					});
	...
	it.effect(
		"uses fresh settlement settings after lookup and ignores unrelated changes",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					yield* TestClock.setTime(Date.parse(NOW));
	...
	it.effect(
		"keeps an unknown pull request active and continues with other candidates",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					yield* TestClock.setTime(Date.parse(NOW));
					...
					yield* Effect.gen(function* () {
						...
						assert.strictEqual(
							(yield* Ref.get(fixture.summaryCalls)).length,
							1,
						);
					}).pipe(Effect.provide(fixture.layer));
				}),
	...
	it.effect(
		"settles inactive linked and branch threads without reading an unavailable host",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					yield* TestClock.setTime(Date.parse(NOW));
	...
	it.effect(
		"settles an inactive thread before its shared pull request lookup completes",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					yield* TestClock.setTime(Date.parse(NOW));
	...
	it.effect(
		"keeps threads active when their pull request project is unavailable",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					yield* TestClock.setTime(Date.parse(NOW));
	...
	it.effect(
		"deduplicates saved-branch and linked pull request lookups within a sweep",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					yield* TestClock.setTime(Date.parse(NOW));
					...
					const fixture = yield* makeHarness({
						...
						branchPullRequest: () =>
							Effect.succeed(makeBranchPullRequest("closed")),
						pullRequestSummary: (input) =>
							Effect.succeed(
								makePullRequestSummary({ ...input, state: "merged" }),
							),
	...
	it.effect(
		"looks up the branch pull request from a thread's live worktree",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					yield* TestClock.setTime(Date.parse(NOW));
					...
					yield* Effect.gen(function* () {
						...
						assert.deepStrictEqual(
							...
							new Set([
								{
									cwd: "/workspace/project-root/.worktrees/live",
									branch: "feature/live",
								},
	...
	it.effect(
		"carries the snapshot guard and survives a stale dispatch rejection",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					yield* TestClock.setTime(Date.parse(NOW));
					...
					yield* Effect.gen(function* () {
						...
						assert.strictEqual(
							firstSweep.find(
								(command) => command.threadId === ThreadId.make("stale"),
							)?.snapshotSequence,
							1,
						);
						assert.strictEqual(
							firstSweep.some(
								(command) =>
									command.threadId === ThreadId.make("next-candidate"),
							),
							true,
						);
