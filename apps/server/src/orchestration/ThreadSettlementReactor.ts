...
export const make = Effect.gen(function* () {
		...
		const lookupKey = (thread: (typeof candidates)[number]) => {
			...
			return JSON.stringify(
				cwd === undefined
					...
					: ["branch", cwd, thread.branch],
			);
		};
		const groups = Map.groupBy(lookupCandidates, lookupKey);

		const pullRequestFor = Effect.fn("ThreadSettlementReactor.pullRequestFor")(
			function* (thread: (typeof candidates)[number]) {
				const reference = thread.linkedPullRequest ?? thread.branchPullRequest;
				if (reference != null) {
					const matchesMerge =
					...
					if (!matchesMerge && !projects.has(reference.projectId)) {
						return yield* Effect.die(
							new Error("linked pull request project not found"),
						);
					}
		...
		yield* Effect.forEach(
			...
			(group) =>
				Effect.gen(function* () {
					...
					yield* Effect.forEach(
						group,
						(thread) => settleThread(thread, pullRequest),
						{
							discard: true,
						},
					);
	...
	const start: ThreadSettlementReactor["Service"]["start"] = Effect.fn(
		...
		const initialSettings = yield* settingsService.getSettings.pipe(
			Effect.orDie,
		);
