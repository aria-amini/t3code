...
it.layer(NodeServices.layer)("active thread ordering", (it) => {
	it.effect(
		"persists changed and repeated slots without changing thread activity timestamps",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"keeps placement through metadata, pin and snooze, then resets it on settlement",
		() =>
			Effect.gen(function* () {
				...
				for (const [index, [step, expectedKey]] of steps.entries()) {
					...
					expect(readModel.threads[0]?.activeOrderKey, command.type).toBe(
						expectedKey,
					);
				}
				expect(readModel.threads[0]).toMatchObject({
