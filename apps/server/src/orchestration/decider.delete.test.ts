...
function normalizeDeleteEvent(
	event: PlannedEvent | ReadonlyArray<PlannedEvent>,
) {
...
it.layer(NodeServices.layer)("decider deletion flows", (it) => {
	...
	it.effect(
		"reuses thread.delete semantics when force-deleting a non-empty project",
		() =>
			Effect.gen(function* () {
				const readModel = yield* seedReadModel;
				const projectDeleteCommand: Extract<
				...
				const forcedEvents = Array.isArray(forcedResult)
					? forcedResult
					: [forcedResult];
				...
				expect(normalizeDeleteEvent(forcedResult)).toEqual(
					normalizeDeleteEvent(sequentialEvents),
				);
			}),
	);
});
