...
it.layer(NodeServices.layer)("thread history import", (it) => {
	it.effect(
		"marks imported thread creation without changing live creation",
		() =>
			Effect.gen(function* () {
	...
	it.effect("settles imported messages at the latest absolute timestamp", () =>
		Effect.gen(function* () {
			...
			expect(
				projected.threads[0]?.messages.map((message) => message.text),
			).toEqual(["Fix the bug", "Fixed"]);
		}),
	);

	it.effect(
		"allows a thread with a newly imported user message to be settled",
		() =>
			Effect.gen(function* () {
				const createdAt = "2026-08-24T10:00:00.000Z";
	...
	it.effect(
		"rejects history import after a client message reaches the thread",
		() =>
			Effect.gen(function* () {
				const createdAt = "2026-08-24T10:00:00.000Z";
	...
	for (const requestKind of [
		...
		it.effect(
			`rejects history import with an open ${requestKind} activity`,
			() =>
				Effect.gen(function* () {
					const createdAt = "2026-08-24T10:00:00.000Z";
					...
					const withThread = yield* projectEvent(
						createEmptyReadModel(createdAt),
						{
							...
							commandId: CommandId.make(
								`command-${requestKind}-thread-created`,
							),
							causationEventId: null,
	...
	it.effect(
		"rejects a live user message in the imported-session namespace",
		() =>
			Effect.gen(function* () {
				const createdAt = "2026-08-24T10:00:00.000Z";
	...
	it.effect(
		"rejects live assistant messages in the imported-session namespace",
		() =>
			Effect.gen(function* () {
				const createdAt = "2026-08-24T10:00:00.000Z";
				...
				const readModel = yield* projectEvent(createEmptyReadModel(createdAt), {
					...
					causationEventId: null,
					correlationId: CommandId.make(
						"command-live-assistant-thread-created",
					),
					metadata: {},
					payload: {
						threadId,
				...
				for (const commandType of [
					...
					const command =
						commandType === "thread.message.assistant.delta"
							? {
									type: commandType,
									commandId: CommandId.make(
										"command-live-assistant-delta-import-id",
									),
									threadId,
									messageId: MessageId.make(
										"import:forged-live-assistant-message",
									),
									delta: "Live work",
									createdAt,
								}
							: {
									type: commandType,
									commandId: CommandId.make(
										"command-live-assistant-complete-import-id",
									),
									threadId,
									messageId: MessageId.make(
										"import:forged-live-assistant-message",
					...
					expect(error.message).toContain(
						"reserved imported-session namespace",
					);
				}
			}),
	);
