		...
		threads: [
			{
				...
				settledAt:
					input.settledAt ?? (input.settledOverride === "settled" ? NOW : null),
				snoozedUntil: input.snoozedUntil ?? null,
				snoozedAt:
					input.snoozedAt ?? (input.snoozedUntil != null ? PINNED_AT : null),
...
it.layer(NodeServices.layer)("pinned thread decider", (it) => {
	...
	it.effect("pinning a settled thread also un-settles it", () =>
		Effect.gen(function* () {
			...
			const unsettled = events.find(
				(entry) => entry.type === "thread.unsettled",
			);
