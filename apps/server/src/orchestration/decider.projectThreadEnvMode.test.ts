...
it.layer(NodeServices.layer)("decider project defaults", (it) => {
	it.effect("only treats metadata updates as explicit model defaults", () =>
		Effect.gen(function* () {
			...
			expect(
				(createdEvent.payload as { defaultModelSelection?: unknown })
			...
			expect(
				(updatedEvent.payload as { defaultModelSelection?: unknown })
					.defaultModelSelection,
			).toEqual(selection);
		}),
	);
	...
	it.effect(
		"propagates defaultThreadEnvMode through meta.update into the read model",
		() =>
			Effect.gen(function* () {
				const readModel = yield* projectEvent(
					createEmptyReadModel(now),
					seedProjectCreated(1),
				);
				...
				expect(
					(event.payload as { defaultThreadEnvMode?: unknown })
						.defaultThreadEnvMode,
				).toBe("worktree");

				const updated = yield* projectEvent(readModel, {
					...event,
					sequence: 2,
				});
	...
	it.effect("omits the field when unset and clears it on explicit null", () =>
		Effect.gen(function* () {
			...
			const unrelatedEvent = Array.isArray(unrelated)
				? unrelated[0]
				: unrelated;
			expect("defaultThreadEnvMode" in (unrelatedEvent.payload as object)).toBe(
				false,
			);
			...
			const afterSet = yield* projectEvent(readModel, {
				...setEvent,
				sequence: 2,
			});
			...
			const afterClear = yield* projectEvent(afterSet, {
				...clearEvent,
				sequence: 3,
			});
