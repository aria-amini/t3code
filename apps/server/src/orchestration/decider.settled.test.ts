		...
		threads: [
			{
				...
				snoozedAt:
					lifecycle.snoozedAt ??
					(lifecycle.snoozedUntil != null ? SETTLED_AT : null),
...
it.layer(NodeServices.layer)("settled thread decider", (it) => {
	...
	it.effect(
		"rejects an automatic settle when the thread is pinned active",
		() =>
			Effect.gen(function* () {
				const command = {
	...
	it.effect(
		"settles awake threads without a redundant wake and re-emits idempotently",
		() =>
			Effect.gen(function* () {
	...
	it.effect("settling a snoozed thread also wakes it", () =>
		Effect.gen(function* () {
			...
			const unsnoozed = events.find(
				(entry) => entry.type === "thread.unsnoozed",
			);
			if (
				settled?.type === "thread.settled" &&
				unsnoozed?.type === "thread.unsnoozed"
			) {
	...
	it.effect(
		"settling a pinned and snoozed thread clears the pin and snooze",
		() =>
			Effect.gen(function* () {
				const result = yield* decideOrchestrationCommand({
					command: {
	...
	it.effect(
		"rejects settling a thread with an open approval or user-input request",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"manual settlement dismisses async questions without starting a turn",
		() =>
			Effect.gen(function* () {
				const question = (
					requestId: string,
	...
	it.effect(
		"async questions do not bypass automatic settlement or other blockers",
		() =>
			Effect.gen(function* () {
				...
				for (const blocker of [
					...
					const error = yield* decideOrchestrationCommand({
						...
						readModel: makeReadModel(
							...
							null,
							makeSession(
								blocker === "running" || blocker === "starting"
									? blocker
									: "ready",
							),
							[
	...
	it.effect(
		"clears an open request when its respond failure marks it stale",
		() =>
			Effect.gen(function* () {
				const activity = (
					kind: string,
	...
	it.effect(
		"bounds the queued-turn grace window against client clock skew",
		() =>
			Effect.gen(function* () {
				const userMessage = (
					createdAt: string,
				...
				const queuedError = yield* decideOrchestrationCommand({
					...
					readModel: makeReadModel(
						...
						null,
						[],
						[userMessage("1969-12-31T23:59:30.000Z")],
					),
				}).pipe(Effect.flip);
				...
				const skewed = yield* decideOrchestrationCommand({
					...
					readModel: makeReadModel(
						...
						null,
						[],
						[userMessage("1970-01-01T01:00:00.000Z")],
					),
				});
	...
	it.effect(
		"maps unsettle reasons to overrides and re-emits idempotently",
		() =>
			Effect.gen(function* () {
				...
				const userAgainEvents = Array.isArray(userAgain)
					? userAgain
					: [userAgain];
	...
	it.effect(
		"an accepted un-settle re-anchors the thread for the active list",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"prepends activity unsets for turn starts and live session updates",
		() =>
			Effect.gen(function* () {
				...
				const turnEvents = Array.isArray(turnResult)
					? turnResult
					: [turnResult];
				...
				const sessionEvents = Array.isArray(sessionResult)
					? sessionResult
					: [sessionResult];
	...
	it.effect("clears a keep-active pin on real activity", () =>
		Effect.gen(function* () {
			...
			const activityEvents = Array.isArray(activityResult)
				? activityResult
				: [activityResult];
	...
	it.effect(
		"unsettles for approval and user-input activities but not others",
		() =>
			Effect.gen(function* () {
				...
				const approvalEvents = Array.isArray(approvalResult)
					? approvalResult
					: [approvalResult];
				...
				const routineEvents = Array.isArray(routineResult)
					? routineResult
					: [routineResult];
				expect(routineEvents.map((event) => event.type)).toEqual([
					"thread.activity-appended",
				]);
			}),
	);
	...
	it.effect(
		"drops an onlyIfSettled session stop when the thread was re-engaged",
		() =>
			Effect.gen(function* () {
				...
				const unconditionalEvents = Array.isArray(unconditional)
					? unconditional
					: [unconditional];
