...
it.layer(NodeServices.layer)("snoozed thread decider", (it) => {
	...
	it.effect(
		"re-emits idempotently for a duplicate snooze to the same wake time",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"unsnoozes with reason user and re-emits idempotently when awake",
		() =>
			Effect.gen(function* () {
				const event = yield* decideOrchestrationCommand({
					command: {
	...
	it.effect(
		"a user message spends the snooze return ticket (activity wake)",
		() =>
			Effect.gen(function* () {
