...
const decodeUserInputRequestedPayload = Schema.decodeUnknownOption(
	UserInputRequestedPayload,
);
const threadPullRequestLinksEqual = Schema.toEquivalence(
	Schema.NullOr(ThreadLinkedPullRequest),
);
 ...
 */
function isStaleRequestFailureDetail(
...
function hasQueuedTurnStartForThread(
	...
	for (const message of thread.messages) {
		if (message.role !== "user" || isImportedAgentSessionMessageId(message.id))
			continue;
	...
	return threadHasQueuedTurnStart(
		{
			latestUserMessageAt: Number.isFinite(latestUserMessageAtMs)
				? latestUserMessageAt
				: null,
...
> {
	switch (command.type) {
			...
			if (command.scripts !== undefined) {
				...
				for (const script of command.scripts) {
					if (
						...
						!isScriptRunCommand(`script.${script.id}.run`)
					) {
		...
		case "thread.auto-settle": {
			...
			return companionEvents.length > 0
				? [settledEvent, ...companionEvents]
				: settledEvent;
		}
		...
		case "thread.pull-request.sync": {
			...
			if (
				...
				project.workspaceRoot !== command.expected.workspaceRoot
			) {
				return yield* new OrchestrationCommandInvariantError({
					commandType: command.type,
		...
		case "thread.history.import": {
			...
			const settledAt = command.messages.reduce(
				(latest, message) =>
					compareDateTimeStrings(message.createdAt, latest) > 0
						? message.createdAt
						: latest,
