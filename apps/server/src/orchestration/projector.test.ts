...
function makeEvent(input: {
	...
	return {
		...
		commandId:
			input.commandId === null ? null : CommandId.make(input.commandId),
...
describe("orchestration projector", () => {
	...
	effectIt.effect(
		"sets and clears branch pull requests without changing manual links",
		() =>
			Effect.gen(function* () {
				const now = "2026-01-01T00:00:00.000Z";
				const eventFields = {
				...
				for (const [index, update] of updates.entries()) {
					...
					expect(model.threads[0]?.linkedPullRequest).toEqual(
						linkedPullRequest,
					);
				}
			}),
	...
	it("prunes reverted turn messages from in-memory thread snapshot", async () => {
		...
		const afterRevert = await events.reduce<
			Promise<ReturnType<typeof createEmptyReadModel>>
		>(
			(statePromise, event) =>
				statePromise.then((state) =>
					Effect.runPromise(projectEvent(state, event)),
				),
