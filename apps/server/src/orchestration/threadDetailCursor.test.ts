...
describe("threadDetailCursor", () => {
	it("round-trips a cursor", () => {
		...
		expect(
			decodeThreadDetailPageCursor(encodeThreadDetailPageCursor(cursor)),
		).toEqual(cursor);
	});
	...
	it("rejects malformed input", () => {
		...
		expect(
			decodeThreadDetailPageCursor(Buffer.from("[]").toString("base64url")),
		).toBeNull();
		expect(
			decodeThreadDetailPageCursor(
		...
		expect(
			decodeThreadDetailPageCursor(
				Buffer.from(JSON.stringify({ t: "thread-1", a: 5, i: "x" })).toString(
					"base64url",
				),
			),
		).toBeNull();
