...
function hydratePosixPath(
	...
	for (const shell of listLoginShellCandidates(platform, env.SHELL)) {
		try {
			...
			logPathHydrationWarning(
				`Failed to read PATH from login shell ${shell}.`,
				error,
			);
		}
	...
	const launchctlPath =
		platform === "darwin" && !shellPath ? readPathFromLaunchctl() : undefined;
...
> {
	...
	yield* Effect.sync(() => hydratePosixHome(env)).pipe(
		Effect.catchDefect((defect) =>
			Effect.sync(() => {
				logPathHydrationWarning(
					"Failed to hydrate HOME from the user account.",
					defect,
				);
			}),
		),
