...
it.effect("names a real SQLite condition without copying query data", () =>
	Effect.gen(function* () {
		...
		const cause = yield* Effect.flip(
			sql`INSERT INTO error_test VALUES (${payload})`,
		);
		const error = toPersistenceSqlError(
...
it.each([{ errno: 1555, code: "SQLITE_CONSTRAINT_PRIMARYKEY" }, { errno: 1 }])(
	...
	(condition) => {
		...
		const error = toPersistenceSqlError("AuthSessionRepository.create:query")(
			cause,
		);
...
it.effect(
	"summarizes a schema cause by issue tag instead of by rejected value",
	() =>
		Effect.gen(function* () {
...
it.effect(
	"maps schema errors without copying rejected payloads into diagnostics",
	() =>
		Effect.gen(function* () {
