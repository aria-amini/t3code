...
const makeEventStore = Effect.gen(function* () {
          ...
          WHERE aggregate_kind = ${request.aggregateKind}
            ...
            AND ${sql.and([
							...
							...(request.type === undefined
								? []
								: [sql`event_type = ${request.type}`]),
						])}
          LIMIT 1
        `,
	});

	const hasEventAfter: OrchestrationEventStoreShape["hasEventAfter"] = (
		input,
	) =>
	...
	const readAggregateRange: OrchestrationEventStoreShape["readAggregateRange"] =
		(input) => {
			...
			if (
				limit === 0 ||
				input.fromSequenceExclusive >= input.toSequenceInclusive
			) {
				return Stream.empty;
			}
			const readPage = (
				...
				Stream.fromEffect(
					...
					Stream.flatMap((events) => {
						...
						return Stream.concat(
							Stream.fromIterable(events),
							readPage(last.sequence, nextRemaining),
						);
					}),
				);
	...
	const getAggregateReplayStats: OrchestrationEventStoreShape["getAggregateReplayStats"] =
		(input) =>
			readAggregateReplayStats({
				...
				Effect.map((row) => ({
					...row,
					hasCreateEvent: row.hasCreateEvent !== 0,
				})),
			);
