...
const makeProjectionCheckpointRepository = Effect.gen(function* () {
	...
	const listByThreadId: ProjectionCheckpointRepositoryShape["listByThreadId"] =
		(input) =>
				...
				Effect.map(
					(rows) =>
						rows as ReadonlyArray<
							Schema.Schema.Type<typeof ProjectionCheckpoint>
						>,
				),
			);
				...
				Effect.flatMap((rowOption) =>
					Option.match(rowOption, {
						...
						onSome: (row) =>
							Effect.succeed(
								Option.some(
									row as Schema.Schema.Type<typeof ProjectionCheckpoint>,
								),
							),
					}),
				),
