...
projectionRepositoriesLayer("Projection repositories", (it) => {
	it.effect(
		"selects the latest-turn plan before checking implementation status",
		() =>
			Effect.gen(function* () {
				...
				assert.isTrue(
					yield* plans.hasActionableByThreadId({
						threadId,
						latestTurnId: null,
					}),
				);
	...
	it.effect(
		"falls back within the thread when the latest turn has no plan",
		() =>
			Effect.gen(function* () {
				const plans = yield* ProjectionThreadProposedPlanRepository;
	...
	it.effect(
		"preserves locale ordering and stable ties when selecting plan status",
		() =>
			Effect.gen(function* () {
				const plans = yield* ProjectionThreadProposedPlanRepository;
				...
				const cases = [
					...
					},
					{
						...
						expected:
							"2026-03-24T00:00:00+00:00".localeCompare(
	...
	it.effect(
		"returns only current-turn status metadata when old plans have large bodies",
		() =>
			Effect.gen(function* () {
				const plans = yield* ProjectionThreadProposedPlanRepository;
      ...
      `;
				...
				const actionable = yield* plans
					.hasActionableByThreadId({ threadId, latestTurnId })
					.pipe(
				...
				if (statement === undefined)
					return yield* Effect.die("Expected a plan status query.");
	...
	it.effect("reads only the requested plan in its thread", () =>
      ...
      `;
			const plan = Option.getOrThrow(
				yield* plans.getByPlanId({ threadId, planId }),
			);
	...
	it.effect("stores SQL NULL for missing project model options", () =>
      ...
      `;
			...
			assert.deepStrictEqual(
				Option.getOrNull(persisted)?.defaultModelSelection,
				{
					instanceId: ProviderInstanceId.make("codex"),
					model: "gpt-5.4",
				},
	...
	it.effect(
		"round-trips non-null settlement values through the thread row",
		() =>
			Effect.gen(function* () {
				const threads = yield* ProjectionThreadRepository;
				...
				if (!row) {
					return yield* Effect.die(
						"Expected settled projection_threads row to exist.",
					);
				}
	...
	it.effect(
		"round-trips manual and branch pull requests through the thread row",
		() =>
			Effect.gen(function* () {
				const threads = yield* ProjectionThreadRepository;
				...
				assert.deepStrictEqual(
					Option.getOrNull(persisted)?.linkedPullRequest,
					linkedPullRequest,
				);
				assert.deepStrictEqual(
					Option.getOrNull(persisted)?.branchPullRequest,
					branchPullRequest,
				);
				...
				const row = Option.getOrNull(persisted);
				if (row === null)
					return yield* Effect.die("Expected linked thread row to exist.");
				...
				assert.deepStrictEqual(
					Option.getOrNull(cleared)?.branchPullRequest,
					branchPullRequest,
				);

				yield* threads.upsert({ ...row, branchPullRequest: null });
				const branchCleared = yield* threads.getById({
					threadId: row.threadId,
				});
				assert.strictEqual(
					Option.getOrNull(branchCleared)?.branchPullRequest,
					null,
				);
				assert.deepStrictEqual(
					Option.getOrNull(branchCleared)?.linkedPullRequest,
					linkedPullRequest,
				);
			}),
	);
