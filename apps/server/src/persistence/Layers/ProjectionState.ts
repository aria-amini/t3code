...
const makeProjectionStateRepository = Effect.gen(function* () {
	...
	const upsert: ProjectionStateRepositoryShape["upsert"] = (row) =>
		upsertProjectionStateRow(row).pipe(
			Effect.mapError(
				toPersistenceSqlError("ProjectionStateRepository.upsert:query"),
			),
		);
	...
	const upsertMany: ProjectionStateRepositoryShape["upsertMany"] = (rows) =>
		upsertProjectionStateRows(rows).pipe(
			Effect.mapError(
				toPersistenceSqlError("ProjectionStateRepository.upsertMany:query"),
			),
		);

	const getByProjector: ProjectionStateRepositoryShape["getByProjector"] = (
		input,
	) =>
		getProjectionStateRow(input).pipe(
			Effect.mapError(
				toPersistenceSqlError("ProjectionStateRepository.getByProjector:query"),
			),
		);
	...
	const listAll: ProjectionStateRepositoryShape["listAll"] = () =>
		listProjectionStateRows(undefined).pipe(
			Effect.mapError(
				toPersistenceSqlError("ProjectionStateRepository.listAll:query"),
			),
		);
