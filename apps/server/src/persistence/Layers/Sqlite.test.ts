	...
	Effect.promise(
		() =>
			new Promise<void>((resolve, reject) => {
				...
				holder.on("exit", () =>
					reject(
						new Error("lock holder exited before acquiring the write lock"),
					),
				);
			}),
	);
...
it.effect(
	"waits out a concurrent writer instead of failing with SQLITE_BUSY",
	() => {
		const tempDir = NodeFS.mkdtempSync(
			NodePath.join(NodeOS.tmpdir(), "t3-sqlite-busy-"),
		);
