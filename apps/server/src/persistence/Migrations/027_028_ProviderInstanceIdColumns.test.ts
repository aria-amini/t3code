...
layer("027_028_ProviderInstanceIdColumns", (it) => {
	it.effect(
		"continues when provider_session_runtime was partially migrated",
		() =>
			Effect.gen(function* () {
      ...
      `;
				assert.ok(
					providerSessionColumns.some(
						(column) => column.name === "provider_instance_id",
					),
				);

				const projectionThreadSessionColumns = yield* sql<{
					readonly name: string;
				}>`
        PRAGMA table_info(projection_thread_sessions)
      `;
				assert.ok(
					projectionThreadSessionColumns.some(
						(column) => column.name === "provider_instance_id",
					),
				);
      ...
      `;
				...
				const projectionThreadSessionIndexes = yield* sql<{
					readonly name: string;
				}>`
