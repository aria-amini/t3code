...
const authSessionLayer = AuthSessions.layer.pipe(
...
describe("persistence error correlation", () => {
	it.effect(
		"correlates auth session SQL and row-decode failures without sensitive fields",
		() =>
			Effect.gen(function* () {
				const sessions = yield* AuthSessions.AuthSessionRepository;
				const sql = yield* SqlClient.SqlClient;
				const sessionId = AuthSessionId.make("session-correlation");
				const currentSessionId = AuthSessionId.make(
					"current-session-correlation",
				);
      ...
      `;
				...
				assert.instanceOf(
					decodeError,
					PersistenceErrors.PersistenceDecodeError,
				);
				...
				assert.equal(
					createError.message,
					"SQL error in AuthSessionRepository.create:query",
				);
				...
				assert.instanceOf(
					revokeOtherError,
					PersistenceErrors.PersistenceSqlError,
				);
				assert.deepStrictEqual(revokeOtherError.correlation, {
					currentSessionId,
				});
				assert.equal(
	...
	it.effect(
		"correlates pairing-link create and revoke failures by id only",
		() =>
			Effect.gen(function* () {
      ...
      `;
				...
				const decodeError = yield* Effect.flip(
					pairingLinks.getByCredential({ credential }),
				);
				assert.instanceOf(
					decodeError,
					PersistenceErrors.PersistenceDecodeError,
				...
				const revokeError = yield* Effect.flip(
					pairingLinks.revoke({ id, revokedAt: now }),
				);
	...
	it.effect(
		"skips undecodable provider runtime rows and correlates SQL failures by thread",
		() =>
			Effect.gen(function* () {
				const runtimes =
