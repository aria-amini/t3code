...
it.layer(PreviewManager.layer)("PreviewManager", (it) => {
	...
	it.effect(
		"keeps the tab's profile across navigation and status reports",
		() =>
			Effect.gen(function* () {
				const threadId = freshThreadId();
				const manager = yield* PreviewManager.PreviewManager;
				...
				expect(
					listed.sessions.find((s) => s.tabId === opened.tabId)?.profileId,
				).toBe("work");
			}),
	);
	...
	it.effect(
		"orders list snapshots and events with one monotonic revision",
		() =>
			Effect.gen(function* () {
				const threadId = freshThreadId();
				const manager = yield* PreviewManager.PreviewManager;
	...
	it.effect("rejects empty URL with PreviewInvalidUrlError", () =>
		Effect.gen(function* () {
			...
			expect((error.cause as PreviewUrlNormalizationError).reason).toBe(
				"empty",
			);
		}),
	);
	...
	it.effect(
		"preserves URL parser failures as the invalid URL cause chain",
		() =>
			Effect.gen(function* () {
				const threadId = freshThreadId();
				const manager = yield* PreviewManager.PreviewManager;
				...
				const error = yield* Effect.flip(
					manager.open({ threadId, url: rawUrl }),
				);
				...
				expect(error.message).not.toContain(
					(normalizationError.cause as Error).message,
				);
	...
	it.effect(
		"resizes a tab and preserves its viewport across navigation reports",
		() =>
			Effect.gen(function* () {
				const threadId = freshThreadId();
				const manager = yield* PreviewManager.PreviewManager;
				...
				expect(resized.viewport).toEqual({
					_tag: "freeform",
					width: 1024,
					height: 768,
				});
	...
	it.effect(
		"list returns every snapshot for the thread sorted by updatedAt",
		() =>
			Effect.gen(function* () {
				const threadId = freshThreadId();
				const manager = yield* PreviewManager.PreviewManager;
