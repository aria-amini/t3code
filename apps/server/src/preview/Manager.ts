...
export class PreviewManager extends Context.Service<
	PreviewManager,
	{
		...
		) => Effect.Effect<PreviewListResult>;
...
const compositeKey = (threadId: string, tabId: string): string =>
	`${threadId}\u0000${tabId}`;
...
export const make = Effect.gen(function* PreviewManagerMake() {
	...
	const mutateExistingSession = <R, E>(
		...
		mutator: (
			...
			{ next: PreviewSessionState; emit: PreviewEventDraft | null; result: R },
			E
		>,
		...
		return SynchronizedRef.modifyEffect(stateRef, (state) => {
			...
			if (!session) {
				return Effect.succeed([
					{
						kind: "fail",
						error: new PreviewSessionLookupError({ threadId, tabId }),
					},
					state,
			...
			return mutator(session).pipe(
				Effect.flatMap(
					Effect.fn("PreviewManager.commitMutation")(function* ({
						...
						return [
							{ kind: "ok", result } as ModifyResult,
							{ sessions, revision },
						] as readonly [ModifyResult, ManagerState];
					}),
				),
			...
			Effect.flatMap((modify) =>
				modify.kind === "fail"
					? Effect.fail(modify.error)
					: Effect.succeed(modify.result),
			),
		);
	};

	const open: PreviewManager["Service"]["open"] = Effect.fn(
		"PreviewManager.open",
	)(function* (input) {
	...
	const navigate: PreviewManager["Service"]["navigate"] = Effect.fn(
		"PreviewManager.navigate",
	)(function* (input) {
	...
	const resize: PreviewManager["Service"]["resize"] = Effect.fn(
	...
	const refresh: PreviewManager["Service"]["refresh"] = Effect.fn(
	...
	const close: PreviewManager["Service"]["close"] = Effect.fn(
		...
		yield* SynchronizedRef.modifyEffect(stateRef, (state) => {
			...
			const targets = input.tabId
				? [
						state.sessions.get(compositeKey(input.threadId, input.tabId)),
					].filter((entry): entry is PreviewSessionState => entry !== undefined)
			...
			return Effect.as(
				Effect.forEach(
					eventsToEmit,
					(event) => PubSub.publish(eventsPubSub, event),
					{
						discard: true,
					},
				),
	...
	const list: PreviewManager["Service"]["list"] = Effect.fn(
		"PreviewManager.list",
	)(function* (input) {
