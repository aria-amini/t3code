...
const processProbeFailure: ProcessRunner.ProcessRunner["Service"]["run"] = (
	input,
) =>
	Effect.fail(
...
const TestIntegrationNet = Layer.succeed(Net.NetService, {
	...
	isPortAvailableOnLoopback: (port) =>
		Effect.sync(() => port !== integrationListeningPort),
	hasListenerOnHost: (port) =>
		Effect.sync(() => port === integrationListeningPort),
...
const openCommonDevServer = Effect.fn("PortScannerTest.openCommonDevServer")(
	function* (
		...
		return yield* Effect.die(
			new Error(
				"No common development port was available for the preview scanner test",
			),
		);
	},
);
 ...
 */
effectIt.layer(TestPortDiscoveryLive)(
	"PortDiscovery integration (TCP probe fallback)",
	(it) => {
		it.effect(
...
effectIt.effect(
	"revalidates a successful HTML probe after its cache entry expires",
	() => {
		let responds = true;
...
effectIt.effect(
	"keeps a full configured URL when the discovered server root fails",
	() => {
		const requests: string[] = [];
...
effectIt.effect(
	"probes configured custom ports through a canonical loopback host",
	() => {
...
effectIt.effect(
	"preserves explicit loopback hosts and bounds wildcard rewrites",
	() => {
...
effectIt.effect(
	"projects configured paths independently for simultaneous subscribers",
	() => {
		...
		return Effect.gen(function* () {
			...
			yield* scanner.subscribe(
				{ configuredUrls: [docsUrl], initialSnapshot: [] },
				(servers) => Effect.sync(() => docsSnapshots.push(servers)),
			);
			yield* scanner.subscribe(
				{ configuredUrls: [adminUrl], initialSnapshot: [] },
				(servers) => Effect.sync(() => adminSnapshots.push(servers)),
			);
			yield* scanner.retain;
...
effectIt.effect(
	"stops probing a subscriber's configured paths after its scope closes",
	() => {
		const docsUrl = `http://localhost:${LSOF_TEST_PORT}/docs`;
		const adminUrl = `http://localhost:${LSOF_TEST_PORT}/admin`;
		const requests: string[] = [];
...
effectIt.effect(
	"uses the current configured fragment when readiness comes from cache",
	() => {
		const requests: string[] = [];
		const fetchFn = ((input: Parameters<typeof globalThis.fetch>[0]) => {
			requests.push(String(input));
...
effectIt.effect(
	"shares a configured root probe with discovered-root classification",
	() => {
		const requests: string[] = [];
...
effectIt.effect(
	"starts fresh cache entries after the probing batch completes",
	() =>
		Effect.gen(function* () {
...
effectIt.effect(
	"caches a failed web probe until its bounded cache entry expires",
	() => {
		let responds = false;
		const requests: string[] = [];
		const fetchFn = ((input: Parameters<typeof globalThis.fetch>[0]) => {
...
effectIt.effect(
	"falls back to HTTPS and does not follow redirects while probing",
	() => {
		...
		const fetchFn = (async (
			...
			if (String(input).startsWith("http:"))
				throw new TypeError("TLS listener");
...
effectIt.effect(
	...
	() => {
		...
		return Effect.gen(function* () {
			...
			pid += 1;
			makeResponse = () =>
				new Response(null, { status: 304, headers: { location: "/cached" } });
			expect(yield* scanner.scan()).toHaveLength(0);
