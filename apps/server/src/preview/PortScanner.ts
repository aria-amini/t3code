...
const normalizeConfiguredUrls = (
	...
	...new Set(
		urls
			...
			.filter(
				(url): url is URL =>
					url !== null && url.href.length <= PREVIEW_URL_MAX_LENGTH,
			)
			.map((url) => {
...
const projectWebProbeSnapshot = (
	...
	return [...visibleByServer.values()].toSorted(
	...
	for (const line of raw.split(/\r?\n/g)) {
		...
		seen.set(port, {
			...
			terminal:
				normalizedPid === null
					? null
					: (terminalByProcessId.get(normalizedPid) ?? null),
		});
	}
...
export const make = Effect.gen(function* PortDiscoveryMake() {
	...
	const webProbeCacheRef = yield* Ref.make<
		ReadonlyMap<string, WebProbeCacheEntry>
	>(new Map());
	const scanSemaphore = yield* Semaphore.make(1);

	const probeCommonPorts = Effect.fn("PortDiscovery.probeCommonPorts")(
		function* () {
	...
	const makeWebProbeGroups = (
		...
		const serversByKey = new Map(
			servers.map(
				(server) => [localServerKey(server.host, server.port), server] as const,
			),
		);
	...
	const probeWebServers = Effect.fn("PortDiscovery.probeWebServers")(function* (
		...
		const batchProbes = new Map<
			string,
			Effect.Effect<{
				readonly probe: WebProbeCacheEntry;
		...
		const nextCache = new Map(
			[...cached].filter(
				([, probe]) => probe.expiresAtMillis > completedAtMillis,
			),
		);
			...
			for (const [key, probe, fresh] of probes) {
				nextCache.set(
					...
					fresh
						? {
								...probe,
								expiresAtMillis: completedAtMillis + WEB_PROBE_CACHE_TTL_MS,
							}
						: probe,
				);
			}
	...
	const recoverProcessProbeFailure =
		(probe: "lsof" | "windows-listeners") =>
		(error: ProcessRunner.ProcessRunError) =>
			Effect.logDebug(
				"preview port process probe failed; falling back to common-port probes",
				{
					cause: error,
					probe,
	...
	const scanUnlocked = Effect.fn("PortDiscovery.scanUnlocked")(function* (
		...
		if (hostPlatform === "win32") {
			const recoverWindowsProbeFailure =
				recoverProcessProbeFailure("windows-listeners");
			const command =
		...
		if (lsofResult !== null)
			return yield* probeWebServers(lsofResult, configuredUrls);
		return yield* probeWebServers(yield* probeCommonPorts(), configuredUrls);
	});
	...
	const pollTick = Effect.fn("PortDiscovery.pollTick")(
		function* () {
			...
			const notifications = yield* Ref.modify(stateRef, (state) => {
				...
				const changed: Array<
					readonly [Listener, ReadonlyArray<DiscoveredLocalServer>]
				> = [];
			...
			yield* Effect.forEach(
				notifications,
				([listener, servers]) => listener(servers),
				{
					discard: true,
				},
			);
	...
	yield* Effect.forkScoped(
		pollTick().pipe(Effect.repeat(Schedule.spaced(POLL_INTERVAL))),
	);
	...
	const subscribe: PortDiscovery["Service"]["subscribe"] = Effect.fn(
	...
	const unregisterTerminal: PortDiscovery["Service"]["unregisterTerminal"] =
		Effect.fn("PortDiscovery.unregisterTerminal")(function* (input) {
			yield* Ref.update(stateRef, (state) => {
				const terminalProcesses = new Map(state.terminalProcesses);
