...
function makeMockDetachedHandle(
	input: MockSpawnResult & { readonly onUnref?: () => void } = {},
) {
...
it.effect.skipIf(windowsHost)(
	"reveals a file in Finder with open -R on macOS",
	() =>
		Effect.gen(function* () {
			const fileSystem = yield* FileSystem.FileSystem;
			const path = yield* Path.Path;
			...
			assert.deepEqual(spawned.args, [
				"-R",
				"/workspace/media/linux-mini-v2.mp4",
			]);
		}).pipe(Effect.scoped, Effect.provide(NodeServices.layer)),
);
...
it.effect("reveals a file in File Explorer through PowerShell on Windows", () =>
	Effect.gen(function* () {
		...
		yield* fileSystem.makeDirectory(path.dirname(powerShellPath), {
			recursive: true,
		});
...
it.skipIf(process.platform !== "win32")(
	...
	async () => {
		const tempDir = NodeFS.mkdtempSync(
			NodePath.join(NodeOS.tmpdir(), "t3-reveal-smoke-"),
		);
		try {
...
it.effect(
	"does not advertise reveal on Windows when PowerShell is missing",
	() =>
		Effect.gen(function* () {
			const fileSystem = yield* FileSystem.FileSystem;
			const path = yield* Path.Path;
...
it.effect.skipIf(windowsHost)(
	...
	() =>
		Effect.gen(function* () {
			...
			const launch = spawnedCommands.find(
				(command) => command.command === "xdg-open",
			);
			assert.ok(launch);
			assert.deepEqual(launch.args, ["/home/t3/workspace/media"]);
			assert.isUndefined(
				spawnedCommands.find((command) => command.command === "explorer.exe"),
			);
		}).pipe(Effect.scoped, Effect.provide(NodeServices.layer)),
);
...
it.effect.skipIf(windowsHost)(
	...
	() =>
		Effect.gen(function* () {
			...
			assert.deepEqual(spawned.args, [
				'\\\\wsl.localhost\\Ubuntu-24.04\\home\\t3\\work "quoted"',
			]);
		}).pipe(Effect.scoped, Effect.provide(NodeServices.layer)),
);

it.effect.skipIf(windowsHost)(
	"reveals by opening the containing directory on Linux",
	() =>
		Effect.gen(function* () {
			const fileSystem = yield* FileSystem.FileSystem;
			const path = yield* Path.Path;
			...
			const spawned = spawnedCommands.find(
				(command) => command.command === "xdg-open",
			);
			assert.ok(spawned);
	...
	() =>
		Effect.gen(function* () {
			...
			const editors = yield* Effect.gen(function* () {
				...
				Effect.provide(testLayer({ platform: "linux", env: { PATH: binDir } })),
			);
			...
			const editors = yield* Effect.gen(function* () {
				...
				Effect.provide(
					testLayer({
						...
						env: { PATH: binDir, DISPLAY: ":0" },
						spawnResult: (command) =>
							command.command === "xdg-mime" ? { stdout: "" } : undefined,
					}),
				),
			);
...
it.live.skipIf(windowsHost)(
	"a stalled handler probe drops only the file manager",
	() =>
		Effect.gen(function* () {
			const fileSystem = yield* FileSystem.FileSystem;
			const path = yield* Path.Path;
			...
			const editors = yield* Effect.gen(function* () {
				...
				Effect.provide(
					testLayer({
						...
						env: { PATH: binDir, DISPLAY: ":0" },
						spawnResult: (command) =>
							command.command === "xdg-mime" ? { stall: true } : undefined,
					}),
				),
			);
...
it.effect(
	"memoizes editor discovery and refreshes after the cache window",
	() => {
		let statCalls = 0;
				...
				Layer.mergeAll(
					...
					Layer.succeed(
						...
						ChildProcessSpawner.make(() =>
							Effect.sync(() => makeMockDetachedHandle()),
						),
					),
				),
			),
...
it.effect(
	"rescans after an interrupted discovery instead of caching the interrupt",
	() => {
		const fileInfo = { type: "File" } as FileSystem.File.Info;
...
it.effect("rejects unknown editors through the service API", () =>
	Effect.gen(function* () {
		...
		assert.instanceOf(
			error,
			ExternalLauncher.ExternalLauncherUnknownEditorError,
		);
