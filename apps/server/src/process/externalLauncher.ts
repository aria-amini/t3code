...
function parseTargetPathAndPosition(
	target: string,
): Option.Option<TargetPathAndPosition> {
...
const resolveAvailableCommand = Effect.fn(
...
const hasUsableLinuxDirectoryHandler = Effect.fn(
...
> {
	...
	return yield* spawner
		...
		.pipe(
			...
			Effect.map(
				([stdout, exitCode]) => exitCode === 0 && stdout.trim().length > 0,
			),
			Effect.scoped,
...
const isUsableFileManagerCommand = Effect.fn(
	"externalLauncher.isUsableFileManagerCommand",
)(function* (
	command: string,
	env: NodeJS.ProcessEnv,
): Effect.fn.Return<
...
> {
	...
	if (
		command !== undefined &&
		(yield* isUsableFileManagerCommand(command, env))
	) {
		return command;
	}
	if (
...
const buildAvailableEditors = Effect.fn(
...
> {
	...
	for (const editor of EDITORS) {
		if (editor.commands === null) {
			if (
				(yield* resolveUsableFileManagerCommand(platform, env)) !== undefined
			) {
...
const resolveBrowserLaunch = Effect.fn("externalLauncher.resolveBrowserLaunch")(
...
const resolveAvailableEditors = Effect.fn(
	"externalLauncher.resolveAvailableEditors",
)(function* () {
	const platform = yield* HostProcessPlatform;
	const env = {
		...(yield* readBrowserLaunchEnv),
		...(yield* readCommandLookupEnv),
	};
	return yield* buildAvailableEditors(platform, env);
});

const resolveFileManagerRevealKind = Effect.fn(
...
export class ExternalLauncher extends Context.Service<
	ExternalLauncher,
	{
		readonly resolveAvailableEditors: () => Effect.Effect<
			ReadonlyArray<EditorId>
		>;
		/**
		 ...
		 */
		readonly resolveFileManagerRevealKind: () => Effect.Effect<
			FileManagerRevealKind | undefined
		>;
...
function fileExplorerRevealLaunch(
	...
	return {
		...
		args: [
			...
			encodeUtf16LeBase64(
				buildFileExplorerRevealPowerShellSource("explorer.exe", explorerTarget),
			),
		],
	};
}
...
> {
	...
	if (
		...
		const explorerTarget = resolveWslFileManagerPath(
			target,
			env.WSL_DISTRO_NAME,
		);
		...
		return {
			...
			command: "explorer.exe",
			args: [
				resolveWslFileManagerPath(path.dirname(target), env.WSL_DISTRO_NAME),
			],
		};
	}
...
const launchEditorProcess = Effect.fn("externalLauncher.launchEditorProcess")(
	function* (
		launch: EditorLaunch,
	): Effect.fn.Return<
		void,
