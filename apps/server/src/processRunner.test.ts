...
function makeSpawner(
	...
	return ChildProcessSpawner.make((command) =>
		f(asChildProcessCommand(command)),
	);
}
...
describe("runProcess", () => {
	...
	it.effect(
		"resolves and escapes Windows command shims before spawning",
		() => {
			const spawner = makeSpawner((command) =>
				Effect.sync(() => {
	...
	it.effect("fails when output exceeds max buffer in default mode", () =>
		Effect.gen(function* () {
			const spawner = makeSpawner(() =>
				Effect.succeed(makeHandle({ stdout: "x".repeat(2048) })),
			);
	...
	it.effect(
		"fails fast on output limit before timeout for long-running output",
		() =>
			Effect.gen(function* () {
					...
					Effect.succeed(
						makeHandle({
							stdout: Stream.fromIterable(
								Array.from({ length: 10 }, () => textChunk),
			...
			const spawner = makeSpawner(() =>
				Effect.succeed(
					makeHandle({
						...
						exitCode: Deferred.await(stdinWritten).pipe(
	...
	it.effect("returns output for non-zero exit codes", () =>
		Effect.gen(function* () {
			const spawner = makeSpawner(() =>
				Effect.succeed(makeHandle({ stderr: "boom", code: 2 })),
			);
	...
	it.effect("fails on timeout", () =>
		Effect.gen(function* () {
			...
			expect(error.message).toBe(
				"Process 'fake' in '/actual' timed out after 50ms",
			);
		}),
	);

	it.effect(
		"returns a synthetic timed out result when timeoutBehavior is timedOutResult",
		() =>
			Effect.gen(function* () {
				const spawner = makeSpawner(() =>
					Effect.succeed(
