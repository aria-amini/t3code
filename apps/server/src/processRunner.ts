...
export class ProcessReadError extends Schema.TaggedError<ProcessReadError>()(
	"ProcessReadError",
	{
		...ProcessInvocationFields,
...
function hasWindowsCommandNotFoundMessage(output: string): boolean {
	return WINDOWS_COMMAND_NOT_FOUND_PATTERNS.some((pattern) =>
		pattern.test(output),
	);
}

export const isWindowsCommandNotFound = Effect.fn(
