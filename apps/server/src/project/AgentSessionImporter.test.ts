...
const makeProjectedThread = (input: {
	...
	return {
		...
		modelSelection: {
			instanceId: sourceThread.providerInstanceId,
			model: "default",
		},
		...
		getProjectShellById: () =>
			Effect.succeed(
				input.project === undefined
					? Option.none()
					: Option.some(input.project),
			),
		getImportedAgentSessionSources: () => Effect.succeed([]),
		getThreadDetailById: (threadId) =>
			Effect.succeed(input.getThread?.(threadId) ?? Option.none()),
	});
...
it.layer(NodeServices.layer)("AgentSessionImporter", (it) => {
	describe("importRecentAgentThreads", () => {
		it.effect(
			"uses the project root and stores provider-specific resume cursors",
			() =>
				Effect.gen(function* () {
					...
					const engine = OrchestrationEngine.OrchestrationEngineService.of({
						dispatch: (command) =>
							Effect.sync(() => ({ sequence: commands.push(command) })),
					...
					const directory =
						ProviderSessionDirectory.ProviderSessionDirectory.of({
							upsert: (binding) =>
								Effect.sync(() => void bindings.push(binding)),
					...
					).toMatchObject([{ historyImport: true }, { historyImport: true }]);
					expect(
						commands
		...
		it.effect("rejects a changed project root before scanning or writing", () =>
			Effect.gen(function* () {
					...
					Effect.provide(
						Layer.mergeAll(
							...
							makeSnapshotsLayer({
								project: {
									...makeProject(),
									workspaceRoot: "/tmp/project-moved",
								},
							}),
						),
					),
				...
				expect(error).toEqual(
					new AgentSessionImportProjectChangedError({ projectId: PROJECT_ID }),
				);
		...
		it.effect(
			"recovers after a rejected history receipt and a failed binding write",
			() =>
				Effect.gen(function* () {
					...
					const scanner = AgentSessionScanner.AgentSessionScanner.of({
						scan: Effect.die("unused"),
						recentThreads: () =>
							Stream.fromIterable([makeThreadOutcome(makeThread("codex"))]),
					});
					const engine = OrchestrationEngine.OrchestrationEngineService.of({
						dispatch: (command) => {
					...
					const importOnce = () =>
						runImport({ scanner, engine, directory, snapshots });
		...
		it.effect(
			"does not replace completed history or an active binding on retry",
			() =>
				Effect.gen(function* () {
					const scanner = AgentSessionScanner.AgentSessionScanner.of({
						scan: Effect.die("unused"),
					...
					const engine = OrchestrationEngine.OrchestrationEngineService.of({
						dispatch: () =>
							Effect.die("must not replay history or settle active work"),
						readEvents: () => Stream.empty,
						readThreadEvents: () => Stream.empty,
						getThreadReplayStats: () => Effect.die("unused"),
						streamDomainEvents: Stream.empty,
		...
		it.effect(
			"skips malformed Claude ids and wrong-project thread collisions",
			() =>
				Effect.gen(function* () {
					const scanner = AgentSessionScanner.AgentSessionScanner.of({
						scan: Effect.die("unused"),
					...
					const directory =
						ProviderSessionDirectory.ProviderSessionDirectory.of({
							upsert: () =>
								Effect.die("must not bind malformed or wrong-project sessions"),
							getProvider: () => Effect.die("unused"),
							recordImportedTranscript: () => Effect.die("unused"),
							getBinding: () => Effect.succeed(Option.none()),
							listThreadIds: () => Effect.die("unused"),
...
const integrationScanner = AgentSessionScanner.AgentSessionScanner.of({
	scan: Effect.die("unused"),
	recentThreads: () =>
		Stream.fromIterable([makeThreadOutcome(integrationThread)]),
});
...
it.layer(integrationLayer)("AgentSessionImporter integration", (it) => {
	it.effect(
		"imports once after the real engine persists an old rejected receipt",
		() =>
			Effect.gen(function* () {
				...
				expect(result).toEqual({ importedCount: 1, skippedCount: 0 });
				expect(
					Option.getOrThrow(importedThread).messages.map(
						(message) => message.text,
					),
				).toEqual(integrationThread.messages.map((message) => message.text));
				expect(Option.getOrThrow(importedThread).settledOverride).toBe(
					"settled",
				);
				expect(Option.getOrThrow(importedThread).updatedAt).toBe(
				...
				expect(
					Option.getOrThrow(afterRevert).messages.map(
						(message) => message.text,
					),
				).toEqual(integrationThread.messages.map((message) => message.text));
			}),
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
					...
					yield* fileSystem.writeFileString(
						...
						[
							encodeTranscriptRecord({
								type: "session_meta",
								payload: {
									id: transcript.providerSessionId,
									cwd: workspaceRoot,
								},
							}),
							encodeTranscriptRecord({
				...
				expect(
					yield* snapshots.getImportedAgentSessionSources(projectId),
				).toEqual([]);
				...
				const transcriptPaths = new Set(
					transcripts.map((transcript) => transcript.filePath),
				);
								...
								if (transcriptPaths.has(filePath)) {
									...
									if (count > 1) {
										...
										if (completedPaths.has(filePath)) {
											return Effect.die(
												new Error(`Completed transcript reopened: ${filePath}`),
											);
										}
									}
								}
				...
				expect(first.fullReads).toEqual(
					transcripts.slice(0, 100).map((entry) => entry.filePath),
				);
				...
				expect(
					Option.getOrThrow(
						yield* snapshots.getThreadDetailById(failed.threadId),
					).messages,
				).toEqual([]);
				expect(
					Option.getOrThrow(yield* directory.getBinding(failed.threadId)),
				).toMatchObject({
					status: "stopped",
				...
				expect(
					Option.isNone(
						yield* snapshots.getThreadDetailById(remaining.threadId),
					),
				).toBe(true);

				const completedPaths = new Set(
					completedSources.map((entry) => entry.source.filePath),
				);
				...
				expect(
					Option.getOrThrow(
						yield* snapshots.getThreadDetailById(legacy.threadId),
					).messages.map((message) => message.text),
				).toEqual(["Legacy imported history"]);
				expect(
					Option.getOrThrow(yield* directory.getBinding(legacy.threadId))
						.resumeCursor,
				).toEqual({
	...
	for (const source of ["codex", "claudeAgent"] as const) {
		it.effect(
			`resumes imported ${source} history only after the first prompt`,
			() =>
				Effect.gen(function* () {
					const engine = yield* OrchestrationEngine.OrchestrationEngineService;
					const snapshots =
					...
					const sourceThread = {
						...makeThread(source),
						providerSessionId:
							source === "codex" ? "codex-first-resume" : CLAUDE_SESSION_ID,
					};
					const threadId = ThreadId.make(
						`import:${sourceThread.providerInstanceId}:${sourceThread.providerSessionId}`,
					);
					...
					yield* Effect.gen(function* () {
						...
						const importedThread = Option.getOrThrow(
						...
						Effect.provideService(
							...
							AgentSessionScanner.AgentSessionScanner.of({
								scan: Effect.die("unused"),
								recentThreads: () =>
									Stream.succeed(makeThreadOutcome(sourceThread)),
							}),
						),
					);
				}),
	...
	it.effect(
		"persists the resume cursor before publishing a new imported thread",
		() =>
			Effect.gen(function* () {
				const engine = yield* OrchestrationEngine.OrchestrationEngineService;
				const snapshots =
				...
				yield* Effect.raceFirst(
					...
					Fiber.join(importFiber).pipe(
						Effect.flatMap((result) =>
							Effect.die(
								new Error(
									`Import completed before the binding write: ${JSON.stringify(result)}`,
								),
							),
						),
					),
				...
				expect(
					Option.isNone(yield* snapshots.getThreadDetailById(threadId)),
				).toBe(true);
	...
	it.effect(
		"does not import history over a turn started on a partial thread",
		() =>
			Effect.gen(function* () {
				const engine = yield* OrchestrationEngine.OrchestrationEngineService;
				const snapshots =
