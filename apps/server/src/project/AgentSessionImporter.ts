...
function hasImportBlockingActivity(
	...
	return (
		...
		thread.messages.some(
			(message) => !isImportedAgentSessionMessageId(message.id),
		) ||
...
export const importRecentAgentThreads = Effect.fn("importRecentAgentThreads")(
	function* (input: AgentSessionImportInput) {
		...
		const project = yield* snapshots.getProjectShellById(input.projectId).pipe(
			...
			Effect.flatMap(
				Option.match({
					onNone: () =>
						Effect.fail(
							new AgentSessionImportProjectNotFoundError({
					...
					if (outcome._tag === "AlreadyImported") {
						...
						if (recorded._tag === "Failure") {
							...
							yield* Effect.logWarning(
								"Could not record an imported transcript copy",
								{
									threadId,
				...
				const imported = yield* Effect.gen(function* () {
					...
					if (
						...
						(existingBinding.value.provider !== provider ||
							existingBinding.value.providerInstanceId !==
