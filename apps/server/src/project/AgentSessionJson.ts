...
export function createTranscriptJsonSelector(schema: {
		...
		switch (node._tag) {
			case "Objects":
				...
				return node.propertySignatures.some(
					(property) =>
						String(property.name) === path[index] &&
						includes(property.type, path, index + 1),
				);
			case "Arrays": {
				...
				return (
					node.rest.length === 0 ||
					node.rest.some((item) => includes(item, path, index + 1))
				);
			}
			case "Union":
...
export function createTranscriptJsonReader(
	...
	const assemble = (token: Token) => {
		reserve(
			64 +
				("value" in token && typeof token.value === "string"
