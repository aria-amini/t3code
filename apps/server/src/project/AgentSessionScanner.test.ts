...
const makeProjectionSnapshotQueryLayer = (
	importedWorkspaceRoots: ReadonlyArray<string>,
) =>
	Layer.succeed(ProjectionSnapshotQuery.ProjectionSnapshotQuery, {
		...
		getShellSnapshot: () =>
			Effect.succeed({
				snapshotSequence: 0,
				projects: importedWorkspaceRoots.map((workspaceRoot) =>
...
const runRecentThreadOutcomes = (
	input: ScannerTestInput & { readonly workspaceRoot: string },
) =>
	Effect.gen(function* () {
		const scanner = yield* AgentSessionScanner.AgentSessionScanner;
...
const makeTempDir = Effect.fn("AgentSessionScanner.test.makeTempDir")(
...
const writeTranscript = Effect.fn("AgentSessionScanner.test.writeTranscript")(
	function* (input: {
		...
		yield* fileSystem.makeDirectory(path.dirname(input.filePath), {
			recursive: true,
		});
...
const encodeTranscriptRecord = Schema.encodeUnknownSync(
...
it.layer(NodeServices.layer)("AgentSessionScanner", (it) => {
	describe("scan", () => {
		...
		it.effect(
			"merges the same cwd seen by both agents and flags imported projects",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const claudeHomePath = yield* makeTempDir("t3code-claude-home-");
		...
		it.effect(
			"merges case aliases and preserves the persisted project path",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const fileSystem = yield* FileSystem.FileSystem;
					...
					const simulatedFileSystem = FileSystem.FileSystem.of({
						...
						stat: (filePath) =>
							fileSystem.stat(
								filePath === workspaceAlias ? workspace : filePath,
							),
					});
					const result = yield* runScan({
						claudeHomePath,
		...
		it.effect(
			"uses explicit provider instance homes instead of overridden legacy homes",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const claudeHomePath = yield* makeTempDir("t3code-claude-legacy-");
					const codexHomePath = yield* makeTempDir("t3code-codex-legacy-");
					const claudeInstanceHome = yield* makeTempDir(
						"t3code-claude-instance-",
					);
					const codexInstanceHome = yield* makeTempDir(
						"t3code-codex-instance-",
					);
					const legacyWorkspace = yield* makeTempDir(
						"t3code-workspace-legacy-",
					);
					const claudeWorkspace = yield* makeTempDir(
						"t3code-workspace-claude-",
					);
		...
		it.effect("scans each distinct home across multiple instances once", () =>
			Effect.gen(function* () {
				...
				expect(
					result.candidates.map((candidate) => candidate.threadCount),
				).toEqual([1, 1]);
				expect(
					result.candidates.map((candidate) => candidate.path).sort(),
				).toEqual([workspace, otherWorkspace].sort());
			}),
		);

		it.effect(
			"honors provider instance home directory environment variables",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const claudeHomePath = yield* makeTempDir("t3code-claude-legacy-");
					const codexHomePath = yield* makeTempDir("t3code-codex-legacy-");
					const claudeEnvironmentHome =
						yield* makeTempDir("t3code-claude-env-");
		...
		it.effect(
			"ignores invalid provider instances while scanning the remaining providers",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const claudeHomePath = yield* makeTempDir("t3code-claude-home-");
		...
		it.effect(
			"does not scan provider instances disabled by the envelope or config",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const claudeHomePath = yield* makeTempDir("t3code-claude-home-");
					const codexHomePath = yield* makeTempDir("t3code-codex-home-");
					const envelopeDisabledHome = yield* makeTempDir(
						"t3code-codex-disabled-envelope-",
					);
					const configDisabledHome = yield* makeTempDir(
						"t3code-codex-disabled-config-",
					);
					const envelopeWorkspace = yield* makeTempDir(
						"t3code-workspace-disabled-envelope-",
					);
					const configWorkspace = yield* makeTempDir(
						"t3code-workspace-disabled-config-",
					);
		...
		it.effect(
			"ignores relative working directories from malformed transcripts",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const claudeHomePath = yield* makeTempDir("t3code-claude-home-");
					...
					yield* writeTranscript({
						...
						contents: claudeSessionLine(
							path.relative(path.resolve(), workspace),
						),
						mtimeMs: Date.parse("2026-01-01T00:00:00.000Z"),
					});
		...
		it.effect("drops candidates whose directory no longer exists", () =>
			Effect.gen(function* () {
				...
				yield* writeTranscript({
					filePath: path.join(claudeHomePath, "projects", "-slug", "a.jsonl"),
					contents: claudeSessionLine(
						path.join(claudeHomePath, "does-not-exist"),
					),
					mtimeMs: Date.parse("2026-01-01T00:00:00.000Z"),
				});
		...
		it.effect(
			"excludes the home directory, temporary root, and T3 data directory",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const claudeHomePath = yield* makeTempDir("t3code-claude-home-");
		...
		it.effect("excludes Codex scratch directories and Downloads", () =>
			Effect.gen(function* () {
				...
				yield* Effect.addFinalizer(() =>
					Effect.all([
						fileSystem
							.remove(scratchRoot, { recursive: true })
							.pipe(Effect.ignore),
						fileSystem
							.remove(downloads, { recursive: true })
							.pipe(Effect.ignore),
					]),
				);
		...
		it.effect(
			"skips linked git worktrees and reports the origin of real checkouts",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const fileSystem = yield* FileSystem.FileSystem;
					...
					expect(
					...
					).toEqual([
						{
							path: submodule,
							git: {
								remoteKey: "github.com/pingdotgg/vendor",
								repository: "pingdotgg/vendor",
							},
						},
						...
						{ path: plain, git: null },
						{
							path: repo,
							git: {
								remoteKey: "github.com/pingdotgg/t3code",
		...
		it.effect(
			"excludes sandboxes under the configured worktrees dir without .t3 in the path",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const claudeHomePath = yield* makeTempDir("t3code-claude-home-");
		...
		it.effect(
			"finds the cwd on a later line when the first records carry none",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const claudeHomePath = yield* makeTempDir("t3code-claude-home-");
		...
		it.effect(
			"reads a complete transcript record at the exact chunk boundary",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const claudeHomePath = yield* makeTempDir("t3code-claude-home-");
		...
		it.effect(
			"finds session metadata after a first record larger than one chunk",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const claudeHomePath = yield* makeTempDir("t3code-claude-home-");
		...
		it.effect.each([64, 65])(
			"shares metadata bytes across homes for %s one-MiB files",
			(count) =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const fileSystem = yield* FileSystem.FileSystem;
					...
					const firstWorkspace = yield* makeTempDir(
						"t3code-metadata-first-project-",
					);
					const secondWorkspace = yield* makeTempDir(
						"t3code-metadata-second-project-",
					);
					const directories = [
					...
					const templates = directories.map((directory) =>
						path.join(directory, "template.jsonl"),
					);
					for (const [index, workspace] of [
						...
						yield* writeTranscript({
							...
							contents:
								" ".repeat(
									1024 * 1024 - new TextEncoder().encode(record).byteLength,
								) + record,
		...
		it.effect.each([50, 51])(
			"bounds metadata open/read calls for %s short-read files",
			(count) =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const fileSystem = yield* FileSystem.FileSystem;
					const claudeHomePath = yield* makeTempDir(
						"t3code-short-metadata-home-",
					);
					const codexHomePath = yield* makeTempDir(
						"t3code-short-metadata-codex-",
					);
					const workspace = yield* makeTempDir(
						"t3code-short-metadata-project-",
					);
					const directory = path.join(claudeHomePath, "projects", "p");
					const template = path.join(directory, "template.jsonl");
					const record = encodeTranscriptRecord({ cwd: workspace });
					const contents =
						" ".repeat(399 - new TextEncoder().encode(record).byteLength) +
						record;
		...
		it.effect(
			"bounds malformed metadata records without excluding another account",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const fileSystem = yield* FileSystem.FileSystem;
					const claudeHomePath = yield* makeTempDir(
						"t3code-record-metadata-home-",
					);
					const secondHome = yield* makeTempDir(
						"t3code-record-metadata-second-",
					);
					const codexHomePath = yield* makeTempDir(
						"t3code-record-metadata-codex-",
					);
					const workspace = yield* makeTempDir(
						"t3code-record-metadata-project-",
					);
					const directory = path.join(claudeHomePath, "projects", "p");
					const template = path.join(directory, "template.jsonl");
					yield* writeTranscript({
		...
		it.effect(
			"returns an empty result when neither home directory exists",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
	...
	describe("recentThreads", () => {
		it.effect.each([false, true])(
			...
			(overflow) =>
				Effect.gen(function* () {
					...
					const claudeHomePath = yield* makeTempDir(
						"t3code-record-limit-claude-",
					);
					const codexHomePath = yield* makeTempDir(
						"t3code-record-limit-codex-",
					);
		...
		it.effect(
			"imports recent Claude and Codex sessions for the selected project only",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
		...
		it.effect("imports history recorded with a case alias", () =>
			Effect.gen(function* () {
				...
				const simulatedFileSystem = FileSystem.FileSystem.of({
					...fileSystem,
					stat: (filePath) =>
						fileSystem.stat(filePath === workspaceAlias ? workspace : filePath),
				});
				const threads = yield* runRecentThreads({
					claudeHomePath,
					codexHomePath,
		...
		it.effect(
			"keeps the provider instance that owns a custom session home",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
		...
		it.effect(
			"suppresses duplicate session copies without reporting a skipped import",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
					...
					const contents = [
						...
						encodeTranscriptRecord({
							...
							payload: {
								type: "user_message",
								message: "Import this session once",
							},
						}),
					].join("\n");
		...
		it.effect(
			"streams large transcripts across providers without hiding projects",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const fileSystem = yield* FileSystem.FileSystem;
					...
					const outcomes = yield* Effect.gen(function* () {
						...
						Effect.provide(
							makeScannerTestLayer({ claudeHomePath, codexHomePath }),
						),
		...
		it.effect(
			"skips excessive records without blocking an older valid transcript",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
					yield* TestClock.setTime(nowMs);
					const claudeHomePath = yield* makeTempDir(
						"t3code-record-budget-claude-",
					);
					const codexHomePath = yield* makeTempDir(
						"t3code-record-budget-codex-",
					);
					const workspace = yield* makeTempDir(
						"t3code-record-budget-workspace-",
					);
					...
					expect(outcomes[1]).toMatchObject({
						thread: { providerSessionId: "older" },
					});
				}),
		);
								...
								[
									...(source === "codex"
										? [
												{
													type: "session_meta",
													payload: { id: "replacement-session", cwd },
												},
												{
													type: "event_msg",
		...
		it.effect(
			"checks file identity and provider before skipping completed history",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const fileSystem = yield* FileSystem.FileSystem;
					...
					yield* Effect.gen(function* () {
						...
						const wrongProvider = yield* scanner
							.recentThreads(workspace, [
								{ ...imported.source, provider: "claudeAgent" },
							])
							.pipe(Stream.runCollect);
						...
						expect(replaced[0]).toMatchObject({
							...
							source: {
								...
								mtimeMs: imported.source.mtimeMs,
							},
						});
					}).pipe(
						Effect.provide(
		...
		it.effect(
			"imports visible history from a transcript with an oversized tool record",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
					...
					const transcript = `${[
						...
						encodeTranscriptRecord({
							...
							payload: {
								type: "user_message",
								message: "Import this large session",
							},
						}),
					].join("\n")}\n${encodeTranscriptRecord({
						type: "tool_result",
		...
		it.effect(
			"does not reopen a transcript that becomes a non-file after discovery",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const fileSystem = yield* FileSystem.FileSystem;
					...
					yield* writeTranscript({
						...
						contents: [
							...
							encodeTranscriptRecord({
								...
								payload: {
									type: "user_message",
									message: "Do not import this session",
								},
							}),
						].join("\n"),
						mtimeMs: nowMs,
					...
					const simulatedFileSystem = FileSystem.FileSystem.of({
						...
						stat: (filePath) => {
							...
							return fileSystem.stat(
								transcriptStatCount === 1 ? transcriptPath : nonFilePath,
							);
						},
						open: (filePath, options) => {
							if (filePath === transcriptPath) transcriptOpenCount += 1;
		...
		it.effect(
			"skips growth during reading without exceeding the reserved bytes",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const fileSystem = yield* FileSystem.FileSystem;
					...
					const contents = [
						...
						encodeTranscriptRecord({
							...
							payload: {
								type: "user_message",
								message: "Do not import a changing file",
							},
						}),
					].join("\n");
					yield* writeTranscript({
		...
		it.effect(
			"does not read the second transcript when the consumer takes one thread",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const fileSystem = yield* FileSystem.FileSystem;
		...
		it.effect(
			"uses one deterministic provider instance for a shared session home",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
					...
					yield* writeTranscript({
						...
						contents: [
							...
							encodeTranscriptRecord({
								...
								payload: {
									type: "user_message",
									message: "Use the shared session",
								},
							}),
						].join("\n"),
						mtimeMs: nowMs,
					...
					expect(threads.map((thread) => thread.providerInstanceId)).toEqual([
						"codex",
					]);
				}),
		);
		...
		it.effect(
			"uses configured order when custom instances share a session home",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
		...
		it.effect(
			"keeps a second account when the first has 5000 newer files",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const fileSystem = yield* FileSystem.FileSystem;
					...
					const recentWorkspace = yield* makeTempDir(
						"t3code-workspace-recent-",
					);
					...
					const resolveTranscript = (filePath: string) =>
						...
						path.basename(filePath).startsWith("old-")
							? oldTranscript
							: filePath;
					const simulatedFileSystem = FileSystem.FileSystem.of({
						...fileSystem,
						...
						open: (filePath, options) =>
							fileSystem.open(resolveTranscript(filePath), options),
					});
		...
		const thread = AgentSessionScanner.parseAgentSessionTranscript({
			contents: [
				...
				JSON.stringify({
					...
					message: {
						role: "user",
						content: [{ type: "text", text: "Fix authentication" }],
					},
				}),
				JSON.stringify({
					type: "user",
					...
					message: {
						role: "user",
						content: [{ type: "tool_result", text: "hidden" }],
					},
				}),
				JSON.stringify({
					type: "assistant",
		...
		const laterAssistantMessages = Array.from({ length: 200 }, (_, index) =>
			encodeTranscriptRecord({
				...
				payload: {
					...
					role: "assistant",
					content: [
						{ type: "output_text", text: `Assistant message ${index}` },
					],
				},
			}),
		);
			...
			contents: [
				...
				encodeTranscriptRecord({
					...
					payload: {
						...
						role: "user",
						content: [
							{ type: "input_text", text: "Keep the canonical prompt" },
						],
					},
				}),
				encodeTranscriptRecord({
	...
	it("restores the canonical first prompt when a later user message remains", () => {
			...
			contents: [
				...
				encodeTranscriptRecord({
					...
					payload: {
						...
						role: "assistant",
						content: [
							{ type: "output_text", text: "Keep this latest response" },
						],
					},
				}),
			].join("\n"),
		...
		expect(
			thread?.messages.filter(
				(message) => message.text.trim() === canonicalPrompt.trim(),
			),
		).toHaveLength(1);
		expect(
			thread?.messages.some(
				(message) => message.text === "Keep this later prompt",
			),
		).toBe(true);
				...
				encodeTranscriptRecord({
					...
					payload: {
						...
						content: [
							{
								type: "input_text",
								text: "Keep this prompt with a blank turn ID",
							},
						],
					},
				}),
	...
	it("skips Codex transcripts without a resumable session ID", () => {
		const thread = AgentSessionScanner.parseAgentSessionTranscript({
			contents: encodeTranscriptRecord({
				...
				payload: {
					type: "user_message",
					message: "This transcript has no session metadata",
				},
			}),
			source: "codex",
			providerInstanceId: ProviderInstanceId.make("codex"),
			...
			contents: [
				...
				encodeTranscriptRecord({
					...
					payload: {
						...
						role: "user",
						content: [
							{ type: "input_text", text: "Initialize Git and add a README." },
						],
					},
				}),
			].join("\n"),
