...
const TranscriptRecord = Schema.Struct({
	...
	payload: Schema.optional(
		Schema.Struct({
			...
			internal_chat_message_metadata_passthrough: Schema.optional(
				Schema.Unknown,
			),
		}),
	),
...
const decodeTranscriptRecord = Schema.decodeUnknownOption(
	Schema.fromJsonString(TranscriptRecord),
);
...
function selectMetadataTranscripts(
...
function extractText(
	content:
		| string
		| ReadonlyArray<typeof TranscriptContentBlock.Type>
		| undefined,
): string {
...
export function parseAgentSessionTranscript(
	...
	const records = lines.flatMap((line) =>
		Option.toArray(decodeTranscriptRecord(line)),
	);
...
function parseAgentSessionRecords(
	...
	const fallbackTimestamp = DateTime.formatIso(
		DateTime.makeUnsafe(input.lastActiveAtMs),
	);
	...
	let providerSessionId =
		input.source === "codex" ? "" : input.fallbackSessionId;
	...
	const finishCodexTurn = () => {
		const canonicalTurnIds = new Set(
			responseUsersInTurn.flatMap((responseUser) =>
				canonicalUserTextsInTurn.has(responseUser.text)
					? [responseUser.turnId]
					: [],
			),
		);
	...
	const retainMessage = (
		message: AgentSessionThreadMessage & {
	...
	for (const record of records) {
		...
		if (record.type === "session_meta") {
			const sessionId =
				record.payload?.id?.trim() || record.payload?.session_id?.trim();
		...
		if (
			...
			for (let index = messages.length - 1; index >= 0; index--) {
				...
				if (
					message?.codexResponseUser === true &&
		...
		if (
			record.payload.role === "user" &&
			hasMatchingCodexEventInTurn(extractedText)
		) {
			continue;
		}
		retainMessage({
	...
	if (providerSessionId.trim().length === 0 || firstUserMessage === undefined)
		return null;
	...
	const derivedTitle = visibleFirstUserMessage.text
		.trim()
		.split("\n")[0]
		?.slice(0, 100)
		.trim();
...
export const make = Effect.gen(function* () {
	...
	const isExcludedProjectPath = (candidatePath: string) =>
		excludedProjectRoots.has(
			normalizeProjectPathForComparison(candidatePath),
		) ||
	...
	const directoryIdentity = Effect.fn("AgentSessionScanner.directoryIdentity")(
		function* (target: string, knownStats?: FileSystem.File.Info) {
			const resolved = path.resolve(target);
			const stats =
				knownStats === undefined
					? yield* statOption(resolved)
					: Option.some(knownStats);
			if (
				Option.isSome(stats) &&
	 ...
	 */
	const readGitIdentity = Effect.fn("AgentSessionScanner.readGitIdentity")(
		function* (directory: string): Effect.fn.Return<
			| {
		...
		> {
			...
			return {
				...
				git: {
					remoteKey:
						originUrl === null ? null : normalizeGitRemoteUrl(originUrl),
					repository:
						parseGitHubRepositoryNameWithOwnerFromRemoteUrl(originUrl),
				},
			} as const;
		},
	);
	 ...
	 */
	const readTranscript = Effect.fn("AgentSessionScanner.readTranscript")(
		function* (
			filePath: string,
				...
				fileSystem.open(filePath, { flag: "r" }).pipe(
					Effect.flatMap((file) =>
						Effect.gen(function* () {
							...
							let reader = createTranscriptJsonReader(
								reserve,
							...
							const finishRecord = () => {
								...
								if (
									...
									shouldRetainDecodedRecord(source, decoded.value)
								) {
									records.push(decoded.value);
									historyBytes += recordBytes;
								}
								recordBytes = 0;
								reader = createTranscriptJsonReader(
									reserve,
									selectTranscriptPath,
								);
							...
							while (bytesRead < expected.size) {
								...
								const withinBudget = yield* Effect.try(() => {
									...
									while (start < next.value.byteLength) {
										...
										const end =
											newline === -1 ? next.value.byteLength : newline;
										recordStarted = true;
										reader.write(
											decoder.decode(next.value.subarray(start, end), {
												stream: true,
											}),
										);
										if (newline === -1) break;
	...
	const discoverClaudeTranscripts = Effect.fn(
		"AgentSessionScanner.discoverClaudeTranscripts",
	)(function* (
		homePath: string,
	...
	const discoverCodexTranscripts = Effect.fn(
		"AgentSessionScanner.discoverCodexTranscripts",
	)(function* (
		homePath: string,
		providerInstanceId: ProviderInstanceId,
		operationBudget: number,
		...
		for (const year of (yield* readDirectory(sessionsDir))
			.toSorted()
			.toReversed()) {
			if (operationsRemaining <= 0) {
				truncated = true;
			...
			for (const month of (yield* readDirectory(path.join(sessionsDir, year)))
				...
				for (const day of (yield* readDirectory(
	...
	const groupTranscriptsByCwd = Effect.fn(
		"AgentSessionScanner.groupTranscriptsByCwd",
	)(function* (
		source: AgentSessionSource,
		transcripts: ReadonlyArray<TranscriptCandidate>,
		budget: MetadataReadBudget,
		...
		for (const transcript of transcripts) {
			...
			if (existing) {
				existing.lastActiveAtMs = Math.max(
	...
	const collectCandidates = Effect.fn("AgentSessionScanner.collectCandidates")(
		function* () {
			const settings = yield* serverSettings.getSettings.pipe(
				Effect.mapError(
					(cause) =>
						new AgentSessionScanError({ operation: "read-settings", cause }),
				),
			);
			...
			for (const source of ["claudeAgent", "codex"] as const) {
				...
				for (const { instanceId, config: instance } of instances) {
					const homeVariable =
						source === "claudeAgent" ? "CLAUDE_CONFIG_DIR" : "CODEX_HOME";
				...
				for (const [index, home] of homes.entries()) {
					const operationBudget =
						baseOperationBudget + (index < extraOperationBudgets ? 1 : 0);
				...
				const selectedTranscripts =
					selectMetadataTranscripts(transcriptCandidates);
	...
	const scan: AgentSessionScanner["Service"]["scan"] = Effect.gen(function* () {
			...
			if (key === undefined) {
				...
				if (isExcludedProjectPath(realPath)) {
					...
					if (gitIdentity._tag === "Worktree") {
						...
						gitIdentities.set(
							key,
							gitIdentity._tag === "Repository" ? gitIdentity.git : null,
						);
					}
				}
		...
		const importedProjectsByRoot = new Map<
			string,
			(typeof shellSnapshot.projects)[number]
		>();
		for (const project of shellSnapshot.projects) {
			const projectRoot = path.resolve(expandHomePath(project.workspaceRoot));
			importedProjectsByRoot.set(
				normalizeProjectPathForComparison(projectRoot),
				project,
			);
			importedProjectsByRoot.set(
				yield* directoryIdentity(projectRoot),
				project,
			);
		}
		...
		for (const [key, entry] of merged.entries()) {
			...
			const importedProject =
				importedProjectsByRoot.get(
					normalizeProjectPathForComparison(entry.path),
	...
	const prepareRecentThreads = Effect.fn(
		"AgentSessionScanner.prepareRecentThreads",
	)(function* (
		workspaceRoot: string,
		completedSources: ReadonlyArray<AgentSessionImportSource>,
	) {
		...
		const eligibleTranscripts: Array<{
			...
			readonly transcript: RawCandidate["transcripts"][number] & {
				readonly mtimeMs: number;
			};
		}> = [];
			...
			Stream.mapEffect(({ candidate, transcript }) =>
				Effect.gen(function* () {
					...
					if (
						...
						(yield* directoryIdentity(path.resolve(expandedCwd))) !==
							rootIdentity
					) {
						return Option.some<AgentSessionRecentThread>({ _tag: "Skipped" });
					}
