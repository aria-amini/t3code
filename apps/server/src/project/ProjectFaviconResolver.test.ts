...
it.layer(TestLayer)("ProjectFaviconResolverLive", (it) => {
	describe("resolvePath", () => {
		it.effect(
			"serves repeated resolves from cache instead of re-walking candidates",
			() =>
				Effect.gen(function* () {
					...
					expect(resolved?.endsWith(path.join("public", "favicon.svg"))).toBe(
						true,
					);
		...
		it.effect("falls back at once when a cached favicon is deleted", () =>
			Effect.gen(function* () {
				...
				yield* fileSystem
					.remove(path.join(cwd, "favicon.svg"))
					.pipe(Effect.orDie);
		...
		it.effect(
			...
			() =>
				Effect.gen(function* () {
					...
					const resolved = yield* resolver.resolvePath(
						cwd,
						"brand/missing.svg",
					);
		...
		it.effect(
			"falls back to well-known files when the t3.json iconPath does not exist",
			() =>
				Effect.gen(function* () {
					const resolver = yield* ProjectFaviconResolver.ProjectFaviconResolver;
					const cwd = yield* makeTempDir;
		...
		it.effect(
			"does not resolve a t3.json iconPath outside the workspace root",
			() =>
				Effect.gen(function* () {
					const resolver = yield* ProjectFaviconResolver.ProjectFaviconResolver;
		...
		it.effect("resolves icon hrefs from project source files", () =>
			Effect.gen(function* () {
				...
				expect(resolved).toBe(
					(yield* Path.Path).join(cwd, "public", "brand", "logo.svg"),
				);
			}),
		);
  ...
  head: () => ({
    links: [
      ...
      { rel: "icon", href: "/brand/logo.svg" },
		...
		it.effect(
			"resolves object-literal icon metadata when href precedes rel",
			() =>
				Effect.gen(function* () {
					const resolver = yield* ProjectFaviconResolver.ProjectFaviconResolver;
					const cwd = yield* makeTempDir;
		...
		it.effect(
			"resolves object-literal icon metadata alongside nested objects",
			() =>
				Effect.gen(function* () {
					const resolver = yield* ProjectFaviconResolver.ProjectFaviconResolver;
					const cwd = yield* makeTempDir;
		...
		it.effect(
			"continues to later sources after an outside-root icon href",
			() =>
				Effect.gen(function* () {
					const resolver = yield* ProjectFaviconResolver.ProjectFaviconResolver;
					const cwd = yield* makeTempDir;
