...
export const make = Effect.gen(function* () {
	...
	const findExistingFile = Effect.fn("ProjectFaviconResolver.findExistingFile")(
		function* (
			projectCwd: string,
			...
			for (const relativePath of relativeCandidates) {
				...
				const stats = yield* optionOnNotFound(
					fileSystem.stat(candidate.value.absolutePath),
				).pipe(
					Effect.mapError(
						(cause) =>
	...
	const resolvePathUncached = Effect.fn(
		"ProjectFaviconResolver.resolvePathUncached",
	)(function* (
		cwd: string,
		faviconPath?: string,
	): Effect.fn.Return<string | null, ProjectFaviconResolutionError> {
		...
		if (
			Option.isSome(projectFile) &&
			projectFile.value.iconPath !== undefined
		) {
			const existing = yield* findExistingFile(
				projectCwd,
	...
	const resolvePath: ProjectFaviconResolver["Service"]["resolvePath"] =
		Effect.fn("ProjectFaviconResolver.resolvePath")(
