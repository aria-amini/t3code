...
it.layer(NodeServices.layer)("RepositoryIdentityResolverLive", (it) => {
	it.effect("refreshes the Git root only when requested", () => {
		...
		return Effect.gen(function* () {
			...
			const refreshed = yield* resolver.resolve("/repo/packages/web", {
				refresh: true,
			});
	...
	it.effect(
		"normalizes equivalent GitHub remotes into a stable repository identity",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"returns the git top-level root path when resolving from a nested workspace",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
			...
			Effect.gen(function* () {
				...
				if (change === "replace") {
					yield* git(cwd, [
						...
						"add",
						"upstream",
						"git@github.com:T3Tools/previous.git",
					]);
				}
	...
	it.effect(
		"uses the last remote path segment as the repository name for nested groups",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const cwd = yield* fileSystem.makeTempDirectoryScoped({
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				expect(refreshedIdentity?.canonicalKey).toBe(
	...
	it.effect(
		"refreshes cached identities after the positive TTL when a remote changes",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const cwd = yield* fileSystem.makeTempDirectoryScoped({
				...
				yield* git(cwd, [
					...
					"set-url",
					"origin",
					"git@github.com:T3Tools/t3code-next.git",
				]);
