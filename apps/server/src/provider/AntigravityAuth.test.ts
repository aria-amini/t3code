					...
					start: () =>
						Effect.gen(function* () {
							...
							if (options.interactive !== false && input.onAuthorizationUrl) {
								for (const url of options.authorizationUrls ?? [
									authorizationUrl,
								]) {
...
it.layer(NodeServices.layer)("AntigravityAuth", (it) => {
	...
	it.effect(
		"accepts a delayed duplicate after callback completion starts",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness();
	...
	it.effect("rejects a different second authorization URL", () =>
		Effect.gen(function* () {
			const harness = yield* makeHarness({
				authorizationUrls: [
					authorizationUrl,
					`${authorizationUrl}&scope=another-request`,
				],
			});
			yield* harness.auth.controller.start(owner);
	...
	it.effect(
		"keeps a remote flow private and waits for native auth and catalog discovery",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness();
				const state = yield* harness.auth.controller.start(owner);
	...
	it.effect(
		"does not call callback HTTP success a successful Google sign-in",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness();
				const state = yield* harness.auth.controller.start(owner);
	...
	it.effect(
		"accepts direct local or cached completion without a callback RPC",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"rejects mismatched callbacks without sending any HTTP request",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness();
				const state = yield* harness.auth.controller.start(owner);
				...
				assert.equal(harness.forwarded(), 0);
				assert.equal(
					(yield* phase(harness.auth, "waiting")).authorizationUrl,
					authorizationUrl,
				);
	...
	it.effect(
		"fails the flow when delivery fails after the requesting client disconnects",
		() =>
			Effect.gen(function* () {
	...
	it.effect("cancel closes the owned process without forwarding a denial", () =>
		Effect.gen(function* () {
			...
			const cancelled = yield* harness.auth.controller.cancel(
				owner,
				state.flowId!,
			);
	...
	it.effect(
		"expires the flow at the official deadline and removes its URL",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness();
				const state = yield* harness.auth.controller.start(owner);
	...
	it.effect(
		"survives subscriber disconnect and does not replace a competing client's flow",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness();
				...
				const competing = yield* harness.auth.controller
					.start(otherOwner)
					.pipe(Effect.exit);
	...
	it.effect(
		"sign-out closes admission and every process before fresh native logout",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness();
	...
	it.effect(
		"closes a stalled sign-out process without clearing its account catalog",
		() =>
			Effect.gen(function* () {
				const entered = yield* Deferred.make<void>();
				const harness = yield* makeHarness({
	...
	it.effect(
		"failed session stopping still closes owned processes and skips native logout",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness();
				const processScope = yield* Scope.make();
	...
	it.effect(
		"does not call logout unless the official process advertises it",
		() =>
			Effect.gen(function* () {
