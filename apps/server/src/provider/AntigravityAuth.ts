...
function visibleSnapshot(
	...
	const busy = ["starting", "waiting", "verifying"].includes(
...
function safeAuthFailure(
	...
	if (Option.isSome(error)) {
		...
		if (isAcpRequestError(error.value)) {
			...
			if (
				/access_denied|denied access|cancelled/i.test(error.value.errorMessage)
			) {
...
>(
	...
	const publishFlow = (flow: AuthFlow, state: ProviderAuthState) => {
		flow.state = state;
		return SubscriptionRef.set(snapshot, {
			ownerSessionId: flow.ownerSessionId,
			state,
		});
	};
			...
			Effect.flatMap((authorization) =>
				lock.withPermits(1)(
					Effect.gen(function* () {
						...
						if (flow.pending) {
							if (
								flow.pending.authorizationUrl === authorization.authorizationUrl
							)
								return;
							return yield* new AcpErrors.AcpTransportError({
								detail:
									"Antigravity started more than one Google sign-in request.",
								cause: undefined,
							});
						}
	...
	const runSignIn = (
		...
		Effect.gen(function* () {
			...
			Effect.timeoutOrElse({
				...
				orElse: () =>
					Effect.fail(
						setupError("start", "Google sign-in expired. Start sign-in again."),
					),
			}),
			Effect.exit,
	...
	const requireFlow = (ownerSessionId: string, flowId: string, name: string) =>
		Effect.gen(function* () {
			...
			if (
				!flow ||
				flow.id !== flowId ||
				flow.ownerSessionId !== ownerSessionId
			) {
				return yield* setupError(
					name,
	...
	const controller: ProviderAuthController = {
		start: (ownerSessionId, stopSessions = Effect.void) =>
			lock.withPermits(1)(
				Effect.uninterruptible(
					Effect.gen(function* () {
						if (
							activeFlow?.ownerSessionId === ownerSessionId &&
							operation === "auth"
						) {
						...
						if (operation !== "idle") {
							return yield* setupError(
								"start",
								"Antigravity setup is already in progress.",
							);
						}
						...
						const expiresAtMillis =
							(yield* Clock.currentTimeMillis) + AUTH_TIMEOUT_MS;
						const state: ProviderAuthState = {
							...emptyState,
							phase: "starting",
							flowId,
							expiresAt: DateTime.formatIso(
								DateTime.makeUnsafe(expiresAtMillis),
							),
							message: usesBrowser
		...
		complete: Effect.fn("AntigravityAuth.complete")(
			function* (ownerSessionId, input) {
				const pending = yield* lock.withPermits(1)(
					Effect.gen(function* () {
						const flow = yield* requireFlow(
							ownerSessionId,
							input.flowId,
							"complete",
						);
		...
		cancel: Effect.fn("AntigravityAuth.cancel")(
			function* (ownerSessionId, flowId) {
				const flow = yield* lock.withPermits(1)(
					requireFlow(ownerSessionId, flowId, "cancel"),
				);
		...
		logout: Effect.fn("AntigravityAuth.logout")(function* (stopSessions) {
					...
					const flow = yield* lock.withPermits(1)(
						Effect.gen(function* () {
							if (operation !== "idle" && operation !== "auth") {
								return yield* setupError(
									"logout",
									"Antigravity setup is already stopping.",
								);
							}
							operation = "logout";
					...
					const result = yield* restore(
						Effect.gen(function* () {
							...
							Effect.timeoutOrElse({
								...
								orElse: () =>
									Effect.fail(
										setupError("logout", "Antigravity sign-out timed out."),
									),
							}),
						),
					).pipe(Effect.exit);
			...
			return yield* Fiber.await(worker).pipe(
				Effect.flatMap((result) => result),
			);
		}),
