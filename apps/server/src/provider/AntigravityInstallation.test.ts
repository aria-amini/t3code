...
const executableName =
	hostPlatform === "win32" ? "agy_acp_server.exe" : "agy_acp_server.par";
const harnessName =
	hostPlatform === "win32"
...
const writeRelease = Effect.fn("test.writeAntigravityRelease")(function* (
	...
	yield* fs.writeFileString(
		path.join(directory, asset.executable.name),
		serverContents,
		{
			mode: 0o755,
		},
	...
	yield* fs.writeFileString(
		path.join(directory, asset.harness.name),
		harnessContents,
		{
			mode: 0o755,
		},
...
const makeHarness = Effect.fn("test.makeAntigravityInstallation")(function* (
	...
	const baseDir =
		options.baseDir ??
		(yield* fs.makeTempDirectoryScoped({ prefix: "t3-agy-test-" }));
		...
		makeTempDirectoryScoped: (settings) =>
			settings?.prefix === ".install-"
				? Effect.acquireRelease(
						installationFs.makeTempDirectory(settings),
						(directory) =>
							fs
								.remove(directory, { recursive: true, force: true })
								.pipe(
									Effect.orDie,
									Effect.andThen(Deferred.succeed(stagingReleased, undefined)),
								),
					)
			...
			: {
					validate: (executable: AntigravityExecutable, version: string) =>
						Effect.sync(() => validations.push({ executable, version })).pipe(
							Effect.andThen(
								options.validate?.(executable, version) ?? Effect.void,
							),
						),
				}),
	}).pipe(
...
const expectPreviousRelease = Effect.fn(
	"test.expectPreviousAntigravityRelease",
)(function* (installation: AntigravityInstallation["Service"]) {
	const fs = yield* FileSystem.FileSystem;
	const path = yield* Path.Path;
	...
	expect(yield* fs.readFileString(resolved.executablePath)).toBe(
		serverContents,
	);
...
it.layer(NodeServices.layer)("Antigravity installation", (it) => {
	it.effect("verifies both files before activating a streamed download", () =>
		Effect.gen(function* () {
			...
			const { installation, fs, path, validations, requests, stagingReleased } =
				yield* makeHarness({
					previous: true,
					validate: () =>
			...
			expect(
				yield* fs.readFileString(validation.executable.executablePath),
			).toBe(serverContents);
			expect(yield* fs.readFileString(validation.executable.harnessPath)).toBe(
				harnessContents,
			);
			...
			expect(path.dirname(selected.harnessPath)).toBe(
				path.dirname(selected.executablePath),
			);
			expect(yield* fs.readFileString(selected.executablePath)).toBe(
				serverContents,
			);
			expect(yield* fs.readFileString(selected.harnessPath)).toBe(
				harnessContents,
			);
			expect(
				yield* fs.readDirectory(
					path.join(installation.managedDirectory, "versions"),
				),
			).toEqual(
	...
	it.effect.each([
		...
		"validates $name with initialize only and removes the disposable profile",
		(testCase) =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
					...
					Effect.fn("test.spawnAntigravityValidator")(function* (command) {
						...
						if (!profile)
							return yield* Effect.die(
								"Expected a disposable validation profile.",
							);
						profiles.add(profile);
						const helper = command.args[0] === "-e";
						const output = yield* Queue.unbounded<Uint8Array>();
						const exited = yield* Deferred.make<ChildProcessSpawner.ExitCode>();
						const terminate = Deferred.succeed(
							exited,
												...
												...(request.method === "initialize"
													...
													: {
															error: {
																code: -32601,
																message:
																	"Validation must not sign in or create sessions.",
															},
														}),
											})}\n`,
										),
	...
	it.effect(
		"accepts an encoded Content-Length when the body is compressed",
		() =>
			Effect.gen(function* () {
	...
	it.effect.each([
		...
		},
		{
			...
			contentLength: completeArchive.byteLength + 1,
		},
		{
			name: "missing harness",
			archive: Buffer.from(zipFixtures.missingHarness, "base64"),
		},
		{
			name: "duplicate executable",
			archive: Buffer.from(zipFixtures.duplicate, "base64"),
		},
		{
			name: "path traversal",
			archive: Buffer.from(zipFixtures.traversal, "base64"),
		},
		{
			name: "symbolic link",
			archive: Buffer.from(zipFixtures.symlink, "base64"),
		},
		{
		...
		Effect.gen(function* () {
			const { installation, validations, stagingReleased, fs, path } =
				yield* makeHarness({
					...options,
					previous: true,
				});
				...
				const fileSystem = FileSystem.FileSystem.of({
					...
					sink: (target, options) =>
						...
						(stage === "extract" && target.endsWith(executableName))
							? fs
									.sink(target, options)
									.pipe(Sink.mapInputEffect(() => Effect.fail(noSpace)))
	...
	it.effect.each(["downloading", "extracting", "verifying"] as const)(
		...
		(phase) =>
			Effect.gen(function* () {
				...
				const { installation, stagingReleased, path, validations } =
					yield* makeHarness({
						previous: true,
						body:
							phase === "downloading"
								? Stream.concat(
										...
										Stream.fromEffect(
											barrier.pipe(Effect.as(completeArchive.subarray(31))),
										),
									)
								: undefined,
				...
				expect(
					(yield* installation.cancel(
						started.operationId ?? "missing-operation-id",
					)).phase,
				).toBe("cancelled");
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				const started = yield* installation.start.pipe(
	...
	it.effect.skipIf(HostProcessPlatform.defaultValue() === "win32")(
		...
		() =>
			Effect.gen(function* () {
				...
				yield* fs.writeFileString(externalExecutable, "external server", {
					mode: 0o755,
				});
				yield* fs.writeFileString(externalHarness, "external harness", {
					mode: 0o755,
				});
				...
				yield* expectPreviousRelease(installation);
				expect(
					yield* installation.resolve(undefined, { PATH: externalDirectory }),
				).toMatchObject({
					source: "managed",
					version: previousVersion,
				});
				...
				expect(
					yield* installation.resolve(externalExecutable).pipe(Effect.flip),
				).toMatchObject({
					operation: "resolve",
				});
				expect(
					yield* installation
						.resolve(path.join(baseDir, "missing"))
						.pipe(Effect.flip),
				).toMatchObject({
					operation: "resolve",
				});
				...
				expect(
					yield* isolated.installation.resolve().pipe(Effect.flip),
				).toMatchObject({
					operation: "resolve",
				});
				expect(
					yield* isolated.installation.resolve(undefined, {
						PATH: externalDirectory,
					}),
				).toMatchObject({
					source: "path",
				...
				expect(
					yield* isolated.installation.resolve(executableName, {
						PATH: externalDirectory,
					}),
				).toMatchObject({
					source: "override",
	...
	it.effect(
		"keeps leased releases available while new sessions resolve the new release",
		() =>
			Effect.gen(function* () {
				const { installation, fs, stagingReleased } = yield* makeHarness({
					previous: true,
				});
				const processScope = yield* Scope.make();
				const oldExecutable = yield* installation
					.acquire()
				...
				expect(yield* fs.readFileString(oldExecutable.executablePath)).toBe(
					serverContents,
				);
				expect(yield* installation.remove().pipe(Effect.flip)).toMatchObject({
					operation: "remove",
				});
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				const { installation, baseDir, fs, path } = yield* makeHarness({
					previous: true,
				});
				...
				yield* fs.makeDirectory(externalDirectory);
				yield* fs.writeFileString(
					path.join(externalDirectory, executableName),
					"external server",
					{
						mode: 0o755,
					},
				...
				yield* fs.writeFileString(
					path.join(externalDirectory, harnessName),
					"external harness",
					{
						mode: 0o755,
					},
				...
				expect(
					yield* restarted.installation.resolve().pipe(Effect.flip),
				).toMatchObject({
					operation: "resolve",
				});
				...
				expect((yield* restarted.installation.resolve()).source).toBe(
					"managed",
				);
			}),
	);
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				const { installation, fs, path, baseDir } = yield* makeHarness({
					previous: true,
				});
				...
				expect(yield* fs.readFileString(externalExecutable)).toBe(
					"external server",
				);
				expect(yield* fs.readFileString(externalHarness)).toBe(
					"external harness",
				);
	...
	it.effect(
		"reports unsupported hosts without downloading or changing state",
		() =>
			Effect.gen(function* () {
