...
const encodeActiveRelease = Schema.encodeEffect(
	Schema.fromJsonString(ActiveRelease),
);
const encodeInstalledRelease = Schema.encodeEffect(
	Schema.fromJsonString(InstalledRelease),
);
...
const wrapFailure = (operation: string, detail: string) => (cause: unknown) =>
	isInstallationError(cause)
		? cause
		: installationError(operation, detail, cause);
...
const openArchive = Effect.fn("AntigravityInstallation.openArchive")(function* (
					...
					if (error || !zip) {
						resume(
							Effect.fail(
								installationError(
									"extract",
									"Could not open the verified archive.",
									error,
								),
							),
						);
	...
	>((resume) => {
		...
		const onError = (cause: unknown) => {
			...
			resume(
				Effect.fail(
					installationError("extract", "The archive could not be read.", cause),
				),
			);
		};
	...
	const streamEntry = (entry: Yauzl.Entry) =>
		Effect.acquireRelease(
			Effect.callback<NodeStream.Readable, AntigravityInstallationError>(
				(resume) => {
						...
						resume(
							cause || !readable
								? Effect.fail(
										installationError(
											"extract",
											"Could not read an archive member.",
											cause,
										),
									)
...
export const makeAntigravityInstallation = Effect.fn(
	"AntigravityInstallation.make",
)(function* (options: AntigravityInstallationOptions) {
	...
	let running:
		| { readonly operationId: string; readonly fiber: Fiber.Fiber<void> }
		| undefined;
	...
	const readRecord = Effect.fn("AntigravityInstallation.readRecord")(function* <
		A,
	>(filePath: string, schema: Schema.Codec<A>) {
	...
	const executableFile = Effect.fn("AntigravityInstallation.executableFile")(
		function* (filePath: string, bytes?: number) {
	...
	const completedRelease = Effect.fn(
		"AntigravityInstallation.completedRelease",
	)(function* (releaseId: string) {
		const directory = path.join(versionsDirectory, releaseId);
		const record = yield* readRecord(
			path.join(directory, RELEASE_RECORD),
			InstalledRelease,
		);
		if (
			...
			!(yield* executableFile(
				path.join(directory, names.executable),
				record.executable.bytes,
			)) ||
			!(yield* executableFile(
				path.join(directory, names.harness),
				record.harness.bytes,
			))
		) {
			return yield* installationError(
				"resolve",
	...
	const fromExternal = Effect.fn("AntigravityInstallation.fromExternal")(
		function* (candidate: string, source: "override" | "path") {
			...
			const realVersions = yield* fs
				.realPath(versionsDirectory)
				.pipe(Effect.option);
			if (
	...
	const pathCandidates = (binary: string, processEnvironment = environment) => {
		const pathValue =
			platform === "win32"
				? Object.entries(processEnvironment).findLast(
			...
			function* (executable: AntigravityExecutable, expectedVersion: string) {
				...
				if (
					...
					!initialized.authMethods?.some(
						(method) => method.id === "oauth-personal",
					)
				) {
					return yield* installationError(
						"verify",
					...
					Stream.tap((chunk) =>
						Effect.gen(function* () {
							...
							if (
								...
								downloadedBytes === asset.archiveBytes
							) {
								lastProgressAt = now;
								yield* SubscriptionRef.update(state, (current) => ({
									...current,
			...
			yield* Effect.gen(function* () {
				...
				for (;;) {
					...
					yield* Effect.gen(function* () {
						...
						yield* EffectNodeStream.fromReadable<
							Uint8Array,
							AntigravityInstallationError
						>({
							evaluate: () => readable,
							onError: wrapFailure(
								"extract",
								"Could not extract the Antigravity runtime.",
							),
						}).pipe(
							Stream.tap((chunk) =>
								Effect.gen(function* () {
						...
						if (extractedBytes !== expected.bytes) {
							return yield* installationError(
								"extract",
								"An archive member was truncated.",
							);
						}
					}).pipe(Effect.scoped);
				}
	...
	const start = gate
		...
		.pipe(
			Effect.mapError(
				wrapFailure("start", "Could not start the Antigravity installation."),
			),
		);

	const cancel = Effect.fn("AntigravityInstallation.cancel")(function* (
		operationId: string,
	) {
					...
					if (Option.isSome(realManaged)) {
						for (const binaryPath of protectedBinaryPaths) {
							...
							if (
								...
								selected.value.managedVersionDirectory
							) {
								return yield* installationError(
									"remove",
							...
							const resolved = yield* fs
								.realPath(binaryPath)
								.pipe(Effect.option);
							const candidate = Option.getOrElse(resolved, () =>
