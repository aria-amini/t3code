...
describe("Claude model catalog", () => {
	it("filters models at runtime-version boundaries and derives the upgrade message", () => {
		...
		assert.deepStrictEqual(
			resolveClaudeModelsForVersion(catalog, "3.2.0").map(
				(model) => model.slug,
			),
	...
	it("resolves aliases and declarative adapter mappings", () => {
		...
		assert.strictEqual(
			resolveClaudeModelSlug(catalog, "synthetic"),
			"claude-synthetic-next",
		);
		assert.strictEqual(
		...
		assert.strictEqual(
			normalizeClaudeCatalogEffort(catalog, "extreme", "synthetic"),
			"high",
		);
		assert.strictEqual(
	...
	it("appends custom models with their own descriptors and keeps bare slugs opaque", () => {
		...
		assert.strictEqual(
			resolveClaudeCatalogEffort(catalog, "synthetic", "extreme"),
			undefined,
		);
