...
export function scopeClaudeModelCatalog(
	...
	const customAliases = new Set(
		customEntries.map((entry) => entry.slug.toLowerCase()),
	);

	const builtInModels = catalog.models.map((entry) => {
		if (
			!entry.model.aliases?.some((alias) =>
				customAliases.has(alias.toLowerCase()),
			)
		) {
			return entry;
		}
		return {
			...
			model: {
				...
				aliases: entry.model.aliases.filter(
					(alias) => !customAliases.has(alias.toLowerCase()),
				),
			},
		};
	});
...
export function resolveClaudeModelSlug(
	catalog: ClaudeModelCatalog,
	slugOrAlias: string,
): string {
	return (
		resolveClaudeCatalogModel(catalog, slugOrAlias)?.model.slug ?? slugOrAlias
	);
}
...
function isVersionSupported(
	...
	if (!compatibility.minVersion && !compatibility.maxVersionExclusive)
		return true;
	if (!version) return false;
	if (
		compatibility.minVersion &&
		compareSemverVersions(version, compatibility.minVersion) < 0
	) {
		return false;
	}
	return !(
	...
	const unavailable = catalog.models
		.filter(
			(entry) =>
				...
				(!version ||
					compareSemverVersions(version, entry.compatibility.minVersion) < 0),
		)
		.toSorted((left, right) =>
			compareSemverVersions(
				left.compatibility.minVersion!,
...
export function normalizeClaudeCatalogEffort(
	...
	const effortMap = resolveClaudeCatalogModel(catalog, model)?.runtime
		.effortMap;
	if (!effortMap || !Object.prototype.hasOwnProperty.call(effortMap, effort))
		return effort;
	return effortMap[effort] ?? undefined;
}

export function isClaudeCatalogUltracodeEffort(
	effort: string | null | undefined,
): boolean {
...
export function resolveClaudeCatalogContextWindow(
	...
	const raw = getModelSelectionStringOptionValue(
		modelSelection,
		"contextWindow",
	);
	const descriptors = getProviderOptionDescriptors({
...
export function resolveClaudeCatalogApiModelId(
	...
	for (const [optionId, suffixes] of Object.entries(
		entry?.runtime.modelSuffixes ?? {},
	)) {
		const value = getProviderOptionCurrentValue(
			descriptors.find((descriptor) => descriptor.id === optionId),
		);
		if (typeof value === "string" && suffixes[value])
			return `${slug}${suffixes[value]}`;
	}
	return slug;
}
...
export function resolveClaudeCatalogContextWindowTokens(
	...
	if (entry.runtime.fixedContextWindowTokens)
		return entry.runtime.fixedContextWindowTokens;
	const contextWindow = resolveClaudeCatalogContextWindow(
		catalog,
		modelSelection,
	);
	return contextWindow
		? entry.runtime.contextWindowTokens?.[contextWindow]
		: undefined;
}
