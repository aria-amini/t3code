...
export const ClaudeCodeProfileSchema = Schema.Struct({
	...
	contextWindowTokens: Schema.optional(
		Schema.Record(TrimmedNonEmptyString, Schema.Number),
	),
...
export type ClaudeCodeCompatibility = NonNullable<
	typeof ClaudeModelAdapterSchema.Type.claudeCode
>;

export const decodeClaudeProfileAdapter = Schema.decodeUnknownOption(
	ClaudeProfileAdapterSchema,
);
export const decodeClaudeModelAdapter = Schema.decodeUnknownOption(
	ClaudeModelAdapterSchema,
);
				...
				Record<
					...
					| {
							readonly profiles: Readonly<
								Record<string, { readonly adapter?: unknown }>
							>;
...
export function hasValidClaudeManifestAdapters(
	manifest: ClaudeManifestAdapterInput,
): boolean {
	...
	return (
		...
		catalog.models.every((model) =>
			Option.isSome(decodeClaudeModelAdapter(model.adapter ?? {})),
		)
	);
}
