...
const makeHarness = Effect.fn("makeAntigravityDriverHarness")(function* (
	...
	const makeExecutable = Effect.fn("AntigravityDriverTest.makeExecutable")(
		function* (name: string, loginRequired = false) {
	...
	if (!refresh)
		return yield* Effect.die("Antigravity does not expose model refresh.");
	...
	it.effect.skipIf(windowsHost)(
		...
		() =>
			Effect.gen(function* () {
				...
				const nativeLaunches = h.launches.filter(
					(launch) => launch.harnessPath !== undefined,
				);
				...
				for (const launch of h.launches)
					expect(launch.credentialKeys).toEqual([]);
	...
	it.effect.skipIf(windowsHost)(
		...
		() =>
			Effect.gen(function* () {
				...
				expect(nativeLaunches.map((launch) => launch.geminiApiKey)).toEqual([
					"fixture-gemini-key",
				]);
				const requests = yield* h.readRequests;
				expect(
