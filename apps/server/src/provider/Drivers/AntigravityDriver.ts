		...
		Effect.gen(function* () {
			...
			const classifyModels = (draft: ServerProviderDraft) =>
				modelManifest.current.pipe(
					Effect.map((manifest) =>
						stampIdentity(
							ModelManifest.applyModelManifest(draft, manifest, DRIVER),
						),
					),
				);
			...
			const makeDisposableRuntime = Effect.fn(
				"AntigravityDriver.makeDisposableRuntime",
			)(function* (
				...
				Effect.gen(function* () {
					...
					yield* Stream.runForEach(runtime.getEvents(), (event) => {
						if (event._tag === "EventStreamBarrier") {
							return Deferred.succeed(event.acknowledge, undefined).pipe(
								Effect.asVoid,
							);
						}
			...
			const provider = yield* makeAntigravityProvider(settings, {
				...
				supportsTextGeneration: isAntigravityTextGenerationAvailable(
					profileDirectory,
				).pipe(
					Effect.provideService(FileSystem.FileSystem, fileSystem),
					Effect.provideService(Path.Path, path),
			...
			const defaultModel = modelManifest.current.pipe(
				Effect.map((manifest) =>
					ModelManifest.manifestDefaultModel(manifest, DRIVER),
				),
			);
					...
					orElse: () =>
						Effect.fail(
							new ProviderDriverError({
								...
								instanceId,
								detail:
									"Antigravity model refresh timed out. Try again or check Google sign-in.",
							}),
						),
				}),
					...
					!enabled
						...
						: discoverAntigravitySkills({ cwd, userHome }).pipe(
								...
								Effect.flatMap((skills) =>
									provider.snapshotForCwd(cwd, skills),
								),
								Effect.mapError(
									(cause) =>
										new ProviderDriverError({
