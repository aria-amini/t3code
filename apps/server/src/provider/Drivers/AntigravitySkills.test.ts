	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				const nested = {
					...input,
					cwd: path.join(input.userHome, "AI", "Projects", "Something"),
				};
				...
				assert.deepEqual(
					(yield* discoverAntigravitySkills({
						...input,
						cwd: input.userHome,
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				const roots = [
					{
						directory: path.join(input.userHome, ".gemini", "config", "skills"),
						scope: "user",
					},
					{
						directory: path.join(input.cwd, ".gemini", "skills"),
						scope: "project",
					},
					{
						directory: path.join(
	...
	it.effect("uses native root order for duplicate names", () =>
		Effect.gen(function* () {
			...
			for (const [index, root] of roots.entries()) {
				...
				assert.equal(
					skills[0]?.path,
					path.join(root, `copy-${index}`, "SKILL.md"),
				);
	...
	it.effect("uses the filename when valid frontmatter has no name", () =>
		Effect.gen(function* () {
			...
			yield* fileSystem.rename(
				lowerCasePath,
				path.join(path.dirname(skillPath), "SKILL.MD"),
			);
			assert.deepEqual(yield* discoverAntigravitySkills(input), []);
		}),
	);
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				const result = yield* discoverAntigravitySkills(input).pipe(
					Effect.result,
				);
	...
	it.effect("bounds the total read size across skills", () =>
		Effect.gen(function* () {
			...
			if (result._tag === "Failure") {
				...
				assert.equal(
					result.failure.path,
					path.join(root, "large-8", "SKILL.md"),
				);
			}
		}),
	);
...
it("resolves the home the agent expands ~ against", () => {
	assert.equal(
		resolveAntigravityUserHome("linux", {
			HOME: "/home/user",
	...
	assert.equal(
		resolveAntigravityUserHome("win32", {
			HOME: "/home/user",
			USERPROFILE: "C:\\Users\\user",
		}),
		"C:\\Users\\user",
	...
	assert.equal(
		resolveAntigravityUserHome("darwin", { HOME: "/Users/a b " }),
		"/Users/a b ",
	);
	assert.equal(
		resolveAntigravityUserHome("darwin", { HOME: "" }).length > 0,
		true,
	);
});
