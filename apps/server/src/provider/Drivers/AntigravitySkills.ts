		...
		Effect.catchTags({
			PlatformError: (cause) =>
				cause.reason._tag === "NotFound"
					...
					: Effect.fail(
							new AntigravitySkillsProbeError({
...
const readSkill = Effect.fn("readAntigravitySkill")(function* (
	...
	const chunks = yield* readIfPresent(
		fileSystem
			.stream(skillPath, { bytesToRead: byteLimit + 1 })
 ...
 */
export const discoverAntigravitySkills = Effect.fn("discoverAntigravitySkills")(
	function* (input: {
		readonly cwd: string;
	...
	> {
		...
		const roots = [
			...
			{
				directory: path.resolve(input.cwd, ".gemini", "skills"),
				scope: "project",
			},
			{ directory: cliSkills, scope: "user" },
			{
				directory: path.resolve(input.cwd, ".agents", "skills"),
				scope: "project",
			},
			{
				directory: path.resolve(input.cwd, ".agent", "skills"),
				scope: "project",
			},
		];
		...
		> {
			...
			const entries = yield* readIfPresent(
			...
			const skillFileName = sortedEntries.find(
