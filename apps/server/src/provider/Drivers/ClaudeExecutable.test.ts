...
describe("resolveClaudeSdkExecutablePath", () => {
	it.effect(
		"returns the configured path unchanged on non-Windows platforms",
		() =>
			Effect.gen(function* () {
				expect(
	...
	it.effect(
		"returns the resolved absolute path for native Windows executables",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"falls back to cli.js when the package ships no native binary",
		() =>
			Effect.gen(function* () {
				expect(
					yield* resolveClaudeSdkExecutablePath("claude", {}).pipe(
	...
	it.effect(
		"returns the configured path when a shim has no known package entry",
		() =>
			Effect.gen(function* () {
				expect(
					yield* resolveClaudeSdkExecutablePath("claude", {}).pipe(
	...
	it.effect(
		"returns the configured path when command resolution finds nothing",
		() =>
			Effect.gen(function* () {
				expect(
					yield* resolveClaudeSdkExecutablePath("claude", {}).pipe(
