...
it.layer(NodeServices.layer)("discoverClaudeSkills", (it) => {
	...
	it.effect(
		"prefers user skills on name collisions even with a stray .agents copy",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
	...
	it.effect(
		"falls back to the directory name and skips malformed frontmatter",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
	...
	it.effect(
		"honors CLAUDE_CONFIG_DIR from the environment when homePath is unset",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
	...
	it.effect(
		"resolves a relative CLAUDE_CONFIG_DIR against the workspace cwd",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
	...
	it.effect("marks skills that only the user can invoke", () =>
		Effect.gen(function* () {
			...
			assert.equal(
				skills.find((skill) => skill.name === "re-release-version")
					?.userInvocationOnly,
				true,
			);
			assert.equal(
				skills.find((skill) => skill.name === "release-version")
					?.userInvocationOnly,
				undefined,
			);
		}),
	...
	it.effect("disables skills switched off by skillOverrides", () =>
		Effect.gen(function* () {
			...
			yield* fs.makeDirectory(path.join(workspace, ".claude"), {
				recursive: true,
			});
			yield* fs.writeFileString(
	...
	it.effect(
		"treats a user-invocable-only override like disable-model-invocation",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
	...
	it.effect(
		"drops every override in a file when one value is invalid, as Claude Code does",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
	...
	it.effect("ignores ancestor settings outside a repository", () =>
		Effect.gen(function* () {
			...
			yield* fs.makeDirectory(path.join(parent, ".claude"), {
				recursive: true,
			});
			yield* fs.makeDirectory(workspace, { recursive: true });
			yield* fs.writeFileString(
	...
	it.effect(
		"lets the administrator's managed policy outrank every other settings file",
		() =>
			Effect.gen(function* () {
				...
				const win32Path = yield* Path.Path.pipe(
	...
	it.effect(
		"records a skill Claude Code keeps out of its own slash commands",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
