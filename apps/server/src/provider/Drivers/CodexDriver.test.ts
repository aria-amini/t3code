...
} from "../Layers/ProviderEventLoggers.ts";
...
const testLayer = ServerConfig.layerTest(process.cwd(), {
	...
	Layer.provideMerge(
		Layer.succeed(
			...
			HttpClient.make(() =>
				Effect.die("Disabled Codex must not make an HTTP request"),
			),
		),
	),
);
...
it.layer(testLayer)("CodexDriver", (it) => {
	it.effect.skipIf(windowsHost)(
		...
		() =>
			Effect.gen(function* () {
				...
				yield* fs.makeDirectory(NodePath.dirname(binaryPath), {
					recursive: true,
				});
	...
	it.effect(
		"stays manual-only when the configured executable does not exist",
		() =>
			Effect.gen(function* () {
				const instance = yield* CodexDriver.create({
				...
				expect(
					(yield* instance.snapshot.resolveMaintenance()).update,
				).toBeNull();
			}).pipe(
				Effect.provideService(ChildProcessSpawner.ChildProcessSpawner, noSpawn),
				Effect.scoped,
	...
	for (const fixture of [
		...
		it.effect.skipIf(windowsHost)(fixture.name, () =>
			Effect.gen(function* () {
				...
				yield* fs.makeDirectory(NodePath.dirname(realBinaryPath), {
					recursive: true,
				});
				yield* fs.makeDirectory(NodePath.dirname(binaryPath), {
					recursive: true,
	...
	for (const layout of ["direct", "wrapper"] as const) {
		it.effect.skipIf(windowsHost)(
			`leaves a mise ${layout} installation manual-only`,
			() =>
				Effect.gen(function* () {
					const fs = yield* FileSystem.FileSystem;
					const tempDir = yield* fs.makeTempDirectoryScoped({
						prefix: `t3-codex-mise-${layout}-`,
					});
					const binaryPath =
						layout === "direct"
							? NodePath.join(
	...
	it.effect.each([
		...
		(fixture) =>
			Effect.gen(function* () {
				...
				yield* fs.symlink(
					misePath,
					NodePath.join(shimDir, fixture.commandName),
				);
				...
				const metadataSpawner = ChildProcessSpawner.make((command) => {
					if (
						...
						command.command !== brewPath
					) {
						return Effect.die(
							"Provider resolution must not execute a provider or updater",
						);
					}
					...
					const stdout =
						command.args[0] === "--prefix"
							? brewPrefix
							: JSON.stringify({
									formulae: [{ versions: { stable: "2026.9.1" } }],
								});
				...
				const latestVersion = yield* resolveLatestProviderVersion(
					...
					Effect.provideService(
						...
						new Map([
							[
								"@openai/codex",
								{ expiresAt: Number.MAX_SAFE_INTEGER, version: "0.153.4" },
							],
						]),
					),
				);
