...
const makeTempDir = Effect.fn("CodexHomeLayout.test.makeTempDir")(function* (
	prefix: string,
) {
...
const writeTextFile = Effect.fn("CodexHomeLayout.test.writeTextFile")(
	function* (filePath: string, contents: string) {
		const fileSystem = yield* FileSystem.FileSystem;
...
it.layer(NodeServices.layer)("CodexHomeLayout", (it) => {
	describe("resolveCodexHomeLayout", () => {
		...
		it.effect(
			"uses the shared home for continuation and the shadow home for runtime",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
		...
		it.effect.skipIf(!symlinksSupported)(
			...
			() =>
				Effect.gen(function* () {
					...
					const sessionsTarget = yield* fileSystem.readLink(
						path.join(shadowHome, "sessions"),
					);
					const configTarget = yield* fileSystem.readLink(
					...
					const authLinkResult = yield* fileSystem
						...
						.pipe(Effect.result);
					const authContents = yield* fileSystem.readFileString(
						path.join(shadowHome, "auth.json"),
					);

					expect(sessionsTarget).toBe(path.join(sharedHome, "sessions"));
					expect(configTarget).toBe(path.join(sharedHome, "config.toml"));
					expect(mcpOauthLocksTarget).toBe(
						path.join(sharedHome, "mcp-oauth-locks"),
					);
		...
		it.effect.skipIf(!symlinksSupported)(
			...
			() =>
				Effect.gen(function* () {
					...
					yield* fileSystem.makeDirectory(path.join(shadowHome, "log"), {
						recursive: true,
					});
					yield* fileSystem.makeDirectory(path.join(shadowHome, "memories"), {
						recursive: true,
					});
					yield* fileSystem.makeDirectory(path.join(shadowHome, "tmp"), {
						recursive: true,
					});
