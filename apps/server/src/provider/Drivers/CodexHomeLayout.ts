...
export const resolveCodexHomeLayout = Effect.fn("resolveCodexHomeLayout")(
	function* (
		...
		if (shadowHomePath.length === 0) {
			return {
				...
				sharedHomePath,
				effectiveHomePath:
					config.homePath.trim().length > 0 ? sharedHomePath : undefined,
...
export class CodexShadowHomeFileSystemError extends Schema.TaggedErrorClass<CodexShadowHomeFileSystemError>()(
	...
	override get message(): string {
		const target =
			this.targetPath === undefined ? "" : ` to '${this.targetPath}'`;
...
const readLinkState = Effect.fn("CodexHomeLayout.readLinkState")(
	function* (input: {
...
const removePrivateSymlink = Effect.fn("CodexHomeLayout.removePrivateSymlink")(
	function* (input: {
		readonly fileSystem: FileSystem.FileSystem;
		readonly sharedHomePath: string;
		readonly effectiveHomePath: string;
...
const ensureSymlink = Effect.fn("CodexHomeLayout.ensureSymlink")(
	function* (input: {
		readonly fileSystem: FileSystem.FileSystem;
		readonly sharedHomePath: string;
		readonly effectiveHomePath: string;
...
const ensureShadowAuthIsPrivate = Effect.fn(
	"CodexHomeLayout.ensureShadowAuthIsPrivate",
)(function* (input: {
	readonly fileSystem: FileSystem.FileSystem;
	readonly sharedHomePath: string;
	readonly effectiveHomePath: string;
...
export const materializeCodexShadowHome = Effect.fn(
	"materializeCodexShadowHome",
)(function* (layout: CodexHomeLayout) {
	...
	const sharedEntryNames = yield* fileSystem
		.readDirectory(layout.sharedHomePath)
		.pipe(
			Effect.catchTags({
				PlatformError: (cause) =>
					new CodexShadowHomeFileSystemError({
