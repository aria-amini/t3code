...
const orUndefined = <A, R>(
	...
	effect.pipe(
		...
		Effect.catchTags({
			PlatformError: (error) => {
				if (error.reason._tag !== "NotFound" && budget)
...
function parseSkillFrontmatter(
	contents: string,
): CursorSkillFrontmatter | undefined {
	...
	return {
		cliVisible:
			surfaces.length === 0 ||
			surfaces.some((surface) => surface.trim().toLowerCase() === "cli"),
...
const discoverSkillsInRoot = Effect.fn("discoverCursorSkillsInRoot")(
	function* (input: {
	...
	> {
		...
		const rootDirectory = yield* orUndefined(
			fileSystem.realPath(input.directory),
			input.budget,
		);
		...
		const visit = Effect.fn("visitCursorSkillDirectory")(function* (
			...
			const resolvedDirectory = yield* orUndefined(
			...
			const skillInfo = yield* orUndefined(
				fileSystem.stat(skillPath),
				input.budget,
			);
			if (skillInfo?.type === "File") {
				let frontmatter: CursorSkillFrontmatter | undefined = {
					cliVisible: true,
				};
				if (
					skillInfo.size <= MAX_SKILL_BYTES &&
					skillInfo.size <= input.budget.remainingBytes
				) {
				...
				if (frontmatter?.cliVisible && name) {
					skills.push({
						...
						...(frontmatter.description
							...
							: {}),
						...(frontmatter.userInvocationOnly
							? { userInvocationOnly: true }
							: {}),
			...
			const entries = yield* orUndefined(
...
const inspectCursorSkills = Effect.fn("inspectCursorSkills")(function* (
	...
	return {
		skills: [...skillsByName.values()].sort((left, right) =>
...
export const discoverCursorSkills = Effect.fn("discoverCursorSkills")(
	function* (cwd?: string, environment: NodeJS.ProcessEnv = process.env) {
