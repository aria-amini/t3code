...
const inspectPayload = (skills: ReadonlyArray<unknown>) =>
	JSON.stringify({ skills });

const makeInspectSpawner = (
	stdout: string,
	exitCode = 0,
	...
	ChildProcessSpawner.make((command) => {
		spawnCwds?.push(
			command._tag === "StandardCommand" ? command.options.cwd : undefined,
		);
			...
			Effect.provideService(
				...
				makeInspectSpawner(
					inspectPayload([
						{
							name: "  ",
							source: { type: "user", path: "/tmp/skills/a/SKILL.md" },
						},
	...
	it.effect("rejects malformed or unexpected output as a decode failure", () =>
		Effect.gen(function* () {
			for (const stdout of ["not json", "null", '{"skills":"nope"}', "{}"]) {
				const error = yield* discoverGrokSkills(
					{ binaryPath: "grok" },
					{},
				).pipe(
					Effect.flip,
