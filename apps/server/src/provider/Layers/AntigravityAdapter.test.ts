...
type Runtime = Effect.Success<
	ReturnType<AntigravityAdapterOptions["makeRuntime"]>
>;
...
const makeHarness = Effect.fn("makeAntigravityAdapterHarness")(
	function* (options?: {
		...
		const launches: Array<
			Parameters<AntigravityAdapterOptions["makeRuntime"]>[0]
		> = [];
		...
		const waitForEvent = Effect.fn("AntigravityAdapterTest.waitForEvent")(
			function* <T extends ProviderRuntimeEvent>(
...
it.layer(layer)("AntigravityAdapter", (it) => {
	it.effect(
		...
		() =>
			Effect.gen(function* () {
						...
						onConfigOptionsUpdated: (configOptions) =>
							Effect.sync(() => {
								...
								if (model?.type === "select")
									modelSelections.push(model.currentValue);
							}),
					},
				);
				yield* adapter.streamEvents.pipe(
				...
				expect(
					modelSelections.every((model) => model === nativeAlternative),
				).toBe(true);
				expect(
					observed
				...
				const lines = (yield* fileSystem.readFileString(requestLog))
					.trim()
					.split("\n");
				...
				expect(
					requests.some((request) => request.method === "session/resume"),
				).toBe(true);
				expect(
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				expect(h.commandUpdates.at(-1)?.map((command) => command.name)).toEqual(
					["plan", "logout"],
				);
				expect(h.adapter.capabilities.supportsConversationRollback).toBe(false);
				const rollback = yield* h.adapter
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				expect(deltas.every((event) => event.turnId === result.turnId)).toBe(
					true,
				);
				...
				expect(
					tool?.type === "item.completed" ? tool.payload.data : undefined,
				).toMatchObject({
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				expect(
					opened.payload.questions[0]?.options.map((option) => option.value),
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				expect(
					h.seen.some((event) => event.type === "user-input.resolved"),
				).toBe(true);
			}),
	);
			...
			Effect.gen(function* () {
				...
				expect(initialPrompt.content).toEqual([
					{ type: "text", text: "First prompt" },
					{
						...
						text: expect.stringContaining(
							`Antigravity harness, as ${nativeDefault}`,
						),
					},
				]);
				...
				expect(replacement.content).toEqual([
					{ type: "text", text: "Steer the turn" },
					{
						type: "text",
						text: expect.stringContaining(
							`Antigravity harness, as ${nativeAlternative}`,
						),
					},
				]);
				...
				const [oldResult, newResult] = yield* Effect.all([
					Fiber.join(first),
					Fiber.join(second),
				]);
				...
				yield* h.waitForEvent((event) => event.type === "turn.completed");
				expect(
					h.seen.filter((event) => event.type === "turn.completed"),
				).toHaveLength(1);
	...
	it.effect("keeps a launched batch active while child tools continue", () =>
		Effect.gen(function* () {
			...
			const launched = yield* h.waitForEvent(
			...
			yield* h.drainEvents;
			expect(
				h.seen.filter((event) => event.type === "task.completed"),
			).toHaveLength(0);
			expect(
				h.seen.filter((event) => event.type === "task.updated"),
			).toHaveLength(0);
			yield* Deferred.succeed(prompt.result, { stopReason: "end_turn" });
			yield* Fiber.join(sending);
			yield* h.waitForEvent((event) => event.type === "turn.completed");
			...
			expect(
				h.seen.find((event) => event.type === "task.updated")?.payload,
			).toMatchObject({
				taskId: started.toolCall.toolCallId,
				status: "idle",
	...
	it.effect("waits for a replayed subagent's final status and result", () =>
		Effect.gen(function* () {
			...
			expect(
				h.seen.filter((event) => event.type.startsWith("task.")),
			).toHaveLength(1);
		}),
	);
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				yield* h.drainEvents;
				expect(
					h.seen.filter((event) => event.type === "task.progress"),
				).toHaveLength(2);
				expect(
					h.seen.filter((event) => event.type === "task.completed"),
				).toHaveLength(0);
				...
				yield* Deferred.succeed(secondPrompt.result, {
					stopReason: "end_turn",
				});
	...
	it.effect("keeps MCP identity when later updates omit metadata", () =>
		Effect.gen(function* () {
			...
			expect(
				h.seen.filter((event) => event.type === "item.updated"),
			).toHaveLength(2);
			expect(
				h.seen.filter((event) => event.type === "item.completed"),
			).toHaveLength(1);
		}),
	);
	...
	it.effect("shows pending subagents and closes a denied invocation", () =>
		Effect.gen(function* () {
			...
			const pending = yield* h.waitForEvent(
				(event) => event.type === "task.progress",
			);
			...
			Effect.gen(function* () {
				...
				if (stop === "disconnect") {
					...
					const steering = yield* h.adapter
						...
						.pipe(Effect.forkChild);
					const replacement = yield* h.nextPrompt;
					yield* Deferred.succeed(replacement.result, {
						stopReason: "end_turn",
					});
				...
				const settled = yield* h.waitForEvent(
	...
	it.effect("retires a prompt cancelled before native dispatch", () =>
		Effect.gen(function* () {
			...
			const cancelled = yield* h.waitForEvent(
				(event) => event.type === "turn.completed",
			);
	...
	it.effect("awaits full process cleanup for concurrent stop requests", () =>
		Effect.gen(function* () {
			...
			if (!registeredStop)
				return yield* Effect.die("Missing process cleanup registration");
			const signOutStop = yield* registeredStop.pipe(
			...
			expect(
				h.seen.filter((event) => event.type === "session.exited"),
			).toHaveLength(1);
	...
	it.effect(
		"reports hidden login requests as sign-in required and clears account metadata",
		() =>
			Effect.gen(function* () {
				const h = yield* makeHarness();
	...
	it.effect(
		"serves client file reads and writes only inside the session roots",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"does not launch a process for a disabled instance or invalid resume cursor",
		() =>
			Effect.gen(function* () {
