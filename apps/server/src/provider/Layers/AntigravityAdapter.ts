...
const resolveClientFilePath = Effect.fn(
	"AntigravityAdapter.resolveClientFilePath",
)(function* (input: {
...
const readClientTextFile = Effect.fn("AntigravityAdapter.readClientTextFile")(
	function* (input: {
		readonly fileSystem: FileSystem.FileSystem;
		readonly path: Path.Path;
		readonly allowedRoots: ReadonlyArray<string>;
		...
		EffectAcpSchema.ReadTextFileResponse,
		EffectAcpErrors.AcpError
	> {
		const filePath = yield* resolveClientFilePath({
			...input,
			...
			.pipe(
				Effect.mapError(() =>
					EffectAcpErrors.AcpRequestError.internalError(
						`Could not read '${input.request.path}'.`,
					),
				),
			);
...
const writeClientTextFile = Effect.fn("AntigravityAdapter.writeClientTextFile")(
	function* (input: {
		readonly fileSystem: FileSystem.FileSystem;
		readonly path: Path.Path;
		readonly allowedRoots: ReadonlyArray<string>;
		...
		EffectAcpSchema.WriteTextFileResponse,
		EffectAcpErrors.AcpError
	> {
		const filePath = yield* resolveClientFilePath({
			...input,
		...
		yield* input.fileSystem
			.makeDirectory(input.path.dirname(filePath), { recursive: true })
			.pipe(
				Effect.andThen(
					input.fileSystem.writeFileString(filePath, input.request.content),
				),
				Effect.mapError(() =>
					EffectAcpErrors.AcpRequestError.internalError(
						`Could not write '${input.request.path}'.`,
					),
				),
			);
		return {};
...
export const makeAntigravityAdapter = Effect.fn("makeAntigravityAdapter")(
	function* (
		...
		const withThreadLock = <A, E, R>(
			...
			SynchronizedRef.modifyEffect(locks, (current) => {
				...
				return Semaphore.make(1).pipe(
					Effect.map(
						(lock) => [lock, new Map(current).set(threadId, lock)] as const,
					),
				);
		...
		const cancelRequests = Effect.fn("AntigravityAdapter.cancelRequests")(
						...
						yield* emit({
							...
							payload: {
								...
								...(context.disconnected
									? { reason: "Antigravity process stopped." }
									: {}),
							},
						});
					}),
		...
		const handlePermission = Effect.fn("AntigravityAdapter.handlePermission")(
			function* (
				context: SessionContext,
				...
				if (isAntigravityUserInputRequest(request)) {
					...
					return yield* Effect.gen(function* () {
						...
						Effect.ensuring(
							Effect.sync(() => context.questions.delete(requestId)),
						),
					);
				}
				...
				const toolCall = parsed.toolCall
					? normalizeAntigravityToolCall(parsed.toolCall)
					: undefined;
				...
				return yield* Effect.gen(function* () {
					yield* emit(
						makeAcpRequestOpenedEvent({
							...
							detail:
								permissionRequest.detail ?? "Antigravity requests permission.",
							args: rawPayload,
							source: "acp.jsonrpc",
							method: "session/request_permission",
							rawPayload,
					...
					Effect.ensuring(
						Effect.sync(() => context.approvals.delete(requestId)),
					),
				);
			},
		);
		...
		const handleEvent = Effect.fn("AntigravityAdapter.handleEvent")(function* (
			...
			switch (event._tag) {
				...
				case "ConfigOptionsUpdated":
					yield* (
						options.onConfigOptionsUpdated?.(event.configOptions) ?? Effect.void
					);
					return;
						...
						Effect.gen(function* () {
							...
							if (!isMcp && (subagent || kind === "subagent")) {
								...
								if (
									...
									context.subagents.set(toolCall.toolCallId, {
										turnId,
										status: undefined,
									});
									return;
								...
								if (toolCall.status === "failed") {
									...
									yield* emit({
										...
										payload: {
											...
											status: "idle",
											description:
												"Individual agent status is unavailable for this earlier batch.",
											timelineBypass: true,
										},
									});
									context.subagents.set(toolCall.toolCallId, "finished");
									...
									const status =
										toolCall.status === "pending" ? "pending" : "running";
									const description =
				...
				Effect.gen(function* () {
					if (!settings.enabled) {
						return yield* new ProviderAdapterValidationError({
							...
							issue:
								"Enable Antigravity in provider settings before starting a thread.",
						});
					}
					if (
						...
						return yield* new ProviderAdapterValidationError({
							...
							operation: "startSession",
							issue:
								"The Antigravity provider instance does not match the requested session.",
						});
					}
					if (!input.cwd?.trim()) {
						return yield* new ProviderAdapterValidationError({
					...
					if (input.resumeCursor !== undefined && Option.isNone(cursor)) {
						return yield* new ProviderAdapterValidationError({
							...
							operation: "startSession",
							issue:
								"The saved Antigravity session is invalid. Start a new thread.",
						});
					}
					...
					const stopOwned = Effect.suspend(() =>
						context
							? stopContext(context).pipe(Effect.ignore)
					...
					return yield* options
						.withProcess(
							stopOwned,
							Effect.gen(function* () {
								const mcp = McpProviderSession.readMcpProviderSession(
									input.threadId,
								);
								...
								const model = yield* applyAntigravityAcpModelSelection({
									...
									defaultModel: yield* (
										options.defaultModel ?? Effect.succeed(undefined)
									),
									mapError: (cause) => cause,
								});
								yield* runtime.setMode(
									antigravityPermissionMode(input.runtimeMode),
								);
								...
								yield* Stream.runForEach(runtime.getEvents(), (event) =>
									...
									Effect.catchCause(() =>
										Effect.logError(
											"Could not process an Antigravity runtime event.",
										),
									),
								...
								yield* emit({
									...
									payload: {
										state: "ready",
										reason: "Antigravity ACP session ready",
									},
								});
								yield* emit({
									type: "thread.started",
		...
		const sendTurn: Adapter["sendTurn"] = Effect.fn(
			...
			const prompt = yield* buildAntigravityPrompt({
				...
				Effect.mapError((cause) =>
					mapAntigravityError(input.threadId, "session/prompt", cause),
				),
			);
			...
			return yield* Effect.gen(function* () {
				const launch = yield* context.promptLock.withPermit(
					Effect.gen(function* () {
						...
						if (
							model &&
							!availableModels.some((option) => option.value === model)
						) {
							return yield* EffectAcpErrors.AcpRequestError.invalidParams(
				...
				const record = context.turns.find(
					(turn) => turn.id === launch.turn.turnId,
				);
		...
		const interruptTurn: Adapter["interruptTurn"] = (threadId) =>
			Effect.gen(function* () {
				...
				yield* context.promptLock
					...
					.pipe(
						Effect.mapError((cause) =>
							mapAntigravityError(threadId, "session/cancel", cause),
						),
					);
			});
		...
		const respondToUserInput: Adapter["respondToUserInput"] = (
			...
			Effect.gen(function* () {
				...
				const result = makeAntigravityUserInputResponse(
					pending.request,
					answers,
				);
				if (!result) {
		...
		return {
			...
			hasSession: (threadId) =>
				Effect.sync(
					() => sessions.has(threadId) && !sessions.get(threadId)?.stopped,
				),
			readThread: (threadId) =>
			...
			rollbackThread: (_threadId: ThreadId, _numTurns: number) =>
				Effect.fail(
					new ProviderAdapterValidationError({
						...
						issue:
							"Antigravity does not support conversation rewind. Start a new thread instead.",
					}),
				),
