...
const makeHarness = Effect.fn("makeAntigravityProviderHarness")(function* (
	...
	const initialProbe = yield* Deferred.make<
		EffectAcpSchema.InitializeResponse,
		ProbeError
	>();
	const probeCalls = yield* Ref.make(0);
	const safetyCalls = yield* Ref.make(0);
	const probe = yield* Ref.make<
		Effect.Effect<EffectAcpSchema.InitializeResponse, ProbeError>
	>(Deferred.await(initialProbe));
	const safety = yield* Ref.make<Effect.Effect<boolean>>(
		Effect.succeed(options.safe ?? true),
	);
	const provider = yield* makeAntigravityProvider(
		decodeSettings({
			enabled: options.enabled ?? true,
			customModels: ["do-not-seed-me"],
		}),
		{
			stampIdentity: (snapshot) =>
				Effect.succeed({ ...snapshot, instanceId, driver }),
			probe: Ref.update(probeCalls, (count) => count + 1).pipe(
				Effect.andThen(Ref.get(probe)),
				Effect.flatten,
			),
			supportsTextGeneration: Ref.update(
				safetyCalls,
				(count) => count + 1,
			).pipe(Effect.andThen(Ref.get(safety)), Effect.flatten),
		},
	);
	const initialUpdate = yield* Stream.toPull(
		provider.snapshot.streamChanges.pipe(
			Stream.filter(
				(snapshot) => snapshot.installed || snapshot.status === "error",
			),
		),
	);
...
describe("Antigravity model catalog", () => {
	...
	it("uses legacy session models only when model config is absent", () => {
		...
		expect(fromLegacy).toEqual(
			buildAntigravityModelsFromSession(sessionSetupResult),
		);
		expect(
			buildAntigravityModelsFromSession({
				...sessionSetupResult,
	...
	it("flattens native option groups without combining distinct model IDs", () => {
					...
					options: [
						{
							group: "Flash",
							name: "Flash",
							options: [modelOptions[3]!, modelOptions[4]!],
						},
						{
							group: "Pro",
							name: "Pro",
							options: [modelOptions[9]!, modelOptions[3]!],
						},
					],
				},
			],
		...
		expect(
			models.find((model) => model.aliases?.includes(ANTIGRAVITY_DEFAULT_MODEL))
				?.slug,
...
it.layer(testLayer)("Antigravity provider snapshots", (it) => {
	...
	it.effect(
		"records explicit sign-in while disabled without starting a health probe",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const harness = yield* makeHarness({ enabled: false });
					...
					expect((yield* harness.provider.snapshot.refresh).models).toEqual(
						snapshot.models,
					);
					expect(yield* Ref.get(harness.probeCalls)).toBe(0);
				}),
	...
	it.effect(
		"publishes the configured sign-in method before any account is checked",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const provider = yield* makeAntigravityProvider(
	...
	it.effect(
		"treats initialize as installation proof, not account or model discovery",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
	...
	it.effect(
		"publishes session metadata and native commands without another health probe",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const harness = yield* makeHarness();
	...
	it.effect(
		"clears all account metadata on sign-out and authentication failure",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const harness = yield* makeHarness();
					yield* harness.initialize;
					for (const clear of [
						harness.provider.onSignedOut,
						harness.provider.onAuthRequired,
					]) {
						yield* harness.provider.onSessionStarted(started, "/workspace");
						yield* harness.provider.onAvailableCommands(commands, "/workspace");
						yield* clear;
						...
						expect(
							(yield* harness.provider.snapshot.getSnapshot).models,
						).toEqual([]);
					}
	...
	it.effect(
		"replaces one account's catalog instead of combining accounts",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const harness = yield* makeHarness();
							...
							configOptions: [
								{
									...modelConfig,
									currentValue: "gemini-pro-agent",
									options: [modelOptions[9]!],
								},
							],
						},
					});
					expect(
						(yield* harness.provider.snapshot.getSnapshot).models.map(
							(model) => model.slug,
						),
	...
	it.effect(
		"retains known account metadata when a local health check fails",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const harness = yield* makeHarness();
					...
					yield* Ref.set(
						harness.probe,
						Effect.fail(
							EffectAcpErrors.AcpRequestError.internalError("probe failed"),
						),
					);
					const snapshot = yield* harness.provider.snapshot.refresh;
					expect(snapshot).toMatchObject({
	...
	it.effect("allows a slow packaged runtime health check to finish", () =>
		Effect.scoped(
			Effect.gen(function* () {
				...
				const initialized =
					yield* Deferred.make<EffectAcpSchema.InitializeResponse>();
				yield* Ref.set(
					harness.probe,
				...
				const refresh = yield* harness.provider.snapshot.refresh.pipe(
					Effect.forkChild,
				);
	...
	it.effect(
		"distinguishes missing executables from a failed installed executable",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const harness = yield* makeHarness();
					...
					const failures = [
						{
							error: new EffectAcpErrors.AcpSpawnError({
								cause: { code: "ENOENT" },
							}),
							installed: false,
						},
	...
	it.effect(
		"does not let an old health result restore a signed-out account",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const harness = yield* makeHarness();
	...
	it.effect(
		"exposes helper support only when the supplied safety check allows it",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					...
					yield* harness.provider.onSessionStarted(started);
					expect(
						(yield* harness.provider.snapshot.getSnapshot)
	...
	it.effect(
		"keeps discovered workspace skills through session and command updates",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const harness = yield* makeHarness();
	...
	it.effect(
		"bounds workspace metadata without starting sessions for workspace lookup",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const harness = yield* makeHarness();
					...
					const snapshot =
						yield* harness.provider.snapshotForCwd("/workspace-34");
