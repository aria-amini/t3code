...
const EMPTY_MODEL_CAPABILITIES = createModelCapabilities({
	optionDescriptors: [],
});
...
function isMissingInstallation(
	error: EffectAcpErrors.AcpError | ProviderSetupError,
): boolean {
...
export const makeAntigravityProvider = Effect.fn("makeAntigravityProvider")(
	function* (
		...
		const checkProvider = Effect.fn("checkAntigravityProvider")(function* () {
			...
			const missingInstallation =
				failure !== undefined && isMissingInstallation(failure);
			const errorMessage =
		...
		const onSessionStarted = Effect.fn("AntigravityProvider.onSessionStarted")(
			function* (started: AcpSessionRuntimeStartResult, cwd?: string) {
				const before = yield* SubscriptionRef.get(metadata);
				...
				yield* SubscriptionRef.update(metadata, (state) => {
					...
					return {
						...
						draft: {
							...
							version:
								started.initializeResult.agentInfo?.version || draft.version,
							auth: {
							...
							checkedAt: updatedAt,
							models: buildAntigravityModelsFromSession(
								...
								? {
										workspaceSnapshots: [
											...workspaces.filter((entry) => entry.cwd !== cwd),
											{
												...
												checkedAt: updatedAt,
												slashCommands:
													workspace?.slashCommands ?? draft.slashCommands,
												skills:
													workspace?.skills ?? discoveredSkills.get(cwd) ?? [],
											},
		...
		const onConfigOptionsUpdated = Effect.fn(
			"AntigravityProvider.onConfigOptionsUpdated",
		)(function* (
		...
		const onAvailableCommands = Effect.fn(
			"AntigravityProvider.onAvailableCommands",
		)(function* (
			commands: ReadonlyArray<EffectAcpSchema.AvailableCommand>,
			cwd?: string,
		) {
							...
							? {
									workspaceSnapshots: [
										...
										),
										{
											...
											slashCommands,
											skills:
												state.draft.workspaceSnapshots?.find(
													(entry) => entry.cwd === cwd,
												)?.skills ??
		...
		const clearAccountMetadata = Effect.fn(
			"AntigravityProvider.clearAccountMetadata",
		)(function* () {
			const updatedAt = DateTime.formatIso(yield* DateTime.now);
		...
		const snapshotForCwd = Effect.fn("AntigravityProvider.snapshotForCwd")(
			function* (cwd: string, skills?: ServerProvider["skills"]) {
				if (skills) discoveredSkills.set(cwd, skills);
				const snapshot = yield* getSnapshot;
				const workspace = snapshot.workspaceSnapshots?.find(
					(entry) => entry.cwd === cwd,
				);
