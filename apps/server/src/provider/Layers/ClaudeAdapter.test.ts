...
const encodeUnknownJsonString = Schema.encodeSync(
...
function makeHarness(config?: {
	...
	const adapterOptions: ClaudeAdapterLiveOptions = {
		...
		...(config?.scopedLimitNames
			? { scopedLimitNames: config.scopedLimitNames }
			: {}),
...
describe("ClaudeAdapterLive", () => {
	it.effect(
		"returns validation error for non-claude provider on startSession",
		() => {
	...
	it.effect(
		"retains Claude session startup causes without exposing their messages",
		() => {
			const cause = new Error(
				"credential material that must remain in the cause chain",
			);
	...
	it.effect(
		"derives bypass permission mode from full-access runtime policy",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"derives auto permission mode from auto runtime policy without skip flag",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"runs Claude SDK sessions with the configured CLAUDE_CONFIG_DIR",
		() => {
			const harness = makeHarness({
	...
	it.effect(
		"forwards Claude thinking toggle for models that support it",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		...
		() => {
			...
			return Effect.gen(function* () {
				const { options: customOptions, prompt: customPrompt } =
					yield* runCustomFlow;
				...
				const builtInOptions = yield* start(
					builtInHarness,
					SYNTHETIC_CLAUDE_CAPABLE_MODEL,
				);
				assert.equal(
					builtInOptions.model,
					`${SYNTHETIC_CLAUDE_CAPABLE_MODEL}[expanded]`,
				);
	...
	it.effect(
		"treats ultrathink as a prompt keyword instead of a session effort",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
				...
				const promptText = yield* Effect.promise(() =>
					readFirstPromptText(createInput),
				);
	...
	it.effect("embeds image attachments in Claude user messages", () => {
		const baseDir = NodeFS.mkdtempSync(
			NodePath.join(NodeOS.tmpdir(), "claude-attachments-"),
		);
		const harness = makeHarness({
		...
		return Effect.gen(function* () {
			...
			const promptMessage = yield* Effect.promise(() =>
				readFirstPromptMessage(createInput),
			);
	...
	it.effect(
		"puts the command text last so attachments do not suppress expansion",
		() => {
			const baseDir = NodeFS.mkdtempSync(
				NodePath.join(NodeOS.tmpdir(), "claude-attachments-"),
			);
			...
			return Effect.gen(function* () {
				...
				for (const attachment of [imageAttachment, fileAttachment]) {
					...
					NodeFS.mkdirSync(NodePath.dirname(attachmentPath), {
						recursive: true,
					});
					NodeFS.writeFileSync(attachmentPath, Uint8Array.from([1, 2, 3, 4]));
				}
	...
	it.effect(
		"dispatches a $skill mention as a trailing slash command block",
		() => {
			...
			const homeDir = NodeFS.mkdtempSync(
				NodePath.join(NodeOS.tmpdir(), "claude-skills-home-"),
			);
			NodeFS.mkdirSync(NodePath.join(homeDir, "skills", "implement"), {
				recursive: true,
			});
	...
	it.effect("keeps the skill command block after image attachments", () => {
		...
		const baseDir = NodeFS.mkdtempSync(
			NodePath.join(NodeOS.tmpdir(), "claude-skill-image-"),
		);
		const homeDir = NodePath.join(baseDir, "claude-home");
		NodeFS.mkdirSync(NodePath.join(homeDir, "skills", "review"), {
			recursive: true,
		});
		NodeFS.writeFileSync(
		...
		return Effect.gen(function* () {
			...
			assert.isDefined(promptMessage);
			const blocks = promptMessage.message.content as Array<{
				type: string;
				text?: string;
			}>;
			assert.deepEqual(
	...
	it.effect(
		"leaves a $ mention of an unknown or disabled skill as prose",
		() => {
			const homeDir = NodeFS.mkdtempSync(
				NodePath.join(NodeOS.tmpdir(), "claude-skills-off-"),
			);
	...
	it.effect(
		"maps Claude stream/runtime messages to canonical provider runtime events",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
				...
				const toolStartedIndex = runtimeEvents.findIndex(
					(event) => event.type === "item.started",
				);
				assert.equal(
	...
	it.effect(
		"places overage-included rate-limit events on the bucket the probe named",
		() => {
			...
			return Effect.gen(function* () {
				...
				assert.deepStrictEqual(
					limitsUpdates(yield* Fiber.join(firstTurnFiber)),
					[],
				);
				...
				assert.deepStrictEqual(
					limitsUpdates(yield* Fiber.join(secondTurnFiber)),
					[
						{
							windows: [
								{
	...
	it.effect(
		"does not emit turn.completed for a result with no active turn",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		...
		() => {
			...
			return Effect.gen(function* () {
				...
				assert.equal(
					String(turnCompletedEvents[0]?.turnId),
					String(turn.turnId),
				);
			}).pipe(
				Effect.provideService(Random.Random, makeDeterministicRandomService()),
	...
	it.effect(
		"maps Claude reasoning deltas, streamed tool inputs, and tool results",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
				...
				const toolResultUpdated = runtimeEvents.find(
					(event) =>
						...
						(
							event.payload.data as
								| { result?: { tool_use_id?: string } }
								| undefined
						)?.result?.tool_use_id === "tool-grep-1",
				);
	...
	it.effect("classifies only streamed Read image inputs as image views", () => {
		...
		return Effect.gen(function* () {
			...
			for (const event of imageEvents.slice(1)) {
				...
				assert.equal(
					(event.payload.data as { input?: { file_path?: string } } | undefined)
						?.input?.file_path,
					imagePath,
				);
			}
	...
	it.effect(
		"falls back to a default plan step label for blank TodoWrite content",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"classifies Claude Task tool invocations as collaboration agent work",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"treats user-aborted Claude results as interrupted without a runtime error",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"treats aborted_tools results as interrupted and hides ede_diagnostic errors",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"fails a turn when the result carries a give-up terminal_reason",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
		...
		({ result, state, errorMessage }) => {
			...
			return Effect.gen(function* () {
				...
				const payload = completedTurn(
					Array.from(yield* Fiber.join(runtimeEventsFiber)),
				);
	...
	it.effect.each([
		...
		},
		{
			...
			messages: [
				rateLimitAssistant,
				{ ...rateLimitAssistant, error: undefined },
			],
		...
		},
		{
			...
			messages: [
				rateLimitAssistant,
				{ ...rateLimitAssistant, error: "server_error" },
			],
			expected: genericApiErrorMessage,
		},
		{
		...
		"classifies the terminal API failure after $name",
		({ messages, expected }) => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"names repeated usage limits without carrying them into a later turn",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
				...
				for (const [index, expected] of [
					...
					const payload = completedTurn(
						Array.from(yield* Fiber.join(eventsFiber)),
					);
					assert.equal(payload.state, "failed");
			...
			return Effect.gen(function* () {
				...
				if (evidence === "assistant-rate-limit") {
					...
					harness.query.emit({
						...
						uuid: "audit-auth",
						parent_tool_use_id:
							evidence === "nested-auth" ? "synthetic-parent-tool" : null,
						error: "authentication_failed",
						is_api_error_message: true,
						message: {
							id: "audit-message",
					...
					if (
						evidence === "two-windows-recovered" ||
						evidence === "rejected-again"
					) {
						harness.query.emit({
							type: "rate_limit_event",
							rate_limit_info: {
								...
								rateLimitType:
									evidence === "rejected-again" ? "five_hour" : "seven_day",
								resetsAt,
							},
							session_id: "sdk-audit",
		...
		({ homePath, inherited }) => {
			...
			return Effect.gen(function* () {
				...
				const expectedConfigDir = homePath
					...
					: inherited;
				assert.equal(
					actualQuery.options.env?.CLAUDE_CONFIG_DIR,
					expectedConfigDir,
				);
				...
				assert(
					completed.payload.errorMessage?.includes(
						`from ${encodeUnknownJsonString(cwd)}`,
					),
				);
			...
			return Effect.gen(function* () {
				...
				if (
					...
					assert.ok(
						...
						`${reason} carries an error message`,
					);
				}
			}).pipe(
				Effect.provideService(Random.Random, makeDeterministicRandomService()),
	...
	it.effect(
		"interruptTurn settles live tasks and closes the provider session",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
				...
				const stoppedTaskEvents = Array.from(
					yield* Fiber.join(stoppedTaskEventFiber),
				);
	...
	it.effect(
		"stopAll attempts every session when one process close fails",
		() => {
	...
	it.effect(
		"completes with result usage without querying current context usage",
		() => {
			const harness = makeHarness();
	...
	it.effect(
		"treats omitted Claude cache counters as zero contributions",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"uses per-turn result usage across consecutive Claude turns",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
				...
				const completedFiber = yield* adapter.streamEvents.pipe(
					Stream.tap((event) =>
						...
						event.payload.reason === "api_retry:1/2"
							? Deferred.succeed(firstTurnSettled, undefined).pipe(
									Effect.asVoid,
								)
							: Effect.void,
					),
				...
				if (
					completed[0]?.type === "turn.completed" &&
					completed[1]?.type === "turn.completed"
				) {
	...
	it.effect(
		"preserves compacted usage when completion follows an older assistant frame",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"workflow member coalescing: identical snapshots suppress, changes emit",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
					...
					Stream.takeUntil(
						...
						(event) =>
							(event.payload as { taskId?: string }).taskId ===
								"wf-coalesce:wf:0" &&
							(event.payload as { typedUsage?: { totalTokens?: number } })
				...
				const tick = (
					usageTotal: number,
					snapshot: ReturnType<typeof memberSnapshot>,
				) =>
					harness.query.emit({
						type: "system",
	...
	it.effect(
		"task.started carries model/effort; subagent snapshots refine the model",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"a subagent snapshot that beats task_started still wins over the seed",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"closes the session when the Claude stream aborts after a turn starts",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect("keeps Claude stream failure events structural", () => {
		...
		return Effect.gen(function* () {
			...
			const runtimeError = runtimeEvents.find(
	...
	it.effect(
		"closes the previous session before replacing an existing thread session",
		() => {
			const queries: FakeClaudeQuery[] = [];
			const layer = Layer.effect(
				ClaudeAdapter,
	...
	it.effect(
		"forwards Claude task progress summaries for subagent updates",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
				...
				if (progressEvent?.type === "task.progress") {
					...
					assert.equal(
						progressEvent.payload.description,
						"Running background teammate",
					);
				}
			}).pipe(
				Effect.provideService(Random.Random, makeDeterministicRandomService()),
	...
	it.effect(
		"consumes undeclared and UX-internal system subtypes without warning rows",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
				const runtimeEventsFiber = yield* adapter.streamEvents.pipe(
					Stream.takeUntil(
						(event) =>
							event.type === "session.state.changed" &&
							event.payload.reason === "api_retry:3/10",
					),
					Stream.runCollect,
					Effect.forkChild,
				);
				...
				const sessionStates = runtimeEvents
					...
					.filter(
						(entry) =>
							entry.startsWith("running:session_state") ||
							entry.includes("session_state"),
					);
	...
	it.effect("surfaces a rejected Claude usage limit once per turn", () => {
		...
		return Effect.gen(function* () {
			...
			assert.deepEqual(
				runtimeEvents.find((event) => event.type === "runtime.warning")?.payload
					.detail,
				rateLimitInfo,
			);
	...
	it.effect(
		"keeps allowed and malformed Claude rate-limit events out of the work log",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"keeps one row per window when two Claude limits interleave",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"drops an unusable Claude reset time, not the row or the session",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"warns for unmapped Claude limits and names the probed model bucket",
		() => {
			const scopedLimitNames = Ref.makeUnsafe<ClaudeScopedLimitNames>({
				overageIncluded: undefined,
			});
		...
		return Effect.gen(function* () {
			...
			const readyFiber = yield* Stream.takeUntil(
				...
				(event) =>
					...
					event.payload.message === readyMessage,
			...
			const runtimeEventsFiber = yield* Stream.takeUntil(
				...
				(event) =>
					event.type === "runtime.warning" &&
					event.payload.message === processedMessage,
			).pipe(Stream.runCollect, Effect.forkChild);
			for (const [state, uuid] of [
	...
	it.effect(
		"emits thread token usage updates from Claude task progress",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"emits Claude context window on result completion usage snapshots",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"clamps oversized Claude usage to the reported context window",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		...
		() => {
			...
			return Effect.gen(function* () {
				...
				const deltaIndex = runtimeEvents.findIndex(
					(event) => event.type === "content.delta",
				);
				...
				assert.equal(
					deltaIndex >= 0 && completedIndex >= 0 && deltaIndex < completedIndex,
					true,
				);
	...
	it.effect(
		"creates a fresh assistant message when Claude reuses a text block index",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
				...
				assert.notEqual(
					firstAssistantDelta.itemId,
					secondAssistantDelta.itemId,
				);
				...
				assert.equal(
					...
					String(firstAssistantDelta.itemId),
				);
				assert.equal(
					String(assistantCompletions[1]?.itemId),
					String(secondAssistantDelta.itemId),
				);
				assert.notEqual(
					String(assistantCompletions[0]?.itemId),
					String(assistantCompletions[1]?.itemId),
	...
	it.effect(
		"falls back to assistant payload text when stream deltas are absent",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"does not fabricate provider thread ids before first SDK session_id",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"bridges approval request/response lifecycle through canUseTool",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"acceptForSession returns session-scoped permission updates",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
				...
				const respondToNextRequest = Effect.gen(function* () {
					...
					if (
						requested._tag !== "Some" ||
						requested.value.type !== "request.opened"
					) {
						return;
					}
					const runtimeRequestId = requested.value.requestId;
				...
				const mcpPermission = (yield* Effect.promise(
					() => mcpPermissionPromise,
				)) as PermissionResult;
	...
	it.effect(
		"classifies Agent tools and read-only Claude tools correctly for approvals",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
				...
				if (
					agentRequested._tag !== "Some" ||
					agentRequested.value.type !== "request.opened"
				) {
					return;
				}
				assert.equal(
					agentRequested.value.payload.requestType,
					"dynamic_tool_call",
				);
				...
				if (
					grepRequested._tag !== "Some" ||
					grepRequested.value.type !== "request.opened"
				) {
					return;
				}
				assert.equal(
					grepRequested.value.payload.requestType,
					"file_read_approval",
				);
	...
	it.effect(
		"passes Claude resume ids without pinning a stale assistant checkpoint",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect("preserves durable resume ids across Claude resume hooks", () => {
		...
		return Effect.gen(function* () {
			...
			const runtimeEvents = Array.from(yield* Fiber.join(runtimeEventsFiber));
			const threadStartedEvents = runtimeEvents.filter(
	...
	it.effect(
		"uses an app-generated Claude session id for fresh sessions",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		...
		() => {
			...
			return Effect.gen(function* () {
				...
				if (
					firstCompleted._tag === "Some" &&
					firstCompleted.value.type === "turn.completed"
				) {
					assert.equal(
						String(firstCompleted.value.turnId),
				...
				if (
					secondCompleted._tag === "Some" &&
					secondCompleted.value.type === "turn.completed"
				) {
					assert.equal(
						String(secondCompleted.value.turnId),
						String(secondTurn.turnId),
					);
				}

				const threadBeforeRollback = yield* adapter.readThread(
					session.threadId,
				);
	...
	it.effect(
		"updates model on sendTurn for the adapter's bound custom instance id",
		() => {
	...
	it.effect(
		"re-sets the Claude model when the effective API model changes",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"sets plan permission mode on sendTurn when interactionMode is plan",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"does not call setPermissionMode when interactionMode is absent",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"captures ExitPlanMode as a proposed plan and denies auto-exit",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
				...
				assert.equal(
					proposedEvent.value.payload.planMarkdown,
					"# Ship it\n\n- one\n- two",
				);
				...
				assert.equal(
					deniedResult.message?.includes("captured your proposed plan"),
					true,
				);
			}).pipe(
				Effect.provideService(Random.Random, makeDeterministicRandomService()),
	...
	it.effect(
		"extracts proposed plans from assistant ExitPlanMode snapshots",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"routes Claude resume compaction through the shared user-input UI",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
				...
				const onUserDialog =
					harness.getLastCreateQueryInput()?.options.onUserDialog;
	...
	it.effect(
		"handles AskUserQuestion via user-input.requested/resolved lifecycle",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
				...
				assert.equal(
					requestedEvent.value.payload.questions[0]?.question,
					"Which framework?",
				);
				...
				assert.equal(
					requestedEvent.value.payload.questions[0]?.id,
					"Which framework?",
				);
				...
				assert.notEqual(
					v121Rendered,
					"",
					"Expected non-empty SDK 2.1.121 tool_result (#2388)",
				);
	...
	it.effect(
		"routes AskUserQuestion through user-input flow even in full-access mode",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
	...
	it.effect(
		"denies AskUserQuestion when the signal aborted before the listener registered",
		() => {
			const harness = makeHarness();
			return Effect.gen(function* () {
				const adapter = yield* ClaudeAdapter;
				...
				const canUseTool =
					harness.getLastCreateQueryInput()?.options.canUseTool;
				assert.equal(typeof canUseTool, "function");
				if (!canUseTool) {
					return;
				}
	...
	it.effect("writes provider-native observability records when enabled", () => {
		...
		return Effect.gen(function* () {
			...
			assert.equal(
				nativeEvents.some(
					(record) => String(record.event?.turnId) === String(turn.turnId),
				),
				true,
			);
			assert.equal(
