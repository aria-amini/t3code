...
const encodeUnknownJsonStringExit = Schema.encodeUnknownExit(
	Schema.fromJsonString(Schema.Unknown),
);
const decodeUnknownJsonStringExit = Schema.decodeUnknownExit(
	Schema.fromJsonString(Schema.Unknown),
);
...
function isClaudeInterruptedCause(
	cause: Cause.Cause<ProviderAdapterProcessError>,
): boolean {
	return (
		...
		cause.reasons.some(
			(reason) =>
				...
				isClaudeInterruptedMessage(toMessage(reason.error.cause, "")),
		)
	);
}
...
function describeClaudeUsageLimit(
	...
	const waitMs =
		resetsAtMs === undefined || !Number.isFinite(nowMs)
			? undefined
			: resetsAtMs - nowMs;
	const wait =
		waitMs !== undefined &&
		waitMs > 0 &&
		waitMs <= CLAUDE_USAGE_LIMIT_MAX_WAIT_MS
...
function makeClaudeTokenUsageSnapshot(input: {
	...
	const lastUsedTokens =
		...
		(maxTokens !== undefined
			...
			: activeTokens);
	const totalProcessedTokens = finiteNonNegativeInteger(
...
function normalizeClaudeTurnTokenUsage(
	...
	const cachedInputTokens = finiteNonNegativeInteger(
		usage.cache_read_input_tokens,
	);
	const cacheCreationTokens = finiteNonNegativeInteger(
	...
	const outputDetails = usage.output_tokens_details as
		...
		| undefined;
	const thinkingTokens = finiteNonNegativeInteger(
		outputDetails?.thinking_tokens,
	);
	const cachedInputContribution =
		usage.cache_read_input_tokens == null ? 0 : cachedInputTokens;
	...
	const inputTokens =
		...
		cacheCreationContribution !== undefined
			? uncachedInputTokens +
				cachedInputContribution +
				cacheCreationContribution
			: undefined;
	const hasKnownUsage =
		uncachedInputTokens !== undefined ||
...
function compactBoundaryTokenUsageSnapshot(
	...
	const { lastUsedTokens: _lastUsedTokens, ...snapshotWithoutBeforeTokens } =
		snapshot;
...
function normalizeClaudeTaskProgressTokenUsage(
	...
	const activeTokens =
		lastUsedTokens !== undefined
			? Math.max(totalTokens, lastUsedTokens)
			: totalTokens;
...
function readToolImagePath(
	...
	return path.length > 0 && isWorkspaceImagePreviewPath(path)
		? path
		: undefined;
}
...
function readClaudeToolUseResult(
	...
	const result = (message as { readonly tool_use_result?: unknown })
		.tool_use_result;
...
function applyClaudeTaskToolResult(
	...
	if (tool.toolName === "TaskCreate") {
		...
		const subject =
			readString(resultTask?.subject) ?? readString(tool.input.subject);
		if (!id || !subject) {
			return false;
		}
		tasks.set(id, {
...
function planStepsFromClaudeTasks(
	tasks: Map<string, ClaudeTaskState>,
): PlanStep[] {
...
function summarizeToolRequest(
	...
	const serialized =
		encodeJsonStringForDiagnostics(input) ?? "[unserializable input]";
...
function resultOutcome(
	...
	return {
		status: resultErrorsText(result).includes("cancel")
			? "cancelled"
			: "failed",
		errorMessage,
	};
...
function describeUnknownSdkMessage(kind: string, message: unknown): string {
	...
	return preview
		? `${kind} — ${preview}`
		: `${kind} (no displayable text content)`;
}
...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
	...
	const boundInstanceId =
		options?.instanceId ?? ProviderInstanceId.make("claudeAgent");
	...
	const emitClaudeTaskPlanUpdated = Effect.fn("emitClaudeTaskPlanUpdated")(
		function* (
			context: ClaudeSessionContext,
			input: {
				readonly toolUseId: string;
	...
	const completeTurn = Effect.fn("completeTurn")(function* (
		...
		const accumulatedTotalProcessedTokens = claudeTotalProcessedTokens(
			result?.usage,
		);
		...
		const hasResultUsageIteration =
			...
			lastClaudeUsageIteration(resultUsageRecord) !== undefined;
		const resultHasActiveUsage =
			resultUsageRecord !== undefined &&
			(hasResultUsageIteration ||
				claudeUsageInputTokens(resultUsageRecord) +
					claudeUsageOutputTokens(resultUsageRecord) >
					0);
	...
	const handleStreamEvent = Effect.fn("handleStreamEvent")(function* (
		...
		if (event.type === "content_block_start") {
			...
			const owningAgentId = agentIdForParentToolUse(
				context.taskAgents,
				parentToolUseId,
			);
	...
	const handleAssistantMessage = Effect.fn("handleAssistantMessage")(function* (
		...
		if (
			...
			assistantParentToolUseId !== undefined
		) {
			...
			const owningTaskId = agentIdForParentToolUse(
				context.taskAgents,
				assistantParentToolUseId,
			);
			const snapshotModel = trimmedString(message.message.model);
			const owningAgent = owningTaskId
				? context.taskAgents.get(owningTaskId)
				: undefined;
			if (snapshotModel) {
				if (owningAgent) {
	 ...
	 */
	const emitWorkflowMemberProgress = Effect.fn("emitWorkflowMemberProgress")(
		function* (
			context: ClaudeSessionContext,
			...
			for (const entry of progress.agents) {
				...
				if (
					context.workflowMemberFingerprints.get(memberTaskId) === fingerprint
				) {
					continue;
				}
	...
	const handleSystemMessage = Effect.fn("handleSystemMessage")(function* (
		...
		switch (message.subtype) {
			...
			case "task_started": {
				...
				const bufferedModel = toolUseId
					? context.pendingTaskModels.get(toolUseId)
					: undefined;
				if (toolUseId) {
				...
				const effort =
					...
					(typeof rawLaunchEffort === "number" &&
					Number.isFinite(rawLaunchEffort)
			...
			case "task_updated": {
				...
				const status =
					patch.status !== undefined
						? CLAUDE_TASK_PATCH_STATUS[patch.status]
						: undefined;
				if (
					status === "completed" ||
					status === "failed" ||
			...
			default: {
				...
				yield* emitRuntimeWarning(
					...
					describeUnknownSdkMessage(
						`Claude system message '${unknownMessage.subtype}'`,
						message,
					),
					message,
				);
		...
		switch (message.type) {
			...
			default: {
				...
				yield* emitRuntimeWarning(
					context,
					describeUnknownSdkMessage(
						`Claude SDK message '${unknownMessage.type}'`,
						message,
					),
					message,
				);
	...
	const startSession: ClaudeAdapterShape["startSession"] = Effect.fn(
		"startSession",
	)(function* (input) {
		...
		const newSessionId =
			existingResumeSessionId === undefined ? yield* randomUUIDv4 : undefined;
		...
		const handleResumeDialog = Effect.fn("handleResumeDialog")(function* (
			...
			callbackOptions: Parameters<
				NonNullable<ClaudeQueryOptions["onUserDialog"]>
			>[1],
		) {
			...
			const question = formatClaudeResumeCompactionQuestion({
				ageMinutes:
					finiteNonNegativeInteger(request.payload.sessionAgeMinutes) ?? 0,
				estimatedTokens:
					finiteNonNegativeInteger(request.payload.estimatedTokens) ?? 0,
			});
		...
		const modelSelection = selectedModel
			...
			: undefined;
		const caps = getClaudeCatalogModelCapabilities(
			modelCatalog,
		...
		const rawEffort = getModelSelectionStringOptionValue(
			modelSelection,
			"effort",
		);
		const effort =
		...
		yield* Effect.annotateCurrentSpan({
			...
			"claude.query.allow_dangerously_skip_permissions":
				permissionMode === "bypassPermissions",
			...
			"claude.query.settings_json":
				encodeJsonStringForDiagnostics(settings) ?? "",
			"claude.query.extra_args_json":
				encodeJsonStringForDiagnostics(extraArgs) ?? "",
	...
	const sendTurn: ClaudeAdapterShape["sendTurn"] = Effect.fn("sendTurn")(
		function* (input) {
			...
			const turnId =
				steeringTurnState?.turnId ?? TurnId.make(yield* randomUUIDv4);
	...
	const interruptTurn: ClaudeAdapterShape["interruptTurn"] = Effect.fn(
		"interruptTurn",
	...
	const rollbackThread: ClaudeAdapterShape["rollbackThread"] = Effect.fn(
		"rollbackThread",
	...
	const respondToRequest: ClaudeAdapterShape["respondToRequest"] = Effect.fn(
		"respondToRequest",
	...
	const stopSession: ClaudeAdapterShape["stopSession"] = Effect.fn(
		"stopSession",
	)(function* (threadId) {
		const context = yield* requireSession(threadId);
		yield* stopSessionInternal(context, {
