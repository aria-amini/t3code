...
class FakeCodexRuntime implements CodexSessionRuntimeShape {
	private readonly eventQueue = Effect.runSync(
		Queue.unbounded<ProviderEvent>(),
	);
...
validationLayer("CodexAdapterLive validation", (it) => {
	it.effect(
		"returns validation error for non-codex provider on startSession",
		() =>
			Effect.gen(function* () {
...
sessionErrorLayer("CodexAdapterLive session errors", (it) => {
	it.effect(
		"maps missing adapter sessions to ProviderAdapterSessionNotFoundError",
		() =>
			Effect.gen(function* () {
				const adapter = yield* CodexAdapter;
				const result = yield* adapter
	...
	it.effect(
		"maps codex model options for the adapter's bound custom instance id",
		() => {
...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
	...
	it.effect(
		"does not charge a late prior-turn update to the next Codex turn",
		() =>
			Effect.gen(function* () {
				const { adapter, runtime } = yield* startLifecycleRuntime();
				const completedFiber = yield* adapter.streamEvents.pipe(
	...
	it.effect(
		"counts the last response when Codex resets its running total mid-turn",
		() =>
			Effect.gen(function* () {
				const { adapter, runtime } = yield* startLifecycleRuntime();
				const completedFiber = yield* adapter.streamEvents.pipe(
	...
	it.effect(
		"uses the last response usage when no prior Codex total exists",
		() =>
			Effect.gen(function* () {
				const adapter = yield* CodexAdapter;
				yield* adapter.startSession({
				...
				yield* runtime.emit(
					codexTurnEvent("turn/started", "turn-after-resume"),
				);
				yield* runtime.emit(
					codexTokenUsageEvent({
						id: "evt-after-resume",
				...
				yield* runtime.emit(
					codexTurnEvent("turn/completed", "turn-after-resume"),
				);

				const firstCompletions = Array.from(
					yield* Fiber.join(firstCompletionsFiber),
				);
				...
				yield* runtime.emit(
					codexTurnEvent("turn/started", "turn-after-rollback"),
				);
				yield* runtime.emit(
					codexTokenUsageEvent({
				...
				yield* runtime.emit(
					codexTurnEvent("turn/completed", "turn-after-rollback"),
				);

				const rollbackCompletion = yield* Fiber.join(rollbackCompletionFiber);
				const completions = [
					...firstCompletions,
					...(rollbackCompletion._tag === "Some"
						? [rollbackCompletion.value]
						: []),
				];
				NodeAssert.deepStrictEqual(
	...
	it.effect("carries child model metadata through every task event", () =>
		Effect.gen(function* () {
			...
			const eventsFiber = yield* Stream.runCollect(
				Stream.take(adapter.streamEvents, 10),
			...
			const blankMetadataPayload = events[9]?.payload as Record<
				string,
				unknown
			>;
	...
	it.effect(
		"does not reactivate an idle child after a parent interaction",
		() =>
			Effect.gen(function* () {
				const { adapter, runtime } = yield* startLifecycleRuntime();
				const eventsFiber = yield* Stream.runCollect(
					Stream.take(adapter.streamEvents, 3),
				).pipe(Effect.forkChild);
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				NodeAssert.equal(
					firstEvent.value.payload.itemType,
					"assistant_message",
				);
			}),
	);
	...
	it.effect("labels MCP lifecycle entries with server and tool names", () =>
		Effect.gen(function* () {
			...
			NodeAssert.equal(firstEvent._tag, "Some");
			if (
				firstEvent._tag !== "Some" ||
				firstEvent.value.type !== "item.completed"
			) {
				return;
			}
			NodeAssert.equal(firstEvent.value.payload.itemType, "mcp_tool_call");
			NodeAssert.equal(
				firstEvent.value.payload.title,
	...
	it.effect(
		"presents browser and computer-use calls with Codex-style titles and sources",
		() =>
			Effect.gen(function* () {
				const { adapter, runtime } = yield* startLifecycleRuntime();
				const eventsFiber = yield* Stream.runCollect(
										...
										openTabs: [
											{
												...
												faviconUrlDark:
													"https://www.mathworks.com/favicon-dark.ico",
				...
				NodeAssert.deepStrictEqual(
					events.map((event) => ({
						...
						toolSurface:
							"toolSurface" in event.payload
								? event.payload.toolSurface
								: undefined,
						toolIcon:
							"toolIcon" in event.payload ? event.payload.toolIcon : undefined,
						toolSource:
							"toolSource" in event.payload
	...
	it.effect(
		"preserves failed and declined outcomes on completed tool items",
		() =>
			Effect.gen(function* () {
				const { adapter, runtime } = yield* startLifecycleRuntime();
	...
	it.effect(
		"maps realtime started notifications with upstream realtime session ids",
		() =>
			Effect.gen(function* () {
				const { adapter, runtime } = yield* startLifecycleRuntime();
				const firstEventFiber = yield* Stream.runHead(
				...
				NodeAssert.equal(firstEvent.value.threadId, "thread-1");
				NodeAssert.equal(
					firstEvent.value.payload.realtimeSessionId,
					"realtime-session-1",
				);
			}),
	);
	...
	it.effect(
		"names the edited files in an apply-patch approval without a reason",
		() =>
			Effect.gen(function* () {
				const { adapter, runtime } = yield* startLifecycleRuntime();
				const firstEventFiber = yield* Stream.runHead(
				...
				if (
					firstEvent._tag !== "Some" ||
					firstEvent.value.type !== "request.opened"
				) {
					return;
				}
				NodeAssert.equal(
	...
	it.effect("keeps the reason when an apply-patch approval carries one", () =>
		Effect.gen(function* () {
			...
			NodeAssert.equal(
				firstEvent.value.payload.detail,
				"Needs to rewrite the changelog",
			);
		}),
	);

	it.effect(
		"falls back to the grant root for a file-change approval without a reason",
		() =>
			Effect.gen(function* () {
				const { adapter, runtime } = yield* startLifecycleRuntime();
				const firstEventFiber = yield* Stream.runHead(
	...
	it.effect(
		"caps the described files in an oversized apply-patch approval",
		() =>
			Effect.gen(function* () {
				const { adapter, runtime } = yield* startLifecycleRuntime();
				const firstEventFiber = yield* Stream.runHead(
	...
	it.effect(
		"leaves an apply-patch approval without changes or a reason undetailed",
		() =>
			Effect.gen(function* () {
				const { adapter, runtime } = yield* startLifecycleRuntime();
				const firstEventFiber = yield* Stream.runHead(
	...
	it.effect(
		"preserves MCP elicitation type when an app access request resolves",
		() =>
			Effect.gen(function* () {
				const { adapter, runtime } = yield* startLifecycleRuntime();
				const firstEventFiber = yield* Stream.runHead(
				...
				if (
					firstEvent._tag !== "Some" ||
					firstEvent.value.type !== "request.resolved"
				) {
					return;
				}
				NodeAssert.equal(
	...
	it.effect("maps async agent questions without ending the turn", () =>
		Effect.gen(function* () {
			...
			NodeAssert.equal(
				events[0]?.requestId,
				"codex-async:thread-1:async-question-1",
			);
	...
	it.effect(
		"unwraps Codex token usage payloads for context window events",
		() =>
			Effect.gen(function* () {
				const { adapter, runtime } = yield* startLifecycleRuntime();
				const firstEventFiber = yield* Stream.runHead(
	...
	// production.
	it.effect(
		"keeps consuming runtime events after the startSession fiber completes",
		() =>
			Effect.gen(function* () {
				const adapter = yield* CodexAdapter;
...
scopedLifecycleLayer("CodexAdapterLive scoped lifecycle", (it) => {
	it.effect("closes the externally owned session scope on stopSession", () =>
		Effect.gen(function* () {
			...
			NodeAssert.deepStrictEqual(
				scopedLifecycleRuntimeFactory.releasedThreadIds,
				[asThreadId("thread-stop")],
			);
			NodeAssert.equal(
				yield* adapter.hasSession(asThreadId("thread-stop")),
				false,
			);
		}),
	);
...
scopedFailureLayer("CodexAdapterLive scoped startup failure", (it) => {
	it.effect(
		"closes the externally owned session scope when startSession fails",
		() =>
			Effect.gen(function* () {
				...
				NodeAssert.deepStrictEqual(
					scopedFailureRuntimeFactory.releasedThreadIds,
					[asThreadId("thread-fail")],
				);
				NodeAssert.equal(
					yield* adapter.hasSession(asThreadId("thread-fail")),
					false,
				);
			}),
	);
...
it.effect("flushes managed native logs when the adapter layer shuts down", () =>
	Effect.gen(function* () {
		...
		try {
			...
			const adapter = yield* Effect.service(CodexAdapter).pipe(
			...
			const threadLogPath = NodePath.join(
				tempDir,
				"provider-native.thread-logger.log",
			);
				...
				...(input.primary
					? {
							primary: {
								...
								resetsAt:
									USAGE_LIMIT_NOW_SECONDS + input.primary.resetsInSeconds,
				...
				...(input.secondary
					? {
							secondary: {
								...
								resetsAt:
									USAGE_LIMIT_NOW_SECONDS + input.secondary.resetsInSeconds,
...
usageLimitLayer("CodexAdapterLive usage limits", (it) => {
	it.effect(
		"names the exhausted window and the workspace's missing credits",
		() =>
			Effect.gen(function* () {
				...
				yield* runtime.emit(
					codexUsageLimitTurnFailed("evt-limit-turn-2", "turn-limit-2"),
				);
	...
	it.effect(
		"falls back to the short message without a rate-limit snapshot",
		() =>
			Effect.gen(function* () {
				const { adapter, runtime } = yield* startUsageLimitRuntime();
				const eventsFiber = yield* adapter.streamEvents.pipe(
