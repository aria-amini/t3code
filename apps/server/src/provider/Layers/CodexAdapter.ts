...
const isCodexAppServerTransportError = Schema.is(
...
function browserNativeAppReference(
	name: string,
...
function themedLogoIcon(
	...
	for (const record of records) {
		...
		if (!logoUrl) continue;
		const logoUrlDark = normalizedImageUrl(
			record?.logoUrlDark ?? record?.logoDarkUrl,
		);
		return {
			_tag: "themed-logo",
			logoUrl,
...
function mcpToolPresentation(
	...
	if (surface?.kind === "browserUse") {
		...
		const faviconUrlDark = normalizedImageUrl(
			selectedPage?.record?.faviconUrlDark ??
				selectedPage?.record?.favIconUrlDark,
		);
		const name =
		...
		const sourceIcon =
			sourceLogo ??
			(nativeBrowserIcon
				? ({ _tag: "native-app", app: nativeBrowserIcon } as const)
				: undefined);
	...
	if (surface?.kind === "computerUse") {
		...
		const name =
			...
			"Computer Use";
		const sourceIcon =
			sourceLogo ?? (app ? ({ _tag: "native-app", app } as const) : undefined);
		const sourceKey = app
			? app._tag === "app-id"
...
function isFatalCodexProcessStderrMessage(message: string): boolean {
	...
	return FATAL_CODEX_STDERR_SNIPPETS.some((snippet) =>
...
function toTurnStatus(
	value:
		| EffectCodexSchema.V2TurnCompletedNotification["turn"]["status"]
		| "cancelled",
...
function computerUseToolTitle(
	...
	const tool = normalizeItemType(normalizedMcpToolName(item.tool)).replace(
		/ /gu,
		"_",
	);
	...
	const args = asUnknownRecord(item.arguments);
	const appName =
		(presentation.toolSource?.kind === "computer" &&
		presentation.toolSource.name !== "Computer Use"
			? presentation.toolSource.name
			: undefined) ??
		normalizedDisplayName(args?.appName) ??
		normalizedDisplayName(args?.application) ??
		normalizedDisplayName(typeof args?.app === "string" ? args.app : undefined);
	const withApp = (label: string) =>
	...
	switch (tool) {
		...
		case "scroll": {
			...
			return withApp(
				`${inProgress ? "Scrolling" : "Scrolled"}${direction ? ` ${direction}` : ""}`,
			);
		}
		case "set_value":
...
function itemTitle(
	...
	if (itemType === "mcp_tool_call" && item?.type === "mcpToolCall") {
		if (normalizedMcpToolName(item.tool) === "js") {
			const intentTitle = normalizeMcpIntentTitle(
				asUnknownRecord(item.arguments)?.title,
			);
...
function itemDetail(
	...
	for (const candidate of candidates) {
		const trimmed =
			typeof candidate === "string" ? trimText(candidate) : undefined;
		if (!trimmed) continue;
		return trimmed;
	}
	return undefined;
...
function describeFileChanges(
	fileChanges:
		| EffectCodexSchema.ServerRequest__ApplyPatchApprovalParams["fileChanges"]
		| undefined,
): string | undefined {
...
function toRequestTypeFromKind(
...
function asRuntimeItemId(
	itemId: ProviderEvent["itemId"] & string,
): RuntimeItemId {
...
function mapCollabAgentEvent(
	...
	const payload =
		typeof event.payload === "object" && event.payload !== null
			...
			: undefined;
	const agentThreadId =
		typeof payload?.agentThreadId === "string" ? payload.agentThreadId : "";
	...
	const agentPath =
		typeof payload.agentPath === "string" ? payload.agentPath : undefined;
	const pathLeaf = agentPath
		?.split("/")
	...
	switch (event.method) {
		...
		case "collabAgent/activity": {
			const activityKind =
				typeof payload.activityKind === "string" ? payload.activityKind : "";
...
function mapToRuntimeEvents(
	...
	if (event.kind === "request") {
		if (event.method === "item/tool/requestUserInput") {
			const payload =
				...
				readPayload(
					EffectCodexSchema.ToolRequestUserInputParams,
					event.payload,
				);
			const questions = payload
				? toUserInputQuestions(payload.questions)
				: undefined;
			if (!questions) {
				return [];
			}
		...
		const elicitation =
			event.method === "mcpServer/elicitation/request"
				...
				: undefined;
		const elicitationApproval = elicitation
			? describeMcpElicitation(elicitation)
			: undefined;
		const detail = (() => {
			switch (event.method) {
				case "item/commandExecution/requestApproval": {
	...
	if (event.method === "thread/name/updated") {
		const payload = readPayload(
			EffectCodexSchema.V2ThreadNameUpdatedNotification,
			event.payload,
		);
		return [
			{
				...
				...runtimeEventBase(event, canonicalThreadId),
				payload: {
					...(trimText(payload?.threadName)
						? { name: trimText(payload?.threadName) }
						: {}),
					...(payload
						? {
								metadata: {
	...
	if (event.method === "thread/tokenUsage/updated") {
		...
		const normalizedUsage = payload
			? normalizeCodexTokenUsage(payload.tokenUsage)
			: undefined;
		if (!normalizedUsage) {
			return [];
		}
	...
	if (event.method === "turn/diff/updated") {
		const payload = readPayload(
			EffectCodexSchema.V2TurnDiffUpdatedNotification,
			event.payload,
		);
		if (!payload) {
			return [];
			...
			return [
				{
					...
					requestId: RuntimeRequestId.make(
						`codex-async:${canonicalThreadId}:${item.id}`,
					),
	...
	if (event.method === "item/plan/delta") {
		const payload = readPayload(
			EffectCodexSchema.V2PlanDeltaNotification,
			event.payload,
		);
	...
	if (event.method === "account/rateLimits/updated") {
		...
		const limits = payload
			? codexRateLimitsToUpdate(payload.rateLimits)
			: undefined;
		if (!limits) {
			return [];
		}
	...
	if (event.method === "thread/realtime/error") {
		const payload = readPayload(
			EffectCodexSchema.V2ThreadRealtimeErrorNotification,
			event.payload,
		);
...
export const makeCodexAdapter = Effect.fn("makeCodexAdapter")(function* (
				...
				const eventFiber = yield* Stream.runForEach(runtime.events, (event) =>
					Effect.gen(function* () {
						...
						if (event.method === "turn/started" && event.turnId) {
							...
							if (
								isCollabSpawn &&
								event.turnId === turnTokenUsage.activeTurnId
							) {
						...
						if (event.method === "account/rateLimits/updated") {
							...
							if (limitsPayload) {
								rateLimits = mergeCodexRateLimits(
									rateLimits,
									limitsPayload.rateLimits,
								);
							}
						...
						const mappedEvents = mapToRuntimeEvents(event, event.threadId).map(
							(runtimeEvent) => {
								if (
	...
	const requireSession = Effect.fn("requireSession")(function* (
		threadId: ThreadId,
	) {
	...
	const respondToRequest: CodexAdapterShape["respondToRequest"] = (
		...
		requireSession(threadId).pipe(
			Effect.flatMap((session) =>
				session.runtime.respondToRequest(requestId, decision),
			),
			Effect.mapError((cause) =>
				cause._tag === "ProviderAdapterSessionNotFoundError"
					? cause
	...
	const respondToUserInput: CodexAdapterShape["respondToUserInput"] = (
		...
		requireSession(threadId).pipe(
			Effect.flatMap((session) =>
				session.runtime.respondToUserInput(requestId, answers),
			),
			Effect.mapError((cause) =>
				cause._tag === "ProviderAdapterSessionNotFoundError"
					? cause
