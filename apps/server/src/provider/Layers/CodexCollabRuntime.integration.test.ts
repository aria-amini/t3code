...
function buildScript() {
	...
	const extras = [
		...
		// parent path (approval correlation cleanup), not be swallowed.
		{
			...
			params: { threadId: CHILD_A, requestId: "req-1" },
		},
	];
	return {
		rootThreadId: ROOT,
...
function capturedStartedActivity(childId = CHILD_A) {
	const captured = wireFixture.notifications.find((entry) => {
		const item = (entry.params as { item?: { type?: string; kind?: string } })
			.item;
...
const scriptPath = NodePath.join(
	import.meta.dirname,
	"../testFixtures/.collab-script.json",
);
...
describe("CodexSessionRuntime collab integration", () => {
	it.effect(
		"looks up child model metadata once after activity registration",
		() =>
			Effect.gen(function* () {
				const script = {
					rootThreadId: ROOT,
					...
					notifications: [
						...
						capturedStartedActivity(),
						{
							...
							params: {
								...
								item: {
									...capturedStartedActivity(CHILD_B).params.item,
									kind: "interacted",
								},
							},
						},
	...
	it.effect(
		"keeps child settings and reroutes newer than the resume snapshot",
		() =>
			Effect.gen(function* () {
				...
				const started = events.find(
					(event) => event.method === "collabAgent/started",
				);
				...
				const childStatus = events.find(
					(event) => event.method === "collabAgent/statusChanged",
				);
	...
	it.effect(
		"replays the captured fan-out into synthetic agent events without child leaks",
		() =>
			Effect.gen(function* () {
				// @effect-diagnostics-next-line preferSchemaOverJson:off
				...
				assert.isDefined(
					childTurnCompleted,
					"child A's turn completion becomes an agent event",
				);
				...
				const leaked = events.filter((event) => {
					...
					const addressedToChild =
						payload?.threadId === CHILD_A || payload?.threadId === CHILD_B;
					return (
						addressedToChild && (event.method?.startsWith("thread/") ?? false)
					);
				});
				assert.deepEqual(
	...
	it.live(
		"Stop interrupts every live child regardless of registration timing",
		() =>
			Effect.gen(function* () {
				...
				const isRegistration = (
					...
					const item = (
						entry.params as { item?: { type?: string; agentThreadId?: string } }
					).item;
					return (
						item?.type === "subAgentActivity" && item.agentThreadId === child
					);
				};
				const turnStartedA = byIndex.find((entry) =>
					isTurnStarted(entry, CHILD_A),
				);
				const turnStartedB = byIndex.find((entry) =>
					isTurnStarted(entry, CHILD_B),
				);
				...
				const rootThreadStarted = byIndex.find(
					(entry) => entry.method === "thread/started",
				);
				...
				const memoryTurnStarted = {
					...
					params: {
						...
						threadId: MEMORY,
						turn: {
							...turnStartedA.params.turn,
							id: "memory-consolidation-turn",
						},
					},
				};
				const script = {
				...
				assert.isTrue(
					childBStarted._tag === "Some",
					"child B turnStarted never arrived",
				);
				...
				const parseInterruptLine = (line: string) =>
					JSON.parse(line) as { threadId?: string };
				...
				assert.isTrue(
					interruptedThreads.has(CHILD_B),
					"registered child B must be interrupted",
				);
				assert.isTrue(
				...
				assert.isTrue(
					interruptedThreads.has(ROOT),
					"parent turn must be interrupted last",
				);
	...
	it.live(
		"Stop targets the active turn when Codex has accepted a queued follow-up",
		() =>
			Effect.gen(function* () {
