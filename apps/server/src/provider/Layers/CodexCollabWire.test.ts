...
describe("codex multi-agent wire capture", () => {
	...
	it("contains a /root self-activity emitted from a CHILD thread", () => {
		...
		const rootSelfActivity = subAgentActivityItems().find(
			(item) => item.agentPath === "/root",
		);
		assert.isDefined(
			rootSelfActivity,
			"capture should contain a /root self-activity",
		);
	...
	it("routes every captured child method to a defined disposition", () => {
		...
		for (const method of childMethods) {
			...
			assert.equal(
				route,
				"agent-event",
				`${method} should map to an agent event`,
			);
		}
	});
});
...
describe("routeCodexChildNotification", () => {
	...
	it("sends parent-owned and UNKNOWN methods to the parent path", () => {
		...
		assert.equal(
			routeCodexChildNotification("serverRequest/resolved"),
			"parent",
		);
		assert.equal(
			routeCodexChildNotification("thread/somethingBrandNew"),
			"parent",
		);
		assert.equal(
			routeCodexChildNotification("account/rateLimits/updated"),
			"parent",
		);
	});
});
