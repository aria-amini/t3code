...
const isCodexAppServerSpawnError = Schema.is(
...
export function mapCodexModelCapabilities(
	...
	const defaultReasoning = reasoningOptions.find(
			...
			options: [
				{
					...
					label: "Standard",
					...(defaultServiceTier === DEFAULT_SERVICE_TIER_ID
						? { isDefault: true }
						: {}),
				},
...
function appendCustomCodexModels(
	...
	const fallbackCapabilities =
		models.find((model) => model.capabilities)?.capabilities ?? null;
 ...
 */
export const withCodexAppServerClient = Effect.fn("withCodexAppServerClient")(
	function* (input: {
...
const probeCodexAppServerProvider = Effect.fn("probeCodexAppServerProvider")(
	function* (input: {
		readonly binaryPath: string;
		...
		const [skillsResponse, models, rateLimits] = yield* Effect.all(
			[
				...
				client.request("account/rateLimits/read", undefined).pipe(
					...
					Effect.catch((error) =>
						Effect.logDebug("Codex rate-limit read failed.", {
							cause: error,
						}).pipe(
...
export const probeCodexSkillsForCwd = Effect.fn("probeCodexSkillsForCwd")(
	function* (input: {
		readonly binaryPath: string;
		readonly homePath?: string;
		readonly launchArgs?: string;
			...
			snapshot.account.account?.type === "apiKey"
				...
				: snapshot.rateLimits === undefined || "failure" in snapshot.rateLimits
					? makeUnavailableUsageLimits({
							...
							reason: "probeFailed",
							...(snapshot.rateLimits
								? { message: snapshot.rateLimits.failure }
								: {}),
						})
					: codexRateLimitsToLimits({
