...
const isCodexAppServerRequestError = Schema.is(
...
describe("buildTurnStartParams", () => {
	it("keeps invalid turn values only in the schema cause", () => {
		...
		NodeAssert.doesNotMatch(
			JSON.stringify(directDiagnostics),
			new RegExp(secret),
		);
	});
...
describe("Codex MCP elicitation approvals", () => {
	...
	it("extracts the app name from a Computer Use request without metadata", () => {
		...
		NodeAssert.equal(
			describeMcpElicitation(requestWithoutMetadata).appName,
			"Safari",
		);
	});
	...
	it("returns session-scoped approval in the MCP response", () => {
		NodeAssert.deepStrictEqual(
			toMcpElicitationResponse(request, "acceptForSession"),
			{
				action: "accept",
	...
	it("returns persistent approval in the MCP response", () => {
		NodeAssert.deepStrictEqual(
			toMcpElicitationResponse(request, "acceptAlways"),
			{
				action: "accept",
	...
	it("supports boolean permanent-approval fields", () => {
		...
		NodeAssert.deepStrictEqual(
			toMcpElicitationResponse(booleanRequest, "acceptAlways"),
			{
				action: "accept",
				_meta: { persist: "always" },
				content: { always: true },
	...
	it("preserves valid nullable MCP form fields and persistence choices", () => {
		...
		NodeAssert.deepStrictEqual(
			toMcpElicitationResponse(nullableRequest, "acceptAlways"),
			{
				action: "accept",
				_meta: { persist: "always" },
				content: { approval: "always" },
	...
	it("declines required form fields that an approval prompt cannot collect", () => {
		...
		NodeAssert.deepStrictEqual(
			toMcpElicitationResponse(inputRequest, "accept"),
			{
				action: "decline",
			},
		);
	...
	it("omits persistence choices that cannot satisfy required form fields", () => {
		...
		NodeAssert.deepStrictEqual(
			describeMcpElicitation(onceOnlyRequest).options,
			[
				{ decision: "cancel", label: "Cancel" },
				{ decision: "decline", label: "Decline" },
				{ decision: "accept", label: "Approve" },
...
describe("buildCodexDeveloperInstructions", () => {
	...
	it("includes runtime info alongside plan mode instructions", () => {
		...
		NodeAssert.match(
			instructions,
			/as gpt-5\.3-codex with medium reasoning effort/,
		);
	});
...
describe("makeMemoryConsolidationNotificationFilter", () => {
	it("suppresses memory consolidation without hiding other Codex subagents", () => {
		...
		for (const source of [
			...
			NodeAssert.equal(
				shouldSuppress(
					makeThreadStartedNotification("visible-subagent", source),
				),
				false,
			);
		}
...
describe("codexSessionAppServerArgs", () => {
	it("keeps the app-server subcommand when explicit args are provided", () => {
		NodeAssert.deepStrictEqual(
			codexSessionAppServerArgs(["-c", "model=gpt-5"], undefined),
			["app-server", "-c", "model=gpt-5"],
		);
	});
...
describe("openCodexThread", () => {
	it.effect(
		"resumes metadata when historical turns contain unknown error values",
		() =>
			Effect.gen(function* () {
				...
				const opened = yield* openCodexThread({
					client: {
						request: () =>
							Effect.die("A valid resumed thread must not start fresh"),
						raw: {
	...
	it.effect(
		"rejects malformed required resume metadata without starting a fresh thread",
		() =>
			Effect.gen(function* () {
				for (const invalidMetadata of [
					...
					const error = yield* openCodexThread({
						client: {
							request: () =>
								Effect.die(
									"Invalid resume metadata must not start a fresh thread",
								),
							raw: {
								request: () =>
									Effect.succeed({
										...makeThreadOpenResponse("saved-thread"),
										...invalidMetadata,
									}),
							},
						},
	...
	it.effect("propagates non-recoverable resume failures", () =>
		Effect.gen(function* () {
			const client = {
				request: () =>
					Effect.die(
						"Non-recoverable resume failures must not start a fresh thread",
					),
				raw: {
					request: () =>
						Effect.fail(
