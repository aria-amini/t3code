...
const decodeV2TurnStartResponse = Schema.decodeUnknownEffect(
...
export function hasConfiguredMcpServer(
	...
	return (
		appServerArgs?.some((argument) => argument.includes("mcp_servers.")) ===
		true
	);
}
...
const McpElicitationForm = Schema.Struct({
	properties: Schema.optionalKey(
		Schema.Record(Schema.String, McpElicitationFormField),
	),
...
const CodexTurnStartParamsWithCollaborationMode =
	EffectCodexSchema.V2TurnStartParams.pipe(
		Schema.fieldsAssign({
...
const decodeCodexTurnStartParamsWithCollaborationMode =
	Schema.decodeUnknownEffect(CodexTurnStartParamsWithCollaborationMode);
const CodexChildResumeMetadata = Schema.Struct({
	thread: Schema.Struct({ id: Schema.String }),
	model: Schema.String,
	reasoningEffort: Schema.optionalKey(Schema.NullOr(Schema.String)),
});
const decodeCodexChildResumeMetadata = Schema.decodeUnknownEffect(
	CodexChildResumeMetadata,
);
...
type CodexServiceTier = NonNullable<
...
export interface CodexSessionRuntimeShape {
	...
	readonly uploadFeedback: (
		...
		EffectCodexSchema.V2FeedbackUploadResponse,
		CodexSessionRuntimeError
	>;
...
function mcpElicitationFormFields(
	payload: EffectCodexSchema.McpServerElicitationRequestParams,
) {
	if (
		payload.mode === "url" ||
		!isMcpElicitationForm(payload.requestedSchema)
	) {
		return undefined;
	}
...
function mcpElicitationFieldOptions(
...
export function describeMcpElicitation(
	...
	const metadata = isMcpElicitationMetadata(payload._meta)
		? payload._meta
		: undefined;
	const appName =
		metadata?.app_name ??
		...
		payload.serverName;
	const persistenceOptions = new Map<
		McpElicitationPersistenceDecision,
		string
	>();
	const persist = metadata?.persist;
	for (const value of typeof persist === "string"
		? [persist]
		: (persist ?? [])) {
				...
				? [
						{
							...
							label:
								persistenceOptions.get("acceptForSession") ||
...
function makeCodexServerNotification<
	M extends CodexRpc.ServerNotificationMethod,
>(
	method: M,
...
function classifyCodexStderrLine(
	rawLine: string,
...
const decodeCodexThreadResumeMetadata = Schema.decodeUnknownEffect(
	CodexThreadResumeMetadata,
);
...
function readThreadSpawnSource(thread: { readonly source: unknown }):
	...
	return {
		...
		parentThreadId:
			typeof record.parent_thread_id === "string"
...
> {
	...
	return Effect.fail(
		new CodexSessionRuntimeInvalidUserInputAnswersError({ questionId }),
	);
}
...
function updateSession(
	...
	updates:
		...
		| ((session: ProviderSession) => Partial<ProviderSession>),
): Effect.Effect<void> {
...
> =>
	Effect.gen(function* () {
		...
		const pendingApprovalsRef = yield* Ref.make(
			new Map<ApprovalRequestId, PendingApproval>(),
		);
		const approvalCorrelationsRef = yield* Ref.make(
			new Map<string, ApprovalCorrelation>(),
		);
		const pendingUserInputsRef = yield* Ref.make(
			new Map<ApprovalRequestId, PendingUserInput>(),
		);
		const collabReceiverTurnsRef = yield* Ref.make(new Map<string, TurnId>());
		const collabChildAgentsRef = yield* Ref.make(
			new Map<string, CollabChildAgentState>(),
		);
		const collabChildMetadataRef = yield* Ref.make(
			new Map<string, CollabChildMetadataState>(),
		);
		/** Child provider-thread id → its currently running provider turn id. */
		const collabChildLiveTurnsRef = yield* Ref.make(new Map<string, string>());
		const suppressMemoryConsolidationNotification =
		...
		const resolvedHomePath = options.homePath
			? expandHomePath(options.homePath)
			: undefined;
		const env = {
			...options.environment,
		...
		const serverNotifications =
			yield* Queue.unbounded<CodexServerNotification>();
		...
		const updateCollabChildMetadata = (
			...
			Ref.modify(collabChildMetadataRef, (current) => {
				...
				const model =
					update.model && (overwriteKnown || !previous.model)
						? update.model
						: previous.model;
				const effort =
					update.effort && (overwriteKnown || !previous.effort)
						? update.effort
						: previous.effort;
		...
		const emitCollabChildMetadataUpdated = Effect.fn(
			...
			const metadata = (yield* Ref.get(collabChildMetadataRef)).get(
				agentThreadId,
			);
				...
				.pipe(
					...
					Effect.flatMap((response) =>
						Effect.gen(function* () {
							...
							const child = (yield* Ref.get(collabChildAgentsRef)).get(
								agentThreadId,
							);
							const metadata = (yield* Ref.get(collabChildMetadataRef)).get(
								agentThreadId,
			...
			Ref.get(pendingApprovalsRef).pipe(
				Effect.flatMap((pendingApprovals) =>
					Effect.forEach(
						...
						(pendingApproval) =>
							Deferred.succeed(pendingApproval.decision, decision).pipe(
								Effect.ignore,
							),
						{ discard: true },
					),
			...
			Ref.get(pendingUserInputsRef).pipe(
				Effect.flatMap((pendingUserInputs) =>
					Effect.forEach(
						...
						(pendingUserInput) =>
							Deferred.succeed(pendingUserInput.answers, answers).pipe(
								Effect.ignore,
							),
						{ discard: true },
					),
		...
		const interceptCollabChildNotification = (
			...
			Effect.gen(function* () {
				...
				if (notification.method === "thread/started") {
					...
					const rootProviderThreadId = currentProviderThreadId(
					...
					const existingChild = (yield* Ref.get(collabChildAgentsRef)).get(
						thread.id,
					);
					...
					const state: CollabChildAgentState = {
						...
						nickname:
							spawn.nickname ?? thread.agentNickname ?? existingChild?.nickname,
				...
				if (
					...
					const registeredChild = (yield* Ref.get(collabChildAgentsRef)).get(
						item.agentThreadId,
					);
					const metadata = (yield* Ref.get(collabChildMetadataRef)).get(
						item.agentThreadId,
					...
					yield* emitEvent({
						...
						...(registeredChild?.spawnTurnId
							? { turnId: registeredChild.spawnTurnId }
							: {}),
						payload: {
							...(registeredChild
				...
				const interceptRootId = currentProviderThreadId(
					yield* Ref.get(sessionRef),
				);
				...
				if (
					...
					if (
						changed &&
						(yield* Ref.get(collabChildAgentsRef)).has(providerConversationId)
					) {
				...
				switch (notification.method) {
					case "turn/started": {
						...
						const childTurnId =
							typeof (notification.params as { turn?: { id?: unknown } }).turn
								?.id === "string"
								? ((notification.params as { turn: { id: string } }).turn
										.id as string)
								: undefined;
						if (childTurnId) {
					...
					case "error": {
						...
						// path.
						const willRetry =
							(notification.params as { willRetry?: boolean }).willRetry ===
							true;
						if (willRetry) {
							return true;
						}
		...
		const handleRawNotification = (notification: CodexServerNotification) =>
			Effect.gen(function* () {
				...
				const suppressRootId = currentProviderThreadId(
					yield* Ref.get(sessionRef),
				);
		...
		const currentSessionProviderThreadId = Effect.map(
			Ref.get(sessionRef),
			...
			currentSessionProviderThreadId.pipe(
				Effect.flatMap((providerThreadId) => {
					...
					if (
						providerThreadId &&
						payloadThreadId &&
						payloadThreadId !== providerThreadId
					) {
						return Effect.void;
					}
		...
		yield* client.handleServerRequest(
			...
			(payload) =>
				Effect.gen(function* () {
					...
					return {
						decision:
							resolved === "acceptAlways" ? "acceptForSession" : resolved,
		...
		yield* client.handleServerRequest(
			...
			(payload) =>
				Effect.gen(function* () {
					...
					const requestId = ApprovalRequestId.make(
						yield* randomUUIDv4("mcp-elicitation-request"),
					);
					const turnId = payload.turnId
						? TurnId.make(payload.turnId)
						: (yield* Ref.get(sessionRef)).activeTurnId;
					const jsonRpcId =
						payload.mode === "url" ? payload.elicitationId : requestId;
					const decision = yield* Deferred.make<ProviderApprovalDecision>();
		...
		yield* client.handleServerRequest("item/tool/requestUserInput", (payload) =>
			Effect.gen(function* () {
				const requestId = ApprovalRequestId.make(
					yield* randomUUIDv4("user-input-request"),
				);
				const turnId = TurnId.make(payload.turnId);
				const itemId = ProviderItemId.make(payload.itemId);
		...
		yield* client.handleUnknownServerRequest((method) =>
			Effect.fail(
				CodexErrors.CodexAppServerRequestError.methodNotFound(method),
			),
		);
		...
		const readProviderThreadId = Effect.gen(function* () {
			const providerThreadId = currentProviderThreadId(
				yield* Ref.get(sessionRef),
			);
			if (!providerThreadId) {
				...
				Effect.gen(function* () {
					...
					if (hasConfiguredMcpServer(options.appServerArgs)) {
						yield* client.request("config/mcpServer/reload", undefined).pipe(
							Effect.catch((cause) =>
								Effect.logWarning(
									"Failed to refresh Codex MCP tool catalog before turn.",
									{
										cause,
									},
								),
					...
					const params = yield* buildTurnStartParams({
						...
						browserToolsAvailable: hasConfiguredMcpServer(
							options.appServerArgs,
						),
					});
					...
					const resumedProviderThreadId = currentProviderThreadId(
						yield* Ref.get(sessionRef),
					);
					return {
						threadId: options.threadId,
