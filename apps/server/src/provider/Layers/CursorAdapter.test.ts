...
class CursorAdapter extends Context.Service<
	CursorAdapter,
	CursorAdapterShape
...
async function makeMockAgentWrapper(
	...
	const dir = await NodeFSP.mkdtemp(
		NodePath.join(NodeOS.tmpdir(), "cursor-acp-mock-"),
	);
	return writeFakeCli({
		directory: dir,
		name: "fake-agent",
...
async function makeProbeWrapper(
	...
	const dir = await NodeFSP.mkdtemp(
		NodePath.join(NodeOS.tmpdir(), "cursor-acp-probe-"),
	);
	return writeFakeCli({
		directory: dir,
		name: "fake-agent",
...
cursorAdapterTestLayer("CursorAdapterLive", (it) => {
	it.effect(
		"rejects a Cursor transport error returned as a successful assistant answer",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				const planUpdate = runtimeEvents.find(
	...
	it.effect("sends selected project skills in Cursor's native slash form", () =>
		Effect.gen(function* () {
			...
			yield* Effect.promise(() =>
				NodeFSP.mkdir(skillDirectory, { recursive: true }),
			);
			yield* Effect.promise(() =>
				NodeFSP.writeFile(
			...
			assert.deepStrictEqual(
				promptRequests.map(
					(request) =>
						(request.params as Record<string, unknown> | undefined)?.prompt,
				),
				[
					[
		...
		() =>
			Effect.gen(function* () {
				...
				yield* Effect.gen(function* () {
					...
					throw new Error(
						"Timed out waiting for the first prompt to be in flight.",
					);
				});
				...
				assert.equal(
					String(turnStartedEvents[0]?.turnId),
		...
		() =>
			Effect.gen(function* () {
				...
				const tempDir = yield* Effect.promise(() =>
					NodeFSP.mkdtemp(
						NodePath.join(NodeOS.tmpdir(), "cursor-adapter-exit-log-"),
					),
				);
				...
				const exitLog = yield* Effect.promise(() =>
					waitForFileContent(exitLogPath),
				);
			...
			Effect.gen(function* () {
				...
				const tempDir = yield* Effect.promise(() =>
					NodeFSP.mkdtemp(
						NodePath.join(
							NodeOS.tmpdir(),
							"cursor-adapter-concurrent-exit-log-",
						),
					),
				);
				const exitLogPath = NodePath.join(tempDir, "exit.log");
	...
	it.effect("maps app plan mode onto the ACP plan session mode", () =>
		Effect.gen(function* () {
			...
			assert.include(
				["architect", "plan"],
				String(
					(modeRequest?.params as Record<string, unknown> | undefined)
						?.modeId ??
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				const requestsAfterStart = yield* Effect.promise(() =>
					readJsonLines(requestLogPath),
				);
				const configIdsAfterStart = requestsAfterStart.flatMap((entry) =>
					entry.method === "session/set_config_option" &&
					typeof (entry.params as Record<string, unknown> | undefined)
						?.configId === "string"
				...
				const finalRequests = yield* Effect.promise(() =>
					readJsonLines(requestLogPath),
				);
				...
				assert.deepStrictEqual(finalConfigIds, [
					"model",
					"reasoning",
					"context",
					"fast",
				...
				assert.equal(
					finalRequests.filter((entry) => entry.method === "session/prompt")
						.length,
					1,
				);
			}),
		...
		() =>
			Effect.gen(function* () {
					...
					Effect.gen(function* () {
						...
						if (
							...
							if (settledEventTypes.size === 3) {
								yield* Deferred.succeed(settledEventsReady, undefined).pipe(
									Effect.orDie,
								);
							}
						}
				...
				const program = Effect.gen(function* () {
					...
					const requestOpened = turnEvents.find(
					...
					if (requestOpened?.type === "request.opened") {
						...
						assert.equal(
							requestOpened.payload.requestType,
							"exec_command_approval",
						);
						assert.equal(
							requestOpened.payload.detail,
							"cat server/package.json",
						);
					}

					const requestResolved = turnEvents.find(
					...
					if (requestResolved?.type === "request.resolved") {
						...
						assert.equal(
							requestResolved.payload.requestType,
							"exec_command_approval",
						);
					...
					const contentDelta = turnEvents.find(
		...
		() =>
			Effect.gen(function* () {
				...
				const firstAssistantDeltaIndex = turnEvents.findIndex(
					(event) =>
						event.type === "content.delta" &&
						event.payload.delta === "before tool",
				);
				...
				const secondAssistantDeltaIndex = turnEvents.findIndex(
					(event) =>
						event.type === "content.delta" &&
						event.payload.delta === "after tool",
				);
				...
				const assistantDeltas = turnEvents.filter(
					(event) => event.type === "content.delta",
				);
				...
				if (
					...
					assert.notEqual(
						String(assistantStarts[0].itemId),
						String(assistantStarts[1].itemId),
					);
					assert.equal(
						String(assistantDeltas[0].itemId),
						String(assistantStarts[0].itemId),
					);
					assert.equal(
						String(assistantDeltas[1].itemId),
						String(assistantStarts[1].itemId),
					);
				}
		...
		() =>
			Effect.gen(function* () {
				...
				const requestResolvedReady =
					yield* Deferred.make<ProviderRuntimeEvent>();
					...
					(event) =>
						Effect.gen(function* () {
							...
							if (event.type === "request.resolved") {
								yield* Deferred.succeed(requestResolvedReady, event).pipe(
									Effect.ignore,
								);
								return;
							}
							if (event.type === "turn.completed") {
								yield* Deferred.succeed(turnCompletedReady, event).pipe(
									Effect.ignore,
								);
							}
						}),
	...
	it.effect("stopping a session settles pending approval waits", () =>
		Effect.gen(function* () {
			...
			yield* Stream.runForEach(adapter.streamEvents, (event) => {
				...
				return Deferred.succeed(approvalRequested, undefined).pipe(
					Effect.ignore,
				);
			}).pipe(Effect.forkChild);
	...
	it.effect("stopping a session settles pending user-input waits", () =>
		Effect.gen(function* () {
			...
			yield* Stream.runForEach(adapter.streamEvents, (event) => {
				...
				return Deferred.succeed(userInputRequested, undefined).pipe(
					Effect.ignore,
				);
			}).pipe(Effect.forkChild);
	...
	it.effect("switches model in-session via session/set_config_option", () =>
		Effect.gen(function* () {
			...
			assert.lengthOf(
				argvRuns,
				1,
				"session should not restart — only one spawn",
			);
			...
			assert.isAbove(
				setConfigRequests.length,
				0,
				"should call session/set_config_option",
			);
			assert.equal(
			...
			assert.isAbove(
				...
				0,
				"should apply fast mode as a separate config",
			);
			const lastFastConfig = fastConfigRequests[fastConfigRequests.length - 1];
			assert.equal(
				(lastFastConfig?.params as Record<string, unknown>)?.value,
				"true",
			);
		...
		() =>
			Effect.gen(function* () {
				...
				assert.isAtLeast(
					fastConfigRequests.length,
					2,
					"should set fast mode on and then off",
				);

				const lastFastConfig =
					fastConfigRequests[fastConfigRequests.length - 1];
				assert.equal(
					(lastFastConfig?.params as Record<string, unknown>)?.value,
					"false",
				);
		...
		() =>
			Effect.gen(function* () {
				...
				const threadId = ThreadId.make(
					"cursor-consumer-outlives-start-session",
				);
					...
					(event) =>
						Effect.sync(() => {
							...
							Effect.andThen(
								event.type === "content.delta" &&
									String(event.threadId) === String(threadId)
									? Deferred.succeed(sawContentDelta, undefined).pipe(
											Effect.asVoid,
										)
									: Effect.void,
							),
				...
				const sendTurnFiber = yield* adapter
					...
					.pipe(Effect.forkChild);
				yield* Deferred.await(sawContentDelta).pipe(
