...
export function makeCursorAdapter(
	...
	return Effect.gen(function* () {
		const boundInstanceId =
			options?.instanceId ?? ProviderInstanceId.make("cursor");
		...
		const startSession: CursorAdapterShape["startSession"] = (input) =>
			withThreadLock(
				...
				Effect.gen(function* () {
					...
					const resumeSessionId = parseCursorResume(
						input.resumeCursor,
					)?.sessionId;
							...
							(params) =>
								mapExtensionFailure(
									Effect.gen(function* () {
										...
										const answers =
											yield* Deferred.make<ProviderUserInputAnswers>();
					...
					const nf = yield* Stream.runDrain(
						...
						Effect.catch((cause) =>
							Effect.logError(
								"Failed to process Cursor runtime notification.",
								{ cause },
							),
						),
					...
					if (rawPrompt) {
						...
						if (
							...
							cursorSkillNames = new Set(
								skills
									.filter(
										(skill) => skill.enabled && skill.userInvocable !== false,
									)
		...
		const rollbackThread: CursorAdapterShape["rollbackThread"] = (
			threadId,
			numTurns,
		) =>
			Effect.gen(function* () {
