...
const makeMockAgentWithAboutWrapper = Effect.fn(
...
const makeProviderStatusEnvFixture = Effect.fn("makeProviderStatusEnvFixture")(
	function* () {
		const fileSystem = yield* FileSystem.FileSystem;
		const path = yield* Path.Path;
...
const makeExitLogFixture = Effect.fn("makeExitLogFixture")(function* (
	prefix: string,
) {
	const fileSystem = yield* FileSystem.FileSystem;
	const path = yield* Path.Path;
...
describe("Cursor skills", () => {
	it("discovers recursive project skills with project precedence", async () =>
		await runNode(
			Effect.gen(function* () {
				...
				yield* fileSystem.makeDirectory(path.join(userHome, ".codex"), {
					recursive: true,
				});
				yield* fileSystem.writeFileString(
				...
				expect(
					(yield* probeCursorSkills(workspace, { HOME: userHome }).pipe(
						Effect.result,
					))._tag,
				).toBe("Failure");
			}),
	...
	it("treats a symlinked skill outside the root as a package boundary", async () =>
		await runNode(
			Effect.gen(function* () {
				...
				yield* writeSkill(
					path.join(library, "shared-review"),
					"---\ndescription: shared\n---\n",
				);
				yield* writeSkill(
					path.join(library, "shared-review", "hidden"),
					"---\n---\n",
				);
	...
	it("rewrites only discovered skill mentions into Cursor slash invocations", () => {
		...
		expect(
			rewriteCursorSkillMentions(
				"use $review, keep $HOME and 5$review",
				new Set(["review"]),
			),
		).toBe("use $review, keep $HOME and 5$review");
		expect(
			rewriteCursorSkillMentions("please $review this", new Set(["review"])),
	...
	it("detects and invokes digit-leading Cursor skills without rewriting money", () => {
		...
		expect(hasCursorSkillMention("use $2spec here")).toBe(true);
		expect(rewriteCursorSkillMentions("use $2spec here", names)).toBe(
			"use /2spec here",
		);
		expect(rewriteCursorSkillMentions("use $2spec here", new Set())).toBe(
			"use $2spec here",
		);
		for (const text of [
			"pay $20 tomorrow",
...
describe("buildCursorCapabilitiesFromConfigOptions", () => {
	it("derives model capabilities from parameterized Cursor ACP config options", () => {
		expect(
			buildCursorCapabilitiesFromConfigOptions(parameterizedGpt54ConfigOptions),
		).toEqual(
	...
	it("detects boolean thinking toggles from model_config options", () => {
		expect(
			buildCursorCapabilitiesFromConfigOptions(
				parameterizedClaudeConfigOptions,
			),
		).toEqual(
			createModelCapabilities({
				optionDescriptors: [
	...
	it("prefers the newer model_option effort control over legacy thought_level", () => {
		expect(
			buildCursorCapabilitiesFromConfigOptions(
				parameterizedClaudeModelOptionConfigOptions,
			),
		).toEqual(
			createModelCapabilities({
				optionDescriptors: [
...
describe("checkCursorProviderStatus", () => {
	...
	it("passes the injected environment to ACP model discovery", async () => {
		const { requestLogPath, wrapperPath } = await runNode(
			makeProviderStatusEnvFixture(),
		);
		...
		await expect(
			runNode(waitForFileContent(requestLogPath)),
		).resolves.toContain("initialize");
	});
});
...
describe("discoverCursorModelsViaAcp", () => {
	it("reuses successful discovery until the CLI version or account changes", async () => {
		await runNode(
			Effect.gen(function* () {
				const { requestLogPath, wrapperPath } =
					yield* makeProviderStatusEnvFixture();
				const fileSystem = yield* FileSystem.FileSystem;
				const settings = {
					enabled: true,
					binaryPath: wrapperPath,
				...
				expect(yield* fileSystem.readFileString(requestLogPath)).toContain(
					"initialize",
				);
				yield* fileSystem.writeFileString(requestLogPath, "");
				yield* discover({
...
describe("Cursor parameterized model picker preview gating", () => {
	...
	it("parses the Cursor CLI channel from cli-config.json", () => {
		...
		expect(parseCursorCliConfigChannel('{ "channel": "stable" }')).toBe(
			"stable",
		);
...
describe("resolveCursorAcpBaseModelId", () => {
	it("drops bracket traits without rewriting raw ACP model ids", () => {
		expect(
			resolveCursorAcpBaseModelId("gpt-5.4[reasoning=medium,context=272k]"),
		).toBe("gpt-5.4");
		expect(resolveCursorAcpBaseModelId("gpt-5.4-medium-fast")).toBe(
			"gpt-5.4-medium-fast",
		);
