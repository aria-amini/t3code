...
const CURSOR_CLI_INSTALLATION_DOCS_URL =
...
function findCursorEffortConfigOption(
	...
	return (
		candidates.find(
			(option) => getCursorConfigOptionCategory(option) === "model_option",
		) ??
		candidates.find((option) => option.id.trim().toLowerCase() === "effort") ??
		candidates.find(
			(option) => getCursorConfigOptionCategory(option) === "thought_level",
		) ??
		candidates[0]
	);
}
...
export function resolveCursorAcpConfigUpdates(
	...
	const requestedContextWindow = getProviderOptionStringSelectionValue(
		selections,
		"contextWindow",
	);
	...
	const requestedFastMode = getProviderOptionBooleanSelectionValue(
		selections,
		"fastMode",
	);
	...
	const requestedThinking = getProviderOptionBooleanSelectionValue(
		selections,
		"thinking",
	);
	...
	withCursorAcpProbeRuntime(
		...
		(acp) =>
			Effect.gen(function* () {
				...
				const decoded =
					yield* decodeCursorListAvailableModelsResponse(response);
...
export const makeCursorModelDiscovery = Effect.fn("makeCursorModelDiscovery")(
	function* (cursorSettings: CursorSettings, environment?: NodeJS.ProcessEnv) {
		const cache = yield* Cache.makeWith(
			(_key: string) => discoverCursorModelsViaAcp(cursorSettings, environment),
			{
				capacity: 1,
				timeToLive: (exit) =>
					Exit.isSuccess(exit) && exit.value.length > 0
						? Duration.minutes(30)
						: Duration.zero,
			},
		);
...
const readCursorCliConfigChannel = Effect.fn("readCursorCliConfigChannel")(
	function* () {
...
export const checkCursorProviderStatus = Effect.fn("checkCursorProviderStatus")(
	function* (
		...
		environment?: NodeJS.ProcessEnv,
		discoverModels?: (
			about: CursorAboutResult,
		) => ReturnType<typeof discoverCursorModelsViaAcp>,
	): Effect.fn.Return<
		ServerProviderDraft,
		never,
	...
	> {
		...
		const aboutProbe = yield* runCursorAboutCommand(
			cursorSettings,
			environment,
