...
const encodeUnknownJsonString = Schema.encodeUnknownEffect(
...
const transientAcpUpdates = new Set([
	"agent_message_chunk",
	"agent_thought_chunk",
]);
...
function logWarning(
	...
	return Effect.logWarning(message, context).pipe(
...
function resolveThreadSegment(raw: string | null | undefined): string {
	const normalized =
		typeof raw === "string" ? toSafeThreadAttachmentSegment(raw) : null;
...
function shouldPersist(stream: EventNdjsonStream, event: unknown): boolean {
	...
	try {
		...
		if (method === "session/update") {
			...
			return (
				typeof updateType !== "string" || !transientAcpUpdates.has(updateType)
			);
		}
...
function isProviderLogFile(
	...
	try {
		...
		return /^\[[^\]\r\n]+\] (?:NTIVE|CANON|ORCH): /u.test(
			header.toString("utf8", 0, bytesRead),
		);
	} finally {
...
function validateOption(input: {
	...
	if (Number.isInteger(input.value) && input.value >= input.minimum)
		return undefined;
...
function resolveOptions(
	...
	const resolved = {
		...
		maxBufferedRecords:
			options.maxBufferedRecords ?? DEFAULT_MAX_BUFFERED_RECORDS,
...
function drainPending(input: {
	...
	for (const [threadSegment, records] of recordsBySegment) {
		const filePath = providerLogPath(
			input.directory,
			input.filePrefix,
			threadSegment,
		);
	...
	const retentionDue =
		input.now - input.state.lastRetentionAt >=
		input.options.retentionCheckIntervalMs;
...
export const makeEventNdjsonLogStore = Effect.fnUntraced(function* (
					...
					resolved.attribution?.record({
						...
						durationMs:
							totalBytes === 0
								? 0
								: Math.round(
										durationMs * (entry.logicalWriteBytes / totalBytes),
									),
					}) ?? Effect.void,
				{ discard: true },
			);
	...
	const scheduleFlush = Effect.fnUntraced(function* () {
		yield* Effect.forkIn(
			Effect.sleep(resolved.batchWindowMs).pipe(
				Effect.andThen(flush(true, false)),
			),
			timerScope,
	...
	const logger = (stream: EventNdjsonStream): EventNdjsonLogger => {
		...
		const write = Effect.fnUntraced(function* (
			...
			const observedAt = yield* DateTime.now.pipe(
