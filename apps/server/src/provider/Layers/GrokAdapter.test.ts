...
async function makeMockGrokWrapper(extraEnv?: Record<string, string>) {
	const dir = await NodeFSP.mkdtemp(
		NodePath.join(NodeOS.tmpdir(), "grok-acp-mock-"),
	);
	return writeFakeCli({
		directory: dir,
		name: "fake-grok",
		...
		Effect.gen(function* () {
			if (remainingAttempts <= 0) {
				return yield* Effect.die(
					new Error(`Timed out waiting for file content at ${filePath}`),
				);
			}
...
const makeTestAdapter = (
	...
	makeGrokAdapter(decodeGrokSettings({ binaryPath }), options).pipe(
		Effect.orDie,
	);
...
it("only sets planModeActive after a successful enter_plan_mode", () => {
	...
	assert.isFalse(
		nextGrokPlanModeActive(false, { ...enter, status: "pending" }),
	);
	assert.isTrue(
		nextGrokPlanModeActive(false, { ...enter, status: "inProgress" }),
	);
	assert.isTrue(
		nextGrokPlanModeActive(false, { ...enter, status: "completed" }),
	);
...
it.layer(grokAdapterTestLayer)("GrokAdapterLive", (it) => {
	it.effect(
		"sends runtime context with the current model without changing saved prompts",
		() =>
			Effect.gen(function* () {
				const threadId = ThreadId.make("grok-runtime-context");
				const tempDir = yield* Effect.promise(() =>
					NodeFSP.mkdtemp(
						NodePath.join(NodeOS.tmpdir(), "grok-runtime-context-"),
					),
				);
	...
	it.effect.skipIf(windowsHost)(
		...
		() =>
			Effect.gen(function* () {
				...
				const tempDir = yield* Effect.promise(() =>
					NodeFSP.mkdtemp(
						NodePath.join(NodeOS.tmpdir(), "grok-adapter-exit-log-"),
					),
				);
		...
		() =>
			Effect.gen(function* () {
				const threadId = ThreadId.make(
					"grok-preparation-failure-while-connecting",
				);
				const wrapperPath = yield* Effect.promise(() => makeMockGrokWrapper());
				const adapter = yield* makeTestAdapter(wrapperPath);
		...
		() =>
			Effect.gen(function* () {
				...
				const toolTitles = runtimeEvents.flatMap((event) =>
					event.type === "item.updated" && event.payload.title
						? [event.payload.title]
						: [],
				);
				...
				(event) =>
					Effect.gen(function* () {
						...
						if (event.type === "turn.started") {
							yield* Deferred.succeed(turnStarted, undefined).pipe(
								Effect.ignore,
							);
						}
	...
	it.effect("fails a Grok turn that stalls after ACP content begins", () =>
		Effect.gen(function* () {
				...
				(event) =>
					Effect.gen(function* () {
						...
						if (event.type === "content.delta") {
							yield* Deferred.succeed(contentDelta, undefined).pipe(
								Effect.ignore,
							);
						}
						if (event.type === "turn.completed") {
			...
			const sendTurnFiber = yield* adapter
				...
				.pipe(Effect.forkChild);
			yield* Deferred.await(contentDelta).pipe(
				Effect.timeout("2 seconds"),
				TestClock.withLive,
			);
	...
	it.effect("refreshes Grok liveness when ACP updates its plan", () =>
		Effect.gen(function* () {
				...
				(event) =>
					Effect.gen(function* () {
						...
						if (event.type === "turn.plan.updated") {
							yield* Deferred.succeed(planUpdated, undefined).pipe(
								Effect.ignore,
							);
						}
						if (event.type === "turn.completed") {
			...
			const sendTurnFiber = yield* adapter
				.sendTurn({
					threadId,
					input: "update plan then stall",
					attachments: [],
				})
				.pipe(Effect.forkChild);
			yield* Deferred.await(planUpdated).pipe(
				Effect.timeout("2 seconds"),
				TestClock.withLive,
			);
	...
	it.effect("settles a stalled Grok turn after the active-tool deadline", () =>
		Effect.gen(function* () {
				...
				(event) =>
					Effect.gen(function* () {
						...
						if (event.type === "item.updated") {
							yield* Deferred.succeed(activeTool, undefined).pipe(
								Effect.ignore,
							);
						}
						if (event.type === "turn.completed") {
			...
			const sendTurnFiber = yield* adapter
				...
				.pipe(Effect.forkChild);
			yield* Deferred.await(activeTool).pipe(
				Effect.timeout("2 seconds"),
				TestClock.withLive,
			);
			for (let yieldAttempt = 0; yieldAttempt < 4; yieldAttempt += 1) {
	...
	it.effect("does not report a synthetic stop reason when xAI omits one", () =>
		Effect.gen(function* () {
			const threadId = ThreadId.make(
				"grok-xai-prompt-complete-missing-stop-reason",
			);
			const wrapperPath = yield* Effect.promise(() =>
				makeMockGrokWrapper({
					T3_ACP_EMIT_XAI_PROMPT_COMPLETE_THEN_HANG: "1",
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				const threadId = ThreadId.make(
					"grok-cancelled-settlement-before-follow-up",
				);
				const tempDir = yield* Effect.promise(() =>
					NodeFSP.mkdtemp(
						NodePath.join(NodeOS.tmpdir(), "grok-acp-cancel-race-"),
					),
				);
				const requestLogPath = NodePath.join(tempDir, "requests.ndjson");
				const wrapperPath = yield* Effect.promise(() =>
					...
					(event) =>
						Effect.gen(function* () {
							...
							if (event.type === "turn.started" && event.turnId !== undefined) {
								yield* Deferred.succeed(firstTurnStarted, event.turnId).pipe(
									Effect.ignore,
								);
								return;
							}
				...
				yield* adapter
					.interruptTurn(threadId, firstTurnId)
				...
				yield* Deferred.await(twoTurnsCompleted).pipe(
					Effect.timeout("2 seconds"),
				);
	...
	it.effect("cancels an in-flight prompt when a mid-turn sendTurn steers", () =>
		Effect.gen(function* () {
			...
			const runtimeEventsFiber = yield* Stream.runForEach(
				...
				(event) =>
					Effect.gen(function* () {
						...
						if (event.type === "turn.completed") {
							yield* Deferred.succeed(turnCompleted, undefined).pipe(
								Effect.ignore,
							);
						}
					}),
			...
			assert.isAtLeast(
				methods.filter((method) => method === "session/prompt").length,
				2,
			);
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				const tempDir = yield* Effect.promise(() =>
					NodeFSP.mkdtemp(
						NodePath.join(NodeOS.tmpdir(), "grok-acp-steer-prep-"),
					),
				);
				const requestLogPath = NodePath.join(tempDir, "requests.ndjson");
				const wrapperPath = yield* Effect.promise(() =>
				...
				const steered = yield* adapter
					.sendTurn({
						threadId,
						input: "steer before first prompt starts",
						attachments: [],
					})
					.pipe(Effect.timeout("3 seconds"));
				yield* Fiber.join(firstSendTurnFiber).pipe(Effect.timeout("3 seconds"));
	...
	it.effect(
		"keeps the original prompt running when a steer fails during preparation",
		() =>
			Effect.gen(function* () {
				const threadId = ThreadId.make(
					"grok-failed-steer-keeps-original-prompt",
				);
				const tempDir = yield* Effect.promise(() =>
					NodeFSP.mkdtemp(
						NodePath.join(NodeOS.tmpdir(), "grok-acp-failed-steer-"),
					),
				);
				const requestLogPath = NodePath.join(tempDir, "requests.ndjson");
				const wrapperPath = yield* Effect.promise(() =>
				...
				const firstSendTurnFiber = yield* adapter
					.sendTurn({
						threadId,
						input: "hang until a failed steer",
						attachments: [],
					})
					.pipe(Effect.forkChild);
				const firstTurnId = yield* Deferred.await(firstTurnStarted).pipe(
				...
				assert.equal(
					String(sessionAfterFailedSteer?.activeTurnId),
					String(firstTurnId),
				);
	...
	it.effect(
		"does not double-complete when interrupt wins before a prompt starts ACP",
		() =>
			Effect.gen(function* () {
				...
				const firstSendTurnFiber = yield* adapter
					.sendTurn({
						threadId,
						input: "interrupt before prompt starts",
						attachments: [],
					})
					.pipe(Effect.forkChild);
				const firstTurnId = yield* Deferred.await(firstTurnStarted).pipe(
				...
				nativeEventLogger: {
					...
					write: (record: unknown) =>
						JSON.stringify(record).includes("late after cancel")
							? Deferred.succeed(lateNativeUpdate, undefined).pipe(
									Effect.asVoid,
								)
							: Effect.void,
					close: () => Effect.void,
					...
					Effect.sync(() => {
						...
						Effect.andThen(
							event.type === "turn.started" &&
								...
								String(event.threadId) === String(threadId)
								? Deferred.succeed(turnStarted, event.turnId).pipe(
										Effect.asVoid,
									)
								: Effect.void,
						),
	...
	it.effect("lets Stop cancel during the xAI completion drain window", () =>
		Effect.gen(function* () {
			...
			const runtimeEventsFiber = yield* Stream.runForEach(
				...
				(event) =>
					Effect.gen(function* () {
						...
						yield* Deferred.succeed(trailingChunkTurnId, turnId).pipe(
							Effect.ignore,
						);
					}),
			).pipe(Effect.forkChild);
			...
			const turnId = yield* Deferred.await(trailingChunkTurnId).pipe(
				Effect.timeout("2 seconds"),
			);
			yield* adapter
				.interruptTurn(threadId, turnId)
				...
				(event) => {
					...
					return Ref.modify(completedCountRef, (count) => {
						...
						Effect.flatMap((count) => {
							...
							if (count === 2) {
								return Deferred.succeed(secondTurnCompleted, undefined).pipe(
									Effect.asVoid,
								);
							}
							return Effect.void;
	...
	it.effect(
		"surfaces Grok usage limits without clearing the selected model",
		() =>
			Effect.gen(function* () {
				...
				assert.isFalse(
					runtimeEvents.some(
						(event) =>
							...
							event.payload.title === "Replay tool",
					),
				);
				assert.isFalse(
					runtimeEvents.some(
						(event) =>
							event.type === "content.delta" &&
							event.payload.delta === "replayed assistant text",
					),
				);
	...
	it.effect(
		"captures xAI exit_plan_mode as a proposed plan and unblocks the turn",
		() =>
			Effect.gen(function* () {
				...
				const eventsFiber = yield* Stream.runForEach(
					...
					(event) => {
						...
						if (event.type === "turn.completed") {
							return Deferred.succeed(turnCompleted, undefined).pipe(
								Effect.ignore,
							);
						}
						return Effect.void;
	...
	it.effect(
		"surfaces plan.md writes as a proposed plan while plan mode is active",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"keeps a Grok turn running when Always allow has no allow_always option",
		() =>
			Effect.gen(function* () {
				const threadId = ThreadId.make(
					"grok-always-allow-without-allow-always",
				);
				const tempDir = yield* Effect.promise(() =>
					NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "grok-acp-")),
				);
	...
	it.effect(
		"asks before a different command after Always allow this session",
		() =>
			Effect.gen(function* () {
	...
	it.effect("captures a plan under the provider instance GROK_HOME", () =>
		Effect.gen(function* () {
			...
			const eventsFiber = yield* Stream.runForEach(
				...
				(event) => {
					if (event.type === "turn.proposed.completed") {
						return Ref.update(plans, (current) => [
							...current,
	...
	it.effect("handles xAI ask_user_question extension requests", () =>
		Effect.gen(function* () {
			...
			assert.equal(
				requestedEvent.payload.questions[0]?.id,
				"Which scope should Grok use?",
			);
			assert.equal(
				requestedEvent.payload.questions[0]?.question,
				"Which scope should Grok use?",
			);
	...
	it.effect(
		"settles a stalled Grok turn after its first activity is user input",
		() =>
			Effect.gen(function* () {
				const threadId = ThreadId.make("grok-xai-ask-user-question");
				const wrapperPath = yield* Effect.promise(() =>
					makeMockGrokWrapper({
						T3_ACP_EMIT_XAI_ASK_USER_QUESTION_THEN_HANG: "1",
					}),
				);
				const adapter = yield* makeTestAdapter(wrapperPath, {
					turnInactivityTimeoutMs: 1_000,
				...
				assert.equal(
					String(resolvedEvent.turnId),
					String(requestedEvent.turnId),
				);
	...
	it.effect(
		"keeps consuming notifications after the startSession fiber completes",
		() =>
			Effect.gen(function* () {
					...
					(event) =>
						Effect.sync(() => {
							...
							Effect.andThen(
								event.type === "turn.completed" &&
									String(event.threadId) === String(threadId)
									? Deferred.succeed(turnCompleted, undefined).pipe(
											Effect.asVoid,
										)
									: Effect.void,
							),
