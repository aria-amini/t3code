...
function settlePendingUserInputsAsCancelled(
	...
	return Effect.forEach(
		...
		(pending) =>
			Deferred.succeed(pending.resolution, { _tag: "cancelled" }).pipe(
				Effect.ignore,
			),
		{ discard: true },
	);
...
const resolveNotificationTurnId = (
	ctx: GrokSessionContext,
): TurnId | undefined => ctx.activeTurnId;

const resolveCallbackTurnId = (ctx: GrokSessionContext): TurnId | undefined =>
	ctx.activeTurnId;
...
export function selectGrokPermissionOptionId(
	...
	const preferred = request.options.find(
		(entry) => entry.kind === preferredKind,
	);
...
function completedStopReasonFromPromptResponse(
	...
	if (
		response === undefined ||
		promptResponseHasMissingXAiStopReason(response)
	) {
		return null;
	}
...
export function makeGrokAdapter(
	...
	return Effect.gen(function* () {
		const boundInstanceId =
			options?.instanceId ?? ProviderInstanceId.make("grok");
		...
		const turnInactivityTimeoutNanos =
			BigInt(turnInactivityTimeoutMs) * NANOS_PER_MILLI;
		const requestedActiveToolInactivityTimeoutMs =
		...
		const clearTurnLiveness = (ctx: GrokSessionContext) => {
			...
			return turnId === undefined
				? Effect.void
				: signalTurnLiveness(ctx, turnId);
		};

		const recordTurnActivity = Effect.fn("GrokAdapter.recordTurnActivity")(
			function* (
				ctx: GrokSessionContext,
				turnId: TurnId,
				event: Extract<
				...
				try {
					...
					ctx.livenessUpdatesInFlight = Math.max(
						0,
						ctx.livenessUpdatesInFlight - 1,
					);
		...
		const livenessTimeoutFor = (ctx: GrokSessionContext) =>
			ctx.activeToolCallIds.size > 0
				...
				: {
						milliseconds: turnInactivityTimeoutMs,
						nanos: turnInactivityTimeoutNanos,
					};
		...
		const signalSessionTurnLiveness = (
			...
			const ctx = sessions.get(threadId);
			return ctx && turnId !== undefined
				? signalTurnLiveness(ctx, turnId)
				: Effect.void;
		};

		const resumeSessionTurnLiveness = Effect.fn(
			"GrokAdapter.resumeSessionTurnLiveness",
		...
		const refreshSessionTurnLiveness = Effect.fn(
			"GrokAdapter.refreshSessionTurnLiveness",
		)(function* (threadId: ThreadId, turnId: TurnId | undefined) {
			const ctx = sessions.get(threadId);
			if (
				!ctx ||
		...
		const markPromptResponseReady = Effect.fn(
			"GrokAdapter.markPromptResponseReady",
		...
		const settlePromptInFlight = (
			...
			Effect.gen(function* () {
				...
				if (options?.settleAllPrompts) {
					...
					if (
						...
						const fallbackTurnId =
							liveCtx.activeTurnId ?? liveCtx.session.activeTurnId;
						if (!fallbackTurnId) {
							if (
								...
								const { activeTurnId: _activeTurnId, ...readySession } =
									liveCtx.session;
				...
				const shouldEmitFailedTurn =
					options?.errorMessage !== undefined && canEmitTurnCompletion;
		...
		const settleStalledTurn = Effect.fn("GrokAdapter.settleStalledTurn")(
			function* (ctx: GrokSessionContext, turnId: TurnId) {
		...
		const runTurnLivenessWatchdog = Effect.fn(
			"GrokAdapter.runTurnLivenessWatchdog",
		)(
			function* (ctx: GrokSessionContext) {
				while (true) {
					...
					const remainingNanos =
						livenessTimeoutFor(ctx).nanos - (nowNanos - lastActivityAtNanos);
					...
					const wakeReason = yield* Effect.raceFirst(
						Effect.sleep(Duration.nanos(remainingNanos)).pipe(
							Effect.as("timeout" as const),
						),
						Queue.take(ctx.livenessSignals).pipe(
							Effect.as("activity" as const),
						),
					);
		...
		const startSession: GrokAdapterShape["startSession"] = (input) =>
			withThreadLock(
				...
				Effect.gen(function* () {
									...
									(params) =>
										mapAcpCallbackFailure(
											Effect.gen(function* () {
												...
												const resolution =
													yield* Deferred.make<PendingUserInputResolution>();
												const turnId = resolveSessionCallbackTurnId(
													sessions,
													input.threadId,
												);
												pendingUserInputs.set(requestId, { resolution });
												yield* signalSessionTurnLiveness(
													input.threadId,
													turnId,
												);
												yield* offerRuntimeEvent({
												...
												yield* resumeSessionTurnLiveness(
													input.threadId,
													turnId,
												);
												...
												switch (resolved._tag) {
													case "answered":
														return makeXAiAskUserQuestionResponse(
															params,
															resolved.answers,
														);
													case "cancelled":
							...
							mapAcpCallbackFailure(
								Effect.gen(function* () {
									...
									if (isRecord(rawInput) && rawInput.variant === "Bash") {
										const { description: _description, ...shellInput } =
											rawInput;
									...
									const approvalKey =
										command ||
										(isRecord(rawInput) && Object.keys(rawInput).length > 0)
											? stableStringify({
													kind,
													title,
													command,
									...
									const alreadyApproved =
										...
										sessionApprovedOperations.has(approvalKey);
									...
									yield* offerRuntimeEvent(
										makeAcpRequestOpenedEvent({
											...
											detail:
												...
												encodeJsonStringForDiagnostics(params)?.slice(
													0,
													2000,
												) ??
					...
					const currentStartReasoningEffort =
						currentGrokReasoningEffortFromSessionSetup(
							started.sessionSetupResult,
						);
					const requestedStartReasoningEffort =
							...
							Effect.gen(function* () {
								...
								switch (event._tag) {
									...
									case "ToolCallUpdated": {
										...
										ctx.planModeActive = nextGrokPlanModeActive(
											ctx.planModeActive,
											event.toolCall,
										);
					...
					yield* runTurnLivenessWatchdog(ctx).pipe(
						Effect.forkIn(ctx.scope),
						Effect.asVoid,
					);
		...
		const sendTurn: GrokAdapterShape["sendTurn"] = (input) =>
			Effect.gen(function* () {
					...
					Effect.gen(function* () {
						...
						return yield* Effect.gen(function* () {
							...
							const requestedTurnModelId = turnModelSelection?.model
								...
								: undefined;
							const requestedTurnReasoningEffort =
								getModelSelectionStringOptionValue(
									turnModelSelection,
									"reasoningEffort",
								);
				...
				const promptFailureMessageRef = yield* Ref.make<string | undefined>(
					undefined,
				);
				...
				return yield* Effect.gen(function* () {
					const promptStart = yield* prepared.promptLifecycle.withPermit(
						Effect.gen(function* () {
							const liveCtx = sessions.get(input.threadId);
							const interrupted =
								liveCtx?.interruptedTurnIds.has(prepared.turnId) === true;
							if (
								!liveCtx ||
						...
						Effect.gen(function* () {
							...
							if (ctx.acpSessionId !== prepared.acpSessionId) {
								yield* settlePromptInFlight(
									...
									{
										errorMessage:
											"Grok session changed before the turn completed.",
										settleAllPrompts: true,
									},
								);
								yield* Ref.set(promptSettled, true);
							...
							if (
								...
								const { activeTurnId: _completedTurnId, ...readySession } =
									ctx.session;
								...
								yield* withThreadLock(
									...
									Effect.gen(function* () {
										...
										yield* settlePromptInFlight(
											...
											{
												completedStopReason:
													completedStopReasonFromPromptResponse(promptResult),
											},
										);
									}),
								);
		...
		const rollbackThread: GrokAdapterShape["rollbackThread"] = (
			threadId,
			numTurns,
		) =>
			Effect.gen(function* () {
