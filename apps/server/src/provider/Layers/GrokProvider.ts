...
function grokReasoningOptionsFromModel(model: EffectAcpSchema.ModelInfo): {
	...
	const currentValue =
		currentEffort && options.some((option) => option.value === currentEffort)
			? currentEffort
			: undefined;
	const advertisedDefaults = options.filter(
		(option) => option.advertisedDefault,
	);
...
export function buildGrokModelCapabilities(
...
export const checkGrokProviderStatus = Effect.fn("checkGrokProviderStatus")(
	...
	> {
		...
		const versionResult = yield* runGrokCliCommand(
			grokSettings,
			["--version"],
			environment,
		...
		if (versionOutput.code !== 0) {
			yield* Effect.logWarning(
				"Grok CLI version probe exited with a non-zero status.",
				{
		...
		const modelsResult = yield* runGrokCliCommand(
			grokSettings,
			["models"],
			environment,
		...
		const acpExit = yield* discoverGrokModelsViaAcpInitialize(
			grokSettings,
			environment,
		).pipe(Effect.timeoutOption(GROK_ACP_INITIALIZE_TIMEOUT_MS), Effect.exit);
		const acpModels = Exit.isSuccess(acpExit)
			? Option.getOrElse(acpExit.value, () => [])
			: [];
		const acpFailed = Exit.isFailure(acpExit) || Option.isNone(acpExit.value);
		if (acpFailed) {
			yield* Effect.logWarning(
				"Grok ACP initialize probe failed or timed out.",
				{
		...
		const discoveredModels =
			acpModels.length > 0 ? acpModels : cliModels.models;
		const models =
