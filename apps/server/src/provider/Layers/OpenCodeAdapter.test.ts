...
class OpenCodeAdapter extends Context.Service<
	OpenCodeAdapter,
...
const runtimeMock = {
	state: {
		...
		questionReplyImplementation: null as
			| ((signal?: AbortSignal) => Promise<void>)
			| null,
		...
		questionListCalls: 0,
		permissionListImplementation: null as
			| (() => Promise<Array<PermissionRequest>>)
			| null,
		questionListImplementation: null as
			| (() => Promise<Array<QuestionRequest>>)
			| null,
		...
		({
			session: {
				create: async (input: Record<string, unknown>) => {
					...
					runtimeMock.state.authHeaders.push(
						serverPassword
							? `Basic ${btoa(`opencode:${serverPassword}`)}`
							: null,
					);
					return {
						data: {
							id:
								runtimeMock.state.createdSessionIds.shift() ??
								`${baseUrl}/session`,
						},
					};
				},
				get: async (
					...
					if (runtimeMock.state.sessionGetImplementation) {
						await runtimeMock.state.sessionGetImplementation(
							sessionID,
							options?.signal,
						);
					}
				...
				abort: async (
					...
					await runtimeMock.state.abortImplementation?.(
						sessionID,
						options?.signal,
					);
					...
					runtimeMock.state.pendingQuestions =
						runtimeMock.state.pendingQuestions.filter(
							(request) => request.sessionID !== sessionID,
						);
				},
				...
				message: async ({
					...
					const message = runtimeMock.state.messages.find(
							...
							try {
								...
								for (const event of runtimeMock.state.subscribedEvents) {
									...
									if (nativeEvent.type === "permission.asked") {
										...
										runtimeMock.state.pendingPermissions.push(
											nativeEvent.properties,
										);
									} else if (nativeEvent.type === "permission.replied") {
										runtimeMock.state.pendingPermissions =
											runtimeMock.state.pendingPermissions.filter(
												(request) =>
													request.id !== nativeEvent.properties.requestID,
											);
								...
								if (
									!runtimeMock.state.endEventStream &&
			...
			permission: {
				...
				reply: async (
					...
					if (options?.signal)
						runtimeMock.state.permissionReplySignals.push(options.signal);
			...
			question: {
				...
				reply: async (
					...
					await runtimeMock.state.questionReplyImplementation?.(
						options?.signal,
					);
					runtimeMock.state.pendingQuestions =
						runtimeMock.state.pendingQuestions.filter(
	...
	loadOpenCodeInventory: () =>
		Effect.fail(
			new OpenCodeRuntimeError({
				...
				detail:
					"OpenCodeRuntimeTestDouble.loadOpenCodeInventory not used in this test",
				cause: null,
			}),
		),
	...
	loadInventoryFromCli: () =>
		Effect.fail(
			new OpenCodeRuntimeError({
				...
				detail:
					"OpenCodeRuntimeTestDouble.loadInventoryFromCli not used in this test",
				cause: null,
			}),
		),
...
it.layer(OpenCodeAdapterTestLayer)("OpenCodeAdapterLive", (it) => {
	it.effect(
		"reuses a configured OpenCode server URL instead of spawning a local server",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"fails startup when the OpenCode event stream does not connect",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect("closes a connecting session when startup is interrupted", () =>
		Effect.gen(function* () {
			...
			runtimeMock.state.autoConnect = false;
			runtimeMock.state.eventSubscribeObserved = () =>
				eventSubscribeObserved.resolve(undefined);
	...
	it.effect(
		"aborts a held teardown request before closing the session scope",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"keeps one session when concurrent starts cross the connection barrier",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				NodeAssert.deepEqual(
					...
					secondSession.resumeCursor,
				);
				NodeAssert.equal(firstSession.status, "ready");
				NodeAssert.equal(secondSession.status, "ready");
				const winnerId = (
					threadSessions[0]?.resumeCursor as { sessionId?: string } | undefined
				)?.sessionId;
	...
	it.effect(
		"reuses a published connecting session after it becomes ready",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				NodeAssert.deepEqual(
					ownedSession.resumeCursor,
					reusedSession.resumeCursor,
				);
	...
	it.effect(
		"replaces a stopped connecting session while its teardown is held",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				runtimeMock.state.eventSubscribeObserved = () =>
					eventSubscribeObserved.resolve(undefined);
				runtimeMock.state.createdSessionIds.push(
					"ses_connecting_old",
					"ses_connecting_replacement",
				);
	...
	it.effect(
		"returns a durable resume cursor for a freshly created session",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"resumes the persisted OpenCode session instead of creating a new one",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				NodeAssert.equal(
					runtimeMock.state.sessionUpdateCalls[0]?.sessionID,
					"ses_persisted",
				);
				NodeAssert.equal(
					runtimeMock.state.sessionUpdateCalls[0]?.permission != null,
					true,
				);
	...
	it.effect("compacts through the native OpenCode session API", () =>
		Effect.gen(function* () {
			...
			const summarizeCall = runtimeMock.state.summarizeCalls[0] as Record<
				string,
				unknown
			>;
	...
	it.effect(
		"falls back to a fresh session when the persisted session is gone",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"surfaces a non-not-found resume probe error instead of silently starting fresh",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"re-applies the current runtimeMode permissions when resuming",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				runtimeMock.state.sessionDirectoryById.set(
					"ses_otherdir",
					"/some/other/worktree",
				);
				...
				NodeAssert.equal(
					runtimeMock.state.forkCalls[0]?.sessionID,
					"ses_otherdir",
				);
				NodeAssert.equal(
					typeof runtimeMock.state.forkCalls[0]?.directory,
					"string",
				);
	...
	it.effect(
		"reuses the resumed session when the stored directory differs only lexically",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"fails sendTurn for missing sessions through the typed error channel",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				NodeAssert.equal(
					result.failure.threadId,
					"thread-opencode-missing-send",
				);
			}),
	);

	it.effect(
		"fails stopSession for missing sessions through the typed error channel",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				const result = yield* adapter
				...
				NodeAssert.equal(
					result.failure.threadId,
					"thread-opencode-missing-stop",
				);
			}),
	);

	it.effect(
		"stops a configured-server session without trying to own server lifecycle",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				runtimeMock.state.sessionChildrenById.set("ses_stop_child", [
					{ id: "ses_stop_grandchild" },
				]);
				yield* adapter.startSession({
					provider: ProviderDriverKind.make("opencode"),
					threadId: asThreadId("thread-opencode"),
	...
	it.effect("completes streamEvents when the adapter scope closes", () =>
		Effect.gen(function* () {
			...
			try {
				...
				const adapter = yield* Effect.service(OpenCodeAdapter).pipe(
				...
				const exit = yield* Fiber.await(eventsFiber).pipe(
	...
	it.effect(
		"rolls back session state when sendTurn fails before OpenCode accepts the prompt",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				yield* adapter.startSession({
	...
	it.effect(
		"waits for steer admission before accepting the only idle event",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				const firstMessageId = (
					runtimeMock.state.promptCalls[0] as { messageID?: string }
				).messageID;
				const steerMessageId = (
					runtimeMock.state.promptCalls[1] as { messageID?: string }
				).messageID;
				NodeAssert.match(
					firstMessageId ?? "",
	...
	it.effect(
		"keeps steer admission until its user message arrives after prompt acceptance",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				NodeAssert.equal(
					sessionAfterStaleIdle?.activeTurnId,
					activeTurn.turnId,
				);
	...
	it.effect(
		"recovers steer admission when reconnect happens before prompt acceptance",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				NodeAssert.equal(completed?.turnId, activeTurn.turnId);
				NodeAssert.equal(
					runtimeMock.state.messageCalls.filter(
						(call) => call.messageID === steerMessageId,
					).length,
					2,
				);
				...
				NodeAssert.equal(
					runtimeMock.state.abortCalls.length,
					abortCallsAfterCompletion,
				);
			}),
	);

	it.effect(
		"resolves admission without a prompt echo when busy and idle still arrive",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"uses polled busy status to admit output after a stopped turn",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				const stoppedMessageId = (
					runtimeMock.state.promptCalls.at(-1) as { messageID: string }
				).messageID;
				firstUserMessageEvent.resolve({
	...
	it.effect(
		"marks subagents when a child is proven related by ancestry lookup",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"sums owned OpenCode step usage and marks unresolved usage partial",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				if (
					completed._tag === "Some" &&
					completed.value.type === "turn.completed"
				) {
	...
	it.effect(
		"keeps the next turn usage while the prior completion is delayed",
		() =>
			Effect.gen(function* () {
				...
				const adapter = yield* makeOpenCodeAdapter(
					openCodeAdapterTestSettings,
					{
						nativeEventLogger: {
							...
							write: (record) => {
								const eventType = (record as { event?: { type?: unknown } })
									.event?.type;
								if (
									blockFirstStepWrite &&
									...
									return Deferred.succeed(
										firstStepWriteStarted,
										undefined,
									).pipe(Effect.andThen(Deferred.await(firstStepWriteRelease)));
								}
								return Effect.void;
							},
							close: () => Effect.void,
	...
	it.effect(
		"ignores a stale admission status response after the next turn starts",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				NodeAssert.notEqual(
					sessionBeforeAcceptance?.activeTurnId,
					stoppedTurn.turnId,
				);
	...
	it.effect(
		"ignores a late prior-turn step after the next prompt is admitted",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				const secondCompleted = terminals.find(
	...
	it.effect(
		"reconciles a sole idle when the matching prompt echo arrives later",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"reconciles the only idle after a stopped turn when the prompt echo is missing",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				NodeAssert.equal(
					runtimeMock.state.messageCalls.some(
						(call) => call.messageID === activeMessageId,
					),
					true,
				);
				NodeAssert.equal(runtimeMock.state.sessionStatusCalls > 0, true);
	...
	it.effect(
		"reconciles a sole idle after a stop when the exact prompt echo arrives",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"recovers an idle before the exact prompt echo while acceptance is held",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				const threadId = asThreadId(
					"thread-idle-and-echo-before-acceptance-after-stop",
				);
				const idleEvent = promiseWithResolvers<unknown>();
				const userMessageEvent = promiseWithResolvers<unknown>();
				const activePromptStarted = promiseWithResolvers<void>();
	...
	it.effect(
		"accepts the only idle event after a steer fails before creating its message",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
		...
		({ permission, decision, reply }) =>
			Effect.gen(function* () {
				...
				const resolvedFiber = yield* adapter.streamEvents.pipe(
					Stream.filter(
						(event) =>
							event.threadId === threadId && event.type === "request.resolved",
					),
					Stream.runHead,
					Effect.forkChild,
				);
				yield* adapter.respondToRequest(
					threadId,
					ApprovalRequestId.make(request.id),
					decision,
				);
	...
	it.effect(
		"keeps a permission reply retryable after its HTTP request times out",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				NodeAssert.equal(
					runtimeMock.state.permissionReplySignals[0]?.aborted,
					true,
				);
				...
				NodeAssert.equal(
					Option.getOrThrow(yield* Fiber.join(resolvedFiber)).requestId,
					request.id,
				);
				yield* adapter.stopSession(threadId);
			}),
	...
	it.effect(
		"keeps a recovering permission retryable until its native request is loaded",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				if (
					reply._tag === "Failure" &&
					reply.failure._tag === "ProviderAdapterRequestError"
				) {
	...
	it.effect(
		"closes pending requests after Stop and ignores late requests from that turn",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"keeps progress live during automatic approval and never reopens a finished turn",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect("keeps automatic approval fallback available after a steer", () =>
		Effect.gen(function* () {
			...
			yield* adapter.respondToRequest(
				threadId,
				ApprovalRequestId.make("per_steer_auto"),
				"accept",
			);
			yield* adapter.stopSession(threadId);
		}),
	...
	it.effect(
		"routes child-session approval requests and replies through the parent thread",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				NodeAssert.equal(
					opened.raw?.source === "opencode.sdk.event" &&
						...
						? (opened.raw.payload.properties as { sessionID?: string })
								.sessionID
						: undefined,
					"ses_child",
				);
				...
				const resolved = yield* Fiber.join(resolvedEventFiber).pipe(
					Effect.timeout("1 second"),
				);
				NodeAssert.equal(
					Option.getOrUndefined(resolved)?.type,
					"request.resolved",
				);
	...
	it.effect(
		"does not reopen a failed full-access auto-reply after its terminal reply",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				const threadId = asThreadId(
					"thread-full-access-reply-failed-after-terminal",
				);
	...
	it.effect(
		"routes child-session questions and replies through the parent thread",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				const requestedEvents = Array.from(
					yield* Fiber.join(requestedEventsFiber).pipe(
						Effect.timeout("1 second"),
					),
				);
				...
				yield* adapter.respondToUserInput(
					threadId,
					ApprovalRequestId.make("que_child"),
					{
						Scope: "Workspace",
					},
				);
	...
	it.effect(
		"recovers pending requests from existing nested child sessions on resume",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				runtimeMock.state.pendingPermissions = [
					permissionRequest("per_existing", "ses_nested"),
				];
				runtimeMock.state.pendingQuestions = [
					questionRequest("que_existing", "ses_child"),
				];
				...
				NodeAssert.deepEqual(
					requests.map((event) => [event.type, event.requestId]).sort(),
					[
				...
				yield* adapter.respondToRequest(
					threadId,
					ApprovalRequestId.make("per_existing"),
					"accept",
				);
				yield* adapter.respondToUserInput(
					threadId,
					ApprovalRequestId.make("que_existing"),
					{
						Scope: "Workspace",
					},
				);
	...
	it.effect.each(["failure", "timeout"] as const)(
		...
		(lookupFailure) =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				const threadId = asThreadId(
					`thread-child-request-ancestry-retry-${lookupFailure}`,
				);
				...
				if (lookupFailure === "timeout") {
					runtimeMock.state.sessionGetImplementation = async (
						_sessionID,
						signal,
					) => {
						lookupSignal = signal;
				...
				yield* adapter.respondToRequest(
					threadId,
					ApprovalRequestId.make("per_retry"),
					"accept",
				);
			}),
	);
	...
	it.effect(
		"does not resurrect a recovered child request after its live reply",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				const response = yield* Effect.exit(
					adapter.respondToRequest(
						threadId,
						ApprovalRequestId.make(stale.id),
						"accept",
					),
				);
	...
	it.effect(
		"lets a child reply supersede an ask while ancestry lookup is retrying",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect("caps terminal ancestry retries after a request finishes", () =>
		Effect.gen(function* () {
			...
			NodeAssert.equal(
				runtimeMock.state.sessionGetIds.filter(
					(sessionID) => sessionID === childId,
				).length,
				callsAfterCap,
			);
	...
	it.effect(
		"reruns recovery when the event stream connects during the startup snapshot",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"stops the full OpenCode child tree before it completes the interrupt",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				runtimeMock.state.sessionChildrenById.set("ses_child_a", [
					{ id: "ses_grandchild" },
				]);
				...
				markerEvent.resolve({
					...
					properties: {
						info: { id: rootSessionId, title: "Parent abort received" },
					},
				});
				...
				NodeAssert.equal(interruptFiber.pollUnsafe(), undefined);
				NodeAssert.equal(
					runtimeMock.state.abortCalls.includes("ses_unrelated"),
					false,
				);
				NodeAssert.equal(
					runtimeMock.state.abortCalls.includes("ses_unrelated_child"),
					false,
				);
	...
	it.effect(
		"attempts every child abort and fails the interrupt when one child abort fails",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				NodeAssert.equal(
					runtimeMock.state.abortCalls.includes("ses_failing_child"),
					true,
				);
				NodeAssert.equal(
					runtimeMock.state.abortCalls.includes("ses_surviving_sibling"),
					true,
				);
				const sessions = yield* adapter.listSessions();
				const session = sessions.find(
	...
	it.effect(
		"keeps an idle event from completing a turn while its abort request is pending",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"treats MessageAbortedError as the acknowledgment for a pending user stop",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"releases stop and send waiters when a native abort times out",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"accepts a native turnless abort before its request times out",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				const acknowledgment = Option.getOrUndefined(
					yield* Fiber.join(acknowledgmentFiber),
				);
	...
	it.effect(
		"interrupts a turn waiting on cancellation when the session stops",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"rechecks a newer idle after an older status call returns busy",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"completes after transient status failures without another idle event",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"keeps idle reconciliation after a delayed abort from the stopped turn",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"keeps the newer turn running while status lookup keeps failing",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"keeps a genuine provider error visible during a pending user stop",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				const secondInterruptFiber = yield* adapter
					.interruptTurn(threadId)
					.pipe(Effect.forkChild);
				const nextTurnFiber = yield* adapter
					.sendTurn({
				...
				yield* Effect.yieldNow;
				NodeAssert.equal(
					runtimeMock.state.abortCalls.filter(
	...
	it.effect(
		"passes agent and variant options for the adapter's bound custom instance id",
		() => {
			...
			return Effect.gen(function* () {
				...
				const { messageID, ...prompt } = runtimeMock.state.promptCalls.at(
					-1,
				) as {
					messageID: string;
					[key: string]: unknown;
	...
	it.effect(
		"uses the bound custom instance id for fallback sendTurn model selection",
		() => {
			const instanceId = ProviderInstanceId.make("opencode_zen");
			const adapterLayer = Layer.effect(
				OpenCodeAdapter,
	...
	it.effect(
		"reverts the first removed assistant message and returns only retained turns",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				for (const remaining of [1, 0]) {
					...
					NodeAssert.deepEqual(
						(yield* adapter.readThread(threadId)).turns,
						snapshot.turns,
					);
				}
				NodeAssert.deepEqual(
	...
	it.effect(
		"classifies a confirmed not-found across the shapes the SDK/runtime can produce",
		() =>
			Effect.sync(() => {
				...
				NodeAssert.equal(
					isOpenCodeNotFound(
						new Error("upstream provider not found", {
							cause: { status: 500 },
						}),
					),
					false,
				...
				NodeAssert.equal(
					isOpenCodeNotFound({ detail: "status=500 body={...not found...}" }),
					false,
				);
				...
				NodeAssert.equal(
					isOpenCodeNotFound({ name: "UpstreamNotFoundError" }),
					false,
				);
				NodeAssert.equal(
					isOpenCodeNotFound({ cause: { name: "ProviderNotFoundError" } }),
					false,
				);
				NodeAssert.equal(
					isOpenCodeNotFound(
						new Error("x", {
							cause: { status: 502, body: { name: "NotFoundError" } },
						}),
					),
					false,
				);
				...
				NodeAssert.equal(
					isOpenCodeNotFound(new Error("boom", { cause: { status: 500 } })),
					false,
				);
				NodeAssert.equal(
					isOpenCodeNotFound({ cause: { response: { status: 401 } } }),
					false,
				);
				NodeAssert.equal(
					isOpenCodeNotFound(new Error("network error (no response)")),
					false,
				);
	...
	it.effect.skipIf(!symlinksSupported)(
		...
		() =>
			Effect.gen(function* () {
				...
				NodeAssert.equal(
					yield* sameDirectory("/repo/project/", "/repo/project"),
					true,
				);
				NodeAssert.equal(
					yield* sameDirectory("/repo/nested/../project", "/repo/project"),
					true,
				);
				// Nonexistent paths degrade to the lexical comparison instead of failing.
				NodeAssert.equal(
					yield* sameDirectory("/repo/project", "/repo/other"),
					false,
				);
				...
				NodeAssert.equal(
					yield* sameDirectory(link, path.join(base, "other")),
					false,
				);
			}).pipe(Effect.scoped),
	);
	...
	it.effect("reconciles assistant text snapshots", () =>
		Effect.sync(() => {
			...
			const changedUpdate = mergeOpenCodeAssistantText(
				"Hello world",
				"Hello there",
			);
	...
	it.effect(
		"does not strip coincidental prefix overlap from OpenCode part deltas",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
						...
						({
							...
							properties: {
								...
								part: {
									...
									type: "tool",
									callID:
										state.status === "error" ? "call-failed" : "call-working",
									tool: "bash",
									state,
								},
							},
	...
	it.effect(
		"processes late assistant metadata without visiting completed turns",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				for (let index = 0; index < 24; index += 1) {
					...
					enqueue({
						...
						properties: {
							sessionID,
							info: { id: `history-message-${index}`, role: "assistant" },
						},
					});
					enqueue({
						type: "message.part.updated",
				...
				enqueue({
					type: "message.part.updated",
					properties: { sessionID, part: step },
				});
				...
				for (const parentID of ["", promptMessageId]) {
					enqueue({
						...
						properties: {
							sessionID,
							info: { id: "current-message", role: "assistant", parentID },
						},
					});
				}
				enqueue({
	...
	it.effect(
		"keeps completed text edits and clears removed parts across reconnects",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
							...
							properties: {
								...
								part: {
									...
									state: {
										...
										title:
											tool === "bash" ? "Working directory" : "Tasks updated",
										metadata: {},
										time: { start: 1, end: 2 },
									},
								},
	...
	it.effect(
		"warns on disconnection and recovers a completion missed during reconnect",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				NodeAssert.equal(
					...
					"OpenCode connection lost. Reconnecting.",
				);
				const completedFiber = yield* adapter.streamEvents.pipe(
					Stream.filter(
						(event) =>
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				NodeAssert.match(
					exited.find((event) => event.type === "runtime.error")?.payload
						.message ?? "",
					/event stream ended/,
				);
				NodeAssert.equal(yield* adapter.hasSession(threadId), false);
	...
	it.effect(
		"lets OpenCode own session title generation and emits title metadata updates",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
				...
				NodeAssert.equal(
					"title" in (runtimeMock.state.sessionCreateInputs[0] ?? {}),
					false,
				);

				const metadataUpdated = events.find(
	...
	it.effect(
		"does not mirror OpenCode's default placeholder session titles",
		() =>
			Effect.gen(function* () {
				const adapter = yield* OpenCodeAdapter;
	...
	it.effect(
		"writes provider-native observability records using the session thread id",
		() =>
			Effect.gen(function* () {
				...
				NodeAssert.equal(
					nativeEvents.some(
						(record) => record.event?.threadId === "thread-native-log",
					),
					true,
				);
				NodeAssert.equal(
					nativeEvents.some(
						(record) => record.event?.type === "message.updated",
					),
					true,
				);
				NodeAssert.equal(
