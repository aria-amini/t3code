...
const decodeOpenCodeSessionStatusMap = Schema.decodeUnknownOption(
...
interface OpenCodePromptAdmission {
	...
	readonly priorIdle:
		| { readonly turnId: TurnId; readonly raw: unknown }
		| undefined;
	idleDuringAdmission:
		| { readonly turnId: TurnId; readonly raw: unknown }
		| undefined;
...
type OpenCodeRoutedRequestEvent =
	| OpenCodeAskedRequestEvent
	| OpenCodeTerminalRequestEvent;
...
function openCodeEventSessionId(
	...
	const info = (properties as { readonly info?: { readonly id?: unknown } })
		.info;
	return info && typeof info.id === "string" ? info.id : undefined;
}

function openCodeEventSessionTitle(
	event: OpenCodeSubscribedEvent,
): string | undefined {
...
type OpenCodeTextPartState = Pick<
	OpenCodeTextPart,
	"id" | "messageID" | "type" | "time"
> & {
...
interface OpenCodeSessionContext {
	...
	readonly textPartsByMessageId: Map<
		string,
		Map<string, OpenCodeTextPartState>
	>;
	...
	readonly firstConnection: Deferred.Deferred<
		void,
		ProviderAdapterRequestError
	>;
	/**
...
interface OpenCodeTurnTokenUsageAccumulator {
	...
	readonly assistantOwnershipByMessageId: Map<
		string,
		"owned" | "other" | "unknown"
	>;
	// Native removal does not undo usage. Keep unresolved counts until this turn settles.
	readonly unresolvedStepsByMessageId: Map<
		string,
		Map<string, OpenCodeStepUsage>
	>;
	inputTokens: number;
...
function retainOpenCodeTextPart(
	...
	const parts =
		context.textPartsByMessageId.get(part.messageID) ??
		new Map<string, OpenCodeTextPartState>();
...
const failPendingOpenCodeCancellation = Effect.fn(
	"failPendingOpenCodeCancellation",
)(function* (context: OpenCodeSessionContext, detail: string) {
...
const abortOpenCodeDescendants = Effect.fn("abortOpenCodeDescendants")(
...
const abortOpenCodeSessionForTeardown = Effect.fn(
...
const cancelPendingOpenCodePrompt = Effect.fn("cancelPendingOpenCodePrompt")(
	function* (context: OpenCodeSessionContext) {
...
const closeStartingOpenCodeContext = Effect.fn("closeStartingOpenCodeContext")(
	function* (context: OpenCodeSessionContext, abortRemote: boolean) {
		...
		yield* Deferred.fail(
			...
			new ProviderAdapterRequestError({
				...
				detail:
					"OpenCode session startup ended before the event stream connected.",
			}),
		).pipe(Effect.ignore);
		yield* cancelPendingOpenCodePrompt(context);
		yield* failPendingOpenCodeCancellation(
			context,
			"OpenCode session startup was cancelled.",
		);
...
export function makeOpenCodeAdapter(
	...
	return Effect.gen(function* () {
		const boundInstanceId =
			options?.instanceId ?? ProviderInstanceId.make("opencode");
		...
		const awaitOpenCodeContextReady = Effect.fn("awaitOpenCodeContextReady")(
			function* (context: OpenCodeSessionContext) {
				yield* Deferred.await(context.firstConnection);
				const current = yield* ensureSessionContext(
					sessions,
		...
		const makeOpenCodeMessageId = Effect.fn("makeOpenCodeMessageId")(
			function* () {
		...
		const writeNativeEvent = (
			...
			nativeEventLogger
				? nativeEventLogger.write(event, threadId)
				: Effect.void;
		...
		const cancelIdleReconciliation = Effect.fn("cancelIdleReconciliation")(
			function* (context: OpenCodeSessionContext) {
		...
		const scheduleIdleReconciliation = Effect.fn("scheduleIdleReconciliation")(
			function* (
				context: OpenCodeSessionContext,
				turnId: TurnId,
				raw: unknown,
				...
				if (
					...
					existing.promptGeneration === context.promptGeneration
				) {
					existing.raw = raw;
					existing.dirty = true;
					return;
					...
					while (context.pendingIdleReconciliation === pending) {
							...
							Effect.match({
								...
								onSuccess: (response) => {
									const data = Option.getOrUndefined(
										decodeOpenCodeSessionStatusMap(response.data),
									);
									if (data === undefined) {
						...
						if (!pending.warned) {
							...
							yield* emit({
								...
								payload: {
									message:
										"OpenCode turn completion is waiting for session status.",
									detail:
				...
				pending.fiber = yield* reconcile.pipe(
					Effect.forkIn(context.sessionScope),
				);
			},
		);

		const failPromptAdmissionRecovery = Effect.fn(
			"failPromptAdmissionRecovery",
		)(function* (
			context: OpenCodeSessionContext,
		...
		const schedulePromptAdmissionRecovery = Effect.fn(
			"schedulePromptAdmissionRecovery",
			...
			const recover = Effect.gen(function* () {
				...
				for (let retryCount = 0; retryCount < 5; retryCount += 1) {
					...
					if (!promptAdmission.messageObserved) {
						...
						const message = Option.isSome(response)
							? response.value.data
							: undefined;
						if (
					...
					const statusResponse = yield* runOpenCodeSdk(
						"session.status",
						(signal) => context.client.session.status(undefined, { signal }),
					).pipe(Effect.timeout("1 second"), Effect.option);
					const stopped = yield* Ref.get(context.stopped);
					if (
						stopped ||
					...
					const statusData = Option.isSome(statusResponse)
						? Option.getOrUndefined(
								decodeOpenCodeSessionStatusMap(statusResponse.value.data),
							)
						: undefined;
			...
			promptAdmission.recoveryFiber = yield* recover.pipe(
				Effect.forkIn(context.sessionScope),
			);
		});
		...
		const emitAssistantTextDelta = Effect.fn("emitAssistantTextDelta")(
			function* (
				context: OpenCodeSessionContext,
		...
		const addRelatedOpenCodeSession = (
			context: OpenCodeSessionContext,
			sessionId: string,
		) => {
		...
		const isRelatedOpenCodeSession = Effect.fn("isRelatedOpenCodeSession")(
			function* (context: OpenCodeSessionContext, candidateSessionId: string) {
							...
							orElse: () =>
								Effect.fail(
									new OpenCodeRuntimeError({
										...
										detail:
											"OpenCode session ancestry lookup did not complete within 10 seconds.",
									}),
								),
						}),
						Effect.catchIf(
		...
		const emitPendingOpenCodeRequest = Effect.fn("emitPendingOpenCodeRequest")(
			function* (
				context: OpenCodeSessionContext,
		...
		const resolvePendingOpenCodeRequest = Effect.fn(
			"resolvePendingOpenCodeRequest",
		)(function* (context: OpenCodeSessionContext, requestId: string) {
		...
		const emitTerminalOpenCodeRequest = Effect.fn(
			"emitTerminalOpenCodeRequest",
		)(function* (
			context: OpenCodeSessionContext,
		...
		const closePendingOpenCodeRequests = Effect.fn(
			"closePendingOpenCodeRequests",
		)(function* (
			context: OpenCodeSessionContext,
		...
		const clearPendingOpenCodeRequests = Effect.fn(
			"clearPendingOpenCodeRequests",
		)(function* (context: OpenCodeSessionContext, raw: unknown) {
		...
		const scheduleRequestRelationRetry = Effect.fn(
				...
				while (context.requestRelationRetries.get(requestId) === retry) {
					...
					if (!retry.warned) {
						...
						yield* emit({
							...
							payload: {
								message:
									"OpenCode request routing is waiting for session ancestry.",
		...
		const schedulePendingRequestRecovery = Effect.fn(
			...
			const recovery: OpenCodePendingRequestRecovery = {
				warned: false,
				rerun: false,
			};
			...
			const run = Effect.gen(function* () {
				...
				while (context.pendingRequestRecovery === recovery) {
					...
					if (responses.type === "failure") {
						if (!recovery.warned) {
							...
							yield* emit({
								...
								payload: {
									message:
										"OpenCode pending request recovery failed and will retry.",
					...
					if (permissions === undefined || questions === undefined) {
						if (!recovery.warned) {
							...
							yield* emit({
								...
								payload: {
									message:
										"OpenCode pending request recovery returned no data and will retry.",
								},
							});
						}
						const delayMs = Math.min(250 * 2 ** retryCount, 5_000);
					...
					yield* closePendingOpenCodeRequests(
						context,
						priorPermissions.filter(
							(request) => !permissionIds.has(request.id),
						),
		...
		const handleSubscribedEvent = Effect.fn("handleSubscribedEvent")(function* (
			...
			if (event.type === "server.connected") {
				...
				const isFirstConnection = !(yield* Deferred.isDone(
					context.firstConnection,
				));
			...
			yield* writeNativeEventBestEffort(context.session.threadId, {
				...
				event: {
					...
					...(!isParentEvent && payloadSessionId
						? { childSessionId: payloadSessionId }
						: {}),
					payload: event,
				},
			});
			...
			switch (event.type) {
					...
					if (event.properties.info.role === "assistant") {
						...
						const ownership =
							priorOwnership === undefined || priorOwnership === "unknown"
								...
								: priorOwnership;
						if (usage) {
							usage.assistantOwnershipByMessageId.set(
								event.properties.info.id,
								ownership,
							);
							if (ownership !== "unknown") {
								const steps = usage.unresolvedStepsByMessageId.get(
									event.properties.info.id,
								);
								...
								usage.unresolvedStepsByMessageId.delete(
									event.properties.info.id,
								);
							}
						}
				...
				case "message.part.removed": {
					const parts = context.textPartsByMessageId.get(
				...
				case "message.part.updated": {
					...
					if (
						...
						const previous = context.textPartsByMessageId
							.get(part.messageID)
							?.get(part.id);
						if (previous) {
				...
				case "session.status": {
					...
					if (event.properties.status.type === "idle" && turnId) {
						...
						if (context.promptAdmission?.turnId === turnId) {
							context.promptAdmission.idleDuringAdmission = {
								turnId,
								raw: event,
							};
				...
				case "session.error": {
					...
					if (terminalCancellation) {
						yield* Deferred.succeed(
							terminalCancellation.acknowledgment,
							undefined,
						).pipe(Effect.ignore);
					}
					break;
		...
		const startSession: OpenCodeAdapterShape["startSession"] = Effect.fn(
			...
			const resumeSessionId = parseOpenCodeResume(
				input.resumeCursor,
			)?.sessionId;
					...
					Effect.gen(function* () {
						...
						const resolved = yield* Effect.gen(function* () {
							...
							if (adopted) {
								...
								const forkedSession = yield* runOpenCodeSdk(
									"session.fork",
									() =>
								...
								if (!forked) {
									return yield* new OpenCodeRuntimeError({
										...
										detail:
											"OpenCode session.fork returned no session payload.",
									});
								}
								yield* runOpenCodeSdk("session.update", () =>
									client.session.update({
							...
							const createdSession = yield* runOpenCodeSdk(
								"session.create",
								() =>
			...
			const context: OpenCodeSessionContext = {
				...
				firstConnection: Deferred.makeUnsafe<
					void,
					ProviderAdapterRequestError
				>(),
					...
					Effect.mapError(
						(cause) =>
							new ProviderAdapterRequestError({
								...
								method: "event.subscribe",
								detail:
									"OpenCode event stream did not connect within 10 seconds.",
								cause,
							}),
					),
				);
			...
			function* (input) {
				...
				if ((!text || text.length === 0) && fileParts.length === 0) {
					return yield* new ProviderAdapterValidationError({
						...
						operation: "sendTurn",
						issue:
							"OpenCode turns require text input or at least one attachment.",
					});
				}
				...
				return yield* context.promptSemaphore.withPermit(
					Effect.gen(function* () {
						const freshTurnId = TurnId.make(
							`opencode-turn-${yield* randomUUIDv4}`,
						);
						...
						if (
							...
							yield* Deferred.succeed(
								promptAdmission.submissionSettled,
								undefined,
							).pipe(Effect.ignore);
							const cancellation = context.cancellation;
							if (cancellation?.turnId === turnId) {
								yield* Deferred.await(cancellation.completion).pipe(
									Effect.result,
								);
							}
							return yield* Effect.interrupt;
								...
								TimeoutError: (cause) => {
									...
									return Effect.fail(
										new ProviderAdapterRequestError({
											...
											detail:
												"OpenCode prompt submission did not complete within 10 seconds.",
											cause,
										}),
									);
								},
									...
									: Effect.gen(function* () {
											if (!promptTimedOut) {
												if (steeringTurnId !== undefined) {
													...
													const idle =
														promptAdmission.idleDuringAdmission ??
														promptAdmission.priorIdle;
													if (idle) {
														yield* scheduleIdleReconciliation(
															context,
															idle.turnId,
												...
												const tokenUsage = takeOpenCodeTurnTokenUsage(
													context,
													false,
												);
												context.promptAdmission = undefined;
											...
											if (Exit.isFailure(cleanupExit)) {
												yield* emit({
													...
													payload: {
														...
														detail: openCodeRuntimeErrorDetail(
						...
						const promptFiber = yield* promptEffect.pipe(
							Effect.forkIn(context.sessionScope),
						);
						...
						if (
							...
							yield* schedulePromptAdmissionRecovery(
								context,
								promptAdmission.recoveryRaw,
							);
						}
						...
						if (
							...
							if (finalCancellation?.turnId === turnId) {
								yield* Deferred.await(finalCancellation.completion).pipe(
									Effect.result,
								);
							}
							if (context.promptAdmission === promptAdmission) {
		...
		const compactThread = Effect.fn("compactThread")(function* (
			...
			if (!parsedModel) {
				return yield* new ProviderAdapterValidationError({
					...
					operation: "compactThread",
					issue:
						"OpenCode compaction requires an active 'provider/model' selection.",
				});
			}
							...
							TimeoutError: (cause) =>
								Effect.fail(
									new ProviderAdapterRequestError({
										...
										detail:
											"OpenCode session compaction did not complete within 10 minutes.",
										cause,
									}),
								),
						}),
		...
		const interruptTurn: OpenCodeAdapterShape["interruptTurn"] = Effect.fn(
			...
			if (
				promptAdmission !== undefined &&
				promptAdmission.turnId === interruptedTurnId
			) {
			...
			const parentAbortExit =
				parentAbortOutcome.source === "request"
					? parentAbortOutcome.exit
					: Exit.void;
						...
						TimeoutError: (cause) =>
							Effect.fail(
								new ProviderAdapterRequestError({
									...
									method: "session.abort",
									detail:
										"OpenCode child session cleanup did not complete within 10 seconds.",
									cause,
								}),
							),
					}),
			...
			const parentAbortFailed =
				Exit.isFailure(parentAbortExit) && !cancellation.acknowledged;
			...
			if (failedExit !== undefined && Exit.isFailure(failedExit)) {
				...
				yield* Deferred.done(cancellation.completion, failedExit).pipe(
					Effect.ignore,
				);
			...
			if (context.cancellation === cancellation) {
				...
				if (cancellation.turnSettled) {
					...
					context.cancellation = undefined;
					context.reconcileIdleStatus = true;
					yield* clearPendingOpenCodeRequests(context, {
						type: "session.abort",
					});
				}
			}
			...
			Effect.fn("respondToRequest")(function* (threadId, requestId, decision) {
				...
				if (!request) {
					...
					return yield* new ProviderAdapterRequestError({
						...
						detail:
							context.pendingRequestRecovery ||
							context.requestRelationRetries.has(requestId)
						...
						orElse: () =>
							Effect.fail(
								new ProviderAdapterRequestError({
									...
									method: "permission.reply",
									detail:
										"OpenCode permission reply did not complete within 10 seconds.",
								}),
							),
					}),
				);
						...
						orElse: () =>
							Effect.fail(
								new ProviderAdapterRequestError({
									...
									method: "question.reply",
									detail:
										"OpenCode question reply did not complete within 10 seconds.",
								}),
							),
					}),
				);
		...
		const stopSession: OpenCodeAdapterShape["stopSession"] = Effect.fn(
			"stopSession",
		)(function* (threadId) {
		...
		const readThread: OpenCodeAdapterShape["readThread"] = Effect.fn(
			"readThread",
		)(function* (threadId) {
			const context = yield* ensureSessionContext(sessions, threadId);
