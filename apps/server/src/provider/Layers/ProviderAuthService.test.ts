...
const makeHarness = Effect.fn("ProviderAuthService.test.makeHarness")(
	function* (
		input: {
			enabled?: boolean;
		...
		const sessions = new Map(
			input.sessions?.map((session) => [session.threadId, session]),
		);
		...
		const checkOwner = Effect.fn("ProviderAuthService.test.checkOwner")(
			function* (
		...
		const auth: ProviderAuthController = {
			...
			subscribe: (ownerSessionId) =>
				Stream.fromEffect(
					Effect.sync(() => (ownerSessionId === flowOwner ? state : idle)),
				),
			isLogoutPrompt: (text, hasAttachments) =>
					...
					Layer.mock(ProviderService)({
						...
						stopSession: ({ threadId }) =>
							Effect.suspend(() => {
								...
								if (binding)
									bindings.set(threadId, { ...binding, status: "stopped" });
								return Effect.void;
							}),
					}),
				),
...
const makeStreamingController = Effect.fn(
	"ProviderAuthService.test.makeStreamingController",
	...
	const unused = () =>
		Effect.die("Unexpected auth operation in a subscription test.");
	const auth: ProviderAuthController = {
		start: unused,
		complete: unused,
		cancel: unused,
		...
		subscribe: (ownerSessionId) =>
			SubscriptionRef.changes(state).pipe(
				Stream.map((current) =>
					ownerSessionId === flowOwner ? current : idleAuthState,
				),
...
const makeSubscriptionHarness = Effect.fn(
	"ProviderAuthService.test.makeSubscriptionHarness",
)(function* (
	initial: ProviderInstance,
	replaceDuringFirstLookup?: ProviderInstance,
) {
					...
					getInstance: () =>
						Effect.gen(function* () {
							...
							if (pendingReplacement) {
								assert.isTrue(
									subscribed,
									"Registry changes must be subscribed before lookup.",
								);
...
const observeAuth = Effect.fn("ProviderAuthService.test.observeAuth")(
	function* (stream: Stream.Stream<ProviderAuthState, ProviderSetupError>) {
...
describe("ProviderAuthService", () => {
	it.effect(
		"stops routed sessions before sign-in, including for a disabled instance",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"keeps sign-in state private and accepts the owner's redirect URL",
		() =>
			Effect.gen(function* () {
				...
				const complete = yield* service.complete(
					{ instanceId, flowId, callbackUrl },
					owner,
				);
	...
	it.effect("lets only the flow owner cancel sign-in", () =>
		Effect.gen(function* () {
			...
			const error = yield* Effect.flip(
				service.cancel({ instanceId, flowId }, otherOwner),
			);
	...
	it.effect.each([
		...
		({ initialEnabled, closeBeforeReplacement }) =>
			Effect.gen(function* () {
				...
				const firstClient = yield* observeAuth(
					service.subscribe({ instanceId }, owner),
				);
				const secondClient = yield* observeAuth(
					service.subscribe({ instanceId }, otherOwner),
				);
				assert.deepStrictEqual(
					yield* Queue.take(firstClient.states),
					waitingAuthState,
				);
				assert.deepStrictEqual(
					yield* Queue.take(secondClient.states),
					idleAuthState,
				);
				...
				const staleState: ProviderAuthState = {
					...idleAuthState,
					phase: "cancelled",
				};
				...
				const newWaiting: ProviderAuthState = {
					...
					authorizationUrl:
						"https://accounts.google.com/o/oauth2/v2/auth?state=replacement-state",
				};
	...
	it.effect(
		"does not miss a replacement during the first instance lookup",
		() =>
			Effect.gen(function* () {
				const first = yield* makeStreamingController(owner);
	...
	it.effect(
		"ends the subscription with a setup error when the instance is removed",
		() =>
			Effect.gen(function* () {
				...
				const { states, fiber } = yield* observeAuth(
					service.subscribe({ instanceId }, owner),
				);
				...
				assert.strictEqual(
					yield* Queue.take(controller.closedSubscriptions),
					owner,
				);
			}),
	);
	...
	it.effect.each([
		...
		"rejects setup for unavailable or unsupported instance $id",
		({ id, detail }) =>
			Effect.gen(function* () {
	...
	it.effect(
		"stops active and persisted sessions once before native logout",
		() =>
			Effect.gen(function* () {
				...
				assert.strictEqual(
					bindings.get(ThreadId.make("starting"))?.status,
					"stopped",
				);
				assert.strictEqual(
					bindings.get(ThreadId.make("persisted"))?.status,
					"stopped",
				);
				assert.strictEqual(
					bindings.get(ThreadId.make("other-instance"))?.status,
					"running",
				);
			}),
	);
	...
	it.effect.each([
		...
		"handles only a standalone logout command %#",
		({ text, hasAttachments, handled }) =>
			Effect.gen(function* () {
				const { service, actions } = yield* makeHarness();
				const result = yield* service.tryHandlePromptCommand({
					instanceId,
					text,
					hasAttachments,
				});
				...
				assert.deepStrictEqual(
					actions,
					handled
						? ["close-gate", "list-bindings", "list-sessions", "native-logout"]
						: [],
				);
			}),
	);
	...
	it.effect(
		"does not intercept commands for providers without an auth controller",
		() =>
			Effect.gen(function* () {
				const { service, actions } = yield* makeHarness();
				for (const id of [
	...
	it.effect(
		"does not log out or consume the command when stopping a session fails",
		() =>
			Effect.gen(function* () {
				const { service, actions, sessions } = yield* makeHarness({
					sessions: [makeSession("active")],
				...
				const error = yield* Effect.flip(
					service.tryHandlePromptCommand({
						instanceId,
						text: "/logout",
						hasAttachments: false,
					}),
