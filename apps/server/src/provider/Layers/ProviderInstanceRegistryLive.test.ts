...
const makeClaudeConfig = (
	overrides: Partial<ClaudeSettings>,
): ClaudeSettings => ({
	enabled: false,
	binaryPath: "claude",
	homePath: "",
...
const makeCursorConfig = (
	overrides: Partial<CursorSettings>,
): CursorSettings => ({
	enabled: false,
...
const makeOpenCodeConfig = (
	overrides: Partial<OpenCodeSettings>,
): OpenCodeSettings => ({
	enabled: false,
	binaryPath: "opencode",
	serverUrl: "",
...
const makeTildeProviderFixtures = Effect.fn(
	...
	yield* fileSystem.copyFile(
		path.join(codexFixtureDir, "codexCollabMockPeer.sh"),
		codexPath,
	);
	yield* fileSystem.copyFile(
	...
	const asTildePath = (filePath: string) =>
		`~/${path.relative(homePath, filePath)}`;
	return {
...
describe("ProviderInstanceRegistryLive — multi-instance codex slice", () => {
	...
	it.live(
		"boots two independent codex instances from a ProviderInstanceConfigMap",
		() =>
			Effect.gen(function* () {
				...
				).toEqual([personalId, workId].toSorted());
				expect(
					instances.every(
						(instance) => instance.driverKind === codexDriverKind,
					),
				).toBe(true);
				expect(
					instances.map((instance) => instance.displayName).toSorted(),
				).toEqual(["Codex (personal)", "Codex (work)"].toSorted());
	...
	it.live(
		"treats an explicit in-config enabled:false as disabling despite the envelope",
		() =>
			Effect.gen(function* () {
	...
	it.live(
		"runs Codex and Claude readiness probes from configured tilde paths",
		() =>
			Effect.gen(function* () {
				...
				const configMap: ProviderInstanceConfigMap = {
					[codexId]: {
						...
						config: makeCodexConfig({
							enabled: true,
							binaryPath: fixtures.codexBinaryPath,
						}),
					},
					[claudeId]: {
...
describe("ProviderInstanceRegistryLive — all drivers slice", () => {
	...
	it.live(
		"boots one instance of every shipped driver from a single config map",
		() =>
			Effect.gen(function* () {
				...
				const { registry } =
					yield* makeProviderInstanceRegistry<BuiltInDriversEnv>({
						drivers: [
							CodexDriver,
							ClaudeDriver,
							CursorDriver,
				...
				expect(
					instances.map((instance) => instance.instanceId).toSorted(),
				).toEqual([codexId, claudeId, cursorId, grokId, openCodeId].toSorted());
				...
				expect(grokSnapshot.continuation?.groupKey).toBe(
					`${grokDriverKind}:instance:${grokId}`,
				);
