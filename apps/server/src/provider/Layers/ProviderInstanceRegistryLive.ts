...
interface RegistryState {
	...
	readonly unavailable: Ref.Ref<
		ReadonlyMap<ProviderInstanceId, ServerProvider>
	>;
...
const resolveEntryEnabled = (
	...
	return (
		entry.enabled ?? providerInstanceConfigEnabledFlag(typedConfig) ?? true
	);
};
...
> =>
	Effect.gen(function* () {
		const { driversById, parentScope, instanceId, rawInstanceId, entry } =
			input;
...
> =>
	Effect.gen(function* () {
		...
		const entries = yield* Ref.make<ReadonlyMap<ProviderInstanceId, LiveEntry>>(
			new Map(),
		);
		const unavailable = yield* Ref.make<
			ReadonlyMap<ProviderInstanceId, ServerProvider>
		>(new Map());
		...
		const registry: ProviderInstanceRegistryShape = {
			getInstance: (id) =>
				Ref.get(entries).pipe(Effect.map((map) => map.get(id)?.instance)),
			...
			listUnavailable: Ref.get(unavailable).pipe(
				Effect.map(
					(map) => Array.from(map.values()) as ReadonlyArray<ServerProvider>,
				),
			),
