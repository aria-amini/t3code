		...
		(providerSource) =>
			providerSource.getSnapshot.pipe(
				Effect.flatMap((snapshot) =>
					correlateSnapshotWithSource(providerSource, snapshot),
				),
			),
		{
...
const makeManualProviderMaintenanceCapabilities = (
...
): boolean => (model.capabilities?.optionDescriptors?.length ?? 0) > 0;
...
const shouldRetainMissingProviderModels = (
	...
	const didInstalledProviderProbeFail =
		provider.installed && provider.status === "error";
	return (
...
const shouldRetainMissingOpenCodeMetadata = (
	provider: ServerProvider,
): boolean =>
	provider.driver === ProviderDriverKind.make("opencode") &&
...
const mergeProviderModels = (
	...
	const previousBySlug = new Map(
		previousModels.map((model) => [model.slug, model] as const),
	);
	...
	return shouldRetainMissingModels
		? [
				...mergedModels,
				...retainablePreviousModels.filter(
					(model) => !nextSlugs.has(model.slug),
				),
			]
		: mergedModels;
};
...
export const mergeProviderSnapshot = (
	...
	const savedAccount = carrySavedAntigravityAccount(
		previousProvider,
		nextProvider,
	);
...
export const ProviderRegistryLive = Layer.effect(
	...
	Effect.gen(function* () {
						...
						Effect.flatMap((cachedProvider) => {
							if (cachedProvider === undefined) {
								return Effect.void.pipe(
									Effect.as(undefined as ServerProvider | undefined),
								);
							}
							const correlation = {
								cachedProvider,
		...
		const liveSubsRef = yield* Ref.make<
			ReadonlyMap<ProviderInstanceId, ProviderInstance>
		>(new Map());
		...
		const getLiveSources: Effect.Effect<ReadonlyArray<ProviderSnapshotSource>> =
			Ref.get(liveSubsRef).pipe(
		...
		const persistProvider = (provider: ServerProvider) =>
			Effect.gen(function* () {
				...
				const { workspaceSnapshots: _workspaceSnapshots, ...machineProvider } =
					provider;
		...
		const applyProviderUpdateState = Effect.fn("applyProviderUpdateState")(
			function* (provider: ServerProvider) {
				const maintenanceActionStates = yield* Ref.get(
					maintenanceActionStatesRef,
				);
				const updateState = maintenanceActionStates.get(
					provider.instanceId,
				)?.update;
				if (!updateState) {
		...
		const upsertProviders = Effect.fn("upsertProviders")(function* (
			...
			const [previousProviders, providers, providersToPersist] =
				yield* Ref.modify(providersRef, (previousProviders) => {
		...
		const refreshAll = Effect.fn("refreshAll")(function* () {
			...
			return yield* Effect.forEach(
				sources,
				(source) => refreshOneSource(source),
				{
					concurrency: "unbounded",
					discard: true,
				},
		...
		const syncLiveSources = syncSemaphore.withPermits(1)(
			Effect.gen(function* () {
				...
				const knownInstanceIds = new Set<ProviderInstanceId>(
					nextByInstance.keys(),
				);
				...
				const newlyAdded: Array<
					readonly [ProviderInstanceId, ProviderInstance]
				> = [];
					...
					const [previousProviders, providers] = yield* Ref.modify(
						...
						(previousProviders) => {
							const providers = previousProviders.map((provider) => {
								if (!rebuiltInstanceIds.has(provider.instanceId))
									return provider;
								const {
		...
		const recoverRefreshFailure = Effect.fn("recoverRefreshFailure")(function* (
			...
			yield* Effect.logError(
				"provider registry refresh failed; preserving cached providers",
				{
					cause: Cause.pretty(cause),
				},
			);
		...
		const refreshWorkspaceSnapshot = Effect.fn("refreshWorkspaceSnapshot")(
			function* (input: {
				readonly instanceId: ProviderInstanceId;
				readonly cwd: string;
			}) {
							...
							: instanceRegistry.getInstance(input.instanceId).pipe(
									Effect.flatMap((currentInstance) => {
										...
										return Ref.modify(providersRef, (currentProviders) => {
											...
											return [
												[currentProviders, nextProviders] as const,
												nextProviders,
											];
										}).pipe(
