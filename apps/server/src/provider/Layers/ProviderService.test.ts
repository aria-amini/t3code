...
const defaultServerSettingsLayer =
	ServerSettings.ServerSettingsService.layerTest();
const serverConfigTestLayer = ServerConfig.layerTest(
	process.cwd(),
	process.cwd(),
...
const fixtureCwdRoot = NodeFS.mkdtempSync(
	NodePath.join(NodeOS.tmpdir(), "provider-service-test-"),
);
...
const assistantQuoteText =
	'Keep the shared parser for "résumé".\nPreserve line breaks.';
...
const decodeAssistantQuoteContext = Schema.decodeUnknownEffect(
	Schema.fromJsonString(
		Schema.Array(
			Schema.Struct({ id: Schema.String, citation: AssistantCitation }),
		),
	),
);
...
function makeFakeCodexAdapter(
	...
	const adapter: ProviderAdapterShape<ProviderAdapterError> = {
		...
		capabilities: {
			...
			...(supportsConversationRollback !== undefined
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				const directory =
					yield* ProviderSessionDirectory.ProviderSessionDirectory.pipe(
...
it.effect("ProviderServiceLive catches stopAll failures during shutdown", () =>
	Effect.gen(function* () {
		...
		const scope = yield* Scope.make();
		const runtimeServices = yield* Layer.build(providerLayer).pipe(
			Scope.provide(scope),
		);
...
it.effect(
	"ProviderServiceLive flushes deferred completions during shutdown",
	() =>
		Effect.gen(function* () {
			...
			codex.sendTurn
				...
				.mockImplementationOnce(() =>
					Effect.gen(function* () {
						...
						return {
							threadId,
							turnId: asTurnId("turn-analytics-stop-all-other"),
						};
					}),
				);
			...
			assert.equal(
				recordedAnalytics.eventsByName("provider.turn.completed").length,
				0,
			);

			const closeExit = yield* Scope.close(scope, Exit.void).pipe(Effect.exit);
			const completed = recordedAnalytics.eventsByName(
...
it.effect(
	"ProviderServiceLive rejects new sessions for disabled providers",
	() =>
		Effect.gen(function* () {
			const codex = makeFakeCodexAdapter();
			const claude = makeFakeCodexAdapter(CLAUDE_AGENT_DRIVER);
		...
		Effect.gen(function* () {
			...
			const session = yield* Effect.gen(function* () {
				...
				return yield* provider.startSession(
					asThreadId("thread-enabled-custom"),
					{
						provider: driverKind,
...
it.effect(
	"ProviderServiceLive rejects new sessions for disabled custom instances",
	() =>
		Effect.gen(function* () {
			const instanceId = ProviderInstanceId.make("codex_personal");
			...
			const failure = yield* Effect.flip(
				Effect.gen(function* () {
					const provider = yield* ProviderService.ProviderService;
					return yield* provider.startSession(
						asThreadId("thread-disabled-instance"),
						{
							provider: ProviderDriverKind.make("codex"),
							providerInstanceId: instanceId,
			...
			assert.include(
				failure.issue,
				"Provider instance 'codex_personal' is disabled",
			);
...
const customCompactionDriver = ProviderDriverKind.make(
...
const unsupportedCompactionInstanceId = ProviderInstanceId.make(
...
declaredCompaction.layer("ProviderService declared compaction", (it) => {
	it.effect(
		"starts declared native compaction instead of sending a prompt",
		() =>
			Effect.gen(function* () {
				const provider = yield* ProviderService.ProviderService;
				...
				const compacted = Option.getOrThrow(
					yield* Fiber.join(compactedEventFiber),
				);
	...
	it.effect("sends the declared slash command as the compaction turn", () =>
		Effect.gen(function* () {
			...
			assert.equal(
				customSlashCompaction.sendTurn.mock.calls[0]?.[0].input,
				"/reduce-context",
			);
			assert.deepEqual(
...
const originalAntigravityInstanceId = ProviderInstanceId.make(
...
const antigravityInstanceRouting = makeProviderServiceLayer({
	registry: {
		...antigravityRegistry,
		getInstanceInfo: (instanceId) =>
			instanceId === originalAntigravityInstanceId &&
			originalAntigravityInstanceAvailable
				? Effect.succeed({
						instanceId,
...
antigravityInstanceRouting.layer(
	"ProviderServiceLive instance-owned conversations",
	(it) => {
		it.effect(
				...
				Effect.gen(function* () {
					...
					for (const originalAvailable of [true, false]) {
						...
						for (const passCursor of [true, false]) {
							...
							assert.equal(
								replacementAntigravity.startSession.mock.calls.length,
								0,
							);
							assert.deepEqual(
...
const unsupportedRollback = makeProviderServiceLayer({
	supportsConversationRollback: false,
});
unsupportedRollback.layer("ProviderServiceLive unsupported rewind", (it) => {
	it.effect(
		"rejects rewind without starting or changing the provider conversation",
		() =>
			Effect.gen(function* () {
				const provider = yield* ProviderService.ProviderService;
				const directory =
				...
				for (const active of [true, false]) {
					...
					assert.equal(
						unsupportedRollback.codex.startSession.mock.calls.length,
						0,
					);
					assert.equal(
						unsupportedRollback.codex.rollbackThread.mock.calls.length,
						0,
					);
					assert.deepEqual(
						yield* directory.getBinding(threadId),
	...
	() =>
		Effect.gen(function* () {
			...
			const registry: ProviderAdapterRegistry.ProviderAdapterRegistry["Service"] =
				{
					...
					getByInstance: (instanceId) => {
						...
						const useReplacement =
							swapAfterFirstLookup && feedbackLookupCount++ > 0;
						return Effect.succeed(
							useReplacement ? replacement.adapter : original.adapter,
						);
					},
				};
			const runtimeRepositoryLayer = ProviderSessionRuntime.layer.pipe(
			...
			yield* Effect.gen(function* () {
				...
				assert.deepStrictEqual(replacement.uploadFeedback.mock.calls, [
					[{ threadId }],
				]);
			}).pipe(Effect.provide(providerLayer));
		}).pipe(Effect.provide(NodeServices.layer)),
...
it.effect(
	"ProviderServiceLive writes canonical events to the emitting thread segment",
	() =>
		Effect.gen(function* () {
			const codex = makeFakeCodexAdapter();
	...
	() =>
		Effect.gen(function* () {
			const tempDir = NodeFS.mkdtempSync(
				NodePath.join(NodeOS.tmpdir(), "t3-provider-service-"),
			);
...
routing.layer("ProviderServiceLive routing", (it) => {
	it.effect.each([CODEX_DRIVER, CLAUDE_AGENT_DRIVER, CURSOR_DRIVER])(
		...
		(driver) =>
			Effect.gen(function* () {
				...
				const savedFailure = yield* provider
					.startSession(threadId, savedInput)
					.pipe(Effect.flip);
				...
				const fileFailure = yield* provider
					.startSession(threadId, input)
					.pipe(Effect.flip);
	...
	it.effect("allows promptless continuation only for capable providers", () =>
		Effect.gen(function* () {
			...
			const claudeThreadId = asThreadId(
				"thread-promptless-continuation-unsupported",
			);
			...
			assert.include(
				stoppedFailure.issue,
				"requires an explicit continuation prompt",
			);
	...
	it.effect("marks a successful fallback compaction as compacted", () =>
		Effect.gen(function* () {
			...
			const observedRequestId = MessageId.make(
				"message-observed-compact-cursor",
			);
			...
			const failedStart = yield* provider
				.compactThread(threadId)
				.pipe(Effect.result);
			assert.equal(failedStart._tag, "Failure");
			assert.equal(
				Option.isSome(yield* Fiber.join(failedStartEventFiber)),
				true,
			);
			yield* provider.stopSession({ threadId });
		}),
	...
	it.effect(
		"serializes native compaction and quarantines timed-out completions",
		() =>
			Effect.gen(function* () {
				const provider = yield* ProviderService.ProviderService;
	...
	it.effect("times out fallback compaction when its turn never settles", () =>
		Effect.gen(function* () {
			...
			const retryFiber = yield* provider
				.compactThread(threadId)
				.pipe(Effect.forkChild);
			yield* advanceTestClock(50);
			routing.cursor.emit({
	...
	it.effect(
		"routes feedback to the Codex adapter and returns its feedback ID",
		() =>
			Effect.gen(function* () {
				const provider = yield* ProviderService.ProviderService;
	...
	it.effect(
		"does not restart an unsupported provider before rejecting feedback",
		() =>
			Effect.gen(function* () {
				const provider = yield* ProviderService.ProviderService;
	...
	it.effect("appends attachment file paths to the turn input text", () =>
		Effect.gen(function* () {
			...
			const turnInput = routing.codex.sendTurn.mock
				.calls[0]?.[0] as ProviderSendTurnInput;
			...
			const imageOnlyInput = routing.codex.sendTurn.mock
				.calls[0]?.[0] as ProviderSendTurnInput;
			assert.equal(
				imageOnlyInput.input?.startsWith('[Attached image "screenshot.png"'),
				true,
			);
			...
			const mixedInput = routing.codex.sendTurn.mock
				.calls[0]?.[0] as ProviderSendTurnInput;
			assert.include(
				mixedInput.input ?? "",
			...
			const fileOnlyInput = routing.codex.sendTurn.mock
				.calls[0]?.[0] as ProviderSendTurnInput;
			assert.include(
				fileOnlyInput.input ?? "",
				'[Attached file "report.pdf" is saved at: ',
	...
	it.effect(
		"preserves captured-window identity without accessibility data",
		() =>
			Effect.gen(function* () {
				const provider = yield* ProviderService.ProviderService;
	...
	it.effect(
		"appends structured captured-window accessibility in image coordinates",
		() =>
			Effect.gen(function* () {
				const provider = yield* ProviderService.ProviderService;
				...
				assert.include(
					turnText,
					"Element bounds are pixels in the attached image",
				);
	...
	it.effect("preserves the persisted binding when stopping a session", () =>
		Effect.gen(function* () {
			...
			if (Option.isSome(persistedAfterStop)) {
				...
				assert.deepEqual(
					persistedAfterStop.value.resumeCursor,
	...
	it.effect(
		"routes explicit claudeAgent provider session starts to the claude adapter",
		() =>
			Effect.gen(function* () {
				const provider = yield* ProviderService.ProviderService;
	...
	it.effect(
		"dies when an active session conflicts with its persisted binding",
		() =>
			Effect.gen(function* () {
				const provider = yield* ProviderService.ProviderService;
				const directory =
	...
	it.effect(
		"stops stale sessions in other providers after a successful replacement start",
		() =>
			Effect.gen(function* () {
				const provider = yield* ProviderService.ProviderService;
	...
	it.effect(
		"recovers stale claudeAgent sessions for sendTurn using persisted cwd",
		() =>
			Effect.gen(function* () {
				const provider = yield* ProviderService.ProviderService;

				const initial = yield* provider.startSession(
					asThreadId("thread-claude-send-turn"),
					{
						provider: ProviderDriverKind.make("claudeAgent"),
						providerInstanceId: claudeAgentInstanceId,
				...
				assert.equal(routing.claude.startSession.mock.calls.length, 1);
				const resumedStartInput =
					routing.claude.startSession.mock.calls[0]?.[0];
				assert.equal(
					typeof resumedStartInput === "object" && resumedStartInput !== null,
					true,
				);
				if (resumedStartInput && typeof resumedStartInput === "object") {
					...
					assert.equal(
						startPayload.cwd,
						fixtureCwd("project-claude-send-turn"),
					);
					assert.deepEqual(
	...
	it.effect(
		"does not persist running after a concurrent send is interrupted",
		() =>
			Effect.gen(function* () {
				const provider = yield* ProviderService.ProviderService;
				const runtimeRepository =
	...
	it.effect(
		"reuses persisted resume cursor when startSession is called after a restart",
		() =>
			Effect.gen(function* () {
				const tempDir = NodeFS.mkdtempSync(
				...
				const initial = yield* Effect.gen(function* () {
					const provider = yield* ProviderService.ProviderService;
					return yield* provider.startSession(
						asThreadId("thread-claude-start"),
						{
							provider: ProviderDriverKind.make("claudeAgent"),
							providerInstanceId: claudeAgentInstanceId,
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				const session = yield* provider.startSession(
					asThreadId("thread-send-metrics"),
					{
						provider: ProviderDriverKind.make("claudeAgent"),
						providerInstanceId: claudeAgentInstanceId,
...
citations.layer("ProviderServiceLive assistant citations", (it) => {
	for (const [driver, adapter] of [
		...
		it.effect(
			`expands quotes and bound comments as JSON data for ${driver}`,
			() =>
				Effect.gen(function* () {
					const provider = yield* ProviderService.ProviderService;
					...
					assert.include(
						turnText,
						'[Attached file "reference.txt" is saved at: ',
					);
					assert.deepStrictEqual(
						adapter.sendTurn.mock.calls[0]?.[0].attachments,
						[attachment],
					);
	...
	it.effect("leaves input without valid citations unchanged", () =>
		Effect.gen(function* () {
			...
			const malformedCitation = serializeAssistantCitation(
				assistantCitation,
			).replace("start=17", "start=invalid");
			const prompts = [
...
turnAnalytics.layer("ProviderServiceLive turn analytics", (it) => {
	it.effect(
		"records one completed-turn event with the allowed properties",
		() =>
			Effect.gen(function* () {
				...
				const completed = recordedTurnAnalytics.eventsByName(
					"provider.turn.completed",
				);
				assert.equal(completed.length, 1);
	...
	it.effect("ignores model metadata bound to another provider instance", () =>
		Effect.gen(function* () {
			...
			const provider = yield* ProviderService.ProviderService;
			const threadId = asThreadId(
				"thread-turn-analytics-mismatched-model-instance",
			);
			yield* provider.startSession(threadId, {
				provider: CODEX_DRIVER,
				providerInstanceId: codexInstanceId,
	...
	it.effect(
		"keeps overlapping request metadata with out-of-order adapter responses",
		() =>
			Effect.gen(function* () {
				recordedTurnAnalytics.reset();
				const provider = yield* ProviderService.ProviderService;
				...
				const runtimeEvents = yield* Stream.take(provider.streamEvents, 5).pipe(
					Stream.tap((event) => {
						if (event.type !== "turn.started" || initialStartCount >= 2)
							return Effect.void;
						initialStartCount += 1;
						return initialStartCount === 2
							? Deferred.succeed(initialStartsObserved, undefined).pipe(
									Effect.asVoid,
								)
							: Effect.void;
					}),
				...
				primaryAnalyticsCodex.emit({
					type: "turn.started",
					eventId: asEventId(
						"evt-turn-analytics-overlap-start-second-duplicate",
					),
					provider: CODEX_DRIVER,
					createdAt: "2026-01-01T00:00:00.000Z",
					threadId,
	...
	it.effect(
		"waits for the adapter response when a turn completes before sendTurn returns",
		() =>
			Effect.gen(function* () {
				recordedTurnAnalytics.reset();
				const provider = yield* ProviderService.ProviderService;
				...
				assert.equal(
					recordedTurnAnalytics.eventsByName("provider.turn.completed").length,
					0,
				);
	...
	it.effect(
		"does not give a synthetic turn the metadata of an in-flight send",
		() =>
			Effect.gen(function* () {
				recordedTurnAnalytics.reset();
				const provider = yield* ProviderService.ProviderService;
				...
				const realCompletion = yield* provider.streamEvents.pipe(
					Stream.filter(
						(event) =>
							event.type === "turn.completed" && event.turnId === realTurnId,
					),
					Stream.runHead,
					Effect.forkChild,
				);
	...
	it.effect(
		"defers overlapping terminal analytics until exact request association",
		() =>
			Effect.gen(function* () {
				recordedTurnAnalytics.reset();
				const provider = yield* ProviderService.ProviderService;
				const threadId = asThreadId(
					"thread-turn-analytics-overlap-fast-completion",
				);
				const firstStarted = yield* Deferred.make<void>();
				const secondStarted = yield* Deferred.make<void>();
				const firstRelease = yield* Deferred.make<void>();
				...
				for (const [turnId, suffix] of [
					...
					primaryAnalyticsCodex.emit({
						type: "turn.started",
						eventId: asEventId(
							`evt-turn-analytics-overlap-fast-start-${suffix}`,
						),
						provider: CODEX_DRIVER,
						createdAt: "2026-01-01T00:00:00.000Z",
						threadId,
					...
					primaryAnalyticsCodex.emit({
						type: "turn.completed",
						eventId: asEventId(
							`evt-turn-analytics-overlap-fast-complete-${suffix}`,
						),
						provider: CODEX_DRIVER,
						createdAt: "2026-01-01T00:00:00.000Z",
						threadId,
				...
				yield* Deferred.succeed(secondRelease, undefined);
				yield* Fiber.join(secondSend);
				let completed = recordedTurnAnalytics.eventsByName(
					"provider.turn.completed",
				);
				assert.equal(completed.length, 1);
				assert.deepInclude(completed[0]?.properties ?? {}, {
				...
				yield* Deferred.succeed(firstRelease, undefined);
				yield* Fiber.join(firstSend);
				completed = recordedTurnAnalytics.eventsByName(
					"provider.turn.completed",
				);
				assert.equal(completed.length, 2);
				assert.deepInclude(completed[1]?.properties ?? {}, {
	...
	it.effect(
		"bounds deferred completions and drains them after sends are canceled",
		() =>
			Effect.gen(function* () {
				recordedTurnAnalytics.reset();
				const provider = yield* ProviderService.ProviderService;
				...
				const runtimeEvents = yield* Stream.take(
					provider.streamEvents,
					turnIds.length * 2,
				...
				for (const [index, turnId] of turnIds.entries()) {
					primaryAnalyticsCodex.emit({
						type: "turn.started",
						eventId: asEventId(
							`evt-turn-analytics-bounded-deferred-start-${index + 1}`,
						),
						provider: CODEX_DRIVER,
						createdAt: "2026-01-01T00:00:00.000Z",
						threadId,
					...
					primaryAnalyticsCodex.emit({
						type: "turn.completed",
						eventId: asEventId(
							`evt-turn-analytics-bounded-deferred-complete-${index + 1}`,
						),
						provider: CODEX_DRIVER,
						createdAt: "2026-01-01T00:00:00.000Z",
						threadId,
	...
	it.effect(
		"keeps the first metadata when steering reuses a rerouted turn",
		() =>
			Effect.gen(function* () {
				recordedTurnAnalytics.reset();
				const provider = yield* ProviderService.ProviderService;
				...
				const completedEvent = yield* Stream.take(
					provider.streamEvents,
					1,
				).pipe(Stream.runDrain, Effect.forkChild);
				yield* Effect.yieldNow;
	...
	it.effect(
		"bounds active metadata while preserving recent delayed completions",
		() =>
			Effect.gen(function* () {
				recordedTurnAnalytics.reset();
				const provider = yield* ProviderService.ProviderService;
...
validation.layer("ProviderServiceLive validation", (it) => {
	it.effect(
		"rejects citation-expanded input over the provider character limit",
		() =>
			Effect.gen(function* () {
				const provider = yield* ProviderService.ProviderService;
				...
				const failure = yield* provider
					.sendTurn({
						threadId: asThreadId("thread-citation-expanded-limit"),
						input,
					})
					.pipe(Effect.flip);
				...
				assert.include(
					failure.issue,
					String(PROVIDER_SEND_TURN_MAX_INPUT_CHARS),
				);
	...
	it.effect(
		"rejects oversized encoded citations even when the expanded input fits",
		() =>
			Effect.gen(function* () {
				const provider = yield* ProviderService.ProviderService;
				...
				const failure = yield* provider
					.sendTurn({
						threadId: asThreadId("thread-citation-encoded-limit"),
						input,
					})
					.pipe(Effect.flip);
	...
	it.effect(
		"rejects session starts without an explicit provider instance id",
		() =>
			Effect.gen(function* () {
				const provider = yield* ProviderService.ProviderService;
				...
				assert.include(
					failure.issue,
					"Provider instance id is required for provider 'codex'.",
				);
...
const boundedListing = makeProviderServiceLayer({
	directory: {
		...
		getProvider: () =>
			Effect.die("ProviderService.listSessions does not use getProvider"),
		getBinding,
		listThreadIds,
		listBindings: () =>
			Effect.die("ProviderService.listSessions does not use listBindings"),
	},
});

boundedListing.layer("ProviderServiceLive session listing", (it) => {
	it.effect(
		"looks up bindings for active sessions without scanning historical threads",
		() =>
			Effect.gen(function* () {
				const provider = yield* ProviderService.ProviderService;
...
const decodeBrowserAccessThreadShell = Schema.decodeUnknownEffect(
	OrchestrationThreadShell,
);
...
describe("agent browser access", () => {
	...
	const startSessionWith = (
		...
		Effect.gen(function* () {
			...
			const providerLayer = makeProviderServiceLive({
				...
				revokeMcpCredential: (revoked) =>
					Effect.sync(() => void revokedThreads.push(revoked)),
			}).pipe(
				Layer.provide(providerAdapterLayer),
				Layer.provide(directoryLayer),
	...
	it.effect(
		"withholds and revokes MCP credentials when the project disables browser access",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"requests an MCP credential when the project overrides browser access to on",
		() =>
			Effect.gen(function* () {
