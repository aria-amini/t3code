...
const encodePromptJson = Schema.encodeSync(
...
function isRedundantWindowButtonDescription(
...
function compactAccessibilityNodeForPrompt(
	...
	const bounds =
		node.bounds &&
		!(options.isRoot && isFullImageBounds(node.bounds, imageSize))
			? node.bounds
			: undefined;
	const name =
		node.role !== "group" && node.name === options.parentName
			? undefined
			: node.name;
	const description = isRedundantWindowButtonDescription(node)
		? undefined
		: node.description;
	const actions = node.actions?.filter(
		(action) => node.role !== "button" || action !== "press",
	);
...
function accessibilityNodeHasBounds(
	node: SnapShotPromptAccessibilityNode,
): boolean {
	return Boolean(
		node.bounds || node.children?.some(accessibilityNodeHasBounds),
	);
}
...
function compactAccessibilityForPrompt(
	...
	const root = compactAccessibilityNodeForPrompt(
		accessibility.root,
		accessibility.imageSize,
		{
			isRoot: true,
		},
	)[0]!;
	...
	return {
		...
		...(hasBounds
			? {
					coordinateSpace: accessibility.coordinateSpace,
...
interface TurnAnalyticsSessionState {
	...
	readonly deferredCompletionsByTurnId: Map<
		string,
		DeferredTurnAnalyticsCompletion
	>;
}
...
function turnAnalyticsSessionKey(
...
type ProviderServiceMethod<
	Name extends keyof ProviderService.ProviderService["Service"],
...
function toRuntimePayloadFromSession(
	...
	return {
		...
		...(extra?.continueAfterServerUpdate !== undefined
			...
			: {}),
		...(extra?.modelSelection !== undefined
			? { modelSelection: extra.modelSelection }
			: {}),
...
const correlateRuntimeEventWithInstance = (
	...
	if (
		...
		event.providerInstanceId !== source.instanceId
	) {
		throw new Error(
...
const makeProviderService = Effect.fn("makeProviderService")(function* (
	...
	const canonicalEventLogger =
		options?.canonicalEventLogger ?? eventLoggers.canonical;
	...
	const finishTurnAnalytics = (
		...
		if (state.completedKeys.has(input.completion.completionKey))
			return undefined;
		...
		return {
			...
			...(metadata
				? {
						durationMs: Math.max(
							0,
							input.completion.completedAtMs - metadata.startedAtMs,
						),
					}
				: {}),
		};
	...
	const recordCompletedTurnProperties = (
		...
		Effect.forEach(
			properties,
			(entry) => analytics.record("provider.turn.completed", entry),
			{
				discard: true,
			},
		);

	const clearTurnAnalyticsSession = (
		providerInstanceId: ProviderInstanceId,
		threadId: ThreadId,
	) =>
		Effect.gen(function* () {
			const properties = yield* Ref.modify(turnAnalytics, (state) => {
				const sessionKey = turnAnalyticsSessionKey(
					providerInstanceId,
					threadId,
				);
				...
				if (session) {
					for (const [
						turnId,
						completion,
					] of session.deferredCompletionsByTurnId) {
	...
	const beginTurnAnalytics = Effect.fn("beginTurnAnalytics")(function* (input: {
		...
		return yield* Ref.modify(turnAnalytics, (state) => {
			const key = turnAnalyticsSessionKey(
	...
	const associateTurnAnalytics = (input: {
		...
		Effect.gen(function* () {
			const properties = yield* Ref.modify(turnAnalytics, (state) => {
				...
				const base = existing?.requestAssociated
					? existing.metadata
					: input.metadata;
					...
					metadata: {
						...
						...(base?.runtimeMode
							...
							: input.metadata.runtimeMode
								...
								: {}),
						mixedModels:
							existingMetadata?.mixedModels ?? input.metadata.mixedModels,
					},
				});
				session.pendingByRequestId.delete(input.metadata.requestId);
				const completion = session.deferredCompletionsByTurnId.get(
					input.turnId,
				);
	...
	const observeTurnStartedForAnalytics = Effect.fn(
		"observeTurnStartedForAnalytics",
	)(function* (
		source: {
			readonly instanceId: ProviderInstanceId;
			readonly provider: ProviderDriverKind;
	...
	const recordTurnCompletedAnalytics = Effect.fn(
		"recordTurnCompletedAnalytics",
	)(function* (
		source: {
			readonly instanceId: ProviderInstanceId;
			readonly provider: ProviderDriverKind;
		...
		const properties = yield* Ref.modify(turnAnalytics, (state) => {
			...
			const needsAssociation =
				(session?.pendingByRequestId.size ?? 0) > 0 &&
				active?.requestAssociated !== true;
			if (!session || !needsAssociation) {
				const completed = finishTurnAnalytics(state, {
					sessionKey,
					turnId,
					completion,
				});
			...
			const completed: Array<Readonly<Record<string, unknown>>> = [];
			while (
				session.deferredCompletionsByTurnId.size >
				MAX_ACTIVE_TURN_ANALYTICS_PER_SESSION
			) {
				const oldest = session.deferredCompletionsByTurnId
					.entries()
					.next().value;
				if (!oldest) break;
	...
	const agentBrowserAccessEnabled = Effect.fn(
		"ProviderService.agentBrowserAccessEnabled",
	)(
	...
	const processFallbackCompactionEvent = (
		...
		Effect.gen(function* () {
			...
			const matchesTurn =
				event.turnId !== undefined && event.turnId === pending.expectedTurnId;
			...
			const settled = yield* settleCompaction(
				event.threadId,
				pending,
				terminal,
			);
	...
	const processRuntimeEvent = (
		...
		Effect.gen(function* () {
			...
			if (canonicalEvent.type === "turn.started") {
				...
				yield* clearTurnAnalyticsSession(
					source.instanceId,
			...
			if (
				...
				(isCompactedEvent(canonicalEvent) ||
					compactionTerminal(canonicalEvent) !== null)
			) {
	...
	const recoverSessionForThread = Effect.fn("recoverSessionForThread")(
		function* (input: {
			...
			return yield* Effect.gen(function* () {
				...
				const hasActiveSession = yield* adapter.hasSession(
	...
	const stopStaleSessionsForThread = Effect.fn("stopStaleSessionsForThread")(
		function* (input: {
			readonly threadId: ThreadId;
	...
	const startSession: ProviderServiceMethod<"startSession"> = Effect.fn(
		"startSession",
		...
		return yield* Effect.gen(function* () {
			...
			if (
				...
				const previousInfo =
					yield* registry.getInstanceInfo(previousInstanceId);
				if (
			...
			if (effectiveCwd !== undefined) {
				...
				const workspaceIsDirectory = yield* fileSystem.stat(effectiveCwd).pipe(
					...
					Effect.catch((statError) =>
						Effect.succeed(statError.reason._tag !== "NotFound"),
					),
				);
			...
			yield* analytics.record("provider.session.started", {
				...
				hasCwd:
					typeof effectiveCwd === "string" && effectiveCwd.trim().length > 0,
				hasModel:
		...
		function* (rawInput) {
			...
			const inputTextWithCitations =
				parsed.input === undefined
					? undefined
					: expandAssistantCitationsForProvider(parsed.input);
			...
			for (const attachment of attachments) {
				const source =
					attachment.type === "image"
						? (attachment as ChatImageAttachment).source
						: undefined;
				const accessibility =
			...
			return yield* Effect.gen(function* () {
				...
				yield* analytics.record("provider.turn.sent", {
					...
					hasInput:
						typeof input.input === "string" && input.input.trim().length > 0,
				});
				return turn;
			}).pipe(
				withMetrics({
	...
	const compactThread: ProviderServiceMethod<"compactThread"> = Effect.fn(
		"compactThread",
		...
		if (
			...
			timedOutNativeCompactions.has(threadId)
		) {
	...
	const interruptTurn: ProviderServiceMethod<"interruptTurn"> = Effect.fn(
		"interruptTurn",
	)(function* (rawInput) {
	...
	const respondToRequest: ProviderServiceMethod<"respondToRequest"> = Effect.fn(
		"respondToRequest",
	)(function* (rawInput) {
		const input = yield* decodeInputOrValidationError({
	...
	const respondToUserInput: ProviderServiceMethod<"respondToUserInput"> =
		Effect.fn("respondToUserInput")(function* (rawInput) {
			const input = yield* decodeInputOrValidationError({
	...
	const stopSession: ProviderServiceMethod<"stopSession"> = Effect.fn(
		"stopSession",
	)(function* (rawInput) {
		const input = yield* decodeInputOrValidationError({
	...
	const listSessions: ProviderServiceMethod<"listSessions"> = Effect.fn(
		"listSessions",
	)(function* () {
		const currentAdapters = yield* getAdapterEntries;
	...
	const getCapabilities: ProviderServiceMethod<"getCapabilities"> = (
		instanceId,
	) =>
		registry
	...
	const getInstanceInfo: ProviderServiceMethod<"getInstanceInfo"> = (
		instanceId,
	...
	const rollbackConversation: ProviderServiceMethod<"rollbackConversation"> =
		Effect.fn("rollbackConversation")(function* (rawInput) {
			const input = yield* decodeInputOrValidationError({
	...
	const uploadFeedback: ProviderServiceMethod<"uploadFeedback"> = Effect.fn(
		"uploadFeedback",
	)(function* (rawInput) {
		const input = yield* decodeInputOrValidationError({
	...
	const runStopAll = Effect.fn("runStopAll")(function* () {
		...
		const activeSessions = yield* Effect.forEach(
			...
			Effect.map((sessionsByAdapter) =>
				sessionsByAdapter.flatMap((sessions) => sessions),
			),
		);
