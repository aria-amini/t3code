...
it.layer(makeDirectoryLayer(SqlitePersistenceMemory))(
	...
	(it) => {
		it.effect("upserts and reads thread bindings", () =>
			Effect.gen(function* () {
				...
				expect(threadIds).toEqual(
					expect.arrayContaining([initialThreadId, nextThreadId]),
				);
			}),
		);
		...
		it.effect(
			"records source files without replacing the current provider session",
			() =>
				Effect.gen(function* () {
					const directory = yield* ProviderSessionDirectory;
					const repository =
					...
					const source = {
						...importedSource,
						providerSessionId: "record-source",
					};
					...
					const before = Option.getOrThrow(
						yield* repository.getByThreadId({ threadId }),
					);
					...
					const secondFile = {
						...source,
						filePath: "/tmp/provider-session-copy.jsonl",
					};
					yield* directory.recordImportedTranscript({
						threadId,
						source: secondFile,
					...
					expect(
						Option.getOrThrow(yield* repository.getByThreadId({ threadId })),
					).toEqual({
						...before,
						runtimePayload: {
							...runtimePayload,
							importedTranscripts: [replacement, secondFile],
						},
					});
				}),
		);
		...
		it.effect(
			"does not create a binding when recording an imported transcript",
			() =>
				Effect.gen(function* () {
					const directory = yield* ProviderSessionDirectory;
					...
					expect(Option.isNone(yield* directory.getBinding(threadId))).toBe(
						true,
					);
				}),
		);
		...
		it.effect(
			"keeps newly recorded sources when a runtime write uses a stale payload",
			() =>
				Effect.gen(function* () {
					const directory = yield* ProviderSessionDirectory;
					const repository =
					...
					const firstSource = {
						...importedSource,
						providerSessionId: "stale-source",
					};
					const threadId = ThreadId.make(
					...
					const secondSource = {
						...firstSource,
						filePath: "/tmp/stale-source-copy.jsonl",
					};
					yield* directory.recordImportedTranscript({
						threadId,
						source: secondSource,
		...
		it.effect(
			"reserves imported source records for the atomic recording method",
			() =>
				Effect.gen(function* () {
					const directory = yield* ProviderSessionDirectory;
					for (const onConflict of ["update", "ignore"] as const) {
						const source = {
							...importedSource,
							providerSessionId: `reserved-source-${onConflict}`,
						};
						const threadId = ThreadId.make(
							`import:${source.providerInstanceId}:${source.providerSessionId}`,
						);
		...
		it.effect(
			"lists persisted bindings with metadata in oldest-first order",
			() =>
				Effect.gen(function* () {
					const directory = yield* ProviderSessionDirectory;
					const runtimeRepository =
					...
					const bindings = (yield* directory.listBindings()).filter(
						(binding) =>
							binding.threadId === olderThreadId ||
							binding.threadId === newerThreadId,
					);
		...
		it.effect("rehydrates persisted mappings across layer restart", () =>
			Effect.gen(function* () {
				const tempDir = NodeFS.mkdtempSync(
					NodePath.join(NodeOS.tmpdir(), "t3-provider-directory-"),
				);
