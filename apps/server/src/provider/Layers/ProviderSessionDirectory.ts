...
const decodeProviderDriverKindValue =
	Schema.decodeUnknownEffect(ProviderDriverKind);
		...
		Effect.map(
			(provider) =>
				({
					...
					// from a driver kind.
					providerInstanceId:
						runtime.providerInstanceId ?? defaultInstanceIdForDriver(provider),
		...
		(input) =>
			repository
				...
				.pipe(
					Effect.mapError(
						toPersistenceError(
							"ProviderSessionDirectory.recordImportedTranscript",
						),
					),
				);
