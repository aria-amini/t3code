...
describe("codexRateLimitsToLimits", () => {
	...
	it.each([undefined, null, {}])(
		...
		(rateLimitsByLimitId) => {
			...
			expect(
				codexRateLimitsToLimits({ checkedAt, snapshot, rateLimitsByLimitId }),
			).toEqual(codexRateLimitsToLimits({ checkedAt, snapshot }));
		},
	);
...
describe("codexResetCreditsToContract", () => {
	it("counts available credits and reports the soonest expiry", () => {
		...
		expect(codexResetCreditsToContract({ availableCount: 0 })).toEqual({
			availableCount: 0,
		});
