...
export function codexResetCreditsToContract(
	...
	const nextExpiresAt =
		expiries.length > 0
			? isoFromEpochSeconds(Math.min(...expiries))
			: undefined;
	return {
...
export function codexRateLimitsToLimits(input: {
	...
	const windows = codexRateLimitsToWindows(
		input.rateLimitsByLimitId?.codex ?? input.snapshot,
	);
	return {
		...makeUsageLimits({
 ...
 */
export function codexRateLimitsFailureMessage(
...
function codexUsageLimitNextStep(
...
export function codexUsageLimitMessage(
	...
	const windows =
		snapshot && Number.isFinite(atMs) ? codexRateLimitsToWindows(snapshot) : [];
	let reset = "";
