...
const ModelManifestEnvelopeSchema = Schema.Struct({
	...
	providers: Schema.optional(
		Schema.Record(Schema.String, ManifestProviderCatalog),
	),
});
	...
	Object.values(manifest.providers ?? {}).every((catalog) => {
		...
		const modelsAreValid = catalog.models.every((model) => {
			...
			return (
				...
				catalog.profiles[model.profile] !== undefined
			);
		});
		return (
			modelsAreValid &&
			(catalog.defaults?.chat === undefined || slugs.has(catalog.defaults.chat))
		);
	});
...
export function resolveProviderCatalog(
	...
	if (catalog.defaults?.chat !== undefined && !seen.has(catalog.defaults.chat))
		return null;
...
export function applyManifestDefault(
	...
	if (slug === undefined || !models.some((model) => model.slug === slug))
		return models;
	const previous = models.find(
		(model) => model.isDefault && model.slug !== slug,
	);
...
export const make = Effect.gen(function* () {
	...
	const ensureDiskCacheLoaded = yield* Effect.cached(
		Effect.gen(function* () {
			...
			if (
				...
				manifestUpdatedAtMs(fromDisk.manifest)
			) {
				return;
			}
	...
	const refresh = Effect.fn("ModelManifest.refresh")(function* () {
		...
		if (settings !== null && !settings.enableProviderUpdateChecks)
			return manifest;
		...
		yield* encodeManifestCache({ fetchedAtMs: now, manifest: fetched }).pipe(
			Effect.flatMap((serialized) =>
				fileSystem.writeFileString(cachePath, serialized),
			),
	...
	return ModelManifest.of({
		...
		refreshInBackground: Effect.forkIn(guardedRefresh, serviceScope).pipe(
			Effect.asVoid,
		),
	});
});
