	...
	Effect.gen(function* () {
		...
		yield* Effect.scoped(
			Effect.gen(function* () {
				...
				expect(yield* Fiber.join(fibers)).toEqual([
					"http://127.0.0.1:1",
					"http://127.0.0.1:1",
				]);
...
it.effect(
	"invalidates an exited process so the next borrower starts a new one",
	() =>
		Effect.gen(function* () {
			const starts = yield* Ref.make(0);
		...
		const runtime: OpenCodeRuntimeShape = {
			startOpenCodeServerProcess: () =>
				Effect.gen(function* () {
					...
					yield* Effect.addFinalizer(() =>
						index === 1
							? Deferred.succeed(firstStartClosed, undefined).pipe(
									Effect.ignore,
								)
							: Effect.void,
					);
...
it.effect(
	"releases an interrupted borrower and closes after the idle TTL",
	() =>
		Effect.gen(function* () {
			const testRuntime = yield* makeRuntime;
			...
			yield* Effect.scoped(
				Effect.gen(function* () {
					...
					const borrower = yield* owner
						.withServer(() =>
							Deferred.succeed(borrowerEntered, undefined).pipe(
