...
export const make = Effect.fn("OpenCodeServerOwner.make")(function* (input: {
	...
	const cancelIdleClose = Effect.fn("OpenCodeServerOwner.cancelIdleClose")(
		function* () {
	...
	const watchServerExit = Effect.fn("OpenCodeServerOwner.watchServerExit")(
		function* (server: OpenCodeRuntime.OpenCodeServerProcess) {
	...
	const releaseServer = (server: OpenCodeRuntime.OpenCodeServerProcess) =>
		mutex.withPermit(
			Effect.gen(function* () {
				...
				yield* cancelIdleClose();
				state.idleCloseFiber = yield* Effect.sleep(
					OPENCODE_SERVER_IDLE_TTL,
				).pipe(
					Effect.andThen(
						mutex.withPermit(
							Effect.gen(function* () {
