...
describe("AcpSessionRuntime", () => {
	for (const setupMethod of ["session/new", "session/resume"] as const) {
		it.effect(
			`buffers root metadata while ${setupMethod} startup is still pending`,
			() =>
				Effect.gen(function* () {
					...
					expect(events[0]).toMatchObject({
						availableCommands: [
							{ name: "plan", description: "Native command" },
						],
					});
					...
					expect(
						(yield* runtime.getConfigOptions).find(
	...
	it.effect(
		"publishes model changes returned by a config request and live notifications",
		() =>
			Effect.gen(function* () {
				...
				const updates = yield* Stream.toPull(
					runtime
						.getEvents()
						.pipe(
							Stream.filter((event) => event._tag === "ConfigOptionsUpdated"),
						),
				);
				const selected = yield* runtime.setConfigOption("model", "composer-2");
				expect((yield* updates)[0]?.configOptions).toEqual(
	...
	it.effect(
		"awaits native resume instead of using the load replay idle fallback",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"waits for native cancellation and drains final updates before another prompt",
		() =>
			Effect.gen(function* () {
					...
					expect(
						events.filter(
							(event) =>
								...
								event.itemId === cancelledDelta.itemId,
						),
					).toHaveLength(1);
				}
	...
	it.effect("reports an idle child exit and rejects later prompts", () =>
		Effect.gen(function* () {
			...
			expect(yield* runtime.notify("_test/exit", {}).pipe(Effect.flip)).toBe(
				event.error,
			);
		}).pipe(Effect.scoped, Effect.provide(NodeServices.layer)),
	);
	...
	it.effect(
		"retires a native runtime when its prompt caller is interrupted",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"fails a pending request when the stderr handler rejects the runtime",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"drains large stderr output and keeps auth-sized logging chunks",
		() =>
			Effect.gen(function* () {
				...
				for (const logStderr of [false, true]) {
					yield* Effect.gen(function* () {
						...
						expect(yield* runtime.initialize()).toMatchObject({
							protocolVersion: 1,
						});
					}).pipe(Effect.scoped);
				}
			...
			yield* runtime.getEvents().pipe(
				Stream.runForEach((event) =>
					event._tag === "EventStreamBarrier"
						? Deferred.succeed(barrierReceived, undefined).pipe(
	...
	it.effect(
		"bounds native cancellation when its event consumer is absent",
		() =>
			Effect.gen(function* () {
				const toolStarted = yield* Deferred.make<void>();
				const cancelReceived = yield* Deferred.make<void>();
				...
				yield* runtime.handleSessionUpdate((notification) => {
					...
					if (notification.update.sessionUpdate === "agent_thought_chunk") {
						return Deferred.succeed(cancelReceived, undefined).pipe(
							Effect.asVoid,
						);
					}
					return Effect.void;
	...
	it.effect(
		"does not restore ambient variables to a sanitized child environment",
		() =>
			Effect.gen(function* () {
					...
					(previous) =>
						Effect.sync(() => {
							if (previous === undefined)
								delete process.env.T3_ACP_RUNTIME_AMBIENT;
	...
	it.effect(
		"merges custom initialize client capabilities into the ACP handshake",
		() => {
			...
			return Effect.gen(function* () {
				...
				const initializeStarted = requestEvents.find(
					(event) =>
						event.method === "initialize" && event.status === "started",
				);
	...
	it.effect(
		"starts a session, prompts, and emits normalized events against the mock agent",
		() =>
			Effect.gen(function* () {
				const runtime = yield* AcpSessionRuntime.AcpSessionRuntime;
	...
	it.effect(
		"keeps assistant item IDs unique when a provider session restarts",
		() => {
			const collectFirstAssistantItemId = Effect.gen(function* () {
				...
				const assistantStart = events.find(
					(event) => event._tag === "AssistantItemStarted",
				);
				expect(assistantStart?._tag).toBe("AssistantItemStarted");
				return assistantStart?._tag === "AssistantItemStarted"
	...
	it.effect(
		"releases a fully silent prompt when session/cancel is requested",
		() =>
			Effect.gen(function* () {
				const runtime = yield* AcpSessionRuntime.AcpSessionRuntime;
				yield* runtime.start();
	...
	it.effect("segments assistant text around ACP tool calls", () =>
		Effect.gen(function* () {
			...
			const notes = Array.from(
				yield* Stream.runCollect(Stream.take(runtime.getEvents(), 7)),
			);
			expect(notes.map((note) => note._tag)).toEqual([
				"AssistantItemStarted",
				"ContentDelta",
	...
	it.effect("emits status-only tool updates through completion", () =>
		Effect.gen(function* () {
			...
			const notes = Array.from(
				yield* Stream.runCollect(Stream.take(runtime.getEvents(), 3)),
			);
			expect(notes.map((note) => note._tag)).toEqual([
				"ToolCallUpdated",
				"ToolCallUpdated",
	...
	it.effect(
		"skips no-op session config writes when the requested value is already active",
		() => {
			const requestEvents: Array<AcpSessionRuntime.AcpSessionRequestLogEvent> =
				[];
			return Effect.gen(function* () {
	...
	it.effect(
		"emits low-level ACP protocol logs for raw and decoded messages",
		() => {
			...
			return Effect.gen(function* () {
				...
				expect(
					protocolEvents.some(
						(event) => event.direction === "outgoing" && event.stage === "raw",
					),
				).toBe(true);
				expect(
					protocolEvents.some(
						(event) =>
							event.direction === "outgoing" && event.stage === "decoded",
					),
				).toBe(true);
				expect(
					protocolEvents.some(
						(event) => event.direction === "incoming" && event.stage === "raw",
					),
				).toBe(true);
				expect(
					protocolEvents.some(
						(event) =>
							event.direction === "incoming" && event.stage === "decoded",
					),
				).toBe(true);
			}).pipe(
				Effect.provide(
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				expect(notes.some((note) => note._tag === "ToolCallUpdated")).toBe(
					false,
				);
			}).pipe(
				Effect.provide(
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				const unexpectedReplayEvent = yield* Stream.runHead(
					runtime.getEvents(),
	...
	it.effect(
		"rejects invalid config option values before sending session/set_config_option",
		() => {
			const tempDir = NodeFS.mkdtempSync(
				NodePath.join(NodeOS.tmpdir(), "acp-runtime-"),
			);
			...
			return Effect.gen(function* () {
				...
				const error = yield* runtime
					.setModel("composer-2[fast=false]")
					.pipe(Effect.flip);
				expect(error._tag).toBe("AcpRequestError");
