...
nodeServicesIt("ACP native logging", (it) => {
	it.effect(
		"records bounded request and protocol diagnostics without raw payloads",
		() =>
			Effect.gen(function* () {
				...
				yield* requestLogger({
					...
					status: "failed",
					cause: Cause.fail(
						AcpErrors.AcpRequestError.internalError(secret, { token: secret }),
					),
				});
	...
	it.effect(
		"keeps request diagnostics without enabling full protocol logging",
		() =>
			Effect.gen(function* () {
				const records: Array<unknown> = [];
				const makeLogger = yield* makeAcpNativeLoggerFactory();
	...
	it.effect(
		"drops transient ACP chunks before formatting verbose protocol logs",
		() =>
			Effect.gen(function* () {
				const records: Array<unknown> = [];
				const makeLogger = yield* makeAcpNativeLoggerFactory();
	...
	it.effect("keeps mixed and incomplete raw diagnostics", () =>
		Effect.gen(function* () {
			...
			const lifecycle = encodeUnknownJson({
				method: "session/new",
				params: {},
			});
	...
	it.effect("logs a structural tag when the native writer defects", () => {
		...
		return Effect.gen(function* () {
			...
			Effect.provide(Logger.layer([logCapture], { mergeWithExisting: false })),
		);
	});
