...
const transientProtocolUpdates = new Set([
	"agent_message_chunk",
	"agent_thought_chunk",
]);

function structuralMethod(value: string): string {
	return value.length <= 128 && /^[A-Za-z][A-Za-z0-9._:/-]*$/.test(value)
		? value
		: "unknown";
}
...
function formatRequestLogPayload(
	event: AcpSessionRuntime.AcpSessionRequestLogEvent,
) {
	return {
...
function formatProtocolLogPayload(
...
function isTransientProtocolMessage(message: unknown): boolean {
	...
	const payload =
		Reflect.get(message, "payload") ?? Reflect.get(message, "params");
	if (typeof payload !== "object" || payload === null) return false;
	const update = Reflect.get(payload, "update");
	if (typeof update !== "object" || update === null) return false;
	const updateType = Reflect.get(update, "sessionUpdate");
	return (
		typeof updateType === "string" && transientProtocolUpdates.has(updateType)
	);
}
...
function filterTransientProtocolLog(
	...
	if (event.stage === "raw" && typeof event.payload === "string") {
		return rawChunkContainsOnlyTransientMessages(event.payload)
			? undefined
			: event;
	}
	...
	const payload = event.payload.filter(
		(message) => !isTransientProtocolMessage(message),
	);
	return payload.length === 0 ? undefined : { ...event, payload };
}

export const makeAcpNativeLoggerFactory = Effect.fn(
	"makeAcpNativeLoggerFactory",
)(function* () {
