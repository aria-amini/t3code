...
function boundToolCallRawOutput(rawOutput: unknown): unknown {
	...
	for (const field of RAW_OUTPUT_TEXT_FIELDS) {
		...
		if (
			typeof value === "string" &&
			value.length > TOOL_CALL_CONTENT_MAX_CHARS
		) {
...
function distributeRetainedTailAcrossContent(
	...
	return content.flatMap((entry, index) => {
		...
		return [
			{ type: "content", content: { type: "text", text: piece } } as const,
		];
	});
}
...
function makeToolCallState(
	input: {
		...
		readonly content?:
			...
			| undefined;
		readonly locations?:
			| ReadonlyArray<EffectAcpSchema.ToolCallLocation>
			| null
			| undefined;
	},
	options?: {
...
function toolCallOutputUnchanged(
...
export function toolCallProgressLength(state: AcpToolCallState): number {
	...
	if (Array.isArray(content)) {
		for (const entry of content) {
			...
			const text = toolCallContentText(
				entry as EffectAcpSchema.ToolCallContent,
			);
			if (text) {
...
export function decideToolCallUpdateEmission(
	...
	if (
		grewMeaningfully ||
		skippedSinceEmit + 1 >= TOOL_CALL_UPDATE_COALESCE_LIMIT
	) {
		return { emit: true, skippedSinceEmit: 0 };
	}
...
function boundToolCallRawPayload(
	...
	const contentBounded =
		update.content !== undefined && boundedContent !== update.content;
	const rawOutputBounded =
		update.rawOutput !== undefined && boundedRawOutput !== update.rawOutput;
