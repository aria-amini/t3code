...
export interface AcpSessionRuntimeOptions {
	...
	readonly requestLogger?: (
		event: AcpSessionRequestLogEvent,
...
export class AcpSessionRuntime extends Context.Service<
	AcpSessionRuntime,
	{
		...
		readonly setMode: (
			...
			EffectAcpSchema.SetSessionModeResponse,
			EffectAcpErrors.AcpError
		>;
		/**
		...
		readonly setSessionModel: (
			...
			EffectAcpSchema.SetSessionModelResponse,
			EffectAcpErrors.AcpError
		>;
		/**
...
> =>
	Effect.gen(function* () {
		...
		const toolCallsRef = yield* Ref.make(
			new Map<string, AcpToolCallTrackedState>(),
		);
			...
			Effect.mapError(
				(cause) =>
					new EffectAcpErrors.AcpTransportError({
						detail:
							"Failed to generate an ACP assistant item runtime identifier.",
						cause,
					}),
			),
		);
		...
		const configOptionsRef = yield* Ref.make(
			sessionConfigOptionsFromSetup(undefined),
		);
		const startStateRef = yield* Ref.make<AcpStartState>({
			_tag: "NotStarted",
		});
		const startupMetadataRef = yield* Ref.make<
			ReadonlyArray<EffectAcpSchema.SessionNotification>
		>([]);
		const notificationSemaphore = yield* Semaphore.make(1);
		const terminationErrorRef = yield* Ref.make<
			Option.Option<EffectAcpErrors.AcpError>
		>(Option.none());
		const stoppingRef = yield* Ref.make(false);
		const stderrFailure = yield* Deferred.make<
			never,
			EffectAcpErrors.AcpError
		>();
		...
		const activePromptRef = yield* Ref.make<Option.Option<AcpActivePrompt>>(
			Option.none(),
		);
		const sessionLoadGateRef = yield* Ref.make<Option.Option<SessionLoadGate>>(
			Option.none(),
		);
		...
		const recordTermination = Effect.fn("AcpSessionRuntime.recordTermination")(
			function* (error: EffectAcpErrors.AcpError) {
				if (yield* Ref.get(stoppingRef)) {
					return;
				}
				const firstTermination = yield* Ref.modify(
					terminationErrorRef,
					(current) =>
		...
		const acpContext = yield* Layer.build(
			EffectAcpClient.layerChildProcess(child, {
				...(options.transformStdout
					? { transformStdout: options.transformStdout }
					: {}),
		...
		yield* Scope.addFinalizer(
			runtimeScope,
			Ref.set(stoppingRef, true).pipe(
				Effect.andThen(Deferred.succeed(runtimeClosed, undefined)),
			),
		);
		const initializeClientCapabilities = {
			...
			...(options.clientCapabilities?.auth
				? { auth: options.clientCapabilities.auth }
				: {}),
		...
		const validateConfigOptionValue = (
			...
			Effect.gen(function* () {
				const configOption = findSessionConfigOption(
					yield* Ref.get(configOptionsRef),
					configId,
				);
				if (!configOption) {
					return;
		...
		const updateConfigOptions = Effect.fn(
			"AcpSessionRuntime.updateConfigOptions",
		)(function* (response: EffectAcpSchema.SetSessionConfigOptionResponse) {
				...
				Effect.flatMap((started) =>
					Ref.get(configOptionsRef).pipe(
						Effect.flatMap((configOptions) => {
							...
							if (
								existing &&
								configOptionCurrentValueMatches(existing, value)
							) {
							...
							Option.isSome(result)
								...
								: Effect.fail(
										new EffectAcpErrors.AcpTransportError({
											...
											method: "session/resume",
											detail:
												"session/resume timed out waiting for the agent response.",
											cause: undefined,
										}),
									),
						),
							...
							startOnce.pipe(
								Effect.tap((result) =>
									notificationSemaphore.withPermit(
										Effect.gen(function* () {
											...
											const metadata = yield* Ref.getAndSet(
												startupMetadataRef,
												[],
											);
								...
								Effect.onError((cause) =>
									Deferred.failCause(deferred, cause).pipe(
										Effect.andThen(
											Ref.set(startStateRef, { _tag: "NotStarted" }),
										),
		...
		const drainEvents = Effect.gen(function* () {
			...
			yield* Effect.raceFirst(
				...
				Deferred.await(runtimeClosed),
			);
		});

		const retireRuntime = Effect.fn("AcpSessionRuntime.retireRuntime")(
			function* (error: EffectAcpErrors.AcpError) {
				yield* recordTermination(error);
		...
		const cancel = Effect.gen(function* () {
			...
			const completed = yield* Effect.gen(function* () {
				...
				Effect.timeoutOption(options.cancelTimeout ?? defaultCancelTimeout),
			);
		...
		return {
								...
								if (
									...
									yield* retireRuntime(
										new EffectAcpErrors.AcpTransportError({
											...
											detail:
												"The ACP prompt stopped before the agent confirmed completion.",
											cause: undefined,
										}),
									);
								}
				...
				Ref.get(modeStateRef).pipe(
					Effect.flatMap((modeState) => {
						if (modeState?.currentModeId === modeId) {
							return Effect.succeed(
								{} satisfies EffectAcpSchema.SetSessionModeResponse,
							);
						}
			...
			setModel: (model) =>
				getStartedState.pipe(
					Effect.flatMap((started) =>
						setConfigOption(started.modelConfigId ?? "model", model),
					),
					Effect.asVoid,
				),
...
function isStartupMetadataUpdate(
	notification: EffectAcpSchema.SessionNotification,
): boolean {
...
const handleSessionUpdate = ({
	...
	readonly configOptionsRef: Ref.Ref<
		ReadonlyArray<EffectAcpSchema.SessionConfigOption>
	>;
	...
	Effect.gen(function* () {
		...
		if (parsed.modeId) {
			yield* Ref.update(modeStateRef, (current) =>
				current === undefined
					? current
					: updateModeState(current, parsed.modeId!),
			);
		}
		for (const event of parsed.events) {
			if (event._tag === "ToolCallUpdated") {
				...
				const { merged, decision } = yield* Ref.modify(
					toolCallsRef,
					(current) => {
...
const closeActiveAssistantSegment = ({
	...
	Ref.modify(assistantSegmentRef, (current) => {
		...
		Effect.flatMap((event) =>
			event ? Queue.offer(queue, event) : Effect.void,
		),
	);
