...
function makeModelRuntime(
	...
	const setModel = Effect.fn("AntigravityAcpSupportTest.setModel")(function* (
		model: string,
	) {
...
describe("applyAntigravityAcpModelSelection", () => {
	...
	it.effect(
		"reapplies an explicit selection even when setup reports the same model",
		() =>
			Effect.gen(function* () {
				const { runtime, selections } = makeModelRuntime([
	...
	it.effect(
		"selects the manifest default for the alias when the account offers it",
		() =>
			Effect.gen(function* () {
				const { runtime, selections } = makeModelRuntime();
				const model = yield* applyAntigravityAcpModelSelection({
				...
				const { runtime: other, selections: otherSelections } =
					makeModelRuntime();
			...
			const { runtime, selections } = makeModelRuntime([
				{
					...modelConfig,
					options: [
						{ group: "gemini", name: "Gemini", options: modelConfig.options },
					],
				},
			]);
			const model = yield* applyAntigravityAcpModelSelection({
	...
	it.effect(
		"reports a native model-selection failure through the adapter error mapper",
		() =>
			Effect.gen(function* () {
				const nativeError = EffectAcpErrors.AcpRequestError.invalidParams(
...
const makeAttachmentFixture = Effect.fn(
	"AntigravityAcpSupportTest.makeAttachmentFixture",
)(function* () {
	...
	const write = Effect.fn("AntigravityAcpSupportTest.writeAttachment")(
		function* (attachment: ChatAttachment, content: string | Uint8Array) {
...
it.layer(NodeServices.layer)("buildAntigravityPrompt", (it) => {
	it.effect(
		"sends image bytes as native image content alongside the user prompt",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"embeds UTF-8 code from the selected environment and keeps the upload in place",
		() =>
			Effect.gen(function* () {
				...
				expect(
					yield* selectedEnvironment.fs.readFileString(upload.filePath),
				).toBe(source);
				expect(
					yield* selectedEnvironment.fs.readDirectory(
						selectedEnvironment.attachmentsDir,
					),
				).toHaveLength(1);
			}),
	);
	...
	it.effect(
		"uses a PDF file resource link without copying or reading its bytes",
		() =>
			Effect.gen(function* () {
				const fixture = yield* makeAttachmentFixture();
			...
			Effect.gen(function* () {
				...
				expect(error).toMatchObject({
					...
					errorMessage: expect.stringContaining(
						`does not support '${attachment.name}'`,
					),
				});
			}),
	);
	...
	it.effect.each([
		{
			...
			bytes: ANTIGRAVITY_MAX_TEXT_ATTACHMENT_BYTES + 1,
		},
		{
			attachment: imageAttachment,
			bytes: PROVIDER_SEND_TURN_MAX_IMAGE_BYTES + 1,
		},
	...
	it.effect(
		"accepts 50 MiB in total but rejects one byte more across files",
		() =>
			Effect.gen(function* () {
				const fixture = yield* makeAttachmentFixture();
				...
				yield* fixture.fs.truncate(
					first.filePath,
					PROVIDER_SEND_TURN_MAX_FILE_BYTES / 2,
				);
				yield* fixture.fs.truncate(
					second.filePath,
					PROVIDER_SEND_TURN_MAX_FILE_BYTES / 2,
				...
				yield* fixture.fs.truncate(
					second.filePath,
					PROVIDER_SEND_TURN_MAX_FILE_BYTES / 2 + 1,
				);
	...
	it.effect(
		"rejects aggregate overflow when a text upload grows after its size check",
		() =>
			Effect.gen(function* () {
				const fixture = yield* makeAttachmentFixture();
				...
				const error = yield* buildAntigravityPrompt({
					...
					Effect.provideService(FileSystem.FileSystem, {
						...fixture.fs,
						stat: Effect.fn("AntigravityAcpSupportTest.growAfterStat")(
				...
				expect(error).toMatchObject({
					...
					code: -32602,
					errorMessage:
						"Attachment 'example.tsx' changed while being read and is too large.",
				});
			}),
	);
	...
	it.effect.each([
		{
			...
			message: "is not a UTF-8 text file",
		},
	...
	it.effect(
		"reports a missing upload instead of sending only the remaining text",
		() =>
			Effect.gen(function* () {
				const fixture = yield* makeAttachmentFixture();
				const error = yield* buildAntigravityPrompt({
