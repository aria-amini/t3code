...
export const makeAntigravityAcpRuntime = Effect.fn("makeAntigravityAcpRuntime")(
	function* (
...
export function resolveAntigravityModel(input: {
	...
	const modelConfig = input.configOptions.find(
	...
	return input.defaultModel &&
		options.some((option) => option.value === input.defaultModel)
		? input.defaultModel
		: current;
}

/** Never replace a saved selection with the default returned by a cold resume. */
export const applyAntigravityAcpModelSelection = Effect.fn(
	...
	const explicit =
		Boolean(input.model) && input.model !== ANTIGRAVITY_DEFAULT_MODEL;
...
export const buildAntigravityPrompt = Effect.fn("buildAntigravityPrompt")(
	function* (input: {
	...
	> {
		...
		for (const attachment of input.attachments ?? []) {
			...
			const image =
				attachment.type === "image" && IMAGE_MIME_TYPES.has(mimeType);
			const audio =
				attachment.type === "file" && AUDIO_MIME_TYPES.has(mimeType);
			const pdf = attachment.type === "file" && mimeType === "application/pdf";
			const textFile =
				attachment.type === "file" &&
				(mimeType.startsWith("text/") ||
					TEXT_MIME_TYPES.has(mimeType) ||
					TEXT_FILE_EXTENSIONS.has(
			...
			const bytes = yield* fileSystem
				.stream(attachmentPath, { bytesToRead: limit + 1 })
				.pipe(
					Stream.runCollect,
			...
			if (image) {
				...
				blocks.push({
					type: "resource",
					resource: { uri, mimeType, text: decoded },
				});
			}
		}
		if (blocks.length === 0) {
