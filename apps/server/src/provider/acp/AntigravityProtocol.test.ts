...
describe("native Antigravity subagent tools", () => {
	it("recognizes only native invocation titles and excludes MCP tools", () => {
		...
		for (const title of ["Running start_subagent", "Run start_subagent?"]) {
			expect(
				classifyAntigravitySubagentToolCall({ ...toolCall, title }, {}),
			).toBe("subagent");
			expect(
				classifyAntigravitySubagentToolCall(
					{ ...toolCall, title },
...
function parseToolUpdate(notification: EffectAcpSchema.SessionNotification) {
	const result = parseSessionUpdateEvent(
		normalizeAntigravitySessionUpdate(notification),
	);
...
describe("Antigravity permissions and questions", () => {
	...
	it("uses only the option IDs offered for each approval decision", () => {
		expect(
			selectAntigravityPermissionOptionId(permissionRequest, "accept"),
		).toBe("run-this-time");
		expect(
			selectAntigravityPermissionOptionId(
				permissionRequest,
		...
		).toBe("remember-this-command");
		expect(
			selectAntigravityPermissionOptionId(permissionRequest, "acceptAlways"),
		).toBe("remember-this-command");
		expect(
			selectAntigravityPermissionOptionId(permissionRequest, "decline"),
		).toBe("stop-this-command");
		expect(
			selectAntigravityPermissionOptionId(permissionRequest, "cancel"),
		).toBeUndefined();
	});
	...
	it("surfaces the agent's prompt injection warning on the remembered approval", () => {
		...
		expect(antigravityApprovalOptions(permissionRequest)[1]).not.toHaveProperty(
			"warning",
		);
	});
	...
	it("does not replace unsupported remembered approval with a single approval", () => {
		const request = {
			...permissionRequest,
			options: permissionRequest.options.filter(
				(option) => option.kind !== "allow_always",
			),
		};
		expect(
			selectAntigravityPermissionOptionId(request, "acceptAlways"),
		).toBeUndefined();
		expect(
			selectAntigravityPermissionOptionId(request, "acceptForSession"),
		).toBeUndefined();
		expect(selectAntigravityPermissionOptionId(request, "accept")).toBe(
			"run-this-time",
		);
		expect(
			selectAntigravityPermissionOptionId(
	...
	it("routes the captured native question to single-choice input, not approval", () => {
		...
		expect(
			selectAntigravityPermissionOptionId(questionRequest, "accept"),
		).toBeUndefined();
		...
		expect(
			extractAntigravityUserInputQuestion(permissionRequest),
		).toBeUndefined();
	});
	...
	it("returns exact opaque IDs and accepts a unique label from an older client", () => {
		...
		expect(
			makeAntigravityUserInputResponse(request, {
				interaction_9960062f: " choice: opaque ",
			}),
		).toEqual({
			outcome: { outcome: "selected", optionId: " choice: opaque " },
		});
	});
	...
	it("preserves duplicate labels and rejects ambiguous label answers", () => {
		...
		expect(
			extractAntigravityUserInputQuestion(request)?.options.map(
				(option) => option.value,
			),
		).toEqual(["1", "2"]);
		expect(
			makeAntigravityUserInputResponse(request, {
				interaction_9960062f: "Same label",
			}),
		).toBeUndefined();
		expect(
			makeAntigravityUserInputResponse(request, { interaction_9960062f: "2" }),
		).toEqual({
	...
	it.each([
		...
		{ answer: "1" },
		1,
	])("keeps the question open for an unsupported answer: %j", (answer) => {
		expect(
			makeAntigravityUserInputResponse(questionRequest, {
				interaction_9960062f: answer,
			}),
	...
	it("rejects missing or duplicate native option IDs", () => {
		for (const optionId of ["", "1"]) {
			const request = {
				...questionRequest,
				options: [
					questionRequest.options[0]!,
					{ ...questionRequest.options[1]!, optionId },
				],
			};
	...
	it("bounds question text without changing choice IDs", () => {
		const request = {
			...
			toolCall: {
				...
				title: "Question ".repeat(2_000),
			},
			options: [
				{
					optionId: "stable-choice",
					name: "Choice ".repeat(2_000),
					kind: "allow_once" as const,
				},
			],
		};
...
describe("Antigravity tool results", () => {
	it("normalizes the captured command and completed output for the existing clients", () => {
		...
		const toolCall = normalizeAntigravityToolCall(
			mergeToolCallState(initial, completed),
		);
	...
	it("removes inline images while preserving valid tool content and the local image path", () => {
		...
		const notification = {
			...
			update: {
				...
				rawOutput: {
					imageName: "result",
					imagePath: "file:///workspace/brain/result%20image.png",
				},
				content: [
					{
						type: "content",
					...
					{
						type: "content",
						content: { type: "text", text: "Saved the image." },
					},
					{
						type: "diff",
		...
		expect(
			normalizeAntigravityToolCall(parseToolUpdate(normalized).toolCall).data
				.imagePath,
		).toBe("/workspace/brain/result image.png");
	});
	...
	it("keeps large assistant replies and startup command metadata unchanged", () => {
		...
		for (const notification of notifications) {
			expect(normalizeAntigravitySessionUpdate(notification)).toBe(
				notification,
			);
		}
	});
	...
	it("tracks only commands that have passed approval and are still running", () => {
		const running = normalizeAntigravityToolCall(
			parseToolUpdate(commandStarted).toolCall,
		);
		expect(isAntigravityOpenCommand(running)).toBe(true);
		expect(isAntigravityOpenCommand({ ...running, status: "pending" })).toBe(
			false,
		);
