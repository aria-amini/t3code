...
const decodeToolCallContent = Schema.decodeUnknownOption(
...
export function antigravitySecurityWarning(
	...
	const warning = Option.getOrUndefined(
		decodeSecurityWarning(meta[SECURITY_WARNING_META_KEY]),
	);
...
export function antigravityApprovalOptions(
	...
	const optionWithKind = (kind: EffectAcpSchema.PermissionOption["kind"]) =>
		request.options.find(
			(entry) => entry.kind === kind && entry.optionId.trim(),
		);
...
export function makeAntigravityUserInputResponse(
	...
	const answer = Option.getOrUndefined(
		decodeSingleAnswer(answers[request.toolCall.toolCallId]),
	);
	...
	const matchingLabels = request.options.filter(
		(option) => questionLabel(option) === value,
	);
	...
	return {
		...
		update: {
			...
			...(update._meta !== undefined
				? { _meta: Predicate.isObject(meta) ? meta : null }
				: {}),
		},
	};
}
...
export function normalizeAntigravityToolCall(
	...
	const input = Option.getOrUndefined(
		decodeNativeToolFields(toolCall.data.rawInput),
	);
	const output = Option.getOrUndefined(
		decodeNativeToolFields(toolCall.data.rawOutput),
	);
	const nativeCommand =
		input?.CommandLine ??
		input?.command_line ??
		...
		output?.command_line ??
		toolCall.command;
	const command = nativeCommand?.trim()
		? boundText(nativeCommand.trim())
		: undefined;
	const nativeCwd =
		input?.Cwd ??
	...
	const aggregatedOutput =
		nativeOutput === undefined ? undefined : boundText(nativeOutput);
	...
	const data: Record<string, unknown> = Predicate.isObject(sanitizedData)
		? sanitizedData
		: {};
	...
	return {
		...
		...(toolCall.title !== undefined
			? { title: boundText(toolCall.title) }
			: {}),
		...(command !== undefined
			? { detail: command }
...
export function classifyAntigravitySubagentToolCall(
	...
	if (
		...
		(toolCall.title !== "Running start_subagent" &&
	...
	return Predicate.isObject(meta) && meta.is_mcp_tool_call === true
		? "mcp"
		: "subagent";
}
...
export function antigravitySubagentOutput(
	toolCall: AcpToolCallState,
): string | undefined {
	const output = toolCall.data.rawOutput;
	return typeof output === "string" && output.trim()
		? boundText(output.trim())
		: undefined;
}
