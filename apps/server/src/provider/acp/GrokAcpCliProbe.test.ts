...
describe.runIf(process.env.T3_GROK_ACP_PROBE === "1")(
	"Grok ACP CLI probe",
	() => {
		...
		it.effect(
			"session/new advertises typed SessionModelState with at least one model",
			() =>
				Effect.gen(function* () {
					const runtime = yield* makeProbeRuntime;
					const started = yield* runtime.start();
		...
		it.effect(
			"session/set_model accepts a no-op switch to the current model",
			() =>
				Effect.gen(function* () {
					const runtime = yield* makeProbeRuntime;
					const started = yield* runtime.start();
					const currentModelId =
						started.sessionSetupResult.models?.currentModelId?.trim();
		...
		it.effect(
			"session/set_model accepts advertised reasoning effort metadata",
			() =>
				Effect.gen(function* () {
					const runtime = yield* makeProbeRuntime;
					const started = yield* runtime.start();
		...
		it.effect.skipIf(process.env.T3_GROK_LIVE_TURN !== "1")(
			...
			() =>
				Effect.gen(function* () {
					...
					yield* runtime.start();
					const chunks: string[] = [];
					const events = yield* Stream.runForEach(
						runtime.getEvents(),
						(event) => {
					...
					const result = yield* runtime.prompt({
						prompt: [
							{
								type: "text",
								text: "Reply exactly GROK_T3_OK. Do not use any tools.",
							},
						],
					});
