...
describe("resolveGrokAcpBaseModelId", () => {
	it("normalizes empty and custom Grok model ids", () => {
		...
		expect(resolveGrokAcpBaseModelId("  grok-test-custom-model  ")).toBe(
...
describe("grokAcpSpawnArgs", () => {
	...
	it("maps Full access to Grok always-approve", () => {
		expect(grokAcpSpawnArgs("full-access")).toEqual([
			"agent",
			"--always-approve",
			"stdio",
		]);
	...
	it("maps Auto-accept edits and Auto onto Grok permission modes", () => {
		...
		expect(grokAcpSpawnArgs("auto")).toEqual([
			"--permission-mode",
			"auto",
			"agent",
			"stdio",
...
describe("applyGrokAcpModelSelection", () => {
	...
	it.effect(
		"calls session/set_model when the requested model differs from current",
		() =>
			Effect.gen(function* () {
	...
	it.effect("applies reasoning effort through session/set_model metadata", () =>
		Effect.gen(function* () {
			...
			expect(modelCalls).toEqual([
				{ modelId: "grok-4.6", meta: { reasoningEffort: "xhigh" } },
			]);
	...
	it.effect(
		"does not clear reasoning when same-model selection omits effort",
		() =>
			Effect.gen(function* () {
				const { runtime, modelCalls } = makeRecordingRuntime();
				const result = yield* applyGrokAcpModelSelection({
	...
	it.effect(
		"keeps the session's current model when the product slug is requested",
		() =>
			Effect.gen(function* () {
				const { runtime, modelCalls } = makeRecordingRuntime();
				const result = yield* applyGrokAcpModelSelection({
	...
	it.effect(
		"applies reasoning to the current model when the product slug is requested",
		() =>
			Effect.gen(function* () {
				const { runtime, modelCalls } = makeRecordingRuntime();
				const result = yield* applyGrokAcpModelSelection({
	...
	it.effect("propagates session/set_model failures via mapError", () =>
		Effect.gen(function* () {
			const failure = EffectAcpErrors.AcpRequestError.invalidParams(
