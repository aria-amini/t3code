...
> =>
	Effect.gen(function* () {
		...
		const runtime = yield* Effect.service(
			AcpSessionRuntime.AcpSessionRuntime,
		).pipe(Effect.provide(acpContext));
...
export function resolveGrokAcpBaseModelId(
	model: string | null | undefined,
): string {
	const trimmed = model?.trim();
	const base =
		trimmed && trimmed.length > 0 ? trimmed : GROK_DEFAULT_MODEL_SLUG;
...
export function applyGrokAcpModelSelection<E>(input: {
	...
	if (
		(!modelChanged && !reasoningEffortChanged) ||
		targetModelId === undefined
	) {
		return Effect.succeed(input.currentModelId);
	}
	const reasoningMeta =
		reasoningProvided && reasoningEffort !== undefined
			? { reasoningEffort }
			: undefined;
