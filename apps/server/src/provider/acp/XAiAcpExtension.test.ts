...
const decodeXAiAskUserQuestionRequest = Schema.decodeUnknownSync(
	XAiAskUserQuestionRequest,
);
...
describe("XAiAcpExtension", () => {
	...
	it.effect(
		"fails a hung standard prompt from an xAI rate-limit completion",
		() =>
			Effect.gen(function* () {
				const runtime = yield* makePromptCompletionRuntime({
	...
	it("extracts plan markdown from exit_plan_mode payloads", () => {
		...
		expect(extractXAiExitPlanMarkdown(wrapped, "  # fallback plan  ")).toBe(
			"# fallback plan",
		);
		expect(extractXAiExitPlanMarkdown(wrapped, "")).toBe(
			XAI_EMPTY_PLAN_MARKDOWN,
		);
	...
	it("identifies Grok plan.md paths and extracts markdown from tool call data", () => {
		...
		expect(
			isGrokPlanMarkdownPath("~/.grok/sessions/abc/plan.md", linuxHost),
		).toBe(true);
		expect(
			isGrokPlanMarkdownPath(
				"/tmp/mock-home/.grok/sessions/sess/plan.md",
				linuxHost,
			),
		).toBe(true);
		expect(
			isGrokPlanMarkdownPath(
				"/home/other/.grok/sessions/sess/plan.md",
				linuxHost,
			),
		).toBe(true);
		expect(
		...
		expect(
			isGrokPlanMarkdownPath(
				"C:/Users/other/.grok/sessions/id/plan.md",
				windowsHost,
			),
		).toBe(true);
		expect(
		...
		expect(
			isGrokPlanMarkdownPath(
				"/opt/grok-data/sessions/sess/plan.md",
				grokHomeHost,
			),
		).toBe(true);
		expect(
		...
		expect(
			isGrokPlanMarkdownPath("/repo/.grok/sessions/example/plan.md", linuxHost),
		).toBe(false);
		expect(
			isGrokPlanMarkdownPath(
				`${home}/project/.grok/sessions/example/plan.md`,
				linuxHost,
			),
		).toBe(false);
		expect(
			isGrokPlanMarkdownPath(
				"/home/other/.grok/sessions/../../project/plan.md",
				linuxHost,
			),
		).toBe(false);
		expect(
			isGrokPlanMarkdownPath(
				"/home/other/.grok/sessions/foo/../../../project/plan.md",
				linuxHost,
			),
		).toBe(false);
		...
		expect(
			extractGrokPlanMarkdownFromToolCallData(
				{
					content: [
						{ type: "diff", path: sessionPlan, oldText: "# old", newText: "" },
					],
				},
				linuxHost,
			),
