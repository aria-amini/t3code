...
function normalizeAnswerForXAi(
	...
	const selectedLabels = resolvedValues.flatMap(({ option }) =>
		option ? [option.label] : [],
	);
	...
	const preview =
		question.multiSelect === true
			? undefined
			: resolvedValues
					.map(({ option }) => trimmed(option?.preview))
					.find((value) => value);
...
export function makeXAiAskUserQuestionResponse(
	...
	const normalized = questions.flatMap((question) => {
		const entry = normalizeAnswerForXAi(
			question,
			findQuestionAnswer(answers, question),
		);
		return entry ? [entry] : [];
	});
...
function unwrapExitPlanModeParams(
	params: XAiExitPlanModeRequest,
): XAiExitPlanModeRequestParams {
	return "params" in params ? params.params : params;
}
...
export function extractXAiExitPlanMarkdown(
	...
	const fromRequest =
		typeof content === "string" ? trimmed(content) : undefined;
	if (fromRequest) {
		return fromRequest;
	}
...
export type XAiExitPlanModeOutcome =
	| "approved"
	| "abandoned"
	| "request_changes";
...
function grokPlanSessionPrefixes(
...
export function isGrokPlanMarkdownPath(
	...
	return (
		win32
			? CASE_INSENSITIVE_CANONICAL_HOME_GROK_SESSION_PATH
			: CANONICAL_HOME_GROK_SESSION_PATH
	).test(haystack);
}
...
export function extractGrokPlanMarkdownFromToolCallData(
	...
	if (isRecord(rawInput)) {
		...
		const content =
			typeof rawInput.content === "string" ? rawInput.content : undefined;
	...
	if (Array.isArray(content)) {
		for (const block of content) {
			...
			const newText =
				typeof block.newText === "string" ? block.newText : undefined;
 ...
 */
export const makeXAiPromptCompletionRuntime = Effect.fn(
	...
	return {
		...
		start: () =>
			runtime
				...
				.pipe(
					Effect.tap((started) =>
						Ref.set(activeSessionIdRef, started.sessionId),
					),
				),
		prompt: (payload, promptOptions?) =>
			Effect.gen(function* () {
				...
				return yield* Effect.raceFirst(
					...
					Effect.ensuring(
						unregisterXAiPromptCompletionFallback(
							pendingRef,
							fallback.deferred,
						),
					),
...
const unregisterXAiPromptCompletionFallback = (
	...
	Ref.update(pendingRef, (pending) =>
		pending.filter((entry) => entry.deferred !== deferred),
	);
	...
	if (notification.stopReason === "error") {
		return Deferred.fail(
			...
			EffectAcpErrors.AcpRequestError.internalError(
				xAiAgentResultMessage(notification.agentResult) ??
					"Grok prompt failed.",
			),
		).pipe(Effect.asVoid);
	}
	return Deferred.succeed(deferred, promptResponseFromXAi(notification)).pipe(
		Effect.asVoid,
	);
};
...
function normalizeXAiStopReason(
	value: string | undefined,
): EffectAcpSchema.StopReason {
	switch (value) {
		case "cancelled":
		case "end_turn":
		case "max_tokens":
