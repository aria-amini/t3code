...
describe("Antigravity process environment", () => {
	const profile: AntigravityProfile = {
		...
		tokenPath:
			"/t3/userdata/providers/antigravity/profile/antigravity-acp/acp_token.json",
	...
	it("isolates the profile and harness after merging overrides without changing the base environment", () => {
		...
		const spawn = buildAntigravityAcpSpawnInput({
			installation: {
				executablePath: "/release/acp",
				harnessPath: "/release/harness",
			},
			profile,
			cwd: "/project",
			baseEnv,
	...
	it("writes the auth method and GCP block into the agent's settings.json", () => {
		...
		expect(
			decodeJson(antigravityProfileSettings(ANTIGRAVITY_PERSONAL_AUTH)),
		).toEqual({
...
describe("Antigravity authorization URL", () => {
	it.effect(
		"returns the official Google URL and its owned loopback target",
		() =>
			Effect.gen(function* () {
				expect(
					yield* parseAntigravityAuthorizationUrl(authorizationUrl),
				).toEqual({
					authorizationUrl,
			...
			Effect.gen(function* () {
				const invalidUrls = [
					...
					authorizationUrl.replace(
						"accounts.google.com",
						"accounts.google.com.example.invalid",
					),
					authorizationUrl.replace(
						"accounts.google.com",
						"secret@accounts.google.com",
					),
				...
				for (const invalidUrl of invalidUrls) {
					const result = yield* parseAntigravityAuthorizationUrl(
						invalidUrl,
					).pipe(Effect.result);
					expect(Result.isFailure(result)).toBe(true);
					if (Result.isSuccess(result)) continue;
					expect(result.failure._tag).toBe("AcpTransportError");
					expect(encodeUnknownJson(result.failure)).not.toContain(
						"test-opaque-state",
					);
					expect(encodeUnknownJson(result.failure)).not.toContain(
						"secret-code",
...
describe("Antigravity sign-in errors", () => {
	it("recognizes native auth-required errors and the blocked-login transport error", () => {
		expect(
			isAntigravitySignInRequiredError(
				AcpErrors.AcpRequestError.authRequired(),
			),
		).toBe(true);
		expect(
			isAntigravitySignInRequiredError(
	...
	it("does not treat other failures or arbitrary text as missing authentication", () => {
		...
		for (const error of otherErrors)
			expect(isAntigravitySignInRequiredError(error)).toBe(false);
	});
});
...
describe("Antigravity stdout compatibility", () => {
	...
	it.effect(
		"ends normal work with a safe sign-in error instead of waiting for OAuth",
		() =>
			Effect.gen(function* () {
				const result = yield* makeAntigravityStdoutTransform()(
					Stream.make(encode(authLine)),
	...
	it.effect(
		"does not suppress malformed protocol data or similar login messages",
		() =>
			Effect.gen(function* () {
				const unrelated = [
					"this is not JSON\n",
...
describe("Antigravity stderr compatibility", () => {
	...
	it.effect(
		"forwards a fragmented browser-helper URL without exposing other stderr",
		() =>
			Effect.gen(function* () {
				const urls: string[] = [];
	...
	it.effect("ignores malformed and similar browser-helper messages", () =>
		Effect.gen(function* () {
			...
			yield* handleStderr(
				`${ANTIGRAVITY_AUTH_BROWSER_MARKER}${authorizationUrl}\n`,
			);
			yield* handleStderr(
				`${ANTIGRAVITY_AUTH_BROWSER_MARKER}${encodeUnknownJson("https://example.com")}\n`,
			);
			yield* handleStderr(
				`${ANTIGRAVITY_AUTH_STDOUT_PREFIX}https://example.com\n`,
			);
			yield* handleStderr(
				` ${ANTIGRAVITY_AUTH_STDOUT_PREFIX}${authorizationUrl}\n`,
			);
...
it.layer(NodeServices.layer)("Antigravity profile preparation", (it) => {
	it.effect(
		"preflights the no-browser helper and creates private directories only",
		() =>
			Effect.gen(function* () {
				...
				expect(profile.geminiHome).toBe(
				...
				if ((yield* HostProcessPlatform) !== "win32") {
					...
					expect((yield* fs.stat(profile.acpDirectory)).mode & 0o777).toBe(
						0o700,
					);
				}
				...
				yield* prepareAntigravityProfile({
					profileDirectory: profile.geminiHome,
				});
				expect(yield* fs.readFileString(profile.tokenPath)).toBe(
	...
	it.effect.skipIf(!symlinksSupported)(
		...
		() =>
			Effect.gen(function* () {
				...
				const cliSkills = path.join(
					userHome,
					".gemini",
					"antigravity-cli",
					"skills",
				);
				yield* fs.makeDirectory(path.join(configSkills, "review"), {
					recursive: true,
				});
				...
				expect(
					yield* fs.exists(
						path.join(profileDirectory, "config", "mcp_config.json"),
					),
				).toBe(false);
				...
				yield* fs.makeDirectory(path.join(configLink, "own-skill"), {
					recursive: true,
				});
				yield* prepareAntigravityProfile({ profileDirectory, userHome });
				expect(yield* fs.readLink(cliLink)).toBe(cliSkills);
	...
	it.effect(
		"rewrites the GCP block on every launch and never stores the API key",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
	...
	it.effect(
		"keeps the browser helper successful when cancellation closes stderr",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
	...
	it.effect(
		"fails before creating a profile when the helper cannot start",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
	...
	it.effect(
		"rejects Python BROWSER delimiter collisions before starting a helper",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
				...
				for (const platform of ["linux", "win32"] as const) {
					...
					const result = yield* prepareAntigravityProfile({
						...
						runtimeExecutablePath:
							platform === "win32" ? "C:/bad;path/node.exe" : "/bad:path/node",
					}).pipe(Effect.result);
					expect(Result.isFailure(result)).toBe(true);
					expect(yield* fs.exists(profileDirectory)).toBe(false);
				}
