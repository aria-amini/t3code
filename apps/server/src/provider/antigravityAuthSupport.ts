...
const maxBrowserHelperLineLength =
	Math.max(
		ANTIGRAVITY_AUTH_BROWSER_MARKER.length,
		ANTIGRAVITY_AUTH_STDOUT_PREFIX.length,
	) +
	maxAuthorizationUrlLength +
	2;
const maxStdoutLineBytes = 16 * 1024 * 1024;
const authPrefixBytes = new TextEncoder().encode(
	ANTIGRAVITY_AUTH_STDOUT_PREFIX,
);
const decodeUrl = Schema.decodeUnknownEffect(Schema.URLFromString);
const decodeBrowserHelperUrl = Schema.decodeUnknownEffect(
...
const encodeProfileSettings = Schema.encodeSync(
	Schema.fromJsonString(ProfileSettingsFile),
);
...
export function antigravityAuthUsesBrowser(
...
export function antigravityAuthLabel(
	authMethod: AntigravityAuthMethod,
): string {
	switch (authMethod) {
		case "oauth-personal":
 ...
 */
export function antigravityAuthConfigIssue(
	...
	switch (auth.authMethod) {
		...
		case "gemini-api-key":
			return auth.apiKey
				? null
				: "Enter a Gemini API key in the Antigravity provider settings.";
		case "agent-platform":
 ...
 */
export function antigravityProfileSettings(
	auth: AntigravityAuthConfig,
): string {
	const gcp = {
...
export function isAntigravitySignInRequiredError(error: unknown): boolean {
	return (
		...
		(isAcpTransportError(error) &&
			error.detail === ANTIGRAVITY_SIGN_IN_REQUIRED_MESSAGE)
	);
}
...
export function resolveAntigravityProfileDirectory(
	...
	const directoryName = NodeCrypto.createHash("sha256")
		.update(instanceId)
		.digest("hex");
 ...
 */
const linkAntigravityUserSkills = Effect.fn("linkAntigravityUserSkills")(
	function* (input: {
		...
		const targets = antigravityUserSkillDirectories(
			path,
		...
		for (const [link, target] of [
			...
			yield* Effect.gen(function* () {
				const existing = yield* fs.readLink(link).pipe(
					Effect.map((value): string | undefined =>
						path.resolve(path.dirname(link), value),
					),
					Effect.catch((error) =>
				...
				Effect.catch((error) =>
					Effect.logWarning(
						"Antigravity user skills are not linked into the profile.",
						{
							link,
							target,
							error,
...
export const prepareAntigravityProfile = Effect.fn("prepareAntigravityProfile")(
	function* (input: {
		readonly profileDirectory: string;
		...
		const userHome =
			input.userHome ??
			resolveAntigravityUserHome(platform, input.baseEnv ?? process.env);
		const runtimeExecutablePath =
			input.runtimeExecutablePath ?? (yield* HostProcessExecutablePath);
		const helperExecutable =
			platform === "win32"
				? runtimeExecutablePath.replaceAll("\\", "/")
		...
		yield* Effect.gen(function* () {
			...
			if (
				...
				stderr.truncated ||
				stderr.text !==
					`${ANTIGRAVITY_AUTH_BROWSER_MARKER}"${browserPreflightUrl}"\n`
			) {
				return yield* authSupportError(
					"Antigravity browser suppression could not be verified.",
				);
			}
		}).pipe(
			Effect.scoped,
			Effect.timeoutOrElse({
				...
				orElse: () =>
					Effect.fail(
						authSupportError(
							"Antigravity browser suppression verification timed out.",
						),
					),
			}),
		...
		for (const directory of [geminiHome, acpDirectory]) {
			yield* fs
				...
				.pipe(
					Effect.mapError(() =>
						authSupportError(
							"The Antigravity profile directory could not be created.",
						),
					),
				);
			if (platform !== "win32") {
				yield* fs
					...
					.pipe(
						Effect.mapError(() =>
							authSupportError(
								"The Antigravity profile directory permissions could not be set.",
							),
						),
					);
			}
		...
		yield* fs
			.writeFileString(
				path.join(acpDirectory, "settings.json"),
				antigravityProfileSettings(auth),
			)
			.pipe(
				Effect.mapError(() =>
					authSupportError(
						"The Antigravity profile settings could not be written.",
					),
				),
			);
		yield* linkAntigravityUserSkills({
			profileDirectory: geminiHome,
			userHome,
			platform,
		});
		return profile;
...
export const parseAntigravityAuthorizationUrl = Effect.fn(
	...
	if (
		authorizationUrl.length > maxAuthorizationUrlLength ||
		/\s/.test(authorizationUrl)
	) {
		return yield* invalidUrl();
	}
	const url = yield* decodeUrl(authorizationUrl).pipe(
		Effect.mapError(invalidUrl),
	);
	...
	const redirect = yield* decodeUrl(redirectUri).pipe(
		Effect.mapError(invalidUrl),
	);
...
export function makeAntigravityStdoutTransform(
	...
	const handleLine = Effect.fn("antigravityAuthSupport.handleStdoutLine")(
	...
	return (
		...
		Uint8Array,
		PlatformError.PlatformError | AcpErrors.AcpError
	> =>
		Stream.suspend(() => {
			...
			return stdout.pipe(
				Stream.mapEffect(
					Effect.fn("antigravityAuthSupport.splitStdoutLines")(function* (
						chunk: Uint8Array,
					) {
				...
				Stream.concat(
					Stream.suspend(() =>
						pendingBytes > 0 ? Stream.succeed(finishLine()) : Stream.empty,
					),
				),
	...
	const handleLine = (line: string) => {
		...
		const url = message.startsWith(ANTIGRAVITY_AUTH_STDOUT_PREFIX)
			...
			: message.startsWith(ANTIGRAVITY_AUTH_BROWSER_MARKER)
				? decodeBrowserHelperUrl(
						message.slice(ANTIGRAVITY_AUTH_BROWSER_MARKER.length),
					)
				: undefined;
			...
			Effect.matchEffect({
				...
				onSuccess: (request) =>
					input.onAuthorizationUrl
						...
						: Effect.fail(
								authSupportError(ANTIGRAVITY_SIGN_IN_REQUIRED_MESSAGE),
							),
			}),
		);
	};
