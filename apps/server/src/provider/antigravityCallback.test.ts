...
it.effect(
	"accepts the exact owned Google callback and an explicit Google denial",
	() =>
		Effect.gen(function* () {
			for (const response of ["code=example-code", "error=access_denied"]) {
				...
				const parsed = yield* validateAntigravityCallbackUrl(
					instanceId,
					pending,
					callback,
				);
...
it.effect(
	"rejects different targets, credentials, fragments, and duplicate OAuth fields",
	() =>
		Effect.gen(function* () {
			const callbacks = [
