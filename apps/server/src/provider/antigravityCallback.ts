...
export const validateAntigravityCallbackUrl = Effect.fn(
	...
	const callback = yield* Effect.try({
		try: () => new URL(callbackUrl),
			...
			invalid("Paste the complete redirect URL from the Google sign-in page."),
	});
	...
	if (
		...
		return yield* invalid(
			"This redirect URL does not belong to the current sign-in.",
		);
	}
		...
		const request = NodeHttp.request(
			...
			(incoming) => {
				...
				incoming.once("end", () => {
					...
					resume(
						status >= 200 && status < 300 ? Effect.void : Effect.fail(failed()),
					);
				});
				incoming.resume();
			},
