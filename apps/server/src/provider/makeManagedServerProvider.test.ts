...
const AlwaysRunTestLayer = Layer.merge(
	BackgroundPolicyAlwaysRunLayer,
	ServerSettingsTestLayer,
);
const NeverRunTestLayer = Layer.merge(
	BackgroundPolicyNeverRunLayer,
	ServerSettingsTestLayer,
);
...
describe("makeManagedServerProvider", () => {
	...
	it.effect(
		"skips periodic provider refreshes without foreground provider-status demand",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const checkCalls = yield* Ref.make(0);
					...
					yield* makeManagedServerProvider<TestSettings>({
						...
						haveSettingsChanged: (previous, next) =>
							previous.enabled !== next.enabled,
						initialSnapshot: () => Effect.succeed(initialSnapshot),
						checkProvider: Ref.updateAndGet(
							checkCalls,
							(count) => count + 1,
						).pipe(
							Effect.tap((count) =>
								count === 1
									? Deferred.succeed(initialCheckDone, undefined).pipe(
											Effect.ignore,
										)
									: Effect.void,
							),
				...
				Effect.provide(Layer.mergeAll(NeverRunTestLayer, TestClock.layer())),
			),
	);

	it.effect(
		"disables periodic provider refreshes when the explicit interval is zero",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const checkCalls = yield* Ref.make(0);
	...
	it.effect(
		"wakes a sleeping provider refresh loop when its interval changes",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					...
					const serverSettingsChanges =
						yield* PubSub.unbounded<typeof initialServerSettings>();
					const serverSettingsLayer = Layer.succeed(
						...
						ServerSettingsService.of({
							...
							updateSettings: () =>
								Effect.die(new Error("unused in this test")),
							streamChanges: Stream.empty,
					...
					yield* makeManagedServerProvider<TestSettings>({
						...
						checkProvider: Ref.updateAndGet(
							...
							Effect.tap((count) =>
								count === 1
									...
									: Deferred.succeed(periodicCheckDone, undefined).pipe(
											Effect.ignore,
										),
							),
							Effect.as(refreshedSnapshot),
						...
						Effect.provide(
							Layer.merge(BackgroundPolicyAlwaysRunLayer, serverSettingsLayer),
						),
					);
				...
				const provider = yield* makeManagedServerProvider<TestSettings>({
					...
					checkProvider: Ref.updateAndGet(
						...
						Effect.flatMap((count) =>
							count === 1
								? Deferred.await(releaseInitialCheck).pipe(
										Effect.as(refreshedSnapshot),
									)
								: Deferred.await(releaseSettingsCheck).pipe(
	...
	it.effect(
		"can update settings and disable periodic checks without probing again",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const settingsChanges = yield* PubSub.unbounded<TestSettings>();
					...
					yield* makeManagedServerProvider<TestSettings>({
						...
						enrichSnapshot: () =>
							Ref.update(enrichmentCalls, (count) => count + 1),
						refreshInterval: "1 second",
					});
	...
	it.effect(
		"streams supplemental snapshot updates after the base provider check completes",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
	...
	it.effect(
		"ignores stale enrichment callbacks after a newer refresh advances generation",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const publishCallbacks: Array<
						(snapshot: ServerProvider) => Effect.Effect<void>
					> = [];
					...
					const provider = yield* makeManagedServerProvider<TestSettings>({
						...
						checkProvider: Ref.updateAndGet(
							...
							Effect.flatMap((count) =>
								count === 1
									? Deferred.await(allowFirstRefresh).pipe(
											Effect.as(refreshedSnapshot),
										)
						...
						enrichSnapshot: ({ publishSnapshot }) =>
							Effect.gen(function* () {
								...
								if (publishCallbacks.length === 1) {
									yield* Deferred.succeed(firstCallbackReady, undefined).pipe(
										Effect.ignore,
									);
								} else if (publishCallbacks.length === 2) {
									yield* Deferred.succeed(secondCallbackReady, undefined).pipe(
										Effect.ignore,
									);
								}
							}),
	...
	it.effect(
		"keeps live usage windows across a failed probe and a stale enrichment",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const releaseEnrichment = yield* Deferred.make<void>();
					...
					assert.strictEqual(
						refreshed.message,
						refreshedSnapshotSecond.message,
					);
