...
export const makeManagedServerProvider = Effect.fn("makeManagedServerProvider")(
	function* <Settings>(input: {
		...
		readonly checkProviderOnSettingsChange?: (
			previous: Settings,
			next: Settings,
		) => boolean;
	}): Effect.fn.Return<
	...
	> {
		...
		const enrichmentFiberRef = yield* Ref.make<Fiber.Fiber<
			void,
			unknown
		> | null>(null);
		const scope = yield* Effect.scope;

		const publishEnrichedSnapshot = Effect.fn("publishEnrichedSnapshot")(
			function* (generation: number, nextSnapshot: ServerProvider) {
				const snapshotToPublish = yield* Ref.modify(
					snapshotStateRef,
					(state) => {
		...
		const restartSnapshotEnrichment = Effect.fn("restartSnapshotEnrichment")(
			function* (
				settings: Settings,
				snapshot: ServerProvider,
				generation: number,
				...
				const fiber = yield* input
					.enrichSnapshot({
						settings,
						snapshot,
						getSnapshot: Ref.get(snapshotStateRef).pipe(
							Effect.map((state) => state.snapshot),
						),
						publishSnapshot: (nextSnapshot) =>
							publishEnrichedSnapshot(generation, nextSnapshot),
					})
		...
		const applySnapshotBase = Effect.fn("applySnapshot")(function* (
			...
			if (
				...
				return yield* Ref.get(snapshotStateRef).pipe(
					Effect.map((state) => state.snapshot),
				);
			}
			...
			if (
				!forceRefresh &&
				input.checkProviderOnSettingsChange?.(
					previousSettings,
					nextSettings,
				) === false
			) {
			...
			const { snapshot: nextSnapshot, generation: nextGeneration } =
				yield* Ref.modify(snapshotStateRef, (state) => {
		 ...
		 */
		const applyUsageLimits: ServerProviderShape["applyUsageLimits"] = (
			update,
		) =>
			Effect.gen(function* () {
				const snapshotToPublish = yield* Ref.modify(
		...
		if (input.refreshInterval === undefined) {
			...
			yield* serverSettingsChanges.pipe(
				...
				Stream.changes,
				Stream.runForEach(() =>
					Queue.offer(refreshIntervalChanges, undefined).pipe(Effect.asVoid),
				),
				Effect.forkScoped,
			);
		}
							...
							Duration.toMillis(Duration.fromInputUnsafe(refreshInterval)) > 0
								? hasProviderStatusDemand.pipe(
										Effect.flatMap((shouldRefresh) =>
											shouldRefresh
												? refreshSnapshot().pipe(Effect.asVoid)
												: Effect.void,
										),
									)
								: Effect.void,
