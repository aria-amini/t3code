...
describe("parseModelsCliOutput", () => {
	...
	it("parses multiple models from multiple providers", () => {
		...
		NodeAssert.equal(
			[...result.connected].sort().join(","),
			"anthropic,openai",
		);
		NodeAssert.equal(
			Object.keys(result.providers.get("anthropic")!.models).length,
			2,
		);
		NodeAssert.equal(
			Object.keys(result.providers.get("openai")!.models).length,
			1,
		);
	});
	...
	it("handles Windows-style CRLF line endings", () => {
		...
		NodeAssert.equal(result.providers.size, 1);
		NodeAssert.ok(
			result.providers.get("anthropic")!.models["claude-sonnet-4-5"],
		);
	});
...
describe("parseAgentListCliOutput", () => {
	...
	it("parses multiple agents", () => {
		const stdout = [
			...
			"  " +
				JSON.stringify([
					{ permission: "edit", action: "ask", pattern: "*.md" },
				]),
		].join("\n");
	...
	it("handles agent names with spaces", () => {
		const stdout = [
			...
			"  " +
				JSON.stringify([
					{ permission: "edit", action: "ask", pattern: "*.ts" },
				]),
		].join("\n");
