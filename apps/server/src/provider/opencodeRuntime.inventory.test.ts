...
it.layer(testLayer)("OpenCodeRuntime inventory", (it) => {
	it.effect(
		"aborts pending SDK requests when inventory loading is interrupted",
		() =>
			Effect.gen(function* () {
	...
	it.effect("keeps provider inventory when agent discovery fails", () =>
		Effect.gen(function* () {
			...
			const client = {
				...
				app: {
					agents: () =>
						Promise.reject(new Error("agents endpoint unavailable")),
		...
		Effect.gen(function* () {
			...
			const client = {
				...
				app: {
					...
					skills: () =>
						Promise.reject(new Error("skills endpoint unavailable")),
				},
			} as unknown as OpencodeClient;
	...
	it.effect(
		"drops oversized CLI skill output without losing the model inventory",
		() =>
			Effect.gen(function* () {
