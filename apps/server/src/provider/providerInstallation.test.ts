...
const makeHarness = Effect.fn("providerInstallation.test.makeHarness")(
	function* (
		input: {
			...
			Effect.provide(
				Layer.mergeAll(
					...
					Layer.mock(ProviderInstanceRegistry)({
						getInstance: (id) =>
							Effect.succeed(
								id === configured.instanceId ? configured : undefined,
							),
...
describe("provider installation routing", () => {
	...
	it.effect(
		"rejects another driver and unknown instances before installation",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({
					instance: instance(ProviderDriverKind.make("codex")),
				});
				const wrongDriver = yield* Effect.flip(
					harness.router.start({ instanceId }),
				);
	...
	it.effect(
		"keeps external installs manual without hiding shared install status",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({
					settings: {
						providers: { antigravity: { binaryPath: "/external/agy" } },
					},
				});
				const start = yield* Effect.flip(harness.router.start({ instanceId }));
				const remove = yield* Effect.flip(
					harness.router.remove({ instanceId }),
				);
				...
				const observed = yield* Stream.runCollect(
					harness.router.subscribe({ instanceId }),
				);
	...
	it.effect(
		"protects another instance's binary found through its own PATH",
		() =>
			Effect.gen(function* () {
				...
				const directory = yield* fs.makeTempDirectoryScoped({
					prefix: "t3-provider-install-route-",
				});
					...
					settings: {
						providerInstances: {
							[other]: {
								...
								environment: [
									{ name: "PATH", value: directory, sensitive: false },
								],
							},
						},
					},
