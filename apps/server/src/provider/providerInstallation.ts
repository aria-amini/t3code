...
const decodeAntigravitySettings =
	Schema.decodeUnknownEffect(AntigravitySettings);

/** Route instance setup to the environment-owned installer without owning the download. */
export const makeProviderInstallation = Effect.fn("makeProviderInstallation")(
	function* () {
		...
		const readEntries = Effect.fn("ProviderInstallation.readEntries")(
			function* (instanceId: ProviderInstanceId, operation: string) {
		...
		const requireInstance = Effect.fn("ProviderInstallation.requireInstance")(
			function* (
				...
				const config = yield* decodeAntigravitySettings(
					entries[instanceId]?.config ?? {},
				).pipe(
					Effect.mapError(
						() =>
							new ProviderSetupError({
		...
		const failure =
			(instanceId: ProviderInstanceId) =>
			(error: AntigravityInstallationError) =>
				new ProviderSetupError({
					instanceId,
		...
		const start = Effect.fn("ProviderInstallation.start")(function* (
			input: ProviderSetupInput,
		) {
			yield* requireInstance(input.instanceId, "install", true);
			return yield* installation.start.pipe(
				Effect.mapError(failure(input.instanceId)),
			);
		});
		...
		const subscribe = (input: ProviderSetupInput) =>
			Stream.unwrap(
				requireInstance(input.instanceId, "observe-install").pipe(
					Effect.as(installation.changes),
				),
			);

		const remove = Effect.fn("ProviderInstallation.remove")(function* (
			input: ProviderSetupInput,
		) {
