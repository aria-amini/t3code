...
it.layer(NodeServices.layer)("providerMaintenance", (it) => {
	it.effect(
		"reads cached versions through the injectable cache reference",
		() =>
			resolveLatestProviderVersion(manualPackageTool).pipe(
				...
				Effect.provideService(
					...
					HttpClient.make(() =>
						Effect.die(
							"cached provider version should not make an HTTP request",
						),
					),
				),
				Effect.map((version) => {
	...
	it.effect(
		"prefers the installer's own latest version over the npm registry",
		() =>
			resolveLatestProviderVersion({
				...manualPackageTool,
				latestVersion: "1.2.0",
				...
				Effect.provideService(
					...
					HttpClient.make(() =>
						Effect.die(
							"installer-reported latest should not make an HTTP request",
						),
					),
				),
				Effect.map((version) => {
	...
	it.effect(
		"does not fetch latest provider versions when update checks are disabled",
		() =>
			enrichProviderSnapshotWithVersionAdvisory(
				...
				Effect.provideService(
					...
					HttpClient.make(() =>
						Effect.die(
							"disabled provider update checks should not make an HTTP request",
						),
					),
				),
				Effect.map((provider) => {
	...
	it.effect(
		"recognizes a Windows standalone install through its junctioned bin dir",
		() =>
			Effect.gen(function* () {
				const visiblePath =
	...
	it.effect(
		"proves Windows npm ownership from the package manifest beside the shim",
		() =>
			Effect.gen(function* () {
				...
				NodeFS.mkdirSync(
					NodePath.join(tempDir, "node_modules", "@example", "package-tool"),
					{
						recursive: true,
					},
				);
	...
	it.effect.skipIf(windowsHost)(
		"switches to native updates and runs the resolved executable",
		() =>
			Effect.gen(function* () {
	...
	it.effect.skipIf(windowsHost)(
		"runs an explicit native updater outside PATH",
		() =>
			Effect.gen(function* () {
	...
	it("quotes copyable command words the shell would split", () => {
		...
		expect(windows.update?.command).toBe(
			"& 'C:\\Program Files\\Tool\\tool.exe' update",
		);
	});

	it.effect.skipIf(windowsHost)(
		"carries the native updater's environment into the action",
		() =>
			Effect.gen(function* () {
				...
				const resolver = makePackageManagedProviderMaintenanceResolver({
					...
					nativeUpdate: {
						args: ["update"],
						isCommandPath: isNativeTestCommandPath(
							"/.local/bin/native-package-tool",
						),
	...
	it("recognizes Homebrew kegs and casks from the real executable path", () => {
		expect(
			homebrewOwnershipFromCommandPath(
				"/opt/homebrew/Cellar/claude-code@latest/2.1.0/bin/claude",
			),
		).toEqual({
			kind: "formula",
		...
		expect(
			homebrewOwnershipFromCommandPath(
				"/usr/local/Caskroom/codex/0.148.0/codex",
			),
		).toEqual({
			kind: "cask",
			name: "codex",
		...
		// `brew --prefix`.
		expect(
			homebrewOwnershipFromCommandPath("/srv/Cellar/claude/1.0.0/bin/claude"),
		).toMatchObject({
			prefix: "/srv",
		});
	});
	...
	it.effect.skipIf(windowsHost)(
		...
		() =>
			Effect.gen(function* () {
				...
				const missingPath = NodePath.join(
					tempDir,
					".local",
					"bin",
					"native-package-tool",
	...
	it("reads the stable formula version from brew info", () => {
		const info = JSON.stringify({
			formulae: [{ versions: { stable: "2.1.5" } }],
		});
		const formula = {
			kind: "formula",
			name: "claude-code",
