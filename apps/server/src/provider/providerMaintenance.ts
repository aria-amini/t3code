...
const PROVIDER_UPDATE_ACTION_TOAST_MESSAGE =
...
const compactEnv = (
	input: Record<string, Option.Option<string>>,
): NodeJS.ProcessEnv =>
	Object.fromEntries(
...
export interface ProviderMaintenanceCapabilitiesResolver {
	readonly resolve: (
		...
		never,
		ProviderMaintenanceResolverServices
	>;
}
...
export const ProviderVersionCache = Context.Reference<
	Map<string, ProviderVersionCacheEntry>
...
function quoteUpdateExecutable(
	executable: string,
...
export function npmGlobalPrefixFromCommandPath(
	...
	if (
		packageIndex < 0 ||
		normalized.slice(0, packageIndex).includes("/node_modules/")
	) {
		return null;
	}
	...
	const miseTool = /\/mise\/installs\/([^/]+)\/[^/]+$/.exec(
		normalized.slice(0, packageIndex),
	)?.[1];
...
export function homebrewOwnershipFromCommandPath(
	...
	const match = HOMEBREW_KEG_PATTERN.exec(
		realCommandPath.replaceAll("\\", "/"),
	);
	if (!match) {
		return null;
	}
		...
		Schema.Array(
			Schema.Struct({
				versions: Schema.optional(
					Schema.Struct({ stable: Schema.optional(Schema.String) }),
				),
			}),
		),
	),
	casks: Schema.optional(
		Schema.Array(Schema.Struct({ version: Schema.optional(Schema.String) })),
	),
});

const decodeHomebrewInfo = Schema.decodeUnknownOption(
	Schema.fromJsonString(HomebrewInfoResponse),
);
...
const runHomebrew = Effect.fn("runHomebrew")(function* (
	...
	const collect = Effect.gen(function* () {
		const child = yield* spawner.spawn(
			ChildProcess.make(brewPath, args, { env, extendEnv: true }),
		);
...
export const resolvePackageManagedProviderMaintenance = Effect.fn(
	...
	if (homebrew) {
		...
		const brewPath = yield* resolveCommandPath("brew", {
			env: context.env,
		}).pipe(
		...
		const brewPrefix = nonEmptyString(
			yield* runHomebrew(brewPath, ["--prefix"], context.env),
		);
		const realBrewPrefix = brewPrefix
			? yield* fileSystem
					.realPath(brewPrefix)
					.pipe(Effect.orElseSucceed(() => brewPrefix))
			: null;
		if (
			!realBrewPrefix ||
...
export function makePackageManagedProviderMaintenanceResolver(
	...
	return {
		resolve: (context) =>
			resolvePackageManagedProviderMaintenance(definition, context),
	};
}
...
export const resolveProviderMaintenanceCapabilitiesEffect = Effect.fn(
	...
	const resolvedCommandPath = yield* resolveCommandPath(binaryPath, {
		env,
	}).pipe(
		Effect.catchTags({ CommandResolutionError: () => Effect.succeed(null) }),
	);
...
export function createProviderVersionAdvisory(input: {
	...
	const capabilities =
		...
		makeManualProviderMaintenanceCapabilities(input.driver);
...
export const resolveLatestProviderVersion = Effect.fn(
...
export const enrichProviderSnapshotWithVersionAdvisory = Effect.fn(
	...
	const capabilities =
		...
		makeManualProviderMaintenanceCapabilities(snapshot.driver);
