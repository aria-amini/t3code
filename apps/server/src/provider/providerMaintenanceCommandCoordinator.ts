...
export const makeProviderMaintenanceCommandCoordinator = Effect.fn(
	...
	const locksRef = yield* Ref.make<ReadonlyMap<string, Semaphore.Semaphore>>(
		new Map(),
	);

	const acquireTarget = Effect.fn("acquireTarget")(function* (
		targetKey: string,
	) {
	...
	const getLock = Effect.fn("getProviderMaintenanceCommandLock")(function* (
		lockKey: string,
	) {
	...
	const withCommandLock: ProviderMaintenanceCommandCoordinatorShape<E>["withCommandLock"] =
		({ targetKey, lockKey, onQueued, run }) =>
			Effect.gen(function* () {
