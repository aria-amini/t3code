...
class ProviderMaintenanceCommandError extends Data.TaggedError(
...
const runProviderMaintenanceCommandWithSpawner = Effect.fn(
	...
	const collectCommandResult = Effect.fn(
		"ProviderMaintenanceRunner.collectCommandResult",
	)(function* () {
...
export const make = Effect.fn("ProviderMaintenanceRunner.make")(function* () {
			...
			Effect.flatMap((providers) => {
				...
				return Effect.forEach(
					...
					Effect.catchCause((cause) =>
						Effect.logWarning(
							"Provider post-update version verification failed",
							{
								provider,
	...
	const updateProvider: ProviderMaintenanceRunnerShape["updateProvider"] =
		Effect.fn("ProviderMaintenanceRunner.updateProvider")(function* (target) {
			...
			const runProviderUpdate = Effect.fn(
				"ProviderMaintenanceRunner.runProviderUpdate",
			)(function* () {
				...
				const runCommandAndVerify = Effect.fn(
					"ProviderMaintenanceRunner.runCommandAndVerify",
				)(function* () {
					...
					const couldNotVerify =
						...
						verifiedProviders.some(
							(verifiedProvider) => !isStillInstalled(verifiedProvider),
						);
					...
					return yield* finish(
						makeUpdateState({
							status:
								couldNotVerify || stillOutdated ? "unchanged" : "succeeded",
							startedAt,
							finishedAt,
				...
				const recordFailedUpdate = Effect.fn(
					"ProviderMaintenanceRunner.recordFailedUpdate",
