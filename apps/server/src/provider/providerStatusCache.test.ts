...
it.layer(NodeServices.layer)("providerStatusCache", (it) => {
	it.effect(
		"logs structural diagnostics without retaining invalid cache contents",
		() => {
	...
	it.effect("writes and reads provider status snapshots", () =>
		Effect.gen(function* () {
			...
			assert.deepStrictEqual(
				yield* readProviderStatusCache(codexPath),
				codexProvider,
			);
			assert.deepStrictEqual(
				yield* readProviderStatusCache(claudePath),
				claudeProvider,
			);
			assert.deepStrictEqual(
				yield* readProviderStatusCache(openCodePath),
				openCodeProvider,
			);
		}),
	);
