...
const mockedExecute =
	vi.fn<AzureDevOpsCli.AzureDevOpsCli["Service"]["execute"]>();
...
layer("AzureDevOpsPullRequestCli.layer", (it) => {
	...
	it.effect(
		"reads an Azure pull request page larger than the VCS default output limit",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"reads the page unnarrowed when asked to search, having nothing to search with",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(output(pullRequests(3, 1))),
	...
	it.effect(
		"steps over what it has already handed over, which is all Azure can be told",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(output(pullRequests(3, 1))),
	...
	it.effect(
		"advances by malformed raw rows and keeps reading until the page is full",
		() =>
			Effect.gen(function* () {
				mockedExecute
					.mockReturnValueOnce(
	...
	it.effect(
		"reads the signed-in account, which az reports as a bare value",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"completes a pull request to merge it, squashing only when asked",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"stores the squash choice with an auto-completion, as a merge now does",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("{}")));
				const cli = yield* AzureDevOpsPullRequestCli.AzureDevOpsPullRequestCli;
	...
	it.effect.each([
		{
			name: "a title",
			rewrite: { title: "Add the page" },
			expected: ["--title=Add the page"],
		},
		{
			name: "a description",
		...
		"rewrites $name, sending nothing it was not given",
		({ rewrite, expected }) =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("{}")));
				const cli = yield* AzureDevOpsPullRequestCli.AzureDevOpsPullRequestCli;
	...
	it.effect(
		"sends a description that starts with a dash as one value, not as a flag",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("{}")));
				const cli = yield* AzureDevOpsPullRequestCli.AzureDevOpsPullRequestCli;
	...
	it.effect(
		"reads the conversation through the REST API, pinned to a version",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(
				...
				const comments = yield* cli.listThreads({
					cwd: "/w",
					threadsUrl:
						"https://dev.azure.com/acme/platform/_apis/git/r/web/pullRequests/42/threads",
				});
	...
	it.effect("reports a pull request it cannot place as its own outcome", () =>
		Effect.gen(function* () {
			...
			const error = yield* Effect.flip(
				cli.getPullRequest({ cwd: "/w", number: 42 }),
			);
	...
	it.effect(
		"takes a reviewer off the pull request with the same command's counterpart",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(Effect.succeed(output("[]")));
				const cli = yield* AzureDevOpsPullRequestCli.AzureDevOpsPullRequestCli;
	...
	it.effect(
		"refuses a reviewer az would read as a flag, before running anything",
		() =>
			Effect.gen(function* () {
				const cli = yield* AzureDevOpsPullRequestCli.AzureDevOpsPullRequestCli;
