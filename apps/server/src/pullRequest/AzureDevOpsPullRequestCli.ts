...
function actionArgs(
	...
	switch (action) {
		...
		case "approve-workflows":
			throw new Error(
				`Azure DevOps pull request action ${action} is unsupported`,
			);
	}
}
...
export const make = Effect.gen(function* () {
			...
			Effect.flatMap((result) => {
				...
				if (lastItemIndex !== undefined) {
					...
					return Effect.succeed({
						items: [
							...input.items,
							...decoded.success.items.slice(0, remaining),
						],
						// A full raw response may have more rows even when malformed entries used the probe.
						truncated:
							consumed < decoded.success.rawCount ||
							decoded.success.rawCount === top,
	...
	return AzureDevOpsPullRequestCli.of({
		getViewer: (input) =>
			executeJson({
				cwd: input.cwd,
				args: ["account", "show", "--query", "user"],
			}).pipe(
				Effect.flatMap(
					(result): Effect.Effect<string, AzureDevOpsPullRequestCliError> => {
						...
						return decoded.success === null
							? Effect.fail(
									new AzureDevOpsViewerUnavailableError({
										command: "az",
										cwd: input.cwd,
									}),
								)
		...
		setPullRequestReviewers: (input) =>
			input.reviewers.some((reviewer) => !isReviewerName(reviewer))
				? Effect.fail(
						new AzureDevOpsReviewerNameError({ command: "az", cwd: input.cwd }),
					)
				: azure
						.execute({
							cwd: input.cwd,
