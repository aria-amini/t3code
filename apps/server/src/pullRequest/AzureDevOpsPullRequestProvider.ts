...
export function azureDevOpsProviderFailure(
	...
	if (error._tag === "AzureDevOpsCliUnavailableError")
		return { reason: "missing-tool" };
	if (error._tag === "AzureDevOpsCliAuthenticationError")
		return { reason: "unauthenticated" };
	if (error._tag === "AzureDevOpsCliRateLimitError")
		...
		getChangeRequest: (input) =>
			cli.getPullRequest({ cwd: input.cwd, number: input.number }).pipe(
				...
				Effect.map((pullRequest): ProviderChangeRequestDetail => ({
					...
					changedFiles: 0,
					mergedAt:
						pullRequest.state === "merged" ? pullRequest.closedAt : null,
					closedAt:
