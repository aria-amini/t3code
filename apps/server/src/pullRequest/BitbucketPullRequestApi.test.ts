...
function page(count: number, firstNumber: number, next?: string): string {
	return JSON.stringify({
		...
		values: Array.from({ length: count }, (_, index) => ({
			...
			links: {
				html: {
					href: `https://bitbucket.org/acme/web/pull-requests/${firstNumber}`,
				},
			},
		})),
		...(next === undefined ? {} : { next }),
...
layer("BitbucketPullRequestApi.layer", (it) => {
	it.effect(
		"asks for reviewers, newest first, at Bitbucket's page ceiling",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"follows the cursor Bitbucket sends rather than counting offsets",
		() =>
			Effect.gen(function* () {
				const next =
					"https://api.bitbucket.org/2.0/repositories/acme/web/pullrequests?page=2";
				mockedRequest
	...
	it.effect("stops at the caller's page and says more remain", () =>
		Effect.gen(function* () {
			...
			mockedRequest.mockReturnValueOnce(
				Effect.succeed(response(page(50, 1, next))),
			);
			const api = yield* BitbucketPullRequestApi.BitbucketPullRequestApi;
	...
	it.effect(
		"searches with a filter expression, which is all Bitbucket offers",
		() =>
			Effect.gen(function* () {
				...
				expect(filterOfCall(0)).toBe(
					'(title ~ "page" OR description ~ "page")',
				);
	...
	it.effect(
		"escapes a quote and a backslash, so a search cannot reshape the filter",
		() =>
			Effect.gen(function* () {
				mockedRequest.mockReturnValueOnce(Effect.succeed(response(page(0, 1))));
				const api = yield* BitbucketPullRequestApi.BitbucketPullRequestApi;
				...
				expect(filterOfCall(0)).toBe(
					`(title ~ "${literal}" OR description ~ "${literal}")`,
				);
			}),
	);
	...
	it.effect("carries on from the instant the last slice ended on", () =>
		Effect.gen(function* () {
			...
			yield* api.listPullRequests({
				...
				limit: 50,
				cursor: {
					updatedBefore: "2026-07-02T00:00:00.123456+00:00",
					delivered: 50,
				},
			});
			...
			expect(filterOfCall(0)).toBe(
				"updated_on <= 2026-07-02T00:00:00.123456+00:00",
			);
	...
	it.effect(
		"narrows by the reader's words and by where it left off at once",
		() =>
			Effect.gen(function* () {
				mockedRequest.mockReturnValueOnce(Effect.succeed(response(page(0, 1))));
				const api = yield* BitbucketPullRequestApi.BitbucketPullRequestApi;
	...
	it.effect(
		"returns the diff verbatim, because Bitbucket already sends a patch",
		() =>
			Effect.gen(function* () {
				const patch =
	...
	it.effect(
		"reads a named commit's own patch, which pages no further than the whole of it",
		() =>
			Effect.gen(function* () {
				const patch =
					"diff --git a/a.ts b/a.ts\n--- a/a.ts\n+++ b/a.ts\n@@ -1 +1 @@\n-a\n+b\n";
	...
	it.effect(
		"refuses a commit that is not a sha rather than reading it into a URL",
		() =>
			Effect.gen(function* () {
				const api = yield* BitbucketPullRequestApi.BitbucketPullRequestApi;
	...
	it.effect("aggregates every diffstat page", () =>
		Effect.gen(function* () {
			...
			mockedRequest
				...
				.mockReturnValueOnce(
					Effect.succeed(
						response(valuePage([{ lines_added: 4, lines_removed: 7 }])),
					),
				);
			const api = yield* BitbucketPullRequestApi.BitbucketPullRequestApi;

			const stat = yield* api.getDiffStat({
				repository: "acme/web",
				number: 7,
			});
	...
	it.effect(
		"returns the complete commit timeline oldest first across pages",
		() =>
			Effect.gen(function* () {
				...
				const commits = yield* api.listCommits({
					repository: "acme/web",
					number: 7,
				});
	...
	it.effect("returns build statuses from every page", () =>
		Effect.gen(function* () {
			...
			mockedRequest
				.mockReturnValueOnce(
					Effect.succeed(
						response(valuePage([{ name: "Build", state: "SUCCESSFUL" }], next)),
					),
				)
				.mockReturnValueOnce(
					Effect.succeed(
						response(valuePage([{ name: "Lint", state: "FAILED" }])),
					),
				);
			const api = yield* BitbucketPullRequestApi.BitbucketPullRequestApi;
	...
	it.effect("reads an empty conflict list as mergeable", () =>
		Effect.gen(function* () {
			...
			const mergeability = yield* api.getMergeability({
				repository: "acme/web",
				number: 7,
			});
			...
			expect(callAt(0).url).toBe(
				"/repositories/acme/web/pullrequests/7/conflicts",
			);
		}),
	);
	...
	it.effect(
		"leaves out the half of the pull request it was not asked about",
		() =>
			Effect.gen(function* () {
				mockedRequest.mockReturnValue(Effect.succeed(response("{}")));
				const api = yield* BitbucketPullRequestApi.BitbucketPullRequestApi;
	...
	it.effect(
		"fails the read when Bitbucket answers with something unreadable",
		() =>
			Effect.gen(function* () {
				mockedRequest.mockReturnValueOnce(
					// @effect-diagnostics-next-line preferSchemaOverJson:off
	...
	it.effect(
		"states a failure once, without stacking one message inside another",
		() =>
			Effect.gen(function* () {
				mockedRequest.mockReturnValueOnce(
					Effect.fail(
	...
	it.effect("fails when the credentials belong to no named account", () =>
		Effect.gen(function* () {
			// @effect-diagnostics-next-line preferSchemaOverJson:off
			mockedRequest.mockReturnValueOnce(
				Effect.succeed(response(JSON.stringify({}))),
			);
			const api = yield* BitbucketPullRequestApi.BitbucketPullRequestApi;
	...
	it.effect(
		"follows Bitbucket's cursor and reassembles a thread that spans two pages",
		() =>
			Effect.gen(function* () {
				mockedRequest.mockReturnValueOnce(
					Effect.succeed(
	...
	it.effect(
		"stops the comment walk at its bound and says the conversation was cut short",
		() =>
			Effect.gen(function* () {
				...
				const { truncated } = yield* api.listComments({
					repository: "acme/web",
					number: 7,
				});
	...
	it.effect(
		"reassembles a thread from the flat comment list, replies included",
		() =>
			Effect.gen(function* () {
				mockedRequest.mockReturnValueOnce(
					Effect.succeed(
				...
				const { threads } = yield* api.listComments({
					repository: "acme/web",
					number: 7,
				});
	...
	it.effect(
		"writes a review's line comments, its summary, then its verdict",
		() =>
			Effect.gen(function* () {
				mockedRequest.mockReturnValue(Effect.succeed(response("{}")));
				const api = yield* BitbucketPullRequestApi.BitbucketPullRequestApi;
	...
	it.effect(
		"resolves by creating the sub-resource and unresolves by deleting it",
		() =>
			Effect.gen(function* () {
				mockedRequest.mockReturnValue(Effect.succeed(response("{}")));
				const api = yield* BitbucketPullRequestApi.BitbucketPullRequestApi;
	...
	it.effect(
		"asks for the credentials' permission on this repository, and nobody else's",
		() =>
			Effect.gen(function* () {
				mockedRequest.mockReturnValue(
					Effect.succeed(
				...
				assert.isFalse(
					yield* api.getRepositoryPermission({ repository: "acme/web" }),
				);
	...
	it.effect(
		"escapes a repository name before it goes inside a filter literal",
		() =>
			Effect.gen(function* () {
				// @effect-diagnostics-next-line preferSchemaOverJson:off
				mockedRequest.mockReturnValue(
					Effect.succeed(response(JSON.stringify({ values: [] }))),
				);
				const api = yield* BitbucketPullRequestApi.BitbucketPullRequestApi;
				...
				assert.strictEqual(
					filterOfCall(0),
					'repository.full_name="acme/we\\"b"',
				);
			}),
	);
	...
	it.effect(
		"still fails the permission read on a failure that is not the removed endpoint",
		() =>
			Effect.gen(function* () {
				mockedRequest.mockReturnValue(
					Effect.fail(
	...
	it.effect(
		"reads the workspace's people and marks whoever is already a reviewer",
		() =>
			Effect.gen(function* () {
				mockedRequest
					.mockReturnValueOnce(
						Effect.succeed(response(pullRequestJson({ reviewers: [octocat] }))),
					)
					.mockReturnValueOnce(
						Effect.succeed(
							response(
								// @effect-diagnostics-next-line preferSchemaOverJson:off
								JSON.stringify({
									values: [{ user: bilal }, { user: octocat }, { user: hubot }],
								}),
							),
						),
					);
				...
				const list = yield* api.listReviewerCandidates({
					repository: "acme/web",
					number: 7,
				});
	...
	it.effect(
		"writes the reviewer set back with the one being asked added to it",
		() =>
			Effect.gen(function* () {
				mockedRequest
					.mockReturnValueOnce(
	...
	it.effect("takes a reviewer out of the set rather than clearing it", () =>
		Effect.gen(function* () {
			mockedRequest
				.mockReturnValueOnce(
					Effect.succeed(
						response(pullRequestJson({ reviewers: [octocat, hubot] })),
					),
				)
				.mockReturnValueOnce(Effect.succeed(response("{}")));
			const api = yield* BitbucketPullRequestApi.BitbucketPullRequestApi;
			...
			// @effect-diagnostics-next-line preferSchemaOverJson:off
			expect(JSON.parse(callAt(1).body ?? "")).toEqual({
				reviewers: [{ uuid: "{octocat}" }],
			});
		}),
	);
});
