...
function repositoryPathOf(segments: {
...
export const make = Effect.gen(function* () {
	...
	const itemPages = <A>(input: {
		...
		readonly decode: (
			...
			{ readonly items: ReadonlyArray<A>; readonly next: string | null },
			unknown
		>;
	...
	return BitbucketPullRequestApi.of({
		getViewer: () =>
			bitbucket.request({ method: "GET", url: "/user" }).pipe(
				Effect.flatMap(
					(response): Effect.Effect<string, BitbucketPullRequestApiError> => {
			...
			withRepository(input.repository, (path) => {
				...
				const predicates = [
					...
					...(input.cursor === undefined
						? []
						: [`updated_on <= ${input.cursor.updatedBefore}`]),
				];
				return listPage({
					...
					url: `${path}/pullrequests?${stateParams(input.state)
						...
						.join(
							"&",
						)}&pagelen=${MAX_PAGE_SIZE}&sort=-updated_on&fields=%2Bvalues.reviewers${
						predicates.length === 0
							? ""
							: `&q=${encodeURIComponent(predicates.join(" AND "))}`
					}`,
					limit: input.limit,
					page: 1,
					collected: [],
		...
		getRepositoryPermission: (input) =>
			withRepository(input.repository, () =>
				...
				Effect.catchIf(isRepositoryPermissionRemovedError, () =>
					Effect.succeed(true),
				),
			),
							...
							candidates: members.items.flatMap((candidate) =>
								candidate.login === author
									...
									: [
											{
												...candidate,
												isRequested: requested.has(candidate.id),
											},
										],
							),
			...
			withRepository(input.repository, (path) =>
				Effect.gen(function* () {
					...
					if (input.verdict === "approve") {
						yield* bitbucket.request({
							method: "POST",
							url: `${pullRequest}/approve`,
						});
					}
					if (input.verdict === "request-changes") {
						yield* bitbucket.request({
							method: "POST",
							url: `${pullRequest}/request-changes`,
						});
					}
				}),
			),
