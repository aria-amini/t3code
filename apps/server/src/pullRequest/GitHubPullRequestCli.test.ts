...
const mockedGetPullRequest =
	vi.fn<GitHubCli.GitHubCli["Service"]["getPullRequest"]>();
...
const diffRefused = new GitHubCli.GitHubCliCommandError({
	...
	cwd: "/w",
	cause: new Error(
		"HTTP 406: the diff exceeded the maximum number of files (300)",
	),
});
...
function searchPage(nodes: ReadonlyArray<unknown>, hasNextPage = false) {
	return output(
		JSON.stringify({ data: { search: { pageInfo: { hasNextPage }, nodes } } }),
	);
}
...
layer("GitHubPullRequestCli.layer", (it) => {
	...
	it.effect(
		"asks for one row more than the page, to probe for a next page",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(output(pullRequests(3, 1))),
				);
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"reports truncation from the extra row, counted before decoding",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(output(pullRequests(11, 1))),
				);
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"carries every repository and every qualifier into one search",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"narrows a search to the author, and to merged on the merged tab",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(searchPage([])));
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"keeps a searched-for qualifier inside the phrase, and out of argv",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(searchPage([])));
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"reports truncation from the extra row, and from a page GitHub says has more",
		() =>
			Effect.gen(function* () {
				mockedExecute
					.mockReturnValueOnce(
	...
	it.effect(
		"reads the line counts in chunks, and files them back by position",
		() =>
			Effect.gen(function* () {
				...
				mockedExecute.mockImplementation(() =>
					...
					Effect.succeed(
						output(
							JSON.stringify({
								data: { s0: { pullRequest: { additions: 4, deletions: 1 } } },
							}),
						),
					),
				);
				...
				expect(document).toContain(
					's0: repository(owner: "acme", name: "web")',
				);
	...
	it.effect(
		"refuses to look up counts for a repository GitHub cannot address",
		() =>
			Effect.gen(function* () {
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"hands a search to GitHub rather than to the rows already read",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("[]")));
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"joins a search onto the tab's own qualifiers instead of replacing them",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("[]")));
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
				...
				expect(searchOfCall(0)).toBe(
					'review-requested:bilal is:unmerged "page" sort:updated-desc',
				);
			}),
	);

	it.effect(
		"carries the further narrowings into the search as qualifiers",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("[]")));
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		'resolves an author filter of "me" to the viewer, not the literal word',
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("[]")));
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"sends one label qualifier per group, its names joined the way GitHub ors them",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("[]")));
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
				...
				expect(searchOfCall(0)).toBe(
					'label:"size:S","size:XS" label:"bug" sort:updated-desc',
				);
	...
	it.effect(
		"fails a checks filter for a row whose checks are still pending",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(Effect.succeed(output("[]")));
				mockedExecute.mockReturnValueOnce(
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(
						output(pullRequests(2, 1, (number) => ({ isDraft: number === 1 }))),
					),
				);
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"quotes a search, so it cannot add a qualifier or a flag of its own",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("[]")));
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"escapes a backslash before the quote it would otherwise let out",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("[]")));
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
				...
				expect(searchOfCall(0)).toBe(
					String.raw`"a\\\" is:merged" sort:updated-desc`,
				);
			}),
	);

	it.effect(
		"asks for nothing but the order when the reader typed only spaces",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("[]")));
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect("carries on from the instant the last slice ended on", () =>
		Effect.gen(function* () {
			...
			expect(searchOfCall(0)).toBe(
				"updated:<=2026-07-02T00:00:00Z sort:updated-desc",
			);
	...
	it.effect(
		"answers a search that found nothing with nothing, not with the whole repository",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"reads a repository GitHub will not search the way gh lists one",
		() =>
			Effect.gen(function* () {
				...
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(
						output(pullRequests(3, 1, () => ({ state: "CLOSED" }))),
					),
				);
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"keeps state and involvement filters on the search-free fallback",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(Effect.succeed(output("[]")));
				mockedExecute.mockReturnValueOnce(
	...
	it.effect(
		"grows the search-free fallback until it fills the filtered page",
		() =>
			Effect.gen(function* () {
				const unrelated = () => ({
				...
				mockedExecute.mockReturnValueOnce(Effect.succeed(output("[]")));
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(output(pullRequests(3, 1, unrelated))),
				);
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(
						output(
				...
				expect(
					firstFallbackArgs[firstFallbackArgs.indexOf("--limit") + 1],
				).toBe("3");
				expect(
					secondFallbackArgs[secondFallbackArgs.indexOf("--limit") + 1],
				).toBe("6");
				assert.isFalse(batch.truncated);
			}),
	);
	...
	it.effect(
		"bounds a sparse search-free fallback and reports the unread tail",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockImplementation((_input) => {
					if (mockedExecute.mock.calls.length === 1)
	...
	it.effect(
		"takes an empty slice for a repository that has run out, not one to read again",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("[]")));
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"updates a stale branch with a merge commit unless asked to rebase",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("")));
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"arms auto-merge with the same strategy a merge would have used",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("")));
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"does not approve action-required runs for a same-repository pull request",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(
	...
	it.effect(
		"refuses a stale workflow approval after the pull request head changes",
		() =>
			Effect.gen(function* () {
				const detail = {
					number: 7,
	...
	it.effect(
		"refuses workflow approval when one head belongs to several pull requests",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(
	...
	it.effect(
		"refuses workflow approval when GitHub omits the head repository",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(
	...
	it.effect(
		"serves a diff GitHub hands over whole in one request, with no next slice",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
		...
		() =>
			Effect.gen(function* () {
				...
				expect(args).toContain(
					"repos/acme/web/pulls/7/files?per_page=100&page=1",
				);
			}),
	);

	it.effect(
		"hands back a cursor for the next page rather than walking on by itself",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(Effect.fail(diffRefused));
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(output(pullRequestFiles(100, 0))),
				);
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect("carries on from a cursor without asking `gh pr diff` again", () =>
		Effect.gen(function* () {
			...
			mockedExecute.mockReturnValueOnce(
				Effect.succeed(output(pullRequestFiles(4, 100))),
			);
			const second = yield* cli.getPullRequestDiff({
				...target,
			...
			expect(second.patch).toContain(
				"diff --git a/src/file100.ts b/src/file100.ts",
			);
			...
			expect(callAt(2).args).toContain(
				"repos/acme/web/pulls/7/files?per_page=100&page=2",
			);
		}),
	);

	it.effect(
		"refuses a cursor it never handed out rather than reading it into a request",
		() =>
			Effect.gen(function* () {
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"pages inside a commit the way it pages the pull request's own files",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(output(pullRequestFiles(100, 0))),
				...
				assert.isNull(second.nextCursor);
				expect(callAt(1).args).toContain(
					"repos/acme/web/commits/a1b2c3d?per_page=100&page=2",
				);
			}),
	);

	it.effect(
		"refuses a commit that is not a sha rather than reading it into a request",
		() =>
			Effect.gen(function* () {
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"expands a new file from a root commit without requiring a parent",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(output("\ta1b2c3d\n")),
				);
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(output("root contents\n")),
				);
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
				...
				assert.strictEqual(mockedExecute.mock.calls.length, 2);
				expect(callAt(1).args.join(" ")).toContain(
					"contents/src/root.ts?ref=a1b2c3d",
				);
			}),
	);
	...
	it.effect("reports unusable diff revisions as a structured error", () =>
		Effect.gen(function* () {
			mockedExecute.mockReturnValueOnce(
				Effect.succeed(output("not-a-sha\tstill-not-a-sha\n")),
			);
			const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect("reports an oversized diff file with its path and reason", () =>
		Effect.gen(function* () {
			mockedExecute.mockReturnValueOnce(
				Effect.succeed(output("a1b2c3d\tb1c2d3e\n")),
			);
			mockedExecute.mockReturnValueOnce(
				Effect.succeed(output("partial", true)),
			);
			const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"returns valid text containing a literal replacement character",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(output("a1b2c3d\tb1c2d3e\n")),
				...
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(output("before\uFFFDafter")),
				);
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"ends the diff on a page with no files rather than asking for it again",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("[]")));
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"reports the refused diff when the files API cannot answer either",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(Effect.fail(diffRefused));
				...
				Effect.succeed(
					output(
						...
						JSON.stringify({
							data: {
								nodes: [
									{ login: "octocat", avatarUrl: "https://avatars/octocat" },
								],
								rateLimit: {
									cost: 1,
									limit: 5_000,
	...
	it.effect("resolves and unresolves through the mutation each one needs", () =>
		Effect.gen(function* () {
			...
			const parse = (index: number) =>
				JSON.parse(callAt(index).stdin ?? "") as { query: string };
	...
	it.effect(
		"refuses a given subject that belongs to a different pull request",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(
	...
	it.effect(
		"looks up the pull request's own node id when no subject was given",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(
						output(
							// @effect-diagnostics-next-line preferSchemaOverJson:off
							JSON.stringify({
								data: { repository: { pullRequest: { id: "PR_kwDOA" } } },
							}),
						),
					),
				);
	...
	it.effect("rewrites a remark through the mutation its kind needs", () =>
		Effect.gen(function* () {
			...
			expect(parse(1).variables).toEqual({
				commentId: "IC_1",
				body: "Reworded.",
			});
			expect(parse(3).query).toContain("updatePullRequestReviewComment(");
			expect(parse(3).variables).toEqual({
				commentId: "IC_1",
				body: "Reworded.",
			});
		}),
	...
	it.effect("fails the read when gh returns something unreadable", () =>
		Effect.gen(function* () {
			mockedExecute.mockReturnValueOnce(
				Effect.succeed(output('{"message":"not found"}')),
			);
			const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"keeps the core detail read separate from conversation activity",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(
	...
	it.effect(
		"fails a files page too large to read rather than calling the diff whole",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(Effect.fail(diffRefused));
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(output(pullRequestFiles(1, 1), true)),
				);
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"pages an oversized patch by file rather than handing back a severed one",
		() =>
			Effect.gen(function* () {
				...
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(output(pullRequestFiles(1, 1))),
				);
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"follows the cursor to the review threads the first page left behind",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(
						output(reviewThreadsPage([thread("PRRT_1", "c1")], "Y3Vyc29yOjE")),
					),
				);
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(
						output(reviewThreadsPage([thread("PRRT_2", "c2")], null)),
					),
				);
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
				...
				expect(conversation.comments.map((comment) => comment.id)).toEqual([
					"c1",
					"c2",
				]);
	...
	it.effect(
		"stops at the thread bound and says the conversation was cut short",
		() =>
			Effect.gen(function* () {
					...
					output(
						reviewThreadsPage(
							[
								{
									...thread("PRRT_1", "c1"),
									comments: threadComments(["c1"], "Y3Vyc29yOjI", 3),
								},
							],
							null,
						),
	...
	it.effect("refuses a review thread from another pull request", () =>
		Effect.gen(function* () {
			mockedExecute.mockReturnValueOnce(
				Effect.succeed(
					output(threadCommentsPage(["foreign"], null, 1, "PR_8")),
				),
			);
			const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"sends the base comparison's variables as gh flags, not as bare words",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(
					Effect.succeed(
				...
				expect(args.at(-1)).toContain(
					`query=${BASE_COMPARISON_GRAPHQL_QUERY.slice(0, -2)}`,
				);
			}),
	);
	...
	it.effect(
		"lets an interactive permission read use the protected reserve",
		() =>
			Effect.gen(function* () {
				mockedExecute
					.mockReturnValueOnce(
	...
	it.effect(
		"reads the viewer's role off the same call as the merge settings",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(
					Effect.succeed(
				...
				expect(access.mergeCapabilities).toEqual({
					merge: false,
					squash: true,
					rebase: true,
				});
	...
	it.effect(
		"asks GitHub to review, naming the collection a request is added to",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("{}")));
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"takes a request back by deleting from the same collection it was added to",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("{}")));
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"puts labels on by posting to the issue's own collection, all at once",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("[]")));
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
	...
	it.effect(
		"takes labels off one at a time, naming each in the path encoded",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("[]")));
				const cli = yield* GitHubPullRequestCli.GitHubPullRequestCli;
				...
				assert.strictEqual(mockedExecute.mock.calls.length, 2);
				expect(callAt(0).args).toContain(
					"repos/acme/web/issues/7/labels/good%20first%20issue",
				);
				expect(callAt(0).args).toContain("DELETE");
				expect(callAt(1).args).toContain(
					"repos/acme/web/issues/7/labels/area%2Fweb",
				);
			}),
	);
});
