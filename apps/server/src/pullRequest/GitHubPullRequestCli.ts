...
export class GitHubPullRequestCli extends Context.Service<
	GitHubPullRequestCli,
	{
		...
		readonly listPullRequestStats: (input: {
			...
			ReadonlyArray<GitHubPullRequestStat>,
			GitHubPullRequestCliError
		>;
...
function filterQualifiers(
	...
	return [
		...
		...(filters.labels ?? []).flatMap((group) =>
			group.length === 0
				? []
				: [`label:${group.map(qualifierValue).join(",")}`],
		),
		...(filters.excludedLabels ?? []).map(
			(label) => `-label:${qualifierValue(label)}`,
		),
		...
		...(filters.draft === undefined
			? []
			: [`draft:${filters.draft === "only"}`]),
		...(filters.review === undefined
			? []
			: [`review:${REVIEW_QUALIFIERS[filters.review]}`]),
	...
	const matchesInvolvement =
		...
		(input.involvement === "authored"
			...
			: item.hasTeamReviewRequest ||
				item.reviewRequestLogins.some(
					(login) => login.toLowerCase() === viewer,
				));
	return (
		matchesState &&
		matchesInvolvement &&
		matchesFilters(item, input.filters, input.viewer)
	);
}
...
function searchQuery(input: {
	...
	if (!repositories.every((repository) => SEARCH_REPOSITORY.test(repository)))
		return null;
	const query = input.query?.trim() ?? "";
	return [
		"is:pr",
...
export const make = Effect.gen(function* () {
	...
	const repositoryArgs = (input: {
		readonly host: string;
		readonly repository: string;
	}) => ["--repo", `${input.host}/${input.repository}`];
	...
	const graphqlRead = <A>(input: {
		...
		return graphQlBudget
			...
			.pipe(
				...
				Effect.tap((result) =>
					graphQlBudget.observe(input.host, result.stdout),
				),
					...
					if (result.stdoutTruncated) {
						return Effect.fail(
							new GitHubPullRequestReadError({
								...
								cause: new Error(
									`Page ${input.page} of the changed files was too large to read.`,
								),
							}),
						);
					}
		...
		(input) =>
			Effect.gen(function* () {
				if (input.commit !== undefined && !isCommitSha(input.commit)) {
					return yield* new GitHubDiffCommitError({
						command: "gh",
						cwd: input.cwd,
					});
				}
				...
				if (
					...
					extraRefs.length > 0 ||
					(!rootCommitNewFile &&
						(baseRef === undefined || !isCommitSha(baseRef))) ||
					!isCommitSha(headRef)
				) {
	...
	const getPullRequestDetail: GitHubPullRequestCli["Service"]["getPullRequestDetail"] =
		(input) =>
			github
				.execute({
					cwd: input.cwd,
				...
				.pipe(
					Effect.flatMap(
						...
						> => {
							...
							if (!Result.isSuccess(decoded)) {
								return Effect.fail(
									workflowApprovalReadError(input.cwd, decoded.failure),
								);
							}
							...
							if (
								exactHeads.length !== 1 ||
								exactHeads[0]?.number !== input.number
							) {
								return Effect.fail(
									new GitHubWorkflowApprovalRefusedError({
										command: "gh",
					...
					Effect.flatMap(
						(
							...
							GitHubPullRequestReadError | GitHubWorkflowApprovalRefusedError
						> => {
							const decoded = decodeWorkflowRunApprovalsJson(
								result.stdout.trim(),
							);
							if (!Result.isSuccess(decoded)) {
								return Effect.fail(
	...
	return GitHubPullRequestCli.of({
				...
				.pipe(
					Effect.flatMap((result) => {
						...
						return login.length > 0
							...
							: Effect.fail(
									new GitHubViewerLoginUnavailableError({
										command: "gh",
										cwd: input.cwd,
									}),
								);
		...
		listPullRequests: (input) => {
			const fallbackMaxRows = Math.max(
				input.limit + 1,
				PULL_REQUEST_FALLBACK_MAX_ROWS,
			);
			const read = (
				continues: boolean,
						...
						Effect.flatMap((result) => {
							...
							if (Result.isSuccess(decoded)) {
								const items = continues
									...
									: decoded.success.items.filter((item) =>
											matchesUnsortedListing(item, input),
										);
								if (
									!continues &&
		...
		listPullRequestStats: (input) => {
			...
			for (
				let start = 0;
				start < input.changeRequests.length;
				start += STAT_ALIASES_PER_REQUEST
			) {
				chunks.push(
					input.changeRequests.slice(start, start + STAT_ALIASES_PER_REQUEST),
				);
			}
		...
		getPullRequestDiff: (input) => {
			...
			if (input.commit !== undefined && !isCommitSha(input.commit)) {
				return Effect.fail(
					new GitHubDiffCommitError({ command: "gh", cwd: input.cwd }),
				);
			}
			...
			if (input.cursor !== undefined) {
				...
				return page === null
					? Effect.fail(
							new GitHubDiffCursorError({ command: "gh", cwd: input.cwd }),
						)
					: filesPage(page);
			}
				...
				.pipe(
					...
					Effect.catchTags({
						GitHubCliCommandError: (error) =>
							filesPage(1).pipe(Effect.mapError(() => error)),
					}),
				);
		},
		...
		listReviewThreadComments: (input) =>
			Effect.gen(function* () {
				...
				let viewer: GitHubReviewThreadPage["viewer"] = {
					canUpdate: true,
					didAuthor: false,
				};
				...
				do {
					...
					if (page === 0) {
						...
						for (const [id, entry] of read.reactionsById)
							reactionsById.set(id, entry);
						...
						for (const [oid, stat] of read.commitStats)
							commitStats.set(oid, stat);
					}
				...
				let dismissalPage = 0;
				while (
					dismissalCursor !== null &&
					dismissalPage < REVIEW_THREAD_PAGES
				) {
					const read: {
				...
				return {
					...
					commentCount: entries.reduce(
						(total, entry) => total + entry.commentCount,
						0,
					),
					truncated:
						cursor !== null ||
						entries.some((entry) => entry.nextCommentCursor !== null),
					reactions,
					reactionsById,
					reviewers,
					avatarsByLogin,
										...
										getPullRequestDetail(input).pipe(
											Effect.flatMap((current) => {
												if (
													...
													current.headRepositoryOwner == null
												) {
													return Effect.fail(
														new GitHubWorkflowApprovalHeadUnavailableError({
															command: "gh",
			...
			return subjectId.pipe(
				Effect.flatMap((subjectId) =>
					graphql({
						...
						variables: {
							subjectId,
							content: gitHubReactionContent(input.content),
						},
					}),
				),
			);
