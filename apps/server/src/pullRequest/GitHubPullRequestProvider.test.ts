...
it.effect("uses one narrow read for a linked pull request summary", () =>
	Effect.gen(function* () {
		...
		if (readSummary === undefined)
			return yield* Effect.die("summary read was not implemented");
...
describe("gitHubViewerPermissions", () => {
	...
	it.effect(
		"keeps fork workflows awaiting approval out of the passing state",
		() =>
			Effect.gen(function* () {
				const provider = yield* make;
				const detail = yield* provider.getChangeRequest({
				...
				expect(detail.checks).toEqual([
					...
					},
					{
						...
						status: "action-required",
						description:
							"A maintainer must approve this workflow before it can run.",
...
it.effect(
	"does not classify same-repository gates as fork workflow approvals",
	() =>
		Effect.gen(function* () {
			const provider = yield* make;
			const detail = yield* provider.getChangeRequest({
			...
			Effect.provide(
				Layer.mock(GitHubPullRequestCli.GitHubPullRequestCli)({
					getPullRequestDetail: () =>
						Effect.succeed({ ...openDetail, isCrossRepository: false }),
					getPullRequestBaseComparison: () =>
						Effect.succeed({ behindBy: 0, viewerCanUpdate: true }),
					listWorkflowRunsRequiringApproval: () =>
						Effect.die(
							"same-repository pull requests must not probe fork workflow approvals",
						),
					getRepositoryAccess: () =>
						Effect.succeed({
							canWrite: true,
...
describe("getViewerPermissions", () => {
	...
	it.effect("offers update-branch when the comparison grants it", () =>
		Effect.gen(function* () {
			...
			Effect.provide(
				layerWithComparison(
					Effect.succeed({ behindBy: 3, viewerCanUpdate: true }),
				),
			),
		),
	);
...
describe("getChangeRequest commits", () => {
	...
	const layerWith = (commits: GitHubReviewThreadComments["commits"]) =>
		Layer.mock(GitHubPullRequestCli.GitHubPullRequestCli)({
			...
			listReviewThreadComments: () =>
				Effect.succeed({ ...baseThreadComments, commits }),
		});

	it.effect(
		"prefers the GraphQL commits, which are the newest, over the gh view list",
		() =>
			Effect.gen(function* () {
				const provider = yield* make;
				...
				expect(detail.commits.map((commit) => commit.oid)).toEqual([
					"graphql-newest",
				]);
			}).pipe(
				Effect.provide(
	...
	it.effect(
		"falls back to the gh view list when the GraphQL read has no commits",
		() =>
			Effect.gen(function* () {
				const provider = yield* make;
				const detail = yield* provider.getChangeRequestActivity({
...
describe("getChangeRequestActivity dismissed reviews", () => {
	...
	const threadComments: GitHubReviewThreadComments = {
		...
		dismissalsByReviewId: new Map([
			["PRR_1", "Dismissing prior approval to re-evaluate 9b66581"],
		]),
		reviewThreads: [],
		commentCount: 0,
		truncated: false,
	...
	it.effect(
		"fills a marker-only dismissed review with the timeline's reason",
		() =>
			...
			readActivity.pipe(
				Effect.map((activity) => {
					expect(activity.comments[0]?.body).toBe(
						"Dismissing prior approval to re-evaluate 9b66581",
					);
				}),
				Effect.provide(
					layerFor("<!-- Macroscope (Approvability) review body marker -->"),
				),
			),
	);
			...
			Effect.provide(
				Layer.mock(GitHubPullRequestCli.GitHubPullRequestCli)({
					updatePullRequest: (input) =>
						Effect.sync(() => void rewrites.push(input)),
					updateComment: (input) =>
						Effect.sync(() => void rewrites.push(input)),
				}),
			),
...
describe("loginAvatarUrl", () => {
	it("serves a user's picture from the host they belong to", () => {
		expect(loginAvatarUrl("octocat", "github.com")).toBe(
