 ...
 */
export function gitHubViewerPermissions(
	access: GitHubViewerAccess,
): PullRequestViewerPermissions {
	return {
		actions: [
			...
			...(access.canWrite
				...
				: []),
			...(access.canUpdate
				? (["ready", "draft", "close", "reopen"] as const)
				: []),
...
function withAvatar(
	...
	const avatarUrl =
		avatarsByLogin.get(actor.login) ?? loginAvatarUrl(actor.login, host);
...
export function loginAvatarUrl(login: string, host: string): string | null {
	return /^[a-z0-9][a-z0-9-]{0,38}$/iu.test(login)
		? `https://${host}/${login}.png?size=80`
		: null;
}
...
export const make = Effect.gen(function* () {
	...
	const repositoryAccessCache = yield* Cache.makeWith(
		(key: string) => {
			const [cwd, repository, host] = JSON.parse(key) as [
				string,
				string,
				string,
			];
	...
	const getRepositoryAccess = (input: {
		...
		Cache.get(
			...
			JSON.stringify([input.cwd, input.repository, input.host]),
		);
	...
	const provider: PullRequestProviderApi = {
					...
					Effect.flatMap((page) =>
						cli
							.listActorAvatars({
								...
								host: input.host,
								ids: [
									...new Set(page.items.flatMap((item) => item.authorId ?? [])),
								],
							})
		...
		getChangeRequestSummary: (input) =>
			cli
				...
				.pipe(Effect.mapError(fail("getChangeRequestSummary"))),
											...
											: cli
													...
													.pipe(
														Effect.matchEffect({
															...
															onSuccess: (runs) =>
																Effect.succeed({ runs, unavailable: false }),
														}),
													),
						...
						...(detail.workflowApprovals.unavailable
							...
							: {
									workflowApprovalsRequired:
										detail.workflowApprovals.runs.length,
								}),
			...
			Effect.all(
				...
				Effect.map(
					([pullRequest, reviewThreads]): ProviderChangeRequestActivity => ({
							...
							.map((comment) => ({
								...
								body:
									...
									rendersEmpty(comment.body)
										? (reviewThreads.dismissalsByReviewId.get(comment.id) ??
											comment.body)
										: comment.body,
								author: withAvatar(
									comment.author,
								...
								reactions:
									comment.reactions ??
									reviewThreads.reactionsById.get(comment.id) ??
									[],
							}))
							.toSorted((left, right) =>
						...
						commentCount:
							pullRequest.comments.length + reviewThreads.commentCount,
		...
		getReviewThreadComments: (input) =>
			cli
				.getReviewThreadComments(input)
				.pipe(Effect.mapError(fail("getReviewThreadComments"))),
						...
						Effect.flatMap((pullRequest) =>
							...
							pullRequest.headRepositoryOwner === null
								...
								: cli
										...
										.pipe(
											Effect.map(
												(comparison) => comparison.viewerCanUpdate === true,
											),
										),
						),
		...
		getDiff: (input) =>
			cli.getPullRequestDiff(input).pipe(Effect.mapError(fail("getDiff"))),
		...
		comment: (input) =>
			cli.commentOnPullRequest(input).pipe(Effect.mapError(fail("comment"))),
		...
		submitReview: (input) =>
			cli.submitReview(input).pipe(Effect.mapError(fail("submitReview"))),
