...
layer("GitLabPullRequestCli.layer", (it) => {
	it.effect(
		"asks GitLab for one row more than the page, to probe for a next page",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(output(mergeRequests(3, 1))),
				);
	...
	it.effect(
		"URL-encodes a search, so it cannot add a parameter of its own",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(Effect.succeed(output("[]")));
				const cli = yield* GitLabPullRequestCli.GitLabPullRequestCli;
	...
	it.effect("stops walking on a short page", () =>
		Effect.gen(function* () {
			mockedExecute.mockReturnValueOnce(
				Effect.succeed(output(mergeRequests(40, 1))),
			);
			const cli = yield* GitLabPullRequestCli.GitLabPullRequestCli;
	...
	it.effect("stops walking when every row on a page fails to decode", () =>
		Effect.gen(function* () {
			...
			const unusable = JSON.stringify(
				Array.from({ length: 100 }, () => ({ iid: "nope" })),
			);
	...
	it.effect("addresses a nested group project by its encoded full path", () =>
		Effect.gen(function* () {
			...
			expect(argsOfCall(0)[1]).toContain(
				"projects/acme%2Fplatform%2Fweb/merge_requests",
			);
		}),
	);
	...
	it.effect(
		"cancels an armed auto-merge through the API glab has no flag for",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(Effect.succeed(output("{}")));
				const cli = yield* GitLabPullRequestCli.GitLabPullRequestCli;
	...
	it.effect(
		"brings a stale branch up to date by rebasing it, the only way GitLab has",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(Effect.succeed(output("")));
				const cli = yield* GitLabPullRequestCli.GitLabPullRequestCli;
	...
	it.effect("reads one diff page and hands back the cursor for the next", () =>
		Effect.gen(function* () {
			...
			expect(argsOfCall(0)[1]).toContain(
				"merge_requests/7/diffs?per_page=100&page=1",
			);
		}),
	);
	...
	it.effect("carries on from a cursor at the page it names", () =>
		Effect.gen(function* () {
			...
			const second = yield* cli.getMergeRequestDiff({
				...target,
	...
	it.effect(
		"refuses a cursor it never handed out rather than reading it into a query",
		() =>
			Effect.gen(function* () {
				const cli = yield* GitLabPullRequestCli.GitLabPullRequestCli;
	...
	it.effect(
		"refuses a commit that is not a sha rather than reading it into a path",
		() =>
			Effect.gen(function* () {
				const cli = yield* GitLabPullRequestCli.GitLabPullRequestCli;
	...
	it.effect("reports a commit with no parent as a structured error", () =>
		Effect.gen(function* () {
			mockedExecute.mockReturnValueOnce(
				// @effect-diagnostics-next-line preferSchemaOverJson:off
				Effect.succeed(
					output(JSON.stringify({ id: "a1b2c3d", parent_ids: [] })),
				),
			);
			const cli = yield* GitLabPullRequestCli.GitLabPullRequestCli;
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(output("first contents\n")),
				);
				const cli = yield* GitLabPullRequestCli.GitLabPullRequestCli;
	...
	it.effect(
		"fails a diff page cut off mid-JSON rather than calling the diff whole",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					// A byte-truncated prefix: valid JSON never survives the cut.
					Effect.succeed({
						...output('[{"old_path":"src/x.ts","new_p'),
	...
	it.effect(
		"offers no squash when the project does not say it allows one",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					// @effect-diagnostics-next-line preferSchemaOverJson:off
	...
	it.effect(
		"reads the project's merge settings as its merge capabilities",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					// @effect-diagnostics-next-line preferSchemaOverJson:off
					Effect.succeed(
						output(
							JSON.stringify({ merge_method: "ff", squash_option: "never" }),
						),
					),
				);
				const cli = yield* GitLabPullRequestCli.GitLabPullRequestCli;
	...
	it.effect(
		"asks the detail read for the divergence GitLab withholds by default",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
	...
	it.effect("fails when the authenticated account has no username", () =>
		Effect.gen(function* () {
			// @effect-diagnostics-next-line preferSchemaOverJson:off
			mockedExecute.mockReturnValueOnce(
				Effect.succeed(output(JSON.stringify({ username: "" }))),
			);
			const cli = yield* GitLabPullRequestCli.GitLabPullRequestCli;
	...
	it.effect(
		"stops the note walk at its bound and says the conversation was cut short",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"reads a positioned discussion as a thread anchored to its line",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(
						output(
							...
							JSON.stringify([
								...
								// A plain note is the timeline's business, not the diff's.
								{
									id: "def456",
									notes: [
										{ id: 3, body: "ship it", created_at: "2026-07-01Z" },
									],
								},
							]),
						),
	...
	it.effect(
		"sends a review as its comments, then its summary, then the verdict",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(
	...
	it.effect(
		"does not ask for diff revisions when a review carries no line comments",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("{}")));
				const cli = yield* GitLabPullRequestCli.GitLabPullRequestCli;
	...
	it.effect(
		"removes an award by listing them and deleting the reader's own id",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					// @effect-diagnostics-next-line preferSchemaOverJson:off
	...
	it.effect(
		"does nothing when the reader has no award of that name to take back",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					// @effect-diagnostics-next-line preferSchemaOverJson:off
	...
	it.effect(
		"names a merge request with no diff revisions rather than calling it unreadable",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValueOnce(
					Effect.succeed(
	...
	it.effect(
		"reads who has access to the project and who is already on the merge request",
		() =>
			Effect.gen(function* () {
				mockedExecute
					.mockReturnValueOnce(
						Effect.succeed(output(mergeRequestJson({ reviewers: [reviewer] }))),
					)
					.mockReturnValueOnce(
						Effect.succeed(
							// @effect-diagnostics-next-line preferSchemaOverJson:off
			...
			mockedExecute
				.mockReturnValueOnce(
					Effect.succeed(
						output(
							mergeRequestJson({
								reviewers: [reviewer, { id: 9, username: "hubot" }],
							}),
						),
					),
				)
	...
	it.effect(
		"ignores an id GitLab could not have handed out, which names nobody",
		() =>
			Effect.gen(function* () {
				mockedExecute
					.mockReturnValueOnce(
	...
	it.effect(
		"sends a rewritten body as GitLab's description, and nothing else",
		() =>
			Effect.gen(function* () {
				mockedExecute.mockReturnValue(Effect.succeed(output("{}")));
				const cli = yield* GitLabPullRequestCli.GitLabPullRequestCli;
