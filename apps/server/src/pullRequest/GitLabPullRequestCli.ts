...
export const make = Effect.gen(function* () {
		...
		return api({
			...
			Effect.flatMap((result) => {
				...
				const consumed = Math.max(
					0,
					decoded.success.rawCount - skipOnFirstPage,
				);
	...
	const getDiffRefs = (input: {
		...
		api({
			...
			Effect.flatMap(
				(result): Effect.Effect<GitLabDiffRefs, GitLabPullRequestCliError> => {
	...
	const awardsPage = (input: {
		...
		readonly collected: {
			readonly reactions: ReadonlyArray<PullRequestReaction>;
			readonly reactionsByNoteId: Map<
				string,
				ReadonlyArray<PullRequestReaction>
			>;
		} | null;
	...
	> =>
		api({
			...
			Effect.flatMap((result) => {
				...
				return decoded.success.nextCursor === null ||
					input.page >= CONVERSATION_PAGES
	...
	const viewerUsername = (input: { readonly cwd: string }) =>
		api({ cwd: input.cwd, path: "user" }).pipe(
			Effect.flatMap(
				(result): Effect.Effect<string, GitLabPullRequestCliError> => {
					...
					return decoded.success === null
						? Effect.fail(
								new GitLabViewerUnavailableError({
									command: "glab",
									cwd: input.cwd,
								}),
							)
	...
	return GitLabPullRequestCli.of({
		...
		listReactions: (input) =>
			awardsPage({ ...input, cursor: null, page: 1, collected: null }),
		...
		getMergeRequestDiff: (input) => {
			...
			return page === null
				? Effect.fail(
						new GitLabDiffCursorError({ command: "glab", cwd: input.cwd }),
					)
		...
		getProjectMergeCapabilities: (input) =>
			api({
				...
				Effect.flatMap((result) => {
					const decoded = decodeProjectMergeCapabilitiesJson(
						result.stdout.trim(),
					);
					return Result.isSuccess(decoded)
						? Effect.succeed(decoded.success)
		...
		listReviewerCandidates: (input) =>
			Effect.all([mergeRequestDetail(input), projectUsers(input)], {
				concurrency: 2,
			}).pipe(
						...
						candidates: users.candidates.flatMap((candidate) =>
							candidate.login === author
								...
								: [
										{
											...candidate,
											isRequested: requested.has(candidate.login),
										},
									],
						),
		...
		listDiscussions: (input) =>
			discussionsPage({ ...input, page: 1, collected: [] }),
