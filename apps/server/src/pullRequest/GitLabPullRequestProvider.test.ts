...
describe("gitLabViewerPermissions", () => {
	...
	it("names no way of updating a branch it will not let this viewer update", () => {
		...
		expect(
			gitLabViewerPermissions({ viewerCanMerge: false }).updateMethods,
		).toBeUndefined();
	});
...
describe("getChangeRequest base freshness", () => {
			...
			Effect.provide(
				Layer.mock(GitLabPullRequestCli.GitLabPullRequestCli)({
					getMergeRequestDetail: () =>
						Effect.succeed({ ...detail, ...divergence }),
	...
	it.effect(
		"reads a counted divergence as a branch that has fallen behind",
		() =>
			Effect.gen(function* () {
...
describe("rewriting what has already been said", () => {
	...
	it.effect(
		"rewrites a positioned comment through the same note as any other",
		() =>
			Effect.gen(function* () {
				const provider = yield* providerWith;
