...
export function gitLabProviderFailure(
	...
	if (error._tag === "GitLabCliUnavailableError")
		return { reason: "missing-tool" };
	if (error._tag === "GitLabCliAuthenticationError")
...
export const make = Effect.gen(function* () {
	...
	const provider: PullRequestProviderApi = {
		...
		getViewer: (input) =>
			cli
				.getViewerUsername({ cwd: input.cwd })
		...
		getChangeRequest: (input) =>
			Effect.all(
				[
					...
					cli.getProjectMergeCapabilities({
						cwd: input.cwd,
						repository: input.repository,
					}),
				],
			...
			Effect.all(
				[
					...
					cli
						...
						.pipe(
							Effect.orElseSucceed(() => ({ threads: [], truncated: true })),
						),
		...
		getDiff: (input) =>
			cli.getMergeRequestDiff(input).pipe(Effect.mapError(fail("getDiff"))),
		...
		comment: (input) =>
			cli.commentOnMergeRequest(input).pipe(Effect.mapError(fail("comment"))),
