...
const FULL_REVIEWERS: PullRequestReviewerCapabilities = {
	request: true,
	listCandidates: true,
};
	...
	() =>
		Effect.gen(function* () {
			...
			const error = yield* Effect.flip(
				service.list({
					state: "open",
					cursors: { "github.com pingdotgg/t3code": "yesterday" },
				}),
			);
...
it.effect("a listing narrowed to some projects is its own cache entry", () =>
	Effect.gen(function* () {
		...
		yield* service.list({ state: "open" });
		const narrowed = yield* service.list({
			state: "open",
...
it.effect(
	"a mutation makes the next listing ask the host again, with no client asking",
	() =>
		Effect.gen(function* () {
			let hostCalls = 0;
			const service = yield* makeService({
