...
const ACTION_ACCESS_REFUSALS: Record<PullRequestAction, string> = {
	...
	revert:
		"You need write access on this repository to open a revert pull request.",
	"approve-workflows":
 ...
 */
const REVIEWER_REQUEST_REFUSAL =
	"You need write access on this repository to ask for a review.";
const LABEL_CHANGE_REFUSAL =
	"You need triage access on this repository to change its labels.";
...
function nextListCursor(
	...
	const oldest = fetched.reduce((left, right) =>
...
function toUnavailableError(
	error: PullRequestProviderError,
): PullRequestUnavailableError {
...
function withRateLimitBackoff(
	...
	return {
		...
		...(api.listChangeRequestsAcross === undefined
			? {}
			: {
					listChangeRequestsAcross: wrap(
						"listChangeRequestsAcross",
						api.listChangeRequestsAcross,
					),
				}),
		...(api.listChangeRequestStats === undefined
			? {}
			: {
					listChangeRequestStats: wrap(
						"listChangeRequestStats",
						api.listChangeRequestStats,
					),
				}),
		...
		...(api.getChangeRequestSummary === undefined
			? {}
			: {
					getChangeRequestSummary: wrap(
						"getChangeRequestSummary",
						api.getChangeRequestSummary,
					),
				}),
		getChangeRequestActivity: wrap(
			"getChangeRequestActivity",
			api.getChangeRequestActivity,
		),
		...(api.getReviewThreadComments === undefined
			? {}
			: {
					getReviewThreadComments: wrap(
						"getReviewThreadComments",
						api.getReviewThreadComments,
					),
				}),
		getViewerPermissions: interactive(
			"getViewerPermissions",
			api.getViewerPermissions,
		),
		getDiff: wrap("getDiff", api.getDiff),
		...(api.getDiffFileContents === undefined
			? {}
			: {
					getDiffFileContents: wrap(
						"getDiffFileContents",
						api.getDiffFileContents,
					),
				}),
		...
		...(api.updateChangeRequest === undefined
			? {}
			: {
					updateChangeRequest: interactive(
						"updateChangeRequest",
		...
		listReviewerCandidates: interactive(
			"listReviewerCandidates",
			api.listReviewerCandidates,
		),
		setReviewerRequest: interactive(
			"setReviewerRequest",
			api.setReviewerRequest,
		),
		...(api.listLabelCandidates === undefined
			? {}
			: {
					listLabelCandidates: interactive(
						"listLabelCandidates",
						api.listLabelCandidates,
					),
				}),
		...(api.setLabels === undefined
			? {}
			: { setLabels: interactive("setLabels", api.setLabels) }),
		replyToThread: interactive("replyToThread", api.replyToThread),
		setReaction: interactive("setReaction", api.setReaction),
		setThreadResolution: interactive(
			"setThreadResolution",
			api.setThreadResolution,
		),
	};
}
...
export function repositoryIdentityOf(
	...
	if (identity.provider === "azure-devops") {
		const segments = (identity.displayName ?? "")
			.split("/")
			.filter((part) => part !== "_git");
...
export const make = Effect.gen(function* () {
	...
	const refineUnknownProjectKinds = (
		...
		for (const project of projects) {
			if (filter.projectId !== undefined && project.id !== filter.projectId)
				continue;
			const identity = project.repositoryIdentity;
			if (
				identity?.provider !== "unknown" ||
				repositoryIdentityOf(project) === null
			)
				continue;
			...
			if (provider !== null) {
				...
				if (candidates === undefined)
					refinements.set(provider.baseUrl, [candidate]);
			...
			Effect.map(({ refinedKinds, snapshot }) => {
				...
				for (const project of snapshot.projects) {
					...
					if (
						...
						!filter.projectIds.includes(project.id)
					)
						continue;
					const identity = project.repositoryIdentity;
					let kind = identity?.provider as
					...
					if (kind === "unknown") {
						...
						kind =
							provider === null
								? kind
								: (refinedKinds.get(provider.baseUrl) ?? kind);
					}
					...
					if (api !== null) {
						...
						if (roots === undefined)
							viewerRoots.set(host, [project.workspaceRoot]);
						else if (!roots.includes(project.workspaceRoot))
					...
					if (api === null) {
						...
						if (counted === undefined)
							unimplemented.set(host, { kind, projectCount: 1 });
	...
	const requireProject = (
		...
		listWorkspaceProjects({ projectId: ref.projectId }).pipe(
			Effect.flatMap(
				({ supported }): Effect.Effect<SupportedProject, PullRequestError> => {
	...
	const decodeCursors = (
		...
		ReadonlyMap<string, ListCursor> | null,
		PullRequestError
	> => {
	...
	const viewerFlights = yield* Cache.makeWith(
		(key: string): Effect.Effect<ResolvedViewer> => {
			...
			return Effect.firstSuccessOf(
				roots.map((cwd) => api.getViewer({ cwd })),
			).pipe(
				Effect.map((viewer) => ({
					host,
					kind,
		...
		},
		{
			...
			timeToLive: (exit) =>
				Exit.isSuccess(exit) && exit.value.error === null
					? Duration.seconds(1)
					: Duration.zero,
		},
	);
		...
		Effect.forEach(
			...
			(host) =>
				Effect.flatMap(
					...
					(now): Effect.Effect<ResolvedViewer> => {
						...
						const roots =
							viewerRoots.get(host) ??
							forHost.map(({ project }) => project.workspaceRoot);
	...
	const matchesRowFilters = (
		...
		const holds = (label: string) =>
			labels.includes(label.trim().toLowerCase());
		return (
			(filters.draft === undefined ||
				item.isDraft === (filters.draft === "only")) &&
	...
	const toEntry = (input: {
		...
		return {
			...
			...(input.item.checksState === undefined ||
			input.item.checksState === null
				? {}
				: { checksState: input.item.checksState }),
			viewerReviewRequested:
				input.item.author?.login.toLowerCase() !== viewer &&
				input.item.reviewRequestLogins.some(
					(login) => login.toLowerCase() === viewer,
				),
			labels: input.item.labels,
			...(input.item.reviewDecision === undefined ||
			input.item.reviewDecision === null
				? {}
		...
		Effect.gen(function* () {
			...
			const providers: ReadonlyArray<PullRequestProviderSummary> = [
				...viewerResults.map((result) => ({
					...
					kind: result.kind,
					searchesOnHost:
						projects.find((project) => project.host === result.host)?.api
			...
			if (readable.length === 0) {
				...
				const errors = viewerResults.flatMap((result) =>
					result.error === null ||
					!selected.some(({ host }) => host === result.host)
						? []
						: [result.error],
				);
			...
			const readTogether = (
				...
				const separately = () =>
					Effect.forEach(chunk, readRepository, {
						concurrency: REPOSITORY_CONCURRENCY,
					});
					...
					Effect.flatMap((page) =>
						Effect.flatMap(Clock.currentTimeMillis, (now) => {
							...
							for (const [key, visibleAt] of searchVisibleAt) {
								if (
									now - visibleAt >
									Duration.toMillis(SEARCH_VISIBILITY_TTL)
								) {
							...
							for (const item of page.items) {
								...
								searchVisibleAt.set(
									searchVisibilityKey(first.host, item.repository),
									now,
								);
							}
							...
							const boundary = page.items.reduce<string | null>(
								(oldest, item) =>
									oldest === null || item.updatedAt < oldest
										? item.updatedAt
										: oldest,
								null,
							);
							return Effect.forEach(
								...
								(project): Effect.Effect<RepositoryBatch> => {
									const fetched =
										rows.get(project.repository.trim().toLowerCase()) ?? [];
			...
			const reads: Array<Effect.Effect<ReadonlyArray<RepositoryBatch>>> =
				separate.map((project) =>
			...
			const batches = (yield* Effect.all(reads, {
				concurrency: REPOSITORY_CONCURRENCY,
			})).flat();
			...
			for (const batch of batches) {
				if (batch.nextCursor !== null)
					nextCursors[batch.key] = batch.nextCursor;
			}
						...
						...(changeRequest.headRepositoryNameWithOwner === undefined
							...
							: {
									headRepositoryNameWithOwner:
										changeRequest.headRepositoryNameWithOwner,
								}),
						baseBranch: changeRequest.baseBranch,
						...
						...(changeRequest.workflowApprovalsRequired === undefined
							...
							: {
									workflowApprovalsRequired:
										changeRequest.workflowApprovalsRequired,
								}),
					})),
				),
			),
	...
	const threadComments: PullRequestService["Service"]["threadComments"] = (
		input,
	) =>
		requireProject(input).pipe(
			Effect.flatMap(
		...
		requireProject(input).pipe(
			Effect.flatMap((project): Effect.Effect<string, PullRequestError> => {
				...
				if (
					...
					!(project.api.capabilities.updateMethods ?? []).includes(
						input.updateMethod,
					)
				) {
					return Effect.fail(
	...
	const update: PullRequestService["Service"]["update"] = (input) =>
		requireProject(input).pipe(
			Effect.flatMap((project): Effect.Effect<void, PullRequestError> => {
				...
				if (
					project.api.capabilities.edit?.changeRequest !== true ||
					rewrite === undefined
				) {
					return Effect.fail(
						new PullRequestOperationError({
	...
	const updateComment: PullRequestService["Service"]["updateComment"] = (
		input,
	) =>
		(input.body.trim().length === 0
			? Effect.fail(
			...
			Effect.flatMap((project): Effect.Effect<void, PullRequestError> => {
				...
				if (
					project.api.capabilities.edit?.comment !== true ||
					rewrite === undefined
				) {
					return Effect.fail(
						new PullRequestOperationError({
		...
		requireProject(input).pipe(
			Effect.flatMap((project): Effect.Effect<void, PullRequestError> => {
				...
				if (!review.verdicts.includes(input.verdict)) {
					return refuse(
						`This host cannot ${VERDICT_LABELS[input.verdict]} a change request.`,
					);
				}
				if (input.comments.length > 0 && !review.inlineComment) {
					return refuse(
						"This host cannot comment on a line of a change request.",
					);
				}
	...
	const replyToThread: PullRequestService["Service"]["replyToThread"] = (
		input,
	) =>
		(input.body.trim().length === 0
			? Effect.fail(
	...
	const setThreadResolution: PullRequestService["Service"]["setThreadResolution"] =
		(input) =>
			requireProject(input).pipe(
				Effect.flatMap((project): Effect.Effect<void, PullRequestError> => {
	 ...
	 */
	const reviewerCandidates: PullRequestService["Service"]["reviewerCandidates"] =
		(input) =>
			requireProject(input).pipe(
				Effect.flatMap(
					(
						...
						if (!project.api.capabilities.reviewers.listCandidates) {
							return Effect.fail(
								new PullRequestOperationError({
									...
									detail:
										"This host cannot say who may review a change request.",
								}),
							);
						}
						return viewerPermissionsOf(
								...
								> =>
									viewer.requestReviewers
										? project.api
												...
												.pipe(
													Effect.mapError(
														toPullRequestError("reviewerCandidates"),
													),
												)
										: Effect.fail(
												new PullRequestOperationError({
	...
	const requestReviewers: PullRequestService["Service"]["requestReviewers"] = (
		input,
	) =>
		requireProject(input).pipe(
			Effect.flatMap((project): Effect.Effect<void, PullRequestError> => {
	 ...
	 */
	const labelCandidates: PullRequestService["Service"]["labelCandidates"] = (
		input,
	) =>
		requireProject(input).pipe(
			Effect.flatMap(
					...
					if (project.api.capabilities.labels !== true || list === undefined) {
						return Effect.fail(
							new PullRequestOperationError({
								...
								detail:
									"This host cannot change the labels on a change request.",
							}),
						);
					}
							...
							> =>
								viewer.labels === false
									...
									: list({
											...
											Effect.mapError(toPullRequestError("labelCandidates")),
										),
						),
					);
				},
	 ...
	 */
	const listStatsUncached: PullRequestService["Service"]["listStats"] = (
		input,
	) =>
		Effect.gen(function* () {
			...
			const byProject = new Map(
				supported.map((project) => [project.project.id, project]),
			);
			...
			for (const ref of input.refs) {
				...
				wanted.set(`${project.project.id} ${ref.number}`, {
					project,
					number: ref.number,
				});
			}
			const byHost = new Map<
				string,
				Array<{ project: SupportedProject; number: number }>
			>();
	...
	const staleDiff = (() => {
		...
		const held = new Map<
			string,
			{ readonly at: number; readonly value: PullRequestDiffResult }
		>();
		...
		return <E>(key: string, read: Effect.Effect<PullRequestDiffResult, E>) => {
			...
			return Effect.flatMap(Clock.currentTimeMillis, (now) => {
				...
				return Effect.sync(() => runFork(Effect.ignore(recorded))).pipe(
	...
	const makeLastGoodRead = <A>(capacity: number) => {
			...
			effect.pipe(
				...
				Effect.catchTags({
					PullRequestOperationError: (error) => {
						...
						if (
							provider.reason !== "failed" &&
							provider.reason !== "rate-limited"
						) {
							return Effect.fail(error);
						}
						return Effect.flatMap(Clock.currentTimeMillis, (now) => {
							...
							return Effect.logWarning(
								"using recent pull request data after a failed refresh",
								{
		...
		return {
			peek: (key: string) => held.get(key)?.value,
			read,
			record,
			serveHeld,
		};
	};
	const lastGoodSummary = makeLastGoodRead<PullRequestSummary>(
		DETAIL_CACHE_CAPACITY,
	);
	const lastGoodDetail = makeLastGoodRead<PullRequestDetail>(
		DETAIL_CACHE_CAPACITY,
	);
	...
	const refScope = (ref: PullRequestRef) =>
		`${ref.projectId} ${ref.repository} ${ref.number}`;
	...
	const summaryCache = yield* Cache.makeWith(
		...
		},
		{
			...
			timeToLive: (exit) =>
				Exit.isSuccess(exit) ? SUMMARY_CACHE_TTL : Duration.zero,
		},
	);
	const summary: PullRequestService["Service"]["summary"] = (
		input,
		options,
	) => {
		...
		return held !== undefined &&
			...
			: cached.pipe(
					Effect.tap((value) =>
						shouldReplaceHeldSummary(key, value)
							? lastGoodSummary.record(key, value)
							: Effect.void,
					),
				);
	};
	...
	const listCache = yield* Cache.makeWith(
		...
		},
		{
			...
			timeToLive: (exit) =>
				Exit.isSuccess(exit) ? LIST_CACHE_TTL : Duration.zero,
		},
	);
	...
	const detailCache = yield* Cache.makeWith(
		(key: string) => {
			...
			return detailUncached({
				...
				Effect.tap(
					Effect.fn("PullRequestService.recordDetailStats")(function* (
		...
		},
		{
			capacity: DETAIL_CACHE_CAPACITY,
			timeToLive: (exit) =>
				Exit.isSuccess(exit) ? DETAIL_CACHE_TTL : Duration.zero,
		},
	);
	...
	const diffCache = yield* Cache.makeWith(
		(key: string) => {
			const [, projectId, repository, number, cursor, commit] = JSON.parse(
				key,
			) as [number, string, string, number, string | null, string | null];
			return diffUncached({
				projectId,
				repository,
				number,
	...
	const listStatsCache = yield* Cache.makeWith(
		(key: string) => {
			const [, refs] = JSON.parse(key) as [
				number,
				ReadonlyArray<[string, string, number, number]>,
			];
		...
		},
		{
			...
			timeToLive: (exit) =>
				Exit.isSuccess(exit) ? LIST_STATS_CACHE_TTL : Duration.zero,
		},
	);
	const statsBatchKey = (refs: Iterable<PullRequestRef>) =>
		JSON.stringify([
			...
			[...refs]
				.map(
					(ref) =>
						[ref.projectId, ref.repository, ref.number, refEpoch(ref)] as const,
				)
				.toSorted((left, right) =>
	...
	const listStats: PullRequestService["Service"]["listStats"] = Effect.fn(
		...
		for (const ref of input.refs) {
			...
			if (
				cached !== undefined &&
				now - cached.at < Duration.toMillis(LIST_STATS_CACHE_TTL)
			) {
	...
	const refreshAfterTurn: PullRequestService["Service"]["refreshAfterTurn"] =
		Effect.suspend(() => {
	...
	const runActionAndInvalidate: PullRequestService["Service"]["runAction"] =
		Effect.fn("PullRequestService.runActionAndInvalidate")(function* (input) {
