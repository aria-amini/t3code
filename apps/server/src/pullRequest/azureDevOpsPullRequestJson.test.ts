...
describe("decodePullRequestListJson", () => {
	it("reads a pull request as a change request", () => {
		const batch = expectSuccess(
			decodePullRequestListJson(asJson([pullRequest()])),
		);
	...
	it("assembles a browser url when Azure reports no web link", () => {
		...
		expect(batch.items[0]?.url).toBe(
			"https://dev.azure.com/acme/platform/_git/web/pullrequest/42",
		);
	});
			...
			decodePullRequestListJson(
				asJson([
					pullRequest({
						_links: {
							web: {
								href: "https://dev.azure.com/acme/platform/_git/web/pullrequest/42",
							},
						},
					}),
				]),
	...
	it.each([
		...
		const batch = expectSuccess(
			decodePullRequestListJson(asJson([pullRequest({ status })])),
		);
	...
	it.each([
		...
		const batch = expectSuccess(
			decodePullRequestListJson(asJson([pullRequest({ mergeStatus })])),
		);
	...
	it("skips a malformed row but still counts it, so paging does not stop early", () => {
		const batch = expectSuccess(
			decodePullRequestListJson(
				asJson([{ pullRequestId: "nope" }, pullRequest()]),
			),
		);
...
describe("decodePullRequestJson", () => {
	...
	it("reads auto-complete from whoever armed it, and its absence as nobody", () => {
		const armed = expectSuccess(
			decodePullRequestJson(
				asJson(
					pullRequest({ autoCompleteSetBy: { displayName: "Bilal Hassan" } }),
				),
			),
		);
		...
		expect(
			expectSuccess(decodePullRequestJson(asJson(pullRequest())))
				?.autoMergeEnabled,
		).toBe(false);
	});
	...
	it("returns nothing when Azure gave no way to place the pull request at all", () => {
		const detail = expectSuccess(
			decodePullRequestJson(
				asJson(pullRequest({ url: null, repository: null })),
			),
		);
...
describe("decodeViewerJson", () => {
	it("reads the signed-in account name", () => {
		expect(
			expectSuccess(
				decodeViewerJson(asJson({ user: { name: "bilal@acme.dev" } })),
			),
		).toBe("bilal@acme.dev");
	});
					...
					value: [
						{
							...
							comments: [
								...
								},
								{
									id: 2,
									content: "Renamed.",
									publishedDate: "2026-07-02T01:00:00Z",
								},
								{
									id: 3,
									content: "Thanks.",
									publishedDate: "2026-07-02T02:00:00Z",
								},
							],
						},
					],
