...
function pullRequest(
	...
	return {
		...
		source: {
			branch: { name: "feat/page" },
			repository: { full_name: "fork/web" },
		},
		destination: { branch: { name: "master" } },
		links: {
			html: { href: "https://bitbucket.org/acme/web/pull-requests/897" },
		},
		...overrides,
	};
}
...
describe("decodePullRequestPageJson", () => {
	it("reads a pull request as a change request", () => {
		const decoded = expectSuccess(
			decodePullRequestPageJson(page([pullRequest()])),
		);
	...
	it("reports the next page as the whole URL Bitbucket sends", () => {
		...
		const decoded = expectSuccess(
			decodePullRequestPageJson(page([pullRequest()], { next })),
		);
	...
	it.each([
		...
		const decoded = expectSuccess(
			decodePullRequestPageJson(page([pullRequest({ state })])),
		);
	...
	it("fails when Bitbucket did not answer with a page", () => {
		expect(
			Result.isFailure(
				decodePullRequestPageJson(JSON.stringify({ error: "nope" })),
			),
		).toBe(true);
	});
});
...
describe("decodePullRequestJson", () => {
	it("reads reviewers as review requests", () => {
		...
		expect(decoded.reviewers).toEqual([
			{ login: "julius", name: "Julius", avatarUrl: null },
		]);
	});
...
describe("decodeViewerJson", () => {
	it("reads the signed-in nickname", () => {
		const decoded = decodeViewerJson(
			JSON.stringify({ nickname: "bilal", display_name: "Bilal" }),
		);
	...
	it("falls back to the display name, which app accounts have instead", () => {
		const decoded = decodeViewerJson(
			JSON.stringify({ display_name: "Release Bot" }),
		);
				...
				page([
					{
						...
						links: {
							html: {
								href: "https://bitbucket.org/acme/web/pull-requests/892#c1",
							},
						},
					},
					{
...
describe("decodeStatusesJson", () => {
	...
	it.each([
		...
		const decoded = expectSuccess(
			decodeStatusesJson(page([{ name: "Pipeline", state }])),
		);
...
describe("decodeConflictsJson", () => {
	...
	it("calls any reported conflict conflicting", () => {
		expect(
			expectSuccess(decodeConflictsJson(page([{ path: "src/app.ts" }]))),
		).toBe("conflicting");
	});
});
...
describe("repository permission decoding", () => {
	...
	it("counts admin and write as write, and read as not", () => {
		expect(
			expectSuccess(decodeRepositoryPermissionJson(permissionPage("admin"))),
		).toBe(true);
		expect(
			expectSuccess(decodeRepositoryPermissionJson(permissionPage("write"))),
		).toBe(true);
		expect(
			expectSuccess(decodeRepositoryPermissionJson(permissionPage("read"))),
		).toBe(false);
	});
