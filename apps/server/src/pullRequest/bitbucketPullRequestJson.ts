		...
		Schema.NullOr(
			Schema.Struct({
				avatar: Schema.optional(
					Schema.NullOr(
						Schema.Struct({ href: Schema.optional(Schema.String) }),
					),
				),
			}),
		),
...
const RawBranchSchema = Schema.Struct({
	...
	repository: Schema.optional(
		Schema.NullOr(Schema.Struct({ full_name: TrimmedNonEmptyString })),
	),
});
...
const RawPullRequestSchema = Schema.Struct({
	...
	links: Schema.Struct({
		html: Schema.Struct({ href: TrimmedNonEmptyString }),
	}),
});
...
const RawCommentSchema = Schema.Struct({
	...
	content: Schema.optional(
		Schema.NullOr(Schema.Struct({ raw: Schema.optional(Schema.String) })),
	),
	user: Schema.optional(Schema.NullOr(RawUserSchema)),
	created_on: Schema.String,
	...
	links: Schema.optional(
		Schema.NullOr(
			Schema.Struct({ html: Schema.optional(Schema.NullOr(RawLinkSchema)) }),
		),
	),
});
...
const RawRepositoryPermissionsSchema = Schema.Struct({
	values: Schema.optional(
		Schema.NullOr(
			Schema.Array(
				Schema.Struct({
					permission: Schema.optional(Schema.NullOr(Schema.String)),
				}),
			),
		),
	),
...
function toReviews(
	...
	return (raw.participants ?? []).flatMap(
		(participant): ReadonlyArray<PullRequestComment> => {
			const author = toActor(participant.user);
			const votedAt = trimmed(participant.participated_on);
			const reviewState =
				trimmed(participant.state) ??
				(participant.approved === true ? "approved" : null);
...
function toPullRequest(
	...
	return {
		...
		reviewers,
		reviewerIds: (raw.reviewers ?? []).flatMap(
			(reviewer) => trimmed(reviewer.uuid) ?? [],
		),
...
const decodeRepositoryPermissions = decodeJsonResult(
	RawRepositoryPermissionsSchema,
);
...
export function decodeViewerJson(
	raw: string,
): Result.Result<string | null, DecodeFailure> {
	const decoded = decodeViewer(raw);
	return Result.isSuccess(decoded)
		? Result.succeed(
				trimmed(decoded.success.nickname) ??
					trimmed(decoded.success.display_name),
			)
		: Result.fail(decoded.failure);
}
...
export function decodeRepositoryPermissionJson(
	...
	const permission =
		trimmed(decoded.success.values?.[0]?.permission)?.toLowerCase() ?? null;
	return Result.succeed(
		permission === null || permission === "admin" || permission === "write",
	);
}
...
export function buildReviewThreads(
	...
	const replies = new Map<
		number,
		Array<Schema.Schema.Type<typeof RawCommentSchema>>
	>();
	for (const comment of comments) {
		...
		if (root.id === comment.id) {
			...
			const side =
				inline?.to === null || inline?.to === undefined ? "left" : "right";
...
export function decodeConflictsJson(
	...
	return Result.isSuccess(decoded)
		? Result.succeed(
				decoded.success.values.length === 0 ? "mergeable" : "conflicting",
			)
		: Result.fail(decoded.failure);
}
