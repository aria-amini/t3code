...
function expectSuccess<A>(result: Result.Result<A, unknown>): A {
	...
	if (!Result.isSuccess(result))
		throw new Error("expected a successful decode");
...
describe("pull request list decoding", () => {
	it("treats a merge timestamp as merged even when the state still says closed", () => {
		const [entry] = expectSuccess(
			decodePullRequestListJson(
				listJson([{ state: "CLOSED", mergedAt: "2026-07-03T00:00:00Z" }]),
			),
		).items;
	...
	it("keeps user review requests and drops team ones, which are not logins", () => {
		const [entry] = expectSuccess(
			decodePullRequestListJson(
				listJson([
					{ reviewRequests: [{ login: "octocat" }, { slug: "web-platform" }] },
				]),
			),
		).items;
...
describe("pull request detail decoding", () => {
	...
	it("keeps a workflow waiting for approval out of the passing state", () => {
		...
		expect(detail.checks.map((check) => check.status)).toEqual([
			"success",
			"action-required",
		]);
			...
			expectSuccess(
				decodeWorkflowRunApprovalsJson(
					JSON.stringify([
						{
							databaseId: 10,
							workflowName: "contributor tests",
	...
	it("reads an auto-merge request and strategy, its null as off and its absence as neither", () => {
		...
		const armed = (entry: Record<string, unknown>) =>
			expectSuccess(
				decodePullRequestDetailJson(JSON.stringify({ ...raw, ...entry })),
			);
		...
		const detail = expectSuccess(
			decodePullRequestActivityJson(
				JSON.stringify({
					...raw,
					reviews: [
						{ id: "r3", body: "  ", submittedAt: "2026-07-07T00:00:00Z" },
					],
				}),
			),
		);
...
describe("review thread decoding", () => {
	const threadsJson = (
		...
		JSON.stringify({
			data: {
				repository: {
					pullRequest: { reviewThreads: { totalCount, pageInfo, nodes } },
				},
			},
		});
			...
			data: {
				repository: {
					pullRequest: {
						...
						reviewRequests: {
							nodes: (input.requested ?? []).map((r) => ({
								requestedReviewer: r,
							})),
						},
						latestReviews: {
							nodes: (input.reviewed ?? []).map((a) => ({ author: a })),
						},
					},
				},
			},
	...
	it("skips a team request, which names nobody to show", () => {
		const result = expectSuccess(
			decodeReviewThreadsJson(reviewJson({ requested: [null] })),
		);
	...
	it("carries a resolved thread into the conversation, which was still said", () => {
			...
			decodeReviewThreadsJson(
				threadsJson([
					...
					},
					{
						...
						comments: {
							nodes: [
								{ id: "t2", body: "done", createdAt: "2026-07-01T00:00:00Z" },
							],
						},
					},
				]),
		...
		const comments = reviewThreadConversation(
			result.threads.map((entry) => entry.thread),
		);
					...
					data: {
						...
						node: {
							...
							comments: {
								...
								nodes: [
									{ id: "t9", body: "last", createdAt: "2026-07-01T00:00:00Z" },
								],
							},
						},
					},
...
describe("reaction decoding", () => {
	const commentWithGroups = (
		reactionGroups: ReadonlyArray<Record<string, unknown>>,
	) =>
		JSON.stringify({
			data: {
				repository: { pullRequest: { id: "PR_1" } },
				...
				commentWithGroups([
					{
						...
						reactors: {
							totalCount: 2,
							nodes: [{ login: "julius" }, { login: "bilal" }],
						},
					},
...
describe("repository access decoding", () => {
	...
	it("counts the roles that can push as write, and the ones that cannot as read", () => {
		for (const permission of ["ADMIN", "MAINTAIN", "WRITE"]) {
			expect(
				expectSuccess(decodeRepositoryAccessJson(repositoryJson(permission)))
					.canWrite,
			).toBe(true);
		}
	...
	it("withholds write where gh names no permission, which is not a standing it gave", () => {
		...
		expect(
			expectSuccess(decodeRepositoryAccessJson(repositoryJson())).canWrite,
		).toBe(false);
		expect(
			expectSuccess(decodeRepositoryAccessJson(repositoryJson(null))).canWrite,
		).toBe(false);
	});
});
...
describe("viewer permission decoding", () => {
	...
	it("reads silence as permission, but not as authorship", () => {
		...
		expect(
			expectSuccess(
				decodeViewerPermissionsJson(viewerJson({ pullRequest: null })),
			),
		).toEqual({
			canWrite: false,
			canTriage: false,
...
describe("label candidate decoding", () => {
		...
		JSON.stringify({
			data: {
				repository: {
					...
					pullRequest: {
						labels: { nodes: (input.applied ?? []).map((name) => ({ name })) },
					},
				},
			},
		});
	...
	it("keeps a worn label the repository no longer defines, so it can be taken off", () => {
		const list = expectSuccess(
			decodeLabelCandidatesJson(
				labelsJson({ defined: [{ name: "bug" }], applied: ["legacy"] }),
			),
		);
		expect(
			list.candidates.map((label) => [label.name, label.isApplied]),
		).toEqual([
			["legacy", true],
			["bug", false],
		]);
	...
	it("says so when the repository defines more labels than the read asked for", () => {
		expect(
			expectSuccess(
				decodeLabelCandidatesJson(
					labelsJson({ defined: [], hasNextPage: true }),
				),
			).truncated,
		).toBe(true);
	});
...
describe("review thread decoding", () => {
	...
	it("carries what the reader may do with the pull request, off the conversation read", () => {
		...
		expect(
			expectSuccess(decodeReviewThreadsJson(threadsJson([]))).viewer,
		).toEqual({
			canUpdate: true,
			didAuthor: false,
		});
				...
				threadsJson([
					{
						...
						comments: {
							totalCount: 2,
							nodes: [comment("c1", "first"), comment("c2", "second")],
						},
					},
				]),
			),
	...
	it("keeps a resolved thread in the conversation as well as against its line", () => {
		...
		expect(
			reviewThreadConversation(threads).map((comment) => comment.id),
		).toEqual(["c4"]);
...
describe("REVIEW_THREADS_GRAPHQL_QUERY", () => {
	...
	it("asks for reactionGroups on the pull request itself, its comments, its reviews and each thread's comments", () => {
		expect(REVIEW_THREADS_GRAPHQL_QUERY.match(/reactionGroups/g)).toHaveLength(
			4,
		);
			...
			data: {
				repository: {
					...
					pullRequest: {
						...
						reviewRequests: {
							nodes: (input.requested ?? []).map((requestedReviewer) => ({
								requestedReviewer,
							})),
						},
					},
...
describe("reviewer request payload", () => {
	...
	it("sends both lists even where one of them is empty, which is what GitHub reads", () => {
		expect(
			JSON.parse(buildReviewerRequestJson([{ id: "octocat", kind: "user" }])),
		).toEqual({
			reviewers: ["octocat"],
			team_reviewers: [],
		});
...
describe("how far a branch trails its base", () => {
	...
	it("answers unknown where the head could not be compared", () => {
		...
		expect(
			expectSuccess(
				decodeBaseComparisonJson(
					comparison({ viewerCanUpdateBranch: true, baseRef: null }),
				),
			).behindBy,
		).toBeNull();
