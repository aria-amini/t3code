...
describe("decodeMergeRequestListJson", () => {
	...
	it("keeps a locked merge request open", () => {
		const batch = expectSuccess(
			decodeMergeRequestListJson(listJson([{ state: "locked" }])),
		);
	...
	it("reads the legacy draft flag", () => {
		const batch = expectSuccess(
			decodeMergeRequestListJson(listJson([{ work_in_progress: true }])),
		);
	...
	it("calls a conflicted merge request conflicting even while the merge check is pending", () => {
		const batch = expectSuccess(
			decodeMergeRequestListJson(
				listJson([{ merge_status: "checking", has_conflicts: true }]),
			),
		);
...
describe("decodeMergeRequestDetailJson", () => {
	...
	it("reads either auto-merge field, and says nothing where GitLab named neither", () => {
		const armed = (entry: Record<string, unknown>) =>
			expectSuccess(decodeMergeRequestDetailJson(detailJson(entry)))
				.autoMergeEnabled;
	...
	it("maps a pipeline waiting on a person to neutral, not failure", () => {
		const detail = expectSuccess(
			decodeMergeRequestDetailJson(
				detailJson({ head_pipeline: { status: "manual" } }),
			),
		);
...
describe("decodeViewerJson", () => {
	it("reads the signed-in username", () => {
		expect(
			expectSuccess(decodeViewerJson(JSON.stringify({ username: "bilal" }))),
		).toBe("bilal");
	});

	it("returns nothing when the account has no username", () => {
		expect(
			expectSuccess(decodeViewerJson(JSON.stringify({ username: "  " }))),
		).toBeNull();
	});
});
				...
				JSON.stringify([
					{
						id: "bbb",
						title: "second",
						committed_date: "2026-07-02T00:00:00Z",
					},
...
describe("decodeMergeRequestDiffsJson", () => {
	...
	it("fails when GitLab did not return a list", () => {
		expect(
			Result.isFailure(decodeMergeRequestDiffsJson('{"message":"404"}')),
		).toBe(true);
	});
});
...
describe("merge request viewer fields", () => {
	it("carries GitLab's own answer for whether this viewer can merge", () => {
		expect(
			expectSuccess(
				decodeMergeRequestDetailJson(
					detailJson({ user: { can_merge: false } }),
				),
			).viewerCanMerge,
		).toBe(false);
		expect(
			expectSuccess(
				decodeMergeRequestDetailJson(detailJson({ user: { can_merge: true } })),
			).viewerCanMerge,
		).toBe(true);
	});
	...
	it("leaves merging permitted where GitLab answered without the field", () => {
		...
		expect(
			expectSuccess(decodeMergeRequestDetailJson(detailJson({})))
				.viewerCanMerge,
		).toBe(true);
		expect(
			expectSuccess(decodeMergeRequestDetailJson(detailJson({ user: null })))
				.viewerCanMerge,
		).toBe(true);
	});
});
								...
								notes: {
									...
									nodes: [
										...
										},
										{
											...
											awardEmoji: {
												nodes: [
													{ name: "partyparrot", user: { username: "bilal" } },
												],
											},
										},
									],
...
describe("decodeOwnAwardIdJson", () => {
	...
	it("finds the reader's own award of that name, which is the one a removal deletes", () => {
		expect(
			expectSuccess(
				decodeOwnAwardIdJson(awards, { content: "thumbs-up", viewer: "bilal" }),
			),
		).toBe(102);
	});
	...
	it("returns nothing where the reader has no award of that name", () => {
		expect(
			expectSuccess(
				decodeOwnAwardIdJson(awards, { content: "heart", viewer: "bilal" }),
			),
		).toBeNull();
	});
});
