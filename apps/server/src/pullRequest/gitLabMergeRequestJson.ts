...
const RawMergeRequestSchema = Schema.Struct({
	...
	user: Schema.optional(
		Schema.NullOr(
			Schema.Struct({ can_merge: Schema.optional(Schema.Boolean) }),
		),
	),
	/**
...
function toDetail(
	...
	const autoMerge =
		raw.merge_when_pipeline_succeeds == null && raw.auto_merge_enabled == null
			? undefined
			: raw.merge_when_pipeline_succeeds === true ||
				raw.auto_merge_enabled === true;
	return {
		...listItem,
...
const decodeProjectMergeSettings = decodeJsonResult(
	RawProjectMergeSettingsSchema,
);
...
export function decodeDiscussionsJson(
	...
	for (const entry of decoded.success) {
		...
		const notes = (discussion.value.notes ?? []).filter(
			(note) => note.system !== true,
		);
		const root = notes[0];
		const position = root?.position;
		if (root === undefined || !position || position.position_type !== "text")
			continue;
		...
		const side =
			position.new_line === null || position.new_line === undefined
				...
				: "right";
		const path = trimmed(
			side === "left" ? position.old_path : position.new_path,
		);
...
export function decodeCommitsJson(
	...
	for (const entry of decoded.success) {
		...
		const committedDate =
			trimmed(commit.value.committed_date) ?? trimmed(commit.value.created_at);
		...
		commits.push({
			...
			authors: (() => {
				const login =
					trimmed(commit.value.author_name) ??
					trimmed(commit.value.author_email);
				return login === null
					? []
					: [
							{
								login,
								name: trimmed(commit.value.author_name),
								avatarUrl: null,
							},
						];
			})(),
...
export function decodeMergeRequestDiffsJson(
	...
	for (const entry of decoded.success) {
		...
		if (hunks.length === 0) {
			...
			truncated =
				truncated || value.too_large === true || value.collapsed === true;
		}
		const { from, to } = diffHeaderPaths(value);
		const header = [
			`diff --git a/${value.old_path} b/${value.new_path}`,
			...(value.new_file === true
				? [`new file mode ${value.b_mode ?? "100644"}`]
				: []),
			...(value.deleted_file === true
				? [`deleted file mode ${value.a_mode ?? "100644"}`]
				: []),
...
const GITLAB_AWARD_BY_CONTENT: Readonly<
...
const CONTENT_BY_GITLAB_AWARD: Readonly<
	Record<string, PullRequestReactionContent>
> = Object.fromEntries(
	Object.entries(GITLAB_AWARD_BY_CONTENT).map(([content, name]) => [
		name,
		content,
	]),
...
function toReactions(
	...
	for (const node of nodes?.nodes ?? []) {
		...
		const content =
			CONTENT_BY_GITLAB_AWARD[trimmed(node?.name)?.toLowerCase() ?? ""];
		...
		if (
			...
			username.toLowerCase() === normalizedViewer
		) {
			group.viewer = true;
		} else {
...
export interface GitLabAwardEmojiPage {
	...
	readonly reactionsByNoteId: ReadonlyMap<
		string,
		ReadonlyArray<PullRequestReaction>
	>;
...
export function decodeAwardEmojiJson(
	...
	const reactionsByNoteId = new Map<
		string,
		ReadonlyArray<PullRequestReaction>
	>();
	...
	return Result.succeed({
		...
		nextCursor:
			pageInfo?.hasNextPage === true
				? (trimmed(pageInfo.endCursor) ?? null)
				: null,
	});
}
