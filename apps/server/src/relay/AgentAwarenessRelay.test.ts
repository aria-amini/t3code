...
function makeMemorySecretStore() {
		...
		get: ((name) =>
			Effect.sync(() => {
				...
				return value === undefined
					? Option.none()
					: Option.some(Uint8Array.from(value));
	...
	return {
		...
		setString: (name: string, value: string) =>
			store.set(name, encodeSecret(value)),
	};
}
...
describe.sequential("signRelayAgentActivityPublishProof", () => {
	...
	it.effect("signs the activity publish JWT and rejects tampering", () =>
		Effect.gen(function* () {
			...
			const proof =
				yield* AgentAwarenessRelay.signRelayAgentActivityPublishProof({
			...
			const rejection = yield* Effect.flip(
				verify(`${header}.${body}.${corruptedSignature}`),
			);
		...
		Effect.scoped(
			Effect.gen(function* () {
				...
				yield* Effect.gen(function* () {
					...
					yield* Deferred.await(threadShellRequested).pipe(
	...
	it.effect(
		"publishes agent activity to the relay transport URL, not the relay issuer",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const originalFetch = globalThis.fetch;
					...
					yield* Effect.gen(function* () {
						...
						const url = yield* Effect.promise(() => fetchSeen).pipe(
							Effect.timeout("2 seconds"),
						);
