...
export function agentAwarenessPublishIdentity(
...
export function sanitizeRelayAgentActivityState(
	...
	const detail = (
		state.phase === "failed"
			? REDACTED_RELAY_AGENT_FAILURE_DETAIL
			: state.detail
	)
		?.trim()
...
function deliveryStats(
	...
	for (const delivery of deliveries) {
		...
		failed += 1;
		failedReasons.push(
			`${delivery.apnsStatus ?? "transport"}:${delivery.apnsReason ?? "unknown"}`,
		);
	}
...
const makePublishProof = Effect.fn("makePublishProof")(function* (input: {
	...
	const payload = {
		...
		environmentId:
			input.environmentId as RelayAgentActivityPublishProofPayload["environmentId"],
...
export function resolveAgentAwarenessRelayActiveThreadIds(input: {
	...
	readonly projects: ReadonlyArray<
		Pick<OrchestrationProjectShell, "id" | "title">
	>;
...
export const make = Effect.gen(function* () {
	...
	const cloudLinkKeyPair =
		yield* getOrCreateEnvironmentKeyPairFromSecretStore(secrets);
	const activeSnapshotPublishedRef = yield* Ref.make(false);
	const publishedStateByThreadRef = yield* Ref.make(
		new Map<ThreadId, string>(),
	);
	...
	const readPublishAgentActivityEnabled = readSecretString(
		PUBLISH_AGENT_ACTIVITY_SECRET,
	).pipe(Effect.map(isAgentActivityPublishingEnabledValue));
	...
	const makeRelayClient = (relayConfig: {
		...
		HttpApiClient.make(RelayApi, {
			...
			transformClient: relayEnvironmentClient(
	...
	const publishThreadUnsafe = Effect.fn("publishThreadUnsafe")(function* (
		...
		if (!publishAgentActivity) {
			yield* Effect.logDebug(
				"agent activity publish skipped; publication disabled",
				{
					threadId,
				},
			);
		...
		const relayConfig = yield* readRelayConfig.pipe(
		...
		if (publishedStateByThread.get(threadId) === publishIdentity) {
			...
			yield* Effect.logDebug(
				"agent activity publish skipped; projected state unchanged",
				{
					environmentId,
					threadId,
		...
		const requiresConfirmation =
			(snapshot.state === null &&
				publishedStateByThread.get(threadId) !==
					agentAwarenessPublishIdentity(null)) ||
		...
		if (requiresConfirmation) {
			...
			if (deadline === undefined) {
				...
				yield* Effect.logInfo(
					"agent activity publish deferred pending confirmation",
					{
						environmentId,
						threadId,
						reason: snapshot.reason,
		...
		if (snapshot.reason === "thread-not-found") {
			yield* Effect.logDebug(
				"publishing agent activity tombstone; thread not found",
				{
					environmentId,
					threadId,
				},
			...
			yield* Effect.logDebug(
				"publishing agent activity tombstone; project not found",
				{
					environmentId,
					threadId,
	...
	const publishThread: AgentAwarenessRelay["Service"]["publishThread"] = (
		threadId,
	) =>
	...
	const publishActiveThreadsUnsafe = Effect.gen(function* () {
		...
		if (!publishAgentActivity) {
			yield* Effect.logDebug(
				"agent activity snapshot skipped; publication disabled",
			);
			return false;
		}
		const relayConfig = yield* readRelayConfig.pipe(
		...
		if (activeThreadIds.length === 0) {
			yield* Effect.logDebug(
				"agent activity snapshot has no publishable threads",
			);
			return true;
		}
	...
	const publishActiveThreadsOnceWhenConfigured = (
		logEnabledWhenReady: boolean,
	) =>
		Effect.gen(function* () {
			while (!(yield* Ref.get(activeSnapshotPublishedRef))) {
				...
				if (published) {
					...
					if (logEnabledWhenReady) {
						...
						yield* Effect.logInfo(
							"agent activity publishing enabled after link reconciliation",
							{
	...
	const start: AgentAwarenessRelay["Service"]["start"] = Effect.fn(
		"AgentAwarenessRelay.start",
	)(function* () {
		...
		switch (startupState) {
			...
			case "disabled":
				yield* Effect.logInfo(
					"agent activity publishing disabled by T3 Connect configuration",
				);
				break;
			case "enabled":
		...
		yield* forkParked(
			Effect.sleep("1 second").pipe(
				Effect.andThen(
					publishActiveThreadsOnceWhenConfigured(startupState !== "enabled"),
				),
			),
		);
		yield* forkParked(
			Stream.runForEach(orchestrationEngine.streamDomainEvents, (event) => {
				...
				if (threadId === null) {
					return Effect.logDebug(
						"agent activity publishing ignored event without thread id",
						{
				...
				return Effect.logDebug(
					"agent activity publishing queued thread publish",
					{
						eventType: event.type,
						threadId,
					},
