...
const isDescriptorUnavailable = Schema.is(
	DesktopTelemetryDescriptorUnavailable,
);
...
export const writeAllToFileDescriptor = Effect.fn(
	...
	while (offset < payload.byteLength) {
		const written = yield* Effect.callback<
			number,
			DesktopTelemetryControlFailed
		>((resume, signal) => {
		...
		yield* requireDesktopTelemetryWriteProgress(
			fd,
			payload.byteLength - offset,
			written,
		);
...
export const make = Effect.fn(
	"resourceTelemetry.desktopTelemetryReceiver.make",
)(function* () {
	...
	const health = yield* Ref.make<DesktopTelemetryReceiverHealth>({
		status:
			config.desktopTelemetryFd === undefined ? "unavailable" : "starting",
			...
			Stream.mapEffect(
				...
				> => {
					...
					return decodeMessage(value).pipe(
						Effect.mapError(
							(cause) => new DesktopTelemetryDecodeFailed({ cause }),
						),
					);
				},
			),
			...
			Effect.andThen(
				updateHealth((current): DesktopTelemetryReceiverHealth => ({
					...
					lastError: Option.some(
						new DesktopTelemetryStreamClosed({ fd }).message,
					),
				})),
			),
			Effect.catch((error) =>
						...
						Effect.gen(function* () {
							...
							const staleSnapshot = yield* Ref.modify(latest, (current) => {
								if (
									...
									return [
										Option.none<DesktopHostTelemetrySnapshot>(),
										current,
									] as const;
								}
							...
							if (Option.isNone(staleSnapshot)) {
								...
								const changed = yield* Ref.modify(health, (current) => {
									if (
										...
										return [
											Option.none<DesktopTelemetryReceiverHealth>(),
											current,
										] as const;
									}
									const next: DesktopTelemetryReceiverHealth = {
							...
							yield* updateHealth((currentHealth) => ({
								...currentHealth,
								status:
									currentHealth.status === "stopped" ? "stopped" : "degraded",
								lastError:
									currentHealth.status === "stopped"
										? currentHealth.lastError
										: Option.some(
												new DesktopTelemetryStale({ fd, staleAfterMs }).message,
											),
							}));
...
export const layerTest = (
	...
	const health =
		...
		Effect.succeed({
			...
			lastError: Option.some(
				"Desktop telemetry test implementation is unavailable.",
			),
		});
