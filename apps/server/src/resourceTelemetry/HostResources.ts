...
function darwinAvailableMemory(output: string): number | null {
	...
	const available =
		(Number(free) + Number(inactive) + Number(speculative)) * Number(pageSize);
	return Number.isSafeInteger(available) && Number(pageSize) > 0
		? available
		: null;
}
	...
	const sample = Effect.fn("HostResources.sample")(function* () {
		...
		if (platform === "linux") {
			...
			const output = yield* spawner
				.string(
					ChildProcess.make("/usr/bin/vm_stat", [], {
						stdin: "ignore",
						stderr: "ignore",
					}),
				)
			...
			availableMemoryBytes =
				darwinAvailableMemory(output) ?? availableMemoryBytes;
		}
		return {
