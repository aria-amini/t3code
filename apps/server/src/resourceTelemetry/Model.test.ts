...
const BASE_TIME_MS = DateTime.toEpochMillis(
...
describe("resource telemetry process model", () => {
	it("builds complete descendant depths and isolates monitor overhead", () => {
		...
		expect(
			result.processes.map((process) => [process.identity.pid, process.depth]),
		).toEqual([
			[100, 0],
			[200, 1],
			[201, 2],
		...
		expect(
			result.processes.find((process) => process.identity.pid === 900)
				?.category,
		).toBe("resource-monitor");
	...
	it("deduplicates Electron metrics and classifies Electron descendants", () => {
		...
		expect(
			result.processes.filter((process) => process.identity.pid === 300),
		).toHaveLength(1);
		expect(
			result.processes.find((process) => process.identity.pid === 300)
				?.category,
		).toBe("electron-main");
		expect(
			result.processes.find((process) => process.identity.pid === 301)
				?.category,
		).toBe("electron-utility");
		expect(
			result.processes.find((process) => process.identity.pid === 301)?.depth,
		).toBe(1);
	...
	it("derives cumulative CPU time for synthetic Electron-only processes", () => {
		...
		expect(
			second.processes.find((process) => process.identity.pid === 300)
				?.cpuTimeMs,
		).toBe(500);
	...
	it("does not apply an explicit Electron root to a reused PID", () => {
		...
		expect(
			reused.processes.find((process) => process.identity.pid === 300)
				?.category,
		).toBe("server-child");
	...
	it("treats reused PIDs as an exit plus a new process", () => {
		...
		const reused = second.processes.find(
			(process) => process.identity.pid === 200,
		)!;
