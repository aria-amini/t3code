...
function matchElectronMetric(
	...
	return Math.abs(metric.creationTimeMs - process.startTimeMs) <=
...
function isElectronDescendant(
	...
	while (!visited.has(currentPid)) {
		...
		if (!current || current.ppid <= 0 || current.ppid === currentPid)
			return false;
...
function orderProcessTree(
	...
	const processesByPid = new Map(
		processes.map((process) => [process.identity.pid, process]),
	);
	...
	for (const process of processes.toSorted(
		(left, right) =>
			left.depth - right.depth || left.identity.pid - right.identity.pid,
	)) {
		visit(process);
	}
	return ordered;
...
function aggregate(
	...
	return {
		...
		peakRssBytes: processes.reduce(
			(total, process) => total + process.peakResidentBytes,
			0,
		),
...
export function mergeProcesses(
	...
	const sampledAtMs = Option.match(input.nativeSnapshot, {
		...
		onSome: (native) =>
			Option.match(input.desktopSnapshot, {
				...
				onSome: (desktop) =>
					Math.max(native.sampledAtUnixMs, desktop.sampledAtUnixMs),
			}),
	});
	...
	const nativeProcessPids = new Set(
		nativeProcesses.map((process) => process.pid),
	);
	const nativeByPid = new Map(
		nativeProcesses.map((process) => [process.pid, process]),
	);
	...
	for (const metric of electronMetrics) {
		...
		if (!nativeProcess) {
			nativeByPid.set(
				...
				syntheticNativeSample(
					...
					input.previous.get(
						processIdentityKey(metric.pid, metric.creationTimeMs),
					),
				),
			);
		...
		if (
			Math.abs(metric.creationTimeMs - nativeProcess.startTimeMs) <=
			ELECTRON_IDENTITY_TOLERANCE_MS
		) {
			metricsByPid.set(metric.pid, metric);
		}
	...
	const explicitElectronRootPids = new Set(
		[...requestedElectronRootPids].filter((pid) => {
			...
			if (expectedStartTime !== undefined) {
				return (
					Math.abs(process.startTimeMs - expectedStartTime) <=
					ELECTRON_IDENTITY_TOLERANCE_MS
				);
			}
					...
					: explicitElectronRootPids.has(process.pid)
						...
						: electronMetric
							...
							: isElectronDescendant(process.pid, processesByPid, electronPids)
								...
								: "server-child";
		const firstSeenAt =
			previous?.process.firstSeenAt ?? DateTime.makeUnsafe(sampledAtMs);
		const preservePreviousRates =
			!input.updatePrevious && previous !== undefined;
