...
describe("resolveNativeSampleIntervalMs", () => {
	it("keeps a recovery cadence while suspended and backs off under host constraints", () => {
		expect(
			resolveNativeSampleIntervalMs({ ...basePower, suspended: true }, 1),
		).toBe(15_000);
		expect(
			resolveNativeSampleIntervalMs({ ...basePower, locked: "true" }, 1),
		).toBe(15_000);
		expect(
			resolveNativeSampleIntervalMs({ ...basePower, lowPowerMode: "true" }, 1),
		).toBe(15_000);
		expect(
			resolveNativeSampleIntervalMs(
				{ ...basePower, thermalState: "critical" },
				1,
			),
		).toBe(15_000);
		expect(
			resolveNativeSampleIntervalMs({ ...basePower, onBattery: "true" }, 1),
		).toBe(5_000);
	});
...
describe("retainRecentNativeTelemetryFailures", () => {
	it("expires old failures so an isolated crash restarts from the initial backoff", () => {
		...
		expect(
			retainRecentNativeTelemetryFailures([30_000, 60_000], 90_000),
...
describe("commitCollectionControlUpdate", () => {
	...
	it.effect(
		"serializes startup synchronization with runtime control updates",
		() =>
			Effect.gen(function* () {
				const initial = {
					hostPower: basePower,
