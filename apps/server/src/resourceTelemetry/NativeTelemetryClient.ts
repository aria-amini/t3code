...
export class NativeTelemetryClient extends Context.Service<
	NativeTelemetryClient,
	{
		...
		>;
		readonly snapshots: Stream.Stream<
...
export function commitCollectionControlUpdate<E, R>(
	...
	apply: (
		previous: CollectionControl,
		next: CollectionControl,
	) => Effect.Effect<void, E, R>,
): Effect.Effect<readonly [CollectionControl, CollectionControl], E, R> {
	return Effect.gen(function* () {
		const [previousDesired, next] = yield* Ref.modify(
			desiredState,
			(previous) => {
...
const decodeMonitorEvent: (
	value: unknown,
) => Effect.Effect<ResourceMonitorEvent, Schema.SchemaError> =
	Schema.decodeUnknownEffect(ResourceMonitorEventSchema);
...
export const make = Effect.fn("resourceTelemetry.nativeTelemetryClient.make")(
	function* () {
		...
		const appliedCollectionControl = yield* Ref.make(
			yield* Ref.get(collectionControl),
		);
		const externalProcesses = yield* Ref.make<
			ReadonlyArray<ResourceMonitorExternalProcess>
		>([]);
		const pendingSamples = yield* Ref.make(
			new Map<
				string,
				Deferred.Deferred<NativeTelemetrySnapshot, NativeTelemetryClientError>
			>(),
		);
		...
		const pendingHistories = yield* Ref.make(
			new Map<string, PendingHistoryRequest>(),
		);
		...
		const currentHealth = Effect.all([
			Ref.get(state),
			Ref.get(collectionControl),
		]).pipe(
			Effect.map(([current, control]) =>
				toHealth(current, control.sampleIntervalMs),
			),
		);
		...
		const failPending = (error: NativeTelemetryClientError) =>
			Effect.gen(function* () {
				...
				const processTables = yield* Ref.getAndSet(
					pendingProcessTables,
					new Map(),
				);
				...
				case "snapshot":
					return Effect.gen(function* () {
						const nativeSnapshot = {
							generation,
							snapshot: event,
						} satisfies NativeTelemetrySnapshot;
					...
					return Effect.gen(function* () {
						...
						const healthChanged = yield* Ref.modify(state, (current) => [
							current.status !== "healthy" || Option.isSome(current.lastError),
							{
								...
								lastSampleAt: latestSnapshot
									? Option.some(
											DateTime.makeUnsafe(latestSnapshot.sampledAtUnixMs),
										)
						...
						const completed = yield* Ref.modify(pendingHistories, (pending) => {
							...
							if (event.done) {
								next.delete(event.requestId);
								return [
									Option.some({ deferred: request.deferred, snapshots }),
									next,
								] as const;
							}
						...
						if (Option.isSome(completed)) {
							yield* Deferred.succeed(
								completed.value.deferred,
								completed.value.snapshots,
							);
						}
					});
				case "error":
		...
		const runAttempt: Effect.Effect<void, NativeTelemetryClientError> =
			Effect.scoped(
				Effect.gen(function* () {
					...
					const handle = yield* Effect.acquireRelease(
						spawner
							...
							.pipe(
								Effect.mapError(
									(cause) =>
										new NativeTelemetrySpawnFailed({
											path: executablePath,
											cause,
										}),
								),
					...
					const helloDeferred =
						yield* Deferred.make<ResourceMonitorHelloEvent>();
					const eventFiber = yield* handle.stdout.pipe(
						Stream.pipeThroughChannel(
							Ndjson.decode({ ignoreEmptyLines: true }),
						),
						Stream.mapEffect(
							(
								value,
							...
							> => {
								...
								return decodeMonitorEvent(value).pipe(
									Effect.mapError(
										(cause) => new NativeTelemetryDecodeFailed({ cause }),
									),
								);
							},
						),
						Stream.runForEach((event) =>
							processEvent(event, helloDeferred, generation),
						),
						Effect.mapError((cause) =>
							isProtocolMismatch(cause) ||
							isDecodeFailed(cause) ||
							isCommandFailed(cause)
								? cause
					...
					const exitEffect = handle.exitCode.pipe(
						...
						Effect.flatMap((exitCode) =>
							Effect.fail(
								new NativeTelemetryExited({ exitCode: Number(exitCode) }),
							),
						),
					);
		...
		yield* Effect.gen(function* () {
			...
			while (true) {
				...
				const recentFailures = retainRecentNativeTelemetryFailures(
					failures,
					now,
				);
				...
				Cause.hasInterruptsOnly(cause)
					...
					: Ref.update(state, (current) => ({
							...
							lastError: Option.some(
								"Resource monitor supervisor stopped unexpectedly.",
							),
						})).pipe(
							Effect.andThen(publishHealth),
							Effect.andThen(
		...
		const setHostPowerState: NativeTelemetryClient["Service"]["setHostPowerState"] =
			(hostPower) =>
		...
		const changeLiveSubscriberCount = Effect.fn(
			...
			yield* updateCollectionControl((current) => {
				const liveSubscriberCount = Math.max(
					0,
					current.liveSubscriberCount + delta,
				);
				return {
					...current,
					liveSubscriberCount,
		...
		const readHistory: NativeTelemetryClient["Service"]["readHistory"] = (
			windowMs,
		) =>
			Effect.gen(function* () {
				const current = yield* Ref.get(state);
		...
		const sampleNow: NativeTelemetryClient["Service"]["sampleNow"] = Effect.gen(
			function* () {
				const current = yield* Ref.get(state);
				if (
					!canCommandNativeTelemetrySidecar(
		...
		const processTable: NativeTelemetryClient["Service"]["processTable"] =
			Effect.gen(function* () {
				const current = yield* Ref.get(state);
				if (
					!canCommandNativeTelemetrySidecar(
		...
		return NativeTelemetryClient.of({
			...
			health,
			subscribeHealth: subscribeBeforeSnapshotWithoutMutex(
				healthChanges,
				health,
			),
		});
...
export const layerTest = (
	...
	const health =
		...
		Effect.succeed({
			...
			lastError: Option.some(
				"Resource monitor test implementation is unavailable.",
			),
			restartCount: 0,
			sampleIntervalMs: UNKNOWN_BACKGROUND_SAMPLE_INTERVAL_MS,
		});
	return Layer.succeed(
		...
		NativeTelemetryClient.of({
			...
			processTable: Effect.fail(
				new NativeTelemetryUnavailable({
					reason:
						"No resource monitor process table was configured for this test.",
				}),
			),
