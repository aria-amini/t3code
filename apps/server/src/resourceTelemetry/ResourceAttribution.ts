...
function key(
	input: Pick<ResourceAttributionRecord, "component" | "operation">,
): string {
...
export const make = Effect.fn("resourceTelemetry.resourceAttribution.make")(
	function* () {
		const entries = yield* Ref.make(
			new Map<string, ResourceAttributionEntry>(),
		);
			...
			Ref.update(entries, (current) => {
				...
				next.set(entryKey, {
					...
					durationMs:
						...
						nonNegativeInteger(input.durationMs, 0),
				});
				return next;
			});
