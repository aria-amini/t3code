...
describe("ResourceMonitorBinary", () => {
		...
		Effect.gen(function* () {
			...
			const service = yield* ResourceMonitorBinary.make().pipe(
				...
				Effect.provideService(
					ResourceMonitorBinary.ResourceMonitorHostLinuxLibc,
					"musl",
				),
				Effect.provideService(HostProcessEnvironment, {
					T3CODE_RESOURCE_MONITOR_PATH: binaryPath,
	...
	it.effect("rejects a non-executable POSIX override", () =>
		Effect.gen(function* () {
			...
			assert.instanceOf(
				error,
				ResourceMonitorBinary.ResourceMonitorBinaryNotExecutable,
			);
	...
	it.effect("rejects unsupported platform and architecture pairs", () =>
		Effect.gen(function* () {
			...
			assert.instanceOf(
				error,
				ResourceMonitorBinary.ResourceMonitorBinaryUnsupported,
			);
		}).pipe(Effect.scoped, Effect.provide(NodeServices.layer)),
	);
