...
describe("ResourceTelemetry", () => {
	it.effect(
		"enables live native and Electron collection only while changes are retained",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"releases live demand when acquisition is interrupted by an idle collector",
		() =>
			Effect.gen(function* () {
				const sampledAtUnixMs = DateTime.toEpochMillis(yield* DateTime.now);
				const demandChanges = yield* Ref.make<ReadonlyArray<boolean>>([]);
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				expect(
					snapshot.processes.find((entry) => entry.identity.pid === 5_000)
						?.category,
				).toBe("electron-main");
	...
	it.effect(
		"rejects buffered snapshots from an earlier sidecar generation",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					...
					yield* Effect.gen(function* () {
						...
						const nextSnapshot = yield* Stream.runHead(
							subscription.changes,
						).pipe(Effect.forkChild);

						yield* PubSub.publish(nativeSnapshots, nativeGeneration(stale, 0));
						yield* Effect.yieldNow;
						expect((yield* telemetry.latest).health.scannedProcessCount).toBe(
							0,
						);
						...
						expect(
							DateTime.toEpochMillis(Option.getOrThrow(received).readAt),
						).toBe(current.sampledAtUnixMs);
					}).pipe(Effect.provide(telemetryLayer));
				}),
			),
	);
	...
	it.effect(
		"retains desktop health changes while aggregate telemetry initializes",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const health =
						yield* Ref.make<DesktopTelemetryReceiver.DesktopTelemetryReceiverHealth>(
							{
								status: "starting",
								lastSampleAt: Option.none(),
								lastError: Option.none(),
					...
					const healthChanges =
						yield* PubSub.sliding<DesktopTelemetryReceiver.DesktopTelemetryReceiverHealth>(
							4,
						);
					...
					const resultFiber = yield* Effect.gen(function* () {
						...
						while (true) {
							...
							if (snapshot.health.desktop.status === "degraded")
								return snapshot;
							yield* Effect.yieldNow;
						}
					}).pipe(
	...
	it.effect(
		"atomically subscribes while a desktop update is being rebuilt",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const startedAt = DateTime.toEpochMillis(yield* DateTime.now);
					...
					const desktopChanges =
						yield* PubSub.unbounded<DesktopHostTelemetrySnapshot>();
					...
					const telemetryLayer = ResourceTelemetry.layer.pipe(
						Layer.provide(
							Layer.mergeAll(nativeLayer, desktopLayer, attributionLayer),
						),
					);
					...
					yield* Effect.gen(function* () {
						...
						const subscriptionFiber = yield* telemetry.subscribe.pipe(
							Effect.forkChild,
						);
						yield* Effect.yieldNow;
						...
						expect(subscription.latest.power).toEqual(
							nextDesktopSnapshot.power,
						);
					}).pipe(Effect.provide(telemetryLayer));
				}),
			),
	...
	it.effect(
		"combines native, Electron, attribution, retry, and history data",
		() =>
			Effect.gen(function* () {
				const startedAt = DateTime.toEpochMillis(yield* DateTime.now);
				const samples = [
				...
				const nativeHealth =
					yield* Ref.make<NativeTelemetryClient.NativeTelemetryClientHealth>({
						status: "healthy",
						hello: Option.some({
							version: 3,
							type: "hello",
				...
				const nativeHealthChanges =
					yield* PubSub.sliding<NativeTelemetryClient.NativeTelemetryClientHealth>(
						4,
					);
				const nativeLayer = NativeTelemetryClient.layerTest({
					setExternalProcesses: (processes) =>
						Ref.set(externalProcesses, processes),
					...
					retry: Ref.updateAndGet(retryCount, (count) => count + 1).pipe(
						Effect.as(true),
					),
				...
				yield* Effect.gen(function* () {
					...
					expect(yield* Ref.get(externalProcesses)).toEqual([
						{ pid: 5_000, startTimeMs: 300 },
					]);
					...
					const codex = second.processes.find(
					...
					expect(
						history.topProcesses.find((entry) => entry.identity.pid === 4_242)
							?.sampleCount,
					).toBe(2);
					expect(
						history.topProcesses.find((entry) => entry.identity.pid === 4_242)
					...
					expect(
						history.buckets.reduce(
							(total, bucket) => total + bucket.ioWriteBytes,
							0,
						),
					).toBe(4_000);
					...
					expect(DateTime.toEpochMillis(restarted.readAt)).toBe(
						startedAt + 2_000,
					);
