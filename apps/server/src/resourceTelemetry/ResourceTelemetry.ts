...
function buildHealth(input: {
	...
	return {
		...
		sidecarVersion: Option.map(
			input.native.hello,
			(hello) => hello.sidecarVersion,
		),
...
export const make = Effect.fn("resourceTelemetry.resourceTelemetry.make")(
	function* () {
		const nativeClient = yield* NativeTelemetryClient.NativeTelemetryClient;
		const desktopReceiver =
			yield* DesktopTelemetryReceiver.DesktopTelemetryReceiver;
		...
		if (Option.isSome(initialDesktop)) {
			...
			yield* nativeClient
				...
				.pipe(Effect.ignore);
			yield* nativeClient
				.setHostPowerState(initialDesktop.value.power)
				.pipe(Effect.ignore);
		}
		...
		const initialMerge = mergeProcesses({
			...
			sidecarPid: Option.map(
				initialNativeHealth.hello,
				(hello) => hello.sidecarPid,
			),
		...
		const initialSnapshot: ResourceTelemetrySnapshot = {
			...
			speedLimitPercent: Option.flatMap(
				initialDesktop,
				(desktop) => desktop.speedLimitPercent,
			),
				...
				Effect.gen(function* () {
					...
					if (
						...
						(incomingNativeSnapshot.generation < nativeHealth.restartCount ||
							incomingNativeSnapshot.generation <
								current.lastNativeGeneration ||
							(incomingNativeSnapshot.generation ===
								current.lastNativeGeneration &&
								incomingNativeSnapshot.snapshot.sequence <=
									current.lastNativeSequence))
					) {
					...
					const electronRootPids = new Set(
						recordedElectronRoots.map((process) => process.pid),
					);
					...
					const electronRootStartTimes = new Map(
						recordedElectronRoots.flatMap((process) =>
							process.startTimeMs === undefined
								? []
								: [[process.pid, process.startTimeMs] as const],
						),
					);
					...
					yield* Ref.set(state, {
						...
						lastNativeSequence:
							incomingNativeSnapshot?.snapshot.sequence ??
							current.lastNativeSequence,
						lastNativeGeneration:
							incomingNativeSnapshot?.generation ??
							current.lastNativeGeneration,
					});
					if (
						!input.publishWhenLive ||
						(yield* Ref.get(liveState)).retainCount > 0
					) {
						yield* PubSub.publish(changes, snapshot);
					}
					return snapshot;
		...
		const ingestNative = (
			snapshot: NativeTelemetryClient.NativeTelemetrySnapshot,
		...
		const ingestDesktop = (snapshot: DesktopHostTelemetrySnapshot) =>
			Effect.gen(function* () {
				...
				yield* nativeClient
					...
					.pipe(Effect.ignore);
				yield* nativeClient
					.setHostPowerState(snapshot.power)
					.pipe(Effect.ignore);
			...
			Effect.uninterruptible(
				Effect.gen(function* () {
					...
					if (current.retainCount > 0) {
						yield* Ref.set(liveState, {
							...current,
							retainCount: current.retainCount + 1,
						});
						return;
					}
			...
			Effect.gen(function* () {
				...
				if (current.retainCount <= 1) {
					...
					if (Option.isSome(current.scope)) {
						yield* Scope.close(current.scope.value, Exit.void).pipe(
							Effect.ignore,
						);
					}
					yield* desktopReceiver
						.setDiagnosticsDemand(false)
						.pipe(Effect.ignore);
					return;
				}
				yield* Ref.set(liveState, {
		...
		const subscribe = subscribeBeforeSnapshot(
			changes,
			Effect.acquireRelease(acquireLive, () => releaseLive).pipe(
				Effect.andThen(latest),
			),
			mutex,
		);
		const liveChanges = Stream.unwrap(
			Effect.map(subscribe, ({ changes }) => changes),
		);
		...
		const readHistory: ResourceTelemetry["Service"]["readHistory"] = (input) =>
			Effect.gen(function* () {
				...
				return buildResourceTelemetryHistory({
					...
					snapshots: Result.isSuccess(historyResult)
		...
		const refresh: ResourceTelemetry["Service"]["refresh"] =
					...
					Effect.map(({ snapshot }) =>
						snapshot.processes.some(
							(process) =>
								...
								process.startTimeMs === identity.startTimeMs,
						),
					),
					Effect.mapError(
						(cause) =>
