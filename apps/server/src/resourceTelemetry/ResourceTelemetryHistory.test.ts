...
const CHILD_PID = 300;
const STARTED_AT_MS = DateTime.toEpochMillis(
	DateTime.makeUnsafe("2026-06-17T12:00:00.000Z"),
);
...
describe("buildResourceTelemetryHistory", () => {
	it("normalizes query bounds before requesting native history", () => {
		expect(
			normalizeResourceTelemetryHistoryInput({ windowMs: 0, bucketMs: 0 }),
		).toEqual({
			windowMs: 1_000,
			bucketMs: 1_000,
		});
	...
	it("replays native snapshots on demand without applying current Electron metrics", () => {
		...
		const child = history.topProcesses.find(
			(process) => process.identity.pid === CHILD_PID,
		);
		const electron = history.topProcesses.find(
			(process) => process.identity.pid === ELECTRON_PID,
		);
		...
		expect(
			history.buckets.reduce((total, bucket) => total + bucket.ioWriteBytes, 0),
		).toBe(4_000);
	});
	...
	it("retains cumulative baselines while a process is absent from an intermediate sample", () => {
		...
		const history = buildResourceTelemetryHistory({
			...
			snapshots: [
				...
				{
					...absent,
					processes: absent.processes.filter(
						(process) => process.pid !== CHILD_PID,
					),
				},
				returned,
			],
	...
	it("uses the preceding sample as a baseline without attributing pre-window deltas", () => {
		...
		expect(
			history.buckets.every(
				(bucket) => DateTime.toEpochMillis(bucket.startedAt) <= readAtMs,
			),
		).toBe(true);
	});
	...
	it("replays the Electron root identity recorded with each native sample", () => {
		...
		const history = buildResourceTelemetryHistory({
			...
			snapshots: [
				...
				},
				{
					...
					processes: [
						...restartedElectron.processes.filter(
							(process) => process.pid !== ELECTRON_PID,
						),
						processSample({
							pid: ELECTRON_PID,
							ppid: SERVER_PID,
		...
		expect(
			history.topProcesses.find(
				(process) =>
					process.identity.pid === ELECTRON_PID &&
					process.identity.startTimeMs === 20,
			)?.category,
		).toBe("electron-main");
		expect(
			history.topProcesses.find(
				(process) =>
					process.identity.pid === ELECTRON_PID &&
					process.identity.startTimeMs === 999,
			)?.category,
		).toBe("server-child");
		expect(
			history.topProcesses.find((process) => process.identity.pid === 201)
				?.category,
		).toBe("electron-main");
	});
});
