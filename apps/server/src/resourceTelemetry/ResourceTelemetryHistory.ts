...
export function normalizeResourceTelemetryHistoryInput(input: {
	...
	const windowMs = Math.max(
		1_000,
		Math.min(MAX_HISTORY_WINDOW_MS, input.windowMs),
	);
	return {
		windowMs,
...
export type ResourceTelemetryHistoryWithLegacyBuckets =
	ResourceTelemetryHistory & {
...
function summarizeProcesses(
	...
	return [...groups.values()]
		.map((processSamples): ResourceTelemetryProcessSummary => {
			const sorted = processSamples.toSorted(
				(left, right) => left.sampledAtMs - right.sampledAtMs,
			);
			const first = sorted[0]!;
			const latest = sorted[sorted.length - 1]!;
			const cpuTotal = sorted.reduce(
				(total, sample) => total + sample.process.cpuPercent,
				0,
			);
			return {
				...
				maxCpuPercent: Math.max(
					...sorted.map((sample) => sample.process.cpuPercent),
				),
				cpuTimeMs: sorted.reduce(
					(total, sample) => total + sample.cpuTimeMs,
					0,
				),
				currentRssBytes: latest.process.residentBytes,
				peakRssBytes: Math.max(
					...sorted.map((sample) => sample.process.residentBytes),
				),
				ioReadBytes: sorted.reduce(
					(total, sample) => total + sample.ioReadBytes,
					0,
				),
				ioWriteBytes: sorted.reduce(
					(total, sample) => total + sample.ioWriteBytes,
					0,
				),
		...
		.toSorted(
			(left, right) =>
				right.cpuTimeMs - left.cpuTimeMs ||
				right.peakRssBytes - left.peakRssBytes,
		);
}
...
function buildBuckets(input: {
	...
	for (
		let startedAtMs = windowStartMs;
		startedAtMs < input.nowMs;
		startedAtMs += input.bucketMs
	) {
		...
		buckets.push({
			...
			ioReadBytes: samples.reduce(
				(total, sample) => total + sample.ioReadBytes,
				0,
			),
			ioWriteBytes: samples.reduce(
				(total, sample) => total + sample.ioWriteBytes,
				0,
			),
			maxProcessCount:
				samples.length === 0
					? 0
					: Math.max(...samples.map((sample) => sample.processCount)),
		});
	}
	return buckets;
}
...
export function buildResourceTelemetryHistory(
	...
	for (const snapshot of snapshots) {
		...
		const electronRootPids = new Set(
			recordedExternalProcesses.map((process) => process.pid),
		);
		...
		aggregateSamples.push({
			...
			ioReadBytes: deltas.reduce(
				(total, process) => total + process.ioReadBytes,
				0,
			),
			ioWriteBytes: deltas.reduce(
				(total, process) => total + process.ioWriteBytes,
				0,
			),
		});
