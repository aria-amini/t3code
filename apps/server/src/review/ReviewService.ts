...
export const make = Effect.gen(function* () {
					...
					cause.reason._tag === "NotFound"
						...
						: Effect.fail(
								new VcsRepositoryDetectionError({
									...
									cwd: resolvedPath,
									detail:
										"Failed to resolve a path while validating the review workspace.",
									cause,
								}),
							),
			}),
	...
	const assertWorkspaceBoundCwd = Effect.fn(
		"ReviewService.assertWorkspaceBoundCwd",
	)(function* (
		operation:
	...
	const getDiffPreview: ReviewService["Service"]["getDiffPreview"] = Effect.fn(
		...
		const handle = yield* vcsRegistry.detect({
			cwd: input.cwd,
			requestedKind: "auto",
		});
		if (!handle) {
	...
	const getDiffFileContents: ReviewService["Service"]["getDiffFileContents"] =
		Effect.fn("ReviewService.getDiffFileContents")(function* (input) {
