...
const GitManagerLayerLive = GitManager.layer.pipe(
	Layer.provideMerge(
		ProjectSetupScriptRunner.layer.pipe(Layer.provide(ServerSettingsLayerLive)),
	),
	Layer.provideMerge(GitVcsDriver.layer),
...
export const makeRoutesLayer = Layer.mergeAll(
	...
	Layer.provide(
		ServerSelfUpdate.layer.pipe(Layer.provide(DesktopAppUpdateLayerLive)),
	),
	...
	Effect.gen(function* () {
		...
		const runtimeStateLayer = Layer.effectDiscard(
			Effect.acquireRelease(
				Effect.gen(function* () {
					yield* Deferred.succeed(runtimeStateParked, undefined).pipe(
						Effect.orDie,
					);
		...
		const tailscaleServeLayer = config.tailscaleServeEnabled
			? Layer.effectDiscard(
					Effect.acquireRelease(
						Effect.gen(function* () {
							yield* Deferred.succeed(tailscaleParked, undefined).pipe(
								Effect.orDie,
							);
							yield* awaitActivation;
							const server = yield* HttpServer.HttpServer;
		...
		const cloudDesiredLinkReconcileLayer = Layer.effectDiscard(
			Effect.gen(function* () {
				if (!hasCloudPublicConfig) {
					yield* Deferred.succeed(cloudLinkParked, undefined).pipe(
						Effect.orDie,
					);
					return;
				}
				...
				yield* forkParked(
					Effect.gen(function* () {
						...
						yield* reconcileDesiredCloudLink(
		...
		const routesLayer = HttpRouter.serve(
			makeRoutesLayer.pipe(Layer.provide(launcherLayer)),
			{
			...
			Layer.tap(() =>
				Deferred.succeed(routesReady, undefined).pipe(Effect.orDie),
			),
		);
