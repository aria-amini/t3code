...
it.effect(
	"marks active running sessions that have persisted resume state",
	() => {
		const active = makeThread(
			"thread-mark-active",
			"running",
			TurnId.make("turn-mark-active"),
		);
					...
					upsert: (binding) =>
						Effect.sync(() => {
							...
							Effect.flatMap((firstMarkerCleared) =>
								firstMarkerCleared
									? Deferred.succeed(continuationCleared, undefined)
									: Effect.void,
							),
						),
					recordImportedTranscript: () => Effect.die("unused"),
...
it.effect(
	"retries continuation preparation before settling a persistent failure",
	() => {
			...
			Effect.tap(() =>
				Effect.sync(() =>
					assert.deepStrictEqual(
						dispatched.map(
							(command) =>
								command.type === "thread.session.set" && command.session.status,
						),
...
it.effect(
	"reconciles multiple active and archived orphans but skips live sessions",
	() => {
			...
			Effect.tap(() =>
				Effect.sync(() => {
					...
					for (const binding of upserts) {
						...
						assert.deepStrictEqual(binding.resumeCursor, {
							cursor: binding.threadId,
						});
					}
				}),
			),
...
it.effect(
	...
	() => {
		...
		const upsertFailure = makeThread(
			"thread-binding-upsert-failure",
			"running",
		);
		const dispatched: OrchestrationCommand[] = [];
...
it.effect(
	"retries failed projections and continues after a persistent failure",
	() => {
		const transient = makeThread(
			"thread-dispatch-transient-failure",
			"running",
		);
		const persistent = makeThread(
			"thread-dispatch-persistent-failure",
			"running",
		);
...
it.effect(
	"does not fail startup when the live provider session inventory cannot be read",
	() => {
		let queried = false;
		return ServerRuntimeStartup.reconcileProviderSessions.pipe(
			...
			Effect.provide(
				Layer.mergeAll(NodeServices.layer, ServerSettings.layerTest()),
			),
...
for (const preparedStatus of [
	...
	it.effect(
		`recovers again if startup exits with a prepared ${preparedStatus} session`,
		() =>
			Effect.gen(function* () {
				const turnId = TurnId.make("turn-interrupted-startup");
				const thread = makeThread(
					"thread-interrupted-startup",
					"running",
					turnId,
				);
					...
					directory: {
						...
						listBindings: () =>
							preparedStatus === "ready with failed scan"
								...
								: Effect.sync(() => [
										{ ...binding, lastSeenAt: "2026-01-01T00:00:00.000Z" },
									]),
					},
...
it.effect(
	"settles failed opt-in recovery without retrying the provider turn",
	() =>
		Effect.gen(function* () {
