...
interface CommandGate {
	...
	readonly failCommandReady: (
		error: ServerRuntimeStartupError,
	) => Effect.Effect<void>;
	readonly enqueueCommand: <A, E>(
		effect: Effect.Effect<A, E>,
	) => Effect.Effect<A, E | ServerRuntimeStartupError>;
...
export const makeCommandGate = Effect.gen(function* () {
	...
	const commandReadinessState =
		yield* Ref.make<CommandReadinessState>("pending");
...
export const recordStartupHeartbeat = Effect.gen(function* () {
	...
	const { threadCount, projectCount } = yield* projectionSnapshotQuery
		...
		.pipe(
			Effect.catch((cause) =>
				Effect.logWarning(
					"failed to gather startup projection counts for telemetry",
					{
						cause,
					},
				).pipe(
...
export const resolveAutoBootstrapWelcomeTargets = Effect.gen(function* () {
	...
	const serverConfig = yield* ServerConfig.ServerConfig;
	const projectionReadModelQuery =
		yield* ProjectionSnapshotQuery.ProjectionSnapshotQuery;
	...
	if (serverConfig.autoBootstrapProjectFromCwd) {
		const settings = yield* (yield* ServerSettings.ServerSettingsService)
			.getSettings;
		...
		yield* Effect.gen(function* () {
			...
			if (Option.isNone(existingProject)) {
				...
				const bootstrapProjectTitle =
					path.basename(serverConfig.cwd) || "project";
			...
			yield* Effect.gen(function* () {
				const existingThreadId =
					yield* projectionReadModelQuery.getFirstActiveThreadIdByProjectId(
						nextProjectId,
					);
...
const resolveStartupBrowserTarget = Effect.gen(function* () {
	...
	return yield* Effect.succeed(
		serverConfig.mode === "desktop" ? baseTarget : undefined,
	).pipe(
		Effect.flatMap((target) =>
			target
				? Effect.succeed(target)
				: serverAuth.issueStartupPairingUrl(baseTarget),
		),
	);
});
...
const isServerUpdateThreadContinuationError = Schema.is(
	ServerUpdateThreadContinuationError,
);

function readServerUpdateContinuationTurnId(
	runtimePayload: unknown,
): TurnId | null {
...
export const markRunningProviderSessionsForContinuation = Effect.gen(
	function* () {
...
export const reconcileProviderSessions = Effect.gen(function* () {
	...
	const continueAfterRestart = yield* settings.getSettings.pipe(
		...
		Effect.catch((cause) =>
			Effect.logWarning("could not read restart continuation preference", {
				cause,
	...
	const preparedThreadIds = new Set(
		(yield* directory.listBindings().pipe(
			Effect.catch((cause) =>
				Effect.logWarning("failed to read prepared provider continuations", {
					cause,
				}).pipe(
					Effect.andThen(
						Effect.forEach(
					...
					Effect.map((bindings) =>
						bindings.flatMap((binding) =>
							Option.isSome(binding) ? [binding.value] : [],
						),
					),
				),
			),
	...
	const orphanedThreads = threads.filter(
		(thread) =>
			...
			(thread.session.status === "starting" ||
				...
				thread.session.activeTurnId !== null ||
				(thread.session.status === "ready" &&
					preparedThreadIds.has(thread.id))) &&
			!liveThreadIds.has(thread.id),
	);
	...
	for (const thread of orphanedThreads) {
		...
		const binding = yield* directory.getBinding(thread.id).pipe(
			Effect.catchCause((cause) =>
				Cause.hasInterrupts(cause)
					? Effect.failCause(cause)
					: Effect.logWarning(
							"failed to read orphaned provider session directory binding",
							{
								threadId: thread.id,
								cause,
							},
		...
		const continuationMarked =
			...
			(session.activeTurnId === null ||
				continuationTurnId === session.activeTurnId) &&
			Option.isSome(binding) &&
			(session.activeTurnId !== null ||
				readRuntimePayload(binding.value.runtimePayload).activeTurnId == null ||
				readRuntimePayload(binding.value.runtimePayload).activeTurnId ===
					continuationTurnId);
				...
				yield* Effect.gen(function* () {
					...
					Effect.catchCause((cause) =>
						Cause.hasInterrupts(cause)
							? Effect.failCause(cause)
							: Effect.logWarning(
									"failed to settle orphaned provider session projection",
									{
										threadId: thread.id,
										cause,
									},
		...
		if (
			...
			if (Exit.isFailure(prepared)) {
				...
				yield* Effect.logWarning(
					"failed to prepare provider session continuation",
					{
						threadId: thread.id,
			...
			yield* forkParked(
				Effect.gen(function* () {
					const continuation = Effect.gen(function* () {
						...
						const capabilities =
							yield* providerService.getCapabilities(providerInstanceId);
					...
					if (
						Exit.isSuccess(continuationExit) ||
						Cause.hasInterrupts(continuationExit.cause)
					) {
						if (Exit.isSuccess(continuationExit)) {
							yield* clearContinuationMarkers(directory, [thread.id]).pipe(
								...
								Effect.catchCause((cause) =>
									Effect.logWarning(
										"failed to clear completed provider session continuation",
										{
											threadId: thread.id,
											cause,
										},
					...
					yield* Effect.logWarning(
						"failed to continue provider session after server restart",
						{
							threadId: thread.id,
...
export const autoPullProjects = Effect.fn("autoPullProjects")(function* (
	...
	const workspaceRoots = [
		...new Set(
			projects
				.filter((project) =>
					resolveProjectAutoPull(settings, project.id, project.autoPull),
				)
...
export const make = (options?: StartupOptions) =>
	Effect.gen(function* () {
		...
		const orchestrationReactor =
			yield* OrchestrationReactor.OrchestrationReactor;
		const providerSessionReaper =
			yield* ProviderSessionReaper.ProviderSessionReaper;
		...
		const syncAutoPullProjects = projectionSnapshotQuery
			.getShellSnapshot()
			.pipe(
				Effect.flatMap((snapshot) =>
					serverSettings.getSettings.pipe(
						Effect.flatMap((settings) =>
							autoPullProjects(snapshot.projects, settings),
						),
					),
				),
				Effect.catch((cause) =>
					Effect.logWarning("Failed to load projects for automatic pull", {
						cause,
					}),
				),
			);
		...
		const startup = Effect.gen(function* () {
			...
			yield* Effect.logDebug(
				"startup phase: parking orchestration roots at activation",
			);
			yield* runStartupPhase(
				"reactors.start",
				Effect.gen(function* () {
					yield* orchestrationReactor.start().pipe(Scope.provide(reactorScope));
					yield* providerSessionReaper
						.start()
			...
			yield* forkParked(
				Effect.gen(function* () {
					...
					if (serverConfig.startupPresentation === "headless") {
						...
						yield* runStartupPhase(
							"browser.open",
							maybeOpenBrowser(startupBrowserTarget),
						);
					}
				}),
			);
			...
			yield* runStartupPhase(
				...
				lifecycleEvents.publish({
					...
					payload: {
						environment,
						...welcomeBase,
						bootstrapStatus: serverConfig.autoBootstrapProjectFromCwd
							? "pending"
							: "complete",
					},
				}),
		...
		return {
			...
			markRunningProviderSessionsForContinuation:
				markRunningProviderSessionsForContinuation.pipe(
