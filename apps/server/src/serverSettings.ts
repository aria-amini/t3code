...
const encodeServerSettingsJson = Schema.encodeUnknownEffect(
	fromJsonStringPretty(ServerSettings),
);
 ...
 */
const foldProviderInstanceEnabledFlags = (
...
function redactProviderEnvironmentVariable(
	...
	return {
		...variable,
		value: "",
		...(variable.value.length > 0 || variable.valueRedacted
			? { valueRedacted: true }
			: {}),
	};
}
	...
	const providerInstances = Object.fromEntries(
		Object.entries(settings.providerInstances).map(([instanceId, instance]) => [
			...
			instance.environment
				? {
						...instance,
						environment: instance.environment.map(
	...
	Effect.gen(function* () {
		...
		const initialSettings = yield* normalizeServerSettings({
			...
			...(automaticGitFetchInterval !== undefined
				? {
						automaticGitFetchInterval:
							automaticGitFetchInterval as Duration.Duration,
					}
				: {}),
			...(providerHealthRefreshInterval !== undefined
				? {
						providerHealthRefreshInterval:
							providerHealthRefreshInterval as Duration.Duration,
					}
				: {}),
		});
...
const decodeServerSettingsJsonExit =
	Schema.decodeUnknownExit(ServerSettingsJson);
...
const decodePersistedOptionalProviderSettingsJsonExit =
	Schema.decodeUnknownExit(fromLenientJson(PersistedOptionalProviderSettings));
...
function restoreUsedProviders(
	...
	const usedProviders = new Set(
		providerHistory.map(({ providerName }) => providerName),
	);
	...
	return {
		...
		providers: {
			...
			cursor: {
				...
				enabled:
					persisted.providers?.cursor?.enabled ?? usedProviders.has("cursor"),
			},
			grok: {
				...settings.providers.grok,
				enabled:
					persisted.providers?.grok?.enabled ?? usedProviders.has("grok"),
			},
			opencode: {
...
function resolveTextGenerationProvider(
	...
	return isModelSelectionProviderEnabled(
		settings,
...
function fallbackTextGenerationProvider(
	settings: ServerSettings,
): ServerSettings {
	...
	const fallbackEntry = Object.entries(settings.providers).find(
		([driver, provider]) => {
			const instance =
				settings.providerInstances[ProviderInstanceId.make(driver)];
	...
	const fallback = fallbackEntry
		? ProviderDriverKind.make(fallbackEntry[0])
		: undefined;
	if (!fallback) {
		return settings;
	}
		...
		Effect.gen(function* () {
			...
			for (const [sourceId, source] of Object.entries(
				...
				usageLimitSources[sourceId] = {
					...source,
					managementKey: Option.isSome(secret)
						? textDecoder.decode(secret.value)
						: "",
				};
			}
			return {
				...
				usageLimitSources:
					usageLimitSources as ServerSettings["usageLimitSources"],
			};
		});
		...
		get subscribeChanges() {
			return PubSub.subscribe(changesPubSub).pipe(
				Effect.map((subscription) =>
					materializeChanges(Stream.fromSubscription(subscription)),
				),
			);
		},
	} satisfies ServerSettingsService["Service"];
