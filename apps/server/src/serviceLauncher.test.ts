...
it("orders exact semantic versions without treating build metadata as precedence", () => {
	...
	assert.equal(
		compareExactServiceVersions("2.0.0-alpha.1", "2.0.0-alpha.2"),
		-1,
	);
	assert.equal(
		compareExactServiceVersions("2.0.0-alpha.2", "2.0.0-alpha.beta"),
		-1,
	);
	assert.equal(
		compareExactServiceVersions("2.0.0-alpha-beta", "2.0.0-alpha-alpha"),
		1,
	);
...
it.layer(NodeServices.layer)("service state persistence", (it) => {
	it.effect("durably replaces and strictly reads one state document", () =>
		Effect.gen(function* () {
			...
			const root = yield* fs.makeTempDirectoryScoped({
				prefix: "t3-service-launcher-test-",
			});
	...
	it.effect("serializes shutdown with launcher recovery", () =>
		Effect.gen(function* () {
			...
			const path = yield* Path.Path;
			const root = yield* fs.makeTempDirectoryScoped({
				prefix: "t3-service-launcher-stop-",
			});
			const statePath = path.join(root, "runtime", "service-state.json");
			...
			assert.isTrue(
				yield* fs.exists(path.join(root, "runtime", SERVICE_STOP_MARKER_FILE)),
			);
	...
	it.effect("commits only after the trial reports prepared", () =>
		Effect.gen(function* () {
			...
			const path = yield* Path.Path;
			const root = yield* fs.makeTempDirectoryScoped({
				prefix: "t3-service-launcher-flow-",
			});
			const statePath = path.join(root, "runtime", "service-state.json");
...
`;
	...
	it.effect("rolls back a trial that reports the wrong update ID", () =>
		Effect.gen(function* () {
			...
			const path = yield* Path.Path;
			const root = yield* fs.makeTempDirectoryScoped({
				prefix: "t3-service-launcher-rollback-",
			});
			const statePath = path.join(root, "runtime", "service-state.json");
			const databasePath = path.join(root, "userdata", "state.sqlite");
			yield* fs.makeDirectory(path.dirname(databasePath), { recursive: true });
...
`;
			...
			assert.equal(
				state.update?.status === "rolled-back"
					? state.update.reason
					: undefined,
				"invalid-prepared",
			);
...
`;
			...
			assert.isFalse(
				yield* fs.exists(path.join(root, "runtime", "db-backup", updateId)),
			);
		}),
	);
});
