...
async function backupDatabaseOnce(
	...
	try {
		...
		await NodeFSP.rm(stagingDir, { recursive: true, force: true }).catch(
			() => undefined,
		);
		throw cause;
	}
...
async function discardDatabaseBackup(
	baseDir: string,
	updateId: string,
): Promise<void> {
...
export async function readServiceState(
	...
	if (state === undefined)
		throw new Error("Service state is invalid or unsupported.");
	return state;
}
...
}): Exclude<ServiceUpdateRecord, PendingServiceUpdate> & {
	readonly status: S;
} {
	return {
		id: input.pending.id,
...
export class Launcher {
	...
	async #recover(): Promise<void> {
		...
		await NodeFSP.rm(stopMarkerPath(this.#baseDir), { force: true }).catch(
			() => undefined,
		);
		const update = this.#state.update;
		if (update?.status !== "pending") {
			if (update !== undefined) {
				await discardDatabaseBackup(this.#baseDir, update.id).catch(
					() => undefined,
				);
			}
	...
	async #startChild(
		...
		child.on("message", (value) => {
			...
			if (message !== undefined)
				this.#enqueue(() => this.#handleMessage(managed, message));
		});
	...
	async #handleUpdateRequest(
		...
		this.#timer = setTimeout(
			() => this.#enqueue(() => this.#beginTrial(child)),
			HANDOFF_DELAY_MS,
		);
	}
	...
	async #handlePrepared(child: ManagedChild, updateId: string): Promise<void> {
		...
		if (
			...
			throw new Error(
				"Trial child reported prepared for an unexpected update.",
			);
		}
		this.#clearTimer();
		...
		this.#state = next;
		child.role = "active";
		await discardDatabaseBackup(this.#baseDir, committed.id).catch(
			() => undefined,
		);
		await sendMessage(child.process, {
			type: "committed",
	...
	async #returnToPrevious(
		...
		await writeServiceState(this.#statePath, next);
		this.#state = next;
		await discardDatabaseBackup(this.#baseDir, pending.id).catch(
			() => undefined,
		);
