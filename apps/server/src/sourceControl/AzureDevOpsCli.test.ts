...
const layer = Layer.mergeAll(
	AzureDevOpsCli.layer.pipe(Layer.provide(supportLayer)),
	supportLayer,
);
...
describe("AzureDevOpsCli.layer", () => {
	...
	it.effect(
		"builds a web URL when Azure returns only the pull request REST URL",
		() =>
			Effect.gen(function* () {
				mockRun.mockReturnValueOnce(
					Effect.succeed(
	...
	it.effect(
		"lists pull requests with Azure status and source branch arguments",
		() =>
			Effect.gen(function* () {
				mockRun.mockReturnValueOnce(
					Effect.succeed(
	...
	it.effect(
		"creates pull requests using the body file as the Azure description",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"preserves VCS causes without copying upstream details into messages",
		() =>
			Effect.gen(function* () {
				...
				const error = yield* az
					.execute({ cwd: "/repo", args: ["repos", "list"] })
					.pipe(Effect.flip);
	...
	it.effect(
		"does not report a missing working directory as a missing Azure CLI",
		() =>
			Effect.gen(function* () {
	...
	it.effect("keeps invalid pull request output diagnostics structured", () =>
		Effect.gen(function* () {
			...
			assert.instanceOf(
				error,
				AzureDevOpsCli.AzureDevOpsPullRequestDecodeError,
			);
			...
			assert.strictEqual(
				error.detail,
				"Azure DevOps CLI returned invalid pull request JSON.",
			);
