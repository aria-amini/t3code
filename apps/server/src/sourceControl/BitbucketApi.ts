...
function normalizeChangeRequestId(reference: string): string {
	...
	const urlMatch =
		/(?:pull-requests|pullrequests|pull-request|pull|pr)\/(\d+)(?:\D.*)?$/i.exec(
			trimmed,
		);
...
function sourceWorkspace(input: {
	...
	return SourceControlProvider.parseSourceControlOwnerRef(input.headSelector)
		?.owner;
}

function toBitbucketStates(
...
function parseBitbucketRepositorySlug(
	value: string,
...
function parseBitbucketRemoteUrl(
	remoteUrl: string,
): BitbucketRepositoryLocator | null {
...
function normalizeRepositoryCloneUrls(
	...
	const httpClone =
		raw.links.clone?.find((entry) => entry.name.toLowerCase() === "https")
			?.href ?? raw.links.html?.href;
	const sshClone = raw.links.clone?.find(
		(entry) => entry.name.toLowerCase() === "ssh",
	)?.href;
...
function defaultChangeRequestTargetBranch(input: {
	...
	if (
		...
		development.is_valid === false
	) {
		return repositoryMainBranch;
	}

	const developmentBranch =
		development.branch?.name?.trim() ?? development.name?.trim() ?? "";
...
function repositoryNameWithOwner(
	repository: Schema.Schema.Type<
		typeof BitbucketPullRequestSchema
	>["source"]["repository"],
): string | null {
...
export const make = Effect.gen(function* () {
	...
	const withAuth = (request: HttpClientRequest.HttpClientRequest) => {
		...
		if (Option.isSome(config.email) && Option.isSome(config.apiToken)) {
			return request.pipe(
				HttpClientRequest.basicAuth(config.email.value, config.apiToken.value),
			);
		}
		return request;
	};
	...
	const executeJson = <S extends Schema.Top>(
		...
		httpClient
			.execute(withAuth(request.pipe(HttpClientRequest.acceptJson)))
			.pipe(
				Effect.mapError(
					(cause) =>
				...
				Effect.flatMap((response) =>
					decodeResponse(operation, schema, response),
				),
			);

	const resolveRepository = Effect.fn("BitbucketApi.resolveRepository")(
		function* (input: {
			readonly cwd: string;
			readonly context?: SourceControlProvider.SourceControlProviderContext;
			readonly repository?: string;
			...
			const fromRepository =
				input.repository !== undefined
					? parseBitbucketRepositorySlug(input.repository)
					: null;
			...
			for (const remote of remotes.remotes) {
				if (
					detectSourceControlProviderFromRemoteUrl(remote.url)?.kind !==
					"bitbucket"
				)
					continue;
	...
	const getBranchingModelFromLocator = (
	...
	const resolveCheckoutRemote = Effect.fn("BitbucketApi.resolveCheckoutRemote")(
		function* (input: {
			readonly cwd: string;
			readonly context?: SourceControlProvider.SourceControlProviderContext;
			...
			const originRemoteUrl = yield* readConfigValueNullable(
				input.cwd,
				"remote.origin.url",
			);
	...
	> => {
		...
		if (url === null) {
			return Effect.fail(
				new BitbucketUntrustedUrlError({
		...
		return httpClient.execute(withAuth(withBody)).pipe(
			Effect.mapError(
				(cause): BitbucketApiError =>
					new BitbucketRequestError({ operation: "request", cause }),
			),
			Effect.flatMap((response) => {
	...
	return BitbucketApi.of({
		...
		probeAuth: executeJson(
			...
			Effect.map((user) => ({
				...
				account: nonEmpty(
					user.username ?? user.display_name ?? user.account_id,
				),
				host: Option.some("bitbucket.org"),
		...
		listPullRequests: (input) =>
			resolveRepository(input).pipe(
				...
				Effect.map((list) =>
					list.values.map(normalizeBitbucketPullRequestRecord),
				),
			),
		...
		createPullRequest: (input) =>
			Effect.gen(function* () {
				...
				const description = yield* fileSystem
					...
					Effect.all(
						{
							...
							branchingModel: getBranchingModelFromLocator(locator).pipe(
								Effect.orElseSucceed(
									(): typeof RawBitbucketBranchingModelSchema.Type | null =>
										null,
								),
							),
						},
		...
		checkoutPullRequest: (input) =>
			Effect.gen(function* () {
				...
				const sourceRepositoryName =
					repositoryNameWithOwner(pullRequest.source.repository) ??
					destinationRepositoryName;
				const isCrossRepository =
					sourceRepositoryName !== destinationRepositoryName;
				...
				yield* Effect.scoped(
					git.switchRef({ cwd: input.cwd, refName: localBranch }),
				);
			}).pipe(
