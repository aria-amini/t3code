...
function makeProvider(
	bitbucket: Partial<BitbucketApi.BitbucketApi["Service"]>,
) {
...
it.effect(
	"maps Bitbucket PR summaries into provider-neutral change requests",
	() =>
		Effect.gen(function* () {
...
it.effect("lists Bitbucket PRs through provider-neutral input names", () =>
	Effect.gen(function* () {
		let listInput:
			| Parameters<BitbucketApi.BitbucketApi["Service"]["listPullRequests"]>[0]
			| null = null;
		const provider = yield* makeProvider({
