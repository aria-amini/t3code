...
function pullRequestSummary(
	input: NormalizedGitHubPullRequestRecord,
		...
		process
			...
			.pipe(
				Effect.mapError((error) =>
					fromVcsError({ command: "gh", cwd: input.cwd }, error),
				),
			);
	...
	return GitHubCli.of({
						...
						: Effect.sync(() => decodeGitHubPullRequestListJson(raw)).pipe(
								Effect.flatMap((decoded) => {
									...
									return Effect.succeed(
										decoded.success.map(pullRequestSummary),
									);
								}),
							),
				),
		...
		createRepository: (input) =>
			execute({
				...
				Effect.map((result) =>
					deriveRepositoryCloneUrlsFromCreateOutput(
						result.stdout,
						input.repository,
					),
				),
