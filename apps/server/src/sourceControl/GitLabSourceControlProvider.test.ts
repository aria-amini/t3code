...
it.effect(
	"maps GitLab MR summaries into provider-neutral change requests",
	() =>
		Effect.gen(function* () {
...
it.effect("lists GitLab MRs through provider-neutral input names", () =>
	Effect.gen(function* () {
		let listInput:
			| Parameters<GitLabCli.GitLabCli["Service"]["listMergeRequests"]>[0]
			| null = null;
		const provider = yield* makeProvider({
...
it.effect("creates GitLab MRs through provider-neutral input names", () =>
	Effect.gen(function* () {
		let createInput:
			| Parameters<GitLabCli.GitLabCli["Service"]["createMergeRequest"]>[0]
			| null = null;
		const provider = yield* makeProvider({
