...
} from "./gitLabAuthStatus.ts";

function toChangeRequest(
	summary: GitLabCli.GitLabMergeRequestSummary,
): ChangeRequest {
	return {
		provider: "gitlab",
...
function parseGitLabAuth(input: SourceControlAuthProbeInput) {
	...
	const authenticatedHost = findAuthenticatedGitLabHost(
		parseGitLabAuthStatusHosts(output),
	);
	const account =
