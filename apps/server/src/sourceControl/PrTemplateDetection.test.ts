...
const writeTemplate = (cwd: string, relativePath: string, contents: string) =>
	Effect.gen(function* () {
		...
		yield* fileSystem.makeDirectory(path.dirname(templatePath), {
			recursive: true,
		});
...
it.effect(
	"uses the first non-empty template in the configured path order",
	() =>
		runWithTempDirectory((cwd) =>
			Effect.gen(function* () {
...
it.effect.each(TEMPLATE_DIRECTORIES)(
	"recognizes the $0 directory",
	(relativeDirectory) =>
		runWithTempDirectory((cwd) =>
			Effect.gen(function* () {
				yield* writeTemplate(
					cwd,
					`${relativeDirectory}/template.MD`,
					"directory template",
				);
				yield* commitTemplates(cwd);
		...
		runWithTempDirectory((cwd) =>
			Effect.gen(function* () {
				...
				yield* fileSystem.makeDirectory(
					path.join(templateDirectory, "b-directory.md"),
					{
						recursive: true,
					},
				);
				yield* fileSystem.writeFileString(
					path.join(templateDirectory, "a-empty.md"),
					" \n",
				);
				...
				yield* fileSystem.writeFileString(
					path.join(templateDirectory, "z-valid.md"),
					"valid",
				);
				yield* commitTemplates(cwd);
...
it.effect("does not guess between multiple directory templates", () =>
	runWithTempDirectory((cwd) =>
		Effect.gen(function* () {
			...
			yield* writeTemplate(
				cwd,
				"PULL_REQUEST_TEMPLATE/fallback.md",
				"fallback",
			);
			yield* commitTemplates(cwd);
	...
	() =>
		runWithTempDirectory((cwd) =>
			Effect.gen(function* () {
				...
				const escapedTemplatePath = path.join(
					cwd,
					".github",
					"pull_request_template.md",
				);
				yield* fileSystem.makeDirectory(path.dirname(escapedTemplatePath), {
					recursive: true,
				});
