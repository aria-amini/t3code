...
export const makeWithProviders = Effect.fn(
	"makeSourceControlProviderRegistryWithProviders",
	...
	const discoverySpecs = registrations.map(
	...
	const detectProviderContext = Effect.fn(
		"SourceControlProviderRegistry.detectProviderContext",
	)(function* (cwd: string) {
	...
	>(detectProviderContext, {
		...
		timeToLive: (exit) =>
			Exit.isSuccess(exit) ? PROVIDER_DETECTION_CACHE_TTL : Duration.zero,
	});

	const resolveHandle: SourceControlProviderRegistry["Service"]["resolveHandle"] =
		(input) =>
	...
	return SourceControlProviderRegistry.of({
		...
		resolveHandle,
		resolve: (input) =>
			resolveHandle(input).pipe(Effect.map((handle) => handle.provider)),
		discover: Effect.all(
...
export const make = Effect.gen(function* () {
	...
	const bitbucketDiscovery =
		yield* BitbucketSourceControlProvider.makeDiscovery;
