...
it("parses Retry-After seconds and HTTP dates", () => {
	...
	assert.equal(
		SourceControlRateLimit.retryAtFromHeader(
			"Thu, 01 Jan 1970 00:02:01 GMT",
			1_000,
		),
		121_000,
	);
...
it.effect(
	"lets an interactive request through without clearing an active pause",
	() =>
		Effect.gen(function* () {
			yield* TestClock.setTime(1_000);
			const limits = yield* SourceControlRateLimit.SourceControlRateLimit;
			...
			const interactiveLease = yield* limits.check(github, {
				allowPaused: true,
			});
...
it.effect("keeps providers and hosts isolated", () =>
	Effect.gen(function* () {
		...
		assert.equal(
			yield* limits.check({ provider: "gitlab", host: "github.com" }),
			0,
		);
		assert.equal(
			yield* limits.check({ provider: "github", host: "github.example.com" }),
			0,
		);
	}).pipe(Effect.provide(SourceControlRateLimit.layer)),
);
