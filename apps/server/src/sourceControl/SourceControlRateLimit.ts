...
export const make = Effect.gen(function* () {
	const entries = yield* Ref.make<ReadonlyMap<string, RateLimitEntry>>(
		new Map(),
	);
	...
	const recordRateLimit: SourceControlRateLimit["Service"]["recordRateLimit"] =
		Effect.fn("SourceControlRateLimit.recordRateLimit")(function* (input) {
			const now = yield* Clock.currentTimeMillis;
	...
	const recordSuccess: SourceControlRateLimit["Service"]["recordSuccess"] =
		Effect.fn("SourceControlRateLimit.recordSuccess")(function* (input) {
			const now = yield* Clock.currentTimeMillis;
			yield* Ref.update(entries, (current) => {
				const key = normalizedKey(input);
				const previous = current.get(key);
				if (
					previous === undefined ||
					previous.generation !== input.lease ||
					previous.retryAt > now
				) {
					return current;
				}
