		...
		Layer.provide(
			ServerConfig.layerTest(
				...
				input.fileSystem
					...
					: { prefix: "t3-source-control-repos-" },
			),
		),
	);
...
it.effect(
	"looks up repositories through the requested provider without search",
	() => {
		...
		return Effect.gen(function* () {
			...
			assert.deepStrictEqual(calls, [
				{ cwd: "/workspace", repository: "octocat/t3code" },
			]);
...
it.effect(
	"preserves provider failures without deriving the repository message from them",
	() => {
...
it.effect(
	"preserves destination probe failures instead of treating them as missing paths",
	() => {
...
it.effect(
	"publishes by creating the repository, adding a remote, and pushing upstream",
	() => {
		...
		return Effect.gen(function* () {
			...
			assert.deepStrictEqual(pushCalls, [
				{ cwd: "/workspace", remoteName: "origin" },
			]);
		}).pipe(
			Effect.provide(
				makeLayer({
...
it.effect("publishes to the remote name returned by ensureRemote", () => {
	...
	return Effect.gen(function* () {
		...
		assert.deepStrictEqual(pushCalls, [
			{ cwd: "/workspace", remoteName: "origin-1" },
		]);
	}).pipe(
		Effect.provide(
			makeLayer({
...
it.effect(
	"publish succeeds with status remote_added when the local repo has no commits",
	() => {
		let pushCalls = 0;
		return Effect.gen(function* () {
			const service =
