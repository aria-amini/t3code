...
export const make = Effect.gen(function* () {
	...
	const lookupRepository = Effect.fn(
		"SourceControlRepositoryService.lookupRepository",
	)(function* (input: SourceControlRepositoryLookupInput) {
	...
	const normalizeDestinationPath = Effect.fn(
		"SourceControlRepositoryService.normalizeDestination",
	...
	const prepareDestination = Effect.fn(
		"SourceControlRepositoryService.prepareDestination",
	)(function* (destinationPath: string) {
		const normalizedDestination =
			yield* normalizeDestinationPath(destinationPath);
		if (yield* fileSystem.exists(normalizedDestination)) {
			...
			yield* fileSystem.makeDirectory(path.dirname(normalizedDestination), {
				recursive: true,
			});
		}
	...
	const cloneRepository = Effect.fn(
		"SourceControlRepositoryService.cloneRepository",
	)(function* (input: SourceControlCloneRepositoryInput) {
	...
	const publishRepository = Effect.fn(
		"SourceControlRepositoryService.publishRepository",
	...
	return SourceControlRepositoryService.of({
		...
		publishRepository: (input) =>
			publishRepository(input).pipe(
				mapRepositoryError("publishRepository", input.provider),
			),
	});
});
