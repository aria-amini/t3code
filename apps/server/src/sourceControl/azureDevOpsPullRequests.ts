...
function normalizeAzureDevOpsPullRequestState(
	status: string,
): "open" | "closed" | "merged" {
...
export function azureDevOpsOrganizationBaseFromRestApiUrl(
	...
	try {
		...
		const isAzureRestUrl = pathSegments.some(
			(segment) => segment.toLowerCase() === "_apis",
		);
		if (!isAzureRestUrl) {
			return null;
		}
...
export function azureDevOpsPullRequestWebUrl(input: {
	...
	const organizationBase = azureDevOpsOrganizationBaseFromRestApiUrl(
		input.restApiUrl,
	);
...
const decodeAzureDevOpsPullRequestList = decodeJsonResult(
	Schema.Array(Schema.Unknown),
);
const decodeAzureDevOpsPullRequest = decodeJsonResult(
	AzureDevOpsPullRequestSchema,
);
const decodeAzureDevOpsPullRequestEntry = Schema.decodeUnknownExit(
	AzureDevOpsPullRequestSchema,
);
...
> {
	...
	if (Result.isSuccess(result)) {
		...
		for (const entry of result.success) {
			...
			pullRequests.push(
				normalizeAzureDevOpsPullRequestRecord(decodedEntry.value),
			);
		}
...
> {
	...
	if (Result.isSuccess(result)) {
		return Result.succeed(
			normalizeAzureDevOpsPullRequestRecord(result.success),
		);
	}
	return Result.fail(result.failure);
}
