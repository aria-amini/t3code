...
function repositoryOwner(
	repository: Schema.Schema.Type<typeof BitbucketRepositoryRefSchema>,
) {
	return (
		trimOptionalString(repository.workspace?.slug) ??
		(repository.full_name?.includes("/")
			? (repository.full_name.split("/")[0] ?? null)
			: null)
	);
}
...
export function normalizeBitbucketPullRequestRecord(
	...
	const headRepositoryNameWithOwner = trimOptionalString(
		raw.source.repository?.full_name,
	);
	const baseRepositoryNameWithOwner = trimOptionalString(
