...
function normalizeGitHubPullRequestRecord(
	...
	const explicitNameWithOwner = trimOptionalString(
	...
	const headRepositoryOwnerLogin =
		...
		(explicitNameWithOwner?.includes("/")
			? (explicitNameWithOwner.split("/")[0] ?? null)
			: null);
...
const decodeGitHubPullRequestList = decodeJsonResult(
	Schema.Array(Schema.Unknown),
);
const decodeGitHubPullRequest = decodeJsonResult(GitHubPullRequestSchema);
const decodeGitHubPullRequestEntry = Schema.decodeUnknownExit(
