const HOST_LINE_PATTERN =
	/^(?:[a-z0-9](?:[a-z0-9.-]*[a-z0-9])?|\[[a-f0-9:.]+\])(?::\d+)?$/iu;
...
export function parseGitLabAuthStatusHosts(
	text: string,
): ReadonlyArray<GitLabAuthStatusHost> {
	...
	const flush = () => {
		...
		const account =
			LOGGED_IN_PATTERN.exec(currentLines.join("\n"))?.[1]?.trim() || null;
