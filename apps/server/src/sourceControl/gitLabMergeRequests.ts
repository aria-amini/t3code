...
function projectPathWithNamespace(
	project:
		| Schema.Schema.Type<typeof GitLabProjectReferenceSchema>
		| null
		| undefined,
): string | null {
	const explicit =
...
function ownerLoginFromPathWithNamespace(
...
function normalizeGitLabMergeRequestRecord(
	...
	const headRepositoryOwnerLogin =
		ownerLoginFromPathWithNamespace(sourceProjectPath);
...
const decodeGitLabMergeRequestList = decodeJsonResult(
	Schema.Array(Schema.Unknown),
);
const decodeGitLabMergeRequest = decodeJsonResult(GitLabMergeRequestSchema);
const decodeGitLabMergeRequestEntry = Schema.decodeUnknownExit(
