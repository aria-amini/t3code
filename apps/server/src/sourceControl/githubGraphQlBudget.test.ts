...
describe("GitHub GraphQL budget", () => {
	it.effect("adds rate metadata to a read query", () =>
		Effect.gen(function* () {
			...
			expect(query).toContain(
				'repository(owner: "acme", name: "web") { name }',
			);
		}).pipe(Effect.provide(GitHubGraphQlBudget.layer)),
	);

	it.effect(
		"protects the last ten percent with the shared provider cooldown error",
		() =>
			Effect.gen(function* () {
				...
				const error = yield* Effect.flip(
					budget.query("github.com", "query { viewer { login } }"),
				);
	...
	it.effect("keeps hosts isolated", () =>
		Effect.gen(function* () {
			...
			expect(
				yield* budget.query("github.example.com", "query { viewer { login } }"),
			).toContain("rateLimit");
		}).pipe(Effect.provide(GitHubGraphQlBudget.layer)),
	);
	...
	it.effect(
		"learns a cheaper observed cost without restoring reserved quota",
		() =>
			Effect.gen(function* () {
				yield* TestClock.setTime(BEFORE_RESET);
				const budget = yield* GitHubGraphQlBudget.GitHubGraphQlBudget;
				...
				for (let count = 0; count < 4; count += 1) {
					expect(yield* budget.query("github.com", query)).toContain(
						"rateLimit",
					);
				}
