...
function snapshotFrom(raw: string): GraphQlBudgetSnapshot | null {
	try {
		...
		return Number.isFinite(resetAtMs)
			? { cost, limit, remaining, resetAtMs }
			: null;
	} catch {
		return null;
	}
...
export const make = Effect.gen(function* () {
	const snapshots = yield* Ref.make<ReadonlyMap<string, GraphQlBudgetSnapshot>>(
		new Map(),
	);

	const query: GitHubGraphQlBudget["Service"]["query"] = Effect.fn(
		"GitHubGraphQlBudget.query",
		...
		const retryAt = yield* Ref.modify(snapshots, (current) => {
			...
			const remaining = Math.max(
				0,
				snapshot.remaining - Math.max(1, snapshot.cost),
			);
			if (
				options?.allowReserve !== true &&
				remaining < snapshot.limit * GRAPHQL_RESERVE_RATIO
			) {
