...
it("prefers localhost when no explicit host is configured", () => {
	...
	expect(resolveHeadlessConnectionString(undefined, 3773)).toBe(
		"http://localhost:3773",
	);
});

it("keeps explicit bind hosts in the connection string", () => {
	expect(resolveHeadlessConnectionString("127.0.0.1", 3773)).toBe(
		"http://127.0.0.1:3773",
	);
	expect(resolveHeadlessConnectionString("::1", 3773)).toBe(
		"http://[::1]:3773",
	);
});
...
it("renders terminal QR codes as a multi-line unicode block grid", () => {
	const qrCode = renderTerminalQrCode(
		"http://192.168.1.42:3773/pair#token=PAIRCODE",
	);
...
it("formats headless serve output with the connection string, token, pairing url, and qr code", () => {
	...
	expect(output).toContain(
		"Pairing URL: http://192.168.1.42:3773/pair#token=PAIRCODE",
	);
	assert.isTrue(
