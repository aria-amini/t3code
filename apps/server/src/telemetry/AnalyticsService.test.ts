...
it.layer(NodeServices.layer)("AnalyticsService test", (it) => {
	it.effect("flush drains all buffered events across multiple batches", () =>
		Effect.gen(function* () {
			...
			const serverConfigLayer = ServerConfig.ServerConfig.layerTest(
				process.cwd(),
				{
			...
			assert.equal(
				batchRequests.every(
					(request) =>
						request.path.endsWith("/batch/") || request.path.endsWith("/batch"),
				),
				true,
			);
				...
				batchRequests.every((request) =>
					request.body.batch.every(
						(event) =>
							...
							event.properties.serverAppVersion ===
								event.properties.t3CodeVersion &&
