...
const findIdentityLog = (
	...
	logs.find(
		(log) =>
			log.annotations.source === source &&
			log.annotations.errorTag === errorTag,
	);

it.layer(NodeServices.layer)("telemetry identity", (it) => {
	it.effect(
		"uses the persisted anonymous id when provider identities are absent",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"logs structured decode context and falls back from malformed Codex auth",
		() => {
			...
			return Effect.gen(function* () {
				...
				yield* fileSystem.makeDirectory(path.dirname(codexAuthPath), {
					recursive: true,
				});
				...
				const identifier =
					yield* Identify.getTelemetryIdentifierForHome(homeDirectory);

				assert.equal(identifier, sha256(anonymousId));
				const decodeLog = findIdentityLog(
					logs,
					"codex",
	...
	it.effect(
		"does not overwrite the anonymous id path after a non-NotFound read failure",
		() => {
			const logs: CapturedLog[] = [];
			const logger = makeCaptureLogger(logs);
			...
			return Effect.gen(function* () {
				...
				assert.deepEqual(
					yield* fileSystem.readDirectory(config.anonymousIdPath),
					[],
				);

				const readLog = findIdentityLog(
					logs,
					"anonymous",
				...
				assert.include(
					errorStack,
					"Failed to read anonymous telemetry identity",
				);
				assert.isUndefined(
					findIdentityLog(
						logs,
