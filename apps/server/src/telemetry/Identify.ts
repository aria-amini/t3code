...
export const TelemetryIdentitySource = Schema.Literals([
	"codex",
	"claude",
	"anonymous",
]);
...
const decodeCodexAuthJson = Schema.decodeEffect(
	Schema.fromJsonString(CodexAuthJsonSchema),
);
const decodeClaudeJson = Schema.decodeEffect(
	Schema.fromJsonString(ClaudeJsonSchema),
);
...
const hash = (source: TelemetryIdentitySource, value: string) =>
	Crypto.Crypto.pipe(
		Effect.flatMap((crypto) =>
			crypto.digest("SHA-256", new TextEncoder().encode(value)),
		),
...
const getCodexAccountId = Effect.fn("TelemetryIdentity.getCodexAccountId")(
...
const getClaudeUserId = Effect.fn("TelemetryIdentity.getClaudeUserId")(
	function* (homeDirectory: string) {
		const fileSystem = yield* FileSystem.FileSystem;
		const path = yield* Path.Path;

		const claudeJsonPath = path.join(homeDirectory, ".claude.json");
		const encoded = yield* readIdentityFile(
			fileSystem,
			"claude",
			claudeJsonPath,
		);
...
const upsertAnonymousId = Effect.gen(function* () {
	...
	const existing = yield* readIdentityFile(
		fileSystem,
		"anonymous",
		anonymousIdPath,
	);
 ...
 */
export const getTelemetryIdentifierForHome = Effect.fn(
		...
		const codexAccountId = yield* getCodexAccountId(homeDirectory).pipe(
			Effect.catchTags({
				TelemetryIdentityReadError: (error) =>
					logTelemetryIdentityError(error).pipe(
