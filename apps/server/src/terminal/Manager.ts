...
function deriveSubprocessInspectResult(
	...
	const normalized = normalizeChildCommandName(
		snapshot.commandById.get(childPid) ?? "",
		platform,
	);
	return {
...
const resolvePosixPsCommand = Effect.fn("terminal.resolvePosixPsCommand")(
	function* () {
...
const posixProcessTableSnapshot = Effect.fn(
	"terminal.posixProcessTableSnapshot",
)(function* (
	psCommand: string,
): Effect.fn.Return<
	TerminalProcessTableSnapshot,
...
const windowsProcessTableSnapshot = Effect.fn(
	"terminal.windowsProcessTableSnapshot",
...
> {
	...
	const result = yield* processRunner
		...
		.pipe(
			Effect.mapError(
				(cause) =>
					new TerminalSubprocessCheckError({ cause, command: "powershell" }),
			),
		);
	if (result.code !== 0 || result.timedOut || result.stdoutTruncated) {
		return yield* new TerminalSubprocessCheckError({
...
export class BoundedTerminalHistory {
	...
	private appendChunk(data: string): void {
		...
		if (
			previous &&
			previous.data.length + data.length <= MAX_HISTORY_CHUNK_LENGTH
		) {
			previous.data += data;
	...
	private trim(): void {
		...
		let linesToDrop =
			this.lineBreaks + (trailingNewline ? 0 : 1) - this.maxLines;
		while (linesToDrop > 0) {
			const first = this.chunks[this.start]!;
			...
			this.trimChunk(
				offset,
				Buffer.byteLength(first.data.slice(0, offset)),
				linesToDrop,
			);
			linesToDrop = 0;
		}
...
export const resolveProviderInstanceTerminalEnvironment = Effect.fn(
	...
	const providerInstanceId = ProviderInstanceId.make(
		input.rawProviderInstanceId,
	);
	const settings = yield* input.serverSettings.getSettings.pipe(
		Effect.mapError(
			(cause) =>
				new TerminalProviderEnvironmentError({ providerInstanceId, cause }),
		),
	);
	const instance =
		deriveProviderInstanceConfigMap(settings)[providerInstanceId];
...
export const makeWithOptions = Effect.fn("TerminalManager.makeWithOptions")(
	function* (options: TerminalManagerOptions) {
		...
		const shellResolver =
			options.shellResolver ?? (() => defaultShellResolver(platform, baseEnv));
		const processRunner = yield* ProcessRunner.ProcessRunner;
		const resolveLaunchInputEnvironment = Effect.fn(
			"terminal.resolveLaunchInputEnvironment",
		)(function* <
			Input extends
				| TerminalOpenInput
				| TerminalAttachInput
		...
		const readHistoryTail = Effect.fn("terminal.readHistoryTail")(function* (
			...
			if (offset > 0n) {
				...
				while (start < length && ((bytes[start] ?? 0) & 0xc0) === 0x80)
					start += 1;
			}
			return {
				history: new TextDecoder("utf-8", { ignoreBOM: true }).decode(
					bytes.subarray(start, length),
				),
				truncated: offset > 0n,
			};
		});
		...
		const readHistory = Effect.fn("terminal.readHistory")(function* (
			...
			if (
				...
				const { history: raw, truncated } = yield* readHistoryTail(
					nextPath,
				).pipe(
					Effect.scoped,
					Effect.mapError(
			...
			if (terminalId !== DEFAULT_TERMINAL_ID) {
				return new BoundedTerminalHistory(
					historyLineLimit,
					"",
					historyByteLimit,
				);
		...
		const pollSubprocessActivity = Effect.fn("terminal.pollSubprocessActivity")(
			function* () {
				...
				const inspectorOption = yield* acquireSubprocessInspector.pipe(
					...
					Effect.catch((reason) =>
						Effect.logWarning(
							"failed to snapshot processes for terminal subprocess polling",
							{
								reason,
							},
						).pipe(
				...
				const { inspector: subprocessInspector, snapshotSucceeded } =
		...
		const restart: TerminalManager["Service"]["restart"] = (input) =>
			withThreadLock(
				...
				resolveLaunchInputEnvironment(input).pipe(
					Effect.flatMap(restartResolved),
				),
			);
