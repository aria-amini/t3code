...
type TextRuntime = Effect.Success<
	ReturnType<AntigravityTextGenerationOptions["makeRuntime"]>
>;
...
const makeFixture = Effect.fn("makeAntigravityTextGenerationFixture")(
	function* (
		options: {
			readonly outputs?: ReadonlyArray<string>;
			readonly prompt?: (
				context: PromptContext,
			) => Effect.Effect<AcpSchema.PromptResponse, AcpError>;
		...
		const root = yield* fs.makeTempDirectoryScoped({
			prefix: "t3-antigravity-text-test-",
		});
		const profileDirectory = path.join(root, "profile");
		const projectDirectory = path.join(root, "project");
		const conversations = path.join(
			profileDirectory,
			"antigravity-acp",
			"conversations",
		);
		...
		const brainDirectory = path.join(
			profileDirectory,
			"antigravity-acp",
			"brain",
			SESSION_ID,
		...
		yield* fs.writeFileString(
			path.join(projectDirectory, "keep.txt"),
			"untouched",
		);
		yield* fs.writeFileString(
			path.join(conversations, "user-session.db"),
			"keep native history",
		);
		...
		const withProcess: AntigravityTextGenerationOptions["withProcess"] = (
			stop,
			task,
		) =>
			Effect.gen(function* () {
		...
		const makeRuntime: AntigravityTextGenerationOptions["makeRuntime"] = (
			cwd,
		) =>
			Effect.gen(function* () {
				...
				yield* Effect.addFinalizer(() =>
					Effect.gen(function* () {
						state.nativeFilesAtClose.push(
							yield* fs.exists(`${sessionBase}.db`).pipe(Effect.orDie),
						);
						state.closed.push(cwd);
					}),
				);
				let sessionUpdate: Parameters<
					TextRuntime["handleSessionUpdate"]
				>[0] = () => Effect.void;
				...
				return {
					start: () =>
						Effect.gen(function* () {
							...
							yield* fs
								.makeDirectory(brainDirectory, { recursive: true })
								.pipe(Effect.orDie);
							yield* fs
								.writeFileString(`${sessionBase}.db`, "helper session")
								.pipe(Effect.orDie);
							yield* fs
								.writeFileString(`${sessionBase}.db-wal`, "helper journal")
								.pipe(Effect.orDie);
							const metadata = yield* encodeMetadata({ cwd }).pipe(
								Effect.orDie,
							);
							yield* fs
								.writeFileString(`${sessionBase}.meta`, metadata)
								.pipe(Effect.orDie);
							yield* fs
								.writeFileString(
									path.join(brainDirectory, "output.txt"),
									"helper artifact",
								)
								.pipe(Effect.orDie);
							return {
		...
		const assertCleaned = Effect.gen(function* () {
			...
			expect(
				yield* fs.readFileString(path.join(projectDirectory, "keep.txt")),
			).toBe("untouched");
			expect(
				yield* fs.readFileString(path.join(conversations, "user-session.db")),
			).toBe("keep native history");
		});
		return {
			fs,
			path,
...
it.layer(NodeServices.layer)("AntigravityTextGeneration", (it) => {
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				expect(
					yield* fixture.textGeneration.generateThreadTitle(fixture.titleInput),
				).toEqual({
					title: "Repair Google login",
				});
				expect(new Set(fixture.state.workspaces).size).toBe(4);
				expect(fixture.state.workspaces).not.toContain(
					fixture.projectDirectory,
				);
				expect(fixture.state.nativeFilesAtClose).toEqual([
					true,
					true,
					true,
					true,
				...
				expect(fixture.state.selectedModels).toEqual(
					Array(4).fill(modelSelection.model),
				);
				expect(fixture.state.prompts[0]?.prompt).toEqual([
					{
						type: "text",
						text: expect.stringContaining("+handleRemoteCallback()"),
					},
				]);
	...
	it.effect(
		"rejects native permission and choice requests instead of answering them",
		() =>
			Effect.gen(function* () {
				...
				const reply = yield* fixture.incoming.permission({
					...
					options: [
						{
							optionId: "first-native-choice",
							name: "Use this title",
							kind: "allow_once",
						},
						{
							optionId: "second-native-choice",
							name: "Use this title",
							kind: "allow_once",
						},
					],
	...
	it.effect(
		"rejects a helper that writes files in its temporary workspace",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
	...
	it.effect(
		"uses the native default without sending T3's default selection as a model ID",
		() =>
			Effect.gen(function* () {
				const fixture = yield* makeFixture();
	...
	it.effect.each([
		...
		"checks $name before starting a helper, available=$available",
		({ name, value, available }) =>
			Effect.gen(function* () {
				const fixture = yield* makeFixture();
				const configDirectory = fixture.path.join(
					fixture.profileDirectory,
					"config",
				);
				yield* fixture.fs.makeDirectory(configDirectory, { recursive: true });
				yield* fixture.fs.writeFileString(
					fixture.path.join(configDirectory, name),
					value,
				);
				expect(
					yield* isAntigravityTextGenerationAvailable(fixture.profileDirectory),
				).toBe(available);
	...
	it.effect(
		"does not start another runtime or prompt after authentication fails",
		() =>
			Effect.gen(function* () {
				const fixture = yield* makeFixture({
					startError: new AcpRequestError({
						code: -32000,
		...
		(source) =>
			Effect.gen(function* () {
				...
				if (source === "caller") {
					...
					if (!fixture.state.stop)
						return yield* Effect.die("No tracked helper to stop.");
	...
	it.effect(
		"refuses unsafe native session IDs without deleting other files",
		() =>
			Effect.gen(function* () {
				...
				expect(fixture.state.prompts).toEqual([]);
				expect(
					yield* fixture.fs.readFileString(`${fixture.sessionBase}.db`),
				).toBe("helper session");
	...
	it.effect(
		"keeps native sessions whose metadata belongs to a different workspace",
		() =>
			Effect.gen(function* () {
				const fixture = yield* makeFixture();
				yield* fixture.fs.writeFileString(
					`${fixture.sessionBase}.db`,
					"keep this session",
				);
				const metadata = yield* encodeMetadata({
					cwd: fixture.projectDirectory,
				});
				yield* fixture.fs.writeFileString(
					`${fixture.sessionBase}.meta`,
					metadata,
				);
				...
				expect(
					yield* fixture.fs.readFileString(`${fixture.sessionBase}.db`),
				).toBe("keep this session");
				expect(
					yield* fixture.fs.readFileString(`${fixture.sessionBase}.meta`),
				).toBe(metadata);
			}).pipe(Effect.scoped),
	);
});
