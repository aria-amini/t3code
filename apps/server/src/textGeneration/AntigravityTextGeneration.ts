...
const decodeConfiguration = Schema.decodeEffect(
		...
		const empty = yield* fs.readFileString(configurationPath).pipe(
			...
			Effect.flatMap((configuration) =>
				decodeConfigurationObject(
					configuration[name === "hooks.json" ? "hooks" : "mcpServers"] ??
						configuration,
				),
			),
...
export const makeAntigravityTextGeneration = Effect.fn(
	"makeAntigravityTextGeneration",
)(function* (options: AntigravityTextGenerationOptions) {
	const fs = yield* FileSystem.FileSystem;
	const path = yield* Path.Path;
	const available = isAntigravityTextGenerationAvailable(
			...
			const helper = Effect.gen(function* () {
				...
				const rawResult = yield* Effect.gen(function* () {
					...
					yield* runtime.getEvents().pipe(
						Stream.runForEach((event) =>
							event._tag === "EventStreamBarrier"
								? Deferred.succeed(event.acknowledge, undefined).pipe(
										Effect.asVoid,
									)
								: Effect.void,
						),
							...
							Effect.andThen(
								Effect.fail(
									new AcpRequestError({
										code: -32601,
										errorMessage:
											"Tools and user input are disabled for text generation.",
									}),
								),
							),
						);
					...
					yield* runtime.handleRequestPermission(() =>
						...
						).pipe(Effect.as({ outcome: { outcome: "cancelled" as const } })),
					);
					...
					return yield* Effect.gen(function* () {
						...
						if (!isNativeSessionId(sessionId)) {
							return yield* new TextGenerationError({
								operation,
								detail:
									"Antigravity returned an invalid text helper session ID.",
							});
						}
						...
						yield* applyAntigravityAcpModelSelection({
							...
							mapError: (cause) =>
								new TextGenerationError({
									operation,
									detail:
										"Could not select the Antigravity model for text generation.",
									cause,
								}),
						});
	...
	const generateCommitMessage: TextGeneration.TextGeneration["Service"]["generateCommitMessage"] =
		Effect.fn("AntigravityTextGeneration.generateCommitMessage")(
			function* (input) {
	...
	const generateBranchName: TextGeneration.TextGeneration["Service"]["generateBranchName"] =
		Effect.fn("AntigravityTextGeneration.generateBranchName")(
			function* (input) {
				const generated = yield* runAntigravityJson({
	...
	const generateThreadTitle: TextGeneration.TextGeneration["Service"]["generateThreadTitle"] =
		Effect.fn("AntigravityTextGeneration.generateThreadTitle")(
			function* (input) {
				const generated = yield* runAntigravityJson({
