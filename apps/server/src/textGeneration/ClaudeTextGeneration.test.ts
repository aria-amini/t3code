...
const ClaudeTextGenerationTestLayer = ServerConfig.ServerConfig.layerTest(
	process.cwd(),
	{
...
function withFakeClaudeEnv<A, E, R>(
	...
	return Effect.gen(function* () {
		...
		const previousConfigDirMustBe =
			process.env.T3_FAKE_CLAUDE_CONFIG_DIR_MUST_BE;
...
it.layer(ClaudeTextGenerationTestLayer)("ClaudeTextGeneration", (it) => {
	it.effect(
		"forwards Claude thinking settings without passing unsupported effort",
		() =>
			withFakeClaudeEnv(
				{
	...
	it.effect(
		"generates branch names from skill prompts without executable capabilities",
		() =>
			withFakeClaudeEnv(
				{
					output: JSON.stringify({
						structured_output: { branch: "call-script" },
					}),
					stdinMustContain: "/call-script",
				},
				(textGeneration) =>
				...
				return yield* withFakeClaudeEnv(
					...
					(textGeneration) =>
						Effect.gen(function* () {
							...
							expect(generated.title).toBe(
								sanitizeThreadTitle("Use Claude home"),
							);
						}),
				);
			}),
	...
	for (const verbose of [false, true]) {
		it.effect(
			`unwraps a JSON title in ${verbose ? "verbose" : "normal"} Claude output`,
			() => {
				const result = {
					type: "result",
					structured_output: {
						title: '{"title": "Refresh ev-stg APP ASG instances"}',
					},
				};
