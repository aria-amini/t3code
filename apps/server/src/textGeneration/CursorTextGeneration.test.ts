...
const CursorTextGenerationTestLayer = ServerConfig.ServerConfig.layerTest(
	process.cwd(),
	{
...
function withFakeAcpAgent<A, E, R>(
	...
	return Effect.gen(function* () {
		const tempDir = NodeFS.mkdtempSync(
			NodePath.join(NodeOS.tmpdir(), "t3code-cursor-text-acp-"),
		);
	...
	return Effect.gen(function* () {
		...
		for (;;) {
			const result = yield* Effect.exit(
				Effect.sync(() => NodeFS.readFileSync(path, "utf8")),
			);
				...
				(textGeneration) =>
					Effect.gen(function* () {
						const generated = yield* textGeneration.generateCommitMessage({
							...
							stagedSummary:
								"M apps/server/src/textGeneration/CursorTextGeneration.ts",
							stagedPatch:
						...
						expect(generated.body).toBe(
							"- verify cursor acp model config path",
						);
						...
						expect(
							requests.find((request) => request.method === "initialize")
			...
			(textGeneration) =>
				Effect.gen(function* () {
					...
					expect(generated.subject).toBe(
						"Update README dummy comment with attribution and date",
					);
					expect(generated.body).toBe("");
				}),
		),
