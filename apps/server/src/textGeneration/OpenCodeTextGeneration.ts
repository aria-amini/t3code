...
type OpenCodeTextGenerationOperation =
	typeof OpenCodeTextGenerationOperation.Type;
...
export class OpenCodeTextGenerationPromptResponseError extends Schema.TaggedErrorClass<OpenCodeTextGenerationPromptResponseError>()(
	...
	override get message(): string {
		const providerError = this.providerErrorName
			? ` ${this.providerErrorName}`
			: "";
...
function getOpenCodeTextResponse(
...
export const makeOpenCodeTextGeneration = Effect.fn(
	"makeOpenCodeTextGeneration",
)(function* (openCodeSettings: OpenCodeSettings) {
	const serverConfig = yield* ServerConfig.ServerConfig;
	const openCodeRuntime = yield* OpenCodeRuntime.OpenCodeRuntime;
	const serverOwner = yield* OpenCodeServerOwner.OpenCodeServerOwner;

	const runOpenCodeJson = Effect.fn("runOpenCodeJson")(function* <
	...
	>(input: {
		...
		const runAgainstServer = Effect.fn("runOpenCodeJson.runAgainstServer")(
			function* (
				...
				const promptFailure = getOpenCodePromptFailure(
				...
				if (promptFailure) {
					return yield* new OpenCodeTextGenerationPromptResponseError({
						...promptContext,
						...(promptFailure.name
							? { providerErrorName: promptFailure.name }
							: {}),
	...
	const generateCommitMessage: TextGeneration.TextGeneration["Service"]["generateCommitMessage"] =
		Effect.fn("OpenCodeTextGeneration.generateCommitMessage")(
			function* (input) {
