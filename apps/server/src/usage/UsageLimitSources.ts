...
export const make = Effect.gen(function* () {
	...
	const readSource = Effect.fn("UsageLimitSources.readSource")(function* (
		...
		if (accounts._tag === "Failure") {
			yield* Effect.logDebug("usage limit source read failed", {
				id,
				cause: accounts.failure,
			});
	...
	const consumeResetCredit = (input: UsageLimitSourceConsumeResetCreditInput) =>
		Effect.gen(function* () {
				...
				Effect.mapError(
					() =>
						new UsageLimitSourceError({
							detail: "Could not read hub settings.",
						}),
				),
			);
			...
			const result = yield* api.consume(
				config,
				input.accountId,
				input.creditId,
			);
			...
			yield* publish(
				previous.map((source) =>
					source.id === input.sourceId ? snapshot : source,
				),
			);
			return result;
