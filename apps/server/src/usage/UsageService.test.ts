				...
				HttpClient.make((request) =>
					Effect.sync(() => {
						...
						return HttpClientResponse.fromWeb(
							request,
							Response.json(input.ratesDocument ?? {}),
						);
					}),
				),
			),
...
function totalOutputTokens(summary: {
	buckets: readonly { totals: { outputTokens: number } }[];
}) {
	return summary.buckets.reduce(
		(sum, bucket) => sum + bucket.totals.outputTokens,
		0,
	);
}
...
describe("UsageService", () => {
	it.live(
		"reprices unchanged transcripts when custom prices are added, edited, or removed",
		() =>
			Effect.gen(function* () {
				const { transcript, settings, home } = yield* setup;
				yield* Effect.promise(() =>
					NodeFSP.writeFile(transcript, claudeLine(1, 5, "example-model")),
				);
				...
				yield* Effect.gen(function* () {
					...
					assert.deepStrictEqual(
						overridden.buckets[0]?.totals,
						original.buckets[0]?.totals,
					);
					...
					Effect.provide(
						serviceLayers({
							prefix: "usage-service-price-overrides-test",
							home,
							settings,
						}),
					),
	...
	it.live("counts appended usage on a rescan of a grown transcript", () =>
		Effect.gen(function* () {
			const { transcript, settings, home } = yield* setup;
			yield* Effect.promise(() =>
				NodeFSP.writeFile(transcript, claudeLine(1, 5)),
			);

			const service = yield* UsageService.make.pipe(
				Effect.provide(
					serviceLayers({ prefix: "usage-service-grow-test", home, settings }),
				),
			);
			...
			yield* Effect.promise(() =>
				NodeFSP.appendFile(transcript, claudeLine(2, 7)),
			);
	...
	it.live("does not share an in-flight scan after custom prices change", () =>
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
						...
						exists: (path) =>
							fileSystem.exists(path).pipe(
								Effect.tap(() => {
									if (
										path !==
										NodePath.join(home, "claude", ".claude", "projects")
									)
										return Effect.void;
									homeProbes += 1;
				...
				Effect.provide(
					serviceLayers({
						prefix: "usage-service-price-race-test",
						home,
						settings,
					}),
				),
	...
	it.live(
		"refetches a rate table inside its TTL only when the client asks",
		() =>
			Effect.gen(function* () {
				const { transcript, settings, home } = yield* setup;
				yield* Effect.promise(() =>
				...
				const [refreshed] = yield* Effect.all(
					[service.refreshRates, service.refreshRates],
					{
						concurrency: 2,
					},
				);
	...
	it.live(
		"does not orphan an in-flight scan when its first caller is interrupted",
		() =>
			Effect.gen(function* () {
				...
				const service = yield* UsageService.make.pipe(
					Effect.provide(
						serviceLayers({
							prefix: "usage-service-interruption-test",
							home,
							settings,
						}),
					),
				...
				for (let interruptAt = 1; interruptAt <= 31; interruptAt += 1) {
					...
					const input: UsageSummaryInput = {
						...
						untilDay: UsageDay.make(
							`2026-08-${String(interruptAt).padStart(2, "0")}`,
						),
					};
