...
const decodeRatesCache = Schema.decodeUnknownEffect(
	Schema.fromJsonString(
		RatesCacheFile as unknown as Schema.Codec<typeof RatesCacheFile.Type>,
	),
);
...
const ScanCacheJson = Schema.fromJsonString(
	Schema.Unknown as unknown as Schema.Codec<unknown>,
);
...
export const make = Effect.gen(function* () {
	...
	const ensureRates = (force: boolean) =>
		ratesLock.withPermit(loadRates(force));
	...
	const resolveTranscriptDirs = Effect.fn("UsageService.resolveTranscriptDirs")(
	...
	const persistScanCache = Effect.fn("UsageService.persistScanCache")(
		function* () {
			...
			yield* encodeScanCacheFile(encodeScanCache(fileCache)).pipe(
				Effect.flatMap((serialized) =>
					fileSystem.writeFileString(scanCachePath, serialized),
				),
				Effect.map(() => {
					cacheDirty = false;
				}),
	...
	const scanSummary = Effect.fn("UsageService.scanSummary")(function* (
		...
		if (input.resolution === "hour") {
			const sinceTime =
				input.sinceTime === undefined
					? Option.none()
					: DateTime.make(input.sinceTime);
			const untilTime =
				input.untilTime === undefined
					? Option.none()
					: DateTime.make(input.untilTime);
		...
		const windowStartMs =
			(hourlyWindow?.sinceTimeMs ?? DateTime.toEpochMillis(windowStart.value)) -
			MTIME_SLACK_MS;
	...
	const inflightScans = new Map<
		string,
		Deferred.Deferred<UsageSummary, UsageReadError>
	>();
