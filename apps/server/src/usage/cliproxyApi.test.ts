...
const decodeRequest = Schema.decodeUnknownSync(
	Schema.fromJsonString(RequestBody),
);
...
describe("CLIProxyAPI built-in management API", () => {
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				expect(
					result.map((account) => account.usageLimits.resetCredits),
				).toEqual([
					{
						availableCount: 2,
						nextCreditId: "first",
				...
				expect(
					calls.find((request) => request.body?.auth_index === "b")?.body
	...
	it.effect(
		"isolates a failed account and never publishes upstream error bodies",
		() =>
			Effect.gen(function* () {
				const test = fixture({
					upstream: (request) =>
	...
	it.effect("maps Claude scoped windows without a scheduler plugin", () =>
		Effect.gen(function* () {
			...
			const result = yield* api.readAccounts(config);
			expect(
				result[0]?.usageLimits.windows.map((window) => [
					window.id,
					window.usedPercent,
				]),
			).toEqual([
	...
	it.effect(
		"pins redemption to the displayed credit and clears only that account's cooldown",
		() =>
			Effect.gen(function* () {
				const test = fixture();
				const api = yield* test.api;
				expect(yield* api.consume(config, "second.json", "credit-b")).toEqual({
					outcome: "reset",
				});
				expect(yield* api.consume(config, "second.json", "credit-b")).toEqual({
					outcome: "reset",
	...
	for (const [code, outcome] of [
		...
		it.effect(`reports ${code} accurately`, () =>
			Effect.gen(function* () {
				...
				expect(
					test.requests.some((request) =>
						request.path.endsWith("/reset-quota"),
					),
				).toBe(code === "already_redeemed");
			}),
		);
	}
	...
	it.effect("skips disabled accounts and rejects redemption on them", () =>
		Effect.gen(function* () {
			...
			expect(
				test.requests.every((request) => request.path.endsWith("/auth-files")),
			).toBe(true);
		}),
	);

	it.effect(
		"keeps the same redemption id after an uncertain upstream failure",
		() =>
			Effect.gen(function* () {
				let attempts = 0;
				const test = fixture({
				...
				expect(
					(yield* api
						.consume(config, "first.json", "credit")
	...
	it.effect("rejects unknown accounts without forwarding a redemption", () =>
		Effect.gen(function* () {
			...
			const result = yield* api
				.consume(config, "missing.json", "credit")
				.pipe(Effect.result);
