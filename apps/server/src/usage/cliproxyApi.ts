					...
					Schema.NullOr(
						Schema.Struct({
							model: Schema.optional(
								Schema.NullOr(Schema.Struct({ display_name: Schema.String })),
							),
						}),
					),
				),
...
const decodeCreditList = Schema.decodeUnknownEffect(
	Schema.fromJsonString(CreditList),
);
const decodeClaudeUsage = Schema.decodeUnknownEffect(
	Schema.fromJsonString(ClaudeUsage),
);
const decodeCodexUsage = Schema.decodeUnknownEffect(
	Schema.fromJsonString(CodexUsage),
);
...
export function creditRedeemRequestId(
	accountId: string,
	creditId: string,
): string {
...
export const makeCliproxyApi = Effect.gen(function* () {
	...
	const management = Effect.fn("CliproxyApi.management")(function* (
		...
		const url = yield* Effect.try({
			try: () => new URL(`/v0/management/${path}`, config.url).toString(),
				...
				new UsageLimitSourceError({ detail: "The hub URL is not valid." }),
		});
		const request = (
			body === undefined
				...
				Effect.mapError(
					() =>
						new UsageLimitSourceError({
							detail: "The hub management request failed.",
						}),
				),
			);
		return response;
	...
	const readAccount = Effect.fn("CliproxyApi.readAccount")(function* (
		...
		const base = {
			...
			driver: ProviderDriverKind.make(
				account.provider === "codex" ? "codex" : "claudeAgent",
			),
		...
		const read = Effect.gen(function* () {
			...
			return {
				...base,
				plan: codexPlanLabel(
					usage.plan_type ?? account.id_token?.chatgpt_plan_type,
				),
				usageLimits: {
									...
									...(next
										? {
												...
												nextExpiresAt: DateTime.formatIso(
													DateTime.makeUnsafe(next.expires_at),
												),
											}
										: {}),
								},
	...
	> {
		const accounts = yield* authFiles(config).pipe(
			Effect.mapError(
				() =>
					new UsageLimitSourceError({
						detail: "The hub could not list accounts.",
					}),
			),
		);
	...
	const consume = Effect.fn("CliproxyApi.consume")(function* (
		...
		const operation = Effect.gen(function* () {
			const account = (yield* authFiles(config)).find(
