...
describe("usage pricing", () => {
	...
	it("uses custom token rates ahead of public and provider-reported costs", () => {
		...
		for (const reportedCostUsd of [null, 99]) {
			expect(
				priceUsage(table, "example-model", totals, reportedCostUsd, overrides),
			).toEqual({
				costUsd: 13.5,
				costSource: "modelPriced",
			});
		}
		expect(cacheSavingsUsd(table, "example-model", totals, overrides)).toBe(
			1.5,
		);
	});
	...
	it("preserves explicit zero rates and matches only the exact trimmed model ID", () => {
		...
		expect(
			priceUsage(table, " vendor/example-model[1m] ", totals, 99, overrides),
		).toEqual({
			costUsd: 0,
			costSource: "modelPriced",
		});
		for (const model of [
			...
			expect(priceUsage(table, model, totals, null, overrides).costSource).toBe(
				"unpriced",
			);
	...
	it("keeps the canonical Fable rate separate from DeepInfra in either order", () => {
		...
		for (const entries of [
			...
			expect(lookupRate(table, "claude-fable-5")?.cacheReadCostPerToken).toBe(
				1e-6,
			);
			expect(
				lookupRate(table, "deepinfra/anthropic/claude-fable-5")
	...
	it("leaves an ambiguous bare name unpriced", () => {
		...
		expect(
			lookupRate(table, "provider-a/example-model")?.inputCostPerToken,
		).toBe(1);
		expect(
			lookupRate(table, "provider-b/example-model")?.inputCostPerToken,
		).toBe(3);
