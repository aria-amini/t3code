...
function position(
	overrides: Partial<CachedFile["position"]> = {},
): CachedFile["position"] {
	return {
		resumeOffset: 120,
		guardLength: 64,
...
function cacheWith(
	entries: readonly [string, number, readonly UsageRecord[]][],
): ScanCache {
...
describe("scan cache round trip", () => {
	it("restores records unchanged", () => {
		const original = cacheWith([
			[
				"/a.jsonl",
				100,
				[record(), record({ dedupeKey: "msg_2:", model: "claude-opus-5" })],
			],
			[
				"/b.jsonl",
				200,
				[record({ sessionId: "session-b", reportedCostUsd: 1.5 })],
			],
		]);
		original.set("/grok.jsonl", {
			...
			tailRecords: [
				record({ provider: "grok", model: "grok-4.5-build", dedupeKey: null }),
			],
		...
		original.set("/codex.jsonl", {
			...
			provider: "codex",
			records: [
				record({ provider: "codex", model: "gpt-5.2-codex", dedupeKey: null }),
			],
			tailRecords: [],
			position: position({
				codexState: {
	...
	it("drops an entry whose persisted parse state is corrupt", () => {
		...
		expect(
			decodeScanCache(JSON.parse(JSON.stringify(poisoned))).has("/a.jsonl"),
		).toBe(false);
	});
		...
		const encoded = encodeScanCache(
			cacheWith([
				[
					"/a.jsonl",
					100,
					[record(), record({ dedupeKey: "msg_2:" }), record()],
				],
			]),
		);
	...
	it("skips malformed file entries but keeps good ones", () => {
		const encoded = encodeScanCache(
			cacheWith([["/good.jsonl", 100, [record()]]]),
		);
		const withJunk = {
			...encoded,
			files: {
				...encoded.files,
				"/bad.jsonl": { s: "nope", m: 1, p: "claude", r: [] },
			},
		};
	...
	it("drops the whole entry when any row is corrupt, forcing a cold re-parse", () => {
		...
		const encoded = encodeScanCache(
			cacheWith([
				["/a.jsonl", 100, [record(), record({ dedupeKey: "msg_2:" })]],
			]),
		);
		...
		const poisoned = {
			...
			files: {
				"/a.jsonl": {
					...
					r: [
						rows[0]!,
						[...rows[1]!.slice(0, 3), "not-a-number", ...rows[1]!.slice(4)],
					],
				},
			},
		};
