...
async function guardMatches(
	...
	try {
		...
		return (
			bytesRead === position.guardLength && fnv1a(window) === position.guardHash
		);
	} catch {
		return false;
	}
...
export async function readTranscriptRecords(
	...
	try {
		...
		const toLineString = (lineBuffer: Buffer): string => {
			const content =
				lineBuffer.length > 0 &&
				lineBuffer[lineBuffer.length - 1] === CARRIAGE_RETURN
		...
		for await (const chunk of stream) {
			...
			const buffer: Buffer =
				pendingChunks.length === 0
					? chunk
					: Buffer.concat([...pendingChunks, chunk]);
			pendingChunks = [];
			let lineStart = 0;
			for (;;) {
			...
			if (lineStart < buffer.length)
				pendingChunks.push(buffer.subarray(lineStart));
		}
