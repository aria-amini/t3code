...
describe("parseClaudeLine", () => {
	it("extracts token totals and a dedupe key", () => {
		const record = parseClaudeLine(
			claudeLine({ messageId: "msg_1", contentType: "text" }),
		);
	...
	it("gives every content block of one message the same dedupe key", () => {
		...
		const text = parseClaudeLine(
			claudeLine({ messageId: "msg_2", contentType: "text" }),
		);
		const toolUse = parseClaudeLine(
			claudeLine({ messageId: "msg_2", contentType: "tool_use" }),
		);
...
describe("parseCodexLine", () => {
	...
	describe("forked rollouts", () => {
		...
		it("keeps the child session id over copied ancestor metas", () => {
			const state = initialCodexScanState();
			parseCodexLine(
				meta({ id: "child", timestamp: "2026-08-01T05:00:00.000Z" }),
				state,
			);
			parseCodexLine(
				meta({ id: "parent", timestamp: "2026-08-01T05:00:00.000Z" }),
				state,
			);
			parseCodexLine(turnContext, state);
		...
		it("drops the re-stamped copied burst and keeps the first real event", () => {
			...
			parseCodexLine(
				meta({ id: "child", timestamp: forkInstant, forkedFromId: "parent" }),
				state,
			);
			...
			expect(
				parseCodexLine(
					stamped("2026-08-01T05:00:00.001Z", tokenCount(100, 0, 10, 0)),
					state,
				),
			).toBeNull();
			expect(
				parseCodexLine(
					stamped("2026-08-01T05:00:00.002Z", tokenCount(200, 0, 20, 0)),
					state,
				),
			).toBeNull();
		...
		it("does not suppress anything in a rollout that is not a fork", () => {
			const state = initialCodexScanState();
			parseCodexLine(
				meta({ id: "root", timestamp: "2026-08-01T05:00:00.000Z" }),
				state,
			);
...
describe("parseGrokLine", () => {
	...
	function turnCompleted(overrides?: {
		...
		return JSON.stringify({
			...
			params: {
				sessionId:
					overrides?.sessionId ?? "019fec1a-12f7-72f2-9b1f-7778a00aea3c",
				update: {
	...
	it("extracts per-model totals and provider-reported cost ticks", () => {
		...
		expect(record?.reportedCostUsd).toBeCloseTo(
			230_272_000 / GROK_COST_USD_TICKS_PER_DOLLAR,
			12,
		);
	...
	it("falls back to a generic grok model when modelUsage is absent", () => {
		...
		expect(record?.dedupeKey).toBe(
			"019fec1a-12f7-72f2-9b1f-7778a00aea3c:prompt-1:grok",
		);
	});
	...
	it("pro-rates top-level cost ticks across multi-model turns without per-model ticks", () => {
		...
		const byModel = Object.fromEntries(
			records.map((record) => [record.model, record]),
		);
		expect(byModel["grok-4.5"]?.reportedCostUsd).toBeCloseTo(0.75, 12);
		expect(byModel["grok-composer-2.5-fast"]?.reportedCostUsd).toBeCloseTo(
			0.25,
			12,
		);
		const sum =
	...
	it("pro-rates aggregate cost when a zero-token sibling carries costUsdTicks: 0", () => {
		...
		expect(records).toHaveLength(2);
		expect(records.every((record) => record.model !== "empty-sibling")).toBe(
			true,
		);
		const byModel = Object.fromEntries(
			records.map((record) => [record.model, record]),
	...
	it("ignores non-turn lines and empty usage", () => {
		expect(
			parseGrokLine(JSON.stringify({ method: "session/update", params: {} })),
		).toEqual([]);
