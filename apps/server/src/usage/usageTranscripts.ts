...
export function parseGrokLine(line: string): readonly UsageRecord[] {
	...
	const sessionId =
		typeof paramsRecord["sessionId"] === "string"
			? paramsRecord["sessionId"]
			: "";
	const promptId =
		typeof updateRecord["prompt_id"] === "string"
			? updateRecord["prompt_id"]
			: null;
	...
	if (typeof meta === "object" && meta !== null) {
		const agentTimestampMs = (meta as Record<string, unknown>)[
			"agentTimestampMs"
		];
		if (
	...
	const remainingCostUsd =
		topLevelCostUsd === null
			? null
			: Math.max(0, topLevelCostUsd - usedTickedCostUsd);
	...
	for (const entry of modelEntries) {
		...
		if (
			...
			reportedCostUsd =
				remainingCostUsd * (totalTokens(totals) / untickedTokenDenominator);
		}
