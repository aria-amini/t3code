	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				const statusCwd =
					location === "nested" ? path.join(cwd, "nested") : cwd;
				...
				for (let poll = 0; poll < 3; poll++) {
					...
					if (Result.isFailure(result))
						assert.include(result.failure.detail, "index is locked");
				}
...
it.effect(
	"uses stable diagnostics for every parsed non-repository command",
	() => {
		...
		const spawner = ChildProcessSpawner.make((command) =>
			Effect.sync(() => {
				...
				commands.push({
					args: command.args,
					...(command.options.env?.LC_ALL
						? { lcAll: command.options.env.LC_ALL }
						: {}),
				});
...
it.effect(
	"invalidates origin remote cache when a driver mutation adds origin",
	() =>
		Effect.gen(function* () {
			const driver = yield* GitVcsDriver.GitVcsDriver;
...
it.effect(
	"re-reads origin remote status after cache TTL expiry and bypassed invalidation",
	() =>
		Effect.gen(function* () {
			const driver = yield* GitVcsDriver.GitVcsDriver;
			const cwd = yield* makeTmpDir();
			...
			assert.equal(
				(yield* driver.statusDetailsLocal(cwd)).hasOriginRemote,
				false,
			);
		...
		Effect.gen(function* () {
			...
			const countingSpawner = ChildProcessSpawner.make((command) =>
				Effect.gen(function* () {
					...
					yield* Ref.update(spawnedArgs, (current) => [
						...current,
						command.args,
					]);
					...
					const shouldDelay =
						isWorktreeScan &&
						(yield* Ref.getAndSet(delayFirstWorktreeScan, false));
					if (shouldDelay) {
					...
					return isRemoteNamesScan
						? ChildProcessSpawner.makeHandle({
								...
								exitCode: handle.exitCode.pipe(
									Effect.tap(() =>
										Deferred.succeed(remoteNamesScanCompleted, undefined),
									),
								),
							})
						: handle;
...
it.effect("retries an in-flight ref snapshot invalidated by a mutation", () =>
	Effect.scoped(
		Effect.gen(function* () {
				...
				Effect.gen(function* () {
					...
					return scan === 1
						? ChildProcessSpawner.makeHandle({
								...
								exitCode: handle.exitCode.pipe(
									Effect.tap(() =>
										Deferred.succeed(firstRefScanCompleted, undefined),
									),
								),
							})
						: handle;
			...
			assert.isTrue(
				refs.refs.some((ref) => ref.name === "feature/during-refresh"),
			);
...
it.effect(
	"invalidates a ref snapshot when a mutation fails after changing Git",
	() =>
		Effect.scoped(
			Effect.gen(function* () {
				const delegate = yield* ChildProcessSpawner.ChildProcessSpawner;
				const partiallyFailingSpawner = ChildProcessSpawner.make((command) =>
					Effect.gen(function* () {
						...
						if (
							...
							command.args[1] === "feature/partial-failure"
						) {
							const handle = yield* delegate.spawn(command);
				...
				yield* driver
					.createRef({ cwd, refName: "feature/partial-failure" })
					.pipe(Effect.flip);

				const refs = yield* driver.listRefs({ cwd });
				assert.isTrue(
					refs.refs.some((ref) => ref.name === "feature/partial-failure"),
				);
			}),
		).pipe(
			Effect.provide(
...
it.effect(
	"marks the current branch when worktree metadata is unavailable",
	() =>
		Effect.scoped(
			Effect.gen(function* () {
				const delegate = yield* ChildProcessSpawner.ChildProcessSpawner;
				const incompleteMetadataSpawner = ChildProcessSpawner.make((command) =>
					Effect.gen(function* () {
						...
						const isWorktreeRoot =
							...
							command.args.includes("--show-toplevel");
				...
				assert.isTrue(
					refs.refs.find((ref) => ref.name === initialBranch)?.current,
				);
			}),
		).pipe(
			Effect.provide(
...
it.effect(
	"ignores worktree metadata for directories that no longer exist",
	() =>
		Effect.scoped(
			Effect.gen(function* () {
				const delegate = yield* ChildProcessSpawner.ChildProcessSpawner;
...
it.effect("refreshes the current branch after an external checkout", () =>
	Effect.scoped(
		Effect.gen(function* () {
			...
			assert.isTrue(
				initialRefs.refs.find((ref) => ref.name === initialBranch)?.current,
			);
			...
			assert.isTrue(
				refreshedRefs.refs.find((ref) => ref.name === "external-checkout")
					?.current,
			);
			assert.isFalse(
				refreshedRefs.refs.find((ref) => ref.name === initialBranch)?.current,
			);
		}),
	).pipe(Effect.provide(TestLayer)),
);
...
it.effect("backs off failed upstream refreshes across linked worktrees", () =>
	Effect.scoped(
		Effect.gen(function* () {
			...
			const initialBranch = (yield* runGit(cwd, [
				"branch",
				"--show-current",
			])).stdout.trim();
			...
			const rootCommonDir = (yield* runGit(cwd, [
				"rev-parse",
				"--git-common-dir",
			])).stdout.trim();
			...
			assert.equal(
				...
				NodeFS.realpathSync.native(
					pathService.resolve(worktreePath, linkedCommonDir),
				),
			);
...
it.layer(TestLayer)("GitVcsDriver core integration", (it) => {
	...
	describe("structured errors", () => {
		...
		it.effect(
			"does not retain git arguments or stderr in command failures",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					const driver = yield* GitVcsDriver.GitVcsDriver;
		...
		it.effect(
			"recovers a structurally identified missing cwd as a non-repository",
			() =>
				Effect.gen(function* () {
					const parent = yield* makeTmpDir();
					const pathService = yield* Path.Path;
		...
		it.effect(
			"does not wrap a remove-worktree command failure in a synthetic error",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					const pathService = yield* Path.Path;
	...
	describe("review diff previews", () => {
		it.effect(
			"drops an unterminated path from truncated NUL-separated git output",
			() =>
				Effect.sync(() => {
		...
		it.effect(
			"keeps the final path when NUL-separated git output is complete",
			() =>
				Effect.sync(() => {
					const paths = splitNullSeparatedGitStdoutPaths({
		...
		it.effect(
			"honors whitespace filtering for worktree and branch previews",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					const { initialBranch } = yield* initRepoWithCommit(cwd);
					...
					assert.isNotEmpty(
						included.sources.find((source) => source.kind === "working-tree")
							?.diff,
					);
					assert.isNotEmpty(
						included.sources.find((source) => source.kind === "branch-range")
							?.diff,
					);
					assert.strictEqual(
						ignored.sources.find((source) => source.kind === "working-tree")
							?.diff,
						"",
					);
					assert.strictEqual(
						ignored.sources.find((source) => source.kind === "branch-range")
							?.diff,
						"",
					);
				}),
		...
		it.effect(
			"keeps a/ and b/ patch prefixes when the repository disables them",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					const { initialBranch } = yield* initRepoWithCommit(cwd);
					...
					const workingTree = preview.sources.find(
						(source) => source.kind === "working-tree",
					)?.diff;
					const branchRange = preview.sources.find(
		...
		it.effect(
			"attributes working-tree filesystem failures to the failing operation",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					yield* initRepoWithCommit(cwd);
					...
					assert.deepInclude(error, {
						_tag: "GitCommandError",
						operation:
							"GitVcsDriver.getReviewDiffFileContents.workingTree.fs.realPath",
						command: "fs.realPath",
						cwd,
		...
		it.effect(
			"loads new and deleted files without reading their missing side",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					yield* initRepoWithCommit(cwd);
		...
		it.effect(
			"loads merge-base and head contents for branch diff expansion",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					const { initialBranch } = yield* initRepoWithCommit(cwd);
	...
	describe("repository status", () => {
		...
		it.effect(
			"reports default-branch delta separately from upstream delta",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					const remote = yield* makeTmpDir("git-vcs-driver-remote-");
		...
		it.effect(
			"reports remote divergence without reading working-tree details",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					const remote = yield* makeTmpDir("git-vcs-driver-remote-");
		...
		it.effect("reports remote status on unborn HEAD without failing", () =>
			Effect.gen(function* () {
				...
				const driver = yield* GitVcsDriver.GitVcsDriver;
				yield* driver.initRepo({ cwd });
				const initialBranch = yield* git(cwd, [
					"symbolic-ref",
					"--short",
					"HEAD",
				]);
		...
		it.effect(
			"can read cached remote divergence without fetching upstream",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					const remote = yield* makeTmpDir("git-vcs-driver-remote-");
		...
		it.effect(
			"uses origin HEAD for default-branch detection with a non-origin upstream",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
		...
		it.effect("makes background upstream status fetches non-interactive", () =>
			Effect.gen(function* () {
				...
				yield* Effect.gen(function* () {
					...
					assert.deepEqual(
						(yield* fileSystem.readFileString(sshLogPath))
							.trim()
							.split(/\r?\n/),
						[
		...
		it.effect(
			"reuses the no-upstream fallback ahead count for default-branch delta",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					const remote = yield* makeTmpDir("git-vcs-driver-remote-");
		...
		it.effect(
			"reports combined staged and unstaged edits to the same file",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					yield* initRepoWithCommit(cwd);
					...
					assert.equal(status.isRepo, true);
					assert.equal(status.hasWorkingTreeChanges, true);
					const file = status.workingTree.files.find(
						(f) => f.path === "feature.ts",
					);
					assert.ok(file);
	...
	describe("refName operations", () => {
		it.effect("optionally includes remote refs that match local branches", () =>
			Effect.gen(function* () {
				...
				assert.equal(
					deduplicated.refs.some(
						(ref) => ref.name === `origin/${initialBranch}`,
					),
					false,
				);
		...
		it.effect(
			"marks the origin default ref as default when no local copy exists",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					const remote = yield* makeTmpDir("git-vcs-driver-remote-");
					...
					const refs = yield* driver.listRefs({ cwd });
					const remoteDefault = refs.refs.find(
						(ref) => ref.name === `origin/${initialBranch}`,
					);
		...
		it.effect("creates, checks out, renames, and lists refs", () =>
			Effect.gen(function* () {
				...
				const refs = yield* driver.listRefs({ cwd });
				assert.equal(
					refs.refs.find((refName) => refName.name === "feature/renamed")
						?.current,
					true,
				);
			}),
		...
		it.effect(
			"returns the existing refName when rename source and target match",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					yield* initRepoWithCommit(cwd);
	...
	describe("worktree operations", () => {
			...
			() =>
				Effect.gen(function* () {
					...
					if (typeof listedPath !== "string") {
						return assert.fail(
							"expected the linked branch to include its worktree path",
						);
					}
					assert.equal(
			...
			Effect.gen(function* () {
				...
				assert.equal(
					yield* fileSystem.exists(
						pathService.join(worktreePath, "shared", "SHARED.md"),
					),
					true,
				);
			}),
		...
		it.effect(
			"allows worktree removal to run longer than the default command timeout",
			() =>
				Effect.gen(function* () {
					const delegate = yield* ChildProcessSpawner.ChildProcessSpawner;
		...
		it.effect(
			"prunes stale registrations when removing an already-gone worktree",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					const { initialBranch } = yield* initRepoWithCommit(cwd);
					...
					const registered = yield* git(cwd, [
						"worktree",
						"list",
						"--porcelain",
					]);
	...
	describe("remote operations", () => {
		it.effect(
			"ensureRemote reuses an existing remote across ssh/https transport variants",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					yield* initRepoWithCommit(cwd);
	...
	describe("remote operations", () => {
		it.effect("creates a worktree from the latest fetched remote commit", () =>
			Effect.gen(function* () {
				...
				const beforeFetch = yield* git(cwd, [
					"rev-parse",
					`refs/remotes/origin/${initialBranch}`,
				]);
				...
				assert.equal(
					yield* driver.readConfigValue(
						worktreePath,
						"branch.t3code/fetched-origin.gh-merge-base",
					),
					initialBranch,
				);
				assert.equal(
					yield* driver.readConfigValue(
						worktreePath,
						"branch.t3code/fetched-origin.remote",
					),
					null,
				);
		...
		it.effect(
			"pushes with upstream setup and skips when already up to date",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					const remote = yield* makeTmpDir("git-remote-");
					...
					yield* (yield* GitVcsDriver.GitVcsDriver).commit(
						cwd,
						"Add feature",
						"",
					);
					...
					const pushed =
						yield* (yield* GitVcsDriver.GitVcsDriver).pushCurrentBranch(
							cwd,
							null,
						);
		...
		it.effect(
			"allows pushes to run longer than the default command timeout",
			() =>
				Effect.gen(function* () {
					const delegate = yield* ChildProcessSpawner.ChildProcessSpawner;
		...
		it.effect(
			"publishes a branch tracking its base under its own name, not the base",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					const remote = yield* makeTmpDir("git-remote-");
					...
					assert.equal(
						yield* git(remote, ["log", "-1", "--pretty=%s", "feature/x"]),
						"Add feature",
					);
					...
					assert.equal(
						yield* driver.readConfigValue(
							cwd,
							"branch.feature/x.gh-merge-base",
						),
						"dev",
					);
				}),
		...
		it.effect(
			"keeps a recorded merge base when publishing a tracked branch",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					const remote = yield* makeTmpDir("git-remote-");
					...
					yield* git(cwd, [
						"config",
						"branch.feature/y.gh-merge-base",
						"release/v2",
					]);
					yield* writeTextFile(cwd, "feature.txt", "feature\n");
					yield* driver.prepareCommitContext(cwd);
		...
		it.effect(
			"still pushes a git-mangled tracking alias to its upstream head",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
					const remote = yield* makeTmpDir("git-remote-");
		...
		it.effect(
			"pushes to the requested remote instead of the primary remote",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTmpDir();
