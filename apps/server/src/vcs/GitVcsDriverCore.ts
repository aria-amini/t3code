...
const NON_REPOSITORY_STATUS_DETAILS =
	Object.freeze<GitVcsDriver.GitStatusDetails>({
		isRepo: false,
...
function statusUpstreamRefreshFailureCooldown(
	consecutiveFailures: number,
): Duration.Duration {
	const exponent = Math.max(0, consecutiveFailures - 1);
	const cooldownMs =
		Duration.toMillis(STATUS_UPSTREAM_REFRESH_FAILURE_BASE_COOLDOWN) *
		Math.pow(2, exponent);
	return Duration.min(
		Duration.millis(cooldownMs),
		STATUS_UPSTREAM_REFRESH_FAILURE_MAX_COOLDOWN,
	);
}
...
function parsePorcelainPath(line: string): string | null {
	...
	if (
		!(line.startsWith("1 ") || line.startsWith("2 ") || line.startsWith("u "))
	) {
		return null;
	}
...
function filterBranchesForListQuery(
	...
	return refs.filter((refName) =>
		refName.name.toLowerCase().includes(normalizedQuery),
	);
}
...
function paginateBranches(input: {
	...
	const nextCursor =
		cursor + refs.length < totalCount ? cursor + refs.length : null;
...
function parseDefaultBranchFromRemoteHeadRef(
	value: string,
	remoteName: string,
): string | null {
...
function trace2ChildKey(record: Record<string, unknown>): string | null {
	...
	return typeof hookName === "string" && hookName.trim().length > 0
		? hookName.trim()
		: null;
}
...
> {
	...
	const hookStartByChildKey = new Map<
		string,
		{ hookName: string; startedAtMs: number }
	>();
	...
	const handleTraceLine = Effect.fn("handleTraceLine")(function* (
		line: string,
	) {
		const trimmedLine = line.trim();
		if (trimmedLine.length === 0) {
				...
				Effect.uninterruptible(
					Ref.modify(traceTailState, ({ processedChars, remainder }) => {
						...
						Effect.flatMap((lines) =>
							Effect.forEach(lines, handleTraceLine, { discard: true }),
						),
					),
				),
			),
	...
	function* () {
		...
		const executeRaw: GitVcsDriver.GitVcsDriver["Service"]["execute"] =
			Effect.fnUntraced(function* (input) {
				...
				const timeoutMs =
					input.timeoutMs === undefined ? DEFAULT_TIMEOUT_MS : input.timeoutMs;
					...
					const [stdout, stderr, exitCode] = yield* Effect.all(
						[
							...
							input.stdin === undefined
								? Effect.void
								: Stream.run(
										Stream.encodeText(Stream.make(input.stdin)),
										child.stdin,
									).pipe(
										Effect.mapError(
											(cause) =>
						...
						Effect.map(
							([stdout, stderr, exitCode]) =>
								[stdout, stderr, exitCode] as const,
						),
					);
		...
		const resolveAvailableBranchName = Effect.fn("resolveAvailableBranchName")(
		...
		const resolveCurrentUpstream = Effect.fn("resolveCurrentUpstream")(
			function* (cwd: string) {
		...
		const resolveRepositoryPathsUncached = Effect.fn(
			"resolveRepositoryPathsUncached",
		)(function* (cwd: string) {
		...
		const normalizeRepositoryPathsCacheKey = (cwd: string) =>
		...
		const resolveRepositoryPaths = (cwd: string, refresh = false) => {
			...
			return Cache.get(
				refresh ? repositoryPathsRefreshCache : repositoryPathsCache,
				cacheKey,
			);
		};
				...
				Effect.gen(function* () {
					...
					return yield* executeGit(
						...
						Effect.map((result) => {
							...
							return parseDefaultBranchFromRemoteHeadRef(
								result.stdout,
								"origin",
							);
						}),
		...
		const statusRemoteRefreshFailureKey = (
		...
		const recordStatusRemoteRefreshFailure = (
			...
			if (
				statusRemoteRefreshFailureCounts.size >
		...
		const clearStatusRemoteRefreshFailures = (
			cacheKey: StatusRemoteRefreshCacheKey,
		) => {
			statusRemoteRefreshFailureCounts.delete(
				statusRemoteRefreshFailureKey(cacheKey),
			);
		};
		const refreshStatusRemoteCacheEntry = Effect.fn(
			"refreshStatusRemoteCacheEntry",
		)(function* (cacheKey: StatusRemoteRefreshCacheKey) {
			return yield* fetchRemoteForStatus(
				cacheKey.gitCommonDir,
				cacheKey.remoteName,
			).pipe(
				Effect.tap(() =>
					Effect.sync(() => clearStatusRemoteRefreshFailures(cacheKey)),
				),
				Effect.tapError(() =>
					Effect.sync(() => recordStatusRemoteRefreshFailure(cacheKey)),
				),
				...
				timeToLive: (exit, cacheKey) =>
					Exit.isSuccess(exit)
						...
						: statusUpstreamRefreshFailureCooldown(
								statusRemoteRefreshFailureCounts.get(
									statusRemoteRefreshFailureKey(cacheKey),
								) ?? 1,
							),
			},
		...
		const refreshStatusUpstreamIfStale = Effect.fn(
		...
		const remoteBranchExists: GitVcsDriver.GitVcsDriver["Service"]["remoteBranchExists"] =
			(input) =>
				executeGit(
					...
					[
						...
						"--verify",
						"--quiet",
						`refs/remotes/${input.remoteName}/${input.refName}`,
					],
					{
						allowNonZeroExit: true,
					},
		...
		const remoteExists: GitVcsDriver.GitVcsDriver["Service"]["remoteExists"] = (
			input,
		) =>
			executeGit(
		...
		const resolvePublishBranchName = Effect.fn("resolvePublishBranchName")(
			function* (cwd: string, branchName: string) {
		...
		const resolvePrimaryRemoteName = Effect.fn("resolvePrimaryRemoteName")(
			function* (cwd: string) {
		...
		const resolvePushRemoteName = Effect.fn("resolvePushRemoteName")(function* (
			...
			return yield* resolvePrimaryRemoteName(cwd).pipe(
				Effect.orElseSucceed(() => null),
			);
		});

		const ensureRemote: GitVcsDriver.GitVcsDriver["Service"]["ensureRemote"] =
			Effect.fn("ensureRemote")(function* (input) {
		...
		const resolveBaseBranchForNoUpstream = Effect.fn(
			"resolveBaseBranchForNoUpstream",
		)(function* (cwd: string, refName: string) {
			...
			for (const candidate of candidates) {
				...
				const remotePrefix =
					primaryRemoteName && primaryRemoteName !== "origin"
						? `${primaryRemoteName}/`
						: null;
		...
		const computeAheadCountAgainstBase = Effect.fn(
		...
		const readStatusDetailsRemote = Effect.fn("readStatusDetailsRemote")(
			function* (cwd: string) {
				...
				if (branchResult.exitCode !== 0) {
					...
					branch =
						branchValue.length > 0 && branchValue !== "HEAD"
							? branchValue
							: null;
				}
				const upstream = yield* resolveCurrentUpstream(cwd);
				...
				if (upstreamRef) {
					...
					if (divergence.exitCode === 0) {
						...
						aheadCount = Number.isFinite(parsedAhead)
							? Math.max(0, parsedAhead)
							: 0;
						behindCount = Number.isFinite(parsedBehind)
							? Math.max(0, parsedBehind)
							: 0;
					}
				} else if (branch) {
		...
		const readStatusDetailsLocal = Effect.fn("readStatusDetailsLocal")(
			function* (cwd: string) {
				...
				const [numstatStdout, defaultBranch, hasPrimaryRemote] =
					yield* Effect.all(
						[
										...
										return Effect.map(
											...
											([unstagedStdout, stagedStdout]) => {
												...
												return Array.from(map.entries())
													.map(
														([p, s]) => `${s.insertions}\t${s.deletions}\t${p}`,
													)
													.join("\n");
											},
										);
							...
							statusCacheKey
								? Cache.get(defaultBranchCache, statusCacheKey).pipe(
										Effect.orElseSucceed(() => null),
									)
								: resolveDefaultBranchName(cwd, "origin").pipe(
										Effect.orElseSucceed(() => null),
									),
							statusCacheKey
								? Cache.get(originExistsCache, statusCacheKey).pipe(
										Effect.orElseSucceed(() => false),
									)
								: originRemoteExists(cwd).pipe(
										Effect.orElseSucceed(() => false),
									),
						],
						{ concurrency: "unbounded" },
		...
		const statusDetails: GitVcsDriver.GitVcsDriver["Service"]["statusDetails"] =
			Effect.fn("statusDetails")(function* (cwd) {
		...
		const prepareCommitContext: GitVcsDriver.GitVcsDriver["Service"]["prepareCommitContext"] =
			Effect.fn("prepareCommitContext")(function* (cwd, filePaths) {
				if (filePaths && filePaths.length > 0) {
					yield* runGit("GitVcsDriver.prepareCommitContext.reset", cwd, [
						"reset",
					]).pipe(
						Effect.catchTags({
		...
		const commit: GitVcsDriver.GitVcsDriver["Service"]["commit"] = Effect.fn(
			"commit",
		)(function* (cwd, subject, body, options?: GitVcsDriver.GitCommitOptions) {
			...
			const commitSha = yield* runGitStdout(
				"GitVcsDriver.commit.revParseHead",
				cwd,
				["rev-parse", "HEAD"],
			).pipe(Effect.map((stdout) => stdout.trim()));
		...
		const pushCurrentBranch: GitVcsDriver.GitVcsDriver["Service"]["pushCurrentBranch"] =
			Effect.fn("pushCurrentBranch")(function* (cwd, fallbackBranch, options) {
				...
				const hasNoLocalDelta =
					details.aheadCount === 0 && details.behindCount === 0;
				if (hasNoLocalDelta) {
					...
					const comparableBaseBranch = yield* resolveBaseBranchForNoUpstream(
						cwd,
						branch,
					).pipe(Effect.orElseSucceed(() => null));
					if (comparableBaseBranch) {
						const publishRemoteName = yield* resolvePushRemoteName(
							cwd,
							branch,
						).pipe(Effect.orElseSucceed(() => null));
		...
		const pullCurrentBranch: GitVcsDriver.GitVcsDriver["Service"]["pullCurrentBranch"] =
			Effect.fn("pullCurrentBranch")(function* (cwd) {
				const details = yield* statusDetails(cwd);
		...
		const readRangeContext: GitVcsDriver.GitVcsDriver["Service"]["readRangeContext"] =
			Effect.fn("readRangeContext")(function* (cwd, baseRef) {
		...
		const readUntrackedReviewDiffs = Effect.fn("readUntrackedReviewDiffs")(
			function* (cwd: string) {
				...
				const untrackedPaths =
					splitNullSeparatedGitStdoutPaths(untrackedResult);
		...
		const getReviewDiffPreview = Effect.fn("getReviewDiffPreview")(function* (
			...
			const dirtyDiff = [
				dirtyTrackedResult.stdout.trimEnd(),
				dirtyUntracked.diff.trimEnd(),
			]
			...
			const sources: ReviewDiffPreviewSource[] = [
				{
					...
					truncated:
						dirtyTrackedResult.stdoutTruncated || dirtyUntracked.truncated,
				},
				{
					id: "branch-range",
					kind: "branch-range",
		...
		const readReviewFileAtRevision = Effect.fn("readReviewFileAtRevision")(
			function* (
				input: ReviewDiffFileContentsInput,
				revision: string,
				relativePath: string,
		...
		const readWorkingTreeReviewFile = Effect.fn("readWorkingTreeReviewFile")(
			function* (input: ReviewDiffFileContentsInput, repositoryRoot: string) {
				...
				const [realRepositoryRoot, realTarget] = yield* Effect.all([
					...
					Effect.mapError((cause) =>
						fileError(
							"fs.realPath",
							`Could not resolve diff file '${input.newPath}'.`,
							cause,
						),
					),
				);
				...
				const info = yield* fileSystem
					...
					.pipe(
						Effect.mapError((cause) =>
							fileError(
								"fs.stat",
								`Could not inspect diff file '${input.newPath}'.`,
								cause,
							),
						),
					);
				if (info.type !== "File") {
					return yield* fileError(
						"fs.stat",
						`Diff path '${input.newPath}' is not a file.`,
					);
				}
				...
				const bytes = yield* fileSystem
					...
					.pipe(
						Effect.mapError((cause) =>
							fileError(
								"fs.readFile",
								`Could not read diff file '${input.newPath}'.`,
								cause,
							),
						),
					);
				if (bytes.includes(0)) {
					return yield* fileError(
						"fs.readFile",
						`Cannot expand binary file '${input.newPath}'.`,
					);
				}
		...
		const getReviewDiffFileContents = Effect.fn("getReviewDiffFileContents")(
		...
		const readConfigValue: GitVcsDriver.GitVcsDriver["Service"]["readConfigValue"] =
			(cwd, key) =>
				runGitStdout(
					"GitVcsDriver.readConfigValue",
					cwd,
					["config", "--get", key],
					true,
				).pipe(
		...
		const readGitRefsSnapshot = Effect.fn("readGitRefsSnapshot")(function* (
			gitCommonDir: string,
		) {
			const fetchCwd =
				path.basename(gitCommonDir) === ".git"
			...
			const remoteNames =
				remoteNamesResult.exitCode === 0
					? parseRemoteNames(remoteNamesResult.stdout)
					: [];
			if (
				remoteNamesResult.exitCode !== 0 &&
				remoteNamesResult.stderr.trim().length > 0
			) {
		...
		const setListRefsGeneration = (
			...
			if (
				listRefsGenerationByCommonDir.size > LIST_REFS_SNAPSHOT_CACHE_CAPACITY
			) {
		...
		const listRefsSnapshotCache = yield* Cache.makeWith(
			(cacheKey: GitRefsSnapshotCacheKey) =>
				readGitRefsSnapshot(cacheKey.gitCommonDir),
			{
				capacity: LIST_REFS_SNAPSHOT_CACHE_CAPACITY,
				timeToLive: (exit) =>
					Exit.isSuccess(exit) ? LIST_REFS_SNAPSHOT_CACHE_TTL : Duration.zero,
			},
		);
		...
		const resolveListRefsSnapshot = Effect.fn("resolveListRefsSnapshot")(
			function* (gitCommonDir: string, refresh: boolean) {
				while (true) {
		...
		const invalidateListRefsSnapshot = Effect.fn("invalidateListRefsSnapshot")(
			function* (cwd: string) {
				const repositoryPathsCacheKey = normalizeRepositoryPathsCacheKey(cwd);
				const repositoryPaths = yield* Cache.get(
					repositoryPathsCache,
					repositoryPathsCacheKey,
				);
				if (repositoryPaths === null) return;
				const previousGeneration = currentListRefsGeneration(
					repositoryPaths.gitCommonDir,
				);
		...
		const listRefs: GitVcsDriver.GitVcsDriver["Service"]["listRefs"] =
			Effect.fn("listRefs")(function* (input) {
				...
				const hasCurrentWorktreeBranch =
					...
					snapshot.localBranches.some(
						(ref) => ref.worktreePath === repositoryPaths.worktreeRoot,
					);
		...
		const createWorktree: GitVcsDriver.GitVcsDriver["Service"]["createWorktree"] =
			Effect.fn("createWorktree")(function* (input) {
		...
		const resolveCommit: GitVcsDriver.GitVcsDriver["Service"]["resolveCommit"] =
			Effect.fn("resolveCommit")(function* (input) {
				const commitSha = yield* runGitStdout(
			...
			Effect.fn("refreshCheckedOutBranch")(function* (input) {
				...
				if (!isAncestor) {
					...
					yield* executeGit(
						...
						{
							fallbackErrorDetail:
								"git failed to record the previous checkout commit",
						},
					);
				}
				...
				yield* executeGit(
						...
						: ["reset", "--merge", input.targetCommit],
					{
						timeoutMs: 30_000,
						fallbackErrorDetail:
							"git failed to move the checkout onto the pull request head",
					},
				);
		...
		const fetchRemote: GitVcsDriver.GitVcsDriver["Service"]["fetchRemote"] =
			Effect.fn("fetchRemote")(function* (input) {
				yield* executeGit(
		...
		const fetchRemoteBranch: GitVcsDriver.GitVcsDriver["Service"]["fetchRemoteBranch"] =
			Effect.fn("fetchRemoteBranch")(function* (input) {
		...
		const setBranchUpstream: GitVcsDriver.GitVcsDriver["Service"]["setBranchUpstream"] =
			(input) =>
		...
		const removeWorktree: GitVcsDriver.GitVcsDriver["Service"]["removeWorktree"] =
			Effect.fn("removeWorktree")(function* (input) {
		...
		const pruneWorktrees: GitVcsDriver.GitVcsDriver["Service"]["pruneWorktrees"] =
			Effect.fn("pruneWorktrees")(function* (input) {
				yield* executeGit(
		...
		const renameBranch: GitVcsDriver.GitVcsDriver["Service"]["renameBranch"] =
			Effect.fn("renameBranch")(function* (input) {
		...
		const switchRef: GitVcsDriver.GitVcsDriver["Service"]["switchRef"] =
			Effect.fn("switchRef")(function* (input) {
				...
				const refName = yield* runGitStdout(
					"GitVcsDriver.switchRef.currentBranch",
					input.cwd,
					["branch", "--show-current"],
		...
		const createRef: GitVcsDriver.GitVcsDriver["Service"]["createRef"] =
			Effect.fn("createRef")(function* (input) {
				yield* executeGit(
		...
		const initRepo: GitVcsDriver.GitVcsDriver["Service"]["initRepo"] = (
			input,
		) =>
		...
		return GitVcsDriver.GitVcsDriver.of({
			...
			pullCurrentBranch: (cwd) =>
				withListRefsInvalidation(cwd, pullCurrentBranch(cwd)),
			readRangeContext,
			...
			createWorktree: (input) =>
				withListRefsInvalidation(input.cwd, createWorktree(input)),
			...
			ensureRemote: (input) =>
				withListRefsInvalidation(input.cwd, ensureRemote(input)),
			resolvePrimaryRemoteName,
			resolveDefaultBranchName,
			fetchRemote: (input) =>
				withListRefsInvalidation(input.cwd, fetchRemote(input)),
			remoteExists,
			remoteBranchExists,
			resolveRemoteTrackingCommit,
			fetchRemoteBranch: (input) =>
				withListRefsInvalidation(input.cwd, fetchRemoteBranch(input)),
			fetchRemoteTrackingBranch: (input) =>
				withListRefsInvalidation(input.cwd, fetchRemoteTrackingBranch(input)),
			setBranchUpstream: (input) =>
				withListRefsInvalidation(input.cwd, setBranchUpstream(input)),
			removeWorktree: (input) =>
				withListRefsInvalidation(input.cwd, removeWorktree(input)),
			pruneWorktrees: (input) =>
				withListRefsInvalidation(input.cwd, pruneWorktrees(input)),
			renameBranch: (input) =>
				withListRefsInvalidation(input.cwd, renameBranch(input)),
			createRef: (input) =>
				withListRefsInvalidation(input.cwd, createRef(input)),
			switchRef: (input) =>
				withListRefsInvalidation(input.cwd, switchRef(input)),
