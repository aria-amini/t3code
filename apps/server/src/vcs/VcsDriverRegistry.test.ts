...
describe("VcsDriverRegistry", () => {
	it.effect(
		"routes directly by VCS driver kind for non-repository workflows",
		() => {
			const layer = Layer.effect(
				...
				Layer.provide(
					Layer.mock(VcsProjectConfig.VcsProjectConfig)({
						resolveKind: (input) =>
							Effect.succeed(input.requestedKind ?? "auto"),
					}),
				),
				Layer.provide(
	...
	it.effect(
		"caches repository detection for repeated resolves in the same cwd and kind",
		() => {
						...
						run: (input) =>
							Effect.sync(() => {
								...
								const normalizedArgs =
									input.args[0] === "-C" && input.args.length >= 2
										? input.args.slice(2)
										: input.args;
			...
			return Effect.gen(function* () {
				const registry = yield* VcsDriverRegistry.VcsDriverRegistry;
				const first = yield* registry.resolve({
					cwd: "/repo",
					requestedKind: "git",
				});
				const second = yield* registry.resolve({
					cwd: "/repo",
					requestedKind: "git",
				});
	...
	it.effect("detects a repository created after a negative lookup", () => {
		...
		return Effect.gen(function* () {
			...
			assert.equal(
				(yield* registry.detect({ cwd: "/repo" }))?.repository.rootPath,
				"/repo",
			);
