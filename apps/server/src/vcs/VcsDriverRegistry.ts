...
export class VcsDriverRegistry extends Context.Service<
	VcsDriverRegistry,
	{
		readonly get: (
			kind: VcsDriverKind,
		) => Effect.Effect<VcsDriver.VcsDriver["Service"], VcsError>;
		readonly detect: (
			input: VcsDriverResolveInput,
		) => Effect.Effect<VcsDriverHandle | null, VcsError>;
		readonly resolve: (
			input: VcsDriverResolveInput,
		) => Effect.Effect<VcsDriverHandle, VcsError>;
	}
...
export const make = Effect.gen(function* () {
	...
	const detectWithDriver = Effect.fn("VcsDriverRegistry.detectWithDriver")(
		function* (
			kind: VcsDriverKind,
	...
	const detectResolvedKind = Effect.fn("VcsDriverRegistry.detectResolvedKind")(
		function* (input: {
			readonly cwd: string;
			readonly requestedKind: VcsDriverKind | "auto";
		}) {
	...
	const detectionCache = yield* Cache.makeWith<
		string,
		VcsDriverHandle | null,
		VcsError
	>((key) => detectResolvedKind(parseDetectionCacheKey(key)), {
		capacity: DETECTION_CACHE_CAPACITY,
		timeToLive: Exit.match({
			onSuccess: (detected) =>
				detected === null ? Duration.zero : DETECTION_CACHE_TTL,
			onFailure: () => Duration.zero,
		}),
	});

	const detect: VcsDriverRegistry["Service"]["detect"] = Effect.fn(
	...
	const resolve: VcsDriverRegistry["Service"]["resolve"] = Effect.fn(
