...
const provideLive = <A, E, R>(
	effect: Effect.Effect<A, E, R | VcsProcess.VcsProcess>,
...
describe("VcsProcess.run", () => {
	...
	it.effect(
		"fails with VcsProcessExitError for non-zero exits by default",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"classifies API rate limits without retaining provider stderr",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"retains spawn causes without exposing process arguments in the error message",
		() =>
			Effect.gen(function* () {
				const secretArgument = "--token=super-secret-token";
				const error = yield* run({
	...
	it.effect(
		"preserves real boundary causes without manufacturing structural ones",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"truncates without the marker when truncation markers are disabled",
		() =>
			Effect.gen(function* () {
				const result = yield* run({
	...
	it.effect(
		"fails with measured byte counts when output must not be truncated",
		() =>
			Effect.gen(function* () {
				const error = yield* run({
