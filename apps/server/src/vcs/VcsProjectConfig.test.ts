...
describe("VcsProjectConfig", () => {
	it.layer(TestLayer)(
		"uses an explicit requested VCS kind before config",
		(it) => {
	...
	it.layer(TestLayer)(
		"discovers .t3code/vcs.json from nested workspaces",
		(it) => {
	...
	it.layer(TestLayer)(
		"continues to parent configs after a candidate inspect failure",
		(it) => {
			it.effect(
				"logs the failed candidate and returns the parent config",
				() => {
					...
					return Effect.gen(function* () {
						...
						assert.equal(
							error.message,
							"Failed to inspect VCS project config at " +
								failedCandidate +
								".",
						);
	...
	it.layer(TestLayer)(
		"falls back to auto when config JSON is malformed",
		(it) => {
			it.effect("returns auto and logs the failed operation and path", () => {
				...
				return Effect.gen(function* () {
					...
					assert.equal(
						error.message,
						"Failed to decode VCS project config at " +
							path.join(configDir, "vcs.json") +
							".",
					);
	...
	it.layer(TestLayer)(
		"falls back to auto when the config path cannot be read",
		(it) => {
			it.effect("retains the read failure context", () => {
				...
				return Effect.gen(function* () {
					...
					assert.equal(
						error.message,
						"Failed to read VCS project config at " + configPath + ".",
					);
	...
	it.layer(TestLayer)(
		"falls back to auto when config kind is invalid",
		(it) => {
			it.effect("returns auto", () =>
				Effect.gen(function* () {
					const fileSystem = yield* FileSystem.FileSystem;
