...
function makeTestLayer(state: {
	...
	return VcsStatusBroadcaster.layer.pipe(
		...
		Layer.provide(
			makeBackgroundPolicyLayer(() => state.backgroundWorkEnabled !== false),
		),
		Layer.provide(
			Layer.mock(GitWorkflowService.GitWorkflowService)({
				...
				remoteStatus: (_input, options) =>
					Effect.sync(() => {
						...
						state.remoteStatusRefreshUpstreamValues?.push(
...
describe("VcsStatusBroadcaster", () => {
	it.effect.skipIf(!symlinksSupported)(
		...
		() => {
			let remoteStatus: VcsStatusRemoteResult = {
				...baseRemoteStatus,
				behindCount: 2,
			};
			let pullCalls = 0;
			...
			return Effect.gen(function* () {
				...
				const status = yield* broadcaster.refreshStatus(
	...
	it.effect(
		"refreshes a loaded cwd without reusing a previous branch's PR",
		() => {
			const state = {
				currentLocalStatus: baseLocalStatus,
				currentRemoteStatus: baseRemoteStatus,
	...
	it.effect(
		"a poll that started before the turn-end refresh cannot overwrite its PR",
		() => {
			...
			return Effect.gen(function* () {
				...
				const poll = yield* broadcaster
					.refreshStatus("/repo")
					.pipe(Effect.forkScoped);
				yield* Deferred.await(firstPollStarted);
				const refresh = yield* broadcaster
	...
	it.effect(
		"an initial status read cannot overwrite an explicit refresh",
		() => {
			...
			return Effect.gen(function* () {
				const broadcaster = yield* VcsStatusBroadcaster.VcsStatusBroadcaster;
				const initial = yield* broadcaster
					.getStatus({ cwd: "/repo" })
					.pipe(Effect.forkScoped);
	...
	it.effect(
		"turn-end refresh skips a loaded cwd when background policy pauses it",
		() => {
			const state = {
				currentLocalStatus: baseLocalStatus,
				currentRemoteStatus: baseRemoteStatus,
	...
	it.effect(
		"keeps the cached snapshot unchanged when a refresh branch fails",
		() => {
			const state = {
				currentLocalStatus: baseLocalStatus,
				currentRemoteStatus: baseRemoteStatus,
			...
			return Effect.gen(function* () {
				...
				const refreshExit = yield* broadcaster
					.refreshStatus("/repo")
					.pipe(Effect.exit);
				const cached = yield* broadcaster.getStatus({ cwd: "/repo" });
	...
	it.effect("streams a local snapshot first and remote updates later", () => {
		...
		return Effect.gen(function* () {
			...
			const remoteUpdatedDeferred =
				yield* Deferred.make<VcsStatusStreamEvent>();
			yield* Stream.runForEach(
				broadcaster.streamStatus({ cwd: "/repo" }),
				(event) => {
					if (event._tag === "snapshot") {
						return Deferred.succeed(snapshotDeferred, event).pipe(
							Effect.ignore,
						);
					}
					if (event._tag === "remoteUpdated") {
						return Deferred.succeed(remoteUpdatedDeferred, event).pipe(
							Effect.ignore,
						);
					}
					return Effect.void;
	...
	it.effect(
		"loads remote status once when periodic refreshes are disabled",
		() => {
			const state = {
				currentLocalStatus: baseLocalStatus,
			...
			return Effect.gen(function* () {
				...
				Effect.provide(Layer.merge(makeTestLayer(state), TestClock.layer())),
			);
		},
	);

	it.effect(
		"retries the initial remote load when periodic refreshes are disabled",
		() => {
			const state = {
				currentLocalStatus: baseLocalStatus,
				localStatusCalls: 0,
			...
			return Effect.gen(function* () {
				...
				yield* Stream.runForEach(
					...
					(event) =>
						event._tag === "remoteUpdated"
							? Deferred.succeed(remoteUpdatedDeferred, event).pipe(
									Effect.ignore,
								)
							: Effect.void,
				).pipe(Effect.forkIn(scope));
				...
				assert.deepStrictEqual(
					messages.find(
						(message) => message[0] === "VCS remote status refresh failed",
					),
					[
	...
	it.effect(
		"delays automatic refresh when a cached remote snapshot is available",
		() => {
			const state = {
				currentLocalStatus: baseLocalStatus,
				currentRemoteStatus: baseRemoteStatus,
	...
	it("backs off remote refresh failures exponentially and honors larger configured intervals", () => {
		assert.equal(
			Duration.toMillis(
				VcsStatusBroadcaster.remoteRefreshFailureDelay(1, Duration.seconds(1)),
			),
			30_000,
		);
		assert.equal(
			Duration.toMillis(
				VcsStatusBroadcaster.remoteRefreshFailureDelay(2, Duration.seconds(1)),
			),
			60_000,
		);
		assert.equal(
			Duration.toMillis(
				VcsStatusBroadcaster.remoteRefreshFailureDelay(3, Duration.seconds(1)),
			),
			120_000,
		);
		assert.equal(
			Duration.toMillis(
				VcsStatusBroadcaster.remoteRefreshFailureDelay(1, Duration.minutes(5)),
			),
			300_000,
		);
		assert.equal(
			Duration.toMillis(
				VcsStatusBroadcaster.remoteRefreshFailureDelay(20, Duration.seconds(1)),
			),
			900_000,
		);
	});
	...
	it("summarizes refresh causes without exposing nested failure details", () => {
		...
		const cause = Cause.combine(
			Cause.fail(failure),
			Cause.die(new TypeError("private defect")),
		);

		assert.deepStrictEqual(
			VcsStatusBroadcaster.remoteRefreshFailureDiagnostics(cause),
			{
				reasonCount: 2,
				failureCount: 1,
				failureTags: ["GitManagerError"],
	...
	it.effect(
		"does not start automatic remote refreshes without foreground client demand",
		() => {
			const state = {
				currentLocalStatus: baseLocalStatus,
				currentRemoteStatus: baseRemoteStatus,
	...
	it.effect(
		"stops the remote poller after the last stream subscriber disconnects",
		() => {
			const state = {
				currentLocalStatus: baseLocalStatus,
				currentRemoteStatus: baseRemoteStatus,
			...
			let remoteInterruptedDeferred: Deferred.Deferred<void, never> | null =
				null;
							...
							Effect.sync(() => {
								...
								Effect.andThen(
									remoteStartedDeferred
										? Deferred.succeed(remoteStartedDeferred, undefined).pipe(
												Effect.ignore,
											)
										: Effect.void,
								),
