...
function addUniqueDiagnosticValue(
	values: Array<string>,
...
export function remoteRefreshFailureDiagnostics(cause: Cause.Cause<unknown>) {
	...
	for (const reason of cause.reasons) {
		if (Cause.isFailReason(reason)) {
			...
			addUniqueDiagnosticValue(
				failureOperations,
				diagnosticFailureOperation(reason.error),
			);
			continue;
		}
...
interface StreamStatusOptions {
	readonly automaticRemoteRefreshInterval?: Effect.Effect<
		Duration.Duration,
		never
	>;
}
		...
		return {
			isEnabled: Effect.fn("VcsAutoPullPolicy.isEnabled")(
				function* (cwd: string) {
					...
					return resolveProjectAutoPull(
						settings,
						project.value.id,
						project.value.autoPull,
					);
				},
...
export function remoteRefreshFailureDelay(
	...
	const backoffMs =
		Duration.toMillis(VCS_STATUS_REFRESH_FAILURE_BASE_DELAY) *
		Math.pow(2, exponent);
...
export const make = Effect.gen(function* () {
	...
	const pollersRef = yield* SynchronizedRef.make(
		new Map<string, ActiveRemotePoller>(),
	);

	const getCachedStatus = Effect.fn("VcsStatusBroadcaster.getCachedStatus")(
		function* (cwd: string) {
			return yield* Ref.get(cacheRef).pipe(
				Effect.map((cache) => cache.get(cwd) ?? null),
			);
		},
	);

	const updateCachedLocalStatus = Effect.fn(
		"VcsStatusBroadcaster.updateCachedLocalStatus",
	)(function* (
		cwd: string,
	...
	const updateCachedRemoteStatus = Effect.fn(
		"VcsStatusBroadcaster.updateCachedRemoteStatus",
	)(function* (
		cwd: string,
	...
	const updateCachedStatus = Effect.fn(
		"VcsStatusBroadcaster.updateCachedStatus",
	)(function* (
		cwd: string,
		local: VcsStatusLocalResult,
		remote: VcsStatusRemoteResult | null,
	...
	const loadLocalStatus = Effect.fn("VcsStatusBroadcaster.loadLocalStatus")(
		function* (cwd: string) {
	...
	const getOrLoadLocalStatus = Effect.fn(
		"VcsStatusBroadcaster.getOrLoadLocalStatus",
	)(function* (cwd: string) {
		...
		return yield* withRemoteWriteLock(
			...
			Effect.gen(function* () {
				...
				const [local, remote] = yield* Effect.all(
					[
						latest?.local
							? Effect.succeed(latest.local.value)
	...
	const refreshLocalStatusCore = Effect.fn(
		"VcsStatusBroadcaster.refreshLocalStatusCore",
	)(function* (cwd: string) {
	...
	const refreshLocalStatus: VcsStatusBroadcaster["Service"]["refreshLocalStatus"] =
		Effect.fn("VcsStatusBroadcaster.refreshLocalStatus")(function* (rawCwd) {
			const cwd = yield* withFileSystem(normalizeCwd(rawCwd));
			return yield* refreshLocalStatusCore(cwd);
		});

	const maybeAutoPull = Effect.fn("VcsStatusBroadcaster.maybeAutoPull")(
		function* (
			cwd: string,
			remote: VcsStatusRemoteResult | null,
			...
			return yield* Effect.gen(function* () {
				const autoPullEnabled = (yield* Effect.forEach(
					policyCwds,
					autoPullPolicy.isEnabled,
					{
						concurrency: "unbounded",
					},
				)).some(Boolean);
				...
				const [refreshedLocal, refreshedRemote] = yield* Effect.all(
					[
						workflow.localStatus({ cwd }),
						workflow.remoteStatus({ cwd }, { refreshUpstream: false }),
					],
					{ concurrency: "unbounded" },
				);
	...
	const refreshRemoteStatus = Effect.fn(
	...
	const refreshStatus: VcsStatusBroadcaster["Service"]["refreshStatus"] =
		Effect.fn("VcsStatusBroadcaster.refreshStatus")(function* (rawCwd) {
			const cwd = yield* withFileSystem(normalizeCwd(rawCwd));
			...
			return yield* withRemoteWriteLock(
				...
				Effect.gen(function* () {
					...
					if (pulled !== null)
						return mergeGitStatusParts(pulled.local, pulled.remote);
					return yield* updateCachedStatus(cwd, local, remote, {
						publish: true,
					});
				}),
	...
	const refreshPullRequestStatus: VcsStatusBroadcaster["Service"]["refreshPullRequestStatus"] =
		Effect.fn("VcsStatusBroadcaster.refreshPullRequestStatus")(
			function* (rawCwd) {
				const cwd = yield* withFileSystem(normalizeCwd(rawCwd));
				return yield* withRemoteWriteLock(
					cwd,
					Effect.gen(function* () {
						...
						const demandCwds = poller
							? [...(yield* Ref.get(poller.demandCwds)).keys()]
							: [rawCwd];
	...
	const makeRemoteRefreshLoop = (
		...
		return Effect.gen(function* () {
			...
			const refreshRemoteStatusIfEnabled = Effect.gen(function* () {
				...
				const interruptionReasons = exit.cause.reasons.filter(
					Cause.isInterruptReason,
				);
				if (interruptionReasons.length > 0) {
					return yield* Effect.failCause(
						Cause.fromReasons<never>(interruptionReasons),
					);
				}
	...
	const retainRemotePoller = Effect.fn(
		"VcsStatusBroadcaster.retainRemotePoller",
	)(function* (
		cwd: string,
		demandCwd: string,
		automaticRemoteRefreshInterval: Effect.Effect<Duration.Duration, never>,
		...
		yield* SynchronizedRef.modifyEffect(pollersRef, (activePollers) => {
			...
			return Ref.make<ReadonlyMap<string, number>>(
				new Map([[demandCwd, 1]]),
			).pipe(
	...
	const releaseRemotePoller = Effect.fn(
		"VcsStatusBroadcaster.releaseRemotePoller",
	)(function* (cwd: string, demandCwd: string) {
		const pollerToInterrupt = yield* SynchronizedRef.modifyEffect(
			pollersRef,
			(activePollers) => {
				const existing = activePollers.get(cwd);
				if (!existing) {
				...
				return Effect.succeed([
					existing.fiber,
					new Map(
						[...activePollers].filter(([activeCwd]) => activeCwd !== cwd),
					),
				] as const);
			},
		);
	...
	const streamStatus: VcsStatusBroadcaster["Service"]["streamStatus"] = (
		input,
		options,
	) =>
		Stream.unwrap(
			Effect.gen(function* () {
				...
				const release = releaseRemotePoller(cwd, input.cwd).pipe(
					Effect.ignore,
					Effect.asVoid,
				);
