...
export function runVcsDriverContractSuite<R, E>(
	input: VcsDriverContractSuiteInput<R, E>,
) {
	const makeTmpDir = (
	...
	it.layer(input.layer)(`${input.name} VCS driver contract`, (it) => {
		describe("repository detection", () => {
			...
			it.effect(
				"detects repository identity inside a repository and nested directories",
				() =>
					Effect.gen(function* () {
						const cwd = yield* makeTmpDir();
						const driver = yield* VcsDriver.VcsDriver;
						...
						assert.isTrue(
							Option.isNone(identity?.freshness.expiresAt ?? Option.none()),
						);
