...
const makeTempDir = Effect.fn(function* (opts?: {
	prefix?: string;
	git?: boolean;
}) {
...
it.layer(TestLayer, { excludeTestServices: true })("WorkspaceEntries", (it) => {
	...
	describe("list", () => {
		it.effect("returns the complete cached workspace index", () =>
			Effect.gen(function* () {
				...
				expect(
					result.entries.some((entry) => entry.path.startsWith("node_modules")),
				).toBe(false);
	...
	describe("search", () => {
		...
		it.effect(
			"supports fuzzy subsequence queries for composer path search",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTempDir({
						prefix: "t3code-workspace-fuzzy-query-",
					});
					yield* writeTextFile(cwd, "src/components/Composer.tsx");
					yield* writeTextFile(cwd, "src/components/composePrompt.ts");
					yield* writeTextFile(cwd, "docs/composition.md");
		...
		it.effect(
			"prioritizes exact basename matches ahead of broader path matches",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTempDir({
						prefix: "t3code-workspace-exact-ranking-",
					});
					yield* writeTextFile(cwd, "src/components/Composer.tsx");
		...
		it.effect("tracks truncation without sorting every fuzzy match", () =>
			Effect.gen(function* () {
				const cwd = yield* makeTempDir({
					prefix: "t3code-workspace-fuzzy-limit-",
				});
				yield* writeTextFile(cwd, "src/components/Composer.tsx");
				yield* writeTextFile(cwd, "src/components/composePrompt.ts");
				yield* writeTextFile(cwd, "docs/composition.md");
		...
		it.effect("applies the file filter before limiting search results", () =>
			Effect.gen(function* () {
				const cwd = yield* makeTempDir({
					prefix: "t3code-workspace-file-limit-",
				});
				yield* writeTextFile(cwd, "src/index.ts");
				...
				expect(["src/index.ts", "src/internal.ts"]).toContain(
		...
		it.effect(
			"answers an empty file-filtered query with a bounded file listing",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTempDir({
						prefix: "t3code-workspace-empty-query-",
					});
					yield* writeTextFile(cwd, "src/index.ts");
					yield* writeTextFile(cwd, "README.md");
					...
					expect(paths).toContain("README.md");
					expect(result.entries.every((entry) => entry.kind === "file")).toBe(
						true,
					);
				}),
		);
		...
		it.effect("returns only directories for the directory filter", () =>
			Effect.gen(function* () {
				const cwd = yield* makeTempDir({
					prefix: "t3code-workspace-directory-filter-",
				});
				yield* writeTextFile(cwd, "src/index.ts");
		...
		it.effect("excludes gitignored paths for git repositories", () =>
			Effect.gen(function* () {
				const cwd = yield* makeTempDir({
					prefix: "t3code-workspace-gitignore-",
					git: true,
				});
				yield* writeTextFile(
					cwd,
				...
				expect(paths.some((entryPath) => entryPath.startsWith("convex/"))).toBe(
					false,
				);
			}),
		);
		...
		it.effect("excludes .convex in non-git workspaces", () =>
			Effect.gen(function* () {
				const cwd = yield* makeTempDir({
					prefix: "t3code-workspace-non-git-convex-",
				});
				yield* writeTextFile(cwd, ".convex/local-storage/data.json", "{}");
				yield* writeTextFile(cwd, "src/keep.ts", "export {};");
		...
		it.effect("supports typo-resistant file search through fff", () =>
			Effect.gen(function* () {
				const cwd = yield* makeTempDir({
					prefix: "t3code-workspace-fff-typo-",
				});
				yield* writeTextFile(cwd, "src/components/Composer.tsx");
		...
		it.effect("rebuilds the cached index after refresh fails", () =>
			Effect.gen(function* () {
				const cwd = yield* makeTempDir({
					prefix: "t3code-workspace-refresh-failure-",
				});
	...
	describe("searchContents", () => {
		it.effect(
			"returns content matches with file paths, line numbers, and ranges",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTempDir({
					...
					expect(
						result.matches.map((match) => [match.path, match.lineNumber]),
					).toEqual([
						["src/shapes.ts", 1],
						["src/shapes.ts", 2],
					]);
					expect(result.matches[0]?.matchRanges).toEqual([
						{ start: 13, end: 19 },
					]);
					expect(result.truncated).toBe(false);
				}),
		...
		it.effect("honors case sensitivity and gitignore rules", () =>
			Effect.gen(function* () {
				const cwd = yield* makeTempDir({
					prefix: "t3code-workspace-content-ignore-",
					git: true,
				});
		...
		it.effect(
			"filters whole-word matches by word boundaries without widening ranges",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTempDir({
						prefix: "t3code-workspace-content-whole-word-",
					});
					yield* writeTextFile(
						cwd,
						"src/words.ts",
		...
		it.effect(
			"finds later whole-word matches in a file after rejected raw matches",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTempDir({
						prefix: "t3code-workspace-content-late-whole-word-",
					});
					yield* writeTextFile(
						cwd,
						"src/words.ts",
		...
		it.effect("treats astral-plane letters as whole word characters", () =>
			Effect.gen(function* () {
				const cwd = yield* makeTempDir({
					prefix: "t3code-workspace-content-astral-word-",
				});
		...
		it.effect(
			"matches punctuation-edged whole-word queries including adjacent occurrences",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTempDir({
						prefix: "t3code-workspace-content-punctuation-",
					});
		...
		it.effect("matches punctuation-edged regex queries as whole words", () =>
			Effect.gen(function* () {
				const cwd = yield* makeTempDir({
					prefix: "t3code-workspace-content-regex-punctuation-",
				});
		...
		it.effect(
			"caps matches per file so one dense file cannot fill the page",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTempDir({
						prefix: "t3code-workspace-content-per-file-cap-",
					});
		...
		it.effect("preserves regex escapes during case-insensitive searches", () =>
			Effect.gen(function* () {
				const cwd = yield* makeTempDir({
					prefix: "t3code-workspace-content-regex-",
				});
		...
		it.effect(
			"preserves invalid regex errors during case-insensitive searches",
			() =>
				Effect.gen(function* () {
					const cwd = yield* makeTempDir({
						prefix: "t3code-workspace-content-invalid-regex-",
					});
		...
		it.effect("maps multi-byte lines to string-indexed ranges", () =>
			Effect.gen(function* () {
				const cwd = yield* makeTempDir({
					prefix: "t3code-workspace-content-multibyte-",
				});
				yield* writeTextFile(
					cwd,
					"src/notes.ts",
	...
	describe("browse", () => {
		it.effect("returns matching directories and excludes files", () =>
			Effect.gen(function* () {
				...
				const path = yield* Path.Path;
				const cwd = yield* makeTempDir({
					prefix: "t3code-workspace-browse-prefix-",
				});
		...
		it.effect(
			"shows dot directories in directory mode and hidden-prefix mode",
			() =>
				Effect.gen(function* () {
					const workspaceEntries = yield* WorkspaceEntries.WorkspaceEntries;
					const path = yield* Path.Path;
					const cwd = yield* makeTempDir({
						prefix: "t3code-workspace-browse-hidden-",
					});
		...
		it.effect("supports relative paths when cwd is provided", () =>
			Effect.gen(function* () {
				...
				const path = yield* Path.Path;
				const cwd = yield* makeTempDir({
					prefix: "t3code-workspace-browse-relative-",
				});
		...
		it.effect(
			"returns an empty listing when the OS denies directory access",
			() =>
				Effect.gen(function* () {
					const workspaceEntries = yield* WorkspaceEntries.WorkspaceEntries;
					const cwd = yield* makeTempDir({
						prefix: "t3code-workspace-browse-eacces-",
					});
