...
it.layer(TestLayer, { excludeTestServices: true })(
	...
	(it) => {
		describe("readFile", () => {
			...
			it.effect(
				"reads host files outside the workspace root by absolute path",
				() =>
					Effect.gen(function* () {
						const workspaceFileSystem =
							yield* WorkspaceFileSystem.WorkspaceFileSystem;
			...
			it.effect.skipIf(!symlinksSupported)(
				...
				() =>
					Effect.gen(function* () {
						...
						const resolvedPath = yield* fileSystem.realPath(
							path.join(outsideDir, "secret.txt"),
						);
			...
			it.effect(
				"rejects binary files without leaking their contents into the error",
				() =>
					Effect.gen(function* () {
						const workspaceFileSystem =
							yield* WorkspaceFileSystem.WorkspaceFileSystem;
		...
		describe("writeFile", () => {
			...
			it.effect("invalidates workspace entry search cache after writes", () =>
				Effect.gen(function* () {
					...
					expect(
						beforeWrite.entries.some(
							(entry) => entry.path === "plans/effect-rpc.md",
						),
					).toBe(false);
					...
					expect(afterWrite.entries).toEqual(
						expect.arrayContaining([
							expect.objectContaining({ path: "plans/effect-rpc.md" }),
						]),
					);
