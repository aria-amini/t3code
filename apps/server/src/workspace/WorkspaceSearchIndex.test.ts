...
it.effect("filters image searches before applying the result limit", () =>
	Effect.scoped(
		Effect.gen(function* () {
			...
			const searchIndex =
				yield* WorkspaceSearchIndex.make("/workspace/project");
			const resultWithoutKind = yield* searchIndex.search(
				"",
				200,
				undefined,
				true,
			);
			const resultWithDirectoryKind = yield* searchIndex.search(
				"",
				200,
				"directory",
				true,
			...
			expect(resultWithoutKind.entries).toEqual([
				{ kind: "file", path: "public/icon.svg" },
			]);
			expect(resultWithDirectoryKind.entries).toEqual([
				{ kind: "file", path: "public/icon.svg" },
			]);
...
it.effect(
	"keeps returned FileFinder creation diagnostics out of the cause chain",
	() =>
		Effect.gen(function* () {
			vi.spyOn(FileFinder, "create").mockReturnValueOnce({
				ok: false,
...
it.effect("waits for the full content index warmup before returning", () =>
	Effect.gen(function* () {
		const waitForIndexReady = vi.fn(async () => ({
			ok: true as const,
			value: true,
		}));
		const finder = {
		...
		yield* Effect.scoped(
			WorkspaceSearchIndex.make("/workspace/project", "content"),
		);
	...
	Effect.gen(function* () {
		...
		if (Exit.isFailure(exit)) {
			...
			expect(error).toBeInstanceOf(
				WorkspaceSearchIndex.WorkspaceSearchIndexDestroyFailed,
			);
			expect(error).toMatchObject({
...
it.effect("continues whole-word searches after a filtered grep page", () =>
	Effect.scoped(
		Effect.gen(function* () {
			...
			const searchIndex = yield* WorkspaceSearchIndex.make(
				"/workspace/project",
				"content",
			);
