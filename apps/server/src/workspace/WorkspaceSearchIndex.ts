...
function toProjectEntry(item: MixedItem): ProjectEntry | null {
	const normalizedPath = trimDirectorySeparator(
		toPosixPath(item.item.relativePath),
	);
	if (!normalizedPath) {
		return null;
	}
...
function mapFileSearchResult(
	...
	const entries = result.items.flatMap((item) => {
		...
		return entry && (!imageOnly || isWorkspaceImagePreviewPath(entry.path))
			? [entry]
			: [];
	});
	return {
		...
		truncated:
			entries.length > limit || result.totalMatched > result.items.length,
	};
}
...
function buildContentSearchQuery(
	input: Omit<ProjectSearchContentsInput, "cwd">,
): {
...
function mapContentMatchRanges(
	...
	const toStringIndex = (byteOffset: number) =>
		lineBytes.subarray(0, byteOffset).toString().length;
...
const waitForIndexReady = Effect.fn("WorkspaceSearchIndex.waitForIndexReady")(
	function* <E>(
		cwd: string,
		finder: FileFinder,
		onFailure: (input: {
...
export const make = Effect.fn("WorkspaceSearchIndex.make")(function* (
	...
	const list: WorkspaceSearchIndex["Service"]["list"] = Effect.fn(
		...
		const sortedEntries = withDirectoryAncestors(mapped.entries).toSorted(
	...
	const searchContents: WorkspaceSearchIndex["Service"]["searchContents"] =
		Effect.fn("WorkspaceSearchIndex.searchContents")(function* (input) {
			...
			do {
				const remainingTimeBudgetMs = Math.max(
					1,
				...
				for (const match of result.items) {
					const matchRanges = mapContentMatchRanges(
						match.lineContent,
						match.matchRanges,
					).filter(
						(range) =>
							!input.wholeWord || isWholeWordRange(match.lineContent, range),
					);
...
export type WorkspaceSearchIndexVariant =
	(typeof WORKSPACE_SEARCH_INDEX_VARIANTS)[number];
