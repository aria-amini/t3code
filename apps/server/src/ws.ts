...
const isOrchestrationDispatchCommandError = Schema.is(
	OrchestrationDispatchCommandError,
);
...
function readClientConnectionOrigin(
	...
	const appVersion =
		url.value.searchParams.get("clientAppVersion")?.trim() ?? "";
	return {
...
function readClientAnalyticsProps(
	request: HttpServerRequest.HttpServerRequest,
) {
	const url = HttpServerRequest.toURL(request);
	if (Option.isNone(url)) {
	...
	const rawOsMajorVersion =
		url.value.searchParams.get("clientOsMajorVersion") ?? "";
	const osMajorVersion = Number(rawOsMajorVersion);
	const deviceModel =
		url.value.searchParams.get("clientDeviceModel")?.trim() ?? "";
	const isMobile = surface === "mobile";
	const hasOsMajorVersion =
		isMobile &&
		rawOsMajorVersion !== "" &&
		Number.isInteger(osMajorVersion) &&
		osMajorVersion > 0;
	const hasDeviceModel =
		isMobile &&
		deviceModel !== "" &&
		deviceModel.length <= MAX_CLIENT_DEVICE_MODEL_LENGTH;
	...
	return {
		...
		...(surface === "web" && isClientWebDeployment(webDeployment)
			? { webDeployment }
			: {}),
		...(surface === "web" &&
		browser !== "" &&
		browser.length <= MAX_CLIENT_BROWSER_LENGTH
			? { clientBrowser: browser }
			: {}),
		...(hasOsMajorVersion
			? { osMajorVersion, clientOsMajorVersion: osMajorVersion }
			: {}),
...
const makeWsRpcLayer = (
	...
	WsRpcGroup.toLayer(
		Effect.gen(function* () {
			...
			const checkpointDiffQuery =
				yield* CheckpointDiffQuery.CheckpointDiffQuery;
			...
			const vcsProvisioning =
				yield* VcsProvisioningService.VcsProvisioningService;
			const vcsStatusBroadcaster =
				yield* VcsStatusBroadcaster.VcsStatusBroadcaster;
			...
			const lifecycleEvents =
				yield* ServerLifecycleEvents.ServerLifecycleEvents;
			const serverSettings = yield* ServerSettings.ServerSettingsService;
			const startup = yield* ServerRuntimeStartup.ServerRuntimeStartup;
			const workspaceEntries = yield* WorkspaceEntries.WorkspaceEntries;
			const workspaceFileSystem =
				yield* WorkspaceFileSystem.WorkspaceFileSystem;
			...
			const agentSessionScanner =
				yield* AgentSessionScanner.AgentSessionScanner;
				...
				Ref.get(rpcClientIds).pipe(
					Effect.flatMap((clientIds) =>
						Effect.forEach(
							clientIds,
							(clientId) =>
								backgroundPolicy.removeRpcClient(currentSessionId, clientId),
							{
								discard: true,
							},
						),
			...
			const sourceControlDiscovery =
				yield* SourceControlDiscovery.SourceControlDiscovery;
			const automaticGitFetchInterval = serverSettings.getSettings.pipe(
				...
				Effect.catch((cause) =>
					Effect.logWarning(
						"Failed to read automatic Git fetch interval setting",
						{
			...
			const processResourceMonitor =
				yield* ProcessResourceMonitor.ProcessResourceMonitor;
			...
			const loadServerConfig = (options: {
				...
				Effect.gen(function* () {
					...
					const availableEditors: ReadonlyArray<EditorId> =
						yield* resolveAvailableEditorsForConfig(
							externalLauncher.resolveAvailableEditors(),
						);
					const fileManagerRevealKind = availableEditors.includes(
						"file-manager",
					)
			...
			return WsRpcGroup.of({
							...
							if (archiveCommand) {
								if (shouldStopSessionAfterCommand) {
									yield* Effect.gen(function* () {
										...
										Effect.catchCause((cause) =>
											Effect.logWarning(
												"failed to stop provider session during archive",
												{
													threadId: archiveCommand.threadId,
													cause,
												},
								...
								yield* terminalManager
									.close({ threadId: archiveCommand.threadId })
									.pipe(
										Effect.catch((error) =>
											Effect.logWarning(
						...
						Effect.gen(function* () {
							...
							const closeLiveBuffer = (error?: OrchestrationGetSnapshotError) =>
								Effect.gen(function* () {
									...
									liveBudget.release(
										yield* Queue.clear(liveBuffer).pipe(Effect.orDie),
									);
									if (error) {
							...
							yield* Effect.forkScoped(
								orchestrationEngine.streamDomainEvents.pipe(
									...
									Stream.runForEach((event) =>
										liveBudget
											.retain({ kind: "event" as const, event }, event)
											.pipe(
							...
							const loadSnapshot = projectionSnapshotQuery
								...
								.pipe(
									Effect.tapError((cause) =>
										Effect.logError(
											"orchestration shell snapshot load failed",
											{ cause },
										),
									),
									Effect.mapError(
							...
							const synchronizedThenLive = liveBudget.deliver(
								input.requestCompletionMarker === true
									? Stream.concat(
											Stream.fromEffect(
												liveBudget
													.retain({ kind: "synchronized" as const })
													.pipe(
														Effect.flatMap((item) =>
															Queue.offer(liveBuffer, item),
														),
														Effect.uninterruptible,
				...
				[ORCHESTRATION_WS_METHODS.getArchivedShellSnapshot]: (_input) =>
					observeRpcEffect(
						...
						projectionSnapshotQuery.getArchivedShellSnapshot().pipe(
							Effect.tapError((cause) =>
								Effect.logError(
									"orchestration archived shell snapshot load failed",
									{ cause },
								),
							),
							Effect.mapError(
								(cause) =>
									new OrchestrationGetSnapshotError({
										message:
											"Failed to load archived orchestration shell snapshot",
										cause,
									}),
							),
						),
				...
				[WS_METHODS.serverRefreshProviders]: (input) =>
					observeRpcEffect(
						...
						Effect.gen(function* () {
							...
							let providers = yield* input.cwd !== undefined &&
							...
							if (input.refreshModels) {
								...
								for (const instance of instances) {
									...
									providers = yield* providerRegistry.refreshInstance(
										instance.instanceId,
									);
								}
							}
					...
					observeRpcEffect(
						...
						Effect.gen(function* () {
							if ("sourceId" in input)
								return yield* usageLimitSources.consumeResetCredit(input);
				...
				[WS_METHODS.serverRemoveKeybinding]: (rule) =>
					observeRpcEffect(
						...
						Effect.gen(function* () {
							const keybindingsConfig =
								yield* keybindings.removeKeybindingRule(rule);
							return { keybindings: keybindingsConfig, issues: [] };
						}),
						{ "rpc.aggregate": "server" },
					),
				...
				[WS_METHODS.pullRequestsSetReaction]: (input) =>
					observeRpcEffect(
						...
						pullRequests.setReaction(input),
						{
							"rpc.aggregate": "pull-requests",
						},
					),
					...
					observeRpcStream(
						...
						Stream.callback<DiscoveredLocalServerList>((queue) =>
							Effect.gen(function* () {
								...
								const initialScannedAt = DateTime.formatIso(
									yield* DateTime.now,
								);
					...
					observeRpcStreamEffect(
						...
						Effect.gen(function* () {
							...
							const providerStatuses = Stream.zipLatestWith(
								...
								(updates) =>
									Stream.concat(Stream.make(config.providers), updates),
								Stream.changesWith(
									(previous, next) =>
				...
				[WS_METHODS.subscribeServerLifecycle]: (_input) =>
					observeRpcStreamEffect(
						...
						Effect.gen(function* () {
							const liveBuffer =
								yield* Queue.unbounded<ServerLifecycleStreamEvent>();
							yield* Effect.forkScoped(
...
export const websocketRpcRouteLayer = Layer.unwrap(
	Effect.gen(function* () {
		...
		const config = yield* ServerConfig.ServerConfig;
		const startup = yield* ServerRuntimeStartup.ServerRuntimeStartup;
		const serverSelfUpdate =
			yield* ServerSelfUpdate.withRunningThreadContinuation({
				mode: config.mode,
					...
					Effect.mapError(
						(cause) =>
							new ServerSelfUpdateError({
								reason:
									"Could not prepare running threads to continue after the update.",
								cause,
							}),
					),
				),
					...
					startup.clearProviderSessionContinuationMarkers(threadIds).pipe(
						Effect.mapError(
							(cause) =>
								new ServerSelfUpdateError({
									reason:
										"Could not clear thread continuation markers after the update failed.",
									cause,
								}),
						),
					),
		...
		return HttpRouter.add(
			...
			Effect.gen(function* () {
				...
				const session = yield* serverAuth
					...
					.pipe(
						Effect.catchIf(
							EnvironmentAuth.isServerAuthCredentialError,
							(error) =>
								failEnvironmentAuthInvalid(
