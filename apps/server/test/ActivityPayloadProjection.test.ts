			...
			changes: [
				{
					oldPath: "src/old.ts",
					newPath: "src/new.ts",
					patch: "large patch".repeat(1_000),
				},
...
describe("projectActivityPayload", () => {
	...
	it("projects a Claude Bash result for the web and mobile expanded rows", () => {
		...
		expect(mobileRow?.getCopyText()).toBe(
			`Command run\n${command}\n\nfirst output line`,
		);
	});
	...
	it("keeps current web and mobile derived fields for every tool item type", () => {
		for (const activity of fixtures) {
			...
			expect(deriveWorkLogEntries([projected])).toEqual(
				deriveWorkLogEntries([activity]),
			);
			expect(comparableThreadFeed([projected])).toEqual(
				comparableThreadFeed([activity]),
			);
		}
	});
...
describe("superseded tool.updated snapshot dedup", () => {
	...
	it("matches on toolCallId when the adapter emits one", () => {
		...
		expect(projectedIds([otherCall, update, completed])).toEqual([
			otherCall.id,
			completed.id,
		]);
	});
	...
	it("keeps updates with no matching completion", () => {
		...
		const completed = makeToolLifecycleActivity(
			"done-other",
			"tool.completed",
			{
				title: "Reading",
		...
		expect(projectedIds([inFlight, other, completed])).toEqual([
			inFlight.id,
			completed.id,
		]);
	});
	...
	it("drops interleaved superseded updates even when a parallel call separates them", () => {
		...
		// comment).
		const updateA = makeToolLifecycleActivity("upd-a", "tool.updated", {
			toolCallId: "call-a",
		});
	...
	it("keeps an update whose completion lives in another turn", () => {
		...
		expect(projectedIds([update, completed])).toEqual([
			update.id,
			completed.id,
		]);
	});
	...
	it("keeps an update that follows its completion", () => {
		...
		expect(projectedIds([completed, nextCall])).toEqual([
			completed.id,
			nextCall.id,
		]);
	});
	...
	it("keeps identity-less rows the clients never collapse", () => {
		...
		expect(projectedIds([anonymous, completed])).toEqual([
			anonymous.id,
			completed.id,
		]);
	});
	...
	it("leaves the collapsed work log identical to the full history", () => {
		const activities = [
			...
			makeToolLifecycleActivity("done-1", "tool.completed", {
				detail: "writing",
			}),
		];
		...
		expect(after.map((entry) => entry.label)).toEqual(
			before.map((entry) => entry.label),
		);
	});
});
...
describe("context-window snapshot dedup", () => {
	...
	it("still resolves a meter value after the client reverts the newest turn", () => {
		...
		const revertedTurn = makeContextWindowActivity(
			"ctx-new",
			9_000,
			"turn-reverted",
		);
	...
	it("matches what the web client derives from the full history", () => {
		...
		expect(
			deriveLatestContextWindowSnapshot(projected.thread.activities),
		).toEqual(deriveLatestContextWindowSnapshot(activities));
	});
	...
	it("does not let a malformed row shadow an earlier valid row in the same turn", () => {
		...
		expect(
			deriveLatestContextWindowSnapshot(projected.thread.activities),
		).toEqual(deriveLatestContextWindowSnapshot([valid, malformed]));
	});
	...
	it("applies only payload slimming when there are no context-window activities", () => {
		...
		expect(projected.thread.activities).toEqual([
			projectActivityPayload(fixtures[4]!),
		]);
	});
});
