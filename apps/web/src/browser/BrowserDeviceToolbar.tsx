...
export function BrowserDeviceToolbar({
	...
	const apply = (
		...
		setPending(true);
		void commitViewportAndAspectRatio(
			next,
			nextAspectRatio,
			onChange,
			onAspectRatioChange,
	...
	const updateCustomDimension = (axis: "width" | "height", value: string) => {
		setCustomSize((current) => {
			const next = {
				width:
					axis === "width" ? value : (current?.width ?? String(setting.width)),
				height:
					axis === "height"
						? value
						: (current?.height ?? String(setting.height)),
			};
		...
		>
			...
			<Tooltip>
				<TooltipTrigger
					render={
						<Button
							...
							className={cn(
								aspectRatio !== null && "bg-accent text-foreground",
							)}
				...
				>
					{aspectRatio === null ? (
						<Unlink2
							className={cn(aspectRatio !== null && "text-foreground")}
						/>
					) : (
