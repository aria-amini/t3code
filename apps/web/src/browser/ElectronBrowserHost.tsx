...
export function ElectronBrowserHost() {
	...
	const sessions = useMemo(
		() =>
			Object.entries(previewByThreadKey).flatMap(
				([threadKey, previewState]) => {
					...
					return threadRef
						? Object.values(previewState.sessions).map((snapshot) => ({
								...
								pictureInPicture:
									previewState.desktopByTabId[snapshot.tabId]
										?.pictureInPicture ?? false,
								zoomFactor:
									previewState.desktopByTabId[snapshot.tabId]?.zoomFactor ?? 1,
							}))
						: [];
				},
			),
