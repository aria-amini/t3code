...
import {
	acquireDesktopTab,
	type AcquiredDesktopTab,
} from "./desktopTabLifetime";
...
export function HostedBrowserWebview(props: {
	...
	const deviceToolbarVisible =
		active && viewport._tag !== "fill" && !presentation.fitSourceContent;
	const {
		activeDrag,
	...
	const renderingActive =
		active || backgroundActivity || pictureInPicture || recordingActive;
		...
		>
			...
			>
				...
				<webview
					...
					{...({ allowpopups: "true" } as unknown as {
					...
					data-preview-viewport-key={browserViewportSettingKey(
						effectiveViewport,
					)}
					data-preview-css-width={
						fittedSourceViewport
							...
							: effectiveViewport._tag === "fill"
								? Math.max(
										1,
										Math.round(layout.viewportWidth / normalizedZoomFactor),
									)
					...
					data-preview-css-height={
						fittedSourceViewport
							...
							: effectiveViewport._tag === "fill"
								? Math.max(
										1,
										Math.round(layout.viewportHeight / normalizedZoomFactor),
									)
