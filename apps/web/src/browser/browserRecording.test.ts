...
const {
	...
	return {
		...
		registrySet: vi.fn(
			(_atom: unknown, value: { readonly tabIds: ReadonlySet<string> }) => {
				events.push(
					value.tabIds.size === 0
						? "clear"
						: `publish:${Array.from(value.tabIds).join(",")}`,
				);
			},
		),
...
class FakeMediaRecorder {
	...
	stop(): void {
		if (FakeMediaRecorder.stopError !== undefined)
			throw FakeMediaRecorder.stopError;
		this.state = "inactive";
...
describe("browser recording", () => {
	...
	it("routes gesture-free starts through the desktop capture trigger", async () => {
		...
		expect(requestDisplayMediaCapture).toHaveBeenCalledWith(
			"automation-recording-tab",
		);
	...
	it("keeps the saved desktop file and releases the recording when transfer fails", async () => {
		...
		expect(save).toHaveBeenCalledOnce();
		expect(readActiveBrowserRecordingTabIds().has("failed-transfer-tab")).toBe(
			false,
		);
		await startBrowserRecording("failed-transfer-tab");
	...
	it("paints and holds a hidden browser surface for the recording lifetime", async () => {
		...
		getDisplayMedia.mockImplementationOnce(async () => {
			expect(animationFrameCount).toBe(2);
			expect(
				useBrowserSurfaceStore.getState().activityByTabId["background-tab"],
			).toBe(1);
	...
	it.each([
		...
		"records the native $width x $height stream at $frameRate fps",
		async (settings) => {
			const stopTrack = vi.fn();
			const stream = {
			...
			expect(stopTrack.mock.invocationCallOrder[0]).toBeLessThan(
	...
	it("clears a failed settings read before retrying recording", async () => {
		...
		expect(
			useBrowserSurfaceStore.getState().activityByTabId[tabId],
		).toBeUndefined();
	...
	it("times out stalled stream acquisition and stops a late stream", async () => {
		...
		expect(readActiveBrowserRecordingTabIds()).toEqual(new Set());
		expect(
			useBrowserSurfaceStore.getState().activityByTabId["recording-tab"],
		).toBeUndefined();
	...
	it("records separate tabs concurrently", async () => {
		...
		expect(readActiveBrowserRecordingTabIds(firstThreadRef)).toEqual(
			new Set(["recording-tab"]),
		);
		expect(readActiveBrowserRecordingTabIds(secondThreadRef)).toEqual(
			new Set(["recording-tab-2"]),
		);
	...
	it("serializes display media grants for concurrent recording starts", async () => {
		...
		await vi.waitFor(() =>
			expect(readActiveBrowserRecordingTabIds().size).toBe(2),
		);
		...
		expect(startScreencast.mock.calls).toEqual([
			["recording-tab"],
			["recording-tab-2"],
		]);
	...
	it("cancels a queued recording when stopped before its media grant", async () => {
		...
		await expect(secondStart).rejects.toBeInstanceOf(
			BrowserRecordingStartCancelledError,
		);
	...
	it("latches a stop that arrives before the start becomes queued", async () => {
		...
		await expect(delayedStart).rejects.toBeInstanceOf(
			BrowserRecordingStartCancelledError,
		);
	...
	it("finishes an uncontended pre-grant start before stopping", async () => {
		...
		const startPromise = startBrowserRecording("recording-tab");
		await vi.waitFor(() =>
			expect(readActiveBrowserRecordingTabIds().has("recording-tab")).toBe(
				true,
			),
		);
	...
	it("keeps a recording reachable through its runtime id after a server epoch changes", async () => {
		...
		expect(readActiveBrowserRecordingTabIds(threadRef)).toEqual(
			new Set([runtimeTabId]),
		);
		expect(readActiveBrowserRecordingTargets(threadRef)).toEqual([
			{ runtimeTabId, serverTabId: "tab_1" },
		]);
		expect(findActiveBrowserRecordingRuntimeTabId(threadRef, "tab_1")).toBe(
			runtimeTabId,
		);

		const replacementRuntimeTabId = previewRuntimeTabId(
			threadRef,
			"epoch-b",
			"tab_1",
		);
	...
	it("shares an in-progress stop with duplicate callers", async () => {
		...
		finishStoppingScreencast?.();
		const [firstArtifact, duplicateArtifact] = await Promise.all([
			firstStop,
			duplicateStop,
		]);
	...
	it("does not release the recording slot until a cancelled start settles", async () => {
		...
		const stopPromise = stopBrowserRecording("recording-tab");
		const restartAfterStop = stopPromise.then(() =>
