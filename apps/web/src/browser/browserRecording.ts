...
const isBrowserRecordingOperationError = Schema.is(
	BrowserRecordingOperationError,
);
const isBrowserRecordingCaptureTimeoutError = Schema.is(
	BrowserRecordingCaptureTimeoutError,
);
export const isBrowserRecordingStartCancelledError = Schema.is(
	BrowserRecordingStartCancelledError,
);
	...
	(): StartingBrowserRecordingLifecycle => {
		...
		const lifecycle: StartingBrowserRecordingLifecycle = {
			...
			cancelBeforeGrant: () => {
				...
				if (
					lifecycle.queuedForGrant &&
					!lifecycle.grantStarted &&
					!lifecycle.cancelledBeforeGrant
				) {
			...
			setQueuedForGrant: (queued) => {
				...
				if (queued && lifecycle.stopRequestedBeforeGrant)
					lifecycle.cancelBeforeGrant();
			},
		};
		return lifecycle;
	};
...
export function readActiveBrowserRecordingTabIds(
...
const prepareTabMediaCapture = (tabId: string, frameRate: number) => {
	...
	return {
		...
		cancel: () => {
			...
			if (pendingTabMediaCaptures.get(tabId) === pending)
...
const recordingStartupCancelledError = (
	...
	cause: unknown = new Error(
		`Browser recording startup was cancelled for tab ${recording.tabId}.`,
	),
): BrowserRecordingOperationError =>
...
const isRecordingStarting = (recording: ActiveRecording): boolean =>
	...
	recording.lifecycle.phase === "starting";

const waitForRecordingStartupToSettle = async (
	recording: ActiveRecording,
): Promise<void> => {
		...
		await Promise.race([
			...
			new Promise<void>((_, reject) => {
				timeout = setTimeout(() => {
					reject(
						new Error(
							`Browser recording startup did not settle for tab ${recording.tabId}.`,
						),
					);
				}, BROWSER_RECORDING_STARTUP_SETTLE_TIMEOUT_MS);
			}),
...
export async function startBrowserRecording(
	...
	const activeLogicalRecording =
		threadRef === null
			? null
			: findActiveBrowserRecordingRuntimeTabId(threadRef, serverTabId);
	...
	try {
			...
			try {
				...
				throw recordingStartupCancelledError(
					...
					new AggregateError(
						[
							new Error(
								`Browser recording startup was cancelled for tab ${tabId}.`,
							),
							cause,
						],
		...
		const grant = queueDisplayMediaGrant(async () => {
			...
			try {
				...
				const cleanupCause = await cleanupFailedRecordingStart(
					bridge,
					recording,
				);
				if (
					isBrowserRecordingCaptureTimeoutError(cause) &&
...
const discardBrowserRecording = async (
	...
	try {
		await bridge.recording
			.stopScreencast(recording.tabId)
			.catch(() => undefined);
...
export function stopBrowserRecording(
	...
	if (recording.lifecycle.phase === "starting")
		recording.lifecycle.cancelBeforeGrant();

	const stopPromise = Promise.resolve()
		.then(() => finalizeBrowserRecording(bridge, recording))
		.catch((error) => {
			if (
				isStartupWaitTimeout(error) &&
				activeRecordings.get(recording.tabId) === recording
			) {
				...
				recording.lifecycle = {
					phase: "stopping",
					stopPromise: cleanupAfterStartup,
				};
...
export async function stopBrowserRecordingForUpload(
	...
	(DesktopPreviewRecordingArtifact & { uploadedAttachmentId: string }) | null
> {
	const recording = activeRecordings.get(tabId);
