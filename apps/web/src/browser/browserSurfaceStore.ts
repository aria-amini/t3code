...
interface BrowserSurfaceStoreState {
	...
	readonly presentContent: (
		tabId: string,
		content: BrowserSurfaceContentPresentation,
	) => void;
				...
				return {
					byTabId: {
						...
						[tabId]: {
							...
							fittedSourceContent: fitSourceContent
...
export function acquireBrowserSurface(
	...
	return {
		present: (rect, visible, cornerRadius = 0, zIndex = 30) => {
			...
			if (useBrowserSurfaceStore.getState().byTabId[tabId]?.owner !== owner)
				return false;
