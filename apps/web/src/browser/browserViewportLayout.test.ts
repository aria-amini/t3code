...
describe("resolveBrowserViewportLayout", () => {
	...
	it("preserves the last logical viewport when fitting a fill-mode surface", () => {
		expect(
			resolveFittedBrowserViewport(
				{ _tag: "fill" },
				{ width: 320, height: 200, scale: 0.25 },
			),
	...
	it("bounds freeform drag sizes and total render area", () => {
		...
		const large = resizeFreeformViewport(
	...
	it("captures the available framed area when responsive mode is enabled", () => {
		expect(
			resolveResponsiveBrowserViewportSize({ width: 1200, height: 900 }),
		).toEqual({
			width: 1180,
			height: 858,
		});
		expect(
			resolveResponsiveBrowserViewportSize({ width: 1200, height: 900 }, 2),
		).toEqual({
			width: 590,
			height: 429,
		});
