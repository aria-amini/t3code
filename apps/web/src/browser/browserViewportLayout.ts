...
export const browserViewportSettingKey = (
	...
	if (sourceContent) {
		return {
			...
			width: Math.max(
				1,
				Math.round(
					sourceContent.width / sourceContent.scale / normalizedZoomFactor,
				),
			),
			height: Math.max(
				1,
				Math.round(
					sourceContent.height / sourceContent.scale / normalizedZoomFactor,
				),
			),
		};
	}
...
const clampViewportDimension = (value: number): number =>
	Math.min(
		PREVIEW_VIEWPORT_MAX_DIMENSION,
		Math.max(PREVIEW_VIEWPORT_MIN_DIMENSION, value),
	);

const validAspectRatio = (
	aspectRatio: number | undefined,
): aspectRatio is number =>
...
function resizeAtAspectRatio(
	...
	if (primaryAxis === "width") {
		const minimum = Math.ceil(
			Math.max(
				PREVIEW_VIEWPORT_MIN_DIMENSION,
				PREVIEW_VIEWPORT_MIN_DIMENSION * aspectRatio,
			),
		);
