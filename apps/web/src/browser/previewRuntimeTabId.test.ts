...
describe("previewRuntimeTabId", () => {
	...
	it("is stable for the same runtime tab", () => {
		...
		expect(previewRuntimeTabId(ref, null, "tab_1")).toBe(
			previewRuntimeTabId(ref, null, "tab_1"),
		);
	});
	...
	it("rejects a pinned operation target after the server epoch changes", () => {
		...
		expect(
			isCurrentPreviewRuntimeTab(ref, "epoch-a", "tab_1", runtimeTabId),
		).toBe(true);
		expect(
			isCurrentPreviewRuntimeTab(ref, "epoch-b", "tab_1", runtimeTabId),
		).toBe(false);
	});
});
