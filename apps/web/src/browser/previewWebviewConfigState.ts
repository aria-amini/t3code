...
export const loadPreviewWebviewConfig = (
	...
	if (bridge === null) {
		return Effect.fail(
			new PreviewWebviewBridgeUnavailableError({ environmentId }),
		);
	}
	...
	return Effect.tryPromise({
		try: () => bridge.getPreviewConfig(environmentId, profileId),
			...
			new PreviewWebviewConfigLoadError({ environmentId, cause }),
	});
};
...
): string => `${environmentId}${CONFIG_KEY_DELIMITER}${profileId ?? ""}`;

const parseConfigKey = (
	key: string,
): { environmentId: EnvironmentId; profileId?: string } => {
	const delimiter = key.indexOf(CONFIG_KEY_DELIMITER);
	const environmentId = (
		delimiter === -1 ? key : key.slice(0, delimiter)
	) as EnvironmentId;
	const profileId =
		delimiter === -1 ? "" : key.slice(delimiter + CONFIG_KEY_DELIMITER.length);
	return {
		environmentId,
		...(profileId === "" ? {} : { profileId }),
	};
...
export function usePreviewWebviewConfig(
	...
	const result = useAtomValue(
		previewWebviewConfigAtom(configKey(environmentId, profileId)),
	);
