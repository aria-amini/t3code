...
describe("browser favicon logic", () => {
	it("keys valid origins canonically while keeping distinct scopes separate", () => {
		expect(
			faviconKey("env:project", "http://myapp.test:3000/admin?x=1", null),
		).toBe("env:project http://myapp.test:3000");
		expect(
			faviconKey("env:project", "http://192.168.64.2:3000/", "192.168.64.2"),
		).toBe(faviconKey("env:project", "http://localhost:3000/", "192.168.64.2"));
	...
	it("retains an exact environment-host alias for offline lookup", () => {
		...
		expect(
			faviconKey("env:project", "http://[2001:4860:4860::8888]/", null),
		).toBe("env:project http://[2001:4860:4860::8888]:80");
	});

	it("accepts only bounded base64 PNG data", () => {
		expect(isStorableFaviconDataUrl(PNG)).toBe(true);
		expect(isStorableFaviconDataUrl("data:image/svg+xml;base64,AAAA")).toBe(
			false,
		);
		expect(isStorableFaviconDataUrl("data:image/png;base64,")).toBe(false);
		expect(isStorableFaviconDataUrl("data:image/png;base64,%%%%")).toBe(false);
		expect(
			isStorableFaviconDataUrl(`data:image/png;base64,${"A".repeat(8192)}`),
		).toBe(false);
	});
		...
		expect(
			migratePersistedBrowserFaviconState({
				byKey: {
					...
					"env:project http://localhost:3002": {
						dataUrl: PNG,
