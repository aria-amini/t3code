...
describe("browser favicon store", () => {
	...
	it("keeps the newest capture for an origin and permits an identical later revisit", () => {
		const recordFavicon = vi.spyOn(
			useBrowserFaviconStore.getState(),
			"recordFavicon",
		);
		recordFaviconForProject(
			projectRef,
			favicon("http://localhost:3000/", 20),
			null,
		);
		recordFaviconForProject(
			projectRef,
		...
		recordFaviconForProject(
			projectRef,
			favicon("http://localhost:3000/new", 30),
			null,
		);
		recordFaviconForProject(
			projectRef,
	...
	it("does not share localhost icons across environments or physical projects", () => {
		...
		const otherProject = scopeProjectRef(
			environmentId,
		...
		recordFaviconForProject(
			otherEnvironment,
			favicon("http://localhost:3000/", 2),
			null,
		);
		recordFaviconForProject(
			otherProject,
			favicon("http://localhost:3000/", 3),
			null,
		);
	...
	it("finds a persisted environment icon after shell hydration without a live connection", () => {
		recordFaviconForProject(
			projectRef,
			favicon("http://192.168.64.2:3000/app", 5),
			"192.168.64.2",
		);
		const byKey = useBrowserFaviconStore.getState().byKey;
		expect(
			lookupFavicon(byKey, null, "http://localhost:3000/app", null),
		).toBeNull();
		expect(
			lookupFavicon(byKey, projectRef, "http://localhost:3000/app", null),
		).toBe(PNG);
		expect(
			lookupFavicon(byKey, projectRef, "http://192.168.64.2:3000/app", null),
		).toBe(PNG);
		expect(
			lookupFavicon(byKey, projectRef, "http://192.168.64.3:3000/app", null),
		).toBeNull();
		expect(
			lookupFavicon(byKey, projectRef, "https://192.168.64.2:3000/app", null),
		).toBeNull();
		expect(
			lookupFavicon(byKey, projectRef, "http://192.168.64.2:3001/app", null),
		).toBeNull();
		expect(
			lookupFavicon(
				byKey,
	...
	it("finds a persisted IPv6 environment icon without a live connection", () => {
		recordFaviconForProject(
			projectRef,
			favicon("http://[fd00::1]:3000/app", 5),
			"fd00::1",
		);
		...
		expect(
			lookupFavicon(migrated, projectRef, "http://[fd00::1]:3000/app", null),
		).toBe(PNG);
	});
	...
	it("keeps exact host aliases attached to the newest canonical icon", () => {
		...
		for (const host of ["192.168.64.2", "192.168.64.3", "192.168.64.4"]) {
			expect(
				lookupFavicon(byKey, projectRef, `http://${host}:3000/`, null),
			).toBe(newerPng);
		}
	...
	it("evicts an icon and its exact host aliases atomically", () => {
		recordFaviconForProject(
			projectRef,
			favicon("http://192.168.64.2:3000/", 100),
			"192.168.64.2",
		);
		...
		recordFaviconForProject(
			projectRef,
			favicon("https://evicts-oldest.example/", 1_000),
			null,
		);
		expect(
			lookupFavicon(
	...
	it("retains multiple origins until project and connection metadata hydrate", () => {
		...
		expect(
			Object.keys(
				Object.values(
					useBrowserFaviconStore.getState().pendingByThreadKey,
				)[0] ?? {},
			),
		).toHaveLength(2);

		expect(
			flushPendingFaviconsForThread(threadRef, projectRef, "192.168.64.2"),
		).toBe(true);
		expect(
			Object.keys(useBrowserFaviconStore.getState().byKey).toSorted(),
		).toEqual([
			"env-1:project-1 http://localhost:3000",
	...
	it("persists unambiguous loopback captures while the environment is offline", () => {
		...
		expect(
			flushPendingFaviconsForThread(threadRef, projectRef, undefined),
		).toBe(false);
		expect(
			Object.values(useBrowserFaviconStore.getState().pendingByThreadKey)[0],
		).toBeDefined();
	});
	...
	it("keeps pending captures in store-owned state independent of bridge lifetime", () => {
		recordFaviconForThread(
			threadRef,
			favicon("http://localhost:3000/", 10),
			null,
			undefined,
		);
		...
		useBrowserFaviconStore.setState({
			pendingByThreadKey: pendingAfterUnmount,
		});
	...
	it("flushes and resolves a pending draft-thread favicon after physical project registration", () => {
		recordFaviconForThread(
			threadRef,
			favicon("http://localhost:8025/", 10),
			null,
			undefined,
		);
		registerFaviconProjectForThread(threadRef, projectRef);
		const registered =
			useBrowserFaviconStore.getState().projectRefByThreadKey["env-1:thread-1"];
		expect(registered).toEqual(projectRef);
		expect(
			flushPendingFaviconsForThread(threadRef, registered!, undefined),
		).toBe(true);
		expect(
			lookupFavicon(
				useBrowserFaviconStore.getState().byKey,
		...
		const merged = mergeBrowserFaviconState(
			{
				byKey: {
					...
					"env-1:project-1 http://localhost:3001": {
						dataUrl: "bad",
						capturedAt: 3,
					},
					"env-1:project-1 http://localhost:3002": {
						dataUrl: PNG,
						capturedAt: 1e308,
					},
