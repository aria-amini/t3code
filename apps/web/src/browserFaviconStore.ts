...
function addPendingFavicon(
	...
	if (existing && existing.capturedAt >= favicon.capturedAt)
	...
	return Object.fromEntries(
		[...Object.entries(withoutThread), [threadKey, boundedForThread]].slice(
			-MAX_PENDING_THREADS,
		),
	);
}
			...
			recordFavicon: (key, dataUrl, capturedAt, aliases = []) =>
				set((state) => {
					...
					const aliasesUnchanged =
						...
						nextAliases.every(
							(alias, index) => alias === existing?.aliases?.[index],
						);
					if (existing && capturedAt <= existing.capturedAt && aliasesUnchanged)
						return state;
					return {
...
export function registerFaviconProjectForThread(
	...
	useBrowserFaviconStore.setState({
		projectRefByThreadKey: Object.fromEntries(
			[
				...Object.entries(state.projectRefByThreadKey).filter(
					([key]) => key !== threadKey,
				),
...
export function useFaviconProjectRefForThread(
	threadRef: ScopedThreadRef,
	...
	const shellProjectRef = useMemo(
		() =>
			shellProjectId
				? scopeProjectRef(threadRef.environmentId, shellProjectId)
				: null,
...
export function recordFaviconForThread(
	...
	const threadKey = scopedThreadKey(threadRef);
	const state = useBrowserFaviconStore.getState();
	const pendingByThreadKey = addPendingFavicon(
...
export function flushPendingFaviconsForThread(
	...
	const threadKey = scopedThreadKey(threadRef);
	const pending =
		useBrowserFaviconStore.getState().pendingByThreadKey[threadKey];
...
export function useFaviconForThreadUrl(
	threadRef: ScopedThreadRef,
	url: string,
): string | null {
...
export function lookupFavicon(
	...
	try {
		...
		const localKey = faviconKey(
			scopedProjectKey(projectRef!),
			url,
			requestedHost,
		);
		const localEntry = localKey ? byKey[localKey] : null;
		return localEntry?.aliases?.includes(requestedHost)
			? localEntry.dataUrl
			: null;
	} catch {
		return null;
