...
function entry(
	overrides: Partial<BrowserHistoryEntry> = {},
): BrowserHistoryEntry {
...
function spyOnPersistWrites() {
	...
	if (!storage)
		throw new Error("Browser history persistence storage is unavailable.");
...
describe("normalizeHistoryUrl", () => {
	...
	it("preserves hash routes and strips credentials", () => {
		...
		expect(normalizeHistoryUrl("https://user:secret@example.com/")).toBe(
			"https://example.com/",
		);
	});
	...
	it("rejects non-http(s), unparseable, and oversized urls", () => {
		...
		expect(
			normalizeHistoryUrl(`http://localhost/${"a".repeat(2048)}`),
		).toBeNull();
	});
});
...
describe("upsertHistoryEntry", () => {
	it("prepends new urls", () => {
		...
		expect(next.map((e) => e.url)).toEqual([
			"http://localhost:5173/",
			"http://localhost:3000/",
		]);
	...
	it("caps the list at the per-project limit", () => {
		const full = Array.from(
			{ length: BROWSER_HISTORY_MAX_ENTRIES_PER_PROJECT },
			(_, i) =>
		...
		expect(next.some((e) => e.url === `http://localhost:${lastPort}/`)).toBe(
			false,
		);
	...
	it("with insertOrdered, slots an older entry below a newer one instead of prepending", () => {
		const existing = [
			entry({ url: "http://newer.test/", lastVisitedAt: 2000 }),
		];
	...
	it("with insertOrdered, replaying an older visit for an existing entry keeps its newer timestamp", () => {
		...
		const next = upsertHistoryEntry(existing, "http://a.test/", 1000, {
			insertOrdered: true,
		});
...
describe("migratePersistedBrowserHistoryState", () => {
	it("drops malformed state and invalid entries", () => {
		expect(migratePersistedBrowserHistoryState(null)).toEqual({
			byProjectKey: {},
		});
		expect(migratePersistedBrowserHistoryState({ byProjectKey: 42 })).toEqual({
			byProjectKey: {},
		});
	...
	it("restores MRU ordering, deduplicates normalized urls, and enforces project bounds", () => {
		...
		expect(Object.keys(migrated.byProjectKey)).toHaveLength(
			BROWSER_HISTORY_MAX_PROJECTS,
		);
...
describe("useBrowserHistoryStore", () => {
	...
	it("records visits for registered threads under the project key", () => {
		useBrowserHistoryStore
			.getState()
			.registerThreadProject(threadRef, "proj-a");
	...
	it("sets titles update-only via the thread helper", () => {
		...
		expect(
			useBrowserHistoryStore.getState().byProjectKey["proj-a"]?.[0]?.title,
		).toBe("My App");
	});
	...
	it("sets a title against a settled url that differs from the stored one only by a trailing slash", () => {
		...
		expect(
			useBrowserHistoryStore.getState().byProjectKey["proj-a"]?.[0],
		).toMatchObject({
	...
	it("removes entries", () => {
		...
		expect(
			useBrowserHistoryStore
				.getState()
				.byProjectKey["proj-a"]?.map((e) => e.url),
		).toEqual(["http://b.test/"]);
	});
});
...
describe("pendingVisitsByThreadKey", () => {
	...
	it("queues a visit recorded before registration and drains it in order on registration", () => {
		...
		expect(
			useBrowserHistoryStore
				.getState()
				.byProjectKey["proj-a"]?.map((e) => e.url),
		).toEqual(["http://b.test/", "http://a.test/"]);
		expect(
			useBrowserHistoryStore.getState().byProjectKey["proj-a"]?.[0]
				?.lastVisitedAt,
		).toBe(2);
		expect(
			useBrowserHistoryStore.getState().byProjectKey["proj-a"]?.[1]
				?.lastVisitedAt,
		).toBe(1);
	...
	it("caps the per-thread pending list at 10, dropping the oldest", () => {
		...
		useBrowserHistoryStore
			.getState()
			.registerThreadProject(threadRef, "proj-a");
		const urls = useBrowserHistoryStore
			.getState()
			.byProjectKey["proj-a"]?.map((e) => e.url);
	...
	it("slots a replayed visit by timestamp instead of hoisting it above a newer live visit", () => {
		...
		useBrowserHistoryStore
			.getState()
			.registerThreadProject(otherThreadRef, "proj-a");
