...
it("initializes React refresh before a shared UI chunk runs in bundled dev", async () => {
	const root = await NodeFSP.mkdtemp(
		NodePath.join(NodeOS.tmpdir(), "t3-bootstrap-"),
	);
...
export const startup = Promise.resolve().then(() => globalThis.onStarted(Shared()));`,
		...
		server = await createServer({
			...
			resolve: {
				alias: {
					react: NodePath.dirname(
						NodeURL.fileURLToPath(import.meta.resolve("react")),
					),
				},
			},
					...
					output: {
						...
						codeSplitting: {
							groups: [
								{
									name: "vendor",
									test: /node_modules|@react-refresh/,
									priority: 10,
								},
								{
									name: "shared-ui",
...
it("hot updates Tailwind classes when a source file changes in bundled dev", async () => {
	const root = await NodeFSP.mkdtemp(
		NodePath.join(NodeOS.tmpdir(), "t3-tailwind-"),
	);
	...
	try {
		...
		const entry = await fetch(
			`http://127.0.0.1:${address.port}/assets/index.js`,
		);
