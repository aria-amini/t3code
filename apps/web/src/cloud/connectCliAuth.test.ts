...
describe("connectCliAuth", () => {
	...
	it("sends the sign-in redirect to the authorize endpoint, not back to /connect", () => {
		...
		const connectUrl =
			"https://app.t3.codes/connect#state=state-1&challenge=challenge-1";
	...
	it("reads the code and state Clerk echoes back to the callback", () => {
		...
		expect(
			readConnectCliCallbackResult(
				new URL("https://app.t3.codes/connect/callback?code=abc"),
			),
		).toBeNull();
		expect(
			readConnectCliCallbackResult(
				new URL("https://app.t3.codes/connect/callback?state=s"),
			),
		).toBeNull();
	});
});
