		...
		(database) =>
			Effect.callback<BrowserDpopKey | null, BrowserDpopError>((resume) => {
				...
				request.addEventListener("error", () =>
					resume(
						Effect.fail(
							dpopError("Could not read DPoP key.", request.error ?? undefined),
						),
					),
				);
				request.addEventListener("success", () =>
					resume(
						Effect.succeed(
							(request.result as BrowserDpopKey | undefined) ?? null,
						),
					),
				);
			}),
...
export const generateBrowserDpopKey = Effect.gen(function* () {
	...
	const privateKey = yield* Effect.tryPromise({
		try: () =>
			importJWK(privateJwk as JWK, "ES256", {
				extractable: false,
...
> {
	return Effect.gen(function* () {
		...
		const jti = yield* Crypto.Crypto.pipe(
			...
			Effect.mapError((cause) =>
				dpopError("Could not generate DPoP proof identifier.", cause),
			),
		);
