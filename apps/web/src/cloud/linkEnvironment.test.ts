// The formatter wraps `JSON.parse(...)` in this file onto lines after where
// upstream put per-line suppression comments, so disable the rule for the
// whole file.
...
vi.mock("./relayClientInstallDialog", () => ({
	requestRelayClientInstallConfirmation:
		relayClientInstallDialog.requestConfirmation,
		...
		Effect.gen(function* () {
			const client = {
				[WS_METHODS.cloudGetRelayClientStatus]: () =>
					Effect.succeed(
						options?.status ?? { status: "available", version: "2026.6.0" },
					),
...
describe("web cloud link environment client", () => {
	it.effect("reads primary cloud link state from the explicit target", () =>
		Effect.gen(function* () {
			...
			const state = yield* withServices(
				readPrimaryCloudLinkState({ target: TARGET }),
			);
	...
	it.effect(
		"updates agent activity publishing for the explicit primary target",
		() =>
			Effect.gen(function* () {
				const fetchMock = vi.fn().mockResolvedValue(
					Response.json({
				...
				expect(
					JSON.parse(bodyText(fetchMock.mock.calls[0]?.[1]?.body)),
				).toEqual({
					publishAgentActivity: true,
				});
			}),
	...
	it.effect(
		"links an available primary environment without invoking installation",
		() =>
			Effect.gen(function* () {
				const fetchMock = vi
					.fn()
				...
				expect(
					JSON.parse(bodyText(fetchMock.mock.calls[1]?.[1]?.body)),
				).toMatchObject({
					challenge: "challenge",
					endpoint: {
						httpBaseUrl: TARGET.httpBaseUrl,
