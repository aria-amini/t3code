...
function ensureRelayClientAvailable(
	...
	return Effect.gen(function* () {
		...
		const status = yield* registry
			...
			.pipe(
				Effect.mapError(
					relayClientRpcError("Could not check relay client availability."),
				),
			);
		...
		const confirmed = yield* Effect.tryPromise({
			try: () => requestRelayClientInstallConfirmation(status.version),
				...
				"Could not confirm relay client installation.",
			),
		});
		if (!confirmed) {
			return yield* new CloudEnvironmentLinkError({
		...
		const installed = yield* registry
			.runStream(
				...
				runStream(WS_METHODS.cloudInstallRelayClient, {}).pipe(
					Stream.tap((event) =>
						Effect.sync(() => reportRelayClientInstallProgress(event)),
					),
				),
			)
			.pipe(
				Stream.runLast,
				Effect.mapError(
					relayClientRpcError("Could not install the relay client."),
				),
...
> {
	return Effect.gen(function* () {
		...
		return yield* client.connect
			...
			.pipe(
				Effect.mapError(
					environmentApiError("Could not read environment cloud link state."),
				),
			);
...
> {
	return Effect.gen(function* () {
		...
		yield* client.connect
			...
			.pipe(
				Effect.mapError(
					environmentApiError("Could not unlink the environment from cloud."),
				),
			);
...
const PUBLISH_ONLY_PROVIDER_KIND =
	"manual" satisfies RelayManagedEndpointProviderKind;
...
> {
	return Effect.gen(function* () {
		...
		const proof = yield* environmentClient.connect
			...
			.pipe(
				Effect.mapError(
					environmentApiError("Could not obtain environment link proof."),
				),
			);
		const link = yield* relayClient
			...
			.pipe(
				Effect.mapError(
					decodedRelayClientError(
						`${configuredRelayUrl}/v1/client/environment-links failed`,
					),
				),
			);
		...
		yield* environmentClient.connect
			...
			.pipe(
				Effect.mapError(
					environmentApiError("Could not configure environment relay access."),
				),
			);
	}).pipe(Effect.provide(primaryEnvironmentHttpLayer));
}
