...
const managedRelayAtomRuntime = Atom.runtime(
	Layer.effect(
		...
		runtime.contextEffect.pipe(
			Effect.map((context) =>
				Context.get(context, ManagedRelay.ManagedRelayClient),
			),
		),
	),
);

const managedRelayQueryManager = createManagedRelayQueryManager(
...
export const deregisterManagedRelayEnvironmentCommand = createRuntimeCommand(
	managedRelayAtomRuntime,
	{
		...
		execute: (input, registry) =>
			deregisterManagedRelayEnvironment(registry, input),
	},
);
...
export function useManagedRelayDevices() {
	...
	const accountId = session?.accountId ?? null;
	const atom = accountId
		? managedRelayQueryManager.devicesAtom(accountId)
		: EMPTY_DEVICES_ATOM;
	const result = useAtomValue(atom);
	const snapshot = readManagedRelaySnapshotState(result);
	useEffect(() => {
