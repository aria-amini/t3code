	...
	return {
		...
		relayTracing: {
			...
			tracesToken: trimNonEmpty(
				import.meta.env.VITE_RELAY_OTLP_TRACES_TOKEN as string | undefined,
			),
		},
	};
}
...
export function resolveRelayTracingConfig() {
	...
	return relayTracing.tracesUrl &&
		relayTracing.tracesDataset &&
		relayTracing.tracesToken
		? {
...
export function hasCloudPublicConfig(): boolean {
	...
	return Boolean(
		config.clerkPublishableKey && config.clerkJwtTemplate && config.relayUrl,
	);
}
...
export function resolveRelayClerkTokenOptions() {
	...
	if (!clerkJwtTemplate) {
		throw new CloudPublicConfigMissingError({
			key: "T3CODE_CLERK_JWT_TEMPLATE",
		});
	}
