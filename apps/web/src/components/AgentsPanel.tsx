	...
	return (
		<span
			...
			className={cn(
				...
				STATUS_VISUALS[status].dotClass,
			)}
		/>
	);
}
...
function AgentRow({ agent }: { agent: RuntimeSubagent }) {
	...
	const role =
		agent.role?.trim().toLocaleLowerCase() ===
		agent.title.trim().toLocaleLowerCase()
			? null
			: agent.role;
	const metadata = [
		modelLabel,
					...
					>
						...
						<span className="flex items-center gap-0.5">
							{phase.members.length === 0 ? (
								<span className="font-mono text-[.6rem] text-muted-foreground/50">
									–
								</span>
							) : (
			...
			>
				...
				<span className="ml-auto flex items-center gap-1.5 font-mono text-[.7rem] text-muted-foreground/80">
					...
					<span className="tabular-nums">
						· {formatSubagentTokenCount(totalTokens)} tok
					</span>
		...
		return (
			<div className="flex h-full flex-col items-center justify-center gap-2 p-6 text-center">
				...
				<p className="max-w-56 text-xs text-muted-foreground">
					When this thread spawns subagents or runs a workflow, they show up
			...
			<footer className="flex items-center justify-between border-t border-border/60 px-3 py-1.5 font-mono text-[.7rem] text-muted-foreground">
				<span className="flex items-center gap-2">
					...
					{model.settledCount > 0 ? (
						<span>{model.settledCount} settled</span>
					) : null}
				</span>
				<span className="tabular-nums">
