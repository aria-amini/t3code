...
function SidebarControl() {
	...
	useEffect(() => {
		const onKeyDown = (event: KeyboardEvent) => {
			...
			if (resolveShortcutCommand(event, keybindings) !== "sidebar.toggle")
				return;
...
export function AppSidebarLayout({ children }: { children: ReactNode }) {
	...
	const routePanelAnimationsActive =
		panelAnimationsActive && !panelAnimationsSuppressed;
	const isOnSettings =
	...
	const [isWindowFullscreen, setIsWindowFullscreen] = useState(() => {
		const getWindowFullscreenState =
			window.desktopBridge?.getWindowFullscreenState;
