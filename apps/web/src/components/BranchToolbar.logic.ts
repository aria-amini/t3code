...
const GENERIC_LOCAL_ENVIRONMENT_LABELS = new Set([
	"local",
	"local environment",
]);
...
export function resolveContextStripLabelsCompact(input: {
	...
	return input.compact
		? input.neededWidth >
				input.availableWidth - CONTEXT_STRIP_COMPACT_EXPAND_HYSTERESIS_PX
...
export function resolvePreviousWorktreeSeed(input: {
	...
	return latest === null
		? null
		: { branch: latest.branch, worktreePath: latest.worktreePath };
}
...
export function resolveBranchToolbarValue(input: {
	...
	const { envMode, activeWorktreePath, activeThreadBranch, currentGitBranch } =
		input;
...
export function resolveBranchToolbarPrBranch(input: {
	...
	return input.activeThreadBranch === input.resolvedActiveBranch
...
export function resolveBranchSelectionTarget(input: {
	...
	const nextWorktreePath =
		activeWorktreePath !== null && refName.isDefault
			? null
			: activeWorktreePath;
