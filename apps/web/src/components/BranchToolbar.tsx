...
function useLabelsOverflow(element: HTMLDivElement | null): boolean {
	...
	const measure = useCallback(() => {
		...
		const stripGap =
			Number.parseFloat(getComputedStyle(current).columnGap) || 0;
		let needed = 0;
		let groups = 0;
		for (const child of current.children) {
			...
			const hostedControls = child.matches(
		...
		for (const label of current.querySelectorAll<HTMLElement>(
			"[data-composer-label]",
		)) {
			...
			pendingLabelRectsRef.current = new Map(
				Array.from(
					current.querySelectorAll<HTMLElement>(
						COMPOSER_CONTEXT_LABEL_SELECTOR,
					),
				...
				[
					{
						...
						maxWidth: `${previousRect.width}px`,
					},
...
export const BranchToolbar = memo(function BranchToolbar({
	...
	const canUsePreviousWorktree =
		draftThread !== null && serverThread === null && !envModeLocked;
