...
export function BranchToolbarBranchSelector({
	...
	const stopThreadSession = useAtomCommand(
	...
	const activeThreadId =
		serverThread?.id ?? (draftThread ? threadId : undefined);
	...
	const hasNextPage =
		branchRefState.data?.nextCursor !== null &&
		branchRefState.data?.nextCursor !== undefined;
	const isFetchingNextPage = branchRefState.isFetchingNextPage;
	const isInitialBranchesLoadPending =
		branchRefState.isPending && branchRefState.data === null;
	...
	const normalizedDeferredBranchQuery =
		deferredTrimmedBranchQuery.toLowerCase();
	...
	const [resolvedActiveBranch, setOptimisticBranch] = useOptimistic(
		...
		(_currentBranch: string | null, optimisticBranch: string | null) =>
			optimisticBranch,
	);
	const listedActiveBranch =
		resolvedActiveBranch === null
			? null
			: (branchByName.get(resolvedActiveBranch) ?? null);
	...
	const [showBottomBranchScrollFade, setShowBottomBranchScrollFade] =
		useState(false);
	...
	const branchPr =
		branchPrBranch !== null &&
		branchStatusQuery.data?.refName === branchPrBranch
		...
		>
            ...
            while refs are loading or a branch action is pending. */}
				<span
					...
					onContextMenu={(event) =>
						handleBranchContextMenu(event, resolvedActiveBranch)
					}
				>
					<ComboboxTrigger
				...
				<div className="flex min-h-0 flex-1 flex-col overflow-hidden">
					...
					{isSelectingWorktreeBase ? (
						<Tooltip>
							...
							>
								Creates the worktree from the latest matching branch on origin
								instead of your local branch.
							</TooltipPopup>
						</Tooltip>
					) : null}
					{branchStatusText ? (
						<ComboboxStatus>{branchStatusText}</ComboboxStatus>
					) : null}
				</div>
			</ComboboxPopup>
		</Combobox>
