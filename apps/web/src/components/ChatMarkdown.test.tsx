...
describe("ChatMarkdown favicon privacy", () => {
	it("suppresses private link images while preserving public links across updates", async () => {
		...
		const markdown = (url: string) => (
			<ChatMarkdown cwd="/tmp/project" text={`[Link](${url})`} />
		);
		try {
			await act(async () => {
			...
			expect(
				renderer!.root.findAllByType("img").map((image) => image.props.src),
			).toEqual([
			...
			await act(async () => {
				renderer!.update(
					markdown("https://github.com/pingdotgg/t3code/pull/1"),
				);
			});
			expect(renderer!.root.findAllByType("img")).toHaveLength(0);
...
describe("ChatMarkdown streaming", () => {
	it("recovers highlighting after a failed fence changes without resetting its controls", async () => {
		...
		try {
			...
			const wrap = codeButton(
				mounted,
				initialWrap ? "Disable line wrap" : "Wrap lines",
			);
			await act(async () => {
				wrap.onClick?.({} as Parameters<NonNullable<typeof wrap.onClick>>[0]);
			});
			expect(
				mounted.root.findAllByProps({ className: "chat-markdown-shiki" }),
			).toHaveLength(0);
			...
			expect(mounted.root.findByProps({ "data-language": "text" })).toBe(
				codeBlock,
			);
	...
	it("preserves code controls and details without highlighting an unchanged fence again", async () => {
		...
		try {
			await act(async () => {
				renderer = create(
					<ChatMarkdown cwd="/tmp/project" text={text} isStreaming />,
				);
			});
			const mounted = renderer!;
			const codeBlock = mounted.root.findByProps({ "data-language": "text" });
			...
			const detailsButton = mounted.root.find(
				(instance) =>
					...
					instance.props["data-markdown-details-summary"] === "",
			);
			await act(async () => {
			...
			expect(codeBlock.props["data-wrap"]).toBe(String(!initialWrap));
			expect(mounted.root.findByProps({ "data-markdown-details": "" })).toBe(
				details,
			);
			expect(details.props["data-markdown-details-open"]).toBe("true");
			await act(async () => {
			...
			await act(async () => {
				copyUpdated.onClick?.(
					{} as Parameters<NonNullable<typeof copyUpdated.onClick>>[0],
				);
			});
			...
			await act(async () => {
				onChange({
					currentTarget: {
						...
						closest: () => ({
							dataset: {
								taskMarkerOffset: String(
									listItem.props["data-task-marker-offset"],
								),
							},
						}),
					},
...
describe("canUseMarkdownFileShellActions", () => {
	...
	it("allows editor and file manager actions for local environments", () => {
		expect(
			canUseMarkdownFileShellActions(environmentId, "local-exec", true),
		).toBe(true);
	});

	it("hides shell actions until the environment mode is resolved", () => {
		expect(
			canUseMarkdownFileShellActions(environmentId, "local-exec", false),
		).toBe(false);
	});

	it("hides editor and file manager actions for remote environments", () => {
		expect(
			canUseMarkdownFileShellActions(environmentId, "remote-links", true),
		).toBe(false);
		expect(
			canUseMarkdownFileShellActions(environmentId, "remote-unavailable", true),
		).toBe(false);
	});

	it("hides shell actions when no environment owns the markdown", () => {
		expect(canUseMarkdownFileShellActions(null, "local-exec", true)).toBe(
			false,
		);
	});
});
...
describe("ChatMarkdown skill chips", () => {
	it("updates digit-leading skill labels when discovered skills change", async () => {
		...
		try {
			...
			await act(async () => {
				mounted.update(
					<ChatMarkdown cwd="/tmp/project" text={text} skills={[]} />,
				);
			});
			expect(labels("2Spec")).toHaveLength(0);
		} finally {
...
describe("ChatMarkdown file option chips", () => {
	...
	it("preserves escaped Codex file citations as literal text", () => {
		const html = renderToStaticMarkup(
			<ChatMarkdown
				cwd="/tmp/project"
				text={
					'Example: \\:codex-file-citation{path="/tmp/project/outputs/report.xlsx"}'
				}
			/>,
		);
	...
	it("does not create a nested link for citations inside link text", () => {
		...
		const html = renderToStaticMarkup(
			<ChatMarkdown
				cwd="/tmp/project"
				text={`[See ${directive}](https://example.com)`}
			/>,
		);
...
describe("ChatMarkdown artifact-template cards", () => {
	...
	it("leaves escaped and similarly named artifact-template directives literal", () => {
		for (const text of [
			...
			ARTIFACT_TEMPLATE_DIRECTIVE.replace(
				"::artifact-template",
...
describe("ChatMarkdown Windows file links", () => {
	...
	it.each([true, false])(
		"preserves drive paths with parseRawHtml=%s",
		(parseRawHtml) => {
			const html = renderToStaticMarkup(
				<ChatMarkdown
	...
	it.each([true, false])(
		"normalizes backslashes with parseRawHtml=%s",
		(parseRawHtml) => {
			const html = renderToStaticMarkup(
				<ChatMarkdown
					cwd="C:/Users/shawn/project"
	...
	it.each([true, false])(
		"preserves reference links with parseRawHtml=%s",
		(parseRawHtml) => {
			const html = renderToStaticMarkup(
				<ChatMarkdown
					cwd="C:/Users/shawn/project"
