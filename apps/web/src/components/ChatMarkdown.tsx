...
import {
	...
	markdownImageItems,
} from "./chat/markdownImageGallery";
...
function CodexArtifactTemplateCard(props: {
	...
	const presentationLabel = codexArtifactTemplatePresentationLabel(
		props.template.artifactKind,
	);
...
function reportMarkdownActionFailure(
...
function findTaskListMarkerOffset(
	markdown: string,
...
function meaningfulHastChildren(
	node: MarkdownImageHastNode,
): MarkdownImageHastNode[] {
...
function soleImageDescendant(
	node: MarkdownImageHastNode,
): MarkdownImageHastNode | undefined {
...
function markStandaloneImages(node: MarkdownImageHastNode) {
	...
	if (
		node.type === "root" ||
		(node.tagName && STANDALONE_IMAGE_BLOCKS.has(node.tagName))
	) {
	...
	return (tree: MarkdownImageHastNode) => {
		const visit = (node: MarkdownImageHastNode) => {
			...
			if (node.type === "element" && node.tagName === "img") {
				node.properties = {
					...node.properties,
					...(typeof src === "string" && isWindowsDrivePathHref(src)
						? { dataLocalSrc: src }
						: {}),
...
function MarkdownDetails({
	...
	return (
		...
		>
			...
			<CollapsiblePanel>
				<div
					className="pb-3 ps-6 text-foreground/80"
...
function extractInlineCodeSpans(text: string): string[] {
	...
	for (let index = 0; index < segments.length; index += 2) {
		for (const match of (segments[index] ?? "").matchAll(
...
function normalizeMarkdownLinkHrefKey(href: string): string {
	...
	const rewrittenHref =
		rewriteMarkdownFileUriHref(normalizedHref) ?? normalizedHref;
...
const CHAT_MARKDOWN_MEDIA_FRAME_CLASS_NAME =
...
function ChatMarkdownMediaUnavailableLabel(props: {
	...
	const label =
		props.kind === "video" ? "Video unavailable" : "Image unavailable";
	return (
...
function ChatMarkdownImage(props: {
	...
	readonly onImageExpand?:
		| ((preview: ExpandedImagePreview) => void)
		| undefined;
}) {
	...
	const failed =
		props.sourceFailed === true || (src !== null && failedSrc === src);
	// A failure forgets the decoded image so the next URL loads behind the slot.
	const settled =
		src !== null && !failed && (!props.standalone || loadedSrc !== null);
...
export const ChatMarkdownAssetImage = memo(
...
function hastHasText(node: unknown): boolean {
	...
	return (
		"children" in node &&
		Array.isArray(node.children) &&
		node.children.some(hastHasText)
	);
}
...
function findMarkdownFragmentTarget(
	...
	const matchesFragment = (element: HTMLElement) =>
		...
		normalizeSanitizedFragmentId(element.id) === normalizedId;
		...
		async (position: { x: number; y: number }) => {
			...
			try {
				const clicked = await api.contextMenu.show(
					[
						...(onOpenMedia
							? ([{ id: "preview-media", label: "Preview media" }] as const)
							: []),
						...(onOpen
...
function useChatMarkdownState({
	...
	const searchProjectEntries = useAtomQueryRunner(
					...
					if (preview && mediaRequestId.current === requestId) {
						...
						expandMedia(
							selected && selected.type !== "video" && markdownRef.current
								? markdownImageGallery(
										clickedImage ?? markdownRef.current,
										selected,
									)
								: preview,
						);
	...
	const markdownFileLinkMetaByHref = useMemo(() => {
		...
		>();
		for (const href of extractMarkdownLinkHrefs(
			renderCodexFileCitationsAsMarkdown(text),
		)) {
	...
	const fileLinkParentSuffixByPath = useMemo(() => {
		const filePaths = [
			...
			...[...inlineCodeFileLinkMetaByText.values()].map(
				(meta) => meta.filePath,
			),
		];
	...
	const linkTargetPreference = useClientSettings(
		(settings) => settings.browserLinkTarget,
	);
	const resolveThreadPullRequest = useCallback(
		(href: string): ThreadLinkedPullRequest | null => {
			if (
				...
				threadServerConfig?.environment.capabilities
		...
		async (href: string, linked: boolean) => {
			...
			if (!linked) {
				const currentPullRequest =
					readThreadShell(threadRef)?.linkedPullRequest;
				if (
	...
	const openExternalLinkInPreview = useCallback(
		(url: string) => {
			...
			return openUrlInPreview({ threadRef, url, openPreview }).then(
				(result) => {
	...
	const revealMarkdownFileInFileManager = useCallback(
		async (fileLinkMeta: MarkdownFileLinkMeta) => {
			...
			const filePath =
				match && cwd
					? resolvePathLinkTarget(match, cwd)
...
const CHAT_MARKDOWN_COMPONENTS = {
	div: function MarkdownDiv({ node, children, ...props }) {
		...
		const artifactTemplate = artifactTemplateFromHastProperties(
			node?.properties,
		);
		if (artifactTemplate) {
			return (
	...
	a: function MarkdownAnchor({
		...
		if (!fileLinkMeta) {
			...
			const canOpenInPreview =
				Boolean(threadRef) && isPreviewSupportedInRuntime();
			const linkChildren = (
				...
				<a
					...
					onClick={(event) => {
						...
						void openExternalLinkInPreview(href).then((result) => {
							...
							if (
								squashAtomCommandFailure(result) instanceof
								BrowserSettingsReadError
							)
								return;
	...
	code: function MarkdownCode({ node, children, className, ...props }) {
		const { cwd, imageBaseDir, inlineCodeFileLinkMetaByText, fileLinkChip } =
			use(ChatMarkdownRendererContext);
	...
	img: function MarkdownImage({ node, title, src, alt, ...props }) {
		const { expandMedia, cwd, imageBaseDir, threadRef } = use(
		...
		const authoredTitle =
			typeof markdownTitle === "string" ? markdownTitle : title;
		const srcString =
			typeof authoredSrc === "string"
				? normalizeMarkdownLinkDestination(authoredSrc)
				: "";
		const classifiedSrc =
		...
		if (imageSource._tag === "Direct") {
			...
			const originalUrl =
				resolveExternalWebLinkHost(imageSource.uri) !== null
					? imageSource.uri
					: undefined;
	...
	pre: function MarkdownPre({ node, children, ...props }) {
		const { resolvedTheme, diffThemeName, isStreaming } = use(
			ChatMarkdownRendererContext,
		);
		...
		>
			<ChatMarkdownRendererContext value={componentState}>
				<ReactMarkdown
					...
					rehypePlugins={
						parseRawHtml ? CHAT_MARKDOWN_REHYPE_PLUGINS : undefined
					}
					skipHtml={false}
