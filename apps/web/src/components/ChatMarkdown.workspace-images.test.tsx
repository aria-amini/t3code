...
vi.mock("../assets/assetUrls", () => ({
	...
	useAssetUrlState: (_environmentId: unknown, resource: unknown) => {
		...
		return {
			...
			...(testState.imageDimensions
				? { imageDimensions: testState.imageDimensions }
				: {}),
		};
	},
}));
...
function renderWithoutThread(markdown: string): string {
	return renderToStaticMarkup(
		<ChatMarkdown cwd={"C:\\Users\\shawn\\project"} text={markdown} />,
	);
}
...
describe("ChatMarkdown workspace images", () => {
	...
	it("loads every Windows workspace path form through a signed asset URL", () => {
		...
		expect(
			html.match(
				/<img[^>]*src="https:\/\/signed\.test\/workspace-image\.svg"/g,
			),
		).toHaveLength(4);
		...
		expect(testState.resources).toEqual([
			{
				_tag: "media-file",
				threadId: threadRef.threadId,
				path: "/tmp/embed-test/2.png",
			},
			{
				_tag: "media-file",
				threadId: threadRef.threadId,
				path: "/tmp/embed-test/5.png",
			},
		]);
		expect(html).not.toContain("Image unavailable");
	});
	...
	it("keeps a tall image placeholder and loaded image at the same proportional bounds", () => {
		const markdown =
			'<img src=".t3/workspace-image.svg" alt="sized" width="96" height="128">';
	...
	it.each([
		...
		"treats a lone authored %s as a cap",
		(axis, constraint, expectedValue) => {
	...
	it("keeps images that share a line inline and lets a standalone one reserve a slot", () => {
		...
		expect(
			html.match(/<img[^>]*class="[^"]*inline-block![^"]*"/g),
		).toHaveLength(1);
	...
	it("reserves a slot for an image that is the only content of its link", () => {
		const html = render(
			"[![shot](.t3/workspace-image.svg)](https://example.com)",
		);
	...
	it.each([
		[
			"a link",
			"Figure: [![shot](.t3/workspace-image.svg)](https://example.com)",
		],
	...
	it("reserves a slot for an image that is alone in a list item", () => {
		expect(render("- ![shot](.t3/workspace-image.svg)")).toContain(
			"aspect-video",
		);
	});
	...
	it("retains an authored SVG fragment on the signed URL", () => {
		...
		expect(html).toContain(
			'src="https://signed.test/workspace-image.svg#logo"',
		);
	});
	...
	it.each(["success", "loading", "failure", "no-thread"] as const)(
		...
		(scenario) => {
			...
			expect(copiedMarkdownFrom(html)).toBe(
				"![diagram](images/diagram.png#preview)",
			);
		},
	);
	...
	it("reserves the same 16:9 frame while the URL, the bytes, and a failure resolve", () => {
		const frameClassName = (html: string) => {
			const frame =
				/<span[^>]*role="(?:status|alert)"[^>]*>/.exec(html)?.[0] ?? "";
		...
		expect(loadingUrl).toEqual(
			expect.arrayContaining(["aspect-video", "w-full"]),
		);
		...
		expect(loadingBytes).toMatch(
			/<img[^>]*src="https:\/\/signed[^>]*class="invisible/,
		);
