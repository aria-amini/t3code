...
describe("floating browser preview", () => {
	...
	it("only hides the duplicate while the same browser is rendered in the panel", () => {
		...
		expect(
			shouldRenderPreviewMiniPlayer("tab-1", { id: "diff", kind: "diff" }),
		).toBe(true);
	});
});
...
describe("toolGroupConsumesUpwardNavigation", () => {
	...
	it("releases upward navigation when an overflowing group is at the top", () => {
		...
		expect(toolGroupConsumesUpwardNavigation(new ScrollElement(group))).toBe(
			false,
		);
	});
	...
	it.each([100, 300])(
		...
		(scrollHeight) => {
			...
			expect(toolGroupConsumesUpwardNavigation(new ScrollElement(result))).toBe(
				true,
			);
		},
	);
