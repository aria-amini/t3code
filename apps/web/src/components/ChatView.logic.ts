...
export function observeProactivePanelUserChoice(
	...
	return {
		...
		userActionTurnId:
			input.runningTurnId ?? (sameThread ? previous.userActionTurnId : null),
		userActionRevision:
			!sameThread || newTurn
				? input.userActionRevision
...
export function shouldRetargetThreadPullRequestPanel(
	...
	if (previous === null || current === null || surface?.kind !== "pull-request")
		return false;
...
export function resolveProactiveTurnDiffAction(input: {
	...
	if (input.checkpoint === undefined || input.checkpoint.status === "missing")
		return "defer";
...
export function codexArtifactTemplatePromptToAppend(
	...
	return appendCodexArtifactTemplateUsePrompt(currentDraft, template) ===
		currentDraft
		? null
...
export function hasEnvironmentReconnectWarningGraceElapsed(
	...
	return (
		activeEnvironmentId !== null && activeEnvironmentId === elapsedEnvironmentId
	);
}
...
export function resolveThreadMetadataUpdateForNextTurn(input: {
	...
	const branchChanged =
		input.nextBranch !== undefined && input.nextBranch !== input.currentBranch;
...
export function shouldWriteThreadErrorToCurrentServerThread(input: {
	...
	return Boolean(
		...
		input.activeServerThread.environmentId ===
			input.routeThreadRef.environmentId &&
...
export function resolveComposerInteractionMode(input: {
	...
	provider:
		| Pick<ServerProvider, "showInteractionModeToggle">
		| null
		| undefined;
...
export function getAntigravitySendBlockReason(
	provider:
		| Pick<
				ServerProvider,
				"driver" | "installed" | "auth" | "models" | "status"
		  >
		| null
		| undefined,
	model: string,
	...
	const hiddenThreadIds = input.currentThreadIds.filter(
		(threadId) =>
			(threadId !== input.activeThreadId ||
				input.retainInactiveActiveThread === true) &&
...
export async function resolveFileAttachmentUrl(input: {
	...
	if (url === null)
		throw new Error("The environment returned an invalid attachment URL.");
	return url;
}
...
export function isBranchMismatchDismissedForSession(
	key: string | null,
): boolean {
...
export function deriveLockedProvider(input: {
	...
	const threadProvider =
		input.providers.find(
			(provider) => provider.instanceId === input.threadProvider,
		)?.driver ?? input.threadProvider;
	const selectedProvider =
		input.providers.find(
			(provider) => provider.instanceId === input.selectedProvider,
		)?.driver ?? input.selectedProvider;
	const narrowedThreadProvider =
		threadProvider && isProviderDriverKind(threadProvider)
			? threadProvider
			: null;
	const narrowedSelectedProvider =
		selectedProvider && isProviderDriverKind(selectedProvider)
			? selectedProvider
			: null;
...
export function createLocalDispatchSnapshot(
	...
	const latestUserMessage = activeThread?.messages.findLast(
	...
	return {
		...
		latestTurnStartFailureId: latestTurnStartFailureId(
			activeThread,
...
export function hasServerAcknowledgedLocalDispatch(input: {
	...
	if (
		...
		input.latestTurnStartFailureId !==
			input.localDispatch.latestTurnStartFailureId
	) {
		return true;
	}
	...
	const latestTurnChanged =
		...
		input.localDispatch.latestTurnRequestedAt !==
			(latestTurn?.requestedAt ?? null) ||
		input.localDispatch.latestTurnStartedAt !==
			(latestTurn?.startedAt ?? null) ||
		input.localDispatch.latestTurnCompletedAt !==
...
export function shouldRefocusComposerOnWindowFocus(
	activeElement:
		| (Pick<Element, "tagName" | "closest" | "getAttribute"> & {
