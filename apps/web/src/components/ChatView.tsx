...
import {
	...
	type PageScrollKey,
} from "./chat/pageScrollController";
...
function eventPathContainsSelector(event: Event, selector: string): boolean {
	...
	return path.some(
		(target) => target instanceof Element && target.closest(selector),
	);
}
...
function shouldRedirectInputToComposer(event: Event): boolean {
	...
	if (eventPathContainsSelector(event, TYPE_TO_FOCUS_EDITABLE_SELECTOR))
		return false;
	if (eventPathContainsSelector(event, TYPE_TO_FOCUS_INTERACTIVE_SELECTOR))
		return false;
...
function shouldTypeToFocusComposer(event: KeyboardEvent): boolean {
	...
	if (
		launcherKeys &&
		launcherKeys.toLowerCase().includes(event.key.toLowerCase())
	)
		return false;
...
function useLocalDispatchState(input: {
	...
	const activeLocalDispatch = serverAcknowledgedLocalDispatch
		? null
		: localDispatch;
			...
			setLocalDispatch((current) => {
				...
				if (active) {
					const submissionIntent =
						options?.submissionIntent ?? active.submissionIntent;
...
const PersistentThreadTerminalDrawer = memo(
	function PersistentThreadTerminalDrawer({
		...
		const openTerminal = useAtomCommand(
			terminalEnvironment.open,
			"terminal open",
		);
		const writeTerminal = useAtomCommand(
			terminalEnvironment.write,
			"terminal write",
		);
		const closeTerminalMutation = useAtomCommand(
		...
		const panelSurfaces = useRightPanelStore(
			(state) =>
				selectThreadRightPanelState(state.byThreadKey, threadRef).surfaces,
		);
		...
		return (
			<div
				className={cn(
					...
					active &&
						visible &&
						"[[data-panel-animations=true]_&]:starting:grid-rows-[0fr]!",
				)}
			>
				<div className="min-h-0 overflow-clip">
					<ThreadTerminalDrawer
						...
						splitVerticalShortcutLabel={
							visible ? splitVerticalShortcutLabel : undefined
						}
...
const PersistentThreadTerminalPanel = memo(
	function PersistentThreadTerminalPanel({
		...
		const threadWorktreePath =
			serverThread?.worktreePath ?? draftThread?.worktreePath ?? null;
		const activeSummary =
			knownTerminalSessions.find(
				(session) => session.target.terminalId === surface.activeTerminalId,
			)?.state.summary ?? null;
		const worktreePath =
			launchContext?.worktreePath ??
			activeSummary?.worktreePath ??
			threadWorktreePath;
		const cwd = useMemo(
			() =>
				launchContext?.cwd ??
		...
		const terminalLabelsById = useMemo(() => {
			...
			for (const terminalId of surface.terminalIds) {
				const summary =
					knownTerminalSessions.find(
						(session) => session.target.terminalId === terminalId,
					)?.state.summary ?? null;
				labels.set(
					terminalId,
...
function releaseChatTimelineAnchor<
	T extends { readonly messageId: MessageId | null },
>(current: T): T {
...
export default function ChatView(props: ChatViewProps) {
	...
	const threadSyncPhase =
		routeKind === "server" ? (props.threadSyncPhase ?? null) : null;
	...
	const updateProjectScriptSettings = useAtomCommand(
	...
	const createAttachmentAssetUrl = useAtomQueryRunner(
	...
	const uploadThreadFeedback = useAtomCommand(
		threadEnvironment.uploadFeedback,
		{
			reportFailure: false,
		},
	);
	...
	const respondToThreadApproval = useAtomCommand(
		threadEnvironment.respondToApproval,
		{
			reportFailure: false,
		},
	);
	const respondToThreadUserInput = useAtomCommand(
		threadEnvironment.respondToUserInput,
		{
			reportFailure: false,
		},
	);
	const dismissThreadUserInput = useAtomCommand(
	...
	const closePreview = useAtomCommand(
		previewEnvironment.close,
		"preview close",
	);
	...
	const routeServerThreadShell = useThreadShell(
	...
	const loadEarlierTurns = useMemo(() => {
		...
		return {
			loading:
				routeThreadState.page._tag === "Some" &&
				routeThreadState.page.value.loadingOlder,
			cursor:
				routeThreadState.page._tag === "Some"
					? routeThreadState.page.value.beforeCursor
					: null,
			onLoadEarlier: () => {
				requestOlderThreadTurns(
	...
	const citationLocation = useLocation({
		select: (location) => ({
			href: location.href,
			key:
				location.state.assistantCitationActivation ?? location.state.__TSR_key,
		}),
	});
	...
	const composerHasNonPromptContent = useComposerDraftStore((store) => {
		const draft = store.getComposerDraft(composerDraftTarget);
		return draft
			? composerDraftHasUserContent({ ...draft, prompt: "" })
			: false;
	});
	...
	const addComposerDraftFiles = useComposerDraftStore(
	...
	const [restingComposerControlsVisible, setRestingComposerControlsVisible] =
		useState(false);
	const citeAssistantText = useCallback(
		(
			...
			const inserted =
				composerRef.current?.citeAssistantText(citation, sourceAnchor) ?? false;
			if (!inserted) {
				toastManager.add({
					type: "warning",
	...
	const [isWorkspaceFileDragActive, setIsWorkspaceFileDragActive] =
		useState(false);
	...
	const [optimisticUserMessages, setOptimisticUserMessages] = useState<
		ChatMessage[]
	>([]);
	const [feedbackSubmissionsByThreadKey, setFeedbackSubmissionsByThreadKey] =
		useState<Record<string, ReadonlyArray<CodexFeedbackSubmission>>>({});
	...
	>({});
	const [localServerErrorsByThreadKey, setLocalServerErrorsByThreadKey] =
		useState<Record<string, LocalThreadErrorEntry>>({});
	const [isConnecting, _setIsConnecting] = useState(false);
	const [isRevertingCheckpoint, setIsRevertingCheckpoint] = useState(false);
	const [maximizedRightPanelThreadKey, setMaximizedRightPanelThreadKey] =
	...
	const shouldUseRightPanelSheet = useMediaQuery(
	...
	const isTimelineAtLogicalEnd = useCallback(
		() =>
			resolveTimelineIsAtEnd(legendListRef.current?.getState()) ??
			isAtEndRef.current,
		[],
	);
	...
	const storeSetTerminalOpen = useTerminalUiStateStore(
		(s) => s.setTerminalOpen,
	);
	...
	const serverThreadKeys = useMemo(
		() => serverThreadRefs.map(scopedThreadKey),
		[serverThreadRefs],
	);
	...
	const localServerError =
		localServerErrorsByThreadKey[routeThreadKey]?.message ?? null;
	...
	useEffect(() => {
		...
		const miniTabStillExists = Boolean(
			activePreviewState.sessions[activePreviewMiniPlayer.tabId],
		);
	...
	const activeEnvironmentShell = useEnvironmentQuery(
		activeThread
			? environmentShell.stateAtom(activeThread.environmentId)
			: null,
	);
	const activeEnvironmentBootstrapComplete =
		activeEnvironmentShell.data?.snapshot._tag === "Some";
	const activeProjectKey = activeProject
		? `${activeProject.environmentId}:${activeProject.workspaceRoot}`
		: null;
	const clientSettingsHydrated = useClientSettingsHydrated();
	const [pendingFileSurfaceIdsByProject, setPendingFileSurfaceIdsByProject] =
		useState<ReadonlyMap<string, ReadonlySet<string>>>(() => new Map());
	const pendingFileSurfaceIds = activeProjectKey
		? (pendingFileSurfaceIdsByProject.get(activeProjectKey) ??
	...
	const handleFilePendingChange = useCallback(
		(relativePath: string, pending: boolean) => {
			...
			setPendingFileSurfaceIdsByProject((currentByProject) => {
				const current =
					currentByProject.get(activeProjectKey) ??
					EMPTY_PENDING_FILE_SURFACE_IDS;
	...
	useEffect(() => {
		...
		useRightPanelStore
			.getState()
			.reconcileFileSurfaces(activeThreadRef, activeProject !== null);
	...
	const activeEnvironment =
		activeThread == null
			? null
			: (environmentById.get(activeThread.environmentId) ?? null);
	const activeEnvironmentConnectionPhase =
		activeEnvironment?.connection.phase ?? "available";
	...
	const [
		...
		setReconnectWarningGraceElapsedEnvironmentId,
	] = useState<EnvironmentId | null>(null);
	const reconnectWarningGraceElapsed =
		hasEnvironmentReconnectWarningGraceElapsed(
	...
	const activeEnvironmentUnavailableState =
	...
	const attachmentEnvironmentConfig =
		environmentById.get(environmentId)?.serverConfig ?? null;
	const attachmentUploadsCapabilityKnown = attachmentEnvironmentConfig !== null;
	const supportsAttachmentUploads =
		attachmentEnvironmentConfig?.environment.capabilities.attachmentUploads ===
		true;
	const advertisedFileAttachmentBytes =
		attachmentEnvironmentConfig?.environment.capabilities.fileAttachments
			?.maxUploadBytes ?? null;
	...
	const autoBalanceUpdateBanner = useAutoBalanceUpdateBanner(
		autoUpdateEnvironments,
	);
	...
	const versionMismatchDismissKey =
		versionMismatch && activeThread
			...
			: null;
	const [dismissedVersionMismatchKey, setDismissedVersionMismatchKey] =
		useState<string | null>(null);
	...
	const versionMismatchSelfUpdate =
		resolveServerSelfUpdateCapability(serverConfig);
	const versionMismatchDesktopAppUpdate =
		supportsDesktopAppUpdate(serverConfig);
	const versionMismatchThreadContinuation =
		supportsServerUpdateThreadContinuation(serverConfig);
	...
	const systemComposerBannerItems = useMemo<ComposerBannerStackItem[]>(() => {
		...
		const unavailableConnection =
			activeEnvironmentUnavailableState?.connection ?? null;
		...
		const suppressUnavailableBanner =
			environmentReconnecting &&
			(updateRunning ||
				(!reconnectingThroughVersionSkew && !reconnectWarningGraceElapsed));
		if (
			...
			if (reconnectingThroughVersionSkew) {
				...
				items.push({
					id: `environment-unavailable:${activeEnvironmentUnavailableState.environmentId}`,
					variant:
						unavailableConnection.phase === "error" ? "error" : "warning",
					icon: <WifiOffIcon />,
	...
	const { enabled: interactionModeEnabled, interactionMode } =
		resolveComposerInteractionMode({
	...
	const conversationProviderStatus =
		providerStatuses.find(
			(status) =>
				status.instanceId === activeThread?.session?.providerInstanceId,
	...
	const latestCheckpointCompletedAt =
		activeThread?.checkpoints.at(-1)?.completedAt ?? null;
	...
	const { approvals: pendingApprovals, userInputs: pendingUserInputs } =
		useMemo(() => derivePendingRequests(threadActivities), [threadActivities]);
	...
	const usageLimitSources =
		serverConfig?.usageLimitSources ?? EMPTY_USAGE_LIMIT_SOURCES;
	...
	const compactionSettled =
		pendingCompactionMessage !== undefined &&
		(latestTurnStartFailureId(activeThread, pendingCompactionMessage.id) !==
			null ||
	...
	const [projectServerMessagePreviews] = useState(
		createMessageAttachmentPreviewProjector,
	);
	const [projectHandoffMessagePreviews] = useState(
		createMessageAttachmentPreviewProjector,
	);
	...
	const openFileAttachment = useCallback(
		(attachment: ChatFileAttachment) => {
			if (isBrowserPreviewAttachment(attachment) && activeThreadRef) {
				useRightPanelStore
					.getState()
					.openAttachment(activeThreadRef, attachment);
				return;
			}
	...
	const [dockedDraftHeroThreadKey, setDockedDraftHeroThreadKey] = useState<
		string | null
	>(null);
	...
	const providerStatusBannerKey =
		getProviderStatusBannerKey(activeProviderStatus);
	const [
		dismissedProviderStatusBannerKey,
		setDismissedProviderStatusBannerKey,
	] = useState<string | null>(null);
	useEffect(() => {
		if (
			providerStatusBannerKey === null &&
			dismissedProviderStatusBannerKey !== null
		) {
	...
	const hasTimelineTopBanner =
		Boolean(visibleThreadError) || visibleProviderStatus !== null;
	...
	const showComposerContextStrip = shouldShowComposerContextStrip({
		...
		hostsRestingComposerControls:
			routeKind === "server" && restingComposerControlsVisible,
	});
	const initialDiffPanelGitScope =
		gitStatusQuery.data?.hasWorkingTreeChanges === true ? "unstaged" : "branch";
	const diffPanelGitStatusResolutionKey = gitStatusQuery.data
		? "resolved"
		: "pending";
	...
	const needsLoadBalancing =
		automaticEnvironment && !draftThread?.loadBalancedEnvironmentId;
	...
	useEffect(() => {
		...
		const target = logicalProjectEnvironments.find(
			(environment) =>
				environment.environmentId === loadBalancing.environmentId,
		);
		if (!target) return;
	...
	const onAutoEnvironment = useCallback(() => {
		...
		loadBalancing.refresh(
			logicalProjectEnvironments.map(
	...
	useEffect(
		() => subscribeSnapShotComposerFocus(focusComposer),
		[focusComposer],
	);
	...
	const updateProjectScript = useCallback(
		async (
			...
			const existingScript = activeProjectScripts.find(
	...
	const deleteProjectScript = useCallback(
		async (scriptId: string): Promise<AtomCommandResult<void, unknown>> => {
			...
			const nextScripts = activeProjectScripts.filter(
				(script) => script.id !== scriptId,
			);

			const deletedName = activeProjectScripts.find(
				(s) => s.id === scriptId,
			)?.name;
	...
	const activeProjectRepository =
		activeProject?.repositoryIdentity?.displayName ?? null;
	...
	useEffect(() => {
		...
		const observation = observeProactivePanelUserChoice(
		...
		const settledTurnId = latestTurnSettled
			? (activeLatestTurn?.turnId ?? null)
			: null;
		...
		const proactivePanelsEnabled =
			settings.proactivePanelsEnabled && !shouldUseRightPanelSheet;
		const eligibleCompletion =
			proactivePanelsEnabled && newlyCompletedTurnId !== null;
		...
		proactivePanelObservationRef.current = {
			...
			runningTurnId:
				diffAction === "defer" ? previousRunningTurnId : activeRunningTurnId,
			targetKey: shouldDeferLink
		...
		useDiffPanelStore
			.getState()
			.selectTurn(activeThreadRef, newlyCompletedTurnId);
		onDiffPanelOpen?.();
	}, [
	...
	const activatePanelTerminal = useCallback(
		(terminalId: string) => {
			if (!activeThreadRef || activeRightPanelSurface?.kind !== "terminal")
				return;
			useRightPanelStore
				.getState()
				.activateTerminal(
	...
	const closeRightPanelSurface = useCallback(
		(surface: RightPanelSurface) => {
			...
			void confirmTerminalClose([activeLabel, ...otherLabels]).then(
				(confirmed) => {
	...
	const closeOtherRightPanelSurfaces = useCallback(
		(surface: RightPanelSurface) => {
			if (!activeThreadRef) return;
			const surfaces = rightPanelState.surfaces.filter(
	...
	const persistThreadSettingsForNextTurn = useCallback(
		async (input: {
			...
			let result: AtomCommandResult<void, unknown> =
	...
	const timelineScrollIntentRef = useRef<"toward-end" | "away-from-end" | null>(
		null,
	);
	...
	const [timelineLiveFollowEnabled, setTimelineLiveFollowEnabled] =
		useState(true);
	...
	const timelineRealContentOverflowsViewport = useCallback(
		(list?: LegendListRef | null) =>
			timelineContentOverflowsViewport(
				(list ?? legendListRef.current)?.getState(),
				{
	...
	const pageScrollControllerRef = useRef<ReturnType<
		typeof createPageScrollController
	> | null>(null);
	const handlePageScrollStart = useEffectEvent((key: PageScrollKey) => {
		timelineScrollIntentRef.current =
			key === "PageUp" ? "away-from-end" : "toward-end";
	...
	useEffect(() => {
		const controller = createPageScrollController({
			...
			getScrollPaddingBottomPx: () =>
				composerOverlayElement?.getBoundingClientRect().height ?? 0,
	...
	useEffect(() => {
		if (!activeThread?.id || terminalUiState.terminalOpen || isMobileViewport)
			return;
		let frame: number | null = null;
		const onWindowFocus = () => {
			...
			frame = window.requestAnimationFrame(() => {
				frame = window.requestAnimationFrame(() => {
					frame = null;
					if (shouldRefocusComposerOnWindowFocus(document.activeElement))
						focusComposer();
				});
			});
		};
	...
	const activeComposerTasksProgress = useMemo(() => {
		...
		return {
			...
			completedSteps: activePlan.steps.filter(
	...
	const publishComposerOverlayHeight = useCallback(
		(height: number) => {
			...
			const button =
				composerOverlayElement?.parentElement?.querySelector<HTMLElement>(
	...
	const addPullRequestSurface = useCallback(() => {
		...
		useRightPanelStore
			.getState()
			.openPullRequest(activeThreadRef, linkedThreadPullRequest);
	}, [activeThreadRef, linkedThreadPullRequest, supportsPullRequests]);
	const pullRequestSurfaceAvailable =
		supportsPullRequests && linkedThreadPullRequest !== null;
	const supportsSettlement =
		serverConfig?.environment.capabilities.threadSettlement === true;
	const supportsSnooze =
		serverConfig?.environment.capabilities.threadSnooze === true;
	const supportsPinning =
		serverConfig?.environment.capabilities.threadPinning === true;
	const activeThreadPinned =
		supportsPinning && activeThreadShell?.pinnedAt != null;
	...
	const activeThreadLastVisitedAt = useUiStateStore((store) =>
		activeThreadKey === null
			? undefined
			: store.threadLastVisitedAtById[activeThreadKey],
	);
	const activeThreadWokeVisible = useMemo(() => {
		...
		const storedVisitMs = activeThreadLastVisitedAt
	...
	const [unsettlingThreadKey, setUnsettlingThreadKey] = useState<string | null>(
		null,
	);
	const isUnsettling =
		unsettlingThreadKey !== null && unsettlingThreadKey === activeThreadKey;
	const handleUnsettleActiveThread = useCallback(async () => {
		...
		try {
			...
			setUnsettlingThreadKey((current) =>
	...
	const [unsnoozingThreadKey, setUnsnoozingThreadKey] = useState<string | null>(
		null,
	);
	const isUnsnoozing =
		unsnoozingThreadKey !== null && unsnoozingThreadKey === activeThreadKey;
	const handleUnsnoozeActiveThread = useCallback(async () => {
		...
		try {
			...
			setUnsnoozingThreadKey((current) =>
				current === threadKey ? null : current,
			);
		}
	}, [activeThreadRef, unsnoozeThreadMutation]);
	const [isRestoringThreadBranch, setIsRestoringThreadBranch] = useState(false);
	const [branchRestoreConfirmOpen, setBranchRestoreConfirmOpen] =
		useState(false);
	// Once revealed for a given mismatch, the banner stays mounted until the
	// mismatch changes or resolves, so clearing the draft doesn't flicker it.
	const [revealedBranchMismatchKey, setRevealedBranchMismatchKey] = useState<
		string | null
	>(null);
	...
	const showBranchMismatchBanner = shouldShowBranchMismatchBanner({
		...
		wasShownForCurrentMismatch:
			revealedBranchMismatchKey !== null &&
			revealedBranchMismatchKey === activeBranchMismatchKey,
	});
	useEffect(() => {
		setRevealedBranchMismatchKey((revealed) => {
			...
			return revealed !== null && revealed !== activeBranchMismatchKey
				? null
				: revealed;
		});
	...
	const activeBackgroundLiveness =
		!isWorking && activeThread
			...
			: null;
	const [isStoppingBackgroundWork, setIsStoppingBackgroundWork] =
		useState(false);
	useEffect(() => {
		...
		if (result._tag === "Failure") {
			...
			if (!isAtomCommandInterrupted(result)) {
				...
				setThreadError(
					...
					error instanceof Error
						? error.message
						: "Failed to stop background work.",
				);
			}
		}
	}, [activeThread, environmentId, interruptThreadTurn, setThreadError]);
	const backgroundLivenessBannerItem =
		useMemo<ComposerBannerStackItem | null>(() => {
	...
	const [dismissedResumeCompactionKeys, setDismissedResumeCompactionKeys] =
	...
	const compactDisabledReason = compactDisabled
		...
		: null;
	const resumeCompactionBannerItem =
		useMemo<ComposerBannerStackItem | null>(() => {
			if (
				!activeThread ||
				!activeContextWindow ||
			...
			const dismiss = () =>
				setDismissedResumeCompactionKeys((keys) =>
					new Set(keys).add(resumeCompactionKey),
				);
			const compactAction = (
				<Button
					size="xs"
			...
			return {
				...
				actions: compactDisabledReason ? (
					<Tooltip>
						<TooltipTrigger
							render={<span className="inline-flex">{compactAction}</span>}
						/>
	...
	const composerBannerItems = useMemo<ComposerBannerStackItem[]>(() => {
		...
		const wokeThreadItems =
			wokeThreadBannerItem === null ? [] : [wokeThreadBannerItem];
					...
					<span className="flex min-w-0 items-baseline gap-1.5">
						...
						<Tooltip>
							...
							<TooltipPopup side="top" className="max-w-80">
								This thread last ran on{" "}
								{localCheckoutBranchMismatch.threadBranch}. Sending will
	...
	useEffect(() => {
		...
		const previous =
			terminalUiOpenByThreadRef.current[activeThreadKey] ?? false;
	...
	useEffect(() => {
		const handler = (event: globalThis.KeyboardEvent) => {
			...
			if (command === "thread.pin") {
				...
				void (
					pinned
						? confirmAndUnpinThread(activeThreadRef)
						: pinThread(activeThreadRef)
				).then((result) => {
					if (result._tag !== "Failure" || isAtomCommandInterrupted(result))
						return;
					const error = squashAtomCommandFailure(result);
			...
			const script = activeProjectScripts.find(
	...
	const onSend = async (
		...
		const annotationImageAlreadyAttached =
			...
			sendContextImages.some(
				(image) => image.id === directAnnotation.image?.id,
			);
		...
		if (
			...
			if (
				composerRef.current?.validateProviderInput(outgoingFollowUpText) ===
				false
			) {
				return;
			}
		...
		const composerAttachmentsSnapshot = [
		...
		const composerReviewCommentsSnapshot: ReviewCommentContext[] = [
		...
		const messageTextWithPreviewAnnotations =
		...
		if (
			composerRef.current?.validateProviderInput(outgoingMessageText) === false
		) {
			return;
		}
			...
			return {
				...
				fileBlockReason: fileAttachmentCapabilityBlockReason({
					...
					maxFileAttachmentBytes:
						config?.environment.capabilities.fileAttachments?.maxUploadBytes ??
						null,
				}),
			};
		};
		...
		if (attachmentCapabilitiesBeforeUpload.fileBlockReason !== null) {
			...
			setThreadError(
				threadIdForSend,
				attachmentCapabilitiesBeforeUpload.fileBlockReason,
			);
			return;
		}
		...
		if (turnUsesAttachmentUploads && composerAttachmentsSnapshot.length > 0) {
			...
			if (attachmentCapabilitiesAfterUpload.fileBlockReason !== null) {
				...
				setThreadError(
					threadIdForSend,
					attachmentCapabilitiesAfterUpload.fileBlockReason,
				);
				return;
			}
			if (
		...
		const attachmentCapabilitiesBeforeDispatch =
			readLiveAttachmentCapabilities();
		...
		const turnAttachmentsPromise = Promise.all(
			composerAttachmentsSnapshot.map(async (attachment) => {
				if (turnUsesAttachmentUploads) {
					...
					if (!uploaded) {
						throw new Error(
							`Attachment '${attachment.name}' did not finish uploading.`,
						);
					}
					return uploaded;
				}
		...
		const turnAttachmentsResult = await settlePromise(async () => {
			...
			const liveFileBlockReason =
				readLiveAttachmentCapabilities().fileBlockReason;
				...
				if (isLocalDraftThread && draftId && wasBootstrapThreadDeleted(error)) {
					...
					if (failedDraftSession?.threadId === threadIdForSend) {
						setLogicalProjectDraftThreadId(
							...
							scopeProjectRef(
								failedDraftSession.environmentId,
								failedDraftSession.projectId,
							),
							draftId,
							{
		...
		async (requestId: ApprovalRequestId) => {
			...
			if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
				...
				setThreadError(
					...
					error instanceof Error
						? error.message
						: "Failed to dismiss the question.",
				);
			}
			setRespondingUserInputRequestIds((existing) =>
				existing.filter((id) => id !== requestId),
	...
	const onImplementPlanInNewThread = useCallback(async () => {
		...
		if (failure !== null) {
			...
			if (
				...
				!isAtomCommandInterrupted(cleanupResult)
			) {
				console.warn(
	...
	const getModelDisabledReason = useCallback(
		(instanceId: ProviderInstanceId, model: string): string | null => {
			...
			return reason
				? `${reason.description} Start a new thread to use this model.`
				: null;
		},
		...
		(instanceId: ProviderInstanceId, model: string) => {
			...
			if (lockedProvider !== null && activeThread.session?.providerInstanceId) {
				const currentEntry = providerStatuses.find(
					(snapshot) =>
						snapshot.instanceId === activeThread.session?.providerInstanceId,
				);
				if (
	...
	const onOpenTurnDiff = useCallback(
		(turnId: TurnId, filePath?: string) => {
			...
			useDiffPanelStore
				.getState()
				.selectTurn(activeThreadRef, turnId, filePath);
			useRightPanelStore.getState().open(activeThreadRef, "diff");
			onDiffPanelOpen?.();
		},
	...
	const panelToggleControls = (
		<PanelLayoutControls
			...
			rightPanelShortcutLabel={shortcutLabelForCommand(
				keybindings,
				"rightPanel.toggle",
			)}
	...
	const rightPanelContent = activeThreadRef ? (
		renderedRightPanelSurface?.kind === "preview" ? (
			...
			<PersistentThreadTerminalPanel
				...
				splitVerticalShortcutLabel={
					splitTerminalVerticalShortcutLabel ?? undefined
				}
	...
	return (
		<div className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
						...
						>
							<AlertDialogPopup>
								<AlertDialogHeader>
									...
									<AlertDialogDescription>
										You have uncommitted changes. They'll carry over to the
										other branch, or block the switch if they conflict.
									</AlertDialogDescription>
								</AlertDialogHeader>
								<AlertDialogFooter>
			...
			{rightPanelPresent && shouldUseRightPanelSheet && activeThreadRef ? (
				<RightPanelSheet
					animationDurationMs={
						panelAnimationsActive ? panelAnimationDurationMs : 0
					}
					open={rightPanelOpen}
