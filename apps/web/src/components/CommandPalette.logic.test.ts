...
describe("reduceCommandPaletteUiState", () => {
	...
	it("routes open intents to command mode", () => {
		...
		expect(
			reduceCommandPaletteUiState(filesOpen, { _tag: "OpenAddProject" }),
		).toEqual({
			open: true,
			mode: "command",
		...
		expect(
			reduceCommandPaletteUiState(filesOpen, { _tag: "OpenNewThreadIn" }),
		).toEqual({
			open: true,
			mode: "command",
	...
	it("preserves the mode on close and resets it on open", () => {
		...
		expect(
			reduceCommandPaletteUiState(filesOpen, { _tag: "SetOpen", open: false }),
		).toEqual({
			open: false,
			mode: "files",
			openIntent: null,
		...
		expect(
			reduceCommandPaletteUiState(filesOpen, { _tag: "SetOpen", open: true }),
		).toEqual({
			open: true,
			mode: "command",
			openIntent: null,
...
describe("enumerateCommandPaletteItems", () => {
	it("assigns positional jump shortcuts to the first nine displayed items", () => {
		...
		expect(
			enumerateCommandPaletteItems(items).map((item) => item.shortcutCommand),
		).toEqual([
			"thread.jump.1",
			"thread.jump.2",
			"thread.jump.3",
...
describe("buildThreadActionItems", () => {
	...
	it("normalizes case independently of the host locale", () => {
		...
		try {
			...
			).toEqual(["setting:version-control"]);
		} finally {
