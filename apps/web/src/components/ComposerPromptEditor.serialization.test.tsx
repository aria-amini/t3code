...
function $firstMention() {
	...
	if (mention.getType() !== "composer-mention")
...
describe("composer mention serialization", () => {
	...
	it("preserves original source when replacing the controlled prompt", async () => {
		for (const prompt of [
			...
			if (prompt === "@README.md control") {
				expect(
					lexicalEditor.getEditorState().read(() => $firstMention().isInline()),
				).toBe(true);
			}
		}
	});
	...
	it("preserves source when Lexical clones the mention and reloads exported state", async () => {
		...
		await renderPrompt(prompt);
		const originalKey = lexicalEditor
			.getEditorState()
			.read(() => $firstMention().getKey());
		...
		expect(
			lexicalEditor.getEditorState().read(() => $firstMention().getKey()),
		...
		await act(() => {
			lexicalEditor.setEditorState(
				lexicalEditor.parseEditorState(exportedState),
			);
		});
		expect(editorRef.current?.readSnapshot().value).toBe(prompt);
		expect(
							...
							children: [
								{
									...
									version: 1,
									children: [
										{ type: "composer-mention", version: 1, path: "README.md" },
									],
								},
							],
						},
