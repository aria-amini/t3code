...
function $citationNodes() {
	...
	if (!$isElementNode(paragraph))
		throw new Error("Expected a composer paragraph");
	return paragraph
		.getChildren()
		.filter((node) => node instanceof ComposerCitationNode);
}
...
describe("registerComposerInlineTokenPaste", () => {
	...
	it("handles a copied mention without also running the plain-text paste fallback", () => {
		...
		const mention =
			"[improve-deploy-error-logging.md](.changeset/improve-deploy-error-logging.md)";
		...
		).toBe("<mention:.changeset/improve-deploy-error-logging.md> ");
	});
	...
	it.each([
		...
		"leaves scoped package command %s to the plain-text paste fallback",
		(command) => {
			vi.stubGlobal("ClipboardEvent", TestClipboardEvent);
			const editor = createEditor();
	...
	it("restores citations alongside mentions and only pads a trailing mention", () => {
		...
		pasteText(
			trailingMentionEditor,
			`${citationSource} [AGENTS.md](AGENTS.md)`,
		);
		...
		).toBe(`${citationSource} <mention:AGENTS.md> `);
	});
	...
	it.each(["what do you mean??\nPlease keep the indentation:\n  café 👋"])(
		...
		(comment) => {
			...
			editor.getEditorState().read(() => {
				...
				expect($citationNodes()[0]!.exportJSON().citation).toEqual(
					commentedCitation,
				);
			});
	...
	it("edits only one repeated quote without moving selection or mutating earlier editor states", () => {
		...
		afterEdit.read(() => {
			...
			expect(
				$citationNodes().map(
					(citationNode) => citationNode.exportJSON().citation,
				),
			).toEqual([citation, { ...citation, comment }]);
		...
		expect(
			editor.getEditorState().read(() => $getRoot().getTextContent()),
		).toBe(`前(${citationSource}),(${source})後`);
	});

	it.each(["", " \n\t "])(
		"clears a comment with %j without removing the quote",
		(comment) => {
			vi.stubGlobal("ClipboardEvent", TestClipboardEvent);
			const editor = createCitationEditor();
			pasteText(editor, citationSource);
			editor.update(
				() => $citationNodes()[0]!.setComment("Remove this comment."),
				{
					discrete: true,
				},
			);
			editor.update(() => $citationNodes()[0]!.setComment(comment), {
				discrete: true,
			});

			editor.getEditorState().read(() => {
				expect($citationNodes()).toHaveLength(1);
				expect($citationNodes()[0]!.exportJSON().citation).toStrictEqual(
					citation,
				);
				expect($getRoot().getTextContent()).toBe(
	...
	it("copies, cuts, and pastes selected citation nodes with all source metadata intact", () => {
		...
		const firstCitation = {
			...citation,
			comment: "Explain the first occurrence.",
		};
		const secondCitation = {
			...citation,
			comment: "Keep the second occurrence.\n  café 👋",
		};
		...
		editor.getEditorState().read(() => {
			...
			expect(
				$citationNodes().map((node) => node.exportJSON().citation),
			).toEqual([firstCitation, secondCitation]);
		});
	});
	...
	it.each([true, false])(
		...
		(backwards) => {
			const editor = createCitationEditor();
			const commentedCitation = {
				...citation,
				comment: "Delete this with its quote.\n  café 👋",
			};
			const source = serializeAssistantCitation(commentedCitation);
			editor.update(
				() => {
			...
			expect(afterDeletion.read(() => $getRoot().getTextContent())).toBe(
				"前()後",
			);
			...
			expect(editor.getEditorState().read(() => $citationNodes())).toHaveLength(
				0,
			);
...
describe("citation comment opening", () => {
	...
	it("opens only the newly inserted occurrence after committing its exact prompt", () => {
		...
		const requestRef = {
			current: request as ComposerCitationCommentRequest | null,
		};
	...
	it("does not reopen comments when an inserted citation is restored or pasted", () => {
		...
		editor.update(
			() => $citationNodes()[0]!.setComment("Keep this attached."),
			{
				discrete: true,
			},
		);
		...
		reloaded.getEditorState().read(() => {
			expect(
				$consumeComposerCitationCommentRequest({ current: null }),
			).toBeNull();
			expect($citationNodes()[0]!.exportJSON().citation.comment).toBe(
				"Keep this attached.",
			);
		});
		const pasted = createCitationEditor();
