...
class ComposerMentionNode extends DecoratorNode<React.ReactElement> {
	...
	constructor(
		path: string,
		source = serializeComposerFileLink(path),
		key?: NodeKey,
	) {
		super(key);
		this.__path = path;
...
function ComposerSkillDecorator(props: {
	...
	return (
		<Tooltip>
			...
			<TooltipPopup
				side="top"
				className="max-w-120 whitespace-normal leading-tight"
			>
...
class ComposerSkillNode extends DecoratorNode<React.ReactElement> {
	...
	static override importJSON(
		serializedNode: SerializedComposerSkillNode,
	): ComposerSkillNode {
	...
	constructor(
		...
		super(key);
		const normalizedSkillName = skillName.startsWith("$")
			? skillName.slice(1)
			: skillName;
...
function $createComposerSkillNode(
	...
	return $applyNodeReplacement(
		new ComposerSkillNode(skillName, skillLabel, skillDescription),
	);
}
...
class ComposerTerminalContextNode extends DecoratorNode<React.ReactElement> {
	...
	static override clone(
		node: ComposerTerminalContextNode,
...
function isComposerInlineTokenNode(
	candidate: unknown,
...
function getComposerInlineTokenExpandedTextLength(
...
function getAbsoluteOffsetForInlineTokenPoint(
	...
	return (
		absoluteOffset +
		(pointOffset > 0 ? getComposerInlineTokenTextLength(node) : 0)
	);
}
...
function getExpandedAbsoluteOffsetForInlineTokenPoint(
	...
	return (
		absoluteOffset +
		(pointOffset > 0 ? getComposerInlineTokenExpandedTextLength(node) : 0)
	);
}
...
function getComposerNodeTextLength(node: LexicalNode): number {
	...
	if ($isElementNode(node)) {
		return node
			.getChildren()
			.reduce((total, child) => total + getComposerNodeTextLength(child), 0);
	}
	return 0;
}
...
function getComposerNodeExpandedTextLength(node: LexicalNode): number {
	...
	if ($isElementNode(node)) {
		return node
			.getChildren()
			.reduce(
				(total, child) => total + getComposerNodeExpandedTextLength(child),
				0,
			);
	}
	return 0;
...
function getExpandedAbsoluteOffsetForPoint(
	node: LexicalNode,
	pointOffset: number,
): number {
	let offset = 0;
...
function $getComposerRootLength(): number {
	...
	return children.reduce(
		(sum, child) => sum + getComposerNodeTextLength(child),
		0,
	);
}
...
function $setSelectionRangeAtComposerOffsets(
	startOffset: number,
	endOffset: number,
): void {
	const root = $getRoot();
...
function getSelectionRangeForExpandedComposerOffsets(
...
function $selectionTouchesInlineToken(
	selection: ReturnType<typeof $getSelection>,
): boolean {
	if (!$isRangeSelection(selection)) {
		return false;
...
function $setComposerEditorPrompt(
	...
	for (const segment of segments) {
		if (segment.type === "citation") {
			paragraph.append(
				$createComposerCitationNode(segment.citation, segment.source),
			);
			continue;
		}
		if (segment.type === "mention") {
			paragraph.append(
				$createComposerMentionNode(segment.path, segment.source),
			);
			continue;
		}
...
function collectTerminalContextIds(node: LexicalNode): string[] {
	...
	if ($isElementNode(node)) {
		return node
			.getChildren()
			.flatMap((child) => collectTerminalContextIds(child));
	}
	return [];
}
...
function caretLineRect(range: Range, edge: "start" | "end"): DOMRect | null {
	...
	const collapsedRect =
		edge === "start" ? collapsedRects.at(-1) : collapsedRects[0];
	...
	if (container.nodeType === Node.TEXT_NODE) {
		...
		const probeRect = Array.from(probeRange.getClientRects()).find(
			(rect) => rect.height > 0,
		);
	...
	if (neighbour instanceof HTMLElement) {
		...
		const isBeforeCaret =
			neighbour === container.childNodes[range.startOffset - 1];
...
function ComposerInlineTokenBackspacePlugin() {
	const [editor] = useLexicalComposerContext();
	const { onRemoveTerminalContext } = use(
			...
			(event) => {
				...
				if ($isTextNode(anchorNode)) {
					...
					if ($isElementNode(parent)) {
						...
						if (
							index > 0 &&
							removeInlineTokenNode(parent.getChildAtIndex(index - 1))
						) {
							return true;
						}
					}
				...
				if ($isElementNode(anchorNode)) {
					...
					if (
						childIndex >= 0 &&
						removeInlineTokenNode(anchorNode.getChildAtIndex(childIndex))
					) {
						return true;
					}
				}
		...
		const applyKeys = (nextKeys: Set<string>) => {
			for (const key of selectedKeys) {
				if (!nextKeys.has(key)) {
					editor
						.getElementByKey(key)
						?.removeAttribute("data-composer-chip-selected");
				}
			}
			for (const key of nextKeys) {
				editor
					.getElementByKey(key)
					?.setAttribute("data-composer-chip-selected", "true");
			}
...
function ComposerSurroundSelectionPlugin(props: {
	...
	const applySurroundInsertion = useEffectEvent(
	...
	useEffect(() => {
		...
		const onBeforeInput = (event: InputEvent) => {
			if (
				...
				pendingDeadKeySelectionRef.current =
					pendingSurroundSelectionRef.current;
				return;
			}
			...
			queueMicrotask(() => {
				editor.update(
					() => {
						...
						replacementSelection.insertText(
							`\`${selectedText}${backtickCloseSymbol}`,
						);
						$setSelectionRangeAtComposerOffsets(
							replacementStart + 1,
		...
		return () => {
			if (activeRootElement) {
				...
				activeRootElement.removeEventListener(
					"beforeinput",
					onBeforeInput,
					true,
				);
...
function ComposerPromptEditorInner({
	...
	const initialExpandedCursor = expandCollapsedComposerCursor(
		value,
		initialCursor,
	);
	...
	const selectionRangeRef = useRef({
		start: initialExpandedCursor,
		end: initialExpandedCursor,
	});
	const isApplyingControlledUpdateRef = useRef(false);
	const citationCommentRequestRef =
		useRef<ComposerCitationCommentRequest | null>(null);
	...
	useLayoutEffect(() => {
		...
		const normalizedExpandedCursor = expandCollapsedComposerCursor(
			value,
			normalizedCursor,
		);
		snapshotRef.current = {
		...
		editor.update(
			() => {
				...
				citationToOpen = $consumeComposerCitationCommentRequest(
					citationCommentRequestRef,
				);
			},
			{
				...(isCiteInsertion
	...
	const readSnapshot = useCallback((): {
		...
		editor.getEditorState().read(() => {
			...
			const selectionRange =
				getSelectionRangeForExpandedComposerOffsets($getSelection());
	...
	const handleEditorChange = useCallback((editorState: EditorState) => {
		editorState.read(() => {
			...
			if (!snapshotChanged) {
				if (
					didComposerSelectionChangeVisibly(
