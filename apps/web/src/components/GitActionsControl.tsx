...
function resolveProgressDescription(
	...
	return formatElapsedDescription(
...
function GitQuickActionIcon({
	...
	if (quickAction.kind === "run_pull")
		return <CloudDownloadIcon className={iconClassName} />;
...
function PublishRepositoryDialog(props: PublishRepositoryDialogProps) {
	...
	const [publishProtocol, setPublishProtocol] =
		useState<SourceControlCloneProtocol>("ssh");
	...
	const [publishResult, setPublishResult] =
		useState<SourceControlPublishRepositoryResult | null>(null);
	...
	const publishRepositoryAction =
		useSourceControlPublishRepositoryAction(sourceControlScope);
	const publishAccountByProvider = useMemo(() => {
		...
		for (const provider of sourceControlDiscovery.data
	...
	const selectedPublishProviderReadiness =
		publishProviderReadiness[publishProvider];
	...
	const publishRepository =
		publishRepositoryOverride ?? publishRepositoryPrefill;
			...
			<DialogPopup className="max-w-xl overflow-hidden">
				<div className="flex min-h-0 flex-col overflow-hidden border-foreground/10 bg-transparent">
					...
					<DialogPanel className="space-y-5 border-b border-border/70 bg-muted/20 px-6 py-5 dark:border-transparent dark:bg-transparent">
						<AnimatedHeight>
							<div
								className={cn("space-y-2", publishWizardStep !== 0 && "hidden")}
							>
								<span
								...
								>
									{sortedPublishProviderOptions.map((option) => {
										...
										const isSelected =
											publishProvider === option.value && readiness.ready;
										if (!readiness.ready) {
											return (
												<div
													key={option.value}
							...
							<div
								className={cn("space-y-5", publishWizardStep !== 1 && "hidden")}
							>
									...
									<RadioGroup
										...
										onValueChange={(value) =>
											setPublishVisibility(
												value as SourceControlRepositoryVisibility,
											)
										}
												...
												>
													{(["ssh", "https"] as const).map((protocol) => (
														<RadioPrimitive.Root
															...
															value={protocol}
															data-pressed={
																publishProtocol === protocol ? "" : undefined
															}
							...
							<div
								className={cn("space-y-4", publishWizardStep !== 2 && "hidden")}
							>
								{publishResult ? (
									<>
										...
										<Button
											...
											className="w-full"
											onClick={() => {
												void openLink(publishResult.repository.url).catch(
													() => undefined,
												);
											}}
										>
								...
								{publishWizardStep < 1 ? (
									<Button
										...
										disabled={
											...
											!selectedPublishProviderReadiness.ready
										}
										onClick={() =>
...
export default function GitActionsControl({
	...
	const serverConfig = useAtomValue(
		serverEnvironment.configValueAtom(activeEnvironmentId),
	);
	...
	const activeGitActionProgressRef = useRef<ActiveGitActionProgress | null>(
		null,
	);
	const sourceControlScope = useMemo(
	...
	const openExistingPr = useCallback(async () => {
		const openPr =
			gitStatusForActions?.pr?.state === "open" ? gitStatusForActions.pr : null;
	...
	const continuePendingDefaultBranchAction = () => {
		...
		const { action, commitMessage, onConfirmed, filePaths } =
		...
		if (quickAction.kind === "run_pull") {
			...
			void (async () => {
				...
				toastManager.update(toastId, {
					type: "success",
					title:
						pullResult.status === "pulled" ? "Pulled" : "Already up to date",
					description:
	...
	const canPublishRepository =
		isRepo && gitStatusForActions !== null && !hasPrimaryRemote;
						...
						<MenuPopup align="end" className="w-full">
									...
									return (
										<Popover key={`${item.id}-${item.label}`}>
											<PopoverTrigger
												...
												nativeButton={false}
												render={
													<span className="block w-max cursor-not-allowed" />
												}
											>
							...
							{gitStatusForActions?.refName === null && (
								<p className="px-2 py-1.5 text-xs text-warning">
									Detached HEAD: create and checkout a refName to enable push
													...
													return (
														...
														>
															...
															<button
																...
																onClick={() =>
																	openChangedFileInEditor(file.path)
																}
															>
																<StartTruncatedPath
																	path={file.path}
