...
const hooks = vi.hoisted(() => {
	...
	return {
		...
		useMemoCache(size: number): unknown[] {
			...
			if (!slots[index]) {
				slots[index] = Array.from({ length: size }, () =>
...
describe("ProjectFavicon", () => {
	beforeEach(() => {
		...
		testState.faviconUrl =
			"https://environment.test/api/assets/token-a/v1-20-favicon.svg";
	});
	...
	it("renders a saved Lucide icon and color ahead of an uploaded favicon", () => {
		...
		expect(element.props.children.props.children.props.name).toBe(
			"alarm-clock",
		);
		expect(element.props.className).toContain("text-violet-600");
		expect(element.props.children.props.children.props.className).toContain(
			"text-violet-600",
		);
	});
	...
	it("falls back when the displayed favicon fails without discarding a valid older image early", () => {
		...
		const afterRefreshError = renderImage(Component, refreshedProps).props
			.children;
		expect(afterRefreshError[1]?.props.src).toBe(props.src);
		afterRefreshError[1]?.props.onError?.();

		const afterDisplayedError = renderImage(Component, refreshedProps).props
			.children;
