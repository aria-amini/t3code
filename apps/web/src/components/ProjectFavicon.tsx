...
export function ProjectFavicon(input: {
	...
	const FallbackIcon =
		...
		(automaticIconName?.kind === "lucide"
			? PROJECT_ICONS[automaticIconName.icon]
			: undefined);
	const fallbackEmoji =
		automaticIconName?.kind === "emoji" ? automaticIconName.emoji : undefined;
	const fallbackColorClassName =
		automaticIconName?.kind === "lucide"
			? projectIconColorClassName(
					PROJECT_ICON_COLOR_BY_NAME[automaticIconName.icon],
				)
			: undefined;
		...
		<>
			...
			{displayedSrc ? (
				<img
					...
					alt=""
					className={cn(
						"size-3.5 shrink-0 rounded-[37.5%] object-contain",
						className,
					)}
