...
function toProviderUpdateOutcome(input: {
	...
	if (input.result._tag === "Failure") {
		...
		return {
			status: "rejected",
			reason:
				error instanceof Error ? error : new Error("Provider update failed."),
		};
	}
...
export function ProviderUpdateEnvironmentRows({
	...
	const handleUpdate = useCallback(
		async (environmentId: EnvironmentId) => {
			...
			const requestVersion =
				(requestVersionRef.current.get(environmentId) ?? 0) + 1;
			...
			setPendingEnvironments((previous) =>
				new Set(previous).add(environmentId),
			);
			...
			try {
				...
				if (isTerminalProviderUpdatePhase(view.phase)) {
					setResultByEnvironment((previous) =>
						new Map(previous).set(environmentId, view),
					);
				}
			} catch (error) {
				if (isCurrentRequest()) {
	...
	const rows = groups
		...
		.filter(
			({ group, status }) =>
				group.candidates.length > 0 || status.kind !== "idle",
		);
