...
function provider(input: {
	...
	readonly advisoryStatus?: NonNullable<
		ServerProvider["versionAdvisory"]
	>["status"];
}): ServerProvider {
...
describe("provider update launch notification logic", () => {
	it("detects enabled providers with a latest-version advisory", () => {
		expect(
			isProviderUpdateCandidate(provider({ driver: driver("codex") })),
		).toBe(true);
		expect(
			isProviderUpdateCandidate(
				provider({ driver: driver("codex"), enabled: false }),
			),
		).toBe(false);
		expect(
			isProviderUpdateCandidate(
		...
		expect(
			isProviderUpdateCandidate(
				provider({ driver: driver("codex"), latestVersion: null }),
			),
		).toBe(false);
	});
	...
	it("keeps the same notification key while the published update version is unchanged", () => {
		...
		expect(providerUpdateNotificationKey([first])).toBe(
			providerUpdateNotificationKey([second]),
		);
	...
	it("keeps the initial prompt and terminal outcomes visible as toasts", () => {
		...
		expect(
			shouldShowPrimaryProviderUpdateToast(
				getProviderUpdateRejectedToastView(1, "boom"),
			),
		).toBe(true);
	});
	...
	it("falls back to a rejected RPC message for transport-level failures", () => {
		const results = [
			AsyncResult.failure(Cause.die(new Error("WebSocket closed"))),
		];

		expect(firstFailedProviderUpdateMessage(results)).toBe("WebSocket closed");
		expect(
			getProviderUpdateRejectedToastView(2, "WebSocket closed"),
		).toMatchObject({
			phase: "failed",
	...
	describe("multi-backend update outcomes", () => {
		...
		const fulfilledOutcome = (
			...
			value: {
				environmentId:
					environment as LocalProviderUpdateOutcome["environmentId"],
				isPrimary,
		...
		it("treats a rejected dispatch as not contributing a snapshot", () => {
			...
			expect(firstRejectedProviderUpdateMessage(results)).toBe(
				"WebSocket closed",
			);
		});
	});
	...
	describe("per-environment update grouping", () => {
		const environment = (
			input: {
				...
				Omit<LocalEnvironmentProvidersInput, "environmentId" | "providers">
			>,
		...
		it("groups each environment's outdated one-click candidates", () => {
			...
			expect(
				result.groups.every((group) => group.candidates.length === 1),
			).toBe(true);
		});
		...
		it("flags settling while a secondary backend is still connecting", () => {
			...
			expect(
				result.groups.find(
					(group) => group.environmentId === ("env-wsl" as EnvironmentId),
				)?.isSettling,
			).toBe(true);
		});
		...
		it("keeps only environments that have a one-click update on offer", () => {
			...
			expect(
				environmentGroupsWithUpdates(groups).map(
		...
		it("keys the notification by environment, driver and latest version", () => {
			...
			expect(
				localEnvironmentUpdateNotificationKey(noUpdates.groups),
			).toBeNull();
