...
export type ProviderUpdateToastType =
	| "warning"
	| "loading"
	| "error"
	| "success";
export type ProviderUpdateToastPhase =
	| "initial"
	| "running"
	| "failed"
	| "unchanged"
 ...
 */
export function isTerminalProviderUpdatePhase(
	phase: ProviderUpdateToastPhase,
): boolean {
...
function dedupeProvidersByDriver<T extends ServerProvider>(
	...
	for (const provider of providers) {
		latestProviderByDriver.set(
			provider.driver,
			chooseRepresentativeProvider(
				latestProviderByDriver.get(provider.driver),
				provider,
			) as T,
		);
	}
...
function dedupeProvidersByInstanceId<T extends ServerProvider>(
	providers: ReadonlyArray<T>,
): T[] {
...
export function hasOneClickUpdateProviderCandidate(
	...
	const driverProviders = providers.filter(
		(provider) => provider.driver === candidate.driver,
	);
...
export function shouldShowPrimaryProviderUpdateToast(
...
export function getProviderUpdateSidebarPillView(
	...
	if (activeProviders.length > 0) {
		...
		return {
			key: `loading:${activeProviders
				.map(
					(provider) =>
						`${provider.driver}:${provider.updateState?.status ?? "idle"}`,
				)
				.toSorted()
				.join("|")}`,
			tone: "loading",
	...
	if (unchangedProviders.length > 0) {
		...
		const unchangedProviderName =
			PROVIDER_DISPLAY_NAMES[unchangedProvider.driver] ??
			unchangedProvider.driver;
		terminalCandidates.push({
		...
		terminalCandidates
			.toSorted((left, right) => {
				...
				const leftFinishedAt =
					latestFinishedAtForProviders(leftProviders) ?? "";
				const rightFinishedAt =
					latestFinishedAtForProviders(rightProviders) ?? "";
...
function providerUpdateOutcomeSeverity(provider: ServerProvider): number {
	return (
		PROVIDER_UPDATE_STATUS_SEVERITY[provider.updateState?.status ?? ""] ?? 0
	);
}
...
export function collectProviderUpdateOutcomeSnapshots(
	...
	for (const result of results) {
		...
		if (
			!current ||
			providerUpdateOutcomeSeverity(provider) >
				providerUpdateOutcomeSeverity(current)
		) {
...
export type EnvironmentUpdateConnectionState =
	| "connecting"
	| "ready"
	| "disconnected"
	| "error";
...
export function buildLocalEnvironmentUpdateGroups(
	...
	const groups = environments.map((environment) => ({
		...
		candidates: collectProviderUpdateCandidates(environment.providers).filter(
			(candidate) =>
	...
	const parts = environmentGroupsWithUpdates(groups)
		.map((group) => {
			const providerParts = group.candidates
				.map(
					(candidate) =>
						`${candidate.driver}:${candidate.versionAdvisory.latestVersion}`,
				)
				.toSorted()
				.join(",");
