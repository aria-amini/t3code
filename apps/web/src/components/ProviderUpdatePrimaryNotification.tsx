...
function ProviderUpdateToastIcon({
	provider,
}: {
...
export function ProviderUpdatePrimaryNotification() {
	...
	const updateProviders = useMemo(
		() => collectProviderUpdateCandidates(providers),
		[providers],
	);
	...
	const oneClickProviders = useMemo(
		() =>
			updateProviders.filter((provider) =>
	...
	useEffect(() => {
		...
		const runUpdates = () => {
			...
			const providerInstanceIds = new Set(
				oneClickProviders.map((provider) => provider.instanceId),
			);
