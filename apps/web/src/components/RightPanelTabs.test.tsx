...
function renderTabs(
	...
	audio?: { audible?: boolean; audioMuted?: boolean },
	previewRuntimeTabId: ((tabId: string) => string) | null = (tabId) =>
		`runtime:${tabId}`,
) {
...
describe("RightPanelTabs preview favicon", () => {
	...
	it("hides a capture while the server session still describes another origin", () => {
		const html = renderTabs(
			favicon("data:image/png;base64,AAAA", "https://example.com/"),
		);
...
describe("surface shortcuts", () => {
	...
	it("matches available surface shortcuts case-insensitively", () => {
		expect(surfaceShortcutActionForKey(actions, shortcutEvent("b"))).toBe(
			actions[0],
		);
		expect(surfaceShortcutActionForKey(actions, shortcutEvent("B"))).toBe(
			actions[0],
	...
	it("leaves modified, composing, and already-handled key events alone", () => {
		expect(
			surfaceShortcutActionForKey(
				actions,
				shortcutEvent("b", { metaKey: true }),
			),
		).toBeNull();
		expect(
			surfaceShortcutActionForKey(
				actions,
				shortcutEvent("b", { isComposing: true }),
			),
		).toBeNull();
		expect(
			surfaceShortcutActionForKey(
				actions,
				shortcutEvent("b", { defaultPrevented: true }),
			),
		).toBeNull();
	});
});
...
describe("surface shortcut typing contexts", () => {
	...
	it("treats form fields and every editable region as typing contexts", () => {
		...
		expect(surfaceShortcutTargetsTypingContext(makeTarget("textarea"))).toBe(
			true,
		);
		expect(surfaceShortcutTargetsTypingContext(makeTarget("select"))).toBe(
			true,
		);
		...
		expect(
			surfaceShortcutTargetsTypingContext(makeTarget("[contenteditable]")),
		).toBe(true);
	});
...
describe("RightPanelTabs audio indicator", () => {
	...
	it.each(cases)(
		"audible=$audible muted=$audioMuted",
		({ audible, audioMuted, label }) => {
...
describe("tabMuteMenuItem", () => {
	const overlay = (audioMuted: boolean) =>
		({ audioMuted, audible: false }) as Parameters<
			typeof tabMuteMenuItem
		>[0]["overlay"];
	...
	it("stays disabled until the desktop tab exists", () => {
		...
		expect(
			tabMuteMenuItem({ overlay: null, canResolveRuntimeTabId: true }),
		).toEqual({
			label: "Mute tab",
			disabled: true,
		});
	...
	it("offers mute and unmute once the tab is addressable", () => {
		...
		expect(
			tabMuteMenuItem({ overlay: overlay(true), canResolveRuntimeTabId: true }),
		).toEqual({
			label: "Unmute tab",
			disabled: false,
		});
