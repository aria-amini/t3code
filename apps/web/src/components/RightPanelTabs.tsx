					...
					{actions.map((action) =>
						action.available ? (
							...
							>
								...
								>
									<Kbd className="absolute top-3 right-3">
										{action.shortcut}
									</Kbd>
										...
										>
											{props.browserProfiles.map((profile) => (
												<MenuItem
													key={profile.id}
													onClick={() =>
														props.onAddBrowserInProfile(profile.id)
													}
												>
...
function SurfaceIcon({
	...
	environmentId: EnvironmentId | null;
	pullRequestStatusSeeds:
		| Readonly<Record<string, PullRequestTabStatusSeed>>
		| undefined;
}) {
	switch (surface.kind) {
		case "preview": {
			...
			const favicon = snapshot
				? (desktopByTabId[snapshot.tabId]?.favicon ?? null)
				: null;
			const capturedUrl =
				favicon && url && sameOrigin(favicon.pageUrl, url)
					? favicon.dataUrl
					: null;
...
export function RightPanelTabs(props: RightPanelTabsProps) {
	...
	const updateTabScrollState = useCallback(() => {
		...
		const hasOverflow =
			viewport.scrollWidth - viewport.clientWidth > TAB_SCROLL_EDGE_TOLERANCE;
		const canScrollLeft =
			hasOverflow && viewport.scrollLeft > TAB_SCROLL_EDGE_TOLERANCE;
		const canScrollRight =
			hasOverflow &&
			viewport.scrollLeft + viewport.clientWidth <
				viewport.scrollWidth - TAB_SCROLL_EDGE_TOLERANCE;
		...
		async (event: ReactMouseEvent, surface: RightPanelSurface) => {
			...
			switch (action) {
				...
				case "toggle-mute": {
					...
					if (runtimeTabId) {
						void previewBridge
							?.setAudioMuted(runtimeTabId, !menuMuted)
							.catch(() => undefined);
					}
					break;
				}
				...
				>
					<div className="flex h-full w-max min-w-full items-center gap-1">
						{props.surfaces.map((surface) => {
							...
							const previewTabId = previewTabIdOf(
								surface,
								props.previewSessions,
							);
													...
													<MenuSubTrigger
														...
														onClick={(event) => {
															const pointerType =
																...
																typeof event.nativeEvent.pointerType ===
																	"string"
