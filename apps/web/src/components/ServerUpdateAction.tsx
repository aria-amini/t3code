		...
		try {
			const result = await updateServer({
				...
				input: {
					...
					target.continueThreadsAfterServerUpdate
...
export function ServerUpdatesAction({
	...
	const handleUpdate = async () => {
		...
		try {
			...
			const desktopTargets = available.filter(
				(target) => target.selfUpdate === "desktop-managed",
			);
			...
			await Promise.all(
				available.map((target) =>
					update(target, `${target.serverLabel} update failed`),
				),
			);
		} finally {
...
export function ServerUpdateProgress({
	...
	if (state.status === "failed") {
		return (
			<div
				className="mt-1 flex min-w-0 items-center gap-2 text-xs text-destructive"
				role="alert"
			>
				<span
				...
				<Tooltip>
					<TooltipTrigger
						render={<span className="min-w-0 truncate">{state.message}</span>}
					/>
