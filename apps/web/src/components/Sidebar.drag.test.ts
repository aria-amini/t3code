...
function layout(
	...
	const activeIndex = items.findIndex(
		(item) => sidebarListItemId(item) === active,
	);
	return {
		activeIndex,
...
describe("sidebar collision detection", () => {
	function collisionArgs(blockedAboveSource = false) {
		...
		return {
			active: {
				...
				data: { current: {} },
				rect: {
					current: { initial: rects[activeIndex]!, translated: collisionRect },
				},
			},
			collisionRect,
	...
	it.each([
		...
		(blockedAboveSource, nearbyTarget) => {
			...
			expect(detector(args).map((collision) => collision.id)).toEqual([
				"source",
			]);
		},
	);
			...
			const at = (center: number) => {
				...
				const args = {
					...
					active: {
						...
						data: { current: {} },
						rect: {
							current: { initial: sourceRect, translated: collisionRect },
						},
					},
					collisionRect,
				...
				return over
					? resolveSidebarDropTarget(items, "source", String(over.id))?.section
					: null;
			};
	...
	it("validates each hovered target once and always allows returning to the source", () => {
		...
		expect(
			detector({
				...args,
				collisionRect: args.droppableRects.get("source")!,
			})[0]?.id,
		).toBe("source");
		expect(
			detector({
				...args,
				collisionRect: args.droppableRects.get(
					sidebarMarkerId("settled-placeholder"),
				)!,
			})[0]?.id,
		).toBe(sidebarMarkerId("settled-placeholder"));
		expect(isValid.mock.calls).toEqual([
			["blocked"],
			[sidebarMarkerId("settled-placeholder")],
		]);
	});
...
describe("sidebar drag projection", () => {
	...
	it.each([
		...
		for (let index = 0; index < pinned.length; index += 1) {
			...
			expect(strategy({ ...args, index })).toEqual(
				verticalListSortingStrategy({ ...args, index }),
			);
		}
	});
	...
	it.each([
		...
		"uses pinned dragging behavior for Active from %s to %s",
		(active, over) => {
			const items = [
				pinnedHeader,
				thread("p", "pinned"),
	...
	it("preserves settled order while opening the zero-height Active target", () => {
		...
		expect(result.get(sidebarMarkerId("active-placeholder"))).toEqual(
			stationary,
		);
		expect(result.get(sidebarMarkerId("settled-header"))).toEqual({
			...stationary,
			y: 36,
		});
	...
	it.each([
		...
		(active, dividerOffset, settledOffset) => {
			...
			expect(result.get(sidebarMarkerId("pinned-divider"))?.y).toBe(
				dividerOffset,
			);
			expect(result.get(sidebarMarkerId("settled-header"))?.y).toBe(
				settledOffset,
			);
		},
	);
	...
	it("keeps a collapsed settled target without inserting a hidden row", () => {
		...
		expect(result.get(sidebarMarkerId("settled-header"))?.y).toBe(-83);
		expect(result.get(sidebarMarkerId("settled-placeholder"))).toEqual({
			...stationary,
			y: -83,
		});
	});
	...
	it("preserves a collapsed snoozed header while another section changes", () => {
		...
		expect(result.get(sidebarMarkerId("snoozed-header"))).toEqual({
			...stationary,
			y: -46,
		});
	});
...
describe("lifted card clearance", () => {
	...
	it.each([36, 82])(
		"keeps a %ipx row below empty Pins even past the top edge",
		(height) => {
