...
export function createSidebarCollisionDetection(
	...
	return (args) => {
		...
		const source = items?.find(
			(item) => item.kind === "thread" && item.key === args.active.id,
		);
		...
		if (items && boundary && source?.kind === "thread" && pointer) {
			...
			if (pointer.x >= boundary.left && pointer.x <= boundary.right) {
				...
				const activeBottom =
					nextHeader?.node.current?.getBoundingClientRect().top;
				if (
					boundarySection === "pinned" ||
					...
					const target = collisions.find((collision) => {
						...
						if (!sections.has(id)) {
							sections.set(
								id,
								resolveSidebarDropTarget(items, String(args.active.id), id)
									?.section ?? null,
							);
						}
					...
					if (target)
						collisions = [
							target,
							...collisions.filter((collision) => collision !== target),
						];
				}
			}
		}
...
export function createSidebarSortingStrategy(input: {
	...
	const indices = new Map(
		items.map((item, index) => [sidebarListItemId(item), index]),
	);
	...
	function project({ rects, activeIndex, overIndex }: Layout) {
		...
		if (
			routeKey &&
			settledOrder.includes(routeKey) &&
			!visible.includes(routeKey)
		) {
		...
		const marker = (name: SidebarListMarker) =>
			projected.push({ kind: "marker", marker: name });
		...
		if (
			...
			((active.section !== "snoozed" || (input.snoozedThreadCount ?? 0) > 1) &&
				items.some(
					(item) => item.kind === "marker" && item.marker === "snoozed-header",
				))
		) {
