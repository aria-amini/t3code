...
describe("animateSidebarLayoutChanges", () => {
	...
	it("keeps layout movement while the user is sorting", () => {
		expect(animateSidebarLayoutChanges({ ...baseArgs, isSorting: true })).toBe(
			true,
		);
	});
});
...
describe("deleteSelectedThreadEntries", () => {
	...
	it("continues after ordinary failures and keeps the first failure", async () => {
		const laterFailure = AsyncResult.failure(
			Cause.fail(new Error("Later failure")),
		);
		...
		const outcome = await deleteSelectedThreadEntries({
			...
			delete: async ({ threadKey }, deletedThreadKeys) => {
				...
				if (threadKey === "four")
					deletedKeysAtLastEntry.push([...deletedThreadKeys]);
				return success;
			},
		});
	...
	it.each([
		...
		"stops on interruption and preserves earlier results %#",
		async (testCase) => {
...
describe("archiveSelectedThreadEntries", () => {
	...
	it("stops at a mutation failure and retains prior successes", async () => {
		const archive = vi.fn(
			async (entry: (typeof entries)[number], onArchived: () => void) => {
...
describe("shouldRecedeSidebarThread", () => {
	...
	it("keeps active and selected working threads prominent", () => {
		...
		expect(shouldRecedeSidebarThread({ ...input, isSelected: true })).toBe(
			false,
		);
	});
});
...
describe("getSidebarThreadIdsToPrewarm", () => {
	it("returns only the first visible thread ids up to the prewarm limit", () => {
		expect(getSidebarThreadIdsToPrewarm(["t1", "t2", "t3"], 2)).toEqual([
			"t1",
			"t2",
		]);
	});
	...
	it("returns all visible thread ids when they fit within the limit", () => {
		expect(getSidebarThreadIdsToPrewarm(["t1", "t2"], 10)).toEqual([
			"t1",
			"t2",
		]);
	});
...
describe("isSidebarNestedLinkClick", () => {
	const linkTarget = {
		closest: (selector: string) =>
			selector === "a[href]" ? ({} as Element) : null,
...
describe("orderItemsByPreferredIds", () => {
	...
	it("honors projectOrder physical keys via getProjectOrderKey", async () => {
		...
		const ordered = orderItemsByPreferredIds({
			items: projects,
			preferredIds: [
				getProjectOrderKey(projects[2]!),
				getProjectOrderKey(projects[0]!),
			],
...
describe("resolveSidebarThreadStatus", () => {
	...
	it("defaults to ready with no session", () => {
		expect(resolveSidebarThreadStatus({ ...idle, session: null })).toBe(
			"ready",
		);
	});
});
...
describe("searchSidebarThreadsByTitle", () => {
	...
	it("matches thread titles case-insensitively and preserves their order", () => {
		expect(searchSidebarThreadsByTitle(threads, "work")).toEqual([
			threads[0],
			threads[2],
		]);
	});
	...
	it("does not match project metadata", () => {
		expect(searchSidebarThreadsByTitle(threads, "workspace")).toEqual([
			threads[0],
		]);
	});
...
describe("planPinnedReorder", () => {
	it("writes only the moved thread when neighbors are keyed", () => {
		...
		expect(
			assignments[0]!.orderKey > "f" && assignments[0]!.orderKey < "m",
		).toBe(true);
	});
...
describe("resolveSidebarDropTarget", () => {
	...
	it("uses arrayMove placement, so a marker hovered from below lands above it", () => {
		...
		expect(resolve("a2", sidebarMarkerId("settled-header"))?.section).toBe(
			"settled",
		);
...
describe("planSidebarThreadDrop", () => {
	...
	const plan = (
		overrides: Partial<
			Omit<Parameters<typeof planSidebarThreadDrop>[0], "target">
		> & {
			activeKey: string;
	...
	it.each(["pinned", "active"] as const)(
		"reserves hidden %s slots during a drop",
		(section) => {
			const order =
				section === "pinned" ? ["p2", "p1", "p3"] : ["a2", "a1", "a3"];
			const keys = new Map(
				section === "pinned" ? pinnedKeysById : activeKeysById,
			);
			const moved = section === "pinned" ? "p1" : "a1";
			const reserved = pinOrderKeyBetween(
				keys.get(order[0]!)!,
				keys.get(order[2]!)!,
			)!;
	...
	it.each([
		...
		"clears a snoozed thread's $state state before waking it into Active",
		(hiddenState) => {
			expect(
				plan({
					activeKey: "z1",
	...
	it("saves the first Active reorder, then moves only one key on subsequent drops", () => {
		...
		const savedKeys = new Map(
			first.assignments.map(({ id, orderKey }) => [id, orderKey]),
		);
		const savedRows = rows.map((row) => ({
			...row,
			activeOrderKey: savedKeys.get(row.id) ?? null,
		}));
		expect(sortThreadsForSidebar(savedRows).map((row) => row.id)).toEqual(
			firstOrder,
		);
		...
		expect(second.assignments).toEqual([
			{ id: "a1", orderKey: expect.any(String) },
		]);
		const finalRows = savedRows.map((row) =>
			row.id === "a1"
				? { ...row, activeOrderKey: second.assignments[0]!.orderKey }
				: row,
		);
		expect(sortThreadsForSidebar(finalRows).map((row) => row.id)).toEqual(
			secondOrder,
		);
	});
	...
	it("settles anything dropped on Settled except a settled thread", () => {
		...
		expect(plan({ activeKey: "s1", activeSection: "settled", target })).toEqual(
			{ kind: "none" },
		);
	});
	...
	it("reorders an already-pinned snoozed thread after pinning wakes it", () => {
		...
		if (result.kind !== "pin") return;
		expect(result.extraAssignments).toEqual([
			{ id: "z1", orderKey: result.orderKey },
		]);
		expect(result.orderKey! > "f" && result.orderKey! < "m").toBe(true);
	});
	...
	it("uses keyed disabled neighbors as anchors without writing to them", () => {
		...
		expect(reorder.assignments).toEqual([
			{ id: "p3", orderKey: expect.any(String) },
		]);
	...
	it.each([
		...
		"rejects $activeSection drops that require rewriting a disabled neighbor",
		(source) => {
			expect(
				plan({
	...
	it("rewrites the section when a foreign thread lands next to a keyless pin", () => {
		...
		if (result.kind !== "pin") return;
		expect(result.orderKey).toBeDefined();
		expect(result.extraAssignments.map((entry) => entry.id)).toEqual([
			"p1",
			"p2",
			"p3",
		]);
	...
	it("reorders within the pinned block, and is a no-op when the order is unchanged", () => {
		...
		expect(down.assignments).toEqual([
			{ id: "p1", orderKey: expect.any(String) },
		]);
...
describe("applySidebarThreadDrop", () => {
	...
	it("previews an un-settle at the same active position as the eventual server row", () => {
		...
		expect(
			sortThreadsForSidebar([newer, preview]).map((row) => row.id),
		).toEqual(["dragged", "newer"]);
		expect(
			sortThreadsForSidebar([newer, preview]).map((row) => row.id),
		).toEqual(sortThreadsForSidebar([newer, final]).map((row) => row.id));
	});
	...
	it.each([
		...
		"preserves the active sort anchor when clearing a $state",
		({ state: _state, ...parked }) => {
			const source = thread({
				...parked,
	...
	it("previews a new settlement at the same position as the eventual server row", () => {
		...
		expect(
			sortSettledThreadsForSidebar([existing, preview]).map((row) => row.id),
		).toEqual(["dragged", "newer"]);
		expect(
			sortSettledThreadsForSidebar([existing, preview]).map((row) => row.id),
		).toEqual(
	...
	it("clears the manual Active position when settling so reopening returns to the top", () => {
		...
		expect(
			sortThreadsForSidebar([newer, reopened]).map((row) => row.id),
		).toEqual(["dragged", "newer"]);
	});
});

describe("sortPinnedThreadsForSidebar", () => {
	const pinnable = (input: {
		id: string;
		createdAt: string;
...
describe("resolveWorkingStartedAt", () => {
	...
	it("returns null with neither a running turn nor a session", () => {
		expect(
			resolveWorkingStartedAt({ latestTurn: null, session: null }),
		).toBeNull();
	});
});
...
describe("sortLogicalProjectsForSidebar", () => {
	it("uses saved order only in manual mode and activity order otherwise", () => {
		...
		const projects = [
			{
				...
				memberProjectRefs: [
					{ environmentId: localEnvironmentId, projectId: olderProjectId },
				],
			},
			{
				...
				memberProjectRefs: [
					{ environmentId: localEnvironmentId, projectId: newerProjectId },
				],
			},
		];
		const threads = [
		...
		expect(sortLogicalProjectsForSidebar(projects, threads, "manual")).toEqual(
			projects,
		);
		expect(
