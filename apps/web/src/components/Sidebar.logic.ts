...
const THREAD_SELECTION_SAFE_SELECTOR =
...
export function useSidebarRowSubscriptionLease(isActive: boolean): {
	...
	React.useEffect(() => {
		...
		const scrollRoot = row.closest<HTMLElement>(
...
export function useRetainedValue<T>(
	...
	return key !== null && retained.current?.key === key
...
export function resolveSidebarDropTarget(
	...
	const activeIndex = items.findIndex(
		(item) => sidebarListItemId(item) === activeKey,
	);
	const overIndex = items.findIndex(
		(item) => sidebarListItemId(item) === overId,
	);
	if (
		activeIndex === -1 ||
		overIndex === -1 ||
...
export function planSidebarThreadDrop(input: {
	...
	if (
		input.supportsSettlement === false &&
		(target.section === "settled" || activeSettled)
	) {
		return { kind: "none" };
	}
	switch (target.section) {
		case "active": {
			...
			if (
				activeReorderableKeys &&
				assignments.some(({ id }) => !activeReorderableKeys.has(id))
			) {
				return { kind: "none" };
			}
			return {
		...
		case "pinned": {
			...
			if (
				reorderableKeys &&
				assignments.some(({ id }) => !reorderableKeys.has(id))
			) {
				return { kind: "none" };
			}
			...
			return {
				kind: "pin",
				order,
				orderKey: assignments.find((assignment) => assignment.id === activeKey)
					?.orderKey,
...
>(
	...
	return {
		...
		...(section === "active" && orderKey !== undefined
...
export function useThreadJumpHintVisibility(): {
	...
	const controllerRef = React.useRef<ThreadJumpHintVisibilityController | null>(
		null,
	);
	...
	const updateThreadJumpHintsVisibility = React.useCallback(
		(shouldShow: boolean) => {
...
>(threads: readonly T[]): T[] {
	...
	return [...threads].toSorted(
		(left, right) =>
			timestampMs(right) - timestampMs(left) || left.id.localeCompare(right.id),
	);
}
...
export function resolveProjectStatusIndicator(
	...
	for (const status of statuses) {
		...
		if (
			...
			THREAD_STATUS_PRIORITY[status.label] >
				THREAD_STATUS_PRIORITY[highestPriorityStatus.label]
		) {
...
export function getProjectSortTimestamp(
	...
	if (projectThreads.length > 0) {
		return projectThreads.reduce(
			(latest, thread) =>
				Math.max(latest, getThreadSortTimestamp(thread, sortOrder)),
	...
	return (
		toSortableTimestamp(project.updatedAt ?? project.createdAt) ??
		Number.NEGATIVE_INFINITY
	);
}
...
function sortProjectsByActivity<TProject extends SidebarProject>(
	...
	return [...projects].toSorted((left, right) => {
		const rightTimestamp = getProjectSortTimestamp(
			right,
			getProjectThreads(right),
			sortOrder,
		);
		const leftTimestamp = getProjectSortTimestamp(
			left,
			getProjectThreads(left),
			sortOrder,
		);
		const byTimestamp =
...
>(
	...
	for (const thread of threads) {
		...
		const projectKey = groupKeyByProjectRef.get(
...
>(
	...
	return sortProjectsByActivity(
		...
		sortOrder,
		(project) =>
			threadsByProject.get(scopedKey(project.environmentId, project.id)) ?? [],
		(left, right) =>
			left.title.localeCompare(right.title) ||
