...
describe("sidebar list motion", () => {
	...
	it("fades a collapsed-shelf exit at its current visual box and a new wake in", () => {
		...
		expect(clone.children[0]!.attributes).toEqual([
	...
	it("skips fades when a large list change would clone too many rows", () => {
		...
		const entering = Array.from(
			{ length: 41 },
			(_, index) => new TestRow(`new-${index}`),
		);
		layout(entering);
		motion.update(true);
		expect(entering.every((row) => row.animate.mock.calls.length === 0)).toBe(
			true,
		);
	});
