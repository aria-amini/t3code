...
describe("sidebar pointer lifecycle", () => {
	...
	const interruptions = {
		...
		escape: () =>
			document.dispatchEvent(
				Object.assign(new Event("keydown"), { code: "Escape" }),
			),
		pointercancel: () => document.dispatchEvent(pointer("pointercancel")),
		"missed release": () =>
			document.dispatchEvent(
				pointer("pointermove", { buttons: 0, clientY: 100 }),
			),
	};
	for (const [name, interrupt] of Object.entries(interruptions)) {
		it.each([false, true])(
			`cancels on ${name}, started=%s, and ignores late events`,
			(started) => {
				const drag = gesture();
				if (started)
	...
	it("ignores events from other pointers", () => {
		const drag = gesture();
		document.dispatchEvent(
			pointer("pointermove", { pointerId: 2, buttons: 0, clientY: 100 }),
		);
	...
	it.each([false, true])(
		"cancels when search unmounts the list, started=%s",
		(started) => {
