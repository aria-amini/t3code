...
export class SidebarPointerSensor {
	...
	private clearClickSuppression = () => {
		this.document.removeEventListener("click", this.suppressClick, {
			capture: true,
		});
	...
	private move = (event: PointerEvent) => {
		if (this.phase === "finished" || event.pointerId !== this.pointer.pointerId)
			return;
		...
		if (this.phase === "pending") {
			...
			this.phase = "dragging";
			this.document.addEventListener("click", this.suppressClick, {
				capture: true,
			});
