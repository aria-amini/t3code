...
describe("resolveSnoozePresets", () => {
	...
	it("drops the evening preset once evening is near or past", () => {
		expect(
			resolveSnoozePresets(localDate(2026, 4, 8, 17, 30), "locale").map(
				(preset) => preset.id,
			),
		).toEqual(["hour", "three-hours", "tomorrow", "next-week"]);
		expect(
			resolveSnoozePresets(localDate(2026, 4, 8, 21), "locale").map(
				(preset) => preset.id,
			),
		).toEqual(["hour", "three-hours", "tomorrow", "next-week"]);
	});
	...
	it("formats preset times with the selected clock preference", () => {
		...
		expect(
			twelveHour.find((preset) => preset.id === "evening")!.whenLabel,
		).toMatch(/PM/i);
		expect(
			twentyFourHour.find((preset) => preset.id === "evening")!.whenLabel,
		).toBe("18:00");
	});
});
	...
	it("uses bare time today, 'tomorrow' next day, weekday within the week", () => {
		expect(
			snoozeWakeDescription(
				localDate(2026, 4, 8, 18).toISOString(),
				now,
				"locale",
			),
		...
		expect(
			snoozeWakeDescription(
				localDate(2026, 4, 9, 9).toISOString(),
				now,
				"locale",
			),
		).toContain("tomorrow");
