...
function WorkingDuration(props: { startedAt: string | null }) {
	const startedMs =
		props.startedAt !== null ? Date.parse(props.startedAt) : Number.NaN;
...
const EMPTY_PROVIDER_ENTRIES: ReadonlyMap<string, ProviderInstanceEntry> =
	new Map();
				...
				<div className="grid gap-1.5 pl-0.5 text-xs text-muted-foreground">
					...
					{branchMismatch ? (
						<div className="flex min-w-0 items-start gap-2 text-warning">
							<CircleAlertIcon
								aria-hidden
								className="mt-0.5 size-3 shrink-0 stroke-current"
							/>
					...
					{driverKind ? (
						<div className="flex min-w-0 items-center gap-2">
							<ProviderInstanceIcon
								...
								showBadge={
									showInstanceBadge && providerEntry?.accentColor !== undefined
								}
								badgeContent="none"
...
const draftPenClassName =
	"size-3 shrink-0 text-amber-600 dark:text-amber-300/80";
...
const SidebarDraftBlock = memo(function SidebarDraftBlock(props: {
	...
	const draftsByThreadKey = useComposerDraftStore(
		(store) => store.draftsByThreadKey,
	);
	...
	const drafts = useMemo(() => {
		...
		for (const [draftKey, session] of Object.entries(draftThreadsByThreadKey)) {
			...
			if (draftKey === props.routeDraftId) {
				...
				if (
					frozenActive.routeDraftId === draftKey &&
			...
			{drafts.map(({ composer, draftId, session }) => {
				...
				return (
					<SidebarDraftRow
						...
						projectDisplayName={
							props.projectDisplayNameByKey.get(projectKey) ?? null
						}
						projectCwd={props.projectCwdByKey.get(projectKey) ?? null}
						projectFaviconPath={
							props.projectFaviconPathByKey.get(projectKey) ?? null
						}
...
const SidebarThreadRow = memo(function SidebarThreadRow(props: {
	...
	const lastVisitedAt = useUiStateStore(
		(state) => state.threadLastVisitedAtById[threadKey],
	);
	...
	const wokeAtDate =
		props.wokeAt === null ? null : parseTimestampDate(props.wokeAt);
	const isWoke =
		wokeAtDate !== null &&
	...
	const settledPrHoverClass = pr
		? settledPrHoverColorClass(pr.state, pr.isDraft)
		: undefined;
	...
	const showPin =
		props.isPinned &&
		(!sortable?.isDragging ||
			(props.dragOverPinned && props.dropVerb === null));
              ...
              the time/jump label yields to the settle affordance. */}
						...
						{sortable?.isDragging ? (
							...
							<span className="relative ml-auto flex h-6 min-w-8 shrink-0 items-center justify-end">
								...
								) : !props.settlementSupported ? null : variantAction ===
								  "unsettle" ? (
									<Tooltip>
										<TooltipTrigger
											render={
									...
									hasUnsentDraft ? (
										<span
											className={cn(
												...
												snoozeMenuOpen &&
													"pointer-events-auto static opacity-100",
											)}
										>
											{hasUnsentDraft ? (
												<Tooltip>
...
export default function Sidebar() {
	...
	const sidebarProjectSortOrder = useClientSettings(
		(s) => s.sidebarProjectSortOrder,
	);
	...
	const unsortedProjectGroups = useMemo(
		() =>
			buildSidebarProjectSnapshots({
				projects:
					sidebarProjectSortOrder === "manual" ? orderedProjects : projects,
			...
			deriveProviderEntriesByEnvironment(
				[...serverConfigs].map(
					([environmentId, config]) =>
						[environmentId, config.providers] as const,
				),
			),
		[serverConfigs],
	);
	...
	const projectScopeKey = useUiStateStore(
		(store) => store.sidebarProjectScopeKey,
	);
	const setProjectScopeKey = useUiStateStore(
		(store) => store.setSidebarProjectScopeKey,
	);
	...
	const selectedProjectScopeItem = useMemo(
		() =>
			projectScopeItems.find(
				(item) => item.value === (projectScopeKey ?? "all"),
			) ?? projectScopeItems[0]!,
	...
	const scopedProjectGroup = useMemo(
		() =>
			projectScopeKey === null
				? null
				: (projectGroups.find(
						(project) => project.projectKey === projectScopeKey,
					) ?? null),
	...
	const routeDraftIdForRows =
		routeTarget?.kind === "draft" ? routeTarget.draftId : null;
	const visibleDraftSessionCount = useComposerDraftStore((store) => {
		let count = 0;
		for (const [draftKey, session] of Object.entries(
	...
	const {
		...
		for (const thread of visible) {
			...
			if (capabilities?.threadActiveReorder === true)
	...
	useEffect(() => {
		...
		const delayMs = Math.min(
			Math.max(0, nextWakeAtMs - Date.now()) + 50,
			2_147_483_647,
		);
	...
	const { showThreadJumpHints, updateThreadJumpHintsVisibility } =
		useThreadJumpHintVisibility();
		...
		(event: ReactKeyboardEvent<HTMLInputElement>) => {
			...
			if (event.key === "ArrowDown") {
				...
				setActiveSearchResultIndex(
					(index) => (index + 1) % threadSearchResults.length,
				);
				return;
			}
			if (event.key === "ArrowUp") {
				...
				setActiveSearchResultIndex(
					(index) =>
						(index - 1 + threadSearchResults.length) %
	...
	const planForwardNavigation = useCallback(
		(
			...
			const nextThread = nextCardKey
				? threadByKeyRef.current.get(nextCardKey)
				: null;
			return nextThread
				? () =>
						navigateToThread(
	...
	const attemptSettle = useCallback(
		(
			...
			void (async () => {
				...
				try {
					const navigateAfterSettle = planForwardNavigation(
						threadKey,
						opts.coSettlingKeys,
					);
	...
	const listMotionRef = useRef<ReturnType<
		typeof createSidebarListMotion
	> | null>(null);
	const attachListMotionRef = useCallback((node: HTMLUListElement | null) => {
		...
		listMotionRef.current =
			node === null ? null : createSidebarListMotion(node);
	...
	const handleThreadDragStart = useCallback(
		(event: DragStartEvent) => {
			...
			if (list && header) {
				...
				const scale =
					list.offsetWidth > 0 ? listRect.width / list.offsetWidth : 1;
	...
	useLayoutEffect(() => {
		...
		listMotionRef.current?.update(
			!listMotionPaused &&
				sidebarListItems.length + visibleDraftSessionCount > 0,
		);
	}, [
		listMotionPaused,
		routeDraftIdForRows,
	...
	const sortableIds = useMemo(
		() => sidebarListItems.map(sidebarListItemId),
		[sidebarListItems],
	);
	const draggedSettledOrder = useMemo(() => {
		...
		return sortSettledThreadsForSidebar([
			...settledThreads.filter(
				(candidate) => key(candidate) !== dragState.activeKey,
			),
	...
	const handleThreadDragEnd = useCallback(
		(event: DragEndEvent) => {
			...
			const plan = planSidebarThreadDrop({
				...
				supportsSettlement:
					serverConfigs.get(activeThread.environmentId)?.environment
			...
			if (
				plan.kind === "settle" &&
				settlingThreadKeysRef.current.has(activeKey)
			)
				return;
			const assignments =
				plan.kind === "pin"
					? [
							...(plan.orderKey === undefined
								? []
								: [{ id: activeKey, orderKey: plan.orderKey }]),
			...
			const drop = {
				...
				keysAtDrop:
					target.section === "active" ? activeKeysById : pinnedKeysById,
				assignedKeys: new Map(
					assignments.map(({ id, orderKey }) => [id, orderKey]),
				),
			};
			...
			void (async () => {
				...
				switch (plan.kind) {
					case "settle": {
						...
						if (settled && routeThreadKeyRef.current === activeKey)
							navigateAfterSettle?.();
						return;
					}
					case "move-active":
						...
						if (
							plan.unpin &&
							!(await run(unpinThread(threadRef), "Failed to unpin thread"))
						)
							return;
						if (
							plan.unsettle &&
				...
				const keyWrites =
					plan.kind === "pin" ? plan.extraAssignments : plan.assignments;
	...
	const performSnooze = useCallback(
		async (
			...
			try {
				...
				const navigateAfterSnooze = planForwardNavigation(
					threadKey,
					opts.coSnoozingKeys,
				);
	...
	const handleMultiSelectContextMenu = useCallback(
		async (position: { x: number; y: number }) => {
			...
			const titleRegenerationMenuItem =
				buildBulkTitleRegenerationContextMenuItem({
			...
			const pinnedSelectedThreads = selectedThreads.filter(
				(thread) =>
					serverConfigs.get(thread.environmentId)?.environment.capabilities
						.threadPinning === true && thread.pinnedAt != null,
			);
			...
			if (clicked.value?.startsWith("snooze:")) {
				...
				if (preset) {
					...
					const snoozedThreadRefs = outcomes.flatMap(
						({ outcome, threadRef }) =>
						...
						toastManager.add(
							stackedThreadToast({
								...
								actionProps: {
									children: "Undo",
									onClick: () => {
										for (const threadRef of snoozedThreadRefs)
			...
			const { deletedThreadKeys, firstFailure } =
							...
							<div className="flex h-8 min-w-0 flex-1 items-center gap-2 rounded-md px-2 py-1.5 text-sm font-medium text-sidebar-muted-foreground hover:bg-sidebar-row-hover hover:text-sidebar-foreground">
								...
								<Input
									...
									aria-activedescendant={
										isSearchingThreads &&
										threadSearchResults[activeSearchResultIndex]
									...
									>
										...
										<ComboboxList>
											{(item: (typeof projectScopeItems)[number]) => {
												const project =
													projectGroupByScopeKey.get(item.value) ?? null;
												return (
													<ComboboxItem
														key={item.value}
														hideIndicator
										...
										return (
											<SidebarSearchResultRow
												...
												providerEntryByInstanceId={
													providerEntriesByEnvironment.get(
														thread.environmentId,
					...
					{!isSearchingThreads ? (
						...
						>
							...
							>
								...
								<SortableContext
									items={sortableIds}
									strategy={sidebarSortingStrategy}
								>
									<ul
													...
													<SidebarThreadRow
														...
														settlementSupported={
															serverConfigs.get(thread.environmentId)
																?.environment.capabilities.threadSettlement ===
															true
														}
														snoozeSupported={
															serverConfigs.get(thread.environmentId)
														...
														pinningSupported={
															serverConfigs.get(thread.environmentId)
																?.environment.capabilities.threadPinning ===
															true
														}
														...
														environmentLabel={
															environmentLabelById.get(thread.environmentId) ??
															null
														}
														environmentMachine={
															environmentMachineById.get(
																thread.environmentId,
															) ?? "server"
														}
														projectCwd={
