...
const THREAD_COMMAND_SUBTITLE_VARIANT: ThreadCommandSubtitleVariant =
	"favicon-workspace-harness";

export const COMMAND_PALETTE_META_ICON_CLASS =
...
function WorkspaceIcon(props: {
	...
	if (props.isWorktree) {
		return (
			<FolderGit2Icon className={COMMAND_PALETTE_META_ICON_CLASS} aria-hidden />
		);
	}
...
export function ThreadCommandSubtitle(props: {
	...
	const isWorktree =
		props.worktreePath != null && props.worktreePath.trim().length > 0;
	const showHarness =
	...
	return (
		...
		>
			...
			{props.isCurrent ? (
				<>
					{projectLabel || branchLabel || showHarness ? (
