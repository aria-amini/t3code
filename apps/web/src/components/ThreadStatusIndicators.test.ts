...
describe("ChangeRequestStatusIcon", () => {
	it.each([
		...
		"uses the %s pull request glyph",
		(_label, state, isDraft, expectedIcon) => {
			expect(ChangeRequestStatusIcon({ state, isDraft }).type).toBe(
				expectedIcon,
			);
		},
	);
...
describe("prStatusIndicator", () => {
	...
	it("uses red for closed pull requests", () => {
		...
		expect(
			prStatusIndicator({ ...closedPr, state: "closed" }, undefined)
				?.colorClass,
	...
	it("uses gray and draft wording for draft pull requests", () => {
		...
		expect(
			prStatusIndicator({ ...draftPr, isDraft: true }, undefined),
		).toMatchObject({
			label: "PR draft",
...
describe("settledPrHoverColorClass", () => {
	it.each([
		...
		"restores the %s pull request color on row hover",
		(state, colorClass) => {
			expect(settledPrHoverColorClass(state)).toContain(
