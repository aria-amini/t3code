...
export function terminalThemeFromApp(
...
export function TerminalViewport({
	...
	const handleAddTerminalContext = useEffectEvent(
		(selection: TerminalContextSelection) => {
			onAddTerminalContext?.(selection);
		},
	);
	const canAddSelectionToChat = useEffectEvent(
	...
	const synchronizedStatusRef =
		useRef<TerminalSessionState["status"]>("closed");
	const synchronizeTerminalStatus = useEffectEvent(
		(
			...
			if (status === "running") {
				...
				writeSystemMessage(
					terminal,
					status === "closed" ? "Terminal closed" : "Process exited",
				);
				window.setTimeout(() => {
	...
	useEffect(() => {
		...
		void terminalRef.current?.setFont(
			terminalFontOptions(terminalFontFamily, terminalFontSize),
		);
	...
	useEffect(() => {
		...
		const setup = async (): Promise<(() => void) | null> => {
			...
			const terminal = await GhosttyTerminalSurface.create(
				mount,
				terminalOptions,
			);
			if (cancelled) {
			...
			if (
				...
				void terminal.setFont(
					terminalFontOptions(currentFont.family, currentFont.size),
				);
			}
			...
			if (latestSession.error !== null)
				writeSystemMessage(terminal, latestSession.error);
			...
			const dismissSelectionAction = (supersede = false) => {
				const ownsMenu =
					openSelectionMenuRequestIdRef.current ===
					selectionActionRequestIdRef.current;
			...
			const readSelectionAction = (
				...
				const normalizedText = selectionText
					.replace(/\r\n/g, "\n")
					.replace(/^\n+|\n+$/g, "");
...
export default function ThreadTerminalDrawer({
	...
	const drawerHeight =
		drawerHeightState.threadId === threadId
			? drawerHeightState.height
			: controlledDrawerHeight;
			...
			setDrawerHeightState((current) => {
				const currentHeight =
					current.threadId === threadId
						...
						: controlledDrawerHeight;
				const nextHeight =
					typeof update === "function" ? update(currentHeight) : update;
	...
	const setDrawerHeightFromWindowResize = useEffectEvent(
		(nextHeight: number) => {
	...
	const terminalLabelById = useMemo(() => {
		...
		for (const terminalId of normalizedTerminalIds) {
			next.set(
				terminalId,
				terminalLabelsById?.get(terminalId) ?? getTerminalLabel(terminalId),
			);
		}
		return next;
	...
	const activeTerminalLaunchLocation = resolveTerminalLaunchLocation(
						...
						{isSplitView ? (
							...
							>
								{visibleTerminalIds.map((terminalId) => {
									const terminalLaunchLocation =
										resolveTerminalLaunchLocation(terminalId);
									return (
										<div
											key={terminalId}
											...
											<div className="h-full">
												<TerminalViewport
													...
													{...(terminalLaunchLocation.worktreePath !== undefined
														? {
																worktreePath:
																	terminalLaunchLocation.worktreePath,
															}
														: {})}
