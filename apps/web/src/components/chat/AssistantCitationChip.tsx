...
export function AssistantCitationChip({
	...
	useEffect(() => {
		...
		const anchor =
			sourceAnchor ?? findAssistantCitationSourceAnchor(document, citation);
		if (!anchor) return;
		...
		? {
				...
				getBoundingClientRect: () => {
					...
					return (
						...
						sourceAnchor.range.getBoundingClientRect()
					);
				},
			}
		: undefined;
	const preview = (citation.comment?.trim() || citation.text).replace(
		/\s+/g,
		" ",
	);
	...
	const composerSourceLink = (
		...
		>
			...
			/>
			<span
				className={cn(COMPOSER_INLINE_CHIP_LABEL_CLASS_NAME, "max-w-[16em]")}
			>
				{label}
			</span>
		</Link>
		...
		>
			...
			{commentEditor ? (
				<Popover
					...
					onOpenChange={commentEditor.onOpenChange}
				>
					<PopoverTrigger
						aria-label={
							citation.comment
								...
								{...(commentEditor.onSaveAndSend
									? {
											onSubmitAndSend: (comment: string) => {
												if (!commentEditor.onSaveAndSend?.(comment))
													return false;
												commentEditor.onOpenChange(false);
												return true;
											},
