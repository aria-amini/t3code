...
class TestElement {
	...
	contains(node: TestElement | null): boolean {
		return (
			node === this ||
			(node?.parentNode ? this.contains(node.parentNode) : false)
		);
	}
}
...
function createSource({ reducedMotion = false } = {}) {
	...
	root.getBoundingClientRect = () =>
		rect(sourceTop - scrollNode.scrollTop, 300);
	mocks.resolveRange.mockImplementation(
		() =>
			new TestRange(text, () =>
				rect(sourceTop - scrollNode.scrollTop, rangeHeight),
			),
	);
	...
	let pending: { options: { offset: number }; resolve: () => void } | null =
		null;
	...
	const list = {
		...
		getState: () => ({
			...
			listenToPosition: (_key: string, callback: () => void) =>
				listen(callback),
	...
	const mutation = (changed: TestElement, removed: TestElement[] = []) => {
		for (const callback of mutationCallbacks) {
			callback(
				[
					{ target: changed, addedNodes: [], removedNodes: removed },
	...
	return {
		...
		highlight: () =>
			highlights.get("t3-assistant-citation")?.values().next().value,
...
describe("assistant citation source lifecycle", () => {
	it("waits for a mounted measurement and rechecks geometry after the list settles", async () => {
		...
		source.flushFrame();
		expect(source.scrollToOffset).toHaveBeenCalledExactlyOnceWith({
			offset: 80,
			animated: true,
		});
		...
		expect(source.target.onComplete).not.toHaveBeenCalled();
		expect(source.scrollToOffset).toHaveBeenLastCalledWith({
			offset: 140,
			animated: true,
		});
		await source.finishScroll();
	...
	it("uses instant navigation and removes the temporary quote highlight for reduced motion", async () => {
		...
		source.mount();
		source.flushFrame();
		expect(source.scrollToOffset).toHaveBeenCalledWith({
			offset: 80,
			animated: false,
		});
		expect(source.highlight()).toBeUndefined();
...
describe("assistant citation comment source lifecycle", () => {
	it("keeps the exact live range highlighted until cleanup without changing selection or navigation", () => {
		...
		const navigationHighlight = new Set([
			new TestRange(source.text, () => rect(20)),
		]);
		...
		expect(source.commentHighlight()?.values().next().value).toBe(
			comment.range,
		);
		...
		expect(source.highlights.get("t3-assistant-citation")).toBe(
			navigationHighlight,
		);
	...
	it.each(["collapsed", "escaped"])(
		"repairs a %s range after a virtual ancestor move",
		(state) => {
			const source = createSource();
			const comment = source.mountComment();
			if (state === "collapsed") {
	...
	it.each(["disconnected", "outside viewport"])(
		"closes once when the source is %s",
		(state) => {
			const source = createSource();
			const comment = source.mountComment();
