...
const CITATION_HIGHLIGHT_TOTAL_MS =
	1.5 * CITATION_PULSE_DURATION_MS +
	CITATION_HIGHLIGHT_HOLD_MS +
	CITATION_HIGHLIGHT_FADE_MS;
...
export function observeAssistantCitationCommentSource({
	...
	const dispose = () => {
		...
		if (
			highlight.size === 0 &&
			registry?.get(COMMENT_HIGHLIGHT_NAME) === highlight
		) {
...
export function observeAssistantCitationSource({
	...
	const clear = () => {
		if (
			highlighted &&
			CSS.highlights.get("t3-assistant-citation") === highlighted
		) {
		...
		if (selected && ownsSelection(selection))
			selection?.removeRange(selected.range);
		selected = null;
	...
	const highlight = () => {
		...
		if (
			activation.pulse &&
			performance.now() - activation.pulse.startedAt >=
				CITATION_HIGHLIGHT_TOTAL_MS
		) {
			finishHighlight();
			return;
		}
		...
		if (!pulseAnimation) {
			...
			const at = (milliseconds: number) =>
				milliseconds / CITATION_HIGHLIGHT_TOTAL_MS;
			const holdEnd = at(
				CITATION_HIGHLIGHT_TOTAL_MS - CITATION_HIGHLIGHT_FADE_MS,
			);
			pulseAnimation = root.animate(
				pulse.reducedMotion
					...
					: [
							...
							{
								offset: at(CITATION_PULSE_DURATION_MS),
								[CITATION_HIGHLIGHT_OPACITY]: 0,
							},
							{
			...
			pulseAnimation.currentTime = Math.max(
				0,
	...
	const observer = new MutationObserver((records) => {
		...
		if (rangeMoved) {
			canReplaceSelection ||=
				selected !== null && ownsSelection(root.ownerDocument.getSelection());
			clear();
		}
		if (
			rangeMoved ||
	...
	const dispose = () => {
		...
		if (activation.cancelScroll === cancelScroll)
			delete activation.cancelScroll;
		clear();
	};
	return dispose;
}
...
export function AssistantCitationSource({
	...
	useEffect(() => {
		...
		if (!root || !list || !request || request.citation.messageId !== messageId)
			return;
