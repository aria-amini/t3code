...
function useComposerRestingTransition(
	...
	const transitionToCurrentGeometry = useCallback(
		(stateChanged: boolean) => {
			...
			const action = visibleTransitionElement(
			...
			const interruptedDuration =
				interruptedAnimation?.effect?.getComputedTiming().duration;
			...
			if (stateChanged) {
				for (const animation of stateChangeAnimationsRef.current)
					animation.cancel();
			...
			const targetChanged =
				interruptedTargetHeight === null ||
				Math.abs(interruptedTargetHeight - nextHeight) >= 0.5;
			...
			if (
				...
				if (stateChanged) {
					...
					const previousPromptHeight =
						previousContentOffsetsRef.current.promptHeight;
					if (
						!nextIsCollapsed &&
						prompt &&
						nextPromptRect &&
			...
			previousContentOffsetsRef.current = {
				promptFromTop:
					nextPromptTop === null ? null : nextPromptTop - nextRect.top,
				promptHeight: nextPromptRect?.height ?? null,
				actionFromBottom:
					nextActionTop === null ? null : nextRect.bottom - nextActionTop,
			};
		},
	...
	useLayoutEffect(() => {
		...
		const overlay = elementRef.current?.closest<HTMLElement>(
			'[data-chat-composer-overlay="true"]',
		);
	...
	useLayoutEffect(() => {
		...
		const observer = new ResizeObserver((entries) => {
			...
			previousContentOffsetsRef.current = {
				promptFromTop:
					promptRect === undefined ? null : promptRect.top - elementRect.top,
				promptHeight: promptRect?.height ?? null,
				actionFromBottom:
					actionTop === undefined ? null : elementRect.bottom - actionTop,
			};
		});
...
function ComposerCommandMenuLayer(props: {
	...
	const [position, setPosition] = useState<ComposerCommandMenuPosition | null>(
		null,
	);
	...
	useLayoutEffect(() => {
		...
		const updatePosition = () => {
			const form = anchor.closest<HTMLElement>(
			...
			const drawerInsetRem = Number.parseFloat(
				window
					.getComputedStyle(form ?? anchor)
					.getPropertyValue("--chat-composer-drawer-inset"),
			);
			...
			setPosition((current) =>
				current && composerCommandMenuPositionsEqual(current, next)
					? current
					: next,
			);
		};
			...
			<>
				...
				<ComposerPrimaryActions
					...
					preserveComposerFocusOnPointerDown={
						props.preserveComposerFocusOnPointerDown ?? false
					}
...
export const ChatComposer = memo(function ChatComposer(
	...
	const activeTasksProgress =
		props.threadSyncPhase === null ? props.activeTasksProgress : null;
	const activeTaskSteps =
		props.threadSyncPhase === null ? props.activeTaskSteps : null;
	// ------------------------------------------------------------------
	...
	const pendingSnapShotIds = useMemo(
		() =>
			pendingSnapShotAnimationIdsForTarget(
				pendingSnapShotAnimations,
				composerDraftTarget,
			),
		[composerDraftTarget, pendingSnapShotAnimations],
	);
	const pendingSnapShotIdSet = useMemo(
		() => new Set(pendingSnapShotIds),
		[pendingSnapShotIds],
	);
	...
	const standaloneComposerImages = useMemo(() => {
		...
		return composerImages.filter(
			(image) => !previewAnnotationIds.has(image.id),
		);
	}, [composerImages, composerPreviewAnnotations]);
	const nonPersistedComposerImageIds = composerDraft.nonPersistedImageIds;
	const uploadsByImageId = useAttachmentUploadStore(
		(state) => state.uploadsByImageId,
	);
	const needsReattachFileCount = composerFiles.filter(
		composerFileNeedsReattach,
	).length;
	...
	const attachmentBlockReason =
		...
		(supportsAttachmentUploads
			? needsReattachFileCount > 0
				...
				: attachmentUploadBlockReason({
						imageIds: [...composerImages, ...composerFiles].map(
	...
	const removeComposerDraftFile = useComposerDraftStore(
		(store) => store.removeFile,
	);
	const setComposerDraftFileUpload = useComposerDraftStore(
		(store) => store.setFileUpload,
	);
	...
	>(() => selectedProviderEntry?.models ?? [], [selectedProviderEntry]);
	...
	const { enabled: planModeUiEnabled, interactionMode } =
		resolveComposerInteractionMode({
			planModeEnabled: settings.planModeEnabled,
	...
	const promptHistoryPositionRef = useRef<ComposerPromptHistoryPosition | null>(
		null,
	);
	...
	const [isComposerPrimaryActionsCompact, setIsComposerPrimaryActionsCompact] =
		useState(false);
	const [isComposerModelPickerOpen, setIsComposerModelPickerOpen] =
		useState(false);
	...
	const [composerSubmissionError, setComposerSubmissionError] = useState<
		string | null
	>(null);
	const [providerInputSubmissionError, setProviderInputSubmissionError] =
		useState<string | null>(null);
	...
	const removeComposerFileFromDraft = useCallback(
		(fileId: string) => {
			...
			const file = composerFilesRef.current.find(
	...
	const shouldBlurMobileComposerOnSubmit = useCallback(() => {
		...
		if (activePendingProgress) {
			return (
				...
				Boolean(activePendingResolvedAnswers)
			);
		}
	...
	const submitComposer = useCallback(
		(
			...
			const submission = submitComposerDraft({
				...
				submissionTarget: activePendingProgress
					? "pending-user-input"
					: "provider-turn",
				event,
				onSend: (sendEvent) => {
	...
	const expandMobileComposer = useCallback(() => {
		...
		mobileComposerExpandFrameRef.current = window.requestAnimationFrame(() => {
			...
			mobileComposerExpandReleaseFrameRef.current =
				window.requestAnimationFrame(() => {
	...
	const navigatePromptHistory = useCallback(
		(direction: "backward" | "forward", event: KeyboardEvent): boolean => {
			...
			if (
				!editor?.isCaretOnVisualEdge(direction === "backward" ? "start" : "end")
			) {
				return false;
			}
			const step = stepComposerPromptHistory({
				...
				entries: buildComposerPromptHistoryEntries(
					promptHistoryMessagesRef.current,
				),
	...
	const finalizeStashEntryImages = usePromptStashStore(
		(state) => state.finalizeEntryImages,
	);
	...
	const restoreStashEntry = useCallback(
		async (menuEntry: PromptStashEntry) => {
			...
			if (
				...
				composerDraftTargetKeyRef.current
			) {
				return;
			}
			...
			if (stashedFiles.length > 0) {
				...
				const existingFileIds = new Set(
					composerFilesNow.map((file) => file.id),
				);
				...
				const existingFileKeys = new Set(
					composerFilesNow.map(composerFileDedupKey),
				);
				const reattachMarkers = composerFilesNow.filter(
					composerFileNeedsReattach,
				);
				...
				for (const file of stashedFiles) {
					...
					const reattachMarker = reattachMarkers.find(
						(marker) =>
							...
							composerFileMatchesReattachMarker(marker, file),
					);
					if (reattachMarker) {
			...
			if (entry.attachments.length > 0) {
				const existingIds = new Set(
					composerImagesRef.current.map((image) => image.id),
				);
	...
	const stashCurrentPrompt = useCallback(async () => {
		...
		try {
			...
			const { kept, droppedNames } =
				partitionStashAttachments(candidateAttachments);

			const { attached, durable: imagesDurable } = finalizeStashEntryImages(
				entryId,
				{
					attachments: kept,
	...
	const expandedComposerImages = isComposerResting
		? standaloneComposerImages.filter((image) =>
	...
	const composerControlsVisibleInStrip =
		composerControlsInStrip && restingControlsVisible;
	...
	composerScrollCollapseEligibleRef.current =
		canScrollCollapseComposer && !isComposerResting;
			...
			const deltaPx =
				...
				(event.deltaMode === WheelEvent.DOM_DELTA_LINE
					...
					: event.deltaMode === WheelEvent.DOM_DELTA_PAGE
						...
						: 1);
			const shouldCollapse = recordComposerScrollGestureEvent(
				composerScrollGestureRef.current,
				{
					...
					collapseEligible:
						targetsTimeline && composerScrollCollapseEligibleRef.current,
	...
	const composerControlsCompact =
		!composerControlsInStrip && isComposerFooterCompact;
			...
			{composerControlsCompact ? (
				...
				<>
					{restingBlockDefs.map((def, index) => {
						...
						const hidden =
							index >= restingBlockDefs.length - restingHiddenBlockCount;
						return (
							<div
								key={def.id}
					...
					{composerControlsInStrip ? (
						<div
							...
							className={cn(
								"min-w-0 shrink-0",
								hiddenRestingBlockIds.length === 0 &&
						...
						>
							<CompactComposerControlsMenu
								...
								size="xs"
								hidden={
									composerControlsHidden || hiddenRestingBlockIds.length === 0
								}
	...
	const addComposerAttachments = async (files: File[]) => {
		...
		const reattachMarkers = composerFilesRef.current.filter(
			composerFileNeedsReattach,
		);
	...
	const insertComposerTextAtEnd = useCallback<
		ChatComposerHandle["insertTextAtEnd"]
	>(
		(text, options) => {
	...
	const onComposerMentionDragLeaveCapture = (
		...
		<form
			...
			onPointerDownCapture={(event) => {
				...
				const isInteractive = Boolean(
					target.closest(
						'button, a, input, select, [role="button"], [role="menuitem"]',
					),
				);
							...
							{isStashMenuOpen &&
								...
								!isComposerApprovalState && (
									<ComposerCommandMenuLayer anchor={composerMenuAnchor}>
										<ComposerStashMenu
											entries={stashQueue}
											stashShortcutLabel={shortcutLabelForCommand(
												keybindings,
												"composer.stash",
												{
													context: {
							...
							{!isComposerCollapsedMobile &&
								...
								composerElementContexts.length > 0 && (
									<ComposerPendingElementContexts
										...
										onRemove={(contextId) =>
											removeComposerDraftElementContext(
												composerDraftTarget,
												contextId,
											)
										}
											...
											.map((image) => {
												...
												return (
													<SnapShotAttachmentFrame
														...
														animationId={
															snapShotAnimationPending ? image.id : undefined
														}
														animationSource={
															snapShotAnimationPending
																? image.source
									...
									<div className="mb-3 flex flex-col gap-1">
										{composerOtherFiles.map((file) => {
											...
											const canReattachFile =
												...
												file.sizeBytes <= fileStagingLimit;
											return (
												<div
													key={file.id}
							...
							>
								...
								>
									...
									<ComposerFooterPrimaryActions
										compact={
											isComposerResting || isComposerPrimaryActionsCompact
										}
										activeContextWindow={
