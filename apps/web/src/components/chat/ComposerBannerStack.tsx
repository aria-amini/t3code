...
export function ComposerBannerStack({
	...
	const [requestedExitingItemId, setExitingItemId] = useState<string | null>(
		null,
	);
	const dismissTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
	const exitingItemId =
		requestedExitingItemId !== null &&
		items.some((item) => item.id === requestedExitingItemId)
	...
	useLayoutEffect(() => {
		if (stackExpanded && pendingFocusRef.current === "notice") {
			...
			(firstControl ?? expandedItemsRef.current)?.focus({
				preventScroll: true,
			});
	...
	const orderedItems = items.toSorted(
		(a, b) => bannerPriority(a) - bannerPriority(b),
	);
...
function ComposerBannerStackAlert({
	...
	return (
		...
		>
			...
			{item.children ? (
				<ComposerBanner.Children>{item.children}</ComposerBanner.Children>
			) : null}
		</ComposerBanner.Root>
	);
}
