...
if (!usage) {
	throw new Error(
		"The context window test fixture did not produce a snapshot.",
	);
}
...
describe("ContextWindowMeter", () => {
	it("keeps the hover popover open while the pointer moves to the compact button", () => {
		const markup = renderToStaticMarkup(
			<ContextWindowMeter usage={usage} onCompact={() => {}} />,
		);
	...
	it("explains why the compact action is disabled", () => {
		...
		expect(markup).not.toContain(
			'aria-label="Send or clear your draft before compacting"',
		);
	});
});
