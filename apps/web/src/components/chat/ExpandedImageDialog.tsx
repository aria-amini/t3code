...
export const ExpandedImageDialog = memo(function ExpandedImageDialog({
	...
	const [accessibilityDetailsSrc, setAccessibilityDetailsSrc] = useState<
		string | null
	>(null);
	const index =
		(preview.index + imageOffset + preview.images.length) %
	...
	const navigateImage = useCallback(
		(direction: -1 | 1) => {
			setImageOffset(
				(current) =>
					(current + direction + preview.images.length) % preview.images.length,
			);
		},
		[preview.images.length],
	);
	...
	const openOriginalLink =
		item.originalUrl &&
		resolveExternalWebLinkHost(item.originalUrl) !== null ? (
			<OpenMediaLink originalUrl={item.originalUrl} />
		) : null;
	const accessibilityDetails = item.source
		? snapShotAccessibilityDetails(item.source)
		: undefined;
								...
								<TooltipTrigger
									render={
										<Button
											...
											onClick={() =>
												setAccessibilityDetailsSrc(
													showingAccessibilityDetails ? null : item.src,
												)
											}
											size="icon-micro"
											variant="ghost-muted"
