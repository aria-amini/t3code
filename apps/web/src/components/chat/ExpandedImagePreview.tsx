...
export async function resolveMarkdownMediaPreview(input: {
	...
	const relativePath =
		reference?.kind === "file" ? reference.relativePath : undefined;
	...
	if (media.access === "direct") {
		...
		if (assetUrl === null)
			throw new Error("The environment returned an invalid media URL.");
		...
		images: [
			{
				...
				...(media.access === "direct" &&
				resolveExternalWebLinkHost(media.uri) !== null
...
export function attachVideoThumbnail(
...
export function buildExpandedImagePreview(
	...
	if (
		...
		return {
			images: [
				{
					src: URL.createObjectURL(selected.file),
					name: selected.name,
					type: "video",
				},
			],
