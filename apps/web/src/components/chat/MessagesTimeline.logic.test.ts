...
describe("streaming row projection", () => {
	...
	it.each([
		...
		"keeps row parity and history identity when text changes from %j to %j",
		(before, after) => {
	...
	it.each(["file", "image", "handoff", "streaming-image"] as const)(
		...
		(kind) => {
			...
			const source = initial.messages.map((message, index) =>
				index === 0 ||
				(kind === "streaming-image" && index === initial.messages.length - 1)
					? {
							...message,
			...
			const materialize = (
				...
				return messages.map((message) => {
					const displayed = server(message, (attachment) =>
						urls.get(attachment.id),
					);
			...
			const firstTimeline = deriveTimelineEntriesWithState(
				firstMessages,
				[],
				initial.work,
			);
			...
			if (kind === "image" || kind === "streaming-image") {
				...
				const renewedRows = deriveMessagesTimelineRowsWithState(
					renewedInput,
					next,
				);
				expect(renewedRows.rows).toEqual(
					deriveMessagesTimelineRows(renewedInput),
				);
	...
	it("owns checkpoint lookups across streaming and equal source snapshots", () => {
		...
		expect(
			previous.rows.some(
				(row) => row.kind === "message" && row.revertTurnCount === 0,
			),
		).toBe(true);
		const last = initial.messages.at(-1)!;
		const messages = [
			...initial.messages.slice(0, -1),
			{ ...last, text: "Partial token" },
		];
		const timeline = deriveTimelineEntriesWithState(
			messages,
			[],
		...
		expect(
			changed.rows.find(
				(row) => row.kind === "message" && row.message.id === messages[0]?.id,
			),
	...
	it("reuses long-thread rows through detail events, selectors, and attachment previews", () => {
		...
		const history = Array.from({ length: 250 }, (_, index) => {
			...
			const assistant = {
				...
				turnId,
				createdAt: new Date(
					Date.UTC(2026, 8, 3, 0, 0, index * 5 + 3),
				).toISOString(),
			};
		...
		const send = (text: string, sequence: number, streaming = true) => {
			...
			if (result.kind !== "updated")
				throw new Error("Message event did not update the thread");
		...
		try {
			...
			expect(completed.rows).toEqual(
				deriveMessagesTimelineRows(completed.input),
			);
			expect(
				completed.rows.find(
					(row) => row.kind === "message" && row.message.id === liveMessage.id,
				),
			).toMatchObject({
...
describe("expanded tool group scrolling", () => {
	...
	it("follows appended calls only at the hard end", () => {
		...
		expect(shouldFollowWorkGroupAppend(entries, appended, Infinity)).toBe(
			false,
		);
	});
	...
	it("does not follow output updates, prepends, or replacements", () => {
		...
		expect(
			shouldFollowWorkGroupAppend(entries, [{ id: "older" }, ...entries], 0),
		).toBe(false);
		expect(
			shouldFollowWorkGroupAppend(
				entries,
	...
	it("restores the visible tool and its offset inside expanded output", () => {
		...
		expect(
			resolveWorkGroupScrollIndex([{ id: "older" }, ...entries], anchor),
		).toEqual({
			index: 2,
			viewOffset: -120,
		});
...
describe("work entry labels", () => {
	...
	it.each([
		...
		"uses the same friendly %s label in both views",
	...
	it("keeps custom titles and output for unrecognized tools", () => {
		...
		expect(
			workEntryDisplayLabel(
				{ ...unknownEntry, detail: "Found 3 issues" },
				undefined,
			),
		).toBe("Found 3 issues");
	});
...
describe("shouldPreserveAssistantLineBreaks", () => {
	it("preserves Claude insight formatting without changing regular markdown", () => {
		...
		expect(
			shouldPreserveAssistantLineBreaks("A normal\\nmarkdown paragraph"),
		).toBe(false);
	});
});
...
describe("normalizeCompactToolLabel", () => {
	it("removes trailing completion wording from command labels", () => {
		expect(normalizeCompactToolLabel("Ran command complete")).toBe(
			"Ran command",
		);
	});
...
describe("deriveMessagesTimelineRows", () => {
	...
	it("projects assistant diff summaries and user revert counts onto the affected rows", () => {
		const assistantTurnDiffSummary = {
			...
			files: [
				{ path: "src/index.ts", kind: "modified", additions: 3, deletions: 1 },
			],
		};
		...
		expect(assistantRow?.assistantTurnDiffSummary).toBe(
	...
	it("folds the first assistant message and settled work before the terminal response", () => {
		...
		expect(
			expandedRows.find(
				(row) => row.kind === "turn-fold" && row.expanded === true,
			),
		).toBeDefined();
	});
	...
	it("keeps the previous turn folded while a newly sent message awaits its turn", () => {
		...
		expect(finalRow?.kind === "message" && finalRow.showAssistantMeta).toBe(
			true,
		);
	...
	it("keeps a promptless restart in one active visual response", () => {
		...
		expect(rows.some((row) => row.kind === "turn-fold")).toBe(false);
		expect(
			rows.filter((row) => row.id === "working-indicator-row"),
		).toHaveLength(1);
		expect(
			rows.findIndex((row) => row.id === "working-indicator-row"),
		).toBeLessThan(rows.findIndex((row) => row.id === "old-work-entry"));
		expect(
			rows.find((row) => row.id === "working-indicator-row"),
		).toMatchObject({
			createdAt: "2026-01-01T00:00:00Z",
		});
		...
		expect(
			rows.filter((row) => row.kind === "work-live" && row.active),
		).toEqual([
			expect.objectContaining({
	...
	it("keeps separated in-progress tool runs visible", () => {
		...
		expect(
			rows.filter((row) => row.kind === "work-live").map((row) => row.entry.id),
	...
	it("reuses one activity row for initial thinking and the latest tool", () => {
		...
		const initialActivityRow = initialRows.find(
			(row) => row.id === "live-activity-row",
		);
		const runningActivityRow = runningRows.find(
			(row) => row.id === "live-activity-row",
		);
		const completedActivityRow = completedRows.find(
			(row) => row.id === "live-activity-row",
		);
		...
		expect(
			initialRows.filter((row) => row.id === "live-activity-row"),
		).toHaveLength(1);
		expect(
			runningRows.filter((row) => row.id === "live-activity-row"),
		).toHaveLength(1);
		expect(
			completedRows.filter((row) => row.id === "live-activity-row"),
		).toHaveLength(1);
		expect(
			failedRows.filter((row) => row.id === "live-activity-row"),
		).toHaveLength(1);
		expect(
			declinedRows.filter((row) => row.id === "live-activity-row"),
		).toHaveLength(1);
	});
	...
	it("does not fold the session's running turn when latestTurn regresses", () => {
		...
		expect(
			rows.filter((row) => row.kind === "turn-fold").map((row) => row.turnId),
		).toEqual(["turn-1"]);
	...
	it("only shows assistant metadata on the terminal assistant message", () => {
		...
		expect(assistantRows.map((row) => row.showAssistantMeta)).toEqual([
			false,
			true,
		]);
	});
	...
	it.each([
		...
		"expands %s through the same activity group",
		(_, middleTone, summary) => {
			const timelineEntries = [
				{
					id: "work-entry-1",
					kind: "work" as const,
			...
			expect(
				collapsedRows.find((row) => row.kind === "work-toggle"),
			).toMatchObject({
			...
			expect(
				expandedRows.find((row) => row.kind === "work-toggle"),
			).toMatchObject({
				expanded: true,
			});
		},
	...
	it.each([true, false])(
		...
		(isWorking) => {
			...
			expect(
				deriveMessagesTimelineRows(input).some((row) => row.kind === "work"),
			).toBe(false);
		},
	);
	...
	it.each([
		...
		"uses the final call for %s tool groups",
...
describe("computeStableMessagesTimelineRows", () => {
	...
	it("returns a new result when row order changes without content changes", () => {
		...
		const reordered = computeStableMessagesTimelineRows(
			[firstRows[1]!, firstRows[0]!],
			initial,
		);
