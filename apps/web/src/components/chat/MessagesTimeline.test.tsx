		...
		return (
			<div
				...
				data-maintain-scroll-at-end={
					props.maintainScrollAtEnd ? "enabled" : undefined
				}
				...
				data-maintain-visible-content-position-restore={
					typeof props.maintainVisibleContentPosition === "object"
						? Boolean(
								props.maintainVisibleContentPosition.shouldRestorePosition,
							)
						: undefined
				}
			>
...
describe("MessagesTimeline", () => {
	...
	it("anchors the first user message using its measured height", () => {
		...
		const firstEntry = buildSnapShotTimelineEntry(
		...
		expect(markup).toContain(
			'data-maintain-visible-content-position-data="true"',
		);
		expect(markup).toContain(
			'data-maintain-visible-content-position-size="true"',
		);
		expect(markup).toContain(
			'data-maintain-visible-content-position-restore="true"',
		);
	...
	it("does not render window details before the preview URL resolves", () => {
		const markup = renderToStaticMarkup(
			<MessagesTimeline
				{...buildProps()}
				timelineEntries={[buildSnapShotTimelineEntry()]}
			/>,
		);
	...
	it("renders collapse controls for long user messages", () => {
		...
		expect(markup).toContain(
			'data-maintain-scroll-at-end-footer-layout="false"',
		);
	...
	it("preserves arbitrary XML-like tags and comparisons in rendered user messages", async () => {
		...
		expect(markup).toContain(
			"&lt;global-agent-instructions scope=&quot;workspace&quot;&gt;",
		);
		expect(markup).toContain(
			"Before &lt;nested data-value=&quot;a&amp;b&quot;&gt;inside&lt;/nested&gt; after",
		);
	...
	it("renders unsafe user HTML as inert source text", async () => {
		...
		expect(markup).toContain(
			"&lt;script&gt;globalThis.__t3Xss = 1&lt;/script&gt;",
		);
		expect(markup).toContain(
	...
	it("renders trailing tool calls as part of the terminal assistant block", () => {
		...
		const messageIndex = markup.indexOf(
			'data-timeline-row-id="assistant-entry"',
		);
		const toolIndex = markup.indexOf(
			'data-timeline-row-id="trailing-work-entry"',
		);
	...
	it("keeps failed lifecycle entries discoverable in mixed activity summaries", () => {
		...
		expect(markup).toContain(
			'aria-label="Received 1 update and used 1 tool, tool call failed"',
		);
