...
function TimelineLoadEarlierHeader({
	...
	return (
		<div
			className={
				fade
					? "pt-[var(--workspace-titlebar-scroll-fade-height)]"
					: "pt-3 sm:pt-4"
			}
		>
...
export const MessagesTimeline = memo(function MessagesTimeline({
	...
	const citationThreadRef = useMemo(
		() => parseScopedThreadKey(routeThreadKey),
		[routeThreadKey],
	);
	...
	const [expandedWorkGroupIds, setExpandedWorkGroupIds] = useState<
		ReadonlySet<string>
	>(new Set());
	...
	const reportContentOverflow = useCallback(() => {
		if (!onContentOverflowChange || contentOverflowFrameRef.current !== null)
			return;
			...
			<TimelineRowActivityCtx value={activityState}>
				...
				>
					...
					<LegendList<MessagesTimelineRow>
						...
						{...(readyCitationRequest
							? { dataVersion: readyCitationRequest.key }
							: {})}
						...
						contentInsetEndAdjustment={
							anchoredEndSpace ? contentInsetEndAdjustment : 0
						}
						maintainScrollAtEnd={
							citationPositioning ||
							anchoredEndSpace ||
...
function resolveTimelineRowHeight(
...
const TimelineRowContent = memo(function TimelineRowContent({
	...
	return (
		...
		>
			...
			{row.kind === "work-toggle" ? (
				<WorkGroupToggleTimelineRow row={row} />
			) : null}
			{row.kind === "turn-fold" ? <TurnFoldTimelineRow row={row} /> : null}
			{row.kind === "context-compaction" ? (
				<ContextCompactionTimelineRow row={row} />
			) : null}
			{row.kind === "message" && row.message.role === "user" ? (
			...
			{row.kind === "assistant-meta" ? (
				<AssistantMetaTimelineRow row={row} />
			) : null}
...
function UserVideoAttachment({ file }: { readonly file: ChatFileAttachment }) {
	...
	const src =
		assetUrl._tag === "Success" ? assetUrl.url : (file.previewUrl ?? null);
	...
	return (
		<MediaVideoPlayer
			...
			actionsSource={
				asset ? { kind: "video", name: file.name, src, asset } : undefined
			}
		/>
	);
}
...
function UserTimelineRow({
	...
	const messageWithPreviews = useMemo(() => {
		...
		return projectPreviews(row.message, (attachment) =>
			urlsById.get(attachment.id),
		);
	...
	const userImages = (messageWithPreviews.attachments ?? []).filter(
		isImageAttachment,
	);
	...
	const unknownAttachments = (row.message.attachments ?? []).filter(
		(attachment) =>
			!isImageAttachment(attachment) && !isFileAttachment(attachment),
	);
...
const WorkGroupSection = memo(function WorkGroupSection({
	...
	const { workspaceRoot, routeThreadKey, onToggleWorkEntry } =
		use(TimelineRowCtx);
...
function ExpandedWorkGroupEntries({
	...
	const { workGroupViewState: viewState, onToggleWorkEntry } =
		use(TimelineRowCtx);
	const [initialScrollIndex] = useState(() =>
		resolveWorkGroupScrollIndex(
			entries,
			viewState.scrollPositions.get(anchorKey),
		),
	);
	...
	const groupView = useMemo(
		() => ({
			state: viewState,
			onToggleEntry: (collapsed: boolean) =>
				onToggleWorkEntry(anchorKey, collapsed),
		}),
...
function LiveActivityContent({
	...
	const showTrailingFailureMark =
		failed &&
		iconName !== undefined &&
		!toolIconAcceptsTint(iconName, toolIcon);
	...
	return (
		...
		>
			...
			{showTrailingFailureMark ? (
				<XIcon
					aria-hidden
					className={cn("size-3 shrink-0", failedToolIconClassName)}
				/>
			) : null}
		</span>
	);
...
function LiveWorkEntryTimelineRow({
	row,
}: {
...
function WorkGroupToggleTimelineRow({
	...
	return (
		<button
			...
			aria-label={
				row.hasFailure ? `${row.summary}, tool call failed` : undefined
			}
			aria-expanded={row.expanded}
			onClick={() => ctx.onToggleWorkGroup(row.groupId, row.id)}
		>
...
const UserMessageElementContextChip = memo(
	function UserMessageElementContextChip(props: {
		...
		return (
			<Tooltip>
				...
				<TooltipPopup
					side="top"
					className="max-w-96 whitespace-pre-wrap leading-tight"
				>
					{tooltipText}
				</TooltipPopup>
			</Tooltip>
...
const UserMessageBody = memo(function UserMessageBody(props: {
	...
	const renderInlineMarkdownSegment = (text: string, key: string) => {
		...
		const trailingWhitespace =
			/\s+$/.exec(textWithoutLeadingWhitespace)?.[0] ?? "";
		...
		return (
			<Fragment key={key}>
				{leadingWhitespace ? (
					<span aria-hidden="true">{leadingWhitespace}</span>
				) : null}
				{content ? (
					<ChatMarkdown
						text={content}
				...
				{trailingWhitespace ? (
					<span aria-hidden="true">{trailingWhitespace}</span>
				) : null}
			</Fragment>
		);
	};
	...
	if (
		reviewCommentSegments.some((segment) => segment.kind === "review-comment")
	) {
		return (
...
function BrowserAppIcon({ className }: { className: string }) {
	return (
		...
		>
			...
			<path
				d="m12.4 11.4 7.5 2.6-3.4 1.6-1.5 3.6z"
				fill="currentColor"
				stroke="none"
			/>
		</svg>
...
function ToolActivityIconView(props: {
	...
	const fallbackClassName = cn(
		props.className,
		props.muted && "opacity-70 light:brightness-[.6]",
	);
	if (!props.icon) {
		return (
...
function ToolActivityImageIcon(props: {
	...
	const handleLoadError = (failedSrc: string) => {
		...
		setDisplayedSrc((currentSrc) =>
			currentSrc === failedSrc ? null : currentSrc,
		);
	};
	return (
		<>
				...
				>
					<img
						...
						className={cn(
							...
							props.muted && "light:brightness-[.6]",
						)}
...
const AgentSpawnCtaRow = memo(function AgentSpawnCtaRow(props: {
	...
	const workflowGroup = spawn.workflowId
		? agentPanelModel.workflows.find(
				(group) => group.workflow.id === spawn.workflowId,
			)
		: undefined;
	...
	const livePhase = workflowGroup?.phases.find(
		(phase) => phase.state === "running",
	);
	const workflowName =
		workflowGroup?.workflow.workflowName ??
		workflowGroup?.workflow.title ??
		null;
...
const SimpleWorkEntryRow = memo(function SimpleWorkEntryRow(props: {
	...
	const { workEntry, workspaceRoot, isExpandedToolGroupEntry, displayLabel } =
		props;
...
const PlainWorkEntryRow = memo(function PlainWorkEntryRow(props: {
	...
	const showDestructiveRowStyle =
		showFailedIndicator &&
		(workEntrySignalsSevereFailure(workEntry) ||
	...
	const viewedImage =
		viewedImagePath && threadRef
			...
			: null;
	const commandMatchesVisibleLabel =
