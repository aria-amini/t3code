...
export const ModelPickerSidebar = memo(function ModelPickerSidebar(props: {
	...
	const [hoveredInstanceId, setHoveredInstanceId] =
		useState<ProviderInstanceId | null>(null);
	const sidebarContentRef = useRef<HTMLDivElement>(null);
	const [selectedIndicatorTop, setSelectedIndicatorTop] = useState<
		number | null
	>(null);
	useLayoutEffect(() => {
		...
		const selectedItem = Array.from(
			...
			(item) => item.dataset.modelPickerProvider === props.selectedInstanceId,
		);
		if (!selectedItem) {
		...
		setSelectedIndicatorTop(
			selectedItem.offsetTop + selectedItem.offsetHeight / 2 - 10,
		);
		...
		>
			<div className="h-full overflow-y-auto overscroll-contain [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
				<div
					...
					className="relative flex min-h-full flex-col gap-1 p-1"
				>
					...
					{props.instanceEntries.map((entry) => {
						...
						const isContextDisabled =
							props.disabledInstanceIds?.has(entry.instanceId) ?? false;
						const unavailableSelectionIsReachable =
							props.selectableUnavailableInstanceIds?.has(entry.instanceId) ??
							false;
						const isDisabled =
							(isUnavailable && !unavailableSelectionIsReachable) ||
							isContextDisabled;
						...
						const tooltip = isUnavailable
							...
							: isContextDisabled
								? (props.getDisabledInstanceTooltip?.(entry) ??
									entry.displayName)
								: showNewBadge
