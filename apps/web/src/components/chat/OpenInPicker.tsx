...
export const OpenInPicker = memo(function OpenInPicker({
	...
	const environmentLabel =
		useEnvironment(environmentId)?.label ?? "this machine";
	...
	const [preferredEditor, setPreferredEditor] =
	...
	const primaryOption =
		options.find(({ value }) => value === preferredEditor) ?? null;
	...
	return (
		<Group aria-label="Open in editor">
			<Button
				...
				variant="outline"
				disabled={
					!preferredEditor || !openInCwd || remote.mode === "remote-unavailable"
				}
						...
						<>
							...
							{options.map(({ label, Icon, value, kind }) => (
								<MenuItem key={value} onClick={() => openInEditor(value)}>
									<Icon
										aria-hidden="true"
										className={getOpenInIconClass(kind)}
									/>
									{label}
