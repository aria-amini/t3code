...
describe("ProviderStatusBanner", () => {
	...
	it("shows Antigravity installation and startup failures before auth is checked", () => {
		...
		expect(
			shouldShowProviderStatusBanner({ ...status, installed: false }, null),
		).toBe(true);
		expect(
			shouldShowProviderStatusBanner({ ...status, status: "error" }, null),
		).toBe(true);
		expect(
			shouldShowProviderStatusBanner(
	...
	it("stays hidden after its current warning is dismissed", () => {
		...
		expect(
			shouldShowProviderStatusBanner(
				status,
				getProviderStatusBannerKey(status),
			),
		).toBe(false);
	});
...
describe("getProviderStatusMessage", () => {
	it("preserves the environment's authentication error", () => {
		const message =
			"SUBSCRIPTION_REQUIRED: This Google account cannot use Antigravity.";
		expect(
			getProviderStatusMessage({
				...warningProvider(),
				driver: ProviderDriverKind.make("antigravity"),
