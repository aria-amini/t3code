...
describe("ThreadErrorBanner", () => {
	...
	it("reappears when a new error arrives on the same thread", () => {
		dismissThreadErrorBannerForSession(
			getThreadErrorBannerKey("env:thread-b", "Turn failed"),
		);
		const newErrorKey = getThreadErrorBannerKey(
			"env:thread-b",
			"Provider crashed",
		);
	...
	it("scopes dismissals to the thread that dismissed them", () => {
		dismissThreadErrorBannerForSession(
			getThreadErrorBannerKey("env:thread-c", "Aborted"),
		);
		const otherThreadKey = getThreadErrorBannerKey(
			"env:other-thread",
			"Aborted",
		);
	...
	it("keeps a dismissal across visiting threads with no error", () => {
		...
		expect(shouldShowThreadErrorBanner("env:thread-d", null, false)).toBe(
			false,
		);
	...
	it("never shows a null error", () => {
		expect(shouldShowThreadErrorBanner("env:thread-e", null, false)).toBe(
			false,
		);
	});
