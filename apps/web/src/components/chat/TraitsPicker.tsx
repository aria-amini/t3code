						...
						<MenuGroup>
							...
							descriptor.id === primarySelectDescriptor?.id ? (
								<div className="px-2 pb-1.5 text-muted-foreground/80 text-xs">
									...
									Remove it to change this option.
								</div>
							) : null}
							<MenuRadioGroup
								value={selectedValue}
...
export function buildTraitsTriggerDisplay(input: {
	...
	for (const descriptor of input.descriptors) {
		...
		if (
			...
			if (
				fastTier &&
				(currentValue === "default" || currentValue === fastTier.id)
			) {
		...
		const label =
			input.ultrathinkPromptControlled &&
			descriptor.id === input.primarySelectDescriptorId
				? "Ultrathink"
				: descriptor.type === "boolean"
