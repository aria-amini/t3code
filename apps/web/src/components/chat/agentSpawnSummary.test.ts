...
const batch = (status: RuntimeSubagent["status"]) => ({
	kind: "subagent_batch" as const,
	status,
});
const agent = (status: RuntimeSubagent["status"]) => ({
...
describe("deriveAgentSpawnSummary", () => {
	it("counts a native batch without claiming the number of children", () => {
		expect(
			deriveAgentSpawnSummary({ agents: [batch("running")], agentCount: 1 }),
		).toEqual({
			live: true,
		...
		expect(
			deriveAgentSpawnSummary({ agents: [batch("idle")], agentCount: 1 }),
		).toEqual({
			live: false,
			lead: "Launched 1 subagent batch",
			status: "1 idle",
	...
	it.each([
		...
		"reports %s accurately alongside a completed agent",
		(state, status, tone) => {
			expect(
				deriveAgentSpawnSummary({
					agents: [agent("completed"), agent(state)],
					agentCount: 2,
				}),
	...
	it("does not claim completion when the roster is missing a member", () => {
		expect(
			deriveAgentSpawnSummary({ agents: [agent("completed")], agentCount: 2 }),
		).toMatchObject({
