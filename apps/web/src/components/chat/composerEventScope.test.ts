...
class FakeElement {
	...
	closest(selector: string): FakeElement | null {
		return this.matchingSelector !== null &&
			selector
				.split(",")
				.some((candidate) => candidate === this.matchingSelector)
			? this
			: null;
	}
}
...
describe("composer event scopes", () => {
	it("recognizes events from the portaled resting controls", () => {
		...
		expect(
			isInsideRestingComposerControlScope(target as unknown as EventTarget),
		).toBe(true);
	});
	...
	it("includes composer-owned floating layers in the resting control scope", () => {
		...
		expect(
			isInsideComposerFloatingLayer(target as unknown as EventTarget),
		).toBe(true);
		expect(
			isInsideRestingComposerControlScope(target as unknown as EventTarget),
		).toBe(true);
	...
	it("recognizes banner and drawer controls docked above the surface", () => {
		...
		expect(
			isInsideCollapsedComposerControls(target as unknown as EventTarget),
		).toBe(true);
		expect(
			isInsideCollapsedComposerControls(
				new FakeElement(null) as unknown as EventTarget,
			),
		).toBe(false);
