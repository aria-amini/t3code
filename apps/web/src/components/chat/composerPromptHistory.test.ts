...
function backward(
	...
	return stepComposerPromptHistory({
		direction: "backward",
		entries,
		position,
		currentPrompt,
...
describe("recallableComposerPrompt", () => {
	...
	it("removes inline terminal labels along with their trailing block", () => {
		...
		const typed = materializeInlineTerminalContextPrompt(
			"Look at \uFFFC please",
			[context],
		);
	...
	it("removes one label per chip and leaves other whitespace alone", () => {
		...
		const sent = appendTerminalContextsToPrompt(typed, [context]);
		expect(recallableComposerPrompt(sent)).toBe(
	...
	it("strips only the review comments appended at the end", () => {
		...
		const midPrompt =
			appendReviewCommentsToPrompt("Before", [comment]) + "\n\nAfter";
	...
	it("returns an empty string for app-composed sends", () => {
		...
		expect(
			recallableComposerPrompt(
				buildPlanImplementationPrompt("# Plan\n1. do it"),
			),
		).toBe("");
	});
});
