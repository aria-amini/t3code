...
export function getComposerPromptInjectionState(
	prompt: string,
...
export function withImplicitFastModeDefault(
	...
	const hasExplicitFastMode = modelOptions?.some(
		(selection) => selection.id === "fastMode",
	);
...
export function getComposerProviderState(
	...
	if (provider === "opencode") {
		...
		const modelIsInCatalog = models.some(
			(candidate) => candidate.slug === normalizedModel,
		);
		if (!modelIsInCatalog) {
			const preservedOptions = modelOptions?.filter(
				(option) =>
					planModeEnabled || option.id !== "agent" || option.value !== "plan",
			);
			return {
				provider,
				promptEffort: null,
				modelOptionsForDispatch:
					preservedOptions && preservedOptions.length > 0
						? preservedOptions
						: undefined,
			};
		}
	...
	const primaryValue = getProviderOptionCurrentValue(
