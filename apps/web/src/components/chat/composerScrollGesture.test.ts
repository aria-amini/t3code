...
describe("composer keyboard scroll collapse", () => {
	...
	it.each(["PageUp", "PageDown", "Home", "End"])(
		"collapses on %s in the timeline",
		(key) => {
	...
	it.each(["PageUp", "Home"])("does not collapse on %s at the top", (key) => {
		expect(
			shouldCollapseComposerForScrollKey({ ...middle, key, scrollTop: 0 }),
		).toBe(false);
	});

	it.each(["PageDown", "End"])(
		"does not collapse on %s at the logical end",
		(key) => {
			expect(
				shouldCollapseComposerForScrollKey({
					...middle,
			...
			expect(
				shouldCollapseComposerForScrollKey({ ...middle, key, scrollTop: 1000 }),
			).toBe(false);
		},
	);
	...
	it("ignores unrelated keys", () => {
		expect(
			shouldCollapseComposerForScrollKey({ ...middle, key: "Enter" }),
		).toBe(false);
	});
});
...
describe("composer scroll gesture", () => {
	...
	it("keeps a boundary momentum tail in the same suppressed gesture", () => {
		...
		expect(record(state, 80, { canScrollInGestureDirection: false })).toBe(
			false,
		);
	...
	it("accumulates small deltas only while collapse remains eligible and scrollable", () => {
		...
		expect(record(state, 60, { deltaPx: 10, collapseEligible: false })).toBe(
			false,
		);
