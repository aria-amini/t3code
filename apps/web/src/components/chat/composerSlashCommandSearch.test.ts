...
describe("searchSlashCommandItems", () => {
	...
	it("moves exact provider command matches ahead of broader description matches", () => {
		...
		expect(searchSlashCommandItems(items, "ui").map((item) => item.id)).toEqual(
			["provider-slash-command:claudeAgent:ui", "slash:default"],
		);
	});
	...
	it("supports fuzzy provider command matches", () => {
		...
		expect(
			searchSlashCommandItems(items, "gfc").map((item) => item.id),
		).toEqual(["provider-slash-command:claudeAgent:gh-fix-ci"]);
	});
	...
	it("includes skills by name and description", () => {
		...
		expect(
			searchSlashCommandItems(items, "browser").map((item) => item.id),
		).toEqual(["skill:claudeAgent:browser"]);
		expect(
			searchSlashCommandItems(items, "control").map((item) => item.id),
		).toEqual(["skill:claudeAgent:browser"]);
	});
	...
	it("matches skills by display name", () => {
		...
		expect(
			searchSlashCommandItems(items, "ask matt").map((item) => item.id),
		).toEqual(["skill:claudeAgent:ask-matt"]);
		expect(
			searchSlashCommandItems(items, "/skill:ask-matt").map((item) => item.id),
		).toEqual(["skill:claudeAgent:ask-matt"]);
	});
	...
	it("matches skills by their rendered prefix", () => {
		...
		expect(
			searchSlashCommandItems(items, "/skill:brow").map((item) => item.id),
		).toEqual(["skill:claudeAgent:browser"]);
		expect(
			searchSlashCommandItems(items, "/sk").map((item) => item.id),
		).toEqual(["skill:claudeAgent:browser"]);
	...
	it("keeps skills alongside commands for an empty slash query", () => {
		const items = [
			...
			Extract<ComposerCommandItem, { type: "slash-command" | "skill" }>
		>;
	...
	it("hides provider commands from slash completion after the first message line", () => {
		...
		expect(
			slashCommandItemsForPromptPosition(items, false).map((item) => item.id),
		).toEqual(["slash:model", "skill:claudeAgent:unslop"]);
		expect(
			slashCommandItemsForPromptPosition(items, true).map((item) => item.id),
		).toEqual([
			"slash:model",
