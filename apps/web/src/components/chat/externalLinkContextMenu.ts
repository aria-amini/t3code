...
const FAILURE_OPERATION_BY_ACTION = {
	...
	ExternalLinkContextMenuAction,
	ExternalLinkContextMenuFailureOperation
>;
...
function externalLinkContextMenuItems(options: {
	...
	const items = options.canOpenInPreview
		...
		: EXTERNAL_LINK_CONTEXT_MENU_ITEMS.filter(
				(item) => item.id !== "open-in-preview",
			);
...
export function resolveExternalWebLinkHost(
