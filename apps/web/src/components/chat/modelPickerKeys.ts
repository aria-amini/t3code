...
export function modelPickerModelKey(
...
export function parseModelPickerModelKey(
	...
	if (
		!Number.isSafeInteger(instanceIdLength) ||
		instanceIdLength > value.length
	) {
		return null;
	}
...
export function modelPickerLegacySectionKey(
	instanceId: ProviderInstanceId,
): string {
	return `${LEGACY_SECTION_KEY_PREFIX}${instanceId}`;
}

export function parseModelPickerLegacySectionKey(
	key: string,
