...
export function getTimelinePageScrollKey({
	...
	const editorCanScroll = canScrollInDirection(
...
export function getPageScrollVelocityPxPerMs({
	...
	return (
		(pageScrollDistancePx * getPageScrollMultiplier(holdElapsedMs)) /
...
export function getPageScrollDistancePx({
	...
	return Math.max(
		0,
		containerHeightPx - PAGE_SCROLL_ALIGNMENT_OFFSET_PX - scrollPaddingBottomPx,
	);
}
...
export function createPageScrollController({
	...
	const smoothScrollBy = (container: PageScrollContainer, deltaY: number) => {
		...
		const step = (now: number) => {
			const progress = Math.min(
				1,
				(now - startTime) / PAGE_SCROLL_ANIMATION_MS,
			);
	...
	const startHoldScroll = (
		...
		const step = (now: number) => {
			...
			container.scrollTop =
				previousScrollTop + velocityPxPerMs * deltaMs * getDirection(key);
	...
	return {
		handleKeyDown(key: PageScrollKey) {
			...
			smoothScrollBy(
				container,
				readPageScrollDistance(container) * getDirection(key),
			);
