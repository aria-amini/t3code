...
function measurePicker(input: {
	...
	vi.stubGlobal("getComputedStyle", (element: unknown) => {
		...
		if (element === picker)
			return { minWidth: "52px", maxWidth: input.maxWidth ?? "none" };
...
describe("measureRestingComposerControls", () => {
	it("keeps controls inline when the model label is deliberately collapsed", () => {
		...
		expect(
			resolveRestingComposerControlsLayout({ ...measurement, hostWidth: 200 }),
		).toEqual({
			hiddenCount: 0,
			visible: true,
		});
	...
	it("still caps recovered text at the model picker's maximum width", () => {
		expect(
			measurePicker({ clientWidth: 0, flexGrow: "1", maxWidth: "180px" })
				.naturalFixedWidth,
		).toBe(180);
	});
});
