...
function providerModelPickerNaturalWidth(picker: HTMLElement): number {
	...
	const labelIsCollapsed =
		label?.clientWidth === 0 && getComputedStyle(label).flexGrow === "0";
	const hiddenLabelWidth =
		label && !labelIsCollapsed
...
export function measureRestingComposerControls(
	...
	const picker = controls.querySelector<HTMLElement>(
	...
	const separator = controls.querySelector<HTMLElement>(
		"[data-resting-controls-separator]",
	);
	const separatorWidth = separator ? elementOuterWidth(separator) : 0;
	const overflow = controls.querySelector<HTMLElement>(
		"[data-resting-controls-overflow]",
	);
	const separatorAndGapWidth = separatorWidth > 0 ? separatorWidth + gap : 0;
	const blocks = Array.from(
		controls.querySelectorAll<HTMLElement>("[data-resting-block]"),
	);
	return {
		gap,
		naturalFixedWidth:
			(picker
				...
				: elementOuterWidth(leadingControl)) + separatorAndGapWidth,
		minimumFixedWidth:
			(picker
