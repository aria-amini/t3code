...
export function useAssistantCitationTarget({
	...
	useEffect(() => {
		if (
			...
			navigationRef.current.target.key !== request?.key
		) {
		...
		if (navigation.done || navigation.target.activationRef.current.dismissed)
			return;
		...
		const source = entries.find(
			(entry) =>
				entry.kind === "message" &&
				entry.message.id === navigation.target.citation.messageId,
		);
		if (!source) {
			if (loadEarlier) {
				...
				if (
					navigation.requestedPages.has(cursor) ||
					navigation.requestedPages.size >= 20
				) {
					fail(
		...
		const index = rows.findIndex(
			(row) =>
				row.kind === "message" &&
				row.message.id === navigation.target.citation.messageId,
		);
		if (index < 0) {
	...
	useEffect(() => {
		...
		const dismiss = (onlyPending: boolean) => {
			...
			if (
				...
				(onlyPending && (activation.scrolled || navigation.done))
			)
				return;
	...
	const sourceRow =
		target && positioning
			? rows.find(
					(row) =>
						row.kind === "message" &&
						row.message.id === target.citation.messageId,
				)
			: undefined;
	return {
		target,
