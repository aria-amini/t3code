...
export function useAutoBalanceUpdateBanner(
		...
		() =>
			Atom.make((get) =>
				environments.map((environment) => ({
					environment,
					state: get(
						serverEnvironment.updateStateAtom(environment.environmentId),
					),
				})),
			),
		[environments],
	...
	const [dismissedNotices, setDismissedNotices] = useState<
		ReadonlySet<string | ServerUpdateState>
	>(() => new Set());
	const machines = states.flatMap(({ environment, state }) => {
		...
		if (
			...
			return [];
		const selfUpdate = resolveServerSelfUpdateCapability(
			environment.serverConfig,
		);
	...
	const running = machines.filter(
		(machine) => machine.state.status === "running",
	).length;
	const failed = machines.filter(
		(machine) => machine.state.status === "failed",
	).length;
