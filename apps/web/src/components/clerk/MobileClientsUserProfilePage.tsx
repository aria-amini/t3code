...
function MobileClientRow({
	...
	readonly device: RelayClientDeviceRecord;
}) {
	return (
...
function MobileClientsSkeleton() {
	return (
		<div
			aria-label="Loading mobile clients"
		...
		<Empty className="min-h-64 gap-4 border-t px-6 py-10 md:p-10">
			...
			<EmptyHeader>
				...
				<EmptyDescription className="text-[0.8125rem] leading-[1.125rem]">
					Sign in to T3 Code on your iPhone to register it for push
...
export function MobileClientsUserProfilePage() {
	...
	const isInitialLoad =
		!devicesState.accountId ||
		(devicesState.data === null && !devicesState.error);
	const hasErrorWithoutData =
		devicesState.error !== null && devicesState.data === null;
