...
const linkedAtFormatter = new Intl.DateTimeFormat(undefined, {
	dateStyle: "medium",
});
	...
	return (
		<ClerkUserProfileRow icon={<ServerIcon className="size-4" />}>
			<Collapsible
				...
				onOpenChange={props.onConfirmationChange}
			>
				<div className="flex items-start gap-4">
					<div className="min-w-0 flex-1">
						...
						<p className="mt-1 text-xs leading-[1.125rem] text-muted-foreground">
							{linkedAtLabel(environment.linkedAt)} ·{" "}
							{endpointLabel(environment)}
						</p>
					</div>
					<CollapsibleTrigger
						render={
					...
					<div className="pt-3">
						...
						>
							...
							<p className="mt-4 max-w-xl text-[0.8125rem] leading-[1.125rem] text-muted-foreground">
								T3 Connect access will be revoked, any managed tunnel will be
								removed, and a host space will become available. Local
...
export function T3ConnectUserProfilePage() {
	...
	const deregisterEnvironment = useAtomCommand(
		deregisterManagedRelayEnvironmentCommand,
		{
			reportFailure: false,
		},
	...
	const [confirmingEnvironmentId, setConfirmingEnvironmentId] =
		useState<EnvironmentId | null>(null);
		...
		if (result._tag === "Success") {
			...
			setRemovedEnvironments((current) => {
				const linkedAtById = new Map(
					current.accountId === accountId ? current.linkedAtById : [],
				);
	...
	const environments = (environmentsState.data ?? []).filter(
		(environment) =>
			removedEnvironmentLinkedAt.get(environment.environmentId) !==
			environment.linkedAt,
	);
	const isInitialLoad =
		...
		>
			<div>
				{environmentsState.error ? (
					<div
						className="mb-4 border-t border-destructive/35 py-3 text-[0.8125rem]"
						role="alert"
					>
				...
				{isInitialLoad ? (
					<p
						className="border-t py-4 text-[0.8125rem] text-muted-foreground"
						role="status"
					>
					...
					<Empty className="min-h-64 gap-4 border-t px-6 py-10 md:p-10">
						...
						<EmptyHeader>
							...
							<EmptyDescription className="text-[0.8125rem] leading-[1.125rem]">
								Link an environment from its local Settings to make it available
								through T3 Connect.
							</EmptyDescription>
						</EmptyHeader>
					</Empty>
