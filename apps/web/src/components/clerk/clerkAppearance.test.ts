...
function mixThemeColors(
	...
	const channels = [first, second].map((value) => {
		...
		return [0, 1, 2].map((channel) =>
			Number.parseInt(hex.slice(channel * 2, channel * 2 + 2), 16),
		);
	});
	const mixed = channels[0]!.map((channel, index) =>
		Math.round(
			channel * firstWeight + channels[1]![index]! * (1 - firstWeight),
		),
	);
...
const builtInThemeModes = [
	...
	[theme.colors, theme.variants?.dark].filter(
		(colors): colors is ThemeColors => !!colors,
	),
);
...
describe("clerkAppearance", () => {
	it("maps theme colors without overriding Clerk's component structure", () => {
		expect(clerkAppearance).toEqual({
			variables: {
				...
				colorMuted:
					"color-mix(in srgb, var(--card) 98%, var(--contrast-foreground))",
	...
	it.each(builtInThemeModes)(
		"keeps Clerk text readable across a built-in palette",
		(colors) => {
			...
			expect(
				contrastRatio(colors.mutedForeground, mutedSurface),
			).toBeGreaterThanOrEqual(4.5);
			expect(
				contrastRatio(colors.text, colors.secondary),
			).toBeGreaterThanOrEqual(4.5);
			expect(
				contrastRatio(colors.updateForeground, colors.surface),
			).toBeGreaterThanOrEqual(4.5);
			expect(
				contrastRatio(colors.errorForeground, colors.surface),
			).toBeGreaterThanOrEqual(4.5);
			expect(
				contrastRatio(colors.warningForeground, colors.surface),
			).toBeGreaterThanOrEqual(4.5);
		},
	);
});
