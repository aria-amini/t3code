...
type DiscoveredEnvironments =
	Discovery.RelayEnvironmentDiscoveryState["environments"];
...
vi.mock("~/state/environments", async () => {
	...
	const read = () => {
		if (discovery.state === null)
			throw new Error("Discovery fixture is not initialized");
...
describe("cloud onboarding discovery", () => {
	...
	it("shows a newly linked computer without remounting and stops polling once found", async () => {
		...
		expect(
			renderer!.root.findAllByType("p").map((node) => node.children),
		).toContainEqual(["Work laptop"]);
		expect(renderer!.root.findByType("button").children).toEqual(["Connect"]);
