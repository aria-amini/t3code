...
export function CloudEnvironmentConnectRows({
	...
	const connectEnvironment = async (
		...
		setConnectingEnvironmentIds(
			(current) => new Set([...current, environment.environmentId]),
		);
		...
		console.error("[t3-connect] Could not connect environment", {
			message,
			traceId,
			cause,
		});
	...
	const visibleEnvironments = [
		...environmentsState.environments.values(),
	].filter(
		({ environment }) =>
	...
	useEffect(() => {
		...
		const schedule = () => {
			...
			if (!disposed && visible()) {
				timer = setTimeout(
					() => void refresh(),
					EMPTY_DISCOVERY_REFRESH_INTERVAL_MS,
				);
			}
		};
		const refresh = async () => {
					...
					<Checkbox
						...
						onCheckedChange={async (checked) => {
							...
							if (checked && !savedEnvironment) {
								...
								if (!connected)
									selection.onChange(environment.environmentId, false);
							}
						}}
					/>
