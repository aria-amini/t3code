...
function ConfiguredConnectOnboardingDialog() {
	...
	const showPublishStep =
		canManageRelay && controller.linkState.target !== null;
	...
	useEffect(() => {
		...
		if (
			...
			(!isSignedIn || userId !== requestedAccount)
		) {
			setRequestedAccount(null);
		}
	...
	useEffect(() => {
		if (
			openForAccount === null ||
			prefilledFromLinkStateRef.current ||
			linkStateData === null
		) {
			return;
		}
		...
		if (linkStateData.linked && linkStateData.cloudUserId === openForAccount) {
			setExposeEnvironment(
				linkStateData.managedTunnelActive ?? linkStateData.linked,
			);
		...
		>
			<DialogPopup className="max-w-xl">
				<DialogHeader>
					...
					<DialogDescription>
						Mesh your devices together — publish this environment and connect
						the rest, all in one place.
					</DialogDescription>
					{steps.length > 1 ? (
						<WizardSteps
			...
			<CloudEnvironmentConnectRows
				...
				empty={
					<p className="px-4 py-6 text-center text-sm text-muted-foreground">
						No other environments are published to your account yet. Publish one
						from another device and it will show up here.
					</p>
				}
			/>
		</div>
