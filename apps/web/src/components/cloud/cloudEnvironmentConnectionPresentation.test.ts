...
describe("saved cloud environment connection presentation", () => {
	it("only labels a live connection as connected", () => {
		expect(
			presentSavedCloudEnvironmentConnection(connection("connected")),
		).toEqual({
		...
		expect(
			presentSavedCloudEnvironmentConnection(connection("connecting")),
		).toEqual({
	...
	it("surfaces a failed attempt while the supervisor reconnects", () => {
		expect(
			presentSavedCloudEnvironmentConnection(
				connection(
					"reconnecting",
					"Relay environment endpoint is unavailable.",
				),
			),
		).toEqual({
	...
	it.each([
		[
			"error",
			"Connection failed",
			"Connection failed. Reason: Access denied.",
			"error",
		],
