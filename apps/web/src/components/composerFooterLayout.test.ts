...
describe("shouldUseCompactComposerFooter", () => {
	...
	it("switches to compact mode below the breakpoint", () => {
		expect(
			shouldUseCompactComposerFooter(COMPOSER_FOOTER_COMPACT_BREAKPOINT_PX - 1),
		).toBe(true);
	});

	it("stays expanded at and above the breakpoint", () => {
		expect(
			shouldUseCompactComposerFooter(COMPOSER_FOOTER_COMPACT_BREAKPOINT_PX),
		).toBe(false);
		expect(
			shouldUseCompactComposerFooter(
				COMPOSER_FOOTER_COMPACT_BREAKPOINT_PX + 48,
			),
		).toBe(false);
	});
...
describe("shouldUseRestingComposerLayout", () => {
	...
	it("keeps the composer expanded until the timeline is scrolled", () => {
		expect(
			shouldUseRestingComposerLayout({ ...resting, isScrollCollapsed: false }),
		).toBe(false);
	});

	it("keeps the composer expanded while the timeline fits above it", () => {
		expect(
			shouldUseRestingComposerLayout({ ...resting, timelineOverflows: false }),
		).toBe(false);
	});

	it("keeps new-thread composers expanded", () => {
		expect(
			shouldUseRestingComposerLayout({ ...resting, isExistingThread: false }),
		).toBe(false);
	});

	it("leaves responsive mobile on its existing collapse path", () => {
		expect(
			shouldUseRestingComposerLayout({ ...resting, isMobileViewport: true }),
		).toBe(false);
	});

	it("keeps drawers and composer-owned menus expanded", () => {
		expect(
			shouldUseRestingComposerLayout({ ...resting, hasExpandedChrome: true }),
		).toBe(false);
	});
...
describe("resolveRestingComposerControlsLayout", () => {
	...
	it("shows everything when the host has room", () => {
		// 149 + 60 + 140 + 4 * 2 = 357
		expect(
			resolveRestingComposerControlsLayout({ ...base, hostWidth: 357 }),
		).toEqual({
			hiddenCount: 0,
			visible: true,
		});
	...
	it("moves trailing blocks into the overflow menu until the rest fits", () => {
		// 149 + 60 + 24 + 4 * 2 = 241
		expect(
			resolveRestingComposerControlsLayout({ ...base, hostWidth: 356 }),
		).toEqual({
			hiddenCount: 1,
			visible: true,
		});
		expect(
			resolveRestingComposerControlsLayout({ ...base, hostWidth: 241 }),
		).toEqual({
			hiddenCount: 1,
			visible: true,
		});
		// 149 + 24 + 4 = 177
		expect(
			resolveRestingComposerControlsLayout({ ...base, hostWidth: 240 }),
		).toEqual({
			hiddenCount: 2,
			visible: true,
		});
	...
	it("shrinks the picker after moving every trailing block into overflow", () => {
		expect(
			resolveRestingComposerControlsLayout({ ...base, hostWidth: 176 }),
		).toEqual({
			hiddenCount: 2,
			visible: true,
		});
		// 105 + 24 + 4 = 133
		expect(
			resolveRestingComposerControlsLayout({ ...base, hostWidth: 133 }),
		).toEqual({
			hiddenCount: 2,
			visible: true,
		});
	...
	it("hides the whole cluster below the picker's minimum readable width", () => {
		expect(
			resolveRestingComposerControlsLayout({ ...base, hostWidth: 132 }),
		).toEqual({
			hiddenCount: 2,
			visible: false,
		});
		expect(
			resolveRestingComposerControlsLayout({ ...base, hostWidth: 0 }),
		).toEqual({
			hiddenCount: 2,
			visible: false,
		});
...
describe("resolveScrollToEndClearance", () => {
	it("removes the side tab gap in both composer states while clearing overlapping attachments", () => {
		for (const overlayHeight of [120, 214]) {
			...
			expect(resolveScrollToEndClearance({ ...layout, attachments: [] })).toBe(
				overlayHeight,
			);
			expect(
				resolveScrollToEndClearance({
