...
export function getRestingComposerImagePreviewCounts(imageCount: number): {
	...
	const visibleCount = Math.min(
		imageCount,
		RESTING_COMPOSER_IMAGE_THUMBNAIL_LIMIT,
	);
	return {
		visibleCount,
...
export function resolveComposerTimelineInset(input: {
	...
	return input.isResting
		? Math.max(
				input.currentInset,
				input.overlayHeight + COMPOSER_RESTING_EXPANSION_MIN_PX,
			)
		: input.overlayHeight;
}
...
export function shouldAnimateComposerRestingTransition(input: {
	...
	return (
		input.hasCompletedInitialLayout &&
		(input.stateChanged || input.hasInterruptedAnimation)
	);
}
...
export function shouldUseCompactComposerPrimaryActions(
	...
	return (
		width !== null && width < COMPOSER_FOOTER_WIDE_ACTIONS_COMPACT_BREAKPOINT_PX
	);
}
...
export function resolveRestingComposerControlsLayout(
	...
	if (previous) {
		const previousHiddenCount = Math.min(
			previous.hiddenCount,
			blockWidths.length,
		);
		while (
			hiddenCount < previousHiddenCount &&
			restingComposerControlsWidth(input, hiddenCount) >
				hostWidth - RESTING_CONTROLS_SLACK_PX
		) {
			hiddenCount += 1;
		}
	}
	const minimumWidth = restingComposerControlsWidth(
		input,
		hiddenCount,
		input.minimumFixedWidth,
	);
	const visible =
...
export function resolveScrollToEndClearance(input: {
	...
	for (const attachment of input.attachments) {
		...
		if (
			attachment.left < input.button.right &&
			attachment.right > input.button.left
		) {
