...
const projectRef = scopeProjectRef(
	environmentId,
	ProjectId.make("snap-shot-project"),
);
...
describe("window capture failures", () => {
	it("dismisses an older failed capture without disturbing a newer capture", async () => {
		...
		await beginSnapShotAnimationWhenReady(
			"older",
		...
		expect(getPendingSnapShotAnimations().map(({ id }) => id)).toEqual([
			"newer",
		]);
	...
	it("does not resurrect a failed capture when its draft becomes ready later", async () => {
		...
		const pendingStarts = new Set<string>();
		const soundedIds = new Set(["older", "newer"]);
		const olderStart = beginSnapShotAnimationWhenReady(
			"older",
			targetReady,
			pendingStarts,
		);
	...
	it("keeps global failures dismissing all active and pending captures", async () => {
		...
		const newerStart = beginSnapShotAnimationWhenReady(
			"newer",
			targetReady,
			pendingStarts,
		);
		...
		async ({ target, accessibleText }) => {
			...
			if (typeof target === "string") {
				useComposerDraftStore
					.getState()
					.setProjectDraftThreadId(projectRef, target, {
...
describe("window capture target resolution", () => {
	it("shares bare-route draft creation between animation start and capture drain", async () => {
		...
		const resolutionRef: { current: Promise<DraftId | null> | null } = {
			current: null,
		};

		const animationTarget = resolveSnapShotTargetOnce(
			resolutionRef,
			resolveTarget,
		);
		const attachmentTarget = resolveSnapShotTargetOnce(
			resolutionRef,
			resolveTarget,
		);
	...
	it("follows a draft to its promoted server thread", () => {
		...
		const promotedRef = scopeThreadRef(
			environmentId,
			ThreadId.make("snap-shot-promoted-thread"),
		);
	...
	it("keeps the routed server thread before its shell loads", () => {
		const routeThreadRef = scopeThreadRef(
			environmentId,
			ThreadId.make("snap-shot-routed-thread"),
		);

		expect(
			resolveExistingSnapShotTarget(routeThreadRef, routeThreadRef),
		).toEqual(routeThreadRef);
	});
});
			...
			try {
				if (staged) {
					...
					store.addImage(target, {
						...
						file: new File([new Uint8Array([1, 2, 3])], capture.name, {
				...
				expect(
					useComposerDraftStore.getState().getComposerDraft(target)?.images,
				).toHaveLength(1);
				expect(
					useComposerDraftStore.getState().getComposerDraft(target)
...
describe("snapshot destination ownership", () => {
	it.each(["unmount", "blur", "disabled animations"])(
		...
		async (reason) => {
			...
			const requested = resolveSnapShotDeliveryTarget(
				targets,
				"capture",
				resolveTarget,
			);
			if (reason !== "disabled animations") {
				...
				if (reason === "unmount") {
					const unmount = scheduleSnapShotAnimationDestination(
						"capture",
						() => undefined,
					);
					unmount();
			...
			expect(getPendingSnapShotAnimations()).toHaveLength(0);
			expect(
				await resolveSnapShotDeliveryTarget(targets, "capture", resolveTarget),
			).toEqual(original);
			expect(
				await resolveSnapShotDeliveryTarget(
					targets,
					"later-capture",
					resolveTarget,
				),
