...
export async function deliverSnapShot(
	...
	const alreadyAttached =
		store
			...
			?.images.some(({ id }) => id === capture.id) ?? false;
	if (
		!alreadyAttached &&
	...
	const persistedAttachments =
		store
			...
			?.persistedAttachments.filter(
				(attachment) => attachment.id !== capture.id,
			) ?? [];
	await store.syncPersistedAttachments(target, [
	...
	if (
		getPendingSnapShotAnimations().some(
			(animation) => animation.id === capture.id,
		)
	) {
		await afterNextPaint();
		await waitForSnapShotAnimationDestination(capture.id).catch(
			() => undefined,
		);
...
export function SnapShotCoordinator() {
	...
	const animateCaptures = useClientSettings(
		(settings) => settings.snapShotAnimations,
	);
	const captureTargetsRef = useRef(
		new Map<string, Promise<CaptureTarget | null>>(),
	);
	const lastTargetRef = useRef<CaptureTarget | null>(null);
	const targetResolutionRef = useRef<Promise<CaptureTarget | null> | null>(
		null,
	);
	...
	const resolveTarget = useCallback(async (): Promise<CaptureTarget | null> => {
		...
		if (lastTarget) {
			const existingTarget = resolveExistingSnapShotTarget(
				lastTarget,
				routeThreadRef,
			);
			if (existingTarget) {
			...
			switch (event.type) {
				case "requested": {
					...
					const target = current
						? resolveExistingSnapShotTarget(current, routeThreadRef)
						: null;
					...
					if (target) {
						void resolveSnapShotDeliveryTarget(
							captureTargetsRef.current,
							event.id,
