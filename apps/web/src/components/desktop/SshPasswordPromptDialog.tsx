...
function describeSshTarget(request: DesktopSshPasswordPromptRequest): string {
	return request.username
		? `${request.username}@${request.destination}`
		: request.destination;
}
...
function getPromptErrorMessage(error: unknown): string {
	const message =
		error instanceof Error ? error.message : "SSH password prompt failed.";
		...
		<ActiveSshPasswordPrompt
			...
			onRemove={(requestId) => {
				setQueue((currentQueue) =>
					currentQueue[0]?.requestId === requestId
		...
		>
			<DialogPopup className="max-w-md" showCloseButton={false}>
				<DialogHeader>
					...
					<DialogDescription>
						T3 needs your SSH password to connect to <code>{target}</code>. The
						password is passed to the local SSH process for this connection
						attempt and is not saved by T3 Code.
					</DialogDescription>
				</DialogHeader>
					...
					>
						...
						{visibleResponseError ? (
							...
							<p className="text-sm text-muted-foreground">
								Use SSH keys to avoid repeated password prompts on new SSH
								sessions.
							</p>
						)}
					</form>
