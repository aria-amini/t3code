...
import {
	StyledDiffCodeView,
	type StyledDiffCodeViewOptions,
...
export type AnnotatableCodeViewHandle =
	CodeViewHandle<DiffCommentAnnotationGroup>;
...
export function AnnotatableCodeView({
	...
	const filesByKey = useMemo(
		() => new Map(files.map((file) => [file.fileKey, file])),
		[files],
	);
	...
	const removeEntry = useCallback(
		(entryId: string) => {
			...
			if (
				draft?.annotation.metadata.entries.some((entry) => entry.id === entryId)
			) {
				setDraft(null);
				setDraftText("");
			} else {
	...
	return (
		<StyledDiffCodeView<DiffCommentAnnotationGroup>
			...
			renderAnnotation={(annotation) => {
				const hasDraft = annotation.metadata.entries.some(
