...
describe("DiffCommentAnnotation", () => {
	it("renders the shared draft composer directly in the selected diff", () => {
		...
		const textareaControl = markup.match(
			/<span[^>]*data-slot="textarea-control"[^>]*>/,
		)?.[0];
	...
	it("lets a pull-request diff configure actions without replacing the composer", () => {
		...
		expect(
			markup.match(/<button[^>]*disabled[^>]*>Add to review<\/button>/),
		).not.toBeNull();
		expect(
			markup.match(/<button[^>]*disabled[^>]*>Add to agent<\/button>/),
		).not.toBeNull();
	});
