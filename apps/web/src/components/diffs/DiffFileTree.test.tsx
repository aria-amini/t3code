...
describe("diff tree file activation", () => {
	...
	const model = (): FileTreeModel =>
		renderer!.root.findByType(FileTree).props.model;
	...
	async function activate(
		path: string,
		modifiers: Partial<MouseEvent<HTMLElement>> = {},
	) {
		const event = {
			button: 0,
			ctrlKey: false,
	...
	it("reveals newly selected files once in either direction", async () => {
		...
		expect(
			targets.map((target) => ("id" in target ? target.id : null)),
		).toEqual(
			["02-short.ts", "01-tall.ts", "03-medium.ts"].map(
				(path) => `${path}\0${path}`,
			),
		);
	});
	...
	it("leaves directory selection and expansion to the tree", async () => {
		...
		if (!("isExpanded" in directory))
			throw new Error("Expected the directory handle");
