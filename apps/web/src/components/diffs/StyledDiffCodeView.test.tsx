			...
			addEventListener(
				...
				this.worker.on("message", (data: WorkerResponse) => {
					...
					if (
						...
						if (testState.heldResponses.length === 2)
							testState.onInitializationHeld?.();
					} else {
						deliver();
					}
				});
...
vi.mock("@pierre/diffs/react", async (importOriginal) => ({
	...
	CodeView: (props: CodeViewProps) => {
		return props.items?.map((item) =>
			item.type === "file" ? (
				<FileOutput key={item.id} file={item.file} />
			) : null,
		);
	},
}));
	...
	return (
		<>
			...
			{files.slice(0, count).map((file) => (
				<StyledDiffCodeView
					key={file.name}
					items={[{ type: "file", id: file.name, file }]}
				/>
			))}
		</>
	);
...
describe("code-view worker lifecycle", () => {
	...
	beforeEach(() => {
		...
		vi.spyOn(WorkerPoolManager.prototype, "initialize").mockImplementation(
			function (this: WorkerPoolManager, ...args) {
	...
	it("starts on demand, keeps quick reopens warm, and expires the last closed pool", async () => {
		...
		expect(
			mounted.root
				.findByProps({ "data-code-file": "notes.txt" })
				.children.join(""),
		).toContain("Plain text is ready.");
		expect(
			testState.requests.filter((request) => request.type === "file"),
		).toHaveLength(0);
		...
		expect(
			mounted.root
				.findByProps({ "data-code-file": "answer.ts" })
				.children.join(""),
		).toContain("answer");
	...
	it("renders through Pierre's main-thread fallback when worker creation fails", async () => {
		...
		const pool = [...testState.pools.keys()][0]!;
		await act(async () => {
			await expect(testState.pools.get(pool)).rejects.toMatchObject({
				_tag: "DiffWorkerError",
			});
...
function RevealViews({
	...
	useEffect(() => {
		...
		viewer.scrollTo({
			type: "item",
			id: selectedRequest.selectedFile,
			align: "start",
		});
...
describe("code-view file reveals", () => {
	...
	it("does not replay a handled tree click or scroll an old handle when a selected file remounts the view", async () => {
		await act(async () => {
			renderer = create(
				<RevealViews scope="first" visible selectedFile="external-a" />,
			);
		});
		await act(async () => {
			(
				renderer!.root.findByType("button").props as { onClick(): void }
			).onClick();
		});
	...
	it("waits for a live viewer and applies each pending tree click only once", async () => {
		await act(async () => {
			renderer = create(
				<RevealViews scope="diff" visible={false} selectedFile="external" />,
			);
		});
		await act(async () => {
			(
		...
		await act(async () => {
			renderer!.update(
				<RevealViews scope="diff" visible selectedFile="external" />,
			);
		});
		expect(revealCalls.map((call) => call.fileKey)).toEqual([
			"external",
			"tree-file",
		]);
		revealCalls.length = 0;
	...
	it("cancels pending tree clicks when an external selection or diff scope changes", async () => {
		await act(async () => {
			renderer = create(
				<RevealViews scope="first" visible={false} selectedFile="external-a" />,
			);
		});
		await act(async () => {
			(
		...
		await act(async () =>
			renderer!.update(<RevealViews scope="first" visible={false} />),
		);
		await act(async () => {
			(
				renderer!.root.findByType("button").props as { onClick(): void }
		...
		await act(async () =>
			renderer!.update(<RevealViews scope="second" visible={false} />),
		);
		await act(async () =>
			renderer!.update(<RevealViews scope="first" visible />),
		);
		expect(revealCalls).toEqual([]);
	});
	...
	it("reveals after a collapsed file expands and honors another click on the same file", async () => {
		...
		const reveal = renderer!.root.findByType("button").props as {
			onClick(): void;
		};
