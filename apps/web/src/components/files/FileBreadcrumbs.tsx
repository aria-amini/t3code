...
function BreadcrumbMenuContent(props: {
	...
	const directoryAvailable =
		...
		entries.some(
			(entry) =>
				entry.kind === "directory" && entry.path === props.directoryPath,
		);
	...
	return (
		<MenuPopup
			...
			onKeyDown={(event) => {
				if (event.key !== "ArrowLeft" || !canGoBack || parentPath === null)
					return;
		...
		>
			{canGoBack && parentPath !== null ? (
				<>
					<MenuItem
						closeOnClick={false}
						onClick={() => props.onDirectoryChange(parentPath)}
					>
						<ArrowLeftIcon />
						<span className="truncate">
							Back to {pathLabel(parentPath, props.projectName)}
						</span>
					</MenuItem>
					<MenuSeparator />
				</>
			...
			<MenuGroup key={props.directoryPath}>
				{entriesQuery.isPending && entriesQuery.data === null ? (
					...
					children.map((entry) => {
						const isCurrentFile =
							entry.kind === "file" && entry.path === props.currentFilePath;
						return (
							<MenuItem
								key={entry.path}
...
function DirectoryBreadcrumb(
	props: FileBreadcrumbsProps & { readonly crumb: FileBreadcrumb },
) {
