...
function WorkspaceImagePreview(props: {
	...
	const imageUrl =
		assetUrl._tag === "Success" ? `${assetUrl.url}${revisionSuffix}` : null;
...
const isPdfPreviewFile = (path: string): boolean =>
	...
	return (
		<BrowserDocumentFrame
			...
			pdf={
				...
				props.attachment.mimeType.split(";", 1)[0]?.trim().toLowerCase() ===
					"application/pdf"
			}
		/>
	);
...
function WorkspaceBrowserPreview(props: {
	...
	const insideWorkspace =
		mediaFileReference(props.absolutePath, props.workspaceRoot).relativePath !==
		undefined;
	const resource = useMemo(
		() => ({
			_tag: insideWorkspace
...
function EditableFileSurface({
	...
	const addReviewComment = useComposerDraftStore(
		(store) => store.addReviewComment,
	);
	const removeReviewComment = useComposerDraftStore(
		...
		(entryId: string) => {
			...
			setLineAnnotations((current) => {
				return current.flatMap((annotation) => {
					const entries = annotation.metadata.entries.filter(
...
function initialExplorerOpen(): boolean {
	try {
		return (
			getLocalStorageItem(FILE_EXPLORER_STORAGE_KEY, Schema.Boolean) ?? true
		);
	} catch (error) {
		console.error(error);
		return true;
...
export default function FilePreviewPanel({
	...
	const isVideo =
		relativePath !== null && isWorkspaceVideoPreviewPath(relativePath);
	const isImage =
		relativePath !== null &&
		!isVideo &&
	...
	const isHtml =
		relativePath !== null && !isPdf && isBrowserPreviewFile(relativePath);
	...
	const [renderBrowserFilePreferred, setRenderBrowserFilePreferred] =
		useLocalStorage(RENDER_BROWSER_FILE_STORAGE_KEY, true, Schema.Boolean);
	...
	const renderBrowserFile =
		isPdf || (isHtml && renderBrowserFilePreferred && revealHandled);
				...
				>
					...
					{canToggleRendered ? (
						<Tooltip>
							...
							/>
							<TooltipPopup>
								{renderedToggleLabel(isMarkdown, rendered)}
							</TooltipPopup>
						</Tooltip>
					) : null}
					{canOpenInBrowser ? (
