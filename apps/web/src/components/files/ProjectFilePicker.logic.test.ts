...
function pathsForQuery(
	entries: Parameters<typeof getProjectFilePickerMatches>[0],
	query: string,
) {
	return getProjectFilePickerMatches(entries, query).map(({ name, path }) => ({
		name,
		path,
	}));
}
...
describe("getProjectFilePickerMatches", () => {
	...
	it("supports space-separated path tokens and a result limit", () => {
		assert.deepEqual(
			getProjectFilePickerMatches(entries, "src index", 1).map(
				({ name, path }) => ({
					name,
					path,
				}),
