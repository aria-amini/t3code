...
type DocumentChange = NonNullable<
	ReturnType<TextDocument<unknown>["applyEdits"]>
>;
interface Tokenizer {
...
function applyChange(change: DocumentChange) {
	...
	renderer.updateRenderCache(
		tokenizer.tokenize(change, range),
		tokenizer.themeType,
	);
...
beforeEach(async () => {
	...
	vi.stubGlobal(
		...
		(frame: ReturnType<typeof setImmediate>) => {
			animationFrames.delete(frame);
	...
	pool = new WorkerPoolManager(
		...
		{
			workerFactory: () =>
				new WorkerTransport() as unknown as globalThis.Worker,
			poolSize: 1,
		},
		options,
	);
...
describe("editable file highlighting", () => {
	...
	it.each([1, 60])(
		...
		async (count) => {
			...
			expect(renderContents()).toContain("EDITED_MARKER");
			const firstLines = renderer.renderFile(file, {
				...range,
				startingLine: 0,
				totalLines: 20,
			});
