...
const { clearRenderQueue } = (await import(
	/* @vite-ignore */ renderingManagerUrl.href
)) as {
...
function drainRenderFrames() {
	...
	try {
		for (let frame = 0; animationFrames.size > 0; frame += 1) {
			if (frame === 10)
				throw new Error("The production render queue did not settle");
...
class MeasuredFile extends VirtualizedFile {
	...
	override attachEditor(
		editor: Parameters<VirtualizedFile["attachEditor"]>[0],
	) {
		this.editor = editor;
		return () => {
	...
	resizeContent(contentWidth: number, codeWidth = contentWidth + 56.75) {
		...
		if (
			!(code instanceof MeasuredElement) ||
			!(content instanceof MeasuredElement)
		) {
	...
	observeLayout() {
		...
		if (!(this.code instanceof MeasuredElement))
			throw new Error("Expected measured code");
		...
		if (observer === undefined)
			throw new Error("The real resize manager did not observe code");
...
async function makeFixture(
	...
	const apply = (
		change: { startLine: number } | undefined,
		passStartLine = true,
	) => {
	...
	const append = () => {
		...
		apply(
			document.applyEdits([
				{ range: { start: position, end: position }, newText: "\n" },
			]),
		);
	};
...
describe("wrapped editor document changes", () => {
	it("preserves the position above an EOF insertion across layout checkpoints", async () => {
		...
		expect(instance.getAdvancedStickySpecs(viewport)).toEqual({
			topOffset: 118240,
			height: 2156,
		});
		...
		expect(instance.getLinePosition(previousLastLine)).toEqual({
			top: before!.top,
			height: 20,
		});
		expect(instance.getLinePosition(document.lineCount)).toEqual({
			top: 120348,
			height: 20,
		});
	...
	it("invalidates changed and shifted rows after an insertion in the middle", async () => {
		...
		expect(instance.getLinePosition(5001)).toEqual({
			top: before!.top,
			height: 20,
		});
	...
	it("keeps preceding measurements when a deletion crosses a checkpoint", async () => {
		...
		apply(
			document.applyEdits([
				{
					range: {
						start: { line: 4999, character: 2 },
						end: { line: 5001, character: 2 },
					},
					newText: "",
				},
			]),
		...
		expect(instance.getLinePosition(5000)).toEqual({
			top: before!.top,
			height: 20,
		});
	...
	it("uses the earliest changed line for edits at multiple selections", async () => {
		...
		expect(instance.getLinePosition(121)).toEqual({
			top: before!.top,
			height: 20,
		});
...
describe("wrapped measurement widths", () => {
	...
	it.each([
		...
		async (before, after) => {
			...
			expect(instance.getLineHeight(0)).toBe(20);
			expect(instance.getLineHeight(120)).toBe(20);
			expect(instance.getLinePosition(6001)).toEqual({
				top: 120008,
				height: 20,
			});
		},
	...
	it.each([
		...
		"handles a %spx to %spx resize before first observer delivery",
		async (before, after) => {
			const { instance } = await makeFixture("wrap", 6001, before);
