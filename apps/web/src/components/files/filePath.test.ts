...
describe("fileBreadcrumbs", () => {
	...
	it("starts host paths outside the workspace at the filesystem root", () => {
		...
		expect(
			fileBreadcrumbs("t3code", "\\\\server\\share\\report.md").map(
				(c) => c.path,
			),
		).toEqual([
			"\\\\server",
...
describe("fileBreadcrumbChildren", () => {
	...
	it("uses natural file-name ordering and preserves input order for equivalent names", () => {
		const files = ["file10.ts", "File2.ts", "file02.ts", "file2.ts"].map(
			(name) => ({
				path: `src/lib/${name}`,
				kind: "file" as const,
			}),
		...
		expect(
			fileBreadcrumbChildren(files, "src/lib").map((entry) => entry.label),
		).toEqual(["File2.ts", "file02.ts", "file2.ts", "file10.ts"]);
	});
