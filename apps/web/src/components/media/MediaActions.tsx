...
function mediaFileName(source: MediaActionSource): string {
	return (
		(source.reference && mediaReferenceFileName(source.reference)) ||
		source.name ||
		source.kind
	);
}
...
function useMediaActions(source: MediaActionSource) {
	...
	const actionUrl = useCallback(async () => {
		if (!source.asset) {
			if (!source.src)
				throw new Error(
					"This media is unavailable. Try reopening the preview.",
				);
			return source.src;
		}
	...
	const showMenu = async (position: { x: number; y: number }) => {
		...
		try {
			...
			if (source.onOpenFile)
				items.push({ id: "open-file", label: "Open in file viewer" });
			...
			if (text !== undefined) {
				await writeTextToClipboard(
					text,
					reference?.kind === "file" ? "file path" : "URL",
				);
				toastManager.add({
					type: "success",
			...
			const toast = stackedThreadToast({
				...
				title: failureTitle,
				description:
					error instanceof Error ? error.message : "The media action failed.",
			});
