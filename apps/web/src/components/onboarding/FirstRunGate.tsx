...
const primaryShellLiveAtom = Atom.make((get) => {
	...
	return (
		serverConfig !== null &&
		get(environmentShell.stateValueAtom(serverConfig.environment.environmentId))
			.status === "live"
	);
...
const workspaceEvidenceLiveAtom = Atom.make((get) => {
	const environmentIds = new Set([
		...get(environmentProjects.projectsAtom).map(
...
export function FirstRunGate({
	...
	const onboardingCompletedAt = useClientSettings(
		(settings) => settings.onboardingCompletedAt,
	);
	...
	useEffect(() => {
		...
		const timer = window.setTimeout(
			() =>
				setGateState((state) =>
					transitionFirstRunGateState(state, { type: "timeout" }),
				),
	...
	if (settingsReadFailed) {
		return (
			<FirstRunRecovery
				reason="settings"
				retrying={hydrationStatus === "retrying"}
			/>
		);
	}
	if (decision !== "app") {
