...
const waitForPreviewPresentation = async (
	...
	while (Date.now() <= deadline) {
		if (useBrowserSurfaceStore.getState().byTabId[runtimeTabId]?.visible)
			return;
...
const findPreviewWebview = (tabId: string): ExecutablePreviewWebview | null =>
	Array.from(
		document.querySelectorAll<ExecutablePreviewWebview>(
			"webview[data-preview-tab]",
		),
	).find((candidate) => candidate.getAttribute("data-preview-tab") === tabId) ??
	null;

const isPreviewWebviewRendering = (runtimeTabId: string): boolean => {
	const wrapper = findPreviewWebview(runtimeTabId)?.closest<HTMLElement>(
...
const waitForRenderedViewport = async (
	...
	while (Date.now() <= deadline) {
		...
		try {
			const webview = findPreviewWebview(runtimeTabId);
			const appliedSettingKey =
				webview?.getAttribute("data-preview-viewport-key") ?? null;
			const declaredViewport = readDeclaredViewport(webview);
			const renderedViewport = webview
				? await readWebviewViewport(webview)
				: null;
			if (
				renderedViewport &&
...
const currentStatus = async (
	...
	const { snapshot, tabId } = resolvePreviewAutomationTarget(
		state,
		requestedTabId,
	);
	...
	const visible = runtimeTabId
		? (useBrowserSurfaceStore.getState().byTabId[runtimeTabId]?.visible ??
			false)
		: false;
	const renderingActive = runtimeTabId
		? isPreviewWebviewRendering(runtimeTabId)
		: false;
	const viewportSetting = snapshot
...
function PreviewAutomationHost(props: {
	...
	const handleRequest = useCallback(
		async (request: PreviewAutomationRequest): Promise<unknown> => {
			...
			const hostDeadlineMs =
				Date.now() + resolveHostWaitBudgetMs(request.timeoutMs);
			...
			try {
				...
				const requireReadyTab = async () => {
					...
					browserActivity.release ??=
						acquireBrowserSurfaceActivity(runtimeTabId);
				...
				switch (request.operation) {
					...
					case "open": {
						...
						const explicitlySuppressed =
							explicitlySuppressesPreviewMiniPlayer(input);
						const suppressedTabs =
							presentationSuppressedRuntimeTabsRef.current.get(
								request.threadId,
							);
						if (explicitlySuppressed) {
							...
							if (suppressedTabs?.size === 0) {
								presentationSuppressedRuntimeTabsRef.current.delete(
									request.threadId,
								);
							}
						}
						...
						if (
							activeSnapshot &&
							previewAutomationOpenNeedsOverlay(input, activeSnapshot)
						) {
					...
					case "resize": {
						...
						const applied = await runBrowserViewportMutation(
							...
							async () => {
								...
								const previousSetting =
									operationState.sessions[ready.tabId]?.viewport ??
									FILL_PREVIEW_VIEWPORT;
						...
						try {
							...
							await runBrowserViewportMutation(ready.runtimeTabId, async () => {
								...
								const latestSetting =
									latestState.sessions[ready.tabId]?.viewport ??
									FILL_PREVIEW_VIEWPORT;
								if (
					...
					case "click": {
						...
						return await ready.bridge.automation.click(
							...
							request.input as Parameters<
								typeof ready.bridge.automation.click
							>[1],
						);
					}
					case "type": {
						...
						return await ready.bridge.automation.type(
							ready.runtimeTabId,
							request.input as Parameters<
								typeof ready.bridge.automation.type
							>[1],
						);
					}
					case "press": {
						...
						return await ready.bridge.automation.press(
							ready.runtimeTabId,
							request.input as Parameters<
								typeof ready.bridge.automation.press
							>[1],
						);
					}
					case "scroll": {
						...
						return await ready.bridge.automation.scroll(
							ready.runtimeTabId,
							request.input as Parameters<
								typeof ready.bridge.automation.scroll
							>[1],
						);
					}
					case "evaluate": {
						...
						return await ready.bridge.automation.evaluate(
							ready.runtimeTabId,
							request.input as Parameters<
								typeof ready.bridge.automation.evaluate
							>[1],
						);
					}
					case "waitFor": {
						...
						return await ready.bridge.automation.waitFor(
							ready.runtimeTabId,
							request.input as Parameters<
								typeof ready.bridge.automation.waitFor
							>[1],
						);
					}
					case "recordingStart": {
					...
					case "recordingStop": {
						...
						const stopRuntimeTabId =
							activeRecordings.find(
								(recording) => recording.serverTabId === stopTabId,
							)?.runtimeTabId ?? null;
	...
	const [requestHandlerAtom] = useState(() =>
		Atom.make({ handle: handleRequest }),
	);
