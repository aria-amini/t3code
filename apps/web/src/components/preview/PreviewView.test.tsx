...
const mocks = vi.hoisted(() => ({
	navigate: vi.fn(
		async (_tabId: string, _url: string): Promise<void> => undefined,
	),
	...
	openPictureInPicture: vi.fn(
		async (_tabId: string): Promise<void> => undefined,
	),
	closePictureInPicture: vi.fn(
		async (_tabId: string): Promise<void> => undefined,
	),
	pickElement: vi.fn(),
...
describe("PreviewView navigation", () => {
	beforeEach(() => {
		...
		mocks.capturePreviewAnnotationScreenshot.mockResolvedValue({
			status: "none",
		});
	...
	it("does not rerender while loading time passes", async () => {
		...
		const root = createRoot(
			document.createElement("div") as unknown as Element,
		);
	...
	it.each([
		...
		"preserves a direct localhost URL in a WSL environment",
	...
	it("keeps the native preview window as a secondary action", async () => {
		...
		mocks.toggleNativePictureInPicture?.();
		await vi.waitFor(() =>
			expect(mocks.openPictureInPicture).toHaveBeenCalledWith(
				TEST_RUNTIME_TAB_ID,
			),
		);
		...
		mocks.toggleNativePictureInPicture?.();
		await vi.waitFor(() =>
			expect(mocks.closePictureInPicture).toHaveBeenCalledWith(
				TEST_RUNTIME_TAB_ID,
			),
		);
	});
	...
	it("warns when main dropped the crop before handing over the pick", async () => {
		...
		const onSendAnnotation = vi.fn();
		mocks.pickElement.mockResolvedValue({
			annotation,
			submission: "send",
	...
	it("still sends when the picked element's crop cannot be captured", async () => {
		...
		await vi.waitFor(() =>
			expect(onSendAnnotation).toHaveBeenCalledWith(sent, null),
		);
		expect(mocks.addPreviewAnnotation).toHaveBeenCalledWith(
			TEST_THREAD_REF,
			sent,
