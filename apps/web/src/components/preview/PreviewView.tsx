...
export function PreviewView({
	...
	const [focusUrlNonce, setFocusUrlNonce] = useState<number | undefined>(
		undefined,
	);
	...
	const addPreviewAnnotation = useComposerDraftStore(
		(store) => store.addPreviewAnnotation,
	);
	const addImage = useComposerDraftStore((store) => store.addImage);
	const environmentHttpBaseUrl = useEnvironmentHttpBaseUrl(
	...
	const resize = useAtomCommand(
		previewEnvironment.resize,
	...
	const desktopOverlay = tabId
		? (previewState.desktopByTabId[tabId] ?? null)
		: null;
	...
	const activeProfileName = previewProfileName(
		browserDefaults.profiles,
		activeProfileId,
	);
	...
	const handleResetZoom = useCallback(() => {
		if (previewBridge && runtimeTabId)
			void previewBridge.resetZoom(runtimeTabId);
	}, [runtimeTabId]);
	...
	const handleCapture = useCallback(
		(record: boolean) => {
			...
			if (record) {
				void startBrowserRecording(runtimeTabId, threadRef, tabId).catch(
					(error) => {
						const description =
							error instanceof Error ? error.message : "An error occurred.";
		...
		void (async () => {
			try {
				...
				const screenshotFile =
					capture.status === "captured" ? capture.file : null;
				const image =
	...
	useEffect(() => {
		return () => {
			...
			if (previewBridge && runtimeTabId) {
				void previewBridge
					.cancelPickElement(runtimeTabId)
					.catch(() => undefined);
			}
			if (isMountedRef.current) setPickActive(false);
		};
	...
	return (
		...
		>
			<PreviewChromeRow
				...
				onPictureInPicture={
					previewBridge && tabId ? handlePictureInPicture : undefined
				}
				pictureInPicture={miniPlayer?.tabId === tabId}
				pictureInPictureDisabled={
					!desktopOverlay?.hasWebContents || isUnreachable
				}
				...
				leadingActions={
					...
					activeProfileId !== browserDefaults.profileId ? (
						...
						<Tooltip>
							<TooltipTrigger
								render={
									<Badge variant="outline" className="max-w-28 shrink-0" />
								}
							>
