...
export function ThreadPreviewMiniPlayer({
	...
	const [container, setContainer] = useState<PreviewMiniPlayerSize | null>(
		null,
	);
				...
				>
							...
							<Tooltip>
								<TooltipTrigger
									render={
										<Button
											variant={
												desktopOverlay?.pictureInPicture ? "secondary" : "ghost"
											}
											size="icon-xs"
											aria-label={
					...
					{RESIZE_HANDLES.map(({ direction, className }) => (
						<div
							...
							className={cn(
								"pointer-events-auto absolute z-[49] touch-none",
								className,
							)}
