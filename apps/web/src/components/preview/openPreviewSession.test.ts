...
describe("openPreviewSession", () => {
	it("creates an idle tab without recording a recently visited URL", async () => {
		...
		const open = vi.fn(async (_input: PreviewOpenInput) =>
			AsyncResult.success(idleSnapshot),
		);
	...
	it.each(["session", "link"] as const)(
		...
		async (entryPoint) => {
			...
			vi.spyOn(browserDefaults, "resolveBrowserDefaults").mockRejectedValueOnce(
				failure,
			);
			...
			const open =
				entryPoint === "session" ? openPreviewSession : openUrlInPreview;
