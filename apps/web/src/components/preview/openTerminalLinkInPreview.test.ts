...
vi.mock("~/browser/browserDefaults", () => ({
	...
	browserDefaultOpenViewport: (defaults: { viewport: unknown }) =>
		defaults.viewport,
	browserDefaultOpenProfileId: (defaults: { profileId: string }) =>
		defaults.profileId,
}));
...
describe("openTerminalLinkInPreview", () => {
	...
	it("waits for hydrated viewport and profile defaults before opening", async () => {
		...
		await vi.waitFor(() =>
			expect(browserDefaultsMocks.resolve).toHaveBeenCalledOnce(),
		);
		expect(openPreview).not.toHaveBeenCalled();
