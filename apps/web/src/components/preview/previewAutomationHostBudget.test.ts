...
describe("resolveHostWaitBudgetMs", () => {
	it("reserves the full response margin once the request budget allows it", () => {
		expect(resolveHostWaitBudgetMs(15_000)).toBe(
			15_000 - PREVIEW_HOST_RESPONSE_MARGIN_MS,
		);
		expect(resolveHostWaitBudgetMs(60_000)).toBe(
			60_000 - PREVIEW_HOST_RESPONSE_MARGIN_MS,
		);
	});
...
describe("waitForHostReadiness", () => {
	...
	it.each([1, 10, 100, 250, 1_000, 15_000])(
		...
		async (requestTimeoutMs) => {
			...
			const result = waitForHostReadiness(deadlineMs, async () => false).then(
				(ready) => {
					finishedAt = Date.now();
					return ready;
				},
