...
const consumerState = (
	handleRequest: (request: PreviewAutomationRequest) => Promise<unknown>,
) => ({
...
describe("previewAutomationRequestConsumer", () => {
	it("acknowledges a replacement stream before consuming requests from it", async () => {
		...
		registry.set(
			requestsAtom,
			AsyncResult.success(requestEvent("request-after-connect")),
		);

		await vi.waitFor(() =>
			expect(registry.get(state.connectionAtom)).toBe(connectionId),
		);
	...
	it("drops late requests from an older stream generation", async () => {
		...
		await vi.waitFor(() =>
			expect(registry.get(state.connectionAtom)).toBe("connection-2"),
		);
	...
	it("consumes every request emitted before React can render", async () => {
		const requestsAtom = Atom.make<
			AsyncResult.AsyncResult<PreviewAutomationStreamEvent, Error>
	...
	it("uses the latest request handler without rebuilding the stream consumer", async () => {
		...
		const respond = vi.fn(
			async (_response: PreviewAutomationResponse) => undefined,
		);
		...
		registry.set(
			requestsAtom,
			AsyncResult.success(requestEvent("request-first")),
		);
		await vi.waitFor(() => expect(respond).toHaveBeenCalledTimes(1));
		registry.set(state.requestHandlerAtom, { handle: secondHandler });
		registry.set(
			requestsAtom,
			AsyncResult.success(requestEvent("request-second")),
		);

		await vi.waitFor(() => expect(respond).toHaveBeenCalledTimes(2));
		expect(firstHandler).toHaveBeenCalledTimes(1);
		expect(secondHandler).toHaveBeenCalledTimes(1);
		expect(respond.mock.calls.map(([response]) => response.result)).toEqual([
			"first",
			"second",
		]);
		registry.dispose();
