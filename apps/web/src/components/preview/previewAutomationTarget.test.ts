...
describe("preview automation target selection", () => {
	...
	it("reuses the provider session's pinned tab instead of the mutable UI tab", () => {
		...
		expect(resolvePreviewAutomationOpenTab(state, agentTab.tabId, true)).toBe(
			agentTab.tabId,
		);
		expect(resolvePreviewAutomationOpenTab(state, undefined, true)).toBe(
			uiActive.tabId,
		);
		expect(
			resolvePreviewAutomationOpenTab(state, agentTab.tabId, false),
		).toBeNull();
	});
});
