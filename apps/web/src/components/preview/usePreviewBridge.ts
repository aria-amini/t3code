...
export function usePreviewBridge(input: {
	...
	const lastReportedKind = useRef<
		DesktopPreviewTabState["navStatus"]["kind"] | null
	>(null);
	const lastDesktopNavStatus = useRef<
		DesktopPreviewTabState["navStatus"] | null
	>(null);
	const handleStateChange = useEffectEvent(
		(changedTabId: string, state: DesktopPreviewTabState): void => {
			...
			if (
				shouldClearBrowserPointer(lastDesktopNavStatus.current, state.navStatus)
			) {
...
export function projectDesktopState(
	...
	const navOrigin =
		state.navStatus.kind === "Idle" ? null : originOf(state.navStatus.url);
	return {
		...
		favicon:
			state.favicon && originOf(state.favicon.pageUrl) === navOrigin
				? state.favicon
				: null,
	};
}
