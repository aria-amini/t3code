...
function PullRequestCodeTab({
	...
	const [visibleCommitCount, setVisibleCommitCount] =
		useState(COMMIT_PAGE_SIZE);
	...
	const [viewer, setViewer] =
		useState<CodeViewHandle<ReviewAnnotationGroup> | null>(null);
	...
	const loadThreadComments = useAtomCommand(
		pullRequestEnvironment.threadComments,
		{
			reportFailure: false,
		},
	);
	const getDiffFileContents = useAtomCommand(
		pullRequestEnvironment.diffFileContents,
	);
	...
	const review = useMemo(() => {
		...
		return {
			...
			verdicts: hostReview.verdicts.filter((verdict) =>
				viewer.verdicts.includes(verdict),
			),
		};
													...
													.map(
														(comment) =>
															`${comment.id}:${comment.author?.login ?? ""}:${comment.createdAt}:${comment.body}:${(
																...
																.map(
																	(r) =>
																		`${r.content}:${r.count}:${r.viewerHasReacted ? "v" : ""}`,
																)
																.join(",")}`,
													)
													.join(";")}`,
	...
	const allFilesCollapsed = areAllDiffFilesCollapsed(
		fileKeys,
		collapsedFileKeys,
	);
	...
	const revealFile = useCallback(
		(path: string) => {
			const item = items.find(
				(candidate) => resolveFileDiffPath(candidate.fileDiff) === path,
			);
	...
	const beginComment = useCallback(
		(
			...
			context: { item: CodeViewItem<ReviewAnnotationGroup> },
		) => {
			if (!range || !canCommentOnLines) return;
			const item = context.item;
			if (item.type !== "diff") return;
			const file = files.find(
				(candidate) => buildFileDiffRenderKey(candidate) === item.id,
			);
			if (!file) return;
	...
	const finishSelection = useCallback(
		(
			anchor: DraftAnchor,
			text: string,
			onFinish: (comment: ReviewCommentContext) => void,
		) => {
			const file = files.find(
				(candidate) => buildFileDiffRenderKey(candidate) === anchor.fileKey,
			);
			const comment =
				file === undefined
					? null
	...
	const diffViewOptions = useMemo(
		() => ({
			diffStyle:
				diffLayout === "split" ? ("split" as const) : ("unified" as const),
	...
	const renderThreadCard = useCallback(
		(thread: PullRequestReviewThread) => (
			<ReviewThreadCard
				...
				onLoadMore={async (
					cursor,
				): Promise<PullRequestThreadCommentsResult | null> => {
					...
					{withheldContent ? (
						<Tooltip>
							...
							<TooltipPopup side="bottom">
								The host withheld part of this diff — a binary file, or a change
								too large to inline.
							</TooltipPopup>
						</Tooltip>
					) : null}
