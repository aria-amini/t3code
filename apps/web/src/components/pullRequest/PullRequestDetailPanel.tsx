...
function PullRequestCopyableCode({
	...
	return (
		<Tooltip>
			...
			<TooltipPopup
				className="max-w-96 wrap-anywhere font-mono"
				side={tooltipSide}
			>
...
export function PullRequestDetailPanel({
	...
	const selectedCodeCommitOid =
		codeCommitScope.pullRequestKey === pullRequestKey
			? codeCommitScope.oid
			: null;
	...
	const mountedTabs =
		tabMountState.key === tabScopeKey
			? tabMountState.tabs
			: new Set<DetailTab>([tab]);
	useEffect(() => {
		setTabMountState((previous) => {
			if (previous.key !== tabScopeKey)
				return { key: tabScopeKey, tabs: new Set([tab]) };
	...
	const mergeMethod =
		mergeMethodSelection.pullRequestKey === pullRequestKey
	...
	const coreDetail = useMemo(
		() =>
			resolvedCoreDetail === null ||
			sharedSummary === null ||
			sharedSummary === resolvedCoreDetail
				? resolvedCoreDetail
				: {
						...resolvedCoreDetail,
						...sharedSummary,
	...
	useEffect(() => {
		if (detail?.autoMergeMethod !== undefined)
			setMergeMethod(detail.autoMergeMethod);
	}, [detail?.autoMergeMethod, pullRequestKey]);
	const repositoryUrl =
		detail === null ? null : changeRequestRepositoryUrl(detail.url);
	const authorProfileUrl =
		...
		repositoryUrl !== null
			? new URL(
					`/${encodeURIComponent(detail.author.login)}`,
					repositoryUrl,
				).toString()
			: null;
	...
	const [pendingAction, setPendingAction] = useState<PullRequestAction | null>(
		null,
	);
	...
	const titleDraft =
		titleScope?.pullRequestKey === pullRequestKey ? titleScope.text : null;
	...
	const unavailableGitHubUrl = useMemo(() => {
		...
		return gitHubPullRequestBrowserUrl(
			identity,
			reference.repository,
			reference.number,
		);
	...
	const chosenEnvironmentId =
		actingScope?.pullRequestKey === pullRequestKey
			? actingScope.environmentId
			: environmentId;
	...
	const acting =
		pickableEnvironments.find(
			(entry) => entry.environmentId === chosenEnvironmentId,
		) ?? null;
	...
	const startHandoff = async (
		...
		if (prepared._tag === "Failure") {
			...
			const detailMessage =
				prepareThread.error instanceof Error
					? prepareThread.error.message
					: null;
			toastManager.update(toastId, {
				type: "error",
	...
	const visibleTabs = TABS.filter(
		(item) =>
			item.value !== "code" || detail === null || detail.capabilities.diff,
	);
									...
									<TooltipTrigger
										render={
											<button
												type="button"
												onClick={() =>
													void readLocalApi()?.shell.openExternal(detail.url)
												}
												onContextMenu={(event) =>
													openNumberContextMenu(event, detail)
												}
												className={cn(
									...
									<MenuPopup align="end" side="bottom" className="min-w-72">
										<MenuItem onClick={() => startCheckout("worktree")}>
											...
											<span className="flex min-w-0 flex-col">
												...
												<span className="text-xs text-muted-foreground">
													Its own folder and thread. Nothing you have open
													moves.
												</span>
											</span>
										</MenuItem>
										<MenuItem onClick={() => startCheckout("local")}>
											...
											<span className="flex min-w-0 flex-col">
												...
												<span className="text-xs text-muted-foreground">
													Switches the branch you are working in, like `gh pr
													checkout`.
												</span>
											</span>
										</MenuItem>
                  ...
                  pressed it: the merge is already asked for, and the host is holding it. */}
							{autoMergeArmed && primaryAction !== "auto-merge-armed" ? (
								<Tooltip>
									...
									/>
									<TooltipPopup side="top">
										{armedAutoMergeLabel}: the host will merge this on its own
							...
							<Menu>
								...
								<MenuPopup align="end" side="bottom" className="min-w-72">
									...
									>
										...
										<span className="flex min-w-0 flex-col">
											<span>
												{handoff === "ask" ? "Opening..." : "Ask a question"}
											</span>
											<span className="text-xs text-muted-foreground">
												{attachTarget !== null
                          ...
                          show the same action twice. */}
											{showsDraftToggle ? (
												<MenuItem
													disabled={actionPending}
													onClick={() =>
														void perform(detail.isDraft ? "ready" : "draft")
													}
												>
													{detail.isDraft ? (
                          ...
                          a choice for someone who may not merge. */}
                              ...
                              this against the separator that opened the group. */}
									...
									<MenuItem
										onClick={() => void writeTextToClipboard(detail.url)}
									>
									...
									{detail.state === "open" && can("close") ? (
										...
										<>
											...
											<MenuItem
												disabled={actionPending}
												onClick={() =>
													setConfirmation({ open: true, action: "revert" })
												}
											>
									...
									<span className="flex min-w-0 flex-1 items-center gap-1.5 font-mono text-[11px] text-muted-foreground/65">
										...
										{freshness ? (
											<PullRequestBaseFreshnessWarning
												...
												onUpdate={(method) =>
													void perform("update-branch", undefined, method)
												}
									...
									<div className="group flex min-w-0 items-center gap-1">
										...
										{canEditPullRequestChangeRequest(detail) ? (
											<Button
												...
												onClick={() =>
													setTitleScope({ pullRequestKey, text: detail.title })
												}
											>
						...
						>
							{visibleTabs.map((item) => (
								<Toggle
									...
									value={item.value}
									onPointerEnter={
										item.value === "code" ? () => void loadCodeTab() : undefined
									}
									onFocus={
										item.value === "code" ? () => void loadCodeTab() : undefined
									}
							...
							<span className="ml-auto inline-flex shrink-0 items-center">
								{workflowApprovalsRequired > 0 && can("approve-workflows") ? (
									...
									<span
										...
										aria-label={
											checksSummary ? `Checks: ${checksSummary}` : "Checks"
										}
									>
										{checksState !== null ? (
			...
			>
				{detailQuery.error && !detail ? (
					<PullRequestsUnavailableState
						...
						{...(unavailableGitHubUrl
							? { gitHubUrl: unavailableGitHubUrl }
							: {})}
					/>
				) : detail ? (
					<PullRequestMarkdownContext
						value={detail.provider === "github" ? repositoryUrl : null}
					>
						{mountedTabs.has("summary") ? (
							<div
								className={cn(
									"absolute inset-0",
									tab !== "summary" && "invisible",
								)}
							>
