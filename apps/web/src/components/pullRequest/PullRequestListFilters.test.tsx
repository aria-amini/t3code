...
function findValueChange(node: ReactNode):
	| ReactElement<{
...
function findLabeledGroup(node: ReactNode, label: string): ReactNode {
	for (const child of Children.toArray(node)) {
		...
		if (props.label === label && typeof child.type === "function") {
			const rendered = (child.type as (properties: unknown) => ReactNode)(
				child.props,
			);
...
function menu(
	overrides: Partial<Parameters<typeof PullRequestFiltersMenu>[0]>,
) {
