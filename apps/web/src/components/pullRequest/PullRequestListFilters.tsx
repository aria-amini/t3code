...
function PullRequestAuthorFilter({
	...
	const selected = options.find(
		(option) => option.actor.login.toLowerCase() === login,
	);
	const visible = [
		...(selected ? [selected] : []),
		...options.filter(
	...
	const select = (next: string) =>
		next.toLowerCase() !== login && onChange(next || undefined);
	return (
		<MenuSub>
			<MenuSubTrigger>
			...
			<MenuSubPopup className="w-80">
				...
				<MenuRadioGroup
					value={selected?.actor.login ?? value ?? ""}
...
export function PullRequestFiltersMenu({
	...
	involvementOptions: ReadonlyArray<
				...
				{hostOptions.length > 2 ? (
					<>
						...
						<PullRequestFilterRadioSubmenu
							...
							options={hostOptions}
							onChange={(next) =>
								onHost(next === ALL_HOSTS_VALUE ? undefined : next)
							}
						/>
					</>
				) : null}
				{serverOptions.length > 2 ? (
					<>
						...
						<PullRequestFilterRadioSubmenu
							...
							onChange={(next) =>
								onServer(
									next === ALL_SERVERS_VALUE
										? undefined
										: (next as EnvironmentId),
								)
							}
						/>
					</>
