...
const LABEL_SLOTS = [
	...
	{
		pill: "hidden @xl/pr-row-meta:inline-flex",
		overflow: "@3xl/pr-row-meta:hidden",
	},
	{ pill: "hidden @3xl/pr-row-meta:inline-flex", overflow: "" },
] as const;

function PullRequestRowLabels({
	labels,
}: {
	labels: EnvironmentPullRequestEntry["labels"];
}) {
			...
			{LABEL_SLOTS.map((slot, index) => {
				...
				return (
					...
					>
						...
						{remaining > 0 ? (
							<span className={cn("shrink-0", slot.overflow)}>
								+{remaining}
							</span>
						) : null}
					</span>
...
function PullRequestRowImpl({
	...
	const { Icon, providerName } = getSourceControlPresentationForKind(
		entry.provider,
	);
	return (
		<button
