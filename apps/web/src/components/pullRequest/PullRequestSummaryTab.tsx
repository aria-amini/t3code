...
function CommentAuthor({ actor }: { actor: PullRequestActor | null }) {
	...
	return (
		<Tooltip>
			<TooltipTrigger
				render={<span className="shrink-0 rounded-full" aria-label={login} />}
			>
			...
			<div className="flex justify-end gap-2">
				{followUpAction === null ? null : (
					<Button
						...
						disabled={
							body.trim().length === 0 || submitting !== null || actionPending
						}
...
export function PullRequestSummaryTab({
	...
	const visibleComments = orderPullRequestComments(
		recentComments,
		commentOrder,
	);
	...
	const editingCommentId =
		commentScope?.pullRequest === detail.url ? commentScope.commentId : null;
	...
	const commentEditing: CommentEditing = {
		...
		onEdit: (comment) =>
			setCommentScope(
				comment === null
					? null
					: { pullRequest: detail.url, commentId: comment.id },
			),
					...
					<>
						{detail.commentsTruncated ? (
							<p className="mb-2 rounded-md border border-amber-500/30 bg-amber-500/5 px-2 py-1.5 text-xs">
								This conversation is longer than this page reads in one go. The
								most recent {detail.comments.length} are here; open it on the
                      ...
                      and offering to fix one would promise more than it says. */}
                          ...
                          and hiding the block would take away the only way back to it. */}
											{body === null &&
											!commentEditing.canEdit(comment) ? null : (
												<CommentBody
													className="mt-2"
													comment={comment}
													editing={commentEditing}
