...
describe("pullRequestChecksState", () => {
	it("lets a failure outrank a run still going, and reports nothing without checks", () => {
		...
		expect(pullRequestChecksState([check("success"), check("cancelled")])).toBe(
			"failing",
		);
		expect(pullRequestChecksState([check("success"), check("pending")])).toBe(
			"pending",
		);
		expect(
			pullRequestChecksState([check("success"), check("action-required")]),
		).toBe("pending");
		...
		expect(pullRequestChecksState([check("skipped"), check("neutral")])).toBe(
			null,
		);
	...
	it("names workflow approval instead of claiming every check passed", () => {
		...
		const manualGate = check("action-required", {
...
function flatten(node: ReactNode): ReadonlyArray<ReturnType<typeof Object>> {
	...
	for (const child of Children.toArray(node)) {
		...
		found.push(child);
		found.push(
			...flatten((child.props as { readonly children?: ReactNode }).children),
		);
	}
...
describe("PullRequestRow checks indicator", () => {
	function indicators(node: ReactNode): number {
		return flatten(node).filter(
			(element) =>
				(element as { type?: unknown }).type === PullRequestChecksPopover,
		).length;
	}
