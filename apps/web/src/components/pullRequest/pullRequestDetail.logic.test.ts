...
describe("whether the panel is showing the thread's own pull request", () => {
	...
	it("matches on project, repository and number together", () => {
		expect(
			isThreadOwnPullRequest(
				{ projectId: "proj-a", repository: "acme/app", number: 7 },
				surface,
			),
		).toBe(true);
	});
	...
	it("rejects a second checkout of the same repository under another project", () => {
		expect(
			isThreadOwnPullRequest(
				{ projectId: "proj-b", repository: "acme/app", number: 7 },
				surface,
			),
		).toBe(false);
	});
	...
	it("rejects another repository or another number", () => {
		expect(
			isThreadOwnPullRequest(
				{ projectId: "proj-a", repository: "acme/web", number: 7 },
				surface,
			),
		).toBe(false);
		expect(
			isThreadOwnPullRequest(
				{ projectId: "proj-a", repository: "acme/app", number: 8 },
				surface,
			),
		).toBe(false);
	});
	...
	it("rejects a thread with no project or no pull request of its own", () => {
		expect(
			isThreadOwnPullRequest(
				{ projectId: null, repository: "acme/app", number: 7 },
				surface,
			),
		).toBe(false);
		expect(
