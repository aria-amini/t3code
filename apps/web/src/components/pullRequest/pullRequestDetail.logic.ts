...
>(base: ReadonlyArray<T>, loaded: ReadonlyArray<T>): ReadonlyArray<T> {
...
export function isStackedPullRequestBase(
	baseBranch: string,
	refs: ReadonlyArray<
		Pick<VcsRef, "name" | "isDefault" | "isRemote" | "remoteName">
	>,
): boolean {
...
>(comments: ReadonlyArray<T>, order: "newest" | "oldest"): ReadonlyArray<T> {
		...
		text: bounded(
			thread.comments
				.flatMap((comment) => {
					...
					return body === null
						? []
						: [`${comment.author?.login ?? "ghost"}: ${body}`];
				})
				.join("\n"),
		),
		diff: "",
...
export function handoffReviewComments(
	...
	return [
		...existing.filter(
			(comment) => !comment.id.startsWith(HANDOFF_COMMENT_ID_PREFIX),
		),
		...incoming,
	];
}
...
export function buildFixFindingsHandoff(input: {
	...
	const unattachable = input.comments
		...
		.flatMap((comment) => {
			...
			if (body === null) return [];
			const where =
				comment.path === null ? "" : ` on \`${boundedField(comment.path)}\``;
			return [
	...
	const includedThreads = threads.slice(
		Math.max(
			0,
			threads.length -
				(FINDING_LIMIT - includedChecks.length - includedRemarks.length),
		),
	);
	const omitted =
		threads.length +
	...
	return {
		...
		reviewComments: includedThreads.map((thread) =>
			reviewThreadContext(thread, input.number),
		),
	};
}
...
export function readableFailure(failure: unknown, hint: string): string {
	...
	if (detail.length === 0 || TOOL_NOISE.some((pattern) => pattern.test(detail)))
		return hint;
	const bounded =
...
export function resolveBaseFreshness(detail: {
	...
	if (detail.state !== "open" || detail.baseComparison !== "behind")
		return null;
...
export function pullRequestActionNeedsHostRefresh(
...
export function resolveDisplayedPullRequestDetail(input: {
	...
	if (
		...
		input.cached.repository.toLowerCase() ===
