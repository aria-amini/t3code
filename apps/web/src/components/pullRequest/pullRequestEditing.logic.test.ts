...
function capabilities(
	edit?: PullRequestCapabilities["edit"],
): PullRequestCapabilities {
	return {
		diff: true,
		comment: true,
...
describe("canEditPullRequestChangeRequest", () => {
	...
	it("matches the author regardless of case", () => {
		expect(
			canEditPullRequestChangeRequest(subject({ viewer: "OctoCat" })),
		).toBe(true);
	});
	...
	it("refuses where the host did not say who the reader is", () => {
		expect(
			canEditPullRequestChangeRequest(subject({ viewer: undefined })),
		).toBe(false);
	});
...
describe("canEditPullRequestComment", () => {
	...
	it("lets the reader rewrite their own remark on a line", () => {
		expect(
			canEditPullRequestComment(subject(), comment({ kind: "review-comment" })),
		).toBe(true);
	});
	...
	it("refuses somebody else's remark", () => {
		expect(
			canEditPullRequestComment(
				subject(),
				comment({ author: actor("someone-else") }),
			),
		).toBe(false);
	});
	...
	it("refuses a remark the host attributes to nobody", () => {
		expect(
			canEditPullRequestComment(subject(), comment({ author: null })),
		).toBe(false);
	});

	it("refuses a review's own summary", () => {
		expect(
			canEditPullRequestComment(subject(), comment({ kind: "review" })),
		).toBe(false);
	});
	...
	it("refuses where the host cannot rewrite a remark at all", () => {
		expect(
			canEditPullRequestComment(
				subject({
					capabilities: capabilities({ changeRequest: true, comment: false }),
				}),
				comment(),
			),
		).toBe(false);
	...
	it("refuses where the host did not say who the reader is", () => {
		expect(
			canEditPullRequestComment(subject({ viewer: undefined }), comment()),
		).toBe(false);
	});
});
