...
function file(
	name: string,
	additionLines: ReadonlyArray<string> = [],
): FileDiffMetadata {
	return {
		name,
		hunks: [],
...
describe("orderDiffFiles", () => {
	...
	it("puts a dependency before what imports it", () => {
		expect(
			order([file("src/a.ts", ['import { b } from "./b";']), file("src/b.ts")]),
	...
	it("resolves a specifier that climbs out of the importer's directory", () => {
		expect(
			order([
				file("src/ui/a.ts", ['import { b } from "../lib/b";']),
				file("src/lib/b.ts"),
			]),
	...
	it("resolves a directory specifier to that directory's index file", () => {
		expect(
			order([
				file("src/a.ts", ['import { b } from "./lib";']),
	...
	it("falls back to the specifier's last segment when the path does not line up", () => {
		...
		expect(
			order([
				file("src/a.ts", ['import { b } from "~/lib/b";']),
				file("lib/b.ts"),
			]),
	...
	it("clusters unrelated files by path", () => {
		expect(
			order([file("src/ui/b.ts"), file("src/lib/z.ts"), file("src/lib/a.ts")]),
		).toEqual(["src/lib/a.ts", "src/lib/z.ts", "src/ui/b.ts"]);
	});
	...
	it("puts a test with no source in the diff after the ones that have theirs", () => {
		expect(
			order([
				file("src/orphan.spec.ts"),
				file("src/a.test.ts"),
				file("src/a.ts"),
			]),
		).toEqual(["src/a.ts", "src/a.test.ts", "src/orphan.spec.ts"]);
	});
