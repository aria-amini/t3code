...
const IMPORT_SPECIFIER =
	/(?:\bfrom|\bimport|\brequire\s*\()\s*\(?\s*["']([^"']+)["']/g;
	...
	for (const line of lines) {
		...
		while (match !== null) {
			...
			const extension = MODULE_EXTENSIONS.find((candidate) =>
				withExtension.endsWith(candidate),
			);
			const resolved =
				extension === undefined
					? withExtension
					: withExtension.slice(0, -extension.length);
			const target =
...
export function orderDiffFiles(
	...
	const sourcePaths = [...byPath.keys()].filter(
		(path) => tiers.get(path) === "source",
	);
	...
	for (const path of sourcePaths) {
		...
		imports.set(
			...
			importedPaths(
				path,
				[...file.additionLines, ...file.deletionLines],
				byModulePath,
				byBaseName,
			),
		);
	...
	const orderedTests = [...byPath.keys()]
		...
		.sort((left, right) => {
			const leftPosition =
				sourcePositions.get(testedBaseName(left)) ?? Number.MAX_SAFE_INTEGER;
			const rightPosition =
				sourcePositions.get(testedBaseName(right)) ?? Number.MAX_SAFE_INTEGER;
	...
	return [...orderedSource, ...orderedTests, ...orderedGenerated].map((path) =>
		byPath.get(path)!,
	);
}
