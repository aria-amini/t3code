...
function entry(
	overrides: Partial<EnvironmentPullRequestEntry> &
		Pick<PullRequestListEntry, "number">,
) {
	return {
		environmentId: "env-1",
		provider: "github",
...
describe("visible pull request line-count targets", () => {
	it("reuses counts supplied by the listing and only requests missing counts", () => {
		...
		const entriesByKey = new Map(
			entries.map((item) => [pullRequestEntryKey(item), item]),
		);
	...
	it("drops historical rows after a long scroll so refresh stays bounded to the viewport", () => {
		...
		const batches = entries.map(
			(item) =>
				pullRequestStatsBatches(
					entriesByKey,
					new Set([pullRequestEntryKey(item)]),
				)[0]!,
		);
		...
		expect(
			retained.flatMap((batch) => batch.input.refs.map((ref) => ref.number)),
		).toEqual(entries.slice(-12).map((item) => item.number));
	});
	...
	it("keeps every per-environment batch within the stats contract limit", () => {
		...
		expect(batches.flatMap((batch) => [...batch.keys])).toEqual(
			entries.map(pullRequestEntryKey),
		);
	});
	...
	it("selects visible rows for date modes and every uncached row for size modes", () => {
		...
		expect(
			visible.flatMap((batch) => batch.input.refs.map((ref) => ref.number)),
		).toEqual([2]);
		...
		expect(
			eager.flatMap((batch) => batch.input.refs.map((ref) => ref.number)),
		).toEqual([2, 3]);
	});
	...
	it("ignores a late refresh after the filter or stats policy changes", () => {
		...
		const refresh = (currentScope: {
			key: string;
			policy: "visible" | "eager";
		}) =>
			pullRequestStatsRefreshBatches({
				requestedScope,
				currentScope,
		...
		expect(
			refresh(requestedScope)?.[0]?.input.refs.map((ref) => ref.number),
		).toEqual([2]);
...
describe("pull request involvement filtering", () => {
	...
	it("matches the viewer's own pull requests case-insensitively", () => {
		expect(
			filterPullRequestsByInvolvement(entries, VIEWERS, "authored").map(
				(item) => item.number,
			),
		).toEqual([1]);
	});
	...
	it("returns nothing for Authored when the viewer is unknown", () => {
		expect(
			filterPullRequestsByInvolvement(entries, NO_VIEWERS, "authored"),
		).toEqual([]);
	});

	it("uses the server-computed review-request flag for Reviewing", () => {
		expect(
			filterPullRequestsByInvolvement(entries, VIEWERS, "reviewing").map(
				(item) => item.number,
			),
		).toEqual([2]);
	});
	...
	it("does not treat a matching login on another host as the viewer", () => {
		...
		expect(
			filterPullRequestsByInvolvement(mixed, VIEWERS, "authored").map(
				(item) => item.number,
			),
		).toEqual([1]);
	});
	...
	it("leaves the superset untouched for All", () => {
		expect(
			filterPullRequestsByInvolvement(entries, VIEWERS, "all"),
		).toHaveLength(3);
	});
...
describe("carrying rows already read into filters nothing has answered yet", () => {
	...
	it("never lets a merged pull request sit under Open", () => {
		expect(
			narrowPullRequestsToFilters(rows, { ...everything, state: "open" }).map(
				(row) => row.number,
			),
		).toEqual([1, 4, 5]);
	});
...
describe("narrowing rows by the filters a host may not have applied", () => {
	...
	const narrow = (filters: Parameters<typeof matchesPullRequestFilters>[1]) =>
		rows
			.filter((row) => matchesPullRequestFilters(row, filters))
			.map((row) => row.number);
	...
	it("resolves author:me to the given viewer, case-insensitively", () => {
		...
		expect(matchesPullRequestFilters(mine, { author: "me" }, "bilal")).toBe(
			true,
		);
		expect(matchesPullRequestFilters(theirs, { author: "me" }, "bilal")).toBe(
			false,
		);
	});
	...
	it("compares author:me against the literal name when there is no viewer", () => {
		...
		expect(matchesPullRequestFilters(someoneElse, { author: "me" })).toBe(
			false,
		);
	});
	...
	it("takes a group of labels as an either-or", () => {
		...
		const kept = (labels: ReadonlyArray<ReadonlyArray<string>>) =>
			sized
				.filter((row) => matchesPullRequestFilters(row, { labels }))
				.map((row) => row.number);
	...
	it("takes two groups as an and, each satisfied on its own", () => {
		...
		expect(
			matchesPullRequestFilters(row, {
				labels: [["size:S", "size:XS"], ["bug"]],
			}),
		).toBe(true);
		// The second group holds nothing this row carries, so the first one satisfied is not enough.
		expect(
			matchesPullRequestFilters(row, { labels: [["size:S"], ["wip"]] }),
		).toBe(false);
	});
...
describe("reading qualifiers out of a typed query", () => {
	it("keeps quoted values whole and separates negation", () => {
		expect(
			parsePullRequestQuery('label:"needs design" -label:wip Fix the parser'),
		).toEqual({
			text: "Fix the parser",
	...
	it("reads a comma-separated list as one group either name satisfies", () => {
		expect(
			parsePullRequestQuery("label:size:S,size:XS").filters.labels,
		).toEqual([["size:S", "size:XS"]]);
	});

	it("keeps the spaces in a quoted name of a list, and drops an empty part", () => {
		expect(
			parsePullRequestQuery('label:"needs design","wip"').filters.labels,
		).toEqual([["needs design", "wip"]]);
		expect(parsePullRequestQuery("label:a,,b").filters.labels).toEqual([
			["a", "b"],
		]);
	});

	it("excludes every name of a negated list", () => {
		expect(parsePullRequestQuery("-label:a,b").filters).toEqual({
			excludedLabels: ["a", "b"],
		});
	});
	...
	it("reads the scalar qualifiers in GitHub's own spelling", () => {
		expect(
			parsePullRequestQuery(
				"author:octocat DRAFT:false review:changes_requested status:failure",
			).filters,
		).toEqual({
			author: "octocat",
			draft: "hide",
	...
	it("leaves an unknown value and a stray colon as text, and reads an unknown key as a label", () => {
		const parsed = parsePullRequestQuery(
			"milestone:v2 draft:maybe status: parser",
		);
	...
	it("reads a quoted name holding a comma as the one label it is", () => {
		expect(
			parsePullRequestQuery('label:"needs,triage"').filters.labels,
		).toEqual([["needs,triage"]]);
		expect(
			parsePullRequestQuery('-label:"needs,triage"').filters.excludedLabels,
		).toEqual(["needs,triage"]);
	});
	...
	it("cuts a typed query back to what the contract carries", () => {
		...
		const parsed = parsePullRequestQuery(
			`label:${many} label:${long} author:${long}`,
		);
...
describe("ranking what a search found", () => {
	const row = (
		overrides: Partial<PullRequestListEntry> & { number?: number },
	...
	it("puts the pull request whose title says it above one that merely mentions it", () => {
		...
		expect(
			rankPullRequestMatches([mentioned, titled], "welcome wizard").map(
				(entry) => entry.number,
			),
		).toEqual([1, 2]);
	});
	...
	it("finds the words in any order, which is how people type them", () => {
		...
		expect(
			scorePullRequestMatch(
				row({ title: "The wizard that welcomes" }),
				"welcome wizard",
			),
		).toBe(70);
		expect(
			scorePullRequestMatch(
				row({ title: "Wizard for the welcome flow" }),
				"welcome wizard",
			),
		).toBe(70);
	...
	it("keeps recency where nothing tells two results apart", () => {
		...
		const newer = row({
			number: 2,
			title: "none here either",
			updatedAt: "2026-08-01T00:00:00Z",
		});
		expect(
			rankPullRequestMatches([older, newer], "wizard").map(
				(entry) => entry.number,
			),
		).toEqual([2, 1]);
	});
	...
	it("leaves a listing with no search in the order it arrived", () => {
		...
		expect(rankPullRequestMatches([first, second], "  ")).toEqual([
			first,
			second,
		]);
	});
...
describe("default merge-readiness ranking", () => {
	...
	it("uses recency when readiness and diff size tie", () => {
		...
		expect(
			rankPullRequestsByMergeReadiness([older, newer]).map((row) => row.number),
		).toEqual([2, 1]);
	});
	...
	it("ranks smaller measured diffs first within a readiness tier as counts arrive", () => {
		...
		expect(
			rankPullRequestsByMergeReadiness([larger, unknown, smaller]).map(
				(row) => row.number,
			),
		).toEqual([2, 1, 3]);
		expect(
	...
	it("keeps authored work first and ranks each group by readiness", () => {
			...
			[
				{
					key: "authored",
					label: "Authored",
					entries: [authoredWaiting, authoredReady],
				},
		...
		expect(
			sorted.flatMap((group) => group.entries).map((row) => row.number),
		).toEqual([2, 1, 3]);
	});
	...
	it.each([
		...
		"keeps authored first while applying the %s sort inside groups",
		(sort, order) => {
				...
				[
					{
						key: "authored",
						label: "Authored",
						entries: [olderLarger, newerSmaller],
					},
...
describe("merging line counts across keyed stats queries", () => {
	it("keeps counts already held while a fresh batch says nothing about them", () => {
		...
		expect(merged.get("env-1 project-1 1")).toEqual({
			additions: 10,
			deletions: 2,
		});
		expect(merged.get("env-1 project-1 2")).toEqual({
			additions: 5,
			deletions: 1,
		});
	});
	...
	it("replaces a count once its replacement arrives, keeping its neighbours", () => {
		...
		expect(merged.get("env-1 project-1 3")).toEqual({
			additions: 7,
			deletions: 0,
		});
	});
	...
	it("does not mutate the map it was handed", () => {
		...
		expect(held.get("env-1 project-1 1")).toEqual({
			additions: 1,
			deletions: 1,
		});
	});
...
describe("merging the environments' own listings", () => {
	const answer = (
		overrides: Partial<
			Parameters<typeof mergePullRequestLists>[0][number][1]
		> = {},
	) =>
		({
			viewers: { "github.com": "Bilal" },
	...
	it("tags every row with the environment that read it, newest first", () => {
		const merged = mergePullRequestLists([
			[
				ENV_1,
				answer({
					entries: [entry({ number: 1, updatedAt: "2026-07-01T00:00:00Z" })],
				}),
			],
			[
				ENV_2,
				answer({
					entries: [entry({ number: 2, updatedAt: "2026-08-01T00:00:00Z" })],
				}),
			],
		]);
		expect(
			merged?.entries.map((row) => [row.environmentId, row.number]),
		).toEqual([
			[ENV_2, 2],
			[ENV_1, 1],
		]);
	...
	it("keeps each environment's continuation to itself", () => {
		const merged = mergePullRequestLists([
			[
				ENV_1,
				answer({ nextCursors: { "github.com pingdotgg/t3code": "cursor-1" } }),
			],
			[ENV_2, answer()],
		]);
...
describe('who "I" am, per server', () => {
	...
	it("does not let one server's account decide who authored another server's rows", () => {
		...
		const merged = mergePullRequestLists([
			[
				ENV_1,
				answer({ "github.com": "Bilal" }, [
					entry({ number: 1, author: byBilal }),
				]),
			],
			[
				ENV_2,
				answer({ "github.com": "Octocat" }, [
					entry({ number: 2, author: byBilal }),
				]),
			],
		])!;
		...
		expect(
			groups
				.find((group) => group.key === "authored")
				?.entries.map((row) => row.number),
		).toEqual([1]);
		expect(
			filterPullRequestsByInvolvement(
				merged.entries,
		...
		const merged = mergePullRequestLists([
			[
				ENV_1,
				{
					...answer({}, []),
					truncated: true,
					nextCursors: { "github.com acme/web": "cursor-1" },
				},
			],
...
describe("the project an id names", () => {
	...
	it("takes the server it was given", () => {
		expect(findScopedProject(projects, ENV_2, "project-1")?.environmentId).toBe(
			ENV_2,
		);
	});

	it("answers a bare id only where one server has it", () => {
		expect(findScopedProject(projects, null, "project-2")?.environmentId).toBe(
			ENV_2,
		);
...
describe("colon-namespaced labels typed as a search", () => {
	it("reads an unknown key as the label it almost always is", () => {
		expect(parsePullRequestQuery("size:XXL").filters.labels).toEqual([
			["size:XXL"],
		]);
		expect(parsePullRequestQuery("vouch:trusted").filters.labels).toEqual([
			["vouch:trusted"],
		]);
	...
	it("reads the key as the namespace of every bare name in its list", () => {
		expect(parsePullRequestQuery("size:S,XS").filters.labels).toEqual([
			["size:S", "size:XS"],
		]);
	});
	...
	it("leaves a name that already carries its own namespace alone", () => {
		...
		expect(parsePullRequestQuery("size:S,size:XS").filters.labels).toEqual([
			["size:S", "size:XS"],
		]);
	});

	it("excludes one the same way", () => {
		expect(parsePullRequestQuery("-size:XXL").filters.excludedLabels).toEqual([
			"size:XXL",
		]);
	...
	it("reads a hyphenated namespace as a label too", () => {
		expect(parsePullRequestQuery("priority-1:high").filters.labels).toEqual([
			["priority-1:high"],
		]);
	...
	it("mixes with the keys it does know, and with plain words", () => {
		const parsed = parsePullRequestQuery(
			"area:web draft:true wizard label:bug",
		);
...
describe("the priority groups against a paginated feed", () => {
	...
	it("shows every authored row the host reported, not only the ones the feed page holds", () => {
		...
		const feed = [
			newest,
			entry({ number: 6123, updatedAt: "2026-08-11T09:00:00Z" }),
		];
