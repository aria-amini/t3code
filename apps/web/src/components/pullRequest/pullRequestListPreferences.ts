...
export type PullRequestListPreferencePatch = {
	[Key in keyof PullRequestListPreferences]?:
		| PullRequestListPreferences[Key]
		| undefined;
};
...
const PullRequestListPreferencesSchema = Schema.Struct({
	...
	labels: Schema.optional(
		Schema.Array(BoundedPreference).check(Schema.isMaxLength(10)),
	),
...
export function pullRequestListPreferences(
	...
	return {
		...
		...(search.labels && search.labels.length > 0
