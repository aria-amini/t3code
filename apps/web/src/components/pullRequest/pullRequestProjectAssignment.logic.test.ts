...
const envs = (...ids: ReadonlyArray<string>) =>
	ids as ReadonlyArray<EnvironmentId>;

const plain = (assignment: Map<EnvironmentId, ProjectId[]>) =>
	Object.fromEntries(
		[...assignment].map(([id, projectIds]) => [id, projectIds]),
	);
...
describe("one server per repository", () => {
	it("lets the first server list a repository both hold", () => {
		...
		expect(plain(assignment)).toEqual({
			"env-1": ["a1", "b1"],
			"env-2": ["c2"],
		});
	});
	...
	it("answers the same whatever order the projects arrive in", () => {
		...
		const forward = assignProjectsToEnvironments(
			projects,
			envs("env-1", "env-2"),
			null,
		);
...
describe("where a pull request can be acted on", () => {
	...
	it("matches copies however the remote is cased", () => {
		...
		expect(pickable.map((entry) => entry.environmentId)).toEqual([
			"env-1",
			"env-2",
		]);
	});
