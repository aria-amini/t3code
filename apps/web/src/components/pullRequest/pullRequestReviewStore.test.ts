...
describe("pull request review drafts", () => {
	...
	it("removes only the line comments included in a submitted snapshot", () => {
		...
		const submittedIds =
			usePullRequestReviewStore
				.getState()
				.drafts["review-a"]?.map((entry) => entry.id) ?? [];

		usePullRequestReviewStore
			.getState()
			.addComment("review-a", comment("added-in-flight"));
		usePullRequestReviewStore
			.getState()
			.removeComments("review-a", submittedIds);
	...
	it("does not clear a summary revised while submission is in flight", () => {
		...
		usePullRequestReviewStore
			.getState()
			.clearSummary("review-a", "Submitted body");

		expect(usePullRequestReviewStore.getState().summaries["review-a"]).toBe(
			"Revised body",
		);
	});
});
