...
export function AddProviderInstanceDialog({
	...
	const [instanceIdOverride, setInstanceIdOverride] = useState<string | null>(
		null,
	);
				...
				<div className="flex min-h-0 flex-col overflow-hidden">
					<DialogHeader>
						...
						<DialogDescription>
							...
							for example, a second Codex install pointed at a different
							workspace.
						</DialogDescription>
					...
					<WizardPanel>
						...
						<label className={cn("grid gap-2", wizardStep !== 1 && "hidden")}>
							...
							{showInstanceIdError ? (
								...
								<span className="text-[11px] text-muted-foreground">
									Routing key used by threads and sessions. Letters, digits,
									'-', or '_'.
								</span>
							)}
						</label>
						...
						{driverSettingsFields.length > 0 ? (
							...
							<div className="grid gap-2">
								<p className="text-sm text-muted-foreground">
									This driver has no required configuration. You can add the
									instance now.
								</p>
							</div>
						) : null}
