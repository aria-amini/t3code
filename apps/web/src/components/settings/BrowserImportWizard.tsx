...
export function BrowserImportWizard({
	...
	const [step, setStep] = useState<WizardStep>(() =>
		initialWizardStep(initialSource),
	);
	...
	const runImport = () => {
		...
		const chosen = resolveWizardTarget(
			target,
			newProfileId.current,
			targetProfiles,
		);
		if (chosen === undefined) {
			setTargetError(
	...
	const recheckFullDiskAccess = () =>
		recheckSource("fullDiskAccess", fullDiskAccessRecheckStep);

	return (
		<Dialog
			open
			onOpenChange={(open) =>
				open || !canCloseWizard(step) ? undefined : onClose()
			}
		>
			<DialogPopup className="max-w-lg" showCloseButton={canCloseWizard(step)}>
				{step.step === "quit" ? (
					...
					<FullDiskAccessStep
						...
						onGranted={
							step.resume === "import" ? runImport : recheckFullDiskAccess
						}
	...
	return (
		<>
			<DialogHeader>
				...
				<DialogDescription>
					{source.name} is open, so its cookies can&rsquo;t be read yet. Quit
					it, then continue.
				</DialogDescription>
			</DialogHeader>
			<DialogFooter>
...
function FullDiskAccessStep({
	...
	return (
		<>
			<DialogHeader>
				<DialogTitle>
					Let T3 Code read {source.name}&rsquo;s cookies
				</DialogTitle>
				<DialogDescription>
					To import cookies from {source.name}, T3 Code needs Full Disk Access.
					Turn it on in System Settings, then come back to finish the import —
					you can revoke it again once the import is done.
				</DialogDescription>
			</DialogHeader>
			{stillRequired ? (
				<DialogPanel>
					<p role="status" className="text-sm text-muted-foreground">
						Full Disk Access is still required. If you just turned it on, quit
						and reopen T3 Code, then try again.
					</p>
				</DialogPanel>
			) : null}
			<DialogFooter>
	...
	return (
		<>
					...
					<section className="flex-1 space-y-2">
						...
						{targetProfiles.map((profile) => (
							<SelectableTile
								key={profile.id}
								selected={
									target.kind === "existing" && target.profileId === profile.id
								}
								title={profile.name}
								subtitle="Existing profile"
								onSelect={() =>
									onTargetChange({ kind: "existing", profileId: profile.id })
								}
							/>
						))}
					</section>
			...
			<DialogFooter>
				...
				<Button
					disabled={
						sourceProfileDirectory === "" || targetMissing || targetUncreatable
					}
					onClick={onImport}
				>
					Import
