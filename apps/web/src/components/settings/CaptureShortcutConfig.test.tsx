...
it.each(["niri", "hyprland"] as const)(
	...
	async (desktop) => {
		...
		expect(
			visitElements(tree, (element) => element.type === "details"),
		).not.toBeNull();
		...
		expect(bridge.applySnapShotConfig).toHaveBeenCalledExactlyOnceWith(
			preview.id,
		);
...
it.each(["niri", "hyprland"] as const)(
	...
	async (desktop) => {
		...
		expect(bridge.setSnapShotShortcutSuppressed).toHaveBeenNthCalledWith(
			1,
			true,
		);
		expect(bridge.setSnapShotShortcutSuppressed).toHaveBeenLastCalledWith(
			false,
		);
		expect(bridge.previewSnapShotConfig).not.toHaveBeenCalled();
		const custom = {
...
it("requires a new diff after changing keys during review", async () => {
	...
	expect(
		visitElements(render(), (element) => "fileDiff" in element.props),
	).toBeNull();
	expect(bridge.applySnapShotConfig).not.toHaveBeenCalled();
...
it("keeps read failures actionable with technical details in Advanced, then permits retry", async () => {
	bridge.previewSnapShotConfig.mockRejectedValueOnce(
		new Error("EACCES: /config/niri/config.kdl"),
	);
	button(render(), "Review changes").onClick();
	await finish(bridge.previewSnapShotConfig.mock.results[0]!.value);
	const failed = render();
	expect(
		visitElements(failed, (element) => element.props.role === "alert")?.props
			.children,
	).toBe("Couldn't prepare the changes. Check Advanced for help.");
	const advanced = visitElements(
		failed,
		(element) => element.type === "details",
	);
	...
	await finish(bridge.previewSnapShotConfig.mock.results[1]!.value);
	expect(
		visitElements(render(), (element) => element.props.role === "alert"),
	).toBeNull();
	expect(button(render(), "Save shortcut").disabled).toBe(false);
	expect(bridge.applySnapShotConfig).not.toHaveBeenCalled();
});
it("withdraws a stale diff and requires another read instead of retrying its write", async () => {
	...
	const tree = render();
	expect(
		visitElements(tree, (element) => element.props.role === "alert")?.props
			.children,
	...
	expect(
		visitElements(
			advanced,
			(element) =>
				element.props.children === "Your config changed since this preview.",
		),
	).not.toBeNull();
	expect(button(tree, "Review changes")).toBeDefined();
	expect(
		visitElements(tree, (element) => "fileDiff" in element.props),
	).toBeNull();
	expect(complete).not.toHaveBeenCalled();
});
it("does not finish or claim success when the desktop could not reload", async () => {
	...
	await finish(bridge.applySnapShotConfig.mock.results[0]!.value);
	expect(
		visitElements(render(), (element) => element.props.role === "status")?.props
			.children,
	...
	expect(
		visitElements(
			advanced,
			(element) =>
				element.props.children === "Config saved, but reload failed.",
		),
	).not.toBeNull();
	expect(complete).not.toHaveBeenCalled();
