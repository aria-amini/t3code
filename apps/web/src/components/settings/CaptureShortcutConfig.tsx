...
export function CaptureShortcutConfig({
	...
	const [preview, setPreview] = useState<DesktopCaptureConfigPreview | null>(
		null,
	);
	const [result, setResult] = useState<DesktopCaptureConfigApplied | null>(
		null,
	);
	...
	const supported = Boolean(
		bridge?.previewSnapShotConfig && bridge.applySnapShotConfig,
	);
	...
	const recorder = useSnapShotShortcutRecorder({
		shortcut: shortcutKeys
			? (parseKeybindingShortcut(shortcutKeys.replace(/super/gi, "meta")) ??
				DEFAULT_SHORTCUT)
			: DEFAULT_SHORTCUT,
		disabled: busy,
	...
	return (
		<div className="space-y-4 text-sm">
			...
			{result ? (
				...
				<>
					<p className="text-muted-foreground">
						Allow T3 Code to read your desktop settings. You'll review any
						changes here before saving.
					</p>
					<Button
			...
			<details className="text-xs text-muted-foreground">
				...
				<div className="mt-3 space-y-3">
					...
					<div className="space-y-1">
						...
						{niri ? (
							<p>T3 Code also reads any files included by this file.</p>
						) : null}
						{preview && preview.resolvedPath !== preview.path ? (
							<p className="break-all">
								Linked to {preview.resolvedPath}. The link will be kept.
							</p>
						) : null}
					</div>
					...
					{result?.backupPath ? (
						<p className="break-all">Backup: {result.backupPath}</p>
					) : null}
