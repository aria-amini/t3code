...
import {
	...
	filterDiscoveredSshHosts,
} from "~/state/desktopSshHosts";
import {
	desktopWslStateAtom,
...
function formatDesktopSshTarget(target: DesktopSshEnvironmentTarget): string {
	const authority = target.username
...
const ENDPOINT_ROW_CLASSNAME =
	"first:rounded-t-xl last:rounded-b-xl px-3 py-2.5 sm:px-4";
...
function sortDesktopPairingLinks(
...
function sortDesktopClientSessions(
...
function toDesktopPairingLinkRecord(
...
function toDesktopClientSessionRecord(
...
function selectPairingEndpoint(
	...
	const availableEndpoints = endpoints.filter(
		(endpoint) => endpoint.status !== "unavailable",
	);
	if (defaultEndpointKey) {
		const selectedEndpoint = availableEndpoints.find(
			(endpoint) =>
				endpointDefaultPreferenceKey(endpoint) === defaultEndpointKey,
		);
		if (selectedEndpoint) {
...
const PairingLinkListRow = memo(function PairingLinkListRow({
	...
	const revealValue =
		failedCopyValue ?? shareablePairingUrl ?? credential ?? "";
	const isRevealValueUrl = revealValue !== credential;
	const isRevealValueHostedAppPairingUrl =
		isRevealValueUrl && isHostedAppPairingUrl(revealValue);
	// Never render a QR for a loopback URL, even in the manual-copy fallback.
	const isRevealValueQrShareable =
		endpointCopyOptions.find((option) => option.url === revealValue)
			?.qrShareable ?? true;
	...
	const copyKindForUrl = useCallback(
		(url: string): "hosted-link" | "link" =>
			isHostedAppPairingUrl(url) ? "hosted-link" : "link",
		[],
	);
			...
			<div className={ITEM_ROW_INNER_CLASSNAME}>
				<div className="min-w-0 flex-1 space-y-1">
					...
					{!credential ? (
						...
						<p className="text-[11px] text-muted-foreground/70">
							Copy the token and pair from another client using this
							backend&apos;s reachable host.
						</p>
					) : null}
				</div>
					...
					<div className="min-w-0 flex-1 space-y-3">
						...
						<div className="flex items-center gap-2 rounded-lg border border-border/60 bg-muted/30 px-2.5 py-1.5">
							...
							<Button
								...
								className="shrink-0"
								onClick={() =>
									copyPairingValue(qrPairingUrl, copyKindForUrl(qrPairingUrl))
								}
							>
								Copy link
							</Button>
					...
					{canRenderQrForSelection ? (
						...
						<div className="flex size-[192px] shrink-0 items-center justify-center self-center rounded-xl border border-border/50 p-4 sm:self-start">
							<p className="text-center text-[11px] text-muted-foreground/70">
								No QR for this endpoint. Another device scanning a loopback link
								would dial itself; copy the URL for use on this machine instead.
							</p>
						</div>
					)}
				</div>
...
const ConnectedClientListRow = memo(function ConnectedClientListRow({
	...
	const deviceInfoBits = [
		clientSession.client.deviceType !== "unknown"
			? clientSession.client.deviceType[0]?.toUpperCase() +
				clientSession.client.deviceType.slice(1)
			: null,
	...
	const primaryLabel =
		...
		([clientSession.client.os, clientSession.client.browser]
			.filter(Boolean)
			.join(" · ") ||
...
const AuthorizedClientsHeaderAction = memo(
	function AuthorizedClientsHeaderAction({
				...
				>
					...
					<DialogPopup className="max-w-md">
						<DialogHeader>
							...
							<DialogDescription>
								Generate a one-time link that another device can use to pair
								with this backend as an authorized client.
							</DialogDescription>
						</DialogHeader>
								...
								<div className="flex items-center justify-between gap-3">
									...
									<div className="flex gap-1">
										<Button
											...
											disabled={isCreatingPairingLink}
											onClick={() =>
												setPairingScopes([AuthOrchestrationReadScope])
											}
										>
											Read only
										</Button>
...
function SavedBackendListRow({
	...
	const serverUpdateState = useAtomValue(
		serverEnvironment.updateStateAtom(environmentId),
	);
	const resumingServerUpdate =
		serverUpdateState.status === "running" &&
		serverUpdateState.stage === "resuming";
	const sshTarget =
	...
	const isWslEnvironment = isDesktopLocalConnectionTarget(
				...
				<div className="flex w-full shrink-0 items-center gap-2 sm:w-auto sm:justify-end">
					...
					(serverUpdateState.status === "idle" ||
						...
						<ServerUpdateAction
							...
							selfUpdate={resolveServerSelfUpdateCapability(
								environment.serverConfig,
							)}
							desktopAppUpdate={supportsDesktopAppUpdate(
								environment.serverConfig,
							)}
					...
					{isWslEnvironment ? (
						<Tooltip>
							...
							>
								The WSL backend is managed by the WSL setting above — turn it on
								or off there.
							</TooltipPopup>
						</Tooltip>
					) : (
						<>
							...
							<Button
								...
								variant="outline"
								disabled={
									isConnecting || removingEnvironmentId === environmentId
								}
								onClick={() =>
									void (isConnected
...
export function ConnectionsSettings() {
	...
	const currentSessionScopes = desktopBridge
		...
		: primarySessionState.data?.authenticated
			...
			: null;
	const currentAuthPolicy = desktopBridge
		? null
	...
	const [
		desktopAccessManagementMutationError,
	...
	const handlePairingLinkCreated = useCallback(
		(created: AuthPairingCredentialResult) => {
			setCreatedPairingCredentials((current) =>
				new Map(current).set(created.id, created.credential),
			);
		},
		[],
	);
	const [revokingDesktopPairingLinkId, setRevokingDesktopPairingLinkId] =
		useState<string | null>(null);
	const [revokingDesktopClientSessionId, setRevokingDesktopClientSessionId] =
		useState<string | null>(null);
	const [isRevokingOtherDesktopClients, setIsRevokingOtherDesktopClients] =
		useState(false);
	const [addBackendDialogOpen, setAddBackendDialogOpen] = useState(false);
	const [savedBackendMode, setSavedBackendMode] = useState<"remote" | "ssh">(
		"remote",
	);
	...
	const highlightedSshHostRef = useRef<DesktopDiscoveredSshHost | undefined>(
		undefined,
	);
	const [savedBackendError, setSavedBackendError] = useState<string | null>(
		null,
	);
	const [isAddingSavedBackend, setIsAddingSavedBackend] = useState(false);
	const [removingSavedEnvironmentId, setRemovingSavedEnvironmentId] =
		useState<EnvironmentId | null>(null);
	const [isUpdatingDesktopServerExposure, setIsUpdatingDesktopServerExposure] =
		useState(false);
	const [
		isDesktopServerExposureDialogOpen,
		setIsDesktopServerExposureDialogOpen,
	] = useState(false);
	const [isUpdatingTailscaleServe, setIsUpdatingTailscaleServe] =
		useState(false);
	const [isUpdatingWslBackend, setIsUpdatingWslBackend] = useState(false);
	const [desktopWslMutationError, setDesktopWslMutationError] = useState<
		string | null
	>(null);
	...
	const [pendingWslChange, setPendingWslChange] =
		useState<PendingWslChange | null>(null);
	...
	const [disableTailscaleServeDialogOpen, setDisableTailscaleServeDialogOpen] =
		useState(false);
	...
	] = useState<DesktopServerExposureState["mode"] | null>(null);
	const primaryServerConfig = primaryEnvironment?.serverConfig ?? null;
	const primaryVersionMismatch =
	...
	const canManageRelay =
		currentSessionScopes?.includes(AuthRelayWriteScope) ?? false;
	...
	const isLoadingDiscoveredSshHosts =
		desktopSshHosts.isPending && desktopSshHosts.data === null;
	...
	const desktopServerExposureState =
		desktopNetworkAccess.data?.serverExposureState ?? null;
	...
	const handleDesktopServerExposureChange = useCallback(
		async (checked: boolean) => {
			...
			try {
				await desktopBridge.setServerExposureMode(
	...
	const handleRevokeOtherDesktopClients = useCallback(async () => {
		...
		try {
			...
			toastManager.add({
				...
				title:
					revokedCount === 1
						? "Revoked 1 other client"
						: `Revoked ${revokedCount} clients`,
				description:
	...
	const handleSelectSshHostSuggestion = useCallback(
		async (target: DesktopDiscoveredSshHost) => {
			...
			setSavedBackendSshPort(
				resolved.port === null ? "" : String(resolved.port),
			);
	...
	const handleSavedBackendSshHostKeyDown = useCallback(
		(event: KeyboardEvent<HTMLInputElement>) => {
			...
			const isSshHostPopupVisible =
				sshHostSuggestionsOpen && hasSshHostSuggestionContent;
			if (isSshHostPopupVisible) {
				...
				const target =
					index === null ? undefined : filteredDiscoveredSshHosts[index];
				if (target) {
					event.preventDefault();
	...
	const handleConnectSavedBackend = useCallback(
		async (environmentId: EnvironmentId) => {
			...
			if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
				const error = squashAtomCommandFailure(result);
				const message =
					error instanceof Error ? error.message : "Failed to connect backend.";
				setSavedBackendError(message);
				toastManager.add(
					stackedThreadToast({
						type: "error",
	...
	const handleRemoveSavedBackend = useCallback(
		async (environmentId: EnvironmentId) => {
			...
			if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
				const error = squashAtomCommandFailure(result);
				const message =
					error instanceof Error ? error.message : "Failed to remove backend.";
				setSavedBackendError(message);
				toastManager.add(
					stackedThreadToast({
						type: "error",
	...
	const visibleDesktopNetworkAdvertisedEndpoints = useMemo(
		() =>
			isLocalBackendNetworkAccessible
				? desktopAdvertisedEndpoints.filter(
						(endpoint) => !isTailscaleHttpsEndpoint(endpoint),
					)
				: [],
			...
			<div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_10rem]">
				<label className="block">
					...
					<Input
						...
						onChange={(event) =>
							handleSavedBackendHostChange(event.target.value)
						}
				...
				<div className="block">
					...
					<Autocomplete
						...
						onValueChange={(value, eventDetails) => {
							...
							const target = filteredDiscoveredSshHosts.find(
								(host) => host.alias === value,
							);
									...
									<AutocompleteList className="max-h-72">
										{filteredDiscoveredSshHosts.map((target, index) => {
											...
											const shortcutCommand =
												index < 9 ? threadJumpCommandForIndex(index) : null;
	...
	const handleSelectWslMode = useCallback(
		(value: string) => {
			...
			const defaultDistroName =
				desktopWslState.distros.find((distro) => distro.isDefault)?.name ??
				null;
			if (value === BACKEND_VALUE_WSL_OFF) {
				...
				void applyWslSettingChange(() =>
					desktopBridge.setWslBackendEnabled(false),
				);
				return;
			}
	...
	const handleToggleWslOnly = useCallback(
		(enabled: boolean) => {
			if (
				!desktopBridge ||
				!desktopWslState ||
				desktopWslState.wslOnly === enabled
			)
				return;
	...
	const handleConfirmWslChange = useCallback(() => {
		...
		if (change.kind === "distro") {
			void applyWslSettingChange(() =>
				desktopBridge.setWslDistro(change.nextDistro),
			);
			return;
		}
		void applyWslSettingChange(() =>
			desktopBridge.setWslOnly(change.nextValue),
		);
		...
		<SettingsRow
			...
			control={
				<Tooltip>
					...
					/>
					<TooltipPopup side="top">
						Network exposure changes restart the backend and must be controlled
						where the server process is launched.
					</TooltipPopup>
				</Tooltip>
			}
		/>
								...
								control={
									...
									primaryServerUpdateState.status !== "running" ? (
										<ServerUpdateAction
											...
											serverLabel={
												primaryEnvironment
													? `${primaryEnvironment.label} server`
													: "server"
											}
											selfUpdate={resolveServerSelfUpdateCapability(
												primaryServerConfig,
						...
						<AlertDialogPopup>
							...
							<AlertDialogFooter>
								<AlertDialogClose
									disabled={isUpdatingWslBackend}
									render={
										<Button variant="outline" disabled={isUpdatingWslBackend} />
									}
								>
									Cancel
								</AlertDialogClose>
								{pendingWslChange?.kind === "enable" ? (
									...
									<Button
										variant={
											pendingWslChange?.kind === "disable" ||
											(pendingWslChange?.kind === "wsl-only" &&
												pendingWslChange.nextValue)
												? "destructive"
												: "default"
										}
					...
					>
						<AlertDialogPopup>
							<AlertDialogHeader>
								...
								<AlertDialogDescription>
									T3 Code will restart the local backend without Tailscale
									Serve.
								</AlertDialogDescription>
							</AlertDialogHeader>
							<AlertDialogFooter>
					...
					>
						<DialogPopup className="max-w-md">
							<DialogHeader>
								...
								<DialogDescription>
									T3 Code will restart the local backend with Tailscale Serve
									enabled and ask Tailscale to proxy HTTPS traffic to this
									backend.
								</DialogDescription>
							</DialogHeader>
							<DialogPanel className="space-y-4">
								<label className="block">
									...
									<Input
										...
										onChange={(event) =>
											setTailscaleServePortInput(event.target.value)
										}
										disabled={isUpdatingTailscaleServe}
									/>
								</label>
							...
							<DialogFooter>
								...
								<Button
									...
									disabled={
										isUpdatingTailscaleServe || !isTailscaleServePortValid
									}
								>
									{isUpdatingTailscaleServe ? (
										<>
