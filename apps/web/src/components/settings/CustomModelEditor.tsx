...
export function CustomModelEditor({
	...
	const [draft, setDraft] = useState<CustomModelDraft>(() =>
		draftFromDefinition(entry),
	);
	...
	const startFromCandidates = useMemo(
		() =>
			builtInModels.filter(
				(model) => (model.capabilities?.optionDescriptors?.length ?? 0) > 0,
			),
		[builtInModels],
	);
	const domId = (suffix: string) =>
		`provider-instance-${instanceId}-custom-model-${suffix}`;
	...
	const removeDescriptor = (key: string) => {
		...
		setDraft((current) => ({
			...current,
			descriptors: current.descriptors.filter(
	...
	const handleStartFrom = (slug: string | null) => {
		...
		const model = startFromCandidates.find(
	...
	const idSelectValue = (descriptor: EditorDescriptor) =>
		presets.some((preset) => preset.id === descriptor.id)
			? descriptor.id
			: CUSTOM_ID_VALUE;
	...
	const renderChoice = (descriptor: EditorDescriptor, choice: EditorChoice) => (
		<div key={choice.key} className="flex items-center gap-2">
			<Input
				...
				value={choice.id}
				onChange={(event) =>
					updateChoice(descriptor.key, choice.key, { id: event.target.value })
				}
				placeholder="value"
			...
			<Button
				...
				onClick={() =>
					updateDescriptor(descriptor.key, {
						choices: descriptor.choices.filter(
			...
			<div className="flex flex-wrap items-center gap-2">
				...
				{idSelectValue(descriptor) === CUSTOM_ID_VALUE ? (
					<Input
						...
						value={descriptor.id}
						onChange={(event) =>
							updateDescriptor(descriptor.key, { id: event.target.value })
						}
				...
				<Input
					...
					onChange={(event) =>
						updateDescriptor(descriptor.key, { label: event.target.value })
					}
					placeholder="Label"
					className="min-w-0 flex-1"
				...
				>
					...
					>
						<SelectValue>
							{descriptor.type === "boolean" ? "Toggle" : "Choices"}
						</SelectValue>
					</SelectTrigger>
