...
describe("clearBrowserProfileData", () => {
	...
	it("does not report success without an environment or bridge", async () => {
		...
		await expect(
			clearBrowserProfileData(bridge, [], "profile-a"),
		).rejects.toThrow();
		await expect(
			clearBrowserProfileData(null, [environmentId], "profile-a"),
		).rejects.toThrow();
...
describe("importFailureReason", () => {
	it("recovers the reason token from the flattened message", () => {
		expect(importFailureReason(failure("needsFullDiskAccess"))).toBe(
			"needsFullDiskAccess",
		);
		expect(importFailureReason(failure("browserRunning"))).toBe(
			"browserRunning",
		);
		expect(importFailureReason(failure("readFailed"))).toBe("readFailed");
		expect(importFailureReason(failure("keychainUnavailable"))).toBe(
			"keychainUnavailable",
		);
		...
		expect(importFailureReason(failure("profileNotSaved"))).toBe(
			"profileNotSaved",
		);
		expect(importFailureReason(failure("profileLimitReached"))).toBe(
			"profileLimitReached",
		);
	});

	it("falls back to readFailed for anything it cannot classify", () => {
		expect(importFailureReason(new Error("something else entirely"))).toBe(
			"readFailed",
		);
