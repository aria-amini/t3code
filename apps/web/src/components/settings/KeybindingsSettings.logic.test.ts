...
describe("KeybindingsSettings.logic", () => {
	...
	it("serializes shortcuts and when expressions for upserts", () => {
		...
		expect(
			parseWhenExpressionDraft("!(terminalFocus || modelPickerOpen)"),
		).toEqual({
			ok: true,
			value: {
				type: "not",
	...
	it("describes the scope of each visual expression removal", () => {
		...
		expect(whenNodeRemoveLabel(group, 1)).toBe(
			"Remove group and its conditions",
		);
	...
	it("formats static and project script command labels", () => {
		expect(commandLabel("commandPalette.toggle")).toBe(
	...
	it("builds command options from all static commands and resolved project bindings", () => {
		...
		expect(
			DEFAULT_RESOLVED_KEYBINDINGS.some(
				(binding) => binding.command === "thread.stop",
			),
		).toBe(false);
	});
	...
	it("reports unknown when variables without rejecting parseable expressions", () => {
		...
		expect(unknownWhenVariables(parsed.ok ? parsed.value : undefined)).toEqual([
			"terminalFoc",
		]);
	});
