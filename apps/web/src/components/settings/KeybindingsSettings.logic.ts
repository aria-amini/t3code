...
export function shortcutToKeybindingInput(
...
export function whenAstToExpression(
...
export function whenNodeRemoveLabel(
	...
	if (
		...
		(node.type === "not" && node.node.type === "identifier")
	) {
...
export function parseWhenExpressionDraft(
	...
	| { ok: true; value: KeybindingWhenNode | undefined }
	...
	const rows = keybindings.map((binding, index) => {
		...
		return {
			...
			defaultKey: defaultBinding
				? shortcutToKeybindingInput(defaultBinding.shortcut)
				: null,
