		...
		return (
			<SettingsSection {...searchableSetting("load-balancing")} variant="plain">
				<p className="px-3 text-sm text-muted-foreground sm:px-4">
					Connect another machine to automatically balance load across
					environments.
				</p>
			</SettingsSection>
		);
	...
	return (
		<SettingsSection {...searchableSetting("load-balancing")}>
			<SettingsRow
				...
				control={
					<Switch
						...
						onCheckedChange={(loadBalancingEnabled) =>
							updateSettings({ loadBalancingEnabled })
						}
					/>
				}
			/>
			{environments.map((environment) => {
				...
				const preference =
					weight === 0 ? 0 : weight < 50 ? 25 : weight === 50 ? 50 : 100;
