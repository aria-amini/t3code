...
export function ProjectDefaultActionsSettings({
	...
	const keybindings =
		representative?.keybindings ?? DEFAULT_RESOLVED_KEYBINDINGS;
		...
		<SettingsSection title="Actions">
			...
			<SettingsRow
				...
				resetAction={
					targets.some(
						(target) =>
							(target.serverConfig?.settings.defaultProjectScripts.length ??
								0) > 0,
					) ? (
						<SettingResetButton
			...
			{mixed ? (
				...
				<ProjectActionsList
					...
					disabled={saving}
					onEdit={(script) =>
						setRequest(editorRequestForScript(script, keybindings))
					}
				/>
			)}
			<ProjectScriptEditorDialog
