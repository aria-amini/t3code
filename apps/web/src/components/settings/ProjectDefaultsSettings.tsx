...
export function ProjectDefaultsSettings({
	...
	const serverSettings =
		representative?.serverConfig?.settings ?? DEFAULT_SERVER_SETTINGS;
	const providers =
		representative?.serverConfig?.providers ?? EMPTY_SERVER_PROVIDERS;
	...
	const mixedWorkspace = targets.some(
		(target) =>
			target.serverConfig?.settings.defaultThreadEnvMode !==
	...
	const disabled = (key: keyof ServerSettingsPatch) =>
		targets.length === 0 || saving.has(key);
	const mixedAutoPull = targets.some(
		(target) =>
			target.serverConfig?.settings.defaultAutoPull !==
			serverSettings.defaultAutoPull,
	);
	...
	function modelDisabledReason(
		...
		for (const target of targets) {
			...
			if (
				...
				!options?.some(
					(option) => option.slug === model && !option.isUnavailable,
				)
			) {
	...
	async function save(patch: ServerSettingsPatch) {
		...
		if (targets.length === 0 || keys.some((key) => savingRef.current.has(key)))
			return;
		const nextModel = patch.defaultModelSelection;
		const reason = nextModel
			? modelDisabledReason(nextModel.instanceId, nextModel.model)
			: null;
		if (reason) {
			toastManager.add({
				type: "error",
		...
		try {
			...
			const failedTargets = targets.filter(
				(_, index) => results[index]?._tag === "Failure",
			);
	...
	const setModel = (value: ModelSelection | null) =>
		void save({ defaultModelSelection: value });
	return (
		<SettingsPageContainer>
			<SettingsSection
				...
				<SettingsRow
					...
					control={
						<Select
							disabled={disabled("defaultThreadEnvMode")}
							value={
								mixedWorkspace ? "mixed" : serverSettings.defaultThreadEnvMode
							}
							onValueChange={(value) => {
					...
					control={
						<Switch
							...
							onCheckedChange={(enabled) =>
								void save({ defaultAutoPull: enabled })
							}
						/>
					}
				/>
