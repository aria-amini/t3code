...
describe("ProjectFaviconPickerDialog", () => {
	...
	it("selects an image from the native file picker", async () => {
		...
		const onPickExternal = vi
			.fn()
			.mockResolvedValue("/Users/me/Pictures/icon.png");
		...
		const button = visitElements(
			picker,
			(element) => element.props.children === "Open in Finder",
		);
	...
	it("hides the native picker for WSL project paths", () => {
		expect(canPickExternalProjectFavicon("/home/me/project", "Win32")).toBe(
			false,
		);
		expect(
			canPickExternalProjectFavicon("C:\\Users\\me\\project", "Win32"),
		).toBe(true);
	});
	...
	it("keeps the dialog open when the native picker fails", async () => {
		...
		hooks.beginRender();
		const picker = ProjectFaviconPickerDialog(props) as ReactElement<
