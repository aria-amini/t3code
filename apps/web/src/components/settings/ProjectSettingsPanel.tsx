...
export function ProjectSettingsPanel({
	...
	useEffect(() => {
		...
		if (last?.key !== projectKey || last.environmentId !== environmentId)
			return;
			...
			for (const { environmentId, settings, keybindings, project } of targets) {
				...
				const changedIds = scriptId
					...
					: current
							.filter(
								(script) =>
									!effectiveScripts.some((next) => next.id === script.id),
							)
				...
				for (const id of changedIds) {
					...
					const retainedElsewhere =
						...
						((project &&
							settings.defaultProjectScripts.some(
								(script) => script.id === id,
							)) ||
							Object.entries(settings.projectScriptOverrides).some(
								([projectId, scripts]) =>
									projectId !== project?.id &&
									scripts?.some((script) => script.id === id),
							) ||
							projects.some(
								(other) =>
									...
									(project
										...
										: other.scripts
									).some((script) => script.id === id),
							));
					const bindingResult = next
						? await upsertKeybinding({
								environmentId,
								input:
									previous && previous.key !== next.key
					...
					if (bindingResult?._tag === "Failure")
						return reportScriptFailure(bindingResult);
				}
			}
	...
	function submit(scriptId: string | null, input: NewProjectScriptInput) {
		const existingIds = [
			...
			...targets.flatMap(({ settings, project }) =>
				[
					...settings.defaultProjectScripts,
					...Object.values(settings.projectScriptOverrides).flatMap(
						(scripts) => scripts ?? [],
					),
					...(project?.scripts ?? []),
				].map((script) => script.id),
...
function ProjectDetail({
	...
	const serverProviders =
		useAtomValue(
			serverEnvironment.providersValueAtom(representative.environmentId),
	...
	const browserOverride =
		projectSettings.projectAgentBrowserAccessOverrides[representative.id];
	const browserMixed = group.memberProjects.some((member, index) => {
		const settings = environmentById.get(member.environmentId)?.serverConfig
			?.settings;
		if (
			!settings ||
			!environmentById.get(representative.environmentId)?.serverConfig
		)
			return false;
		return (
		...
		if (selection !== null) {
			for (const member of group.memberProjects) {
				...
				if (
					...
					!entry.isAvailable ||
					!options?.some(
						(model) => model.slug === selection.model && !model.isUnavailable,
					)
				) {
					toastManager.add({
						type: "warning",
	...
	const autoPullOverridden = group.memberProjects.some(
		(member) =>
			member.autoPull ||
			environmentById.get(member.environmentId)?.serverConfig?.settings
				.projectAutoPullOverrides[member.id] !== undefined,
	);
	const mixedAutoPull = group.memberProjects.some((member) => {
		...
		return (
			settings &&
			resolveProjectAutoPull(settings, member.id, member.autoPull) !== autoPull
		);
	});
	...
	const keybindings =
		selectedServerConfig?.keybindings ?? DEFAULT_RESOLVED_KEYBINDINGS;
	...
	const scriptsInherited = projectScriptsInheritDefaults(
		scriptSettings,
		selectedCheckout,
	);
	...
	const inheritedEnvModeSource =
		t3File.file?.defaultThreadEnvMode != null ? "t3.json" : "global";
	...
	const importFileScript = useCallback(
		async (fileScript: T3ProjectFileScript) => {
			const payload: NewProjectScriptInput = {
				...
				autoOpenPreview: fileScript.previewUrl
					? (fileScript.autoOpenPreview ?? false)
					: false,
			};
			...
			if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
				...
				setEditorRequest({
					...
					initial: payload,
					error:
						error instanceof Error ? error.message : "Failed to import action.",
				});
			}
		},
	...
	const removeMembers = useCallback(
		async (members: ReadonlyArray<SidebarProjectGroupMember>) => {
			...
			const targetKind =
				hasOtherMembers || !isWholeGroup ? "checkout" : "project";
		...
		<>
			<SettingsPageContainer className="gap-6">
				<SettingsSection title="Project" hideTitle>
					...
					<SettingsRow
						...
						resetAction={
							group.memberProjects.some(
								(member) => member.defaultModelSelection !== null,
							) ? (
								<SettingResetButton
					...
					<SettingsRow
						...
						resetAction={
							group.memberProjects.some(
								(member) => member.defaultThreadEnvMode !== null,
							) ? (
								<SettingResetButton
					...
					<SettingsRow
						...
						control={
							...
							>
								<SelectTrigger
									size="sm"
									aria-label={`Grouping rule for ${selectedCheckoutLabel}`}
								>
									<SelectValue>
										{selectedCheckoutGrouping === "inherit"
									...
									<MenuPopup align="end" className="w-72">
										<MenuGroup>
											...
											<p className="px-2 pb-2 text-pretty text-sm text-muted-foreground">
												Add actions declared by this checkout without editing
												them first.
											</p>
										</MenuGroup>
										<MenuSeparator />
			...
			<ProjectFaviconPickerDialog
				...
				{...(pickProjectFavicon
					? {
							onPickExternal: () =>
								pickProjectFavicon(representative.workspaceRoot),
						}
					: {})}
				onSelect={(path) =>
					void setProjectIcon({ faviconPath: path, projectIcon: null })
				}
