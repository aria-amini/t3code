...
function ScopePicker({
	...
	const items = [
		{ value: "all", label: `All ${label}s`, icon: allIcon },
		...options,
	];
	return (
		<Combobox
		...
		>
			<ComboboxTrigger
				...
				className={cn(
					selectTriggerVariants({ size: "compact" }),
					"w-auto min-w-0 max-w-52",
				)}
			>
				<span className="flex min-w-0 items-center gap-1.5">
					...
					<span className="truncate">
						{value === null
							? `All ${label}s`
							: (selected?.label ?? `Unavailable ${label}`)}
					</span>
				</span>
...
export function ProjectsSettings({
	...
	const machine = environments.find(
		(environment) => environment.environmentId === machineId,
	);
						...
						{environments.length > 3 ? (
							...
							<ToggleGroup
								...
								onValueChange={(next) => {
									const value = next[0];
									if (value)
										onScopeChange(projectKey, value === "all" ? null : value);
								}}
							>
