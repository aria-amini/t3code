...
const nextEnvironmentVariableDraftId = () =>
	`provider-env-${environmentVariableDraftId++}`;
...
export function deriveProviderModelsForDisplay(input: {
	...
	const liveCustomModelsBySlug = new Map(
		Arr.filterMap(input.liveModels ?? [], (model) =>
			model.isCustom
				? Result.succeed([model.slug, model] as const)
				: Result.failVoid,
		),
	);
	const serverModels =
		input.liveModels?.filter((model) => !model.isCustom) ?? [];
	const customModels = input.customModels.map((entry) => ({
		...
		capabilities:
			entry.capabilities ??
			liveCustomModelsBySlug.get(entry.slug)?.capabilities ??
			null,
	}));
...
function ProviderEnvironmentSection(props: {
	...
	const updateVariable = (
		id: string,
		patch: Partial<Omit<EnvironmentDraftRow, "id">>,
	) => {
			...
			<div className="flex min-h-[1.875rem] flex-wrap items-center justify-end gap-x-3 gap-y-1">
				{rows.length > 0 ? (
					<span className="mr-auto text-xs text-muted-foreground">
						Sensitive values are stored separately and never returned to the
						app.
					</span>
				) : null}
				<Button
...
export function ProviderInstanceCard({
	...
	// verbatim behaviour.
	const driverKind: ProviderDriverKind | null = isProviderDriverKind(
		instance.driver,
	)
		? instance.driver
		: null;
	...
	const updateEnvironment = (
		...
		const cleaned = environment.filter(
					...
					<SettingsRow
						...
						description={
							<span>
								...
								, which is not available in this build. Its configuration is
								preserved.
							</span>
						}
					/>
