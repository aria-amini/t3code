...
describe("ProviderSettingsForm helpers", () => {
	it("derives visible provider config fields from the client definition schema", () => {
		...
		expect(
			deriveProviderSettingsFields(codex!).map((field) => field.key),
		).toEqual(["binaryPath", "homePath", "shadowHomePath", "launchArgs"]);
	});

	it("sources labels and descriptions from schema annotations", () => {
		const opencode =
			DRIVER_OPTION_BY_VALUE[ProviderDriverKind.make("opencode")];
	...
	it("derives a select control with its choices for the Antigravity sign-in method", () => {
		const antigravity =
			DRIVER_OPTION_BY_VALUE[ProviderDriverKind.make("antigravity")];
		...
		expect(fields.find((field) => field.key === "apiKey")?.control).toBe(
			"password",
		);
	});
	...
	it("shows the auto-compaction threshold for Claude providers", () => {
		const claude =
			DRIVER_OPTION_BY_VALUE[ProviderDriverKind.make("claudeAgent")];
		expect(claude).toBeDefined();

		expect(
			deriveProviderSettingsFields(claude!).map((field) => field.key),
		).toEqual(["binaryPath", "homePath", "autoCompactWindow", "launchArgs"]);
	});
