...
function readFieldAnnotations(
	...
	return (
		Schema.resolveAnnotationsKey(fieldSchema) ??
...
function readProviderSettingsFormSchemaAnnotation(
	...
	return (
		Schema.resolveAnnotations(definition.settingsSchema)
...
function readFieldBooleanDefault(
	...
	const decodeDefault = Schema.decodeUnknownOption(
		fieldSchema as Schema.Decoder<unknown>,
	);
	const decoded = decodeDefault(undefined);
	return Option.isSome(decoded) && typeof decoded.value === "boolean"
		? decoded.value
		: undefined;
}
...
export function deriveProviderSettingsFields(
	...
	return Object.keys(definition.settingsSchema.fields)
		...
		.flatMap(({ key }) => {
			...
			const annotatedDescription = readFieldAnnotationString(
				fieldSchema,
				"description",
			);
			return [
	...
	if (variant === "settings") {
		...
		const control =
			field.control === "switch" ? (
				<Switch
					checked={readProviderConfigBoolean(
						value,
						field.key,
					...
					onCheckedChange={(checked) =>
						onChange(
							nextProviderConfigWithFieldValue(value, field, Boolean(checked)),
						)
					}
					aria-label={field.label}
