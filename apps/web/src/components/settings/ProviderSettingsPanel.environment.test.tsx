	...
	return (
		...
		children.some(
			(child) =>
				typeof child === "object" &&
				child !== null &&
				(child as ReactElement<Record<string, unknown>>).props?.className ===
					"sr-only" &&
...
describe("EnvironmentProviderSettings routing", () => {
	...
	it("opens the requested provider instance instead of the first provider", () => {
		...
		const editor = visitElements(
			panel,
			(element) => element.props.mode === "editor",
		);
	...
	it("does not substitute another account when the requested instance was removed", () => {
		...
		const panel = renderPanel({ targetInstanceId: customId });
		expect(
			visitElements(panel, (element) => element.props.mode === "editor"),
		).toBeNull();
	...
	it("keeps provider selection available while write controls are read only", () => {
		...
		const inertWrapper = visitElements(
			panel,
			(element) => element.props.inert === true,
		);
		...
		const customRow = visitElements(
			panel,
			(element) =>
				element.props.instanceId === customId && element.props.mode === "list",
		);
		...
		const customEditor = visitElements(
			panel,
			(element) =>
				element.props.instanceId === customId &&
		...
		const notice = visitElements(
			panel,
			(element) => element.props.title === "Limited permissions",
		);
	...
	it("keeps the editable layout interactive when not read only", () => {
		...
		const panel = renderPanel();
		expect(
			visitElements(panel, (element) => element.props.inert === true),
		).toBeNull();
		expect(
			visitElements(
				panel,
	...
	it("keeps Advanced visible when search targets the provider health interval", () => {
		...
		expect(
			visitElements(panel, (element) => element.props.title === "Advanced"),
		).not.toBeNull();
		expect(
			visitElements(
				panel,
	...
	it("deletes and resets provider configuration without erasing shared preferences", () => {
		...
		const defaultRow = visitElements(
			panel,
			(element) =>
				element.props.instanceId === codexId && element.props.mode === "list",
		);
		...
		const defaultCard = visitElements(
			panel,
			(element) =>
				element.props.instanceId === codexId && element.props.mode === "editor",
		);
		...
		const resetPatch = settingsState.updateSettings.mock.lastCall?.[0] as
			...
			| undefined;
		expect(Object.keys(resetPatch ?? {}).sort()).toEqual([
			"providerInstances",
			"providers",
		]);
