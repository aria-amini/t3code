...
const providerCardHeightClassName =
	"lg:h-[min(44rem,calc(100dvh-11rem))] lg:min-h-[32rem]";
...
function ProviderSettingsPlaceholder({
	...
	return (
		...
		>
			{deviceTabs ? (
				<div className="flex min-h-11 min-w-0 items-center px-3 sm:px-4">
					{deviceTabs}
				</div>
			) : null}
			<div
	...
	const deviceTabs =
		!onlyPrimaryDevice && options.length > 0 ? (
			<ScrollArea
				hideScrollbars
				scrollFade
				className="h-11 min-w-0 flex-1 rounded-none"
			>
				<ToggleGroup
					aria-label="Devices"
					variant="segmented"
					...
					onValueChange={(next) => {
						const environment = options.find(
							(option) => option.environmentId === next[0],
						);
						if (environment)
		...
		<>
			{targetEnvironmentMissing ? (
				<ProviderSettingsPlaceholder
					...
					icon={
						<EnvironmentMachineIcon
							kind={resolveEnvironmentMachineKind(null)}
						/>
					}
...
export function EnvironmentProviderSettings({
	...
	const serverProviders =
		useAtomValue(serverEnvironment.providersValueAtom(environmentId)) ??
	...
	const [selectedInstanceId, setSelectedInstanceId] =
		useState<ProviderInstanceId | null>(targetInstanceId ?? null);
	const [updatingProviderInstanceIds, setUpdatingProviderInstanceIds] =
		useState<ReadonlySet<ProviderInstanceId>>(() => new Set());
	...
	const defaultProviderHealthRefreshIntervalSeconds =
		durationToSeconds(providerHealthPreset);
	const lastCheckedAt =
	...
	const runProviderUpdate = useCallback(
		async (candidate: ProviderSettingsUpdateCandidate) => {
			...
			setUpdatingProviderInstanceIds((previous) =>
				new Set(previous).add(candidate.instanceId),
			);
	...
	const renderProviderInstance = (
		...
		const updateCandidate = providerUpdateCandidateByInstanceId.get(
			row.instanceId,
		);
		...
		const canRunInlineUpdate =
			updateCandidate !== undefined && !isInstanceUpdateRunning;
		...
		return (
			<ProviderInstanceCard
				...
				onFavoriteModelsChange={(next) =>
					updateProviderFavoriteModels(row.instanceId, next)
				}
				...
				isUpdating={
					mode === "editor" && showInlineUpdateButton
						? isInstanceUpdateRunning
						: undefined
				}
			/>
		);
				...
				<SettingsRow
					...
					title={
						<span className="inline-flex items-center gap-1.5">
							...
							<PolicyTooltip>
								This interval is configured here, then the shared Background
								activity policy decides whether provider probes may run when the
								timer fires. Custom intervals appear as Advanced in General
								settings.
							</PolicyTooltip>
						</span>
					}
