...
function authState(patch: Partial<ProviderAuthState> = {}): ProviderAuthState {
	return {
		...
		flowId: "flow-1",
		authorizationUrl:
			"https://accounts.google.com/o/oauth2/v2/auth?state=test-only",
...
function countElements(
	...
	if (Array.isArray(node)) {
		return node.reduce(
			(total, child) => total + countElements(child, predicate),
			0,
		);
	}
...
describe("Antigravity setup", () => {
	...
	it("offers sign-in again when credentials expire after a completed auth flow", () => {
		...
		expect(
			visitElements(
				expired,
				(element) => element.props.children === "Google sign-in complete.",
			),
		).toBeNull();
	});
	...
	it("does not send a callback left over from a replaced sign-in flow", async () => {
		setCallback(
			renderSetup(),
			"http://127.0.0.1:5555/?state=old-flow&code=test-only",
		);
	...
	it("coalesces repeated sign-in clicks while start is pending", async () => {
		...
		let completeStart: (value: {
			_tag: "Success";
			value: undefined;
		}) => void = () => {
			throw new Error("Missing start resolver.");
		};
		const pending = new Promise<{ _tag: "Success"; value: undefined }>(
			(resolve) => {
	...
	it("shows a repeated runtime status message only once", () => {
		...
		expect(
			countElements(
				view,
				(element) =>
					element.props.children === "Checking the downloaded runtime.",
			),
		).toBe(1);
	});
	...
	it.each(["read-only", "older-server"] as const)(
		...
		(mode) => {
			...
			renderSetup(
				mode === "read-only" ? { readOnly: true } : { provider: olderProvider },
			);
