...
export function readAntigravityAuthMethod(
	config: unknown,
	...
	const value =
		config !== null && typeof config === "object" && "authMethod" in config
			...
			: undefined;
	return (
		ANTIGRAVITY_AUTH_METHODS.find((method) => method.value === value)?.value ??
		"oauth-personal"
	);
}
	...
	return (
		<section aria-label="Antigravity setup" className="grid gap-3 text-xs">
			...
			{props.readOnly ? (
				...
				<p className="text-muted-foreground">
					Update this environment to install Antigravity and sign in with Google
					here.
				</p>
			) : (
...
> & {
	...
	const usesBrowser =
		authMethod === "oauth-personal" || authMethod === "oauth-business";
	const phaseLabels = usesBrowser ? AUTH_PHASE_LABELS : CREDENTIAL_PHASE_LABELS;
	const methodLabel =
		ANTIGRAVITY_AUTH_METHODS.find((method) => method.value === authMethod)
	...
	const startAuth = useAtomCommand(
		serverEnvironment.startProviderAuth,
		commandOptions,
	);
	...
	const cancelAuth = useAtomCommand(
		serverEnvironment.cancelProviderAuth,
		commandOptions,
	);
	const logoutAuth = useAtomCommand(
		serverEnvironment.logoutProviderAuth,
		commandOptions,
	);
	...
	const callbackUrl =
		callbackDraft.flowId === auth?.flowId ? callbackDraft.value : "";
	const authActive =
		auth?.phase === "starting" ||
		auth?.phase === "waiting" ||
	...
	const installed =
		provider.installed ||
		(!usesCustomBinary && installation?.installedVersion != null);
	...
	const authorizationUrl =
		auth?.phase === "waiting" ? auth.authorizationUrl : null;
	...
	return (
		<div className="grid gap-3">
			<div className="grid gap-2">
				...
				{usesCustomBinary ? (
					<p className="text-muted-foreground">
						This instance uses the binary path below. Installing a managed
						runtime does not change that path.
					</p>
				) : null}
				{!installed &&
				!usesCustomBinary &&
				...
				installation?.totalBytes ? (
					<p className="text-muted-foreground">
						Downloads {Math.ceil(installation.totalBytes / 1_000_000)} MB from
						Google.
					</p>
				) : null}
				{!installed && !provider.setup?.canInstall ? (
					<p className="text-muted-foreground">
						Automatic installation is unavailable here. Set an existing binary
						path below or use a supported remote environment.
					</p>
				) : null}
				<div className="flex flex-wrap gap-2">
					{installActive && installation.operationId ? (
						...
						<Button
							...
							onClick={() =>
								void runCommand("Starting installation", () =>
									startInstall(target),
								)
							}
						>
					...
					<>
						...
						>
							<label htmlFor={`provider-callback-${instanceId}`}>
								If the final localhost page does not load, paste its full URL
								here.
							</label>
							<Input
					...
					{authActive && auth?.flowId ? (
					  ...
					  provider.setup?.canAuthenticate ? (
						<Button
							...
							variant="outline"
							disabled={
								actionsDisabled || !installed || auth === null || installActive
							}
							onClick={() =>
