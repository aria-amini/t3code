...
describe("shouldShowResourceMonitorRetry", () => {
	...
	it("does not show retry for an initial load without an error or a healthy snapshot", () => {
		expect(
			shouldShowResourceMonitorRetry({ nativeStatus: null, error: null }),
		).toBe(false);
		expect(
			shouldShowResourceMonitorRetry({ nativeStatus: "healthy", error: null }),
		).toBe(false);
	});
});
...
describe("visibleResourceTelemetryProcesses", () => {
	it("hides descendants by parentage even when rows are not depth-first", () => {
		...
		expect(
			visibleResourceTelemetryProcesses(processes, new Set(["1:1000"])),
		).toEqual([processes[0], processes[1], processes[3]]);
	});
});
