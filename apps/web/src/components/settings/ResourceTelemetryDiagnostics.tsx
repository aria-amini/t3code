...
function processSummaryIdentityKey(
	process: ResourceTelemetryProcessSummary,
): string {
	return `${process.identity.pid}:${process.identity.startTimeMs}`;
}
...
function formatProcessName(
	...
	return (
		normalized.split(/[\\/]/).findLast((segment) => segment.length > 0) ??
		normalized
	);
}
...
function IconStat({
	...
	return (
		<div className="group min-w-0 px-4 py-4 sm:px-5">
			...
			{detail ? (
				<div className="mt-1.5 truncate text-[10px] text-muted-foreground/60">
					{detail}
				</div>
			) : null}
		</div>
...
function AggregateCard({
	...
	return (
		<div className="relative overflow-hidden border-t border-border/60 px-4 py-4 first:border-t-0 md:border-t-0 md:border-l md:first:border-l-0 sm:px-5">
			<span
				className={cn(
					"absolute inset-x-5 top-0 h-0.5 rounded-full opacity-75",
					accentClass,
				)}
			/>
			<div className="flex items-center justify-between gap-3">
				...
				<div className="rounded-md bg-muted/55 px-1.5 py-0.5 font-mono text-[9px] tabular-nums text-muted-foreground/70">
					...
					{aggregate.processCount === 1 ? "process" : "processes"}
				</div>
			</div>
...
function HistoryWindowSelector({
	...
	return (
		<ToggleGroup
			...
			onValueChange={(next) => {
				const selected = HISTORY_WINDOWS.find(
...
function ResourceHistoryChart({
	...
	const maxIo = Math.max(
		1,
		...buckets.map((bucket) => bucket.ioReadBytes + bucket.ioWriteBytes),
	);
		...
		<div className="border-t border-border/60 px-4 py-4 sm:px-5">
			<div className="mb-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-muted-foreground/65">
				<span className="inline-flex items-center gap-1.5">
					<span className="h-1.5 w-3 rounded-full bg-foreground/70" /> CPU
					average
				</span>
				<span className="inline-flex items-center gap-1.5">
	...
	return (
		...
		>
			...
			<span
				className={cn(
					"size-1.5 rounded-full",
					categoryDotClass(process.category),
				)}
			/>
			<Tooltip>
				<TooltipTrigger
							...
							<td className="px-3 py-2 text-right font-mono tabular-nums text-muted-foreground">
								<Tooltip>
									...
									<TooltipPopup side="top">
										{ioSemanticsLabel(process.ioSemantics)}
									</TooltipPopup>
								</Tooltip>
							</td>
							<td className="px-3 py-2 text-right font-mono tabular-nums text-muted-foreground">
...
export function ResourceTelemetryDiagnostics() {
	...
	const selectedWindow =
		HISTORY_WINDOWS.find((option) => option.windowMs === windowMs) ??
		HISTORY_WINDOWS[1];
	...
	const [signalingKeys, setSignalingKeys] = useState<ReadonlySet<string>>(
		() => new Set(),
	);
	...
	const signalProcess = useCallback(
		async (process: ResourceTelemetryProcess, signal: ServerProcessSignal) => {
			...
			const nextSignalingKeys = new Set(signalingKeysRef.current).add(
				identityKey,
			);
	...
	const speedLimit = snapshot
		? Option.getOrNull(snapshot.speedLimitPercent)
		: null;
							...
							<TooltipTrigger
								render={
									...
									>
										<RefreshIcon
											className="size-3"
											refreshing={telemetry.isPending}
										/>
									</Button>
								}
							/>
				...
				<div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03),0_8px_30px_rgb(0_0_0/0.035)]">
					<div className="flex flex-col gap-3 border-b border-border/60 bg-linear-to-r from-muted/45 via-muted/20 to-transparent px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
						<div>
							...
							<p className="mt-1 max-w-xl text-xs leading-relaxed text-muted-foreground">
								Live native counters for the server, providers, terminals,
								desktop processes, and the monitor itself.
							</p>
						</div>
						<div className="flex items-center gap-2 text-[10px] text-muted-foreground/65">
							...
							Sampling every{" "}
							{snapshot
								? formatSampleInterval(snapshot.sampleIntervalMs)
								: "..."}
						</div>
					</div>
					<div className="grid grid-cols-2 divide-x divide-y divide-border/55 md:grid-cols-3">
						<IconStat
							...
							detail={
								allT3
									? `${formatCpuTime(allT3.cpuTimeMs)} observed CPU time`
									: undefined
							}
						/>
						<IconStat
							...
							detail={
								allT3
									? `${formatBytes(allT3.peakRssBytes)} combined process peaks`
									: undefined
							}
						/>
						<IconStat
							...
							detail={
								allT3
									? `${allT3.processStarts} starts · ${allT3.processExits} exits`
									: undefined
							}
						/>
						<IconStat
							...
							detail={
								allT3 ? `${formatBytes(allT3.ioReadBytes)} observed` : undefined
							}
						/>
						<IconStat
							...
							detail={
								allT3
									? `${formatBytes(allT3.ioWriteBytes)} observed`
									: undefined
							}
							tone={
						...
						<IconStat
							...
							detail={
								snapshot
									? `${snapshot.power.thermalState} thermal state`
									: undefined
							}
							tone={
								speedLimit !== null && speedLimit < 80 ? "warning" : "default"
							}
						/>
					</div>
					{telemetry.error ? (
						...
						{hasHostPowerSignal && snapshot ? (
							...
							<div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-4 py-5">
								...
								<p className="mt-1.5 max-w-sm text-[11px] leading-relaxed text-muted-foreground/70">
									Power, idle, lock, and thermal state are supplied by the
									desktop host. Process telemetry remains fully active in this
									browser session.
								</p>
							</div>
						)}
							...
							<>
								...
								<DetailRow
									...
									value={formatDurationMicros(
										snapshot.health.collectionDurationMicros,
									)}
								/>
								<DetailRow
									label="Process scan"
			...
			>
				<div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03)]">
					<div className="bg-muted/15 px-4 py-3 text-[11px] leading-relaxed text-muted-foreground sm:px-5">
						Native counters identify which process is reading or writing. These
						application-level counters identify known T3 operations so process
						spikes can be correlated with specific persistence and logging
						paths.
					</div>
