...
const SnapShotIcon = createLucideIcon("snap-shot", [
	...
	[
		"rect",
		{ width: "10", height: "8", x: "7", y: "8", rx: "2", key: "window" },
	],
...
const SETTINGS_PAGE_SECTIONS: Partial<
	Readonly<
		Record<SettingsPath, ReadonlyArray<{ label: string; targetId: string }>>
	>
> = {
	"/settings/general": [
...
export function SettingsSidebarNav({ pathname }: { pathname: string }) {
	...
	const [sectionVisibility, setSectionVisibility] =
		useState<SettingsSectionVisibilityState | null>(null);
	...
	const observedVisibilityScope = useMemo(() => {
		const path = SETTINGS_NAV_ITEMS.find(
			(item) =>
				...
				resolvedPathname?.startsWith(`${item.to}/`) === true,
		)?.to;
	...
	useEffect(() => {
		...
		return observeSettingsSectionVisibility({
			container,
			targetIds: observedVisibilityScope.pageSections.map(
	...
	const handlePageSectionClick = useCallback(
		(to: SettingsPath, targetId: string) => {
			...
			if (
				pathname === to &&
				scrollToSettingsTarget(targetId, { highlight: false })
			) {
				return;
			}
			void navigate({
													...
													{pageSections.map((section) => (
														<SidebarMenuSubItem key={section.targetId}>
															<SidebarMenuSubButton
																...
																size="sm"
																data-visible={visiblePageSectionIds.has(
																	section.targetId,
																)}
																className={cn(
