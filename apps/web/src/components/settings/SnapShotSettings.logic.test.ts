...
it.each([
	...
	[
		"camera-shutter",
		{ snapShotPlaySound: true, snapShotSound: "camera-shutter" },
	],
...
it("offers effects only with a capable GNOME extension, explaining how to upgrade v1", () => {
	...
	expect(
		snapShotFeedbackUnavailableMessage({ ...state, linuxBackend: "picker" }),
	).toContain("aren't available");
	expect(
		snapShotFeedbackUnavailableMessage({ ...state, mode: "direct" }),
	).toBeUndefined();
});
...
it("reports unavailable capture support without browser globals", () => {
	expect(snapShotUnavailableMessage(false)).toBe(
...
it("distinguishes Hyprland helper setup, action registration, and verified shortcut delivery", () => {
	...
	expect(snapShotStatus(state, false)).toBe(
		"Turn this on to set up snapshots.",
	);
	...
	expect(
		snapShotShortcutStatus({ ...ready, shortcutPending: true }),
	).not.toContain("permission");
	expect(
		snapShotStatus({ ...ready, shortcutActionRegistered: true }, true),
	).toBe("Use your shortcut from another app");
...
it("distinguishes saved shortcuts from observed delivery without making users repeat setup", () => {
	...
	expect(
		snapShotSetupButtonLabel({ ...state, shortcutRegistered: false }),
	).toBe("Manage capture");
	expect(
		snapShotSetupButtonLabel({
			...state,
	...
	expect(
		snapShotSetupSummary({ ...state, shortcutVerified: true }, false),
	).toContain("Enable capture");
	expect(
		snapShotSetupSummary(
			{
				...state,
...
it("keeps picker limitations visible after shortcut verification without recommending a GNOME extension", () => {
	...
	expect(
		snapShotFeedbackUnavailableMessage({ ...state, linuxBackend: "kde" }),
	).toContain("capture helper");
	expect(
		snapShotFeedbackUnavailableMessage({
			...state,
			linuxBackend: "kde",
		...
		snapShotStatus(
			{
				...state,
				linuxBackend: "kde",
				kdeHelper: { status: "not-installed", message: "Install" },
			},
			true,
		),
...
it("reports pending, denied, and assigned shortcuts without inferring consent from saved keys", () => {
	...
	expect(
		snapShotShortcutStatus({ ...approved, shortcutPending: true }),
	).toContain("Approve the shortcut permission prompt");
	expect(
		snapShotShortcutStatus({
			...approved,
			shortcutRegistered: false,
...
it("hides macOS setup only while permissions and the shortcut are all in place", () => {
	...
	expect(
		snapShotSetupComplete({ ...ready, macPermissions: undefined }, true),
	).toBe(false);
	expect(
		snapShotSetupComplete({ ...ready, shortcutRegistered: false }, true),
	).toBe(false);
	const revoked = {
		...ready,
	...
	expect(snapShotSetupComplete({ ...revoked, message: null }, false)).toBe(
		true,
	);
	expect(
		snapShotSetupComplete(
