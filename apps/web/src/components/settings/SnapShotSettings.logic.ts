...
export function snapShotStatus(
	...
	if (state.mode === "unavailable")
		return state.message ?? "Not supported on this platform.";
...
export function snapShotSetupSummary(
	...
	if (
		...
		state.hyprlandHelper?.status !== "ready"
	)
	...
	if (
		...
		state.gnomeExtension?.status !== "enabled"
	)
		return "Set up active-window snapshots";
	if (
		captureSetupBackend(state) === "kde" &&
		state.kdeHelper?.status !== "ready"
	)
	...
	if (state.shortcutRegistered)
		return state.shortcutLabel ? "Ready to capture" : "Shortcut saved";
...
export function snapShotShortcutStatus(
	...
	if (state.shortcutPending)
		return "Approve the shortcut permission prompt to continue.";
	if (state.shortcutRegistered)
		return state.mode === "portal" ? null : "Shortcut saved.";
...
export function snapShotFeedbackUnavailableMessage(
	...
	if (state?.mode !== "portal" || state.linuxFeedbackAvailable)
		return undefined;
	...
	if (state.linuxBackend === "niri")
		return "Capture effects aren't available on Niri.";
...
export function snapShotAccessibilityUnavailableMessage(
	...
	if (
		...
		state.linuxBackend === "screenshot-portal"
	)
...
export function snapShotSoundPatch(
	sound: SnapShotSoundSelection,
): ClientSettingsPatch {
	return sound === "off"
