...
it.each(["niri", "hyprland"] as const)(
	...
	async (desktop) => {
		...
		expect(
			visitElements(
				opened,
				(element) => element.type === CaptureShortcutConfig,
			),
		).toBeNull();
		...
		await (dialog!.props.onClose as (completed: boolean) => Promise<void>)(
			false,
		);
...
it.each(["direct", "gnome-extension", "kde"] as const)(
	...
	async (backend) => {
		state = {
			...
			shortcutLabel:
				backend === "direct" ? undefined : "Press <Shift><Control>2",
			shortcutMessage:
				backend === "direct"
					? null
					: "Desktop shortcut: Press <Shift><Control>2",
		};
		const tree = await mount();
		...
		if (backend !== "direct") {
			...
			const keycaps = visitElements(
				tree,
				(element) => element.type === SnapShotShortcutKeys,
			);
			...
			expect(
				visitElements(
					tree,
					(element) => element.props.id === "snap-shot-shortcut",
				)?.props.status,
			).toBeNull();
		}
		(recorder(tree).onClick as () => void)();
		await finish(
			bridge.setSnapShotShortcutSuppressed.mock.results.at(-1)!.value,
		);
		...
		expect(recorder(render())["aria-label"]).toBe(
			"Record snapshot shortcut, currently Ctrl+Alt+Y",
		);
...
it("turns capture on directly on Windows without opening setup", async () => {
	...
	expect(
		visitElements(
			renderWithEffects(),
			(element) => element.props.children === "Manage capture",
		),
	).toBeNull();
});
...
it("keeps the approved desktop shortcut when recording is cancelled in setup", async () => {
	...
	expect(shortcut()["aria-label"]).toBe(
		"Record snapshot shortcut, currently Ctrl+Alt+8",
	);
...
it("shows permission errors once in a toast and allows retrying", async () => {
	...
	bridge.setupSnapShot.mockRejectedValueOnce(
		new Error("The desktop service disconnected."),
	);
	button(await mount(), "Shortcut permissions").onClick();
	await finish(
		bridge.setupSnapShot.mock.results[0]!.value.catch(() => undefined),
	);
	...
	expect(
		visitElements(tree, (element) => element.props.role === "alert"),
	).toBeNull();
...
it("keeps a failed preference unchanged and reports the save error in a toast", async () => {
	...
	settingsStore.update.mockRejectedValueOnce(
		new Error("The settings file is read-only."),
	);
	flash(await mount()).onCheckedChange(false);
	await finish(
		settingsStore.update.mock.results[0]!.value.catch(() => undefined),
	);
...
it("keeps setup errors in the wizard and does not toast them after closing it", async () => {
	...
	const action = wizard(render())!.props.onAction as (
...
it.each([false, true])(
	...
	async (completed) => {
		const { readSnapShotSetupResume } =
			await import("../../lib/snapShotSetupResume");
		...
		await (
			wizard(render())!.props.onAction as (action: string) => Promise<void>
		...
		await (resumed!.props.onClose as (completed: boolean) => Promise<void>)(
			completed,
		);
		expect(wizard(render())).toBeNull();
...
it("requires a successful macOS test capture before enabling and allows retry", async () => {
	...
	const enable = () =>
		(wizard(render())!.props.onEnable as () => Promise<boolean>)();
	...
	expect(bridge.setupSnapShot.mock.calls).toEqual([
		["test-mac-capture"],
		["test-mac-capture"],
	]);
	expect(settingsStore.current.snapShotEnabled).toBe(true);
