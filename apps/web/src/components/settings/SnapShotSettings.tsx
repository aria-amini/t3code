...
export function SnapShotSettings() {
	...
	const [setupError, setSetupError] = useState<ReturnType<
		typeof captureSettingsError
	> | null>(null);
	const [wizard, setWizard] = useState<{
		initialStep: CaptureSetupStep;
		wasEnabled: boolean;
	} | null>(null);
	const [candidate, setCandidate] = useState<SnapShotShortcut>(
		settings.snapShotShortcut,
	);
	...
	const captureAvailable =
		Boolean(bridge) && state !== null && state.mode !== "unavailable";
	const feedbackUnavailable = snapShotFeedbackUnavailableMessage(state);
	const savedShortcut = settings.snapShotShortcut;
	const managedShortcut =
		state?.linuxBackend === "niri" || state?.linuxBackend === "hyprland";
	const shortcutChanged =
		!managedShortcut && !sameSnapShotShortcut(candidate, savedShortcut);
	...
	const soundSelection = settings.snapShotPlaySound
		? settings.snapShotSound
		: "off";
	const soundLabel =
		soundSelection === "off"
			? "Off"
	...
	const refreshState = useCallback(async () => {
		...
		try {
			...
			if (requestId === stateRequestIdRef.current)
				setSetupError(
					captureSettingsError("Couldn't check capture setup", error),
				);
		}
	}, [bridge]);
	...
	const save = useCallback(
		async (patch: ClientSettingsPatch) => {
			...
			try {
				...
				setSetupError(
					captureSettingsError("Couldn't save capture settings", error),
				);
			}
		},
	...
	const saveIncludeAccessibility = useCallback(
		async (includeAccessibility: boolean) => {
			try {
				...
				setSetupError(
					captureSettingsError("Couldn't allow app text capture", error),
				);
			}
		},
		...
		try {
			if (state?.macPermissions) {
				...
				if (!bridge?.setupSnapShot)
					throw new Error("Restart T3 Code to finish capture setup.");
				await bridge.setupSnapShot("test-mac-capture");
			}
			if (state?.mode === "direct")
				await bridge?.requestSnapShotPermissions(
			...
			setSetupError(
				captureSettingsError("Couldn't verify capture access", error),
			);
			return false;
		} finally {
			setSetupBusy(false);
	...
	const closeSetup = async (completed: boolean) => {
		...
		try {
			...
			setSetupError(
				captureSettingsError("Couldn't close capture setup", error),
			);
		} finally {
			setSetupBusy(false);
		}
								...
								<Switch
									...
									onCheckedChange={(checked) => {
										if (!checked) void save({ snapShotEnabled: false });
											...
											void save({ snapShotEnabled: true });
										else void openSetup();
									}}
								/>
							</>
						...
						<>
							<SettingsRow
								...
								control={
									<Switch
										...
										disabled={
											!captureAvailable ||
											Boolean(snapShotAccessibilityUnavailableMessage(state))
										}
										aria-label="Include app text in snapshots"
										onCheckedChange={(checked) =>
											void saveIncludeAccessibility(checked)
										}
									/>
								}
							/>
									...
									<Menu>
										...
										<MenuPopup align="end">
											<MenuRadioGroup
												onValueChange={(value) =>
													void save(
														snapShotSoundPatch(value as SnapShotSoundSelection),
													)
												}
							...
							<SettingsRow
								...
								control={
									<Switch
										...
										onCheckedChange={(checked) =>
											void save({ snapShotFlash: checked })
										}
									/>
								}
							/>
							<SettingsRow
								...
								control={
									<Switch
										checked={
											!feedbackUnavailable && settings.snapShotAnimations
										}
										disabled={!captureAvailable || Boolean(feedbackUnavailable)}
										aria-label="Animate snapshots"
										onCheckedChange={(checked) =>
											void save({ snapShotAnimations: checked })
										}
									/>
								}
							/>
