...
it("derives Hyprland setup from its own helper and registered action, not old GNOME approval", () => {
	...
	expect(
		captureSetupShortcutReady({ ...installed, shortcutPending: true }, false),
	).toBe(false);
	expect(
		captureSetupShortcutReady(
			{ ...installed, shortcutActionRegistered: true },
			false,
		),
	).toBe(true);
	expect(
		captureSetupAccessReady({ ...installed, message: "Check access" }),
	).toBe(false);
});
...
it("does not declare access ready before the enabled extension's capture endpoint is available", () => {
	expect(captureSetupAccessReady({ ...gnome, linuxBackend: "picker" })).toBe(
		false,
	);
...
it("resumes after login from real extension state without an onboarding-completed flag", () => {
	...
	expect(captureSetupInitialStep({ ...gnome, shortcutRegistered: false })).toBe(
		"shortcut",
	);
...
it("finishes setup with a saved shortcut without requiring a separate delivery test", () => {
	expect(captureSetupInitialStep({ ...gnome, shortcutVerified: false })).toBe(
		"shortcut",
	);
...
it("resumes KDE setup after the helper passes its real permission check", () => {
	...
	expect(captureSetupInitialStep({ ...state, shortcutRegistered: false })).toBe(
		"shortcut",
	);
});
...
it("does not report a successful check when capture support could not be read", () => {
	expect(
		captureSetupCheckMessage({ ...gnome, message: "Desktop disconnected" }),
	...
	expect(
		captureSetupCheckMessage({
			...
			gnomeExtension: {
				status: "error",
				message: "Could not read extension state",
			},
		}),
	).toContain("Still unable to check access");
});
...
it("requires saving a changed chord before finishing setup", () => {
	...
	expect(captureSetupShortcutReady(gnome, false)).toBe(true);
	expect(
		captureSetupShortcutReady({ ...gnome, shortcutRegistered: false }, false),
	).toBe(false);
});
...
it("allows leaving setup while the desktop prompt is pending but never after an explicit denial", () => {
	...
	expect(
		captureSetupShortcutReady({ ...pending, shortcutPending: false }, false),
	).toBe(false);
});
...
it("blocks finishing if desktop access is lost during the wizard", () => {
	...
	expect(captureSetupAccessReady({ ...gnome, mode: "unavailable" })).toBe(
		false,
	);
});
...
it.each([
	...
	(wasEnabled, completed, disable) => {
		expect(captureSetupShouldDisableOnClose(wasEnabled, completed)).toBe(
			disable,
		);
	},
);
...
it("gates Continue on macOS permissions, requiring accessibility only when app text is on", () => {
	...
	expect(
		captureSetupMacPermissionsReady(
			{ ...mac, macPermissions: undefined },
			true,
		),
	).toBe(true);
});

it.each(["kde", "hyprland"] as const)(
	"ignores errors from inactive helpers on %s",
	(backend) => {
		const state: DesktopSnapShotState = {
			...gnome,
			linuxBackend: backend,
			kdeHelper: {
				status: backend === "kde" ? "ready" : "error",
				message: "KDE",
			},
			hyprlandHelper: {
				status: backend === "hyprland" ? "ready" : "error",
				message: "Hyprland",
			},
		};
		expect(captureSetupAccessReady(state)).toBe(true);
