...
export function captureSetupDesktopName(
...
export function captureSetupBackend(state: DesktopSnapShotState) {
	...
	if (
		...
		state.gnomeExtension?.status === "unsupported"
	)
		return "picker";
	if (state.linuxBackend === "gnome-extension" || state.gnomeExtension)
		return "gnome";
	return "picker";
}
...
export function captureSetupAccessReady(state: DesktopSnapShotState): boolean {
	...
	if (captureSetupBackend(state) === "gnome")
		return (
			...
			state.linuxBackend === "gnome-extension"
		);
	if (captureSetupBackend(state) === "kde")
		return state.kdeHelper?.status === "ready";
	if (captureSetupBackend(state) === "hyprland")
...
export function captureSetupCheckMessage(state: DesktopSnapShotState): string {
	...
	if (captureSetupBackend(state) === "picker")
		return "Ready. You'll choose a window each time.";
...
export function captureSetupShortcutReady(
	...
	if (state.linuxBackend === "hyprland")
		return Boolean(state.shortcutActionRegistered);
...
export function captureSetupShouldDisableOnClose(
	wasEnabled: boolean,
	completed: boolean,
): boolean {
