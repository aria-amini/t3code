...
export function SnapShotSetupDialog({
	...
	const [step, setStep] = useState(() =>
		captureSetupInitialStep(state, initialStep),
	);
	...
	const helper =
		backend === "hyprland" ? state.hyprlandHelper : state.kdeHelper;
	const helperBackend = backend === "kde" || backend === "hyprland";
	const installHelper =
		backend === "hyprland" ? "install-hyprland-helper" : "install-kde-helper";
	const removeHelper =
		backend === "hyprland" ? "remove-hyprland-helper" : "remove-kde-helper";
	const accessReady = captureSetupAccessReady(state);
	const macPermissions = state.macPermissions;
	const macPermissionsReady = captureSetupMacPermissionsReady(
		state,
		includeAccessibility,
	);
		...
		>
			<DialogPopup className="max-w-xl" showCloseButton={!busy}>
				<DialogHeader>
					...
					<WizardSteps
						steps={SETUP_STEPS.map((item, index) => ({
							...item,
					...
					<div className="space-y-4 text-sm">
						...
						{step === "access" ? (
							<>
								...
								>
									{checked && !busy && !error
										? captureSetupCheckMessage(state)
										: null}
								</p>
								{macPermissions ? (
					...
					{step === "access" ? (
						...
						<Button
							disabled={
								busy ||
								!accessReady ||
								(shortcutChanged ? !canSaveShortcut : !shortcutReady)
							}
							onClick={async () => {
								if (!shortcutChanged || (await onSaveShortcut()))
									await onClose(true);
							}}
						>
