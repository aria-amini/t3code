...
function ThemeColorPickerPanel({
	...
	const updateFromPlane = useCallback(
		(event: PointerEvent<HTMLDivElement>) => {
			...
			const saturation = clampThemeColor(
				(event.clientX - bounds.left) / bounds.width,
			);
			const value =
				1 - clampThemeColor((event.clientY - bounds.top) / bounds.height);
	...
	const updateFromHue = useCallback(
		(event: PointerEvent<HTMLDivElement>) => {
			const bounds = event.currentTarget.getBoundingClientRect();
			const hue =
				clampThemeColor((event.clientX - bounds.left) / bounds.width) * 360;
			commitHsv({ ...hsv, h: hue });
		},
		[commitHsv, hsv],
	);
