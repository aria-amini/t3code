...
vi.mock("./themeEditorStore", () => ({
	useThemeEditorStore: (select: (store: typeof state) => unknown) =>
		select(state),
}));
vi.mock("../ui/toast", () => ({
...
describe("ThemeEditorHost", () => {
	...
	it.each(["editingTheme", "seedTheme"] as const)(
		...
		(field) => {
			...
			expect(
				themeColorToHex(renderEditor()?.[field]?.colors.accent ?? ""),
			).toBe("#1f6e4a");

			updateCustomTheme({
				...theme,
				colors: { ...theme.colors, accent: "#7241b8" },
			});
			expect(
				themeColorToHex(getThemeDefinition(theme.id)?.colors.accent ?? ""),
			).toBe("#7241b8");
			state.session = null;
	...
	it.each(["editingTheme", "seedTheme"] as const)(
		...
		(field) => {
			...
			expect(themeColorToHex(editor?.[field]?.colors.accent ?? "")).toBe(
				"#7241b8",
			);
		},
	);
