...
function highlightJson(value: string): string {
	...
	for (const match of value.matchAll(tokenPattern)) {
		...
		let tokenClass =
			"text-[var(--app-theme-secondary-foreground,var(--color-amber-600))]";
		if (token.startsWith('"')) {
			...
			tokenClass =
				"text-[var(--app-theme-accent-surface-foreground,var(--color-violet-600))]";
		}
...
export function ThemeImportDialog({
	...
	const [conflicts, setConflicts] =
		useState<ReadonlyArray<ThemeDefinition> | null>(null);
	...
	const readThemeBatch = useCallback(
		async (files: ReadonlyArray<ImportableThemeFile>) => {
			...
			try {
				for (const file of files) {
					...
					try {
						...
						parsed.push({
							...
							theme: isVsCodeThemeFile(value)
								? parseVsCodeThemeFile(value)
								: parseThemeFile(value),
						});
					} catch (cause) {
						failures.push(
				...
				for (const theme of pairVsCodeThemes(
					resolveThemeLabelCollisions(parsed),
				)) {
	...
	const versionedCopy = (
		...
		if (
			preferredName &&
			preferredName.toLowerCase() !== theme.label.toLowerCase()
		) {
			...
			if (!getCustomThemes().some((existing) => existing.id === candidate.id))
				return candidate;
		}
		...
		(mode: "update" | "copy") => {
			...
			for (const theme of conflicts) {
				try {
					...
					failures.push(
						`${theme.label}: ${cause instanceof Error ? cause.message : "failed"}`,
					);
				}
			}
			if (resolved.length > 0)
				onImportedMany(resolved, { updated: mode === "update" });
			setConflicts(null);
	...
	const handleSubmit = useCallback(() => {
		...
		try {
			...
			setError(
				cause instanceof Error ? cause.message : "That theme file is invalid.",
			);
		}
							...
							return (
								<div className="space-y-3">
									...
									<div className="flex flex-wrap items-center gap-2">
										<Button
											size="sm"
											onClick={() => resolveConflicts("update")}
										>
											Update existing
										</Button>
										<Button
