...
export function ThemeSearchSection({
	...
	const [results, setResults] =
		useState<ReadonlyArray<OpenVsxThemeExtension> | null>(null);
	const [error, setError] = useState<string | null>(null);
	const [isSearching, setIsSearching] = useState(false);
	const [installingId, setInstallingId] = useState<string | null>(null);
	const [pendingUpdate, setPendingUpdate] =
		useState<OpenVsxThemeExtension | null>(null);
		...
		async (searchText: string, nextSort = sortBy) => {
			...
			try {
				...
				if (!controller.signal.aborted) {
					...
					lastSearchKeyRef.current = null;
					setError(
						cause instanceof Error ? cause.message : "Open VSX search failed.",
					);
				}
			}
	...
	const handleInstall = useCallback(
		async (extension: OpenVsxThemeExtension, allowUpdate: boolean) => {
			...
			try {
				installedCollection = getStoredCustomThemeCollection(
				...
				setError(
					cause instanceof Error
						? cause.message
						: "Installed themes could not be read.",
				);
				return;
			}
			...
			try {
				const themes = await importOpenVsxThemeExtension(
					extension,
					controller.signal,
				);
				if (!controller.signal.aborted) {
					const imported = replaceCustomThemeCollection(
						extension.collectionId,
						themes,
						{
									...
									<div className="flex min-w-0 gap-3">
										...
										<div className="min-w-0 flex-1">
											...
											<p className="truncate text-muted-foreground text-xs">
												{extension.publisher} ·{" "}
												{DOWNLOAD_FORMAT.format(extension.downloadCount)}{" "}
												downloads
											</p>
										</div>
									</div>
			...
			>
				<AlertDialogPopup>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							This replaces its installed variants, including any local edits.
							Variants no longer in the extension will be removed.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
