...
describe("wizard target selection", () => {
	...
	it("rejects an existing target that is no longer listed", () => {
		expect(
			resolveWizardTarget(
				{ kind: "existing", profileId: "removed" },
				"generated",
...
const source = (
	over: Partial<BrowserImportSource> = {},
): BrowserImportSource => ({
	id: "helium",
	name: "Helium",
...
describe("initialWizardStep", () => {
	it("opens on the quit screen when the browser is running", () => {
		expect(
			initialWizardStep(source({ unavailable: "browserRunning" })),
		).toEqual({ step: "quit" });
	});
	...
	it("asks for Full Disk Access before choosing an import target", () => {
		expect(
			initialWizardStep(
				source({ profiles: [], unavailable: "needsFullDiskAccess" }),
			),
		).toEqual({
			step: "fullDiskAccess",
			resume: "configure",
	...
	it("blocks on a reason nothing local can fix", () => {
		expect(
			initialWizardStep(source({ unavailable: "unsupportedPlatform" })),
		).toEqual({
			step: "blocked",
	...
	it("handles source availability before checking its profiles", () => {
		expect(
			initialWizardStep(
				source({ profiles: [], unavailable: "needsKeychainApproval" }),
			),
...
describe("canCloseWizard", () => {
	it("keeps the wizard open while an import writes its target profile", () => {
		...
		expect(canCloseWizard({ step: "blocked", reason: "readFailed" })).toBe(
			true,
		);
	});
});
...
describe("outcomeToStep", () => {
	...
	it("routes a reopened browser back to the quit screen", () => {
		expect(
			outcomeToStep({ kind: "blocked", reason: "browserRunning" }),
		).toEqual({ step: "quit" });
	});

	it("routes a Full Disk Access refusal to its own screen", () => {
		expect(
			outcomeToStep({ kind: "blocked", reason: "needsFullDiskAccess" }),
		).toEqual({
			step: "fullDiskAccess",
			resume: "import",
			checked: true,
...
describe("refreshedSourceStep", () => {
	...
	it("stays on quit while the browser is still running", () => {
		expect(
			refreshedSourceStep(source({ unavailable: "browserRunning" })),
		).toEqual({
			step: "quit",
		});
	});
	...
	it("blocks when the source vanished from the list", () => {
		expect(refreshedSourceStep(undefined)).toEqual({
			step: "blocked",
...
describe("fullDiskAccessRecheckStep", () => {
	it("marks a still-denied access check for visible feedback", () => {
		expect(
			fullDiskAccessRecheckStep(source({ unavailable: "needsFullDiskAccess" })),
		).toEqual({
			step: "fullDiskAccess",
			resume: "configure",
			checked: true,
...
describe("refreshedSourceProfileDirectory", () => {
	...
	it("preserves a selected non-first profile after the browser quits", () => {
		expect(refreshedSourceProfileDirectory("Profile 2", refreshed)).toBe(
			"Profile 2",
		);
	});

	it("falls back only when the old profile vanished and clears an empty result", () => {
		expect(refreshedSourceProfileDirectory("Removed", refreshed)).toBe(
			"Default",
		);
		expect(
			refreshedSourceProfileDirectory("Removed", source({ profiles: [] })),
		).toBe("");
	});
});
...
describe("formatSkippedDomains", () => {
	...
	it("summarizes a long list", () => {
		expect(
			formatSkippedDomains(["a.com", "b.com", "c.com", "d.com", "e.com"]),
		).toBe("a.com, b.com, c.com and 2 more");
	});
});
