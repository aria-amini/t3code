...
describe("customModelEditor.logic", () => {
	it("round-trips a definition through the draft, marking the current value as default", () => {
		...
		expect(
			reopened.descriptors[0]!.choices.map((choice) => choice.isDefault),
		).toEqual([false, true]);
	});
	...
	it("preserves the current choice when it differs from the built-in default", () => {
		...
		expect(descriptors[0]!.choices.map((choice) => choice.isDefault)).toEqual([
			false,
			true,
		]);
		expect(
			definitionFromDraft(draft({ descriptors })).capabilities
				?.optionDescriptors?.[0],
	...
	it.each([true, false, undefined])(
		...
		(currentValue) => {
			...
			expect(
				definitionFromDraft({ ...reopened, name: "Renamed" }).capabilities,
	...
	it("excludes Claude context choices from presets and copies without changing other providers or authored entries", () => {
		...
		expect(copied.capabilities?.optionDescriptors).toEqual([
			capabilities.optionDescriptors![1],
		]);
		const presets = definitionFromDraft(
			draft({
				descriptors: (DESCRIPTOR_PRESETS_BY_KIND[claude] ?? []).map(
					descriptorFromPreset,
				),
			}),
		);
		expect(
			presets.capabilities?.optionDescriptors?.some(
				(option) => option.id === "contextWindow",
			),
		).toBe(false);
		const cursorCopy = descriptorsFromCapabilities(
			capabilities,
			ProviderDriverKind.make("cursor"),
		);
		expect(cursorCopy.map((option) => option.id)).toEqual([
			"contextWindow",
			"thinking",
		]);
		const authored = { slug: "custom", name: "Custom", capabilities };
		expect(
			definitionFromDraft(draftFromDefinition(authored)).capabilities
				?.optionDescriptors?.[0],
			...
			optionDescriptors: [
				{
					...
					options: [
						{ id: "high", label: "High", isDefault: true },
						{
							id: "ultracode",
							label: "Ultracode",
							description: "Uses additional reasoning.",
						},
					],
				},
			],
	...
	it("collapses a blank name and no options back to a bare definition", () => {
		...
		expect(
			draftFromDefinition({ slug: "x", name: "x", capabilities: null }).name,
		).toBe("");
	});
	...
	it("rejects duplicate ids, blank ids, and selects without choices", () => {
		...
		expect(
			validateDraft(draft({ descriptors: [select("", [{ id: "a" }])] })),
		).toBe("Option 1 needs an id.");
		expect(
			validateDraft(
				draft({
					descriptors: [
		...
		expect(
			validateDraft(
				draft({ descriptors: [select("effort", [{ id: "a" }, { id: "a" }])] }),
			),
		).toBe('Option 1: choice "a" is used twice.');
		expect(
			validateDraft(draft({ descriptors: [select("effort", [{ id: "a" }])] })),
		).toBeNull();
	});
});
