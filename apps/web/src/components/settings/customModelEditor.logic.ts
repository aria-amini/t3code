...
export function draftFromDefinition(
	...
	return {
		...
		descriptors: (entry.capabilities?.optionDescriptors ?? []).map(
			descriptorToEditor,
		),
	};
}
...
export function descriptorsFromCapabilities(
	...
	return (capabilities?.optionDescriptors ?? [])
		.filter(
			(descriptor) =>
				driverKind !== "claudeAgent" || descriptor.id !== "contextWindow",
		)
...
export function definitionFromDraft(
	...
	const descriptors: ProviderOptionDescriptor[] = draft.descriptors.map(
		(descriptor) => {
			const id = descriptor.id.trim();
