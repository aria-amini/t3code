...
describe("settings pairing URL helpers", () => {
	...
	it("uses direct backend pairing URLs for HTTP endpoints", () => {
		expect(
			resolveHostedPairingUrl("http://192.168.1.44:3773", "PAIRCODE"),
		).toBeNull();
		expect(
			resolveDesktopPairingUrl("http://192.168.1.44:3773", "PAIRCODE"),
		).toBe("http://192.168.1.44:3773/pair#token=PAIRCODE");
	});
