...
const SettingsSearchTargetContext =
	createContext<SettingsSearchTargetContextValue>({
		targetId: null,
...
export function SettingsSearchTargetProvider({
	...
	return (
		<SettingsSearchTargetContext value={value}>
			{children}
	...
	const markedScrollTarget =
		typeof target.querySelector === "function"
			? target.querySelector<HTMLElement>(
					":scope > [data-settings-scroll-target]",
				)
			: null;
	const scrollTarget =
		markedScrollTarget ??
...
export function useSettingsSearchTarget<T extends HTMLElement>(
	id: string | undefined,
) {
	const { targetId, highlightTarget, onTargetHandled } = useContext(
		SettingsSearchTargetContext,
	);
...
export function SettingsSearchTarget({
	...
	return (
		<div
			{...targetProps}
			ref={targetRef}
			tabIndex={targetProps.id ? -1 : targetProps.tabIndex}
		>
			{children}
		</div>
	);
		...
		unavailable && control ? (
			<Tooltip>
				...
				>
					<div
						inert
						className="flex w-full items-center gap-2 opacity-50 sm:w-auto"
					>
						{control}
					</div>
				</TooltipTrigger>
		...
		>
			<div className="flex flex-col gap-3 sm:grid sm:grid-cols-[minmax(0,1fr)_minmax(10rem,auto)] sm:items-center sm:gap-8">
				<div className="min-w-0 flex-1 space-y-1">
					...
					{status ? (
						<div className="pt-0.5 text-xs text-muted-foreground">{status}</div>
					) : null}
				</div>
				{renderedControl ? (
