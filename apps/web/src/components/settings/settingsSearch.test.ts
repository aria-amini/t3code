...
describe("searchSettings", () => {
	it("matches titles, sections, and remembered setting details", () => {
		expect(searchSettings("word", ITEMS).map((item) => item.id)).toEqual([
			"word-wrap",
		]);
		expect(searchSettings("network", ITEMS).map((item) => item.id)).toEqual([
			"network-access",
		]);
		expect(searchSettings("connections", ITEMS).map((item) => item.id)).toEqual(
			["network-access"],
		);
		expect(searchSettings("claude", ITEMS).map((item) => item.id)).toEqual([
			"providers",
		]);
		expect(searchSettings("long lines", ITEMS).map((item) => item.id)).toEqual([
			"word-wrap",
		]);
	});
	...
	it("matches normalized title substrings", () => {
		expect(
			searchSettings("  WORD   WRAP  ", ITEMS).map((item) => item.id),
		).toEqual(["word-wrap"]);
		expect(searchSettings("glass").map((item) => item.id)).toEqual([
			"setting-glass-opacity",
		]);
		expect(searchSettings("panel animations").map((item) => item.id)).toEqual([
			"panel-animations",
		]);
	...
	it("matches query words across fields and ranks the strongest result first", () => {
		expect(
			searchSettings("pairing remote", ITEMS).map((item) => item.id),
	...
	it("finds settings that used to be reachable only through their section", () => {
		expect(searchSettings("pull request template")[0]?.id).toBe(
			"follow-change-request-templates",
		);
		expect(searchSettings("git security keys")[0]?.id).toBe(
			"git-fetch-interval",
		);
		expect(searchSettings("push notifications")[0]?.id).toBe(
		...
		expect(searchSettings("authorized clients")[0]?.id).toBe(
			"connections-environment",
		);
		expect(searchSettings("administrative access")[0]?.id).toBe(
			"connections-environment",
		);
	});
	...
	it("hides desktop-only settings from browser search", () => {
		expect(
			SETTINGS_SEARCH_ITEMS.some((item) => item.id === "quit-confirmation"),
		).toBe(true);
	...
	it("registers the WSL backend as a desktop-only setting", () => {
		expect(
			SETTINGS_SEARCH_ITEMS.find((item) => item.id === "wsl-backend"),
		).toMatchObject({
			id: "wsl-backend",
			title: "WSL backend",
			to: "/settings/connections",
	...
	it("hides settings whose controls are unavailable", () => {
		...
		expect(
			available.map((item) => item.id).filter((id) => gatedIds.has(id)),
		).toEqual([]);
	});
	...
	it("shows automatic settlement settings when the server supports them", () => {
		...
		expect(
			searchSettings("auto-settle", available).map((item) => item.id),
		).toEqual([
			"auto-settle-inactive-threads",
			"auto-settle-merged-threads",
			"days-before-auto-settle",
	...
	it("serves anchor props to panels from the catalog", () => {
		expect(searchableSetting("word-wrap")).toEqual({
			id: "word-wrap",
			title: "Word wrap",
		});
	...
	it("routes where links open to integrations", () => {
		...
		expect(searchSettings("external links")[0]).toMatchObject({
			id: "browser-link-target",
		});
	});
