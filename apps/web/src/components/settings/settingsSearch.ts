...
const SEARCH_ITEMS_BY_ID = new Map(
...
export function filterAvailableSettingsSearchItems(
	...
	return items.filter(
		(item) =>
			...
			(!item.providerSettingsOnly ||
				availability.hasProviderSettingsEnvironment) &&
			(!item.localBackendManagementOnly ||
				availability.canManageLocalBackend) &&
			(!item.wslAvailableOnly || availability.isWslSettingsRowVisible) &&
			(!item.requiresThreadAutoSettlement ||
				availability.hasThreadAutoSettlement),
	);
}
...
export function searchSettings(
	...
	return items
		.flatMap((item, index) => {
			...
			const exactPhraseField = fields.findIndex((field) =>
