...
export function observeSettingsSectionVisibility({
	...
	const emit = () => {
		const visibleInOrder = orderedTargetIds.filter((targetId) =>
			visibleTargetIds.has(targetId),
		);
	...
	const handleEntries = (
		entries: ReadonlyArray<VisibilityEntry>,
		generation: number,
	) => {
	...
	const mutationObserver = environment.createMutationObserver(
		syncTargets,
		container,
	);
	syncTargets();
