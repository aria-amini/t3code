...
const THEME_UTILITY_PREFIXES: Readonly<
	Record<ThemePaintKind, ReadonlyArray<string>>
> = {
	background: ["bg-"],
...
export function showThemeInspectorHover(
	inspection: ThemeElementInspection,
	label: string,
): void {
	...
	const tokenLabel = hover.querySelector<HTMLElement>(
		"[data-theme-inspector-hover-label]",
	);
...
function renderThemeInspectorSpotlight(elements: ReadonlyArray<Element>): void {
	const rectangles = new Map<
		string,
		NonNullable<ReturnType<typeof spotlightRect>>
	>();
	...
	let spotlight = document.getElementById(
		THEME_SPOTLIGHT_ID,
...
function getThemePaintSnapshot(
	...
	for (const [color, borderStyle, width] of [
		...
		if (borderStyle !== "none" && Number.parseFloat(width) > 0)
			borderPaints.push(color);
	}
	if (
	...
	if (style.borderImageSource !== "none")
		borderPaints.push(style.borderImageSource);
	if (style.boxShadow !== "none") borderPaints.push(style.boxShadow);

	const foregroundPaints: Array<string> = [];
	if (forHitTest || elementHasVisibleText(element))
		foregroundPaints.push(style.color);
	if (element instanceof SVGElement)
		foregroundPaints.push(style.fill, style.stroke);
	if (element.matches("input, textarea"))
		foregroundPaints.push(style.caretColor);
	if (style.textDecorationLine !== "none")
		foregroundPaints.push(style.textDecorationColor);
...
function applyThemeTokenProbes(
...
function themeInspectionCandidates(
 ...
 */
export function highlightThemeRoleUsage(
	roles: ReadonlyArray<ThemeColorRole>,
): number {
	...
	const matches = withThemeTokenProbeSession(() => {
		...
		try {
			for (const [element, before] of baseline) {
				...
				if (after && changedThemePaintKinds(before, after).length > 0)
					changed.add(element);
			}
		} finally {
			restore();
	...
	if (import.meta.env.DEV) {
		...
		if (spotlight) {
			...
			spotlight.dataset.themeInspectorTotalMs = (
...
export function inspectThemeRoleAtElement(
	initialElement: Element,
): ThemeElementInspection | null {
	if (initialElement.closest("[data-theme-editor-panel]")) return null;
	...
	const utilityInspection =
		inspectThemeRoleFromUtilitiesAtElement(initialElement);
	...
	const rolesByElement = withThemeTokenProbeSession(() => {
		...
		const roles = new Map<
			Element,
			Partial<Record<ThemePaintKind, ThemeColorRole>>
		>();
