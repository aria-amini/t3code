...
it("suppresses capture while recording and uses the physical key for shifted digits", async () => {
	...
	recorder.input.props.onKeyDown(
		event("@", "Digit2", { ctrlKey: true, shiftKey: true }),
	);
...
it("records macOS Command without treating it as Control", async () => {
	...
	(await start(true)).input.props.onKeyDown(
		event("k", "KeyK", { metaKey: true }),
	);
	expect(recorded).toHaveBeenCalledWith(
...
it.each(["Escape", "blur", "unmount"])(
	"cancels on %s without selecting keys",
	async (cancel) => {
		const recorder = await start();
		if (cancel === "Escape") {
...
it("ignores late suppression acknowledgements after cancellation", async () => {
	...
	suppress.mockImplementation((enabled) =>
...
it("does not record Tab, held repeats, or modifier keys on their own", async () => {
	...
	recorder.input.props.onKeyDown(
		event("y", "KeyY", { ctrlKey: true, repeat: true }),
	);
	recorder.input.props.onKeyDown(
...
it("rejects modifier pairs on Wayland but accepts the next key chord", async () => {
	const recorder = await start();
	recorder.input.props.onKeyDown(
		event("Shift", "ShiftLeft", { shiftKey: true }),
	);
	recorder.input.props.onKeyDown(
		event("Shift", "ShiftRight", { shiftKey: true }),
	);
	expect(recorded).not.toHaveBeenCalled();
	expect(error).toHaveBeenCalledWith(
		expect.stringContaining("Add a letter, number"),
	);
	expect(render().recording).toBe(true);
	render().input.props.onKeyDown(
		event("y", "KeyY", { ctrlKey: true, altKey: true }),
	);
