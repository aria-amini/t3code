...
const MODIFIER_CODES: Readonly<
	Record<SnapShotModifier, readonly [string, string]>
> = {
...
export function useSnapShotShortcutRecorder({
	...
	const displayShortcut = shortcutLabel
		? parseDesktopSnapShotShortcut(shortcutLabel)
		: shortcut;
		...
		try {
			...
			onError(
				error instanceof Error
					? error.message
					: "Could not start shortcut recording.",
			);
		}
	};
	useEffect(
