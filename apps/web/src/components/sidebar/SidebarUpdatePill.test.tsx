...
const nightlyState: DesktopUpdateState = {
	...
	releaseNotes: [
		{ version: "0.0.36-nightly.3", items: ["Newest change"], totalItems: 1 },
	],
...
describe("sidebar update release notes popover", () => {
	it("uses the popover only for visible nightly release notes", () => {
		expect(shouldUseSidebarUpdateReleaseNotesPopover(true, nightlyState)).toBe(
			true,
		);
		expect(shouldUseSidebarUpdateReleaseNotesPopover(false, nightlyState)).toBe(
			false,
		);
		expect(
