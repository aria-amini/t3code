...
function allIds(state: ThreadActionMenuState): string[] {
	const flatten = (
		items: ReturnType<typeof buildThreadActionMenuItems>,
	): string[] =>
		items.flatMap((item) => [
			item.id,
...
describe("buildThreadActionMenuItems", () => {
	...
	it("flips lifecycle labels with thread state", () => {
		expect(
			ids({ ...baseState, isPinned: true, isSettled: true, isSnoozed: true }),
		).toEqual(expect.arrayContaining(["unpin", "unsettle", "unsnooze"]));
		expect(ids(baseState)).toEqual(
			expect.arrayContaining(["pin", "settle", "snooze"]),
		);
	});
