		...
		return (
			<>
				<Toast.Description
					...
					data-slot="toast-description"
				/>
				<ToastExpandableSection labels={labels}>
					{expandableContent}
				</ToastExpandableSection>
			</>
		);
	}
...
interface ToastBodyDescriptor {
	...
	readonly secondaryActionVariant: NonNullable<
		ThreadToastData["secondaryActionVariant"]
	>;
...
function deriveToastBodyDescriptor(toast: {
	...
	const secondaryActionVariant: NonNullable<
		ThreadToastData["secondaryActionVariant"]
...
function ToastBodyContent({
	...
	return (
		<>
			<div
				className={cn("flex min-w-0 gap-2", !stackedActionLayout && "flex-1")}
			>
				{leadingIcon ? (
					<div
