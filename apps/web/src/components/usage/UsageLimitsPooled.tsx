...
function SegmentPopover({
	...
	const where =
		account.environments.length > 0
			...
			: account.sourceLabel;
	const credits =
		redeem && account.limits.resetCredits?.availableCount
	...
	return (
		<div className="flex w-72 max-w-[calc(100vw-3rem)] flex-col gap-2.5 text-xs">
			...
			<div className="flex flex-col gap-1 border-t border-border/60 pt-2.5">
				...
				{where ? (
					<Row label={account.environments.length > 0 ? "Signed in" : "Via"}>
						{where}
					</Row>
				) : null}
			</div>
...
function PoolBar({
	...
	const restores = new Map(
		pool.resets.map((reset) => [reset.member.account.key, reset]),
	);
	return (
		<div className="@container/pool min-w-0">
			<div
				...
				style={{
					gridTemplateColumns: `repeat(${pool.members.length}, minmax(0, 1fr))`,
				}}
			>
