...
function isUsageWindowDays(
	value: number,
): value is UsagePagePreferences["windowDays"] {
...
export function UsagePage() {
	...
	const activeProviders = useMemo(
		() => providersWithUsage(merged.providers),
		[merged.providers],
	);
			...
			<div className="col-span-2 ms-auto flex min-w-0 items-center justify-end gap-1 xl:hidden">
				...
				>
					...
					>
						<SelectValue>
							{
								WINDOW_OPTIONS.find((option) => option.days === windowDays)
									?.label
							}
						</SelectValue>
					</SelectTrigger>
										...
										<table className="w-full table-fixed text-sm">
											<colgroup>
												...
												{activeProviders.map((provider) => (
													<col
														key={provider}
														style={{ width: timeValueColumnWidth }}
													/>
												))}
													...
													breakdownPeriods.map((period) => (
														...
														>
															...
															{activeProviders.map((provider) => (
																...
																>
																	{formatUsd(
																		period.byProvider.get(provider)?.costUsd ??
																			0,
																	)}
																</td>
															))}
						...
						{showUsageStatus && pendingCount > 0 ? (
							<>
								...
								<span className="sr-only">
									{pendingCount}{" "}
									{pendingCount === 1 ? "environment" : "environments"} still
									scanning
				...
				<MenuPopup align="start" className="w-80 max-w-[calc(100vw-2rem)]">
					<MenuCheckboxItem
						...
						onCheckedChange={(checked) =>
							onSelectionChange(checked ? null : new Set())
						}
					>
						All environments
					</MenuCheckboxItem>
						...
						return (
							<MenuCheckboxItem
								...
								onCheckedChange={(nextChecked) => {
									const next = new Set(
										selectedEnvironments.map((entry) => entry.environmentId),
									);
