...
const priceTargetsAtom = Atom.make((get): readonly UsagePriceTarget[] =>
	[...get(environmentPresentations.presentationsAtom)].map(
...
export function UsagePriceOverrides({
	...
	const selected = environments.filter(
		(environment) =>
			selectedIds === null || selectedIds.has(environment.environmentId),
	);
	...
	const customModels = [
		...new Set(
			selected.flatMap((environment) => Object.keys(environment.prices ?? {})),
		),
	].sort();
	const models = [
		...new Set([
			...
			...usage
				...
				.flatMap(
					(environment) =>
						environment.summary?.buckets.map((bucket) => bucket.model) ?? [],
				),
		]),
	].sort();
	...
	for (const draft of stagedDrafts) {
		...
		if (draft.model.trim() === "") errors.set(draft.id, "Enter a model ID.");
			...
			(customModels.includes(draft.model.trim()) ||
				drafts.some(
					(other) =>
						other.id !== draft.id && other.model.trim() === draft.model.trim(),
				))
		)
			errors.set(
				draft.id,
	...
	const failedDestinations =
		attempt?.destinations.filter(
			(destination) =>
				attempt.results.get(destination.environmentId)?.status === "failed",
		) ?? [];
	...
	const selectionLabel =
		selectedIds === null ? "All environments" : destinationLabel;
	const discard = () => {
		setDrafts([]);
		setAttempt(null);
	};
	...
	const save = async (retry = false) => {
		...
		const targets = destinations.map(
			(destination) =>
				environments.find(
					(environment) =>
						environment.environmentId === destination.environmentId,
				) ?? {
					...destination,
					prices: null,
					unavailable: "Environment removed",
				},
		);
		const changes = new Map(
			targets.map((target) => [
				target.environmentId,
				usagePriceTableChanges(target, edits),
			]),
		);
		setPending(true);
		setAttempt((previous) => ({
			...
			results: new Map([
				...
				...targets.map(
					(target) =>
						[target.environmentId, { status: "saving" } as const] as const,
				),
			]),
		}));
		let failed = false;
			...
			onResult: (environmentId, result) => {
				...
				setAttempt((previous) =>
					previous === null
						...
						: {
								...previous,
								results: new Map(previous.results).set(environmentId, result),
							},
				);
			},
		});
		...
		>
			<DialogPopup className="max-w-3xl">
				<DialogHeader>
					...
					<DialogDescription>
						Prices apply to all past and future usage on the environments you
						select.
					</DialogDescription>
				</DialogHeader>
								...
								>
									<MenuCheckboxItem
										...
										onCheckedChange={(checked) =>
											selectEnvironments(checked ? null : new Set())
										}
									>
										All environments
									</MenuCheckboxItem>
						...
						<>
							<div className="min-w-0 overflow-hidden rounded-lg border border-border">
								<Table
									...
									aria-label="Custom model prices"
								>
									<colgroup>
										<col className="w-[34%]" />
									...
									<TableBody>
										{rows.length === 0 ? (
											<TableRow className="hover:bg-transparent">
												...
												>
													{selected.some(
														(environment) => environment.prices === null,
													)
					...
					<div className="flex w-full flex-wrap justify-end gap-2 sm:w-auto">
						...
						<Button
							disabled={
								...
								(failedDestinations.length === 0 &&
									(errors.size > 0 || selected.length === 0))
							}
