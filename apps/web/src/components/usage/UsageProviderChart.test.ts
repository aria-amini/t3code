...
describe("niceScale", () => {
	it("never puts the peak above the top of the scale", () => {
		...
		for (const peak of [
			1122.71, 999, 1, 0.04, 1_400_000_000, 37.5, 5000, 100.001,
		]) {
...
describe("buildPeriodColumns", () => {
	...
	it("plots each day on its own", () => {
		expect(
			buildPeriodColumns(days, byDay, "cost").map((column) => column.total),
		).toEqual([30, 0, 5]);
	});

	it("reads the requested metric", () => {
		expect(
			buildPeriodColumns(days, byDay, "tokens").map((column) => column.total),
		).toEqual([300, 0, 50]);
	});
	...
	it("reports the total as the sum of its bands", () => {
		for (const column of buildPeriodColumns(days, byDay, "cost")) {
			const sum = column.bands.reduce(
				(running, band) => running + band.value,
				0,
			);
		...
		const byHour = new Map([
			[
				"2026-08-11T09:37:00.000Z",
				{
					...
					totalTokens: 40,
					byProvider: new Map([
						["codex" as const, { costUsd: 4, totalTokens: 40 }],
					]),
				},
			],
		]);
