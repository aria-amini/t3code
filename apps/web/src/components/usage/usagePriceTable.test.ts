...
describe("price table edits", () => {
	it("shows shared and mixed cells separately, and never treats missing settings as automatic", () => {
		...
		expect(
			usagePriceCell(targets, "example", "inputCostPerMillionTokens").value,
		).toBe("2");
		expect(
			usagePriceCell(targets, "example", "outputCostPerMillionTokens"),
		).toEqual({
			value: "",
			placeholder: "Mixed",
		});
		expect(
			usagePriceCell(targets, "example", "cacheReadCostPerMillionTokens"),
		).toEqual({
			value: "",
			placeholder: "Input rate",
		});
		expect(
			usagePriceCell([target("a", {})], "example", "inputCostPerMillionTokens")
				.placeholder,
		).toBe("Automatic");
		expect(
			usagePriceCell(
				[targets[0]!, target("b", null)],
				"example",
	...
	it.each(["constructor", "toString", "__proto__"])(
		...
		(model) => {
			...
			expect(
				usagePriceCell([environment], model, "inputCostPerMillionTokens"),
			).toEqual({
				value: "",
	...
	it("changes only edited columns while preserving each environment's other prices and models", () => {
		...
		const a = usagePriceTableChanges(
			target("a", {
				example: { ...price, cacheReadCostPerMillionTokens: 0 },
				untouched: price,
			}),
			edits,
		);
		...
		expect(b.changes).toEqual([
			{
				model: "example",
				price: { inputCostPerMillionTokens: 3, outputCostPerMillionTokens: 10 },
			},
		]);
	});
	...
	it("batches new rows and resets, and distinguishes blank cache rates from explicit zero", () => {
		...
		expect(plan.changes).toEqual([
			...
			{
				model: "vendor/Model",
				price: { ...price, cacheReadCostPerMillionTokens: 0 },
			},
		]);
	});
	...
	it("requires complete prices before turning automatic pricing into an override", () => {
		const edits = [draft({ inputCostPerMillionTokens: "3" })];
		expect(
			usagePriceTableChanges(target("a", { example: price }), edits).errors
				.size,
		).toBe(0);
	...
	it.each(["Offline", "Read-only access", "Update server to edit prices"])(
		...
		(unavailable) => {
			...
			expect(
				usagePriceTableErrors([writable, { ...other, unavailable }], edits)
					.size,
			).toBe(0);
			expect(
				usagePriceTableErrors([writable, other], edits).has("model:example"),
			).toBe(true);
		},
	);
	...
	it("does not write unchanged rates and rejects invalid edited cells", () => {
		...
		expect(
			usagePriceTableChanges(environment, [
				draft({ inputCostPerMillionTokens: "2.00" }),
			]).changes,
		).toEqual([]);
		expect(
			usagePriceTableChanges(environment, [
				draft({ cacheReadCostPerMillionTokens: "-1" }),
			]).errors.size,
		).toBe(1);
		expect(
			usagePriceTableChanges(environment, [
				draft({ outputCostPerMillionTokens: "" }),
			]).errors.size,
		).toBe(1);
	});
});
