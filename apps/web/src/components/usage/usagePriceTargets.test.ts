...
const target = (
	id: string,
	overrides: Partial<UsagePriceTarget> = {},
): UsagePriceTarget => ({
...
describe("model price writes", () => {
	it("sends all table changes in one patch per environment and skips unchanged destinations", async () => {
		...
		expect(write).toHaveBeenCalledExactlyOnceWith({
			...
			input: {
				patch: { usagePriceOverrides: { "new-model": price, example: null } },
			},
		});
	...
	it("saves independently, skips unavailable targets, and retries failures without rewriting successes", async () => {
		...
		const targets = [
			target("fast"),
			target("slow"),
			target("offline", { unavailable: "Offline" }),
		];
		const results = new Map<EnvironmentId, UsagePriceWriteResult>();
		const write = vi.fn(
			async ({ environmentId }: { environmentId: EnvironmentId }) =>
		...
		const running = writeUsagePrices({
			targets,
			changes: new Map(
				targets.map((target) => [target.environmentId, [change]]),
			),
			write,
			onResult: (id, result) => {
		...
		await fastSaved;
		expect(results.get(EnvironmentId.make("fast"))).toEqual({
			status: "saved",
		});
		...
		await writeUsagePrices({
			targets: targets
				.filter(
					(entry) => results.get(entry.environmentId)?.status === "failed",
				)
		...
		expect(retry).not.toHaveBeenCalledWith(
			expect.objectContaining({ environmentId: "fast" }),
		);
		expect(
			[...results.values()].every((result) => result.status === "saved"),
		).toBe(true);
	});
	...
	it("resets only the chosen model on the selected destination", async () => {
		...
		await writeUsagePrices({
			...
			changes: new Map([
				[EnvironmentId.make("selected"), [{ model: "example", price: null }]],
			]),
			write,
			onResult,
		});
	...
	it("reports thrown writes and permission/version restrictions without aborting other saves", async () => {
		...
		await writeUsagePrices({
			...
			changes: new Map([
				[EnvironmentId.make("lost"), [{ model: "example", price }]],
			]),
			write,
			onResult,
		});
