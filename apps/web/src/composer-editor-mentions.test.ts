...
describe("splitPromptIntoComposerSegments", () => {
	...
	it("splits quoted mention tokens containing whitespace", () => {
		expect(
			splitPromptIntoComposerSegments('Inspect @"My File.md" please'),
		).toEqual([
			{ type: "text", text: "Inspect " },
	...
	it("unescapes quoted mention token content", () => {
		expect(
			splitPromptIntoComposerSegments(
				'Inspect @"docs/My \\"File\\".md" please',
			),
		).toEqual([
			{ type: "text", text: "Inspect " },
			{
	...
	it("does not turn normal web links into file mention segments", () => {
		expect(
			...
			{ type: "text", text: "Read [the docs](https://example.com/docs) first" },
		]);
	});
	...
	it("keeps multiple assistant citations atomic next to punctuation and Unicode", () => {
		...
		expect(
			splitPromptIntoComposerSegments(`前(${source}),${otherSource}後`),
		).toEqual([
			{ type: "text", text: "前(" },
	...
	it("keeps malformed citation links as editable text", () => {
		const prompt =
			"[Assistant quote](t3-citation://v1/env/thread/message?text=missing+metadata)";
	...
	it("turns canonical scoped folder links into file mention segments", () => {
		expect(
			splitPromptIntoComposerSegments("Inspect [sub](@scope/pkg/sub) next"),
		).toEqual([
			{ type: "text", text: "Inspect " },
			{
				type: "mention",
	...
	it("splits skill tokens followed by whitespace into skill segments", () => {
		expect(
			splitPromptIntoComposerSegments("Use $review-follow-up please"),
		).toEqual([
			{ type: "text", text: "Use " },
