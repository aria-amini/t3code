...
const citationSource = serializeAssistantCitation(citation).replaceAll(
	"+",
	"%20",
);
...
describe("formatAssistantCitationForComposer", () => {
	...
	it("binds a multiline comment to its citation without adding standalone prompt text", () => {
		...
		expect(
			collectAssistantCitations(text).map((entry) => entry.citation),
		).toEqual([boundCitation]);
		expect(expandAssistantCitationsForProvider(text)).toMatch(
			/^\[assistant-quote-1\] \n\n/,
		);
	});
});
...
describe("expandCollapsedComposerCursor", () => {
	...
	it("maps collapsed markdown file links to their expanded source offsets", () => {
		...
		const expandedCursorAfterMention = "what's in [AGENTS.md](AGENTS.md) "
			.length;
...
describe("collapseExpandedComposerCursor", () => {
	...
	it("keeps package-like text expanded when another mention already exists earlier", () => {
		...
		expect(collapsedCursor).toBe(
			"open ".length + 1 + " then @src/index.ts ".length,
		);
		expect(expandCollapsedComposerCursor(text, collapsedCursor)).toBe(
			expandedCursor,
		);
	});
	...
	it("collapses only genuine mentions when package-like text exists earlier", () => {
		...
		expect(collapsedCursor).toBe(
			"install @scope/pkg then ".length + 1 + " ".length,
		);
		expect(expandCollapsedComposerCursor(text, collapsedCursor)).toBe(
			expandedCursor,
		);
...
describe("assistant citation cursor offsets", () => {
	it("roundtrips every collapsed offset across citations, mentions, skills, and Unicode", () => {
		...
		const boundaries = [
			...
			[
				prefix.length + 1 + between.length,
				prefix.length + citationSource.length + between.length,
			],
			[
	...
	it("snaps serialized offsets inside a citation to the end of its chip", () => {
		...
		for (const offset of [
			1,
			citationSource.length - 1,
			citationSource.length,
		]) {
			expect(collapseExpandedComposerCursor(text, prefix.length + offset)).toBe(
				prefix.length + 1,
			);
		}
		expect(
			isCollapsedCursorAdjacentToInlineToken(text, prefix.length, "right"),
		).toBe(true);
		expect(
			isCollapsedCursorAdjacentToInlineToken(text, prefix.length + 1, "left"),
		).toBe(true);
		expect(
			isCollapsedCursorAdjacentToInlineToken(text, prefix.length + 2, "left"),
		).toBe(false);
	});
	...
	it("deletes one collapsed citation without changing its neighbor or surrounding punctuation", () => {
		...
		expect(
			collapseExpandedComposerCursor(result.text, result.text.length),
		).toBe("(),□!".length);
	});
});
...
describe("replaceTextRange trailing space consumption", () => {
	it("double space after insertion when replacement ends with space", () => {
		...
		const withoutConsume = replaceTextRange(
			text,
			rangeStart,
			rangeEnd,
			"@AGENTS.md ",
		...
		const withConsume = replaceTextRange(
			text,
			rangeStart,
			extendedEnd,
			"@AGENTS.md ",
...
describe("isCollapsedCursorAdjacentToInlineToken", () => {
	it("returns false when no mention exists", () => {
		expect(
			isCollapsedCursorAdjacentToInlineToken("plain text", 6, "left"),
		).toBe(false);
		expect(
			isCollapsedCursorAdjacentToInlineToken("plain text", 6, "right"),
		).toBe(false);
	});
	...
	it("detects left adjacency only when cursor is directly after a mention", () => {
		...
		expect(
			isCollapsedCursorAdjacentToInlineToken(text, mentionStart, "left"),
		).toBe(false);
		expect(
			isCollapsedCursorAdjacentToInlineToken(text, mentionEnd + 1, "left"),
		).toBe(false);
	});
	...
	it("detects right adjacency only when cursor is directly before a mention", () => {
		...
		expect(
			isCollapsedCursorAdjacentToInlineToken(text, mentionEnd, "right"),
		).toBe(false);
		expect(
			isCollapsedCursorAdjacentToInlineToken(text, mentionStart - 1, "right"),
		).toBe(false);
	});
	...
	it("treats terminal pills as inline tokens for adjacency checks", () => {
		...
		expect(isCollapsedCursorAdjacentToInlineToken(text, tokenEnd, "left")).toBe(
			true,
		);
		expect(
...
describe("parseStandaloneComposerSlashCommand", () => {
	...
	it("ignores slash commands with extra message text", () => {
		expect(
			parseStandaloneComposerSlashCommand("/plan explain this"),
		).toBeNull();
	});
});
