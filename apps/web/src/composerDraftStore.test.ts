...
type ProviderOptionSelectionsByProvider = Partial<
	Record<string, ProviderOptionSelectionBag>
>;
...
function selectionsByProvider(
	options: Partial<
		Record<ProviderDriverKind, Record<string, string | boolean | undefined>>
	>,
...
function draftFor(
	...
	return (
		store[threadKeyFor(threadId, environmentId)] ?? store[threadId] ?? undefined
	);
}
...
describe("composerDraftStore assistant citations", () => {
	...
	it("keeps quotes, comments, and remote source IDs through persistence and removes them on clear", async () => {
		...
		try {
			...
			expect(
				collectAssistantCitations(restored).map((entry) => entry.citation),
			).toEqual([citation]);
			useComposerDraftStore.getState().clearComposerContent(threadRef);
			expect(
				collectAssistantCitations(
					draftFor(threadId, TEST_ENVIRONMENT_ID)?.prompt ?? "",
				),
			).toEqual([]);
		} finally {
			await useComposerDraftStore.persist.clearStorage();
...
describe("composerDraftStore addImages", () => {
	...
	it("deduplicates against existing images across calls by file signature", () => {
		...
		expect(useComposerDraftStore.getState().addImage(threadRef, first)).toBe(
			true,
		);
		expect(
	...
	it("returns false when addImage receives an existing image id", () => {
		...
		expect(
			useComposerDraftStore.getState().addImage(threadRef, duplicate),
		).toBe(false);
	...
	it("does not revoke blob URLs that are still used by an accepted duplicate image", () => {
		...
		useComposerDraftStore
			.getState()
			.addImages(threadRef, [first, duplicateSameUrl]);
...
describe("composerDraftStore unsent draft marker", () => {
	...
	it("reports content for typed text and clears when the composer is emptied", () => {
		const hasDraft = () =>
			composerDraftHasUserContent(
				useComposerDraftStore.getState().getComposerDraft(threadRef),
			);
		...
		useComposerDraftStore
			.getState()
			.setPrompt(threadRef, "follow up on the relay case");
...
describe("composerDraftStore file attachments", () => {
	...
	it("replaces a needs-reattach marker when the same file is picked again", () => {
		...
		expect(
			store.getComposerDraft(threadRef)?.files.every(composerFileNeedsReattach),
		).toBe(true);
	...
	it("replaces a legacy video marker after its MIME type is normalized", () => {
		...
		const file = new File(["report"], marker.name, {
		...
		store.addFiles(threadRef, [
			repicked,
			{ ...repicked, id: "file-repicked-duplicate" },
		]);
	...
	it("still dedupes a re-pick against a file that does not need reattaching", () => {
		...
		expect(
			store.getComposerDraft(threadRef)?.files.map((file) => file.id),
		).toEqual(["file-original"]);
	});
	...
	it("keeps the remaining file slot available after a duplicate is skipped", () => {
		...
		expect(
			store.getComposerDraft(threadRef)?.files.map((file) => file.id),
		).toEqual(["file-original", "file-unique"]);
	});
});
...
describe("composerDraftStore syncPersistedAttachments", () => {
	...
	it("treats malformed persisted draft storage as empty", async () => {
		...
		expect(
			draftFor(threadId, TEST_ENVIRONMENT_ID)?.persistedAttachments,
		).toEqual([]);
		expect(
			draftFor(threadId, TEST_ENVIRONMENT_ID)?.nonPersistedImageIds,
		).toEqual([image.id]);
	});
});
...
describe("composerDraftStore terminal contexts", () => {
	...
	it("deduplicates identical terminal contexts by selection signature", () => {
		...
		useComposerDraftStore
			.getState()
			.addTerminalContexts(threadRef, [first, duplicate]);
	...
	it("inserts terminal contexts at the requested inline prompt position", () => {
		const firstInsertion = insertInlineTerminalContextPlaceholder(
			"alpha beta",
			6,
		);
		const secondInsertion = insertInlineTerminalContextPlaceholder(
	...
	it("omits terminal context text from persisted drafts", () => {
		useComposerDraftStore
			...
			.addTerminalContext(
				threadRef,
				makeTerminalContext({ id: "ctx-persist" }),
			);
		...
		expect(
			persistedState.draftsByThreadKey?.[
	...
	it("hydrates persisted terminal contexts without in-memory snapshot text", () => {
		...
		expect(
			mergedState.draftsByThreadKey[threadKeyFor(threadId)]?.terminalContexts,
		).toMatchObject([
			{
				id: "ctx-rehydrated",
				terminalId: "default",
	...
	it("sanitizes malformed persisted drafts during merge", () => {
		...
		expect(
			mergedState.draftsByThreadKey[threadKeyFor(threadId)],
		).toBeUndefined();
...
describe("composerDraftStore element contexts", () => {
	...
	it("adds an element context and stamps id + threadId + pickedAt", () => {
		const accepted = useComposerDraftStore
			.getState()
			.addElementContext(threadRef, baseSelection);
	...
	it("dedupes by selector + tag + componentName + pageUrl signature", () => {
		...
		expect(
			draftFor(threadId, TEST_ENVIRONMENT_ID)?.elementContexts,
		).toHaveLength(1);
	});
	...
	it("removeElementContext drops by id + leaves siblings intact", () => {
		...
		const ids = draftFor(threadId, TEST_ENVIRONMENT_ID)!.elementContexts.map(
			(c) => c.id,
		);
...
describe("composerDraftStore project draft thread mapping", () => {
	...
	it("clears composer data for one environment without touching another", () => {
		...
		try {
			...
			expect(next.getComposerDraft(remoteDraftId)?.prompt).toBe(
				"remote thread draft",
			);
			expect(next.getComposerDraft(localThreadRef)).toBeNull();
			expect(next.getComposerDraft(remoteThreadRef)?.prompt).toBe(
				"remote thread draft",
			);
	...
	it("stores and reads project draft thread ids via actions", () => {
		...
		expect(
			useComposerDraftStore.getState().getDraftThreadByProjectRef(projectRef),
		).toMatchObject({
			threadId,
			environmentId: TEST_ENVIRONMENT_ID,
			projectId,
		...
		expect(
			useComposerDraftStore.getState().getDraftThread(draftId),
		).toMatchObject({
			environmentId: TEST_ENVIRONMENT_ID,
			projectId,
			logicalProjectKey: scopedProjectKey(projectRef),
	...
	it("removes a draft's previous project mapping when retargeted in place", () => {
		...
		expect(store.getDraftThreadByProjectRef(projectRef)).toBeNull();
		expect(store.getDraftThreadByProjectRef(otherProjectRef)?.draftId).toBe(
			draftId,
		);
	...
	it("keeps invested composer drafts alive unmapped when remapping a project to a new draft thread", () => {
		...
		expect(
			useComposerDraftStore.getState().getDraftThread(draftId)?.threadId,
		).toBe(threadId);
	...
	it("clears every session for a project, including unmapped invested drafts", () => {
		...
		expect(useComposerDraftStore.getState().getDraftThread(draftId)).toBeNull();
		expect(
			useComposerDraftStore.getState().getDraftThread(otherDraftId),
		).toBeNull();
		expect(draftByKey(draftId)).toBeUndefined();
	});
	...
	it("keeps composer drafts when the thread is still mapped by another project", () => {
		...
		expect(
			useComposerDraftStore
				.getState()
				.getDraftThreadByProjectRef(otherProjectRef)?.threadId,
		).toBe(threadId);
	...
	it("marks a promoted draft by scoped ref without deleting composer state", () => {
		...
		expect(
			useComposerDraftStore.getState().getDraftThread(draftId)?.promotedTo,
		).toEqual(scopeThreadRef(TEST_ENVIRONMENT_ID, threadId));
	...
	it("does not clear composer drafts for existing server threads during promotion cleanup", () => {
		...
		expect(
			useComposerDraftStore.getState().getDraftThread(threadRef),
		).toBeNull();
	...
	it("promotes matching thread ids separately for each environment", () => {
		...
		expect(store.getDraftThreadByProjectRef(projectRef)).toBeNull();
		expect(store.getDraftThreadByProjectRef(remoteProjectRef)?.threadId).toBe(
			threadId,
		);
		expect(store.getDraftThreadByRef(localThreadRef)?.promotedTo).toEqual(
			localThreadRef,
		);
		...
		expect(store.getDraftThreadByRef(remoteThreadRef)?.promotedTo).toEqual(
			remoteThreadRef,
		);
		expect(draftByKey(localDraftId)?.prompt).toBe("local draft");
		expect(draftByKey(remoteDraftId)?.prompt).toBe("remote draft");
	...
	it("only marks promoted drafts for the matching environment ref", () => {
		...
		markPromotedDraftThreadByRef(
			scopeThreadRef(OTHER_TEST_ENVIRONMENT_ID, threadId),
		);
	...
	it("stores the start-from-origin choice with the draft thread", () => {
		...
		expect(
			useComposerDraftStore.getState().getDraftThread(draftId)?.startFromOrigin,
		).toBe(true);
...
describe("composerDraftStore modelSelection", () => {
	...
	it("stores a model selection in the draft", () => {
		...
		expect(
			draftFor(threadId, TEST_ENVIRONMENT_ID)?.modelSelectionByProvider[
				CODEX_INSTANCE
			],
		).toEqual(
			modelSelection(CODEX_DRIVER, "gpt-5.3-codex", {
	...
	it("marks picker writes explicit and seeding writes non-explicit", () => {
		...
		store.setModelSelection(threadRef, modelSelection(CODEX_DRIVER, "gpt-5.4"));
		expect(
			draftFor(threadId, TEST_ENVIRONMENT_ID)?.modelSelectionExplicit,
		).toBeUndefined();
	...
	it("does not clear other provider options when setting options for a single provider", () => {
		...
		expect(
			draft?.modelSelectionByProvider[CLAUDE_AGENT_INSTANCE]?.options,
		).toEqual(
			createModelSelection(
				CLAUDE_AGENT_INSTANCE,
				"claude-opus-4-6",
	...
	it("stores provider option changes on a selected custom instance", () => {
		...
		expect(draftFor(threadId, TEST_ENVIRONMENT_ID)?.activeProvider).toBe(
			CODEX_SECONDARY_INSTANCE,
		);
...
describe("composerDraftStore sticky composer settings", () => {
	...
	it("replaces a non-explicit stale model and its options with sticky state", () => {
		...
		store.setStickyModelSelection(
			modelSelection(CODEX_DRIVER, "sticky-model", {
				reasoningEffort: "xhigh",
			}),
		);
	...
	it("clears a non-explicit stale model when there is no sticky state", () => {
		...
		store.setModelSelection(
			draftId,
			modelSelection(CODEX_DRIVER, "stale-model"),
		);
		store.applyStickyState(draftId);
...
describe("composerDraftStore model seed migration", () => {
	...
	const serverThreadKey = scopedThreadKey(
		scopeThreadRef(TEST_ENVIRONMENT_ID, serverThreadId),
	);
	...
	it("strips seeded models only from empty draft sessions when upgrading storage", async () => {
		...
		try {
			...
			expect(
				useComposerDraftStore.getState().draftThreadsByThreadKey[staleDraftId],
			).toMatchObject({
				environmentId: TEST_ENVIRONMENT_ID,
				projectId,
				logicalProjectKey,
	...
	it("keeps v8 file-only draft sessions and their seeded models", async () => {
		...
		try {
			...
			expect(
				draftByKey(markerDraftId)?.files.every(composerFileNeedsReattach),
			).toBe(true);
		} finally {
			vi.useRealTimers();
		}
...
describe("composerDraftStore runtime and interaction settings", () => {
	...
	it("stores runtime mode overrides in the composer draft", () => {
		...
		expect(draftFor(threadId, TEST_ENVIRONMENT_ID)?.runtimeMode).toBe(
			"approval-required",
		);
	});
	...
	it("stores interaction mode overrides in the composer draft", () => {
		...
		expect(draftFor(threadId, TEST_ENVIRONMENT_ID)?.interactionMode).toBe(
			"plan",
		);
	});
...
describe("composer draft persistence", () => {
	it("defers attachment reads and serialization until typing stops, then restores the last draft", async () => {
		...
		try {
			...
			resetComposerDraftStore();
			await useComposerDraftStore.persist.rehydrate();
			expect(draftFor(typingThreadId, TEST_ENVIRONMENT_ID)?.prompt).toBe(
				"Draft 20",
			);
			expect(
				draftFor(heavyThreadId, TEST_ENVIRONMENT_ID)?.persistedAttachments,
			).toEqual(attachments);
		} finally {
