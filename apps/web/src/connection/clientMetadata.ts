...
export function browserClientOs(identity: BrowserIdentity): ClientOs {
	...
	if (/iphone|ipad|ipod/i.test(userAgent) || isIpadosDesktopUserAgent(identity))
		return "iOS";
...
export function browserDeviceType(
...
export function clientPresentationMetadata(input: {
	...
	if (input.desktopBridge !== undefined) {
		return {
			...
			deviceType: "desktop",
			os: clientOsFromElectronPlatform(
				input.desktopBridge.getClientPlatform?.(),
			),
			surface: "desktop",
