...
describe("desktop SSH pairing", () => {
	it.effect(
		"fetches the descriptor before consuming the one-time credential",
		() =>
			Effect.gen(function* () {
				const calls: string[] = [];

				const provisioned = yield* provisionDesktopSshEnvironment(
					makeBridge(calls),
					TARGET,
				);

				expect(provisioned.environmentId).toBe(
	...
	it.effect(
		"does not consume the credential when descriptor discovery fails",
		() =>
			Effect.gen(function* () {
				const calls: string[] = [];
...
describe("desktop-local bearer cache", () => {
	...
	it("refreshes a secondary bearer before it expires", () => {
		...
		const refreshAtEpochMs = secondaryBearerRefreshAtEpochMs(
			issuedAtEpochMs,
			60,
		);
		const expiresAtEpochMs = secondaryBearerExpiresAtEpochMs(
			issuedAtEpochMs,
			60,
		);
		const cached = {
		...
		expect(
			canReuseCachedPlatformRegistration(cached, cached.signature, 64_999),
		).toBe(true);
		expect(
			canReuseCachedPlatformRegistration(cached, cached.signature, 65_000),
		).toBe(false);
		expect(
			canRetainCachedPlatformRegistrationAfterRefreshFailure(
				cached,
	...
	it("does not cache credentials whose lifetime is shorter than the refresh skew", () => {
		...
		expect(
			canReuseCachedPlatformRegistration(cached, cached.signature, 10_000),
		).toBe(false);
	});
