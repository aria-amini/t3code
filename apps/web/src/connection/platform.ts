...
const capabilitiesLayer = Layer.effectContext(
	Effect.sync(() => {
		...
		const ssh = SshEnvironmentGateway.of({
			...
			prepare: Effect.fn("web.connectionPlatform.ssh.prepare")(
				function* (input) {
					const bridge = window.desktopBridge;
					if (bridge === undefined) {
						return yield* new ConnectionBlockedError({
...
const loadSecondaryConnectionRegistration = Effect.fn(
	...
	return {
		...
		expiresAtEpochMs: secondaryBearerExpiresAtEpochMs(
			issuedAtEpochMs,
			access.expires_in,
		),
		refreshAtEpochMs: secondaryBearerRefreshAtEpochMs(
			issuedAtEpochMs,
			access.expires_in,
		),
	};
...
export function primaryRegistrationToRetainAfterTopologyRead(
	...
	return topologyRead._tag === "Failure"
		? previous.get(PRIMARY_LOCAL_ENVIRONMENT_ID)
		: undefined;
}
...
export function canReuseCachedPlatformRegistration(
	...
	return (
		...
		(cached.refreshAtEpochMs === undefined ||
...
const platformConnectionSourceLayer = Layer.effect(
	...
	Effect.gen(function* () {
		...
		const cacheRef = yield* Ref.make(
			new Map<string, CachedPlatformRegistration>(),
		);
		...
		const buildPlatformRegistrations = Effect.gen(function* () {
			...
			if (primaryTopologyRead._tag === "Failure") {
				yield* Effect.logWarning(
					"Could not read the primary environment topology.",
					{
				...
				if (
					...
					const built = yield* loadPrimaryConnectionRegistration(
						primaryTarget,
					).pipe(
			...
			for (const [
				id,
				cached,
			] of secondaryRegistrationsToRetainAfterTopologyRead(
				previous,
				topologyRead,
				nowEpochMs,
			)) {
			...
			if (topologyRead._tag === "Failure") {
				yield* Effect.logWarning(
					"Could not read the desktop-local backend topology.",
					{
