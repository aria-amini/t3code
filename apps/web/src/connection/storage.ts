...
const ConnectionCatalogDocumentJson = Schema.fromJsonString(
	ConnectionCatalogDocument,
);
const decodeConnectionCatalogDocument = Schema.decodeUnknownEffect(
	ConnectionCatalogDocumentJson,
);
const encodeConnectionCatalogDocument = Schema.encodeEffect(
	ConnectionCatalogDocumentJson,
);
const decodeStoredShellSnapshot = Schema.decodeUnknownEffect(
	StoredShellSnapshotJson,
);
...
const openDatabase = Effect.fn("web.connectionStorage.openDatabase")(
	function* () {
		return yield* Effect.callback<IDBDatabase, ConnectionTransientError>(
			(resume) => {
				...
				request.addEventListener("upgradeneeded", () => {
					...
					if (
						!request.result.objectStoreNames.contains(SERVER_CONFIG_STORE_NAME)
					) {
				...
				request.addEventListener("error", () => {
					resume(
						Effect.fail(
							catalogError("open", request.error ?? "Unknown IndexedDB error"),
						),
					);
				});
...
function readDatabaseValue(
	...
	return Effect.callback<unknown, ConnectionTransientError>((resume) => {
		...
		request.addEventListener("error", () => {
			resume(
				Effect.fail(
					catalogError("read", request.error ?? "Unknown IndexedDB read error"),
				),
			);
		});
		request.addEventListener("success", () => {
		...
		transaction.addEventListener("error", () => {
			resume(
				Effect.fail(
					catalogError(
						"write",
						transaction.error ?? "Unknown IndexedDB write error",
					),
				),
			);
		});
		...
		transaction.addEventListener("error", () => {
			resume(
				Effect.fail(
					catalogError(
						"remove",
						transaction.error ?? "Unknown IndexedDB remove error",
					),
				),
			);
		});
		...
		transaction.addEventListener("error", () => {
			resume(
				Effect.fail(
					catalogError(
						"remove",
						transaction.error ?? "Unknown IndexedDB cursor error",
					),
				),
			);
		});
		...
		request.addEventListener("error", () => {
			resume(
				Effect.fail(
					catalogError(
						"remove",
						request.error ?? "Unknown IndexedDB cursor error",
					),
				),
			);
		});
...
const decodeCatalog = Effect.fn("web.connectionStorage.decodeCatalog")(
	function* (raw: string) {
...
const encodeCatalog = Effect.fn("web.connectionStorage.encodeCatalog")(
	function* (catalog: ConnectionCatalogDocumentType) {
...
export function makeCatalogBackend(database: IDBDatabase): CatalogBackend {
	...
	return {
		...
		write: (raw) =>
			writeDatabaseValue(database, CATALOG_STORE_NAME, CATALOG_KEY, raw),
		quarantine: (raw) =>
			writeDatabaseValue(
				database,
				CATALOG_STORE_NAME,
				`${CATALOG_KEY}:corrupt:${Date.now()}`,
				raw,
			),
	};
}
...
export const makeCatalogStore = Effect.fn(
	"web.connectionStorage.makeCatalogStore",
)(function* (backend: CatalogBackend) {
	const state = yield* Ref.make<Option.Option<ConnectionCatalogDocumentType>>(
		Option.none(),
	);
	const lock = yield* Semaphore.make(1);

	const loadUnlocked = Effect.fn("web.connectionStorage.loadCatalog")(
		function* () {
					...
					Effect.catch((error) =>
						Effect.gen(function* () {
							yield* Effect.logWarning(
								"Discarding a corrupt web connection catalog.",
								{
									error: error.message,
								},
							);
							...
							const encoded = yield* encodeCatalog(
								EMPTY_CONNECTION_CATALOG_DOCUMENT,
							);
							yield* backend.write(encoded).pipe(
								Effect.catch((cause) =>
									Effect.logWarning(
										"Could not persist the recovered web connection catalog.",
										{
											error: cause.message,
										},
									),
	...
	const update: CatalogStore["update"] = Effect.fn(
		"web.connectionStorage.updateCatalog",
	)(function* (transform) {
	...
	Effect.gen(function* () {
		...
		const registrationStore = ConnectionRegistrationStore.of({
			register: (registration) =>
				catalog
					.update((document) =>
						registerConnectionInCatalog(document, registration),
					)
					.pipe(
						Effect.mapError((cause) =>
							persistenceError("register-connection", cause),
						),
					),
			remove: (target) =>
				catalog
					...
					.pipe(
						Effect.mapError((cause) =>
							persistenceError("remove-connection", cause),
						),
					),
		});
		...
		const remoteTokenStore = TokenStore.make({
			get: (environmentId) =>
				catalog.read.pipe(
					Effect.map((document) =>
						Option.fromUndefinedOr(
							document.remoteDpopTokens.find(
			...
			put: (token) =>
				catalog.update((document) =>
					putRemoteDpopTokenInCatalog(document, token),
				),
		...
		const cacheStore = EnvironmentCacheStore.of({
			...
			removeVcsRefs: (environmentId, cwd) =>
				removeDatabaseValue(
					...
					Effect.mapError((cause) =>
						persistenceError("remove-vcs-refs", cause),
					),
				),
			...
			clear: (environmentId) =>
				Effect.all(
					[
						Effect.promise(() =>
							projectFaviconCache.clearEnvironment(environmentId),
						),
					...
					Effect.mapError((cause) =>
						persistenceError("clear-environment", cause),
					),
				),
		});
