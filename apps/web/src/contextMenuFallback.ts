...
> = {
	archive: [
		{
			tag: "rect",
			attrs: { width: "20", height: "5", x: "2", y: "3", rx: "1" },
		},
	...
	"mail-open": [
		...
		{
			tag: "path",
			attrs: { d: "m22 10-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 10" },
		},
	],
	"message-square-plus": [
		{
	...
	"pin-off": [
		...
		{
			tag: "path",
			attrs: { d: "M15 9.34V7a1 1 0 0 1 1-1 2 2 0 0 0 0-4H7.89" },
		},
...
function isNodeWithinMenuStack(
	target: EventTarget | null,
	menuStack: readonly HTMLDivElement[],
) {
...
export function showContextMenuFallback<T extends string>(
	...
	return new Promise<T | null>((resolve) => {
		...
		const cleanup = (result: T | null) => {
			...
			const shouldRestoreFocus = isNodeWithinMenuStack(
				document.activeElement,
				menuStack,
			);
		...
		const onPointerDown = (event: PointerEvent) => {
			if (
				!canDismissFromPointer ||
				isNodeWithinMenuStack(event.target, menuStack)
			) {
				return;
			}
			cleanup(null);
		...
		const openMenu = (
			...
			for (const item of entries) {
				...
				const isLeafDestructive =
					!hasChildren &&
					(item.destructive === true || item.id === ("delete" as T));
				...
				if (typeof item.icon === "string") {
					const icon = createIconElement(
						item.icon,
						isLeafDestructive ? "destructive" : "neutral",
					);
					if (icon) {
