...
describe("environment grouping", () => {
	it("groups matching repository identities across environments", () => {
		...
		expect(deriveLogicalProjectKey(primary)).toBe(
			repositoryIdentity.canonicalKey,
		);
		expect(deriveLogicalProjectKey(remote)).toBe(
			repositoryIdentity.canonicalKey,
		);
	});
	...
	it("keeps projects without repository identity physically scoped", () => {
		...
		expect(deriveLogicalProjectKey(primary)).toBe(
			derivePhysicalProjectKey(primary),
		);
		expect(deriveLogicalProjectKey(remote)).toBe(
			derivePhysicalProjectKey(remote),
		);
		expect(deriveLogicalProjectKey(primary)).not.toBe(
	...
	it("reports the effective grouping mode after applying an override", () => {
		...
		expect(resolveProjectGroupingMode(project, defaultGroupingSettings)).toBe(
			"repository",
		);
		expect(
	...
	it("routes duplicate physical project keys to the winning logical group", () => {
		...
		expect(
			physicalToLogicalKey.get(
				derivePhysicalProjectKey(staleWithoutRepositoryIdentity),
			),
		).toBe(repositoryIdentity.canonicalKey);
		...
		expect(
			deriveLogicalProjectKeyFromSettings(
				staleWithoutRepositoryIdentity,
				defaultGroupingSettings,
			),
	...
	it("keeps the current environment when available and falls back otherwise", () => {
		...
		const destination = entries.find(
			(entry) =>
				entry.group.projectKey === destinationRepositoryIdentity.canonicalKey,
		);
		const fallback = entries.find(
			(entry) => entry.group.displayName === "fallback",
		);
