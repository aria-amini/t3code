...
type PrimaryEnvironmentRequestOperation =
	typeof PrimaryEnvironmentRequestOperation.Type;
...
export class PrimaryEnvironmentRequestError extends Schema.TaggedErrorClass<PrimaryEnvironmentRequestError>()(
	...
	static fromCause(input: {
		...
		return new PrimaryEnvironmentRequestError({
			...
			status,
			...(input.pairingLinkId !== undefined
				? { pairingLinkId: input.pairingLinkId }
				: {}),
...
const isPrimaryEnvironmentRequestError = Schema.is(
	PrimaryEnvironmentRequestError,
);
...
function getDesktopBootstrapCredential(): string | null {
	...
	const primary = bootstraps.find(
		(entry) => entry.id === PRIMARY_LOCAL_ENVIRONMENT_ID,
	);
	return typeof primary?.bootstrapToken === "string" &&
		primary.bootstrapToken.length > 0
...
function readEnvironmentHttpErrorStatus(
	error: EnvironmentHttpCommonErrorType,
): number {
	switch (error._tag) {
			...
			return await runPrimaryHttp(
				PrimaryEnvironmentHttpClient.pipe(
					Effect.flatMap((client) =>
						client.auth.browserSession({ payload: { credential } }),
					),
				),
			);
		} catch (error) {
...
export async function revokeServerPairingLink(id: string): Promise<void> {
	try {
		await runPrimaryHttp(
			PrimaryEnvironmentHttpClient.pipe(
				Effect.flatMap((client) =>
					client.auth.revokePairingLink({ headers: {}, payload: { id } }),
				),
			),
		);
	} catch (error) {
...
export async function revokeServerClientSession(
