...
describe("environmentBootstrap", () => {
	...
	it("uses https descriptor urls when the primary environment uses wss", async () => {
		...
		await expect(resolveInitialPrimaryEnvironmentDescriptor()).resolves.toEqual(
			BASE_ENVIRONMENT,
		);
		expect(
			resolvePrimaryEnvironmentHttpUrl("/.well-known/t3/environment"),
		).toBe("https://remote.example.com/.well-known/t3/environment");
	});
	...
	it("uses the current origin as the descriptor base for local dev environments", async () => {
		...
		expect(
			resolvePrimaryEnvironmentHttpUrl("/.well-known/t3/environment"),
		).toBe("http://localhost:5735/.well-known/t3/environment");
	});
	...
	it("uses the vite proxy for desktop-managed loopback descriptor requests during local dev", async () => {
		...
		expect(
			resolvePrimaryEnvironmentHttpUrl("/.well-known/t3/environment"),
		).toBe("http://127.0.0.1:5733/.well-known/t3/environment");
	});
	...
	it("retains the URL parser cause without exposing the configured URL in its message", () => {
		...
		expect(error).toMatchObject({
			...
			message:
				"Could not parse http-base-url for the configured primary environment target.",
		});
	...
	it("describes which desktop bootstrap endpoint is missing", () => {
		...
		expect(error).toMatchObject({
			...
			hasWsBaseUrl: false,
			message:
				"Desktop bootstrap is missing wsBaseUrl for the local environment.",
		});
	});
	...
	it("preserves an unsupported window-origin protocol", () => {
		...
		if (!isPrimaryEnvironmentProtocolUnsupportedError(error)) {
			throw new Error(
				"Expected a structured primary environment protocol error.",
			);
		}
		expect(error).toMatchObject({
			source: "window-origin",
			protocol: "file:",
			message:
				"The window-origin primary environment target uses unsupported protocol file:.",
		});
	});
});
