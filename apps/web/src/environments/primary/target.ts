...
type PrimaryEnvironmentTargetSource =
	typeof PrimaryEnvironmentTargetSource.Type;
...
export const isPrimaryEnvironmentUrlInvalidError = Schema.is(
	PrimaryEnvironmentUrlInvalidError,
);
...
function getDesktopLocalEnvironmentBootstrap(): DesktopEnvironmentBootstrap | null {
	...
	return (
		bootstraps.find((entry) => entry.id === PRIMARY_LOCAL_ENVIRONMENT_ID) ??
		null
	);
}
...
function resolveConfiguredPrimaryTarget(): PrimaryEnvironmentTarget | null {
	const configuredHttpBaseUrl =
		import.meta.env.VITE_HTTP_URL?.trim() || undefined;
