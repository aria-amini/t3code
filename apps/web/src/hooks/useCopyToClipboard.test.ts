...
describe("writeTextToClipboard", () => {
	...
	it.each(["success", "denied", "throws"] as const)(
		...
		async (result) => {
			...
			if (result === "success") {
				...
				await expect(pendingCopy).rejects.toBeInstanceOf(
					ClipboardApiUnavailableError,
				);
			}
	...
	it("uses the Clipboard API without touching the fallback when it is available", async () => {
		...
		await expect(
			writeTextToClipboard("remote command", "command"),
		).resolves.toBe(true);
	...
	it("preserves the exact clipboard failure without exposing copied contents", async () => {
		...
		const error = await writeTextToClipboard(
			"secret clipboard contents",
			"error-message",
		).then(
			() => undefined,
