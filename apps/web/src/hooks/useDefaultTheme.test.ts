...
describe("default theme adoption", () => {
	...
	it("does not replay a generation it already applied", () => {
		const generation = defaultThemeGeneration(
			BASE.defaultTheme,
			BASE.defaultThemeSetAt,
		);
		expect(
			defaultThemeToApply({ ...BASE, appliedGeneration: generation }),
		).toBe(null);
	});
	...
	it("applies the same theme again when the environment re-sets it", () => {
		const applied = defaultThemeGeneration(
			BASE.defaultTheme,
