...
const testState = vi.hoisted(() => {
	...
	const router = {
		...
		navigate: vi.fn(
			async (request: { readonly params: { readonly draftId: string } }) => {
...
vi.mock("@t3tools/client-runtime/environment", () => ({
	...
	scopeThreadRef: (environmentId: string, threadId: string) => ({
		environmentId,
		threadId,
	}),
}));
...
describe("useNewThreadHandler", () => {
	it.each([
		...
		"abandons a delayed %s draft open when the user navigates elsewhere",
		async (_, draft) => {
			...
			expect(
				testState.draftStore.setLogicalProjectDraftThreadId,
