	...
	return useCallback(
		(
			...
			const targetServerSettings =
				environmentServerConfigs.get(projectRef.environmentId)?.settings ??
				DEFAULT_SERVER_SETTINGS;
			const {
				getComposerDraft,
			...
			const composerActiveProvider =
				carrySourceComposer?.activeProvider ?? null;
			...
			const resolveDefaultEnvMode = async (): Promise<DraftThreadEnvMode> => {
				const consultProjectFile =
					project !== undefined && project.defaultThreadEnvMode == null;
			...
			if (emptyStoredDraftThread) {
				return (async () => {
					...
					const storedDraftHasExplicitModelPick =
						hasExplicitComposerModelSelection(storedDraft);
