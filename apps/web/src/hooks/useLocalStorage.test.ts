...
describe("local storage errors", () => {
	it("preserves read failure context", async () => {
		...
		const { getLocalStorageItem, LocalStorageOperationError } =
			await loadWithStorage(
				createStorage({
					getItem: () => {
						throw cause;
					},
	...
	it.each(["", "not-json"])(
		"preserves decode failure context for %j",
		async (value) => {
			const { getLocalStorageItem, LocalStorageOperationError } =
				await loadWithStorage(createStorage({ getItem: () => value }));
	...
	it("preserves write failure context", async () => {
		...
		const { LocalStorageOperationError, setLocalStorageItem } =
			await loadWithStorage(
				createStorage({
					setItem: () => {
						throw cause;
	...
	it("preserves removal failure context", async () => {
		const cause = new Error("storage unavailable");
		const { LocalStorageOperationError, removeLocalStorageItem } =
			await loadWithStorage(
				createStorage({
					removeItem: () => {
						throw cause;
