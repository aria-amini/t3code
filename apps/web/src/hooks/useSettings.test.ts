...
describe("client settings hydration", () => {
	const savedSettings = {
		...
		favorites: [
			{ provider: ProviderInstanceId.make("codex_work"), model: "gpt-5.6" },
		],
	};
	const onboardingCompletedAt = "2026-09-05T12:00:00.000Z";
	const complete = (current: ClientSettings) => ({
		...current,
	...
	it("rejects completion after a failed read and preserves saved preferences on retry", async () => {
		...
		await expect(persistClientSettingsUpdate(complete)).resolves.toEqual(
			completedSettings,
		);
		expect(persistenceMocks.setClientSettings).toHaveBeenCalledExactlyOnceWith(
			completedSettings,
		);
	...
	it("preserves patch order across hydration and a blocked completion write", async () => {
		...
		const finalWrite = persistClientSettingsUpdate(
			(current) => current,
			persist,
		);
...
describe("persistClientSettingsPatch", () => {
	it("waits for settings writes in request order", async () => {
		...
		expect(persistenceMocks.setClientSettings).toHaveBeenNthCalledWith(
			1,
			firstSettings,
		);
		expect(persistenceMocks.setClientSettings).toHaveBeenNthCalledWith(
...
describe("persistClientSettingsUpdate", () => {
	it("publishes the update only after persistence succeeds", async () => {
		...
		expect(getClientSettings().timestampFormat).toBe(
			DEFAULT_CLIENT_SETTINGS.timestampFormat,
		);
		finishPersistence();
	...
	it("preserves an optimistic write made while an awaited update persists", async () => {
		...
		expect(persist.mock.calls[1]?.[0].browserProfiles).toContainEqual(
			importedProfile,
		);
	...
	it("continues the queue after a rejected write", async () => {
		...
		await expect(
			persistClientSettingsUpdate(
				(current) => ({ ...current, wordWrap: false }),
				persist,
			),
...
describe("onboarding completion persistence", () => {
	it("keeps onboarding incomplete after a failed save and preserves preferences on retry", async () => {
		...
		await expect(persistClientSettingsUpdate(complete, persist)).rejects.toBe(
			failure,
		);
		...
		await expect(
			persistClientSettingsUpdate(complete, persist),
		).resolves.toEqual(completedSettings);
