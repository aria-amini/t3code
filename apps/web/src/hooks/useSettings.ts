...
type ClientSettingsHydrationStatus =
	| "pending"
	| "ready"
	| "failed"
	| "retrying";
...
function setClientSettingsHydrationStatus(
	nextStatus: ClientSettingsHydrationStatus,
): void {
...
async function hydrateClientSettings(): Promise<void> {
	...
	setClientSettingsHydrationStatus(
		clientSettingsHydrationStatus === "failed" ||
			clientSettingsHydrationStatus === "retrying"
			? "retrying"
			: "pending",
	);
...
const defaultClientSettingsPersistence = (
	settings: ClientSettings,
): Promise<void> => ensureLocalApi().persistence.setClientSettings(settings);

function enqueueClientSettingsPersistence<A>(
	work: () => Promise<A>,
): Promise<A> {
...
export function persistClientSettingsPatch(
	...
	persist: (
		settings: ClientSettings,
	) => Promise<void> = defaultClientSettingsPersistence,
): void {
...
export function useEnvironmentIdentificationMode(): EnvironmentIdentificationMode {
	...
	return resolveEnvironmentIdentificationMode({
		...
		paletteThemeAllowsArtwork:
			previewSidebarArtwork ?? themeAllowsSidebarArtwork(activeTheme),
	});
}
...
export function useSharedSettingsSync() {
	...
	const primaryCapabilities =
		primaryEnvironment?.serverConfig?.environment.capabilities;
	...
	const primarySettings =
		...
		supportsSharedSettingsSync(primaryEnvironment)
