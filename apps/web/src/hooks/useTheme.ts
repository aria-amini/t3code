...
function isThemePreferenceMode(
	value: string | null,
): value is ThemePreferenceMode {
	return value === "light" || value === "dark" || value === "system";
}

export function readAppearanceModePreference(
	theme: Theme,
): ThemePreferenceMode {
...
function writeAppearanceModePreference(
	appearanceMode: ThemePreferenceMode,
): void {
	if (typeof window === "undefined") return;
	try {
...
export function syncBrowserChromeTheme() {
	...
	const themeChromeColor = document.documentElement.dataset.themeId
		? normalizeThemeColor(
				rootStyles.getPropertyValue("--app-chrome-background"),
			)
		: null;
...
function applyTheme(
	theme: Theme,
	{ suppressTransitions = false, preservePreview = true } = {},
) {
	...
	if (
		...
		themeHalvesSignature(lastAppliedTheme.themeHalves) ===
			themeHalvesSignature(themeHalves)
	) {
...
export function syncDesktopTheme(
	...
	if (
		!bridge ||
		typeof bridge.setTheme !== "function" ||
...
function getSnapshot(): ThemeSnapshot {
	...
	if (
		...
		themeHalvesSignature(lastSnapshot.themeHalves) ===
			themeHalvesSignature(themeHalves)
	) {
		return lastSnapshot;
	}
...
export function useTheme() {
	...
	const setTheme = useCallback((next: Theme): boolean => {
		...
		try {
			...
			const previousHalvesRaw = window.localStorage.getItem(
				THEME_HALVES_STORAGE_KEY,
			);
	...
	const setAppearanceMode = useCallback(
		(nextAppearanceMode: ThemePreferenceMode): boolean => {
			if (typeof window === "undefined") return false;
			try {
