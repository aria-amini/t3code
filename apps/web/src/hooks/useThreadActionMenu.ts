	...
	const openMenu = useCallback(
		(position: { x: number; y: number }) => {
			...
			void (async () => {
				...
				const now = new Date();
				const supports = {
					settlement: readEnvironmentSupportsSettlement(
						threadRef.environmentId,
					),
					snooze: readEnvironmentSupportsSnooze(threadRef.environmentId),
					pinning: readEnvironmentSupportsPinning(threadRef.environmentId),
					titleRegeneration: readEnvironmentSupportsTitleRegeneration(
						threadRef.environmentId,
					),
				};
				...
				const items = buildThreadActionMenuItems({
					...
					isSettled:
						supports.settlement && thread.settledOverride === "settled",
					isSnoozed:
						supports.snooze &&
				...
				if (action.startsWith("snooze:")) {
					const preset = snoozePresets.find(
						(candidate) => `snooze:${candidate.id}` === action,
					);
					if (!preset) return;
							...
							actionProps: {
								...
								onClick: () => {
									void unsnoozeThread(threadRef).then((undone) => {
										if (
											undone._tag === "Failure" &&
											!isAtomCommandInterrupted(undone)
										) {
											failureToast(
				...
				switch (action) {
					...
					case "settle":
						await reportFailure("Failed to settle thread", () =>
							settleThread(threadRef),
						);
						return;
					case "unsettle":
						await reportFailure("Failed to un-settle thread", () =>
							unsettleThread(threadRef),
						);
						return;
					case "unsnooze":
						await reportFailure("Failed to wake thread", () =>
