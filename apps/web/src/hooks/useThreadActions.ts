...
export function useThreadActions() {
	...
	const reorderPinnedThreadMutation = useAtomCommand(
		threadEnvironment.reorderPin,
		{
			reportFailure: false,
		},
	);
	const reorderActiveThreadMutation = useAtomCommand(
	...
	const confirmThreadUnpin = useClientSettings(
	...
	const clearTerminalUiState = useTerminalUiStateStore(
	...
	const deleteThread = useCallback(
		async (
			...
			const { thread, threadRef } = resolved;
			const threads = readEnvironmentThreadRefs(
				...
				opts.deletedThreadKeys && opts.deletedThreadKeys.size > 0
					? new Set<ThreadId>(
							[...opts.deletedThreadKeys].flatMap((threadKey) => {
								...
								return ref && ref.environmentId === threadRef.environmentId
									? [ref.threadId]
									: [];
							}),
						)
			...
			const survivingThreads =
				deletedIds && deletedIds.size > 0
					? threads.filter(
							(entry) =>
								entry.id === threadRef.threadId || !deletedIds.has(entry.id),
						)
					: threads;
