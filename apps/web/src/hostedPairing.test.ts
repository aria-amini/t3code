...
describe("hostedPairing", () => {
	...
	it("reads hosted pairing host and query token parameters", () => {
		const url = new URL(
			"https://app.t3.codes/pair?host=100.64.1.2:3773&token=ABCD1234",
		);
	...
	it("ignores incomplete hosted pairing requests", () => {
		expect(
			hasHostedPairingRequest(
				new URL("https://app.t3.codes/pair?host=backend.example.com"),
			),
		).toBe(false);
		expect(
			hasHostedPairingRequest(
				new URL("https://app.t3.codes/pair?token=ABCD1234"),
			),
		).toBe(false);
	});
	...
	it("detects the hosted static app only when no backend URL is configured", () => {
		...
		expect(isHostedStaticApp(new URL("https://preview.t3.codes/pair"))).toBe(
			true,
		);
		expect(isHostedStaticApp(new URL("https://backend.example.com/"))).toBe(
			false,
		);
	...
	it("detects hosted channel aliases as static apps", () => {
		...
		expect(isHostedStaticApp(new URL("https://nightly.app.t3.codes/"))).toBe(
			true,
		);
