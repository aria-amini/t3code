...
describe("copy thread reference shortcut", () => {
	it("resolves Cmd+Shift+C on macOS and Ctrl+Shift+C elsewhere", () => {
		assert.equal(
			resolveShortcutCommand(
				event({ key: "c", metaKey: true, shiftKey: true }),
				DEFAULT_BINDINGS,
				{
					platform: "MacIntel",
				},
...
describe("shortcutLabelForCommand", () => {
	...
	it("returns null for commands shadowed by a later conflicting shortcut", () => {
		...
		assert.isNull(
			shortcutLabelForCommand(bindings, "thread.jump.1", "MacIntel"),
		);
		assert.strictEqual(
			shortcutLabelForCommand(bindings, "thread.jump.7", "MacIntel"),
			"⇧⌘1",
		);
	});
...
describe("thread navigation helpers", () => {
	...
	it("maps traversal commands to directions", () => {
		assert.strictEqual(
			threadTraversalDirectionFromCommand("thread.previous"),
			"previous",
		);
		assert.strictEqual(
			threadTraversalDirectionFromCommand("thread.next"),
			"next",
		);
...
describe("model picker navigation helpers", () => {
	it("maps jump commands to visible model indices", () => {
		...
		assert.strictEqual(
			modelPickerJumpIndexFromCommand("modelPicker.jump.1"),
			0,
		);
		assert.strictEqual(
			modelPickerJumpIndexFromCommand("modelPicker.jump.3"),
			2,
		);
...
describe("resolveShortcutCommand", () => {
	...
	it("resolves a custom right panel maximize binding", () => {
		...
		assert.strictEqual(
			resolveShortcutCommand(
				event({ key: "m", metaKey: true, shiftKey: true }),
				keybindings,
				{
					platform: "MacIntel",
				},
	...
	it("does not let a punctuation position shadow a Latin layout key", () => {
		...
		assert.strictEqual(
			resolveShortcutCommand(
				event({ key: "m", code: "Semicolon", metaKey: true }),
				keybindings,
				{
					platform: "MacIntel",
				},
	...
	it("matches non-Latin layout letters using the physical key code", () => {
		...
		assert.strictEqual(
			resolveShortcutCommand(
				event({ key: "в", code: "KeyD", metaKey: true }),
				keybindings,
				{
					platform: "MacIntel",
				},
	...
	it("ignores the physical key code when the layout types a different Latin letter", () => {
		...
		assert.isNull(
			resolveShortcutCommand(
				event({ key: "a", code: "KeyD", metaKey: true }),
				keybindings,
				{
					platform: "MacIntel",
				},
		...
		assert.strictEqual(
			resolveShortcutCommand(
				event({ key: "d", code: "KeyL", metaKey: true }),
				keybindings,
				{
					platform: "MacIntel",
				},
...
describe("formatShortcutLabel", () => {
	...
	it("formats labels for plus key", () => {
		...
		assert.strictEqual(
			formatShortcutLabel(modShortcut("+"), "Linux"),
			"Ctrl++",
		);
	});
});
...
describe("terminalNavigationShortcutData", () => {
	...
	it("maps Cmd+Arrow on macOS to line movement", () => {
		assert.strictEqual(
			terminalNavigationShortcutData(
				event({ key: "ArrowLeft", metaKey: true }),
				"MacIntel",
			),
			"\u0001",
		);
	...
	it("maps Ctrl+Arrow on non-macOS to word movement", () => {
		assert.strictEqual(
			terminalNavigationShortcutData(
				event({ key: "ArrowLeft", ctrlKey: true }),
				"Win32",
			),
			"\u001bb",
		);
...
describe("plus key parsing", () => {
	it("matches the plus key shortcut", () => {
		const plusBindings = compile([
			{ shortcut: modShortcut("+"), command: "terminal.toggle" },
		]);
		assert.isTrue(
			isTerminalToggleShortcut(
