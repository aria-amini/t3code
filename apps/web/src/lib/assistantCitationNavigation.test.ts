...
describe("assistant citation navigation", () => {
	it("preserves encoded quote whitespace through router navigation and reload", async () => {
		...
		expect(assistantCitationFromLocation(router.state.location.href)).toEqual(
			citation,
		);
	...
	it("ignores non-citation fragments and missing fragments", () => {
		expect(
			assistantCitationFromLocation("/environment-one/thread-one"),
		).toBeNull();
		expect(
			assistantCitationFromLocation(
				"/environment-one/thread-one#ordinary-heading",
			),
		).toBeNull();
		expect(
			assistantCitationFromLocation("/a/b#assistant-citation=%invalid"),
		).toBeNull();
		expect(
			assistantCitationFromLocation(
				`/a/b#assistant-citation=${"a".repeat(140_000)}`,
			),
		).toBeNull();
	});
});
