...
function assistantSource(...children: SelectionNode[]) {
	return new SelectionNode("DIV", "", {
		"data-assistant-citation-source": "assistant",
	}).append(...children);
}
...
function nativeSelection(
	...
	index(root);
	const at = (node: SelectionNode, offset: number) =>
		positions.get(node)![offset]!;
	...
	class SelectionRange {
		...
		get commonAncestorContainer() {
			...
			while (!ancestor.contains(this.endContainer))
...
function capture(viewport: SelectionNode, selection: Selection) {
	return captureAssistantTextSelection(
...
describe("captureAssistantTextSelection", () => {
	it.each([
		...
		({ boundary, backwards }) => {
			...
			const nextBlock = assistantSource(
				new SelectionNode("P").append(nextText),
			);
	...
	it("finds the source when both endpoints are parent boundaries", () => {
		...
		const viewport = new SelectionNode("MAIN").append(
			source,
			assistantSource(textNode("Next.")),
		);
		expect(
			capture(viewport, nativeSelection([viewport, 0], [viewport, 1]))
				?.selector,
		).toEqual(selector(quote.data));
	});
	...
	it("captures the final paragraph when Chromium extends to the timestamp paragraph boundary", () => {
		...
		new SelectionNode("MAIN").append(
			new SelectionNode("DIV").append(
				source,
				new SelectionNode("DIV").append(timestamp),
			),
		);
		...
		expect(
			capture(source, nativeSelection([quote, 0], [timestampText, 1])),
		).toBeNull();
	});
	...
	it.each([false, true])(
		...
		(backwards) => {
			...
			expect(
				capture(viewport, nativeSelection([first, 0], [second, 1], backwards)),
			).toBeNull();
		},
	);
	...
	it("accepts a control endpoint only when none of its text is selected", () => {
		...
		expect(
			capture(source, nativeSelection([quote, 0], [control, 0]))?.selector,
		).toEqual(selector(quote.data));
		expect(
			capture(source, nativeSelection([quote, 0], [control, 1])),
		).toBeNull();
	});
	...
	it.each([
		...
		expect(
			capture(source, nativeSelection([control, 0], [control, control.length])),
		).toBeNull();
	});
	...
	it("keeps exact multiline Unicode text and omits interior controls and hidden text", () => {
		...
		const source = assistantSource(
			new SelectionNode("P").append(
				first,
				new SelectionNode("EM").append(textNode("cafe\u0301")),
			),
		...
		expect(
			capture(source, nativeSelection([first, 0], [code, code.length], true))
				?.selector,
		).toEqual(
			selector("Before 😀 cafe\u0301\nfirst line\r\n  second line\t🚀"),
		);
	});
	...
	it("rejects a source outside the supplied viewport", () => {
		...
		expect(
			capture(
				new SelectionNode("MAIN"),
				nativeSelection([quote, 0], [quote, quote.length]),
			),
		).toBeNull();
	});
});
...
describe("createAssistantTextSelector", () => {
	...
	it.each([
		...
		"keeps complete context code points with %i neighboring UTF-16 units",
		(paddingLength) => {
	...
	it("preserves multiline quotes and their matching location through a citation URL", () => {
		...
		const captured = createAssistantTextSelector(
			text,
			rawStart,
			rawStart + quote.length,
		);
	...
	it.each(["prefix", "suffix"])(
		...
		(side) => {
			const prefix = "x".repeat(
				ASSISTANT_CITATION_CONTEXT_LENGTH - (side === "prefix" ? 1 : 0),
			);
			const suffix = "y".repeat(
				ASSISTANT_CITATION_CONTEXT_LENGTH - (side === "suffix" ? 1 : 0),
			);
	...
	it.each(["", " \n\t\r\n"])(
		"rejects empty or whitespace-only selections: %j",
		(text) => {
...
describe("findAssistantCitationText", () => {
	...
	it.each([
		...
		({ expected, ...context }) => {
			expect(
				findAssistantCitationText(
					"first quote / quote last",
					selector("quote", context),
				),
			).toEqual(expected);
		},
	);
	...
	it("does not guess between quotes without context, even at the saved position", () => {
		expect(
			findAssistantCitationText("quote / quote", selector("quote")),
		).toBeNull();
	});
	...
	it("uses up to 32 UTF-16 context units without requiring the entire surrounding text", () => {
		...
		expect(
			findAssistantCitationText(text, selector("quote", { prefix, suffix })),
		).toEqual({
			start,
			end: start + 5,
		});
	...
	it.each([
		...
		"treats invalid stored offsets as unusable hints: $start / $end",
		(offsets) => {
			expect(
				findAssistantCitationText("a quote", selector("quote", offsets)),
			).toEqual({
				start: 2,
				end: 7,
			});
