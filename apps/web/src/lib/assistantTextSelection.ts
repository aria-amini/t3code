...
export function findAssistantCitationSourceAnchor(
	...
	const source = [
		...document.querySelectorAll<HTMLElement>(
			"[data-assistant-citation-source]",
		),
	].find(
		(element) =>
...
export function createAssistantTextSelector(
	...
	if (
		rawStart > 0 &&
		/\s/.test(text[rawStart - 1]!) &&
		/\s/.test(text[rawStart]!)
	) {
		start -= 1;
	}
	...
	let suffixEnd = Math.min(
		normalized.length,
		end + ASSISTANT_CITATION_CONTEXT_LENGTH,
	);
...
export function captureAssistantTextSelection(
	...
	const first = selectedTextBoundary(
		range,
	...
	const source = first.parentElement?.closest<HTMLElement>(
		"[data-assistant-citation-source]",
	);
	...
	range.setEnd(
		last,
		last === range.endContainer ? range.endOffset : last.length,
	);
	...
	for (const chunk of stream.chunks) {
		...
		const end =
			range.endContainer === chunk.node ? range.endOffset : chunk.node.length;
...
function rawTextOffset(text: string, normalizedOffset: number): number {
	...
	for (const match of text.matchAll(/\s+|\S+/g)) {
		...
		if (normalizedOffset <= offset + length) {
			return (
				match.index +
				(whitespace && normalizedOffset > offset
					? match[0].length
