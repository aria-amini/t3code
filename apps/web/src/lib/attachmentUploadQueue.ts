...
export function readAttachmentUpload(
	imageId: string,
...
function resolveCurrentFileDraftTarget(
	job: UploadJob,
): ComposerThreadTarget | undefined {
...
async function runUpload(job: UploadJob): Promise<void> {
	...
	const mimeType =
		job.image.type === "file"
			...
			: PROVIDER_SEND_TURN_SUPPORTED_IMAGE_MIME_TYPES.find(
					(supportedMimeType) =>
						supportedMimeType === job.image.mimeType.toLowerCase(),
				);
	if (!mimeType) {
		setUploadState(job.image.id, {
			status: "failed",
	...
	const result = await runAttachmentUploadCycle({
		...
		resolveUploadUrl: (relativeUrl) => {
			...
			return connection
				? resolveAssetUrl(connection.httpBaseUrl, relativeUrl)
				: null;
		},
		transport: (url) =>
			uploadBytes({
	...
	for (let index = 0; index < queue.length;) {
		...
		void runUpload(job)
			...
			.finally(() => {
				...
				const remaining =
					(activeUploadsByEnvironment.get(job.environmentId) ?? 1) - 1;
				if (remaining > 0) {
...
export function startAttachmentUpload(input: {
	...
	const job: UploadJob = {
		...
		...(input.draftTarget !== undefined
			? { draftTarget: input.draftTarget }
			: {}),
		...(previous ? { previous } : {}),
...
export async function awaitAttachmentUploads(
	...
	await Promise.all(
		imageIds.map((imageId) => jobsByImageId.get(imageId)?.settled),
	);
}
...
export function releaseDraftAttachment(
	...
	if (
		...
		if (
			jobsByImageId.has(attachment.id) ||
			readAttachmentUpload(attachment.id)
		) {
