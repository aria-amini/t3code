...
export function wasRecentlyInteracted(
...
function createActivityReport(
	...
	return {
		...
		recentlyInteracted: wasRecentlyInteracted(
			lastInteractionAtMs,
			observedAtMs,
		),
...
function scopeForSubscription(
	...
	return typeof input.cwd === "string"
		? { type: "vcs-status", cwd: input.cwd }
		: null;
}

function retainBackgroundScope(
	environmentId: EnvironmentId,
	scope: BackgroundScope,
): () => void {
...
export function observeBackgroundActivitySubscription(
	...
	return Effect.sync(() => {
		const release = retainBackgroundScope(
...
export const backgroundActivityReporterLayer = Layer.effectDiscard(
	Effect.gen(function* () {
		...
		const recordInteraction = () => {
			...
			const wasRecent = wasRecentlyInteracted(
				lastInteractionAtMs,
				observedAtMs,
			);
			lastInteractionAtMs = observedAtMs;
