...
function makeHeicFile(options?: {
	...
	const makeBox = (name: string, ...contents: Uint8Array[]) => {
		const bytes = new Uint8Array(
			8 + contents.reduce((size, content) => size + content.length, 0),
		);
	...
	const properties = makeBox(
		"iprp",
		makeBox("ipco", makeBox("ispe", dimensions)),
	);
	...
	vi.stubGlobal(
		...
		class {
			...
			async convertToBlob({
				...
				const resolvedType =
					type === "image/webp" && !supportsWebp ? "image/png" : type;
				return new Blob([new Uint8Array(sizeForQuality(quality))], {
					type: resolvedType,
				});
			}
		},
...
describe("compressImageForStash", () => {
	it("stores a small image verbatim without re-encoding", async () => {
		...
		expect(result.ok && result.image.dataUrl.startsWith("data:image/png")).toBe(
			true,
		);
	...
	it("re-encodes an oversized image to WebP within the budget", async () => {
		...
		expect(
			result.ok &&
				result.image.dataUrl.length <= MAX_STASH_IMAGE_DATA_URL_CHARS,
		).toBe(true);
	...
	it("falls back to JPEG with a white matte when WebP encoding is unavailable", async () => {
		const { fillRect } = stubCanvasPipeline(() => 120_000, {
			supportsWebp: false,
		});
	...
	it("steps quality down until the encoded image fits", async () => {
		...
		const { close } = stubCanvasPipeline((quality) =>
			quality <= 0.68 ? 400_000 : 3_000_000,
		);
...
describe("HEIC attachment preparation", () => {
	it("recognizes HEIC and HEIF MIME types and case-insensitive file extensions", () => {
		expect(isHeicImageFile({ name: "photo.bin", type: "image/heic" })).toBe(
			true,
		);
		expect(isHeicImageFile({ name: "photo.bin", type: "image/heif" })).toBe(
			true,
		);
		expect(
			isHeicImageFile({ name: "photo.heic", type: "image/heic-sequence" }),
		).toBe(false);
		expect(
			isHeicImageFile({ name: "photo.heif", type: "image/heif-sequence" }),
		).toBe(false);
		expect(isHeicImageFile({ name: "IMG_1234.HEIC", type: "" })).toBe(true);
		expect(
			isHeicImageFile({ name: "photo.heif", type: "application/octet-stream" }),
		).toBe(true);
		expect(isHeicImageFile({ name: "photo.png", type: "image/png" })).toBe(
			false,
		);
		expect(isHeicImageFile({ name: "photo.heic", type: "image/png" })).toBe(
			false,
		);
		expect(isHeicImageFile({ name: "photo.heif", type: "image/jpeg" })).toBe(
			false,
		);
	});
	...
	it("rejects unsafe HEIC sources before loading the decoder", async () => {
		...
		Object.defineProperty(original, "size", {
			value: MAX_COMPRESSIBLE_SOURCE_BYTES + 1,
		});
		...
		async (path) => {
			...
			expect(restored?.source?.accessibility).toMatchObject({
				...
				root: {
					...
					children: [
						{ bounds: { x: 1920, y: 1120, width: 80, height: 80 } },
						{ bounds: null },
					],
				},
			});
			expect(source.accessibility).toMatchObject({
				imageSize: { width: 2560, height: 1600 },
			});
		},
	);
