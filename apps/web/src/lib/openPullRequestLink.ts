 ...
 */
export function pullRequestCandidateUrlFromReferenceAutolink(
	targetUrl: string,
): string | null {
	let url: URL;
	try {
	...
	return useCallback(
		(event, targetUrl, targetThreadRef, targetEnvironmentId) => {
			...
			const reads = (environmentId: string) =>
				serverConfigs.get(environmentId as EnvironmentId)?.environment
			...
			const projects = resolvedThreadRef
				? allProjects.filter(
						(project) =>
							project.environmentId === resolvedThreadRef.environmentId,
					)
				: targetEnvironmentId
					? allProjects.filter(
...
export function useOpenPrLink(threadRef?: ScopedThreadRef) {
	...
	return useCallback(
		(
			...
			event.preventDefault();
			if (!openInBrowser && openChangeRequest(event, prUrl, targetThreadRef))
				return true;
			...
			void openLink(prUrl, { event, threadRef: targetThreadRef }).catch(
				(error: unknown) => {
					console.error(error);
					toastManager.add(
						stackedThreadToast({
