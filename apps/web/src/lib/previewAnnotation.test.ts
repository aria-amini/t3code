...
describe("preview annotations", () => {
	...
	it("extracts multiple trailing annotations one at a time", () => {
		...
		const extractedFirst = extractTrailingPreviewAnnotation(
			extractedSecond.promptText,
		);
...
describe("preview annotation capture", () => {
	...
	it("fails instead of hanging when the crop never arrives", async () => {
		...
		const capturePromise = capturePreviewAnnotationScreenshot(
			annotation,
			1_000,
		);
	...
	it("fails when the crop fetch throws", async () => {
		...
		expect(await capturePreviewAnnotationScreenshot(annotation)).toEqual({
			status: "failed",
		});
	});
});
