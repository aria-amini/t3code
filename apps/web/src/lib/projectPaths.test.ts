...
describe("projectPaths", () => {
	...
	it("normalizes windows-style paths for comparison", () => {
		expect(normalizeProjectPathForComparison("C:/Work/Repo/")).toBe(
			"c:\\work\\repo",
		);
		expect(normalizeProjectPathForComparison("C:\\Work\\Repo\\")).toBe(
			"c:\\work\\repo",
	...
	it("infers project titles from normalized paths", () => {
		...
		expect(inferProjectTitleFromPath("/home/user\\project/")).toBe(
			"user\\project",
		);
	});
	...
	it("only treats windows-style paths as browse queries on windows", () => {
		...
		expect(isUnsupportedWindowsProjectPath("C:\\Work\\Repo\\", "Win32")).toBe(
			false,
		);
	});
	...
	it("resolves explicit relative paths against the current project", () => {
		...
		expect(resolveProjectPathForDispatch("./docs", "/repo/app")).toBe(
			"/repo/app/docs",
		);
		expect(resolveProjectPathForDispatch("../docs", "/repo/app")).toBe(
			"/repo/docs",
		);
		expect(resolveProjectPathForDispatch("./Repo", "C:\\Work")).toBe(
			"C:\\Work\\Repo",
		);
	...
	it("navigates browse paths with matching separators", () => {
		...
		expect(appendBrowsePathSegment("C:\\Work\\", "Repo")).toBe(
			"C:\\Work\\Repo\\",
		);
		...
		expect(getBrowseParentPath("\\\\server\\share\\repo\\")).toBe(
			"\\\\server\\share\\",
		);
		expect(getBrowseParentPath("C:\\")).toBeNull();
		expect(getBrowseParentPath("/home/user\\project/docs/")).toBe(
			"/home/user\\project/",
		);
	});
	...
	it("detects browse path boundaries", () => {
		...
		expect(getBrowseDirectoryPath("/home/user\\project/docs")).toBe(
			"/home/user\\project/",
		);
