...
function createSelectionSurface({ interactiveActions = false } = {}) {
	...
	const up = ({ x = 300, y = 200, detail = 1, button = 0 } = {}) => {
		pointerUp(button);
		view.dispatchEvent(
			event("mouseup", { clientX: x, clientY: y, detail, button }),
		);
	};
	const flushFrame = () => {
...
describe("selection action gestures", () => {
	...
	it("uses the text fallback after keyboard selection rather than a stale pointer", () => {
		...
		surface.up();
		surface.flush();
		surface.document.dispatchEvent(
			event("keydown", { key: "ArrowRight", shiftKey: true }),
		);
		surface.change();
		surface.flush();
	...
	it.each([
		...
		(reason) => {
			...
			surface.up({ detail: 2 });
			if (reason === "Escape")
				surface.document.dispatchEvent(event("keydown", { key: reason }));
