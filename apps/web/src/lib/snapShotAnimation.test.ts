...
describe("window capture animation", () => {
	...
	it("keeps each reserved composer slot with its target and source", () => {
		...
		expect(pendingSnapShotAnimationIdsForTarget(pending, draft)).toEqual([
			"capture-1",
		]);
	...
	it("keeps one card through Strict Mode and the attachment handoff", async () => {
		...
		const stopFirstSetup = scheduleSnapShotAnimationDestination(
			"capture-1",
			start,
		);
		stopFirstSetup();
		const stopPlaceholder = scheduleSnapShotAnimationDestination(
			"capture-1",
			start,
		);
		...
		stopPlaceholder();
		const stopAttachment = scheduleSnapShotAnimationDestination(
			"capture-1",
			start,
		);
		await Promise.resolve();
	...
	it("uses composer arrival feedback only for a fresh capture", () => {
		...
		expect(shouldAnimateSnapShotArrival("2026-09-01T12:00:02.000Z", now)).toBe(
			true,
		);
		expect(shouldAnimateSnapShotArrival("2026-09-01T11:59:00.000Z", now)).toBe(
			false,
		);
