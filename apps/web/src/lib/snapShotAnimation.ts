...
export function shouldAnimateSnapShotArrival(
	capturedAt: string,
	now = Date.now(),
): boolean {
...
export function beginSnapShotAnimation(
	id: string,
...
export function updateSnapShotAnimationSource(
	id: string,
...
export function subscribeToPendingSnapShotAnimations(
	listener: () => void,
): () => void {
...
export async function waitForSnapShotAnimationDestination(
	id: string,
): Promise<void> {
...
export function scheduleSnapShotAnimationDestination(
	id: string,
	start: () => void,
): () => void {
	let active = true;
	...
	return () => {
		if (!active) return;
		active = false;
		const remainingMounts = Math.max(
			0,
			(destinationMountCounts.get(id) ?? 1) - 1,
		);
