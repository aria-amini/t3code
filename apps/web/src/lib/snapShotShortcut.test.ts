...
describe("window capture shortcut labels", () => {
	it("labels the default physical Shift pair", () => {
		expect(
			formatSnapShotShortcutLabel({ kind: "both-shift-keys" }, "MacIntel"),
		).toBe("Shift + Shift");
	});
	...
	it("labels other modifier pairs per platform", () => {
		...
		expect(
			formatSnapShotShortcutLabel(
				{ kind: "modifier-pair", modifier: "alt" },
				"MacIntel",
			),
		).toBe("Option + Option");
	});
	...
	it.each([
		[
			"MacIntel",
			{ kind: "modifier-pair", modifier: "meta" } as const,
			["⌘", "⌘"],
		],
		["MacIntel", { kind: "both-shift-keys" } as const, ["⇧", "⇧"]],
		[
			"MacIntel",
			{ kind: "modifier-pair", modifier: "alt" } as const,
			["⌥", "⌥"],
		],
		[
			"MacIntel",
			{ kind: "modifier-pair", modifier: "control" } as const,
			["⌃", "⌃"],
		],
...
describe("window capture keybinding conflicts", () => {
	...
	it("does not conflict with regular keybindings for both Shift keys", () => {
		expect(
			snapShotKeybindingConflict({ kind: "both-shift-keys" }, [], "Linux"),
		).toBeNull();
	});
