...
export function parseDesktopSnapShotShortcut(
	label: string,
	...
	const input = (
		gtk ? gtk[1]!.replace(/<([^<>]+)>/g, "$1+") + gtk[2]! : text
	).replace(/\b(super|win|primary)\b/gi, (modifier) =>
		modifier.toLowerCase() === "primary" ? "ctrl" : "meta",
	);
...
export function sameSnapShotShortcut(
	...
	return (
		shortcutConflictKey(left, platform) === shortcutConflictKey(right, platform)
	);
}
...
export function snapShotKeybindingConflict<Command extends string>(
	...
	return (
		keybindings.find(
			(binding) => shortcutConflictKey(binding.shortcut, platform) === key,
		)?.command ?? null
	);
}
