...
export function appendTerminalContextsToPrompt(
	...
	const trimmedPrompt = materializeInlineTerminalContextPrompt(
		prompt,
		contexts,
	).trim();
...
export function extractTrailingTerminalContexts(
	prompt: string,
...
export function deriveDisplayedUserMessageState(
	...
	const extractedTerminal = extractTrailingTerminalContexts(
...
export function ensureInlineTerminalContextPlaceholders(
	...
	const missingCount =
		terminalContextCount - countInlineTerminalContextPlaceholders(prompt);
	if (missingCount <= 0) {
		return prompt;
	}
...
function isInlineTerminalContextBoundaryWhitespace(
...
export function insertInlineTerminalContextPlaceholder(
	...
	const needsLeadingSpace = !isInlineTerminalContextBoundaryWhitespace(
		prompt[cursor - 1],
	);
