...
describe("LocalApi", () => {
	...
	it("uses the browser context-menu fallback without a desktop bridge", async () => {
		...
		await expect(
			createLocalApi().contextMenu.show(items, { x: 4, y: 5 }),
	...
	it("uses the themed confirmation host when it is available", async () => {
		...
		await expect(
			createLocalApi().dialogs.confirm("Delete this thread?", options),
		).resolves.toBe(true);
		expect(requestConfirmDialogMock).toHaveBeenCalledWith(
			"Delete this thread?",
			options,
		);
	});
	...
	it("fails closed in a browser when no themed host is available", async () => {
		...
		await expect(
			createLocalApi().dialogs.confirm("Delete this thread?"),
		).resolves.toBe(false);
	});
	...
	it("rejects opening System Settings when the desktop bridge is unavailable", async () => {
		...
		await expect(
			createLocalApi().shell.openSystemSettings("full-disk-access"),
		).rejects.toThrow("Unable to open System Settings.");
	});
	...
	it("delegates host capabilities and persistence to the desktop bridge", async () => {
		...
		await expect(api.dialogs.pickFolder({ initialPath: "/tmp" })).resolves.toBe(
			"/tmp/project",
		);
