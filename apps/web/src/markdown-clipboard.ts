...
function isSkippedElement(element: Element): boolean {
	if (SKIPPED_TAGS.has(element.tagName) || element.localName === "svg")
		return true;
	if (element.getAttribute("aria-hidden") === "true") return true;
	return SKIPPED_CLASS_NAMES.some((className) =>
...
function serializeDetails(details: Element): string {
	const summary =
		details
			.querySelector(":scope > [data-markdown-details-summary]")
			?.textContent?.trim() ?? "Details";
	const contentNode = details.querySelector(
		":scope > * [data-markdown-details-content]",
	);
	const content = contentNode ? serializeChildren(contentNode).trim() : "";
	const open =
		details.getAttribute("data-markdown-details-open") === "true"
			? " open"
			: "";
...
function serializeNode(node: Node): string {
	...
	switch (element.tagName) {
		...
		case "CODE": {
			...
			return isBlockCodeElement(element, content)
				? content
				: wrapInlineCode(content);
		}
		case "STRONG":
		case "B":
