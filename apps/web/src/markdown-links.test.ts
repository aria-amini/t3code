...
describe("shouldOpenMarkdownFileLinkInBrowserByDefault", () => {
	it("keeps PDFs browser-first while source files open in the file viewer", () => {
		expect(shouldOpenMarkdownFileLinkInBrowserByDefault("report.pdf")).toBe(
			true,
		);
		expect(
			shouldOpenMarkdownFileLinkInBrowserByDefault("report.PDF?download=1"),
		).toBe(true);
		expect(shouldOpenMarkdownFileLinkInBrowserByDefault("report.html")).toBe(
			false,
		);
		expect(shouldOpenMarkdownFileLinkInBrowserByDefault("report.xml")).toBe(
			false,
		);
	});
});
...
describe("rewriteMarkdownFileUriHref", () => {
	it("rewrites file uri hrefs into direct path hrefs", () => {
		...
		).toBe("/Users/julius/project/src/main.ts#L42");
	});
	...
	it("preserves encoded octets so file paths are decoded only once later", () => {
		...
		).toBe("/Users/julius/project/file%2520name.md");
	});
	...
	it("preserves file uri authorities as windows UNC paths", () => {
		expect(
			rewriteMarkdownFileUriHref("file://server/share/workspace-image.svg"),
		).toBe("\\\\server\\share\\workspace-image.svg");
	});

	it("treats a localhost file uri as a local path", () => {
		expect(
			rewriteMarkdownFileUriHref("file://localhost/home/me/notes.md"),
...
describe("resolveMarkdownFileLinkTarget", () => {
	it("resolves absolute posix file paths", () => {
		expect(
			resolveMarkdownFileLinkTarget("/Users/julius/project/AGENTS.md"),
		).toBe("/Users/julius/project/AGENTS.md");
	});
	...
	it("resolves relative file paths against cwd", () => {
		...
		).toBe("/Users/julius/project/src/processRunner.ts:71");
	});

	it("does not treat filename line references as external schemes", () => {
		expect(
			resolveMarkdownFileLinkTarget("script.ts:10", "/Users/julius/project"),
		).toBe("/Users/julius/project/script.ts:10");
	});

	it("resolves bare file names against cwd", () => {
		expect(
			resolveMarkdownFileLinkTarget("AGENTS.md", "/Users/julius/project"),
		).toBe("/Users/julius/project/AGENTS.md");
	});
	...
	it("ignores external urls", () => {
		expect(
			resolveMarkdownFileLinkTarget("https://example.com/docs"),
		).toBeNull();
		expect(
			resolveMarkdownFileLinkTarget("//cdn.example.com/clip.mp4", "/workspace"),
		).toBeNull();
	});
	...
	it("does not double-decode file URLs", () => {
		...
		).toBe("/Users/julius/project/file%20name.md");
	});

	it("resolves file uri authorities as windows UNC paths", () => {
		expect(
			resolveMarkdownFileLinkTarget("file://server/share/workspace-image.svg"),
		).toBe("\\\\server\\share\\workspace-image.svg");
	});

	it("resolves a localhost file uri as a local path", () => {
		expect(
			resolveMarkdownFileLinkTarget("file://localhost/home/me/notes.md"),
		).toBe("/home/me/notes.md");
	});
	...
	it.each(["md", "html", "xml"])(
		...
		(extension) => {
			const href = renderMarkdownLinkHref(
				`[checklist](<Updated cutover checklist.${extension}>)`,
			);
	...
	it("does not create a preview path for files outside the workspace", () => {
		expect(
			resolveMarkdownFileLinkMeta("/tmp/report.ts", "/repo/project"),
		).toMatchObject({
	...
	it("keeps drive-root workspace comparisons case-insensitive", () => {
		expect(
			resolveMarkdownFileLinkMeta("C:/Users/MIKE/project.ts", "c:/"),
		).toMatchObject({
	...
	it.each([
		...
		"preserves the preview target for %s in workspace %s",
		(href, cwd, workspaceRelativePath) => {
			expect(resolveMarkdownFileLinkMeta(href, cwd)).toMatchObject({
				workspaceRelativePath,
			});
		},
	);
	...
	it("keeps an encoded final space in the absolute target", () => {
		expect(
			resolveMarkdownFileLinkTarget("/tmp/repo/file.ts%20", "/tmp/repo"),
		).toBe("/tmp/repo/file.ts ");
	});
	...
	it("normalizes slash-prefixed windows drive paths before resolving", () => {
		expect(
			...
			"D:/Programme/t3code/apps/web/src/components/chat/OpenInPicker.tsx:69",
		);
	});
	...
	it("does not treat app routes as file links, even with a line anchor", () => {
		...
		expect(
			resolveMarkdownFileLinkTarget("/chat/settings#L3", "/repo"),
		).toBeNull();
	});
...
describe("relative links inside a rendered host file", () => {
	it("anchor to the file's directory while workspace membership follows cwd", () => {
		...
		const inline = resolveInlineCodeFileLinkMeta(
			"Makefile:12",
			"/repo",
			"/tmp/report",
		);
		...
		expect(
			resolveMarkdownFileLinkMeta("src/main.ts", "/repo", "/repo/docs"),
		).toMatchObject({
...
describe("resolveInlineCodeFileLinkMeta", () => {
	...
	it("links absolute posix paths", () => {
		expect(
			resolveInlineCodeFileLinkMeta("/Users/julius/project/AGENTS.md"),
		).toMatchObject({
	...
	it("links windows drive paths", () => {
		expect(
			resolveInlineCodeFileLinkMeta("C:\\Users\\mike\\project\\src\\main.ts"),
		).toMatchObject({
			basename: "main.ts",
		});
	});
	...
	it("links bare filenames only when a line suffix marks them as file references", () => {
		expect(
			resolveInlineCodeFileLinkMeta("script.ts:10", "/Users/julius/project"),
		).toMatchObject({
			targetPath: "/Users/julius/project/script.ts:10",
			line: 10,
		});
		expect(
			resolveInlineCodeFileLinkMeta("AGENTS.md", "/Users/julius/project"),
		).toBeNull();
	});

	it("links extensionless bare filenames with a line suffix", () => {
		expect(
			resolveInlineCodeFileLinkMeta("Makefile:12", "/Users/julius/project"),
		).toMatchObject({
		...
		expect(
			resolveInlineCodeFileLinkMeta("Dockerfile:8:2", "/Users/julius/project"),
		).toMatchObject({
			line: 8,
			column: 2,
		});
	...
	it("does not treat arbitrary name:digits shapes as files", () => {
		expect(
			resolveInlineCodeFileLinkMeta("error:1", "/Users/julius/project"),
		).toBeNull();
		expect(
			resolveInlineCodeFileLinkMeta("TODO:12", "/Users/julius/project"),
		).toBeNull();
		expect(
			resolveInlineCodeFileLinkMeta("exit:0", "/Users/julius/project"),
		).toBeNull();
		expect(
			resolveInlineCodeFileLinkMeta("port:3000", "/Users/julius/project"),
		).toBeNull();
		expect(
			resolveInlineCodeFileLinkMeta("http:80", "/Users/julius/project"),
		).toBeNull();
	});
	...
	it("links relative windows-style paths by normalizing backslashes", () => {
		expect(
			resolveInlineCodeFileLinkMeta("src\\main.ts", "/Users/julius/project"),
		).toMatchObject({
	...
	it("ignores hosts, ports, and versions", () => {
		expect(
			resolveInlineCodeFileLinkMeta("127.0.0.1:3000", "/Users/julius/project"),
		).toBeNull();
		expect(
			resolveInlineCodeFileLinkMeta("localhost:3000", "/Users/julius/project"),
		).toBeNull();
		expect(
			resolveInlineCodeFileLinkMeta(
		...
		expect(
			resolveInlineCodeFileLinkMeta("10.0.0.1:80:1", "/Users/julius/project"),
		).toBeNull();
		expect(
			resolveInlineCodeFileLinkMeta(
	...
	it("still links files whose extension merely resembles a tld", () => {
		...
		expect(
			resolveInlineCodeFileLinkMeta("src/setup.sh:3", "/Users/julius/project"),
		).not.toBeNull();
		expect(
			resolveInlineCodeFileLinkMeta("Makefile.in:12", "/Users/julius/project"),
		).not.toBeNull();
		expect(
			resolveInlineCodeFileLinkMeta(
				"conf.d/nginx.conf",
	...
	it("prefers file over country-code host when a line suffix is present", () => {
		expect(
			resolveInlineCodeFileLinkMeta("script.pl:10", "/Users/julius/project"),
		).toMatchObject({
			targetPath: "/Users/julius/project/script.pl:10",
			line: 10,
		});
		expect(
			resolveInlineCodeFileLinkMeta("model.pt:3", "/Users/julius/project"),
		).not.toBeNull();
		expect(
			resolveInlineCodeFileLinkMeta(
	...
	it("ignores commands, flags, and expressions", () => {
		expect(
			resolveInlineCodeFileLinkMeta("git worktree list --porcelain"),
		).toBeNull();
		expect(
			resolveInlineCodeFileLinkMeta("node.meta", "/Users/julius/project"),
		).toBeNull();
		expect(
			resolveInlineCodeFileLinkMeta("pnpm install", "/Users/julius/project"),
		).toBeNull();
		expect(
			resolveInlineCodeFileLinkMeta("src/**/*.ts", "/Users/julius/project"),
		).toBeNull();
	});

	it("ignores extension-less relative segments like git refs and directories", () => {
		expect(
			resolveInlineCodeFileLinkMeta("origin/main", "/Users/julius/project"),
		).toBeNull();
		expect(
			resolveInlineCodeFileLinkMeta("apps/web", "/Users/julius/project"),
		).toBeNull();
	});

	it("ignores external urls", () => {
		expect(
			resolveInlineCodeFileLinkMeta("https://example.com/docs.html"),
		).toBeNull();
	});

	it("ignores relative paths without a cwd to resolve against", () => {
		expect(
			resolveInlineCodeFileLinkMeta("docs/internals/workspace-layout.md"),
		).toBeNull();
	});
});
...
describe("directory paths with a trailing separator", () => {
	it("keeps the final segment for a POSIX directory path", () => {
		expect(
			resolveMarkdownFileLinkMeta("/tmp/favicons/", "/repo/project"),
		).toMatchObject({
			basename: "favicons",
		});
	});
	...
	it("matches the label of the same path without a trailing separator", () => {
		const withSlash = resolveMarkdownFileLinkMeta(
			"/tmp/favicons/",
			"/repo/project",
		);
		const withoutSlash = resolveMarkdownFileLinkMeta(
			"/tmp/favicons",
			"/repo/project",
		);
