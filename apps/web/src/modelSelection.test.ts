...
describe("instance-scoped model selection", () => {
	it("preserves server-provided legacy model metadata", () => {
		...
		expect(
			getAppModelOptionsForInstance(settingsWithProviderInstances(), stock)[0]
				?.isLegacy,
		).toBe(true);
	});
	...
	it("preserves a custom slug that collides with a provider alias", () => {
		...
		expect(
			getAppModelOptionsForInstance(settings, openrouter).map(
				(option) => option.slug,
			),
	...
	it("includes Grok custom models from the selected provider instance", () => {
		...
		expect(
			getAppModelOptionsForInstance(settings, grok).map(
				(option) => option.slug,
			),
		).toContain("grok-test-custom-model");
	});
	...
	it("hides server models from the instance option list", () => {
		...
		expect(
			getAppModelOptionsForInstance(settings, stock).map(
				(option) => option.slug,
			),
	...
	it("applies persisted per-instance model ordering", () => {
		...
		).toEqual(["claude-haiku-4-5", "claude-opus-4-6", "claude-sonnet-4-6"]);
	});
	...
	describe.each([
		...
		"$driverName catalog gaps",
		({ driverName, availableModel, missingModel }) => {
			...
			it("keeps a missing option scoped to the selected instance", () => {
				...
				const otherInstanceId = ProviderInstanceId.make(
					`${driverName}_personal`,
				);
			...
			it("replaces the unavailable marker with catalog metadata after recovery", () => {
				...
				const recoveredProviders = [
					provider({
						provider: driver,
						instanceId,
						models: [availableModel, selectedModel],
					}),
				];
			...
			it("does not resurrect a hidden model when the raw catalog omits it", () => {
				...
				expect(
					getAppModelOptionsForInstance(settings, entry, missingModel),
				).toEqual([]);
				expect(
					resolveAppModelSelectionForInstance(
						instanceId,
	...
	it("falls back from an explicit non-OpenCode draft with a missing model", () => {
		...
		const providers = [
			provider({ provider: driver, instanceId, models: ["gpt-5.6-sol"] }),
		];
	...
	it("preserves an explicit draft OpenCode selection while the catalog is empty", () => {
		...
		const draftSelection = createModelSelection(
			instanceId,
			"openrouter/kimi-k3",
			[
	...
	it("does not borrow a default model while a new Antigravity account has no catalog", () => {
		...
		for (const draft of [
			null,
			{
				activeProvider: instanceId,
				modelSelectionByProvider: {
					[otherAccountId]: createModelSelection(
						otherAccountId,
						"gemini-other-account",
					),
				},
			},
		]) {
	...
	it("offers only account catalog models for Antigravity despite custom model settings", () => {
		...
		for (const entry of entries) {
			expect(
				getAppModelOptionsForInstance(settings, entry).map(
					(model) => model.slug,
				),
			).toEqual([nativeModel]);
		}
	...
	it("does not select a provider that cannot generate system text", () => {
		...
		expect(
			resolveAppModelSelectionState(settings, [unsupported, supported]),
		).toEqual(createModelSelection(supported.instanceId, "gpt-5.6-sol"));
...
describe("resolvePlanAgentHealPatch", () => {
	const instance = ProviderInstanceId.make("opencode");
	const model = "opencode/gpt-5.4";
	const healed = createModelSelection(instance, model, [
		{ id: "variant", value: "high" },
	]);
