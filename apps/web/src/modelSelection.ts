...
function readInstanceCustomModels(
	...
	return readCustomModelEntries(
		legacyProviders[driverKind]?.customModels ?? [],
	);
}
...
function appendUnavailableDynamicModelSelection(
	...
	if (provider === "antigravity" && slug === ANTIGRAVITY_DEFAULT_MODEL)
		return options;
	...
	if (resolveSelectableModel(provider, slug, rawModels) !== null)
		return options;
	...
	return [
		...options,
		{ slug, name: slug, isCustom: false, isUnavailable: true },
	];
}
...
function getAppModelOptions(
	...
	const customModels = readInstanceCustomModels(
		settings,
		defaultInstanceId,
		provider,
	);
	for (const entry of normalizeCustomModelEntries(
		customModels,
		builtInModelSlugs,
	)) {
...
export function getAppModelOptionsForInstance(
	...
	for (const custom of normalizeCustomModelEntries(
		customModels,
		builtInModelSlugs,
	)) {
...
export function resolveAppModelSelectionForInstance(
	...
	const resolvedSelection = resolveSelectableModel(
		entry.driverKind,
		selectedModel,
		options,
	);
	...
	if (
		...
		if (
			...
			resolveSelectableModel(entry.driverKind, selectedModel, entry.models) ===
				null &&
			(entry.driverKind !== "antigravity" ||
				unavailableSelection !== ANTIGRAVITY_DEFAULT_MODEL)
		) {
...
export function resolvePlanAgentHealPatch(input: {
	...
	const healedText = withoutPlanAgentSelection(
	...
	const patch: ServerSettingsPatch = {
		...(healedText && healedText !== input.textGenerationModelSelection
			...
			: {}),
		...(healedSourceControl &&
		healedSourceControl !== input.sourceControlWriterModelSelection
