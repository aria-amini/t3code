...
export function configureClientTracing(
...
async function applyClientTracingConfig(
	...
	const otlpTracesUrl = resolvePrimaryEnvironmentHttpUrl(
		"/api/observability/v1/traces",
	);
	const exportIntervalMs = Math.max(
		10,
		config.exportIntervalMs ?? DEFAULT_EXPORT_INTERVAL_MS,
	);
...
async function disposeTracerRuntime(
	...
	await settleAsyncResult(() =>
		runtime.runPromiseExit(Scope.close(scope, Exit.void)),
	);
	runtime.dispose();
}
