...
export function transitionFirstRunGateState(
	...
	if (event.type === "timeout") {
		return state.decision === "pending" && !state.stalled
			? { ...state, stalled: true }
			: state;
	}
...
export function isFreshFirstRunWorkspace(
	input: FirstRunWorkspaceInput,
): boolean {
...
export function resolveHostedFirstRunDecision(
	input: HostedFirstRunDecisionInput,
): {
	readonly decision: FirstRunDecision;
	readonly persistCompletion: boolean;
} {
