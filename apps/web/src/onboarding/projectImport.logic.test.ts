...
describe("partitionOnboardingProjects", () => {
	...
	it("keeps non-git folders and thin histories out of the default selection", () => {
		...
		expect(
			partitionOnboardingProjects([repo, folder, thin], now).recent,
		).toEqual([repo]);
	});
...
describe("groupOnboardingProjects", () => {
	it("groups clones by origin, keeps local repos separate, and folds non-git folders away", () => {
		...
		const grouped = groupOnboardingProjects([
			main,
			clone,
			older,
			local,
	...
	it("lists candidates without git identity as standalone repositories", () => {
		const { git: _git, ...legacy } = candidate("/code/legacy", {
			title: "legacy",
		});
		...
		expect(
			grouped.repositories.map((group) => [group.label, group.repository]),
		).toEqual([["legacy", null]]);
	});
});
...
describe("projects on multiple computers", () => {
	it("keeps identical paths independent when selecting a landing project after partial imports", () => {
		const first = onboardingProjectKey(
			EnvironmentId.make("first"),
			"/code/app",
		);
		const second = onboardingProjectKey(
		...
		expect(
			resolveOnboardingLandingProject([second], completed, completed),
		).toBeUndefined();
		completed.set(second, "second-project");
		expect(
			resolveOnboardingLandingProject([second, first], completed, completed),
		).toBe("second-project");
	});
	...
	it("preserves computer identity when choosing recent projects", () => {
		...
		expect(partitionOnboardingProjects([first, second], now).recent).toEqual([
			first,
			second,
		]);
	});
