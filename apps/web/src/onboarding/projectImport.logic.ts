...
export function partitionOnboardingProjects<
	T extends AgentSessionProjectCandidate,
>(candidates: ReadonlyArray<T>, now = Date.now()) {
...
>(candidates: ReadonlyArray<T>) {
	const groups = new Map<
		string,
		OnboardingProjectGroup<T> & { candidates: Array<T> }
	>();
	const other: Array<T> = [];
...
export function resolveOnboardingProjectId(
	...
	const currentRootMatch = findProjectByPath(
		environmentProjects,
		candidate.path,
	);
...
export function onboardingProjectKey(
	environmentId: EnvironmentId,
	path: string,
): string {
