...
describe("getOnboardingProviderState", () => {
	...
	it("offers sign-in only when the server reports an authentication failure", () => {
		...
		expect(getOnboardingProviderState({ ...readyCodex, status: "error" })).toBe(
			"attention",
		);
		expect(
			getOnboardingProviderState({ ...readyCodex, status: "warning" }),
		).toBe("attention");
	});
	...
	it("does not offer installation or sign-in for disabled providers", () => {
		...
		expect(
			getOnboardingProviderState({ ...readyCodex, status: "disabled" }),
		).toBe("disabled");
	});
...
describe("selectOnboardingProvidersByDriver", () => {
	it("prefers a ready instance with unknown authentication to an unauthenticated instance", () => {
		...
		expect(
			selectOnboardingProvidersByDriver([signedOutCodex, readyCodex]).get(
				"codex",
			),
		).toBe(readyCodex);
	});
	...
	it("prefers a provider with an actionable sign-in over a failed provider", () => {
		...
		expect(
			selectOnboardingProvidersByDriver([failedCodex, signedOutCodex]).get(
				"codex",
			),
		).toBe(signedOutCodex);
	});
	...
	it("keeps a ready custom account when the default account is signed out", () => {
		...
		expect(
			selectOnboardingProvidersByDriver([signedOutDefault, readyCustom]).get(
				"codex",
			),
		).toBe(readyCustom);
	});
...
describe("resolveOnboardingProviderLoginCommand", () => {
	...
	it.each([
		...
		"keeps the home prefix expandable while quoting %s",
		(binaryPath, expectedCommand) => {
			expect(
				resolveOnboardingProviderLoginCommand(
					readyCodex,
					{
	...
	it.each(["darwin", "linux"] as const)(
		"quotes backslashes in a Codex path on %s",
		(platform) => {
			expect(
				resolveOnboardingProviderLoginCommand(
					readyCodex,
...
describe("resolveOnboardingProviderInstallCommand", () => {
	it("uses the PowerShell installer on Windows environments", () => {
		...
		expect(
			resolveOnboardingProviderInstallCommand("claudeAgent", "windows"),
		).toBe("irm https://claude.ai/install.ps1 | iex");
	});

	it.each(["darwin", "linux", "unknown"] as const)(
		"uses the shell installer on %s",
		(platform) => {
			expect(resolveOnboardingProviderInstallCommand("codex", platform)).toBe(
				"curl -fsSL https://chatgpt.com/codex/install.sh | sh",
			);
			expect(
				resolveOnboardingProviderInstallCommand("claudeAgent", platform),
