...
export function resolveOnboardingTargetEnvironment<
	TEnvironment extends OnboardingEnvironment,
>({
	mode,
	environments,
	primaryEnvironment,
	...
	if (mode === "direct" && pairedEnvironmentId !== null) {
		...
		return pairedEnvironment?.connection.phase === "connected"
			? pairedEnvironment
			: null;
	}
	...
	const connectedRelayEnvironments = environments.filter(
		(environment) =>
			...
			isOnboardingRelayEnvironment(environment),
	);

	if (mode === "connect" && connectedRelayEnvironments.length > 0) {
		return (
			connectedRelayEnvironments[connectedRelayEnvironments.length - 1] ?? null
		);
	}
