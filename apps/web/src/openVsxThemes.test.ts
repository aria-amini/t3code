...
describe("Open VSX themes", () => {
	it("searches theme extensions and keeps only supported open-source licenses", async () => {
		const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
			...
			if (url.endsWith("/icons/theme/1.0.0/file/package.json")) {
				return new Response(
					JSON.stringify({ contributes: { iconThemes: [{}] } }),
					{ status: 200 },
				);
			}
		...
		vi.stubGlobal(
			...
			vi.fn(async (input: RequestInfo | URL) => {
				if (String(input).includes("/-/search?")) {
					return new Response(
						JSON.stringify({
							extensions: [{ namespace: "demo", name: "theme" }],
						}),
						{ status: 200 },
					);
				}
		...
		vi.stubGlobal(
			...
			vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
				...
				if (url.endsWith("/demo/theme")) {
					return new Response(JSON.stringify(extensionDetail()), {
						status: 200,
					});
				}
	...
	it("downloads a verified VSIX, reads JSONC includes, and pairs contributed variants", async () => {
		...
		const manifest = {
			contributes: {
				themes: [{ label: "Wrong", path: "./themes/missing.json" }],
			},
		};
		...
		vi.stubGlobal(
			...
			vi.fn(async (input: string | URL | Request) => {
				...
				if (url === extension.sha256Url)
					return new Response(`${checksum}  demo.vsix`);
		...
		expect(
			themes.every((theme) => /^ovx-[a-z0-9-]+-[0-9a-f]{12}$/.test(theme.id)),
		).toBe(true);
		expect(
			themes.every((theme) => theme.collection?.id === "open-vsx:demo.theme"),
		).toBe(true);
		...
		expect(themeColorToHex(getThemeColorsForMode(paired, "dark")!.canvas)).toBe(
			"#111111",
		);
		expect(themeColorToHex(getThemeColorsForMode(paired, "dark")!.text)).toBe(
			"#eeeeee",
		);
		...
		expect(renamedThemes.map((theme) => theme.id)).toEqual(
		...
		expect(
			themes.every(({ id }) =>
				duplicatePathThemes.some((theme) => theme.id === id),
			),
		).toBe(true);
	...
	it("stops import work when the request is cancelled", async () => {
		...
		await expect(
			importOpenVsxThemeExtension(extension, controller.signal),
