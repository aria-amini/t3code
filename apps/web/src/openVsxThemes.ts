...
function trustedOpenVsxUrl(value: unknown): string | null {
	...
	try {
		...
		return url.protocol === "https:" &&
			url.hostname.toLowerCase() === "open-vsx.org"
			? url.toString()
			: null;
	} catch {
		return null;
...
function manifestLicenseMatches(
	manifest: Record<string, unknown>,
	license: string,
): boolean {
	return (
...
function extensionFromDetail(value: unknown): OpenVsxThemeExtension | null {
	...
	return {
		...
		downloadCount:
			...
			Number.isFinite(value.downloadCount)
				? value.downloadCount
				: 0,
...
export async function searchOpenVsxThemes(
	...
	const identities = value.extensions.flatMap(
		(candidate): Array<[string, string]> => {
			if (!isRecord(candidate)) return [];
			const namespace =
				typeof candidate.namespace === "string" ? candidate.namespace : "";
			...
			withSearchTimeout(async (requestSignal) => {
				...
				try {
					...
					const packageLength = Number(
						packageResponse.headers.get("content-length"),
					);
					if (
	...
	const completedDetails = details.filter(
...
function inspectZipDirectory(bytes: Uint8Array): Uint8Array {
	...
	if (endOffset < minimumOffset)
		throw new Error("That extension package has no ZIP directory.");
	...
	while (offset < directoryEnd) {
		...
		if (totalUncompressed > MAX_UNCOMPRESSED_BYTES) {
			throw new Error(
				"That extension package expands beyond the safe import limit.",
			);
		}
		if (
			uncompressed > 0 &&
			...
			throw new Error(
				"That extension package has an unsafe compression ratio.",
			);
		}
...
function inspectZip(zip: JSZip): void {
	...
	for (const entry of entries) {
		if (entry.unsafeOriginalName)
			normalizePackagePath(entry.unsafeOriginalName);
	}
}
...
async function readZipText(
	...
	if (!file)
		throw new Error(`${description} is missing from the extension package.`);
	if (
...
export async function importOpenVsxThemeExtension(
	...
	try {
		...
		if (
			...
			cause.message.startsWith("That extension package")
		)
			throw cause;
	...
	if (
		...
		packagedManifest.publisher.toLowerCase() !==
			extension.publisher.toLowerCase() ||
	...
	if (!manifestLicenseMatches(packagedManifest, extension.license)) {
		throw new Error(
			"That extension package does not match its advertised license.",
		);
	}
	...
	for (const contribution of contributions) {
		...
		try {
			...
			if (!isVsCodeThemeFile(decorated))
				throw new Error("not a VS Code color theme");
			parsed.push({
			...
			signal?.throwIfAborted();
			failures.push(
				cause instanceof Error ? cause.message : "theme could not be read",
			);
		}
	}
	...
	for (const { sourcePath } of parsed) {
		sourcePathCounts.set(
			sourcePath,
			(sourcePathCounts.get(sourcePath) ?? 0) + 1,
		);
	}
	...
	const paired = pairVsCodeThemes(resolved, {
		pairedId: (light, dark) =>
			openVsxThemeId(extensionId, [light.id, dark.id].sort().join(":")),
	});
	const themes = resolveThemeLabelCollisions(
		paired.map((theme) => ({ theme })),
	);
	const collection = {
