...
describe("togglePendingUserInputOptionSelection", () => {
	...
	it("selects and removes options with the same label independently", () => {
		...
		const firstSelected = togglePendingUserInputOptionSelection(
			question,
			undefined,
			" first\t",
		);
		const bothSelected = togglePendingUserInputOptionSelection(
			question,
			firstSelected,
			"second",
		);
		...
		expect(
			buildPendingUserInputAnswers([question], { result: bothSelected }),
		).toEqual({
			result: [" first\t", "second"],
		});
		...
		expect(
			buildPendingUserInputAnswers([question], { result: secondSelected }),
		).toEqual({
			result: ["second"],
		});
	});
...
describe("buildPendingUserInputAnswers", () => {
	...
	it.each([" first\t", ""])(
		"preserves the exact selected option value %j",
		(value) => {
			const question = {
				...nativeChoiceQuestion,
				options: [
					{ ...nativeChoiceQuestion.options[0], value },
					nativeChoiceQuestion.options[1],
				],
			};
			...
			expect(
				buildPendingUserInputAnswers([question], { result: draft }),
			).toEqual({ result: value });
			expect(
				derivePendingUserInputProgress([question], { result: draft }, 0),
			).toMatchObject({
...
describe("pending user input question progress", () => {
	...
	it("requires an option when custom answers are disabled", () => {
		...
		expect(
			buildPendingUserInputAnswers([nativeChoiceQuestion], drafts),
		).toBeNull();
		expect(
			derivePendingUserInputProgress([nativeChoiceQuestion], drafts, 0),
		).toMatchObject({
			customAnswer: "",
