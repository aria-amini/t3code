...
export function countAnsweredPendingUserInputQuestions(
	...
	return questions.reduce((count, question) => {
		return resolvePendingUserInputAnswer(
			question,
			draftAnswers[question.id],
		) !== null
			? count + 1
...
export function derivePendingUserInputProgress(
	...
	const isLastQuestion =
		questions.length === 0
			? true
			: normalizedQuestionIndex >= questions.length - 1;

	return {
		questionIndex: normalizedQuestionIndex,
		activeQuestion,
		activeDraft,
		selectedOptionValues: normalizeSelectedOptionValues(
			activeDraft?.selectedOptionValues,
		),
		customAnswer,
		resolvedAnswer,
