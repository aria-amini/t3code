...
export function boundConfiguredLocalServerUrls(
	...
	for (const raw of urls ?? []) {
		if (
			raw.length === 0 ||
			raw.length > PREVIEW_URL_MAX_LENGTH ||
			raw.trim().length !== raw.length
		)
			continue;
		try {
			...
			if (
				...
				url.href.length > PREVIEW_URL_MAX_LENGTH
			)
				continue;
	...
	const query = useEnvironmentQuery(
		environmentId === null
			...
			: previewEnvironment.discoveredServers({
					environmentId,
					input: boundedConfiguredUrls.length
						? { configuredUrls: boundedConfiguredUrls }
						: {},
				}),
	);
	return useMemo(
