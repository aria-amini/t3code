...
const applyPreviewServerEvent = (
	eventRef: typeof ref,
	event: PreviewEventDraft,
): void => {
...
describe("previewStateStore (single-tab)", () => {
	it("keeps independent state atoms for each thread", () => {
		expect(previewStateAtom(scopedThreadKey(ref))).toBe(
		...
		expect(readThreadPreviewState(otherRef)).toEqual(
	...
	it("rememberUrl dedupes and caps at limit", () => {
		...
		const state = readThreadPreviewState(ref);
		expect(state.recentlySeenUrls.length).toBeLessThanOrEqual(
			__testing.RECENT_URL_LIMIT,
		);
