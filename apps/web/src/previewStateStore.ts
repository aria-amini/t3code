...
const emptyPreviewStateAtom = Atom.make<ThreadPreviewState>(
	EMPTY_THREAD_PREVIEW_STATE,
).pipe(Atom.withLabel("preview:empty-thread"));
...
const removeSession = (
	...
	const activeTabId =
		current.activeTabId === tabId
			...
			: current.activeTabId;
	const snapshot = activeTabId
		? (sessions[activeTabId] ?? nextSnapshot)
		: nextSnapshot;
	return {
		...current,
		sessions,
...
export function useThreadPreviewState(
	ref: ScopedThreadRef | null | undefined,
): ThreadPreviewState {
	const atom = ref
		? previewStateAtom(scopedThreadKey(ref))
		: emptyPreviewStateAtom;
...
export function readThreadPreviewState(
	ref: ScopedThreadRef,
): ThreadPreviewState {
...
export function applyPreviewServerEvent(
	...
	updateThreadPreviewState(ref, (current) => {
		if (
			current.serverEpoch !== null &&
			event.serverEpoch !== current.serverEpoch
		)
			return current;
		...
		const next = (() => {
			switch (event.type) {
				...
				case "resized": {
					...
					const activeTabId =
						event.type === "opened" ? snapshot.tabId : current.activeTabId;
					const activeSnapshot =
						sessions[activeTabId ?? snapshot.tabId] ?? snapshot;
					return {
						...current,
						sessions,
						...
						desktopOverlay:
							current.desktopByTabId[activeSnapshot.tabId] ?? null,
						recentlySeenUrls,
					};
				}
				case "failed": {
...
export function applyPreviewServerSnapshot(
	...
	updateThreadPreviewState(ref, (current) => {
		...
		const recentlySeenUrls = rememberSnapshotUrl(
			current.recentlySeenUrls,
			snapshot,
		);
		return {
...
export function reconcilePreviewServerSessions(
	...
	updateThreadPreviewState(ref, (current) => {
		...
		const currentSuppressedTabIds = sameServer
			? current.suppressedTabIds
			: new Set<string>();
		...
		for (const snapshot of snapshots) {
			...
			const existing = sameServer
				? current.sessions[snapshot.tabId]
				: undefined;
			const next =
				existing && existing.updatedAt > snapshot.updatedAt
					? existing
					: snapshot;
		...
		return {
			...
			snapshot,
			desktopByTabId,
			desktopOverlay: activeTabId
				? (desktopByTabId[activeTabId] ?? null)
				: null,
			recentlySeenUrls,
...
export function applyPreviewDesktopState(
	...
	updateThreadPreviewState(ref, (current) => {
		...
		return {
			...
			desktopByTabId,
			desktopOverlay:
				current.activeTabId === tabId ? overlay : current.desktopOverlay,
		};
	});
}
