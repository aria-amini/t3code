...
const PROJECT_ICON_MODEL: ReadonlyArray<ProjectIconClass> = [
	{
		icon: "ai",
		terms: ["ai", "agent", "bot", "gpt", "llm", "ml", "model", "neural"],
	},
	{
		icon: "mobile",
		terms: [
...
function termScore(token: string, term: string): number {
	...
	if (term.length >= 4 && (token.startsWith(term) || token.endsWith(term)))
		return 1;
	return 0;
}
...
export function selectProjectIcon(
	...
	const name =
		projectName.trim() ||
		workspaceRoot.split(/[\\/]+/).findLast(Boolean) ||
		"project";
	...
	const iconName =
		bestIcon ??
		GENERIC_PROJECT_ICONS[stableIndex(cacheKey, GENERIC_PROJECT_ICONS.length)]!;
