...
describe("partitionStashAttachments", () => {
	it("keeps attachments within the budget and reports dropped names in order", () => {
		...
		const { kept, droppedNames } = partitionStashAttachments([
			small,
			huge,
			alsoSmall,
		]);
...
describe("promptStashStore", () => {
	...
	it("restores citations and bound comments from a persisted stash", () => {
		...
		const restored = usePromptStashStore
			.getState()
			.takeEntry("citation-stash").entry;
		expect(restored?.prompt).toBe(prompt);
		expect(
			collectAssistantCitations(restored?.prompt ?? "").map(
				(entry) => entry.citation,
			),
		).toEqual([citation]);
		expect(
			usePromptStashStore.getState().takeEntry("citation-stash").entry,
		).toBeNull();
	});
	...
	it("evicts the oldest entry past the cap and returns it", () => {
		...
		for (let index = 0; index < MAX_STASH_ENTRIES; index += 1) {
			expect(
				store.stashEntry(makeEntry({ id: `entry-${index}` })).evicted,
			).toBeNull();
		}
	...
	it("distinguishes a memory-only write (written, not durable) from a failed one", () => {
		...
		expect(
			usePromptStashStore.getState().entries.map((entry) => entry.id),
		).toEqual(["memory-only"]);
	});
