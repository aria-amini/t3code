...
const decodePersistedPromptStashState = Schema.decodeUnknownSync(
	PersistedPromptStashState,
);
...
const { storage: baseStashStorage, durable: storageIsDurable } =
	resolveBaseStorage();
...
function persistEntries(entries: ReadonlyArray<PromptStashEntry>): {
	...
	try {
		...
		console.error(
			"[PROMPT-STASH] Could not persist stash (storage quota?).",
			error,
		);
...
function readPersistedEntries(): ReadonlyArray<PromptStashEntry> | null {
	try {
		...
		return clearOrphanedPendingImages(
			decodePersistedPromptStashState(state).entries,
		);
	} catch {
		return null;
	}
...
interface PromptStashStoreState {
	 ...
	 */
	takeEntry: (entryId: string) => {
		entry: PromptStashEntry | null;
		durable: boolean;
	};
	/**
...
export const usePromptStashStore = create<PromptStashStoreState>()(
	(set, get) => ({
		...
		stashEntry: (entry) => {
			...
			const evicted =
				nextEntries.length > MAX_STASH_ENTRIES
					? (nextEntries.pop() ?? null)
					: null;
		...
		takeEntry: (entryId) => {
			...
			const entry =
				entries.find((candidate) => candidate.id === entryId) ?? null;
