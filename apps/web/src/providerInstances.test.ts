...
describe("isProviderInstancePickerVisible", () => {
	it("keeps enabled instances in the rail and removes disabled instances", () => {
		...
		expect(enabledEntry && isProviderInstancePickerVisible(enabledEntry)).toBe(
			true,
		);
		expect(
			disabledEntry && isProviderInstancePickerVisible(disabledEntry),
		).toBe(false);
	});
});
...
describe("deriveProviderEntriesByEnvironment", () => {
	it("keeps same-id default instances distinct per environment", () => {
		...
		expect(byEnvironment.get("local")?.get("claude")?.displayName).toBe(
			"Claude Local",
		);
		expect(byEnvironment.get("local")?.get("claude")?.accentColor).toBe(
			"#112233",
		);
		expect(byEnvironment.get("remote")?.get("claude")?.displayName).toBe(
			"Claude Remote",
		);
		expect(byEnvironment.get("remote")?.get("claude")?.accentColor).toBe(
			"#445566",
		);
	});
...
describe("resolveSelectableProviderInstance", () => {
	...
	it("falls back to the first enabled and available instance", () => {
		...
		expect(resolveSelectableProviderInstance(providers, disabled)).toBe(
			fallback,
		);
	});
	...
	it("prefers an unprobed (warning) instance over one whose probe errored", () => {
		...
		expect(resolveSelectableProviderInstance(providers, undefined)).toBe(
			unprobed,
		);
	});
	...
	it("does not invent an errored instance as a new-user default", () => {
		...
		expect(
			resolveSelectableProviderInstance(providers, undefined),
		).toBeUndefined();
	});
	...
	it("does not return disabled, unavailable, or unknown instances when none are sendable", () => {
		...
		expect(
			resolveSelectableProviderInstance(providers, disabled),
		).toBeUndefined();
		expect(
			resolveSelectableProviderInstance(providers, unavailable),
		).toBeUndefined();
		expect(
			resolveSelectableProviderInstance(providers, unknown),
		).toBeUndefined();
	});
});
		...
		const providers = [
			provider({
				...
				models: [
					...
					model("claude-opus-4-8", false, true),
				],
			}),
		];
...
describe("resolveDefaultProviderModelSelection", () => {
	...
	it("preserves a valid stored selection including its options", () => {
		...
		expect(resolveDefaultProviderModelSelection(providers, stored)).toBe(
			stored,
		);
	});
