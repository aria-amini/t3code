...
describe("getProviderModelCapabilities", () => {
	it("resolves model-declared aliases", () => {
		...
		expect(
			getProviderModelCapabilities(models, "legacy-synthetic-model", PROVIDER),
		).toEqual(aliasCapabilities);
	});
	...
	it("prefers an exact custom slug over a built-in model alias", () => {
		...
		expect(
			getProviderModelCapabilities(models, " custom-model ", PROVIDER),
		).toEqual(customCapabilities);
	});
	...
	it("returns empty capabilities for an unknown slug", () => {
		...
		expect(
			getProviderModelCapabilities(models, "unknown-model", PROVIDER),
		).toEqual({
			optionDescriptors: [],
		});
	});
