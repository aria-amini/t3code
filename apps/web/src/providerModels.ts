...
export function formatProviderDriverKindLabel(
...
export function resolveSelectableProvider(
	...
	const requestedEntry = providers.find(
		(candidate) => candidate.instanceId === provider,
	);
	...
	return {
		...
		optionDescriptors: (caps.optionDescriptors ?? []).flatMap((descriptor) => {
			...
			const currentValue =
				...
				options.some((option) => option.id === descriptor.currentValue)
					? descriptor.currentValue
					: (options.find((option) => option.isDefault)?.id ?? options[0]?.id);
			return [
				{ ...descriptor, options, ...(currentValue ? { currentValue } : {}) },
			];
		}),
	};
}
