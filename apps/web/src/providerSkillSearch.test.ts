...
describe("searchProviderSkills", () => {
	it("moves exact ui matches ahead of broader ui matches", () => {
		...
		expect(
			searchProviderSkills(skills, "ui").map((skill) => skill.name),
		).toEqual(["ui", "building-native-ui"]);
	});
	...
	it("uses fuzzy ranking for abbreviated queries", () => {
		...
		expect(
			searchProviderSkills(skills, "gfc").map((skill) => skill.name),
		).toEqual(["gh-fix-ci"]);
	});
	...
	it("keeps user-only skills and omits agent-only ones", () => {
		...
		expect(
			searchProviderSkills(skills, "release").map((skill) => skill.name),
		).toEqual(["release-version", "re-release-version"]);
	});
