...
const AZURE_DEVOPS_CLI_PR_CHECKOUT_PATTERN =
	/^az\s+repos\s+pr\s+checkout\s+(.+)$/i;
...
export function parsePullRequestReference(input: string): string | null {
	...
	const azureDevOpsCliCheckoutMatch =
		AZURE_DEVOPS_CLI_PR_CHECKOUT_PATTERN.exec(trimmed);
