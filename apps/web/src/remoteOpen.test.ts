...
describe("buildRemoteOpenUrl", () => {
	it("builds a vscode-remote deep link", () => {
		expect(
			...
			"vscode://vscode-remote/ssh-remote+sol.tail1234.ts.net/home/theo/code/my%20repo",
		);
	});
	...
	it("uses the fork's scheme", () => {
		...
		).toBe("cursor://vscode-remote/ssh-remote+sol/tmp/x");
	});
