...
export function resolveRemoteOpenState(input: {
	...
	if (input.sshAlias !== null && input.sshAlias.length > 0) {
		return {
			mode: "remote-links",
			host: { kind: "ssh-alias", host: input.sshAlias },
		};
	}
...
export function useRemoteOpenResolution(
	environmentId: EnvironmentId | null,
): RemoteOpenResolution {
...
export function useRemoteOpenState(
	environmentId: EnvironmentId | null,
): RemoteOpenState {
...
export function useRemoteCapableEditors(): ReadonlyArray<EditorId> {
	...
	useEffect(() => {
		...
		probe().then(
			(ids) => {
				const remoteCapable = ids.filter((id) =>
					REMOTE_CAPABLE_EDITOR_IDS.includes(id),
				);
				cachedProbedEditors =
					remoteCapable.length > 0 ? remoteCapable : REMOTE_FALLBACK_EDITORS;
				if (!cancelled) {
...
export function useRemoteOpenHint(): readonly [
	seen: boolean,
	markSeen: () => void,
] {
	const [seen, setSeen] = useLocalStorage(
		REMOTE_OPEN_HINT_KEY,
		false,
		Schema.Boolean,
	);
