...
export function parseReviewCommentMessageSegments(
	...
	for (const match of value.matchAll(REVIEW_COMMENT_BLOCK_PATTERN)) {
		...
		const comment = parseReviewCommentContext(
			match[1] ?? "",
			match[2] ?? "",
			parsedCommentIndex,
		);
...
export function formatReviewCommentFence(
	language: string,
	contents: string,
): string {
	...
	switch (line.change) {
		case "add":
			return line.newLineNumber === null
				? null
				: { kind: "added", newLine: line.newLineNumber };
		case "delete":
			return line.oldLineNumber === null
				? null
				: { kind: "deleted", oldLine: line.oldLineNumber };
		case "context":
...
export function buildReviewCommentRenderablePatch(
	comment: ReviewCommentContext,
): string {
