...
describe("rightPanelStore", () => {
	...
	it.each(["diff-first", "pull-request-first"])(
		...
		(order) => {
			...
			for (const surface of requests)
				store.openProactive(refA, surface, revision);
			...
			expect(
				selectActiveRightPanel(useRightPanelStore.getState().byThreadKey, refA),
			).toBe("diff");
		},
	);
	...
	it.each([
		...
		},
		{
			...
			choose: () =>
				useRightPanelStore
					.getState()
					.openPullRequest(refA, { ...linkedPullRequest, number: 41 }),
		},
		{
			choice: "browser",
		...
		"keeps a later $choice choice when automatic requests arrive",
		({ choose }) => {
			const store = useRightPanelStore.getState();
			store.open(refA, "diff");
			const revision = store.getUserActionRevision(refA);
			choose();
			const chosen = selectThreadRightPanelState(
				useRightPanelStore.getState().byThreadKey,
				refA,
			);

			expect(store.openProactive(refA, completedDiff, revision)).toBe(false);
			expect(store.openProactive(refA, linkedPullRequest, revision)).toBe(
				false,
			);
			expect(
				selectThreadRightPanelState(
	...
	it("allows automatic panels for a later turn after a manual choice", () => {
		...
		expect(store.openProactive(refA, completedDiff, firstTurnRevision)).toBe(
			false,
		);

		const nextTurnRevision = store.getUserActionRevision(refA);
		expect(store.openProactive(refA, completedDiff, nextTurnRevision)).toBe(
			true,
		);
		expect(
			selectActiveRightPanel(useRightPanelStore.getState().byThreadKey, refA),
	...
	it("keeps manual choices scoped to their thread and environment", () => {
		const otherEnvironment = scopeThreadRef(
		...
		expect(
			selectActiveRightPanel(useRightPanelStore.getState().byThreadKey, refB),
		).toBe("file");
		expect(
			selectActiveRightPanel(
				useRightPanelStore.getState().byThreadKey,
	...
	it("tracks one surface per pull request", () => {
		...
		const url =
			"https://gitlab.example.com/pingdotgg/t3code/-/merge_requests/4909";
	...
	it("keeps the page's panel tabs reachable when the set of connected servers changes", () => {
		...
		const stableRef = scopeThreadRef(
		...
		const refWhileBothConnected = scopeThreadRef(
			"server-a" as EnvironmentId,
			panelId,
		);
		const refAfterServerADisconnects = scopeThreadRef(
			"server-b" as EnvironmentId,
			panelId,
		);
	...
	it("tracks split panes and the active pane within a terminal surface", () => {
		...
		useRightPanelStore
			.getState()
			.splitTerminal(refA, "terminal:term-1", "term-2");
		...
		useRightPanelStore
			.getState()
			.activateTerminal(refA, "terminal:term-1", "term-1");
		useRightPanelStore
			.getState()
			.closeTerminal(refA, "terminal:term-1", "term-1");
		expect(
			selectActiveRightPanelSurface(
				useRightPanelStore.getState().byThreadKey,
				refA,
	...
	it("tracks vertical layout for a terminal surface", () => {
		...
		useRightPanelStore
			.getState()
			.splitTerminal(refA, "terminal:term-1", "term-2", "vertical");
	...
	it("reconciles browser surfaces without deleting other surface kinds", () => {
		...
		useRightPanelStore
			.getState()
			.reconcileBrowserSurfaces(refA, ["tab-b", "tab-c"]);
