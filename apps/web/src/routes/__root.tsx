		...
		<ToastProvider>
			<AnchoredToastProvider>
				...
				>
					...
					{primaryEnvironmentAuthenticated ? (
						<EventRouter
							skipInitialBootstrapNavigation={returningFromWelcomeRef.current}
						/>
					) : null}
...
function ContrastAppearanceSync() {
	const appearanceContrast = useClientSettings(
		(settings) => settings.appearanceContrast,
	);
...
function FontAppearanceSync() {
	const fontFamilySans = useClientSettings(
		(settings) => settings.fontFamilySans,
	);
	const fontFamilyCode = useClientSettings(
		(settings) => settings.fontFamilyCode,
	);
	const fontFamilyComposer = useClientSettings(
		(settings) => settings.fontFamilyComposer,
	);
	const fontSizeInterface = useClientSettings(
		(settings) => settings.fontSizeInterface,
	);
	const fontSizePrompt = useClientSettings(
		(settings) => settings.fontSizePrompt,
	);
...
function EventRouter({
	...
	const [keybindingsToastController] =
		useState<KeybindingsUpdateToastController>(() =>
			createKeybindingsUpdateToastController({}),
		);
	...
	const handleWelcome = useEffectEvent(
		(payload: ServerLifecycleWelcomePayload | null) => {
			...
			void (async () => {
				...
				useUiStateStore
					.getState()
					.setProjectExpanded(bootstrapProjectKey, true);
