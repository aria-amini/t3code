...
const INVOLVEMENT_TABS = [
	...
	PullRequestFilterOption<PullRequestInvolvement>
>;
...
const STATE_TABS = [
	...
	PullRequestFilterOption<PullRequestListState>
>;
...
const SORT_OPTIONS = [
	...
	PullRequestFilterOption<PullRequestListSort>
>;
 ...
 */
const PULL_REQUESTS_PANEL_ENVIRONMENT_ID =
	"pull-requests-panel" as EnvironmentId;
/** Stable so a read that is not wanted right now does not re-key on every render. */
const NO_LIST_TARGETS: ReadonlyArray<
	EnvironmentQueryTarget<PullRequestListInput>
> = [];
...
const pullRequestListEntryId = (
	target: Parameters<typeof pullRequestSurfaceId>[0],
) =>
	pullRequestSurfaceId({
		...target,
...
function pullRequestSearchLabels(
	raw: unknown,
): Partial<Pick<PullRequestsSearch, "labels">> {
	const values = (
		Array.isArray(raw) ? raw : typeof raw === "string" ? [raw] : []
	).slice(0, MAX_SEARCH_LABEL_CANDIDATES);
...
export const Route = createFileRoute("/_chat/pull-requests")({
	validateSearch: (raw: Record<string, unknown>): PullRequestsSearch => ({
		involvement:
			raw.involvement === "reviewing" || raw.involvement === "authored"
				? raw.involvement
				: "all",
		state:
...
function PullRequestsRouteView() {
	...
	const statsPolicy: PullRequestStatsPolicy =
		sort === "ready" || sort === "largest" || sort === "smallest"
			? "eager"
			: "visible";
	...
	const capableEnvironments = useMemo(
		() =>
			environments
				.filter(
					(environment) =>
						environment.serverConfig?.environment.capabilities.pullRequests ===
						true,
				)
				.toSorted((left, right) =>
			...
			capableEnvironments
				.filter(
					(environment) =>
						...
						environment.environmentId === scopedEnvironmentId,
				)
	...
	const capabilityKnown = environments.some(
		(environment) => environment.serverConfig !== null,
	);
	...
	const selectedProjectId =
		linkedProjectId ?? projectIdForRepository ?? scopedProjectId;
	...
	const selectedPullRequestSurface =
		selectedRightPanelSurface?.kind === "pull-request"
			...
			: null;
	const activePullRequestSurface = rightPanelState.isOpen
	...
	const renderedPullRequestSurface =
	...
	const typedParsed = useMemo(
		() => parsePullRequestQuery(typedQuery),
		[typedQuery],
	);
	const sentParsed = useMemo(
		() => parsePullRequestQuery(sentQuery),
		[sentQuery],
	);
	...
	const menuFilters = useMemo(
		(): PullRequestListFilters => ({
			...
			...(search.labels
				? { labels: search.labels.map((label) => [label]) }
				: {}),
		}),
	...
	const environmentQueries = useMemo((): ReadonlyArray<{
		...
		for (const project of projects) {
			totals.set(
				...
				(totals.get(project.environmentId) ?? 0) + 1,
			);
		}
	...
	const partitionsWanted =
		search.involvement === "all" && typedQuery.length === 0;
	...
	const refreshFromHost = async (includeDetail = true) => {
		...
		try {
			...
			await Promise.all(
				queryEnvironmentIds.map((environmentId) =>
					invalidate({ environmentId, input: {} }),
				),
			);
		} finally {
		...
		const batches = pullRequestStatsRefreshBatches({
			...
			candidateKeys:
				visible.key === requestedStatsScope.key ? visible.values : new Set(),
			statsByRow: statsByRowRef.current,
		});
		if (batches !== null) {
			setStatsTargetState({ key: requestedStatsScope.key, batches });
			statsQuery.refresh(
				batches.map(({ environmentId, input }) => ({ environmentId, input })),
			);
		}
	...
	useEffect(() => {
		...
		setLoaded((current) => {
			...
			if (current !== null && current.environmentKey === environmentKey)
				return current;
	...
	useEffect(() => {
		...
		setLoaded((current) => {
			...
			const partitions =
				partitionsWanted &&
				authoredQuery.data !== null &&
				reviewingQuery.data !== null
					? {
			...
			if (environmentKey.length > 0 && sentQuery.length === 0) {
				const accumulatedEntries =
					ordered?.key === filterKey ? ordered.entries : data.entries;
				writePullRequestListSnapshot(
					typeof window === "undefined" ? undefined : window.localStorage,
					environmentKey,
					{
	...
	const answered =
		(sentQuery.length === 0 && sentCursors === null && pageSize === PAGE_SIZE
			? baselineQuery.data
			: listQuery.data) ??
		(loaded?.scope === scopeKey && loaded.query === sentQuery
			? loaded.data
			: null);
		...
		setOrdered((previous) => {
			...
			if (sentCursors !== null) {
				...
				const arrived = answered.entries.filter(
					(entry) => !held.has(pullRequestEntryKey(entry)),
				);
				const appended = rankPullRequestMatches(
					arrived.toSorted((left, right) =>
		...
		if (canContinue) {
			setPage({
				...
				size:
					regrown.length === 0
						? pageSize
						: Math.min(pageSize + PAGE_SIZE, MAX_PAGE_SIZE),
				cursors: nextCursors,
				regrown,
			});
			return;
	...
	const refreshList = (includeRelated = false) => {
		...
		const loadedCount =
			ordered?.key === filterKey ? ordered.entries.length : pageSize;
		setPage({
			key: filterKey,
			regrown: [],
			size: Math.min(
	...
	const viewers =
		baselineQuery.data?.viewers ?? listData?.viewers ?? EMPTY_VIEWERS;
	...
	const searchingHosts = useMemo(
		() =>
			new Set(
				(baselineQuery.data?.providers ?? listData?.providers ?? []).flatMap(
					(provider) => (provider.searchesOnHost ? [provider.host] : []),
				),
			),
	...
	const listedPullRequestTabStatuses = useMemo<
	...
	const groups = useMemo(() => {
		...
		const narrow = (
			rows: ReadonlyArray<EnvironmentPullRequestEntry> | undefined,
		) =>
		...
		const authored = narrow(
			partitionsWanted
				? (authoredQuery.data?.entries ?? held?.authored)
				: undefined,
		);
	...
	const entriesByStatsKey = useRef<
		ReadonlyMap<string, EnvironmentPullRequestEntry>
	>(new Map());
	entriesByStatsKey.current = new Map(
		groups.flatMap((group) =>
			group.entries.map(
				(entry) => [pullRequestEntryKey(entry), entry] as const,
			),
		),
	);
	...
	const [statsByRow, setStatsByRow] = useState<PullRequestDiffStats>(
		() => new Map(),
	);
	...
	const statsBatches =
		statsTargetState.key === filterKey ? statsTargetState.batches : [];
	...
	const registerStatsRow = useCallback((node: HTMLButtonElement | null) => {
		...
		return () => {
			...
			setStatsTargetState((current) => {
				...
				const batches = retainVisiblePullRequestStatsBatches(
					current.batches,
					visible.values,
				);
				return batches.length === current.batches.length
					? current
					: { key: current.key, batches };
			});
		};
	}, []);
	useEffect(() => {
		...
		const observer = new IntersectionObserver(
			(observed) => {
				...
				setStatsTargetState((current) => {
					...
					if (added.length === 0 && retained.length === batches.length)
						return current;
	...
	useEffect(() => {
		...
		const visible = visibleStatsKeys.current;
		setStatsTargetState((current) => {
			if (current.key !== filterKey || visible.key !== filterKey)
				return current;
			const batches = retainVisiblePullRequestStatsBatches(
				current.batches,
				visible.values,
	...
	useEffect(() => {
		...
		useRightPanelStore
			.getState()
			.openPullRequest(rightPanelRef, linkedSelection);
	...
	useEffect(() => {
		...
		setHosts((previous) =>
			search.host === undefined || previous.length === 0
				? answered.providers
				: previous,
		);
								...
								return (
									<PullRequestRow
										...
										environmentLabels.get(entry.environmentId) !== undefined
											? {
													environmentLabel: environmentLabels.get(
														entry.environmentId,
													)!,
												}
											: {})}
										...
										matchedElsewhere={
											...
											scorePullRequestMatch(entry, typedParsed.text) <=
	...
	const closeSurfacesToRight = (surface: PullRequestSurface) => {
		...
		useRightPanelStore
			.getState()
			.closeSurfacesToRight(rightPanelRef, surface.id);
		const next = selectActiveRightPanelSurface(
			useRightPanelStore.getState().byThreadKey,
			rightPanelRef,
		);
	...
	const closeActiveSurfaceFromShortcut = useEffectEvent(
		(event: KeyboardEvent) => {
