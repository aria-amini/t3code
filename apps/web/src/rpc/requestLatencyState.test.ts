...
describe("requestLatencyState", () => {
	...
	it.each(
		Object.values(WS_METHODS).filter((method) =>
			method.startsWith("pullRequests."),
		),
	)("ignores pull request workspace request %s", (method) => {
	...
	it("evicts the oldest pending requests once the tracker reaches capacity", () => {
		...
		expect(slowRequests.at(-1)?.requestId).toBe(
			String(MAX_TRACKED_RPC_ACK_REQUESTS),
		);
	});
});
