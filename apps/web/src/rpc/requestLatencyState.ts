...
const slowRpcAckRequestsAtom = Atom.make<ReadonlyArray<SlowRpcAckRequest>>(
	[],
).pipe(Atom.keepAlive, Atom.withLabel("slow-rpc-ack-requests"));
...
export function acknowledgeRpcRequest(requestId: string): void {
	...
	setSlowRpcAckRequests(
		slowRequests.filter((request) => request.requestId !== requestId),
	);
}
