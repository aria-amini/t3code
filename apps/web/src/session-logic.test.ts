					...
					agentKind: classifyTaskAgentKind({
						taskType:
							typeof rawPayload.taskType === "string"
								...
								: undefined,
						agentId:
							typeof rawPayload.agentId === "string"
								? rawPayload.agentId
								: undefined,
					}),
				}
...
describe("deriveActivePlanState", () => {
	...
	it("starts timing again after a plan is cleared and recreated", () => {
		...
		expect(
			deriveActivePlanState(activities, TurnId.make("turn-1"))?.steps,
		).toEqual([{ durationMs: 3_000, step: "Check", status: "completed" }]);
	});
	...
	it("keeps the latest task progress without emitting plan-update log entries", () => {
		const activities = [
			...
			makeActivity({
				...
				sequence: 1,
				payload: {
					plan: [{ step: "Verify the composer", status: "inProgress" }],
				},
			}),
			makeActivity({
				id: "plan-2",
				...
				sequence: 2,
				payload: {
					plan: [{ step: "Verify the composer", status: "completed" }],
				},
			}),
			makeActivity({
				id: "after",
		...
		expect(
			deriveActivePlanState(activities, TurnId.make("turn-1"))?.steps,
		).toMatchObject([{ step: "Verify the composer", status: "completed" }]);
	});
...
describe("image asset requests", () => {
	...
	it("requests offscreen handoffs without signing the rest of the loaded history", () => {
		...
		expect(
			selectHandoffImageResources(
				[history, message, offscreen, empty, assistant],
				{
	...
	it("does not scan history when no handoff is pending", () => {
		...
		expect(
			selectHandoffImageResources(undefined, { missing: ["blob:missing"] }),
		).toBe(empty);
	...
	it("hands signed URLs to a mounted row only after the local preview is released", () => {
		...
		expect(
			selectHandoffImageResources([message], {
				[message.id]: ["blob:pending"],
			}),
		).toEqual([{ _tag: "attachment", attachmentId: image.id }]);

		const ready = server(message, () => "https://server.test/image");
		expect(
			selectMessageImageResources(
				handoff(ready, () => "blob:pending").attachments,
			),
		).toEqual([]);
		...
		expect(
			row(released, () => "https://server.test/renewed").attachments?.[0],
		).toMatchObject({
			previewUrl: "https://server.test/renewed",
		});
		expect(displayed.attachments?.[0]).toMatchObject({
...
describe("deriveTimelineEntries", () => {
	...
	it("reuses preview objects while preserving URL and attachment metadata changes", () => {
		...
		const message = {
			...streamingMessage,
			attachments: Object.freeze([image, file]),
		};
		...
		expect(
			project(message, (attachment) => new Map(urls).get(attachment.id)),
		).toBe(first);
		const streamed = project({ ...message, text: "Next" }, (attachment) =>
		...
		expect(renewed.attachments?.[0]).toMatchObject({
	...
	it("keeps pending preview handoffs stable and restores the current server URL", () => {
		...
		expect(ready.attachments?.[0]).toMatchObject({
			previewUrl: "https://server.test/image",
		});
		expect(first.attachments?.[0]).toMatchObject({
			previewUrl: "blob:handoff",
	...
	it("reuses ordered history without changing an earlier projection", () => {
		...
		const firstBranch = deriveTimelineEntriesWithState(
			[history, firstMessage],
			[],
			work,
			first,
		);
		const secondBranch = deriveTimelineEntriesWithState(
		...
		expect(firstBranch.entries).toEqual(
			deriveTimelineEntries([history, firstMessage], [], work),
		);
		expect(secondBranch.entries).toEqual(
			deriveTimelineEntries([history, secondMessage], [], work),
		);
		...
		expect(firstBranch.entries[1]).toMatchObject({
			message: { text: "First" },
		});
	});
	...
	it("preserves stable source ordering for ties, append, and older pages", () => {
		...
		const prepended = deriveTimelineEntriesWithState(
			[older, ...messages],
			[plan],
			work,
			appended,
		);
		expect(prepended.entries).toEqual(
			deriveTimelineEntries([older, ...messages], [plan], work),
		);
		const corrected = {
			...streamingMessage,
		...
		expect(
			deriveTimelineEntriesWithState(
				[corrected, appendedMessage],
				[plan],
				work,
				appended,
			).entries,
		).toEqual(
			deriveTimelineEntries([corrected, appendedMessage], [plan], work),
		);
	});
...
describe("deriveWorkLogEntries quiet-timeline guarantee", () => {
	it("N concurrent subagents produce exactly N lifecycle rows, zero attributed tool rows", () => {
		...
		expect(
			entries.some((entry) => entry.sourceActivityKind?.startsWith("tool.")),
		).toBe(false);
	});
	...
	it("folds timelineBypass agent rows into one CTA (Codex children, workflow members)", () => {
		...
		expect(entries).toHaveLength(1);
		expect(entries[0]!.agentSpawn?.agentTaskIds).toEqual([
			"child-1",
			"child-2",
		]);
	});
...
describe("rerun workflows", () => {
	...
	it("each workflow run gets its own CTA row (distinct coordinator ids)", () => {
		...
		const spawnRows = entries.filter((entry) => entry.agentSpawn !== undefined);
		expect(spawnRows.map((row) => row.agentSpawn!.workflowId)).toEqual([
			"wf-run1",
			"wf-run2",
		]);
	...
	it("reuses entries when appending to 20,000 ordered tool activities", () => {
		...
		const updatedActivities = [
			...
			makeActivity({
				...
				createdAt: new Date(
					1_700_000_000_000 + activities.length,
				).toISOString(),
				kind: "tool.completed",
				summary: "Ran command",
		...
		expect(
			initialEntries.every((entry, index) => updatedEntries[index] === entry),
		).toBe(true);
