...
function planStateFromActivity(
...
function addPlanStepDurations(
	...
	const timings = new Map<
		string,
		{ completedAt?: number; startedAt?: number }
	>();
	...
	for (const [key, timing] of [...timings.entries()].toSorted(
		(left, right) =>
			(left[1].completedAt ?? Infinity) - (right[1].completedAt ?? Infinity),
	)) {
...
export function deriveActivePlanState(
	...
	return addPlanStepDurations(
		plan,
		matchingActivities.slice(latestClearIndex + 1),
	);
}
...
function toDerivedWorkLogEntry(
	...
	if (detail) {
		...
		if (
			message &&
			normalizePreviewForComparison(message) !==
				normalizePreviewForComparison(activity.summary)
		) {
	...
	for (const entry of entries) {
		...
		if (isTaskRow && entry.taskId !== undefined) {
			...
			if (existingIndex !== undefined) {
				...
				const agentTaskIds = existing.agentSpawn?.agentTaskIds.includes(
					entry.taskId,
				)
		...
		if (lifecycleKey !== undefined) {
			...
			if (
				...
				collapsed[matchingLifecycleIndex] = mergeDerivedWorkLogEntries(
					matchingEntry,
					entry,
				);
				continue;
...
function extractToolCallId(
	...
	const data = asRecord(payload?.data);
	return (
		asTrimmedString(payload?.toolCallId) ?? asTrimmedString(data?.toolCallId)
	);
}
...
function timelineEntryFromProposedPlan(
	proposedPlan: ProposedPlan,
): TimelineEntry {
	return {
		id: proposedPlan.id,
...
function compareTimelineEntriesByCreatedAt(
	left: TimelineEntry,
	right: TimelineEntry,
): number {
...
function shouldTakePreviousTimelineEntry(
	previous: TimelineEntry,
	suffix: TimelineEntry,
): boolean {
	const createdAtComparison = compareTimelineEntriesByCreatedAt(
		previous,
		suffix,
	);
...
function hasExactArrayPrefix<T>(
...
function mergeTimelineEntrySuffix(
	...
	for (let index = 1; index < suffix.length; index += 1) {
		if (
			compareTimelineEntriesByCreatedAt(suffix[index - 1]!, suffix[index]!) > 0
		) {
	...
	if (
		...
		(previousLast === undefined ||
			shouldTakePreviousTimelineEntry(previousLast, suffix[0]!))
	) {
	...
	while (previousIndex < previous.length || suffixIndex < suffix.length) {
		...
		if (
			...
			(suffixEntry === undefined ||
				shouldTakePreviousTimelineEntry(previousEntry, suffixEntry))
		) {
...
type AttachmentResource = Extract<
	AssetResource,
...
export function createMessageAttachmentPreviewProjector() {
	...
	return (
		...
		for (const [index, attachment] of source.entries()) {
			...
			const previousUrl =
				"previewUrl" in previousAttachment
					? previousAttachment.previewUrl
					: undefined;
			const next =
...
function replaceStreamingTimelineMessages(
	...
	return previous.entries.map((entry) => {
		const replacement =
			entry.kind === "message" ? replacements.get(entry.message) : undefined;
...
export function deriveTimelineEntries(
	...
	return deriveTimelineEntriesWithState(messages, proposedPlans, workEntries)
		.entries;
}
