...
function getDesktopNetworkAccessBridge():
	| DesktopNetworkAccessBridge
	| undefined {
...
export function createDesktopNetworkAccessStateAtom(
	...
	const loadDesktopNetworkAccess = Effect.fn("loadDesktopNetworkAccess")(
		function* () {
				...
				[
					Effect.tryPromise({
						try: () => bridge.getServerExposureState(),
							...
							new DesktopServerExposureStateLoadError({ cause }),
					}),
					Effect.tryPromise({
						try: () => bridge.getAdvertisedEndpoints(),
						catch: (cause) =>
							new DesktopAdvertisedEndpointsLoadError({ cause }),
					}),
				],
...
export const desktopNetworkAccessStateAtom =
	createDesktopNetworkAccessStateAtom(getDesktopNetworkAccessBridge);
