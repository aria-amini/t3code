...
import {
	createDesktopSshHostsStateAtom,
	filterDiscoveredSshHosts,
} from "./desktopSshHosts";
...
describe("filterDiscoveredSshHosts", () => {
	...
	it("trims the query", () => {
		expect(filterDiscoveredSshHosts(suggestions, "  pi ")).toEqual([
			suggestions[1],
		]);
	});
	...
	it("matches case-insensitively", () => {
		expect(filterDiscoveredSshHosts(suggestions, "PINOT")).toEqual([
			suggestions[1],
		]);
	});
...
describe("desktopSshHostsState", () => {
	...
	it("retains the desktop bridge failure as the discovery error cause", async () => {
		...
		if (!AsyncResult.isFailure(result))
			throw new Error("Expected SSH host discovery to fail.");
