...
const EMPTY_THREAD_SHELL_ATOM = Atom.make<EnvironmentThreadShell | null>(
	null,
).pipe(Atom.withLabel("web-thread-shell:empty"));
const EMPTY_THREAD_DETAIL_ATOM = Atom.make<EnvironmentThread | null>(null).pipe(
	Atom.withLabel("web-thread-detail:empty"),
);
const EMPTY_THREAD_STATUS_ATOM = Atom.make<EnvironmentThreadStatus>(
	"empty",
).pipe(Atom.withLabel("web-thread-status:empty"));
...
export function resolveThreadDetailRef(
	...
	return ref !== null && (!options.waitForShell || options.shellExists)
		? ref
		: null;
}
...
export function readThreadShell(
	ref: ScopedThreadRef,
): EnvironmentThreadShell | null {
...
export function readEnvironmentSupportsSettlement(
...
export function readEnvironmentSupportsSnooze(
	environmentId: EnvironmentId,
): boolean {
	return (
		appAtomRegistry.get(environmentServerConfigsAtom).get(environmentId)
...
/** Whether the environment's server understands thread.pin/unpin.
    Same version-skew contract as settlement. */
export function readEnvironmentSupportsPinning(
	environmentId: EnvironmentId,
): boolean {
	return (
		appAtomRegistry.get(environmentServerConfigsAtom).get(environmentId)
...
/** Whether the environment's server understands thread title regeneration.
    Same version-skew contract as settlement. */
export function readEnvironmentSupportsTitleRegeneration(
	environmentId: EnvironmentId,
): boolean {
	return (
		appAtomRegistry.get(environmentServerConfigsAtom).get(environmentId)
...
export function readEnvironmentSupportsPinReorder(
	environmentId: EnvironmentId,
): boolean {
	return (
		appAtomRegistry.get(environmentServerConfigsAtom).get(environmentId)
...
export function readEnvironmentSupportsActiveReorder(
	environmentId: EnvironmentId,
): boolean {
	return (
		appAtomRegistry.get(environmentServerConfigsAtom).get(environmentId)
