...
export function newestPullRequestSummary(
	...
	return Date.parse(observed.updatedAt) >= Date.parse(current.updatedAt)
		? observed
		: current;
}
...
function createMergedEnvironmentQuery<Input, A>(
	...
	return function useMergedQuery(
		targets: ReadonlyArray<EnvironmentQueryTarget<Input>>,
	) {
...
const usePullRequestTurnRefreshQuery = createMergedEnvironmentQuery(
	...
	({
		environmentId,
	}: EnvironmentQueryTarget<Readonly<Record<string, never>>>) =>
...
export function usePullRequestList(
	...
	const data = useMemo(
		() => mergePullRequestLists(query.values),
		[query.values],
	);
	return {
		data,
