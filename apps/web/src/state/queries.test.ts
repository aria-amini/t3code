...
describe("areProjectPathSearchTargetsEqual", () => {
	...
	it("requires the environment, workspace, query, entry kind, and image filter to match", () => {
		...
		expect(
			areProjectPathSearchTargetsEqual(target, { ...target, query: "readme" }),
		).toBe(false);
		expect(
			areProjectPathSearchTargetsEqual(target, { ...target, kind: "file" }),
		).toBe(false);
		expect(
			areProjectPathSearchTargetsEqual(target, { ...target, imageOnly: true }),
		).toBe(false);
	});
});
