...
> {
	...
	readonly run: (
		...
		AtomCommandResult<
			...
			AtomCommandFailure<R> | VcsActionUnavailableError
		>
	>;
	...
	const run = useCallback(
		async (...args: TArgs) => {
			...
			> => {
				...
				return result as AtomCommandResult<
					AtomCommandSuccess<R>,
					AtomCommandFailure<R>
				>;
			};
...
export function useGitStackedAction(scope: SourceControlActionScope) {
	const runStackedAction = useAtomCommand(
		vcsActionManager.runStackedAction(scope),
		{
			reportFailure: false,
		},
	);
...
export function useSourceControlPublishRepositoryAction(
	scope: SourceControlActionScope,
) {
	const publishRepository = useAtomCommand(
		sourceControlEnvironment.publishRepository,
		{
			reportFailure: false,
		},
	);
...
export function usePreparePullRequestThreadAction(
	scope: SourceControlActionScope,
) {
	const preparePullRequestThread = useAtomCommand(
...
export function usePullRequestResolutionState(
	target: PullRequestResolutionTarget,
) {
