...
describe("selectKnownTerminalSessions", () => {
	it("preserves numeric terminal order and stable picker ties across threads", () => {
		...
		const selected = selectKnownTerminalSessions(
			metadata,
			environmentA,
			threadB,
		);
		...
		expect(selectKnownTerminalSessions(repeated, environmentA, threadB)).toBe(
			selected,
		);

		const reordered = metadata.toReversed();
		expect(selectKnownTerminalSessions(reordered, environmentA, threadB)).toBe(
			selected,
		);
		expect(
	...
	it("changes only the affected group and preserves earlier snapshots", () => {
		...
		const beforeA = selectKnownTerminalSessions(
			metadata,
			environmentA,
			threadA,
		);
		const beforeB = selectKnownTerminalSessions(
			metadata,
			environmentA,
			threadB,
		);
		...
		expect(selectKnownTerminalSessions(next, environmentA, threadB)).toBe(
			beforeB,
		);
		...
		expect(selectKnownTerminalSessions(metadata, environmentA, threadA)).toBe(
			beforeA,
		);

		const renamed = [first, exited, { ...other, label: "Renamed" }];
		expect(selectKnownTerminalSessions(renamed, environmentA, threadA)).toBe(
			afterA,
		);
		expect(
			selectKnownTerminalSessions(renamed, environmentA, threadB)[0]?.state
				.summary?.label,
		).toBe("Renamed");
		expect(selectKnownTerminalSessions([other], environmentA, threadA)).toEqual(
			[],
		);
		expect(selectKnownTerminalSessions([other], environmentA, threadB)).toBe(
			beforeB,
		);
	});
	...
	it("keeps environment targets separate when snapshots share the same objects", () => {
		...
		expect(
			selectKnownTerminalSessions([...metadata], environmentA, threadA),
		).toBe(first);
		expect(
			selectKnownTerminalSessions([...metadata], environmentB, threadA),
		).toBe(second);
	});
	...
	it("returns stable empty results without a connected environment or matching sessions", () => {
		...
		expect(selectKnownTerminalSessions(metadata, environmentA, threadB)).toBe(
			empty,
		);
	});
	...
	it("does not materialize unrelated raw thread identifiers", () => {
		const metadata = [
			summary(" untrimmed-thread ", "terminal-1"),
			summary(threadA, "terminal-2"),
		];
		expect(
	...
	it("reads the metadata array once for all thread selectors", () => {
		...
		expect(
			selectKnownTerminalSessions(metadata, environmentA, null),
