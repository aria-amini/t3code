...
const EMPTY_KNOWN_TERMINAL_SESSIONS = Object.freeze<
...
interface TerminalMetadataIndex {
	...
	readonly views: Map<
		EnvironmentId,
		Map<ThreadId | null, ReadonlyArray<KnownTerminalSession>>
	>;
}

const metadataIndexes = new WeakMap<
	ReadonlyArray<TerminalSummary>,
	TerminalMetadataIndex
>();
const sessionsBySummary = new WeakMap<
	TerminalSummary,
	Map<EnvironmentId, KnownTerminalSession>
>();
...
function terminalMetadataIndex(
	metadata: ReadonlyArray<TerminalSummary>,
): TerminalMetadataIndex {
	let index = metadataIndexes.get(metadata);
	if (!index) {
		const compare = new Intl.Collator(undefined, { numeric: true }).compare;
		const all = metadata.toSorted((left, right) =>
			compare(left.terminalId, right.terminalId),
		);
...
export function selectKnownTerminalSessions(
	...
	const summaries =
		threadId === null ? index.all : index.byThreadId.get(threadId);
	...
	const sessions =
		...
		previous.every(
			(session, index) => session.state.summary === summaries[index],
		)
			? previous
		...
		const summary =
			(metadata.data === null
				...
				: terminalMetadataIndex(metadata.data)
						...
						?.find(
							(terminal) => terminal.terminalId === input.terminal?.terminalId,
						)) ?? null;
		const state = combineTerminalSessionState(
			summary,
			attach.data ?? EMPTY_TERMINAL_BUFFER_STATE,
		);
		return attach.error === null
			? state
...
export function useThreadRunningTerminalIds(input: {
	...
	return useMemo(
		() => selectRunningSubprocessTerminalIds(sessions),
		[sessions],
	);
}
