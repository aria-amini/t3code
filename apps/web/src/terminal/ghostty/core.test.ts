...
describe("GhosttyTerminalCore snapshots", () => {
	...
	it("preserves styles, wide cells, and selection after shared memory grows", async () => {
		...
		core.write(
			`\x1b[1;3;4;8;9;53;38;2;123;45;67;48;2;9;8;7m${grapheme}\x1b[0m界🙂`,
		);
		...
		expect(core.snapshot().rowData[0]!.cells[0]).toEqual({
			...cells[0],
			selected: true,
		});
		core.clearSelection();
	...
	it("reuses a grown grapheme buffer and releases it on disposal", async () => {
		...
		if (allocation.type !== "return")
			throw new Error("Grapheme allocation did not return");
	...
	it.each(["varied", "identical"] as const)(
		...
		async (kind) => {
			...
			const first = readTerminalOutputUpdate(
				state.output,
				INITIAL_TERMINAL_OUTPUT_CURSOR,
			);
	...
	it("answers a live VT query when batched reads cross a chunk compaction", async () => {
		...
		expect({ type: update.type, replies }).toEqual({
			type: "append",
			replies: ["\x1b[0n"],
		});
	});
	...
	it("recovers a lagging renderer once from bounded output and resumes appending", async () => {
		...
		if (recovery.type !== "reset")
			throw new Error(`Expected reset, received ${recovery.type}`);
