 ...
 */
export function ghosttyCellText(
	codepointView: DataView,
	graphemeLength: number,
): string {
	if (graphemeLength === 1)
		return String.fromCodePoint(codepointView.getUint32(0, true));
...
export class GhosttyTerminalCore {
	...
	dispose(): void {
		...
		if (this.rowCellsSlot) {
			...
			if (cells)
				this.runtime.call("ghostty_render_state_row_cells_free", cells);
		}
	...
	private readRow(
		...
		const { size: styleSize, fields: styleFields } =
			this.runtime.layout("GhosttyStyle");
		...
		while (
			...
			cells.push({
				...
				strikethrough:
					styleView.getUint8(styleFields.strikethrough!.offset) !== 0,
				overline: styleView.getUint8(styleFields.overline!.offset) !== 0,
				underline:
					styleView.getInt32(styleFields.underline!.offset, true) !== 0,
				selected,
			});
		}
