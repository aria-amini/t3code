...
function decodeWasmDataUrl(dataUrl: string): Uint8Array {
	...
	if (!encoded)
		throw new Error("The vendored Ghostty WASM data URL is invalid");
...
describe("vendored libghostty-vt WebAssembly", () => {
	...
	it("creates, writes multi-codepoint graphemes, and frees repeated terminals", async () => {
		...
		const call = (name: string, ...args: number[]) =>
			(exports[name] as WasmFunction)(...args);
		...
		const layouts = JSON.parse(
			new TextDecoder().decode(jsonBytes.subarray(0, jsonEnd)),
		) as Record<string, { size: number }>;
		const optionsSize = layouts.GhosttyTerminalOptions?.size;
		expect(optionsSize).toBe(8);
		if (optionsSize === undefined)
			throw new Error("GhosttyTerminalOptions layout is missing");
		...
		for (let iteration = 0; iteration < 25; iteration += 1) {
			...
			const terminal = new DataView(memory.buffer).getUint32(
				terminalSlot,
				true,
			);
	...
	it("blinks the default cursor until a program asks for a steady one", async () => {
		...
		const renderState = new DataView(memory.buffer).getUint32(
			renderStateSlot,
			true,
		);
		...
		const blinking = () => {
			expect(call("ghostty_render_state_update", renderState, terminal)).toBe(
				0,
			);
			expect(call("ghostty_render_state_get", renderState, 12, scratch)).toBe(
				0,
			);
	...
	it("reports and scrolls the viewport with Ghostty's scrollbar state", async () => {
		...
		expect(
			[0, 8, 16].map((offset) =>
				Number(scrollbarView.getBigUint64(offset, true)),
			),
		).toEqual([51, 41, 10]);
	...
	it("routes terminal-generated replies through the shared callback table", async () => {
		...
		const main =
			mainResult instanceof WebAssembly.Instance
				? mainResult
				: mainResult.instance;
	...
	it("formats the active selection with Ghostty's copy semantics", async () => {
		...
		const outputSize = new DataView(memory.buffer, written, 4).getUint32(
			0,
			true,
		);
		...
		expect(
			new TextDecoder().decode(
				new Uint8Array(memory.buffer, output, outputSize),
			),
		).toBe("a\n\nb");
	...
	it("uses Ghostty for mouse encoding, word selection, and OSC 8 hit testing", async () => {
		...
		const terminalSlot = call("ghostty_wasm_alloc_opaque");
		expect(call("ghostty_terminal_new", 0, terminalSlot, terminalOptions)).toBe(
			0,
		);
		const terminal = new DataView(memory.buffer).getUint32(terminalSlot, true);
		const input = new TextEncoder().encode(
		...
		new Uint8Array(memory.buffer, anyEventPointer, anyEventInput.length).set(
			anyEventInput,
		);
		call(
			...
			terminal,
			anyEventPointer,
			anyEventInput.length,
		);
		expect(call("ghostty_terminal_mode_get", terminal, 1003, modeFlag)).toBe(0);
		expect(new Uint8Array(memory.buffer, modeFlag, 1)[0]).toBe(1);
		const anyEventReset = new TextEncoder().encode(
			"\u001b[?1003l\u001b[?1000h",
		);
		const anyEventResetPointer = alloc(anyEventReset.length);
		new Uint8Array(
			memory.buffer,
			anyEventResetPointer,
			anyEventReset.length,
		).set(anyEventReset);
		call(
			"ghostty_terminal_vt_write",
			terminal,
		...
		const written = call("ghostty_wasm_alloc_usize");
		expect(call("ghostty_grid_ref_hyperlink_uri", gridRef, 0, 0, written)).toBe(
			-3,
		);
		const hyperlinkSize = new DataView(memory.buffer, written, 4).getUint32(
			0,
			true,
		);
		...
		expect(
			new TextDecoder().decode(
				new Uint8Array(memory.buffer, hyperlink, hyperlinkSize),
			),
		).toBe("https://t3.codes/docs");
		...
		new DataView(memory.buffer, selection, 32).setUint32(0, 32, true);
		expect(
			call("ghostty_terminal_select_word", terminal, wordOptions, selection),
		).toBe(0);
		expect(call("ghostty_terminal_set", terminal, 21, selection)).toBe(0);
		const formatOptions = alloc(16);
		const formatView = new DataView(memory.buffer, formatOptions, 16);
		...
		const selectionSize = new DataView(memory.buffer, written, 4).getUint32(
			0,
			true,
		);
		...
		expect(
			new TextDecoder().decode(
				new Uint8Array(memory.buffer, selectionText, selectionSize),
			),
		).toBe("linked");
		...
		expect(
			call("ghostty_terminal_select_line", terminal, lineOptions, selection),
		).toBe(0);
		expect(call("ghostty_terminal_set", terminal, 21, selection)).toBe(0);
		expect(
			call(
		...
		const lineSelectionSize = new DataView(memory.buffer, written, 4).getUint32(
			0,
			true,
		);
		...
		expect(
			new TextDecoder().decode(
				new Uint8Array(memory.buffer, lineSelectionText, lineSelectionSize),
			),
		).toBe("linked plain");
		...
		const mouseOutputSize = new DataView(memory.buffer, written, 4).getUint32(
			0,
			true,
		);
		expect(
			new TextDecoder().decode(
				new Uint8Array(memory.buffer, mouseOutput, mouseOutputSize),
			),
		).toBe("\u001b[<0;2;2M");
	...
	it("encodes modified printable keys in Kitty keyboard mode", async () => {
		...
		new Uint8Array(memory.buffer, kittyModePointer, kittyMode.length).set(
			kittyMode,
		);
		call(
			"ghostty_terminal_vt_write",
		...
		const written = call("ghostty_wasm_alloc_usize");
		expect(
			call("ghostty_key_encoder_encode", keyEncoder, keyEvent, 0, 0, written),
		).toBe(-3);
		const outputSize = new DataView(memory.buffer, written, 4).getUint32(
			0,
			true,
		...
		const outputLength = new DataView(memory.buffer, written, 4).getUint32(
			0,
			true,
		);
		expect(
			new TextDecoder().decode(
				new Uint8Array(memory.buffer, output, outputLength),
			),
		).toBe("\u001b[99;5u");

		const remappedText = new TextEncoder().encode("j");
		const remappedTextPointer = alloc(remappedText.length);
		new Uint8Array(memory.buffer, remappedTextPointer, remappedText.length).set(
			remappedText,
		);
		call(
			"ghostty_key_event_set_unshifted_codepoint",
		...
		const remappedOutputSize = new DataView(
			memory.buffer,
			written,
			4,
		).getUint32(0, true);
		...
		const remappedOutputLength = new DataView(
			memory.buffer,
			written,
			4,
		).getUint32(0, true);
		expect(
			new TextDecoder().decode(
				new Uint8Array(memory.buffer, remappedOutput, remappedOutputLength),
			),
		).toBe("\u001b[106;5u");
		...
		new Uint8Array(memory.buffer, reportEventsPointer, reportEvents.length).set(
			reportEvents,
		);
		call(
			"ghostty_terminal_vt_write",
		...
		const releaseSize = new DataView(memory.buffer, written, 4).getUint32(
			0,
			true,
		);
		...
		const releaseLength = new DataView(memory.buffer, written, 4).getUint32(
			0,
			true,
		);
		expect(
			new TextDecoder().decode(
				new Uint8Array(memory.buffer, releaseOutput, releaseLength),
			),
		).toBe("\u001b[106;5:3u");
