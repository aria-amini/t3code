...
describe("GhosttyTerminalSurface visibility", () => {
	...
	function createHarness() {
		...
		const paint = vi.fn(
			(_operation: string, _args: ReadonlyArray<unknown>) => {},
		);
		let frameId = 0;
		...
		vi.stubGlobal("document", {
			createElement: (tag: string) =>
				tag === "canvas" ? canvas : new TerminalTestElement(),
		...
		return {
			...
			get renderedSnapshot() {
				...
				if (result?.type !== "return")
					throw new Error("No terminal snapshot was rendered");
				return result.value;
			},
			flushFrame() {
			...
			async create(options: Partial<GhosttyTerminalSurfaceOptions> = {}) {
				const surface = await GhosttyTerminalSurface.create(
	...
	it("keeps the selection on reveal and applies a hidden selection clear", async () => {
		...
		expect(
			harness.renderedSnapshot.rowData[0]?.cells
				.slice(0, 5)
		...
		expect(
			harness.renderedSnapshot.rowData[0]?.cells.some((cell) => cell.selected),
		).toBe(false);
	});
	...
	it("stops zero-size mounts and repaints when the same size returns", async () => {
		...
		harness.resize();
		expect(harness.snapshot).toHaveBeenCalledTimes(1);
		expect(harness.renderedSnapshot.rowData[0]?.text).toContain(
			"visible!xxxxxxxx",
		);
	...
	it.each([false, true])(
		...
		async (visible) => {
			...
			vi.spyOn(GhosttyTerminalCore, "create").mockImplementation(
				async (...args) => {
...
describe("terminalGridCellAt", () => {
	...
	it("maps points inside the rendered grid without clamping its padding", () => {
		expect(
			terminalGridCellAt({ ...options, clientX: 104, clientY: 224 }),
		).toEqual({
			x: 0,
			y: 0,
		});
		expect(
			terminalGridCellAt({ ...options, clientX: 133, clientY: 263 }),
		).toEqual({
			x: 2,
			y: 1,
		});
		expect(
			terminalGridCellAt({ ...options, clientX: 103, clientY: 224 }),
		).toBeNull();
		expect(
			terminalGridCellAt({ ...options, clientX: 104, clientY: 223 }),
		).toBeNull();
		expect(
			terminalGridCellAt({ ...options, clientX: 134, clientY: 224 }),
		).toBeNull();
		expect(
			terminalGridCellAt({ ...options, clientX: 104, clientY: 264 }),
		).toBeNull();
	});
});
...
describe("shouldBlinkTerminalCursor", () => {
	...
	it("holds the cursor steady when blinking would be unwanted", () => {
		...
		expect(shouldBlinkTerminalCursor({ ...blinking, focused: false })).toBe(
			false,
		);
		expect(
			shouldBlinkTerminalCursor({ ...blinking, cursorBlinking: false }),
		).toBe(false);
		expect(
			shouldBlinkTerminalCursor({ ...blinking, cursorVisible: false }),
		).toBe(false);
		expect(
			shouldBlinkTerminalCursor({ ...blinking, reducedMotion: true }),
		).toBe(false);
	});
});
...
describe("terminalLinkAtPositionWithRange", () => {
	it("maps terminal cells to UTF-16 offsets after a wide emoji", () => {
		...
		expect(
			terminalLinkAtPositionWithRange([row], 0, cells.length - 1)?.text,
		).toBe("https://t3.codes");
	...
	it("uses shared path matching and reconstructs soft-wrapped links", () => {
		...
		expect(terminalLinkAtPositionWithRange(rows, 0, 8)?.text).toBe(
			"https://example.com/reference",
		);
		expect(terminalLinkAtPositionWithRange(rows, 1, 4)?.text).toBe(
			"https://example.com/reference",
		);
		expect(terminalLinkAtPositionWithRange(rows, 2, 2)?.text).toBe(
			"~/project/file",
		);
		expect(terminalLinkAtPositionWithRange(rows, 3, 4)?.text).toBe(
			"C:\\repo\\file.ts",
		);
...
describe("isTerminalCopyShortcut", () => {
	const event = (
		overrides: Partial<Parameters<typeof isTerminalCopyShortcut>[0]> = {},
	) => ({
		ctrlKey: false,
		key: "c",
		metaKey: false,
	...
	it("keeps Ctrl+C available for SIGINT on macOS", () => {
		expect(isTerminalCopyShortcut(event({ ctrlKey: true }), "MacIntel")).toBe(
			false,
		);
		expect(isTerminalCopyShortcut(event({ metaKey: true }), "MacIntel")).toBe(
			true,
		);
	});
	...
	it("copies with Ctrl+C and Ctrl+Shift+C elsewhere", () => {
		expect(
			isTerminalCopyShortcut(event({ ctrlKey: true }), "Linux x86_64"),
		).toBe(true);
		expect(
			isTerminalCopyShortcut(
	...
	it("uses the produced character instead of the physical key position", () => {
		expect(
			isTerminalCopyShortcut(event({ key: "C", metaKey: true }), "MacIntel"),
		).toBe(true);
		expect(
...
describe("isTerminalPasteShortcut", () => {
	const event = (
		overrides: Partial<Parameters<typeof isTerminalPasteShortcut>[0]> = {},
	) => ({
		ctrlKey: false,
		key: "v",
		metaKey: false,
	...
	it("uses Cmd+V on macOS", () => {
		expect(isTerminalPasteShortcut(event({ metaKey: true }), "MacIntel")).toBe(
			true,
		);
		expect(isTerminalPasteShortcut(event({ ctrlKey: true }), "MacIntel")).toBe(
			false,
		);
	});
	...
	it("preserves Ctrl+V and uses Ctrl+Shift+V elsewhere", () => {
		expect(
			isTerminalPasteShortcut(event({ ctrlKey: true }), "Linux x86_64"),
		).toBe(false);
		expect(
			isTerminalPasteShortcut(
				event({ ctrlKey: true, shiftKey: true }),
	...
	it("supports the conventional Shift+Insert paste shortcut", () => {
		expect(
			isTerminalPasteShortcut(
				event({ key: "Insert", shiftKey: true }),
				"Linux x86_64",
			),
		).toBe(true);
		expect(
			isTerminalPasteShortcut(event({ key: "Insert" }), "Linux x86_64"),
		).toBe(false);
		expect(
			isTerminalPasteShortcut(
...
describe("isTerminalCompositionCommitInput", () => {
	it("identifies browser composition follow-up input", () => {
		...
		expect(
			isTerminalCompositionCommitInput({ inputType: "insertCompositionText" }),
		).toBe(true);
		expect(
	...
	it("keeps a fast repeated input as legitimate text", () => {
		expect(isTerminalCompositionCommitInput({ inputType: "insertText" })).toBe(
			false,
		);
	});
});
...
describe("isTerminalCompositionKey", () => {
	...
	it("treats in-progress IME keydowns as composition so the copy primer cannot wipe them", () => {
		expect(isTerminalCompositionKey(event({ isComposing: true }), false)).toBe(
			true,
		);
		expect(isTerminalCompositionKey(event(), true)).toBe(true);
		expect(isTerminalCompositionKey(event({ key: "Process" }), false)).toBe(
			true,
		);
...
describe("application mouse reporting", () => {
	const event = (
		overrides: Partial<Parameters<typeof shouldReportTerminalMouse>[1]> = {},
	) => ({
		ctrlKey: false,
		metaKey: false,
		shiftKey: false,
	...
	it("keeps Shift and link activation modifiers available to the browser", () => {
		...
		expect(shouldReportTerminalMouse(true, event({ shiftKey: true }))).toBe(
			false,
		);
		expect(shouldReportTerminalMouse(true, event({ ctrlKey: true }))).toBe(
			false,
		);
		expect(shouldReportTerminalMouse(true, event({ metaKey: true }))).toBe(
			false,
		);
	...
	it("drops repeated motion reports until another mouse action resets the cell", () => {
		...
		const duplicate = resolveTerminalMouseData(
			"motion",
			"\u001b[<35;8;4M",
			first.nextMotionData,
		);
		...
		const press = resolveTerminalMouseData(
			"press",
			"\u001b[<0;8;4M",
			duplicate.nextMotionData,
		);
	...
	it("clears the motion baseline when application mouse tracking changes", () => {
		expect(
			resolveTerminalMouseTrackingState(true, false, "\u001b[<35;8;4M"),
		).toEqual({
			tracking: false,
			motionData: "",
		});
		expect(
			resolveTerminalMouseTrackingState(false, true, "\u001b[<35;8;4M"),
		).toEqual({
			tracking: true,
			motionData: "",
		});
		expect(
			resolveTerminalMouseTrackingState(true, true, "\u001b[<35;8;4M"),
		).toEqual({
			tracking: true,
			motionData: "\u001b[<35;8;4M",
		});
		expect(
			resolveTerminalMouseTrackingState(false, false, "\u001b[<35;8;4M"),
		).toEqual({
			tracking: false,
			motionData: "\u001b[<35;8;4M",
		});
...
describe("terminal font resolution", () => {
	...
	it("quotes families the canvas font shorthand would otherwise reject", () => {
		expect(
			terminalFontFamily("3270 Nerd Font").startsWith('"3270 Nerd Font", '),
		).toBe(true);
		expect(terminalFontFamily("M+ 1m").startsWith('"M+ 1m", ')).toBe(true);
		expect(
			terminalFontFamily("Cascadia Code, Menlo").startsWith(
				'"Cascadia Code", Menlo, ',
			),
		).toBe(true);
		expect(terminalFontFamily(" , ")).toBe(DEFAULT_TERMINAL_FONT_FAMILY);
...
describe("terminalWheelDeltaRows", () => {
	it("converts line-mode deltas into whole rows", () => {
		const result = terminalWheelDeltaRows(
...
describe("terminal scrollbar", () => {
	it("maps Ghostty's viewport state to a proportional thumb", () => {
		expect(
			terminalScrollbarGeometry({ total: 100, offset: 40, len: 20 }, 200),
		).toEqual({
			thumbHeight: 40,
			thumbTop: 80,
			maxOffset: 80,
		...
		expect(
			terminalScrollbarGeometry({ total: 100, offset: 0, len: 100 }, 200),
		).toBeNull();
	});
