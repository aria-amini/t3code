...
function createDefaultThreadTerminalUiState(): ThreadTerminalUiState {
	return {
		...
		terminalGroups: copyTerminalGroups(
			DEFAULT_THREAD_TERMINAL_UI_STATE.terminalGroups,
		),
	};
}
...
function reconcileThreadTerminalSessionIds(
	...
	return normalizeThreadTerminalUiState({
		...
		terminalGroups,
		activeTerminalGroupId:
			activeGroupIdFromTerminal ?? terminalGroups[0]?.id ?? "",
	});
}
...
export function selectThreadTerminalUiState(
	...
	return (
		terminalUiStateByThreadKey[terminalThreadKey(threadRef)] ??
		...
		(set, get) => {
				...
				set((state) => {
					...
					if (
						nextTerminalUiStateByThreadKey ===
							state.terminalUiStateByThreadKey &&
						nextSuppressedTerminalIdsByThreadKey ===
							state.suppressedTerminalIdsByThreadKey
					) {
						return state;
					}
			...
			return {
				...
				splitTerminal: (threadRef, terminalId) =>
					updateTerminal(
						threadRef,
						(state) => splitThreadTerminal(state, terminalId),
						{
							terminalId,
							suppressed: false,
						},
				...
				splitTerminalVertical: (threadRef, terminalId) =>
					updateTerminal(
						threadRef,
						(state) => splitThreadTerminal(state, terminalId, "vertical"),
						{
							terminalId,
							suppressed: false,
						},
				...
				newTerminal: (threadRef, terminalId) =>
					updateTerminal(
						threadRef,
						(state) => newThreadTerminal(state, terminalId),
						{
							terminalId,
							suppressed: false,
						},
				...
				closeTerminal: (threadRef, terminalId) =>
					updateTerminal(
						threadRef,
						(state) => closeThreadTerminal(state, terminalId),
						{
							terminalId,
							suppressed: true,
						},
				...
				removeTerminalUiState: (threadRef) =>
					set((state) => {
						const threadKey = terminalThreadKey(threadRef);
						const hadTerminalUiState =
							state.terminalUiStateByThreadKey[threadKey] !== undefined;
						const hadSuppressedTerminalIds =
							state.suppressedTerminalIdsByThreadKey[threadKey] !== undefined;
