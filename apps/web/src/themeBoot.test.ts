...
function runBootScript(options: {
	...
	new Function("window", "document", "CSS", bootScript)(
		fakeWindow,
		fakeDocument,
		fakeCss,
	);
...
describe("index.html boot script", () => {
	const parityCases: ReadonlyArray<{
		...
		{
			name: "no stored preference on a dark OS",
			storage: {},
			prefersDark: true,
		},
		{
		...
		},
		{
			...
			storage: {
				...
				[THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "1",
			},
			prefersDark: true,
		},
		{
	...
	it.each(parityCases)(
		"matches the runtime appearance: $name",
		({ storage, prefersDark }) => {
			const boot = runBootScript({ storage, prefersDark });
			expect(boot.isDark).toBe(
				runtimeResolvedAppearance(storage, prefersDark) === "dark",
			);
		},
	);
	...
	it("marks built-in and custom themes on the document element", () => {
		...
		expect(aurora.bootVariables["--boot-background"]).toBe(
			AURORA_DUAL.variants.dark.canvas,
		);
