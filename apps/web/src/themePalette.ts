...
export type ThemeColorOverrides = Readonly<
	Partial<Record<ThemeColorRole, string>>
>;
export type ThemeVariantOverrides = Readonly<
	Partial<Record<ThemeAppearance, ThemeColorOverrides>>
>;
...
function parseStoredTheme(value: unknown): ThemeDefinition | null {
	...
	if (!isThemeLabel(value.label) || !isThemeAppearance(value.appearance))
		return null;
...
function readCustomThemeLibrarySnapshot(): CustomThemeLibrarySnapshot {
	...
	return {
		status: "ready",
		storedThemes: parsed,
		themes: parseStoredThemes(parsed),
	};
}
 ...
 */
export function setEnvironmentThemes(
	themes: ReadonlyArray<ThemeDefinition>,
): boolean {
...
export function canonicalThemePreference(theme: string): string {
	return theme === LEGACY_T3_CHAT_DARK_THEME_ID
		? theme
 ...
 * files.
 */
export function getStandardThemeColors(
	appearance: ThemeAppearance,
): ThemeColors {
	if (appearance === "dark") {
		return (standardDarkThemeColors ??= decodeThemeColors(
			T3_CODE_DARK_THEME_COLORS,
		));
	}
	return (standardLightThemeColors ??= decodeThemeColors(
...
function formatOklchThemeColor(color: ThemeOklch, alpha = 1): string {
	...
	return alpha < 1
		? `oklch(${body} / ${formatThemeColorNumber(alpha, 4)})`
		: `oklch(${body})`;
}
...
export function themeColorToHex(value: string): string | null {
	...
	const parsed = color
		? { rgb: themeOklchToRgb(color.color), alpha: color.alpha }
		: null;
	if (!parsed) return null;
...
function mapThemeOklchToSrgbGamut(color: ThemeOklch): ThemeOklch {
	...
	const steps = Math.max(
		1,
		Math.ceil(
			Math.log2(Math.max(color.C, chromaResolution)) -
...
function standardStatusColors(canvas: ThemeRgbColor): {
	...
	const appearance: ThemeAppearance =
		themeRelativeLuminance(canvas) < 0.179 ? "dark" : "light";
...
export function createVividThemeColors(
	...
	const secondary = surfaceAt(
		dark ? 0.1 : 0.06,
		Math.min(0.09, accent.C * 0.5),
	);
	...
	const accentSurface = surfaceAt(
		dark ? 0.13 : 0.08,
		Math.min(0.11, accent.C * 0.55),
	);
	const accentSurfaceRgb = themeOklchToRgb(accentSurface);
	const messageSurface = surfaceAt(
		dark ? 0.16 : 0.1,
		Math.min(0.13, accent.C * 0.6),
	);
	const messageSurfaceRgb = themeOklchToRgb(messageSurface);
	const codeBackground = surfaceAt(0.035, tintC * 0.8);
	const updateSurface = surfaceAt(
		dark ? 0.14 : 0.09,
		Math.min(0.12, accent.C * 0.55),
	);
	...
	const mutedForeground = themeRgbToThemeColor(
		readableThemeText(mutedRgb, textRgb, 1, 4.6),
	);
	const placeholder = themeRgbToThemeColor(
		readableThemeText(surfaceRaisedRgb, textRgb, 1, 4.6),
	);

	const actionHover: ThemeOklch = {
		...action,
		L: action.L + (dark ? 0.06 : -0.06),
	};
	...
	return {
		...
		toolbarBorder: themeColor(
			surfaceAt(dark ? 0.14 : 0.1, Math.min(0.08, accent.C * 0.4)),
		),
		...
		sidebarMutedForeground: themeRgbToThemeColor(
			standardMutedThemeText(sidebarRgb, textRgb),
		),
		sidebarControlSurface: themeColor(
			surfaceAt(dark ? 0.1 : 0.07, tintC * 1.5),
		),
		sidebarRowHover: themeColor(
			surfaceAt(dark ? 0.08 : 0.06, Math.min(0.08, accent.C * 0.45)),
		),
		sidebarRowActive: themeColor(
			surfaceAt(dark ? 0.12 : 0.09, Math.min(0.1, accent.C * 0.55)),
		),
		sidebarRowSelected: themeColor(
			surfaceAt(dark ? 0.14 : 0.1, Math.min(0.11, accent.C * 0.6)),
		),
		sidebarBorder: themeColor(
			surfaceAt(dark ? 0.17 : 0.12, Math.min(0.08, accent.C * 0.4)),
		),
		...
		terminalSelection: themeColor(
			surfaceAt(dark ? 0.18 : 0.12, Math.min(0.12, accent.C * 0.55)),
		),
...
function readableThemeText(
	...
	for (let index = 0; index < 12; index += 1) {
		...
		const candidate = mixThemeRgbColors(
			foreground,
			background,
			candidateAmount,
		);
...
export function updateThemeColorFamily(
	...
	const normalized = formatOklchThemeColor(
	...
	const foregroundOn = (background: ThemeRgbColor) =>
		colorOf(readableThemeForeground(background));
	...
	const statusColors = () => {
		const surface = mixThemeRgbColors(
			canvas,
			selectedOnCanvas,
	...
	switch (role) {
		...
		case "messageAction": {
			...
			const towardOpposite =
				actionForeground === THEME_LIGHT_FOREGROUND ||
				actionForeground === THEME_WHITE_FOREGROUND
					? THEME_BLACK_FOREGROUND
					: THEME_WHITE_FOREGROUND;
		...
		case "sidebarRowSelected": {
			...
			return {
				...colors,
				sidebarRowHover: colorOf(
					mixThemeRgbColors(sidebar, selectedOnSidebar, 0.5),
				),
				sidebarRowActive: colorOf(
					mixThemeRgbColors(sidebar, selectedOnSidebar, 0.8),
				),
...
export function getThemeDefinition(
	...
	return (
		BUILT_IN_THEME_DEFINITIONS.find(
			(definition) => definition.id === themeId,
		) ??
		...
		environmentThemeDefinitions.find(
			(definition) => definition.id === themeId,
		) ??
		null
	);
 ...
 */
export function singleAppearanceOf(
...
export function getThemeModes(
	theme: ThemeDefinition,
): ReadonlyArray<ThemeAppearance> {
	return (["light", "dark"] as const).filter(
		(mode) => getThemeColorsForMode(theme, mode) !== null,
	);
}
...
function getWritableCustomThemeLibrary(): Extract<
...
function storedThemeHasCollectionId(
	storedTheme: unknown,
	collectionId: string,
): boolean {
	return (
...
export function installCustomTheme(theme: ThemeDefinition): ThemeDefinition {
	...
	if (
		...
		library.storedThemes.some((storedTheme) =>
			storedThemeHasId(storedTheme, theme.id),
		)
	) {
...
export function replaceCustomThemeCollection(
	...
	if (themes.length === 0)
		throw new Error("A theme collection cannot be empty.");

	const validated = themes.map((theme) => parseStoredTheme(theme));
	if (
		validated.some(
			(theme) => theme === null || theme.collection?.id !== collectionId,
		) ||
	...
	const currentCollection = current.filter(
		(theme) => theme.collection?.id === collectionId,
	);
	if (
		options?.expectedCollection &&
		JSON.stringify(currentCollection) !==
			JSON.stringify(options.expectedCollection)
	) {
		throw new Error(
	...
	if (conflictingTheme) {
		throw new Error(
			`A theme named "${conflictingTheme.label}" is already installed.`,
		);
	}
...
export function removeCustomThemes(themeIds: ReadonlyArray<string>): void {
	...
	const library = getWritableCustomThemeLibrary();
	const nextThemes = library.themes.filter(
		(theme) => !removedIds.has(theme.id),
	);
...
export function parseThemeFile(value: unknown): ThemeDefinition {
	...
	if (value.variants !== undefined) {
		...
		for (const [variantAppearance, variantColors] of Object.entries(
			value.variants,
		)) {
...
export function serializeThemeFile(theme: ThemeDefinition): string {
	...
	const file: ThemeFile = {
		...
		...(canonicalTheme.collection
			? { collection: canonicalTheme.collection }
			: {}),
...
export function applyThemeColorPreview(
	...
	for (const [role, value] of Object.entries(colors) as Array<
...
export function resolveThemeAppearance(
	...
	const mode =
		appearanceMode ?? ((followSystem ?? theme === "system") ? "system" : null);
	if (mode === "system") {
		...
		return definition &&
			getThemeColorsForMode(definition, systemAppearance) === null
...
export function resolveDesktopTheme(
	...
	if (mode === "system") {
		...
		const hasLightMode =
			...
			(definition !== null &&
				getThemeColorsForMode(definition, "light") !== null);
		const hasDarkMode =
			halves?.dark !== undefined ||
			(definition !== null &&
				getThemeColorsForMode(definition, "dark") !== null);
		return definition && (!hasLightMode || !hasDarkMode)
			? definition.appearance
			: "system";
	}
	if (mode === "light" || mode === "dark") {
...
export function parseThemeHalves(raw: string | null): ThemeHalves | null {
	...
	try {
		...
		for (const appearance of ["light", "dark"] as const) {
			...
			if (
				definition &&
				getThemeColorsForMode(definition, appearance) !== null
			) {
		...
		return halves.light !== undefined || halves.dark !== undefined
			? halves
			: null;
	} catch {
		return null;
