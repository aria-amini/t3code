...
export function getThreadKeysToDeselectAfterDelete(
	...
	return selectedThreadKeys.filter(
		(key) => deletedThreadKeys.has(key) || !hasThread(key),
	);
}

export const useThreadSelectionStore = create<ThreadSelectionStore>(
	(set, get) => ({
		...
		toggleThread: (threadKey) => {
			set((state) => {
				...
				return {
					...
					anchorThreadKey: next.has(threadKey)
						? threadKey
