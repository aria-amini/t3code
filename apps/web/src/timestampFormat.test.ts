...
describe("formatShortTimestamp", () => {
	...
	it.each([
		...
		"honors %s and the explicit hour-cycle settings",
		async (locale, localTime) => {
			vi.stubGlobal("window", {
				desktopBridge: { getSystemLocale: () => locale },
			});
			vi.resetModules();
			const { formatShortTimestamp: format } =
			...
			expect(format(date, "12-hour").replace(/[  ]/g, " ")).toMatch(
				/^3:44 [ap]m$/i,
			);
...
describe("formatChatTimestampTooltip", () => {
	...
	it.each(["de-DE", "it-IT"])(
		"keeps the English date label in a %s runtime",
		async (locale) => {
			...
			vi.resetModules();
			const { formatChatTimestampTooltip: format } =
				await import("./timestampFormat");
...
describe("formatDayAwareTimestamp", () => {
	...
	it("labels the previous calendar day as yesterday even when under 24h old", () => {
		...
		expect(
			formatDayAwareTimestamp(messageAt, "12-hour", justPastMidnight),
		).toBe(`yesterday at ${time(messageAt)}`);
	});
