...
describe("uiStateStore pure functions", () => {
	it("stores server timestamps without moving visit state backwards", () => {
		...
		const visited = markThreadVisited(
			initialState,
			threadId,
			"2026-02-25T12:30:00.700Z",
		);

		expect(visited.threadLastVisitedAtById[threadId]).toBe(
			"2026-02-25T12:30:00.700Z",
		);
		expect(
			markThreadVisited(visited, threadId, "2026-02-25T12:30:00.000Z"),
		).toBe(visited);
	...
	it("marks a completed thread unread using the server completion timestamp", () => {
		...
		expect(next.threadLastVisitedAtById[threadId]).toBe(
			"2026-02-25T12:29:59.999Z",
		);
	...
	it("resolves project expansion from logical, physical, and legacy preference keys", () => {
		...
		expect(
			resolveProjectExpanded({ logical: false, [physicalKey]: true }, [
				"logical",
			]),
		).toBe(false);
		expect(
			resolveProjectExpanded({ [physicalKey]: false }, [
				"new-logical",
				physicalKey,
			]),
		).toBe(false);
		expect(
			resolveProjectExpanded({ [legacyKey]: false }, [
				"new-logical",
				legacyKey,
			]),
		).toBe(false);
	...
	it("stores the endpoint preference by stable key", () => {
		const next = setDefaultAdvertisedEndpointKey(
			makeUiState(),
			"desktop-core:lan:http",
		);

		expect(next.defaultAdvertisedEndpointKey).toBe("desktop-core:lan:http");
		expect(setDefaultAdvertisedEndpointKey(next, "desktop-core:lan:http")).toBe(
			next,
		);
	...
	it("stores the sidebar project scope and resets it to all projects", () => {
		const scoped = setSidebarProjectScopeKey(
			makeUiState(),
		...
		expect(
			setSidebarProjectScopeKey(scoped, "github.com/pingdotgg/t3code"),
		).toBe(scoped);
		expect(
			setSidebarProjectScopeKey(scoped, null).sidebarProjectScopeKey,
		).toBeNull();
		expect(
			setSidebarProjectScopeKey(scoped, "").sidebarProjectScopeKey,
		).toBeNull();
	});
});
...
describe("parsePersistedState", () => {
	...
	it.each([undefined, 1])(
		"ignores changed-file expansion version %s",
		(version) => {
			const parsed = parsePersistedState({
				...(version === undefined
					? {}
					: { threadChangedFilesExpansionVersion: version }),
				threadChangedFilesExpandedById: {
					"environment:thread-1": {
						"turn-1": false,
					},
	...
	it("migrates legacy CWD project preferences into local alias keys", () => {
		...
		expect(
			resolveProjectExpanded(parsed.projectExpandedById, [projectAKey]),
		).toBe(true);
		expect(
			resolveProjectExpanded(parsed.projectExpandedById, [projectBKey]),
		).toBe(false);
		expect(
			resolveProjectExpanded(parsed.projectExpandedById, ["unknown"]),
		).toBe(true);
	});
...
describe("uiStateStore persistence", () => {
	...
	it("restores the sidebar project scope across reloads", () => {
		persistState(
			makeUiState({ sidebarProjectScopeKey: "github.com/pingdotgg/t3code" }),
		);
	...
	it("drops the temporary expanded-only migration fallback when rewriting state", () => {
		...
		expect(
			resolveProjectExpanded(persisted.projectExpandedById ?? {}, ["unknown"]),
		).toBe(true);
	});
});
