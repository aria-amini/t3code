...
export interface UiState
	extends UiProjectState, UiThreadState, UiEndpointState {}
...
export function parsePersistedState(parsed: PersistedUiState): UiState {
	const projectExpandedById =
		parsed.projectExpandedById === undefined
			? (() => {
					...
					const collapsedProjectCwds = sanitizeStringArray(
						parsed.collapsedProjectCwds,
					);
					const expandedProjectCwds = sanitizeStringArray(
					...
					if (
						!Array.isArray(parsed.collapsedProjectCwds) &&
						expandedProjectCwds.length > 0
					) {
	...
	const projectOrder =
		parsed.projectOrder === undefined
			? sanitizeStringArray(parsed.projectOrderCwds).map(
	...
	return {
		projectExpandedById,
		projectOrder,
		threadLastVisitedAtById: sanitizeTimestampRecord(
		...
		threadChangedFilesExpandedById:
			...
			THREAD_CHANGED_FILES_EXPANSION_VERSION
				...
				: {},
		defaultAdvertisedEndpointKey: sanitizeOptionalKey(
...
export function markThreadVisited(
	...
	const previousVisitedAtMs = previousVisitedAt
		? Date.parse(previousVisitedAt)
		: NaN;
	if (
...
export function setThreadChangedFilesExpanded(
	...
	const currentThreadState =
		state.threadChangedFilesExpandedById[threadId] ?? {};
...
export function setDefaultAdvertisedEndpointKey(
	state: UiState,
	key: string | null,
): UiState {
...
export function setSidebarProjectScopeKey(
	state: UiState,
...
export function setProjectExpanded(
	...
	const nextEntries = ids.filter(
		(projectId) => state.projectExpandedById[projectId] !== expanded,
	);
...
export function reorderProjects(
	...
	const originalTargetIndex = currentProjectOrder.findIndex((id) =>
		targetSet.has(id),
	);
	...
	const insertIndex =
		originalTargetIndex - Math.max(0, draggedBeforeTarget - 1);
...
export const useUiStateStore = create<UiStateStore>((set) => ({
	...
	setThreadChangedFilesExpanded: (threadId, turnId, expanded) =>
		set((state) =>
			setThreadChangedFilesExpanded(state, threadId, turnId, expanded),
		),
