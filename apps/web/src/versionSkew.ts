...
const VERSION_MISMATCH_DISMISSALS_STORAGE_KEY =
...
export function isServerUpdateFailureDismissed(
...
export function supportsServerUpdateThreadContinuation(
	...
	return (
		serverConfig?.environment.capabilities.serverUpdateThreadContinuation ===
		true
	);
}
...
function writeVersionMismatchDismissals(
	document: VersionMismatchDismissals,
): void {
	try {
		setLocalStorageItem(
			VERSION_MISMATCH_DISMISSALS_STORAGE_KEY,
