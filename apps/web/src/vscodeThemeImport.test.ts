...
describe("VS Code theme import", () => {
	...
	it("keeps a fallback placeholder dimmer than entered text", () => {
		...
		expect(
			contrastRatio(theme.colors.placeholder, theme.colors.canvas),
		).toBeGreaterThanOrEqual(4.5);
		expect(
			contrastRatio(theme.colors.placeholder, theme.colors.canvas),
		).toBeLessThan(contrastRatio(theme.colors.text, theme.colors.canvas));
	});
	...
	it("stays readable when the file replaces a surface but not its foreground", () => {
		...
		expect(
			contrastRatio(
				theme.colors.terminalForeground,
	...
	it("pairs light and dark files from one family into dual-mode themes", () => {
		...
		expect(asHex(getThemeColorsForMode(github, "dark")!.canvas)).toBe(
			"#101014",
		);
	...
	it("does not guess when a family is ambiguous", () => {
		...
		expect(themes.map((theme) => theme.label).sort()).toEqual([
			"Solar",
			"Solar Dark Soft",
		]);
	});
	...
	it("tells same-named variants apart by their file names", () => {
		...
		expect(numbered.map((theme) => theme.label)).toEqual([
			"Dracula",
			"Dracula 2",
		]);
	});
	...
	it("keeps a pair whose stripped name is reserved as two single themes", () => {
		...
		const themes = pairVsCodeThemes([
			make("grove-light", "light"),
			make("grove-dark", "dark"),
		]);
		expect(themes.map((theme) => theme.label).sort()).toEqual([
			"Grove Dark",
			"Grove Light",
