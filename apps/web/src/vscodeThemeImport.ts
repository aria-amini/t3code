...
function parseColorFunction(value: string): VsCodeRgba | null {
	...
	const srgb = [
		...
		-0.0196375547643 * linearRed -
			0.0786360655012 * linearGreen +
			1.0982736202656 * linearBlue,
...
function hexToRgb(value: string): VsCodeRgb {
	return (
		parseVsCodeColor(themeColorToHex(value) ?? value) ?? {
			r: 0,
			g: 0,
			b: 0,
			a: 1,
...
export function isVsCodeThemeFile(value: unknown): boolean {
	...
	const hasWorkbenchColors =
		...
		Object.keys(value.colors).some((key) => key.includes("."));
...
export function parseVsCodeThemeFile(value: unknown): ThemeDefinition {
	...
	const sidebarHex =
		solidOver(canvas, "sideBar.background", "activityBar.background") ??
		derived.sidebar;
	const sidebar = hexToRgb(sidebarHex);
	const terminalHex =
		solidOver(canvas, "terminal.background", "panel.background") ??
		derived.terminalBackground;
	...
	const readableOn = (
		...
		const isReadable = (candidate: string) =>
			contrastRatio(hexToRgb(candidate), surfaceRgb) >= 4.5;
	...
	const overrides: Partial<Record<ThemeColorRole, string>> = {
		...
		surfaceRaised:
			solidOver(canvas, "editorWidget.background", "dropdown.background") ??
		...
		input:
			solidOver(canvas, "input.border", "dropdown.border") ?? derived.input,
		...
		codeBackground:
			solidOver(canvas, "textCodeBlock.background") ?? derived.codeBackground,
		sidebar: sidebarHex,
		sidebarForeground: readableOn(
			sidebarHex,
			derived.sidebarForeground,
			"sideBar.foreground",
		),
		sidebarBorder:
			solidOver(sidebar, "sideBar.border") ?? derived.sidebarBorder,
		sidebarRowHover:
			solidOver(sidebar, "list.hoverBackground") ?? derived.sidebarRowHover,
		sidebarRowActive:
			solidOver(
				sidebar,
		...
		sidebarRowSelected:
			solidOver(sidebar, "list.activeSelectionBackground") ??
			derived.sidebarRowSelected,
		terminalBackground: terminalHex,
		terminalForeground: readableOn(
			terminalHex,
			derived.terminalForeground,
			"terminal.foreground",
		),
		terminalCursor:
			solidOver(
		...
		terminalScrollbar:
			solidOver(terminal, "scrollbarSlider.background") ??
			derived.terminalScrollbar,
	};
	if (accentHex) {
