...
const authenticatedAuth: Context.Service.Shape<
	typeof EnvironmentAuthenticatedAuth
> = (httpEffect) =>
	httpEffect.pipe(
...
export async function installEnvironmentHttpTest(
	scenario: EnvironmentHttpTestScenario,
) {
				...
				HttpApiBuilder.group(EnvironmentHttpApi, "metadata", (handlers) =>
					handlers.handle(
						...
						Effect.fn("test.environment.metadata.descriptor")(function* () {
							...
							return yield* (
								scenario.descriptor?.() ??
									unexpectedEndpoint("metadata.descriptor")
							);
						}),
					),
				),
					...
					handlers
						.handle(
							...
							Effect.fn("test.environment.auth.session")(function* () {
								calls.session += 1;
								return yield* (
									scenario.session?.() ?? unexpectedEndpoint("auth.session")
								);
							}),
						)
						.handle(
							"browserSession",
							Effect.fn("test.environment.auth.browserSession")(function* ({
								payload,
							}) {
								calls.browserSession.push(payload);
								return yield* (
									scenario.browserSession?.(payload) ??
										unexpectedEndpoint("auth.browserSession")
								);
							}),
						)
						.handle("token", () => unexpectedEndpoint("auth.token"))
						.handle("webSocketTicket", () =>
							unexpectedEndpoint("auth.webSocketTicket"),
						)
						.handle(
							"pairingCredential",
							Effect.fn("test.environment.auth.pairingCredential")(function* ({
								payload,
							}) {
						...
						.handle("pairingLinks", () =>
							unexpectedEndpoint("auth.pairingLinks"),
						)
						.handle("revokePairingLink", () =>
							unexpectedEndpoint("auth.revokePairingLink"),
						)
						.handle("clients", () => unexpectedEndpoint("auth.clients"))
						.handle("revokeClient", () =>
							unexpectedEndpoint("auth.revokeClient"),
						)
						.handle("revokeOtherClients", () =>
							unexpectedEndpoint("auth.revokeOtherClients"),
						),
				),
			]),
	...
	const runtime = ManagedRuntime.make(
		Layer.succeed(PrimaryEnvironmentHttpClient, client),
	);
