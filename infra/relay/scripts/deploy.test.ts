...
describe("hasDeployChanges", () => {
	it("detects resource, binding, and deletion changes", () => {
		expect(hasDeployChanges({ resources: {}, deletions: {} } as never)).toBe(
			false,
		);
		expect(
			hasDeployChanges({
	...
	it.effect("uses an artifact instead of a masked cross-job token output", () =>
		Effect.gen(function* () {
			...
			expect(workflow).not.toContain(
				"needs.relay_public_config.outputs.client_tracing_token",
			);
...
describe("publicConfigFromOutput", () => {
	...
	it("rejects incomplete stack output", () => {
		expect(
			publicConfigFromOutput({ url: "https://relay.example.test" }),
		).toBeNull();
	});
});
