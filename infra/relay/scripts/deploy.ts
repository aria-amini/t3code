...
export class RelayDeployError extends Schema.TaggedError<RelayDeployError>()(
	"RelayDeployError",
	{
...
export function reconcileRootEnvPublicConfig(
	contents: string,
...
export function reconcileRootEnvRelayUrl(
	contents: string,
	relayUrl: string,
): string {
...
export function hasDeployChanges(plan: Plan.Plan): boolean {
	return (
		...
		Object.values(plan.resources).some(
			(node) =>
				node.action !== "noop" ||
				node.bindings.some((binding) => binding.action !== "noop"),
		)
	);
}
...
export function serializeRelayClientTracingEnvironment(
	config: RelayPublicConfig,
): string {
...
const loadDeployConfigProvider = Effect.fn("relay.deploy.loadConfigProvider")(
	function* (envFileOverride: Option.Option<string>) {
		const path = yield* Path.Path;
		const root = yield* relayRoot;

		if (Option.isSome(envFileOverride)) {
			return yield* ConfigProvider.fromDotEnv({
				path: path.resolve(root, envFileOverride.value),
			});
		}
...
const relayDeployStage = Config.nonEmptyString("stage").pipe(
	...
	Config.map(
		Option.getOrElse(
			() => `dev_${process.env.USER ?? process.env.USERNAME ?? "unknown"}`,
		),
	),
);
...
const reconcileRootEnv = Effect.fn("relay.deploy.reconcileRootEnv")(function* (
	...
	const contents = (yield* fs.exists(rootEnvPath))
		? yield* fs.readFileString(rootEnvPath)
		: "";

	yield* fs.writeFileString(
		rootEnvPath,
		reconcileRootEnvPublicConfig(contents, config),
	);
	yield* Console.log(
		`Updated ${rootEnvPath} with relay public client configuration`,
	);
});

const writeGithubOutput = Effect.fn("relay.deploy.writeGithubOutput")(
	function* (outcome: RelayDeployOutcome) {
		const fs = yield* FileSystem.FileSystem;
...
const writeGithubEnvFile = Effect.fn("relay.deploy.writeGithubEnvFile")(
	function* (outcome: RelayDeployOutcome, outputPath: string, stage: string) {
		...
		const fs = yield* FileSystem.FileSystem;
		yield* Console.log(
			`::add-mask::${outcome.publicConfig.value.clientTracingToken}`,
		);
		yield* fs.writeFileString(
			outputPath,
...
export function publicConfigFromOutput(
	output: unknown,
): RelayPublicConfig | null {
	const values = relayPublicConfigValues(output);
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (
	stage: string,
) {
...
const runRelayDeploy = Effect.fn("relay.deploy.run")(
	function* (
		...
		const output = yield* Apply.apply(plan).pipe(
			Effect.provide(stack.services),
		);
		const publicConfig = publicConfigFromOutput(output);
		if (publicConfig === null) {
			...
			Effect.provide(
				Layer.mergeAll(
					Layer.effect(
						AlchemyContext,
						AlchemyContext.pipe(
							Effect.map((context) => ({ ...context, adopt: options.adopt })),
						),
					),
...
export const deploy = Effect.fn("relay.deploy")(function* (
	options: RelayDeployOptions,
) {
...
export const relayDeployCommand = Command.make(
	"relay-deploy",
	{
		...
		adopt: Flag.boolean("adopt").pipe(
			Flag.withDescription(
				"Adopt pre-existing cloud resources that conflict with this stack.",
			),
			Flag.withDefault(false),
		),
		githubOutput: Flag.boolean("github-output").pipe(
			Flag.withDescription(
				"Append relay deployment metadata to GITHUB_OUTPUT.",
			),
			Flag.withDefault(false),
		),
		...
		readState: Flag.boolean("read-state").pipe(
			Flag.withDescription(
				"Read the deployed stack output without planning or applying changes.",
			),
			Flag.withDefault(false),
		),
	},
