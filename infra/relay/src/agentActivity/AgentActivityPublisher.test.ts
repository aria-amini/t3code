...
describe("AgentActivityPublisher", () => {
	it.effect(
		"replays the latest aggregate when a Live Activity token registers",
		() => {
			...
			return Effect.gen(function* () {
				const result = yield* Effect.gen(function* () {
					const publisher =
						yield* AgentActivityPublisher.AgentActivityPublisher;
							...
							Layer.provide(
								Layer.mergeAll(
									...
									Layer.succeed(
										...
										makeLiveActivities({
											listTargets: () =>
												Effect.succeed([registeredTarget, target("device-2")]),
										}),
									),
									Layer.succeed(
	...
	it.effect(
		"publishes listed targets through the APNs delivery service",
		() => {
			...
			const upserts: Array<
				Parameters<AgentActivityRows.AgentActivityRows["Service"]["upsert"]>[0]
			> = [];
							...
							Layer.provide(
								Layer.mergeAll(
									...
									Layer.succeed(
										...
										makeLiveActivities({
											listTargets: () =>
												Effect.succeed([firstTarget, secondTarget]),
										}),
									),
									Layer.succeed(
										ApnsDeliveries.ApnsDeliveries,
										makeApnsDeliveries({
											sendForTarget: (input) =>
												Effect.sync(() => {
													...
													return input.target.device_id === "device-1"
														? deliveryResult
														: null;
												}),
										}),
	...
	it.effect(
		"ends the last remote Live Activity with a terminal content state",
		() => {
	...
	it.effect(
		"queues push notifications for notification-only environment links",
		() => {
			...
			const pushAggregates: Array<
				Parameters<
					ApnsDeliveries.ApnsDeliveries["Service"]["sendPushNotificationForTarget"]
				>[0]
			> = [];
								...
								Layer.mergeAll(
									Layer.succeed(
										...
										makeAgentActivityRows({
											listForUser: () =>
												Effect.die(
													"Notification-only delivery must not read rows",
												),
										}),
									),
									Layer.succeed(
...
describe("isExpiredAgentActivityState", () => {
	...
	it("expires running rows after two hours without an update", () => {
		expect(
			AgentActivityPublisher.isExpiredAgentActivityState(state, 2 * hourMs - 1),
		).toBe(false);
		expect(
			AgentActivityPublisher.isExpiredAgentActivityState(state, 2 * hourMs + 1),
		).toBe(true);
	...
	it("keeps waiting rows for a day", () => {
		...
		expect(
			AgentActivityPublisher.isExpiredAgentActivityState(waiting, 23 * hourMs),
		).toBe(false);
		expect(
			AgentActivityPublisher.isExpiredAgentActivityState(waiting, 25 * hourMs),
		).toBe(true);
	});

	it("treats rows with unparseable timestamps as expired", () => {
		expect(
			AgentActivityPublisher.isExpiredAgentActivityState(
...
describe("makeAggregateState", () => {
	...
	it("keeps showing recently finished work when nothing is active", () => {
		...
		expect(
			AgentActivityPublisher.makeAggregateState({
				activeStates: [
					{ ...lingeringCompleted, updatedAt: "1970-01-01T00:44:00.000Z" },
				],
				terminalState: null,
				nowMs: hourMs,
			}),
