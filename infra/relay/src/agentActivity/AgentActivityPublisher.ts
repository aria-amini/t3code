...
export const make = Effect.gen(function* () {
	...
	const publishForDeliveryUser = Effect.fnUntraced(function* (input: {
		...
		const liveActivityAggregate = input.deliveryUser.liveActivitiesEnabled
			? makeAggregateState({
					activeStates,
					terminalState:
						input.state && isTerminalPhase(input.state) ? input.state : null,
					nowMs: input.nowMs,
				})
			: null;
		const notificationOnlyAggregate =
			...
			input.state !== null
				...
				: null;
		const targets = yield* liveActivities.listTargets({
	...
	return AgentActivityPublisher.of({
		replayForLiveActivityRegistration: Effect.fn(
			...
			const target =
				targets.find((row) => row.device_id === input.deviceId) ?? null;
			if (target === null) {
				return null;
			}
		...
		publish: Effect.fn("relay.agent_activity_publisher.publish")(
			function* (input) {
				yield* Effect.annotateCurrentSpan({
...
function isRecentTerminalState(
	state: RelayAgentActivityState,
	nowMs: number,
): boolean {
...
export function makeAggregateState(input: {
	...
	const updatedAt = [...activeStates, ...recentTerminalStates].reduce(
		(latest, state) =>
