...
describe("AgentActivityRows", () => {
	it.effect(
		"validates database JSON objects and ignores invalid activity rows",
		() => {
			const queryRows = [
				{ stateJson: null },
	...
	it.effect("preserves activity context on persistence failures", () => {
		...
		return Effect.gen(function* () {
			...
			Effect.provide(
				AgentActivityRows.layer.pipe(
					Layer.provide(Layer.succeed(RelayDb.RelayDb, failingDb)),
				),
			),
		);
	});
