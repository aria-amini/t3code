...
const decodeRelayAgentActivityState = Schema.decodeUnknownOption(
	RelayAgentActivityStateSchema,
);
...
export const make = Effect.gen(function* () {
	...
	return AgentActivityRows.of({
		upsert: Effect.fn("relay.agent_activity_rows.upsert")(function* (input) {
			...
			const stateJson = yield* encodeRelayAgentActivityStateJson(
				input.state,
			).pipe(
		...
		pruneTerminal: Effect.fn("relay.agent_activity_rows.prune_terminal")(
			function* (input) {
				yield* Effect.annotateCurrentSpan({
		...
		listForUser: Effect.fn("relay.agent_activity_rows.list_for_user")(
			function* (input) {
				return yield* db
					...
					.pipe(
						Effect.map((rows) =>
							rows.flatMap((row) =>
								Option.toArray(decodeRelayAgentActivityState(row.stateJson)),
							),
						),
						Effect.mapError(
							(cause) =>
		...
		getForUserThread: Effect.fn(
			"relay.agent_activity_rows.get_for_user_thread",
		)(function* (input) {
			return yield* db
				.select({ stateJson: relayAgentActivityRows.stateJson })
				.from(relayAgentActivityRows)
