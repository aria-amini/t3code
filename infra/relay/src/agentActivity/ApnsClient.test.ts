...
describe("ApnsClient", () => {
	...
	it.effect(
		"requests an update push token when remotely starting a Live Activity",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"builds a high-priority alerting update payload when an alert is attached",
		() =>
			Effect.gen(function* () {
				const apns = yield* ApnsClient.ApnsClient;
				const request = apns.makeLiveActivityRequest({
	...
	it.effect("preserves APNs request context and the HTTP cause", () => {
		...
		return Effect.gen(function* () {
			...
			expect(error).toMatchObject({
				...
				status: null,
				message:
					"APNs push-notification request failed during send in sandbox.",
			});
			expect(error.cause).toBeInstanceOf(HttpClientError.HttpClientError);
			expect(
				(error.cause as HttpClientError.HttpClientError).reason,
			).toMatchObject({
				_tag: "TransportError",
				cause: httpCause,
			});
	...
	it.effect(
		"reuses the signed provider JWT across pushes within the reuse window",
		() => {
			...
			const layer = ApnsClient.layer.pipe(
				Layer.provide(ApnsProviderTokens.layer),
				Layer.provide(
					Layer.succeed(HttpClient.HttpClient, capturingHttpClient),
				),
			);
	...
	for (const requestKind of ["live-activity", "push-notification"] as const) {
		for (const stage of ["send", "read-response"] as const) {
			it.effect(
				`aborts a stalled ${requestKind} ${stage} after ten seconds`,
				() =>
					Effect.gen(function* () {
						const started = yield* Deferred.make<void>();
						const signals: AbortSignal[] = [];
						const stalledHttpClient = HttpClient.make(
							(request, _url, signal) => {
								signals.push(signal);
								const stall = Deferred.succeed(started, undefined).pipe(
						...
						const apns = yield* ApnsClient.ApnsClient.pipe(
