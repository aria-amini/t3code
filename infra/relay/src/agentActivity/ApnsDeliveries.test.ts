...
function makeLayer(input: {
	...
	readonly sourceJobClaims?: ReadonlyMap<
		string,
		DeliveryAttempts.DeliverySourceJobClaimResult
	>;
	...
	readonly invalidatedTokens?: Array<
		Parameters<
			LiveActivities.LiveActivities["Service"]["invalidateDeliveryToken"]
		>[0]
	>;
	...
	return ApnsDeliveries.layer.pipe(
		...
		Layer.provide(
			ApnsDeliveryQueue.layer.pipe(Layer.provide(NodeCryptoLayer.layer)),
		),
		Layer.provide(
			Layer.mergeAll(
				Layer.succeed(AgentActivityRows.AgentActivityRows, {
					...
					getForUserThread: ({ environmentId, threadId }) =>
						Effect.succeed(
							(
								input.currentActivityStates ??
								input.activityStates ?? [state]
							).find(
								(current) =>
...
describe("ApnsDeliveries", () => {
	it.effect(
		"never starts an activity remotely when no update token is registered",
		() => {
			...
			const queuedStarts: Array<
				Parameters<
					LiveActivities.LiveActivities["Service"]["markStartQueued"]
				>[0]
			> = [];
			...
			return Effect.gen(function* () {
				...
				Effect.provide(makeLayer({ attempts, queuedJobs, queuedStarts })),
			);
		},
	);
	...
	it.effect(
		"does not queue a remote start when Live Activities are disabled",
		() => {
			const attempts: Array<DeliveryAttempts.DeliveryAttemptInput> = [];
			const queuedJobs: Array<SignedApnsDeliveryJob> = [];
	...
	it.effect(
		"does not queue a duplicate remote start while a start is already queued",
		() => {
			const attempts: Array<DeliveryAttempts.DeliveryAttemptInput> = [];
			const queuedJobs: Array<SignedApnsDeliveryJob> = [];
	...
	it.effect("queues bounded Live Activity aggregate payloads", () => {
		...
		return Effect.gen(function* () {
			...
			expect(
				payloadAggregate?.activities[0]?.projectTitle.length,
			).toBeLessThanOrEqual(120);
			expect(
				payloadAggregate?.activities[0]?.status.length,
	...
	it.effect(
		"sends signed jobs to the device's APNs environment and bundle topic",
		() => {
			const attempts: Array<DeliveryAttempts.DeliveryAttemptInput> = [];
			...
			return Effect.gen(function* () {
				...
				expect(requests[0]?.url).toBe(
					"https://api.sandbox.push.apple.com/3/device/activity-token",
				);
	...
	it.effect(
		"queues an end for an active Live Activity when Live Activities are disabled",
		() => {
			const attempts: Array<DeliveryAttempts.DeliveryAttemptInput> = [];
			const queuedJobs: Array<SignedApnsDeliveryJob> = [];
	...
	it.effect(
		"does not queue alert pushes when notification permission is disabled",
		() => {
			const attempts: Array<DeliveryAttempts.DeliveryAttemptInput> = [];
			const queuedJobs: Array<SignedApnsDeliveryJob> = [];
			const inputAggregate: RelayAgentActivityAggregateState = {
	...
	it.effect(
		"does not queue a push notification when a thread starts working",
		() => {
			const attempts: Array<DeliveryAttempts.DeliveryAttemptInput> = [];
			const queuedJobs: Array<SignedApnsDeliveryJob> = [];
	...
	it.effect("preserves the schema cause for invalid queue payloads", () => {
		...
		return Effect.gen(function* () {
			...
			const error = yield* Effect.flip(
				deliveries.processSignedJob({ invalid: true }),
			);
	...
	it.effect(
		"skips a queued start when the user no longer has live work",
		() => {
			const attempts: Array<DeliveryAttempts.DeliveryAttemptInput> = [];
			const clearedStarts: Array<
				Parameters<
					LiveActivities.LiveActivities["Service"]["clearStartQueued"]
				>[0]
			> = [];
			const payload = makeApnsDeliveryJobPayload({
			...
			return Effect.gen(function* () {
				...
				expect(clearedStarts).toMatchObject([
					{ userId: target.user_id, deviceId: target.device_id },
				]);
			}).pipe(
				Effect.provide(
					makeLayer({ attempts, clearedStarts, activityStates: [] }),
				),
			);
		},
	);
	...
	it.effect("processes signed jobs through APNs and records attempts", () => {
		...
		return Effect.gen(function* () {
			...
			expect(
				(error.cause as ApnsClient.ApnsJwtSigningError).cause,
			).toBeDefined();
		}).pipe(
			Effect.provide(
				Layer.mergeAll(
	...
	it.effect(
		"processes signed push notification jobs through APNs and records attempts",
		() => {
			const attempts: Array<DeliveryAttempts.DeliveryAttemptInput> = [];
			const payload = makeApnsDeliveryJobPayload({
				kind: "push_notification",
	...
	it.effect(
		"skips stale signed Live Activity jobs when the registered token changed",
		() => {
			const attempts: Array<DeliveryAttempts.DeliveryAttemptInput> = [];
			let executeCount = 0;
			const payload = makeApnsDeliveryJobPayload({
	...
	it.effect(
		"skips stale signed push notification jobs when the device token changed",
		() => {
			const attempts: Array<DeliveryAttempts.DeliveryAttemptInput> = [];
			let executeCount = 0;
			const payload = makeApnsDeliveryJobPayload({
	...
	it.effect(
		"skips a queued Done Live Activity update after the thread resumes working",
		() => {
			const attempts: Array<DeliveryAttempts.DeliveryAttemptInput> = [];
			let executeCount = 0;
	...
	it.effect(
		"skips a queued Done notification after the thread resumes working",
		() => {
			const attempts: Array<DeliveryAttempts.DeliveryAttemptInput> = [];
			let executeCount = 0;
			const completedAt = "1970-01-01T00:00:01.000Z";
	...
	it.effect(
		"retries signed queue jobs that are already claimed but not completed",
		() => {
			const attempts: Array<DeliveryAttempts.DeliveryAttemptInput> = [];
			let executeCount = 0;
			const payload = makeApnsDeliveryJobPayload({
	...
	it.effect(
		"invalidates dead device push tokens after permanent APNs alert failures",
		() => {
			const attempts: Array<DeliveryAttempts.DeliveryAttemptInput> = [];
	...
	it.effect(
		"invalidates dead push-to-start tokens after permanent APNs start failures",
		() => {
			const attempts: Array<DeliveryAttempts.DeliveryAttemptInput> = [];
			const invalidatedTokens: Array<
				Parameters<
			...
			return Effect.gen(function* () {
				...
				Effect.provide(
					makeLayer({
						...
						invalidatedTokens,
						activityStates: [
							{ ...state, updatedAt: "9999-01-01T00:00:00.000Z" },
						],
						config: signingConfig,
						execute,
					}),
	...
	it.effect(
		"invalidates dead Live Activity tokens after APNs unregisters them",
		() => {
			const attempts: Array<DeliveryAttempts.DeliveryAttemptInput> = [];
			const invalidatedTokens: Array<
				Parameters<
