...
type ChosenDelivery =
	...
	| ChosenPushNotificationDelivery;
...
export const isApnsDeliveryTransportError = Schema.is(
	ApnsDeliveryTransportError,
);
...
const decodeSignedApnsDeliveryJob = Schema.decodeUnknownEffect(
	SignedApnsDeliveryJob,
);
...
function aggregateNeedsAttention(
...
function isFreshTerminalRow(
	...
	return (
		updatedAtMs !== null &&
		nowMs - updatedAtMs <= TERMINAL_NOTIFICATION_FRESHNESS_MS
	);
}
...
export function alertForNewlyTerminal(input: {
	...
	const newlyTerminal = newlyTerminalRows(
...
function shouldUpdateLiveActivity(input: {
	...
	if (
		JSON.stringify(input.previousAggregate) ===
		JSON.stringify(input.nextAggregate)
	) {
		return false;
	}
	...
	if (
		newlyTerminalRows(input.previousAggregate, input.nextAggregate).length > 0
	) {
		return true;
	}
...
function notificationForAggregate(input: {
	...
	if (activity.phase === "completed" || activity.phase === "failed") {
		...
		if (
			updatedAtMs === null ||
			input.nowMs - updatedAtMs > TERMINAL_NOTIFICATION_FRESHNESS_MS
		) {
			return null;
		}
	}
	const enabled =
		(activity.phase === "waiting_for_approval" &&
...
function chooseLiveActivityDelivery(input: {
	...
	if (input.aggregate === null) {
		...
		if (
			armedAtMs !== null &&
			input.nowMs - armedAtMs < FRESHLY_ARMED_GRACE_MS
		) {
			return null;
		}
		return {
...
function deliveryEvent(
	kind: LiveActivityDeliveryKind,
): Apns.ApnsLiveActivityEvent {
	switch (kind) {
...
const recoverApnsDeliveryTransportError = (
	...
	return Effect.logError(error.message).pipe(
		Effect.annotateLogs({
			...
			...(error.requestStage === null
				? {}
				: { "error.request_stage": error.requestStage }),
			...(error.stack === undefined ? {} : { "error.stack": error.stack }),
			"relay.mobile.device_id": error.deviceId,
			"relay.delivery.kind": error.kind,
			...(error.sourceJobId === null
				? {}
				: { "relay.delivery.job_id": error.sourceJobId }),
		}),
		Effect.as({
			ok: false,
			status: 0,
...
export const make = Effect.gen(function* () {
	...
	const stateIdentityIsCurrent = Effect.fnUntraced(function* (input: {
		...
		return yield* activityRows
			...
			.pipe(
				...
				Effect.catchCause((cause) =>
					Effect.logWarning(
						"agent-activity state recheck failed; allowing queued delivery",
						{
							cause,
							environmentId: input.environmentId,
							threadId: input.threadId,
		...
		return yield* activityRows.listForUser({ userId: input.userId }).pipe(
			Effect.map((currentStates) => {
				...
				return input.aggregate.activities.every((row) => {
					const current = currentByThread.get(
			...
			Effect.catchCause((cause) =>
				Effect.logWarning(
					"agent-activity aggregate recheck failed; allowing queued delivery",
					{
						cause,
						userId: input.userId,
					},
	...
	const isCurrentSignedJobToken = Effect.fnUntraced(function* (input: {
		...
		return yield* liveActivities
			.listTargets({ userId: input.target.user_id })
			.pipe(
				Effect.map((targets) => {
					const currentTarget = targets.find(
						(row) => row.device_id === input.target.device_id,
					);
					return (
	...
	const sendLiveActivity: ApnsDeliveries["Service"]["sendLiveActivity"] =
		Effect.fn("relay.apns_deliveries.send_live_activity")(function* (input) {
			...
			const aggregate =
				input.aggregate === null
					? null
					: sanitizeAgentActivityAggregateState(input.aggregate);
	...
	const sendPushNotification: ApnsDeliveries["Service"]["sendPushNotification"] =
		Effect.fn("relay.apns_deliveries.send_push_notification")(
			function* (input) {
				yield* Effect.annotateCurrentSpan({
					"relay.mobile.device_id": input.target.device_id,
	...
	const processSignedJob: ApnsDeliveries["Service"]["processSignedJob"] =
		Effect.fn("relay.apns_deliveries.process_signed_job")(function* (body) {
