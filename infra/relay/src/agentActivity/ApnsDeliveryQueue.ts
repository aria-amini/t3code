	...
	ApnsDeliveryQueue,
	{
		...
		readonly enqueuePushNotification: (input: {
			...
			readonly notification: NonNullable<
				ApnsDeliveryJobPayload["notification"]
			>;
		}) => Effect.Effect<RelayDeliveryResult, ApnsDeliveryQueueError>;
	}
...
export const make = Effect.gen(function* () {
	...
	return ApnsDeliveryQueue.of({
		enqueueLiveActivity: Effect.fn(
			"relay.apns_delivery_queue.enqueue_live_activity",
		)(function* (input) {
		...
		enqueuePushNotification: Effect.fn(
			"relay.apns_delivery_queue.enqueue_push_notification",
		)(function* (input) {
			yield* Effect.annotateCurrentSpan({
				"relay.mobile.device_id": input.deviceId,
