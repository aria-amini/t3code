...
describe("ApnsProviderTokens", () => {
	...
	it.effect("rolls to a new token at the window boundary", () => {
		...
		return Effect.gen(function* () {
			...
			const next = yield* tokens.getJwt({
				...signingInput,
				issuedAtUnixSeconds: WINDOW * 2,
			});
	...
	it.effect(
		"serves repeat pushes from the isolate cache without re-signing",
		() => {
			ApnsProviderTokens.__resetApnsProviderTokenCacheForTest();
			return Effect.gen(function* () {
				const tokens = yield* ApnsProviderTokens.ApnsProviderTokens;
				...
				const again = yield* tokens.getJwt({
					...signingInput,
					issuedAtUnixSeconds: WINDOW + 500,
				});
