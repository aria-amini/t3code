...
export class ApnsProviderTokens extends Context.Service<
	ApnsProviderTokens,
	{
		...
		) => Effect.Effect<string, ApnsJwtError>;
	}
...
export function quantizedApnsJwtIssuedAt(nowUnixSeconds: number): number {
	return (
		Math.floor(nowUnixSeconds / APNS_JWT_REUSE_SECONDS) * APNS_JWT_REUSE_SECONDS
	);
}
...
export const make = () =>
	ApnsProviderTokens.of({
		getJwt: Effect.fnUntraced(function* (input) {
			const issuedAtUnixSeconds = quantizedApnsJwtIssuedAt(
