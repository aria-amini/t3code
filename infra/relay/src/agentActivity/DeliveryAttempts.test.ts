...
describe("DeliveryAttempts", () => {
	it.effect(
		"records the signed queue source job id for APNs delivery auditability",
		() => {
	...
	it.effect(
		"reports completed source jobs when the durable claim already exists",
		() => {
			const fakeDb = {
				insert: () => ({
					values: () => ({
	...
	it.effect(
		"reports in-flight source jobs while an active claim lease exists",
		() => {
			const fakeDb = {
				insert: () => ({
					values: () => ({
	...
	it.effect(
		"preserves operation context and causes for persistence failures",
		() => {
			...
			return Effect.gen(function* () {
				...
				expect(claimError).toMatchObject({
					...
					cause,
					message:
						"Failed to persist APNs delivery attempt during claim-source-job.",
				});
				expect(completionError).toMatchObject({
					...
					cause,
					message:
						"Failed to persist APNs delivery attempt during complete-source-job.",
				});
			}).pipe(
				Effect.provide(
					DeliveryAttempts.layer.pipe(
