...
const registration: RelayDeviceRegistrationRequest = {
	...
	bundleId:
		"com.t3tools.t3code.preview" as RelayDeviceRegistrationRequest["bundleId"],
	apsEnvironment: "production",
	pushToken: "apns-device-token" as RelayDeviceRegistrationRequest["pushToken"],
	pushToStartToken:
		"push-to-start-token" as RelayDeviceRegistrationRequest["pushToStartToken"],
	preferences: {
...
describe("Devices", () => {
	it.effect(
		"claims APNs tokens globally before upserting the current user device",
		() => {
	...
	it.effect("unregisters APNs state only for the current user device", () => {
		...
		return Effect.gen(function* () {
			...
			expect(
				deleteConditions.map((condition) => dialect.sqlToQuery(condition)),
			).toEqual([
				{
					sql:
	...
	it.effect(
		"lists safe notification state without exposing APNs tokens",
		() => {
			const dialect = new PgDialect();
	...
	it.effect("identifies the failed device registration stage", () => {
		...
		const fakeDb = {
			update: () => ({
				set: (values: Record<string, unknown>) => ({
					where: () =>
						"pushToken" in values ? Effect.fail(cause) : Effect.void,
				}),
			}),
		} as unknown as RelayDb.RelayDb["Service"];
		...
		return Effect.gen(function* () {
			...
			const error = yield* devices
				.register({ userId: "user-2", registration })
				.pipe(Effect.flip);
	...
	it.effect("identifies the failed device unregistration stage", () => {
		...
		const fakeDb = {
			delete: (table: unknown) => ({
				where: () =>
					table === relayLiveActivities ? Effect.fail(cause) : Effect.void,
			}),
		} as unknown as RelayDb.RelayDb["Service"];
