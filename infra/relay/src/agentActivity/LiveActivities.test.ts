	...
	activities: [
		{
			...
			threadId:
				"thread" as RelayAgentActivityAggregateState["activities"][number]["threadId"],
...
describe("LiveActivities", () => {
	it.effect(
		...
		() => {
			const registration: RelayLiveActivityRegistrationRequest = {
				deviceId:
					"device-1" as RelayLiveActivityRegistrationRequest["deviceId"],
				activityPushToken:
			...
			return Effect.gen(function* () {
				...
				expect(
					updateConditions.map((condition) => dialect.sqlToQuery(condition)),
				).toEqual([
					{
	...
	it.effect(
		"preserves ended state when a delayed update delivery is marked",
		() => {
			const insertedValues: Array<Record<string, unknown>> = [];
			const conflictConfigs: Array<{
				readonly set?: Record<string, unknown>;
	...
	it.effect(
		"retires the previous activity token when a start or end is delivered",
		() => {
	...
	it.effect(
		"preserves correlation context and causes for persistence failures",
		() => {
			...
			return Effect.gen(function* () {
				...
				const targetListError = yield* Effect.flip(
					liveActivities.listTargets({ userId: "user-1" }),
				);
				const deliveryErrors = yield* Effect.all(
					[
						...
						liveActivities.clearStartQueued({
							userId: "user-1",
							deviceId: "device-1",
						}),
				...
				expect(targetListError).toMatchObject({
					...
					cause,
					message:
						"Failed to list Live Activity delivery targets for user user-1.",
				});
				...
				for (const [
					index,
					[operation, kind],
				] of expectedDeliveryContext.entries()) {
