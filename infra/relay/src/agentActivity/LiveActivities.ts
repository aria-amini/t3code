...
const decodeJsonString = Schema.decodeEffect(
...
export const make = Effect.gen(function* () {
	...
	return LiveActivities.of({
		register: Effect.fn("relay.live_activities.register")(function* (input) {
			...
			yield* Effect.gen(function* () {
				...
				yield* db
					...
					.where(
						eq(
							relayLiveActivities.activityPushToken,
							registration.activityPushToken,
						),
					);
		...
		listTargets: Effect.fn("relay.live_activities.list_targets")(
			function* (input) {
				return yield* db
					.select({
						...
						last_live_activity_delivery_at:
							relayLiveActivities.lastLiveActivityDeliveryAt,
					})
				...
				yield* Effect.gen(function* () {
					const aggregateJson =
						input.aggregate === null
							? null
							: yield* encodeRelayAgentActivityAggregateStateJson(
									input.aggregate,
								).pipe(
									Effect.flatMap(decodeJsonString),
									Effect.map(
										Function.cast<unknown, RelayAgentActivityAggregateState>,
									),
								);
					...
					yield* db
						...
						.values({
							...
							remoteStartedAt:
								input.kind === "live_activity_start" ? input.deliveredAt : null,
							remoteStartQueuedAt: null,
							endedAt:
								input.kind === "live_activity_end" ? input.deliveredAt : null,
						...
						.onConflictDoUpdate({
							target: [
								relayLiveActivities.userId,
								relayLiveActivities.deviceId,
							],
							set: {
		...
		markStartQueued: Effect.fn("relay.live_activities.mark_start_queued")(
			function* (input) {
				yield* Effect.annotateCurrentSpan({
					"relay.mobile.device_id": input.deviceId,
				});
		...
		clearStartQueued: Effect.fn("relay.live_activities.clear_start_queued")(
			function* (input) {
				yield* Effect.annotateCurrentSpan({
					"relay.mobile.device_id": input.deviceId,
				});
		...
		invalidateDeliveryToken: Effect.fn(
			"relay.live_activities.invalidate_delivery_token",
		)(function* (input) {
			yield* Effect.annotateCurrentSpan({
				"relay.mobile.device_id": input.deviceId,
				"relay.delivery.kind": input.kind,
