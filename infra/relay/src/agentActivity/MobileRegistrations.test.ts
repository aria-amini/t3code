	...
	return MobileRegistrations.layer.pipe(
		...
		Layer.provide(
			ApnsDeliveries.layer.pipe(
				Layer.provide(
					ApnsClient.layer.pipe(Layer.provide(ApnsProviderTokens.layer)),
				),
			),
		),
		Layer.provide(
...
describe("MobileRegistrations", () => {
	it.effect("registers devices through the device persistence service", () => {
		let registered:
			| Parameters<Devices.Devices["Service"]["register"]>[0]
			| null = null;
		let replayed:
			| Parameters<
		...
		return Effect.gen(function* () {
			const result = yield* Effect.gen(function* () {
				...
				return yield* registrations.registerDevice({
					userId: "dev:julius",
					payload: device,
				});
			}).pipe(
	...
	it.effect(
		"keeps device registration successful when activity replay fails",
		() => {
	...
	it.effect("unregisters the current user's device", () => {
		let unregistered:
			| Parameters<Devices.Devices["Service"]["unregister"]>[0]
			| null = null;
	...
	it.effect(
		"replays the latest activity state after registering a Live Activity token",
		() => {
			const liveActivity = {
			...
			let registered:
				| Parameters<LiveActivities.LiveActivities["Service"]["register"]>[0]
				| null = null;
			let replayed:
				| Parameters<
						AgentActivityPublisher.AgentActivityPublisher["Service"]["replayForLiveActivityRegistration"]
	...
	it.effect(
		"returns the current aggregate for the app's arming decision",
		() => {
			return Effect.gen(function* () {
				const registrations = yield* MobileRegistrations.MobileRegistrations;
				const snapshot = yield* registrations.getAgentActivitySnapshot({
					userId: "dev:julius",
				});
	...
	it.effect(
		...
		() => {
			...
			return Effect.gen(function* () {
				...
				Effect.provide(
					makeRegistrationReplayLayer({ devices, liveActivities, queuedJobs }),
				),
			);
		},
	);
