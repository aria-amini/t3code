...
export const make = Effect.gen(function* () {
	...
	return MobileRegistrations.of({
		registerDevice: Effect.fn("relay.mobile_registrations.register_device")(
			function* (input) {
		...
		registerLiveActivity: Effect.fn(
			"relay.mobile_registrations.register_live_activity",
		)(function* (input) {
			yield* Effect.annotateCurrentSpan({
				"relay.mobile.device_id": input.payload.deviceId,
			});
			yield* liveActivities.register({
				userId: input.userId,
				registration: input.payload,
			});
			yield* publisher
		...
		getAgentActivitySnapshot: Effect.fn(
			"relay.mobile_registrations.get_agent_activity_snapshot",
		)(function* (input) {
		...
		unregisterDevice: Effect.fn("relay.mobile_registrations.unregister_device")(
			function* (input) {
				yield* Effect.annotateCurrentSpan({
