...
describe("apnsDeliveryJobs", () => {
	...
	it("rejects Live Activity start jobs without aggregate state", () => {
		...
		expect(result).toMatchObject({
			...
			deviceId: "device-1",
			message:
				"APNs live activity start job job-start-invalid requires an aggregate.",
		});
	});
	...
	it("rejects push notification jobs carrying aggregate state", () => {
		...
		expect(result).toMatchObject({
			...
			deviceId: "device-1",
			message:
				"APNs push notification job job-push-invalid must not carry aggregate state.",
		});
	});
	...
	it("rejects jobs with invalid or overlong time windows", () => {
		...
		expect(
			...
			createdAt: "not-a-date",
			message:
				"APNs delivery job job-window has invalid creation time not-a-date.",
		});
		expect(
			verifySignedApnsDeliveryJob({
				secret,
