...
export const ApnsDeliveryJobPayload = Schema.Struct({
	...
	target: Schema.Struct({
		...
		apsEnvironment: Schema.optional(
			Schema.NullOr(Schema.Literals(["sandbox", "production"])),
		),
	}),
...
export type ApnsDeliveryJobVerificationError =
	typeof ApnsDeliveryJobVerificationError.Type;

export const isApnsDeliveryJobVerificationError = Schema.is(
...
export function expiresAtForJob(createdAtMs: number): string {
	return DateTime.formatIso(
		Option.getOrThrow(DateTime.make(createdAtMs + MAX_JOB_AGE_MS)),
	);
}

function validatePayloadShape(
	payload: ApnsDeliveryJobPayload,
): ApnsDeliveryJobInvalid | null {
	switch (payload.kind) {
