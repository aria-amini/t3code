...
export const makeApnsJwt = Effect.fn("relay.apns.make_jwt")(function* (
	input: ApnsJwtSigningInput,
) {
	const headerJson = yield* encodeApnsJwtHeaderJson({
		alg: "ES256",
		kid: input.keyId,
	}).pipe(
		Effect.mapError(
	...
	return yield* Effect.try({
		try: () => {
			...
			const signature = p256
				.sign(sha256(new TextEncoder().encode(signingInput)), scalar, {
					prehash: false,
				})
				.toCompactRawBytes();
...
function apnsSigningScalar(privateKeyPem: string): Uint8Array {
	...
	const jwk = NodeCrypto.createPrivateKey(
