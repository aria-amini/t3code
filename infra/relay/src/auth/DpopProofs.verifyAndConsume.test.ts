...
describe("DpopProofReplay.verifyAndConsume", () => {
	it.effect("reports a signed proof outside the time window", () => {
		...
		return Effect.gen(function* () {
			...
			Effect.provide(layer(() => Effect.die("unexpected replay persistence"))),
		);
	});

	it.effect(
		"rejects replayed proofs after persistence consumes the jti once",
		() => {
			const now = DateTime.makeUnsafe("2026-05-25T12:00:00.000Z");
			const proof = makeDpopProof({
				method: "POST",
	...
	it.effect("rejects proofs missing the expected access token hash", () => {
		...
		return Effect.gen(function* () {
			...
			Effect.provide(
				layer(() => Effect.die("unexpected DPoP replay persistence")),
			),
		);
	});
