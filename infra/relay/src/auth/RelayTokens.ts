...
export type RelayDpopAccessTokenClaims = Omit<
	typeof RelayDpopAccessTokenClaims.Type,
	"scope"
> & {
	readonly scope: ReadonlyArray<RelayDpopAccessTokenScope>;
};

const decodeLinkChallengeClaims =
	Schema.decodeUnknownEffect(LinkChallengeClaims);
const decodeDpopAccessTokenClaims = Schema.decodeUnknownEffect(
	RelayDpopAccessTokenClaims,
);
...
const make = Effect.gen(function* () {
	...
	const issueLinkChallenge: RelayTokens["Service"]["issueLinkChallenge"] =
		Effect.fn("relay.tokens.issue_link_challenge")(function* (input) {
	...
	const verifyLinkChallenge: RelayTokens["Service"]["verifyLinkChallenge"] =
		Effect.fn("relay.tokens.verify_link_challenge")((input) =>
			verifyRelayJwt({
				...
				Effect.map((claims) => {
					if (
						...
						(input.request.notificationsEnabled &&
							claims.notificationsEnabled !== true) ||
						(input.request.liveActivitiesEnabled &&
							claims.liveActivitiesEnabled !== true) ||
						(input.request.managedTunnelsEnabled &&
							claims.managedTunnelsEnabled !== true)
					) {
						return null;
					}
					return claims;
	...
	const issueDpopAccessToken: RelayTokens["Service"]["issueDpopAccessToken"] =
		Effect.fn("relay.tokens.issue_dpop_access_token")(function* (input) {
			return yield* signRelayJwt({
				privateKey: Redacted.value(config.cloudMintPrivateKey),
	...
	const verifyDpopAccessToken: RelayTokens["Service"]["verifyDpopAccessToken"] =
		Effect.fn("relay.tokens.verify_dpop_access_token")((input) =>
			verifyRelayJwt({
				publicKey: config.cloudMintPublicKey,
				token: input.token,
				typ: ACCESS_TOKEN_TYP,
