...
const isRelayPublicDomainLabelTooLongError = Schema.is(
	RelayPublicDomainLabelTooLongError,
);

describe("relayPublicDomainForStage", () => {
	it("uses the canonical relay hostname for production", () => {
		expect(relayPublicDomainForStage("prod", ".example.com.")).toBe(
			"relay.example.com",
		);
	});
...
describe("relayResourceNameForStage", () => {
	it("isolates production and personal stages", () => {
		...
		expect(
			relayResourceNameForStage("t3-code-relay-traces", "dev_julius"),
		).toBe("t3-code-relay-traces-dev-julius");
	});
});
...
describe("managed endpoint names", () => {
	it("uses the stage slug and a stable stage-scoped digest suffix", () => {
		...
		expect(
			managedEndpointDigestInput("dev_julius", "user_123", "env_123"),
		).toBe("dev_julius:user_123:env_123");
	...
	it("accepts allocated hostnames within the relay zone", () => {
		...
		expect(
			managedEndpointForHostname("dev-julius-abcdef0123456789.example.com"),
		).toEqual({
	...
	it("rejects hostnames outside the relay zone", () => {
		expect(
			isManagedEndpointHostname("internal.example.net", "example.com"),
		).toBe(false);
		expect(
			isManagedEndpointHostname("example.com.attacker.test", "example.com"),
		).toBe(false);
		expect(
			isManagedEndpointHostname("dev-julius.example.com.", "example.com"),
		).toBe(false);
	});
});
