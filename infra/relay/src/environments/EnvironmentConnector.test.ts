...
function signTestJwt(payload: object, typ: string, privateKey: string): string {
	const header = Buffer.from(JSON.stringify({ alg: "EdDSA", typ })).toString(
		"base64url",
	);
...
function signMintResponse(
	...
	const requestProof = decodeRequestProof<RelayCloudMintCredentialProofPayload>(
		request.proof,
	);
	const payload = {
...
function signHealthResponse(
	...
	const requestProof =
		decodeRequestProof<RelayCloudEnvironmentHealthProofPayload>(request.proof);
	const payload = {
		iss: `t3-env:${requestProof.environmentId}`,
		aud: "https://relay.example.test",
		sub: requestProof.environmentId,
		...
		checkedAt: DateTime.formatIso(
			DateTime.makeUnsafe(requestProof.iat * 1_000),
		),
		...payloadOverrides,
...
function makeLinks(
	...
	return {
		...
		listPublicKeysForEnvironment: () =>
			Effect.succeed([environmentKeyPair.publicKey]),
...
describe("EnvironmentConnector", () => {
	it.effect(
		"loads the environment link and managed allocation concurrently",
		() =>
			Effect.gen(function* () {
				...
				const execute = (request: HttpClientRequest.HttpClientRequest) =>
					Effect.sync(() => {
						const healthRequest = decodeHealthRequestBody(
					...
					Effect.provide(
						connectorTestLayer(execute, {
							links: {
								...links,
								getForUser: (input) =>
									waitForPeer.pipe(Effect.andThen(links.getForUser(input))),
							},
							allocations: {
								...allocations,
								get: (input) =>
									waitForPeer.pipe(Effect.andThen(allocations.get(input))),
							},
						}),
					),
				);
	...
	it.effect(
		"checks linked environment health through the managed endpoint",
		() => {
			...
			return Effect.gen(function* () {
				...
				expect(seenUrls).toEqual([
					"https://env.example.test/api/t3-connect/health",
				]);
	...
	it.effect(
		"rejects stale managed endpoints before sending a mint request",
		() => {
			let requestCount = 0;
			...
			return Effect.gen(function* () {
				...
				const resolutionSpan = spans.find(
					(span) =>
						span.name ===
						"relay.environment_connector.resolve_managed_endpoint",
				);
				expect(
					Object.fromEntries(resolutionSpan?.attributes ?? []),
				).toMatchObject({
	...
	it.effect(
		"rejects unready managed endpoint allocations before sending a request",
		() => {
			let requestCount = 0;
			const execute = () =>
				Effect.sync(() => {
	...
	it.effect(
		"rejects signed health responses with stale checkedAt timestamps",
		() => {
			const execute = (request: HttpClientRequest.HttpClientRequest) =>
				Effect.sync(() => {
					const healthRequest = decodeHealthRequestBody(
	...
	it.effect(
		"reports offline status when the managed endpoint health request fails",
		() => {
			const execute = (request: HttpClientRequest.HttpClientRequest) =>
				Effect.succeed(
			...
			return Effect.gen(function* () {
				...
				expect(result).toMatchObject({
					...
					status: "offline",
					error:
						"Managed endpoint health request failed: Environment is unavailable.",
	...
	it.effect(
		"rejects health responses with a mismatched top-level environment id",
		() => {
			const execute = (request: HttpClientRequest.HttpClientRequest) =>
				Effect.sync(() => {
					const healthRequest = decodeHealthRequestBody(
					...
					return HttpClientResponse.fromWeb(
						...
						Response.json(
							signHealthResponse(healthRequest, environmentKeyPair.privateKey, {
								environmentId:
									"other-env" as RelayEnvironmentHealthResponse["environmentId"],
							}),
							{ status: 200 },
						),
					);
	...
	it.effect(
		"rejects health responses with an unsigned top-level descriptor mutation",
		() => {
			const execute = (request: HttpClientRequest.HttpClientRequest) =>
				Effect.sync(() => {
					const healthRequest = decodeHealthRequestBody(
	...
	it.effect(
		"rejects health responses when the linked environment public key is malformed",
		() => {
			const execute = (request: HttpClientRequest.HttpClientRequest) =>
				Effect.sync(() => {
					const healthRequest = decodeHealthRequestBody(
	...
	it.effect(
		"mints a one-time environment credential through the linked endpoint",
		() => {
			const seenUrls: Array<string> = [];
			...
			return Effect.gen(function* () {
				...
				expect(seenUrls).toEqual([
					"https://env.example.test/api/t3-connect/mint-credential",
				]);
				expect(seenProofs[0]).toMatchObject({
					iss: "https://relay.example.test",
					aud: "t3-env:env-connector-test",
	...
	it.effect(
		"only accepts mint responses signed by the user's linked environment key",
		() => {
			const execute = (request: HttpClientRequest.HttpClientRequest) =>
				Effect.sync(() => {
					const mintRequest = decodeMintRequestBody(requestBodyText(request));
	...
	it.effect(
		"rejects mint responses when the linked environment public key is malformed",
		() => {
			const execute = (request: HttpClientRequest.HttpClientRequest) =>
				Effect.sync(() => {
					const mintRequest = decodeMintRequestBody(requestBodyText(request));
	...
	it.effect(
		"rejects environment mint responses with an overlong credential window",
		() => {
			const execute = (request: HttpClientRequest.HttpClientRequest) =>
				Effect.sync(() => {
					const mintRequest = decodeMintRequestBody(requestBodyText(request));
	...
	it.effect("times out hung managed endpoint mint requests", () => {
		...
		const execute = () =>
			Effect.sync(() => {
				...
				Effect.andThen(
					Effect.never as Effect.Effect<HttpClientResponse.HttpClientResponse>,
				),
			);
		...
		return Effect.gen(function* () {
			...
			yield* TestClock.adjust(
				Duration.millis(
					EnvironmentConnector.ENVIRONMENT_MINT_REQUEST_TIMEOUT_MS,
				),
			);
			...
			Effect.provide(
				Layer.merge(TestClock.layer(), connectorTestLayer(execute)),
			),
		);
	});
});
