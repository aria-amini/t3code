		...
		Effect.map(
			(proof) =>
				...
				Option.match(DateTime.make(input.response.expiresAt), {
					onNone: () => false,
					onSome: (expiresAt) =>
						Math.floor(expiresAt.epochMilliseconds / 1_000) === proof.exp,
				}),
		),
	);
}
	...
	return verifyWithEnvironmentKeys({
		...
		Effect.map((proof) => {
			if (
				...
				stableStringify(proof.descriptor) !==
					stableStringify(input.response.descriptor)
			) {
				return false;
			}
			...
			return (
				checkedAt.value.epochMilliseconds >=
					input.requestIssuedAt.epochMilliseconds -
						ENVIRONMENT_HEALTH_CLOCK_SKEW_MILLIS &&
				checkedAt.value.epochMilliseconds <=
...
const make = Effect.gen(function* () {
	...
	const resolveManagedEndpoint = Effect.fn(
		...
		if (input.link.endpoint.providerKind !== "cloudflare_tunnel") {
			yield* Effect.annotateCurrentSpan({
				"relay.authorization.endpoint_provider_kind":
					input.link.endpoint.providerKind,
			});
		...
		const allocationAttributes = {
			...
			"relay.authorization.allocation_has_ready_at":
				input.allocation.readyAt !== null,
			"relay.authorization.allocation_has_tunnel_id":
				input.allocation.tunnelId !== null,
			"relay.authorization.allocation_has_dns_record_id":
		...
		if (
			!isManagedEndpointHostname(
				input.allocation.hostname,
				settings.managedEndpointBaseDomain,
			)
		) {
			yield* Effect.annotateCurrentSpan({
				...allocationAttributes,
				"relay.authorization.managed_endpoint_base_domain":
					settings.managedEndpointBaseDomain,
			});
			return yield* new EnvironmentConnectNotAuthorized({
				environmentId: input.link.environmentId,
	...
	return EnvironmentConnector.of({
		status: Effect.fn("relay.environment_connector.status")(function* (input) {
			...
			const environmentClient = yield* makeEnvironmentClient(
				endpoint.httpBaseUrl,
			);
			const responseOption = yield* environmentClient.connect
				.health({ payload: { proof } })
				.pipe(
					withoutRedirects,
					Effect.match({
					...
					Effect.timeoutOption(
						Duration.millis(ENVIRONMENT_MINT_REQUEST_TIMEOUT_MS),
					),
				);
			...
			if (responseOption.value._tag === "Failure") {
				const failureReason = environmentHealthRequestFailureReason(
					responseOption.value.cause,
				);
				yield* Effect.annotateCurrentSpan({
				...
				return {
					...
					status: "offline" as const,
					checkedAt,
					error: environmentHealthRequestFailureMessage(
						responseOption.value.cause,
					),
					traceId,
				};
