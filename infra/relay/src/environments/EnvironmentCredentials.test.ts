...
describe("EnvironmentCredentials", () => {
	it.effect(
		"reports the credential creation persistence stage and preserves its cause",
		() => {
	...
	it.effect(
		"does not retain credential tokens when lookup persistence fails",
		() => {
			const cause = new Error("database unavailable");
			...
			return Effect.gen(function* () {
				...
				expect(query.sql).toContain(
					'"relay_environment_links"."environment_id"',
				);
				expect(query.sql).toContain(
					'"relay_environment_links"."environment_public_key"',
				);
				expect(query.sql).toContain(
	...
	it.effect(
		...
		() => {
			...
			return Effect.gen(function* () {
				...
				expect(insertedValues[0]?.credentialHash).toMatch(
					/^[A-Za-z0-9_-]{43}$/,
				);
				...
				expect(query.sql).toContain(
					'"relay_environment_credentials"."environment_id" = $1',
				);
				expect(query.sql).toContain(
					'"relay_environment_credentials"."environment_public_key" = $2',
				);
				expect(query.sql).toContain(
					'"relay_environment_credentials"."credential_id" <> $3',
				);
				expect(query.sql).toContain(
					'"relay_environment_credentials"."revoked_at" is null',
				);
	...
	it.effect("revokes active credentials for an environment public key", () => {
		...
		return Effect.gen(function* () {
			...
			expect(query.sql).toContain(
				'"relay_environment_links"."environment_id" = $3',
			);
			expect(query.sql).toContain(
				'"relay_environment_links"."environment_public_key" = $4',
			);
			expect(query.sql).toContain(
				'"relay_environment_links"."revoked_at" is null',
			);
