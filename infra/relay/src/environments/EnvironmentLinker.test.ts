...
const isEnvironmentLinkProofInvalid = Schema.is(
	EnvironmentLinker.EnvironmentLinkProofInvalid,
);
...
const makeRequest = Effect.gen(function* () {
	...
	const payload = {
		...
		challenge,
		environmentId:
			"env-link-test" as RelayEnvironmentLinkProofPayload["environmentId"],
		descriptor: {
			environmentId:
				"env-link-test" as RelayEnvironmentLinkProofPayload["environmentId"],
...
describe("EnvironmentLinker", () => {
	...
	it.effect(
		"links a publish-only environment with a non-secure nominal endpoint",
		() => {
	...
	it.effect("rejects a tampered compact proof before persistence", () => {
		...
		return Effect.gen(function* () {
			...
			const result = yield* Effect.result(
				linker.link({ userId: "user_123", request: tampered }),
			);
	...
	it.effect("rejects replayed JWT ids", () =>
		Effect.gen(function* () {
			...
			const linker = yield* EnvironmentLinker.EnvironmentLinker;
			const result = yield* Effect.result(
				linker.link({ userId: "user_123", request }),
			);
			expect(Result.isFailure(result)).toBe(true);
			if (Result.isFailure(result)) {
				expect(isEnvironmentLinkProofInvalid(result.failure)).toBe(true);
			...
			Effect.provide(testLayer({ consume: () => Effect.succeed(false) })),
		),
	);
});
