...
function isSecureManagedEndpoint(
	endpoint: RelayEnvironmentLinkProofPayload["endpoint"],
): boolean {
	try {
	...
	return EnvironmentLinker.of({
		link: Effect.fn("relay.environment_linker.link")(function* (input) {
			...
			yield* Effect.annotateCurrentSpan({
				...
				"relay.link.live_activities_enabled":
					input.request.liveActivitiesEnabled,
				"relay.link.managed_tunnels_enabled":
			...
			if (candidate.exp <= nowSeconds) {
				return yield* new EnvironmentLinkProofExpired({
					...
					expiresAt: DateTime.formatIso(
						DateTime.makeUnsafe(candidate.exp * 1_000),
					),
				});
			}
				...
				yield* managedEndpointProvider
					...
					.pipe(
						Effect.tapError((error) =>
							Effect.logWarning(
								"managed endpoint deprovision on publish-only link failed",
								{
									environmentId: verified.environmentId,
									errorTag: error._tag,
								},
			...
			if (
				input.request.managedTunnelsEnabled &&
				!isSecureManagedEndpoint(endpoint)
			) {
				return yield* new EnvironmentLinkProofInvalid({
					userId: input.userId,
					environmentId: verified.environmentId,
