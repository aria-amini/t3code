...
describe("EnvironmentLinks", () => {
	it.effect(
		"retains link lookup failures with user and environment identity",
		() => {
	...
	it.effect(
		"identifies delivery-user list failures without retaining key material",
		() => {
			const cause = new Error("database unavailable");
			const fakeDb = {
				select: () => ({
	...
	it.effect(
		"selects users when either notifications or Live Activities are enabled",
		() => {
			...
			return Effect.gen(function* () {
				const links = yield* EnvironmentLinks.EnvironmentLinks;
				expect(
					yield* links.listUsersForEnvironment({ environmentId: "env-1" }),
				).toEqual([]);
				expect(whereConditions).toHaveLength(1);

				const query = new PgDialect().sqlToQuery(whereConditions[0] as never);
				expect(query.sql).toContain(
					'"relay_environment_links"."environment_id" = $1',
				);
				expect(query.sql).toContain(
					'"relay_environment_links"."revoked_at" is null',
				);
				expect(query.sql).toContain(
					'"relay_environment_links"."notifications_enabled" = $2',
				);
				expect(query.sql).toContain(
					'"relay_environment_links"."live_activities_enabled" = $3',
				);
	...
	it.effect("revokes only the active link owned by the requesting user", () => {
		...
		return Effect.gen(function* () {
			...
			expect(query.sql).toContain(
				'"relay_environment_links"."environment_id" = $2',
			);
			expect(query.sql).toContain(
				'"relay_environment_links"."revoked_at" is null',
			);
