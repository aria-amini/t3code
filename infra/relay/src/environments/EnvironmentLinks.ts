...
export class EnvironmentLinks extends Context.Service<
	EnvironmentLinks,
	{
		...
		readonly getForUser: (input: {
			...
			RelayLinkedEnvironmentRecord | null,
			EnvironmentLinkLookupPersistenceError
		>;
...
const make = Effect.gen(function* () {
	...
	return EnvironmentLinks.of({
		...
		listUsersForEnvironment: Effect.fn(
			"relay.environment_links.list_users_for_environment",
		)(function* (input) {
			yield* Effect.annotateCurrentSpan({
		...
		listForUser: Effect.fn("relay.environment_links.list_for_user")(
			function* (input) {
				return yield* db
					.select({
					...
					.pipe(
						Effect.map((rows) =>
							rows.map((row) => ({
								environmentId:
									row.environmentId as RelayClientEnvironmentRecord["environmentId"],
								label:
		...
		getForUser: Effect.fn("relay.environment_links.get_for_user")(
			function* (input) {
				yield* Effect.annotateCurrentSpan({
					"relay.environment_id": input.environmentId,
				});
