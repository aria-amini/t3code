...
const make = Effect.gen(function* () {
	...
	return EnvironmentPublishSignatures.of({
		verify: Effect.fn("relay.environment_publish_signatures.verify")(
			function* (input) {
				...
				if (
					...
					return yield* new EnvironmentPublishSignatureExpired({
						...
						expiresAt: DateTime.formatIso(
							DateTime.makeUnsafe(decoded.exp * 1_000),
						),
					});
				}
				...
				if (
					...
					stableStringify(proof.state) !==
						stableStringify(input.request.state) ||
