...
describe("ManagedEndpointAllocations", () => {
	it.effect(
		"returns a claim generation only when deprovision wins the allocation CAS",
		() => {
	...
	it.effect(
		"does not remove an allocation superseded after a deprovision claim",
		() => {
			const fakeDb = {
	...
	it.effect(
		"retains database failures with allocation operation and identity",
		() => {
	...
	it.effect(
		"reports an unresolved reservation without manufacturing a cause",
		() => {
			const fakeDb = {
