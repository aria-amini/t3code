...
function providerLayer(
	...
	return ManagedEndpointProvider.layer.pipe(
		...
		Layer.provide(
			Layer.succeed(ManagedTunnelLimits.ManagedTunnelLimits, tunnelLimits),
		),
	);
}

function expectedManagedHostname(
...
function expectedManagedTunnelName(
	environmentId: string,
	userId = "user_ABC",
): string {
	const hash = NodeCrypto.createHash("sha256")
...
describe("ManagedEndpointProvider", () => {
	it.effect(
		"does not require the deployment RuntimeContext when building the Worker layer",
		() => {
			const tunnelClient = {
	...
	it.effect(
		"provisions a Cloudflare tunnel endpoint and connector token",
		() => {
	...
	it.effect(
		"checks the managed tunnel limit before reserving an allocation",
		() => {
			const limitCalls: Array<{
				readonly userId: string;
				readonly environmentId: string;
			}> = [];
			...
			return Effect.gen(function* () {
				...
				expect(limitCalls).toEqual([
					{ userId: "user_ABC", environmentId: "env_ABC" },
				]);
			}).pipe(
				Effect.provide(
					providerLayer(
	...
	it.effect(
		"refuses to provision past the managed tunnel limit without side effects",
		() => {
			const tunnelCalls: TunnelCall[] = [];
			const dnsCalls: DnsCall[] = [];
			const allocationCalls: AllocationCall[] = [];
	...
	it.effect(
		"uses stage-scoped stable names without leaking unusual environment ids",
		() => {
			const tunnelCalls: TunnelCall[] = [];
			...
			return Effect.gen(function* () {
				...
				expect(requestedName).toMatch(
					/^t3coderelay-managedendpoint-dev-julius-[a-f0-9]{16}$/,
				);
				const configBody = (
					tunnelCalls.find((call) => call.operation === "putConfiguration")
						?.input as { readonly tunnelConfig?: unknown } | undefined
				)?.tunnelConfig;
				expect(configBody).toMatchObject({
					ingress: [
						{
							hostname: expect.stringMatching(
								/^dev-julius-[a-f0-9]{16}\.t3code\.test$/,
							),
						},
						{ service: "http_status:404" },
					],
				...
				expect(
					tunnelCalls.find((call) => call.operation === "create")?.input,
				).toMatchObject({
					name: requestedName,
	...
	it.effect(
		"formats IPv6 loopback origins as valid Cloudflare ingress service URLs",
		() => {
			const tunnelCalls: TunnelCall[] = [];
	...
	it.effect(
		"rejects non-loopback managed endpoint origins before calling Cloudflare",
		() => {
			const dnsCalls: DnsCall[] = [];
			...
			return Effect.gen(function* () {
				...
				Effect.provide(
					providerLayer(makeTunnelClient(), makeDnsClient(dnsCalls)),
				),
			);
		},
	);
	...
	it.effect(
		"rejects invalid managed endpoint origin ports before calling Cloudflare",
		() => {
			const dnsCalls: DnsCall[] = [];
	...
	it.effect(
		"reconciles an existing same-host DNS record through the DNS client",
		() => {
			const dnsCalls: DnsCall[] = [];
			return Effect.gen(function* () {
				const provider = yield* ManagedEndpointProvider.ManagedEndpointProvider;
				...
				expect(dnsCalls[1]?.input).toMatchObject({
					dnsRecordId: "existing-record-id",
				});
			}).pipe(
				Effect.provide(
					providerLayer(
						makeTunnelClient(),
						makeDnsClient(dnsCalls, [{ id: "existing-record-id" }]),
					),
				),
			);
		},
	...
	it.effect(
		"reuses checkpointed resources when provisioning is retried",
		() => {
			const tunnelCalls: TunnelCall[] = [];
			const dnsCalls: DnsCall[] = [];
			const allocationCalls: AllocationCall[] = [];
	...
	it.effect(
		"recreates a checkpointed DNS record when it was removed externally",
		() => {
			const dnsCalls: DnsCall[] = [];
			const allocationCalls: AllocationCall[] = [];
	...
	it.effect(
		"does not deprovision an allocation superseded by a concurrent relink",
		() => {
			const tunnelCalls: TunnelCall[] = [];
			const dnsCalls: DnsCall[] = [];
			const allocationCalls: AllocationCall[] = [];
			...
			return Effect.gen(function* () {
				...
				expect(
					allocationCalls
						.slice(allocationCallCount)
						.map((call) => call.operation),
				).toEqual(["claimDeprovision"]);
			}).pipe(Effect.provide(layer));
		},
	);

	it.effect(
		"releases the tunnel while keeping the allocation, DNS record, and hostname",
		() => {
			const tunnelCalls: TunnelCall[] = [];
			const dnsCalls: DnsCall[] = [];
			const allocationCalls: AllocationCall[] = [];
			...
			return Effect.gen(function* () {
				...
				expect(allocationCalls.map((call) => call.operation)).not.toContain(
					"remove",
				);
			}).pipe(Effect.provide(layer));
		},
	...
	it.effect(
		"treats an environment without a recorded tunnel as already released",
		() => {
			const tunnelCalls: TunnelCall[] = [];
			const dnsCalls: DnsCall[] = [];
			const layer = providerLayer(
	...
	it.effect(
		"keeps the tunnel alive when a concurrent provision outdates the release claim",
		() => {
			const tunnelCalls: TunnelCall[] = [];
	...
	it.effect(
		"surfaces non-not-found tunnel deletion failures when releasing",
		() => {
			const failure =
				new ManagedEndpointProvider.ManagedEndpointTunnelClientError({
					operation: "delete",
	...
	it.effect(
		"keeps the allocation when tunnel cleanup fails so unlink can retry",
		() => {
			const allocationCalls: AllocationCall[] = [];
			const tunnelCalls: TunnelCall[] = [];
	...
	it.effect(
		"treats already deleted remote resources as successfully deprovisioned",
		() => {
			const allocationCalls: AllocationCall[] = [];
			const notFound = { _tag: "NotFound" } as const;
			const tunnelClient =
			...
			return Effect.gen(function* () {
				...
				expect(allocationCalls.map((call) => call.operation)).toContain(
					"removeClaimed",
				);
			}).pipe(Effect.provide(layer));
		},
	...
	it.effect("scopes managed endpoint resources by user", () => {
		...
		return Effect.gen(function* () {
			...
			expect(
				...
				{
					name: expectedManagedTunnelName("env_shared", "user_ABC"),
					isDeleted: false,
				},
				{
					name: expectedManagedTunnelName("env_shared", "user_DEF"),
					isDeleted: false,
				},
			]);
		}).pipe(Effect.provide(providerLayer(makeTunnelClient(tunnelCalls))));
	...
	it.effect(
		"recovers when DNS creation reports failure after the record became visible",
		() => {
			const dnsCalls: DnsCall[] = [];
			const failure = new ManagedEndpointProvider.ManagedEndpointDnsClientError(
				{
	...
	it.effect(
		"reports mismatched tunnel responses without manufacturing a cause",
		() => {
			const dnsCalls: DnsCall[] = [];
			const tunnelClient =
				ManagedEndpointProvider.ManagedEndpointTunnelClient.of({
					...
					create: () =>
						Effect.succeed({
							id: "returned-tunnel-id",
							name: "unexpected-tunnel",
						}),
				});
			...
			return Effect.gen(function* () {
				...
				Effect.provide(providerLayer(tunnelClient, makeDnsClient(dnsCalls))),
			);
		},
	);
