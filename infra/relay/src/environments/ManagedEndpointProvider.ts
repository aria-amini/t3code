...
export type ManagedEndpointDeprovisionTarget =
...
export class ManagedEndpointProvider extends Context.Service<
	ManagedEndpointProvider,
	{
		readonly provision: (input: {
			...
			ManagedEndpointProvisioningResult,
			ManagedEndpointProviderError
		>;
		/**
...
export const layerTunnelClient = (
	client: ManagedEndpointTunnelClient["Service"],
...
const requireCloudflareSettings = Effect.fnUntraced(function* (
	...
	const missingSettings: Array<
		"managedEndpointBaseDomain" | "managedEndpointNamespace"
	> = [];
	if (!baseDomain) {
...
type ManagedEndpointClientError =
	| ManagedEndpointTunnelClientError
	| ManagedEndpointDnsClientError;
...
export const make = Effect.gen(function* () {
	...
	const allocations =
		yield* ManagedEndpointAllocations.ManagedEndpointAllocations;
	...
	const updateExistingDnsRecords = Effect.fnUntraced(function* (
		...
		const keptRecord =
			records.find((record) => record.id === preferredDnsRecordId) ??
			records[0];
		...
		yield* Effect.forEach(
			records,
			(record) =>
				record.id === keptRecord.id ? Effect.void : dns.deleteRecord(record.id),
			{ discard: true },
		);
	...
	const prepareDeprovision = Effect.fn(
		"relay.managed_endpoint_provider.prepare_deprovision",
	)(function* (input: {
		readonly userId: string;
		readonly environmentId: string;
	}) {
	...
	return ManagedEndpointProvider.of({
		prepareDeprovision,
		deprovision: Effect.fn("relay.managed_endpoint_provider.deprovision")(
			function* (input) {
				...
				const allocation =
					input.target === undefined
						? yield* prepareDeprovision(input)
						: input.target;
				if (allocation === null) {
					return;
				}
		...
		release: Effect.fn("relay.managed_endpoint_provider.release")(
			function* (input) {
				yield* Effect.annotateCurrentSpan({
					"relay.user_id": input.userId,
					"relay.environment_id": input.environmentId,
		...
		provision: Effect.fn("relay.managed_endpoint_provider.provision")(
			function* (input) {
				yield* Effect.annotateCurrentSpan({
					"relay.user_id": input.userId,
					"relay.environment_id": input.environmentId,
				...
				const requestedTunnelName = managedEndpointTunnelName(
					cf.namespace,
					environmentHash,
				);
				yield* tunnelLimits
				...
				const tunnelResponse = yield* tunnels
					.list({ name: tunnelName, isDeleted: false })
					.pipe(
						...
						Effect.flatMap(
							Option.match({
								...
								onNone: () =>
									tunnels.create({ name: tunnelName, configSrc: "cloudflare" }),
							}),
						),
						Effect.mapError(
							(cause) =>
				...
				if (!tunnelResponse.id || tunnelResponse.name !== tunnelName) {
					return yield* new ManagedEndpointProvisioningFailed({
						...
						tunnelName,
						...(tunnelResponse.id
							? { returnedTunnelId: tunnelResponse.id }
							: {}),
						...(tunnelResponse.name
							? { returnedTunnelName: tunnelResponse.name }
							: {}),
					});
				}
