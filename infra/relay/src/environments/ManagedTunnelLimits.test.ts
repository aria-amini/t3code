...
const layerWithDb = (db: RelayDb.RelayDb["Service"]) =>
	ManagedTunnelLimits.layer.pipe(
		Layer.provide(Layer.succeed(RelayDb.RelayDb, db)),
	);

function makeFakeDb(input: {
	readonly overrideRows?: Effect.Effect<
		ReadonlyArray<{ readonly maxTunnels: number }>,
		Error
	>;
	readonly countRows?: Effect.Effect<
		ReadonlyArray<{ readonly activeTunnels: number }>,
		Error
	>;
}) {
	return {
					...
					innerJoin: (table: unknown, condition: unknown) => {
						...
						return {
							where: (where: unknown) => {
								...
								return (
									input.countRows ?? Effect.succeed([{ activeTunnels: 0 }])
								);
							},
						};
					},
...
describe("ManagedTunnelLimits", () => {
	...
	it.effect(
		"retains database failures with operation and user identity",
		() => {
