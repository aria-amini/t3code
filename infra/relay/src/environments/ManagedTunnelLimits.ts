...
export class ManagedTunnelLimits extends Context.Service<
	ManagedTunnelLimits,
	{
		readonly ensureCapacity: (input: {
			...
			void,
			ManagedTunnelLimitExceeded | ManagedTunnelLimitPersistenceError
		>;
	}
...
export const make = Effect.gen(function* () {
	...
	return ManagedTunnelLimits.of({
		ensureCapacity: Effect.fn("relay.managed_tunnel_limits.ensure_capacity")(
			function* (input) {
				...
				const maxTunnels =
					overrides[0]?.maxTunnels ?? DEFAULT_MANAGED_TUNNEL_LIMIT;
