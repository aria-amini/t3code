...
describe("relay client authentication", () => {
	it.effect("preserves the existing Clerk session JWT path", () =>
		Effect.gen(function* () {
			...
			expect(
				yield* verifyRelayClientBearerToken(relaySettings, "session-token"),
			).toEqual({
				sub: "user_session",
	...
	it.effect(
		"falls back to Clerk OAuth token verification for the headless CLI",
		() =>
			Effect.gen(function* () {
				vi.mocked(verifyToken).mockRejectedValue(
					new Error("not a session JWT"),
				);
				...
				expect(
					yield* verifyRelayClientBearerToken(relaySettings, "oauth-token"),
				).toEqual({
					sub: "user_oauth",
...
describe("relay environment authentication", () => {
	it.effect(
		"preserves credential lookup persistence failures as internal errors",
		() => {
			const failure =
			...
			return Effect.gen(function* () {
				...
				Effect.provideService(
					...
					HttpServerRequest.fromWeb(
						new Request("https://relay.test/v1/server/link"),
					),
				),
		...
		Layer.succeed(
			...
			EnvironmentLinks.EnvironmentLinks.of({
				...
				listUsersForEnvironment: () =>
					Effect.die("unused listUsersForEnvironment"),
		...
		Layer.succeed(
			...
			EnvironmentCredentials.EnvironmentCredentials.of({
				...
				revokeForEnvironmentPublicKey:
					input?.revokeCredential ?? (() => Effect.succeed(false)),
			}),
		),
		Layer.succeed(
			...
			ManagedEndpointProvider.ManagedEndpointProvider.of({
				...
				prepareDeprovision:
					input?.prepareDeprovision ?? (() => Effect.succeed(null)),
...
describe("relay environment unlink", () => {
	it.effect(
		"revokes the link and its credentials in one database transaction",
		() => {
	...
	it.effect(
		"commits database revocation before deprovisioning the managed endpoint",
		() => {
			const calls: Array<string> = [];
...
describe("relay request tracing", () => {
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				yield* traceRelayHttpRequestWith(
					endpoint,
				...
				expect(
					spans[0]?.attributes.get("http.request.header.authorization"),
				).toBe("<redacted>");
				expect(spans[0]?.attributes.get("http.request.header.dpop")).toBe(
					"<redacted>",
				);
				expect(Option.isNone(spans[0]!.parent)).toBe(true);
				expect(Option.getOrUndefined(spans[1]!.parent)?.spanId).toBe(
					spans[0]?.spanId,
				);
			}),
	);
	...
	it.effect(
		"fails hung requests with a 504 before the client's 10s abort",
		() =>
			Effect.gen(function* () {
				const spans: Array<Tracer.NativeSpan> = [];
				const tracer = Tracer.make({
				...
				const request = HttpServerRequest.fromWeb(
					new Request("https://relay.test/v1/mobile/devices", {
						method: "POST",
					}),
				);
				...
				expect(
					spans[0]?.attributes.get("relay.request.deadline_exceeded"),
				).toBe(true);
...
describe("relay routing fallback", () => {
	...
	it.effect("returns a CORS-compatible 404 response for unmatched paths", () =>
		Effect.gen(function* () {
			...
			const httpEffect = yield* HttpRouter.toHttpEffect(
				Layer.merge(relayNotFoundRoute, relayCors),
			);
			const response = yield* httpEffect.pipe(
				Effect.provideService(HttpServerRequest.HttpServerRequest, request),
			);
