...
const appendRelayCredentialResponseHeaders =
...
const appendRelayDpopChallengeHeader = HttpEffect.appendPreResponseHandler(
	(_request, response) =>
		Effect.succeed(
			response.status === 401
...
export const withoutCapturedParentSpan = <A, E, R>(
	...
	Effect.withFiber((fiber) => {
		...
		return effect.pipe(
			Effect.ensuring(Effect.sync(() => fiber.setContext(context))),
		);
	});
...
export const relayClientAuthLayer = Layer.effect(
	...
	Effect.gen(function* () {
		...
		return {
			clientBearer: Effect.fn("relay.auth.client.bearer")(function* (
				httpEffect,
				{ credential },
			) {
...
export const relayDpopClientAuthLayer = Layer.effect(
	...
	Effect.gen(function* () {
		...
		return {
			relayDpop: Effect.fn("relay.auth.dpop_client")(function* (
				httpEffect,
				{ credential },
			) {
...
function readHttpAuthorizationCredential(
...
export const unlinkEnvironmentRecord = Effect.fn(
	"relay.api.client.unlinkEnvironmentRecord",
)(function* (input: {
	readonly userId: string;
	readonly environmentId: string;
}) {
	...
	const managedEndpointProvider =
		yield* ManagedEndpointProvider.ManagedEndpointProvider;
		...
		return handlers
			.handle(
				...
				Effect.fn("relay.api.mobile.registerDevice")(function* (args) {
					...
					const proofKeyThumbprint = yield* requireDpopPrincipalScope(
						"mobile:registration",
					);
	...
	Effect.fnUntraced(function* (handlers) {
		...
		return handlers
			...
			.handle(
				...
				Effect.fn("relay.api.client.createEnvironmentLinkChallenge")(function* (
					args,
				) {
					yield* appendRelayCredentialResponseHeaders;
					const { userId } = yield* RelayClientPrincipal;
					...
					const challenge = yield* relayTokens
						.issueLinkChallenge({
							...
							expiresAtEpochSeconds: Math.floor(
								expiresAt.epochMilliseconds / 1_000,
							),
						})
						.pipe(
							Effect.catch(() => relayInternalErrorResponse("internal_error")),
			...
			.handle(
				...
				Effect.fn("relay.api.client.releaseEnvironmentTunnel")(function* (
					args,
				) {
					const { params } = args;
					const { userId } = yield* RelayClientPrincipal;
	...
	Effect.fnUntraced(function* (handlers) {
		...
		return handlers.handle(
			...
			Effect.fn("relay.api.token.exchangeDpopAccessToken")(function* (args) {
				...
				const verified = yield* verifyClerkBearerToken(
					config,
				...
				const expiresAt = DateTime.addDuration(
					now,
					RelayTokens.RELAY_DPOP_ACCESS_TOKEN_TTL,
				);
				const jti = yield* crypto.randomUUIDv4.pipe(
					Effect.catch(() => relayInternalErrorResponse("internal_error")),
				);
				...
				Effect.fn("relay.api.dpopClient.connectEnvironment")(
					function* (args) {
						...
						const requestedThumbprint =
							resolveConnectClientKeyThumbprint(payload);
						if (
							!requestedThumbprint ||
							requestedThumbprint !== proofKeyThumbprint
						) {
							return yield* new HttpApiError.Unauthorized({});
						}
				...
				Effect.fn("relay.api.dpopClient.getEnvironmentStatus")(
					function* (args) {
						...
						const proofKeyThumbprint =
							yield* requireDpopPrincipalScope("environment:status");
						yield* requireDpopThumbprint(proofKeyThumbprint, {
							expectedAccessToken: token,
						}).pipe(
							Effect.provideService(DpopProofs.DpopProofReplay, dpopProofs),
...
export const serverApi = HttpApiBuilder.group(
	...
	Effect.fnUntraced(function* (handlers) {
		...
		const publishSignatures =
			yield* EnvironmentPublishSignatures.EnvironmentPublishSignatures;
		return handlers.handle(
...
type MapRelayCommonApiError<E> =
	| Exclude<
			...
			| DpopProofs.DpopProofRejected
			| RelayCommonPersistenceError
	  >
	| (Extract<E, HttpApiError.Unauthorized> extends never
			? never
			: RelayAuthInvalidError)
	| (Extract<E, DpopProofs.DpopProofRejected> extends never
			? never
			: RelayAuthInvalidError)
	| (Extract<E, RelayCommonPersistenceError> extends never
			? never
			: RelayInternalError);
...
function relayInternalErrorResponse(reason: RelayInternalError["reason"]) {
	return currentTraceId.pipe(
		Effect.flatMap((traceId) =>
			Effect.fail(
				new RelayInternalError({ code: "internal_error", reason, traceId }),
			),
		),
	);
}
...
function mapRelayCommonApiErrors(authReason: RelayAuthInvalidReason) {
	...
	return <A, E, R>(
		...
		effect.pipe(Effect.catch(mapError));
}
...
type CatchTagCases<E, Cases> = {
	...
	? {}
	: { readonly [K in Exclude<keyof Cases, TaggedErrorTag<E>>]: never });
...
>(cases: Cases) {
	const catchCases = Record.map(
		...
		(makeError) => (error: never) =>
			currentTraceId.pipe(
				Effect.flatMap((traceId) => Effect.fail(makeError(error, traceId))),
			),
...
function resolveConnectClientKeyThumbprint(
...
function hasExpectedClerkAudience(
	audience: unknown,
	...
	return typeof audience === "string"
		...
		: Array.isArray(audience) &&
				audience.some(
					(entry) => typeof entry === "string" && entry === expectedAudience,
				);
}
	...
	return verifyClerkBearerToken(config, token).pipe(
		Effect.flatMap((verified) =>
			...
			hasExpectedClerkAudience(verified.aud, config.clerkJwtAudience)
				? Effect.succeed({
						sub: verified.sub,
						mode: "clerk_session_bearer" as const,
					})
				: Effect.fail(
						new ClerkTokenVerificationFailed({
		...
		Effect.catch(() =>
			verifyClerkOAuthBearerToken(config, token).pipe(
				Effect.map((verified) => ({
					...verified,
					mode: "clerk_oauth_bearer" as const,
				})),
			),
		),
	);
...
const requireDpopPrincipalScope = Effect.fn(
	"relay.api.require_dpop_principal_scope",
...
const requireDpopThumbprint = Effect.fn("relay.api.require_dpop_thumbprint")(
	function* (
...
const requireDpopProof = Effect.fn("relay.api.require_dpop_proof")(
	function* (options?: { readonly expectedAccessToken?: string }) {
		const request = yield* HttpServerRequest.HttpServerRequest;
		const now = yield* DateTime.now;
		const url = HttpServerRequest.toURL(request);
		if (url._tag === "None") {
...
const relayAuthInvalidError = Effect.fnUntraced(function* (
