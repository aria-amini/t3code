...
export const RelayObservability = Effect.gen(function* () {
	...
	const workerIngestToken = yield* Axiom.ApiToken(
		"RelayWorkerAxiomIngestToken",
		{
			name: relayResourceNameForStage("t3-code-relay-otel-ingest", stage),
			description:
				"Owned by Alchemy. Scoped OTLP ingest token for relay HTTP spans.",
	...
	const mobileIngestToken = yield* Axiom.ApiToken(
		"RelayMobileAxiomIngestToken",
		{
	...
	const clientIngestToken = yield* Axiom.ApiToken(
		"RelayClientAxiomIngestToken",
		{
			name: relayResourceNameForStage(
				"t3-code-relay-client-otel-ingest",
				stage,
			),
			description:
				"Owned by Alchemy. Scoped OTLP ingest token for first-party relay client spans.",
			datasetCapabilities: Output.map(traces.name, (dataset) => ({
				[dataset]: { ingest: ["create" as const] },
			})),
		},
...
const schemaErrorAttributes = (
	...
	const encoded = Schema.encodeUnknownOption(
		constructor as unknown as Schema.Encoder<unknown>,
	)(error);
	if (
