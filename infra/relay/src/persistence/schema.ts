...
export const relayMobileDevices = pgTable(
	"relay_mobile_devices",
	{
		...
		apsEnvironment: varchar("aps_environment", { length: 16 }).$type<
			"sandbox" | "production"
		>(),
...
export const relayLiveActivities = pgTable(
	"relay_live_activities",
	{
		...
		lastAggregateJson: jsonb(
			"last_aggregate_json",
		).$type<RelayAgentActivityAggregateState>(),
		lastLiveActivityDeliveryAt: varchar("last_live_activity_delivery_at", {
			length: 64,
		}),
		createdAt: varchar("created_at", { length: 64 }).notNull(),
		updatedAt: varchar("updated_at", { length: 64 }).notNull(),
...
export const relayEnvironmentLinks = pgTable(
	"relay_environment_links",
	{
		...
		notificationsEnabled: boolean("notifications_enabled")
			.notNull()
			.default(true),
		liveActivitiesEnabled: boolean("live_activities_enabled")
			.notNull()
			.default(true),
		managedTunnelsEnabled: boolean("managed_tunnels_enabled")
			.notNull()
			.default(false),
	...
	(table) => [
		...
		index("idx_relay_environment_links_environment").on(
			table.environmentId,
			table.revokedAt,
		),
	],
...
export const relayManagedEndpointAllocations = pgTable(
	...
	(table) => [
		primaryKey({ columns: [table.userId, table.environmentId] }),
		uniqueIndex("idx_relay_managed_endpoint_allocations_hostname").on(
			table.hostname,
		),
		uniqueIndex("idx_relay_managed_endpoint_allocations_tunnel_name").on(
			table.tunnelName,
		),
	],
);
...
export const relayEnvironmentCredentials = pgTable(
	...
	(table) => [
		uniqueIndex("idx_relay_environment_credentials_hash").on(
			table.credentialHash,
		),
		index("idx_relay_environment_credentials_environment").on(
			table.environmentId,
			table.revokedAt,
		),
