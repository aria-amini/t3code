		...
		digest: (algorithm, data) =>
			Effect.promise(async () => {
				...
				return new Uint8Array(
					await globalThis.crypto.subtle.digest(algorithm, input.buffer),
				);
			}),
	}),
);
...
const httpPlatformNotSupportedLayer = Layer.succeed(HttpPlatform.HttpPlatform, {
	...
	fileResponse: () =>
		Effect.die("Relay API does not serve filesystem responses"),
...
const ApnsDeliveryJobSigningSecret = Alchemy.makeRandom(
...
export const ApiLive = Api.make(
	...
	Effect.gen(function* () {
		...
		const hyperdrive = yield* Cloudflare.Hyperdrive.Connect(
			yield* RelayDb.RelayHyperdrive,
		);
		const db = yield* Drizzle.Postgres(hyperdrive.connectionString);

		const managedEndpointTunnelBinding =
		...
		const managedEndpointDnsBinding =
			yield* Cloudflare.DNS.ReadWriteDns(managedEndpointZone);
		const managedEndpointZoneName = yield* managedEndpointZone.name;

		//
		// 3. Runtime layers and app construction
		//
		const alchemyRuntimeContext: Alchemy.BaseRuntimeContext =
		...
		yield* Cloudflare.Queues.consumeQueueMessages<unknown>(
			apnsDeliveryQueue,
			{
				...
				deadLetterQueue:
					apnsDeliveryDeadLetterQueue.queueName as unknown as string,
			},
			(stream) =>
				stream.pipe(
					...
					Stream.runForEach((message) =>
						ApnsDeliveries.ApnsDeliveries.pipe(
							Effect.flatMap((deliveries) =>
								deliveries.processSignedJob(message.body),
							),
					...
					Effect.all([AgentActivityRows.AgentActivityRows, DateTime.now]).pipe(
						Effect.flatMap(([activityRows, now]) =>
							activityRows.pruneTerminal({
								updatedBefore: DateTime.formatIso(
