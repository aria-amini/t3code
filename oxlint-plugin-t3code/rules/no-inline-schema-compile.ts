...
const getSchemaCompilerMethod = (callee: unknown): Option.Option<string> => {
	...
	if (
		...
		expression.value.type !== "MemberExpression"
	) {
		return Option.none();
	}
...
const isNestedStaticSchemaCall = (node: unknown): boolean => {
	const expression = unwrapExpression(node);
	if (Option.isNone(expression) || expression.value.type !== "CallExpression")
		return false;

	const callee = unwrapExpression(expression.value.callee);
	if (Option.isNone(callee) || callee.value.type !== "MemberExpression")
		return false;
...
const isImmediatelyInvoked = (node: unknown): boolean => {
	...
	const parent =
		"parent" in expression.value
			? unwrapExpression(expression.value.parent)
			: Option.none();
	return (
		...
		return {
			...
			CallExpression(node) {
				...
				context.report({
					node: node.callee,
					message: high
						? messageHigh(method.value)
						: messageMedium(method.value),
				});
			},
		};
	},
