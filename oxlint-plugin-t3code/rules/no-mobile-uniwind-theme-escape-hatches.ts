...
const reportsAppearanceVariant = (value: string) =>
	APPEARANCE_VARIANT_PATTERN.test(value);
...
export default defineRule({
	...
	create(context) {
		...
		const resolveVariable = (node: unknown): Variable | undefined => {
			...
			if (Option.isNone(identifier) || identifier.value.type !== "Identifier")
				return undefined;
		...
		return {
				...
				for (const specifier of node.specifiers) {
					...
					if (
						...
						const variable = declaredVariables.find(
							(candidate) => candidate.name === localName,
						);
					...
					if (!isTypeOnly && importsModule(source.value, "/useThemeColor")) {
						context.report({
							node: specifier,
							message:
								"useThemeColor was replaced by semantic Uniwind classes.",
						});
					}
			...
			VariableDeclarator(node) {
				...
				if (namespace === undefined || !uniwindNamespaces.has(namespace))
					return;
				...
				for (const propertyNode of binding.value.properties) {
					if (propertyNode.type === "RestElement") {
						...
						if (
							Option.isNone(restBinding) ||
							restBinding.value.type !== "Identifier"
						)
							continue;

						const restName = restBinding.value.name;
						const variable = declaredVariables.find(
							(candidate) => candidate.name === restName,
						);
						if (variable !== undefined) uniwindNamespaces.add(variable);
						continue;
					}
			...
			Literal(node) {
				if (
					...
					!reportsAppearanceVariant(node.value)
				)
					return;
