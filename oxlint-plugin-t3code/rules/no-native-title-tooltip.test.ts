...
describe("t3code/no-native-title-tooltip", () => {
	...
	rule.valid(
		"allows the svg title child element",
		`const el = <svg><title>QR code</title></svg>;`,
	);

	rule.valid(
		"allows document.title assignments",
		`document.title = "Thread · T3 Code";`,
	);
	...
	rule.invalid(
		"reports title on an anchor",
		`const el = <a href="#" title="Learn more">More</a>;`,
	);
});
