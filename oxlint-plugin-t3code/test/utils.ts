...
class OxlintFixtureExpectedFailure extends Data.TaggedError(
...
const spawnAndCollectOutput = Effect.fnUntraced(function* (
...
export const createOxlintRuleHarness = (
	...
	const run: RuleHarness["run"] = Effect.fnUntraced(function* (source: string) {
		...
		const oxlintBin = path.join(
			path.dirname(oxlintPackageJsonPath),
			"bin",
			"oxlint",
		);
			...
			Effect.matchEffect({
				onFailure: (error) =>
					OxlintFixtureFailure.is(error)
						...
						: Effect.fail(error),
				onSuccess: () =>
					Effect.fail(new OxlintFixtureExpectedFailure({ ruleName })),
			}),
		);
