...
const makeHarness = Effect.fn("TestRemoteAuthorization.makeHarness")(
	function* (input: {
		...
		readonly beforeBootstrap?: Effect.Effect<
			void,
		...
		const relayInputs = yield* Ref.make<
			ReadonlyArray<
				Parameters<
					ManagedRelay.ManagedRelayClient["Service"]["connectEnvironment"]
				>[0]
			>
		>([]);
		const session = yield* Ref.make<
			Option.Option<ClientCapabilities.CloudSessionIdentity>
...
describe("RemoteEnvironmentAuthorization", () => {
	it.effect(
		"reuses a validated bearer descriptor while issuing fresh websocket tickets",
		() =>
			Effect.gen(function* () {
				...
				const [first, second] = yield* Effect.gen(function* () {
					const remote =
						yield* RemoteEnvironmentAuthorization.RemoteEnvironmentAuthorization;
					const authorize = () =>
				...
				expect(
					harness.fetch.calls.filter(([url]) =>
						String(url).endsWith("/api/auth/websocket-ticket"),
					),
				).toHaveLength(2);
			}),
	);
	...
	it.effect(
		"reuses a same-account persisted environment token without contacting the relay",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"evicts an auth-invalid cached token and obtains a fresh bootstrap",
		() =>
			Effect.gen(function* () {
				const cached = new TokenStore.RemoteDpopAccessToken({
					environmentId: ENVIRONMENT_ID,
	...
	it.effect(
		"presents clock skew as one possible cause for a generic DPoP rejection",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({
	...
	it.effect(
		"refreshes a cached endpoint after its first transient failure",
		() =>
			Effect.gen(function* () {
				const cached = new TokenStore.RemoteDpopAccessToken({
					environmentId: ENVIRONMENT_ID,
	...
	it.effect(
		"removes a newly refreshed token when websocket ticket authorization fails",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({
					responses: [
				...
				expect((yield* Ref.get(harness.tokens)).has(ENVIRONMENT_ID)).toBe(
					false,
				);
				expect(yield* Ref.get(harness.bootstrapCalls)).toBe(1);
				expect(harness.fetch.calls).toHaveLength(3);
	...
	it.effect(
		"uses the current cached HTTP token without a relay request or websocket ticket",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({
				...
				const authorized = yield* Effect.gen(function* () {
					const remote =
						yield* RemoteEnvironmentAuthorization.RemoteEnvironmentAuthorization;
					return yield* remote.authorizeDpopHttp({
						expectedEnvironmentId: ENVIRONMENT_ID,
					});
				}).pipe(Effect.provide(harness.layer));
		...
		() =>
			Effect.gen(function* () {
				...
				yield* Effect.gen(function* () {
					const remote =
						yield* RemoteEnvironmentAuthorization.RemoteEnvironmentAuthorization;
					const before = yield* remote.authorizeDpopHttp({
						expectedEnvironmentId: ENVIRONMENT_ID,
					});
					expect(before.httpAuthorization.accessToken).toBe(
						"cached-access-token",
					);
					yield* TestClock.adjust("1 second");
					const after = yield* remote.authorizeDpopHttp({
						expectedEnvironmentId: ENVIRONMENT_ID,
					});
					expect(after.httpAuthorization.accessToken).toBe(
						"fresh-access-token",
					);
					expect(after.httpAuthorization.expiresAtEpochMs).toBe(
						now + 3_601_000,
					);
				}).pipe(Effect.provide(harness.layer));
	...
	it.effect(
		"does not evict a replacement token when a late request rejects the old one",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({
					initialToken: persistedToken(),
				...
				yield* Effect.gen(function* () {
					const remote =
						yield* RemoteEnvironmentAuthorization.RemoteEnvironmentAuthorization;
					const initial = yield* remote.authorizeDpopHttp({
						expectedEnvironmentId: ENVIRONMENT_ID,
					});
					const retry = {
						expectedEnvironmentId: ENVIRONMENT_ID,
				...
				expect(
					(yield* Ref.get(harness.tokens)).get(ENVIRONMENT_ID)?.accessToken,
				).toBe("replacement-token");
			}),
	);

	it.effect(
		"shares one in-flight renewal between concurrent HTTP requests",
		() =>
			Effect.gen(function* () {
				...
				yield* Effect.gen(function* () {
					...
					expect(
						results.map((result) => result.httpAuthorization.accessToken),
					).toEqual(Array(6).fill("shared-token"));
				}).pipe(Effect.provide(harness.layer));
	...
	it.effect(
		"does not return or persist credentials after logout during renewal",
		() =>
			Effect.gen(function* () {
				const started = yield* Deferred.make<void>();
				const release = yield* Deferred.make<void>();
	...
	it.effect(
		"removes an obsolete credential if logout overlaps the persistent write",
		() =>
			Effect.gen(function* () {
				const started = yield* Deferred.make<void>();
				const release = yield* Deferred.make<void>();
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				expect((yield* authorize).httpAuthorization.accessToken).toBe(
					"cached-access-token",
				);
				yield* Ref.set(
					harness.session,
					Option.some({ accountId: "account-2" }),
				);
				expect((yield* authorize).httpAuthorization.accessToken).toBe(
					"account-2-token",
				);
				expect(
					(yield* Ref.get(harness.tokens)).get(ENVIRONMENT_ID)?.accountId,
				).toBe("account-2");
	...
	it.effect(
		"renews a legacy unowned token once and reuses its account-bound replacement",
		() =>
			Effect.gen(function* () {
				...
				const harness = yield* makeHarness({
					...
					responses: [
						Response.json(DESCRIPTOR),
						accessToken("account-bound-token"),
					],
				});
				const authorize = Effect.gen(function* () {
					const remote =
	...
	it.effect(
		"requires fresh credentials after signing back into the same account",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({
					initialToken: persistedToken(),
				...
				yield* Effect.gen(function* () {
					const remote =
						yield* RemoteEnvironmentAuthorization.RemoteEnvironmentAuthorization;
					yield* remote.authorizeDpopHttp({
						expectedEnvironmentId: ENVIRONMENT_ID,
					});
					yield* Ref.set(harness.session, Option.none());
					...
					const signedIn = yield* remote.authorizeDpopHttp({
						expectedEnvironmentId: ENVIRONMENT_ID,
					});
					expect(signedIn.httpAuthorization.accessToken).toBe(
						"new-session-token",
					);
				}).pipe(Effect.provide(harness.layer));
				expect(yield* Ref.get(harness.bootstrapCalls)).toBe(1);
	...
	it.effect("renews a cached credential bound to a different signing key", () =>
		Effect.gen(function* () {
			...
			expect(
				(yield* Ref.get(harness.tokens)).get(ENVIRONMENT_ID)?.dpopThumbprint,
			).toBe("thumbprint-2");
		}),
	);

	it.effect(
		"rejects a relay bootstrap for a different environment before contacting it",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({
					responses: [],
	...
	it.effect(
		"keeps a shared renewal alive when one waiting request is interrupted",
		() =>
			Effect.gen(function* () {
				const started = yield* Deferred.make<void>();
				const release = yield* Deferred.make<void>();
				...
				yield* Effect.gen(function* () {
					...
					const cached = yield* remote.authorizeDpopHttp({
						expectedEnvironmentId: ENVIRONMENT_ID,
					});
	...
	it.effect(
		"times out a stalled cloud token read so later requests can renew again",
		() =>
			Effect.gen(function* () {
				const started = yield* Deferred.make<void>();
