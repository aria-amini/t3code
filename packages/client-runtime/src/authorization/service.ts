...
export interface AuthorizedRemoteHttpEnvironment {
	...
	readonly label: string;
	readonly httpBaseUrl: string;
	readonly httpAuthorization: Extract<
...
function mapDpopSocketError(
	error: RemoteEnvironmentAuthError | ConnectionAttemptError,
) {
...
const fetchDescriptor = Effect.fn(
	"clientRuntime.connection.remote.fetchDescriptor",
)(function* (httpBaseUrl: string, connectionMethod: ClientConnectionMethod) {
	return yield* fetchRemoteEnvironmentDescriptor({ httpBaseUrl }).pipe(
		Effect.mapError((error) =>
			mapRemoteEnvironmentError(error, connectionMethod),
		),
	);
});
...
export const make = Effect.gen(function* () {
	...
	const pendingTokens = new Map<
		EnvironmentId,
		{
			...
			readonly token: Effect.Effect<
				TokenStore.RemoteDpopAccessToken,
				ConnectionAttemptError
			>;
		}
	>();
	...
	const authorizeBearer = Effect.fn(
		...
		const canReuseDescriptor =
			...
			cachedDescriptor.validatedAtEpochMs + BEARER_DESCRIPTOR_CACHE_TTL_MS >
				now;
	...
	const createDpopSocketUrl = Effect.fn(
		"clientRuntime.connection.remote.createDpopSocketUrl",
	)(function* (token: TokenStore.RemoteDpopAccessToken, timeoutMs?: number) {
		const ticketProof = yield* signer
			.createProof({
				method: "POST",
				url: environmentEndpointUrl(
					token.endpoint.httpBaseUrl,
	...
	const obtainBootstrap = Effect.fn("relay.connection.bootstrap.obtain")(
		function* (
			environmentId: EnvironmentId,
			identity: ClientCapabilities.CloudSessionIdentity,
		) {
	...
	const exchangeDpopToken = Effect.fn(
		"clientRuntime.connection.remote.exchangeDpopToken",
	)(function* (
		environmentId: EnvironmentId,
		thumbprint: string,
		identity: ClientCapabilities.CloudSessionIdentity,
	...
	const getDpopToken = Effect.fn(
		"clientRuntime.connection.remote.getDpopToken",
	)(function* (
		input: Parameters<
		...
		const selected = yield* tokenLock.withPermits(1)(
			Effect.gen(function* () {
				...
				if (
					...
					cached.value.expiresAtEpochMs >
						now + DPOP_ACCESS_TOKEN_REFRESH_SKEW_MS &&
					cached.value.accessToken !== input.rejectedAccessToken &&
					(owner?.accessToken !== cached.value.accessToken ||
						...
						tokenLock.withPermits(1)(
							Effect.gen(function* () {
								...
								yield* tokenStore
									...
									.pipe(
										Effect.withSpan(
											"environment.authorization.accessToken.persist",
										),
									);
								const current = yield* cloudSession.identity;
								if (Option.isNone(current) || current.value !== identity) {
	...
	const authorizeDpop = Effect.fn(
		"clientRuntime.connection.remote.authorizeDpop",
	)(function* (
		input: Parameters<
		...
		const socket = yield* createDpopSocketUrl(selected.token).pipe(
			Effect.result,
		);
