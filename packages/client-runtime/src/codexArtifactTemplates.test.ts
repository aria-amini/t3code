...
describe("artifact template presentation", () => {
	it("shares labels and copy text across clients", () => {
		expect(codexArtifactTemplatePresentationLabel("document")).toBe(
			"Document template",
		);
	});
});
...
describe("appendCodexArtifactTemplateUsePrompt", () => {
	...
	it("adds the prompt to an empty draft", () => {
		expect(appendCodexArtifactTemplateUsePrompt("", HELLO_WORLD_TEMPLATE)).toBe(
			prompt,
		);
	});
	...
	it("preserves existing draft text", () => {
		...
		).toBe(`Write about otters ${prompt}`);
	});
	...
	it.each([prompt, `${prompt}\n`, `Notes\n\n${prompt}`])(
		...
		(draft) => {
			expect(
				appendCodexArtifactTemplateUsePrompt(draft, HELLO_WORLD_TEMPLATE),
			).toBe(draft);
		},
	);
