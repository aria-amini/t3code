...
export type CodexArtifactTemplateKind =
	(typeof CODEX_ARTIFACT_TEMPLATE_KINDS)[number];

const CODEX_ARTIFACT_TEMPLATE_GALLERY_KINDS = [
	"imagegen",
	"product-design",
] as const;
...
export type CodexArtifactTemplateAttributes = Readonly<
...
function isCodexArtifactTemplateKind(
	value: unknown,
): value is CodexArtifactTemplateKind {
...
export function resolveCodexArtifactTemplate(
	...
	const displayName =
		typeof displayNameValue === "string" ? displayNameValue.trim() : undefined;
	...
	if (
		...
		(galleryKind !== undefined &&
			!isCodexArtifactTemplateGalleryKind(galleryKind))
	) {
		return null;
	}
...
const USE_PROMPT_BY_KIND: Record<
	CodexArtifactTemplateKind,
	(skill: string) => string
> = {
	document: (skill) => `Create a document using this ${skill} about…`,
	presentation: (skill) =>
		`Create a presentation using the ${skill} template about…`,
	...
	"google-slides": (skill) =>
		`Create a Google Slides presentation using this ${skill} about…`,
	"google-sheets": (skill) =>
		`Create a Google Sheet using this ${skill} about…`,
...
export function codexArtifactTemplateUsePrompt(
	template: CodexArtifactTemplate,
): string {
	return USE_PROMPT_BY_KIND[template.artifactKind](`$${template.skillName}`);
}

export function codexArtifactTemplatePresentationLabel(
	kind: CodexArtifactTemplateKind,
): string {
