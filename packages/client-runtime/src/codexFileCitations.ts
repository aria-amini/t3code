...
export type CodexFileCitationAttributes = Readonly<
...
function citationLabel(path: string): string {
	...
	return (
		normalized.slice(normalized.lastIndexOf("/") + 1) || normalized || "File"
	);
}
...
export function resolveCodexFileCitationLink(
	...
	return {
		path,
		href:
			lineRangeStart === undefined
				? destinationPath
				: `${destinationPath}#L${lineRangeStart}`,
...
export function codexFileCitationMarkdown(
	citation: CodexFileCitationLink,
): string {
