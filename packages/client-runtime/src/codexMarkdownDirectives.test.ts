...
const FILE_CITATION =
	':codex-file-citation{path="outputs/report.xlsx" purpose="output"}';
...
describe("native Markdown adapters", () => {
	it("uses the same parser to render file citations as portable links", () => {
		expect(
			renderCodexFileCitationsAsMarkdown(`Created ${FILE_CITATION}.`),
		).toBe("Created [report.xlsx](<outputs/report.xlsx>).");
	});
