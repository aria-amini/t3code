...
function asConstruct(
	value: Construct | Construct[] | undefined,
	label: string,
): Construct {
	...
	return {
		...
		tokenize(effects, ok, nok) {
			return effects.check(
				gate,
				construct.tokenize.call(this, effects, ok, nok),
				nok,
			);
		},
	};
...
function codexDirectiveSyntax(): Extension {
	...
	const textDirective = asConstruct(
		genericSyntax.text?.[COLON],
		CODEX_FILE_CITATION_NAME,
	);
	...
	if (!leafDirective)
		throw new Error(
			`Missing ${CODEX_ARTIFACT_TEMPLATE_NAME} directive construct`,
		);
	...
	return {
		...
		flow: {
			[COLON]: restrictedDirective(
				leafDirective,
				2,
				CODEX_ARTIFACT_TEMPLATE_NAME,
			),
		},
	};
}
...
function renderFileCitation(
	...
	node.data = {
		codexFileCitationMarkdown: codexFileCitationMarkdown(citation),
	};
	delete node.name;
	delete node.attributes;
	delete node.value;
	...
	node.data = {
		...
		hProperties: {
			...
			...(template.galleryKind === undefined
				? {}
				: { dataGalleryKind: template.galleryKind }),
...
function transformCodexDirectives(
	node: MarkdownAstNode,
	source: string,
	insideLink = false,
): void {
...
function collectDirectiveMatches(
	...
	if (start !== undefined && end !== undefined) {
		if (node.data?.codexFileCitationMarkdown !== undefined) {
			matches.push({
				...
				markdown: node.data.codexFileCitationMarkdown,
			});
			return;
		}
	...
	for (const child of node.children ?? [])
		collectDirectiveMatches(child, matches);
}
...
function renderDirectiveMatches(
	...
	for (const match of matches.sort((left, right) => right.start - left.start)) {
		...
		if (replacement !== undefined) {
			rendered =
				rendered.slice(0, match.start) +
				replacement +
				rendered.slice(match.end);
		}
	}
	return rendered;
}
