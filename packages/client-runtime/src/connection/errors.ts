...
export function profileMissingError(
	connectionId: string,
): ConnectionBlockedError {
...
export function credentialMissingError(
	connectionId: string,
): ConnectionBlockedError {
	return new ConnectionBlockedError({
...
export function mapManagedRelayError(
	error: ManagedRelayClientError,
): ConnectionAttemptError {
	switch (error._tag) {
...
export function mapRemoteEnvironmentError(
	...
	const networkHint =
		connectionMethod === "relay" ? ` ${NETWORK_BLOCKING_HINT}` : "";
	switch (error._tag) {
