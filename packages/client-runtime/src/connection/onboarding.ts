...
const resolvePairingTarget = Effect.fn(
	"clientRuntime.connection.onboarding.resolvePairingTarget",
			...
			new ConnectionBlockedError({
				...
				detail:
					cause instanceof Error
						? cause.message
						: "The pairing details are invalid.",
			}),
	});
});
		...
		try: () => normalizeHttpBaseUrl(options.input.httpBaseUrl),
			...
			new ConnectionBlockedError({
				...
				detail:
					cause instanceof Error
						? cause.message
						: "The environment URL is invalid.",
			}),
	});
...
export const prepareSshRegistration = Effect.fn(
	...
	const label =
		input.label?.trim() ||
		provisioned.label ||
		provisioned.bootstrap.target.alias;
