...
const makeHarness = Effect.fn("TestEnvironmentRegistry.makeHarness")(function* (
	...
	const registrationStore = Persistence.ConnectionRegistrationStore.of({
		register: (registration) =>
			Effect.gen(function* () {
				yield* (
					options?.beforeRegistrationRegister?.(registration) ?? Effect.void
				);
	...
	const driver = ConnectionDriver.ConnectionDriver.of({
		connect: (entry, reportProgress) =>
			Effect.gen(function* () {
				...
				yield* (
					options?.beforeSessionConnect?.(target.environmentId) ?? Effect.void
				);
...
describe("EnvironmentRegistry", () => {
	it.effect(
		"replays connected state when arming a desktop commit observer",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"does not acquire a session after the registry scope has already closed",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness([TARGET]);
				const registryScope = yield* Scope.make();
				const context = yield* Layer.build(harness.layer).pipe(
