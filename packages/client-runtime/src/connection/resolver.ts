...
const makePrimaryBroker = Effect.fn(
	"clientRuntime.connection.broker.makePrimary",
)(function* () {
...
const makeBearerBroker = Effect.fn(
	"clientRuntime.connection.broker.makeBearer",
)(function* () {
	const credentials =
	...
	return Effect.fn("clientRuntime.connection.broker.bearer")(function* (
		...
		const credential = yield* credentials.get(target.connectionId).pipe(
			Effect.flatMap(
				Option.match({
					onNone: () =>
						Effect.fail(credentialMissingError(target.connectionId)),
					onSome: Effect.succeed,
				}),
			),
		);
...
const makeRelayBroker = Effect.fn("clientRuntime.connection.broker.makeRelay")(
	function* () {
		const remote =
			yield* RemoteEnvironmentAuthorization.RemoteEnvironmentAuthorization;
...
export const make = Effect.gen(function* () {
	...
	const prepare = Effect.fn("clientRuntime.connection.broker.prepare")(
