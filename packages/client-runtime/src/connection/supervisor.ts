	...
	return Effect.matchCauseEffect(effect, {
		onFailure: (cause) =>
			Cause.hasInterrupts(cause)
				? Effect.interrupt
				: Effect.succeed(Exit.failCause(cause)),
...
export class EnvironmentSupervisor extends Context.Service<
	EnvironmentSupervisor,
	{
		...
		>;
		readonly prepared: SubscriptionRef.SubscriptionRef<
...
> {
	...
	const reportProgress = Effect.fn("EnvironmentSupervisor.reportProgress")(
		function* (
			attempt: number,
			generation: number,
			lastFailure: ConnectionAttemptError | null,
	...
	const runAttempt = Effect.fnUntraced(function* (
		...
		if (Exit.isFailure(establishment.exit)) {
			...
			if (isUnexpectedDefect) {
				const defect = establishment.exit.cause.reasons.find(
					Cause.isDieReason,
				)?.defect;
				yield* Effect.logError(
					"Connection attempt failed with an unexpected defect.",
				).pipe(
					Effect.annotateLogs({
						"environment.id": target.environmentId,
						"environment.label": target.label,
	...
	const run = Effect.fnUntraced(function* () {
		...
		for (;;) {
			...
			const attemptSpan: Option.Option<Tracer.Span> =
				outcome.failure.attemptSpan;
			...
			if (failedWakeProbe) {
				...
				resetRetryLadder();
				yield* setState(
					connectingState(yield* Ref.get(intent), generation, 1, error),
				);
				continue;
			}
	...
	yield* Effect.addFinalizer(() =>
		Queue.shutdown(signals).pipe(Effect.andThen(clearLease)),
	);
