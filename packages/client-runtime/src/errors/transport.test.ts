...
describe("isTransportConnectionErrorMessage", () => {
	it("returns true for SocketCloseError", () => {
		expect(
			isTransportConnectionErrorMessage("SocketCloseError: connection reset"),
		).toBe(true);
	});

	it("returns true for SocketOpenError", () => {
		expect(
			isTransportConnectionErrorMessage("SocketOpenError: ECONNREFUSED"),
		).toBe(true);
	});
	...
	it("recognizes connection errors emitted by the Effect RPC session", () => {
		expect(
			isTransportConnectionErrorMessage("Test environment disconnected."),
		).toBe(true);
		expect(
			isTransportConnectionErrorMessage(
		...
		expect(
			isTransportConnectionErrorMessage("Test environment is not connected."),
		).toBe(true);
		expect(
			isTransportConnectionErrorMessage("ClientProtocolError: socket closed"),
		).toBe(true);
	});
	...
	it("returns false for business logic errors", () => {
		...
		expect(isTransportConnectionErrorMessage("Invalid model selection")).toBe(
			false,
		);
	});
...
describe("sanitizeThreadErrorMessage", () => {
	...
	it("preserves non-transport errors", () => {
		expect(sanitizeThreadErrorMessage("Thread not found")).toBe(
			"Thread not found",
		);
		expect(
			sanitizeThreadErrorMessage("Select a base branch before sending."),
