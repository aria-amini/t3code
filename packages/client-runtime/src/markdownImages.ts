...
export function markdownImageSourceFragment(source: string): string {
	return splitMarkdownLinkSearchAndHash(
		normalizeMarkdownLinkDestination(source),
	).hash;
}
...
export function classifyMarkdownImageSource(
	...
	if (/^file:/i.test(source)) {
		...
		return target === null
			...
			: {
					_tag: "WorkspaceFile",
					path: stripSlashPrefixedWindowsDrive(
						safeDecodeURIComponent(target.path),
					),
				};
	}
	...
	return {
		_tag: "WorkspaceFile",
		path: joinWorkspacePath(workspaceRoot, path),
	};
}
