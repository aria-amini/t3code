...
describe("inlineCodeFilePathCandidate", () => {
	it.each([
		...
		"distinguishes file paths from code and hostnames in %s",
		(source, candidate) => {
...
describe("parseFileUrlHref", () => {
	...
	it("keeps percent-encoding so the caller decodes once", () => {
		expect(
			parseFileUrlHref("file:///Users/julius/project/file%2520name.md")?.path,
		).toBe("/Users/julius/project/file%2520name.md");
		expect(parseFileUrlHref("file:///c%3A/Users/x/shot.png")?.path).toBe(
			"/c%3A/Users/x/shot.png",
		);
	});

	it.each(["https://example.com/a.ts", "file://%", "/Users/julius/a.ts"])(
		"rejects %s",
		(href) => {
...
describe("parseMarkdownFileLink", () => {
	...
	it.each([
		...
		[
			"</D:/Programme/t3code/ChatMarkdown.tsx:1>",
			"D:/Programme/t3code/ChatMarkdown.tsx",
		],
		[
	...
	it("accepts conventional extensionless names with or without a position", () => {
		...
		expect(parseMarkdownFileLink("Dockerfile:8")).toEqual({
			path: "Dockerfile",
			line: 8,
		});
		expect(parseMarkdownFileLink("/srv/app/Makefile")).toEqual({
			path: "/srv/app/Makefile",
		});
	});

	it("reads positions from suffixes and line anchors", () => {
		expect(
			parseMarkdownFileLink("/Users/julius/project/src/main.ts#L42C7"),
		).toEqual({
		...
		expect(
			parseMarkdownFileLink("file://server/share/src/main.ts#L42C7"),
		).toMatchObject({
...
describe("workspaceRelativeFilePath", () => {
	it.each([
		...
		["/repo/project/src/main.ts", "/repo/project/", "src/main.ts"],
		[
			"C:\\Users\\mike\\t3code\\apps\\web\\a.ts",
			"C:/Users/mike/t3code",
			"apps/web/a.ts",
		],
		[
			"/C:/Users/mike/t3code/apps/web/a.ts",
			"C:/Users/mike/t3code",
			"apps/web/a.ts",
		],
