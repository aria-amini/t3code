...
describe("mediaFileReference", () => {
	it.each([
		...
		"preserves %s and only labels paths inside %s as relative",
...
describe("mediaReferenceFileName", () => {
	it.each([
		...
		[
			{ kind: "url", url: "//cdn.example/clip%20one%2Emp4?sig=a+b#t=2" },
			"clip one.mp4",
		],
		[{ kind: "url", url: "https://cdn.example/clip%2520.mp4" }, "clip%20.mp4"],
		[
			{ kind: "url", url: "https://cdn.example/clip%20%oops.mp4" },
			"clip%20%oops.mp4",
		],
