...
export function mediaFileReference(
	...
	const workspace = workspaceRoot
		? absolutePathParts(workspaceRoot)
		: undefined;
	if (
		!target ||
		!workspace ||
...
export function mediaReferenceFileName(
	reference: MediaReference,
): string | undefined {
	if (reference.kind === "file") {
		const windows =
			isWindowsAbsolutePath(reference.path) || reference.path.startsWith("//");
	...
	try {
		...
		basename = new URL(url.startsWith("//") ? `https:${url}` : url).pathname
			.split("/")
			.at(-1);
	} catch {
		return undefined;
