...
describe("resolveMediaSource", () => {
	describe("direct URLs", () => {
		...
		it("accepts extensionless image embeds only when asked", () => {
			...
			expect(
				resolveMediaSource(href, { threadId, imageEmbed: true }),
			).toMatchObject({
				kind: "image",
				mimeType: "image/*",
				name: "render",
	...
	describe("host paths", () => {
		...
		it.each([
			...
			expect(
				resolveMediaSource(href, { threadId, workspaceRoot: "/repo" }),
			).toEqual({
				kind: "image",
				mimeType: "image/png",
		...
		it("joins relative paths to the workspace and records the relative reference", () => {
			expect(
				resolveMediaSource("screens/logo.svg?v=2#mark", {
					threadId,
					workspaceRoot: "/repo",
				}),
			).toEqual({
		...
		it("separates a playback fragment from literal filename characters", () => {
			expect(
				resolveMediaSource("/tmp/clip%23one.mp4#t=2", { threadId }),
			).toMatchObject({
				kind: "video",
				srcFragment: "#t=2",
		...
		it("cannot be loaded without a thread to mint through", () => {
			expect(
				resolveMediaSource("/tmp/clip.mp4", { threadId: undefined }),
			).toMatchObject({
				kind: "video",
				access: "unavailable",
			});
	...
	it("serves T3 attachment files in place like any other host path", () => {
		...
		expect(
			resolveMediaSource(path, { threadId, workspaceRoot: "/repo" }),
		).toMatchObject({
			kind: "video",
			access: "environment",
			resource: { _tag: "media-file", threadId, path },
