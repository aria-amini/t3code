...
describe("add project shared logic", () => {
	...
	it("keeps explicit clone URLs and local paths unchanged", () => {
		expect(
			normalizePastedCloneUrl("https://gitlab.com/group/project.git"),
		).toBe("https://gitlab.com/group/project.git");
		expect(normalizePastedCloneUrl("git@github.com:owner/repo.git")).toBe(
			"git@github.com:owner/repo.git",
		);
		expect(normalizePastedCloneUrl("group/subgroup/project")).toBe(
			"group/subgroup/project",
		);
		expect(normalizePastedCloneUrl("/srv/git/repo.git")).toBe(
			"/srv/git/repo.git",
		);
	});
	...
	it("derives the clone folder name from any pasted clone URL", () => {
		expect(getCloneDirectoryName("https://github.com/owner/repo.git")).toBe(
			"repo",
		);
		expect(getCloneDirectoryName("https://github.com/owner/repo")).toBe("repo");
		expect(getCloneDirectoryName("https://github.com/owner/repo/")).toBe(
			"repo",
		);
		expect(getCloneDirectoryName("git@github.com:owner/repo.git")).toBe("repo");
		expect(
			getCloneDirectoryName("ssh://git@github.com:22/owner/repo.git"),
		).toBe("repo");
		expect(
			getCloneDirectoryName("https://user@bitbucket.org/owner/repo.git"),
		).toBe("repo");
		expect(
			getCloneDirectoryName("https://dev.azure.com/org/project/_git/repo"),
		).toBe("repo");
		expect(
			getCloneDirectoryName(
	...
	it("keeps a numeric repository name that sits on a path", () => {
		expect(getCloneDirectoryName("https://github.com/acme/123.git")).toBe(
			"123",
		);
	...
	it("proposes the clone destination inside the selected directory", () => {
		expect(getCloneDestinationPath("~/Projects/", "repo")).toBe(
			"~/Projects/repo",
		);
		expect(getCloneDestinationPath("~/Projects", "repo")).toBe(
			"~/Projects/repo",
		);
		expect(getCloneDestinationPath("C:\\work\\", "repo")).toBe(
			"C:\\work\\repo",
		);
	...
	it("marks authenticated source control providers as ready", () => {
		...
		expect(readiness.gitlab).toEqual({
			ready: false,
	...
	it("finds existing projects by normalized path in the target environment", () => {
		...
		expect(
			findExistingAddProject({ projects, environmentId: env, path: "/repo" })
				?.id,
		).toBe("project");
	});
