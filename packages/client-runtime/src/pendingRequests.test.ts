...
describe("pending questions", () => {
	it("preserves native answer keys while ignoring malformed options", () => {
		const question = {
			...
			question: "  Which path?\n",
			options: [
				{ label: " Keep spaces ", description: "", value: " native\t" },
			],
			multiSelect: false,
		};
		const requested = makeActivity({
			...
			payload: {
				...
				questions: [
					null,
					{ ...question, options: [...question.options, { label: 42 }] },
				],
			},
		});

		expect(derivePendingRequests([requested]).userInputs[0]?.questions).toEqual(
			[question],
		);
	});
	...
	it("only marks async questions as dismissible", () => {
		...
		expect(derivePendingRequests([native]).userInputs[0]?.dismissible).toBe(
			false,
		);
	});
	...
	it("preserves native choice values and the custom-answer restriction", () => {
		...
		expect(derivePendingRequests(activities).userInputs[0]?.questions).toEqual([
			question,
		]);
	});
...
describe.each(["approval", "user-input"])(
	"%s request completion",
	(requestKind) => {
		const requested = makeActivity({
		...
		it.each([
			...
			const replayedRequest = {
				...requested,
				id: EventId.make("replayed-request"),
				sequence: 43,
			};
			...
			for (const activities of [
				...
				expect(derivePendingRequests(activities)).toEqual({
					approvals: [],
					userInputs: [],
				});
			}
		...
		it("keeps a failed reply retryable unless the text names a stale request", () => {
			const failed = makeActivity({
				...
				payload: {
					requestId: "request-1",
					detail: "Provider adapter request failed: timeout",
				},
			});
			const pending = derivePendingRequests([requested, failed]);
			expect(
				[...pending.approvals, ...pending.userInputs].map(
			...
			expect(
				derivePendingRequests([requested, failed, retried, failed]),
			).toEqual({
				approvals: [],
				userInputs: [],
			});
