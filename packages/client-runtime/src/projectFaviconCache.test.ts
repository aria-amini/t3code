...
describe("persistent project favicon cache", () => {
	...
	it("reuses the image when signed URLs or connection origins change", async () => {
		...
		expect(
			await cache.resolve(
				target,
				"https://new.test/api/assets/token-b/vabc-icon.svg",
				signal(),
			),
		).toBe(image);
		expect(load).toHaveBeenCalledTimes(1);
	...
	it("keeps the old image during refresh and failures, then persists its replacement", async () => {
		...
		const refreshing = cache.resolve(
			target,
			url.replace("vabc", "vdef"),
			signal(),
		);
		...
		await cache.flush();
		expect(
			await createProjectFaviconCache({ storage, load }).resolve(
				target,
				null,
				signal(),
			),
	...
	it("isolates environments, workspaces, and icon selections", async () => {
		...
		expect(
			cache.peek({ ...target, environmentId: EnvironmentId.make("other") }),
		).toBeNull();
	});
...
describe("project favicon image loader", () => {
	...
	function loader(
		response: Response,
		downscale = vi.fn(async () => replacement),
	) {
		return {
			downscale,
	...
	it("inlines small icons exactly as served without rasterizing", async () => {
		...
		expect(await load(url, signal())).toBe(
	...
	it("falls back to the file extension when the response has no image type", async () => {
		const { load } = loader(
			new Response(svg, { headers: { "content-type": "text/plain" } }),
		);
		expect(await load(url, signal())).toBe(
			`data:image/svg+xml;base64,${svgBase64}`,
		);
	...
	it("downscales large bitmaps and refuses large vector icons", async () => {
		...
		expect(
			await bitmap.load(
				"https://remote.test/api/assets/t/v1-icon.png",
				signal(),
			),
		).toBe(replacement);
		...
		await expect(vector.load(url, signal())).rejects.toThrow(
			"exceeds the cache limit",
		);
	...
	it("stops reading a response that exceeds the source limit", async () => {
		...
		expect(pulled).toBeLessThan(
			PROJECT_FAVICON_MAX_SOURCE_BYTES / chunk.byteLength + 3,
		);
		expect(downscale).not.toHaveBeenCalled();
