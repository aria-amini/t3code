...
function keyFor(target: ProjectFaviconTarget) {
	return getProjectFaviconResourceKey(
...
async function readBounded(response: Response, maxBytes: number) {
	...
	if (declared > maxBytes)
		throw new Error("Project icon is too large to decode.");
	if (!response.body) {
...
export function createProjectFaviconImageLoader(input: {
	...
	return async (url: string, signal: AbortSignal): Promise<string> => {
		...
		if (!response.ok)
			throw new Error(`Project icon request failed with ${response.status}.`);
		...
		if (mimeType === "image/svg+xml")
			throw new Error("Project icon exceeds the cache limit.");
...
export function createProjectFaviconCache(input: {
			...
			try {
				for (const record of await input.storage.list()) {
					...
					if (Option.isSome(entry))
						entries.set(keyFor(entry.value), entry.value);
				}
				trim();
			} catch {
	...
	const peek = (target: ProjectFaviconTarget) =>
		entries.get(keyFor(target))?.dataUrl ?? null;
	...
	const resolve = async (
		...
		const revision = getProjectFaviconCacheKey(
			target.environmentId,
			target.cwd,
			url,
		);
	...
	const clear = async (environmentId?: EnvironmentId) => {
		if (environmentId === undefined) generation += 1;
			...
			environmentRevisions.set(
				environmentId,
				(environmentRevisions.get(environmentId) ?? 0) + 1,
			);
