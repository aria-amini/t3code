	...
	workspaceSnapshots: [
		{
			...
			skills: [
				{
					name: "project",
					path: "/workspace/project-a/SKILL.md",
					enabled: true,
				},
			],
		},
...
describe("dedupeProviderSkillsByName", () => {
	it("keeps the first resolved skill and preserves unrelated skill order", () => {
		...
		expect(
			dedupeProviderSkillsByName([firstSkill, otherSkill, duplicateSkill]),
		).toEqual([firstSkill, otherSkill]);
	});
});
...
describe("getProviderSkillsForSlashMenu", () => {
	it("keeps the skill alias when the provider also exposes it as a slash command", () => {
		...
		expect(
			getProviderSkillsForSlashMenu([askMatt], true).map((skill) => skill.name),
		).toEqual(["ask-matt"]);
	});
	...
	it("shows one row when enabled skills share a name", () => {
		...
		expect(
			getProviderSkillsForSlashMenu(skills, true).map((skill) => skill.name),
		).toEqual(["babysit-pr", "browser"]);
	});
...
describe("getProviderSlashCommandsForSlashMenu", () => {
	...
	it("lets the skill alias win when a provider command has the same name", () => {
		expect(
			getProviderSlashCommandsForSlashMenu(commands, skills).map(
	...
	it("keeps the provider command when the matching skill alias is hidden", () => {
		...
		expect(
			getProviderSlashCommandsForSlashMenu(commands, visibleSkills).map(
				(command) => command.name,
			),
...
describe("workspace provider snapshots", () => {
	it("uses the cwd snapshot after a provider session has populated it", () => {
		...
		expect(
			resolveProviderSlashCommandsForCwd(provider, "/workspace/project-a"),
		).toEqual([{ name: "project" }]);
	});

	it("keeps the machine snapshot before this cwd has a provider snapshot", () => {
		expect(
			resolveProviderSkillsForCwd(provider, "/workspace/project-b"),
		).toEqual(provider.skills);
		expect(resolveProviderSlashCommandsForCwd(provider, null)).toEqual(
