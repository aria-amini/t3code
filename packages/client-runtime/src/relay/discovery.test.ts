...
const makeHarness = Effect.fn("RelayDiscoveryTest.makeHarness")(function* () {
	...
	const listFailure =
		yield* Ref.make<ManagedRelay.ManagedRelayClientError | null>(null);
		...
		getEnvironmentStatus: ({ environmentId }) =>
			Ref.get(statusRequests).pipe(
				Effect.flatMap((requests) =>
					Deferred.await(requests.get(environmentId)!),
				),
			),
...
describe("RelayEnvironmentDiscovery", () => {
	it.effect(
		"publishes each environment status as soon as that lookup completes",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness();
				yield* Effect.gen(function* () {
					const discovery =
						yield* RelayEnvironmentDiscovery.RelayEnvironmentDiscovery;
					...
					expect(
						[...checking.environments.values()].every(
					...
					const partiallyResolved = yield* SubscriptionRef.changes(
						discovery.state,
					).pipe(
						Stream.filter(
							(state) =>
								state.environments.get(environments[1]!.environmentId)
									?.availability === "online",
						),
						Stream.runHead,
						Effect.map(Option.getOrThrow),
					...
					expect(
						partiallyResolved.environments.get(environments[0]!.environmentId)
							?.availability,
					).toBe("checking");
					...
					expect(
						complete.environments.get(environments[0]!.environmentId)
							?.availability,
					).toBe("offline");
		...
		() =>
			Effect.gen(function* () {
				...
				yield* Effect.gen(function* () {
					...
					const offlineFiber = yield* SubscriptionRef.changes(
						discovery.state,
					).pipe(
					...
					expect(
						(yield* SubscriptionRef.get(discovery.state)).environments.size,
					).toBe(2);
	...
	it.effect(
		"publishes listing failures without rejecting the refresh command",
		() =>
			Effect.gen(function* () {
				const networkStatus =
					yield* SubscriptionRef.make<NetworkStatus>("online");
				const client = ManagedRelay.ManagedRelayClient.of({
					relayUrl: "https://relay.example.test",
					listEnvironments: () =>
						Effect.fail(
	...
	it.effect(
		"refreshes proactively when credentials change before any manual refresh",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness();
				yield* Effect.gen(function* () {
					...
					const populated = yield* SubscriptionRef.changes(
						discovery.state,
					).pipe(
						Stream.filter(
							(state) =>
								state.environments.size === environments.length &&
								!state.refreshing,
						),
						Stream.runHead,
						Effect.map(Option.getOrThrow),
	...
	it.effect(
		"settles to a clean empty state when refreshed while signed out",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness();
				yield* Effect.gen(function* () {
	...
	it.effect(
		"does not republish stale rows after sign-out invalidates an in-flight refresh",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness();
				yield* Effect.gen(function* () {
					...
					yield* SubscriptionRef.changes(discovery.state).pipe(
						Stream.filter(
							(state) => state.environments.size === environments.length,
						),
						Stream.runHead,
					);
