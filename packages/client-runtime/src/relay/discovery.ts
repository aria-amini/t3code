...
export type RelayEnvironmentAvailability =
	| "checking"
	| "online"
	| "offline"
	| "error";
...
export const make = Effect.fn("RelayEnvironmentDiscovery.make")(function* () {
	...
	const offlineReportFingerprints = yield* Ref.make<
		ReadonlyMap<string, string>
	>(new Map());

	const clearOfflineReport = Effect.fn(
		"RelayEnvironmentDiscovery.clearOfflineReport",
	...
	const updateEnvironment = Effect.fn(
		"RelayEnvironmentDiscovery.updateEnvironment",
	)(function* (
		generation: number,
	...
	const refreshStatus = Effect.fn("RelayEnvironmentDiscovery.refreshStatus")(
		function* (
			generation: number,
			clerkToken: string,
			environment: RelayClientEnvironmentRecord,
			...
			if (result._tag === "Success") {
				if (result.success.status === "offline") {
					...
					const shouldReport = yield* Ref.modify(
						offlineReportFingerprints,
						(current) => {
							...
							return [
								true,
								new Map(current).set(environment.environmentId, fingerprint),
							];
						},
					);
					if (shouldReport) {
						yield* Effect.logWarning(
							"Relay environment health check reported offline",
							{
		...
		Effect.gen(function* () {
			...
			if (
				...
				(!Option.isSome(accountId) ||
					previousAccountId.value !== accountId.value)
			) {
				generation = yield* Ref.updateAndGet(
					accountGeneration,
					(current) => current + 1,
				);
				yield* Ref.set(refreshGeneration, generation);
			...
			networkStatus === "offline"
				...
				: Ref.get(hasRefreshed).pipe(
						Effect.flatMap((shouldRefresh) =>
							shouldRefresh ? refresh : Effect.void,
						),
					),
		),
		Effect.forkScoped,
