...
const encodeRelayError = Schema.encodeEffect(
	ManagedRelay.ManagedRelayClientError,
);
const decodeRelayError = Schema.decodeUnknownEffect(
	ManagedRelay.ManagedRelayClientError,
);
...
describe("ManagedRelayClient", () => {
	it.effect("owns tracing at service and implementation boundaries", () => {
		...
		return Effect.gen(function* () {
			...
			Effect.provide(managedRelayTestLayer(fetchFn)),
		);
	});
	...
	it.effect("rejects unsafe relay URLs before sending credentials", () => {
		...
		return Effect.gen(function* () {
			...
			Effect.provide(
				managedRelayTestLayer(fetchFn, "http://relay.example.test"),
			),
		);
	});

	it.effect(
		"reuses usable DPoP tokens and refreshes cleared or expiring cache entries",
		() => {
	...
	it.effect(
		"uses a cached token while another scope waits for an exchange",
		() => {
			...
			return Effect.gen(function* () {
				...
				Effect.provide(
					managedRelayTestLayer(fetchFn, undefined, accessTokenStore),
				),
			);
		},
	);
	...
	it.effect(
		"reuses a persisted token across runtimes and Clerk session token rotation",
		() => {
			let tokenExchangeCount = 0;
			let persistedTokens: ReadonlyArray<ManagedRelay.ManagedRelayAccessTokenCacheEntry> =
				[];
			...
			return Effect.gen(function* () {
				yield* Effect.gen(function* () {
					const relayClient = yield* ManagedRelay.ManagedRelayClient;
					yield* relayClient.getEnvironmentStatus(
						statusInput(clerkToken("user-1", "session-1")),
					);
				}).pipe(
					Effect.provide(
						managedRelayTestLayer(fetchFn, undefined, accessTokenStore),
				...
				yield* Effect.gen(function* () {
					const relayClient = yield* ManagedRelay.ManagedRelayClient;
					yield* relayClient.getEnvironmentStatus(
						statusInput(clerkToken("user-1", "session-2")),
					);
				}).pipe(
					Effect.provide(
						managedRelayTestLayer(fetchFn, undefined, accessTokenStore),
	...
	it.effect(
		"refreshes a persisted DPoP token once when the relay rejects it",
		() => {
			let tokenExchangeCount = 0;
			const statusTokens: Array<string | null> = [];
			let persistedTokens: ReadonlyArray<ManagedRelay.ManagedRelayAccessTokenCacheEntry> =
			...
			return Effect.gen(function* () {
				...
				expect(statusTokens).toEqual([
					"DPoP stale-relay-token",
					"DPoP fresh-relay-token",
				]);
				expect(tokenExchangeCount).toBe(1);
	...
	it.effect(
		"does not persist tokens when the Clerk subject cannot be decoded",
		() => {
			let persistedTokens: ReadonlyArray<ManagedRelay.ManagedRelayAccessTokenCacheEntry> =
				[];
			const accessTokenStore: ManagedRelay.ManagedRelayAccessTokenStore = {
	...
	it.effect("times out stalled relay environment listing requests", () => {
		...
		return Effect.gen(function* () {
			...
			yield* TestClock.adjust(
				Duration.millis(ManagedRelay.MANAGED_RELAY_REQUEST_TIMEOUT_MS),
			);
			...
			Effect.provide(
				Layer.merge(TestClock.layer(), managedRelayTestLayer(fetchFn)),
			),
		);
	});

	it.effect(
		"suggests checking network filtering when fetch fails without a response",
		() => {
			const fetchFn = (() =>
				Promise.reject(
	...
	it.effect(
		"does not suggest network filtering for an HTTP server error",
		() => {
			const fetchFn = (() =>
				Promise.resolve(
			...
			return Effect.gen(function* () {
				...
				expect(error.message).toBe(
					"Could not list relay-managed environments.",
				);
			}).pipe(Effect.provide(managedRelayTestLayer(fetchFn)));
		},
	);
	...
	it.effect(
		"accepts generic DPoP errors from relays without the optional reason",
		() => {
			const fetchFn = (() =>
				Promise.resolve(
					Response.json(
	...
	it.effect(
		"lists account devices through the Clerk bearer client endpoint",
		() => {
			const fetchFn = ((input, init) => {
				expect(String(input)).toBe(
					"https://relay.example.test/v1/client/devices",
				);
