...
import {
	encodeOAuthScope,
	oauthScopeSetEquals,
} from "@t3tools/shared/oauthScope";
...
export type ManagedRelayDpopSignerError =
	typeof ManagedRelayDpopSignerError.Type;
...
export type ManagedRelayRequestActivity =
	typeof ManagedRelayRequestActivity.Type;
...
export interface ManagedRelayAccessTokenStore {
	...
	>;
	readonly save: (
		entries: ReadonlyArray<ManagedRelayAccessTokenCacheEntry>,
	) => Effect.Effect<void>;
...
function disabledManagedRelayClient(
	relayUrl: string,
): ManagedRelayClient["Service"] {
	...
	return ManagedRelayClient.of({
		...
		listEnvironments: unavailable(
			"clientRuntime.managedRelay.listEnvironments",
		),
		...
		getAgentActivitySnapshot: unavailable(
			"clientRuntime.managedRelay.getAgentActivitySnapshot",
		),
...
export const make = Effect.fn("ManagedRelayClient.make")(function* (
	...
	const initialTokens = options.accessTokenStore
		? yield* options.accessTokenStore.load
		: [];
	...
	const dpopProofTargets = {
		...
		getEnvironmentStatus: (
			...
			url: urlBuilder.dpopClient.getEnvironmentStatus({
		...
		connectEnvironment: (
			...
			url: urlBuilder.dpopClient.connectEnvironment({
				params: { environmentId },
			}),
		}),
	...
	const exchangeAccessToken = Effect.fn(
				...
				Effect.mapError(
					(error) =>
						new ManagedRelayTokenProofCreationError(
							proofCreationErrorFields(error),
						),
				),
			);
	...
	const obtainAccessToken = Effect.fn(
		...
		return yield* SynchronizedRef.modifyEffect(cachedTokens, (tokens) =>
			...
			Effect.withSpan("clientRuntime.managedRelay.tokenCacheCriticalSection"),
		);
	});

	const authorize = Effect.fn("clientRuntime.managedRelay.authorize")(
		function* (input: {
			readonly clerkToken: string;
			readonly scopes: ReadonlyArray<RelayDpopAccessTokenScope>;
	...
	const invalidateAccessToken = Effect.fn(
		"clientRuntime.managedRelay.invalidateAccessToken",
		...
		return yield* SynchronizedRef.modifyEffect(cachedTokens, (tokens) => {
			...
			return (
				options.accessTokenStore
					? options.accessTokenStore.save(nextTokens)
					: Effect.void
							...
							return invalidateAccessToken(authorization.accessToken).pipe(
								Effect.tap((invalidated) =>
									Effect.annotateCurrentSpan({
										...
										"relay.token_cache.retry_after_invalidation":
								...
								Effect.andThen(
									refreshRejectedToken ? attempt(false) : Effect.fail(error),
								),
							);
						}),
					),
	...
	return ManagedRelayClient.of({
			...
			function* (input) {
				return yield* client.client
					...
					.pipe(
						...
						Effect.mapError(
							relayRequestError("list relay-managed environments"),
						),
		...
		createEnvironmentLinkChallenge: Effect.fnUntraced(
			function* (input) {
				return yield* client.client
					...
					.pipe(
						Effect.mapError(
							relayRequestError("create relay environment link challenge"),
						),
				...
				return yield* runDpopRequest(
					...
					(authorization) =>
						client.dpopClient
							...
							.pipe(
								Effect.mapError(
									relayRequestError("get relay environment status"),
								),
				...
				return yield* mobileRegistrationRequest(
					...
					(authorization) =>
						client.mobile
							...
							.pipe(
								Effect.mapError(
									relayRequestError("register relay mobile device"),
								),
				...
				return yield* mobileRegistrationRequest(
					...
					(authorization) =>
						client.mobile
							...
							.pipe(
								Effect.mapError(
									relayRequestError("unregister relay mobile device"),
								),
				...
				return yield* mobileRegistrationRequest(
					...
					(authorization) =>
						client.mobile
							...
							.pipe(
								Effect.mapError(
									relayRequestError("read relay agent activity snapshot"),
								),
				...
				return yield* mobileRegistrationRequest(
					...
					(authorization) =>
						client.mobile
							...
							.pipe(
								Effect.mapError(
									relayRequestError("register relay live activity"),
								),
		...
		resetTokenCache: SynchronizedRef.set(cachedTokens, []).pipe(
			Effect.andThen(
				options.accessTokenStore ? options.accessTokenStore.clear : Effect.void,
			),
			Effect.withSpan("clientRuntime.managedRelay.resetTokenCache"),
			withRelayClientTracing,
		),
