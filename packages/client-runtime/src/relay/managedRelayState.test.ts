...
describe("createManagedRelayQueryManager", () => {
	...
	it.effect(
		"deregisters an environment through the current Clerk session",
		() =>
			Effect.gen(function* () {
				...
				yield* deregisterManagedRelayEnvironment(registry, {
					...
					Effect.provideService(
						...
						createClient({ unlinkEnvironment }),
					),
				);
	...
	it.effect(
		"deduplicates concurrent Clerk token reads and reuses the token until JWT expiry",
		() =>
			Effect.gen(function* () {
				...
				const readsFiber = yield* Effect.all(
					[session.readClerkToken(), session.readClerkToken()],
					{
	...
	it.effect(
		"updates the token provider without replacing a same-account session",
		() =>
			Effect.gen(function* () {
				...
				const secondRead = vi.fn(() =>
					Promise.resolve<string | null>("refreshed-token"),
				);
				setManagedRelaySession(registry, {
					accountId: "account-1",
	...
	it.effect(
		"does not pin a refreshed session to an older pending token read",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"emits credential changes only when the managed relay account changes",
		() =>
			Effect.gen(function* () {
				setManagedRelaySession(registry, {
					accountId: "account-1",
				...
				yield* Effect.promise(() =>
					vi.waitFor(() => {
						expect(
							registry.getNodes().get(managedRelaySessionAtom)?.listeners.size,
						).toBeGreaterThan(0);
					}),
				);
	...
	it("shares one Clerk token read across concurrent relay list and status queries", async () => {
		...
		await vi.waitFor(() => {
			expect(
				readManagedRelaySnapshotState(registry.get(firstStatusAtom)).data
					?.status,
			).toBe("online");
			expect(
				readManagedRelaySnapshotState(registry.get(secondStatusAtom)).data
					?.status,
			).toBe("online");
		});
		expect(readClerkToken).toHaveBeenCalledTimes(1);
	...
	it("reports token and relay request phases for environment status queries", async () => {
		...
		await vi.waitFor(() => {
			expect(
				readManagedRelaySnapshotState(registry.get(atom)).data?.status,
			).toBe("online");
		});
