...
const isManagedRelayRequestFailedError = Schema.is(
	ManagedRelay.ManagedRelayRequestFailedError,
);

export interface ManagedRelaySession {
	readonly accountId: string;
	readonly readClerkToken: () => Effect.Effect<
		string | null,
...
export class ManagedRelaySessionError extends Data.TaggedError(
	"ManagedRelaySessionError",
)<{
	readonly message: string;
...
export class ManagedRelaySnapshotError extends Data.TaggedError(
	"ManagedRelaySnapshotError",
)<{
	readonly message: string;
}> {}

export const managedRelaySessionAtom = Atom.make<ManagedRelaySession | null>(
	null,
).pipe(Atom.keepAlive, Atom.withLabel("managed-relay:session"));

const managedRelaySessionControls = new WeakMap<
	ManagedRelaySession,
	ManagedRelaySessionControl
>();

export function createManagedRelaySession(
	input: ManagedRelaySessionInput,
): ManagedRelaySession {
	let cachedToken: {
	...
	const readCachedClerkToken = async (
		nowMillis: number,
	): Promise<string | null> => {
		if (
			cachedToken &&
			cachedToken.expiresAtMillis > nowMillis + CLERK_TOKEN_EXPIRY_SKEW_MS
		) {
	...
	const session: ManagedRelaySession = {
		...
		readClerkToken: Effect.fn(
			"clientRuntime.managedRelaySession.readClerkToken",
		)(function* () {
