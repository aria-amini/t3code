...
describe("environment RPC", () => {
	it.effect(
		"reuses the session config stream instead of opening a duplicate subscription",
		() =>
			Effect.gen(function* () {
				...
				const received = yield* subscribe(
					WS_METHODS.subscribeServerConfig,
					{},
				).pipe(
					Stream.runHead,
	...
	it.effect("binds finite streaming commands to one active session", () =>
		Effect.gen(function* () {
			const firstEvents =
				yield* Queue.unbounded<RelayClientInstallProgressEvent>();
			const secondEvents =
				yield* Queue.unbounded<RelayClientInstallProgressEvent>();
			const firstClient = {
			...
			yield* SubscriptionRef.set(
				activeSession,
				Option.some(session(firstClient)),
			);
			...
			yield* SubscriptionRef.set(
				activeSession,
				Option.some(session(secondClient)),
			);
	...
	it.effect(
		"switches durable subscriptions when the supervisor replaces the session",
		() =>
			Effect.gen(function* () {
				...
				const awaitSubscriptions = Effect.fn(
					"TestEnvironmentRpc.awaitSubscriptions",
					...
					return yield* Effect.die(
						new Error(`Expected ${count} durable subscriptions.`),
					);
				});
	...
	it.effect(
		"keeps the producer session on an old value buffered across a session switch",
		() =>
			Effect.gen(function* () {
				...
				const firstClient = {
					[WS_METHODS.subscribeServerLifecycle]: () =>
						Stream.fromEffect(
							...
							Stream.drain,
							Stream.concat(
								Stream.fromIterable([firstValue, bufferedFirstValue]),
							),
				...
				const secondClient = {
					[WS_METHODS.subscribeServerLifecycle]: () =>
						Stream.fromEffect(
							Deferred.succeed(secondSubscribed, undefined),
						).pipe(
							Stream.drain,
	...
	it.effect(
		"keeps durable subscriptions alive across a transport failure and new session",
		() =>
			Effect.gen(function* () {
				const subscriptions: string[] = [];
				const firstClient = {
				...
				for (
					let attempt = 0;
					attempt < 100 && subscriptions.length < 1;
					attempt += 1
				) {
					yield* Effect.yieldNow;
				}
				...
				for (
					let attempt = 0;
					attempt < 100 && subscriptions.length < 2;
					attempt += 1
				) {
					yield* Effect.yieldNow;
				}
	...
	it.effect(
		"keeps handled domain failures dormant until a replacement session arrives",
		() =>
			Effect.gen(function* () {
				const domainError = new Error("terminal subscription rejected");
				const subscriptions: string[] = [];
				...
				for (
					let attempt = 0;
					attempt < 100 && observedFailures.length < 1;
					attempt += 1
				) {
					yield* Effect.yieldNow;
				}
	...
	it.effect(
		"retries handled domain failures within the same session when configured",
		() =>
			Effect.gen(function* () {
					...
					[WS_METHODS.subscribeTerminalEvents]: () =>
						Stream.unwrap(
							Ref.getAndUpdate(subscriptionCount, (count) => count + 1).pipe(
								Effect.map((count) =>
									count === 0 ? Stream.fail(domainError) : Stream.never,
								),
							),
						),
				} as unknown as WsRpcProtocolClient;
				...
				const subscriptionFiber = yield* subscribe(
					...
					{
						onExpectedFailure: () =>
							Ref.update(expectedFailureCount, (count) => count + 1),
				...
				const exit = yield* subscribeDynamicWithSession(
					...
					() =>
						Effect.sync(() => {
							...
							Effect.andThen(
								where === "input" ? Effect.die(defect) : Effect.succeed({}),
							),
						),
					{
						onDefect: (cause) =>
	...
	it.effect(
		"reports an initializer defect once after an expected failure retries",
		() =>
			Effect.gen(function* () {
					...
					() =>
						Effect.sync(() => {
							...
							Effect.flatMap((attempt) =>
								attempt === 1 ? Effect.succeed({}) : Effect.die(defect),
							),
						),
					{
						onDefect: (cause) =>
						...
						onExpectedFailure: () =>
							Effect.sync(() => {
								...
								Effect.andThen(Deferred.succeed(expectedFailure, undefined)),
								Effect.asVoid,
							),
						retryExpectedFailureAfter: "250 millis",
					},
