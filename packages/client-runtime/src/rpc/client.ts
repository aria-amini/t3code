...
export type EnvironmentUnaryRpcTag = Exclude<
	EnvironmentRpcTag,
...
export type EnvironmentRpcInput<TTag extends EnvironmentRpcTag> = Parameters<
	RpcMethod<TTag>
>[0];
...
export function subscribeDynamicWithSession<
...
> {
	return subscribeDynamicMapped(
		...
		makeInput,
		(session, stream) =>
			stream.pipe(Stream.map((value) => [session, value] as const)),
		options,
	);
}
