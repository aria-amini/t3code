...
const encodeServerConfigStreamEvent = Schema.encodeSync(
	ServerConfigStreamEvent,
);
...
const SOURCE_SERVER_CONFIG: ServerConfigType = {
	...
	environment: {
		...
		capabilities: {
			...THEME_SERVER_CONFIG.environment.capabilities,
...
const makeFactory = Effect.fn("TestRpcSessionFactory.make")(function* (
	...
	const layer = RpcSession.layerWithOptions(options).pipe(
...
const awaitSocket = Effect.fn("TestRpcSessionFactory.awaitSocket")(function* (
	...
	return yield* Effect.die(
		new Error("Expected the RPC protocol to create a websocket."),
	);
});
...
const awaitRequest = Effect.fn("TestRpcSessionFactory.awaitRequest")(function* (
	...
	return yield* Effect.die(
		new Error("Expected the RPC protocol to send a request."),
	);
});

const completeInitialConfig = Effect.fn(
	"TestRpcSessionFactory.completeInitialConfig",
)(function* (
	socket: TestWebSocket,
...
const publishConfigEvents = Effect.fn(
	"TestRpcSessionFactory.publishConfigEvents",
)(function* (
	socket: TestWebSocket,
...
describe("RpcSessionFactory", () => {
	it.effect(
		"owns one scoped websocket attempt and exposes readiness and closure",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"replays current config and broadcasts updates to every subscriber",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const { factory, sockets } = yield* makeFactory();
				...
				Effect.scoped(
					Effect.gen(function* () {
						...
						yield* completeInitialConfig(
							socket,
							ENCODED_THEME_SERVER_CONFIG,
							options,
						);
						yield* Fiber.join(readyFiber);

						const shared = yield* session
							.subscribeServerConfig(options)
							.pipe(Stream.runHead);
		...
		(options) =>
			Effect.scoped(
				Effect.gen(function* () {
					...
					yield* completeInitialConfig(
						socket,
						encodeServerConfig(SOURCE_SERVER_CONFIG),
						options,
					);
					yield* Fiber.join(ready);
					const observed =
						yield* Queue.unbounded<ServerConfigStreamEventType>();
					...
					const themes: ServerConfigStreamEventType[] =
					...
					for (const event of events) {
						...
						for (const item of replayed)
							projection = applyServerConfigProjection(projection, item);
						expect(
							Option.getOrThrow(projection).config.usageLimitSources,
						).toEqual(
	...
	it.effect("replays theme updates and deletion as authoritative events", () =>
		Effect.scoped(
			Effect.gen(function* () {
				...
				const subscriber = yield* session
					...
					.pipe(
						Stream.mapEffect((event) =>
							Deferred.succeed(subscriberStarted, undefined).pipe(
								Effect.as(event),
							),
						),
						Stream.take(4),
				...
				expect(
					Option.getOrThrow(projection).config.environmentThemes,
				).toBeUndefined();
			}),
		),
	);
	...
	it.effect(
		"recovers a slow subscriber after it misses theme and usage-source deletion",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const { factory, sockets } = yield* makeFactory({
					...
					const observedByFastSubscriber =
						yield* Queue.unbounded<ServerConfigStreamEventType>();
					yield* session
						.subscribeServerConfig({
							environmentThemes: true,
						...
						.pipe(
							Stream.runForEach((event) =>
								Queue.offer(observedByFastSubscriber, event),
							),
							Effect.forkChild,
						);
					expect((yield* Queue.take(observedByFastSubscriber)).type).toBe(
						"snapshot",
					);
	...
	it.effect("closes the session when the config source dies", () =>
		Effect.scoped(
			Effect.gen(function* () {
				...
				const closedFiber = yield* session.closed.pipe(
					Effect.exit,
					Effect.forkChild,
				);
				socket.serverMessage(
	...
	it.effect.each([
		{ failure: "defect" as const },
		{ failure: "typed" as const },
	])(
		...
		({ failure }) =>
			Effect.scoped(
				Effect.gen(function* () {
					...
					const awaitConfig = (
						predicate: (config: ServerConfigType) => boolean,
					) =>
					...
					expect(
						(yield* Fiber.join(firstThemeState)).environmentThemes,
					).toEqual(firstThemes);

					const firstClosed = yield* firstSession.closed.pipe(
						Effect.exit,
						Effect.forkChild,
					);
					...
					if (failure === "typed" && Exit.isFailure(firstClosedExit)) {
						expect(Cause.squash(firstClosedExit.cause)).toBeInstanceOf(
							ConnectionBlockedError,
						);
					...
					yield* completeInitialConfig(
						secondSocket,
						encodeServerConfig(recoveredConfig),
						{
							environmentThemes: true,
						},
					);
					...
					expect((yield* Fiber.join(recoveredState)).environmentThemes).toEqual(
						firstThemes,
					);
					...
					expect(
						(yield* Fiber.join(liveRecoveredState)).environmentThemes,
					).toEqual(recoveredThemes);
				}),
			),
	);
	...
	it.effect(
		"tolerates two missed pong windows before closing the session",
		() =>
			Effect.gen(function* () {
				const { factory, sockets } = yield* makeFactory();
				const session = yield* factory.connect(PREPARED);
				const readyFiber = yield* Effect.forkChild(session.ready);
				const closedFiber = yield* Effect.forkChild(
					Effect.flip(session.closed),
				);
				const socket = yield* awaitSocket(sockets);
				...
				expect(
					socket.sent.map((message) => decodeJson(message)).filter(isPing),
				).toEqual([{ _tag: "Ping" }, { _tag: "Ping" }, { _tag: "Ping" }]);
	...
	it.effect(
		"reaches ready when a newer server sends unknown config members",
		() =>
			Effect.gen(function* () {
				const { factory, sockets } = yield* makeFactory();
				const session = yield* factory.connect(PREPARED);
	...
	it.effect(
		"uses the legacy config RPC for probes when the server lacks the capability",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const { factory, sockets } = yield* makeFactory();
	...
	for (const relay of [false, true]) {
		it.effect(
			`fails readiness when the ${relay ? "relay" : "direct"} websocket never opens`,
			() =>
				Effect.gen(function* () {
					const { factory, sockets } = yield* makeFactory();
