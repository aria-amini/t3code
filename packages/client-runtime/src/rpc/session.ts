...
>;
type ProbeError = Effect.Error<
	ReturnType<WsRpcProtocolClient[typeof WS_METHODS.serverProbe]>
>;
...
export const make = Effect.fn("RpcSessionFactory.make")(function* (
	...
	const connect = Effect.fnUntraced(function* (connection: PreparedConnection) {
		...
		const protocolClient = yield* makeWsRpcProtocolClient.pipe(
		...
		const serverConfigExit = yield* Deferred.make<
			...
			ServerConfigSubscriptionError
		>();
		const configSubscriptionClosed = yield* Deferred.make<
			never,
			ConnectionAttemptError
		>();
		const serverConfigState = yield* Ref.make(
			Option.none<ServerConfigReplayState>(),
		);
		const serverConfigUpdates =
			yield* PubSub.sliding<BufferedServerConfigEvent>(64);
				...
				Effect.gen(function* () {
					const buffered = yield* Ref.modify(serverConfigState, (current) => {
						...
						if (Option.isNone(projection)) {
							return [
								Option.none<BufferedServerConfigEvent>(),
								current,
							] as const;
						}
						const next = {
							...
							themesEvent:
								event.type === "environmentThemesUpdated"
									? event
									: event.type === "snapshot" &&
										  event.config.environment.capabilities
			...
			Effect.onExit((exit) => {
				if (Exit.isSuccess(exit)) {
					return Effect.all([
						...
						Deferred.fail(
							configSubscriptionClosed,
							configSubscriptionEndedError,
						),
					]).pipe(Effect.asVoid);
				}
				...
				return Effect.all([
					...
					Deferred.failCause(
						configSubscriptionClosed,
						Cause.map(exit.cause, mapRpcError),
					),
				]).pipe(Effect.asVoid);
			}),
		);
		...
		const serverConfigEvents = Stream.unwrap(
			Effect.gen(function* () {
				...
				const updates = Stream.fromSubscription(subscription).pipe(
					Stream.filter(
						(buffered) => buffered.revision > snapshot.value.revision,
					),
					Stream.mapAccum(
