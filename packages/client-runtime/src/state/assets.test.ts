...
describe("asset collection keys", () => {
	...
	it("rejects invalid asset collection shapes", () => {
		...
		expect(() => parseAssetCollectionKey(key)).toThrowError(
			InvalidAssetCollectionKeyError,
		);
	});
});
...
describe("createAssetEnvironmentAtoms", () => {
	for (const scenario of [
		...
		{
			name: "literal filename characters",
			path: "/tmp/frame#one?two.png",
			fallback: true,
		},
		...
		it.effect(`uses the correct environment for ${scenario.name}`, () =>
			Effect.gen(function* () {
				...
				const localEnvironment = Atom.make<typeof localTarget | null>(
					scenario.primary === "none" || scenario.primary === "reconnecting"
						? null
						: localTarget,
				);
				...
				const result = AtomRegistry.getResult(registry, query, {
				...
				expect(calls).toEqual(
					scenario.fallback ? [remoteId, localId] : [remoteId],
				);
...
describe("project favicon URL cache", () => {
	it("renders a persisted thumbnail immediately in a fresh registry and refreshes it remotely", async () => {
		...
		const previousCache = createProjectFaviconCache({
			storage,
			load: async () => image,
		});
		...
		const result = Atom.make<
			AsyncResult.AsyncResult<AssetCreateUrlResult, unknown>
		>(AsyncResult.initial());
		const connection = Atom.make<Option.Option<{ httpBaseUrl: string }>>(
			Option.none(),
		);
		...
		try {
			...
			registry.set(
				connection,
				Option.some({ httpBaseUrl: "https://remote.test" }),
			);
			registry.set(
				result,
				AsyncResult.success({
	...
	it("retains icons across outages and remounts, then accepts refreshed and missing icons", () => {
		...
		try {
			...
			expect(registry.get(favicon)).toBe(
				"https://remote.test/api/assets/token-a/icon.svg",
			);
			...
			registry.set(
				connection,
				Option.some({ httpBaseUrl: "https://reconnected.test" }),
			);
			expect(registry.get(favicon)).toBe(
				"https://remote.test/api/assets/token-a/icon.svg",
			);
			...
			expect(registry.get(favicon)).toBe(
				"https://reconnected.test/api/assets/token-b/icon.svg",
			);
	...
	it("does not reuse another environment, workspace, or selected icon's cached URL", () => {
		...
		const favicon = createProjectFaviconUrlAtomFamily({
			createUrl: () => result,
			preparedConnection: () =>
				Atom.make(Option.some({ httpBaseUrl: "https://remote.test" })),
		});
		const target = {
			environmentId: EnvironmentId.make("remote"),
			cwd: "/workspace",
		...
		try {
			...
			expect(
				registry.get(
					favicon({ ...target, environmentId: EnvironmentId.make("other") }),
				),
			).toBeNull();
			expect(registry.get(favicon({ ...target, cwd: "/other" }))).toBeNull();
			expect(
				registry.get(favicon({ ...target, faviconPath: "brand.svg" })),
			).toBeNull();
