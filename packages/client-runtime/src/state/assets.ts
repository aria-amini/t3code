...
export const EMPTY_ASSET_URL_ATOM = Atom.make(
	AsyncResult.initial<never, never>(false),
).pipe(Atom.withLabel("asset-url:empty"));
...
export function assetUrlStateFromResult(
	...
	if (httpBaseUrl === null || result._tag !== "Success")
		return { _tag: "Loading" };
...
export function createAssetEnvironmentAtoms<R, E>(
	...
	const execute = Effect.fn("assets.createUrl")(function* (
		...
		const result = yield* request(WS_METHODS.assetsCreateUrl, input).pipe(
			Effect.result,
		);
		...
		if (
			...
			mediaMimeTypeFromExtension(
				resource.path.slice(resource.path.lastIndexOf(".")),
			) === null
		)
			return yield* error;
	...
	return {
		...
		createUrls: (target: {
			...
			createUrlsFamily(
				JSON.stringify([target.environmentId, target.resources]),
			),
	};
}
	...
	const family = Atom.family((key: string) => {
		...
		const resolvedUrl = Atom.make((get): string | null => {
			...
			return state._tag === "Success"
				? state.url
				: Option.getOrNull(get.self<string | null>());
