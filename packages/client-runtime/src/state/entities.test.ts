...
function makeHarness(
	...
	const threadDetails = createEnvironmentThreadDetailAtoms(
	...
	return {
		...
		threadStateAtom: (threadId: ThreadId) =>
			threadStateAtoms(`${ENVIRONMENT_ID}\u0000${threadId}`),
		projects,
		threadShells,
		threadDetails,
	};
...
describe("environment entity projections", () => {
	...
	it("preserves untouched project and thread identities across unrelated shell updates", () => {
		...
		const projectRefsAtom =
			harness.projects.environmentProjectRefsAtom(ENVIRONMENT_ID);
		const threadRefsAtom =
			...
			AsyncResult.success(
				shellState({
					...
					threads: SNAPSHOT.threads.map((thread) =>
						thread.id === OTHER_THREAD_ID
							? { ...thread, title: "Updated elsewhere" }
							: thread,
					),
				}),
			),
	...
	it("keeps scoped identities and list order across project and environment changes", () => {
		...
		try {
			...
			const local = harness.registry.get(
				harness.threadShells.threadShellAtom(localRef),
			);
			const remote = harness.registry.get(
				harness.threadShells.threadShellAtom(remoteRef),
			);
			...
			expect(harness.registry.get(selectedAtom)).toEqual([
				remote,
				original[1],
				local,
			]);
			...
			harness.registry.set(
				...
				AsyncResult.success(
					shellState({ ...SNAPSHOT, threads: SNAPSHOT.threads.toReversed() }),
				),
			);
			expect([
				...harness.registry
					.get(
						harness.threadShells.environmentThreadRefsByProjectAtom(
							remoteEnvironmentId,
						),
					)
					.keys(),
			...
			const moved = harness.registry.get(
				harness.threadShells.threadShellAtom(localRef),
			);
			expect(harness.registry.get(selectedAtom)).toEqual([
				remote,
			...
			harness.registry.set(harness.catalogValueAtom, {
				...catalog,
				entries: new Map([
					[remoteEnvironmentId, catalog.entries.get(remoteEnvironmentId)!],
				]),
			});
			...
			expect(harness.registry.get(listAtom)).toEqual([]);
			expect(
				harness.registry.get(harness.threadShells.threadShellAtom(remoteRef)),
			).toBeNull();
		} finally {
			disposeSelected();
			disposeList();
	...
	it("updates only the requested thread detail and preserves untouched field identities", () => {
		...
		const otherThreadDetailAtom =
			harness.threadDetails.detailAtom(otherThreadRef);
