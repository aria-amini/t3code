...
function makeHarness(
	...
	const authorizations: Array<
		Parameters<
			RemoteEnvironmentAuthorization["Service"]["authorizeDpopHttp"]
		>[0]
	> = [];
	const proofs: Array<ManagedRelayDpopProofInput> = [];
	const remoteAuthorization = RemoteEnvironmentAuthorization.of({
		authorizeBearer: () =>
			Effect.die("Unexpected bearer connection preparation."),
		authorizeDpop: () =>
...
const LOADERS: ReadonlyArray<{
...
}> = [
	{
		...
		load: (input: HttpInput) =>
			fetchEnvironmentPullRequestDiff({ ...input, diff: DIFF }),
	},
	{
...
describe("authenticated environment HTTP requests", () => {
	it.effect.each(LOADERS)(
		"uses current relay authorization and endpoint for $name",
		(loader) =>
			Effect.gen(function* () {
				...
				expect(PREPARED.httpAuthorization).toMatchObject({
					accessToken: "expired-token",
				});
			}),
	);
	...
	it.effect(
		"retries a rejected diff once with a new token, endpoint, and proof",
		() =>
			Effect.gen(function* () {
				const harness = makeHarness((requestNumber) =>
					requestNumber === 1
						? credentialRejectedResponse()
						: Response.json(DIFF_RESULT),
				);
				...
				expect(
					harness.calls.map((call) =>
						new Headers(call.init.headers).get("dpop"),
					),
				...
				expect(harness.calls[1]!.init.body).toEqual(
					harness.calls[0]!.init.body,
				);
			}),
	);

	it.effect(
		"uses the authorization service captured by the diff loader layer",
		() =>
			Effect.gen(function* () {
				...
				const loader = yield* PullRequestDiffLoader.pipe(
				...
				).toBe("DPoP current-token");
			}),
	);

	it.effect(
		"preserves the credential rejection after the one recovery attempt fails",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"recovers a session's unauthenticated 200 response before checking permissions",
		() =>
			Effect.gen(function* () {
				const harness = makeHarness((requestNumber) =>
					Response.json(
						requestNumber === 1 ? UNAUTHENTICATED_SESSION : SESSION,
					),
				);
	...
	it.effect(
		"reports persistent session rejection instead of showing missing permissions",
		() =>
			Effect.gen(function* () {
				const harness = makeHarness(() =>
					Response.json(UNAUTHENTICATED_SESSION),
				);
				...
				expect(error).toMatchObject({
					...
					message:
						"The environment rejected the renewed session authorization.",
				});
				expect(harness.calls).toHaveLength(2);
			}),
	);
	...
	it.effect.each([
		...
		{
			name: "bearer",
			authorization: { _tag: "Bearer", token: "bearer-token" },
		},
		...
		({ authorization }) =>
			Effect.gen(function* () {
				...
				expect(
					new Headers(harness.calls[0]!.init.headers).get("authorization"),
				).toBe(authorization === null ? null : "Bearer bearer-token");
	...
	it.effect(
		"reports the current endpoint when the total timeout expires during HTTP",
		() =>
			Effect.gen(function* () {
				...
				const remoteAuthorization = RemoteEnvironmentAuthorization.of({
					...
					authorizeDpopHttp: (input) =>
						Deferred.succeed(authorizing, undefined).pipe(
							...
							Effect.andThen(
								harness.remoteAuthorization.authorizeDpopHttp(input),
							),
						),
				});
				const pending = yield* fetchEnvironmentSessionState({
				...
				expect(harness.calls.map((call) => call.url)).toEqual([
					`${CURRENT_ORIGIN}/api/auth/session`,
				]);
	...
	it.effect(
		"reports renewal failure without sending the expired prepared token",
		() =>
			Effect.gen(function* () {
				const harness = makeHarness(() => Response.json(SESSION));
	...
	it.effect(
		"does not fall back to a captured DPoP token when authorization is unavailable",
		() =>
			Effect.gen(function* () {
				const harness = makeHarness(() => Response.json(SESSION));
				const error = yield* fetchEnvironmentSessionState({
				...
				expect(error).toMatchObject({
					_tag: "RemoteEnvironmentAuthFetchError",
					message:
						"No relay authorization service is available for the environment request.",
				});
				expect(harness.calls).toEqual([]);
			}),
	);
