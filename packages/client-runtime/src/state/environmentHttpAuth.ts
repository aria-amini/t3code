	...
	Effect.gen(function* () {
		...
		if (Option.isNone(signer)) {
			return yield* new RemoteEnvironmentAuthFetchError({
				message:
					"No DPoP signer is available to authorize the environment request.",
				...
				Effect.mapError(
					(cause) =>
						new RemoteEnvironmentAuthFetchError({
							message:
								"Could not create the environment request authorization proof.",
							cause,
						}),
				),
			);
...
export const executeAuthenticatedEnvironmentHttpRequest = Effect.fn(
	...
	readonly request: (input: {
		readonly client: Effect.Success<
