...
describe("filesystem browse model", () => {
	it("derives the browse target and navigation state", () => {
		...
		expect(getFilesystemBrowsePath("~/projects/", "", false).isBrowsing).toBe(
			false,
		);
	});
	...
	it("filters names, hidden directories, and exact matches consistently", () => {
		...
		expect(filterFilesystemBrowseEntries(entries, "").visibleEntries).toEqual(
			entries.slice(1),
		);
		expect(filterFilesystemBrowseEntries(entries, ".").visibleEntries).toEqual(
			entries.slice(0, 1),
		);
		expect(filterFilesystemBrowseEntries(entries, "Code").exactEntry).toEqual(
			entries[1],
		);
	});
});
...
describe("browse navigation", () => {
	it("only commits the latest valid navigation", async () => {
		...
		const invalidatedRun = navigation.run(
			() => invalidated.promise,
			commit("stale"),
		);
