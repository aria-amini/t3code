...
const EMPTY_PROJECT_INDEX: ReadonlyMap<ProjectId, OrchestrationProjectShell> =
	new Map();
...
export function createEnvironmentProjectAtoms(input: {
	...
	const environmentProjectIndexAtom = Atom.family(
		(environmentId: EnvironmentId) =>
	...
	const environmentProjectRefsAtom = Atom.family(
		(environmentId: EnvironmentId) => {
			let previous: ReadonlyArray<ScopedProjectRef> = [];
			return Atom.make((get) => {
				const next = get(environmentProjectsAtom(environmentId)).map(
					(project) => ({
						environmentId,
	...
	const projectAtomFamily = Atom.family((key: string) => {
		...
		return Atom.make((get) => {
			const source =
				get(environmentProjectIndexAtom(ref.environmentId)).get(
					ref.projectId,
				) ?? null;
			...
			previousValue =
				source === null ? null : scopeProject(ref.environmentId, source);
			return previousValue;
