...
describe("load balancing shared project machines", () => {
	...
	it("compares three machines using free capacity and preference", () => {
		const candidates = [
			{
				environmentId: "busy",
				resources: { ...resources, cpuUtilization: 0.9 },
				weight: 1,
			},
			{ environmentId: "idle", resources, weight: 1 },
			{
				environmentId: "preferred",
				resources: { ...resources, cpuCount: 4 },
				weight: 3,
			},
		];
		expect(chooseLoadBalancedEnvironment(candidates, now)).toBe("preferred");
		expect(chooseLoadBalancedEnvironment(candidates.slice(0, 2), now)).toBe(
			"idle",
		);
	});
	...
	it("uses client receipt time when host clocks differ", () => {
		...
		expect(chooseLoadBalancedEnvironment([candidate], now)).toBe(
			"different-clock",
		);
...
describe("buildProjectGroups", () => {
	...
	it("uses a shared custom title as the repository group's label", () => {
		...
		expect(
			buildProjectGroups({ projects, settings: settings("repository") })[0]
				?.label,
		).toBe("Custom project");
	});
	...
	it("applies a physical-project override without dropping its siblings", () => {
		...
		expect(
			groups.flatMap((group) =>
				group.members.map((member) => member.project.id),
			),
		).toEqual(["t3code", "t3code-3", "t3code-2"]);
	});
