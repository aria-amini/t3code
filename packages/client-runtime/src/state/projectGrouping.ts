...
export interface ProjectGroupingSettings {
	...
	readonly sidebarProjectGroupingOverrides: Record<
		string,
		SidebarProjectGroupingMode
	>;
}
...
export function selectProjectGroupingSettings(
...
function deriveRepositoryRelativeProjectPath(
	...
	const normalizedProjectPath = normalizeProjectPathForComparison(
...
export function deriveProjectGroupLabel(input: {
	readonly representative: Pick<
		EnvironmentProject,
		"title" | "repositoryIdentity"
	>;
	readonly members: ReadonlyArray<
		Pick<EnvironmentProject, "title" | "repositoryIdentity">
	>;
}): string {
	const sharedTitles = uniqueNonEmptyValues(
		input.members.map((member) => member.title),
	);
...
function shouldReplacePhysicalProjectWinner<
	TProject extends EnvironmentProject,
>(existing: TProject, candidate: TProject): boolean {
	const freshnessDelta =
		projectFreshnessTime(candidate) - projectFreshnessTime(existing);
	return (
		freshnessDelta > 0 || (freshnessDelta === 0 && candidate.id > existing.id)
	);
}
...
export function buildProjectGroups<TProject extends EnvironmentProject>(input: {
	...
	for (const [physicalProjectKey, physicalProjects] of projectsByPhysicalKey) {
		const winner = physicalProjects.reduce((current, candidate) =>
			shouldReplacePhysicalProjectWinner(current, candidate)
				? candidate
				: current,
		);
		const identitySource = selectProjectIdentitySource(
			physicalProjects,
			winner,
		);
	...
	return Array.from(groupedMembers, ([key, members]) => {
		const representative =
			(preferredEnvironmentId
				? members.find(
						(member) => member.project.environmentId === preferredEnvironmentId,
					)?.project
