		...
		Effect.gen(function* () {
			...
			const client = {
				[WS_METHODS.pullRequestsSubscribeRefreshes]: () =>
					Stream.fromPubSub(refreshEvents),
			...
			const initial = yield* Effect.promise(() =>
				executeAtomQuery(registry, activity),
			);
			...
			expect(
				(yield* AtomRegistry.getResult(registry, activity, {
			...
			const stop = registry.subscribe(activity, (result) => {
				if (
					...
					result.value.comments[0]?.body === "after turn"
				) {
