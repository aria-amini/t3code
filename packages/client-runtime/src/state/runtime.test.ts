...
class TestQueryError extends Schema.TaggedErrorClass<TestQueryError>()(
	"TestQueryError",
	{
...
const makeEnvironmentQueryHarness = Effect.fn(
	"TestEnvironmentQuery.makeHarness",
)(function* <A, E>(execute: Effect.Effect<A, E>) {
	const supervisorState = yield* SubscriptionRef.make(queryConnectionState());
	const supervisorSession = yield* SubscriptionRef.make(
		Option.some(QUERY_RPC_SESSION),
	);
...
const mountEnvironmentQuery = Effect.fn("TestEnvironmentQuery.mount")(
	function* <A, E>(atom: Atom.Atom<AsyncResult.AsyncResult<A, E>>) {
...
describe("settleAsyncResult", () => {
	it("preserves successful values and typed failures", async () => {
		const success = await settleAsyncResult(() =>
			Promise.resolve(Exit.succeed("done")),
		);
		...
		const failure = await settleAsyncResult(() =>
			Promise.resolve(Exit.fail(expectedFailure)),
		);
	...
	it("encodes thrown and rejected promises as defects", async () => {
		...
		const rejected = await settleAsyncResult<void, never>(() =>
			Promise.reject(rejectedDefect),
		);
...
describe("atom command result helpers", () => {
	...
	it("reports expected failures and defects through separate policies", async () => {
		...
		await executeAtomCommand(
			() => Promise.resolve(Exit.fail("nope")),
			{ label: "save" },
			reporter,
		);
		await executeAtomCommand(
...
describe("environmentRpcKey", () => {
	it("isolates subscription state by environment and cwd", () => {
		...
		expect(environmentRpcKey(originalTarget)).not.toBe(
			environmentRpcKey(nextTarget),
		);
		expect(environmentRpcKey(originalTarget)).toBe(
...
describe("environment query lifecycle", () => {
	...
	it.effect.each([
		...
		"settles as unavailable $condition",
	...
	it.effect("keeps a genuine query failure settled while reconnecting", () =>
		Effect.scoped(
			Effect.gen(function* () {
				const expectedFailure = new TestQueryError({
...
describe("Atom.fn mutation semantics", () => {
	it.effect(
		"interrupts the previous invocation when the same mutation atom is written again",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"keeps stream mutations waiting until the final emitted value",
		() =>
			Effect.gen(function* () {
				...
				const mutation = Atom.fn(() =>
					Stream.make("progress").pipe(
						Stream.concat(
							Stream.fromEffect(completionLatch.await.pipe(Effect.as("done"))),
						),
					),
				);
				const registry = AtomRegistry.make();
...
describe("runtime command runner", () => {
	...
	it.effect(
		"releases a shared scheduler lane before the outer command finishes",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"keeps an outer reconnect observer alive after a scheduled handoff returns",
		() =>
			Effect.gen(function* () {
							...
							yield* Deferred.await(commitStarting).pipe(
								Effect.andThen(
									Stream.fromQueue(states).pipe(
										Stream.tap(() =>
											Deferred.succeed(observerArmed, undefined),
										),
				...
				expect(
					yield* Effect.promise(() => command.run(registry, undefined)),
				).toMatchObject({
					_tag: "Success",
				});
				registry.dispose();
