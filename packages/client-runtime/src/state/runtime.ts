...
export type SettledAsyncResult<A, E> =
	| AsyncResult.Success<A, E>
	| AsyncResult.Failure<A, E>;

export type AtomCommandResult<A, E> = SettledAsyncResult<A, E>;

export type AtomCommandSuccess<R> =
	R extends AtomCommandResult<infer A, infer _E> ? A : never;

export type AtomCommandFailure<R> =
	R extends AtomCommandResult<infer _A, infer E> ? E : never;
...
interface AtomCommandLatestBatch {
	...
	readonly resolve: Array<
		(result: AtomCommandResult<unknown, unknown>) => void
	>;
}
	...
	return {
		schedule: <W, A, E>(
			...
			if (concurrency.mode === "serial") {
				...
				const current =
					previous === undefined ? execute() : previous.then(execute, execute);
			...
			const result = new Promise<AtomCommandResult<A, E>>((resolve) => {
				...
				activeLane.pending.execute = execute as () => Promise<
					AtomCommandResult<unknown, unknown>
				>;
...
export async function runAtomCommand<W, A, E>(
	...
	reportAtomCommandResult(
		result,
		{ ...options, label: options.label ?? command.label },
		reporter,
	);
	return result;
}
...
export function isAtomCommandInterrupted(
						...
						return Effect.fail(
							new EnvironmentRpcUnavailableError({
								...
								message: `Environment ${target.environmentId} is ${
									connectionState.phase === "available"
										? "not connected"
...
>(
	...
	return createEnvironmentQueryAtomFamily(runtime, {
		...
		...(options.refreshTrigger === undefined
			? {}
