...
describe("update restart reconnect nudges", () => {
		...
		Effect.gen(function* () {
			const readyEvents =
				yield* Queue.unbounded<
					Parameters<typeof matchesServerUpdateReadyEvent>[1]
				>();
	...
	it.effect(
		"observes a fast reconnect even when the caller awaits it later",
		() =>
			Effect.gen(function* () {
				const states = yield* Queue.unbounded<{ readonly phase: string }>();
				const reconnected = yield* waitForNextEnvironmentReconnect(
	...
	it.effect(
		"retries rejected credentials only while the update restart is in progress",
		() =>
			Effect.gen(function* () {
				const retries = yield* Ref.make(0);
...
describe("server state projection", () => {
	it("only treats a legacy transport interruption as an unacknowledged handoff", () => {
		...
		expect(
			isLegacyUpdateHandoffLoss(Cause.fail(new Error("Install failed."))),
		).toBe(false);
	});

	it.effect(
		"resumes after the progress stream disconnects following completion",
		() => {
			const result = {
	...
	it("keeps active update state and hides stale failures after a version change", () => {
		...
		expect(serverUpdateStateForServerVersion(failed, "0.0.31")).toEqual({
			status: "idle",
		});
	});

	it.effect(
		"correlates launcher outcomes and fails immediately after rollback",
		() =>
			Effect.gen(function* () {
				const result = {
					targetVersion: "0.0.31",
				...
				const ready = (status: "committed" | "rolled-back") =>
					({
						...
						payload: {
							...
							environment: {
								serverVersion: status === "committed" ? "0.0.31" : "0.0.30",
							},
							updateOutcome: {
								id: "update-1",
								fromVersion: "0.0.30",
				...
				expect(matchesServerUpdateReadyEvent(result, ready("committed"))).toBe(
					true,
				);
	...
	it("carries published environment themes in and out of the projected snapshot", () => {
		...
		expect(Option.getOrThrow(published).config.environmentThemes).toEqual(
			themes,
		);
		...
		expect(
			Option.getOrThrow(unpublished).config.environmentThemes,
		).toBeUndefined();
	});
	...
	it("keeps published themes across a reconnect snapshot", () => {
		...
		expect(Option.getOrThrow(withThemes).config.environmentThemes).toEqual(
			themes,
		);
		...
		expect(Option.getOrThrow(afterReconnect).config.environmentThemes).toEqual(
			themes,
		);
		...
		expect(
			Option.getOrThrow(downgraded).config.environmentThemes,
		).toBeUndefined();
	});
	...
	it("keeps a current welcome on ready and rejects a buffered welcome from the old session", () => {
		...
		const afterBufferedOldWelcome = applyServerWelcomeEvent(
			afterSwitch,
			firstSession,
			{
				type: "welcome",
	...
	it.effect(
		"checks the authoritative session before accepting a buffered welcome",
		() =>
			Effect.gen(function* () {
				...
				const firstClient = {
					[WS_METHODS.subscribeServerLifecycle]: () =>
						Stream.fromEffect(
							Deferred.succeed(firstSubscribed, undefined),
						).pipe(Stream.drain, Stream.concat(Stream.fromQueue(firstEvents))),
				...
				yield* Effect.scoped(
					Effect.gen(function* () {
						...
						const handled = yield* SubscriptionRef.changes(state).pipe(
							Stream.filter(
								(value) =>
									value.currentSession === secondSession ||
									value.welcome === staleWelcome,
							),
							Stream.runHead,
	...
	it.effect(
		"reads the authoritative session after waiting for the welcome state lock",
		() =>
			Effect.gen(function* () {
				const firstSubscribed = yield* Deferred.make<void>();
				const firstClient = {
				...
				yield* Effect.scoped(
					Effect.gen(function* () {
						...
						expect((yield* Fiber.join(changed)).currentSession).toBe(
							thirdSession,
						);
					}),
				);
	...
	it.effect(
		"clears a welcome until the reconnected session sends its own",
		() =>
			Effect.gen(function* () {
				const firstEvents = yield* Queue.unbounded<{
					readonly type: "welcome" | "ready";
				...
				yield* Effect.scoped(
					Effect.gen(function* () {
						...
						const nextResolved = (
							predicate: (
								value: ServerLifecycleWelcomePayload | null,
							) => boolean,
						) =>
						...
						const first = yield* nextResolved(
							(value) => value === firstWelcome,
						).pipe(Effect.forkChild);
						yield* Queue.offer(firstEvents, {
							type: "welcome",
							payload: firstWelcome,
						});
						expect(yield* Fiber.join(first)).toBe(firstWelcome);

						const cleared = yield* nextResolved((value) => value === null).pipe(
							Effect.forkChild,
						);
						yield* SubscriptionRef.set(
							supervisorSession,
						...
						expect(
							resolveServerWelcomeState(yield* SubscriptionRef.get(state)),
						).toBeNull();

						const second = yield* nextResolved(
							(value) => value === secondWelcome,
						).pipe(Effect.forkChild);
						yield* Queue.offer(secondEvents, {
							type: "welcome",
							payload: secondWelcome,
						});
	...
	it.effect(
		"starts from cached configuration and persists the live projection",
		() =>
			Effect.gen(function* () {
				...
				const cache = Persistence.EnvironmentCacheStore.of({
					...
					saveServerConfig: (_environmentId, config) =>
						Queue.offer(savedConfigs, config),
				...
				yield* Effect.scoped(
					Effect.gen(function* () {
						...
						expect(
							Option.getOrThrow(yield* SubscriptionRef.get(state)).config,
						).toBe(CONFIG);
						...
						const projected = yield* SubscriptionRef.changes(state).pipe(
							Stream.filter((value) =>
								Option.match(value, {
									onNone: () => false,
									onSome: (projection) =>
										projection.latestEvent.type === "providerStatuses",
								}),
							),
							Stream.runHead,
						);
						expect(
							Option.getOrThrow(Option.getOrThrow(projected)).config.providers,
						).toBe(providers);
					}),
				);
	...
	it.effect(
		"does not rewrite cached configuration when no live update arrives",
		() =>
			Effect.gen(function* () {
				const client = {
