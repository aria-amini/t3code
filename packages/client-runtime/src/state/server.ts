...
const EMPTY_SERVER_UPDATE_STATE_ATOM = Atom.make<ServerUpdateState>(
...
export function nudgeReconnectDuringUpdateRestart(input: {
	...
	return input.stateChanges.pipe(
		...
		Stream.runForEach(() =>
			Effect.sleep(input.interval ?? Duration.seconds(1)).pipe(
				Effect.andThen(input.retryNow),
			),
		),
...
export const waitForDesktopUpdateTarget = Effect.fn(
	"waitForDesktopUpdateTarget",
)(function* <EReady, ECommit>(
	...
	EReady | ECommit | ServerUpdateTerminalError
> {
...
export function isLegacyUpdateHandoffLoss(
	...
	return (
		...
		cause.reasons.every(
			(reason) => Cause.isFailReason(reason) && isRpcSocketError(reason.error),
		)
	);
}
...
export function resolveServerUpdateProgressResult<E>(
	...
	E | ServerUpdateProgressIncompleteError
> {
	if (
	...
	return Effect.fail(
		new ServerUpdateProgressIncompleteError({ targetVersion }),
	);
}

const cachedConfigSnapshotEvent = (
	config: ServerConfig,
...
export const makeEnvironmentServerConfigState = Effect.fn(
	"EnvironmentServerConfigState.make",
	...
	const pendingPersistence = yield* Ref.make<Option.Option<ServerConfig>>(
		Option.none(),
	);
	...
	const persist = Effect.fn("EnvironmentServerConfigState.persist")(function* (
		...
		return yield* cache
			.saveServerConfig(environmentId, withoutEnvironmentThemes(config))
			.pipe(
				Effect.as(true),
				Effect.catch((error) =>
					Effect.logWarning(
						"Could not persist cached server configuration.",
					).pipe(
						Effect.annotateLogs({
							environmentId,
							...safeErrorLogAttributes(error),
	...
	const persistPending = Effect.fn(
		"EnvironmentServerConfigState.persistPending",
	)(function* (config: ServerConfig) {
		if (!(yield* persist(config))) {
			return;
		}
		yield* Ref.update(pendingPersistence, (pending) =>
			Option.isSome(pending) && pending.value === config
				? Option.none()
				: pending,
		);
	});
	...
	yield* subscribe(WS_METHODS.subscribeServerConfig, {
		...(subscription.environmentThemes === true
			? { environmentThemes: true }
			: {}),
		...(subscription.usageLimitSources === true
			? { usageLimitSources: true }
			: {}),
		...(subscription.usageLimitsCommand === true
...
export const makeEnvironmentServerWelcomeState = Effect.fn(
	"EnvironmentServerWelcomeState.make",
)(function* () {
	const supervisor = yield* EnvironmentSupervisor;
	const initialSession = Option.getOrNull(
			...
			SubscriptionRef.get(supervisor.session).pipe(
				Effect.map(
					(latestSession) =>
						[
							undefined,
							update(current, Option.getOrNull(latestSession)),
						] as const,
				),
			),
		);
	...
	yield* subscribeDynamicWithSession(
		...
		Effect.fn("EnvironmentServerWelcomeState.makeSubscribeInput")(
			function* (session) {
...
export function createServerEnvironmentAtoms<R, E>(
		...
		runtime
			.atom(
				serverConfigStateChanges(environmentId, {
					...(options.environmentThemes === true
						? { environmentThemes: true }
						: {}),
					...(options.usageLimitSources === true
						? { usageLimitSources: true }
						: {}),
					...(options.usageLimitsCommand === true
						? { usageLimitsCommand: true }
						: {}),
				}),
			)
			.pipe(
				...
				Atom.withLabel(
					`environment-data:server:config-projection:${environmentId}`,
				),
			),
	);
	const configProjection = (target: {
		readonly environmentId: EnvironmentId;
		readonly input: EnvironmentRpcInput<
	...
	const updateStateValueAtom = Atom.family((environmentId: EnvironmentId) =>
		Atom.make((get) =>
			...
			Atom.withLabel(
				`environment-data:server:update-state-value:${environmentId}`,
			),
		),
	);
	const updateStateAtom = (environmentId: EnvironmentId | null) =>
		environmentId === null
			...
			: updateStateValueAtom(environmentId);
	...
	>(runtime, {
		...
		execute: (target, atomRegistry) => {
			...
			let fromVersion =
				atomRegistry.get(configValueAtom(target.environmentId))?.environment
			...
			return Effect.gen(function* () {
				...
				yield* Deferred.await(desktopCommitStarting).pipe(
					Effect.andThen(
						environmentRegistry.stateChanges(target.environmentId).pipe(
							Stream.tap(() =>
								Deferred.succeed(desktopReconnectObserverArmed, undefined),
							),
							Stream.dropWhile((state) => state.phase === "connected"),
							Stream.filter((state) => state.phase === "connected"),
							Stream.runHead,
				...
				const result = yield* scheduleAtomCommandEffect(
					...
					target,
					Effect.gen(function* () {
						const currentConfig = atomRegistry.get(
							configValueAtom(target.environmentId),
						);
						fromVersion =
							currentConfig?.environment.serverVersion ?? targetVersion;
						atomRegistry.set(stateAtom, {
							status: "running",
							stage: currentStage,
							fromVersion,
						...
						const supportsProgress =
							currentConfig?.environment.capabilities
								.serverSelfUpdateProgress === true;
						const updateResult: ServerSelfUpdateResult = supportsProgress
							? yield* Effect.gen(function* () {
									const terminal = yield* Ref.make<
										Option.Option<ServerSelfUpdateResult>
									>(Option.none());
							...
							: yield* Effect.gen(function* () {
									const selfUpdateMethod =
										currentConfig?.environment.capabilities.serverSelfUpdate;
						...
						if (
							...
							const commitExit = yield* environmentRegistry
								...
								.pipe(Effect.exit);
							if (
								Exit.isFailure(commitExit) &&
								!isLegacyUpdateHandoffLoss(commitExit.cause)
							) {
								return yield* Effect.failCause(commitExit.cause);
							}
				...
				const waitForReady = environmentRegistry
					.followStream(
						target.environmentId,
						subscribe(WS_METHODS.subscribeServerLifecycle, {}),
					)
					.pipe(
						Stream.filter(
							(event): event is ServerLifecycleStreamReadyEvent =>
								event.type === "ready" &&
								matchesServerUpdateResumeEvent(result, event),
						),
						Stream.runHead,
						Effect.timeoutOption(SERVER_UPDATE_RESUME_TIMEOUT),
	...
	const settingsValueAtom = Atom.family((environmentId: EnvironmentId) =>
		Atom.make(
			(get) => get(configValueAtom(environmentId))?.settings ?? null,
		).pipe(Atom.withLabel(`environment-data:server:settings:${environmentId}`)),
	);
	const usagePricesAtom = Atom.family((environmentId: EnvironmentId) =>
		Atom.make((get) => {
			const overrides =
				get(settingsValueAtom(environmentId))?.usagePriceOverrides ?? {};
			...
			Atom.withLabel(`environment-data:server:usage-prices:${environmentId}`),
		),
	);
	const providersValueAtom = Atom.family((environmentId: EnvironmentId) =>
		Atom.make(
			(get) => get(configValueAtom(environmentId))?.providers ?? null,
		).pipe(
	...
	const welcomeStateFamily = Atom.family((environmentId: EnvironmentId) =>
		runtime
			...
			.pipe(
				Atom.setIdleTTL(5 * 60_000),
				Atom.withLabel(
					`environment-data:server:welcome-state:${environmentId}`,
				),
			),
	);
	const welcomeFamily = Atom.family((environmentId: EnvironmentId) =>
		Atom.make((get) => {
			...
			return result.value === null
				? AsyncResult.initial<ServerLifecycleWelcomePayload, never>(
						result.waiting,
					)
	...
	return {
		...
		startProviderAuth: createEnvironmentRpcCommand(runtime, {
			...
			concurrency: {
				...
				key: ({ environmentId, input }) =>
					JSON.stringify([environmentId, input]),
			},
		}),
		...
		hostResources: createEnvironmentQueryAtomFamily(runtime, {
			...
			staleTimeMs: 5_000,
			execute: (
				input: EnvironmentRpcInput<typeof WS_METHODS.serverGetHostResources>,
			) =>
				request(WS_METHODS.serverGetHostResources, input).pipe(
