...
export function withoutEnvironmentThemes(config: ServerConfig): ServerConfig {
	if (
		config.environmentThemes === undefined &&
		config.usageLimitSources === undefined
	) {
		return config;
	}
	const {
		environmentThemes: _themes,
		usageLimitSources: _sources,
		...rest
	} = config;
	return rest;
}
	...
	switch (event.type) {
		case "snapshot": {
			...
			return Option.some({
				config: {
					...event.config,
					...(carriedThemes === undefined
						? {}
						: { environmentThemes: carriedThemes }),
					...(carriedSources === undefined
						? {}
						: { usageLimitSources: carriedSources }),
				},
				latestEvent: event,
		...
		case "environmentThemesUpdated":
			return Option.map(current, (projection) => ({
				config: {
					...projection.config,
					environmentThemes:
						event.payload.themes.length > 0 ? event.payload.themes : undefined,
				},
				latestEvent: event,
				source: "live",
			}));
