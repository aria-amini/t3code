...
const makeHarness = Effect.fn("ServerUsageTest.makeHarness")(function* (
	...
	const client = {
		...
		[WS_METHODS.serverGetUsageSummary]: (input: UsageSummaryInput) =>
			Effect.gen(function* () {
				requests += 1;
				const price =
					settings.usagePriceOverrides["custom-model"]
	...
	const updateSettings = Effect.fn("ServerUsageTest.updateSettings")(function* (
		...
		yield* AtomRegistry.toStream(
			registry,
			atoms.settingsValueAtom(TARGET.environmentId),
		).pipe(
	...
	return {
		...
		updateSettings,
		summary: (input = INPUT) =>
			atoms.usageSummary({ environmentId: TARGET.environmentId, input }),
	};
});
...
it.effect(
	"refreshes cached usage windows only when override prices change",
	() =>
		Effect.scoped(
			Effect.gen(function* () {
				const harness = yield* makeHarness();
				const current = harness.summary();
				const previous = harness.summary({
					...INPUT,
