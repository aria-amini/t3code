...
describe("splitSharedServerPatch", () => {
	it.each([
		...
		"shares the text generation model and options, including reset (%j)",
		(selection) => {
			...
			expect(
				pickSharedServerSettings({ ...DEFAULT_SERVER_SETTINGS, ...patch }),
			).toMatchObject(patch);
			const environment = {
				environmentId: boxId,
				label: "Remote Box",
...
describe("pickSharedServerSettings", () => {
	it("returns only the shared keys", () => {
		expect(
			Object.keys(
				pickSharedServerSettings(DEFAULT_SERVER_SETTINGS, restartCapabilities),
			).sort(),
		).toEqual([
...
describe("filterSharedServerPatch", () => {
	...
	it.each(["missing", "disabled", "different-driver", "enabled"] as const)(
		...
		(availability) => {
			...
			const settings = {
				...DEFAULT_SERVER_SETTINGS,
				providerInstances:
					availability === "missing" ? {} : { [instanceId]: instance },
			};
			const patch = {
				sidebarAutoSettleAfterDays: 7,
	...
	it.each([true, false])(
		"preserves supported restart preference %s",
		(enabled) => {
			const patch = {
	...
	it.each([undefined, {}, { threadRestartContinuation: false }])(
		...
		(capabilities) => {
			...
			expect(
				pickSharedServerSettings(DEFAULT_SERVER_SETTINGS, capabilities),
			).not.toHaveProperty("continueThreadsAfterServerUpdate");
		},
	);
});
			...
			environments: [
				{
					...
					settings: {
						...
						defaultThreadEnvMode:
							primarySettings.defaultThreadEnvMode === "local"
								? "worktree"
								: "local",
					},
				},
