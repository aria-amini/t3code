...
export type SharedServerSettingKey =
	(typeof SHARED_SERVER_SETTING_KEYS)[number];
	...
	if (
		...
		(!settings ||
			(instanceId !== undefined &&
				(sourceSettings?.providerInstances[instanceId]?.driver ??
					instanceId) !==
...
export function supportsSharedSettingsSync(environment: {
	...
	return (
		...
		environment.serverConfig?.environment.capabilities.threadAutoSettlement ===
			true
	);
}
