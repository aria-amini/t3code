...
describe("environment shell synchronization", () => {
	it.effect(
		"publishes live state before persistence and preserves it when ready",
		() =>
			Effect.gen(function* () {
				...
				const synchronizing = yield* SubscriptionRef.changes(shellState).pipe(
					Stream.filter(
						(state) =>
							state.status === "synchronizing" && Option.isSome(state.snapshot),
					),
					Stream.runHead,
				);
				expect(
					Option.getOrThrow(Option.getOrThrow(synchronizing).snapshot),
				).toEqual(LIVE_SHELL_SNAPSHOT);
	...
	it.live.each([
		...
		"batches live events with a $bufferSize event buffer",
		({ bufferSize, expectedSequences }) =>
			Effect.gen(function* () {
				const events =
					yield* Queue.bounded<OrchestrationShellStreamItem>(bufferSize);
				const client = {
					[ORCHESTRATION_WS_METHODS.subscribeShell]: () =>
						Stream.fromQueue(events),
				} as unknown as WsRpcProtocolClient;
				...
				const observed = yield* SubscriptionRef.changes(shellState).pipe(
					...
					Stream.takeUntil(
						(state) =>
							...
							state.snapshot.value.threads.length === 50,
					),
					Stream.runCollect,
				...
				const snapshots = states.map((state) =>
					Option.getOrThrow(state.snapshot),
				);
				expect(snapshots.map((snapshot) => snapshot.snapshotSequence)).toEqual(
					expectedSequences,
				);
		...
		Effect.gen(function* () {
			...
			const client = {
				[ORCHESTRATION_WS_METHODS.subscribeShell]: (input: {
					...
					Stream.unwrap(
						Queue.offer(subscribeInputs, input).pipe(
	...
	it.effect(
		"resubscribes from the in-memory shell cursor when the app becomes active",
		() =>
			Effect.gen(function* () {
				const events = yield* Queue.unbounded<OrchestrationShellStreamItem>();
				const wakeups =
				...
				yield* SubscriptionRef.changes(shellState).pipe(
					Stream.filter(
						(value) =>
							...
							value.snapshot.value.snapshotSequence === 40,
					),
					Stream.runHead,
				);
				...
				expect(yield* Ref.get(capturedAfterSequences)).toEqual([
					10, 40, 40, 20,
				]);
