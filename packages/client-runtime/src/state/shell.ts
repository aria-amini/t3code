...
export type EnvironmentShellStatus =
	| "empty"
	| "cached"
	| "synchronizing"
	| "live";
...
const SHELL_SYNCHRONIZATION_ERROR_MESSAGE =
	"Could not synchronize environment data.";

export const makeEnvironmentShellState = Effect.fn(
	"EnvironmentShellState.make",
)(function* () {
	...
	const setStreamError = (error: unknown) =>
		Ref.set(awaitingCompletion, false).pipe(
			Effect.andThen(
				Effect.logWarning("Could not synchronize the environment shell."),
			),
			Effect.annotateLogs({
				environmentId,
				...safeErrorLogAttributes(error),
	...
	yield* Effect.forkScoped(
		subscribeDynamic(
			...
			Effect.fn("EnvironmentShellState.makeSubscribeInput")(
				function* (session) {
...
function shellStateChanges(environmentId: EnvironmentId) {
	return followStreamInEnvironment(
		environmentId,
		Stream.unwrap(
			makeEnvironmentShellState().pipe(Effect.map(SubscriptionRef.changes)),
		),
	);
}
...
const EMPTY_SERVER_CONFIGS: ReadonlyMap<EnvironmentId, ServerConfig> =
	new Map();
...
export function createEnvironmentShellSummaryAtom(input: {
	...
	return Atom.make((get) => {
		...
		for (const environmentId of get(input.catalogValueAtom).entries.keys()) {
			...
			if (
				latestSnapshotUpdatedAt === null ||
				updatedAt > latestSnapshotUpdatedAt
			) {
