...
describe("foldSubagentActivities", () => {
	...
	it("bounds repeated strings at 180 chars and the activity ring at 6 deduped entries", () => {
		...
		const rows = [
			activity("task.started", { taskId: "task-8", taskType: "local_agent" }),
		];
		...
		rows.push(
			activity("task.progress", { taskId: "task-8", summary: `${long}-9` }),
		);
	...
	it("plan tasks are not agents", () => {
		const agents = fold([
			activity("task.started", { taskId: "plan-1", taskType: "plan" }),
		]);
...
describe("deriveAgentPanelModel", () => {
	...
	it("keeps direct spawns in first-seen order as their activity changes", () => {
		const directRoster = fold([
			activity(
				"task.started",
				{ taskId: "direct-a", title: "First" },
				"2026-08-01T11:00:00.000Z",
			),
			activity(
				"task.started",
				{ taskId: "direct-b", title: "Second" },
		...
		expect(
			deriveAgentPanelModel({ agents: directRoster }).directAgents.map(
				(agent) => agent.id,
			),
	...
	it("keeps first-seen order after the roster retention ranking runs", () => {
		...
		const ids = deriveAgentPanelModel({
			agents: cappedRoster,
...
describe("model and effort attribution", () => {
	...
	it("applies metadata-only updates without changing the current status", () => {
		...
		expect(
			formatSubagentModelLabel(waitingAgent.model, waitingAgent.effort),
	...
	it("formatSubagentModelLabel compacts ids and appends effort", () => {
		expect(formatSubagentModelLabel("claude-sonnet-5[1m]", "high")).toBe(
			"sonnet-5[1m] · high",
		);
		expect(formatSubagentModelLabel("claude-opus-4-20250514", null)).toBe(
			"opus-4",
		);
		expect(formatSubagentModelLabel("gpt-5.6-sol", "low")).toBe(
			"gpt-5.6-sol · low",
		);
...
describe("session-derived interruption", () => {
	it("dead session interrupts live agents but preserves idle and settled", () => {
		...
		expect(dead.find((agent) => agent.id === "live-1")?.status).toBe(
			"interrupted",
		);
		expect(dead.find((agent) => agent.id === "idle-1")?.status).toBe("idle");
		expect(dead.find((agent) => agent.id === "done-1")?.status).toBe(
			"completed",
		);
		const alive = foldSubagentActivities(rows, { sessionLive: true });
		expect(alive.find((agent) => agent.id === "live-1")?.status).toBe(
			"running",
		);
	});
});
		...
		const agents = fold([
			...
			activity(
				"task.updated",
				{
					taskId: "te-1",
					status: "completed",
					endedAt: "2026-08-01T10:59:00.000Z",
				},
				"2026-08-01T11:00:00.000Z",
			),
			activity(
...
describe("task type classification is a denylist", () => {
	it("unknown agent-flavored types (local_agent, future names) join the roster", () => {
		const agents = fold([
			...
			activity("task.started", {
				taskId: "a2",
				taskType: "some_future_agent_kind",
				title: "X",
			}),
		]);
