...
function bounded(value: string): string {
	return value.length <= SUMMARY_CHAR_LIMIT
		? value
		: `${value.slice(0, SUMMARY_CHAR_LIMIT - 1)}…`;
}
...
function appendActivity(
	...
	if (
		entries.length > 0 &&
		entries[entries.length - 1]?.summary === boundedSummary
	) {
		return entries;
	}
	const next = [...entries, { at, summary: boundedSummary }];
	return next.length > RECENT_ACTIVITY_LIMIT
		? next.slice(-RECENT_ACTIVITY_LIMIT)
		: next;
}
...
function mergeUsageMax(
	...
	if (reasoningOutputTokens !== undefined)
		merged.reasoningOutputTokens = reasoningOutputTokens;
...
function fillMetadata(
	...
	if (typeof payload.runHandles === "object" && payload.runHandles !== null) {
		...
		if (sessionUrl && /^https?:\/\//i.test(sessionUrl))
			runHandles.sessionUrl = sessionUrl;
...
const TASK_COMPLETED_STATUS: ReadonlyMap<string, RuntimeSubagentStatus> =
	new Map([
		["completed", "completed"],
		["failed", "failed"],
		["stopped", "interrupted"],
...
export function foldSubagentActivities(
	...
	for (const activity of activities) {
		...
		switch (activity.kind) {
			...
			case "task.completed": {
				...
				const status =
					TASK_COMPLETED_STATUS.get(asString(payload.status) ?? "") ??
					"completed";
	...
	for (const agent of agents.values()) {
		...
		for (const member of agents.values()) {
			...
			member.status =
				agent.status === "completed" ? "completed" : "interrupted";
			member.completedAt =
				member.completedAt ?? agent.completedAt ?? agent.updatedAt;
...
export function deriveAgentPanelModel({
	...
	const workflowGroups: AgentPanelWorkflowGroup[] = workflows.map(
		(workflow) => {
				...
				workflow.phases.length > 0
					...
					: (() => {
							...
							for (const member of workflowMembers) {
								if (
									member.phaseIndex !== null &&
									!derived.has(member.phaseIndex)
								) {
									derived.set(
										member.phaseIndex,
			...
			const knownPhaseIndices = new Set(
				knownPhases.map((phase) => phase.index),
			);
			...
			const unphasedMembers = workflowMembers
				.filter(
					(member) =>
						member.phaseIndex === null ||
						!knownPhaseIndices.has(member.phaseIndex),
				)
				.slice()
				.sort((a, b) => (a.agentIndex ?? 0) - (b.agentIndex ?? 0));
