...
export const INITIAL_TERMINAL_OUTPUT_CURSOR =
	Object.freeze<TerminalOutputCursor>({
		generation: -1,
		resetVersion: -1,
		offset: 0,
	});
 ...
 */
function splitStringByUtf8Bytes(
	data: string,
	maxBytes: number,
): ReadonlyArray<Utf8Chunk> {
...
function appendOutput(
	...
	let retainedChunks =
		firstRetainedIndex === 0 ? chunks : chunks.slice(firstRetainedIndex);
...
export function readTerminalOutputUpdate(
	...
	if (
		...
		return {
			type: "reset",
			data: terminalOutputText(output),
			cursor: nextCursor,
		};
	}
	...
	return {
		...
		data: appended
			.map((chunk) =>
				chunk.data.slice(Math.max(0, cursor.offset - chunk.startOffset)),
			)
			.join(""),
		cursor: nextCursor,
	};
