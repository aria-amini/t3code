...
describe("terminal session reducers", () => {
	it("prefers live attach status over stale metadata after the attach stream starts", () => {
		...
		const attached = applyTerminalAttachStreamEvent(
			EMPTY_TERMINAL_BUFFER_STATE,
			{
				type: "error",
				threadId: TARGET.threadId,
	...
	it("uses metadata status before an attach stream has emitted", () => {
		...
		expect(
			combineTerminalSessionState(summary, EMPTY_TERMINAL_BUFFER_STATE).status,
		).toBe("running");
	});
	...
	it("does not treat an idle running shell as a running subprocess", () => {
		...
		expect(
			selectRunningSubprocessTerminalIds([idleSession, activeSession]),
		).toEqual(["term-2"]);
	});
	...
	it("preserves a BOM code point at a new output chunk boundary", () => {
		...
		const cursor = readTerminalOutputUpdate(
			initial.output,
			INITIAL_TERMINAL_OUTPUT_CURSOR,
		).cursor;
		...
		expect(state.output.retainedBytes).toBe(
	...
	it("preserves a leading BOM in the retained snapshot tail", () => {
		const state = applyTerminalAttachStreamEvent(
			...
			{
				type: "snapshot",
				snapshot: { ...BASE_SNAPSHOT, history: "discard\uFEFFtail" },
			},
			7,
		);
	...
	it("trims whole Unicode code points from a partially retained chunk", () => {
		...
		let cursor = readTerminalOutputUpdate(
			state.output,
			INITIAL_TERMINAL_OUTPUT_CURSOR,
		).cursor;
	...
	it("delivers all output when several events arrive before a renderer reads", () => {
		...
		expect(readTerminalOutputUpdate(state.output, update.cursor).type).toBe(
			"none",
		);
	});
	...
	it("preserves the byte-limited tail and resets a cursor before a partially trimmed chunk", () => {
		...
		const caughtUpCursor = readTerminalOutputUpdate(
			first.output,
			staleCursor,
		).cursor;
		const state = applyTerminalAttachStreamEvent(
		...
		expect(
			readTerminalOutputUpdate(state.output, caughtUpCursor),
		).toMatchObject({
			type: "append",
			data: " world",
		});
	...
	it("does not encode retained history again when appending at the byte limit", () => {
		let state = applyTerminalAttachStreamEvent(EMPTY_TERMINAL_BUFFER_STATE, {
			...
			snapshot: {
				...BASE_SNAPSHOT,
				history: "x".repeat(DEFAULT_MAX_TERMINAL_BUFFER_BYTES),
			},
		});
		...
		try {
			...
			expect(
				encode.mock.calls.reduce(
					(total, [text]) => total + (text?.length ?? 0),
					0,
				),
			).toBe(data.length * 100);
			expect(state.output.retainedBytes).toBe(
				DEFAULT_MAX_TERMINAL_BUFFER_BYTES,
			);
		} finally {
	...
	it.each([0, -1])(
		"discards output without encoding when the byte budget is %s",
		(maxBytes) => {
			const initial = applyTerminalAttachStreamEvent(
				EMPTY_TERMINAL_BUFFER_STATE,
				{
			...
			try {
				...
				expect(discarded.output.nextOffset).toBe(
					initial.output.nextOffset + data.length,
				);
				expect(discarded.output.retainedBytes).toBe(0);
				expect(
					readTerminalOutputUpdate(discarded.output, cursor),
				).toMatchObject({
					type: "reset",
					data: "",
				});
	...
	it("resets for repeated snapshots, clear, and restart even when output text repeats", () => {
		...
		expect(
			readTerminalOutputUpdate(state.output, cleared.cursor),
		).toMatchObject({
			type: "reset",
			data: "hello",
		});
	...
	it("keeps appending while compacting metadata for many small writes", () => {
		...
		for (let index = 0; index < 2500; index += 1) {
			...
			if (update.type !== "append")
				throw new Error(`Expected append, received ${update.type}`);
	...
	it("appends every unread character across a compaction boundary", () => {
		...
		for (const [index, data] of writes.entries()) {
			...
			if (index === 99)
				cursor = readTerminalOutputUpdate(state.output, cursor).cursor;
		}
		expect(readTerminalOutputUpdate(state.output, cursor)).toMatchObject({
			type: "append",
