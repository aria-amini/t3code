...
describe("parseCodexFeedbackCommand", () => {
	...
	it("preserves a feedback description", () => {
		expect(
			parseCodexFeedbackCommand("/feedback The agent stopped early."),
		).toEqual({
...
describe("submitCodexFeedback", () => {
	...
	it("reports upload progress and clears the draft before the upload finishes", async () => {
		...
		let finishUpload:
			| ((
					result: ReturnType<
						typeof AsyncResult.success<{ feedbackId: string }>
					>,
			  ) => void)
			| undefined;
		const states: CodexFeedbackSubmission[] = [];
		const upload = new Promise<
			ReturnType<typeof AsyncResult.success<{ feedbackId: string }>>
		>((resolve) => {
		...
		expect(codexFeedbackNotice(states.at(-1)!)?.description).toContain(
			"codex-thread-1",
		);
	});
	...
	it("records a failed upload without losing its user-facing error", async () => {
		...
		await submitCodexFeedback({
			...
			upload: () =>
				Promise.resolve(
					AsyncResult.failure<{ feedbackId: string }, Error>(Cause.fail(error)),
				),
		});
		...
		expect(codexFeedbackNotice(states.at(-1)!)?.description).toBe(
			"Upload rejected.",
		);
	});
	...
	it("marks interruptions without reporting them as upload failures", async () => {
		...
		await submitCodexFeedback({
			...
			upload: () =>
				Promise.resolve(
					AsyncResult.failure<{ feedbackId: string }, never>(
						Cause.interrupt(1),
					),
				),
		});
	...
	it("lets another feedback submission finish while the first remains in flight", async () => {
		...
		const second = await submitCodexFeedback({
			...
			upload: () =>
				Promise.resolve(AsyncResult.success({ feedbackId: "codex-thread-2" })),
		});
