...
const proposedPlanOrder = O.combine<
	...
	switch (event.type) {
		...
		case "thread.reverted": {
			...
			const retainedTurnIds = new Set(
				Arr.map(checkpoints, (entry) => entry.turnId),
			);
		...
		case "thread.activity-appended": {
			...
			activityIdIndex.set(
				activities,
				new Set(activities.map((entry) => entry.id)),
			);
...
function reuseLatestTurn(
	...
	return previous.turnId === next.turnId &&
		...
		previous.sourceProposedPlan?.threadId ===
			next.sourceProposedPlan?.threadId &&
