...
const EMPTY_THREAD_INDEX: ReadonlyMap<ThreadId, OrchestrationThreadShell> =
	new Map();
...
export function createEnvironmentThreadShellAtoms(input: {
	...
	const environmentThreadIndexAtom = Atom.family(
		(environmentId: EnvironmentId) =>
	...
	const environmentThreadRefsAtom = Atom.family(
		(environmentId: EnvironmentId) => {
			let previous: ReadonlyArray<ScopedThreadRef> = [];
			return Atom.make((get) => {
				const next = get(environmentThreadsAtom(environmentId)).map(
					(thread) => ({
						environmentId,
						threadId: thread.id,
					}),
	...
	const environmentThreadRefsByProjectAtom = Atom.family(
		(environmentId: EnvironmentId) => {
			...
			return Atom.make((get) => {
				...
				for (const [projectId, refs] of grouped) {
					...
					next.set(
						projectId,
						previousRefs !== undefined && threadRefsEqual(previousRefs, refs)
							? previousRefs
							: refs,
					);
				}
				...
				if (
					...
					[...next].every(
						([projectId, refs], index) =>
							previousProjectIds[index] === projectId &&
							previous.get(projectId) === refs,
					)
				) {
					return previous;
				}
				...
				Atom.withLabel(`environment-thread-refs-by-project:${environmentId}`),
			);
		},
	);
	...
	const threadShellsForProjectRefsAtomFamily = Atom.family((key: string) => {
		...
		return Atom.make((get) => {
			...
			for (const projectRef of projectRefs) {
				...
				const threads = get(
					environmentThreadIndexAtom(projectRef.environmentId),
				);
	...
	return {
		...
		threadShellAtom: (ref: ScopedThreadRef) =>
			threadShellAtomFamily(threadKey(ref)),
	};
}
