...
function makeShell(input: {
	...
	return {
		...
		snoozedAt:
			input.snoozedAt ?? (input.snoozedUntil != null ? SNOOZED_AT : null),
...
function makeQueuedTurnShell(
	overrides: Partial<QueuedTurnShell> = {},
): QueuedTurnShell {
	return {
...
describe("effectiveSnoozed", () => {
	it("hides a thread whose wake time is in the future", () => {
		expect(
			effectiveSnoozed(makeShell({ snoozedUntil: FUTURE_WAKE }), { now: NOW }),
		).toBe(true);
	});

	it("stops classifying as snoozed once the wake time passes (timer wake, no event)", () => {
		expect(
			effectiveSnoozed(makeShell({ snoozedUntil: PAST_WAKE }), { now: NOW }),
		).toBe(false);
	});
	...
	it("never hides on malformed wake data", () => {
		expect(
			effectiveSnoozed(makeShell({ snoozedUntil: "not-a-date" }), { now: NOW }),
		).toBe(false);
	});
	...
	it("wakes early when the agent is blocked on the user", () => {
		expect(
			effectiveSnoozed(
				makeShell({ snoozedUntil: FUTURE_WAKE, pending: "approval" }),
				{
					now: NOW,
				},
			),
		...
		expect(
			effectiveSnoozed(
				makeShell({ snoozedUntil: FUTURE_WAKE, pending: "user-input" }),
				{
					now: NOW,
				},
			),
	...
	it("stays snoozed while the session keeps working — snooze never pauses the agent", () => {
		expect(
			effectiveSnoozed(
				makeShell({ snoozedUntil: FUTURE_WAKE, sessionStatus: "running" }),
				{
					now: NOW,
				},
			),
...
describe("threadRaisedHandWhileSnoozed", () => {
	it("is false for a quiet snoozed thread", () => {
		expect(
			threadRaisedHandWhileSnoozed(makeShell({ snoozedUntil: FUTURE_WAKE })),
		).toBe(false);
	});
...
describe("canSnooze", () => {
	it("allows snoozing quiet and working threads alike", () => {
		expect(
			canSnooze({ ...makeShell({}), latestUserMessageAt: null }, { now: NOW }),
		).toBe(true);
		expect(
			canSnooze(
				{
					...makeShell({ sessionStatus: "running" }),
					latestUserMessageAt: null,
				},
				{ now: NOW },
			),
	...
	it("refuses blocked-on-you work", () => {
		expect(
			canSnooze(
				{ ...makeShell({ pending: "approval" }), latestUserMessageAt: null },
				{ now: NOW },
			),
		).toBe(false);
		expect(
...
describe("threadWokeAt", () => {
	it("is null for never-snoozed and still-snoozed threads", () => {
		...
		expect(
			threadWokeAt(makeShell({ snoozedUntil: FUTURE_WAKE }), { now: NOW }),
		).toBe(null);
	});

	it("reports the wake time for a timer wake", () => {
		expect(
			threadWokeAt(makeShell({ snoozedUntil: PAST_WAKE }), { now: NOW }),
		).toBe(PAST_WAKE);
	});
...
describe("snoozeWakeLabel", () => {
	...
	it("never reads zero or negative while still snoozed", () => {
		...
		expect(snoozeWakeLabel("2026-06-02T09:00:00.000Z", { now: "bad" })).toBe(
			"now",
		);
	});
});
...
describe("resolveSnoozePresets", () => {
	it("offers the shared desktop and mobile choices", () => {
		...
		).toBe(localDate(2026, 4, 8, 13).toISOString());
		expect(presets.find((preset) => preset.id === "three-hours")?.label).toBe(
			"In 3 hours",
		);
		expect(presets.find((preset) => preset.id === "evening")?.label).toBe(
			"This evening",
		);
		expect(
			new Date(
	...
	it("drops the evening choice once evening is near or past", () => {
		expect(
			resolveSnoozePresets(localDate(2026, 4, 8, 17, 30)).map(
				(preset) => preset.id,
			),
	...
	it("puts next week on the following Monday", () => {
		const nextWeek = new Date(
			resolveSnoozePresets(localDate(2026, 4, 6, 10)).find(
