...
export function resolveSettledThreadTimestamp(
	thread: SettledThreadTimestampInput,
): string | null {
	if (
		thread.settledAt != null &&
		toSortableTimestamp(thread.settledAt) !== null
	) {
	...
	return toSortableTimestamp(thread.updatedAt) === null
		? null
		: thread.updatedAt;
}

function getFirstSortableTimestamp(
...
function getLatestUserMessageTimestamp(thread: ThreadSortInput): number {
	if (thread.latestUserMessageAt) {
		const latestUserMessageTimestamp = toSortableTimestamp(
			thread.latestUserMessageAt,
		);
...
export function getThreadSortTimestamp(
	...
	sortOrder:
		| SidebarThreadSortOrder
		| Exclude<SidebarProjectSortOrder, "manual">,
): number {
...
function pinOrderMidpoint(a: string, b: string): string {
	if (b !== "" && a >= b)
		throw new Error("pinOrderMidpoint: bounds out of order");
	if (b !== "") {
	...
	const digitB =
		b === "" ? PIN_ORDER_DIGITS.length : PIN_ORDER_DIGITS.indexOf(b.charAt(0));
	if (digitB - digitA > 1) {
...
export function pinOrderKeyBetween(
	before: string | null,
	after: string | null,
): string | null {
	const a = before ?? "";
...
export function planPinnedReorder(input: {
	...
	const reservedKeys = new Set(
		[...keysById].flatMap(([id, key]) =>
			!visibleIds.has(id) && key != null ? [key] : [],
		),
	);
	...
	const afterId =
		movedIndex < orderedIds.length - 1 ? orderedIds[movedIndex + 1] : null;
	...
	if (beforeUsable && afterUsable) {
		...
		while (key !== null && reservedKeys.has(key))
			key = pinOrderKeyBetween(key, afterKey);
...
>(threads: readonly T[]): T[] {
	...
	keyless.sort((left, right) => {
		...
		return (
			(Number.isNaN(rightMs) ? 0 : rightMs) -
				(Number.isNaN(leftMs) ? 0 : leftMs) || identityTiebreak(left, right)
		);
	});
