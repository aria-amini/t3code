...
const makeHarness = Effect.fn("TestThreadAtoms.makeHarness")(
	function* (options?: {
		readonly snapshot?: OrchestrationThreadDetailSnapshot;
		readonly connected?: boolean;
		readonly httpNone?: boolean;
		readonly initialLoad?: Effect.Effect<
			...
			[ORCHESTRATION_WS_METHODS.subscribeThread]: (input: {
				...
				Stream.unwrap(
					Effect.gen(function* () {
						...
						yield* Queue.offer(subscriptions, {
		...
		const environmentRegistry = EnvironmentRegistry.of({
			...
			reconcilePlatform: () =>
				Effect.die("Unexpected environment reconciliation"),
			remove: () => Effect.die("Unexpected environment removal"),
			removeRelayEnvironments: () =>
				Effect.die("Unexpected environment removal"),
			retryNow: () => Effect.void,
		...
		const makeRegistry = Effect.acquireRelease(
			Effect.sync(() =>
				AtomRegistry.make({ defaultIdleTTL: 60_000, timeoutResolution: 1 }),
			),
...
describe("createEnvironmentThreadStateAtoms", () => {
	...
	it.effect(
		"exposes snapshot loader defects before the RPC subscription starts",
		() =>
			Effect.gen(function* () {
				...
				const h = yield* makeHarness({
					...
					initialLoad: Effect.die(
						new Error(
							"SYNTHETIC_RAW_SNAPSHOT_DEFECT_SHOULD_NOT_REACH_THREAD_UI",
						),
				...
				expect(failed.error).toEqual(
					Option.some("Could not synchronize the thread."),
				);
		...
		({ kind, httpNone }) =>
			Effect.gen(function* () {
				...
				const error = new Error(
					"SYNTHETIC_RAW_DEFECT_SHOULD_NOT_REACH_THREAD_UI",
				);
				...
				expect(failed.data).toEqual(
					httpNone ? Option.none() : Option.some(THREAD),
				);
	...
	it.effect(
		"retries a protocol failure on foreground without replacing the session",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"keeps transport loss nonterminal and recovers with a replacement session",
		() =>
			Effect.gen(function* () {
				const h = yield* makeHarness({ connected: true, httpNone: true });
				const unmount = h.registry.mount(h.stateAtom);
	...
	it.effect(
		"retains ordinary domain error reporting and same-session retries",
		() =>
			Effect.gen(function* () {
				const h = yield* makeHarness({ connected: true, httpNone: true });
				const unmount = h.registry.mount(h.stateAtom);
	...
	it.effect(
		"shares one live stream and closes it after the last detail consumer leaves",
		() =>
			Effect.gen(function* () {
				const h = yield* makeHarness();
	...
	it.effect(
		"keeps warm data and resumes a completed cursor without loading another snapshot",
		() =>
			Effect.gen(function* () {
				const h = yield* makeHarness();
				const unmount = h.registry.mount(h.stateAtom);
	...
	it.effect.each([
		...
		({ replayed, statuses }) =>
			Effect.gen(function* () {
				...
				expect(
					observed.filter((status, index) => observed[index - 1] !== status),
				).toEqual(statuses);
				stop();
				remount();
				yield* Deferred.await(next.closed);
	...
	it.effect(
		"downgrades a warm resume when the connection dropped while away",
		() =>
			Effect.gen(function* () {
				const h = yield* makeHarness({ connected: true });
				const unmount = h.registry.mount(h.stateAtom);
	...
	it.effect(
		"keeps warm data when the raw atom family's weak entry is collected",
		() =>
			Effect.gen(function* () {
				const h = yield* makeHarness();
	...
	it.effect("expires the plain snapshot after five idle minutes", () =>
		Effect.gen(function* () {
			...
			yield* Effect.promise(() =>
				vi.advanceTimersByTimeAsync(THREAD_SNAPSHOT_IDLE_TTL_MS + 1),
			);
			const remount = h.registry.mount(h.stateAtom);
			const next = yield* Queue.take(h.subscriptions);
			expect(h.counts()).toEqual({
	...
	it.effect(
		"cancels older-page work on unmount and permits it again on a warm return",
		() =>
			Effect.gen(function* () {
				const h = yield* makeHarness({
					snapshot: {
				...
				const unmount = h.registry.mount(h.stateAtom);
				const first = yield* Queue.take(h.subscriptions);
				expect(requestOlderThreadTurns(TARGET.environmentId, THREAD_ID)).toBe(
					true,
				);
				const older = yield* Queue.take(h.olderLoads);
				expect(
					Option.getOrThrow(h.registry.get(h.stateAtom).page).loadingOlder,
				).toBe(true);
				unmount();
				yield* Deferred.await(first.closed);
