...
const OLDER_MESSAGE = message(
	"message-old",
	"turn-1",
	"2026-04-01T00:00:00.000Z",
);
const RECENT_MESSAGE = message(
	"message-recent",
	"turn-2",
...
const makeHarness = Effect.fn("TestThreadPagination.makeHarness")(
	function* (options?: {
		...
		const loaderWindows = yield* Ref.make<
			ReadonlyArray<ThreadSnapshotWindow | undefined>
		>([]);
		...
		const pendingPageResponses =
			yield* Queue.unbounded<Deferred.Deferred<LoaderResponse>>();
					...
					Effect.andThen(
						window?.beforeCursor === undefined
							...
							: Deferred.make<LoaderResponse>().pipe(
									Effect.tap((deferred) =>
										Queue.offer(pendingPageResponses, deferred),
									),
...
describe("thread pagination state", () => {
	it.effect(
		"windows the initial load when the server advertises pagination",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"merges an older page below the loaded window and clears the cursor",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({
					initialResponse: Option.some(WINDOWED_SNAPSHOT),
	...
	it.effect(
		"discards an in-flight older page when a revert rewrites history",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({
					initialResponse: Option.some(WINDOWED_SNAPSHOT),
	...
	it.effect(
		"discards an in-flight older page when a fresh snapshot replaces the thread",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({
					initialResponse: Option.some(WINDOWED_SNAPSHOT),
	...
	it.effect(
		"discards an older page read from a projection behind the loaded state",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({
					initialResponse: Option.some(WINDOWED_SNAPSHOT),
				...
				yield* harness.resolveNextPage(
					Option.some({ ...OLDER_PAGE, snapshotSequence: 5 }),
				);
	...
	it.effect(
		"a merged history page never advances the live-event dedupe sequence",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({
					initialResponse: Option.some(WINDOWED_SNAPSHOT),
				...
				const state = yield* harness.awaitState(
					(value) =>
						!hasMessage(value, "message-recent") &&
						hasMessage(value, "message-old"),
				);
	...
	it.effect(
		"parks a page read ahead of the live state until events catch up",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"a revert keeps the page cursor and triggers no refresh fetch",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"drops a windowed cache when the server lacks the pagination capability",
		() =>
			Effect.gen(function* () {
