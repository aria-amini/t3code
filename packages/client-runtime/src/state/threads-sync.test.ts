...
const makeHarness = Effect.fn("TestEnvironmentThreads.makeHarness")(
	function* (options?: {
		readonly cached?: OrchestrationThread;
		readonly httpSnapshot?: Option.Option<OrchestrationThreadDetailSnapshot>;
		readonly completionMarker?: boolean;
		readonly resumeCache?: NonNullable<
			Parameters<typeof makeEnvironmentThreadState>[1]
		>;
		readonly loadCached?: Effect.Effect<
		...
		const latest = yield* Ref.make<EnvironmentThreadState>(
			EMPTY_ENVIRONMENT_THREAD_STATE,
		);
		const retryCount = yield* Ref.make(0);
		const subscriptionCount = yield* Ref.make(0);
		const loaderCalls = yield* Ref.make(0);
		const lastSubscribeAfterSequence = yield* Ref.make<number | undefined>(
			undefined,
		);
		const lastRequestCompletionMarker = yield* Ref.make<boolean | undefined>(
			undefined,
		);
		const savedThreads = yield* Ref.make<
			ReadonlyArray<OrchestrationThreadDetailSnapshot>
		>([]);
		const removedThreads = yield* Ref.make<ReadonlyArray<ThreadId>>([]);
		const wakeups =
			yield* Queue.unbounded<ConnectionWakeups.ConnectionWakeup>();
		const supervisorState =
			yield* SubscriptionRef.make<SupervisorConnectionState>(
				AVAILABLE_CONNECTION_STATE,
			);
		...
		const cache = Persistence.EnvironmentCacheStore.of({
			...
			saveThread: (environmentId, thread) =>
				Ref.update(savedThreads, (current) => [...current, thread]).pipe(
					Effect.andThen(
						options?.saveThread?.(environmentId, thread) ?? Effect.void,
					),
				),
...
const synchronized = (): OrchestrationThreadStreamItem => ({
	kind: "synchronized",
});
...
describe("EnvironmentThreads", () => {
	for (const source of ["disk", "HTTP"] as const) {
		it.effect(
			`does not rewrite an unchanged ${source} snapshot on navigation or warm return`,
			() =>
				Effect.gen(function* () {
					const resumeCache: NonNullable<
						Parameters<typeof makeEnvironmentThreadState>[1]
					> = {
						snapshot: undefined,
						owner: undefined,
					...
					expect(yield* Ref.get(firstSaved)).toHaveLength(
						source === "disk" ? 0 : 1,
					);
	...
	it.effect(
		"retries a failed background cache write when the scope closes",
		() =>
			Effect.gen(function* () {
				let attempts = 0;
				yield* Effect.scoped(
	...
	it.effect(
		"flushes newer data after an older background write completes",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"does not resume past a canceled event whose data was not applied",
		() =>
			Effect.gen(function* () {
				const resumeCache: NonNullable<
					Parameters<typeof makeEnvironmentThreadState>[1]
				...
				if (update.kind !== "event")
					return yield* Effect.die("Expected an event");
				...
				expect(yield* Ref.get(resumed.lastSubscribeAfterSequence)).toBe(
	...
	it.effect(
		"prevents an old scope from replacing its successor's cached data",
		() =>
			Effect.gen(function* () {
				const resumeCache: NonNullable<
					Parameters<typeof makeEnvironmentThreadState>[1]
				...
				yield* Queue.offer(
					successor.inputs,
					titleUpdated("Successor title", 9),
				);
				yield* awaitThreadState(
					successor.observed,
				...
				expect(Option.getOrThrow(resumeCache.snapshot!.state.data).title).toBe(
					"Successor title",
				);
	...
	it.effect(
		"does not let a delayed cache read replace an initialized successor",
		() =>
			Effect.gen(function* () {
				const resumeCache: NonNullable<
					Parameters<typeof makeEnvironmentThreadState>[1]
				...
				yield* Deferred.succeed(
					...
					Option.some({
						snapshotSequence: 3,
						thread: { ...BASE_THREAD, title: "Old disk data" },
					}),
				);
	...
	it.effect(
		"does not let an old deletion remove its successor's persisted cache",
		() =>
			Effect.gen(function* () {
				const resumeCache: NonNullable<
					Parameters<typeof makeEnvironmentThreadState>[1]
	...
	it.effect(
		"retains a deletion instead of restoring the old disk snapshot",
		() =>
			Effect.gen(function* () {
				const resumeCache: NonNullable<
					Parameters<typeof makeEnvironmentThreadState>[1]
				...
				yield* Effect.scoped(
					Effect.gen(function* () {
						...
						if (update.kind !== "event")
							return yield* Effect.die("Expected an event");
						yield* Queue.offer(first.inputs, {
							...update,
							event: { ...update.event, sequence: 8 },
						});
						yield* awaitThreadState(
	...
	it.effect(
		"resumes a warm cache via afterSequence without an HTTP fetch",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({ cached: BASE_THREAD });
				...
				yield* Queue.offer(
					harness.inputs,
					titleUpdated("Live title", CACHED_SNAPSHOT_SEQUENCE + 1),
				);
				yield* awaitThreadState(
					harness.observed,
					(value) =>
	...
	it.effect("reduces live events and persists the latest thread", () =>
		Effect.gen(function* () {
			...
			expect((yield* Ref.get(harness.savedThreads)).at(-1)?.thread.title).toBe(
				"Live title",
			);
			expect(
				(yield* Ref.get(harness.savedThreads)).at(-1)?.snapshotSequence,
			).toBe(2);
		}),
	);

	it.effect(
		"does not persist active thread snapshots during streaming or teardown",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"seeds the thread from the HTTP snapshot and resumes live events",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"ignores replayed thread events at or below the snapshot sequence",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({ cached: BASE_THREAD });
				yield* Queue.offer(harness.inputs, snapshot(BASE_THREAD));
	...
	it.effect(
		"does not resurrect a deleted thread when the app returns to the foreground",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({
					cached: BASE_THREAD,
	...
	it.effect(
		"preserves data after a domain failure and resumes on a replacement session",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({ cached: BASE_THREAD });
				yield* Queue.offer(harness.inputs, snapshot(BASE_THREAD));
	...
	it.effect(
		"recovers from a transient domain failure without replacing the session",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness();
	...
	it.effect(
		"does not overwrite a live snapshot when the supervisor becomes ready",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({ cached: BASE_THREAD });
	...
	it.effect(
		"keeps replayed updates synchronizing until the completion marker arrives",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({
					cached: BASE_THREAD,
	...
	it.effect(
		"resumes replacement sessions from the latest applied sequence",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({
					cached: BASE_THREAD,
	...
	it.effect(
		"resubscribes on app foreground from the latest applied sequence",
		() =>
			Effect.gen(function* () {
				const harness = yield* makeHarness({
					cached: BASE_THREAD,
