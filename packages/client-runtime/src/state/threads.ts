...
function statusWithoutLiveData(
	data: Option.Option<OrchestrationThread>,
...
function matchesThreadSnapshot(
	...
	sequence: number,
	page:
		| Pick<EnvironmentThreadPageState, "beforeCursor" | "hasMore">
		| undefined,
): boolean {
	if (
		current.sequence !== sequence ||
		Option.getOrNull(current.state.data) !== thread
	)
		return false;
...
function cachedThreadState(
	value: EnvironmentThreadState,
): EnvironmentThreadState {
	return {
		...value,
		status:
			value.status === "deleted" ||
			(value.status === "live" && Option.isSome(value.data))
				? value.status
...
export const makeEnvironmentThreadState = Effect.fn(
	"EnvironmentThreadState.make",
)(function* (threadId: ThreadIdType, resumeCache?: ThreadResumeCache) {
	const supervisor = yield* EnvironmentSupervisor;
	const cache = yield* EnvironmentCacheStore;
	const snapshotLoader = yield* ThreadSnapshotLoader;
	const wakeups = yield* Effect.serviceOption(
	...
	const persistence =
		yield* Queue.sliding<OrchestrationThreadDetailSnapshot>(1);
	...
	const setThread = Effect.fn("EnvironmentThreadState.setThread")(function* (
		...
		if (shouldPersistThread(thread)) {
			...
			const currentPage = yield* SubscriptionRef.get(state).pipe(
				Effect.map((value) => value.page),
			);
	...
	const setDeleted = Effect.fn("EnvironmentThreadState.setDeleted")(
		function* () {
			yield* Ref.set(awaitingCompletion, false);
	...
	const applyItemLocked = Effect.fn("EnvironmentThreadState.applyItemLocked")(
		function* (item: OrchestrationThreadStreamItem) {
	...
	const tryMergePendingOlderPage = Effect.fn(
		"EnvironmentThreadState.tryMergePendingOlderPage",
	)(function* () {
		...
		if (epochNow !== pending.epoch) {
			...
			yield* SubscriptionRef.update(state, (value) => ({
				...value,
				page: Option.map(value.page, (existing) => ({
					...existing,
					loadingOlder: false,
				})),
			}));
	...
	const applyItem = Effect.fn("EnvironmentThreadState.applyItem")(function* (
		...
		yield* applyLock.withPermits(1)(
			applyItemLocked(item).pipe(Effect.andThen(remember)),
		);
	});
	...
	const mergeOlderPage = Effect.fn("EnvironmentThreadState.mergeOlderPage")(
		function* (snapshot: OrchestrationThreadDetailSnapshot) {
			...
			yield* SubscriptionRef.update(state, (value) => {
				...
				const mergeById = <T extends { readonly id: string }>(
					...
					return [
						...olderRows.filter((row) => !seen.has(row.id)),
						...loadedRows,
					];
				};
				const seenCheckpoints = new Set(
					loaded.checkpoints.map((row) => row.turnId),
				);
				merged = {
					...
					checkpoints: [
						...older.checkpoints.filter(
							(row) => !seenCheckpoints.has(row.turnId),
						),
						...loaded.checkpoints,
					],
				};
	...
	const loadOlderTurns = Effect.fn("EnvironmentThreadState.loadOlderTurns")(
		function* () {
	...
	yield* Effect.forkScoped(
		subscribeDynamic(
			...
			Effect.fn("EnvironmentThreadState.makeSubscribeInput")(
				function* (session) {
					...
					if (!supportsPagination) {
						yield* applyLock.withPermits(1)(
							Effect.gen(function* () {
								if (Option.isNone((yield* SubscriptionRef.get(state)).page))
									return;
								yield* Ref.update(historyEpoch, (epoch) => epoch + 1);
								yield* SubscriptionRef.update(state, (value) => ({
									...value,
					...
					if (Option.isNone(current.data) && current.status !== "deleted") {
						const prepared = yield* SubscriptionRef.get(
							supervisor.prepared,
						).pipe(
							Effect.flatMap(
								Option.match({
					...
					if (!supportsCompletionMarker && canResume) {
						yield* SubscriptionRef.update(state, (value) => ({
							...value,
							status:
								value.status === "deleted" ? value.status : ("live" as const),
							error: Option.none(),
						}));
					}
			...
			.atom(
				(get) => {
					...
					return resume.snapshot === undefined
						? live
						: Stream.concat(
								Stream.succeed(cachedThreadState(resume.snapshot.state)),
								live,
							);
				},
				{
