...
function harness(ids = ["a"]) {
	...
	const options = {
		...
		presentations: {
			presentationAtom: (environmentId: EnvironmentId) =>
				get(environmentId).presentation,
		},
...
describe("manual usage refresh", () => {
	it.each(["success", "failure"])(
		"waits for the rescan after a pricing %s",
		async (result) => {
			const {
	...
	it("waits for healthy environments without waiting for a recovering environment", async () => {
		const { registry, environments, refresh } = harness([
			"healthy",
			"recovering",
		]);
