...
describe("cached VCS refs", () => {
			...
			Effect.gen(function* () {
				...
				const client = {
					[WS_METHODS.vcsListRefs]: (input: VcsListRefsInput) =>
						Ref.update(requests, (current) => [...current, input]).pipe(
							Effect.as(LIVE_REFS),
						),
	...
	it.effect("does not repersist a refresh superseded by ref invalidation", () =>
		Effect.scoped(
			Effect.gen(function* () {
				...
				const revisionsObservedDuringClear = yield* Ref.make<
	...
	it.effect(
		"invalidates persisted refs when ref-affecting commands settle",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
	...
	it.effect(
		"suppresses persisted snapshots after an environment-wide clear fails",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const registry = yield* Effect.acquireRelease(
					...
					const cache = cacheWithRefs(Option.none(), {
						loadVcsRefs: () =>
							Ref.update(loads, (count) => count + 1).pipe(
								Effect.andThen(Ref.get(persisted)),
							),
						saveVcsRefs: (_environmentId, _cwd, refs) =>
							Ref.set(persisted, Option.some(refs)),
						clearVcsRefs: () =>
	...
	it.effect("retries a transient live failure while connected", () =>
		Effect.scoped(
			Effect.gen(function* () {
				...
				for (
					let attempt = 0;
					attempt < 100 && (yield* Ref.get(calls)) < 1;
					attempt += 1
				) {
					yield* Effect.yieldNow;
				}
	...
	it.effect(
		"cancels an in-flight refresh when the connection generation changes",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const calls = yield* Ref.make(0);
					...
					for (
						let attempt = 0;
						attempt < 100 && (yield* Ref.get(interruptions)) < 1;
						attempt += 1
					) {
						yield* Effect.yieldNow;
					}
