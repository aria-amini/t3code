...
export const commitVcsRefsRefresh = Effect.fn(
	"CachedVcsRefsState.commitRefresh",
)(function* (
			...
			if (input.persist) {
				if (!persistedCacheReadable) {
					persistedCacheReadable = yield* cache
						.clearVcsRefs(input.environmentId)
						.pipe(
							Effect.as(true),
							Effect.catch((error) =>
								Effect.logWarning(
									"Could not recover invalidated cached Git refs.",
								).pipe(
									Effect.annotateLogs({
										environmentId: input.environmentId,
										cwd: input.cwd,
				...
				yield* cache
					.saveVcsRefs(input.environmentId, input.cwd, input.refs)
					.pipe(
						Effect.catch((error) =>
 ...
 * misleading.
 */
export const makeCachedVcsRefsChanges = Effect.fn(
	"CachedVcsRefsState.makeChanges",
)(function* (
	...
	const refreshedRefs = Stream.concat(
		...
		Stream.map((connection) =>
			connection.phase === "connected" ? connection.generation : null,
		),
		Stream.changes,
