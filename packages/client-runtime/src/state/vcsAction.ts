...
function failVcsActionState(
	...
	return {
		...
		actionId,
		error:
			error instanceof Error ? error.message : "Source control action failed.",
	};
}
	...
	return Effect.suspend(() => {
		...
		return stream.pipe(
			Stream.runForEach((event) => {
				...
				if (
					normalized.kind === "action_finished" ||
					normalized.kind === "action_failed"
				) {
...
export function createVcsActionManager<R, E>(
	...
	const getRunStackedActionCommand = (requestedTarget: VcsActionTarget) => {
		...
		const target =
			targetKey === null ? null : parseVcsActionTargetKey(targetKey);
		const stateAtom =
			targetKey === null
		...
		>(runtime, {
			...
			execute: (input: RunVcsStackedActionInput, registry) => {
				...
				const transportActionId = createVcsActionTransportId(
					target,
					input.actionId,
				);
				registry.set(
					...
					),
					{
						...
						onProgress: (event) =>
							Effect.sync(() => {
								...
								registry.set(
									stateAtom,
									applyVcsActionProgressEvent(current, event),
								);
	...
	return {
		...
		runStackedAction: (target: VcsActionTarget) =>
			getRunStackedActionCommand(target),
		track: async <A, E>(
			registry: AtomRegistry.AtomRegistry,
			target: VcsActionTarget,
			input: BeginVcsActionInput,
		...
		resetError: (
			...
			setState(registry, target, (current) =>
				!current.isRunning && current.operation === operation
					? EMPTY_VCS_ACTION_STATE
					: current,
			);
		},
