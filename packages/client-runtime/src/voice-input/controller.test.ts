...
function draft(
	overrides: Partial<VoiceDraftSnapshot> = {},
): VoiceDraftSnapshot {
	return {
...
describe("resolveTranscriptCommit", () => {
	it("replaces the recorded UTF-16 selection around emoji and composer tokens", () => {
		...
		expect(
			...
			selection: {
				start: tokenStart + "use the mobile skill".length,
				end: tokenStart + 20,
			},
		});
	});
	...
	it("adds English spacing at empty start, middle, and end caret boundaries", () => {
		...
		expect(
			resolveTranscriptCommit(atEnd, atEnd, "Also fix tests.", "en-US"),
		).toMatchObject({
			kind: "commit",
			text: "Fix cache. Also fix tests.",
		});
		expect(
			resolveTranscriptCommit(atEnd, atEnd, "Also fix tests.", "en_US"),
		).toMatchObject({
			kind: "commit",
			text: "Fix cache. Also fix tests.",
		});
		...
		expect(
			resolveTranscriptCommit(atStart, atStart, "First", "en-US"),
		).toMatchObject({
			kind: "commit",
			text: "First Fix cache.",
		});

		const inMiddle = draft({
			text: "Fix cache.",
			selection: { start: 4, end: 4 },
		});
		expect(
			resolveTranscriptCommit(inMiddle, inMiddle, "also", "en-US"),
		).toMatchObject({
			kind: "commit",
			text: "Fix also cache.",
		});
	...
	it("does not add English boundary spaces to CJK or selected inline text", () => {
		...
		expect(
			resolveTranscriptCommit(cjk, cjk, "テストも", "ja-JP"),
		).toMatchObject({
			kind: "commit",
			text: "修正キャッシュテストも",
		});
		...
		expect(
			resolveTranscriptCommit(selected, selected, "new", "en-US"),
		).toMatchObject({
			kind: "commit",
			text: "one new two",
		});
...
describe("VoiceInputController", () => {
	...
	it("checks support and permission before recording", async () => {
		...
		expect(unsupported.controller.currentState.error).toContain(
			"not available",
		);
				...
				prepare: async () =>
					preparedTranscription((_uri, { signal }) => {
						signal.addEventListener(
							"abort",
							() => transcription.reject(new Error("aborted")),
							{
								once: true,
							},
						);
	...
	it("leaves transcription with an error when recorder finalization fails", async () => {
		...
		expect(harness.controller.currentState.error).toContain(
	...
	it("does not start the microphone for an owner that changed during preparation", async () => {
		...
		await preparationEntered.promise;
		harness.setDraft(
			draft({ ownerKey: "environment:other-thread", text: "other draft" }),
		);
		preparation.resolve(preparedTranscription());
		await starting;
