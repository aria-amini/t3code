...
async function runTranscriptionOperation<T>(
	...
	try {
		...
		if (activeTranscriptionOperation === promise)
...
export class VoiceInputController {
	...
	async start(): Promise<void> {
		...
		try {
			...
			if (
				!capturedDraft ||
				capturedDraft.ownerKey !== initiatingDraft.ownerKey
			) {
				this.setError("This draft is no longer available.", "retry");
				return;
			}
			this.capturedDraft = capturedDraft;
			this.dependencies.recorder.record({
	...
	appMovedToBackground(): Promise<void> | void {
		if (this.state.phase === "preparing") {
			this.invalidateOperation();
			this.setError(
				"Voice input stopped when the app moved to the background.",
				"retry",
			);
			return;
		}
	...
	private async finishRecording(
		...
		try {
			...
			this.recordingUri =
				completedUri ?? this.dependencies.recorder.uri ?? this.recordingUri;
			this.rememberRecordingUri(this.recordingUri);
			if (!this.isCurrent(operationToken)) return;
			if (
				!this.recordingUri ||
	...
	private setError(
		error: string,
		errorAction: VoiceInputState["errorAction"],
	): void {
