...
describe("commandProgramName", () => {
	...
	it.each([
		...
		"preserves ordinary programs and actual shell launches: %s",
		(command, program) => {
			expect(commandProgramName(command)).toBe(program);
		},
	);
	...
	it.each([
		...
		"falls back for shell syntax and internal control commands: %s",
		(command) => {
			expect(commandProgramName(command)).toBeNull();
		},
	);
	...
	it.each([
		...
		"skips leading cd commands and uses the next useful program: %s",
		(command, program) => {
			expect(commandProgramName(command)).toBe(program);
		},
	);
	...
	it.each([
		...
		"skips shell setup commands and uses the next useful program: %s",
		(command, program) => {
			expect(commandProgramName(command)).toBe(program);
		},
	);
	...
	it.each([
		...
		"skips non-descriptive shell commands before a useful program: %s",
		(command, program) => {
			expect(commandProgramName(command)).toBe(program);
		},
	);
	...
	it.each(
		...
		"handles shell setup followed by every command separator: %s",
		(command, program) => {
			expect(commandProgramName(command)).toBe(program);
		},
	);
	...
	it.each([
		...
		"labels commands inside simple PowerShell assignments: %s",
		(command, program) => {
			expect(commandProgramName(command)).toBe(program);
		},
	);
	...
	it.each([
		...
		"handles common PowerShell setup and launch commands: %s",
		(command, program) => {
			expect(commandProgramName(command)).toBe(program);
		},
	);
	...
	it.each([
		...
		"keeps process-launch wrappers when no safe payload is present: %s",
		(command, program) => {
			expect(commandProgramName(command)).toBe(program);
		},
	);
	...
	it.each([
		...
		"does not treat shell lookup and commandless wrapper forms as executions: %s",
		(command) => {
			expect(commandProgramName(command)).toBeNull();
		},
	);
	...
	it.each([
		...
		"does not hide legitimate or case-distinct program names: %s",
		(command, program) => {
			expect(commandProgramName(command)).toBe(program);
		},
	);
	...
	it.each([
		...
		"parses shell setup inside an explicitly launched shell: %s",
		(command, program) => {
			expect(commandProgramName(command)).toBe(program);
		},
	);
	...
	it.each([
		...
		"does not treat qualified paths as shell syntax: %s",
		(command, program) => {
			expect(commandProgramName(command)).toBe(program);
		},
	);
	...
	it.each([
		...
		"resolves literal command aliases from earlier shell segments: %s",
		(command, program) => {
			expect(commandProgramName(command)).toBe(program);
		},
	);
	...
	it.each([
		...
		"does not evaluate dynamic or non-persistent command aliases: %s",
		(command) => {
			expect(commandProgramName(command)).toBeNull();
		},
	);
	...
	it.each([
		...
		"skips heredoc bodies before finding the next command: %s",
		(command, program) => {
			expect(commandProgramName(command)).toBe(program);
		},
	);
	...
	it.each([
		"cd apps/web && [ -f package.json ]",
		"cd apps/web || exit 1",
		"cd one && cd two",
	])("falls back when no useful program follows cd: %s", (command) => {
		expect(commandProgramName(command)).toBeNull();
	});
	...
	it.each([
		...
		"does not label commands inside multiline shell control flow: %s",
		(command) => {
			expect(commandProgramName(command)).toBeNull();
		},
	);
