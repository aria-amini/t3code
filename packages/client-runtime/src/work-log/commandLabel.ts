...
const POWERSHELL_SETUP_PROGRAMS = new Set([
	"pop-location",
	"push-location",
	"set-location",
]);
const POWERSHELL_FLAGS = new Set([
	"-mta",
	"-nologo",
	"-noninteractive",
	"-noprofile",
...
function shellCommandArgumentIndex(
	...
	for (let index = start; index < tokens.length; index += 1) {
		...
		if (option === "--command" || /^-[a-zA-Z]*c[a-zA-Z]*$/.test(option))
			return index + 1;
	}
	return null;
}

const COMMAND_WRAPPER_OPTIONS_WITH_VALUE: Record<
	CommandWrapper,
	ReadonlySet<string>
> = {
...
function tokenizeShellCommand(command: string): string[] | null {
	...
	for (let index = 0; index < input.length; index += 1) {
		...
		if (character === "\\" && quote !== "'") {
			...
			const isWindowsPath =
				quote === null && /^(?:[A-Za-z]:|\.{1,2})(?:\\[^\s]*)?$/u.test(current);
			if (
				(quote === '"' || isWindowsPath) &&
...
function commandAfterHeredocs(
	...
	for (const heredoc of heredocs) {
		...
		while (cursor <= command.length) {
			...
			const comparableLine = heredoc.stripTabs
				? line.replace(/^\t+/u, "")
				: line;
...
function splitFirstShellCommand(command: string): ShellCommandSplit {
	...
	for (let index = 0; index < command.length; index += 1) {
		...
		if (character === "\n" && heredocs.length > 0) {
			...
			const commandBeforeHeredocs = separator
				...
				: "";
			const commandFollowingHeredocs = commandAfterHeredocs(
				command,
				index + 1,
				heredocs,
			);
	...
	return {
		firstCommand: commandWithoutShellComments(
			command,
			command.length,
			comments,
		).trim(),
...
function indexAfterShellRedirection(
	tokens: ReadonlyArray<string>,
	index: number,
): number | null {
...
function transparentWrapperCommandIndex(
	...
	if (wrapper === "bundle") {
		return tokens[index + 1] === "exec" && tokens[index + 2] !== undefined
			? index + 2
			: null;
	}
	...
	if (wrapper === "script") {
		...
		return /^-[adkpqr]+$/u.test(tokens[index + 1] ?? "") &&
	...
	if (wrapper === "arch") {
		...
		return tokens[index + 1] === "-arch" && tokens[index + 3] !== undefined
			? index + 3
			: null;
	}
...
function staticProgramName(value: string): string | null {
	...
	if (!trimmedValue || /^[A-Za-z][A-Za-z0-9+.-]*:(?![\\/])/u.test(trimmedValue))
		return null;
...
function leadingPowerShellLiteral(command: string): string | null {
	...
	const quote = input[0];
	if (quote !== '"' && quote !== "'")
		return staticProgramName(input.match(/^\S+/u)?.[0] ?? "");
...
function powerShellCallOperatorProgramName(
	command: string,
...
function powerShellAssignmentProgramName(
	...
	if (calledProgram !== undefined)
		return { matched: true, program: calledProgram };

	const directCommand = value.match(
		/^(?:@?\(\s*)?([A-Za-z][A-Za-z0-9_.-]*)\b/u,
	)?.[1];
	if (
		directCommand &&
		!NON_DESCRIPTIVE_SHELL_PROGRAMS.has(directCommand.toLowerCase())
	) {
		const parsedCommand = commandProgramNameInternal(
			value,
			depth + 1,
			"shell",
			segmentsRemaining,
...
function windowsShellPayloadProgramName(
	...
	const parsePayload = (payload: string | undefined): string | null => {
		...
		const command =
			remainingCommand && separator
				? `${payload} ${separator} ${remainingCommand}`
				: payload;
	...
	for (let index = start; index < tokens.length; index += 1) {
		...
		if (option === "-file" || option === "-f") {
			return {
				matched: true,
				program: staticProgramName(tokens[index + 1] ?? ""),
			};
		}
...
function startProcessProgramName(
	...
	for (let index = start; index < tokens.length; index += 1) {
		...
		if (option === "-filepath")
			return staticProgramName(tokens[index + 1] ?? "");
...
function literalCommandAliasProgramName(command: string): string | null {
	...
	for (
		let segmentCount = 0;
		remainingCommand && segmentCount < 64;
		segmentCount += 1
	) {
		const commandWithoutComments =
			commandWithoutLeadingShellComments(remainingCommand);
		...
		const tokens = tokenizeShellCommand(
			withoutShellLineContinuations(commandSplit.firstCommand),
		);
		if (tokens === null) return null;

		let commandIndex = tokens[0] === "do" || tokens[0] === "then" ? 1 : 0;
		while (commandIndex < tokens.length) {
			const indexAfterRedirection = indexAfterShellRedirection(
				tokens,
				commandIndex,
			);
			if (
				indexAfterRedirection !== null &&
				indexAfterRedirection <= tokens.length
			) {
...
function wrappedShellCommandProgramName(
	...
	while (targetIndex < wrappedTokens.length) {
		...
		if (
			indexAfterRedirection === null ||
			indexAfterRedirection > wrappedTokens.length
		)
			break;
...
function parseCommandProgramName(
	...
	if (/^[A-Za-z_][A-Za-z0-9_]*\s*\(\s*\)\s*\{/u.test(commandWithoutComments))
		return null;
	...
	while (index < tokens.length) {
		...
		if (wrapper !== null && token.startsWith("-")) {
			...
			if (token.startsWith("--") && equalsIndex > 2) {
				if (
					!COMMAND_WRAPPER_OPTIONS_WITH_VALUE[wrapper].has(
			...
			if (/^-[A-Za-z].+/.test(token) && !token.startsWith("--")) {
				...
				for (const [optionIndex, option] of token
					.slice(1)
					.split("")
					.entries()) {
		...
		if (
			tokenProgram &&
			SHELL_PROGRAMS.has(tokenProgram.replace(/\.exe$/i, "").toLowerCase())
		) {
		...
		if (isUnqualifiedToken && tokenProgram) {
			const targetIndex = transparentWrapperCommandIndex(
				tokenProgram,
				tokens,
				index,
			);
		...
		if (
			isUnqualifiedToken &&
			tokenProgram &&
			SHELL_COMMAND_WRAPPERS.has(tokenProgram)
		) {
		...
		if (
			(executionContext === "shell" ||
				(wrapper === "sudo" &&
					tokenProgram &&
					SKIPPABLE_SUDO_PROBES.has(tokenProgram))) &&
			isUnqualifiedToken &&
			tokenProgram &&
