...
describe("workEntryIndicatesToolFailure", () => {
	...
	it("treats successful tool rows as success candidates", () => {
		...
		expect(
			workEntryIndicatesToolSuccess({ ...base, tone: "thinking", detail: "…" }),
		).toBe(false);
		expect(
			workEntryIndicatesToolSuccess({
				...base,
...
describe("resolveWorkEntryToolPresentation", () => {
	...
	it.each([
		...
		"describes the tool's own %s state",
		(toolLifecycleStatus, displayName) => {
			expect(
				resolveWorkEntryToolPresentation({
	...
	it("uses the summary's state only when the provider omitted a lifecycle status", () => {
		...
		expect(
			resolveWorkEntryToolPresentation(entry, "inProgress")?.displayName,
		).toBe("Clicking in the preview browser");
		expect(
			resolveWorkEntryToolPresentation(entry, "completed")?.displayName,
	...
	it.each([
		[
			"preview_type",
			"Typing in the preview browser",
			"Typed in the preview browser",
		],
		[
		...
		"preserves verb forms and the rest of %s's label",
...
describe("browser group summaries", () => {
	...
	it.each([1, 18])(
		"counts %s browser calls separately from generic tools",
		(count) => {
	...
	it("combines command and browser counts in a single sentence", () => {
		...
		expect(summarizeToolGroup(entries)).toBe(
			"Ran 4 commands and used browser 15 times",
		);
...
describe("command work-log details", () => {
	it("extracts Claude result blocks and projected output", () => {
		...
		expect(
			extractCommandOutputText({ rawOutput: { content: "projected summary" } }),
		).toBe("projected summary");
	});
...
describe("resolveViewedImageAsset", () => {
	...
	it("serves t3 attachment paths in place like any other host path", () => {
		const path =
			"/Users/demo/.t3/dev/attachments/11111111-1111-4111-8111-111111111111.png";
		expect(
			resolveViewedImageAsset(path, { threadId, workspaceRoot: "/workspace" }),
		).toEqual({
	...
	it("normalizes workspace image sources", () => {
		...
		expect(
			resolveViewedImageAsset("https://example.com/logo.png", { threadId }),
		).toBeNull();
	});
});
