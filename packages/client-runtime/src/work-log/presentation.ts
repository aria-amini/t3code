...
> = {
	...
	preview_set_appearance: [
		...
		"Setting",
		"Set",
		"preview browser appearance",
	],
	preview_recording_start: [
		"Start",
		"Starting",
		"Started",
		"recording the preview browser",
	],
...
export function liveActivityToolStatus(
	...
	if (status === "failed" || status === "declined" || status === "stopped")
		return status;
...
export function resolveWorkEntryToolPresentation(
	entry: Pick<
		...
		"label" | "toolTitle" | "toolData" | "toolLifecycleStatus"
	>,
	...
	const acpContent = Array.isArray(data?.content)
		? data.content
				.flatMap((entryValue) => {
					...
					const text =
						entry?.type === "content" ? nonEmptyString(content?.text) : null;
					return text ? [text] : [];
				})
				.join("\n")
		: null;
...
export function workLogEntryIsToolLike(
	...
	return (
		entry.itemType !== undefined && isToolLifecycleItemType(entry.itemType)
	);
}
...
function toolDetailTextLooksLikeFailure(text: string): boolean {
	...
	return (
		...
		(normalized.includes("cannot find path") &&
			normalized.includes("because it does not exist")) ||
		(normalized.includes("is not recognized") &&
			normalized.includes("the term '")) ||
...
function workLogEntryIsLocalCodeSearch(
	entry: WorkLogPresentationEntry,
): boolean {
	return (
...
export function toolGroupAction(
	entry: WorkLogPresentationEntry,
): ToolGroupAction {
	if (
	...
	if (resolveWorkEntryToolPresentation(entry)?.icon === "browser")
		return "browser";
	if (
...
export function summarizeToolGroup(
	...
	const summaryEntries = omitSupersededLifecycleMarkers(
		entries,
		(entry) => entry,
	);
	...
	if (sources.size > 0) {
		...
		const allIntegrations = sourceValues.every(
			(source) => source.kind === "integration",
		);
		labels.unshift(
...
export function omitSupersededLifecycleMarkers<T>(
	...
	for (let index = entries.length - 1; index >= 0; index -= 1) {
		...
		const normalizedLabel = normalizeCompactToolLabel(
			workEntry.toolTitle ?? workEntry.label,
		);
		const identity = [
...
export function toolGroupSummaryKind(
	...
	const fallbackKinds = new Set(
		entries.map((entry): ToolGroupSummaryKind => {
			...
			if (entry.itemType === "collab_agent_tool_call" || entry.taskId)
				return "agent-tool";
	...
	return fallbackKinds.size === 1
		? fallbackKinds.values().next().value!
		: "mixed";
}
