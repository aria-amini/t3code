...
describe("tool-group scroll anchors", () => {
	...
	it.each([
		...
		"uses the measured position at $scroll rather than cached row $start",
		(example) => {
	...
	it("skips incomplete layout snapshots instead of saving an invalid anchor", () => {
		expect(
			resolveWorkGroupScrollAnchor({ ...state, data: [], scroll: 0 }),
		).toBeUndefined();
		expect(
			resolveWorkGroupScrollAnchor({ ...state, scroll: Number.NaN }),
		).toBeUndefined();
		expect(
			resolveWorkGroupScrollAnchor({
				...state,
