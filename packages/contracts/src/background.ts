...
export const BackgroundBooleanState = Schema.Literals([
	"true",
	"false",
	"unknown",
]);
...
export const ClientKind = Schema.Literals([
	"web",
	"desktop-renderer",
	"mobile",
	"unknown",
...
export const ClientActivityReportInput = Schema.Struct({
	...
	appState: Schema.optionalKey(
		Schema.Literals(["active", "inactive", "background", "unknown"]),
	),
	lowPowerMode: Schema.optionalKey(BackgroundBooleanState),
	batteryState: Schema.optionalKey(
		Schema.Literals(["unknown", "unplugged", "charging", "full"]),
	),
