...
const decodeResolvedRule = Schema.decodeUnknownEffect(
	ResolvedKeybindingRule as never,
);
const encodeResolvedKeybindings = Schema.encodeEffect(
	ResolvedKeybindingsConfig,
);
...
it.effect("parses keybinding rules", () =>
	Effect.gen(function* () {
		...
		assert.strictEqual(
			parsedRightPanelToggleMaximized.command,
		...
		assert.strictEqual(
			parsedThreadCopyReference.command,
