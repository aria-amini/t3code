...
const decodeFullThreadDiffInput = Schema.decodeUnknownEffect(
	OrchestrationGetFullThreadDiffInput,
);
...
const decodeClientOrchestrationCommand = Schema.decodeUnknownEffect(
	ClientOrchestrationCommand,
);
const decodeOrchestrationMessage =
	Schema.decodeUnknownEffect(OrchestrationMessage);
const decodeThreadMessageSentPayload = Schema.decodeUnknownEffect(
	ThreadMessageSentPayload,
);
...
const decodeOrchestrationThread =
	Schema.decodeUnknownEffect(OrchestrationThread);
...
const decodeOrchestrationCommand =
	Schema.decodeUnknownEffect(OrchestrationCommand);
const decodeOrchestrationEvent = Schema.decodeUnknownEffect(OrchestrationEvent);
const decodeThreadMetaUpdatedPayload = Schema.decodeUnknownEffect(
	ThreadMetaUpdatedPayload,
);
const decodeDispatchCommandError = Schema.decodeUnknownEffect(
	OrchestrationDispatchCommandError,
);
const decodeSnapShotAccessibility = Schema.decodeUnknownEffect(
	SnapShotAccessibility,
);

it.effect(
	"decodes a dispatch error after its bootstrap thread was deleted",
	() =>
		Effect.gen(function* () {
...
it.effect(
	"parses full thread diff input with whitespace ignoring enabled",
	() =>
		Effect.gen(function* () {
...
it.effect(
	"decodes project.create with createWorkspaceRootIfMissing enabled",
	() =>
		Effect.gen(function* () {
			const parsed = yield* decodeProjectCreateCommand({
				type: "project.create",
...
it.effect(
	"decodes historical project.created payloads with a default provider",
	() =>
		Effect.gen(function* () {
...
it.effect(
	"decodes project.meta-updated payloads with explicit default provider",
	() =>
		Effect.gen(function* () {
			...
			assert.strictEqual(
				parsed.defaultModelSelection?.instanceId,
				"claudeAgent",
			);
		}),
);
...
it.effect(
	"accepts inline images, uploaded images, and uploaded files from clients",
	() =>
		Effect.gen(function* () {
...
it.effect(
	"tolerates attachment types from newer builds when decoding messages",
	() =>
		Effect.gen(function* () {
...
it.effect(
	"rejects malformed known attachment types instead of tolerating them",
	() =>
		Effect.gen(function* () {
			const base = {
...
it.effect(
	"rejects accessibility trees above the serialized payload limit",
	() =>
		Effect.gen(function* () {
			const result = yield* Effect.exit(
...
it.effect("accepts bootstrap metadata in thread.turn.start", () =>
	Effect.gen(function* () {
		...
		assert.strictEqual(
			parsed.bootstrap?.prepareWorktree?.startFromOrigin,
			true,
		);
...
it.effect("decodes thread.meta-updated payloads with explicit provider", () =>
	Effect.gen(function* () {
		...
		assert.strictEqual(
			parsed.titleRegeneration?.requestId,
...
it.effect("accepts provider-scoped model options in thread.turn.start", () =>
	Effect.gen(function* () {
		...
		assert.strictEqual(
			getOptionValue(parsed.modelSelection?.options, "fastMode"),
			true,
		);
	}),
);
...
it.effect(
	"normalizes legacy object-shaped modelSelection.options on decode",
	() =>
		Effect.gen(function* () {
			const parsed = yield* decodeThreadCreatedPayload({
				threadId: "thread-1",
...
it.effect(
	"normalizes legacy object-shaped defaultModelSelection.options on decode",
	() =>
		Effect.gen(function* () {
			const parsed = yield* decodeProjectCreatedPayload({
				projectId: "project-1",
...
it.effect(
	...
	() =>
		Effect.gen(function* () {
			...
			assert.deepStrictEqual(encoded.modelSelection.options, [
				{ id: "fastMode", value: true },
			]);
		}),
);
...
it.effect(
	"decodes active reorder commands through client and orchestration boundaries",
	() =>
		Effect.gen(function* () {
			const input = {
...
it.effect(
	"decodes active placement on existing metadata events while accepting old payloads",
	() =>
		Effect.gen(function* () {
			const payload = {
				threadId: "thread-1",
...
it.effect(
	"accepts pull request synchronization only as an internal command",
	() =>
		Effect.gen(function* () {
			const pullRequest = {
			...
			assert.ok(
				yield* decodeClientOrchestrationCommand(command).pipe(Effect.flip),
			);

			const cleared = { ...command, branchPullRequest: null };
			assert.deepStrictEqual(
				yield* decodeOrchestrationCommand(cleared),
				cleared,
			);
...
it.effect(
	"decodes thread.turn-start-requested source proposed plan metadata when present",
	() =>
		Effect.gen(function* () {
			const parsed = yield* decodeThreadTurnStartRequestedPayload({
				threadId: "thread-2",
...
it.effect(
	"decodes latest turn source proposed plan metadata when present",
	() =>
		Effect.gen(function* () {
...
it.effect(
	"defaults proposed plan implementation metadata for historical rows",
	() =>
		Effect.gen(function* () {
...
it.effect(
	"ModelSelection migrates legacy `provider` field to `instanceId`",
	() =>
		Effect.gen(function* () {
...
it.effect(
	"ModelSelection prefers explicit instanceId over legacy provider",
	() =>
		Effect.gen(function* () {
			const parsed = yield* decodeModelSelection({
				provider: "codex",
...
it("isProviderSendTurnSupportedImageMimeType accepts raster formats and rejects svg", () => {
	assert.strictEqual(
		isProviderSendTurnSupportedImageMimeType("image/png"),
		true,
	);
	assert.strictEqual(
		isProviderSendTurnSupportedImageMimeType("IMAGE/JPEG"),
		true,
	);
	assert.strictEqual(
		isProviderSendTurnSupportedImageMimeType("image/svg+xml"),
		false,
	);
});
