...
export function isProviderSendTurnSupportedImageMimeType(
	mimeType: string,
): boolean {
	return PROVIDER_SEND_TURN_SUPPORTED_IMAGE_MIME_TYPE_SET.has(
...
export const SnapShotAccessibilityNode: Schema.Codec<SnapShotAccessibilityNode> =
	Schema.Struct({
		role: TrimmedNonEmptyString.check(Schema.isMaxLength(100)),
		name: Schema.optionalKey(
			TrimmedNonEmptyString.check(Schema.isMaxLength(1_000)),
		),
		value: Schema.optionalKey(
			TrimmedNonEmptyString.check(Schema.isMaxLength(8_000)),
		),
		description: Schema.optionalKey(
			TrimmedNonEmptyString.check(Schema.isMaxLength(2_000)),
		),
		...
		children: Schema.mutable(
			Schema.Array(
				Schema.suspend(
					(): Schema.Codec<SnapShotAccessibilityNode> =>
...
const SnapShotAccessibilityWire = Schema.Union([
	Schema.Struct({
		...
		text: TrimmedNonEmptyString.check(
			Schema.isMaxLength(SNAP_SHOT_ACCESSIBLE_TEXT_MAX_CHARS),
		),
...
export const SnapShotAccessibility = SnapShotAccessibilityWire.check(
	Schema.makeFilter((accessibility: typeof SnapShotAccessibilityWire.Type) => {
		...
		return (
			JSON.stringify(accessibility).length <=
...
export const OrchestrationThread = Schema.Struct({
	...
	settledAt: Schema.NullOr(IsoDateTime).pipe(
		Schema.withDecodingDefault(Effect.succeed(null)),
	),
...
export const OrchestrationThreadShell = Schema.Struct({
	...
	backgroundLiveness: Schema.optional(
		Schema.NullOr(Schema.Literals(["working", "monitoring"])),
	),
	/**
...
export type OrchestrationSubscribeShellInput =
	typeof OrchestrationSubscribeShellInput.Type;
...
export type OrchestrationThreadDetailWindow =
	typeof OrchestrationThreadDetailWindow.Type;
...
export type OrchestrationThreadDetailPage =
	typeof OrchestrationThreadDetailPage.Type;
...
const ClientThreadTurnStartCommand = Schema.Struct({
	...
	message: Schema.Struct({
		...
		attachments: Schema.Array(
			Schema.Union([UploadChatAttachment, ChatAttachment]),
		),
	}),
	modelSelection: Schema.optional(ModelSelection),
	titleSeed: Schema.optional(TrimmedNonEmptyString),
...
export type OrchestrationThreadSearchSource =
	typeof OrchestrationThreadSearchSource.Type;
...
export type OrchestrationSearchThreadsInput =
	typeof OrchestrationSearchThreadsInput.Type;
...
export type OrchestrationThreadSearchMatch =
	typeof OrchestrationThreadSearchMatch.Type;

export const OrchestrationSearchThreadsResult = Schema.Struct({
	matches: Schema.Array(OrchestrationThreadSearchMatch),
});
export type OrchestrationSearchThreadsResult =
	typeof OrchestrationSearchThreadsResult.Type;
...
export type OrchestrationGetWorkflowScriptInput =
	typeof OrchestrationGetWorkflowScriptInput.Type;
...
export type OrchestrationGetWorkflowScriptResult =
	typeof OrchestrationGetWorkflowScriptResult.Type;
