...
const decodeConfiguredLocalServerUrls = Schema.decodeUnknownSync(
	ConfiguredLocalServerUrls,
);
const decodeViewport = Schema.decodeUnknownSync(PreviewViewportSetting);
const decodeResizeInput = Schema.decodeUnknownSync(
	PreviewAutomationResizeInput,
);
const decodeOpenInput = Schema.decodeUnknownSync(PreviewAutomationOpenInput);
const decodeResizeResult = Schema.decodeUnknownSync(
	PreviewAutomationResizeResult,
);
...
const decodeAutomationStatus = Schema.decodeUnknownSync(
	PreviewAutomationStatus,
);
...
describe("PreviewNavStatus", () => {
	...
	it("rejects empty url", () => {
		expect(() =>
			decodeNavStatus({ _tag: "Loading", url: "", title: "" }),
		).toThrow();
	});
});
...
describe("PreviewViewportSetting", () => {
	it("decodes fill, freeform, and preset modes", () => {
		...
		expect(
			decodeViewport({ _tag: "freeform", width: 1024, height: 768 }),
		).toEqual({
			_tag: "freeform",
			width: 1024,
			height: 768,
	...
	it("rejects unsafe dimensions and oversized render areas", () => {
		expect(() =>
			decodeViewport({ _tag: "freeform", width: 100, height: 800 }),
		).toThrow();
		expect(() =>
			decodeViewport({ _tag: "freeform", width: 3840, height: 3840 }),
		).toThrow();
	});
});
...
describe("PreviewAutomationResizeInput", () => {
	it("requires fields that match the selected mode", () => {
		...
		expect(() =>
			decodeResizeInput({ mode: "preset", preset: "pixel-8" }),
		).toThrow();
		expect(() =>
			decodeResizeInput({ mode: "freeform", width: 1024 }),
		).toThrow();
		expect(() =>
			decodeResizeInput({ mode: "fill", width: 1024, height: 768 }),
		).toThrow();
	});
...
describe("preview automation tab targeting", () => {
	it("accepts an explicit tab and rejects contradictory open behavior", () => {
		...
		expect(
			decodeOpenInput({ tabId: "tab-app", reuseExistingTab: true }),
		).toMatchObject({
			tabId: "tab-app",
			reuseExistingTab: true,
		});
		expect(() =>
			decodeOpenInput({ tabId: "tab-app", reuseExistingTab: false }),
		).toThrow();
	});
});
