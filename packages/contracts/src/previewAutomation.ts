...
const OptionalTimeoutMs = Schema.optional(
	Schema.Int.check(Schema.isGreaterThan(0))
		...
		.annotate({
			description:
				"Maximum wait in milliseconds. Defaults to 15000; maximum 60000.",
		}),
).annotate({
	description:
		"Maximum wait in milliseconds. Defaults to 15000; maximum 60000.",
...
export const PreviewAutomationTabTargetInput = Schema.Struct(
	PreviewAutomationTabTargetFields,
);
export type PreviewAutomationTabTargetInput =
	typeof PreviewAutomationTabTargetInput.Type;
...
export const BrowserNavigationTarget = Schema.Union([
	...
	Schema.Struct({
		kind: Schema.Literal("environment-port").annotate({
			description:
				"Selects a dev-server port relative to the current execution environment.",
		}),
		port: Schema.Int.check(Schema.isGreaterThan(0))
			.check(Schema.isLessThan(65_536))
			.annotate({
				description: "Dev-server TCP port inside the current environment.",
			}),
		...
		path: Schema.optional(
			Schema.String.annotate({
				description:
					"Optional path, query, and fragment, for example /settings?tab=account.",
			}),
		),
	}),
]);
...
export const PreviewAutomationNavigateInput = Schema.Struct({
	...
	.check(
		Schema.makeFilter(
			(input) =>
				Number(input.url !== undefined) + Number(input.target !== undefined) ===
					1 || "Provide exactly one of url or target.",
		),
	)
	.annotate({
		description:
...
export type PreviewAutomationNavigateInput =
	typeof PreviewAutomationNavigateInput.Type;
...
export const PreviewAutomationResizeInput = Schema.Struct({
	...
	preset: Schema.optional(
		PreviewViewportPresetId.annotate({
			description:
				"Named viewport from Chrome DevTools' standard device catalog.",
		}),
	).annotate({
	...
	width: Schema.optional(
		PreviewViewportSize.fields.width.annotate({
			description:
				"Freeform viewport width in CSS pixels. Required only in freeform mode.",
		}),
	).annotate({
		description:
			"Freeform viewport width in CSS pixels. Required only in freeform mode.",
	...
	height: Schema.optional(
		PreviewViewportSize.fields.height.annotate({
			description:
				"Freeform viewport height in CSS pixels. Required only in freeform mode.",
		}),
	).annotate({
		description:
			"Freeform viewport height in CSS pixels. Required only in freeform mode.",
	...
	.check(
		Schema.makeFilter((input) => {
			...
			if (
				hasWidth &&
				hasHeight &&
				input.width! * input.height! > PREVIEW_VIEWPORT_MAX_AREA
			) {
...
export type PreviewAutomationResizeInput =
	typeof PreviewAutomationResizeInput.Type;
...
export type PreviewAutomationResizeResult =
	typeof PreviewAutomationResizeResult.Type;

/** Mirrors DesktopPreviewColorScheme; declared here to keep this module free of ipc.ts imports. */
export const PreviewAutomationColorScheme = Schema.Literals([
	"system",
	"light",
	"dark",
]);
export type PreviewAutomationColorScheme =
	typeof PreviewAutomationColorScheme.Type;
...
export type PreviewAutomationSetColorSchemeInput =
	typeof PreviewAutomationSetColorSchemeInput.Type;
...
export const PreviewAutomationClickInput = Schema.Struct({
	...
	x: Schema.optional(
		Schema.Finite.annotate({
			description:
				"Viewport-relative X coordinate in CSS pixels. Must be paired with y.",
		}),
	),
	y: Schema.optional(
		Schema.Finite.annotate({
			description:
				"Viewport-relative Y coordinate in CSS pixels. Must be paired with x.",
		}),
	),
	timeoutMs: OptionalTimeoutMs,
})
	.check(
		Schema.makeFilter((input) => {
			...
			return (
				...
				"Provide exactly one click target."
			);
		}),
	)
	.annotate({
...
export type PreviewAutomationClickInput =
	typeof PreviewAutomationClickInput.Type;
...
export const PreviewAutomationTypeInput = Schema.Struct({
	...
	clear: Schema.optional(
		Schema.Boolean.annotate({
			description:
				"Clear the existing input value before inserting text. Defaults to false.",
		}),
	),
	timeoutMs: OptionalTimeoutMs,
})
...
export const PreviewAutomationPressInput = Schema.Struct({
	...
	description: "Presses one keyboard key in the active browser tab.",
});
export type PreviewAutomationPressInput =
	typeof PreviewAutomationPressInput.Type;
...
export const PreviewAutomationScrollInput = Schema.Struct({
	...
	deltaX: Schema.optional(
		Schema.Finite.annotate({
			description:
				"Horizontal scroll delta in CSS pixels. Positive scrolls right. Defaults to 0.",
		}),
	),
	deltaY: Schema.optional(
		Schema.Finite.annotate({
			description:
				"Vertical scroll delta in CSS pixels. Positive scrolls down. Defaults to 0.",
		}),
	),
	selector: Schema.optional(LegacySelector).annotate({
		description:
			"Legacy CSS selector for a scrollable container. Omit to scroll the viewport.",
	}),
	locator: Schema.optional(Locator).annotate({
		description:
			"Playwright selector for a scrollable container. Omit to scroll the viewport.",
	}),
})
	.check(
		Schema.makeFilter((input) => {
...
export type PreviewAutomationScrollInput =
	typeof PreviewAutomationScrollInput.Type;
...
export type PreviewAutomationEvaluateInput =
	typeof PreviewAutomationEvaluateInput.Type;

export const PreviewAutomationWaitForInput = Schema.Struct({
	...PreviewAutomationTabTargetFields,
	selector: Schema.optional(LegacySelector).annotate({
		description:
			"Legacy CSS selector that must match an element. Prefer locator.",
	}),
	locator: Schema.optional(Locator).annotate({
		description:
	...
	text: Schema.optional(
		TrimmedNonEmptyString.annotate({
			description:
				"Case-sensitive substring that must appear in visible document text.",
		}),
	).annotate({
		description:
			"Case-sensitive substring that must appear in visible document text.",
...
export type PreviewAutomationWaitForInput =
	typeof PreviewAutomationWaitForInput.Type;
...
export type PreviewAutomationConsoleEntry =
	typeof PreviewAutomationConsoleEntry.Type;
...
export type PreviewAutomationNetworkEntry =
	typeof PreviewAutomationNetworkEntry.Type;
...
export type PreviewAutomationActionEvent =
	typeof PreviewAutomationActionEvent.Type;
...
export type PreviewAutomationRecordingStatus =
	typeof PreviewAutomationRecordingStatus.Type;
...
export type PreviewAutomationRecordingArtifact =
	typeof PreviewAutomationRecordingArtifact.Type;

export const PreviewAutomationClientId = TrimmedNonEmptyString.check(
	Schema.isMaxLength(128),
);
export type PreviewAutomationClientId = typeof PreviewAutomationClientId.Type;
export const PreviewAutomationConnectionId = TrimmedNonEmptyString.check(
	Schema.isMaxLength(64),
);
export type PreviewAutomationConnectionId =
	typeof PreviewAutomationConnectionId.Type;

export const PreviewAutomationHostIdentity = Schema.Struct({
	clientId: PreviewAutomationClientId,
	environmentId: EnvironmentId,
});
export type PreviewAutomationHostIdentity =
	typeof PreviewAutomationHostIdentity.Type;
...
export type PreviewAutomationStreamEvent =
	typeof PreviewAutomationStreamEvent.Type;
...
const PreviewAutomationOptionalRemoteDiagnosticFields = {
	...
	remoteMessageLength: Schema.optional(
...
export class PreviewAutomationInvalidSelectorError extends Schema.TaggedError<PreviewAutomationInvalidSelectorError>()(
	"PreviewAutomationInvalidSelectorError",
	{
		...
		selectorLength: Schema.optional(
			Schema.Int.check(Schema.isGreaterThanOrEqualTo(0)),
		),
	},
) {
...
export class PreviewAutomationTargetNotEditableError extends Schema.TaggedError<PreviewAutomationTargetNotEditableError>()(
	"PreviewAutomationTargetNotEditableError",
	{
		...PreviewAutomationRequestErrorFields,
		...PreviewAutomationRemoteDiagnosticFields,
		selectorKind: Schema.optional(
			Schema.Literals(["focused-element", "locator", "selector"]),
		),
		selectorLength: Schema.optional(
			Schema.Int.check(Schema.isGreaterThanOrEqualTo(0)),
		),
