...
const decodeSearchEntriesInput = Schema.decodeUnknownSync(
	ProjectSearchEntriesInput,
);
const decodeSearchContentsInput = Schema.decodeUnknownSync(
	ProjectSearchContentsInput,
);
...
describe("project RPC errors", () => {
	it("derives stable messages from structured request context while retaining causes", () => {
		...
		expect(searchError.message).toBe(
			"Failed to search workspace entries in '/workspace'.",
		);
		expect(searchError.message).not.toContain(cause.message);
		expect(searchError.normalizedCwd).toBe("/workspace");
		expect(searchError.queryLength).toBe(
			"authorization: Bearer secret-token".length,
		);
		...
		expect(readError.message).toBe(
			"Failed to read workspace file 'src/index.ts' in '/workspace'.",
		);
		...
		expect(contentSearchError.message).toBe(
	...
	it("decodes legacy message-only errors during rolling upgrades", () => {
		const decodeSearchError = Schema.decodeUnknownSync(
			ProjectSearchEntriesError,
		);
