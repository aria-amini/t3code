...
export const ProjectSearchContentsInput = Schema.Struct({
	...
	limit: PositiveInt.check(
		Schema.isLessThanOrEqualTo(PROJECT_SEARCH_CONTENTS_MAX_LIMIT),
	),
...
export type ProjectSearchContentsResult =
	typeof ProjectSearchContentsResult.Type;
...
export class ProjectListEntriesError extends Schema.TaggedError<ProjectListEntriesError>()(
	...
	constructor(props: ProjectEntriesFailureContext & { readonly cwd: string }) {
		super({
			...
			message:
				decodedProjectErrorMessage(props) ??
				`Failed to list workspace entries in '${props.cwd}'.`,
		} as any);
	}
}
