...
const decodeProviderUploadFeedbackInput = Schema.decodeUnknownSync(
	ProviderUploadFeedbackInput,
);
...
describe("ProviderSessionStartInput", () => {
	it("accepts codex-compatible payloads", () => {
		...
		expect(getOptionValue(parsed.modelSelection?.options, "fastMode")).toBe(
			true,
		);
	});
	...
	it("accepts claude runtime knobs", () => {
		...
		expect(getOptionValue(parsed.modelSelection?.options, "thinking")).toBe(
			true,
		);
		expect(getOptionValue(parsed.modelSelection?.options, "effort")).toBe(
			"max",
		);
		expect(getOptionValue(parsed.modelSelection?.options, "fastMode")).toBe(
			true,
...
describe("provider feedback", () => {
	...
	it("returns the shareable Codex feedback identifier", () => {
		expect(
			decodeProviderUploadFeedbackResult({ feedbackId: "provider-thread-1" }),
		).toEqual({
	...
	it("keeps the failed thread and original cause without exposing upstream text", () => {
		...
		expect(error.message).toBe(
			"Failed to upload feedback for thread thread-1.",
		);
