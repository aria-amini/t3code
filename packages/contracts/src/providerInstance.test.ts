...
const decodeProviderInstanceConfig = Schema.decodeUnknownSync(
	ProviderInstanceConfig,
);
const decodeProviderInstanceConfigMap = Schema.decodeUnknownSync(
	ProviderInstanceConfigMap,
);
...
describe("ProviderInstanceConfig", () => {
	...
	it("rejects a blank displayName (must be trimmed non-empty)", () => {
		expect(() =>
			decodeProviderInstanceConfig({ driver: "codex", displayName: "   " }),
		).toThrow();
	});
	...
	it("rejects driver values that do not satisfy the slug pattern", () => {
		...
		expect(() =>
			decodeProviderInstanceConfig({ driver: "has spaces" }),
		).toThrow();
	});
});
...
describe("ProviderInstanceConfigMap", () => {
	it("decodes a multi-instance map mixing first-party and fork drivers", () => {
		...
		expect(decoded[ProviderInstanceId.make("codex_personal")]?.driver).toBe(
			"codex",
		);
		expect(decoded[ProviderInstanceId.make("codex_work")]?.config).toEqual({
			homePath: "~/.codex_work",
		});
		expect(decoded[ProviderInstanceId.make("ollama_local")]?.driver).toBe(
			"ollama",
		);
	});
