...
describe("ProviderRuntimeEvent", () => {
	it("includes every runtime event in the public event type", () => {
		...
		>().toEqualTypeOf<ProviderRuntimeEventType>();
	});
...
describe("classifyTaskAgentKind", () => {
	it("classifies agent-flavored, watch-loop, and inert types", () => {
		...
		expect(classifyTaskAgentKind({ taskType: "brand_new_agent_type" })).toBe(
			"agent",
		);
		expect(classifyTaskAgentKind({ taskType: "local_bash" })).toBe(
			"background",
		);
	...
	it("agent-owned tasks are background unless themselves agent-flavored", () => {
		expect(
			classifyTaskAgentKind({ taskType: "local_bash", agentId: "owner" }),
		).toBe("background");
		expect(
			classifyTaskAgentKind({ taskType: undefined, agentId: "owner" }),
		).toBe("background");
		// Nested agent: outlives its parent, stays in the roster.
		expect(
			classifyTaskAgentKind({ taskType: "local_agent", agentId: "owner" }),
		).toBe("agent");
	});
});
