...
export type ToolActivityNativeAppReference =
	typeof ToolActivityNativeAppReference.Type;
...
export const ToolActivityIcon = Schema.Union([
	Schema.TaggedStruct("website", {
		...
		faviconUrl: Schema.optional(
			TrimmedNonEmptyStringSchema.check(Schema.isMaxLength(4096)),
		),
		faviconUrlDark: Schema.optional(
			TrimmedNonEmptyStringSchema.check(Schema.isMaxLength(4096)),
		),
	}),
...
export function classifyTaskAgentKind(input: {
	...
	const nonAgentType =
		taskType !== undefined &&
		(MONITOR_TASK_TYPES.has(taskType) || INERT_TASK_TYPES.has(taskType));
...
export type ProviderRuntimeTaskUpdatedEvent =
	typeof ProviderRuntimeTaskUpdatedEvent.Type;
...
export type ProviderRuntimeToolDeniedEvent =
	typeof ProviderRuntimeToolDeniedEvent.Type;
