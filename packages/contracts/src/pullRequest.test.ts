...
const decodeReviewerRequest = Schema.decodeUnknownSync(
	PullRequestReviewerRequestInput,
);
	...
	entries: [
		{
			...
			host: "github.com",
			projectId:
				"project-1" as PullRequestListResult["entries"][number]["projectId"],
			projectTitle: "t3code",
...
describe("PullRequestListResult", () => {
	...
	it("round-trips through the JSON codec the RPC serializes with", () => {
		...
		const decoded = Schema.decodeUnknownSync(codec)(
			Schema.encodeUnknownSync(codec)(LIST_RESULT),
		);
...
describe("PullRequestListInput", () => {
	it("trims a search, so what is sent is what was typed", () => {
		expect(decodeListInput({ state: "open", query: "  page  " }).query).toBe(
			"page",
		);
	});
	...
	it("bounds a search, because it travels into a command and a query string", () => {
		expect(
			decodeListInput({ state: "open", query: "p".repeat(200) }).query,
		).toHaveLength(200);
		expect(() =>
			decodeListInput({ state: "open", query: "p".repeat(201) }),
		).toThrow();
	});

	it("takes back the continuation a result handed out, keyed the way it arrived", () => {
		const cursors = {
			"github.com pingdotgg/t3code": "2026-07-02T00:00:00Z|99|1,2",
		};

		expect(decodeListInput({ state: "open", cursors }).cursors).toStrictEqual(
			cursors,
		);
	});
	...
	it("bounds a continuation, because it comes back from the page and goes into a filter", () => {
		const long = (length: number) => ({
			"github.com acme/web": "c".repeat(length),
		});
		expect(
			decodeListInput({ state: "open", cursors: long(4096) }),
		).toBeDefined();
		expect(() =>
			decodeListInput({ state: "open", cursors: long(4097) }),
		).toThrow();
	});
});
...
describe("PullRequestReviewerRequestInput", () => {
	...
	it("carries the same shape whichever direction the request goes", () => {
		...
		for (const requested of [true, false]) {
			expect(
				decodeReviewerRequest({ ...ref, reviewers: [reviewer], requested })
					.requested,
			).toBe(requested);
		}
	});
	...
	it("refuses a request that names nobody, which no host would do anything with", () => {
		expect(() =>
			decodeReviewerRequest({ ...ref, reviewers: [], requested: true }),
		).toThrow();
	});
	...
	it("bounds the reviewers, because they travel into a body the page composed", () => {
		...
		expect(
			decodeReviewerRequest({ ...ref, reviewers: many(25), requested: true })
				.reviewers,
		).toHaveLength(25);
		expect(() =>
			decodeReviewerRequest({ ...ref, reviewers: many(26), requested: true }),
		).toThrow();
	});
...
describe("updating a branch that has fallen behind its base", () => {
	...
	it("carries the way the branch should be brought up to date", () => {
		expect(
			decodeAction({ ...ref, action: "update-branch", updateMethod: "rebase" }),
		).toMatchObject({
	...
	it("takes the action without a method, which is the host's own default", () => {
		expect(
			decodeAction({ ...ref, action: "update-branch" }).updateMethod,
		).toBeUndefined();
	});
...
describe("leaving a merge for the host to make once it is ready", () => {
	...
	it("takes the arming back without a strategy, because there is nothing to choose", () => {
		expect(
			decodeAction({ ...ref, action: "disable-auto-merge" }).mergeMethod,
		).toBeUndefined();
	});
});
...
describe("naming the reader as the author to narrow by", () => {
	...
	it("leaves any other name as it was typed", () => {
		expect(resolvePullRequestAuthorFilter(" mercedes ", "octocat")).toBe(
			"mercedes",
		);
