...
export type RelayAgentAwarenessPlatform =
	typeof RelayAgentAwarenessPlatform.Type;
...
export type RelayAgentAwarenessPreferences =
	typeof RelayAgentAwarenessPreferences.Type;
...
export type RelayDeviceRegistrationRequest =
	typeof RelayDeviceRegistrationRequest.Type;
...
export type RelayLiveActivityRegistrationRequest =
	typeof RelayLiveActivityRegistrationRequest.Type;

export const RelayDeviceUnregistrationParams = Schema.Struct({
	deviceId: TrimmedNonEmptyString,
});
export type RelayDeviceUnregistrationParams =
	typeof RelayDeviceUnregistrationParams.Type;
...
export type RelayAgentActivityAggregateRow =
	typeof RelayAgentActivityAggregateRow.Type;
...
export type RelayAgentActivityAggregateState =
	typeof RelayAgentActivityAggregateState.Type;
...
export type RelayManagedEndpointProviderKind =
	typeof RelayManagedEndpointProviderKind.Type;
...
export type RelayManagedEndpointRuntimeConfig =
	typeof RelayManagedEndpointRuntimeConfig.Type;
...
export type RelayEnvironmentConfigRequest =
	typeof RelayEnvironmentConfigRequest.Type;
...
export const RelayAgentActivityPublishRequest = Schema.Struct({
	state: Schema.NullOr(RelayAgentActivityState).annotate({
		description:
			"Current agent-awareness state, or null to remove the published state.",
	}),
	proof: TrimmedNonEmptyString.annotate({
		description:
			"Environment-signed JWT covering this published activity state.",
	}),
}).annotate({
	description: "Publishes a signed agent-awareness update from an environment.",
});
export type RelayAgentActivityPublishRequest =
	typeof RelayAgentActivityPublishRequest.Type;
...
export type RelayEnvironmentLinkProofPayload =
	typeof RelayEnvironmentLinkProofPayload.Type;
...
export const RelayEnvironmentLinkChallengeRequest = Schema.Struct({
	...
	managedTunnelsEnabled: Schema.Boolean.annotate({
		description:
			"Whether the relay should provision a managed tunnel for this environment.",
	}),
}).annotate({
	description: "Requested capabilities for a new environment-link challenge.",
});
export type RelayEnvironmentLinkChallengeRequest =
	typeof RelayEnvironmentLinkChallengeRequest.Type;
...
export const RelayEnvironmentLinkRequest = Schema.Struct({
	deviceId: Schema.optional(
		TrimmedNonEmptyString.annotate({
			description:
				"Optional client device identifier associated with this link.",
		}),
	),
	proof: RelayEnvironmentLinkProof.annotate({
		description:
			"Environment-signed proof bound to a previously issued link challenge.",
	}),
	notificationsEnabled: Schema.Boolean,
	liveActivitiesEnabled: Schema.Boolean,
...
export type RelayEnvironmentLinkRequest =
	typeof RelayEnvironmentLinkRequest.Type;
...
export type RelayEnvironmentLinkResponse =
	typeof RelayEnvironmentLinkResponse.Type;
...
export type RelayEnvironmentLinkFailedReason =
	typeof RelayEnvironmentLinkFailedReason.Type;
...
export type RelayClientEnvironmentRecord =
	typeof RelayClientEnvironmentRecord.Type;

export const RelayListEnvironmentsResponse = Schema.Struct({
	environments: Schema.Array(RelayClientEnvironmentRecord),
});
export type RelayListEnvironmentsResponse =
	typeof RelayListEnvironmentsResponse.Type;

export const RelayEnvironmentConnectRequest = Schema.Struct({
	deviceId: Schema.optional(
		TrimmedNonEmptyString.annotate({
			description:
				"Optional client device identifier requesting the connection.",
		}),
	),
	...
	clientProofKeyThumbprint: Schema.optional(
		TrimmedNonEmptyString.annotate({
			description:
				"JWK thumbprint that the minted environment credential must be bound to.",
		}),
	),
}).annotate({
	description:
		"Requests a short-lived credential for connecting to an environment.",
});
export type RelayEnvironmentConnectRequest =
	typeof RelayEnvironmentConnectRequest.Type;
...
export const RelayJwtSubjectTokenType =
	"urn:ietf:params:oauth:token-type:jwt" as const;
export const RelayAccessTokenType =
...
export const RelayDpopAccessTokenRequest = Schema.Struct({
	...
	resource: TrimmedNonEmptyString.annotate({
		description:
			"Relay issuer URL that will receive the DPoP-bound access token.",
	}),
	...
	.annotate({
		description:
			"OAuth token exchange request for a DPoP-bound relay access token.",
	})
	.pipe(HttpApiSchema.asFormUrlEncoded());
export type RelayDpopAccessTokenRequest =
	typeof RelayDpopAccessTokenRequest.Type;
...
export type RelayDpopAccessTokenResponse =
	typeof RelayDpopAccessTokenResponse.Type;
...
export type RelayEnvironmentUnlinkParams =
	typeof RelayEnvironmentUnlinkParams.Type;
...
export type RelayEnvironmentConnectResponse =
	typeof RelayEnvironmentConnectResponse.Type;

export const RelayEnvironmentStatusValue = Schema.Literals([
	"online",
	"offline",
]);
export type RelayEnvironmentStatusValue =
	typeof RelayEnvironmentStatusValue.Type;
...
export type RelayEnvironmentStatusResponse =
	typeof RelayEnvironmentStatusResponse.Type;
...
export type RelayCloudMintCredentialProofPayload =
	typeof RelayCloudMintCredentialProofPayload.Type;

export const RelayCloudMintCredentialProof = TrimmedNonEmptyString;
export type RelayCloudMintCredentialProof =
	typeof RelayCloudMintCredentialProof.Type;

export const RelayCloudMintCredentialRequest = Schema.Struct({
	proof: RelayCloudMintCredentialProof,
});
export type RelayCloudMintCredentialRequest =
	typeof RelayCloudMintCredentialRequest.Type;
...
export type RelayCloudEnvironmentHealthProof =
	typeof RelayCloudEnvironmentHealthProof.Type;

export const RelayCloudEnvironmentHealthRequest = Schema.Struct({
	proof: RelayCloudEnvironmentHealthProof,
});
export type RelayCloudEnvironmentHealthRequest =
	typeof RelayCloudEnvironmentHealthRequest.Type;
...
export type RelayEnvironmentHealthResponse =
	typeof RelayEnvironmentHealthResponse.Type;
...
export type RelayEnvironmentMintResponse =
	typeof RelayEnvironmentMintResponse.Type;
...
export type RelayAgentActivitySnapshotResponse =
	typeof RelayAgentActivitySnapshotResponse.Type;
...
const RelayClientGroup = HttpApiGroup.make("client")
	.add(
		...
		HttpApiEndpoint.delete(
			"unlinkEnvironment",
			"/v1/client/environment-links/:environmentId",
			{
				headers: RelayBearerRequestHeaders,
	...
	.annotate(
		OpenApi.Description,
		"Cloud-user environment links and registered devices.",
	)
...
const RelayTokenGroup = HttpApiGroup.make("token")
	...
	.annotate(
		OpenApi.Description,
		"OAuth token exchange for DPoP-bound client access.",
	);
...
const RelayDpopClientGroup = HttpApiGroup.make("dpopClient")
	...
	.annotate(
		OpenApi.Description,
		"DPoP-authenticated client access to linked environments.",
	)
	.middleware(RelayDpopClientAuth);
...
const RelayServerGroup = HttpApiGroup.make("server")
	...
	.annotate(
		OpenApi.Description,
		"Environment-authenticated activity publication.",
	)
