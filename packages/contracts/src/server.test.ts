...
const decodeUpsertKeybindingResult = Schema.decodeUnknownSync(
	ServerUpsertKeybindingResult,
);
const decodeAvailableEditors = Schema.decodeUnknownSync(
	ServerConfig.fields.availableEditors,
);
...
describe("ServerProvider", () => {
	...
	it("decodes continuation group metadata", () => {
		...
		expect(parsed.continuation?.groupKey).toBe(
			"codex:home:/Users/julius/.codex",
		);
	});
...
describe("server config forward compatibility", () => {
	...
	it("drops editor ids this build does not know", () => {
		const parsed = decodeAvailableEditors([
			"zed",
			"some-future-editor",
			"vscode",
		]);
