...
describe("ServerSettings usage price overrides", () => {
	...
	it("accepts zero rates, optional cache rates, and per-model deletion", () => {
		...
		expect(
			decodeServerSettingsPatch({ usagePriceOverrides: overrides })
				.usagePriceOverrides,
		).toEqual(overrides);
	});
	...
	it.each([
		...
		for (const value of [-1, Number.NaN, Number.POSITIVE_INFINITY, "2"]) {
			const usagePriceOverrides = {
				"example-model": { ...prices, [field]: value },
			};
			expect(() => decodeServerSettings({ usagePriceOverrides })).toThrow();
			expect(() =>
				decodeServerSettingsPatch({ usagePriceOverrides }),
			).toThrow();
		}
	});
...
describe("custom model settings", () => {
	...
	it("accepts entries at the settings patch boundary", () => {
		...
		expect(() =>
			decodeServerSettingsPatch({
				providers: { codex: { customModels: [{ name: "no slug" }] } },
			}),
		).toThrow();
	});
});
...
describe("ClaudeSettings auto-compaction", () => {
	...
	it.each(["100000", "300000", "1000000"])(
		...
		(value) => {
			expect(
				decodeClaudeSettings({ autoCompactWindow: value }).autoCompactWindow,
			).toBe(value);
		},
	);
	...
	it.each(["99999", "1000001", "300k", "invalid"])(
		...
		(value) => {
			expect(() =>
				decodeClaudeSettings({ autoCompactWindow: value }),
			).toThrow();
		},
	);
	...
	it("rejects an unsupported threshold at the settings patch boundary", () => {
		expect(() =>
			decodeServerSettingsPatch({
				providers: { claudeAgent: { autoCompactWindow: "300k" } },
			}),
		).toThrow();
		expect(
			decodeServerSettingsPatch({
...
describe("ClientSettings load balancing", () => {
	it("requires opt-in when settings are new or omit load balancing", () => {
		...
		expect(
			decodeClientSettings({ loadBalancingWeights: {} }).loadBalancingEnabled,
		).toBe(false);
	});

	it.each([true, false])(
		"preserves a saved choice of %s",
		(loadBalancingEnabled) => {
			const settings = decodeClientSettings({ loadBalancingEnabled });
			expect(encodeClientSettings(settings).loadBalancingEnabled).toBe(
				loadBalancingEnabled,
			);
			expect(
				decodeClientSettingsPatch({ loadBalancingEnabled })
					.loadBalancingEnabled,
			).toBe(loadBalancingEnabled);
		},
	);
});
...
describe("ClientSettings window capture", () => {
	...
	it("rejects unknown capture sounds", () => {
		expect(() =>
			decodeClientSettingsPatch({ snapShotSound: "doorbell" }),
		).toThrow();
	});
...
describe("ClientSettings proactive panels", () => {
	it("is opt-in and accepts client-local updates", () => {
		...
		expect(
			decodeClientSettingsPatch({ proactivePanelsEnabled: true })
...
describe("ClientSettings quit confirmation", () => {
	...
	it.each(["direct", "hold", "double-click"] as const)(
		"accepts the %s mode",
		(mode) => {
			expect(decodeClientSettings({ confirmQuit: mode }).confirmQuit).toBe(
				mode,
			);
			expect(decodeClientSettingsPatch({ confirmQuit: mode }).confirmQuit).toBe(
				mode,
			);
		},
	);
...
describe("ClientSettings browser recording frame rate", () => {
	...
	it.each([30, 60])("accepts a supported frame rate: %s", (frameRate) => {
		expect(
			decodeClientSettings({ browserRecordingFrameRate: frameRate })
				.browserRecordingFrameRate,
		).toBe(frameRate);
		expect(
			decodeClientSettingsPatch({ browserRecordingFrameRate: frameRate })
				.browserRecordingFrameRate,
		).toBe(frameRate);
	});

	it.each([24, 59, 120])(
		"rejects an unsupported frame rate: %s",
		(frameRate) => {
			expect(() =>
				decodeClientSettings({ browserRecordingFrameRate: frameRate }),
			).toThrow();
...
describe("ClientSettings glass opacity", () => {
	...
	it.each([40, 75, 100])(
		"accepts a glass opacity within the supported range: %s",
		(value) => {
			expect(decodeClientSettings({ glassOpacity: value }).glassOpacity).toBe(
				value,
			);
			expect(
				decodeClientSettingsPatch({ glassOpacity: value }).glassOpacity,
			).toBe(value);
		},
	);
});
...
describe("ClientSettings appearance contrast", () => {
	...
	it.each([49, 201, 92.5])(
		"rejects an invalid appearance contrast: %s",
		(value) => {
			expect(() =>
				decodeClientSettings({ appearanceContrast: value }),
			).toThrow();
			expect(() =>
				decodeClientSettingsPatch({ appearanceContrast: value }),
			).toThrow();
		},
	);

	it.each([50, 100, 150, 200])(
		"accepts an appearance contrast in range: %s",
		(value) => {
			expect(
				decodeClientSettings({ appearanceContrast: value }).appearanceContrast,
			).toBe(value);
			expect(
				decodeClientSettingsPatch({ appearanceContrast: value })
					.appearanceContrast,
...
describe("ClientSettings panel animations", () => {
	...
	it.each([0, 400])("accepts a panel animation duration: %s", (value) => {
		expect(
			decodeClientSettingsPatch({ panelAnimationDurationMs: value }),
		).toEqual({
	...
	it.each([-1, 401, 150.5])(
		"rejects an invalid panel animation duration: %s",
		(value) => {
			expect(() =>
				decodeClientSettingsPatch({ panelAnimationDurationMs: value }),
			).toThrow();
...
describe("ClientSettings environment identification", () => {
	it("defaults to artwork and accepts each presentation mode", () => {
		expect(decodeClientSettings({}).environmentIdentificationMode).toBe(
			"artwork",
		);
	...
	it("rejects unsupported presentation modes", () => {
		expect(() =>
			decodeClientSettings({ environmentIdentificationMode: "badge" }),
		).toThrow();
		expect(() =>
			decodeClientSettingsPatch({ environmentIdentificationMode: "badge" }),
		).toThrow();
	});
});
...
describe("ClientSettings sidebar", () => {
	...
	it("preserves an explicit legacy sidebar opt-in", () => {
		expect(
			decodeClientSettings({ legacySidebarEnabled: true }).legacySidebarEnabled,
		).toBe(true);
		expect(
			decodeClientSettingsPatch({ legacySidebarEnabled: true })
	...
	it("keeps unpin confirmation opt-in and patchable", () => {
		...
		expect(
			decodeClientSettingsPatch({ confirmThreadUnpin: true })
				.confirmThreadUnpin,
		).toBe(true);
		expect(() =>
			decodeClientSettingsPatch({ confirmThreadUnpin: "yes" }),
		).toThrow();
	});
});
...
describe("ClientSettings context window meter", () => {
	it("defaults off and preserves an explicit legacy opt-in", () => {
		...
		expect(
			decodeClientSettings({ contextWindowMeterEnabled: true })
				.contextWindowMeterEnabled,
		).toBe(true);
		expect(
			decodeClientSettingsPatch({ contextWindowMeterEnabled: true })
				.contextWindowMeterEnabled,
		).toBe(true);
	});
});
...
describe("ClientSettings composer collapse", () => {
	it("collapses on scroll by default and accepts opting out", () => {
		...
		expect(
			decodeClientSettingsPatch({ composerCollapseOnScroll: false })
...
describe("ServerSettings thread settlement", () => {
	...
	it.each([-1, 0, 91])(
		"rejects an auto-settle threshold outside 1..90: %s",
		(value) => {
			expect(() =>
				decodeServerSettings({ sidebarAutoSettleAfterDays: value }),
			).toThrow();
			expect(() =>
				decodeServerSettingsPatch({ sidebarAutoSettleAfterDays: value }),
			).toThrow();
		},
	);
});
...
describe("provider enabled defaults", () => {
	...
	it("keeps Cursor enabled when an existing user explicitly opted in", () => {
		...
		expect(
			resolveProviderInstanceEnabled(decoded.providerInstances[cursorId]!),
		).toBe(true);
	});
	...
	it("resolves instance enabled state with explicit false winning", () => {
		...
		expect(resolveProviderInstanceEnabled({ driver: grok, config: {} })).toBe(
			false,
		);
		expect(resolveProviderInstanceEnabled({ driver: codex, config: {} })).toBe(
			true,
		);
...
describe("ServerSettings worktree defaults", () => {
	...
	it("accepts start-from-origin updates", () => {
		expect(
			decodeServerSettingsPatch({ newWorktreesStartFromOrigin: false })
...
describe("ServerSettingsPatch string normalization", () => {
	it("trims string settings while decoding patches", () => {
		...
		expect(patch.providers?.codex?.launchArgs).toBe(
...
describe("ServerSettings environment icon", () => {
	...
	it("keeps a kind this build knows", () => {
		expect(
			decodeServerSettings({ environmentIcon: "mac-mini" }).environmentIcon,
		).toBe("mac-mini");
		expect(
			decodeServerSettings({ environmentIcon: "linux" }).environmentIcon,
		).toBe("linux");
	});

	it("decodes a kind from a newer server as null instead of failing the snapshot", () => {
		expect(
			decodeServerSettings({ environmentIcon: "toaster" }).environmentIcon,
		).toBeNull();
	});
