...
const encodeVersion = Schema.encodeEffect(
	Schema.fromJsonString(MetaJsonSchema.fields.version),
);

const decodeUpstreamSchema = Schema.decodeEffect(
	Schema.fromJsonString(UpstreamJsonSchemaSchema),
);
const decodeMetaJson = Schema.decodeEffect(
	Schema.fromJsonString(MetaJsonSchema),
);
...
const downloadSchemas = Effect.fn("downloadSchemas")(function* (tag: string) {
	...
	yield* Effect.addFinalizer(() =>
		Effect.all([
			fs.remove(upstreamSchemaPath),
			fs.remove(upstreamMetaPath),
...
const generateSchemas = Effect.fn("generateSchemas")(function* (
	...
	if (!skipDownload) {
		yield* Effect.log(
			`Downloading ACP schema assets for ${CURRENT_SCHEMA_RELEASE}`,
		);
	...
	const upstreamMeta = yield* readFileString(upstreamMetaPath).pipe(
		Effect.flatMap(decodeMetaJson),
	);
	...
	const sortedEntries = Object.entries(normalizedDefinitions).toSorted(
	...
	yield* Effect.service(ChildProcessSpawner.ChildProcessSpawner).pipe(
		Effect.flatMap((spawner) =>
			spawner.spawn(ChildProcess.make("bun", ["oxfmt", generatedDir])),
		),
...
const generateCommand = Command.make(
	...
	Command.withDescription(
		"Generate Effect ACP schemas from the pinned ACP release assets.",
	),
);
