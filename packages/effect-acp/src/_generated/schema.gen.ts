...
export const SessionConfigOptionCategory = Schema.Union([
	...
	Schema.Literal("thought_level").annotate({
...
export const StringFormat = Schema.Literals([
	"email",
	"uri",
	"date",
	"date-time",
	...
	[
		Schema.Struct({
			...
			default: Schema.optionalKey(
				Schema.Union([
					Schema.String.annotate({ description: "Default value." }),
					Schema.Null,
				]),
			),
			description: Schema.optionalKey(
		...
		Schema.Struct({
			...
			default: Schema.optionalKey(
				Schema.Union([
					Schema.Boolean.annotate({ description: "Default value." }),
					Schema.Null,
				]),
			),
			description: Schema.optionalKey(
		...
		Schema.Struct({
			...
			items: Schema.Union([
				Schema.Struct({
					enum: Schema.Array(Schema.String).annotate({
				...
				Schema.Struct({
					anyOf: Schema.Array(EnumOption).annotate({
				...
				[
					Schema.Struct({
						...
						annotations: Schema.optionalKey(
							Schema.Union([Annotations, Schema.Null]),
						),
						text: Schema.String,
					}).annotate({ description: "Text provided to or from an LLM." }),
					Schema.Struct({
	...
	readonly params?:
		...
		| {
				...
				readonly requestedSchema: {
					readonly description?: string | null;
					readonly properties?: {
						readonly [x: string]: ElicitationPropertySchema;
					};
				...
				| {
						...
						readonly action:
							| {
									...
									readonly content?: {
										readonly [x: string]: ElicitationContentValue;
									} | null;
							  }
...
export const PlanEntryPriority = Schema.Literals([
	"high",
	"medium",
	"low",
]).annotate({
...
export const PlanEntryStatus = Schema.Literals([
	"pending",
	"in_progress",
	"completed",
]).annotate({
