...
const ExtPingNotification = jsonRpcNotification(
	"x/ping",
...
const decodeInitializeResponse = Schema.decodeEffect(
		...
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				...
				yield* agent.handleCancel((notification) =>
					Ref.update(cancelNotifications, (current) => [
						...current,
						notification.sessionId,
					]).pipe(Effect.andThen(Deferred.succeed(cancelReceived, undefined))),
				);
				yield* agent.handleExtNotification(
					"x/ping",
					Schema.Struct({ count: Schema.Number }),
					(payload) =>
						Ref.update(extNotifications, (current) => [
							...current,
							payload.count,
						]).pipe(Effect.andThen(Deferred.succeed(extReceived, undefined))),
				);
				...
				const permissionRequest = yield* decodeRequestPermissionRequest(
				...
				const initResponse = yield* decodeInitializeResponse(
					yield* Queue.take(output),
				);
	...
	() =>
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				...
				const decodedExt = Schema.decodeEffect(
					Schema.fromJsonString(ExtRequest),
				);
