...
const decodeCancelNotification = Schema.decodeUnknownEffect(
		...
		client: {
			...
			readTextFile: (payload) =>
				callRpc(
					...
					rpc[CLIENT_METHODS.fs_read_text_file](payload),
				),
			...
			createTerminal: (payload) =>
				callRpc(
					...
					rpc[CLIENT_METHODS.terminal_create](payload),
				).pipe(
					Effect.map(
						(response) =>
							({
			...
			sessionUpdate: (payload) =>
				transport.notify(CLIENT_METHODS.session_update, payload),
...
export const layer = (
	stdio: Stdio.Stdio,
	options: AcpAgentOptions = {},
): Layer.Layer<AcpAgent> => Layer.effect(AcpAgent, make(stdio, options));
