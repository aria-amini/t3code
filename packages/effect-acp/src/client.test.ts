...
const decodePromptRequestLine = Schema.decodeEffect(
	Schema.fromJsonString(PromptRequest),
);
...
it.layer(NodeServices.layer)("effect-acp client", (it) => {
	...
	it.effect(
		"transforms fragmented stdout before parsing and preserves UTF-8",
		() =>
			Effect.gen(function* () {
							...
							transformStdout: (stdout) =>
								stdout.pipe(
									Stream.flatMap((chunk) =>
										Stream.fromIterable(
											Array.from(chunk, (byte) => Uint8Array.of(byte)),
										),
									),
	...
	it.effect(
		"reports malformed JSON that remains after a stdout transform",
		() =>
			Effect.gen(function* () {
				const handle = yield* makeHandle({
					ACP_MOCK_STDOUT_PREFIX: `${mockStartupNotice}\n`,
	...
	it.effect(
		"preserves stdout transform errors without logging rejected input",
		() =>
			Effect.gen(function* () {
				const privateNotice =
					"Mock sign-in URL: https://example.test/login?token=mock-private-token";
				const handle = yield* makeHandle({
					ACP_MOCK_STDOUT_PREFIX: `${privateNotice}\n`,
				});
				const termination = yield* Deferred.make<AcpError.AcpError>();
				...
				const error = yield* Effect.gen(function* () {
					...
					Effect.provide(
						AcpClient.layerChildProcess(handle, {
							logIncoming: true,
							logger: (event) =>
								Ref.update(logs, (current) => [...current, event]),
							transformStdout: (stdout) =>
								stdout.pipe(
									Stream.decodeText,
									Stream.splitLines,
	...
	it.effect(
		"retains only transformed session updates in raw notifications and handlers",
		() =>
			Effect.gen(function* () {
				...
				const [retained] = yield* acp.raw.notifications.pipe(
					Stream.take(1),
					Stream.runCollect,
				);
	...
	it.effect(
		"reports idle child termination and rejects later requests and notifications",
		() =>
			Effect.gen(function* () {
				const handle = yield* makeHandle();
				const termination = yield* Deferred.make<AcpError.AcpError>();
				yield* Effect.gen(function* () {
					...
					const rawRequestError = yield* acp.raw.request("x/echo", {}).pipe(
						Effect.match({
							...
							onSuccess: () =>
								assert.fail("Expected the request to fail after process exit"),
						}),
					);
					assert.strictEqual(rawRequestError, error);
					assert.strictEqual(
						yield* acp.raw.notify("x/notify", {}).pipe(Effect.flip),
						error,
					);
					assert.strictEqual(
						yield* acp.agent
							.cancel({ sessionId: "mock-session-1" })
							.pipe(Effect.flip),
						error,
					);
			...
			Effect.gen(function* () {
				...
				const ext = yield* Effect.gen(function* () {
					...
					yield* acp.handleElicitationComplete((notification) =>
						Ref.update(elicitationCompletions, (current) => [
							...current,
							notification,
						]),
					);
					yield* acp.handleExtRequest(
						...
						(payload) =>
							Ref.update(typedRequests, (current) => [
								...current,
								payload,
							]).pipe(
								Effect.as({
					...
					yield* acp.handleExtNotification(
						...
						(payload) =>
							Ref.update(typedNotifications, (current) => [
								...current,
								payload,
							]),
					);
					...
					const streamed = yield* Stream.runCollect(
						Stream.take(acp.raw.notifications, 2),
					);
		...
		() =>
			Effect.gen(function* () {
				...
				if (result._tag !== "Failure") {
					assert.fail(
						"Expected prompt to fail for invalid typed extension payload",
					);
				}
				const rendered = Cause.pretty(result.cause);
				assert.include(
					rendered,
					"Invalid payload for ACP extension method 'x/typed_request'.",
				);
		...
		() =>
			Effect.gen(function* () {
				...
				yield* Effect.gen(function* () {
					...
					yield* acp.handleSessionUpdate(() =>
						Ref.update(successfulHandlers, (count) => count + 1),
					);
	...
	it.effect("uses distinct ids for RPC calls and extension requests", () =>
		Effect.gen(function* () {
			...
			const decodedInitialize = Schema.decodeEffect(
