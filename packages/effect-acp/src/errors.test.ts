...
describe("effect-acp errors", () => {
	it.effect(
		"retains RPC method and cause without deriving the message from the cause",
		() => {
			const rootCause = new Error(
				"connection details that must not become the public message",
			);
			...
			return Effect.gen(function* () {
				const error = yield* callRpc("session/new", Effect.fail(failure)).pipe(
					Effect.flip,
				);
				...
				expect(error.message).toBe(
					"ACP transport operation call-rpc failed for method session/new.",
				);
	...
	it.effect("preserves typed ACP failures carried by the RPC protocol", () => {
		...
		return Effect.gen(function* () {
			const error = yield* callRpc("initialize", Effect.fail(failure)).pipe(
				Effect.flip,
			);
	...
	it.effect("preserves protocol request errors as request errors", () => {
		...
		return Effect.gen(function* () {
			const error = yield* callRpc("session/load", Effect.fail(failure)).pipe(
				Effect.flip,
			);
	...
	it("preserves structured extension handler failures behind stable request errors", () => {
		...
		const error = AcpError.AcpRequestError.fromExtensionHandlerError(
			cause,
			"x/test",
		);
		...
		expect(error.message).toBe(
			"ACP extension request handler failed for method 'x/test'",
		);
	...
	it.effect(
		"keeps invalid extension payload values only in the exact schema cause",
		() =>
			Effect.gen(function* () {
				const secret = "acp-schema-payload-secret";
				const cause = yield* decodeNestedNumberPayload({
					profile: { token: secret },
				}).pipe(Effect.flip);
				const error = AcpError.AcpRequestError.invalidExtensionPayload(
					"x/private",
					cause,
				);
				...
				expect(error.message).toBe(
					"Invalid payload for ACP extension method 'x/private'.",
				);
				expect(error.message).not.toContain(secret);
				expect(encodeUnknownJson(directDiagnostics)).not.toContain(secret);
				expect(encodeUnknownJson(error.toProtocolError())).not.toContain(
					secret,
				);
