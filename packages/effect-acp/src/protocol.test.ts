...
const RequestPermissionResponse = jsonRpcResponse(
	AcpSchema.RequestPermissionResponse,
);
const ExtRequest = jsonRpcRequest(
	"x/test",
...
const decodeExtResponse = Schema.decodeEffect(
	Schema.fromJsonString(ExtResponse),
);
...
const makeHandle = (env?: Record<string, string>) =>
	Effect.gen(function* () {
		...
		const command = ChildProcess.make(
			process.execPath,
			mockPeerArgs(yield* mockPeerPath),
			{
...
it.layer(NodeServices.layer)("effect-acp protocol", (it) => {
		...
		() =>
			Effect.gen(function* () {
				...
				yield* transport.incoming.pipe(
					Stream.take(2),
					Stream.runCollect,
					Effect.flatMap((notificationChunk) =>
						Deferred.succeed(notifications, notificationChunk),
					),
					Effect.forkScoped,
				);
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				const retained = yield* transport.incoming.pipe(
					Stream.take(32),
					Stream.runCollect,
				);
	...
	it.effect(
		"keeps invalid core notification values only in the schema cause",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"fails notification encoding through the declared ACP error channel",
		() =>
			Effect.gen(function* () {
				const { stdio } = yield* makeInMemoryStdio();
				const transport = yield* AcpProtocol.makeAcpPatchedProtocol({
	...
	it.effect(
		"supports generic extension requests over the patched transport",
		() =>
			Effect.gen(function* () {
				const { stdio, input, output } = yield* makeInMemoryStdio();
				const transport = yield* AcpProtocol.makeAcpPatchedProtocol({
	...
	it.effect(
		"correlates extension response errors with the originating request",
		() =>
			Effect.gen(function* () {
				const { stdio, input, output } = yield* makeInMemoryStdio();
				const transport = yield* AcpProtocol.makeAcpPatchedProtocol({
	...
	it.effect(
		"cleans up interrupted extension requests before a late response arrives",
		() =>
			Effect.gen(function* () {
				const { stdio, input, output } = yield* makeInMemoryStdio();
				const transport = yield* AcpProtocol.makeAcpPatchedProtocol({
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				yield* transport.clientProtocol
					.run(0, (message) =>
						Deferred.succeed(firstMessage, message).pipe(Effect.asVoid),
					)
					.pipe(Effect.forkScoped);
				...
				assert.equal(
					(message as { readonly _tag?: string })._tag,
					"ClientProtocolError",
				);
				const defect = (
					message as { readonly error: { readonly reason: unknown } }
				).error.reason as {
		...
		() =>
			Effect.gen(function* () {
				...
				const transport = yield* AcpProtocol.makeAcpPatchedProtocol({
					...
					serverRequestMethods: new Set(),
					onTermination: () =>
						Ref.update(terminationCalls, (count) => count + 1),
				});
	...
	it.effect(
		"keeps client send failure messages independent from the cause",
		() =>
			Effect.gen(function* () {
				const { stdio } = yield* makeInMemoryStdio();
				const transport = yield* AcpProtocol.makeAcpPatchedProtocol({
	...
	it.effect(
		"fails pending extension requests with the propagated exit code",
		() =>
			Effect.gen(function* () {
				const { stdio, input, output } = yield* makeInMemoryStdio();
				const transport = yield* AcpProtocol.makeAcpPatchedProtocol({
					stdio,
					terminationError: Effect.succeed(
				...
				const error = yield* Fiber.join(response).pipe(
					Effect.match({
						onFailure: (error) => error,
						onSuccess: () =>
							assert.fail("Expected request to fail after process exit"),
					}),
				);
	...
	for (const operation of ["request", "notification"] as const) {
		it.effect(
			`rejects a ${operation} if the connection ends while its logger is running`,
			() =>
				Effect.gen(function* () {
					const { stdio, input, output } = yield* makeInMemoryStdio();
					...
					const failure = yield* Fiber.join(send).pipe(
						Effect.match({
							...
							onSuccess: () =>
								assert.fail("Expected the send to fail after termination"),
						}),
					);
