...
export interface AcpStdio extends Omit<Stdio.Stdio, "stdin"> {
	readonly stdin: Stream.Stream<
		Uint8Array,
		PlatformError.PlatformError | AcpError.AcpError
	>;
}
...
export const makeAcpPatchedProtocol = Effect.fn("makeAcpPatchedProtocol")(
	function* (
		...
		const ensureActive = Ref.get(terminationHandled).pipe(
			Effect.flatMap((terminated) =>
				terminated ? Deferred.await(terminationFailure) : Effect.void,
			),
		);
		...
		const offerOutgoing = Effect.fn("offerOutgoing")(function* (
			...
			const encoded = yield* Effect.try({
				try: () => parser.encode(message),
					...
					AcpError.AcpProtocolParseError.fromEncodingError(
						method,
						requestId,
						cause,
					),
		...
		const completeExtPendingFailure = (
			requestId: AcpError.AcpRequestId,
			error: AcpError.AcpError,
		) =>
		...
		const handleTermination = (
			classify: () => Effect.Effect<AcpError.AcpError>,
		) =>
		...
		const handleExtRequest = (message: RpcMessage.RequestEncoded) => {
			if (!options.onExtRequest) {
				return respondWithError(
					message.id,
					AcpError.AcpRequestError.methodNotFound(message.tag),
				);
			}
			return options.onExtRequest(message.tag, message.payload).pipe(
				Effect.matchEffect({
					onFailure: (error) =>
						respondWithError(
							message.id,
							AcpError.AcpRequestError.fromExtensionHandlerError(
								error,
								message.tag,
							),
						),
		...
		const handleExitEncoded = (message: RpcMessage.ResponseExitEncoded) =>
			Ref.get(extPending).pipe(
				Effect.flatMap((pending) => {
					...
					const failure = message.exit.cause.find(
						(entry) => entry._tag === "Fail",
					);
		...
		const sendNotification = Effect.fn("sendNotification")(function* (
			...
			const exit = encodeJsonRpcNotification({
				jsonrpc: "2.0",
				method,
				params: payload,
			});
