...
it.effect(
	"preserves schema decode diagnostics without deriving the message from the cause",
	() =>
		Effect.gen(function* () {
			const error = yield* Shared.decodeOptionalPayload(
				"thread/start",
				Schema.String,
				42,
			).pipe(Effect.flip);
...
it.effect("passes request errors through without adding a wrapper", () =>
	Effect.gen(function* () {
		const source =
			CodexError.CodexAppServerRequestError.invalidParams("Invalid thread id");
		const error = yield* Shared.runHandler(
			(_payload: void) => Effect.fail(source),
			undefined,
			"thread/start",
