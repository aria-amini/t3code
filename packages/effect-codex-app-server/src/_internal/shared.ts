...
export const decodeOptionalPayload = <A, I>(
	...
	if (!schema) {
		...
		return Effect.fail(
			CodexError.CodexAppServerRequestError.unexpectedPayload(
				method,
				"decode-payload",
				raw,
			),
	...
	return Schema.decodeUnknownEffect(schema)(raw).pipe(
		Effect.mapError((error) =>
			CodexError.CodexAppServerRequestError.invalidPayload(
				method,
				"decode-payload",
				error,
			),
...
export const runHandler = Effect.fnUntraced(function* <A, B>(
	handler:
		| ((payload: A) => Effect.Effect<B, CodexError.CodexAppServerError>)
		| undefined,
	payload: A,
	method: string,
) {
