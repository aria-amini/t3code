...
const make = Effect.fn("effect-codex-app-server/CodexAppServerClient.make")(
	function* (
		stdio: Stdio.Stdio,
		...
		const notificationHandlers = new Map<
			string,
			Array<ServerNotificationHandler>
		>();
		...
		const getServerRequestParamSchema = <
			M extends CodexRpc.ServerRequestMethod,
		>(
			method: M,
		):
			| Schema.Codec<
		...
		const getServerRequestResponseSchema = <
			M extends CodexRpc.ServerRequestMethod,
		>(
			method: M,
		):
		...
		const getClientRequestParamSchema = <
		...
		const getClientRequestResponseSchema = <
			M extends CodexRpc.ClientRequestMethod,
		>(
			method: M,
		):
		...
		const getClientNotificationParamSchema = <
			M extends CodexRpc.ClientNotificationMethod,
		>(
			method: M,
		):
			| Schema.Codec<
		...
		const dispatchRequest = (
			...
			if (request.method in CodexRpc.SERVER_REQUEST_PARAMS) {
				...
				return decodeOptionalPayload(
					...
					Effect.flatMap((result) =>
						encodeOptionalPayload(method, responseSchema, result),
					),
				);
			}
			...
			return unknownRequestHandler
				...
				: Effect.fail(
						CodexError.CodexAppServerRequestError.methodNotFound(
							request.method,
						),
					);
		};
