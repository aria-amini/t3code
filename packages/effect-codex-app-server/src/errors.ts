...
type CodexAppServerRequestOperation =
	typeof CodexAppServerRequestOperation.Type;
...
): CodexAppServerSchemaIssueDiagnostics => {
	let issueCount = 0;
...
const CodexAppServerProtocolMessageField = Schema.Literals(
...
export type CodexAppServerProtocolParseOperation =
	typeof CodexAppServerProtocolParseOperation.Type;

export const CodexAppServerTransportOperation = Schema.Literals([
	"read-input-stream",
	"read-process-exit-status",
]);
export type CodexAppServerTransportOperation =
	typeof CodexAppServerTransportOperation.Type;
...
export type CodexAppServerIdentifierPurpose =
	typeof CodexAppServerIdentifierPurpose.Type;
...
export class CodexAppServerProtocolParseError extends Schema.TaggedError<CodexAppServerProtocolParseError>()(
	"CodexAppServerProtocolParseError",
	{
		...
		presentFields: Schema.optionalKey(
			Schema.Array(CodexAppServerProtocolMessageField),
		),
	...
	override get message() {
		const method =
			this.method === undefined ? "" : ` for method '${this.method}'`;
	...
	static fromRequestError(
		...
		return new CodexAppServerProtocolParseError({
			operation,
			method,
			...(cause.issueCount === undefined
				? {}
				: { issueCount: cause.issueCount }),
			...(cause.issueKinds === undefined
				? {}
				: { issueKinds: cause.issueKinds }),
	...
	static fromUnroutableMessage(message: unknown) {
		...
		const presentFields = protocolMessageFields.filter(
