...
const encodeUnknownJsonString = Schema.encodeUnknownSync(
...
it.layer(NodeServices.layer)("effect-codex-app-server protocol", (it) => {
	it.effect(
		"maps account usage responses to the upstream token usage schema",
		() =>
			Effect.gen(function* () {
				assert.strictEqual(
					CodexRpc.CLIENT_REQUEST_RESPONSES["account/usage/read"],
	...
	it.effect(
		"maps earned rate-limit reset credits from account rate-limit snapshots",
		() =>
			Effect.gen(function* () {
				assert.strictEqual(
					CodexRpc.CLIENT_REQUEST_RESPONSES["account/rateLimits/read"],
				...
				assert.deepEqual(
					yield* decodeAccountRateLimitsResponse(response),
					response,
				);
				assert.deepEqual(
	...
	it.effect(
		"maps the earned rate-limit reset consume request and response",
		() =>
			Effect.gen(function* () {
				assert.equal(
					CodexRpc.CLIENT_REQUEST_METHODS[
						"account/rateLimitResetCredit/consume"
					],
					"account/rateLimitResetCredit/consume",
				);
				assert.strictEqual(
					CodexRpc.CLIENT_REQUEST_PARAMS[
						"account/rateLimitResetCredit/consume"
					],
					CodexSchema.V2ConsumeAccountRateLimitResetCreditParams,
				);
				assert.strictEqual(
					CodexRpc.CLIENT_REQUEST_RESPONSES[
						"account/rateLimitResetCredit/consume"
					],
				...
				assert.deepEqual(
					yield* decodeConsumeRateLimitResetCreditResponse({
						outcome: "reset",
					}),
					{
						outcome: "reset",
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				...
				const transport =
					yield* CodexProtocol.makeCodexAppServerPatchedProtocol({ stdio });

				const notificationDeferred =
					yield* Deferred.make<
						ReadonlyArray<CodexProtocol.CodexAppServerIncomingNotification>
					>();
				const requestDeferred =
					yield* Deferred.make<
						ReadonlyArray<CodexProtocol.CodexAppServerIncomingRequest>
					>();
				...
				yield* transport.incomingNotifications.pipe(
					...
					Effect.flatMap((notifications) =>
						Deferred.succeed(notificationDeferred, notifications),
					),
					Effect.forkScoped,
				);
				...
				yield* transport.incomingRequests.pipe(
					...
					Effect.flatMap((requests) =>
						Deferred.succeed(requestDeferred, requests),
					),
					Effect.forkScoped,
				);
	...
	it.effect(
		"routes a large notification fragmented across thousands of input chunks",
		() =>
			Effect.gen(function* () {
				const { stdio, input, output } = yield* makeInMemoryStdio();
	...
	it.effect.each([1, 7, 1024])(
		...
		(chunkSize) =>
			Effect.gen(function* () {
				...
				const termination =
					yield* Deferred.make<CodexError.CodexAppServerError>();
				yield* CodexProtocol.makeCodexAppServerPatchedProtocol({
					stdio,
					logIncoming: true,
					logger: (event) =>
					...
					onTermination: (error) =>
						Deferred.succeed(termination, error).pipe(Effect.asVoid),
				});
	...
	it.effect(
		"reports a malformed fragmented final line before input stream termination",
		() =>
			Effect.gen(function* () {
				const { stdio, input, output } = yield* makeInMemoryStdio();
				const termination =
				...
				const responseError = yield* Fiber.join(response).pipe(
					Effect.match({
						...
						onSuccess: () =>
							assert.fail(
								"Expected the malformed response to fail the request",
							),
					}),
				);
	...
	it.effect(
		"keeps only recent raw notifications after their callbacks run",
		() =>
			Effect.gen(function* () {
				const { stdio, input } = yield* makeInMemoryStdio();
	...
	it.effect(
		"keeps processing protocol messages while an approval is pending",
		() =>
			Effect.gen(function* () {
				const { stdio, input, output } = yield* makeInMemoryStdio();
				const approvalStarted = yield* Deferred.make<void>();
				const approvalDecision = yield* Deferred.make<{
				...
				const transport =
					yield* CodexProtocol.makeCodexAppServerPatchedProtocol({
						...
						onNotification: () =>
							Deferred.succeed(notificationReceived, undefined).pipe(
								Effect.asVoid,
							),
					});
				...
				assert.deepEqual(yield* Fiber.join(pendingRequest), {
					threadId: "thread-1",
				});
	...
	it.effect(
		"rejects incoming requests after the active handler limit is reached",
		() =>
			Effect.gen(function* () {
				const { stdio, input, output } = yield* makeInMemoryStdio();
				...
				yield* CodexProtocol.makeCodexAppServerPatchedProtocol({
					...
					onRequest: () =>
						Effect.sync(() => ++activeHandlers).pipe(
							Effect.flatMap((count) =>
								count === 32
									? Deferred.succeed(handlersStarted, undefined).pipe(
											Effect.asVoid,
										)
									: Effect.void,
							),
	...
	it.effect(
		"interrupts pending request handlers when the protocol terminates",
		() =>
			Effect.gen(function* () {
				const { stdio, input } = yield* makeInMemoryStdio();
				const approvalStarted = yield* Deferred.make<void>();
				...
				yield* CodexProtocol.makeCodexAppServerPatchedProtocol({
					...
					onRequest: () =>
						Deferred.succeed(approvalStarted, undefined).pipe(
							...
							Effect.onInterrupt(() =>
								Deferred.succeed(approvalInterrupted, undefined).pipe(
									Effect.asVoid,
								),
							),
						),
					onTermination: () =>
						Deferred.succeed(terminated, undefined).pipe(Effect.asVoid),
				});
	...
	it.effect(
		"rejects outgoing messages after an approval response cannot be encoded",
		() =>
			Effect.gen(function* () {
				...
				const stdio = Stdio.make({
					...
					stdin: baseStdio.stdin.pipe(
						Stream.ensuring(
							Deferred.succeed(readerStopped, undefined).pipe(Effect.asVoid),
						),
					),
					...
					yield* CodexProtocol.makeCodexAppServerPatchedProtocol({
						...
						onRequest: () =>
							Effect.sync(() => ++requestCount).pipe(
								Effect.map((count) =>
									count === 1 ? { invalid: 1n } : { ok: true },
								),
							),
						onNotification: () =>
							Effect.sync(() => notificationCount++).pipe(Effect.asVoid),
						onTermination: (error) =>
				...
				const requestFailure = yield* transport.request("thread/read", {}).pipe(
					Effect.match({
						...
						onSuccess: () =>
							assert.fail("Expected a terminated protocol request to fail"),
					}),
				);
				const notificationFailure = yield* transport
					.notify("initialized")
					.pipe(Effect.flip);
	...
	it.effect(
		"fails pending requests before interrupted handler cleanup completes",
		() =>
			Effect.gen(function* () {
				const { stdio, input, output } = yield* makeInMemoryStdio();
				...
				const pendingFailure = yield* Fiber.join(pending).pipe(
					Effect.match({
						onFailure: (error) => error,
						onSuccess: () =>
							assert.fail("Expected the pending request to fail"),
					}),
				);
	...
	it.effect("surfaces JSON encoding failures as protocol parse errors", () =>
		Effect.gen(function* () {
			...
			const bigintError = yield* transport
				.notify("x/test", 1n)
				.pipe(Effect.flip);
			assert.instanceOf(
				bigintError,
				CodexError.CodexAppServerProtocolParseError,
			);
		...
		Effect.gen(function* () {
			...
			const error = yield* Fiber.join(response).pipe(
				Effect.match({
					onFailure: (error) => error,
					onSuccess: () =>
						assert.fail("Expected Codex App Server request to fail"),
				}),
			);
	...
	it.effect(
		"logs decode failures without copying the cause or wire payload",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"describes unroutable messages with safe structural diagnostics",
		() =>
			Effect.gen(function* () {
