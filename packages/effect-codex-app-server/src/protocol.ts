...
export interface CodexAppServerPatchedProtocolOptions {
	...
	readonly onTermination?: (
		error: CodexError.CodexAppServerError,
	) => Effect.Effect<void, never>;
}
...
function isIncomingRequest(
	value: unknown,
): value is CodexAppServerIncomingRequest {
...
function isIncomingResponse(
	value: unknown,
): value is typeof JsonRpcResponseEnvelope.Type {
...
const decodeWireMessage = (
	...
	decodeJsonString(line).pipe(
		Effect.mapError((cause) =>
			CodexError.CodexAppServerProtocolParseError.fromSchemaError(
				"decode-wire-message",
				cause,
			),
		),
...
export const makeCodexAppServerPatchedProtocol = Effect.fn(
	...
	const incomingNotifications =
		yield* Queue.sliding<CodexAppServerIncomingNotification>(
			MAX_BUFFERED_RAW_MESSAGES,
		);
	const incomingRequests = yield* Queue.sliding<CodexAppServerIncomingRequest>(
		MAX_BUFFERED_RAW_MESSAGES,
	);
	const pending = yield* Ref.make(
		new Map<string, CodexAppServerPendingRequest>(),
	);
	const nextRequestId = yield* Ref.make(1);
	const remainder: Array<string> = [];
	const terminationHandled = yield* Ref.make(false);
	const terminationFailure = yield* Ref.make(
		Option.none<CodexError.CodexAppServerError>(),
	);
	...
	const handleTermination = (
		classify: () => Effect.Effect<CodexError.CodexAppServerError>,
	) =>
	...
	const respondError = (
		...
		offerOutgoing(
			toProtocolMessage(requestId, { error: error.toProtocolError() }),
		);
		...
		Queue.offer(incomingRequests, request).pipe(
			Effect.flatMap(() => {
				...
				return Ref.modify(activeRequestHandlers, (count) =>
					count >= MAX_BUFFERED_RAW_MESSAGES
						? [false, count]
						: [true, count + 1],
				).pipe(
					Effect.flatMap((accepted) => {
						...
						return handler(request).pipe(
							...
							Effect.ensuring(
								Ref.update(activeRequestHandlers, (count) =>
									Math.max(0, count - 1),
								),
							),
							Effect.catch((error) =>
	...
	const routeMessage = Effect.fnUntraced(function* (message: unknown) {
		...
		if (isIncomingNotification(message))
			return yield* handleNotification(message);
		if (isIncomingResponse(message)) return yield* handleResponse(message);
		return yield* CodexError.CodexAppServerProtocolParseError.fromUnroutableMessage(
			message,
		);
	});
