...
const isThreadRollbackResponse = Schema.is(
	CodexSchema.V2ThreadRollbackResponse,
);
const isThreadForkResponse = Schema.is(CodexSchema.V2ThreadForkResponse);
const isTurnCompletedNotification = Schema.is(
	CodexSchema.V2TurnCompletedNotification,
);
const decodeThreadResumeResponse = Schema.decodeUnknownSync(
...
it("accepts Codex 0.150 multi-agent values", () => {
	...
	for (const tool of [
		...
		assert.equal(
			Schema.is(CodexSchema.ServerNotification__CollabAgentTool)(tool),
			true,
		);
		assert.equal(
			Schema.is(CodexSchema.V2ThreadResumeResponse__CollabAgentTool)(tool),
			true,
		);
	}

	assert.equal(
		Schema.is(CodexSchema.ServerNotification__CollabAgentToolCallStatus)(
			"interrupted",
		),
		true,
	);
	assert.equal(
		Schema.is(CodexSchema.V2ThreadResumeResponse__CollabAgentToolCallStatus)(
			"interrupted",
		),
		true,
	);
	...
	assert.equal(
		Schema.is(CodexSchema.V2ThreadResumeResponse)(resumeResponse),
		true,
	);
});
