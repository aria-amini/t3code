...
const openServer = (
	host?: string,
): Effect.Effect<NodeNet.Server, NetService.NetError> =>
	Effect.callback<NodeNet.Server, NetService.NetError>((resume) => {
		const server = NodeNet.createServer();
		let settled = false;

		const settle = (
			effect: Effect.Effect<NodeNet.Server, NetService.NetError>,
		) => {
			if (settled) return;
			settled = true;
			resume(effect);
			...
			settle(
				Effect.fail(
					new NetService.NetError({
						message: "Failed to open test server",
						cause,
					}),
				),
			);
...
it.layer(NetService.layer)("NetService", (it) => {
	describe("Net helpers", () => {
		...
		it.effect(
			"findAvailablePort falls back when a wildcard listener occupies IPv4",
			() =>
				Effect.acquireUseRelease(
					openServer("0.0.0.0"),
					(server) =>
