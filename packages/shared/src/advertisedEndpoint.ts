...
export function normalizeHttpBaseUrl(rawValue: string): string {
	...
	if (url.protocol !== "http:" && url.protocol !== "https:") {
		throw new Error(
			`Endpoint must use HTTP or HTTPS. Received ${url.protocol}`,
		);
	}
...
export function createAdvertisedEndpoint(
	input: CreateAdvertisedEndpointInput,
): AdvertisedEndpoint {
	...
	return {
		...
		compatibility: {
			hostedHttpsApp:
				...
				classifyHostedHttpsCompatibility(httpBaseUrl),
