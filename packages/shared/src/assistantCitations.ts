...
export function formatAssistantCitationHref(
	citation: AssistantCitation,
): string {
...
export function parseAssistantCitationHref(
	href: string,
	...
	try {
		...
		if (
			url.searchParams.size !==
				requiredKeys.length + (comment === null ? 0 : 1) ||
...
export function serializeAssistantCitation(
	citation: AssistantCitation,
): string {
...
export function expandAssistantCitationsForProvider(prompt: string): string {
	...
	const data = JSON.stringify(citations, null, 2)
		...
		.replace(/>/g, "\\u003e")
		.replace(/&/g, "\\u0026");
	const description = citations.some(
		({ citation }) => citation.comment !== undefined,
	)
