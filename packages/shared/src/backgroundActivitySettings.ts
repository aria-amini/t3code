...
export function resolveBackgroundActivitySettings(
	...
	const overrides =
		backgroundActivity.profile === "custom" ? backgroundActivity.overrides : {};
	return {
		profile: baseProfile,
		...
		hostPowerMonitorIdleInterval:
			...
			preset.hostPowerMonitorIdleInterval,
		idleClientTtl: overrides.idleClientTtl ?? preset.idleClientTtl,
		pauseWhenHostLocked:
			overrides.pauseWhenHostLocked ?? preset.pauseWhenHostLocked,
		pauseWhenHostLowPower:
			overrides.pauseWhenHostLowPower ?? preset.pauseWhenHostLowPower,
		pauseWhenClientLowPower:
			overrides.pauseWhenClientLowPower ?? preset.pauseWhenClientLowPower,
		pauseWhenOnBattery:
			overrides.pauseWhenOnBattery ?? preset.pauseWhenOnBattery,
	};
}
...
function resolvedSettingsEqual(
	...
	return (
		...
		durationsEqual(
			a.providerHealthRefreshInterval,
			b.providerHealthRefreshInterval,
		) &&
		durationsEqual(
			a.hostPowerMonitorActiveInterval,
			b.hostPowerMonitorActiveInterval,
		) &&
		durationsEqual(
			a.hostPowerMonitorIdleInterval,
			b.hostPowerMonitorIdleInterval,
		) &&
...
export function normalizeBackgroundActivitySettings(
	...
	const overrides: BackgroundActivitySettings["overrides"] = {
		...(!durationsEqual(
			resolved.automaticGitFetchInterval,
			preset.automaticGitFetchInterval,
		)
	...
	if (backgroundActivityIsDefault && hasLegacyOverrides) {
		return resolveBackgroundActivitySettings({
			...
			profile:
				Duration.toMillis(settings.automaticGitFetchInterval) ===
					Duration.toMillis(
						getBackgroundActivityPresetSettings(legacyProfile)
							.automaticGitFetchInterval,
					) &&
				Duration.toMillis(settings.providerHealthRefreshInterval) ===
					Duration.toMillis(
