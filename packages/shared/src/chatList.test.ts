...
describe("resolveChatListAnchoredEndSpace", () => {
	...
	it("skips ineligible rows before the first anchor", () => {
		expect(
			resolveChatListAnchoredEndSpace(rows.slice(1), "latest", getAnchorId),
		).toEqual({
			anchorIndex: 1,
			anchorOffset: CHAT_LIST_ANCHOR_OFFSET,
		});
	...
	it("ignores ineligible rows and missing anchors", () => {
		expect(
			resolveChatListAnchoredEndSpace(rows, "ignored", getAnchorId),
		).toBeUndefined();
		expect(
			resolveChatListAnchoredEndSpace(rows, "missing", getAnchorId),
		).toBeUndefined();
		expect(
			resolveChatListAnchoredEndSpace(rows, null, getAnchorId),
		).toBeUndefined();
	});
});
