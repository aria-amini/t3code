...
describe("tokenizeCliArgs", () => {
	...
	it("preserves literal backslashes in path values", () => {
		expect(
			tokenizeCliArgs(
				String.raw`--config cacheDir=C:\Users\me --config "quoted=C:\Users\me"`,
			),
		).toEqual([
			"--config",
...
describe("parseCliArgs", () => {
	...
	it("parses --append-system-prompt with value and --chrome", () => {
		expect(
			parseCliArgs("--append-system-prompt always-think-step-by-step --chrome"),
		).toEqual({
			flags: {
	...
	it("boolean flag does not consume next token as value", () => {
		expect(
			parseCliArgs(["--github-output", "1.2.3"], {
