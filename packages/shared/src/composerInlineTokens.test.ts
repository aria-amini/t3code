...
describe("collectComposerInlineTokens", () => {
	it("collects file links, mentions, and skills with source ranges", () => {
		const text =
			"Use $ui and inspect [Chat.tsx](src/Chat.tsx) with @AGENTS.md please";
	...
	it("preserves a confirmed pill when only its trailing delimiter is removed", () => {
		...
		expect(
			collectComposerInlineTokens(withDelimiter.trimEnd(), {
	...
	it("does not preserve a pill after its source is edited", () => {
		const confirmed = collectComposerInlineTokens(
	...
	it("ignores normal web links", () => {
		expect(
			collectComposerInlineTokens("Read [docs](https://example.com) first"),
		).toEqual([]);
	});
	...
	it("keeps canonical file links for scoped paths as mentions", () => {
		expect(
			collectComposerInlineTokens("Inspect [sub](@scope/pkg/sub) next"),
		).toEqual([
			{
				type: "mention",
				value: "@scope/pkg/sub",
	...
	it("leaves a file link past the label cap as plain text", () => {
		...
		expect(
			collectComposerInlineTokens(`see [${label}](src/${label}) ok`),
		).toEqual([]);
	});
