...
describe("connectAuth", () => {
	...
	it("rejects authorize requests missing state or challenge", () => {
		expect(
			readConnectAuthorizeRequest(new URL("https://app.t3.codes/connect")),
		).toBeNull();
		expect(
			readConnectAuthorizeRequest(
				new URL("https://app.t3.codes/connect#state=abc"),
			),
		).toBeNull();
		expect(
			readConnectAuthorizeRequest(
				new URL("https://app.t3.codes/connect#challenge=abc"),
			),
		).toBeNull();
	});
	...
	it("round-trips the loopback port through the authorize URL fragment", () => {
		...
		expect(connectLoopbackRedirectUri(34338)).toBe(
	...
	it("round-trips the out-of-band authorization code and preserves dots inside it", () => {
		const blob = encodeConnectAuthCode({
			code: "az9.code.chunk",
			state: "state-uuid",
		});
		expect(parseConnectAuthCode(blob)).toEqual({
			code: "az9.code.chunk",
			state: "state-uuid",
		});
