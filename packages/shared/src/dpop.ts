...
export type DpopVerificationFailureCode =
	typeof DpopVerificationFailureCode.Type;
...
export function computeDpopJwkThumbprint(jwk: DpopPublicJwkType): string {
	return Encoding.encodeBase64Url(
		sha256(new TextEncoder().encode(dpopThumbprintInput(jwk))),
	);
}
	...
	try {
		...
		if (Option.isNone(header)) {
			return {
				ok: false,
				code: "malformed_proof",
				reason: "Invalid DPoP JWT header.",
			};
		}
		if (Option.isNone(payload)) {
			return {
				ok: false,
				code: "malformed_proof",
				reason: "Invalid DPoP JWT payload.",
			};
		}
		...
		if (input.expectedThumbprint && thumbprint !== input.expectedThumbprint) {
			return {
				ok: false,
				code: "key_mismatch",
				reason: "DPoP key thumbprint mismatch.",
			};
		}
		...
		const signatureInputHash = sha256(
