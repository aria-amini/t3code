...
describe("faviconUrlForOrigin", () => {
	...
	it("keeps the public origin, port and requested size", () => {
		expect(
			faviconUrlForOrigin(
				"https://github.com:8443/pingdotgg/t3code?private=query",
				64,
			),
	...
	])("rejects an invalid or unsupported origin %s", (origin) => {
		expect(faviconUrlForOrigin(origin)).toBeNull();
	});
});
...
describe("toolActivityFaviconUrl", () => {
	it("uses the page origin instead of a third-party favicon service", () => {
		expect(
			toolActivityFaviconUrl(
				{ pageUrl: "https://example.com/docs/page?q=1" },
				"light",
			),
		).toBe("https://example.com/favicon.ico");
		expect(
			toolActivityFaviconUrl({ pageUrl: "http://localhost:5173/app" }, "light"),
	...
	it("selects site-owned light and dark variants without filtering full-color icons", () => {
		expect(
			toolActivityFaviconUrl(
				{ pageUrl: "https://github.com/openai/codex" },
				"light",
			),
		...
		expect(toolActivityFaviconUrl(fullColorIcon, "light")).toBe(
			fullColorIcon.faviconUrl,
		);
		expect(toolActivityFaviconUrl(fullColorIcon, "dark")).toBe(
			fullColorIcon.faviconUrl,
		);
	});
	...
	it("accepts provider-supplied image URLs but rejects extension URLs", () => {
		...
		expect(
			toolActivityFaviconUrl(
				{
					pageUrl: "https://example.com/docs",
					faviconUrl: "chrome-extension://example/_favicon/",
				},
				"light",
			),
		).toBe("https://example.com/favicon.ico");
