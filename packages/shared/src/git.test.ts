...
describe("normalizeGitRemoteUrl", () => {
	...
	it("preserves nested group paths for providers like GitLab", () => {
		expect(
			normalizeGitRemoteUrl("git@gitlab.com:T3Tools/platform/T3Code.git"),
		).toBe("gitlab.com/t3tools/platform/t3code");
		expect(
			normalizeGitRemoteUrl("https://gitlab.com/T3Tools/platform/T3Code.git"),
		).toBe("gitlab.com/t3tools/platform/t3code");
	});

	it("drops explicit ports from URL-shaped remotes", () => {
		expect(
			normalizeGitRemoteUrl("https://gitlab.company.com:8443/team/project.git"),
		).toBe("gitlab.company.com/team/project");
		expect(
			normalizeGitRemoteUrl(
...
describe("parseOriginUrlFromGitConfig", () => {
	it("reads the origin url and ignores other remotes", () => {
		...
		expect(parseOriginUrlFromGitConfig(config)).toBe(
...
describe("isTemporaryWorktreeBranch", () => {
	...
	it("matches generated temporary worktree refs", () => {
		expect(
			isTemporaryWorktreeBranch(`${WORKTREE_BRANCH_PREFIX}/deadbeef`),
		).toBe(true);
		expect(
			isTemporaryWorktreeBranch(` ${WORKTREE_BRANCH_PREFIX}/deadbeef `),
		).toBe(true);
	...
	it("rejects UUID-shaped refs that are not RFC 4122 v4", () => {
		...
		expect(
			isTemporaryWorktreeBranch(
				`${WORKTREE_BRANCH_PREFIX}/f4ae4e0e-f971-1d48-b4f2-9cf0aa54ab12`,
			),
		).toBe(false);
		// variant nibble is not [89ab]
		expect(
			isTemporaryWorktreeBranch(
				`${WORKTREE_BRANCH_PREFIX}/f4ae4e0e-f971-4d48-c4f2-9cf0aa54ab12`,
			),
		).toBe(false);
	});
	...
	it("rejects non-temporary refName names", () => {
		expect(
			isTemporaryWorktreeBranch(`${WORKTREE_BRANCH_PREFIX}/feature/demo`),
		).toBe(false);
		expect(isTemporaryWorktreeBranch("main")).toBe(false);
		expect(
			isTemporaryWorktreeBranch(`${WORKTREE_BRANCH_PREFIX}/deadbeef-extra`),
		).toBe(false);
	});
});
...
describe("applyGitStatusStreamEvent", () => {
	it("treats a remote-only update as a repository when local state is missing", () => {
		...
		expect(
			applyGitStatusStreamEvent(null, { _tag: "remoteUpdated", remote }),
		).toEqual({
			isRepo: true,
	...
	it("preserves local-only fields when applying a remote update", () => {
		...
		expect(
			applyGitStatusStreamEvent(current, { _tag: "remoteUpdated", remote }),
		).toEqual({
			...current,
			hasUpstream: true,
			aheadCount: 2,
