...
export function sanitizeFeatureBranchName(raw: string): string {
	...
	if (sanitized.includes("/")) {
		return sanitized.startsWith("feature/")
			? sanitized
			: `feature/${sanitized}`;
	}
...
export function resolveAutoFeatureBranchName(
	...
	const resolvedBase = sanitizeFeatureBranchName(
		preferred && preferred.length > 0
			? preferred
			: AUTO_FEATURE_BRANCH_FALLBACK,
	);
	const existingNames = new Set(
		existingBranchNames.map((refName) => refName.toLowerCase()),
	);
...
export function deriveLocalBranchNameFromRemoteRef(branchName: string): string {
	...
	if (
		...
		firstSeparatorIndex === branchName.length - 1
	) {
		return branchName;
	}
...
export function normalizeGitRemoteUrl(value: string): string {
	...
	const scpStyleHostAndPath =
		/^[a-zA-Z0-9._-]+@([^:/\s]+):([^/\s]+(?:\/[^/\s]+)+)$/i.exec(normalized);
...
function deriveLocalBranchNameCandidatesFromRemoteRef(
	...
	if (remoteName) {
		...
		if (
			branchName.startsWith(remotePrefix) &&
...
export function dedupeRemoteBranchesWithLocalMatches(
	...
	return refs.filter((refName) => {
		...
		return !localBranchCandidates.some((candidate) =>
			localBranchNames.has(candidate),
		);
	});
}
