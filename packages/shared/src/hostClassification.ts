...
const parseIpv4MappedIpv6Address = (host: string): readonly number[] | null => {
	...
	if (
		hextets.length !== 2 ||
		hextets.some((part) => !/^[\da-f]{1,4}$/u.test(part))
	)
		return null;
...
const parseIpv6Address = (host: string): readonly number[] | null => {
	...
	if ([...head, ...tail].some((part) => !/^[\da-f]{1,4}$/u.test(part)))
		return null;
...
const ipv6PrefixMatches = (
	...
	if (
		address.slice(0, fullHextets).some((part, index) => part !== prefix[index])
	)
		return false;
...
export const isPrivateNetworkHost = (host: string): boolean => {
	...
	const parts =
		parseIpv4Address(normalized) ?? parseIpv4MappedIpv6Address(normalized);
	...
	return (
		...
		((firstIpv6Hextet & 0xfe00) === 0xfc00 ||
			(firstIpv6Hextet & 0xffc0) === 0xfe80)
	);
};
...
export const isPublicFaviconHost = (host: string): boolean => {
	...
	if (ipv6PrefixMatches(ipv6, [0x2001, 0x0db8, 0, 0, 0, 0, 0, 0], 32))
		return false;
