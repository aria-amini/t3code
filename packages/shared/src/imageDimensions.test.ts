...
function bytes(
	...parts: ReadonlyArray<number | string | ReadonlyArray<number>>
): Uint8Array {
	const out: number[] = [];
...
describe("readImageDimensions", () => {
	...
	it("reads a GIF logical screen", () => {
		expect(
			readImageDimensions(bytes("GIF89a", u16le(320), u16le(240))),
		).toEqual({
			width: 320,
			height: 240,
		});
	...
	it("reads a JPEG start-of-frame after an APP segment", () => {
		...
		expect(
			readImageDimensions(bytes([0xff, 0xd8], [...app1], [...sof0])),
		).toEqual({
			width: 720,
			height: 1400,
		});
	...
	it("swaps the axes for a JPEG whose EXIF orientation rotates it 90 degrees", () => {
		...
		expect(
			readImageDimensions(bytes([0xff, 0xd8], [...app1], [...xmp], [...sof0])),
		).toEqual({
			width: 3024,
			height: 4032,
		});
		...
		const upright = [...tiff];
		upright[19] = 0x01;
		const app1Upright = bytes(
			[0xff, 0xe1],
			u16(2 + 6 + upright.length),
			"Exif\0\0",
			upright,
		);
		expect(
			readImageDimensions(bytes([0xff, 0xd8], [...app1Upright], [...sof0])),
		).toEqual({
			width: 4032,
			height: 3024,
		});
	...
	it("steps over standalone TEM and restart markers", () => {
		...
		expect(
			readImageDimensions(
				bytes([0xff, 0xd8], [0xff, 0x01], [0xff, 0xd3], [...sof0]),
			),
	...
	it("does not mistake a Huffman table marker for a frame", () => {
		...
		expect(
			readImageDimensions(bytes([0xff, 0xd8], [...dht], [...sof2])),
		).toEqual({
			width: 20,
			height: 10,
		});
	...
	it("reads each WebP container flavour", () => {
		...
		expect(
			readImageDimensions(
				riff("VP8 ", [0, 0, 0, 0x9d, 0x01, 0x2a, ...u16le(800), ...u16le(600)]),
			),
	...
	it("returns null for unsupported, truncated, or zero-sized input", () => {
		expect(
			readImageDimensions(bytes("<svg xmlns='http://www.w3.org/2000/svg'/>")),
		).toBeNull();
		expect(readImageDimensions(bytes([0x89], "PNG"))).toBeNull();
		expect(
			readImageDimensions(bytes("GIF89a", u16le(0), u16le(240))),
		).toBeNull();
