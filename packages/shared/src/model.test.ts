...
describe("descriptor helpers", () => {
	...
	it("reads typed option selection values", () => {
		...
		expect(
			getProviderOptionStringSelectionValue(selection.options, "fastMode"),
		).toBeUndefined();
		expect(
			getProviderOptionBooleanSelectionValue(selection.options, "fastMode"),
		).toBe(true);
		expect(
			getProviderOptionBooleanSelectionValue(
				selection.options,
		...
		expect(getModelSelectionBooleanOptionValue(selection, "fastMode")).toBe(
			true,
		);
	});
});
...
describe("applyClaudePromptEffortPrefix", () => {
	it("keeps slash commands intact when ultrathink is selected", () => {
		expect(applyClaudePromptEffortPrefix("/compact", "ultrathink")).toBe(
			"/compact",
		);
		expect(
			applyClaudePromptEffortPrefix(
		...
		).toBe("/compact keep recent errors");
		expect(
			applyClaudePromptEffortPrefix(" /review src/model.ts ", "ultrathink"),
		).toBe("/review src/model.ts");
		expect(
			applyClaudePromptEffortPrefix("/security-review", "ultrathink"),
		).toBe("/security-review");
		expect(
			applyClaudePromptEffortPrefix("/plugin:skill run", "ultrathink"),
		).toBe("/plugin:skill run");
		expect(
			applyClaudePromptEffortPrefix("/deploy.prod to staging", "ultrathink"),
		).toBe("/deploy.prod to staging");
	});

	it("still adds the ultrathink prefix to ordinary prompts", () => {
		expect(
			applyClaudePromptEffortPrefix("Investigate this failure", "ultrathink"),
...
describe("readCustomModelEntries", () => {
	...
	it("drops unparseable capabilities but keeps the entry", () => {
		expect(
			readCustomModelEntries([
				{ slug: "x", capabilities: { optionDescriptors: "nope" } },
			]),
	...
	it("writes the compact stored shape back", () => {
		expect(
			toCustomModelSetting({ slug: "x", name: "x", capabilities: null }),
		).toBe("x");
		expect(
			toCustomModelSetting({
				slug: "x",
		...
		expect(
			toCustomModelSetting({ slug: "x", name: "X", capabilities }),
		).toEqual({
			slug: "x",
			name: "X",
			capabilities,
