...
export function buildExplicitProviderOptionSelectionsFromDescriptors(
	...
	const normalized = buildProviderOptionSelectionsFromDescriptors(
		descriptors,
	)?.filter((selection) => explicitIds.has(selection.id));
...
const decodeCustomModelCapabilities =
	Schema.decodeUnknownOption(ModelCapabilities);
	...
	for (const raw of value) {
		...
		const name =
			(typeof record.name === "string"
				? normalizeCustomModelSlug(record.name)
				: null) ?? slug;
		const capabilities =
