...
export function encodeOAuthScope(scopes: ReadonlyArray<string>): string {
	const invalidScopes = scopes.filter(
		(scope) => !OAUTH_SCOPE_TOKEN.test(scope),
	);
...
export function oauthScopeSetEquals(
	value: string,
...
export function parseAllowedOAuthScope<Scope extends string>(input: {
	...
	if (
		scopes === null ||
		!scopes.every((scope): scope is Scope =>
			input.allowedScopes.has(scope as Scope),
		)
	) {
		return null;
	}
