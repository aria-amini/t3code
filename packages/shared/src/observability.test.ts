...
const decodeTraceRecordLine = Schema.decodeUnknownSync(
...
const readTraceRecords = Effect.fn("readTraceRecords")(function* (
	tracePath: string,
) {
...
describe("truncateTraceAttributes", () => {
	it("clamps oversized strings at any depth without mutating the input", () => {
		...
		assert.equal(
			(truncated["db.query.text"] as string).length,
			200 + "…[truncated]".length,
		);
		assert.equal(truncated["short"], "ok");
		const error = truncated["error"] as {
			stack: string;
			nested: Array<string>;
		};
...
describe("observability", () => {
	...
	nodeServicesIt("node services", (it) => {
			...
			Effect.scoped(
				Effect.gen(function* () {
					...
					const sink = yield* makeTraceSink({
						...
						batchWindowMs: 10_000,
						onFlush: (stats) =>
							Ref.update(reported, (current) => [...current, stats]),
					});
		...
		it.effect(
			"rotates the trace file when the configured max size is exceeded",
			() =>
				Effect.scoped(
					Effect.gen(function* () {
						const fileSystem = yield* FileSystem.FileSystem;
						...
						const matchingFiles = Arr.sort(
							(yield* fileSystem.readDirectory(tempDir)).filter(
								(entry) =>
									entry === "shared.trace.ndjson" ||
									entry.startsWith("shared.trace.ndjson."),
							),
							Order.String,
						);
		...
		it.effect(
			"keeps every trace file within the configured limit for threshold flushes",
			() =>
				Effect.scoped(
					Effect.gen(function* () {
						const fileSystem = yield* FileSystem.FileSystem;
						...
						const matchingFiles = (yield* fileSystem.readDirectory(
							tempDir,
						)).filter(
							(entry) =>
								entry === "shared.trace.ndjson" ||
		...
		it.effect(
			"drops a single trace record that cannot fit within the configured limit",
			() =>
				Effect.scoped(
					Effect.gen(function* () {
						const fileSystem = yield* FileSystem.FileSystem;
		...
		it.effect(
			"drops only the invalid trace record when serialization fails",
			() =>
				Effect.scoped(
					Effect.gen(function* () {
						const fileSystem = yield* FileSystem.FileSystem;
		...
		it.effect(
			"writes nested spans to disk and captures log messages as span events",
			() =>
				Effect.scoped(
					Effect.gen(function* () {
						const fileSystem = yield* FileSystem.FileSystem;
						...
						assert.equal(
							child.events.some(
								(event) => event.attributes["effect.logLevel"] === "INFO",
							),
							true,
						);
					}),
		...
		it.effect(
			"serializes interrupted spans with an interrupted exit status",
			() =>
				Effect.scoped(
					Effect.gen(function* () {
						const fileSystem = yield* FileSystem.FileSystem;
