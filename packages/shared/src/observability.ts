...
const ALWAYS_TRUNCATED_TRACE_ATTRIBUTES: ReadonlySet<string> = new Set([
	"db.query.text",
]);
...
function truncateNestedValue(value: unknown): unknown {
	...
	if (Array.isArray(value)) {
		...
		return truncated.some((entry, index) => entry !== value[index])
			? truncated
			: value;
	}
	if (isPlainObject(value)) {
 ...
 */
export function truncateTraceAttributes(
	...
	for (const [key, value] of Object.entries(attributes)) {
		if (
			typeof value === "string" &&
			ALWAYS_TRUNCATED_TRACE_ATTRIBUTES.has(key)
		) {
...
function spanToTraceRecord(span: SerializableSpan): EffectTraceRecord {
	...
	return {
		...
		links: span.links.map((link) => ({
			...
			attributes: truncateTraceAttributes(
				compactTraceAttributes(link.attributes),
			),
		})),
...
export const makeTraceSink = Effect.fn("makeTraceSink")(function* (
	...
	const flushUnsafe = () => {
		...
		while (persistedCount < records.length) {
			const firstRecordBytes = textEncoder.encode(
				records[persistedCount],
			).byteLength;
			...
			while (nextIndex < records.length) {
				const nextRecordBytes = textEncoder.encode(
					records[nextIndex],
				).byteLength;
				...
				records.push(
					otlpSpanToTraceRecord({
						...
						scopeAttributes: decodeAttributes(
							"attributes" in scopeSpan.scope &&
								Array.isArray(scopeSpan.scope.attributes)
								? scopeSpan.scope.attributes
								: [],
						),
						scopeName: scopeSpan.scope.name,
						scopeVersion:
							"version" in scopeSpan.scope &&
							typeof scopeSpan.scope.version === "string"
...
function otlpSpanToTraceRecord(input: {
	...
	return {
		...
		durationMs:
			Number(
				parseBigInt(input.span.endTimeUnixNano) -
					parseBigInt(input.span.startTimeUnixNano),
			) / 1_000_000,
...
function decodeEvents(
	input: ReadonlyArray<OtlpSpanEvent>,
...
function decodeLinks(
	input: ReadonlyArray<OtlpSpanLink>,
