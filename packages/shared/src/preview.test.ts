...
describe("isLoopbackHost", () => {
	...
	it.each(["example.com", "192.168.1.10", "10.0.0.1", ""])(
		"%s is not loopback",
		(host) => {
...
describe("normalizePreviewUrl", () => {
	...
	it("respects explicit schemes", () => {
		expect(normalizePreviewUrl("https://localhost:5173")).toBe(
			"https://localhost:5173/",
		);
		expect(normalizePreviewUrl("http://example.com/path?q=1")).toBe(
	...
	it("rejects unparseable input without retaining credentials or tokens", () => {
		...
		try {
			...
			expect(error).not.toHaveProperty("rawUrl");
			expect((error as PreviewUrlNormalizationError).cause).toBeInstanceOf(
				Error,
			);
