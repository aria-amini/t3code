...
describe("RelayClient", () => {
			...
			Effect.gen(function* () {
				...
				Effect.provide(
					Layer.mergeAll(
						NodeServices.layer,
						makeHttpClientLayer(
							new TextEncoder().encode("test-cloudflared-binary"),
						),
						makeSpawnerLayer([]),
						hostRuntimeLayer(),
					),
	...
	it.effect(
		"rejects downloads whose checksum does not match the pinned manifest",
		() =>
			Effect.gen(function* () {
				const fileSystem = yield* FileSystem.FileSystem;
				const baseDir = yield* fileSystem.makeTempDirectoryScoped({
	...
	it.effect.skipIf(windowsHost)(
		"serializes concurrent installs within one runtime",
		() => {
