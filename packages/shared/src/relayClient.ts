...
export type AvailableRelayClient = Extract<
	RelayClientStatus,
	{ readonly status: "available" }
>;

export class RelayClientInstallError extends Data.TaggedError(
...
class CloudflaredCommandError extends Data.TaggedError(
...
export const makeCloudflaredRelayClient = Effect.fn("cloudflared.make")(
	function* (
	...
	> {
		...
		const releaseAsset =
			options.releaseAsset ?? resolveReleaseAsset(platform, arch);
		const loadCloudflaredConfig = Effect.suspend(() => CloudflaredConfig).pipe(
			Effect.orDie,
		);
		...
		const isExecutableFile = Effect.fn("cloudflared.isExecutableFile")(
		...
		const acquireInstallLock = Effect.fn("cloudflared.acquireInstallLock")(
			function* (lockPath: string) {
				for (
					let attempt = 0;
					attempt < INSTALL_LOCK_RETRY_COUNT;
					attempt += 1
				) {
					const acquired = yield* fileSystem
						.writeFileString(lockPath, "", { flag: "wx" })
						.pipe(
							Effect.as(true),
							Effect.catch((error) =>
					...
					if (
						Option.isSome(mtime) &&
						now - mtime.value.getTime() > INSTALL_LOCK_STALE_MS
					) {
			...
			return yield* Effect.gen(function* () {
				...
				yield* fileSystem
					...
					.pipe(
						wrapInstallFailure(
							"write_failed",
							"Could not write the relay client download.",
						),
					);
				...
				if (releaseAsset.archive === "tgz") {
					yield* runCommand("tar", [
						...
						wrapInstallFailure(
							"write_failed",
							"Could not extract the relay client.",
						),
					);
				}
				if (platform !== "win32") {
					yield* fileSystem
						...
						.pipe(
							wrapInstallFailure(
								"write_failed",
								"Could not make the relay client executable.",
							),
						);
				}
				...
				yield* fileSystem
					...
					.pipe(
						wrapInstallFailure(
							"write_failed",
							"Could not stage the relay client.",
						),
					);
				yield* fileSystem
					...
					.pipe(
						wrapInstallFailure(
							"write_failed",
							"Could not activate the relay client.",
						),
						Effect.ensuring(
							fileSystem
		...
		const installWithProgress: RelayClientShape["installWithProgress"] = (
			report,
		) =>
