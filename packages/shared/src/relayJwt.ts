...
export class RelayJwtError extends Schema.TaggedError<RelayJwtError>()(
	...
	static diagnosticCode(error: RelayJwtError): string {
		...
		return error.cause instanceof Error && error.cause.name
			? error.cause.name
			: "unknown";
	}
}
...
export function signRelayJwt(input: {
	...
	return Effect.tryPromise({
		try: async () => {
			...
			new RelayJwtError({ operation: "sign", typ: input.typ, cause }),
	});
}
		...
		try: async () => {
			...
			const verified = await jwtVerify(input.token, key, {
				...
				currentDate: DateTime.toDate(
					DateTime.makeUnsafe(input.nowEpochSeconds * 1_000),
				),
			});
