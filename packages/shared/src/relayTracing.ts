...
function cleanTraceStack(error: Error): string {
	...
	const effectFrameIndex = lines.findIndex(
		(line, index) =>
			index > 0 && /(?:Generator\.next|~effect\/Effect)/.test(line),
	);
	return effectFrameIndex < 0
		? stack
		: lines.slice(0, effectFrameIndex).join("\n");
}
...
export function makeRelayClientTracingLayer(
	...
	return Layer.effect(
		RelayClientTracer,
		Tracer.Tracer.pipe(
			Effect.map(nonInterferingTracer),
