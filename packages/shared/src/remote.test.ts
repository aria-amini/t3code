...
describe("remote", () => {
	...
	it("normalizes a protocol-relative host from a hosted pairing link", () => {
		expect(
			resolveRemotePairingTarget({
				pairingUrl:
					"https://app.t3.codes/pair?host=%2F%2Fremote.example.com#token=pairing-token",
			}),
		).toEqual({
			credential: "pairing-token",
			httpBaseUrl: "https://remote.example.com/",
	...
	it("rejects unsupported direct pairing URL protocols", () => {
		...
		expect(
			(pairingUrlError as RemotePairingUrlInvalidError).cause,
		).toBeUndefined();
	});
	...
	it("uses distinct structural errors for missing pairing inputs", () => {
		expect(() => resolveRemotePairingTarget({})).toThrowError(
			RemoteBackendUrlMissingError,
		);
		expect(() =>
			resolveRemotePairingTarget({
				pairingUrl: "https://remote.example.com/pair",
			}),
	...
	it("preserves URL parsing causes with their input source", () => {
		...
		expect((hostError as RemoteBackendUrlInvalidError).cause).toBeInstanceOf(
			TypeError,
		);
	});
});
