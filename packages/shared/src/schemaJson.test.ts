...
const decodeLenientJson = Schema.decodeUnknownSync(
	fromLenientJson(Schema.Unknown),
);
...
Done.`),
  ...
  "body": ""
	...
	it("returns trimmed input when no JSON object starts", () => {
		expect(extractJsonObject("  no structured output  ")).toBe(
	...
	it("preserves commas before brackets inside string values", () => {
		...
		expect(decodeLenientJson('{"values":[1, 2,],}')).toEqual({
			values: [1, 2],
		});
	});

	it("formats schema failures with paths without exposing invalid values", () => {
		const decodeCredential = decodeJsonResult(
			Schema.Struct({ token: Schema.Number }),
		);
		const decoded = decodeCredential('{"token":"credential=secret-value"}');

		expect(Result.isFailure(decoded)).toBe(true);
		if (Result.isFailure(decoded)) {
			expect(formatSchemaError(decoded.failure)).toBe(
				'Invalid type\n  at ["token"]',
			);
		}
	});
	...
	it("summarizes unexpected defects without serializing their messages", () => {
		const diagnostic = formatSchemaError(
			Cause.die(new Error("credential=secret-value")),
		);
	...
	it("retains the omitted issue count when bounding long diagnostics", () => {
		const longPath = Array.from(
			{ length: 16 },
			(_, index) => `${index}-${"segment".repeat(16)}`,
		);
		const decode = decodeJsonResult(
			Schema.String.check(
				Schema.makeFilter(() => ({
