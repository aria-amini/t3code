...
function formatDiagnosticIssue(issue: SchemaDiagnosticIssue): string {
	...
	const path = issue.path
		...
		.join("");
	const suffix =
		issue.path.length > MAX_SCHEMA_DIAGNOSTIC_PATH_SEGMENTS ? "[...]" : "";
	return `${issue.message}\n  at ${path}${suffix}`;
}
...
function collectSchemaDiagnosticIssues(
	...
	switch (issue._tag) {
		...
		case "Filter":
			...
			break;
		case "Pointer":
			return collectSchemaDiagnosticIssues(
				issue.issue,
				[...path, ...issue.path],
				diagnostics,
			);
		case "Composite":
			return issue.issues.reduce(
				(count, issue) =>
					count + collectSchemaDiagnosticIssues(issue, path, diagnostics),
				0,
			);
		case "AnyOf":
...
export const formatSchemaError = (cause: Cause.Cause<Schema.SchemaError>) => {
		...
		switch (reason._tag) {
			case "Fail":
				...
				if (Schema.isSchemaError(reason.error)) {
					issueCount += collectSchemaDiagnosticIssues(
						reason.error.issue,
						[],
						issues,
					);
	...
	return (
		truncateDiagnostic(
			formatted,
			MAX_SCHEMA_DIAGNOSTIC_LENGTH - suffix.length,
		) + suffix
	);
};
