...
function parseYaml<E extends string>(
	...
	return SchemaGetter.transformOrFail((input: E) =>
		Effect.try({
			try: () => parseYamlString(input, options) as unknown,
				...
				new SchemaIssue.InvalidValue({ message: formatYamlParseError(error) }),
		}),
	);
}
	...
	return SchemaGetter.transformOrFail((input: unknown) =>
		Effect.try({
			try: () => stringifyYamlValue(input, options),
				...
				new SchemaIssue.InvalidValue({ message: "Failed to stringify YAML." }),
		}),
	);
}
 ...
 * ```
 */
export const fromYamlString = new SchemaTransformation.Transformation<
	unknown,
	string
