...
function parsePrereleaseSegments(
 ...
 */
export const satisfiesSemverRange: (
	rawVersion: string,
	range: string,
) => boolean = function satisfiesSemverRange(rawVersion, range) {
	...
	return range.split("||").some((group) => {
		...
		return comparators.every((comparator) => {
			...
			const targetMatch = targetVersion.match(
				/^(\d+)(?:\.(\d+))?(?:\.(\d+))?$/,
			);
			if (!targetMatch) {
				return false;
