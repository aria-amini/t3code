...
describe("serverSettings helpers", () => {
	it("inherits actions, preserves existing actions, and supports empty overrides and reset", () => {
		...
		const existing = {
			...project,
			scripts: [{ ...action, command: "npm run lint" }],
		};
		...
		expect(resolveProjectScripts(reset, existing)).toEqual(
			changedDefault.defaultProjectScripts,
		);
	...
	it("preserves other projects' actions when overriding, clearing, or resetting one project", () => {
		...
		expect(resolveProjectScripts(secondUpdate, firstProject)).toEqual([
			firstAction,
		]);
		expect(resolveProjectScripts(secondUpdate, secondProject)).toEqual([
			secondAction,
		]);
		...
		expect(resolveProjectScripts(cleared, secondProject)).toEqual([
			secondAction,
		]);
		...
		expect(
			resolveProjectScripts(reset, { ...firstProject, scripts: [firstAction] }),
		).toEqual([defaultAction]);
	...
	it("inherits automatic pull while preserving legacy opt-ins and explicit overrides", () => {
		...
		expect(
			resolveProjectAutoPull(DEFAULT_SERVER_SETTINGS, projectId, false),
		).toBe(false);
		expect(
			resolveProjectAutoPull(DEFAULT_SERVER_SETTINGS, projectId, true),
		).toBe(true);
		...
		expect(
			resolveProjectAutoPull(disabled, ProjectId.make("other-project"), false),
		).toBe(false);
	});
	...
	it("inherits browser access and restores inheritance when a project override is removed", () => {
		...
		expect(resolveProjectAgentBrowserAccess(overridden, otherProjectId)).toBe(
			true,
		);
		const reset = applyServerSettingsPatch(overridden, {
		...
		expect(resolveProjectAgentBrowserAccess(enabled, otherProjectId)).toBe(
			false,
		);
	});
	...
	it("preserves other projects' boolean overrides across separate updates and resets", () => {
		...
		for (const projectId of [firstProjectId, secondProjectId]) {
			expect(resolveProjectAutoPull(secondUpdate, projectId, false)).toBe(
				false,
			);
			expect(resolveProjectAgentBrowserAccess(secondUpdate, projectId)).toBe(
				false,
			);
		}
		...
		expect(resolveProjectAgentBrowserAccess(reset, secondProjectId)).toBe(
			false,
		);
		expect(reset.projectAutoPullOverrides[firstProjectId]).toBeUndefined();
		expect(
			reset.projectAgentBrowserAccessOverrides[firstProjectId],
		).toBeUndefined();
		expect(resolveProjectAutoPull(secondUpdate, firstProjectId, false)).toBe(
			false,
		);
		expect(resolveProjectAgentBrowserAccess(secondUpdate, firstProjectId)).toBe(
			false,
		);
	});
	...
	it("replaces and clears conversation model defaults without retaining old options", () => {
		...
		expect(
			applyServerSettingsPatch(updated, { defaultModelSelection: null })
	...
	it("falls back from a disabled source control writer provider without clearing its selection", () => {
		...
		expect(settings.sourceControlWriterModelSelection).toBe(
			sourceControlWriterModelSelection,
		);
	});
	...
	it("falls back from an unavailable source control writer provider", () => {
		...
		expect(
			resolveSourceControlWriterModelSelection(settings, [unavailableProvider]),
	...
	it("replaces and removes individual usage prices without clobbering other models", () => {
		...
		const current = applyServerSettingsPatch(DEFAULT_SERVER_SETTINGS, {
			usagePriceOverrides: {
				"example-model": { ...prices, cacheReadCostPerMillionTokens: 0.5 },
			},
		});
		const added = applyServerSettingsPatch(current, {
		...
		expect(
			current.usagePriceOverrides["example-model"]
	...
	it("turns legacy interval patches into custom background activity overrides", () => {
		...
		expect(resolveServerBackgroundActivitySettings(next).profile).toBe(
			"balanced",
		);
		expect(
			Duration.toMillis(
				resolveServerBackgroundActivitySettings(next).automaticGitFetchInterval,
			),
		).toBe(15_000);
	});
