...
export function resolveProjectAgentBrowserAccess(
	settings: Pick<
		ServerSettings,
		"enableAgentBrowserAccess" | "projectAgentBrowserAccessOverrides"
	>,
	projectId: ProjectId,
): boolean {
	return (
...
function extractPersistedServerObservabilitySettings(input: {
	...
	return {
		otlpTracesUrl: normalizePersistedServerSettingString(
			input.observability?.otlpTracesUrl,
		),
		otlpMetricsUrl: normalizePersistedServerSettingString(
...
function shouldReplaceTextGenerationModelSelection(
	...
	return Boolean(
		patch && (patch.instanceId !== undefined || patch.model !== undefined),
	);
}
...
function mergeModelSelectionOptionsById(input: {
	...
	const merged = new Map(
		(input.current ?? []).map((selection) => [selection.id, selection.value]),
	);
...
export function applyServerSettingsPatch(
	...
	const currentBackgroundActivity =
		normalizeServerBackgroundActivitySettings(current);
		...
		backgroundActivityProfile !== undefined
			...
			: automaticGitFetchInterval !== undefined ||
				...
				? {
						...
						baseProfile: getBackgroundActivityBaseProfile(
		...
		...(patch.sourceControlWriterModelSelection !== undefined
			? {
					sourceControlWriterModelSelection:
						patch.sourceControlWriterModelSelection,
				}
			: {}),
		...(automaticGitFetchInterval !== undefined
			? { automaticGitFetchInterval }
