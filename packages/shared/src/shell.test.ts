...
describe("readEnvironmentFromLoginShell", () => {
	...
	it("preserves surrounding whitespace in captured values", () => {
		...
		expect(
			readEnvironmentFromLoginShell("/bin/zsh", ["CUSTOM_VAR"], execFile),
		).toEqual({
...
describe("listLoginShellCandidates", () => {
	it("returns env shell, user shell, then the platform fallback without duplicates", () => {
		expect(
			listLoginShellCandidates("darwin", " /opt/homebrew/bin/nu ", "/bin/zsh"),
		).toEqual(["/opt/homebrew/bin/nu", "/bin/zsh"]);
	});

	it("falls back to the platform default when no shells are available", () => {
		expect(listLoginShellCandidates("linux", undefined, "")).toEqual([
			"/bin/bash",
		]);
	});
});
...
describe("mergePathEntries", () => {
	...
	it("uses the platform-specific delimiter", () => {
		expect(
			mergePathEntries("C:\\Tools;C:\\Windows", "C:\\Windows;C:\\Git", "win32"),
		).toBe("C:\\Tools;C:\\Windows;C:\\Git");
	});
});
...
describe("mergePathValues", () => {
	...
	it("dedupes case-sensitively on POSIX", () => {
		expect(
			mergePathValues("/usr/local/bin:/usr/bin", "/usr/bin:/USR/BIN", "linux"),
		).toBe("/usr/local/bin:/usr/bin:/USR/BIN");
	});
});
...
effectIt.layer(NodeServices.layer)("resolveCommandPath", (it) => {
			...
			Effect.gen(function* () {
				...
				const resolved = yield* resolveCommandPath(command, {
					...
					Effect.provideService(FileSystem.FileSystem, {
						...
						stat: (filePath) =>
							fs.stat(
								filePath === executable ? filePath : path.join(cwd, "missing"),
							),
					}),
				);
	...
	it.effect(
		"keeps cached misses until expiry while allowing explicit paths",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
				const cwd = yield* fs.makeTempDirectoryScoped({
					prefix: "t3-path-cache-",
				});
				const executable = path.join(cwd, "appeared.CMD");
				const options = { env: { PATH: `${cwd};${cwd}`, PATHEXT: ".CMD" } };

				expect(
					(yield* resolveCommandPath("appeared", options).pipe(Effect.result))
						._tag,
				).toBe("Failure");
				yield* fs.writeFileString(executable, "@echo off\n");
				expect(
	...
	it.effect("keeps upper and lowercase PATHEXT candidates", () =>
		Effect.gen(function* () {
			...
			expect(probed).toHaveLength(8);
			expect(
				probed.filter((filePath) => /\.(COM|EXE|BAT|CMD)$/.test(filePath)),
			).toHaveLength(4);
			expect(
				probed.filter((filePath) => /\.(com|exe|bat|cmd)$/.test(filePath)),
			).toHaveLength(4);
...
effectIt.layer(NodeServices.layer)("resolveSpawnCommand", (it) => {
	...
	it.effect(
		"escapes the executable and arguments for Windows command shims",
		() =>
			Effect.gen(function* () {
				const command = yield* resolveSpawnCommand(
					"vp",
	...
	it.effect(
		"resolves against the effective environment when extending host env",
		() =>
			Effect.gen(function* () {
	...
	it.effect(
		"does not fall back to a shell for unresolved Windows commands",
		() =>
			Effect.gen(function* () {
				const command = yield* resolveSpawnCommand(
					"missing & calc",
...
effectIt.layer(NodeServices.layer)("resolveWindowsEnvironment", (it) => {
	it.effect(
		"uses known CLI directories as a fallback without changing shell PATH priority",
		() =>
			Effect.gen(function* () {
				...
				expect(readEnvironment).toHaveBeenCalledWith(["PATH"], {
					loadProfile: false,
				});
	...
	it.effect(
		"loads the PowerShell profile when baseline env cannot resolve node",
		() =>
			Effect.gen(function* () {
				const readEnvironment = vi.fn(
					(
	...
	it.effect(
		"keeps the baseline env when profiled probe still does not resolve node",
		() =>
			Effect.gen(function* () {
				const readEnvironment = vi.fn(
					(
