...
export class CommandResolutionError extends Data.TaggedError(
...
function resolveSpawnExecutableWithNode(
	...
	const isExecutable = (candidate: string) => {
		try {
			...
			if (platform === "win32") {
				return windowsPathExtensions.includes(
					path.extname(candidate).toUpperCase(),
				);
			}
	...
	for (const pathEntry of (readEnvPath(env) ?? "").split(
...
export const SpawnExecutableResolution =
	Context.Reference<SpawnExecutableResolver>(
...
export const WindowsShellEnvironment =
	Context.Reference<WindowsShellEnvironmentReader>(
...
export const CommandAvailability =
	Context.Reference<CommandAvailabilityChecker>(
...
function normalizePathEntryForComparison(
	entry: string,
	platform: NodeJS.Platform,
): string {
...
function resolveCommandCandidates(
	...
	if (
		extension.length > 0 &&
		windowsPathExtensions.includes(normalizedExtension)
	) {
...
export const CommandResolutionCache = Context.Reference<
	Map<string, CommandResolutionCacheEntry>
...
const resolveCommandPathForPlatform = Effect.fn(
...
export const resolveCommandPath = Effect.fn("shell.resolveCommandPath")(
...
export const resolveSpawnCommand = Effect.fn("shell.resolveSpawnCommand")(
	function* (
		...
		const resolvedCommand =
			resolveExecutable(command, platform, env) ?? command;
...
export const isCommandAvailable = Effect.fn("shell.isCommandAvailable")(
	function* (command: string, options: CommandAvailabilityOptions = {}) {
...
export const resolveWindowsEnvironment = Effect.fn(
	"shell.resolveWindowsEnvironment",
)(function* (
	env: NodeJS.ProcessEnv,
): Effect.fn.Return<
...
> {
	...
	const baselinePatch: Partial<NodeJS.ProcessEnv> = baselinePath
		? { PATH: baselinePath }
		: {};
	...
	const profiledPath = mergePathValues(
		profiledEnvironment.PATH,
		baselinePath,
		"win32",
	);
	const profiledPatch: Partial<NodeJS.ProcessEnv> = {
		...(profiledPath ? { PATH: profiledPath } : {}),
		...(profiledEnvironment.FNM_DIR
			? { FNM_DIR: profiledEnvironment.FNM_DIR }
			: {}),
