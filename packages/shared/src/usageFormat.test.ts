...
describe("hourly usage formatting", () => {
	it("enumerates 24 fixed buckets across a rolling window", () => {
		const hours = enumerateHourStarts(
			"2026-08-10T12:37:00.000Z",
			"2026-08-11T12:37:00.000Z",
		);
	...
	it("formats rolling instants in the requested time zone", () => {
		...
		expect(formatDateTimeShort("2026-08-11T17:37:00.000Z", "UTC")).toBe(
			"Aug 11, 5 PM",
		);
	});
	...
	it("disambiguates repeated hours during a fall-back transition", () => {
		expect(
			formatHourShort("2026-11-01T05:37:00.000Z", "America/New_York"),
		).toBe("1 AM EDT");
		expect(
			formatHourShort("2026-11-01T06:37:00.000Z", "America/New_York"),
		).toBe("1 AM EST");
	});
	...
	it("makes hourly tooltip dates relative to the window in its requested time zone", () => {
		...
		expect(
			formatRelativeHourShort("2026-08-10T17:37:00.000Z", windowEnd, "UTC"),
		).toBe("5 PM yesterday");
		expect(
			formatRelativeHourShort("2026-08-11T14:37:00.000Z", windowEnd, "UTC"),
		).toBe("2 PM today");
		expect(
			formatRelativeHourShort(
