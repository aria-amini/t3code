...
describe("pace", () => {
	...
	it("has no pace without a reset or a duration", () => {
		...
		expect(
			paceOf({ ...window, windowDurationMins: undefined }, now),
		).toBeNull();
	...
	it("phrases the reset as a countdown", () => {
		...
		expect(
			formatResetsIn({ ...window, resetsAt: "2026-09-06T15:30:00.000Z" }, now),
		).toBe("resets in 3d 3h");
		expect(
			formatResetsIn({ ...window, resetsAt: "2026-09-03T11:00:00.000Z" }, now),
		).toBe("resets now");
	});
});
...
describe("limitsNotice", () => {
	it("explains empty bars and passes provider messages through", () => {
		...
		expect(limitsNotice({ checkedAt, windows: [] })).toBe(
			"No limits reported.",
		);
		expect(
			limitsNotice({
...
describe("collectLimitsGroups", () => {
	it("labels environments only when more than one reports limits", () => {
		...
		expect(
			collectLimitsGroups(one as never).map((group) => group.environmentLabel),
		).toEqual([null]);
		...
		expect(
			collectLimitsGroups(two as never).map((group) => group.environmentLabel),
...
describe("collectLimitSources", () => {
		...
		return new Map([
			[
				EnvironmentId.make("env-a"),
				{
					...
					serverConfig: {
						providers,
						usageLimitSources: [{ ...source, accounts }],
					},
				},
			],
		]);
	...
	it("keeps other providers, other emails, and unidentified accounts with the same plan", () => {
		...
		expect(
			collectLimitSources(presentations([native], accounts)),
		).toMatchObject([{ accounts: accounts.slice(1), hiddenAccountCount: 1 }]);
		expect(
			collectLimitSources(
				presentations(
					[{ ...native, auth: { status: "authenticated" } }],
					accounts,
				),
			)[0]?.accounts,
		).toEqual(accounts);
	...
	it.each([
		...
		(overrides) => {
			expect(
				collectLimitSources(presentations([{ ...native, ...overrides }])),
			).toMatchObject([{ accounts: [account], hiddenAccountCount: 0 }]);
		},
	);
	...
	it("keys sources per environment and names the environment only when several have some", () => {
		...
		expect(
			collectLimitSources(one as never).map((entry) => [
				entry.key,
				entry.label,
			]),
		).toEqual([["env-a:cliproxy-hub", "hub"]]);
		...
		expect(
			collectLimitSources(two as never).map((entry) => entry.label),
		).toEqual(["Laptop · hub", "Desktop · hub"]);
	});
});
...
describe("pools", () => {
	...
	it("merges one account reported natively on two environments and by a hub into one entry", () => {
		...
		const input = new Map([
			[
				EnvironmentId.make("env-a"),
				{ ...laptop, serverConfig: { providers: [native] } },
			],
			[
	...
	it("takes windows from a fresher hub read but credits and redeem from the native instance", () => {
		...
		expect(account?.environments).toEqual([
			{ environmentId: "env-a", label: "Laptop" },
		]);
	});
	...
	it("redeems through the hub when it holds a credit, even with a fresher native read", () => {
		...
		expect(account?.redeem).toEqual({
			...
			input: {
				sourceId: "hub",
				accountId: "claude-same@example.com.json",
				creditId: "hub-credit",
			},
		});
	...
	it("redeems on the environment whose snapshot supplied the credits on show", () => {
		...
		const input = new Map([
			[
				EnvironmentId.make("env-a"),
				{ ...laptop, serverConfig: { providers: [stale] } },
			],
			[
				EnvironmentId.make("env-b"),
				{
	...
	it("uses the freshest hub credit and its environment even when the account is also native", () => {
		...
		expect(account?.redeem).toEqual({
			...
			input: {
				sourceId: "hub",
				accountId: "codex-same.json",
				creditId: "credit-2",
			},
		});
		hubAccount.usageLimits.resetCredits.availableCount = 0;
		expect(
			collectLimitAccounts(input)[0]?.limits.resetCredits?.availableCount,
		).toBe(0);
	});
	...
	it("keeps distinct hub accounts redeemable through their own source", () => {
		...
		expect(
			collectLimitAccounts(input).map((account) => account.redeem),
		).toEqual(
			...
			[
				EnvironmentId.make("env-a"),
				{
					...
					serverConfig: {
						providers: [
							shared,
							{ ...shared, instanceId: ProviderInstanceId.make("work") },
						],
					},
				},
			],
	...
	it("keys a hub account without an email by hub, so two environments on one hub share it", () => {
		...
		const input = new Map([
			[
				EnvironmentId.make("env-a"),
				{ ...laptop, serverConfig: { usageLimitSources: [hub] } },
			],
			[
				EnvironmentId.make("env-b"),
				{
	...
	it("pools windows by id across accounts and orders resets by when they land", () => {
			...
			collectLimitAccounts(input).map((account) =>
				account.key === "hub:b"
					? {
							...
							limits: {
								...account.limits,
								windows: account.limits.windows.map((w) => ({
									...w,
									resetsAt: undefined,
								})),
							},
		...
		expect(
			session?.resets.map((reset) => [
				reset.member.account.key,
		...
		expect(
			mixed[0]?.windows.map((window) => [window.kind, window.members.length]),
		).toEqual([
			["session", 2],
			["weekly", 1],
			["monthly", 1],
...
describe("collectLimitNotices", () => {
	...
	it("names failures and silence, skips unsupported accounts, and labels environments only when several", () => {
				...
				EnvironmentId.make("env-a"),
				{
					...
					serverConfig: {
						...
						usageLimitSources: [
							...
							{
								...hub,
								id: UsageLimitSourceId.make("down"),
								label: "down",
								error: "ECONNREFUSED",
							},
						],
		...
		expect(collectLimitNotices(one)[0]).toBe(
			"Laptop · Claude Max: Could not read limits.",
		);
	});
});
...
describe("/usage-limits", () => {
	...
	it("uses hub credit balances and redemption targets in the composer, including native duplicates", () => {
		const hubs = sources.map((source) => ({
			...
			accounts: source.accounts.map((account) => ({
				...
				usageLimits: {
					...
					resetCredits: {
						availableCount: 2,
						nextCreditId: `${account.id}-credit`,
					},
				},
			})),
		}));
		...
		expect(
			report?.accounts.find((account) => account.id === "hub:oss")
				?.resetCreditInput,
		).toEqual({
			sourceId: "hub",
			accountId: "oss",
	...
	it("supports a source-only provider and keeps duplicates when the native probe failed", () => {
		...
		expect(
			collectProviderUsageLimits(selected.instanceId, [provider({})], [], now),
		).toBeNull();
		expect(
			collectProviderUsageLimits(
				selected.instanceId,
	...
	it("advertises global and workspace commands only for providers present in Limits", () => {
		...
		expect(supported?.slashCommands.map((command) => command.name)).toEqual([
			"usage-limits",
		]);
		expect(
			supported?.workspaceSnapshots?.[0]?.slashCommands.map(
				(command) => command.name,
			),
		).toEqual(["usage-limits"]);
		expect(
			withUsageLimitsCommands([withWorkspace], [])[0]?.slashCommands,
		).toEqual([]);
		...
		expect(
			withUsageLimitsCommands(
				[withWorkspace],
				[unreadable],
			)[0]?.slashCommands.map((command) => command.name),
		).toEqual(["usage-limits"]);
		expect(
			withUsageLimitsCommands([selected], [])[0]?.slashCommands.map(
				(command) => command.name,
			),
		).toEqual(["usage-limits"]);
	});
...
describe("sameUsageLimitCommandCoverage", () => {
	...
	it("ignores quota movement but not the drivers offered the command", () => {
		...
		expect(
			sameUsageLimitCommandCoverage(withCodex, [{ ...base, accounts: [] }]),
		).toBe(false);
	});
	it("treats a failed read as a change in coverage, in both directions", () => {
		...
		expect(
			sameUsageLimitCommandCoverage(failed, [
				{ ...base, accounts: [], error: "still down" },
			]),
		).toBe(true);
	});
});
