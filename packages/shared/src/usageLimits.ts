...
export function collectLimitsGroups(
	...
	for (const [environmentId, presentation] of presentations) {
		const providers = providersWithLimits(
	...
	return groups.length > 1
		? groups
		: groups.map((group) => ({ ...group, environmentLabel: null }));
}
...
> {
	...
	return perEnvironment.flatMap(
		({ environmentId, environmentLabel, sources }) =>
			sources.map((source) => {
...
export function collectLimitAccounts(
	...
	const merge = (key: string, next: LimitAccount) => {
		...
		if (
			...
			(!previousHub ||
				Date.parse(next.limits.checkedAt) >
					Date.parse(previousHub.limits.checkedAt))
		) {
		...
		if (
			...
			(!previousCredit ||
				Date.parse(next.limits.checkedAt) >
					Date.parse(previousCredit.limits.checkedAt))
		) {
			...
			...next.environments.filter(
				(candidate) =>
					!previous.environments.some(
						(seen) => seen.environmentId === candidate.environmentId,
					),
			),
		];
	...
	for (const [environmentId, presentation] of presentations) {
		...
		for (const provider of providersWithLimits(
			...
			if (!provider.usageLimits || limitsNotice(provider.usageLimits) !== null)
				continue;
			merge(
...
export function collectLimitNotices(
	...
	for (const presentation of presentations.values()) {
		...
		for (const source of presentation.serverConfig?.usageLimitSources ?? []) {
			if (source.error) {
				notices.push(
					`${label(environmentLabel, source.label)}: ${source.error}`,
				);
			} else if (source.accounts.length === 0) {
				notices.push(
					`${label(environmentLabel, source.label)}: No accounts reported.`,
				);
			}
		}
	}
...
function poolWindows(
	...
	const pools = [...byKey.values()].map((unordered): LimitPoolWindow => {
		...
		const usedPercent =
			members.reduce((sum, m) => sum + m.window.usedPercent, 0) /
			members.length;
		...
		const timed = members.flatMap((m) => {
			...
			return share === null
				? []
				: [{ used: m.window.usedPercent, elapsed: share }];
		});
		const timedUsed = timed.reduce((sum, t) => sum + t.used, 0) / timed.length;
		const meanElapsed =
			timed.length > 0
				? timed.reduce((sum, t) => sum + t.elapsed, 0) / timed.length
				: null;
					...
					: [
							{
								...
								restoresPercent: Math.round(
									member.window.usedPercent / members.length,
								),
							},
						];
			})
...
export function limitsNotice(limits: ServerProviderUsageLimits): string | null {
	if (limits.unavailable?.reason === "unsupported") {
		return (
			limits.unavailable.message ?? "This account has no subscription limits."
		);
	}
...
export function sameUsageLimitCommandCoverage(
	...
	return (
		...
		[...before].every((driver) => after.has(driver))
	);
}
...
export function withUsageLimitsCommands(
	...
	return providers.map((provider) => {
		if (!hasProviderUsageLimits(provider.driver, providers, sources))
			return provider;
...
export function collectProviderUsageLimits(
	...
	if (!selected || !hasProviderUsageLimits(selected.driver, providers, sources))
		return null;
	...
	for (const provider of native) {
		...
		const hubCredits = sources
			.flatMap((source) =>
				source.accounts.map((account) => ({ source, account })),
			)
			.filter(
				({ account }) =>
					key !== null &&
			...
			.sort(
				(a, b) =>
					Date.parse(b.account.usageLimits.checkedAt) -
					Date.parse(a.account.usageLimits.checkedAt),
			)[0];
		...
		accounts.push({
			...
			limits: showHubCredits
				? {
						...
						resetCredits: hubCredits.account.usageLimits.resetCredits,
					}
				: provider.usageLimits,
		});
	}
	for (const source of sources) {
		const matching = source.accounts.filter(
			(account) => account.driver === selected.driver,
		);
