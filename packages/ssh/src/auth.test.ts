...
describe("ssh auth", () => {
	it.effect(
		"detects ssh auth failures from common permission denied messages",
		() =>
			Effect.sync(() => {
				assert.equal(
					isSshAuthFailure(
				...
				assert.equal(
					isSshAuthFailure(new Error("Permission denied (publickey).")),
					true,
				);
				assert.equal(
					isSshAuthFailure(new Error("Connection timed out")),
					false,
				);
				assert.equal(
					isSshAuthFailure(new Error("mkdir: Permission denied")),
					false,
				);
			}),
	);
