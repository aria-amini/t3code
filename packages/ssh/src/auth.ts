...
const getDefaultSshAskpassDirectory = Effect.fn(
	"ssh/auth.getDefaultSshAskpassDirectory",
)(function* () {
	const fs = yield* FileSystem.FileSystem;
	const path = yield* Path.Path;
	const parentDirectory = yield* fs.makeTempDirectory({
		prefix: "t3code-ssh-runtime-",
	});
...
export const buildSshAskpassHelperDescriptor = Effect.fn(
	...
	if (platform === "win32") {
		const powershellPath = joinSshAskpassPath(
			directory,
			"ssh-askpass.ps1",
			platform,
		);
...
const ensureSshAskpassHelpers = Effect.fn("ssh/auth.ensureSshAskpassHelpers")(
	function* (input: {
		readonly directory: string;
	}): Effect.fn.Return<
		string,
	...
	> {
		...
		yield* fs.makeDirectory(path.dirname(descriptor.launcherPath), {
			recursive: true,
		});
...
export const buildSshChildEnvironment = Effect.fn(
	"ssh/auth.buildSshChildEnvironment",
)(function* (
...
> {
	...
	const directory =
		input.askpassDirectory ?? (yield* getDefaultSshAskpassDirectory());
	...
	return {
		...
		...(platform === "win32" || baseEnv.DISPLAY || hostDisplay
			? {}
			: { DISPLAY: "t3code" }),
	};
});
