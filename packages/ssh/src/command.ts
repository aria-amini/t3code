...
export const resolveSshCommand = Effect.map(
	HostProcessPlatform,
...
export function parseSshResolveOutput(
	alias: string,
	stdout: string,
): DesktopSshEnvironmentTarget {
...
export function targetConnectionKey(
	...
	Effect.try({
		try: () => buildSshHostSpec(target),
			...
			new SshInvalidTargetError({
				message:
					cause instanceof Error ? cause.message : "SSH target is invalid.",
			}),
	});
...
function stdinStream(input: string | undefined) {
	return input === undefined
		? Stream.empty
		: Stream.make(encoder.encode(input));
}

const runSshCommandInScope = Effect.fn("ssh/command.runSshCommand.inScope")(
	function* (
		target: DesktopSshEnvironmentTarget,
...
> {
	return yield* Effect.scopedWith((commandScope) =>
		...
		Effect.timeoutOption(
			Duration.millis(input.timeoutMs ?? DEFAULT_SSH_COMMAND_TIMEOUT_MS),
		),
...
export const resolveSshTarget = Effect.fn("ssh/command.resolveSshTarget")(
	function* (
		alias: string,
	): Effect.fn.Return<
		DesktopSshEnvironmentTarget,
	...
	> {
		...
		if (trimmedAlias.length === 0) {
			return yield* new SshInvalidTargetError({
				message: "SSH host alias is required.",
			});
		}
		...
		return yield* runSshCommand(
			...
			Effect.map((result) =>
				parseSshResolveOutput(trimmedAlias, result.stdout),
			),
			Effect.tap((target) =>
				Effect.logDebug(
					"ssh.target.resolve.succeeded",
					sshTargetLogFields(target),
				),
			),
			Effect.catch((cause) =>
