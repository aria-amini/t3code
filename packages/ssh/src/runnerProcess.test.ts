									...
									["-s", "--", "serve", "a path with spaces"],
									{
										...
										stdin: Stream.make(
											new TextEncoder().encode(
												buildRemoteT3RunnerScript({ packageSpec }),
											),
										),
									},
								),
							...
							const receipt = yield* Effect.raceFirst(
								...
								Fiber.join(output).pipe(
									...
									Effect.flatMap((message) =>
										Effect.die(
											new Error(`Runner exited before listening: ${message}`),
										),
									),
								),
							);
								...
								Effect.gen(function* () {
									if (yield* child.isRunning) {
										yield* Effect.callback<void>((resume) => {
											const connection = NodeNet.connect(
												receipt.port,
												"127.0.0.1",
											);
...
`,
					...
					if (mode === "timeout") {
						...
						for (const [name, contents] of Object.entries(savedState)) {
							assert.equal(
								yield* fs.readFileString(path.join(fixture, name)),
								contents,
							);
						}
					...
					if (mode === "external") {
						...
						assert.equal(yield* child.exitCode, 0);
						assert.equal(
							yield* fs.readFileString(signalPath),
							mode === "timeout" ? "2" : "1",
						);
					}
				}).pipe(Effect.provide(NodeServices.layer), Effect.scoped),
...
`,
					...
					if (installFailed) {
						...
						assert.include(
							stderr,
							`Remote host could not install ${packageSpec}.`,
						);
					...
					const usesInstaller =
						mode !== "existing-cli" && mode !== "node-override";
					const calls = yield* fs.readFileString(callsPath);
					if (usesInstaller) {
						assert.deepEqual(
							calls
