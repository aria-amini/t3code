...
describe("ssh tunnel scripts", () => {
	it("builds the remote t3 runner with npx and npm fallbacks", () => {
		...
		assert.include(
			script,
			"require_installed_t3_cli npx --yes --package 't3@latest'",
		);
		assert.include(
			script,
			"require_installed_t3_cli npm exec --yes --package 't3@latest'",
		);
	...
	it("uses the remote t3 runner for launch and pairing scripts", () => {
		...
		assert.include(
			buildRemoteLaunchScript(),
			'"$RUNNER_FILE" serve --host 127.0.0.1',
		);
		assert.include(
			buildRemoteLaunchScript(),
		...
		assert.include(
			buildRemoteLaunchScript(),
			"Remote T3 server did not become ready",
		);
	...
	it.effect("accepts launch JSON after remote shell startup noise", () => {
		...
		return Effect.gen(function* () {
			...
			assert.deepEqual(spawnedCommands[0]?.slice(-5, -1), [
				"sh",
				"-l",
				"-s",
				"--",
	...
	it.effect(
		"allows cold remote launches to exceed the default SSH command timeout",
		() => {
			const target = {
				alias: "devbox",
				hostname: "devbox.example.com",
			...
			return Effect.gen(function* () {
				const fiber = yield* Effect.forkChild(
					launchOrReuseRemoteServer(target),
				);
	...
	it.effect(
		"bounds each HTTP readiness probe so retries cannot hang on one request",
		() =>
			Effect.gen(function* () {
				const fiber = yield* Effect.forkChild(
					Effect.result(
...
`),
	...
	it.effect(
		"accepts pretty-printed pairing JSON after remote shell startup noise",
		() => {
			const target = {
				alias: "devbox",
				hostname: "devbox.example.com",
	...
	it.effect.each(["successful stop", "failed stop"] as const)(
		...
		(mode) => {
			...
			return Effect.gen(function* () {
				...
				const firstTunnelArgs = spawnedCommands.find((args) =>
					args.includes("-N"),
				);
				...
				const disconnected = yield* Effect.result(
				...
				assert.equal(
					spawnedCommands.filter((args) => args.includes("-N")).length,
					2,
				);
				assert.equal(tunnelKillCount, 1);
			}).pipe(
		...
		(stalledStep) =>
			Effect.gen(function* () {
				...
				yield* Effect.gen(function* () {
					...
					const firstReconnect = yield* Effect.forkChild(
						manager.ensureEnvironment(target),
					);
					const secondReconnect = yield* Effect.forkChild(
						manager.ensureEnvironment(target),
					);
