...
const decodeRemoteLaunchResult = Schema.decodeEffect(
	fromLenientJson(RemoteLaunchResult),
);
const decodeRemotePairingResult = Schema.decodeEffect(
	fromLenientJson(RemotePairingResult),
);
...
function normalizeSshErrorMessage(
	stderr: string,
...
export function buildRemoteT3RunnerScript(
	...
	const packageSpec = shellSingleQuote(
		input?.packageSpec?.trim() || "t3@latest",
	);
...
export function buildRemoteNodeEnvScript(
	input?: RemoteT3RunnerOptions,
): string {
	return stripTrailingNewlines(
		applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
			...
			T3_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(
...
export const launchOrReuseRemoteServer = Effect.fn(
	"ssh/tunnel.launchOrReuseRemoteServer",
)(function* (
	target: DesktopSshEnvironmentTarget,
...
export const issueRemotePairingToken = Effect.fn(
	"ssh/tunnel.issueRemotePairingToken",
)(function* (
	target: DesktopSshEnvironmentTarget,
	input?: SshAuthOptions,
	runner?: RemoteT3RunnerOptions,
...
const readRemoteServerLogTail = Effect.fn("ssh/tunnel.readRemoteServerLogTail")(
	function* (
		target: DesktopSshEnvironmentTarget,
		input?: SshAuthOptions,
	): Effect.fn.Return<
...
export const resolveLoopbackSshHttpBaseUrl = Effect.fn(
	"ssh/tunnel.resolveLoopbackSshHttpBaseUrl",
)(function* (
	...
	return yield* Effect.try({
		try: () => {
			if (
				typeof rawHttpBaseUrl !== "string" ||
				rawHttpBaseUrl.trim().length === 0
			) {
			...
			new SshHttpBridgeError({
				message:
					cause instanceof Error
						? cause.message
						: "Invalid SSH forwarded http base URL.",
				cause,
			}),
	});
});

const reserveLocalTunnelPort = Effect.fn("ssh/tunnel.reserveLocalTunnelPort")(
	function* () {
...
const startSshTunnel = Effect.fn("ssh/tunnel.startSshTunnel")(
	function* (input: {
		readonly key: string;
			...
			Effect.tapError((cause) =>
				Effect.gen(function* () {
					...
					yield* Effect.logWarning("ssh.tunnel.ready.failed", {
						...
						localPort: input.localPort,
						localPortListening:
							localPortAvailable === null ? null : !localPortAvailable,
						remotePort: input.remotePort,
						httpBaseUrl: input.httpBaseUrl,
...
const makeSshEnvironmentManager = Effect.fn(
	"ssh/tunnel.SshEnvironmentManager.make",
)(function* (
	...
	const withTargetLock = Effect.fn("ssh/tunnel.withTargetLock")(function* <
		A,
		E,
		R,
	>(key: string, effect: Effect.Effect<A, E, R>): Effect.fn.Return<A, E, R> {
	...
	const promptForPassword = Effect.fn("ssh/tunnel.promptForPassword")(
		function* (
			target: DesktopSshEnvironmentTarget,
			attempt: number,
		): Effect.fn.Return<
	...
	const handleSshAuthFailure = Effect.fn(
		"ssh/tunnel.runWithSshAuthAttempt.handleFailure",
	)(function* <T>(
	...
	> {
		...
		const nextAuthSecret = yield* promptForPassword(
			input.target,
			nextPromptCount,
		);
	...
	const runWithSshAuthAttempt = Effect.fn("ssh/tunnel.runWithSshAuthAttempt")(
		function* <T>(
			input: SshAuthAttemptInput<T>,
		): Effect.fn.Return<
			T,
		...
		> {
			...
			const authOptions =
				input.authSecret === null
					? {
							batchMode: promptService.isAvailable
								? ("yes" as const)
								: ("no" as const),
			...
			return yield* input
				...
				.pipe(
					Effect.catch((error) => handleSshAuthFailure({ ...input, error })),
				);
		},
	);
	...
	const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(
		function* (input: {
			readonly key: string;
			readonly resolvedTarget: DesktopSshEnvironmentTarget;
	...
	const ensureTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry")(
		function* (
			key: string,
	...
	const ensureEnvironment = Effect.fn("ssh/tunnel.ensureEnvironment")(
		function* (
			target: DesktopSshEnvironmentTarget,
	...
	const disconnectEnvironment = Effect.fn("ssh/tunnel.disconnectEnvironment")(
		function* (
			target: DesktopSshEnvironmentTarget,
		): Effect.fn.Return<
			void,
			...
			yield* withTargetLock(
				...
				Effect.gen(function* () {
					...
					yield* runWithSshAuth({
						...
						operation: (authOptions) =>
							stopRemoteServer(resolvedTarget, authOptions),
					});
