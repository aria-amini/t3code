...
describe("tailscale", () => {
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				const error = yield* parseTailscaleStatus("{not-json").pipe(
					Effect.flip,
				);
	...
	it.effect("turns spawn defects into typed spawn failures", () => {
		...
		const defect = Object.assign(new Error("spawn tailscale ENOTDIR"), {
			code: "ENOTDIR",
		});
		const layer = spawnerLayer(
		...
		return Effect.gen(function* () {
			const statusError = yield* readTailscaleStatus.pipe(
				Effect.flip,
				Effect.provide(layer),
			);
			...
			const serveError = yield* ensureTailscaleServe({
				localPort: 13773,
				servePort: 8443,
	...
	it.effect("classifies unrecognized stderr without quoting it", () => {
		const layer = mockSpawnerLayer(() => ({
			code: 3,
			stderr:
				"something novel went wrong for node fluffy-badger tskey-auth-secret-token-value",
		}));
	...
	it.effect("times out tailscale status through TestClock", () => {
		const layer = Layer.merge(
			...
			spawnerLayer(
				ChildProcessSpawner.make(() =>
					Effect.succeed(neverFinishingMockHandle()),
				),
			),
		);
	...
	it.effect(
		"configures tailscale serve through the process spawner service",
		() => {
			const layer = mockSpawnerLayer((command, args) => {
				assert.equal(command, "tailscale");
	...
	it.effect(
		...
		() => {
			...
			return Effect.gen(function* () {
				yield* disableTailscaleServe({ servePort: 8443 }).pipe(
					Effect.provide(layer),
				);
