...
const tailscaleCommandForPlatform = (
...
const stderrDiagnosticOf = (
	...
	return (
		STDERR_DIAGNOSTIC_PATTERNS.find(([pattern]) => pattern.test(stderr))?.[1] ??
		"unknown"
	);
};
...
const decodeTailscaleStatusJson = Schema.decodeEffect(
	Schema.fromJsonString(TailscaleStatusJson),
);
	...
	return yield* Effect.gen(function* () {
		const child = yield* spawner
			...
			.pipe(
				Effect.mapError(
					(cause) =>
						new TailscaleCommandSpawnError({ ...commandContext, cause }),
				),
