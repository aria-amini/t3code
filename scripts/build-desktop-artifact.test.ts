...
const stageWslRuntimeTreeFixture = Effect.fn("stageWslRuntimeTreeFixture")(
	function* (root: string, serverSource: string) {
		...
		yield* fs.makeDirectory(
			path.join(root, "node_modules/node-pty/prebuilds/linux-x64"),
			{
				recursive: true,
			},
		);
...
const makeWindowsPayloadFixture = Effect.fn("test.makeWindowsPayloadFixture")(
	function* (input: {
		...
		yield* fs.writeFileString(
			serverEntryPath,
			input.serverEntrySource ?? "console.log('server');\n",
		);
		...
		yield* fs.copyFile(
			generatedAsarPath,
			path.join(resourcesDir, WINDOWS_SERVER_ASAR_RESOURCE),
		);
		...
		yield* fs.writeFileString(
			path.join(packagedAppDir, appExecutableName),
			"electron",
		);
		yield* fs.writeFileString(
			path.join(packagedAppDir, "chrome_crashpad_handler.exe"),
			"crashpad",
		);
		...
		if (input.wslRuntime !== undefined) {
			...
			yield* fs.writeFileString(
				path.join(linuxPrebuildDir, "pty.node"),
				"linux-pty",
			);
			yield* fs.writeFileString(
			...
			if (input.wslRuntime === "forbidden") {
				...
				yield* fs.writeFileString(
					path.join(windowsPrebuildDir, "pty.node"),
					"windows-pty",
				);
			}
			...
			const tar = yield* spawner.spawn(
				ChildProcess.make(
					...
					[
						"-czf",
						wslRuntimeArchiveTarTarget(
							path.relative(wslSourceDir, archivePath),
						),
						"apps/server/dist",
						"node_modules",
					],
			...
			yield* fs
				...
				.pipe(
					Stream.runForEach((chunk) =>
						Effect.sync(() => archiveDigest.update(chunk)),
					),
				);
			yield* fs.writeFileString(
				hashPath,
...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
	...
	it.effect("applies platform-specific packaging to the build config", () =>
		Effect.gen(function* () {
			...
			assert.deepStrictEqual(linux.extraResources, [
				...
				...LINUX_CAPTURE_EXTRA_RESOURCES,
				{
					from: "apps/desktop/prod-resources/browser-secret",
					to: "browser-secret",
				},
			]);
				...
				Effect.gen(function* () {
					...
					assert.isFalse(
						yield* fs.exists(
							path.join(
								unpackedRoot,
								"node_modules/node-pty/third_party/conpty/1.0.0/win10-arm64",
							),
						),
					);
				}),
	...
	it.effect(
		"reports every missing Linux desktop build prerequisite with an install command",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					...
					assert.include(
						error.message,
						"Rust compiler and Cargo (cargo, rustc)",
					);
					...
					assert.isTrue(
						commands.some(
							(command) =>
								command.command === "rustc" &&
								command.args.includes("aarch64-unknown-linux-gnu"),
						),
					);
				}),
			),
	...
	it.effect(
		"reports missing macOS tools and Rust targets before building",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const spawner = Layer.succeed(
						...
						ChildProcessSpawner.make((command) => {
							...
							const fails =
								childProcess.command === "rustc" ||
								(childProcess.command === "xcrun" &&
									childProcess.args.includes("iconutil"));
							return Effect.succeed(mockProcess(fails ? 1 : 0));
						}),
					);
					...
					assert.deepStrictEqual(error.rustTargets, [
	...
	it.effect(
		"reports missing Windows toolchain capabilities before building",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const fs = yield* FileSystem.FileSystem;
	...
	it.effect(
		"does not require MSVC when reusing a prebuilt Windows resource monitor",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const fs = yield* FileSystem.FileSystem;
					...
					const spawner = Layer.succeed(
						...
						ChildProcessSpawner.make((command) => {
							...
							return Effect.succeed(
								mockProcess(childProcess.command === "powershell.exe" ? 1 : 0),
							);
						}),
					);
	...
	it.effect(
		"rejects a PATH-discovered Python executable that is not Python 3",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const fs = yield* FileSystem.FileSystem;
	...
	it.effect(
		"builds and stages native capture helpers for each Linux architecture",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const fs = yield* FileSystem.FileSystem;
					const path = yield* Path.Path;
					const repoRoot = yield* fs.makeTempDirectoryScoped({
						prefix: "t3-kde-stage-test-",
					});
					const protocols = path.join(
						repoRoot,
						"native/hyprland-snap-shot/protocols",
					);
					yield* fs.makeDirectory(protocols, { recursive: true });
					yield* fs.writeFileString(
						path.join(protocols, "capture.xml"),
						"BSD protocol notice",
					);
					for (const backend of ["kde", "hyprland"] as const) {
						for (const [arch, target] of [
								...
								ChildProcessSpawner.make((command) =>
									Effect.gen(function* () {
										...
										assert.deepEqual(command.args, [
											...
											path.join(
												repoRoot,
												`native/${backend}-snap-shot/Cargo.toml`,
											),
											"--target",
											target,
										]);
										yield* fs.makeDirectory(path.dirname(binary), {
											recursive: true,
										});
							...
							assert.equal(
								yield* fs.readFileString(installed),
								`helper-${arch}`,
							);
							...
							if (backend === "hyprland")
								assert.equal(
									yield* fs.readFileString(
										path.join(
											stageResourcesDir,
											"hyprland-capture/protocols/capture.xml",
										),
									),
									"BSD protocol notice",
								);
		...
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					...
					const secondAsarPath = path.join(
						path.dirname(fixture.generatedAsarPath),
						"second.asar",
					);
					yield* packWindowsServerAsar({
	...
	it.effect(
		"rejects an emitted WSL archive whose sidecar digest does not match",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const fixture = yield* makeWindowsPayloadFixture({
	...
	it.effect(
		"probes fff through the packaged Windows primary instead of helper executables",
		() => {
			const commands: Array<{
				readonly command: string;
				readonly args: ReadonlyArray<string>;
			...
			return Effect.scoped(
				Effect.gen(function* () {
					...
					if (primaryProbe === undefined)
						return assert.fail("Windows primary probe was not spawned");
	...
	it.effect(
		"builds the Linux browser secret helper for a concrete architecture",
		() => {
			const commands: Array<{
				readonly command: string;
				readonly args: ReadonlyArray<string>;
	...
	it.effect(
		"refuses a Linux build on a host that cannot build the browser secret helper",
		() => {
	...
	it.effect(
		"rejects a cross-architecture Windows payload without its primary executable",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const fs = yield* FileSystem.FileSystem;
	...
	it.effect(
		"rejects a packaged sidecar whose ASAR-unpacked native is missing",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const fixture = yield* makeWindowsPayloadFixture({
	...
	it.effect(
		"rejects a Windows payload that regresses above the file-count budget",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const fixture = yield* makeWindowsPayloadFixture({
	...
	it.effect(
		"rasterizes staged DMG backgrounds at standard and Retina sizes",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const fs = yield* FileSystem.FileSystem;
	...
	it.effect(
		"fails clearly when the selected DMG background source is missing",
		() =>
			Effect.scoped(
				Effect.gen(function* () {
					const fs = yield* FileSystem.FileSystem;
					...
					const error = yield* stageDesktopDmgBackground(
						stageResourcesDir,
						"latest",
						false,
					).pipe(Effect.flip);
	...
	it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
		...
		assert.include(
			entitlements,
			"<string>ABC1234567.com.t3tools.t3code</string>",
		);
		assert.include(
			entitlements,
	...
	it("rejects incomplete macOS passkey signing configuration", () => {
		...
		assert.equal(
			...
			"T3CODE_CLERK_PUBLISHABLE_KEY is invalid.",
		);
	...
	it("packages the WSL server and production dependencies as one compressed runtime", () => {
		...
		assert.isTrue(
			bundlesWslRuntime({ arch: "x64", prebuildPath: "/tmp/pty.node" }),
		);
		assert.isTrue(
			bundlesWslRuntime({ arch: "arm64", prebuildPath: "/tmp/pty.node" }),
		);
		assert.isFalse(bundlesWslRuntime({ arch: "x64", prebuildPath: undefined }));
		assert.isFalse(
			bundlesWslRuntime({ arch: "universal", prebuildPath: "/tmp/pty.node" }),
		);
	...
	it.effect(
		"spawns tar with an archive target relative to the staged source tree",
		() => {
			const commands: Array<{
				readonly command: string;
				readonly args: ReadonlyArray<string>;
			...
			return Effect.scoped(
				Effect.gen(function* () {
					...
					yield* stageWslRuntimeTreeFixture(
						sourceDir,
					...
					const tarCommand = commands.find(
						(command) => command.command === "tar",
					);
					if (tarCommand === undefined)
						return assert.fail("tar was not spawned");
...
it.effect.skipIf(!symlinksSupported)(
	"rebases packaged links into the isolated tree",
	() =>
		Effect.gen(function* () {
			const fs = yield* FileSystem.FileSystem;
			const path = yield* Path.Path;
			...
			const relativePackageLink = path.join(
				source,
				"node_modules/example-relative",
			);
			const absolutePackageLink = path.join(
				source,
				"node_modules/example-absolute",
			);
			...
			assert.equal(
				yield* fs.readLink(
					path.join(destination, "node_modules/example-relative"),
				),
				copiedPackage,
			);
			assert.equal(
				yield* fs.readLink(
					path.join(destination, "node_modules/example-absolute"),
				),
				copiedPackage,
			);
			assert.equal(
