...
export function resourceMonitorExecutableName(
	platform: typeof BuildPlatform.Type,
): string {
...
export class LinuxDesktopBuildPrerequisitesMissingError extends Schema.TaggedError<LinuxDesktopBuildPrerequisitesMissingError>()(
	...
	override get message(): string {
		const missingRequirements = LINUX_DESKTOP_BUILD_PREREQUISITES.filter(
			(requirement) => this.missing.includes(requirement.id),
		);
		...
		const packages = [
			...new Set(
				missingRequirements.flatMap((requirement) => requirement.packages),
			),
		];
		return [
			...
			...(this.missing.includes("rust-target")
				? [
						"Add the requested Rust target with:",
...
const WINDOWS_DESKTOP_BUILD_PREREQUISITES = [
	...
	{ id: "python", description: "Python 3 for node-gyp" },
	{
		id: "msvc",
		description:
			"Visual Studio Build Tools with C++, Windows SDK, and Spectre libraries",
	},
...
const resolvePythonForNodeGyp = Effect.fn("resolvePythonForNodeGyp")(
	function* () {
		...
		if (
			configured &&
			(yield* fs.exists(configured)) &&
			(yield* isPython3(configured))
		) {
			return configured;
		}
		...
		if (
			!executable ||
			!(yield* fs.exists(executable)) ||
			!(yield* isPython3(executable))
		) {
			return undefined;
		}
...
export function resolveWindowsServerAsarIgnoreGlobs(
	arch: typeof BuildArch.Type,
) {
	const unusedArch = arch === "arm64" ? "x64" : "arm64";
...
export const WINDOWS_SERVER_RESOURCE_SOURCE_DIR =
	"apps/desktop/prod-resources/windows-server";
...
export const WSL_RUNTIME_ARCHIVE_CONTENT_ROOTS = [
	"apps/server/dist",
	"node_modules",
] as const;
...
export const resolveWslPrebuildArch = (
	arch: typeof BuildArch.Type,
): "x64" | "arm64" | undefined =>
...
export const bundlesWslRuntime = (input: {
	...
	resolveWslPrebuildArch(input.arch) !== undefined;
 ...
 */
const stageKeyringNativeBinaries = Effect.fn("stageKeyringNativeBinaries")(
	function* (
		stageAppDir: string,
		platform: typeof BuildPlatform.Type,
		arch: typeof BuildArch.Type,
			...
			const sourcePath = yield* Effect.try({
				try: () =>
					packageRequire.resolve(
						`${artifact.packageName}/${artifact.binaryFileName}`,
					),
				catch: (cause) =>
...
const desktopBuildProbeSucceeds = Effect.fn("desktopBuildProbeSucceeds")(
	function* (command: ChildProcess.Command, label: string) {
...
const rustTargetIsInstalled = Effect.fn("rustTargetIsInstalled")(function* (
	target: string,
) {
	const fs = yield* FileSystem.FileSystem;
	const result = yield* spawnAndCollectOutput(
		ChildProcess.make("rustc", [
			"--print",
			"target-libdir",
			"--target",
			target,
...
export const preflightLinuxDesktopBuild = Effect.fn(
	"preflightLinuxDesktopBuild",
)(function* (arch: typeof BuildArch.Type = "x64") {
	...
	const checks = yield* Effect.all(
		{
			cargo: needsRust
				? desktopBuildProbeSucceeds(
						ChildProcess.make("cargo", ["--version"]),
						"cargo",
					)
				: Effect.succeed(true),
			"rust-target": needsRust
				? rustTargetIsInstalled(rustTarget)
				: Effect.succeed(true),
			cc: desktopBuildProbeSucceeds(
				ChildProcess.make("cc", ["--version"]),
				"cc",
			),
			make: desktopBuildProbeSucceeds(
				ChildProcess.make("make", ["--version"]),
				"make",
			),
			...
			imagemagick: Effect.all([
				desktopBuildProbeSucceeds(
					ChildProcess.make("magick", ["-version"]),
					"magick",
				),
				desktopBuildProbeSucceeds(
					ChildProcess.make("convert", ["-version"]),
					"convert",
				),
...
export const preflightMacDesktopBuild = Effect.fn("preflightMacDesktopBuild")(
	function* (arch: typeof BuildArch.Type) {
		...
		const checks = yield* Effect.all(
			{
				...
				clang: desktopBuildProbeSucceeds(
					ChildProcess.make("clang", ["--version"]),
					"clang",
				),
				make: desktopBuildProbeSucceeds(
					ChildProcess.make("make", ["--version"]),
				...
				sips: desktopBuildProbeSucceeds(
					ChildProcess.make("sips", ["--help"]),
					"sips",
				),
				...
				lipo:
					arch === "universal"
						? desktopBuildProbeSucceeds(
								ChildProcess.make("lipo", ["-version"]),
								"lipo",
							)
						: Effect.succeed(true),
			},
...
export const preflightWindowsDesktopBuild = Effect.fn(
	"preflightWindowsDesktopBuild",
)(function* (input: {
	readonly arch: typeof BuildArch.Type;
	readonly bundlesWslRuntime: boolean;
}) {
	...
	const checks = yield* Effect.all(
		{
			...
			tar: input.bundlesWslRuntime
				? desktopBuildProbeSucceeds(
						ChildProcess.make("tar.exe", ["--version"]),
						"tar",
					)
				: Effect.succeed(true),
		},
...
const findStorePackageDirectory = Effect.fn("findStorePackageDirectory")(
	function* (repoRoot: string, packageName: string) {
		const fs = yield* FileSystem.FileSystem;
		const path = yield* Path.Path;
...
const hasNativeLoaderMarkers = Effect.fn("hasNativeLoaderMarkers")(function* (
	packageDir: string,
) {
	const fs = yield* FileSystem.FileSystem;
	const path = yield* Path.Path;
	...
	const manifest = yield* Effect.try(() =>
		decodeNativeMarkerManifest(source),
	).pipe(Effect.orElseSucceed(() => null));
...
export const copyDirectoryPreservingSymlinks = Effect.fn(
...
const verifyPackagedBundleIsSelfContained = Effect.fn(
...
export const stageLinuxCaptureHelper = Effect.fn("stageLinuxCaptureHelper")(
	function* (input: {
		...
		const reuseHelpers = yield* Config.boolean(
			"T3CODE_DESKTOP_REUSE_LINUX_CAPTURE_HELPERS",
		).pipe(Config.withDefault(false));
		...
		if (!reuseHelpers) {
			const spawnCommand = yield* resolveSpawnCommand("cargo", [
				...
				path.join(
					input.repoRoot,
					`native/${input.backend}-snap-shot/Cargo.toml`,
				),
				"--target",
				rustTarget!,
			]);
...
export const stageResourceMonitor = Effect.fn("stageResourceMonitor")(
	function* (input: {
		readonly repoRoot: string;
		readonly stageResourcesDir: string;
		readonly platform: typeof BuildPlatform.Type;
		...
		const rustTargets = resolveResourceMonitorRustTargets(
			input.platform,
			input.arch,
		);
		const reuseResourceMonitor = yield* Config.boolean(
		...
		for (const rustTarget of rustTargets) {
			...
			if (reuseResourceMonitor) {
				yield* Effect.log(
					`[desktop-artifact] Reusing cached resource monitor (${rustTarget}).`,
				);
			}
		...
		yield* fs
			.remove(destinationDirectory, { recursive: true, force: true })
			.pipe(Effect.ignore);
...
export const stageBrowserSecret = Effect.fn("stageBrowserSecret")(
	function* (input: {
		readonly repoRoot: string;
		readonly stageResourcesDir: string;
		readonly platform: typeof BuildPlatform.Type;
		...
		yield* runCommand(
			ChildProcess.make(
				...
				[
					path.join(
						input.repoRoot,
						"apps/desktop/scripts/build-browser-secret.mjs",
					),
					"--arch",
...
export const stageDesktopDmgBackground = Effect.fn("stageDesktopDmgBackground")(
	function* (
		stageResourcesDir: string,
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
	...
	const buildConfig: Record<string, unknown> = {
		...
		extraResources: [
			...
			...(platform === "win" && wslRuntimeBundled
...
export const wslRuntimeArchiveTarTarget = (
	relativeArchivePath: string,
): string => relativeArchivePath.replaceAll("\\", "/");
...
export const buildWslRuntimeArchiveArgs = (
	...
	"-czf",
	archivePath,
	...WSL_RUNTIME_ARCHIVE_EXCLUDED_PREFIXES.map(
		(prefix) => `--exclude=${prefix}*`,
	),
	...WSL_RUNTIME_ARCHIVE_CONTENT_ROOTS,
];

export const parseWslRuntimeArchiveMembers = (
	listing: string,
): ReadonlyArray<string> =>
	listing
		.split(/\r?\n/)
...
export const stageWslRuntimeArchive = Effect.fn("stageWslRuntimeArchive")(
	function* (input: {
		...
		const tarTarget = wslRuntimeArchiveTarTarget(
			path.relative(input.sourceDir, input.archivePath),
		);
		yield* runCommand(
...
export const packWindowsServerAsar = Effect.fn("packWindowsServerAsar")(
	function* (input: {
		readonly sourceDir: string;
		...
		const archiveStream = yield* Effect.tryPromise({
			try: () =>
				...
				new WindowsServerSidecarPackError({ asarPath: input.asarPath, cause }),
		});
		yield* Effect.tryPromise({
			try: () =>
		...
		if (!(yield* fs.exists(unpackedDirPath))) {
			return yield* new WindowsServerSidecarPackError({
				...
				cause: new Error(
					`expected native binaries at ${unpackedDirPath}, but none were unpacked`,
				),
			});
		}
	},
...
export const stageWindowsServerSidecar = Effect.fn("stageWindowsServerSidecar")(
	function* (input: {
		...
		yield* fs.copy(
			input.serverDistDir,
			path.join(serverStageDir, "apps/server/dist"),
		);
		...
		const sidecarPackageJsonString =
			yield* encodeJsonString(sidecarPackageJson);
		yield* fs.writeFileString(
		...
		const sidecarWorkspaceConfigString = yield* encodeStageWorkspaceConfig(
			sidecarWorkspaceConfig,
		);
		yield* fs.writeFileString(
		...
		if (Object.keys(sidecarPatchedDependencies).length > 0) {
			yield* fs.copy(
				path.join(input.repoRoot, "patches"),
				path.join(serverStageDir, "patches"),
			);
		}

		yield* Effect.log(
			"[desktop-artifact] Installing server sidecar runtime externals...",
		);
...
const countPayloadFiles = Effect.fn("desktopArtifact.countPayloadFiles")(
...
export const verifyWindowsPrimaryFffNativeLoad = Effect.fn(
	...
	if (executableStat?.type !== "File") {
		return yield* new WindowsPrimaryNativeProbeError({
			...
			exitCode: -1,
			output:
				"The unpacked application does not contain its expected primary executable.",
		});
	}
...
export const validateWindowsPackagedPayload = Effect.fn(
	...
	const unpackedFiles = yield* Effect.try({
		try: () => {
			...
			return [
				...collectUnpackedAsarFiles(getRawHeader(asarPath).header),
			].sort();
		},
		catch: (cause) =>
	...
	if (unpackedFiles.length === 0) {
		return yield* new WindowsPackagedPayloadValidationError({
			...
			packagedAppDir,
			cause: new Error(
				"server.asar does not declare any unpacked native files",
			),
		});
	}
	...
	for (const unpackedFile of unpackedFiles) {
		...
		if (!(yield* isFile(unpackedPath))) {
			missingFiles.push(
				`${WINDOWS_SERVER_ASAR_RESOURCE}.unpacked/${unpackedFile}`,
			);
		}
	}
	...
	const wslArchiveHashPath = path.join(
		resourcesDir,
	...
	if (
		...
		(!hasWslArchive || !hasWslArchiveHash)
	) {
		return yield* new WindowsPackagedPayloadValidationError({
	...
	if (hasWslArchive && hasWslArchiveHash) {
		...
		if (actualHash !== expectedHash) {
			return yield* invalidWslRuntime(
				new Error(
					`WSL runtime SHA-256 mismatch: expected ${expectedHash}, got ${actualHash}`,
				),
			);
		}
		...
		if (listing.exitCode !== 0) {
			return yield* invalidWslRuntime(
				new Error(
					`tar could not list WSL runtime archive: ${listing.stderr.trim()}`,
				),
			);
		}
		const members = parseWslRuntimeArchiveMembers(listing.stdout);
		const forbiddenMember = members.find((member) =>
			WSL_RUNTIME_ARCHIVE_EXCLUDED_PREFIXES.some((prefix) =>
		...
		if (forbiddenMember !== undefined) {
			return yield* invalidWslRuntime(
				new Error(
					`WSL runtime archive contains forbidden member ${forbiddenMember}`,
				),
			);
		}
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
	...
	{
		const chunkNames = (yield* fs.readDirectory(distDirs.serverDist)).filter(
		...
		for (const chunkName of chunkNames) {
			const source = yield* fs.readFileString(
				path.join(distDirs.serverDist, chunkName),
			);
	...
	yield* Effect.log(
		`[desktop-artifact] Applied ${webAssetBrand} web client branding.`,
	);
	...
	const stageProdResourcesDir = path.join(
		stageAppDir,
	...
	yield* stageKeyringNativeBinaries(
		stageAppDir,
		options.platform,
		options.arch,
	);
	...
	if (options.platform === "win" && windowsServerAsarPath) {
		yield* stageWindowsServerSidecar({
			...
			wslRuntimeArchivePath: path.join(
				stageAppDir,
	...
	buildEnv.npm_config_user_agent = resolvePackageManagerUserAgent(
	...
	if (options.verbose) {
		...
		buildEnv.DEBUG = [buildEnv.DEBUG, ...debugNamespaces]
			.filter(Boolean)
			.join(",");
	}
