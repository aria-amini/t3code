...
it.layer(NodeServices.layer)("dev-runner", (it) => {
	...
	describe("createDevRunnerEnv", () => {
		it.effect(
			"leaves the shared home implicit and disables browser auto-open",
			() =>
				Effect.gen(function* () {
		...
		it.effect(
			"requires the browser flag even when the environment enables auto-open",
			() =>
				Effect.gen(function* () {
					const env = yield* createDevRunnerEnv({
						mode: "dev",
		...
		it.effect(
			"pins desktop dev to a stable backend port and websocket url",
			() =>
				Effect.gen(function* () {
					const path = yield* Path.Path;
					const env = yield* createDevRunnerEnv({
	...
	describe("findFirstAvailableOffset", () => {
		...
		it.effect(
			"skips browser-blocked web ports before probing availability",
			() =>
				Effect.gen(function* () {
		...
		it.effect(
			"does not reject a server-only offset because its unused web port is blocked",
			() =>
				Effect.gen(function* () {
					const offset = yield* findFirstAvailableOffset({
						startOffset: 833,
		...
		it.effect(
			"allows offsets where the non-required server port exceeds max",
			() =>
				Effect.gen(function* () {
					const offset = yield* findFirstAvailableOffset({
						startOffset: 59_802,
	...
	describe("isBrowserAllowedPort", () => {
		...
		it.each([5733, 5900, 6567, 6670, 8733])(
			"allows browser-safe web port %s",
			(port) => {
	...
	describe("checkPortAvailabilityOnHosts", () => {
		it.effect(
			"checks overlapping hosts sequentially to avoid self-interference",
			() =>
				Effect.gen(function* () {
					let inFlightCount = 0;
	...
	describe("devPortProbeHosts", () => {
		it.effect("probes loopback only when no bind host is configured", () =>
			Effect.sync(() => {
				assert.deepStrictEqual(devPortProbeHosts(undefined), [
					"127.0.0.1",
					"::1",
				]);
		...
		it.effect("adds a non-loopback bind host to the probe list", () =>
			Effect.sync(() => {
				assert.deepStrictEqual(devPortProbeHosts("0.0.0.0"), [
					"127.0.0.1",
					"::1",
					"0.0.0.0",
				]);
		...
		it.effect(
			"does not probe loopback twice when it is the configured host",
			() =>
				Effect.sync(() => {
					assert.deepStrictEqual(devPortProbeHosts("127.0.0.1"), [
						"127.0.0.1",
						"::1",
					]);
				}),
		...
		it.effect(
			"passes the port role so only the server port sees the bind host",
			() =>
				Effect.gen(function* () {
					const probed: Array<{ port: number; role: string | undefined }> = [];
	...
	describe("runDevRunnerWithInput", () => {
		it.effect("preserves invalid configuration as the exact cause", () =>
			Effect.gen(function* () {
						...
						Layer.merge(
							...
							ConfigProvider.layer(
								ConfigProvider.fromEnv({
									env: { T3CODE_PORT_OFFSET: "not-an-integer" },
								}),
							),
						),
					),
				...
				assert.ok(
					!error.message.includes(String((error.cause as Error).message)),
				);
			}),
		);

		it.effect(
			"preserves process spawn context and the exact platform cause",
			() => {
				...
				return Effect.gen(function* () {
					const error = yield* runDevRunnerWithInput(devServerInput).pipe(
						Effect.provide(
							Layer.mergeAll(emptyConfigLayer, netServiceLayer, spawnerLayer),
						),
		...
		it.effect(
			"declines to share for dev:desktop and still starts the stack",
			() => {
				let spawnCount = 0;
				const spawnerLayer = Layer.succeed(
					ChildProcessSpawner.ChildProcessSpawner,
		...
		it.effect(
			"rejects a specific non-loopback --host for browser dev modes",
			() => {
				const spawnerLayer = Layer.succeed(
					ChildProcessSpawner.ChildProcessSpawner,
		...
		it.effect(
			"still spawns the stack for a wildcard --host in dev mode",
			() => {
				let spawnCount = 0;
				const spawnerLayer = Layer.succeed(
					ChildProcessSpawner.ChildProcessSpawner,
		...
		describe("--share bundled dev default", () => {
			const shareSpawnedEnv = (input: {
				readonly ambientBundledDev: string | undefined;
			}) =>
				Effect.gen(function* () {
		...
		describe("t3 home precedence", () => {
			const makeWorktree = Effect.acquireRelease(
				Effect.sync(() => {
					const root = NodeFS.mkdtempSync(
						NodePath.join(NodeOS.tmpdir(), "t3-devrunner-"),
					);
			...
			it.effect(
				"prefers an explicit --home-dir over the worktree default",
				() =>
					Effect.gen(function* () {
						const path = yield* Path.Path;
			...
			it.effect(
				"treats a blank --home-dir as unset rather than as a selection",
				() =>
					Effect.gen(function* () {
						const path = yield* Path.Path;
						const root = yield* makeWorktree;
			...
			it.effect(
				"leaves the home implicit with no worktree and no ambient value",
				() =>
					Effect.gen(function* () {
						const home = yield* spawnedHome({
							t3Home: undefined,
