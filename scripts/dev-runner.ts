...
> {
	...
	if (seed) {
		...
		return Effect.succeed({
			offset,
			source: `hashed T3CODE_DEV_INSTANCE=${seed}`,
		});
	}
 ...
 */
export function devPortProbeHosts(
	configuredHost: string | undefined,
): ReadonlyArray<string> {
	const host = configuredHost?.trim();
	if (
		!host ||
		DEV_PORT_PROBE_HOSTS.includes(host as (typeof DEV_PORT_PROBE_HOSTS)[number])
	) {
...
const makeDefaultCheckPortAvailability =
	(
		configuredHost: string | undefined,
	): PortAvailabilityCheck<NetService.NetService> =>
	(port, role) =>
		Effect.gen(function* () {
...
const defaultCheckPortAvailability =
	makeDefaultCheckPortAvailability(undefined);
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
	return Effect.gen(function* () {
		...
		const worktreePath = yield* resolveGitWorktreePath(
		...
		const worktreeHome = yield* resolveWorktreeT3Home(
			yield* HostProcessWorkingDirectory,
		);
		...
		if (input.share) {
			if (input.mode === "dev:server") {
				yield* Effect.logInfo(
					"[dev-runner] --share has no effect for dev:server (no web server).",
				);
				...
				if (shared) {
					...
					yield* Effect.logInfo(
						`[dev-runner] shared on tailnet: ${shared.url}`,
					);
				}
			}
		}
...
const devRunnerCli = Command.make("dev-runner", {
	...
	browser: Flag.boolean("browser").pipe(
		Flag.withDescription(
			"Open a browser automatically (disabled by default for web dev).",
		),
	...
	dryRun: Flag.boolean("dry-run").pipe(
		Flag.withDescription(
			"Resolve mode/ports/env and print, but do not spawn Vite+.",
		),
		Flag.withDefault(false),
	),
