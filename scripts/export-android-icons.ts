...
const renderDevelopmentBackground = Effect.fn(
	"androidIcons.renderDevelopmentBackground",
	...
	const annotations = yield* readLayerSource(
		repositoryRoot,
		"dev",
		"annotations.svg",
	);
	...
	const overlay = yield* rasterize(
		...
		canvasSvg(
			size,
			`${defs}<g transform="${wordmarkTransform(size)}">${body}</g>`,
		),
		size,
	);
...
const renderNightlyBackground = Effect.fn(
	"androidIcons.renderNightlyBackground",
)(function* (repositoryRoot: string, size: number) {
	...
	const clouds = [
		{
			file: "cloud-lower-left.svg",
			scale: 25,
			translation: [-309.6375, 268.66077693836917],
		},
		{
	...
	const overlays = yield* Effect.forEach(clouds, (cloud) =>
		Effect.gen(function* () {
			...
			const left = Math.round(
				(COMPOSER_CANVAS_PT / 2 + cloud.translation[0]) * k - width / 2,
			);
			const top = Math.round(
				(COMPOSER_CANVAS_PT / 2 + cloud.translation[1]) * k - height / 2,
			);
			const source = yield* readLayerSource(
				repositoryRoot,
				"nightly",
				cloud.file,
			);
			...
			const clipped = yield* Effect.tryPromise({
				try: () =>
					...
					new AndroidIconRenderError({ layer: cloud.file, cause }),
			});
			return { input: clipped, left: x0, top: y0 };
		}),
	);
	const sky = yield* readLayerSource(
		repositoryRoot,
		"nightly",
		"background.svg",
	);
