...
describe("brand-assets", () => {
	...
	it("can target hosted web dist directly", () => {
		expect(
			resolveWebIconOverrides("production", "apps/web/dist"),
		).toContainEqual({
			sourceRelativePath: BRAND_ASSET_PATHS.productionWebAppleTouchIconPng,
	...
	it("maps package versions to web asset brands", () => {
		...
		expect(
			resolveWebAssetBrandForPackageVersion("0.0.29-nightly.20260723.882"),
		).toBe("nightly");
	});
	...
	it("keeps development, nightly, and production icon families separate", () => {
		...
		expect(BRAND_ASSET_PATHS.developmentDesktopIconPng).toMatch(
			/^assets\/dev\/blueprint-/,
		);
		expect(BRAND_ASSET_PATHS.nightlyMacIconPng).toMatch(
