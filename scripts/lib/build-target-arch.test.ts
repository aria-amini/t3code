...
describe("build-target-arch", () => {
	it.effect(
		"uses the resolved host arch when selecting the default Windows build arch",
		() =>
			Effect.gen(function* () {
				...
				const arch = yield* getDefaultBuildArch("win", {
	...
	it.effect(
		"does not apply Windows host env heuristics for non-Windows targets",
		() =>
			Effect.gen(function* () {
				const arch = yield* getDefaultBuildArch("linux", {
					archChoices: ["x64", "arm64"],
				}).pipe(
