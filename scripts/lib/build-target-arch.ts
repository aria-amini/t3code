...
const WindowsProcessorArchitectureConfig = Config.all({
	processorArchitecture: Config.string("PROCESSOR_ARCHITECTURE").pipe(
		Config.option,
	),
	processorArchitectureW6432: Config.string("PROCESSOR_ARCHITEW6432").pipe(
		Config.option,
	),
});

function normalizeWindowsArch(
...
const resolveHostProcessArch = Effect.fn("resolveHostProcessArch")(
	function* () {
		...
		if (processArch === "x64") {
			...
			return (
				normalizeWindowsArch(
					optionToUndefined(env.processorArchitectureW6432),
				) ??
