...
describe("shouldBundleCliDependency", () => {
	...
	it("leaves bun-only entry points external", () => {
		assert.strictEqual(
			shouldBundleCliDependency("@effect/platform-bun"),
			false,
		);
		assert.strictEqual(
			shouldBundleCliDependency("@effect/sql-sqlite-bun"),
			false,
		);
	});
	...
	it("treats prefix-matched siblings as external", () => {
		assert.strictEqual(
			shouldBundleCliDependency("node-gyp-build-optional-packages"),
			false,
		);
	});
});
		...
		for (const entry of yield* fileSystem.readDirectory(storeDir)) {
			...
			for (const owner of yield* fileSystem.readDirectory(modulesDir)) {
				...
				for (const name of names) {
					...
					installed.set(
						name,
						decodeManifest(yield* fileSystem.readFileString(manifestPath)),
					);
				}
			}
		}
...
`;
	...
	it("reports no regions when the marker format is absent", () => {
		const result = findInlinedExternalPackages(
			"var x = 1; // node_modules/detect-libc/lib.js",
		);
