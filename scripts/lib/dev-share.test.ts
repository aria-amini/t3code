...
const TAILNET_STATUS = JSON.stringify({
	Self: { DNSName: "host.example.ts.net." },
});
const NO_HANDLER_STDERR =
...
const spawnerLayer = (input: {
	...
	Layer.succeed(
		...
		ChildProcessSpawner.make((command) => {
			const args =
				"args" in command ? (command.args as ReadonlyArray<string>) : [];
...
describe("unshareDevServer", () => {
	...
	it.effect("treats a missing handler as cleared", () =>
		Effect.gen(function* () {
			const result = yield* unshareDevServer(5788).pipe(
				Effect.provide(
					spawnerLayer({ off: { exitCode: 1, stderr: NO_HANDLER_STDERR } }),
				),
			);
			assert.isTrue(result.cleared);
		}),
	...
	it.effect("reports a genuine removal failure as not cleared", () =>
		Effect.gen(function* () {
			const result = yield* unshareDevServer(5788).pipe(
				Effect.provide(
					spawnerLayer({ off: { exitCode: 1, stderr: "permission denied" } }),
				),
			);
...
describe("shareDevServer", () => {
	...
	it.effect("reports that the prior mapping was cleared when serve fails", () =>
		Effect.gen(function* () {
			const error: DevShareError = yield* shareDevServer({
				webPort: 5788,
			}).pipe(
				Effect.provide(
					spawnerLayer({
	...
	it.effect(
		"refuses to serve when the existing mapping could not be cleared",
		() =>
			Effect.gen(function* () {
				const error: DevShareError = yield* shareDevServer({
					webPort: 5788,
