...
export function parseUpdateManifest(
	...
	for (const [index, rawLine] of lines.entries()) {
		...
		if (fileUrlMatch?.[1]) {
			const finalized = parseFileRecord(
				currentFile,
				sourcePath,
				lineNumber,
				platformLabel,
	...
	if (!version) {
		throw new Error(
			`Invalid ${platformLabel} update manifest at ${sourcePath}: missing version.`,
		);
	}
	if (!releaseDate) {
		throw new Error(
	...
	if (files.length === 0) {
		throw new Error(
			`Invalid ${platformLabel} update manifest at ${sourcePath}: missing files.`,
		);
	}
...
export function mergeUpdateManifests(
	...
	for (const file of [...primary.files, ...secondary.files]) {
		...
		if (
			existing &&
			(existing.sha512 !== file.sha512 || existing.size !== file.size)
		) {
			throw new Error(
