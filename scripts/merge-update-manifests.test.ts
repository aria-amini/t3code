...
const runCli = Command.runWith(mergeUpdateManifestsCommand, {
	version: "0.0.0",
});
...
`;
	...
	it.effect(
		"writes the merged manifest back to the primary path by default",
		() =>
			Effect.gen(function* () {
	...
	it.effect("rejects invalid platform values during cli parsing", () =>
		Effect.gen(function* () {
			...
			if (!platformError || platformError._tag !== "InvalidValue") {
				assert.fail(
					`Expected InvalidValue, got ${String(platformError?._tag)}`,
				);
			}
