...
export const mergeUpdateManifestFiles = Effect.fn("mergeUpdateManifestFiles")(
	function* (
		platform: UpdateManifestPlatform,
		...
		const merged = mergePlatformUpdateManifests(
			platform,
			primaryManifest,
			secondaryManifest,
		);
...
export const mergeUpdateManifestsCommand = Command.make(
	"merge-update-manifests",
	{
		...
		primaryPath: Argument.string("primary-path").pipe(
			Argument.withDescription(
				"Primary update manifest path. Defaults to the output path.",
			),
		),
	...
	Command.withDescription(
		"Merge two Electron updater manifests into a multi-arch manifest.",
	),
);
