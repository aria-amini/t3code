...
const runSwiftLint = runCommand(
	"swiftlint",
	["lint", "--strict"],
	"/repo/apps/mobile",
).pipe(Effect.provideService(HostProcess.HostProcessPlatform, "linux"));

it.layer(NodeServices.layer)("mobile native source discovery", (it) => {
	it.effect(
		"preserves the failed discovery operation, path, and exact cause",
		() =>
			Effect.gen(function* () {
				...
				const error = yield* collectSources(missingDirectory, root).pipe(
					Effect.flip,
				);
				...
				assert.equal(
					error.message,
					"Native source discovery operation 'read-directory' failed.",
				);
			}),
	);
});
...
it.effect("preserves process wait context and the exact cause", () => {
	...
	return Effect.gen(function* () {
		const error = yield* runSwiftLint.pipe(
			Effect.provide(
				provideSpawner(() => Effect.succeed(processHandle(Effect.fail(cause)))),
			),
			Effect.flip,
		);
	...
	Effect.gen(function* () {
		const error = yield* runSwiftLint.pipe(
			Effect.provide(
				provideSpawner(() =>
					Effect.succeed(
						processHandle(Effect.succeed(ChildProcessSpawner.ExitCode(2))),
					),
				),
			),
			Effect.flip,
