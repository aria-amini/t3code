		...
		for (const entry of entries) {
			...
			if (stat.type === "Directory") {
				...
				if (
					...
					isGeneratedNativeProjectDirectory
				) {
					continue;
				}
...
const runNativeStaticChecks = Effect.fn("runNativeStaticChecks")(function* () {
	...
	const swiftSources = sources.filter(
		(source) => path.extname(source) === ".swift",
	);
	...
	if (kotlinSources.length > 0) {
		const relativeKotlinSources = kotlinSources.map((source) =>
...
export const mobileNativeStaticCheckCommand = Command.make(
	"mobile-native-static-check",
	{},
	() => runNativeStaticChecks(),
).pipe(
	Command.withDescription(
		"Run mobile native static analysis when native tools are available.",
	),
);
