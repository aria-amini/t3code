...
export const SHOWCASE_THREADS = [
	...
	},
	{
		...
		request:
			"Make switching between desktop, phone, and tablet feel like one continuous session.",
		response:
	...
	},
	{
		...
		minutesAgo: 52,
		request:
			"Make delayed work easier to follow without adding noise to the scheduler trace.",
		response:
	...
	// a store screenshot has to show that history exists, not just imply it.
	{
		...
		settled: true,
		request:
			"Give the desktop-to-phone handoff a haptic that lands with the animation.",
		response:
	...
	},
	{
		...
		settled: true,
		request:
			"Make out-of-memory kills legible without adding a single allocation to the hot path.",
		response:
...
async function seedT3CodeWorkspace(workspaceRoot: string): Promise<void> {
	await NodeFSP.mkdir(
		NodePath.join(workspaceRoot, "apps/mobile/src/features/home"),
		{
			recursive: true,
		},
	);
	...
	await NodeFSP.writeFile(
		NodePath.join(
			workspaceRoot,
			"apps/mobile/src/features/home/RemoteHandoffCard.tsx",
		),
		REMOTE_HANDOFF_CARD,
	);
}
...
function hasSeedableSchema(dbPath: string): boolean {
	...
	try {
		...
		return SEEDED_THREAD_COLUMNS.every((column) =>
...
async function waitForSeedableSchema(
	dbPath: string,
	timeoutMs = 60_000,
): Promise<void> {
...
function seedDatabase(
	...
	try {
		...
		for (const [index, project] of projects.entries()) {
			...
			if (!workspaceRoot)
				throw new Error(`Missing workspace root for ${project.id}.`);
		...
		for (const thread of threads) {
			...
			if (!workspaceRoot)
				throw new Error(`Missing workspace root for ${thread.projectId}.`);
...
export async function seedShowcaseEnvironment(input: {
	...
	if (projects.length === 0)
		throw new Error("At least one showcase project must be selected.");
	const threads = SHOWCASE_THREADS.filter((thread) =>
		selectedProjectIds.has(thread.projectId),
	);
	const workspaceBase = NodePath.join(input.baseDir, "workspace");
	const workspaceRoots = new Map(
		projects.map(
			(project) =>
				[project.id, NodePath.join(workspaceBase, project.directory)] as const,
		),
	);
	const primaryProject =
		projects.find((project) => project.id === SHOWCASE_PROJECT_ID) ??
		projects[0];
	if (!primaryProject)
		throw new Error("The primary showcase workspace is not configured.");
	...
	const terminalDirectory = NodePath.join(
		input.baseDir,
		"userdata",
		"logs",
		"terminals",
