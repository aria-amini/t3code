...
it("rejects unsupported themes instead of capturing the default palette", () => {
	assert.throws(
		() => parseShowcaseCliArgs(["--theme", "sunset"]),
		/Unsupported theme 'sunset'/u,
	);
});
...
it("selects an explicit CI Android ABI without changing the local default", () => {
	...
	assert.throws(
		...
		/Unsupported T3_SHOWCASE_ANDROID_ABI/u,
	);
});
...
it("rejects unknown devices instead of silently capturing another target", () => {
	...
	assert.throws(
		() => planShowcaseCaptures(config, options),
		/Unknown device 'missing'/u,
	);
});
...
it("validates exact Apple and Google Play upload assets", () => {
	...
	const google = normalizeStorePng(
		rgbaPng(googleSpec.width, googleSpec.height),
	);
...
it("rejects wrong dimensions and alpha-bearing PNGs", () => {
	...
	assert.throws(
		() => validateStoreAsset(appleSpec, wrongSize),
		/requires 1284×2778/u,
	);
	assert.throws(
		() => validateStoreAsset(appleSpec, rgbaPng(1284, 2778)),
		/without alpha/u,
	);
});
...
it("enforces store screenshot count limits", () => {
	...
	assert.throws(
		() => validateStoreAssetCount(googleSpec, 1, true),
		/requires at least 2/u,
	);
	assert.throws(
		() => validateStoreAssetCount(googleSpec, 9, false),
		/allows at most 8/u,
	);
});
...
it("defaults every device to the app's own palette", () => {
	...
	assert.equal(
		showcaseConfig.devices.every(
			(device) => device.theme === DEFAULT_SHOWCASE_THEME,
		),
		true,
	);
});
...
it("seeds a playful multi-environment project spectrum", () => {
	...
	assert.equal(
		new Set(SHOWCASE_THREADS.map((thread) => thread.projectId)).size,
		3,
	);
	const snoozedThreads = SHOWCASE_THREADS.filter(
		(thread) => "snoozeMinutes" in thread,
	);
	...
	assert.equal(
		new Set(snoozedThreads.map((thread) => thread.snoozeMinutes)).size,
		2,
	);
	for (const thread of snoozedThreads) {
		assert.equal(
			...
			true,
			`${thread.title} is not completed`,
		);
		assert.equal(
			"state" in thread,
			false,
			`${thread.title} is blocked or working`,
		);
		assert.equal("settled" in thread, false, `${thread.title} is settled`);
		assert.equal(
			thread.snoozeMinutes > 60,
			true,
			`${thread.title} wakes too soon`,
		);
	}
	const primaryThread = SHOWCASE_THREADS.find(
		(thread) => thread.id === "remote-command-center",
	);
	assert.equal(
		primaryThread !== undefined && !("snoozeMinutes" in primaryThread),
		true,
	);
	...
	for (const project of SHOWCASE_PROJECTS) {
		const projectThreads = SHOWCASE_THREADS.filter(
...
it("encodes Android pairing URLs without shell-sensitive JSON quotes", () => {
	const urls = [
		"http://10.0.2.2:65164/#token=ONE",
		"http://10.0.2.2:65198/#token=TWO",
	];
	const encoded = encodeAndroidPairingUrls(urls);
	assert.equal(encoded.startsWith("json-uri:"), true);
	assert.deepStrictEqual(
		JSON.parse(decodeURIComponent(encoded.slice("json-uri:".length))),
		urls,
	);
