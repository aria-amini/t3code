...
export function resolveAndroidSdkRoot(
	...
	return path.join(
		home,
		platform === "darwin" ? "Library/Android/sdk" : "Android/Sdk",
	);
}
...
function lanIpv4Address(): string {
	...
	if (!address) {
		throw new Error(
			"No LAN IPv4 address is available for the iOS Simulator to reach Metro.",
		);
	}
	return address;
}
...
export function validateStoreAsset(
	...
	if (spec.store === "google-play") {
		...
		if (metadata.width * 16 !== metadata.height * 9) {
			throw new Error(
				`${label} must use Google Play's recommended portrait 9:16 aspect ratio.`,
			);
		}
	}
	return metadata;
...
async function validateCaptureSet(
	...
	if (missingFiles.length > 0) {
		throw new Error(
			`${capture.device.id} is missing ${missingFiles.join(", ")} in ${directory}.`,
		);
	}
	validateStoreAssetCount(
		capture.device.storeAsset,
		files.length,
		requireMinimum,
	);
	...
	for (let index = 0; index < args.length; index += 1) {
		...
		if (argument === "--platform") {
			...
			if (value !== "ios" && value !== "android" && value !== "all") {
				throw new Error(
					`Unsupported platform '${value}'. Use ios, android, or all.`,
				);
			}
			if (value === "all") {
				platforms.add("ios");
			...
			if (!SHOWCASE_SCENES.includes(value as ShowcaseScene)) {
				throw new Error(
					`Unsupported scene '${value}'. Use ${SHOWCASE_SCENES.join(", ")}.`,
				);
			}
			...
			if (value !== "light" && value !== "dark" && value !== "both") {
				throw new Error(
					`Unsupported appearance '${value}'. Use light, dark, or both.`,
				);
			}
			if (value === "both") {
...
export function planShowcaseCaptures(
	...
	options: Pick<
		CliOptions,
		"platforms" | "deviceIds" | "scenes" | "appearances" | "themes"
	>,
): ReadonlyArray<ShowcaseCapture> {
	const captures = config.devices
		.filter(
			(device) =>
				options.platforms.size === 0 || options.platforms.has(device.platform),
		)
		.filter(
			(device) =>
				options.deviceIds.size === 0 || options.deviceIds.has(device.id),
		)
		.flatMap((device) => {
			const appearances =
				options.appearances.size === 0
					? [device.appearance]
					: options.appearances;
			const themes =
				options.themes.size === 0 ? [device.theme] : options.themes;
	...
	for (const id of options.deviceIds) {
		if (!knownDeviceIds.has(id)) {
			throw new Error(
				`Unknown device '${id}'. Run with --list to see configured devices.`,
			);
		}
	}
	if (captures.length === 0) {
		throw new Error(
			"No captures match the selected platform, device, and scene filters.",
		);
	}
	return captures;
}
...
async function waitForPort(
	...
	throw new Error(
		`${label} did not begin listening on port ${port} within ${timeoutMs}ms.`,
	);
}
...
async function waitForFileContent(
	...
	throw new Error(
		`${label} was not written to ${filePath} within ${timeoutMs}ms.`,
	);
}
	...
	return await new Promise<number>((resolve, reject) => {
		...
		server.listen(0, "127.0.0.1", () => {
			...
			if (!address || typeof address === "string") {
				...
				reject(
					new Error(
						"Could not reserve a local port for the showcase environment.",
					),
				);
				return;
			}
...
async function createShowcaseLabelProbe(
	baseDir: string,
	label: string,
): Promise<string> {
...
function buildShowcasePairingUrl(
	host: string,
	port: number,
	credential: string,
): string {
...
export function showcaseSceneUrl(
	scene: ShowcaseScene,
...
export function encodeAndroidPairingUrls(
...
async function buildAndroid(abis: ReadonlyArray<string>): Promise<string> {
	await runCommand(
		"pnpm",
		["exec", "expo", "prebuild", "--clean", "--platform", "android"],
		{
			cwd: MOBILE_ROOT,
			env: MOBILE_BUILD_ENV,
		},
...
async function ensureIosSimulator(device: ShowcaseIosDevice): Promise<{
	...
	const udid = (
		await commandOutput("xcrun", [
			...
			"create",
			device.simulator,
			device.simulatorDeviceType,
		])
	).trim();
	if (!udid)
		throw new Error(`Could not create iOS simulator '${device.simulator}'.`);
	return {
		simulator: {
...
async function iosSimulatorDataPath(udid: string): Promise<string> {
	const parsed = JSON.parse(
		await commandOutput("xcrun", ["simctl", "list", "devices", "-j"]),
	) as {
		readonly devices: Readonly<
			Record<
				string,
				ReadonlyArray<SimctlDevice & { readonly dataPath: string }>
			>
		>;
	};
	...
	if (!dataPath)
		throw new Error(
			`Could not resolve the data path of iOS simulator ${udid}.`,
		);
	return dataPath;
}
...
async function suppressIosSystemFollowUps(udid: string): Promise<void> {
	...
	await NodeFSP.writeFile(
		NodePath.join(
			preferences,
			"com.apple.generativeexperiences.corefollowup.plist",
		),
...
async function normalizeIosSimulator(
...
async function ensureIosFullScreenAppsMode(udid: string): Promise<void> {
	...
	if (applied.trim() !== "0") {
		throw new Error(
			`Simulator ${udid} did not switch to Full Screen Apps mode.`,
		);
	}
}
...
async function waitForIosShowcaseScene(
	...
	throw new Error(
		`iOS showcase scene '${scene}' did not render within ${timeoutMs}ms.`,
	);
}
...
async function captureIos(
	...
	const { simulator, createdByRunner } = await ensureIosSimulator(
		capture.device,
	);
	...
	for (const [sceneIndex, scene] of capture.scenes.entries()) {
		...
		await delay(
			scene === "review"
				? Math.max(config.settleDelayMs, 8_000)
		...
		if (capture.device.orientation === "landscape") {
			...
			const { width, height } = readPngDimensions(
				await NodeFSP.readFile(destination),
			);
			if (height > width) {
...
async function adbOutput(
	...
	return await commandOutput(androidSdkTool("platform-tools/adb"), [
		"-s",
		serial,
		...args,
	]);
...
async function runAdb(
	...
	await runCommand(androidSdkTool("platform-tools/adb"), [
		"-s",
		serial,
		...args,
	]);
...
async function runningAndroidAvds(): Promise<ReadonlyMap<string, string>> {
	...
	for (const serial of devices) {
		const avdName = (await adbOutput(serial, ["emu", "avd", "name"]))
			.split("\n")[0]
			?.trim();
...
async function waitForAndroidSerial(
	...
	throw new Error(
		`Android AVD '${avd}' did not finish booting within ${timeoutMs}ms.`,
	);
}
	...
	if (device.viewport) {
		...
		if (device.viewport.density) {
			await runAdb(serial, [
				...
				"wm",
				"density",
				String(device.viewport.density),
			]);
		}
	}
}
...
async function waitForAndroidShowcaseScene(
	...
	throw new Error(
		`Android showcase scene '${scene}' did not render within ${timeoutMs}ms.`,
	);
}

async function writeAndroidShowcaseScene(
	serial: string,
	scene: ShowcaseScene,
): Promise<void> {
	await runAdb(serial, [
...
async function captureAndroid(
	...
	const serial =
		existingSerial ??
		(await waitForAndroidSerial(capture.device.avd).catch(
			async (error: unknown) => {
	...
	await runAdb(serial, [
		"reverse",
		`tcp:${config.metroPort}`,
		`tcp:${config.metroPort}`,
	]);
	const metroUrl = encodeURIComponent(
		`http://127.0.0.1:${config.metroPort}?disableOnboarding=1`,
	);
	const firstScene = capture.scenes[0] ?? "threads";
	await runAdb(serial, [
		"shell",
	...
	for (const [sceneIndex, scene] of capture.scenes.entries()) {
		...
		await delay(
			Math.max(config.settleDelayMs, scene === "review" ? 8_000 : 5_000),
		);
		const destination = NodePath.join(
			showcaseCaptureDirectory(outputDirectory, capture),
			`${scene}.png`,
...
async function main(): Promise<void> {
	...
	const outputDirectory = NodePath.resolve(
		REPO_ROOT,
		showcaseConfig.outputDirectory,
	);
	...
	const hasAndroid = captures.some(
		(capture) => capture.device.platform === "android",
	);
	...
	try {
		for (const environment of SHOWCASE_ENVIRONMENTS) {
			...
			if (!project)
				throw new Error(
					`Showcase environment '${environment.id}' has no project.`,
				);
			...
			const labelProbeDirectory = await createShowcaseLabelProbe(
				baseDir,
				environment.label,
			);
		...
		if (!options.skipMetro) {
			...
			await Promise.all([
				hasIos
					? warmMetroBundle("ios", metroHost, showcaseConfig)
					: Promise.resolve(),
				hasAndroid
					? warmMetroBundle("android", "127.0.0.1", showcaseConfig)
					: Promise.resolve(),
			]);
		}
		...
		const androidAbis = captures.flatMap((capture) =>
			capture.device.platform === "android" && capture.device.abi
				? [capture.device.abi]
				: [],
		);
		...
		for (const capture of captures) {
			const pairingHost =
				capture.device.platform === "ios" ? "127.0.0.1" : "10.0.2.2";
			const pairingUrls = await Promise.all(
				showcaseEnvironments.map(async (environment) => {
					...
					return buildShowcasePairingUrl(
						pairingHost,
						environment.port,
						credential,
					);
		...
		if (options.keepRunning) {
			...
			for (const cleanup of androidCleanups) {
				await cleanupAndroidViewport(cleanup.device, cleanup.serial).catch(
					() => undefined,
				);
			...
			for (const cleanup of iosCleanups) {
				if (cleanup.startedByRunner || cleanup.createdByRunner) {
					await runCommand("xcrun", ["simctl", "shutdown", cleanup.udid]).catch(
						() => undefined,
					);
				}
				if (cleanup.createdByRunner) {
					await runCommand("xcrun", ["simctl", "delete", cleanup.udid]).catch(
						() => undefined,
					);
				}
			}
