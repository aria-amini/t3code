...
const resolveMockUpdateServerConfig = Effect.gen(function* () {
	...
	const config = yield* Config.all({
		port: Config.port("T3CODE_DESKTOP_MOCK_UPDATE_SERVER_PORT").pipe(
...
const isWithinRoot = (rootRealPath: string, filePath: string) =>
	Effect.gen(function* () {
		...
		return (
			...
			!(yield* isOutsideRoot(rootRealPath, resolvedFilePath))
		);
	});

const resolveRequestedFilePath = (
	rootRealPath: string,
	...
	Effect.gen(function* () {
		...
		return (yield* isOutsideRoot(rootRealPath, filePath))
			? undefined
			: filePath;
	});
...
export const makeMockUpdateRouteLayer = (rootRealPath: string) => {
	return HttpRouter.add(
		...
		Effect.gen(function* () {
			...
			const filePath = yield* resolveRequestedFilePath(
				rootRealPath,
				request.url,
			);
			if (!filePath) {
				yield* Effect.logWarning(
					`Attempted to access file outside of root: ${request.url ?? "/"}`,
				);
			...
			if (!(yield* isServableFile(rootRealPath, filePath))) {
				yield* Effect.logWarning(
					`Attempted to access invalid file: ${filePath}`,
				);
				return HttpServerResponse.text("Not Found", { status: 404 });
			}
			...
			Effect.catchCause((cause) =>
				Effect.gen(function* () {
					yield* Effect.logError(
						`Unhandled mock update request failure: ${cause}`,
					);
