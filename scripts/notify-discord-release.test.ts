...
it.effect(
	"preserves webhook request context and the full client cause chain",
	() => {
		...
		return Effect.gen(function* () {
			const error = yield* postDiscordWebhook(
				...
				payload,
				latestAnnouncement,
			).pipe(Effect.provide(httpClientLayer), Effect.flip);
			...
			assert.equal(
				(error.cause as HttpClientError.HttpClientError).cause,
				requestCause,
			);
...
it.effect(
	"preserves a non-success response error with structured status context",
	() => {
		const payload = buildDiscordReleaseAnnouncement(latestAnnouncement);
		const httpClientLayer = Layer.succeed(
			HttpClient.HttpClient,
