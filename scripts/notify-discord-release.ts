...
export type DiscordReleaseAnnouncementError =
	typeof DiscordReleaseAnnouncementError.Type;
export const isDiscordReleaseAnnouncementError = Schema.is(
	DiscordReleaseAnnouncementError,
);
...
export const postDiscordWebhook = Effect.fn("postDiscordWebhook")(function* (
	...
	const errorContext = {
		...
		webhookPathnameSegmentCount: webhookUrl.pathname.split("/").filter(Boolean)
			.length,
		...summarizePayload(payload),
	} as const;
...
export const notifyDiscordReleaseCommand = Command.make(
	"notify-discord-release",
	{
		target: Argument.choice("target", DISCORD_RELEASE_TARGETS).pipe(
			Argument.withDescription(
				"Discord announcement target: prerelease or latest.",
			),
		),
		roleId: Flag.string("role-id").pipe(
			Flag.withSchema(DiscordRoleIdSchema),
			Flag.withDescription(
				"Discord role ID to mention in the release announcement.",
			),
		),
	...
	Command.withDescription("Post a T3 Code release announcement to Discord."),
);
