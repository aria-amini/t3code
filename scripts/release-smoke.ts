...
function writeWindowsManifestFixtures(
	...
	const arm64Path = NodePath.resolve(
		assetDirectory,
...
try {
	...
	const { arm64Path: winArm64Path, x64Path: winX64Path } =
		writeWindowsManifestFixtures(tempRoot, "latest");
	const mergedWindowsManifestPath = NodePath.resolve(
		tempRoot,
	...
	const mergedNightlyWindowsManifestPath = NodePath.resolve(
		tempRoot,
		"release-assets/nightly.yml",
	);
	const { arm64Path: previewWinArm64Path, x64Path: previewWinX64Path } =
		writeWindowsManifestFixtures(tempRoot, "preview");
	const mergedPreviewWindowsManifestPath = NodePath.resolve(
		tempRoot,
		"release-assets/preview.yml",
	);
	...
	const mergedWindowsManifest = NodeFS.readFileSync(
		mergedWindowsManifestPath,
		"utf8",
	);
	assertContains(
	...
	assertMissing(
		winX64Path,
		"Windows release smoke unexpectedly kept the x64 updater manifest.",
	);
	assertMissing(
		nightlyWinArm64Path,
