...
it.effect(
	"bumps the patch version before deriving nightly prerelease versions",
	() =>
		Effect.gen(function* () {
			assert.equal(yield* resolveNightlyTargetVersion("0.0.17"), "0.0.18");
			assert.equal(
				yield* resolveNightlyTargetVersion("9.9.9-smoke.0"),
				"9.9.10",
			);
			assert.equal(
				yield* resolveNightlyTargetVersion("1.2.3-beta.4+build.9"),
				"1.2.4",
			);
		}),
);
...
it.effect("reports the invalid desktop package version", () =>
	Effect.gen(function* () {
		const error = yield* resolveNightlyTargetVersion("nightly").pipe(
			Effect.flip,
		);
...
it.effect("preserves the GITHUB_OUTPUT configuration cause", () => {
	...
	const configCause = new ConfigProvider.SourceError({
...
it.layer(NodeServices.layer)("readDesktopBaseVersion", (it) => {
	it.effect(
		"preserves desktop package read context and its platform cause",
		() =>
			Effect.gen(function* () {
				...
				assert.notInclude(
					error.message,
					String((error.cause as Error).message),
				);
			}),
	);

	it.effect(
		"preserves desktop package decode context and its schema cause",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
				...
				const packageJsonPath = path.join(rootDir, "apps/desktop/package.json");
				yield* fs.makeDirectory(path.dirname(packageJsonPath), {
					recursive: true,
				});
