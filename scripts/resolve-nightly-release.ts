...
export const resolveNightlyBaseVersion = (version: string) =>
	version.replace(/[-+].*$/, "");
...
export const readDesktopBaseVersion = Effect.fn("readDesktopBaseVersion")(
...
export const writeNightlyReleaseOutput = Effect.fn("writeNightlyReleaseOutput")(
	function* (metadata: NightlyReleaseMetadata, writeGithubOutput: boolean) {
		const fs = yield* FileSystem.FileSystem;
		...
		if (writeGithubOutput) {
			const githubOutputPath = yield* Config.nonEmptyString(
				"GITHUB_OUTPUT",
			).pipe(
				Effect.mapError(
					(cause) =>
	...
	"resolve-nightly-release",
	{
		...
		root: Flag.string("root").pipe(
			Flag.withDescription(
				"Workspace root used to resolve apps/desktop/package.json.",
			),
			Flag.optional,
		),
	},
	({ date, runNumber, sha, githubOutput, root }) =>
		readDesktopBaseVersion(Option.getOrUndefined(root)).pipe(
			Effect.map((baseVersion) =>
				resolveNightlyReleaseMetadata(baseVersion, date, runNumber, sha),
			),
			Effect.flatMap((metadata) =>
				writeNightlyReleaseOutput(metadata, githubOutput),
			),
		),
