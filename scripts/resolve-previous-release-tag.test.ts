...
it.effect(
	"accepts legacy nightly tags when selecting the previous nightly",
	() =>
		Effect.gen(function* () {
			const previous = yield* resolvePreviousReleaseTag(
				"nightly",
				"v1.2.0-nightly.20260620.2",
				["nightly-v1.2.0-nightly.20260620.1", "v1.1.0-nightly.20260619.9"],
			);
...
it.effect("reports the invalid tag with its release channel", () =>
	Effect.gen(function* () {
		const error = yield* resolvePreviousReleaseTag(
			"nightly",
			"v1.2.0",
			[],
		).pipe(Effect.flip);
...
it.effect(
	"preserves git tag spawn context and the exact platform cause",
	() => {
...
it.effect("preserves the GITHUB_OUTPUT append path and exact cause", () => {
	...
	return Effect.gen(function* () {
		const appendError = yield* writePreviousReleaseTagOutput(
			"v1.2.3",
			true,
		).pipe(
			Effect.provideService(
