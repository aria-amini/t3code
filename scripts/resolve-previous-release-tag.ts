...
const parseNightlyTag = (tag: string): NightlyVersion | undefined => {
	...
	const match =
		/^(?:nightly-)?v(\d+)\.(\d+)\.(\d+)-nightly\.(\d{8})\.(\d+)$/.exec(tag);
	if (!match) return undefined;
	...
	Effect.gen(function* () {
		if (channel === "stable") {
			...
			const candidates = tags
				...
				.toSorted((left, right) =>
					compareStableVersions(right.parsed, left.parsed),
				);
...
export const writePreviousReleaseTagOutput = Effect.fn(
	"writePreviousReleaseTagOutput",
)(function* (previousTag: string | undefined, writeGithubOutput: boolean) {
...
const command = Command.make(
	"resolve-previous-release-tag",
	{
		channel: Flag.choice("channel", ReleaseChannel.literals).pipe(
			Flag.withDescription(
				"Release channel whose previous tag should be resolved.",
			),
		),
	...
	({ channel, currentTag, githubOutput }) =>
		listGitTags().pipe(
			Effect.flatMap((tags) =>
				resolvePreviousReleaseTag(channel, currentTag, tags),
			),
			Effect.flatMap((previousTag) =>
				writePreviousReleaseTagOutput(previousTag, githubOutput),
			),
		),
).pipe(
	Command.withDescription(
		"Resolve the previous release tag for a stable or nightly series.",
	),
);
