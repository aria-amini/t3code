...
it.layer(NodeServices.layer)("sync-reference-repos", (it) => {
	...
	it.effect("uses the latest branch without reading package versions", () =>
		Effect.gen(function* () {
			...
			assert.equal(
				yield* resolveReferenceRepoRef(effectSmol, rootDir, true),
				"main",
			);
		}),
	);
	...
	it.effect(
		"preserves version source read context and the filesystem cause",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
				...
				const error = yield* resolveReferenceRepoRef(
					effectSmol,
					rootDir,
					false,
				).pipe(Effect.flip);
	...
	it.effect(
		"reports the unresolved package path without inventing a cause",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
				...
				assert.equal(error.repoId, alchemyEffect.id);
				assert.equal(error.sourcePath, sourcePath);
				assert.deepStrictEqual(error.packageVersionPath, [
					"dependencies",
					"alchemy",
				]);
	...
	it.effect("resolves the alchemy-effect tag from the relay package", () =>
		Effect.gen(function* () {
			...
			yield* fs.makeDirectory(path.join(rootDir, "infra", "relay"), {
				recursive: true,
			});
			yield* fs.writeFileString(
	...
	it.effect(
		"plans an add for a missing subtree and a pull for an existing subtree",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
				...
				const addPlan = yield* planReferenceRepoSync(
					effectSmol,
					rootDir,
					false,
				);
				...
				yield* fs.makeDirectory(path.join(rootDir, effectSmol.prefix), {
					recursive: true,
				});
				assert.equal(
					(yield* planReferenceRepoSync(effectSmol, rootDir, false)).action,
					"pull",
				);
			}),
	);
	...
	it.effect(
		"runs the planned git subtree command through the process service",
		() => {
	...
	it.effect(
		"reports non-zero git exits without retaining process output",
		() => {
			const commands: Array<{
				readonly command: string;
				readonly args: ReadonlyArray<string>;
				...
				const error = yield* syncReferenceRepos({
					...
					Effect.provide(
						mockSpawnerLayer(
							...
							mockHandle({
								exitCode: 23,
								stderr: "subtree failed secret-token-value\n",
							}),
						),
					),
					Effect.flip,
