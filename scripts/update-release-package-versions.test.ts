...
const runCli = Command.runWith(updateReleasePackageVersionsCommand, {
	version: "0.0.0",
});
...
const writePackageJsonFixtures = Effect.fn("writePackageJsonFixtures")(
	function* (rootDir: string, version: string) {
...
const readReleaseVersions = Effect.fn("readReleaseVersions")(function* (
	rootDir: string,
) {
	const fs = yield* FileSystem.FileSystem;
	const path = yield* Path.Path;
...
it.layer(ScriptTestLayer)("update-release-package-versions", (it) => {
	it.effect(
		"updates all release package versions under the provided root",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				...
				const result = yield* updateReleasePackageVersions("1.2.3", {
					rootDir: baseDir,
				});
	...
	it.effect("preserves manifest read context and the filesystem cause", () =>
		Effect.gen(function* () {
			...
			assert.equal(
				error.message,
				`Failed to read release package manifest '${filePath}'.`,
			);
		}),
	);
	...
	it.effect("preserves manifest decode context and the schema cause", () =>
		Effect.gen(function* () {
			...
			assert.equal(
				error.message,
				`Failed to decode release package manifest '${filePath}'.`,
			);
		}),
	);
	...
	it.effect("preserves manifest write context and the filesystem cause", () =>
		Effect.gen(function* () {
			...
			const error = yield* updateReleasePackageVersions("1.2.3", {
				...
				Effect.flip,
				Effect.ensuring(fs.chmod(filePath, 0o600).pipe(Effect.orDie)),
			);
			...
			assert.equal(
				error.message,
				`Failed to write release package manifest '${filePath}'.`,
			);
		}),
	);

	it.effect(
		"accepts flags before the version positional and appends changed output",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const path = yield* Path.Path;
		...
		captureLogs(
			...
			Effect.tap(({ logs }) => {
				assert.deepStrictEqual(logs, [
					"All package.json versions already match release version.",
				]);
				return Effect.void;
			}),
		),
	...
	it.effect(
		"preserves GITHUB_OUTPUT write context and the filesystem cause",
		() =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const baseDir = yield* fs.makeTempDirectoryScoped({
	...
	it.effect("rejects a missing version positional during cli parsing", () =>
		Effect.gen(function* () {
			...
			if (!versionError || versionError._tag !== "MissingArgument") {
				assert.fail(
					`Expected MissingArgument, got ${String(versionError?._tag)}`,
				);
			}
