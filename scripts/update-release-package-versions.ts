...
export const updateReleasePackageVersions = Effect.fn(
...
const writeGithubOutput = Effect.fn("writeGithubOutput")(function* (
	...
	yield* fs
		.writeFileString(githubOutputPath, `changed=${changed}\n`, { flag: "a" })
		.pipe(
			Effect.mapError(
				(cause) =>
...
export const updateReleasePackageVersionsCommand = Command.make(
	"update-release-package-versions",
	{
		version: Argument.string("version").pipe(
			Argument.withDescription(
				"Release version to write into each releasable package.json.",
			),
		),
		root: Flag.string("root").pipe(
			Flag.withDescription(
				"Workspace root used to resolve the release package manifests.",
			),
			Flag.optional,
		),
	...
	({ version, root, githubOutput }) =>
		updateReleasePackageVersions(version, {
			...
			Effect.tap(({ changed }) =>
				githubOutput ? writeGithubOutput(changed) : Effect.void,
			),
		),
).pipe(
	Command.withDescription(
		"Update release package versions across the workspace.",
	),
);
